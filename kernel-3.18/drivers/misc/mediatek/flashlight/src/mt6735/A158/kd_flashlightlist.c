...
struct pinctrl_state *flash_back_h = NULL;
struct pinctrl_state *flash_backpwm_l = NULL;
struct pinctrl_state *flash_backpwm_h = NULL;
struct pinctrl_state *flash_sub_l = NULL;
struct pinctrl_state *flash_sub_h = NULL;
int flash_gpio_init(struct platform_device *dev)
{
	...
	if (IS_ERR(flashlightctrl)) {
		dev_err(&pltfm_dev->dev, "luminjie Cannot find flashlight pinctrl!");
		ret = PTR_ERR(flashlightctrl);
	}
	flash_back_l = pinctrl_lookup_state(flashlightctrl, "back_low");
	if (IS_ERR(flash_back_l)) {
		ret = PTR_ERR(flash_back_l);
		pr_debug("%s : pinctrl err, flash_back_l\n", __func__);
	}
	
	flash_back_h = pinctrl_lookup_state(flashlightctrl, "back_high");
	if (IS_ERR(flash_back_h)) {
		ret = PTR_ERR(flash_back_h);
		pr_debug("%s : pinctrl err, flash_back_h\n", __func__);
	}
	flash_backpwm_l = pinctrl_lookup_state(flashlightctrl, "backpwm_low");
	if (IS_ERR(flash_backpwm_l)) {
		ret = PTR_ERR(flash_backpwm_l);
		pr_debug("%s : pinctrl err, flash_backpwm_l\n", __func__);
	}
	flash_backpwm_h = pinctrl_lookup_state(flashlightctrl, "backpwm_high");
	if (IS_ERR(flash_backpwm_h)) {
		ret = PTR_ERR(flash_backpwm_h);
		pr_debug("%s : pinctrl err, flash_backpwm_h\n", __func__);
	}
	flash_sub_l = pinctrl_lookup_state(flashlightctrl, "sub_low");
	if (IS_ERR(flash_sub_l)) {
		ret = PTR_ERR(flash_sub_l);
		pr_debug("%s : pinctrl err, flash_sub_l\n", __func__);
	}
	flash_sub_h = pinctrl_lookup_state(flashlightctrl, "sub_high");
	if (IS_ERR(flash_sub_h)) {
		ret = PTR_ERR(flash_sub_h);
		pr_debug("%s : pinctrl err, flash_sub_h\n", __func__);
	}
	
	return 0;
}
int flash_gpio_back_high(void){
    if(flashlightctrl == NULL || flash_back_h == NULL)
	{
		printk("luminjie fail \n");
	    return 0;
	}
	return pinctrl_select_state(flashlightctrl, flash_back_h);
}
int flash_gpio_back_low(void){
    if(flashlightctrl == NULL || flash_back_l == NULL)
	 {
		printk("luminjie fail \n");
	    return 0;
	}
	return pinctrl_select_state(flashlightctrl, flash_back_l);
}
int flash_gpio_backpwm_high(void){
    if(flashlightctrl == NULL || flash_backpwm_h == NULL)
	 {
		printk("luminjie fail \n");
	    return 0;
	}
	return pinctrl_select_state(flashlightctrl, flash_backpwm_h);
}
int flash_gpio_backpwm_low(void){
    if(flashlightctrl == NULL || flash_backpwm_l == NULL)
	 {
		printk("luminjie fail \n");
	    return 0;
	}
	return pinctrl_select_state(flashlightctrl, flash_backpwm_l);
}
int flash_gpio_sub_high(void){
    if(flashlightctrl == NULL || flash_sub_h == NULL)
	 {
		printk("luminjie fail \n");
	    return 0;
	}
	return pinctrl_select_state(flashlightctrl, flash_sub_h);
}
int flash_gpio_sub_low(void){
    if(flashlightctrl == NULL || flash_sub_l == NULL)
	{
		printk("luminjie fail \n");
	    return 0;
	}
	return pinctrl_select_state(flashlightctrl, flash_sub_l);
}
/*
...
switch (PwrType) {
	case FLASHLIGHT_BACK:
			if (Val == 0)
				pinctrl_select_state(flashlightctrl, flash_back_l);
			else
				pinctrl_select_state(flashlightctrl, flash_back_h);
		break;
	case FLASHLIGHT_backpwm:
			if (Val == 0)
				pinctrl_select_state(flashlightctrl, flash_backpwm_l);
			else
				pinctrl_select_state(flashlightctrl, flash_backpwm_h);
		break;
	case FLASHLIGHT_SUB:
			if (Val == 0)
				pinctrl_select_state(flashlightctrl, flash_sub_l);
			else
				pinctrl_select_state(flashlightctrl, flash_sub_h);
		break;
	default:
