...
#include "s5k4h8mipi_Sensor_hlt.h"
...
kal_uint16 hlt_R_gain=0,hlt_B_gain=0,hlt_gd_R_gain=0,hlt_gd_B_gain=0,hlt_in_af,hlt_ma_af;
kal_uint8  hlt_vendor_id=0;
extern int read_s5k4h8_eeprom_hlt_mtk_fmt(void);
#endif
//luminjie@wind-mobi.com close the sensor otp 20161216 end
static DEFINE_SPINLOCK(imgsensor_drv_lock);
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H8_SENSOR_ID_HLT,
...
static int read_otp_s5k4h8(struct S5K4H8_otp_struct *otp_ptr)
{
    ...
    if((otp_flag & 0xc0) == 0x40) 
	{
	  ...
	  hlt_vendor_id = otp_ptr->MID;//vendor id group 1
	...
	else if((otp_flag & 0x30) == 0x10) 
	{
	  ...
	  hlt_vendor_id = otp_ptr->MID;//vendor id group 2
	...
	if(addr != 0)
	{
	 ...
	 hlt_R_gain = (otp_ptr->R_gain_h<<8)|(otp_ptr->R_gain_l) ;
	 hlt_B_gain = (otp_ptr->B_gain_h<<8)|(otp_ptr->B_gain_l) ;
	 hlt_gd_R_gain = (otp_ptr->gd_R_gain_h<<8)|(otp_ptr->gd_R_gain_l) ;
	 hlt_gd_B_gain = (otp_ptr->gd_B_gain_h<<8)|(otp_ptr->gd_B_gain_l) ;
	 LOG_INF("luminjie s5k5e8 read R_G_B gain= %x,%x,%x,%x,%x,%x,%x,%x \n",otp_ptr->R_gain_h,otp_ptr->R_gain_l,otp_ptr->B_gain_h,otp_ptr->B_gain_l,otp_ptr->G_gain_h,otp_ptr->G_gain_l,hlt_R_gain,hlt_B_gain);
	 LOG_INF("luminjie s5k5e8 read R_G_B gain= %x,%x,%x,%x,%x,%x,%x,%x \n",otp_ptr->gd_R_gain_h,otp_ptr->gd_R_gain_l,otp_ptr->gd_B_gain_h,otp_ptr->gd_B_gain_l,otp_ptr->gd_G_gain_h,otp_ptr->gd_G_gain_l,hlt_gd_R_gain,hlt_gd_B_gain);
	}
	else{
	 otp_ptr->R_gain_h = 0;
	 otp_ptr->R_gain_l = 0;
	...
	if(af_addr !=0){
		hlt_in_af = ((read_cmos_sensor_8(af_addr)&0xff)<<2) | ((read_cmos_sensor_8(af_addr+1)&0xff)>>6);
		hlt_ma_af = ((read_cmos_sensor_8(af_addr+2)&0xff)<<2) | ((read_cmos_sensor_8(af_addr+3)&0xff)>>6);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12)+2;
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {               
                ...
                read_otp_s5k4h8(otp_ptr1);
                //luminjie@wind-mobi.com close the sensor otp 20161216 begin
                read_s5k4h8_eeprom_hlt_mtk_fmt();
                kfree(otp_ptr1);
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	   
                if(hlt_vendor_id == 0x42){
                printk("luminjie hlt success \n");
                    return ERROR_NONE;
                }
                else{
