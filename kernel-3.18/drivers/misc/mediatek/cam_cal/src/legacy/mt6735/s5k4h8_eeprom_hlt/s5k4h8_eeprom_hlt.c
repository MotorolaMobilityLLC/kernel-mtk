...
static int s5k4h8_eeprom_hlt_read = 0;
...
OTP_DATA s5k4h8_eeprom_hlt_data = {{0}};
//u8 af_inf_data=0;

extern kal_uint16 hlt_R_gain,hlt_B_gain,hlt_gd_R_gain,hlt_gd_B_gain,hlt_in_af,hlt_ma_af; 
extern kal_uint8  hlt_vendor_id;
#define CAL_VERSION_MAGIC ""
int read_s5k4h8_eeprom_hlt_mtk_fmt(void)
{
#if 1
	//int i = 0;
	int offset = 0;
	...
	CAM_CALINF("OTP readed =%d \n",s5k4h8_eeprom_hlt_read);
	if(1 == s5k4h8_eeprom_hlt_read ) {
		CAM_CALDB("OTP readed ! skip\n");
		return 1;
	}
	spin_lock(&g_CAM_CALLock);
	s5k4h8_eeprom_hlt_read = 1;
	spin_unlock(&g_CAM_CALLock);
	offset = 0;
	s5k4h8_eeprom_hlt_data.Data[2] = hlt_vendor_id;
	s5k4h8_eeprom_hlt_data.Data[3] = hlt_R_gain & 0xFF; 
	s5k4h8_eeprom_hlt_data.Data[4] = hlt_R_gain >> 8;
	s5k4h8_eeprom_hlt_data.Data[5] = hlt_B_gain & 0xFF;
	s5k4h8_eeprom_hlt_data.Data[6] = hlt_B_gain >> 8;
	s5k4h8_eeprom_hlt_data.Data[7] = hlt_gd_R_gain & 0xFF; 
	s5k4h8_eeprom_hlt_data.Data[8] = hlt_gd_R_gain >> 8;
	s5k4h8_eeprom_hlt_data.Data[9] = hlt_gd_B_gain & 0xFF;
	s5k4h8_eeprom_hlt_data.Data[10] = hlt_gd_B_gain >> 8;
	s5k4h8_eeprom_hlt_data.Data[11] = hlt_in_af & 0xFF;
	s5k4h8_eeprom_hlt_data.Data[12] = hlt_in_af >> 8;
	s5k4h8_eeprom_hlt_data.Data[13] = hlt_ma_af & 0xFF;
	s5k4h8_eeprom_hlt_data.Data[14] = hlt_ma_af >> 8;
	CAM_CALINF("luminjie s5k4h8 vendor_id =%x\n",hlt_vendor_id);
	CAM_CALINF("luminjie s5k4h8 R_gain_h =%x,R_gain_l=%x\n",s5k4h8_eeprom_hlt_data.Data[4],s5k4h8_eeprom_hlt_data.Data[3]);
	CAM_CALINF("luminjie s5k4h8 B_gain_h =%x,B_gain_l=%x\n",s5k4h8_eeprom_hlt_data.Data[6],s5k4h8_eeprom_hlt_data.Data[5]);
	CAM_CALINF("luminjie s5k4h8 in_af_h =%x,in_af_l=%x\n",s5k4h8_eeprom_hlt_data.Data[12],s5k4h8_eeprom_hlt_data.Data[12]);
	CAM_CALINF("luminjie s5k4h8 R_gain_h =%x,B_gain_l=%x\n",s5k4h8_eeprom_hlt_data.Data[14],s5k4h8_eeprom_hlt_data.Data[13]);
#endif
	return 0;
}
#if 0
static u8 read_s5k4h8_eeprom_hlt_vendor_id(void)
{
  CAM_CALINF("wangkangmin s5k4h8 vendor id =%x \n",s5k4h8_eeprom_hlt_data.Data[3956]);
  return s5k4h8_eeprom_hlt_data.Data[3956];
}
#endif
#ifdef CONFIG_COMPAT
...
static long s5k4h8eeprom_hlt_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	CAM_CALDB("[CAMERA SENSOR] S5K4H8_EEPROM_HLT_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
{
    memcpy((void *)data,(void *)&s5k4h8_eeprom_hlt_data.Data[offset],size);
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, S5K4H8_EEPROM_HLT_DEVICE_ID,ptempbuf->u4Length);
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = s5k4h8eeprom_hlt_Ioctl_Compat,
#endif
...
static int __init CAM_CAL_init(void)
{
  ...
  //  i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        CAM_CALERR("failed to register s5k4h8_eeprom_hlt driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        CAM_CALERR("failed to register s5k4h8_eeprom_hlt driver, 2nd time\n");
        return -ENODEV;
    }
