...
extern kal_uint16 R_gain,B_gain,gd_R_gain,gd_B_gain,in_af,ma_af; 
...
int read_s5k4h8_eeprom_mtk_fmt(void)
{
	...
	spin_unlock(&g_CAM_CALLock);
	offset = 0;
	s5k4h8_eeprom_data.Data[2] = vendor_id;
	s5k4h8_eeprom_data.Data[3] = R_gain & 0xFF; 
	s5k4h8_eeprom_data.Data[4] = R_gain >> 8;
	s5k4h8_eeprom_data.Data[5] = B_gain & 0xFF;
	s5k4h8_eeprom_data.Data[6] = B_gain >> 8;
...
{
  CAM_CALINF("wangkangmin s5k4h8 vendor id =%x \n",s5k4h8_eeprom_data.Data[3956]);
  return s5k4h8_eeprom_data.Data[3956];
}
#endif
#ifdef CONFIG_COMPAT
...
static long s5k4h8eeprom_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	CAM_CALDB("[CAMERA SENSOR] S5K4h8_EEPROM_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
{
    memcpy((void *)data,(void *)&s5k4h8_eeprom_data.Data[offset],size);
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, S5K4H8_EEPROM_DEVICE_ID,ptempbuf->u4Length);
