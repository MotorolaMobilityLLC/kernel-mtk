...
#define SPI_PR_ERR(fmt, args...) pr_err("spi.c:%5d: <%s>" fmt, __LINE__, __func__, ##args)
#ifdef SPI_DEBUG
...
int secspi_session_open(u32 spinum)
{
	...
	do {
		...
		if (mc_ret != MC_DRV_OK) {
			pr_err("secspi_session_open fail: %d\n", mc_ret);
	...
	pr_info("secspi_session_open: ret=%d, ref=%d\n", mc_ret, secspi_session_ref);

	mutex_unlock(&secspi_lock);
	pr_info("secspi_session_open end\n");
...
int secspi_execute(u32 cmd, tciSpiMessage_t *param)
{
	...
	pr_info("secspi_execute\n");
	mutex_lock(&secspi_lock);

	if (secspi_tci == NULL) {
		mutex_unlock(&secspi_lock);
		pr_err("secspi_tci not exist\n");
		return -ENODEV;
	}

	/*set transfer data para */
	if (param == NULL) {
		pr_err("secspi_execute parameter is NULL !!\n");
	} else {
	...
	if (mc_ret != MC_DRV_OK) {
		pr_err("mc_notify failed: %d", mc_ret);
		goto exit;
	}

	pr_info("SPI mc_wait_notification\n");
	mc_ret = mc_wait_notification(&secspi_session, -1);

	if (mc_ret != MC_DRV_OK) {
		pr_err("SPI mc_wait_notification failed: %d", mc_ret);
		goto exit;
	}
...
static int secspi_session_close(void)
{
	...
	do {
		...
		if (secspi_session_ref == 0) {
			pr_notice("spi_session already closed\n");
			break;
		}
		...
		if (mc_ret != MC_DRV_OK) {
			pr_err("SPI mc_close_session failed: %d\n", mc_ret);
			break;
		}
		...
		if (mc_ret != MC_DRV_OK) {
			pr_err("SPI mc_free_wsm failed: %d\n", mc_ret);
			break;
		}
		secspi_tci = NULL;
		...
		if (mc_ret != MC_DRV_OK)
			pr_err("SPI mc_close_device failed: %d\n", mc_ret);

	} while (0);

	pr_info("secspi_session_close: ret=%d, ref=%d\n", mc_ret, secspi_session_ref);
...
static int mt_spi_next_xfer(struct mt_spi_t *ms, struct spi_message *msg)
{
	...
	if (ms->cur_transfer.len > SPI_DATA_SIZE) {
		...
		pr_info("SPI auto select DMA mode\n");
		reg_val = spi_readl(ms, SPI_CMD_REG);
		reg_val |= ((1 << SPI_CMD_TX_DMA_OFFSET) | (1 << SPI_CMD_RX_DMA_OFFSET));
		spi_writel(ms, SPI_CMD_REG, reg_val);
		pr_info("SPI auto select CMD = 0x%x\n", spi_readl(ms, SPI_CMD_REG));
	} else {
		pr_info("SPI auto select do nothing\n");
	}
#endif
...
static void mt_spi_next_message(struct mt_spi_t *ms)
{
	...
	/* t_rec[1] = sched_clock(); */
	/* pr_info("msgs rec consume time%lld", t_rec[1] - t_rec[0]); */
	...
	spi_rec_time("clke");
	t_rec[1] = sched_clock();
	pr_info("clke rec consume time%lld", t_rec[1] - t_rec[0]);
	reset_spi(ms);
#endif
...
static irqreturn_t mt_spi_interrupt(int irq, void *dev_id)
{
	...
	/* pr_info("xfer:0x%p interrupt status:%x\n", xfer, reg_val & 0x3); */
