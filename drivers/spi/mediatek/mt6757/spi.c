...
#define SPI_INFO(dev, fmt, args...)                                            \
	dev_dbg(dev, "spi.c:%5d: <%s>" fmt, __LINE__, __func__, ##args)
 ...
 * Trustlet UUID.
 */
u8 spi_uuid[6][16] = {{0x09, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static int mt_spi_next_xfer(struct mt_spi_t *ms, struct spi_message *msg)
{
	...
	if (unlikely(!ms)) {
		dev_dbg(&msg->spi->dev, "master wrapper is invalid\n");
		ret = -EINVAL;
		goto fail;
	}
	if (unlikely(!msg)) {
		dev_dbg(&msg->spi->dev, "msg is invalid\n");
		ret = -EINVAL;
		goto fail;
	}
	if (unlikely(!msg->state)) {
		dev_dbg(&msg->spi->dev, "msg config is invalid\n");
		ret = -EINVAL;
		goto fail;
	}

	if (unlikely(!is_interrupt_enable(ms))) {
		dev_dbg(&msg->spi->dev, "interrupt is disable\n");
		ret = -EINVAL;
		goto fail;
	}
#ifdef SPI_AUTO_SELECT_MODE
	...
	if (ms->running == PAUSED) {
		...
		dev_dbg(&msg->spi->dev, "Wrong status\n");
		ret = -1;
		goto fail;
	}
...
static void mt_spi_next_message(struct mt_spi_t *ms)
{
	...
	/* t_rec[1] = sched_clock(); */
	/* pr_debug("msgs rec consume time%lld", t_rec[1] - t_rec[0]); */
	...
	spi_rec_time("clke");
	t_rec[1] = sched_clock();
	pr_debug("clke rec consume time%lld", t_rec[1] - t_rec[0]);
	reset_spi(ms);
#endif
...
static int mt_spi_transfer(struct spi_device *spidev, struct spi_message *msg)
{
	...
	if (unlikely(!msg)) {
		dev_dbg(&spidev->dev, "msg is NULL pointer.\n");
		msg->status = -EINVAL;
		goto out;
	}
	if (unlikely(list_empty(&msg->transfers))) {
		dev_dbg(&spidev->dev, "the message is NULL.\n");
		msg->status = -EINVAL;
		msg->actual_length = 0;
		goto out;
	}
	...
	if (master->setup(spidev)) {
		dev_dbg(&spidev->dev, "set up error.\n");
		msg->status = -EINVAL;
		msg->actual_length = 0;
		goto out;
	}
...
/* alert if PA > 4GB for mt6757 */
		...
		if ((xfer->tx_dma >> 32) != 0x0) {
			...
			SPI_ERR("[Warning] please allocate low-end memory(< "
				"0xffff_ffff) for tx_dma!!\n");
			tx_dma_above_4GB = 1;
		}
...
static irqreturn_t mt_spi_interrupt(int irq, void *dev_id)
{
	...
	if (is_pause_mode(msg)) {
		if (ms->running == INPROGRESS)
			...
			dev_dbg(&msg->spi->dev, "Wrong spi status.\n");
	} else
		ms->running = IDLE;
...
static int mt_spi_setup(struct spi_device *spidev)
{
	...
	if (!spidev) {
		...
		/* dev_dbg(&spidev->dev, "spi device %s: error.\n",
		 * dev_name(&spidev->dev)); */
		return -EINVAL;
	}
	...
	if (spidev->chip_select >= master->num_chipselect) {
		dev_dbg(&spidev->dev, "spi device chip select excesses the number of master's chipselect number.\n");
		return -EINVAL;
	}
	...
	if (!chip_config) {
		...
		if (!chip_config) {
			dev_dbg(&spidev->dev,
				  "spidev %s: memory allocation issue.\n",
				  dev_name(&spidev->dev));
			return -ENOMEM;
		}
 ...
 */
	...
	if (!res) {
		dev_dbg(&pdev->dev, "get resource res NULL.\n");
		return -ENXIO;
	}
	...
	if (!request_mem_region(res->start, resource_size(res), pdev->name)) {
		dev_dbg(&pdev->dev, "SPI register memory region failed");
		return -ENOMEM;
	}
#endif
	...
	if (!spi_base) {
		dev_dbg(&pdev->dev, "SPI iomap failed\n");
		return -ENODEV;
	}

	irq = irq_of_parse_and_map(pdev->dev.of_node, 0);
	if (!irq) {
		dev_dbg(&pdev->dev, "SPI get irq failed\n");
		return -ENODEV;
	}
#endif
	if (of_property_read_u32(pdev->dev.of_node, "cell-index", &pdev->id)) {
		dev_dbg(&pdev->dev, "SPI get cell-index failed\n");
		return -ENODEV;
	}
#if 0
	...
	if (pdev->dev.of_node) {
		...
		if (of_property_read_u32(pdev->dev.of_node, "spi-padmacro",
					 &(ms->pad_macro))) {
			dev_dbg(&pdev->dev, "SPI get pad macro fail failed, using DCT defined SPI pin to set macro\n");
		...
		if (IS_ERR(ms->clk_main)) {
			dev_dbg(&pdev->dev, "cannot get spi main clock or dma clock. main clk err : %ld .\n",
				PTR_ERR(ms->clk_main));
...
#endif
	if (ret) {
		dev_dbg(&pdev->dev, "registering interrupt handler fails.\n");
		/* goto out; */
		return ret;
	}
	...
	if (ret) {
		dev_dbg(&pdev->dev, "spi_register_master fails.\n");
		goto out_free;
	} else {
