...
static irqreturn_t (*user_handlers[CPUXGPTNUMBERS])(int irq,
							void *dev_id) = {0};
...
static unsigned int __cpuxgpt_set_cmp(enum cpuxgpt_num cpuxgpt_num, int countH,
					  int countL)
{
	spin_lock(&cpuxgpt_reg_lock);
...
void mt_cpuxgpt_map_base(void)
{
	...
	if (save_flags)
		pr_debug("map phy addr of CPUXGPT fail !!");

	mt_cpuxgpt_base_phys = cpuxgpt_r.start;

#ifdef CONFIG_ARM64
	pr_debug("cpuxgpt_r.start = 0x%llx\n", mt_cpuxgpt_base_phys);
#else
	pr_debug("cpuxgpt_r.start = 0x%lx\n",
...
static int __init mt_cpuxgpt_init(struct device_node *node)
{
	...
	/* pr_debug("clock-frequency not set in the .dts file"); */
	...
	if (cpuxgpt_regs == NULL) {
		...
		pr_debug("cpuxgpt base address had mapped!!\n");

#if 0
		pr_debug("cpuxgpt_regs=0x%x, irq0=%d, irq1=%d, irq2=%d, irq3=%d\n"
			cpuxgpt_regs, cpuxgpt_irq[0], cpuxgpt_irq[1],
			cpuxgpt_irq[2], cpuxgpt_irq[3]);

		pr_debug("irq4=%d, irq5=%d, irq6=%d, irq7=%d\n",
			cpuxgpt_irq[4], cpuxgpt_irq[5],
				...
				irqreturn_t (*func)(int irq, void *dev_id))
{
	...
	/*trigger IRQ to CPUx mt_gic_cfg_irq2cpu(irq_id,(irq_id -
	 * CPUXGPT_IRQID_BASE)%num_possible_cpus(),1);
	 */
...
void cpu_xgpt_halt_on_debug_en(int en) { __cpuxgpt_halt_on_debug_en(en); }
