...
/* static void mt_gpt_set_mode(enum clock_event_mode mode, struct
...
static void __gpt_set_clk(struct gpt_device *dev, unsigned int clksrc,
			  unsigned int clkdiv)
{
...
static void setup_gpt_dev_locked(struct gpt_device *dev, unsigned int mode,
				 ...
				 unsigned int cmp, void (*func)(unsigned long),
				 unsigned int flags)
{
...
static int __init mt_gpt_init(struct device_node *node)
{
	...
	pr_debug("%s: tmr_regs=0x%lx, tmr_irq=%d, freq=%d\n", __func__,
	...
	setup_syscnt();
	pr_debug("%s: get_cnt_GPT2=%lld\n", __func__,
		 mt_gpt_read(NULL)); /* /TODO: remove */
	gpt_update_unlock(save_flags);

	/* Init GPT3 as 32K free-run counter */
	request_gpt(GPT3, GPT_FREE_RUN, GPT_CLK_SRC_RTC, GPT_CLK_DIV_1, 0, NULL,
			0);
	return 0;
}
...
unsigned int gpt_boot_time(void) { return boot_time_value; }
