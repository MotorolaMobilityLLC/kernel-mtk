...
{
	struct mtk_drm_private *priv;
	if (!drm_dev || IS_ERR_OR_NULL(drm_dev->dev_private)) {
		DDPMSG("%s, drm_dev or priv is null!\n", __func__);
		return;
	}
	priv = drm_dev->dev_private;
	if (IS_ERR_OR_NULL(priv)) {
...
int mtk_read_ddic_cellid(unsigned char *cellid, int reg, int offset_reg, int len)
{
	struct mtk_ddic_dsi_msg *cmd_msg =
		vmalloc(sizeof(struct mtk_ddic_dsi_msg));
	u8 tx[10] = {0};
