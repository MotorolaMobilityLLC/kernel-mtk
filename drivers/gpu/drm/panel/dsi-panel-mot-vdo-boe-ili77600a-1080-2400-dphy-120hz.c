...
#include "include/dsi-panel-mot-vdo-boe-ili77600a-1080-2400-dphy-120hz.h"
#endif
...
#define BOE_ILI_PANEL_VENDOR_ID    0x91070501
//#define BOE_ILI_PANEL_V0_VENDOR_ID  	(BOE_ILI_PANEL_VENDOR_ID | (0xF << 24))
enum panel_version {
    PANEL_EVT = 1,
    PANEL_DVT1 = 2,
    PANEL_DVT2 = 3,
...
#define boe_ili77600a_dcs_write_seq(ctx, seq...)                                     \
	({                                                                     \
		const u8 d[] = {seq};                                          \
		BUILD_BUG_ON_MSG(ARRAY_SIZE(d) > 64,                           \
				 "DCS sequence too big for stack");            \
		boe_ili77600a_dcs_write(ctx, d, ARRAY_SIZE(d));                      \
	})

#define boe_ili77600a_dcs_write_seq_static(ctx, seq...)                              \
	({                                                                     \
		static const u8 d[] = {seq};                                   \
		boe_ili77600a_dcs_write(ctx, d, ARRAY_SIZE(d));                      \
	})

static inline struct boe_ili77600a *panel_to_boe_ili77600a(struct drm_panel *panel)
{
	return container_of(panel, struct boe_ili77600a, panel);
}

#ifdef PANEL_SUPPORT_READBACK
static int boe_ili77600a_dcs_read(struct boe_ili77600a *ctx, u8 cmd, void *data, size_t len)
{
...
static void boe_ili77600a_panel_get_data(struct boe_ili77600a *ctx)
{
	u8 buffer[3] = {0};
	static int ret;

	if (ret == 0) {
		ret = boe_ili77600a_dcs_read(ctx, 0x0A, buffer, 1);
...
static void boe_ili77600a_dcs_write(struct boe_ili77600a *ctx, const void *data, size_t len)
{
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
	ssize_t ret;
	char *addr;
...
static void lcm_parse_panel_version(struct boe_ili77600a *ctx)
{
	int rc;
...
static void boe_ili77600a_panel_init(struct boe_ili77600a *ctx)
{
	pr_info("disp: %s+\n", __func__);
	...
	pr_info("boe ili77600a init start!\n");
	boe_ili77600a_dcs_write_seq_static(ctx,0xFF, 0x5A,0xA5,0x06);
	...
	boe_ili77600a_dcs_write_seq_static(ctx,0x89, 0xF0);
	boe_ili77600a_dcs_write_seq_static(ctx,0x8A, 0xF6);
	boe_ili77600a_dcs_write_seq_static(ctx,0x8B, 0xFF);
	boe_ili77600a_dcs_write_seq_static(ctx,0x87, 0x4D);
	boe_ili77600a_dcs_write_seq_static(ctx,0x8C, 0xD2);
	boe_ili77600a_dcs_write_seq_static(ctx,0x8D, 0xD6);
	boe_ili77600a_dcs_write_seq_static(ctx,0x8E, 0xDA);
	boe_ili77600a_dcs_write_seq_static(ctx,0x8F, 0xDE);
	boe_ili77600a_dcs_write_seq_static(ctx,0x90, 0xDF);
	boe_ili77600a_dcs_write_seq_static(ctx,0x91, 0xE6);
	boe_ili77600a_dcs_write_seq_static(ctx,0x92, 0xE9);
	boe_ili77600a_dcs_write_seq_static(ctx,0x93, 0xED);
	boe_ili77600a_dcs_write_seq_static(ctx,0x94, 0xF0);
	boe_ili77600a_dcs_write_seq_static(ctx,0x95, 0xFF);
	boe_ili77600a_dcs_write_seq_static(ctx,0x96, 0xB1);
	boe_ili77600a_dcs_write_seq_static(ctx,0x97, 0xB4);
	boe_ili77600a_dcs_write_seq_static(ctx,0x98, 0xB9);
	boe_ili77600a_dcs_write_seq_static(ctx,0x99, 0xBE);
	boe_ili77600a_dcs_write_seq_static(ctx,0x9A, 0xC9);
	boe_ili77600a_dcs_write_seq_static(ctx,0x9B, 0xCD);
	boe_ili77600a_dcs_write_seq_static(ctx,0x9C, 0xD3);
	boe_ili77600a_dcs_write_seq_static(ctx,0x9D, 0xE6);
	boe_ili77600a_dcs_write_seq_static(ctx,0x9E, 0xF2);
	boe_ili77600a_dcs_write_seq_static(ctx,0x9F, 0xF3);
	boe_ili77600a_dcs_write_seq_static(ctx,0xAF, 0x18);
...
static int boe_ili77600a_disable(struct drm_panel *panel)
{
...
static int boe_ili77600a_unprepare(struct drm_panel *panel)
{
	struct boe_ili77600a *ctx = panel_to_boe_ili77600a(panel);
...
static int boe_ili77600a_prepare(struct drm_panel *panel)
{
	struct boe_ili77600a *ctx = panel_to_boe_ili77600a(panel);
	int ret;
...
static int boe_ili77600a_enable(struct drm_panel *panel)
{
	struct boe_ili77600a *ctx = panel_to_boe_ili77600a(panel);
...
#if 0 // HBM RAMPING
static int panel_hbm_set_cmdq(struct boe_ili77600a *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t hbm_state)
{
	unsigned int para_count = 0;
	struct mtk_panel_para_table *pTable = NULL;
...
static int boe_ili77600a_get_modes(struct drm_panel *panel,
...
static const struct drm_panel_funcs boe_ili77600a_drm_funcs = {
	.disable = boe_ili77600a_disable,
	.unprepare = boe_ili77600a_unprepare,
	.prepare = boe_ili77600a_prepare,
	.enable = boe_ili77600a_enable,
...
static int boe_ili77600a_probe(struct mipi_dsi_device *dsi)
{
	...
	struct boe_ili77600a *ctx;
	struct device_node *backlight;
	int ret;

	pr_info("%s+ disp:zkd boe_ili77600a_probe start!\n", __func__);
	...
	ctx = devm_kzalloc(dev, sizeof(struct boe_ili77600a), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;
	...
	drm_panel_init(&ctx->panel, dev, &boe_ili77600a_drm_funcs, DRM_MODE_CONNECTOR_DSI);
	...
	pr_info("[%d  %s]- boe,ili77600a,vdo,120hz ret:%d\n", __LINE__, __func__,ret);

	return ret;
}

static int boe_ili77600a_remove(struct mipi_dsi_device *dsi)
{
	struct boe_ili77600a *ctx = mipi_dsi_get_drvdata(dsi);
#if defined(CONFIG_MTK_PANEL_EXT)
...
static const struct of_device_id boe_ili77600a_of_match[] = {
	{
