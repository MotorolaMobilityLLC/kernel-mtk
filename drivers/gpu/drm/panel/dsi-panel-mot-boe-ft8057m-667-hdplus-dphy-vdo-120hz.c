...
#include "include/dsi-panel-mot-boe-ft8057m-667-hdplus-dphy-vdo-120hz.h"
#endif
...
//struct boe_ft8057m *g_ctx = NULL;

#define boe_ft8057m_dcs_write_seq(ctx, seq...)                                         \
	({                                                                     \
		const u8 d[] = { seq };                                        \
		BUILD_BUG_ON_MSG(ARRAY_SIZE(d) > 64,                           \
				 "DCS sequence too big for stack");            \
		boe_ft8057m_dcs_write(ctx, d, ARRAY_SIZE(d));                          \
	})

#define boe_ft8057m_dcs_write_seq_static(ctx, seq...)                                  \
	({                                                                     \
		static const u8 d[] = { seq };                                 \
		boe_ft8057m_dcs_write(ctx, d, ARRAY_SIZE(d));                          \
	})

static inline struct boe_ft8057m *panel_to_boe_ft8057m(struct drm_panel *panel)
{
	return container_of(panel, struct boe_ft8057m, panel);
}

#ifdef PANEL_SUPPORT_READBACK
static int boe_ft8057m_dcs_read(struct boe_ft8057m *ctx, u8 cmd, void *data, size_t len)
{
...
static void boe_ft8057m_panel_get_data(struct boe_ft8057m *ctx)
{
	u8 buffer[3] = {0};
	static int ret;

	if (ret == 0) {
		ret = boe_ft8057m_dcs_read(ctx, 0x0A, buffer, 1);
...
static void boe_ft8057m_dcs_write(struct boe_ft8057m *ctx, const void *data, size_t len)
{
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
	ssize_t ret;
	char *addr;
...
static int boe_ft8057m_disable(struct drm_panel *panel)
{
...
static int boe_ft8057m_set_gesture_flag(int state)
{
	if(state == 1)
		tp_gesture_flag = 1;
	else
...
static int boe_ft8057m_unprepare(struct drm_panel *panel)
{
	struct boe_ft8057m *ctx = panel_to_boe_ft8057m(panel);
	...
	boe_ft8057m_dcs_write_seq_static(ctx, 0x28);
	udelay(10 * 1000);
	boe_ft8057m_dcs_write_seq_static(ctx, 0x10);
	msleep(60);
	boe_ft8057m_dcs_write_seq_static(ctx, 0x00, 0x00);
	boe_ft8057m_dcs_write_seq_static(ctx, 0xf7, 0x5a, 0xa5, 0x95, 0x27);
...
static int boe_ft8057m_prepare(struct drm_panel *panel)
{
	struct boe_ft8057m *ctx = panel_to_boe_ft8057m(panel);
	int ret;
	...
	boe_ft8057m_panel_init(ctx);
	ctx->cabc_mode = 0;

	ret = ctx->error;
	if (ret < 0) {
		pr_info("disp: %s error ret=%d\n", __func__, ret);
		boe_ft8057m_unprepare(panel);
	}
...
#ifdef PANEL_SUPPORT_READBACK
	boe_ft8057m_panel_get_data(ctx);
#endif*/
	pr_info("disp: %s-\n", __func__);
	return ret;
}

static int boe_ft8057m_enable(struct drm_panel *panel)
{
	struct boe_ft8057m *ctx = panel_to_boe_ft8057m(panel);
...
static struct mtk_panel_params ext_params_60hz = {
	...
	.panel_ver = 1,
	//.panel_id = 0x15025892,
	.panel_name = "boe_ft8057m_vid_667_720_120hz",
	.panel_supplier = "boe",
	.lcm_index = 1,      //color index for 2nd source panel
	.max_bl_level = 2047,
...
static int boe_ft8057m_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle,
				 unsigned int level)
{
...
static enum mtk_lcm_version ft8057m_get_lcm_version(void)
{
	return MTK_LEGACY_LCM_DRV_WITH_BACKLIGHTCLASS;
}

static int panel_cabc_set_cmdq(struct boe_ft8057m *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t cabc_mode)
{
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.set_backlight_cmdq = boe_ft8057m_setbacklight_cmdq,
	.ext_param_set = mtk_panel_ext_param_set,
	.get_lcm_version = ft8057m_get_lcm_version,
//	.ata_check = panel_ata_check,
	.set_gesture_flag = boe_ft8057m_set_gesture_flag,
	.panel_feature_set = panel_feature_set,
};
#endif

static int boe_ft8057m_get_modes(struct drm_panel *panel,
...
static const struct drm_panel_funcs boe_ft8057m_drm_funcs = {
	.disable = boe_ft8057m_disable,
	.unprepare = boe_ft8057m_unprepare,
	.prepare = boe_ft8057m_prepare,
	.enable = boe_ft8057m_enable,
	.get_modes = boe_ft8057m_get_modes,
};

static int boe_ft8057m_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct device_node *dsi_node, *remote_node = NULL, *endpoint = NULL;
	struct boe_ft8057m *ctx;
	struct device_node *backlight;
	int ret;

	pr_info("%s+ disp:boe,ft8057m,vdo,120hz\n", __func__);
	...
	ctx = devm_kzalloc(dev, sizeof(struct boe_ft8057m), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;
...
//			 | MIPI_DSI_CLOCK_NON_CONTINUOUS;
	...
	ctx->prepared = true;
	ctx->enabled = true;
	drm_panel_init(&ctx->panel, dev, &boe_ft8057m_drm_funcs, DRM_MODE_CONNECTOR_DSI);
	...
	pr_info("[%d  %s]- boe,ft8057m,vdo,120hz ret:%d\n", __LINE__, __func__,ret);

	return ret;
}

static int boe_ft8057m_remove(struct mipi_dsi_device *dsi)
{
	struct boe_ft8057m *ctx = mipi_dsi_get_drvdata(dsi);
#if defined(CONFIG_MTK_PANEL_EXT)
...
static const struct of_device_id boe_ft8057m_of_match[] = {
	{
#if defined(CONFIG_DRM_PANEL_NUM_NO_LIMIT)
		.compatible = "boe_ft8057m_vid_667_720_120hz",
#else
		.compatible = "boe,ft8057m,vdo,120hz",
#endif
	},
	{}
};

MODULE_DEVICE_TABLE(of, boe_ft8057m_of_match);

static struct mipi_dsi_driver boe_ft8057m_driver = {
	.probe = boe_ft8057m_probe,
	.remove = boe_ft8057m_remove,
	.driver = {
		.name = "boe_ft8057m_vid_667_720_120hz",
		.owner = THIS_MODULE,
		.of_match_table = boe_ft8057m_of_match,
	},
};

module_mipi_dsi_driver(boe_ft8057m_driver);

MODULE_AUTHOR("mediatek");
MODULE_DESCRIPTION("boe ft8057m incell 120hz Panel Driver");
