...
#include "include/dsi-panel-mot-vdo-txd-ft8725-1080-2388-dphy-fhd-120hz.h"
#endif
...
static struct mtk_panel_params ext_params_mode_30 = {
	...
	.panel_ver = 1,
	//.panel_id = 0x01050791,
	.panel_name = "txd_ft8725_vid_1080_2388_fhd_120hz",
	.panel_supplier = "txd",
	.lcm_index = 4,
...
static int panel_cabc_set_cmdq(struct txd *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t cabc_mode)
{
...
#if 0 // HBM RAMPING
static int panel_hbm_set_cmdq(struct txd *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t hbm_state)
{
	unsigned int para_count = 0;
	struct mtk_panel_para_table *pTable = NULL;
...
#if 0
static void txd_parse_panel_version(struct txd *ctx)
{
	...
	if(chosen) {
		...
		if (!rc) {
			if (TM_ILI_PANEL_V0_VENDOR_ID == tmp_id) {
				...
				pr_info("txd panel version v0, ver=%d, vendor_id=0x%x\n", ctx->version, tmp_id);
			}
			else
				pr_info("txd get vendor_id:0x%x\n", tmp_id);
		}
		else
			pr_info("txd mmi,panel_vendor_id not get\n");
	}
	else
		pr_info("txd_parse_panel_version: chosen node null\n");

	pr_info("parse txd panel version:%d\n", ctx->version);
#endif
...
static int txd_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("%s+ disp:zkd txd_probe start!\n", __func__);
	...
	pr_info("[%d  %s]- txd,ft8725,vdo,fhd,120hz ret:%d\n", __LINE__, __func__,ret);
...
static const struct of_device_id txd_of_match[] = {
	{
		...
		.compatible = "txd_ft8725_vid_1080_2388_fhd_120hz",
#else
		.compatible = "txd,ft8725,vdo,fhd,120hz",
#endif
	},
	{}
};
...
static struct mipi_dsi_driver txd_driver = {
	...
	.driver = {
		.name = "txd_ft8725_vid_1080_2388_fhd_120hz",
		.owner = THIS_MODULE,
