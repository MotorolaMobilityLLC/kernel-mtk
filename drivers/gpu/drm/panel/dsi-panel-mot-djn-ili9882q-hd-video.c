...
static int lcm_get_modes(struct drm_panel *panel)
{
	...
	panel->connector->display_info.panel_id = 0x0D058475;
	strcpy(panel->connector->display_info.panel_name, "mipi_mot_vid_djn_hdp_653");
	strcpy(panel->connector->display_info.panel_supplier, "djn-ili9882q");
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (strstr(saved_command_line, "mipi_mot_vid_djn_ili9882q_720p_653")) {
		pr_err("%s+ ili9882q\n", __func__);
	} else {
		pr_err("not match ili9882q !!!\n");
		return -ENODEV;
	}
	pr_info("%s+ ili9882q\n", __func__);
...
MODULE_DESCRIPTION("ili9882q djn VDO LCD Panel Driver");
