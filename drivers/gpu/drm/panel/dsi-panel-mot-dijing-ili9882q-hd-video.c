...
static int lcm_get_modes(struct drm_panel *panel)
{
	...
	panel->connector->display_info.panel_id = 0x0D058473;
	strcpy(panel->connector->display_info.panel_name, "mipi_mot_vid_dijing_hdp_653");
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (strstr(saved_command_line, "mipi_mot_vid_dijing_ili9882q_720p_653")) {
		pr_err("%s+ ili9882q\n", __func__);
	} else {
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-ili9882q-dijing-vdo",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("ili9882q dijing VDO LCD Panel Driver");
