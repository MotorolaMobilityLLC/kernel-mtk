...
#include "include/dsi-panel-mot-tongxingda-ft8726-fhdp-dphy-vdo-120hz.h"
#endif
...
static int tongxingda_unprepare(struct drm_panel *panel)
{
	...
	msleep(120);
	tongxingda_dcs_write_seq_static(ctx, 0x00, 0x00);//cmd2 enable
	...
	tongxingda_dcs_write_seq_static(ctx, 0x00, 0x00);//cmd2 disable
	tongxingda_dcs_write_seq_static(ctx, 0xFF, 0x00, 0x00, 0x00);
	tongxingda_dcs_write_seq_static(ctx, 0x00, 0x80);
	tongxingda_dcs_write_seq_static(ctx, 0xFF, 0x00, 0x00);
...
static int tongxingda_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("%s+ tongxingda,FT8726,vdo,120hz\n", __func__);
	...
	pr_info("[%d  %s]- tongxingda,FT8726,vdo,120hz  ret:%d  \n", __LINE__, __func__,ret);
...
static struct mipi_dsi_driver tongxingda_driver = {
	...
	.driver = {
		.name = "panel-tongxingda-FT8726-vdo-120hz",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("tongxingda FT8726 incell 120hz Panel Driver");
