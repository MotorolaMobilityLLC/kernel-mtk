...
#include "include/dsi-panel-mot-tm-nt36528a-667-hdp-dphy-vdo-120hz.h"
#endif
...
//struct tm_nt36528a *g_ctx = NULL;

#define tm_nt36528a_dcs_write_seq(ctx, seq...)                                         \
	({                                                                     \
		const u8 d[] = { seq };                                        \
		BUILD_BUG_ON_MSG(ARRAY_SIZE(d) > 64,                           \
				 "DCS sequence too big for stack");            \
		tm_nt36528a_dcs_write(ctx, d, ARRAY_SIZE(d));                          \
	})

#define tm_nt36528a_dcs_write_seq_static(ctx, seq...)                                  \
	({                                                                     \
		static const u8 d[] = { seq };                                 \
		tm_nt36528a_dcs_write(ctx, d, ARRAY_SIZE(d));                          \
	})

static inline struct tm_nt36528a *panel_to_tm_nt36528a(struct drm_panel *panel)
{
	return container_of(panel, struct tm_nt36528a, panel);
}

#ifdef PANEL_SUPPORT_READBACK
static int tm_nt36528a_dcs_read(struct tm_nt36528a *ctx, u8 cmd, void *data, size_t len)
{
...
static void tm_nt36528a_panel_get_data(struct tm_nt36528a *ctx)
{
	u8 buffer[3] = {0};
	static int ret;

	if (ret == 0) {
		ret = tm_nt36528a_dcs_read(ctx, 0x0A, buffer, 1);
...
static void tm_nt36528a_dcs_write(struct tm_nt36528a *ctx, const void *data, size_t len)
{
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
	ssize_t ret;
	char *addr;
...
static void tm_nt36528a_panel_init(struct tm_nt36528a *ctx)
{
	pr_info("disp: %s+\n", __func__);
	...
	tm_nt36528a_dcs_write_seq_static(ctx, 0xFF, 0x23);
	tm_nt36528a_dcs_write_seq_static(ctx, 0xFB, 0x01);
	tm_nt36528a_dcs_write_seq_static(ctx, 0x00, 0x60);
	tm_nt36528a_dcs_write_seq_static(ctx, 0x07, 0x00);
	tm_nt36528a_dcs_write_seq_static(ctx, 0x08, 0x01);
	tm_nt36528a_dcs_write_seq_static(ctx, 0x09, 0xAE);

	tm_nt36528a_dcs_write_seq_static(ctx, 0xFF, 0x10);
	tm_nt36528a_dcs_write_seq_static(ctx, 0xFB, 0x01);

	tm_nt36528a_dcs_write_seq_static(ctx, 0x51, 0x07, 0xFF);
	tm_nt36528a_dcs_write_seq_static(ctx, 0x53, 0x24);
	tm_nt36528a_dcs_write_seq_static(ctx, 0x55, 0x00);

/* 	tm_nt36528a_dcs_write_seq_static(ctx, 0xFF, 0x25);
	tm_nt36528a_dcs_write_seq_static(ctx, 0xFB, 0x01);
	tm_nt36528a_dcs_write_seq_static(ctx, 0xEC, 0x01); */

	tm_nt36528a_dcs_write_seq_static(ctx, 0x11);
	msleep(100);
...
static int tm_nt36528a_disable(struct drm_panel *panel)
{
	struct tm_nt36528a *ctx = panel_to_tm_nt36528a(panel);
...
static int tm_nt36528a_unprepare(struct drm_panel *panel)
{
	...
	tm_nt36528a_dcs_write_seq_static(ctx, 0x28);
	udelay(10 * 1000);
	tm_nt36528a_dcs_write_seq_static(ctx, 0x10);
	udelay(10 * 1000);
...
static int tm_nt36528a_prepare(struct drm_panel *panel)
{
	struct tm_nt36528a *ctx = panel_to_tm_nt36528a(panel);
	int ret;
...
#ifdef PANEL_SUPPORT_READBACK
	tm_nt36528a_panel_get_data(ctx);
#endif*/
	pr_info("disp: %s-\n", __func__);
	return ret;
}

static int tm_nt36528a_enable(struct drm_panel *panel)
{
	struct tm_nt36528a *ctx = panel_to_tm_nt36528a(panel);
...
static int tm_nt36528a_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle,
				 unsigned int level)
{
...
static enum mtk_lcm_version nt36528a_get_lcm_version(void)
{
	return MTK_LEGACY_LCM_DRV_WITH_BACKLIGHTCLASS;
}

#if 0
static int panel_cabc_set_cmdq(struct tm_nt36528a *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t cabc_mode)
{
...
static int panel_hbm_set_cmdq(struct tm_nt36528a *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t hbm_state)
{
	unsigned int para_count = 0;
	struct mtk_panel_para_table *pTable = NULL;
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.set_backlight_cmdq = tm_nt36528a_setbacklight_cmdq,
...
static int tm_nt36528a_get_modes(struct drm_panel *panel,
...
static const struct drm_panel_funcs tm_nt36528a_drm_funcs = {
	.disable = tm_nt36528a_disable,
	.unprepare = tm_nt36528a_unprepare,
	.prepare = tm_nt36528a_prepare,
	.enable = tm_nt36528a_enable,
	.get_modes = tm_nt36528a_get_modes,
};

static int tm_nt36528a_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct device_node *dsi_node, *remote_node = NULL, *endpoint = NULL;
	struct tm_nt36528a *ctx;
	struct device_node *backlight;
	int ret;

	pr_info("%s+ disp:tm,nt36528a,vdo,120hz\n", __func__);
	...
	ctx = devm_kzalloc(dev, sizeof(struct tm_nt36528a), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;
...
//			 | MIPI_DSI_CLOCK_NON_CONTINUOUS;
	...
	ctx->prepared = true;
	ctx->enabled = true;
	drm_panel_init(&ctx->panel, dev, &tm_nt36528a_drm_funcs, DRM_MODE_CONNECTOR_DSI);
	...
	pr_info("[%d  %s]- tm,nt36528a,vdo,120hz ret:%d\n", __LINE__, __func__,ret);

	return ret;
}

static int tm_nt36528a_remove(struct mipi_dsi_device *dsi)
{
	struct tm_nt36528a *ctx = mipi_dsi_get_drvdata(dsi);
#if defined(CONFIG_MTK_PANEL_EXT)
...
MODULE_DEVICE_TABLE(of, tm_nt36528a_of_match);

static struct mipi_dsi_driver tm_nt36528a_driver = {
	.probe = tm_nt36528a_probe,
	.remove = tm_nt36528a_remove,
	.driver = {
		.name = "tm_nt36528a_vid_667_720",
		.owner = THIS_MODULE,
		.of_match_table = tm_nt36528a_of_match,
	},
};

module_mipi_dsi_driver(tm_nt36528a_driver);

MODULE_AUTHOR("mediatek");
MODULE_DESCRIPTION("tm nt36528a incell 120hz Panel Driver");
