...
#include "include/dsi-panel-mot-csot-td4160-667-hdplus-dphy-vdo-120hz.h"
#endif
...
//struct csot_td4160 *g_ctx = NULL;

#define csot_td4160_dcs_write_seq(ctx, seq...)                                         \
	({                                                                     \
		const u8 d[] = { seq };                                        \
		BUILD_BUG_ON_MSG(ARRAY_SIZE(d) > 64,                           \
				 "DCS sequence too big for stack");            \
		csot_td4160_dcs_write(ctx, d, ARRAY_SIZE(d));                          \
	})

#define csot_td4160_dcs_write_seq_static(ctx, seq...)                                  \
	({                                                                     \
		static const u8 d[] = { seq };                                 \
		csot_td4160_dcs_write(ctx, d, ARRAY_SIZE(d));                          \
	})

static inline struct csot_td4160 *panel_to_csot_td4160(struct drm_panel *panel)
{
	return container_of(panel, struct csot_td4160, panel);
}

#ifdef PANEL_SUPPORT_READBACK
static int csot_td4160_dcs_read(struct csot_td4160 *ctx, u8 cmd, void *data, size_t len)
{
...
static void csot_td4160_panel_get_data(struct csot_td4160 *ctx)
{
	u8 buffer[3] = {0};
	static int ret;

	if (ret == 0) {
		ret = csot_td4160_dcs_read(ctx, 0x0A, buffer, 1);
...
static void csot_td4160_dcs_write(struct csot_td4160 *ctx, const void *data, size_t len)
{
	struct mipi_dsi_device *dsi = to_mipi_dsi_device(ctx->dev);
	ssize_t ret;
	char *addr;
...
static void csot_panel_tp_reset(struct csot_td4160 *ctx)
{
	pr_info("%s:csot_td4160: +\n", __func__);
	...
	if (IS_ERR(ctx->tp_reset_gpio)) {
		...
		pr_info("%s:csot_td4160: tp_reset_gpio 1\n", __func__);
	}
}
...
static void csot_td4160_panel_init(struct csot_td4160 *ctx)
{
	...
	csot_td4160_dcs_write_seq_static(ctx, 0xB8, 0x06, 0x90, 0x00, 0x00, 0x00, 0x32);
	csot_td4160_dcs_write_seq_static(ctx, 0xB9, 0x06, 0x90, 0x00, 0x00, 0x00, 0x80);
	csot_td4160_dcs_write_seq_static(ctx, 0xBA, 0x06, 0x90, 0x00, 0x00, 0x00, 0x80);
	csot_td4160_dcs_write_seq_static(ctx, 0xCE, 0x5D, 0x58, 0x62, 0x6C, 0x76, 0x80, 0x8B, 0x95, 0x9F, 0xA9, 0xB3, 0xCA, 0xE2, 0xFA, 0xFC, 0xFE, 0xFF, 0x09, 0x46, 0x04, 0x04, 0x00, 0x03, 0x6C);

	csot_td4160_dcs_write_seq_static(ctx, 0x53, 0x2C);
	csot_td4160_dcs_write_seq_static(ctx, 0x55, 0x00);
	csot_td4160_dcs_write_seq_static(ctx, 0x35, 0x00);
	csot_td4160_dcs_write_seq_static(ctx, 0xB0, 0x03);

	//if(kpi_log_level > 1) pr_info("disp:%s 0x11 start\n", __func__);
	csot_td4160_dcs_write_seq_static(ctx, 0x11);
	msleep(120);
...
static int csot_td4160_disable(struct drm_panel *panel)
{
...
static int csot_td4160_set_gesture_flag(int state)
{
	if(state == 1)
		tp_gesture_flag = 1;
	else
...
static int csot_td4160_prepare(struct drm_panel *panel)
{
	struct csot_td4160 *ctx = panel_to_csot_td4160(panel);
	int ret;
...
#ifdef PANEL_SUPPORT_READBACK
	csot_td4160_panel_get_data(ctx);
#endif*/
	pr_info("disp: %s-\n", __func__);
	return ret;
}

static int csot_td4160_enable(struct drm_panel *panel)
{
	struct csot_td4160 *ctx = panel_to_csot_td4160(panel);
...
/*		.page_table = {
	...
	.panel_ver = 1,
	//.panel_id = 0x01012891,
	.panel_name = "csot_td4160_vid_667_720_120hz",
	.panel_supplier = "csot",
	.lcm_index = 2,
	.max_bl_level = 2047,
...
static enum mtk_lcm_version td4160_get_lcm_version(void)
{
	return MTK_LEGACY_LCM_DRV_WITH_BACKLIGHTCLASS;
}

static int panel_cabc_set_cmdq(struct csot_td4160 *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t cabc_mode)
{
...
#if 0
static int panel_hbm_set_cmdq(struct csot_td4160 *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t hbm_state)
{
	unsigned int para_count = 0;
	struct mtk_panel_para_table *pTable = NULL;
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.set_backlight_cmdq = csot_td4160_setbacklight_cmdq,
	.ext_param_set = mtk_panel_ext_param_set,
	.get_lcm_version =  td4160_get_lcm_version,
//	.ata_check = panel_ata_check,
	.set_gesture_flag = csot_td4160_set_gesture_flag,
	.panel_feature_set = panel_feature_set,
};
#endif

static int csot_td4160_get_modes(struct drm_panel *panel,
...
static const struct drm_panel_funcs csot_td4160_drm_funcs = {
	.disable = csot_td4160_disable,
	.unprepare = csot_td4160_unprepare,
	.prepare = csot_td4160_prepare,
	.enable = csot_td4160_enable,
	.get_modes = csot_td4160_get_modes,
};

static int csot_td4160_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct device_node *dsi_node, *remote_node = NULL, *endpoint = NULL;
	struct csot_td4160 *ctx;
	struct device_node *backlight;
	int ret;

	pr_info("%s+ disp:csot,td4160,vdo,120hz\n", __func__);
	...
	ctx = devm_kzalloc(dev, sizeof(struct csot_td4160), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;
...
//			 | MIPI_DSI_CLOCK_NON_CONTINUOUS;
	...
	ctx->prepared = true;
	ctx->enabled = true;
	drm_panel_init(&ctx->panel, dev, &csot_td4160_drm_funcs, DRM_MODE_CONNECTOR_DSI);
	...
	pr_info("[%d  %s]- csot,td4160,vdo,120hz ret:%d\n", __LINE__, __func__,ret);

	return ret;
}

static int csot_td4160_remove(struct mipi_dsi_device *dsi)
{
...
static const struct of_device_id csot_td4160_of_match[] = {
	{
		...
		.compatible = "csot_td4160_vid_667_720_120hz",
#else
		.compatible = "csot,td4160,vdo,120hz",
#endif
	},
	{}
};

MODULE_DEVICE_TABLE(of, csot_td4160_of_match);
...
static struct mipi_dsi_driver csot_td4160_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		.of_match_table = csot_td4160_of_match,
	},
};

module_mipi_dsi_driver(csot_td4160_driver);

MODULE_AUTHOR("mediatek");
MODULE_DESCRIPTION("csot td4160 incell 120hz Panel Driver");
