...
static void usb_phy_tuning(struct mtk_phy_instance *instance)
{
	...
	s32 u2_host_vrt_ref, u2_host_term_ref, u2_host_enhance;
	...
	if (!instance->phy_tuning.inited) {
		...
		instance->phy_tuning.u2_host_vrt_ref =  -1;
		instance->phy_tuning.u2_host_term_ref =  -1;
		instance->phy_tuning.u2_host_enhance =  -1;
		of_node = of_find_compatible_node(NULL, NULL,
		...
		of_node = of_find_compatible_node(NULL, NULL,
			instance->phycfg->tuning_host_node_name);
		if (of_node) {
			/* value won't be updated if property not being found */
			of_property_read_u32(of_node, "u2_vrt_ref",
				(u32 *) &instance->phy_tuning.u2_host_vrt_ref);
			of_property_read_u32(of_node, "u2_term_ref",
				(u32 *) &instance->phy_tuning.u2_host_term_ref);
			of_property_read_u32(of_node, "u2_enhance",
				(u32 *) &instance->phy_tuning.u2_host_enhance);
		}
	...
	u2_host_vrt_ref = instance->phy_tuning.u2_host_vrt_ref;
	u2_host_term_ref = instance->phy_tuning.u2_host_term_ref;
	u2_host_enhance = instance->phy_tuning.u2_host_enhance;
	...
	if (mt_usb_is_device()) {
		...
		if (u2_host_vrt_ref != -1) {
			if (u2_host_vrt_ref <= VAL_MAX_WIDTH_3) {
				u3phywrite32(U3D_USBPHYACR1,
					RG_USB20_VRT_VREF_SEL_OFST,
					RG_USB20_VRT_VREF_SEL, u2_host_vrt_ref);
			}
		}
		if (u2_host_term_ref != -1) {
			if (u2_host_term_ref <= VAL_MAX_WIDTH_3) {
				u3phywrite32(U3D_USBPHYACR1,
					RG_USB20_TERM_VREF_SEL_OFST,
					RG_USB20_TERM_VREF_SEL, u2_host_term_ref);
			}
		}
		if (u2_host_enhance != -1) {
			if (u2_host_enhance <= VAL_MAX_WIDTH_2) {
				u3phywrite32(U3D_USBPHYACR6,
					RG_USB20_PHY_REV_6_OFST,
					RG_USB20_PHY_REV_6, u2_host_enhance);
			}
		}
	}
}
...
{
	.name		= "port0",
	.tuning_node_name = "mediatek,devices_phy_tuning",
	.tuning_host_node_name = "mediatek,host_phy_tuning",
	.port_num	= 0,
	.reg_offset = 0,
