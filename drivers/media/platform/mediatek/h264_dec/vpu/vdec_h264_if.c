...
#ifdef DBG_VDEC_H264_SHOW_INPUT_STREAM
	vdec_h264_dump_input_stream(bs->va, bs->size);
#endif
#ifdef DBG_VDEC_H264_DUMP_INPUT_FILE
	vdec_h264_dump_input_file(bs->va, bs->size);
#endif
...
#else
	mtk_vcodec_debug(inst, "+ [%d] FB dma=%llx va=%p\n",
			 inst->num_nalu, fb_dma, fb);
#endif

#ifdef DBG_VDEC_H264_SHOW_INPUT_STREAM
	if (buf != NULL)
		vdec_h264_dump_input_stream((unsigned long)buf, buf_sz);
#endif
...
#else
	err = vdec_h264_vpu_dec_start(inst, buf_sz, nal_start,
				      (u64)bs->dma_addr, fb_dma, vdec_fb_va);
#endif
			     ...
			     bool disp_list, struct vdec_fb **out_fb)
{
	...
	char *list_name = disp_list ? "disp" : "free";
	struct vdec_fb *fb;

	if (list->count == 0) {
		mtk_vcodec_debug(inst, "[FB] there is no %s fb", list_name);
		*out_fb = NULL;
		return;
	}
	...
	*out_fb = fb;
	mtk_vcodec_debug(inst, "[FB] get %s fb st=%d poc=%d %llx", list_name,
			       ...
			       enum vdec_get_param_type type, void *out)
{
	...
	case GET_PARAM_DISP_FRAME_BUFFER:
		...
		vdec_h264_dump_display_frame(inst, out);
#endif
		break;
