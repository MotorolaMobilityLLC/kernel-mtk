 ...
 * @bs_dma       : Input bit-stream buffer dma address
 * @fb_dma       : Frame buffer dma address
...
static void handle_init_ack_msg(struct vdec_h264_ipi_init_ack *msg)
{
	...
	for (i = 0; i < VDEC_SHMEM_MAX; i++) {
		...
		shmem_va[i] = vpu_mapping_dm_addr(inst->dev, (void *)vpu_addr);
...
static void h264_dec_vpu_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	mtk_vcodec_debug(inst, "- id=%X", msg->msg_id);
	inst->vpu.signaled = 1;
	wake_up_interruptible(&inst->vpu.wq);
}

static int h264_dec_vpu_wait_ack(struct vdec_h264_inst *inst, unsigned int ms)
{
	int ret;
	struct vdec_h264_vpu_inst *vpu = &inst->vpu;

	ret = wait_event_interruptible_timeout(vpu->wq,
					       vpu->signaled == 1,
					       msecs_to_jiffies(ms));
	if (0 == ret) {
				 ...
				 void *msg, int len)
{
	...
	err = vpu_ipi_send(inst->dev, IPI_VDEC_H264, msg, len, wait);
	if (err != 0) {
		mtk_vcodec_err(inst, "vpu_ipi_send fail status=%d", err);
		return err;
	}

	return h264_dec_vpu_wait_ack(inst, VDEC_H264_WAIT_VPU_TIMEOUT_MS);
}
...
int vdec_h264_vpu_init(void *vdec_inst, uint64_t bs_dma, uint32_t bs_sz)
{
	...
	err = vpu_ipi_registration(inst->dev, IPI_VDEC_H264,
...
static int h264_dec_send_ap_ipi(void *vdec_inst, unsigned int msg_id)
{
	struct vdec_h264_inst *inst = vdec_inst;
...
{
	return h264_dec_send_ap_ipi(vdec_inst, AP_IPIMSG_DEC_END);
}

int vdec_h264_vpu_deinit(void *vdec_inst)
{
	return h264_dec_send_ap_ipi(vdec_inst, AP_IPIMSG_DEC_DEINIT);
}

int vdec_h264_vpu_reset(void *vdec_inst)
{
	return h264_dec_send_ap_ipi(vdec_inst, AP_IPIMSG_DEC_RESET);
}
