...
static void handle_init_ack_msg(struct vdec_vp9_inst *inst, void *data)
{
	...
	inst->vpu.drv = (struct vdec_vp9_vpu_drv *)vpu_mapping_dm_addr(
		inst->dev, (void *)(unsigned long)msg->md32_id);
...
static void vp9_dec_vpu_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	mtk_vcodec_debug(inst, "+ id=%X status=%d len=%d", msg->msg_id, msg->status, len);
...
static int vp9_dec_vpu_wait_ack(struct vdec_vp9_inst *inst,
				unsigned int timeout_ms)
{
	...
	if (0 == ret) {
		mtk_vcodec_err(inst, "wait vpu ack time out !");
		return -EINVAL;
	} else if (-ERESTARTSYS == ret) {
		mtk_vcodec_err(inst, "wait m32 ack interrupted by a signal");
		return -EINVAL;
	}
				...
				int len)
{
	...
	err = vpu_ipi_send(inst->dev, IPI_VDEC_VP9, msg, len, wait);
	if (0 != err) {
		mtk_vcodec_err(inst, "vpu_ipi_send fail status=%d", err);
		return err;
	}

	return vp9_dec_vpu_wait_ack(inst, 5000);
...
int vp9_dec_vpu_init(void *vdec_inst, unsigned int *data, unsigned int items)
{
	...
	err = vpu_ipi_registration(inst->dev, IPI_VDEC_VP9,
				vp9_dec_vpu_ipi_handler, "vp9_dec", NULL);
	if (0 != err) {
		mtk_vcodec_err(inst, "vpu_ipi_register fail status=%d", err);
		return err;
	}
	mtk_vcodec_debug(inst, "vpu_ipi_registration success %p", inst);
