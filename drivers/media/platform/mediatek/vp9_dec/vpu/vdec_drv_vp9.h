...
#ifndef _VDEC_DRV_VP9_H_
#define _VDEC_DRV_VP9_H_
...
#define VP9_COMPLIANCE_TEST 0

#define VP9_CRC_ENABLE VP9_COMPLIANCE_TEST
#define VP9_DECODE_BENCHMARK 1

#define VP9_RES_CHG_FLOW !VP9_COMPLIANCE_TEST

#define VP9_SUPER_FRAME_SUPPORT 1 /* Fix Me for over 2 sub-frames */
#define VP9_SUPER_FRAME_DBG 0
...
struct vdec_vp9_work_buf {
	...
	unsigned int pic_sz_y_bs;
	unsigned int pic_sz_c_bs;
	unsigned int ufo_len_sz_y;
	unsigned int ufo_len_sz_c;
#endif
	...
	/* Check size field to know if it's allocated or not */
#if VP9_SUPER_FRAME_SUPPORT
#ifdef Y_C_SEPARATE
	struct vdec_fb sf_ref_buf[VP9_MAX_FRM_BUFF_NUM-1];
#else
	struct mtk_vcodec_mem sf_ref_buf[VP9_MAX_FRM_BUFF_NUM-1];
#endif
#endif
};
...
struct vdec_vp9_inst {
	...
	unsigned long total_init_time;
	unsigned long total_decode_time;
	unsigned long total_vp9_dec_time;
	unsigned long total_vp9_check_time;
	unsigned long total_get_dispbuf_time;
	unsigned long total_get_freebuf_time;
	unsigned long total_get_param_time;
	unsigned long total_set_param_time;
	unsigned long total_deinit_time;
	unsigned long total_input_bs_size;
#endif
