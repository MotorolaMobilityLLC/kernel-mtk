...
static void mtk_venc_set_param(struct mtk_vcodec_ctx *ctx, void *param)
{
	...
	case V4L2_PIX_FMT_YUV420M:
		p->yuv_fmt = VENC_YUV_FORMAT_420;
		break;
	case V4L2_PIX_FMT_YVU420:
	case V4L2_PIX_FMT_YVU420M:
		p->yuv_fmt = VENC_YUV_FORMAT_YV12;
		break;
	case V4L2_PIX_FMT_NV21:
	case V4L2_PIX_FMT_NV21M:
		p->yuv_fmt = VENC_YUV_FORMAT_NV21;
		break;
	}
	...
	mtk_v4l2_debug(1,
			 ...
			 p->yuv_fmt, p->h264_profile, p->h264_level, p->width,
...
static int mtk_venc_encode_header(void *priv)
{
	...
	mtk_v4l2_debug(1,
			 "buf idx=%d va=0x%p dma_addr=0x%llx size=0x%zx",
			 dst_buf->v4l2_buf.index, bs_buf.va,
...
{
	if (ctx->h_dec) {
		venc_if_deinit(ctx->h_dec);
		venc_if_release(ctx->h_dec);
	}
#if MTK_V4L2_BENCHMARK
	mtk_v4l2_debug(0, "\n\nMTK_V4L2_BENCHMARK");
