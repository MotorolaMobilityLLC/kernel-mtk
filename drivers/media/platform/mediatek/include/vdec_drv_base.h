...
/**
 * struct vdec_handle - video decode handle, return by vdec_if_create()
 * @drv_init_flag   : flag to indicate driver initialized or not
 * @fourcc          : video fourcc format
 * @dec_if          : driver common intefrace
 * @drv_handle      : driver handle, return by (*init)() in vdec_common_if
 * @ctx             : mtk v4l2 context
 */
struct vdec_handle {
	bool drv_init_flag;
	unsigned int fourcc;
