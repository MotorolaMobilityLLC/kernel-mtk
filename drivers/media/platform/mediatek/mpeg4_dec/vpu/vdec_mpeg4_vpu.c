...
static void handle_init_ack_msg(struct vdec_mpeg4_ipi_init_ack *msg)
{
	...
	vpu->shmem_addr = vpu_mapping_dm_addr(vpu->dev, (void *)msg->shmem_addr);
	vpu->h_drv = msg->h_drv;
...
static void mpeg4_dec_vpu_ipi_handler(void *msg, unsigned int len, void *priv)
{
	...
	vpu->signaled = 1;
	wake_up_interruptible(&vpu->wq);
}

static int mpeg4_dec_vpu_wait_ack(struct vdec_mpeg4_vpu_inst *vpu, unsigned int ms)
{
	int ret;

	ret = wait_event_interruptible_timeout(vpu->wq, vpu->signaled == 1, msecs_to_jiffies(ms));
	if (0 == ret) {
		mtk_vcodec_err(vpu, "wait time out !");
		return -EBUSY;
	} else if (-ERESTARTSYS == ret) {
		mtk_vcodec_err(vpu, "wait interrupted by a signal");
		return -ERESTARTSYS;
	}
...
static int mpeg4_dec_vpu_send_msg(struct vdec_mpeg4_vpu_inst *vpu, void *msg, int len)
{
	...
	err = vpu_ipi_send(vpu->dev, IPI_VDEC_MPEG4, msg, len, wait);
	if (err != 0) {
		mtk_vcodec_err(vpu, "vpu_ipi_send fail status=%d", err);
		return err;
	}

	err = mpeg4_dec_vpu_wait_ack(vpu, VDEC_MPEG4_WAIT_VPU_TIMEOUT_MS);
	if (err != 0) {
		mtk_vcodec_err(vpu, "mpeg4_dec_vpu_wait_ack fail status=%d", err);
		return err;
	}
...
int vdec_mpeg4_vpu_init(struct vdec_mpeg4_vpu_inst *vpu, struct vdec_mpeg4_bs *bs)
{
	...
	err = vpu_ipi_registration(vpu->dev, IPI_VDEC_MPEG4,
