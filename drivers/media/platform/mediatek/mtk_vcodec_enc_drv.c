...
int mtk_enc_dbg_level = 0;
module_param(mtk_enc_dbg_level, int, S_IRUGO | S_IWUSR);
...
static int fops_vcodec_open(struct file *file)
{
	...
	mtk_v4l2_debug(0, "%s", dev_name(&dev->plat_dev->dev));
	...
	mtk_v4l2_debug(0, "%s dev->ctx[%d]  =%p, sizeof(state)=%lu\n",
			 dev_name(&dev->plat_dev->dev), ctx->idx, ctx,
			 (unsigned long)sizeof(ctx->state));
...
#if IS_ENABLED(CONFIG_MEDIATEK_VPU)
	if (dev->num_instances == 1) {
		ret = vpu_load_firmware(vpu_get_plat_device(dev->plat_dev), 0);
		if (ret) {
...
static int mtk_vcodec_venc_pm_runtime_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mtk_vcodec_dev *m_dev = platform_get_drvdata(pdev);
...
static int mtk_vcodec_venc_pm_runtime_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mtk_vcodec_dev *m_dev = platform_get_drvdata(pdev);
...
static const struct dev_pm_ops mtk_vcodec_venc_pm_ops = {
	...
	SET_RUNTIME_PM_OPS(mtk_vcodec_venc_pm_runtime_suspend,
			   mtk_vcodec_venc_pm_runtime_resume,
			   NULL)
};
