...
static const struct mtk_codec_framesizes mtk_vdec_framesizes[] = {
	{
		.fourcc	= V4L2_PIX_FMT_H264,
		.stepwise = {  64, 4096, 16, 64, 2304, 16 },
	},
	{
		.fourcc	= V4L2_PIX_FMT_VP8,
	...
	{
		.fourcc = V4L2_PIX_FMT_MPEG4,
		.stepwise = {  64, 2048, 16, 64, 1152, 16 },
	},
};
...
#ifndef Y_C_SEPARATE
static int calc_plane_size(int width, int height)
{
...
static struct vb2_buffer *get_display_buffer(struct mtk_vcodec_ctx *ctx)
{
	...
	mtk_v4l2_debug(8, "[%d]", ctx->idx);
	if (0 != vdec_if_get_param(ctx->h_dec,
	...
	if (NULL == disp_frame_buffer) {
		mtk_v4l2_debug(8, "No display frame buffer");
		return NULL;
	}
	...
	if (dstbuf->used) {
		...
		decsize = calc_plane_size(ctx->picinfo.buf_w,
						ctx->picinfo.buf_h);
		vb2_set_plane_payload(&dstbuf->b, 0, decsize);
#else
		vb2_set_plane_payload(&dstbuf->b, 0, ctx->picinfo.y_bs_sz + ctx->picinfo.y_len_sz);
		vb2_set_plane_payload(&dstbuf->b, 1, ctx->picinfo.c_bs_sz + ctx->picinfo.c_len_sz);
#endif
...
static struct vb2_buffer *get_free_buffer(struct mtk_vcodec_ctx *ctx)
{
	...
	if (NULL == tmp_frame_addr) {
		mtk_v4l2_debug(8, " No free frame buffer");
		return NULL;
	}
...
static void mtk_vdec_flush_decoder(struct mtk_vcodec_ctx *ctx)
{
	...
	ret = vdec_if_decode(ctx->h_dec, NULL, NULL, &res_chg);
	if (ret)
...
static bool mtk_vdec_check_res_change(struct mtk_vcodec_ctx *ctx)
{
	...
	if ((ctx->last_decoded_picinfo.pic_w != ctx->picinfo.pic_w) ||
		...
		vdec_if_get_param(ctx->h_dec, GET_PARAM_DPB_SIZE, &dpbsize);
		if (dpbsize == 0)
			dpbsize = 1;
		ctx->dpb_count = dpbsize;

		mtk_v4l2_debug(1,
				"[%d]-> new(%d,%d), old(%d,%d), real(%d,%d) dpbsize=%d\n",
				 ctx->idx, ctx->last_decoded_picinfo.pic_w,
				 ctx->last_decoded_picinfo.pic_h,
				 ctx->picinfo.pic_w, ctx->picinfo.pic_h,
				 ctx->last_decoded_picinfo.buf_w,
...
void mtk_vdec_worker(struct work_struct *work)
{
	...
	if (test_bit(0, &dev->enter_suspend)) {
		mtk_v4l2_debug(0, "[%d] Enter Suspend", ctx->idx);
		ctx->state &= ~MTK_STATE_RUNNING;
		dev->int_cond = 1;
		dev->int_type = MTK_INST_WORK_THREAD_SUSPEND_DONE;
		wake_up_interruptible(&dev->queue);
		return;
	}

	if (ctx->state & MTK_STATE_RES_CHANGE) {
		mtk_v4l2_debug(0, "[%d] [MTK_INST_RES_CHANGE]", ctx->idx);
		ctx->state &= ~MTK_STATE_RUNNING;
		return;
	}
	...
	if (!pfb->base.va) {
		mtk_v4l2_err("[%d] idx=%d dst_addr is NULL!!",
			       ctx->idx, dst_buf->v4l2_buf.index);
		ctx->state &= ~MTK_STATE_RUNNING;
		return;
	}
#endif
...
#else
	mtk_v4l2_debug(8,
			"idx=%d Framebuf  pfb=%p VA=%p DMA=%llx Size=0x%zx",
			dst_buf->v4l2_buf.index, pfb,
			pfb->base.va, (u64)pfb->base.dma_addr, pfb->base.size);
#endif
	...
	if (src_buf_info->lastframe == true) {
		...
		ret = vdec_if_decode(ctx->h_dec, &buf, pfb, &res_chg);
		mtk_v4l2_debug(1,
...
{
	mtk_v4l2_debug(2, "[%d]", ctx->idx);
	if (ctx->h_dec) {
		vdec_if_deinit(ctx->h_dec);
		vdec_if_release(ctx->h_dec);
	}
#if MTK_V4L2_BENCHMARK
	mtk_v4l2_debug(0, "\n\nMTK_V4L2_BENCHMARK");
				...
				enum v4l2_buf_type type)
{
	...
	mtk_v4l2_debug(8, "[%d] (%d)", ctx->idx, type);
			       ...
			       struct v4l2_requestbuffers *reqbufs)
{
	...
	mtk_v4l2_debug(8, "[%d] (%d) count=%d", ctx->idx,
				...
				struct v4l2_buffer *buf)
{
	...
	mtk_v4l2_debug(8, "[%d] (%d) id=%d", ctx->idx,
			 buf->type, buf->index);
			    ...
			    struct v4l2_buffer *buf)
{
	...
	mtk_v4l2_debug(1, "[%d] (%d) id=%d (%d,%d, %d) ",
...
static int vidioc_try_fmt(struct file *file, void *priv, struct v4l2_format *f)
{
	...
	mtk_v4l2_debug(8, "Type is %d", f->type);
			 ...
			 struct v4l2_crop *cr)
{
	...
	if (!(ctx->state &
	    (MTK_STATE_HEADER | MTK_STATE_RUNNING))) {
			     ...
			     struct v4l2_format *f)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		ret = vdec_if_create(ctx, q_data->fmt->fourcc, &ctx->h_dec);
		if (ret) {
		...
		mtk_v4l2_debug(1,
				 "vdec_if_create() success  ctx->h_dec =%lu!",
				 ctx->h_dec);
	} else if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		char str[10];
			     ...
			     struct v4l2_format *f)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE &&
...
#else
		q_data->sizeimage[0] = calc_plane_size(ctx->picinfo.buf_w,
							ctx->picinfo.buf_h);
		q_data->sizeimage[1] = 0;
		q_data->bytesperline[0] = ctx->last_decoded_picinfo.buf_w;
		q_data->bytesperline[1] = 0;
#endif
		/*
		...
		mtk_v4l2_err("Format could not be read --with error");
		return -EINVAL;
	}
			      ...
			      struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_MIN_BUFFERS_FOR_CAPTURE:
		if (ctx->state & MTK_STATE_HEADER &&
			...
			break;
		} else if (!(ctx->state & MTK_STATE_INIT)) {
				   ...
				   unsigned int sizes[], void *alloc_ctxs[])
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
		...
		*nplanes = 1;
		...
		if (*nbuffers > ctx->dpb_count + MTK_VCODEC_MAX_EXTRA_DPB)
			*nbuffers = ctx->dpb_count + MTK_VCODEC_MAX_EXTRA_DPB;
		if (*nbuffers > MTK_VIDEO_MAX_FRAME)
			*nbuffers = MTK_VIDEO_MAX_FRAME;
...
static void vb2ops_vdec_buf_queue(struct vb2_buffer *vb)
{
	...
	mtk_v4l2_debug(8, "[%d] (%d) id=%d, vb=%p",
...
#ifndef Y_C_SEPARATE
{
		...
		dst_addr = (unsigned long)vb2_plane_vaddr(vb, 0);
		if (dst_addr == 0) {
			mtk_v4l2_err("[%d] idx = %d dst_addr is NULL!!",
		...
		memset((char *)dst_addr, 0x10,
			ctx->picinfo.buf_w * ctx->picinfo.buf_h);
		memset((char *)dst_addr +
			ctx->picinfo.buf_w * ctx->picinfo.buf_h,
			0x80,
			(ctx->picinfo.buf_w * ctx->picinfo.buf_h/2));
}
#endif
...
static int vb2ops_vdec_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	ctx->aborting = 0;
	if (((ctx->state & MTK_STATE_HEADER) == 0) &&
		...
		ret = vdec_if_init(ctx->h_dec, &buf, &ctx->picinfo);
...
#else
		ctx->q_data[MTK_Q_DATA_DST].sizeimage[0] =
					calc_plane_size(ctx->picinfo.buf_w,
							ctx->picinfo.buf_h);
		ctx->q_data[MTK_Q_DATA_DST].bytesperline[0] =
					ctx->picinfo.buf_w;
		ctx->q_data[MTK_Q_DATA_DST].sizeimage[1] = 0;
		ctx->q_data[MTK_Q_DATA_DST].bytesperline[1] = 0;
#endif
	...
	ctx->state &= ~(MTK_STATE_ABORT | MTK_STATE_FLUSH |
			MTK_STATE_RES_CHANGE);

	mtk_v4l2_debug(1, "[%d] (%d) state=%d dpbsize=%d",
			ctx->idx, q->type, ctx->state, ctx->dpb_count);
...
static void vb2ops_vdec_stop_streaming(struct vb2_queue *q)
{
	...
	while ((ctx->state & MTK_STATE_RUNNING) && (retry < 10)) {
		...
		ret = mtk_vcodec_wait_for_done_ctx(ctx,
					...
					500, true);
		mtk_v4l2_debug(1, "[%d] (%d) state=%x, retry=%d",
...
static void m2mops_vdec_device_run(void *priv)
{
	...
	if (ctx->state & MTK_STATE_RES_CHANGE) {
		mtk_v4l2_debug(0, "[%d] state=MTK_INST_RES_CHANGE %x",
				 ctx->idx, ctx->state);
		return;
	}
...
static int m2mops_vdec_job_ready(void *m2m_priv)
{
	...
	if (ctx->aborting) {
		mtk_v4l2_debug(3, "[%d] not ready: aborting.", ctx->idx);
		return 0;
	}

	mtk_v4l2_debug(8, "[%d] ready!", ctx->idx);
...
static int mtk_vdec_g_v_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MIN_BUFFERS_FOR_CAPTURE:
		if (ctx->state & MTK_STATE_HEADER &&
			...
			mtk_v4l2_err("Decoding not initialised");
			return -EINVAL;
		} else if (!(ctx->state & MTK_STATE_HEADER)) {
