...
static void handle_init_ack_msg(struct vdec_vp8_inst *inst, void *data)
{
	...
	inst->vsi = (struct vdec_vp8_vsi *)vpu_mapping_dm_addr(
		    inst->dev, (void *)(unsigned long)msg->h_drv);
...
static void vp8_dec_vpu_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	mtk_vcodec_debug(inst, "- id=%X", msg->msg_id);
	inst->vpu.signaled = 1;
	wake_up_interruptible(&inst->vpu.wq_hd);
}

static int vp8_dec_vpu_wait_ack(struct vdec_vp8_inst *inst,
				unsigned int timeout_ms)
{
	int ret;

	ret = wait_event_interruptible_timeout(inst->vpu.wq_hd,
					       inst->vpu.signaled == 1,
					       msecs_to_jiffies(timeout_ms));
	if (0 == ret) {
		mtk_vcodec_err(inst, "wait time out !");
		return -EBUSY;
	} else if (-ERESTARTSYS == ret) {
		mtk_vcodec_err(inst, "wait interrupted by a signal");
		return -ERESTARTSYS;
	}
				...
				int len)
{
	...
	err = vpu_ipi_send(inst->dev, IPI_VDEC_VP8, msg, len, wait);
	if (err != 0) {
		mtk_vcodec_err(inst, "vpu_ipi_send fail status=%d", err);
		return -EINVAL;
	}

	return vp8_dec_vpu_wait_ack(inst, VDEC_VP8_WAIT_VPU_TIMEOUT_MS);
}
...
int vdec_vp8_vpu_init(void *vdec_inst, unsigned int data)
{
	...
	err = vpu_ipi_registration(inst->dev, IPI_VDEC_VP8,
...
static int vp8_dec_send_ap_ipi(void *vdec_inst, unsigned int msg_id)
{
	struct vdec_vp8_inst *inst = vdec_inst;
...
{
	return vp8_dec_send_ap_ipi(vdec_inst, AP_IPIMSG_DEC_START);
}

int vdec_vp8_vpu_dec_end(void *vdec_inst)
{
	return vp8_dec_send_ap_ipi(vdec_inst, AP_IPIMSG_DEC_END);
}

int vdec_vp8_vpu_reset(void *vdec_inst)
{
	return vp8_dec_send_ap_ipi(vdec_inst, AP_IPIMSG_DEC_RESET);
}

int vdec_vp8_vpu_deinit(void *vdec_inst)
{
	return vp8_dec_send_ap_ipi(vdec_inst, AP_IPIMSG_DEC_DEINIT);
}
