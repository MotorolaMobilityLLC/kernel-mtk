...
static irqreturn_t mtk_vcodec_dec_irq_handler(int irq, void *priv)
{
	...
	if (ctx == NULL) {
		mtk_v4l2_err("mtk_vcodec_dec_irq_handler +   ctx  = NULL\n");
		return IRQ_HANDLED;
	}
	mtk_v4l2_debug(8, "mtk_vcodec_dec_irq_handler:  ctx =%d\n", ctx->idx);
	...
	mtk_v4l2_debug(8, "mtk_vcodec_dec_irq_handler -\n");
...
static int fops_vcodec_open(struct file *file)
{
	...
	mtk_v4l2_debug(0, "Created instance [%d]@%p m2m_ctx=%p type=%d\n",
...
static int mtk_vcodec_probe(struct platform_device *pdev)
{
	...
	dev->decode_workqueue =
			create_singlethread_workqueue(MTK_VCODEC_DEC_NAME);
...
static int mtk_vcodec_vdec_suspend(struct device *dev)
{
	...
	mutex_lock(&m_dev->dec_mutex);
	mtk_vcodec_dec_clock_off();

	return 0;
}

static int mtk_vcodec_vdec_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mtk_vcodec_dev *m_dev = platform_get_drvdata(pdev);
...
static const struct dev_pm_ops mtk_vcodec_vdec_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(mtk_vcodec_vdec_suspend,
							mtk_vcodec_vdec_resume)
