...
/* lenovo-sw zhangrc2 add head_type feature begin 0 is usa standard 1 is europe */
static int headset_type = 0; 
static ssize_t arizona_headset_type_show(struct device *dev,
				   struct device_attribute *attr,
				   char *buf);
/* lenovo-sw zhangrc2 add head_type feature end 0 is usa standard 1 is europe */
static DEVICE_ATTR(headset_type, S_IRUGO, arizona_headset_type_show, NULL);
...
static const char *arizona_extcon_get_micbias(struct arizona_extcon_info *info)
{
 ...
 /*the following code to check the headset of amereica and europe mode */
    ...
    dev_info(arizona->dev, "arizona_extcon_get_micbias currentBiasSelect %d\n", currentBiasSelect);
...
int arizona_micd_mic_reading(struct arizona_extcon_info *info, int val)
{
	 ...
	 */
    dev_info(arizona->dev, "val=%d info->micd_ranges[0].max=%d,info->micd_num_modes=%d\n",val,info->micd_ranges[0].max,info->micd_num_modes);
	...
	/* Improve cross-talk */
#if 1 /*remove for delete mic polarity detect*/		
	if (!info->mic) {
		regmap_update_bits(arizona->regmap,
				ARIZONA_ACCESSORY_DETECT_MODE_1,
				ARIZONA_ACCDET_SRC,
...
/*work around for power on reset because of lots of irq appear*/
	...
	if (ret != 0)
		dev_err(&pdev->dev,
			...
			ret);
/* lenovo-sw zhangrc2 add head_type feature begin 0 is usa standard 1 is europe */
	ret = device_create_file(&pdev->dev, &dev_attr_headset_type);
	if (ret != 0)
		dev_err(&pdev->dev,
			"Failed to create sysfs node for headset_type %d\n",
			ret);
