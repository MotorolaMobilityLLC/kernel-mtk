...
#define ET5904_REG_NUM (ET5904_ILIMT_COAR-SGM38121_CHIP_REV+1)
...
#define ET5904_ILIMT_COAR		0x10	//Only ET5904

#define ET5904_DVDD_MIN_UV			600000
#define ET5904_DVDD_MAX_UV			1800000
#define ET5904_DVDD_MIN_REG_VAL		0x00
#define ET5904_DVDD_MAX_REG_VAL		0xFF
#define ET5904_DVDD_STEP_UV			6000

#define ET5904_AVDD_MIN_UV			1200000
#define ET5904_AVDD_MAX_UV			4387500
#define ET5904_AVDD_MIN_REG_VAL		0x00
#define ET5904_AVDD_MAX_REG_VAL		0xFF
#define ET5904_AVDD_STEP_UV			12500
...
static int sgm38121_output_voltage_set(struct sgm38121 *sgm38121, int channel, int mV)
{
	...
	switch(channel)
	{
	case SGM38121_REGULATOR_LDO1: //Vout =  (504 + 8 × d) × 1mV //et5904 vout = (6000 + 6 * d) * 1mv
...
static int et5904_output_voltage_set(struct sgm38121 *sgm38121, int channel, int uV)
{
	uint8_t reg = 0;
	uint8_t reg_val = 0;
	int ret = 0;
	...
	switch(channel)
	{
	case SGM38121_REGULATOR_LDO1: //et5904 vout = (6000 + 6 * d) * 1mv
		reg = SGM38121_LDO1_VOUT;
		if (uV < ET5904_DVDD_MIN_UV) {
			uV = ET5904_DVDD_MIN_UV;
			reg_val = ET5904_DVDD_MIN_REG_VAL;
		} else if (uV > ET5904_DVDD_MAX_UV) {
			uV = ET5904_DVDD_MAX_UV;
			reg_val = ET5904_DVDD_MAX_REG_VAL;
		} else {
			reg_val = (uV - ET5904_DVDD_MIN_UV) / ET5904_DVDD_STEP_UV;
		}
		break;
	case SGM38121_REGULATOR_LDO2:
		reg = SGM38121_LDO2_VOUT;
	...
	case SGM38121_REGULATOR_LDO3:	//VOUT = (1384 + 8 × d) × 1mV
		reg = SGM38121_LDO3_VOUT;
		if (uV < ET5904_AVDD_MIN_UV) {
			uV = ET5904_AVDD_MIN_UV;
			reg_val = ET5904_AVDD_MIN_REG_VAL;
		} else if (uV > ET5904_AVDD_MAX_UV) {
			uV = ET5904_AVDD_MAX_UV;
			reg_val = ET5904_AVDD_MAX_REG_VAL;
		} else {
			reg_val = (uV - ET5904_AVDD_MIN_UV) / ET5904_AVDD_STEP_UV;
		}
		break;
	case SGM38121_REGULATOR_LDO4:
		reg = SGM38121_LDO4_VOUT;
	...
	ret = regmap_write(sgm38121->regmap, reg, reg_val);
	dev_info(sgm38121->dev,"set ldo:%d, voltage = %d",channel+ 1, uV);
	return ret;
}
...
static int et5904_output_voltage_get(struct sgm38121 *sgm38121, int channel, int *value)
{
	uint8_t reg = 0;
	int reg_val = 0;
	int ret = 0;
	switch(channel)
	{
	case SGM38121_REGULATOR_LDO1:
		reg = SGM38121_LDO1_VOUT;
		ret = regmap_read(sgm38121->regmap, reg, &reg_val);
		if (reg_val < ET5904_DVDD_MIN_REG_VAL) reg_val =  ET5904_DVDD_MIN_REG_VAL;
		else if (reg_val > ET5904_DVDD_MAX_REG_VAL) reg_val =  ET5904_DVDD_MAX_REG_VAL;
		*value = reg_val * ET5904_DVDD_STEP_UV + ET5904_DVDD_MIN_UV;
		break;
	case SGM38121_REGULATOR_LDO2:
		reg = SGM38121_LDO2_VOUT;
		ret = regmap_read(sgm38121->regmap, reg, &reg_val);
		...
		*value = reg_val * ET5904_DVDD_STEP_UV + ET5904_DVDD_MIN_UV;
		break;
	case SGM38121_REGULATOR_LDO3:
		reg = SGM38121_LDO3_VOUT;
		ret = regmap_read(sgm38121->regmap, reg, &reg_val);
		if (reg_val < ET5904_AVDD_MIN_REG_VAL) reg_val =  ET5904_AVDD_MIN_REG_VAL;
		else if (reg_val > ET5904_AVDD_MAX_REG_VAL) reg_val =  ET5904_AVDD_MAX_REG_VAL;
		*value = reg_val * ET5904_AVDD_STEP_UV + ET5904_AVDD_MIN_UV;
		break;
	case SGM38121_REGULATOR_LDO4:
		reg = SGM38121_LDO4_VOUT;
		ret = regmap_read(sgm38121->regmap, reg, &reg_val);
...
static int et5904_regulator_set_voltage(struct regulator_dev *rdev, int min_uV, int max_uV, unsigned int *selector)
{
	struct sgm38121 *chip = rdev_get_drvdata(rdev);
	int ret = 0;
	int index = 0;
	dev_info(chip->dev,"et5904_regulaton_set_voltage: name = %s\n", rdev->desc->name);
	...
	if (0 == strcmp(rdev->desc->name,"ldo1")) {
		...
		dev_err(chip->dev,"et5904_regulaton_set_voltage: name error.\n");
	}
	ret = et5904_output_voltage_set(chip, index, max_uV);
	dev_info(chip->dev,"et5904_regulaton_set_voltage: uV:%d,index:%d\n", max_uV ,index);
...
static int et5904_regulator_get_voltage(struct regulator_dev *rdev)
{
	...
	if (0 == strcmp(rdev->desc->name,"ldo1")) {
		...
		dev_err(chip->dev,"et5904_regulaton_get_voltage: name error.\n");
	}
	ret = et5904_output_voltage_get(chip,index, &chip->ldo_output[index]);
	if (ret < 0)
		return -1;
	dev_info(chip->dev,"et5904_regulaton_get_voltage:voltage = %d, name = %s\n",
					chip->ldo_output[index], rdev->desc->name);
	return chip->ldo_output[index];
}
...
static struct regulator_desc et5904_regls_desc[SGM38121_MAX_REGULATORS] = {
...
static int sgm38121_regulator_init(struct sgm38121 *chip)
{
	...
	for (id = 0; id < SGM38121_MAX_REGULATORS; id++) {
		...
		else if ((ldo_chipid == ET5904_ID) || (ldo_chipid == ET5924A_ID))
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (ret < 0 ) {
		...
		} else if (ldo_chipid == ET5904_ID) {
			dev_info(chip->dev, "ET5904 CHIP ID matched!\n");
		} else if (ldo_chipid == ET5924A_ID) {
			dev_info(chip->dev, "ET5924A CHIP ID matched!\n");
		} else {
