...
static struct regulator_dev *regulator_lookup_device(
    struct device *dev, const char *name)
{
	...
	// lookup available regulator device node
	list = of_get_property(dev->of_node, name, &size);
	if (!list) {
		pr_err("get regulator property: %s failed\n", name);
		return NULL;
	}
	size = size / sizeof(*list);
	// lookup available regulator device
	for (i = 0; i < size; i++) {
		node = of_parse_phandle(dev->of_node, name, i);
		if (node) {
			pr_info("regulator node name: %s\n", node->name);
			rdev = of_find_regulator_by_node(node);
			if (rdev) {
				pr_info("found regulator, name: %s\n", rdev->desc->name);
				return rdev;
			}
		}
	}
...
struct regulator *regulator_get_name(struct device *dev, const char *name)
{
	int ret;
	struct regulator_dev *rdev;
	struct regulator *regulator;
	...
	if (name == NULL) {
		pr_err("regulator get name with no identifier\n");
		return ERR_PTR(-EINVAL);
	}

	rdev = regulator_lookup_device(dev, name);
	if (!rdev) {
		dev_err(dev, "%s supply %s not found\n", devname, name);
		return ERR_PTR(-EINVAL);
	}
	...
	regulator = create_regulator(rdev, dev, name);
	if (regulator == NULL) {
		regulator = ERR_PTR(-ENOMEM);
		module_put(rdev->owner);
		put_device(&rdev->dev);
