...
void ion_mm_heap_free_buffer_info(struct ion_buffer *buffer)
{
	...
	if (buffer_info) {
		...
		if ((buffer_info->fix_module_id != -1) &&
			...
			IONMSG
			    (" %s fix mva(%d-0x%x), mva(%d-0x%x)\n",
			     __func__, buffer_info->fix_module_id,
			     buffer_info->FIXED_MVA,
			     buffer_info->module_id, buffer_info->MVA);
...
void ion_mm_heap_memory_detail(void)
{
	...
	ION_PRINT_LOG_OR_SEQ(NULL, " %s", seq_line);
	...
	if (mutex_trylock(&dev->buffer_lock)) {
		for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
			...
			*buffer = rb_entry(n, struct ion_buffer, node);
			...
			if ((1 << heap_id) & ION_HEAP_MULTIMEDIA_MASK) {
				/* heap = buffer->heap; */
