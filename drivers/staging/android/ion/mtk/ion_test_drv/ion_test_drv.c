...
static struct ion_client *gClient;

static char *g_pBuf;

static int ion_test_release(struct inode *inode, struct file *file)
{
	pr_err("ion_test_drv: ion_test_release()\n");
	return 0;
}
...
static int ion_test_open(struct inode *inode, struct file *file)
{
	...
	if (gClient) {
		...
		if (IS_ERR_OR_NULL(handle)) {
			pr_err("ion_test_drv: Cannot allocate buffer. handle=0x%08X\n", (unsigned int) handle);
			return 0;
		}
		pBuf = ion_map_kernel(gClient, handle);
		if (IS_ERR_OR_NULL(pBuf)) {
			pr_err("ion_test_drv: Cannot map buffer to kernel. pBuf=0x%08X\n", (unsigned int) pBuf);
			return 0;
		}
		...
		if (ion_kernel_ioctl(gClient, ION_CMD_MULTIMEDIA, (unsigned long)&mm_data) < 0)
			pr_err("ion_test_drv: Config buffer failed.\n");
		if (ion_phys(gClient, handle, &phy_addr, &len) < 0) {
			pr_err("ion_test_drv: Cannot get physical address.\n");
			return 0;
		}
		pr_err("ion_test_drv: Physical Address: 0x%08X Len: 0x%X\n", (unsigned int) phy_addr, len);
...
static long ion_test_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pr_err("ion_test_drv: ion_test_ioctl() fd=%d\n", (int) arg);
	handle = ion_import_dma_buf(gClient, (int)arg);
	if (IS_ERR_OR_NULL(handle)) {
		pr_err("ion_test_drv: Cannot import buffer. handle=0x%08X\n", (unsigned int)handle);
		return 0;
	}
	pBuf = (char *) ion_map_kernel(gClient, handle);
	if (IS_ERR_OR_NULL(pBuf)) {
		pr_err("ion_test_drv: Cannot map kernel. pBuf=0x%08X\n", (unsigned int)pBuf);
		return 0;
	}
	...
	for (i = 0; i < 1024*1024*20+256; i += 4) {
		if (*(volatile unsigned int *)(pBuf+i) != i) {
			...
			pr_err("ion_test_drv: read data error from kernel!!!!!!!!!!!!\n\n");
		}
	}
	ion_unmap_kernel(gClient, handle);
/*
    struct task_struct* pTask;
    pr_err("ion_test_drv: ioctl(%d)\n", arg);
    pTask = find_task_by_pid_ns(arg, &init_pid_ns);
    pr_err("ion_test_drv: Current task is 0x%08X pid=%d\n", (unsigned int) current, current->pid);
    if (pTask)
		pr_err("ion_test_drv: Target task is 0x%08X pid=%d\n", (unsigned int) pTask, pTask->pid);
    else
		pr_err("ion_test_drv: Target task is NULL\n");
*/
	return 0;
}
...
struct const file_operations ion_test_fops = {
	.owner   = THIS_MODULE,
...
static int __init ion_test_init(void)
{
	...
	pr_err("ion_test_drv: ion_test_init()\n");
	...
	if (g_ion_device) {
		...
		if (IS_ERR_OR_NULL(gClient)) {
			pr_err("ion_test_drv: Cannot create client. gClient=0x%08X\n", (unsigned int) gClient);
			return 0;
		}
	} else
		pr_err("ion_test_drv: ion device not initialized.\n");
...
{
	pr_err("ion_test_drv: ion_test_exit()\n");
