...
static u32 calc_pll_vco_freq(u32 fin, u32 pcw, u32 vcodivsel, u32 prediv,
				 u32 pcwfbits)
{
...
static int calc_pll_freq_cfg(u32 *pcw, u32 *postdiv_idx, u32 freq, u32 fin,
				 int pcwfbits)
{
	static const u64 freq_max = 3000UL * 1000 * 1000; /* 3000 MHz */
...
static unsigned long clk_sdm_pll_recalc_rate(struct clk_hw *hw,
...
static void clk_sdm_pll_set_rate_regs(struct clk_hw *hw, u32 pcw,
					  u32 postdiv_idx)
{
	unsigned long flags = 0;
	struct mtk_clk_pll *pll = to_mtk_clk_pll(hw);
...
static int clk_sdm_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				unsigned long parent_rate)
{
	u32 pcwfbits = 14;
	u32 pcw = 0;
...
static unsigned long clk_arm_pll_recalc_rate(struct clk_hw *hw,
						 unsigned long parent_rate)
{
	struct mtk_clk_pll *pll = to_mtk_clk_pll(hw);
...
static void clk_arm_pll_set_rate_regs(struct clk_hw *hw, u32 pcw,
					  u32 postdiv_idx)
{
	unsigned long flags = 0;
	struct mtk_clk_pll *pll = to_mtk_clk_pll(hw);
...
static int clk_arm_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				unsigned long parent_rate)
{
	u32 pcwfbits = 14;
	u32 pcw = 0;
...
static long clk_mm_pll_round_rate(struct clk_hw *hw, unsigned long rate,
				  unsigned long *prate)
{
	u32 pcwfbits = 14;
	u32 pcw = 0;
...
static int clk_mm_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				   unsigned long parent_rate)
{
	u32 pcwfbits = 14;
	u32 pcw = 0;
...
static unsigned long clk_univ_pll_recalc_rate(struct clk_hw *hw,
						  unsigned long parent_rate)
{
	struct mtk_clk_pll *pll = to_mtk_clk_pll(hw);
...
static void clk_univ_pll_set_rate_regs(struct clk_hw *hw, u32 pcw,
					   u32 postdiv_idx)
{
	unsigned long flags = 0;
	struct mtk_clk_pll *pll = to_mtk_clk_pll(hw);
...
static long clk_univ_pll_round_rate(struct clk_hw *hw, unsigned long rate,
					unsigned long *prate)
{
	u32 pcwfbits = 0;
	u32 pcw = 0;
...
static int clk_univ_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				 unsigned long parent_rate)
{
	u32 pcwfbits = 0;
	u32 pcw = 0;
...
static unsigned long clk_aud_pll_recalc_rate(struct clk_hw *hw,
						 unsigned long parent_rate)
{
	struct mtk_clk_pll *pll = to_mtk_clk_pll(hw);
...
static void clk_aud_pll_set_rate_regs(struct clk_hw *hw, u32 pcw,
					  u32 postdiv_idx)
{
	unsigned long flags = 0;
	struct mtk_clk_pll *pll = to_mtk_clk_pll(hw);
	...
	if (pll_en)
		con0 |= PLL_PCW_CHG; /* aud PLL_PCW_CHG move from con1[31] to
				      *con0[31]
				      */
	...
	writel_relaxed(
		con0,
		con0_addr); /* aud PLL_PCW_CHG move from con1[31] to con0[31] */
...
static long clk_aud_pll_round_rate(struct clk_hw *hw, unsigned long rate,
				   unsigned long *prate)
{
	u32 pcwfbits = 24;
	u32 pcw = 0;
...
static int clk_aud_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				unsigned long parent_rate)
{
	u32 pcwfbits = 24;
	u32 pcw = 0;
...
static unsigned long clk_spm_pll_recalc_rate(struct clk_hw *hw,
						 unsigned long parent_rate)
{
#if MT_CCF_BRINGUP
	pr_debug("[CCF] %s: %s: S\n", __func__, __clk_get_name(hw->clk));
...
static long clk_spm_pll_round_rate(struct clk_hw *hw, unsigned long rate,
				   unsigned long *prate)
{
...
static int clk_spm_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				unsigned long parent_rate)
{
