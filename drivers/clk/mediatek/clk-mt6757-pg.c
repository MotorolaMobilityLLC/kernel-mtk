...
#define MD1_PROT_BIT_MASK                                                      \
	((0x1 << 16) | (0x1 << 17) | (0x1 << 18) | (0x1 << 19) | (0x1 << 20) | \
	 (0x1 << 21))
#define MD1_PROT_BIT_ACK_MASK                                                  \
	((0x1 << 16) | (0x1 << 17) | (0x1 << 18) | (0x1 << 19) | (0x1 << 20) | \
	 (0x1 << 21))
...
#define MFG_PROT_BIT_MASK ((0x1 << 21) | (0x1 << 22))
#define MFG_PROT_BIT_ACK_MASK ((0x1 << 21) | (0x1 << 22))
#define C2K_PROT_BIT_MASK ((0x1 << 22) | (0x1 << 23) | (0x1 << 24))
#define C2K_PROT_BIT_ACK_MASK ((0x1 << 22) | (0x1 << 23) | (0x1 << 24))
#define MDSYS_INTF_INFRA_PROT_BIT_MASK ((0x1 << 3) | (0x1 << 4))
#define MDSYS_INTF_INFRA_PROT_BIT_ACK_MASK ((0x1 << 3) | (0x1 << 4))
...
int spm_topaxi_protect(unsigned int mask_value, int en)
{
	...
	if (count > _TOPAXI_TIMEOUT_CNT_) {
		...
		pr_debug(
		    "[CCF]TOPAXI Bus Protect Timeout Error (%d us)(%d) !!\n",
		    tm_val, count);
		pr_debug("[CCF]INFRA_TOPAXI_PROTECTEN = 0x%x\n",
			 clk_readl(INFRA_TOPAXI_PROTECTEN));
		pr_debug("[CCF]INFRA_TOPAXI_PROTECTSTA0 = 0x%x\n",
			 clk_readl(INFRA_TOPAXI_PROTECTSTA0));
		pr_debug("[CCF]INFRA_TOPAXI_PROTECTSTA1 = 0x%x\n",
...
void mdsys_intf_mon_dump(void)
{
	...
	for (i = 0; i < 7; i++) {
		...
		pr_debug("[CCF]mon8 = 0x%x (mon0=0x%x)\n",
...
static void iomap(void)
{
	...
	if (!node)
		pr_debug("[CCF][INFRA_MD] find node failed\n");
	infra_md_base = of_iomap(node, 0);
	if (!infra_md_base)
		pr_debug("[CCF][INFRA_MD] base failed\n");
}
...
int spm_topaxi_protect_md(unsigned int mask_value, int en)
{
	...
	if (count > _TOPAXI_TIMEOUT_CNT_) {
		...
		tm_val = (tm_e.tv_sec - tm_s.tv_sec) * 1000000 +
			 (tm_e.tv_usec - tm_s.tv_usec);
		pr_debug(
		    "[CCF]TOPAXI_1 Bus Protect Timeout Error (%d us)(%d) !!\n",
		    tm_val, count);
		pr_debug("[CCF]INFRA_TOPAXI_PROTECTEN_1 = 0x%x\n",
			 clk_readl(INFRA_TOPAXI_PROTECTEN_1));
		pr_debug("[CCF]INFRA_TOPAXI_PROTECTSTA0_1 = 0x%x\n",
			 clk_readl(INFRA_TOPAXI_PROTECTSTA0_1));
		pr_debug("[CCF]INFRA_TOPAXI_PROTECTSTA1_1 = 0x%x\n",
...
/* TINFO="Set bus protect" */
		...
		while ((spm_read(INFRA_TOPAXI_PROTECTSTA1_1) &
		...
		spm_write(MD1_OUTPUT_PISO_S_EN_IZ,
			  spm_read(MD1_OUTPUT_PISO_S_EN_IZ) | (0x1 << 0));
		...
		spm_write(MD_EXT_BUCK_ISO,
			  spm_read(MD_EXT_BUCK_ISO) | (0x1 << 0));
...
#else
		spm_write(INFRA_TOPAXI_PROTECTEN,
			  spm_read(INFRA_TOPAXI_PROTECTEN) | DIS_PROT_BIT_MASK);
#ifndef IGNORE_MTCMOS_CHECK
		while ((spm_read(INFRA_TOPAXI_PROTECTSTA1) &
...
#else
		spm_write(INFRA_TOPAXI_PROTECTEN,
			  spm_read(INFRA_TOPAXI_PROTECTEN) | MFG_PROT_BIT_MASK);
#ifndef IGNORE_MTCMOS_CHECK
		while ((spm_read(INFRA_TOPAXI_PROTECTSTA1) &
...
#ifndef IGNORE_MTCMOS_CHECK
		while ((spm_read(INFRA_TOPAXI_PROTECTSTA1_1) &
			C2K_PROT_BIT_ACK_MASK) != C2K_PROT_BIT_ACK_MASK)
			;
#endif
#endif
#ifndef IGNORE_MTCMOS_CHECK
...
/* TINFO="Set bus protect" */
		...
		spm_write(INFRA_TOPAXI_PROTECTEN,
			  spm_read(INFRA_TOPAXI_PROTECTEN) |
			      MDSYS_INTF_INFRA_PROT_BIT_MASK);
#ifndef IGNORE_MTCMOS_CHECK
		while ((spm_read(INFRA_TOPAXI_PROTECTSTA1) &
		...
		spm_write(MDSYS_INTF_INFRA_PWR_CON,
			  spm_read(MDSYS_INTF_INFRA_PWR_CON) | PWR_ISO);
		/* TINFO="Set PWR_CLK_DIS = 1" */
		spm_write(MDSYS_INTF_INFRA_PWR_CON,
		...
		/* TINFO="Set PWR_ON = 0" */
		spm_write(MDSYS_INTF_INFRA_PWR_CON,
			  spm_read(MDSYS_INTF_INFRA_PWR_CON) & ~PWR_ON);
		/* TINFO="Set PWR_ON_2ND = 0" */
		spm_write(MDSYS_INTF_INFRA_PWR_CON,
		...
		/* TINFO="Wait until PWR_STATUS = 0 and PWR_STATUS_2ND = 0" */
		while (
		    (spm_read(PWR_STATUS) & MDSYS_INTF_INFRA_PWR_STA_MASK) ||
...
struct mtk_power_gate scp_clks[] __initdata = {
	...
	PGATE(SCP_SYS_MDSYS_INTF_INFRA, pg_mdsys_intf_infra, NULL, NULL,
	  SYS_MDSYS_INTF_INFRA),
...
static void __init mt_scpsys_init(struct device_node *node)
{
	...
	if (!infracfg_reg || !spm_reg || !ckgen_reg) {
		pr_debug("[CCF]clk-pg-mt6757: missing reg\n");
		return;
	}
...
static int __init debug_init(void)
{
	...
	entry = proc_create("test_pg", 0x0, 0x0, &test_pg_fops);
	if (!entry)
		return -ENOMEM;
