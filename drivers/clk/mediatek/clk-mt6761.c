...
#define CAMSYS_DISABLE_CG	0x0FC1
...
static const struct mtk_gate ifr_clks[] __initconst = {
	...
	GATE_IFR3(CLK_IFR_DEBUGSYS, "ifr_debugsys", "axi_ck", 24),
	...
	GATE_IFR5(CLK_IFR_PWRAP_TMR, "ifr_pwrap_tmr", "f_f26m_ck", 12),
	GATE_IFR5(CLK_IFR_PWRAP_SPI, "ifr_pwrap_spi", "f_f26m_ck", 13),
	GATE_IFR5(CLK_IFR_PWRAP_SYS, "ifr_pwrap_sys", "f_f26m_ck", 14),
...
static const struct mtk_gate gce_clks[] = {
	GATE_GCE(CLK_GCE_FAXI, "gce", "axi_ck", 16),
};
...
static const struct mtk_pll_data plls[] = {
	...
	PLL(CLK_APMIXED_ARMPLL, "armpll", 0x030C, 0x0318, BIT(0),
		PLL_AO, 22, 8, 0x0310, 24, 0, 0, 0, 0x0310, 0, 1),
	...
	PLL(CLK_APMIXED_MFGPLL, "mfgpll", 0x0218, 0x0224, BIT(0),
		0, 22, 8, 0x021C, 24, 0, 0, 0, 0x021C, 0, 1),
	PLL(CLK_APMIXED_MMPLL, "mmpll", 0x0330, 0x033C, BIT(0),
		0, 22, 8, 0x0334, 24, 0, 0, 0, 0x0334, 0, 1),
	PLL(CLK_APMIXED_UNIVPLL, "univpll", 0x0208, 0x0214, BIT(0),
		HAVE_RST_BAR, 22, 8, 0x020C, 24, 0, 0, 0, 0x020C, 0, 2),
	PLL(CLK_APMIXED_MSDCPLL, "msdcpll", 0x0350, 0x035C, BIT(0),
		0, 22, 8, 0x0354, 24, 0, 0, 0, 0x0354, 0, 1),
	PLL(CLK_APMIXED_APLL1, "apll1", 0x031C, 0x032C, BIT(0),
		0, 32, 8, 0x0320, 24, 0x0040, 0x000C, 0, 0x0324, 0, 1),
	PLL(CLK_APMIXED_MPLL, "mpll", 0x0340, 0x034C, BIT(0),
		PLL_AO, 22, 8, 0x0344, 24, 0, 0, 0, 0x0344, 0, 1),
#if 0
...
static void __init mtk_apmixedsys_init(struct device_node *node)
{
	...
	clk_writel(PLLON_CON0, clk_readl(PLLON_CON0) & 0x11111111);
	clk_writel(PLLON_CON1, clk_readl(PLLON_CON1) & 0x11);
}
...
static struct clk_onecell_data *mt6761_top_clk_data;
...
static void __init mtk_topckgen_init(struct device_node *node)
{
	...
	mt6761_top_clk_data = mtk_alloc_clk_data(CLK_TOP_NR_CLK);

	mtk_clk_register_fixed_clks(fixed_clks,
		ARRAY_SIZE(fixed_clks), mt6761_top_clk_data);

	mtk_clk_register_factors(top_divs,
		ARRAY_SIZE(top_divs), mt6761_top_clk_data);
	mtk_clk_register_mux_clr_set_upds(top_muxes,
		ARRAY_SIZE(top_muxes), base,
		&mt6761_clk_lock, mt6761_top_clk_data);
	mtk_clk_register_gates(node, top_clks,
		ARRAY_SIZE(top_clks), mt6761_top_clk_data);
