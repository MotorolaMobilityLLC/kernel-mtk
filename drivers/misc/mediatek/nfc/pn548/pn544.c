...
struct pinctrl *gpctrl_pn548 = NULL;
struct pinctrl_state *st_ven_pn548_h = NULL;
struct pinctrl_state *st_ven_pn548_l = NULL;
struct pinctrl_state *st_dwn_pn548_h = NULL;
struct pinctrl_state *st_dwn_pn548_l = NULL;

struct pinctrl_state *st_eint_pn548_int = NULL;
			       ...
			       size_t count, loff_t *offset)
{
    ...
    if (count < 8) {
		...
		//pn544_dev->client->addr = ((PN544_I2C_ADDR& I2C_MASK_FLAG) | (I2C_ENEXT_FLAG));
				...
				size_t count, loff_t *offset)
{
   ...
   for (i = 0; i < count; i++){
    ...
    if (count < 8) {
	  ...
	  printk("[fangzhihua] %s ,CONFIG_MTK_I2C_EXTENSION    w1 \n",__func__);
	...
	switch (cmd) 
	{
		case PN544_SET_PWR:
			if (arg == 2) {
				...
				ret = pn544_platform_pinctrl_select(gpctrl_pn548, st_dwn_pn548_h);
				msleep(10);
			...
			const struct i2c_device_id *id)
{
	...
	*/    
...
#if 1//def CONFIG_64BIT
	I2CDMAWriteBuf =
	    (char *)dma_zalloc_coherent(&client->dev, MAX_BUFFER_SIZE,
				       (dma_addr_t *) &I2CDMAWriteBuf_pa,
				       GFP_KERNEL);
#else
	I2CDMAWriteBuf =
	    (char *)dma_zalloc_coherent(NULL, MAX_BUFFER_SIZE,
				       (dma_addr_t *) &I2CDMAWriteBuf_pa,
				       GFP_KERNEL);
#endif
	...
	if (node) {
		...
		ret =
		    request_irq(client->irq, pn544_dev_irq_handler,
				IRQF_TRIGGER_HIGH, "IRQ_NFC-eint", pn544_dev);
	...
	pn544_platform_pinctrl_select(gpctrl_pn548, st_ven_pn548_h);
...
static int pn544_platform_pinctrl_init(struct platform_device *pdev)
{
	...
	gpctrl_pn548 = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(gpctrl_pn548)) {
		dev_err(&pdev->dev, "Cannot find pinctrl!");
		ret = PTR_ERR(gpctrl_pn548);
		goto end;
	}

	st_ven_pn548_h = pinctrl_lookup_state(gpctrl_pn548, "ven_high");
	if (IS_ERR(st_ven_pn548_h)) {
		ret = PTR_ERR(st_ven_pn548_h);
		printk("%s: pinctrl err, ven_high\n", __func__);
	}

	st_ven_pn548_l = pinctrl_lookup_state(gpctrl_pn548, "ven_low");
	if (IS_ERR(st_ven_pn548_l)) {
		ret = PTR_ERR(st_ven_pn548_l);
		printk("%s: pinctrl err, ven_low\n", __func__);
	}
//modify by tzf@meitu.begin
	st_dwn_pn548_h = pinctrl_lookup_state(gpctrl_pn548, "rst_high");
    if (IS_ERR(st_dwn_pn548_h)) {
    	ret = PTR_ERR(st_dwn_pn548_h);
    	printk("%s: pinctrl err, dwn_high\n", __func__);
    }
    
    
	st_dwn_pn548_l = pinctrl_lookup_state(gpctrl_pn548, "rst_low");
    if (IS_ERR(st_dwn_pn548_l)) {
    	ret = PTR_ERR(st_dwn_pn548_l);
    	printk("%s: pinctrl err, dwn_low\n", __func__);
    }
    
    
	st_eint_pn548_int = pinctrl_lookup_state(gpctrl_pn548, "irq_init");
    if (IS_ERR(st_eint_pn548_int)) {
    	ret = PTR_ERR(st_eint_pn548_int);
    	printk("%s: pinctrl err, st_eint_pn548_int\n", __func__);
    }
    pn544_platform_pinctrl_select(gpctrl_pn548, st_eint_pn548_int);
