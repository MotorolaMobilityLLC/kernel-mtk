...
{
	PWRAPLOG(
	    "[PMIC_WRAP]There is no PMIC real chip, PMIC_WRAP do Nothing.\n");
	return 0;
}
			     ...
			     unsigned long long timeout_time_ns)
{
	...
	if (timeout_time_ns <= elapse_time) {
		/* timeout */
		PWRAPERR("@@@@Timeout: elapse time%lld,start%lld setting "
			 "timer%lld\n",
...
#else
static unsigned long long _pwrap_get_current_time(void) { return 0; }
static int _pwrap_timeout_ns(unsigned long long start_time_ns,
...
/* ##################################################################### */
typedef unsigned int (*loop_condition_fp)(
    unsigned int); /* define a function pointer */
...
#endif
{
	...
	do {
		if (_pwrap_timeout_ns(start_time_ns, timeout_ns)) {
			PWRAPERR("wait_for_state_ready_init timeout when "
				 "waiting for idle\n");
...
wait_for_state_idle(loop_condition_fp fp, unsigned int timeout_us,
		    unsigned int wacs_register,
...
#endif
{
	...
	do {
		if (_pwrap_timeout_ns(start_time_ns, timeout_ns)) {
			PWRAPERR("wait_for_state_idle timeout when waiting for "
				 "idle\n");
				     ...
				     unsigned int wdata, unsigned int *rdata)
{
	...
	/* Check IDLE */
	return_value = wait_for_state_ready_init(
	    wait_for_fsm_idle, TIMEOUT_WAIT_IDLE, PMIC_WRAP_WACS2_RDATA, 0);
	if (return_value != 0) {
		PWRAPERR(
		    "%s write command fail,return_value=%x\n", __func__,
		    return_value);
		return return_value;
	}
	...
	if (write == 0) {
		...
		return_value = wait_for_state_ready_init(
		    wait_for_fsm_vldclr, TIMEOUT_READ, PMIC_WRAP_WACS2_RDATA,
		    &reg_rdata);
		if (return_value != 0) {
			PWRAPERR("wait_for_fsm_vldclr fail,return_value=%d\n",
				 return_value);
...
static signed int _pwrap_init_cipher(void)
{
	...
	PWRAPLOG("[%s]Config CIPHER of PMIC 0 ok\n", __func__);
	...
	PWRAPLOG("[%s]Config CIPHER of PMIC 1 ok\n", __func__);
#endif
	/*wait for cipher data ready@AP */
	return_value = wait_for_state_ready_init(
	    wait_for_cipher_ready, TIMEOUT_WAIT_IDLE, PMIC_WRAP_CIPHER_RDY, 0);
	if (return_value != 0) {
...
/* Signature */
	...
	WRAP_WR32(PMIC_WRAP_ADC_RDATA_ADDR,
	...
	WRAP_WR32(PMIC_WRAP_ADC_RDATA_ADDR_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_BY_GPS_ADDR);

	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR_LATEST_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR_WP_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR0_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR1_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR2_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR3_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR4_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR5_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR6_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR7_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR8_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR9_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR10_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR11_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR12_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR13_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR14_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR15_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR16_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR17_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR18_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR19_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR20_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR21_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR22_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR23_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR24_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR25_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR26_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR27_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR28_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR29_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR30_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
	WRAP_WR32(PMIC_WRAP_MD_ADC_RDATA_ADDR31_1,
		  PMIC_AUXADC_ADC_OUT_DCXO_MDRT_ADDR);
}
...
static signed int _pwrap_init_sistrobe(int dual_si_sample_settings)
{
	...
	/* TINFO = "[DrvPWRAP_InitSiStrobe] SI Strobe Calibration For PMIC
	 * 0............" */
	/* TINFO = "[DrvPWRAP_InitSiStrobe] Scan For The First Valid Sampling
	 * Clock Edge......" */
	found = 0;
	for (si_en_sel = 0; si_en_sel < 8; si_en_sel++) {
		for (si_ck_sel = 0; si_ck_sel < 2; si_ck_sel++) {
			...
			pwrap_read_nochk(PMIC_DEW_READ_TEST_ADDR, &rdata);
			if (rdata == 0x5aa5) {
				PWRAPLOG("[DrvPWRAP_InitSiStrobe]The First "
					 "Valid Sampling Clock Edge Is Found "
					 "!!!\n");
				PWRAPLOG("si_en_sel = %x, si_ck_sel = %x, "
					 "si_sample_ctrl = %x, rdata = %x\n",
					 si_en_sel, si_ck_sel, si_sample_ctrl,
					 rdata);
				found = 1;
				break;
			}
			PWRAPERR("si_en_sel = %x, si_ck_sel = %x, "
				 "*PMIC_WRAP_SI_SAMPLE_CTRL = %x, rdata = %x\n",
	...
	for (si_dly = 0; si_dly < 10; si_dly++) {
...
#endif
		{
			...
			if ((rdata & 0x7fff) != (test_data[i] & 0x7fff)) {
				PWRAPERR("[DrvPWRAP_InitSiStrobe] (%x, %x, %x) "
	...
	/* TINFO = "[DrvPWRAP_InitSiStrobe] Change The Sampling Clock Edge To
	 * The Next One."
	 */
	...
	if (si_dly == 10) {
		PWRAPLOG(
		    "SI Strobe Calibration For PMIC 0 Done, (%x, si_dly%x)\n",
	...
	pwrap_read_nochk(PMIC_DEW_READ_TEST_ADDR, &rdata);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		PWRAPERR("[_pwrap_init_dio] Read Test Failed, DEW_READ_TEST "
			 "rdata = %x, exp = 0x5aa5\n",
			 rdata);
		return 0x10;
	}
...
{
	pwrap_write_nochk(MT6355_FILTER_CON0,
			  0xf); /* turn on IO filter function */
	pwrap_write_nochk(PMIC_TOP_CKHWEN_CON0_SET_ADDR,
			  0x80); /* turn on SPI slave DCM */
	pwrap_write_nochk(
	    MT6355_SMT_CON1,
	    0xf); /* turn on IO SMT function to improve noise immunity */
	pwrap_write_nochk(
	    PMIC_GPIO_PULLEN0_CLR_ADDR,
	    0x3c0); /* turn off IO pull function for power saving */
	pwrap_write_nochk(MT6355_RG_SPI_CON0,
			  0x1); /* turn off IO pull function for power saving */
#ifdef DUAL_PMICS
	pwrap_write_nochk(EXT_FILTER_CON0,
			  0xf); /* turn on IO filter function */
	pwrap_write_nochk(EXT_TOP_CKHWEN_CON0_SET,
			  0x80); /* turn on SPI slave DCM */
	pwrap_write_nochk(
	    EXT_SMT_CON1,
	    0xf); /* turn on IO SMT function to improve noise immunity */
	pwrap_write_nochk(EXT_RG_SPI_CON,
			  0x1); /* turn off IO pull function for power saving */
#endif
...
static signed int _pwrap_reset_spislv(void)
{
	...
	WRAP_WR32(PMIC_WRAP_MAN_CMD, (OP_WR << 13) | (OP_OUTS << 8));
	return_value = wait_for_state_ready_init(
	    wait_for_sync, TIMEOUT_WAIT_IDLE, PMIC_WRAP_WACS2_RDATA, 0);
...
static int _pwrap_wacs2_write_test(int pmic_no)
{
	...
	if (pmic_no == 0) {
		...
		pwrap_read_nochk(PMIC_DEW_WRITE_TEST_ADDR, &rdata);
		if (rdata != 0xa55a) {
			PWRAPERR("Error: Write Test Failed. DEW_WRITE_TEST "
				 "rdata = %x, exp = 0xa55a\n",
				 rdata);
	...
	if (pmic_no == 1) {
		...
		if (rdata != 0xa55a) {
			PWRAPERR("Error: Write Test Failed. EXT_DEW_WRITE_TEST "
				 "rdata = %x, exp = 0xa55a\n",
				 rdata);
			return E_PWR_WRITE_TEST_FAIL;
		}
...
static unsigned int pwrap_read_test(void)
{
	...
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		PWRAPERR(
		    "Read Test fail,rdata=0x%x, exp=0x5aa5,return_value=0x%x\n",
		    rdata, return_value);
		return E_PWR_READ_TEST_FAIL;
	}
...
static unsigned int pwrap_write_test(void)
{
	...
	if ((rdata != DEFAULT_VALUE_READ_TEST) || (sub_return != 0) ||
	    (sub_return1 != 0)) {
		PWRAPERR("write test "
			 "error,rdata=0x%x,exp=0xa55a,sub_return=0x%x,sub_"
			 "return1=0x%x\n",
...
signed int pwrap_init(void)
{
	...
	if (sub_return != 0) {
		PWRAPERR("Error: DrvPWRAP_InitSPISLV Failed, sub_return = %x",
			 sub_return);
		return -1;
	}
	...
	if (sub_return != 0) {
		PWRAPERR(
		    "_pwrap_init_dio test error,error code=%x, sub_return=%x\n",
		    0x11, sub_return);
	...
	*/
	...
	/* WRAP_WR32(PMIC_WRAP_BWC_OPTIONS,  WRAP_RD32(PMIC_WRAP_BWC_OPTIONS) |
	 * 0x8);
	 */
...
#if (PMIC_WRAP_KERNEL)
		reg_value =
		    WRAP_RD32(((unsigned int *)(PMIC_WRAP_BASE + i * 4)));
#else
...
void pwrap_dump_all_register(void) { pwrap_dump_ap_register(); }
...
static void pwrap_of_iounmap(void) { iounmap(topckgen_base); }
#endif
				  ...
				  unsigned int flag)
{
	...
	err = sspm_ipi_send_sync(IPI_ID_PMIC, 1, (void *)ipi_buf, 0,
				 &ipi_data_ret);
	if (err != 0)
...
static void pwrap_int_test(void)
{
	...
	while (1) {
		...
		PWRAPREG("Pwrap INT status "
			 "check,PMIC_WRAP_EINT_STA=0x%x,INT_STA[0x01B4]=0x%x\n",
			 rdata1, rdata2);
		msleep(500);
	}
}
...
static signed int mt_pwrap_store_hal(const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "-h", 2)) {
		PWRAPREG("PWRAP debug: "
			 "[-dump_reg][-trace_wacs2][-init][-rdap][-wrap][-"
			 "rdpmic][-wrpmic][-readtest][-writetest]\n");
...
/********************************************************************************************/
/* extern API for PMIC driver, INT related control, this INT is for PMIC chip to
 * AP */
/********************************************************************************************/
