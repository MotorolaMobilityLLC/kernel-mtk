...
/* Write test using WACS2 */
	...
	if ((rdata != MT6351_WRITE_TEST_VALUE) || (sub_return != 0) ||
	    (sub_return1 != 0)) {
...
/*-pwrap
 * debug--------------------------------------------------------------------*/
void pwrap_dump_all_register(void) { PWRAPLOG("Nothing to do\n"); }
...
static u32 pwrap_read_test(void) { return 0; }

static u32 pwrap_write_test(void) { return 0; }
	...
	if (!strncmp(buf, "-h", 2)) {
		...
		/* pwrap_read_reg_on_ap(reg_addr); */
		    ...
		    /* pwrap_read_reg_on_pmic(reg_addr); */
			     ...
			     (sscanf(buf + 7, "%x %x", &reg_addr, &reg_value) ==
			      2))
...
static u64 _pwrap_time2ns(u64 time_us) { return time_us * 1000; }

#else
static u64 _pwrap_get_current_time(void) { return 0; }

static BOOL _pwrap_timeout_ns(u64 start_time_ns,
			      u64 elapse_time) /* ,u64 timeout_ns) */
{
	return 0;
}

static u64 _pwrap_time2ns(u64 time_us) { return 0; }
...
static inline u32 wait_for_cipher_ready(u32 x) { return x != 3; }
					   ...
					   u32 *read_reg)
{
	...
	do {
		...
		/* read command state
		 * machine:FSM_REQ-->wfdle-->WFVLDCLR;write:FSM_REQ-->idle
		 */
				      ...
				      void *wacs_vldclr_register, u32 *read_reg)
{
	...
	do {
		if (_pwrap_timeout_ns(start_time_ns, timeout_ns)) {
			PWRAPERR(
			  "wait_for_state_idle timeout when waiting foridle\n");
			pwrap_dump_ap_register();
			return E_PWR_WAIT_IDLE_TIMEOUT;
		}
		reg_rdata = WRAP_RD32(wacs_register);
 ...
 */
/*****************************************************************************/
u32 mt_pmic_wrap_eint_status(void) { return WRAP_RD32(PMIC_WRAP_EINT_STA); }
...
static s32 _pwrap_init_dio(u32 dio_en)
{
	...
	/*	if (rdata != MT6351_DEFAULT_VALUE_READ_TEST) { */
	/*		PWRAPERR("[Dio_mode][Read Test] fail,dio_en = %x,
	 * READ_TEST rdata=%x, exp=%x\n", */
...
/* Read Test */
	...
	if (rdata != MT6351_DEFAULT_VALUE_READ_TEST) {
		PWRAPERR(
		  "%s,read test error,error code=%x, rdata=%x\n", __func__,
			 1, rdata);
		return E_PWR_READ_TEST_FAIL;
	}
#endif
...
static s32 _pwrap_init_sistrobe(void)
{
	...
	u32 test_data[30] = {0x6996, 0x9669, 0x6996, 0x9669, 0x6996, 0x9669,
	...
	for (ind = 0; ind < 24; ind++) { /* 24 sampling clock edge */
		...
		fail_cnt = 0;
		for (i = 0; i < 30; i++) {
			_pwrap_wacs2_nochk(1, MT6351_DEW_WRITE_TEST,
					   test_data[i], &rdata);
			_pwrap_wacs2_nochk(0, MT6351_DEW_WRITE_TEST, 0, &rdata);

			if (rdata != test_data[i]) {
				fail_cnt++;
				PWRAPLOG("%s tuning, index=%d rdata =%x\n",
					 __func__,
					 ind, rdata);
				break;
			}
			PWRAPLOG(
			    "%s pass,index=%d rdata=%x\n", __func__,
			    ind, rdata);
		}
		if (fail_cnt == 0)
			result |= (0x1 << ind);
	...
	if ((tmp1 - tmp2) != result) {
		PWRAPERR(
		  "%s Fail, result = %llx, leading_one:%d, tailing_one:%d\n",
		  __func__,
	...
	if (result_faulty == 0) {
		...
		PWRAPERR("%s Fail,result_faulty=%x\n", __func__,
			 result_faulty);
		ret = result_faulty;
	}
	return ret;
...
s32 pwrap_init(void)
{
	...
	PWRAPLOG("kernel %s start!!!!!!!!!!!!!\n", __func__);

	/* toggle PMIC_WRAP and pwrap_spictl reset */
	__pwrap_soft_reset();
	PWRAPLOG("soft reset ok\n");

/* Set SPI_CK_freq = 26MHz. It can marked when use fpga verify, because it has
 * no address on MT6797 fpga
 */
...
#ifdef SLV_6351
	sub_return =
	    pwrap_write_nochk(MT6351_DEW_WRITE_TEST, MT6351_WRITE_TEST_VALUE);
...
static void pwrap_int_test(void)
{
	...
	while (1) {
		...
		PWRAPREG("Pwrap INT status "
			 "check,PMIC_WRAP_EINT_STA=0x%x,MT6351_INT_STA[0x01B4]="
			 "0x%x\n",
			 rdata1, rdata2);
#endif
		msleep(500);
