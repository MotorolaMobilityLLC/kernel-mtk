			     ...
			     unsigned long long timeout_time_ns)
{
	...
	if (timeout_time_ns <= elapse_time) {
		/* timeout */
		PWRAPLOG
		    ("Timeout: elapse time%lld,start%lld setting timer%lld\n",
				  ...
				  unsigned int wdata, unsigned int *rdata)
{
	...
	return_value =
	    wait_for_state_idle(wait_for_fsm_idle, TIMEOUT_WAIT_IDLE,
				PMIC_WRAP_WACS2_RDATA, PMIC_WRAP_WACS2_VLDCLR,
				0);
	if (return_value != 0) {
		PWRAPLOG("fsm_idle fail,ret=%d\n", return_value);
		goto FAIL;
	}
	wacs_write = write << 31;
	wacs_adr = (adr >> 1) << 16;
	...
	if (write == 0) {
		...
		if (return_value != 0) {
			PWRAPLOG("fsm_vldclr fail,ret=%d\n", return_value);
			return_value += 1;
			goto FAIL;
		}
...
static void _pwrap_InitStaUpd(void)
{
	...
	WRAP_WR32(PMIC_WRAP_MD_AUXADC_RDATA_LATEST_ADDR,
		  (MT6356_AUXADC_ADC41 << 16) + MT6356_AUXADC_ADC36);
...
{
	pwrap_write_nochk(MT6356_FILTER_CON0, 0xf0);
	pwrap_write_nochk(MT6356_SMT_CON1, 0xf);
	pwrap_write_nochk(MT6356_GPIO_PULLEN0_CLR, 0xf0);
	pwrap_write_nochk(MT6356_RG_SPI_CON0, 0x1);
#ifdef DUAL_PMICS
	pwrap_write_nochk(EXT_FILTER_CON0, 0xf);
	pwrap_write_nochk(EXT_TOP_CKHWEN_CON0_SET, 0x80);
...
static unsigned int pwrap_write_test(void)
{
	...
	sub_return =
	    pwrap_wacs2_write(PMIC_DEW_WRITE_TEST_ADDR,
...
signed int pwrap_init(void)
{
	...
	/* WRAP_WR32(PMIC_WRAP_BWC_OPTIONS,
	 * WRAP_RD32(PMIC_WRAP_BWC_OPTIONS) | 0x8);
	 */
...
static int pwrap_of_iomap(void)
{
	...
	if (!infracfg_ao_node) {
		PWRAPLOG("get INFRACFG_AO failed\n");
		return -ENODEV;
	}

	infracfg_ao_base = of_iomap(infracfg_ao_node, 0);
	if (!infracfg_ao_base) {
		PWRAPLOG("INFRACFG_AO iomap failed\n");
		return -ENOMEM;
	}
	...
	if (!topckgen_node) {
		PWRAPLOG("get TOPCKGEN failed\n");
		return -ENODEV;
	}

	topckgen_base = of_iomap(topckgen_node, 0);
	if (!topckgen_base) {
		PWRAPLOG("TOPCKGEN iomap failed\n");
		return -ENOMEM;
	}
	return 0;
}
				  ...
				  unsigned int flag)
{
	...
	err =
	    sspm_ipi_send_sync_new(IPI_ID_PMIC_WRAP, IPI_OPT_POLLING,
				   (void *)ipi_buf, 3, &ipi_data_ret, 1);
	if (err != 0)
		PWRAPLOG("ipi_write error: %d\n", err);
	else
...
static int pwrap_ipi_register(void)
{
	...
	if (retry >= 10)
		PWRAPLOG("pwrap_ipi_register fail\n");
	return 0;
}
#endif
...
static int __init pwrap_hal_init(void)
{
	...
	if (!pwrap_node) {
		PWRAPLOG("PWRAP get node failed\n");
		return -ENODEV;
	}

	pwrap_base = of_iomap(pwrap_node, 0);
	if (!pwrap_base) {
		PWRAPLOG("PWRAP iomap failed\n");
		return -ENOMEM;
	}

	pwrap_irq = irq_of_parse_and_map(pwrap_node, 0);
	if (!pwrap_irq) {
		PWRAPLOG("PWRAP get irq fail\n");
		return -ENODEV;
	}
...
#endif
		if (ret) {
			PWRAPLOG("register IRQ failed (%d)\n", ret);
			return ret;
		}
	} else {
		PWRAPLOG("not init (%d)\n", ret);
...
/*****************************************************************************/
/* extern API for PMIC driver, INT related control, */
/*****************************************************************************/
...
void mt_pmic_wrap_eint_clr(int offset)
{
	if ((offset < 0) || (offset > 3))
		PWRAPLOG("clear EINT flag error, only 0-3 bit\n");
	else
