...
static void mtktspmic_read_efuse(void)
{
	...
	mtktspmic_info(
	    "[pmic_debug] 6356_efuse: g_o_vts		= %d\n", g_o_vts);
...
void mtktspmic_cali_prepare2(void)
{
	...
	mtktspmic_info(
		       "[Thermal calibration] SLOPE1=%d SLOPE2=%d INTERCEPT=%d, Vbe = %d\n",
		       g_tsbuck1_slope1, g_tsbuck1_slope2, g_tsbuck1_intercept,
		       vbe_t);
	...
	mtktspmic_info(
		       "[Thermal calibration] SLOPE1=%d SLOPE2=%d INTERCEPT=%d, Vbe = %d\n",
		       g_tsbuck2_slope1, g_tsbuck2_slope2, g_tsbuck2_intercept,
		       vbe_t);
}
