 ...
 */
/*
 * The tscpu_g_tc structure controls the polling rates and sensor mapping
 * tables
...
struct thermal_controller tscpu_g_tc[THERMAL_CONTROLLER_NUM] = {
	 [0] = {
	     ...
	     .ts_number = 3,
	     .tc_offset = 0x0,
	     .tc_speed = {0x0000000C, 0x0001003B, 0x0000030D} /* 4.9ms */
	 },
	 [1] = {
	 .ts = {TS_MCU4, TS_MCU5},
	 .ts_number = 2,
 ...
 * else if cur_temp >= tscpu_polling_trip_temp2 && curr_temp <
 * tscpu_polling_trip_temp1,
...
#else
	/* Use CCF instead */
	tscpu_dpr_debug("%s CCF\n", __func__);
	ret = clk_prepare_enable(therm_main);
	if (ret)
		pr_debug("Cannot enable thermal clock.\n");
#endif
	return ret;
}
				 ...
				 enum thermal_bank_name ts_bank)
{
	 ...
	 *	the dominant sensor. You can observe it in the thermal doc
	 *provided
	 * by Thermal DE.
	 *	For example,
	 *	Bank 1 is for SOC + GPU. Observe all scenarios related to GPU
	 *tests
	 * to
	...
	/*
	 *	The equations in this function are confirmed by Thermal DE
	 *Alfred
	 * Tsai.
	 *	Don't have to change until using next generation thermal
	 *sensors.
	 */
	...
	tscpu_dpr_debug("ts_MTS=%d, ts_BTS=%d\n", ts_ptpod.ts_MTS,
		      ts_ptpod.ts_BTS);
}
...
{
	pr_debug("[cal] g_adc_ge_t	  = %d\n", g_adc_ge_t);
	pr_debug("[cal] g_adc_oe_t	  = %d\n", g_adc_oe_t);
	pr_debug("[cal] g_degc_cali	 = %d\n", g_degc_cali);
	pr_debug("[cal] g_adc_cali_en_t = %d\n", g_adc_cali_en_t);
	pr_debug("[cal] g_o_slope		= %d\n", g_o_slope);
	pr_debug("[cal] g_o_slope_sign  = %d\n", g_o_slope_sign);
	pr_debug("[cal] g_id			= %d\n", g_id);

	pr_debug("[cal] g_o_vtsmcu1	 = %d\n", g_o_vtsmcu1);
	pr_debug("[cal] g_o_vtsmcu2	 = %d\n", g_o_vtsmcu2);
	pr_debug("[cal] g_o_vtsmcu3	 = %d\n", g_o_vtsmcu3);
	pr_debug("[cal] g_o_vtsmcu4	 = %d\n", g_o_vtsmcu4);
	pr_debug("[cal] g_o_vtsmcu5	 = %d\n", g_o_vtsmcu5);
	pr_debug("[cal] g_o_vtsabb	  = %d\n", g_o_vtsabb);

#if CFG_THERM_LVTS
	pr_debug("[cal] lvts_count1_b30c=  %d\n", lvts_count1_b30c);
	pr_debug("[cal] lvts_count2_b30c=  %d\n", lvts_count2_b30c);
	pr_debug("[cal] lvts_count3_b30c=  %d\n", lvts_count3_b30c);
	pr_debug("[cal] lvts_golden_temp1= %d\n", lvts_golden_temp1);
	pr_debug("[cal] lvts_golden_temp2= %d\n", lvts_golden_temp2);
	pr_debug("[cal] lvts_golden_temp3= %d\n", lvts_golden_temp3);
#endif
}
...
void eDataCorrector(void)
{
	...
	if (g_adc_ge_t < 265 || g_adc_ge_t > 758) {
		pr_debug("[thermal] Bad efuse data, g_adc_ge_t\n");
		g_adc_ge_t = 512;
	}
	if (g_adc_oe_t < 265 || g_adc_oe_t > 758) {
		pr_debug("[thermal] Bad efuse data, g_adc_oe_t\n");
		g_adc_oe_t = 512;
	}
	if (g_o_vtsmcu1 < -8 || g_o_vtsmcu1 > 484) {
		pr_debug("[thermal] Bad efuse data, g_o_vtsmcu1\n");
		g_o_vtsmcu1 = 260;
	}
	if (g_o_vtsmcu2 < -8 || g_o_vtsmcu2 > 484) {
		pr_debug("[thermal] Bad efuse data, g_o_vtsmcu2\n");
		g_o_vtsmcu2 = 260;
	}
	if (g_o_vtsmcu3 < -8 || g_o_vtsmcu3 > 484) {
		pr_debug("[thermal] Bad efuse data, g_o_vtsmcu3\n");
		g_o_vtsmcu3 = 260;
	}
	if (g_o_vtsmcu4 < -8 || g_o_vtsmcu4 > 484) {
		pr_debug("[thermal] Bad efuse data, g_o_vtsmcu4\n");
		g_o_vtsmcu4 = 260;
	}
	if (g_o_vtsmcu5 < -8 || g_o_vtsmcu5 > 484) { /* TODO */
		pr_debug("[thermal] Bad efuse data, g_o_vtsmcu5\n");
		g_o_vtsmcu5 = 260;
	}
	if (g_o_vtsabb < -8 || g_o_vtsabb > 484) {
		pr_debug("[thermal] Bad efuse data, g_o_vtsabb\n");
		g_o_vtsabb = 260;
	}
	if (g_degc_cali < 1 || g_degc_cali > 63) {
		pr_debug("[thermal] Bad efuse data, g_degc_cali\n");
		g_degc_cali = 40;
	}
}
...
void tscpu_thermal_cal_prepare_2(__u32 ret)
{
	...
	/* pr_debug("tscpu_thermal_cal_prepare_2\n"); */
	...
	pr_debug("[T_De][cal] g_ge		 = %d\n", g_ge);
	pr_debug("[T_De][cal] g_gain		= %d\n", g_gain);

	for (i = 0; i < TS_ENUM_MAX; i++)
		pr_debug("[T_De][cal] g_x_roomt%d	= %d\n", i,
			     g_x_roomt[i]);
}
...
{
	/* Ycurr = [(Tcurr -
	 * DEGC_cali/2)*(1528+O_slope*10)/10*(18/15)*(1/10000)
	 * +X_roomtabb]*Gain*4096 + OE
	 */
	...
	/* tscpu_dpr_debug("temperature_to_raw_room\n"); */
...
static __s32 raw_to_temperature_roomt(__u32 ret, enum thermal_sensor ts_name)
{
	...
	/* tscpu_dpr_debug("raw_to_temperature_room,ts_num=%d,xtoomt=%d\n",
	 * ts_name,xtoomt);
	 */
	...
	if (g_o_slope_sign == 0)
		format_4 = ((format_3 * 1000) /
			    (1528 + g_o_slope * 10)); /* uint = 0.1 deg */
	else
		format_4 = ((format_3 * 1000) /
			    (1528 - g_o_slope * 10)); /* uint = 0.1 deg */
	...
	/* tscpu_dpr_debug("raw_to_temperature_room,t_current=%d\n",t_current);
	 */
	return t_current;
}

int get_immediate_none_wrap(void) { return -127000; }

/* chip dependent */
int get_immediate_cpuB_wrap(void) { return -127000; }
...
int (*max_temperature_in_bank[THERMAL_BANK_NUM])(void) = {
	...
	get_immediate_mcucci_wrap, get_immediate_gpu_wrap,
...
int get_immediate_ts6_wrap(void) { return -127000; }

int get_immediate_ts7_wrap(void) { return -127000; }

int get_immediate_ts8_wrap(void) { return -127000; }
...
int (*get_immediate_tsX[TS_ENUM_MAX])(void) = {
	...
	get_immediate_ts4_wrap, get_immediate_ts5_wrap, get_immediate_tsabb_wrap
};
...
static void thermal_interrupt_handler(int tc_num)
{
	...
	mt_reg_sync_writel(
	    ret, offset + TEMPMONINTSTS); /* write to clear interrupt status */
	tscpu_dpr_debug(
...
irqreturn_t tscpu_thermal_all_tc_interrupt_handler(int irq, void *dev_id)
{
	...
	tscpu_dpr_debug("thermal_interrupt_handler : THERMINTST = 0x%x\n", ret);
...
static void thermal_reset_and_initial(int tc_num)
{
	...
	/*bus clock 66M counting unit is 12 * 1/66M * 256 = 12 * 3.879us =
	 * 46.545 us
	 */
	...
	mt_reg_sync_writel(
	    0x00000492,
	    offset + TEMPMSRCTL0); /* temperature sampling control, 2 out of 4
				    * samples
				    */
#elif THERMAL_CONTROLLER_HW_FILTER == 4
	mt_reg_sync_writel(
	    0x000006DB,
	    offset + TEMPMSRCTL0); /* temperature sampling control, 4 out of 6
				    * samples
				    */
#elif THERMAL_CONTROLLER_HW_FILTER == 8
	mt_reg_sync_writel(0x00000924,
			   offset + TEMPMSRCTL0); /* temperature sampling
						   * control, 8 out of 10
						   * samples
						   */
#elif THERMAL_CONTROLLER_HW_FILTER == 16
	mt_reg_sync_writel(0x00000B6D,
			   offset + TEMPMSRCTL0); /* temperature sampling
						   * control, 16 out of 18
						   * samples
						   */
#else /* default 1 */
	mt_reg_sync_writel(
	    0x00000000,
	    offset + TEMPMSRCTL0); /* temperature sampling control, 1 sample */
#endif

	mt_reg_sync_writel(
	    0xFFFFFFFF, offset + TEMPAHBTO); /* exceed this polling time, IRQ
					      * would be inserted
					      */

	mt_reg_sync_writel(
	    0x00000000,
	    offset + TEMPMONIDET0); /* times for interrupt occurrance for SP0*/
	mt_reg_sync_writel(
	    0x00000000,
	    offset + TEMPMONIDET1); /* times for interrupt occurrance for SP1*/
	mt_reg_sync_writel(
	    0x00000000,
	    offset + TEMPMONIDET2); /* times for interrupt occurrance for SP2*/

	mt_reg_sync_writel(0x800, offset + TEMPADCMUX);
	mt_reg_sync_writel(
	    (__u32)AUXADC_CON1_CLR_P,
	    offset +
		TEMPADCMUXADDR); /* AHB address for auxadc mux selection */

	mt_reg_sync_writel(
	    0x800, offset + TEMPADCEN); /* AHB value for auxadc enable */
	...
	mt_reg_sync_writel(
	    (__u32)AUXADC_DAT11_P,
	    offset + TEMPADCVALIDADDR); /* AHB address for auxadc valid bit */
	mt_reg_sync_writel(
	    (__u32)AUXADC_DAT11_P,
	    offset +
		TEMPADCVOLTADDR); /* AHB address for auxadc voltage output */

	mt_reg_sync_writel(0x0, offset + TEMPRDCTRL); /* read valid & voltage
						       * are at the same
						       * register
						       */
	...
	mt_reg_sync_writel(
	    0x0, offset + TEMPADCVOLTAGESHIFT); /* do not need to shift */
}
				      ...
				      int tc_num)
{
	...
	mt_reg_sync_writel(temp & 0x00000000,
			   offset +
			       TEMPMONINT); /* disable trigger SPM interrupt */

	mt_reg_sync_writel(
	    0x20000,
	    offset + TEMPPROTCTL); /* set hot to wakeup event control */

	mt_reg_sync_writel(
	    raw_high, offset + TEMPPROTTC); /* set hot to HOT wakeup event */

	mt_reg_sync_writel(
	    temp | 0x80000000,
	    offset + TEMPMONINT); /* enable trigger Hot SPM interrupt */
}
...
static int read_tc_raw_and_temp(u32 *tempmsr_name, enum thermal_sensor ts_name)
{
	...
	tscpu_dpr_debug("read_tc_raw_temp,ts_raw=%d,temp=%d\n",
			raw, temp * 100);
...
void tscpu_thermal_read_tc_temp(int tc_num, enum thermal_sensor type,
				int order)
{
	__u32 offset;

	tscpu_dpr_debug("%s tc_num %d type %d order %d\n", __func__, tc_num,
		      type, order);
	offset = tscpu_g_tc[tc_num].tc_offset;
	...
	case 0:
		...
		tscpu_dpr_debug("%s order %d tc_num %d type %d temp %d\n",
...
int tscpu_thermal_fast_init(int tc_num)
{
	...
	mt_reg_sync_writel((0x00001000 + temp),
			   PTPSPARE2); /* write temp to spare register */

	mt_reg_sync_writel(
	    1,
	    offset + TEMPMONCTL1); /* counting unit is 320 * 31.25us = 10ms */
	mt_reg_sync_writel(
	    1, offset +
		   TEMPMONCTL2); /* sensing interval is 200 * 10ms = 2000ms */
	mt_reg_sync_writel(1, offset + TEMPAHBPOLL); /* polling interval to
						      * check if temperature
						      * sense is ready
						      */

	mt_reg_sync_writel(
	    0x000000FF, offset + TEMPAHBTO); /* exceed this polling time, IRQ
					      * would be inserted
					      */
	mt_reg_sync_writel(
	    0x00000000,
	    offset + TEMPMONIDET0); /* times for interrupt occurrance */
	mt_reg_sync_writel(
	    0x00000000,
	    offset + TEMPMONIDET1); /* times for interrupt occurrance */

	mt_reg_sync_writel(
	    0x0000000,
	    offset +
		TEMPMSRCTL0); /* temperature measurement sampling control */
	...
	mt_reg_sync_writel(
	    (__u32)PTPSPARE0_P,
	    offset +
		TEMPPNPMUXADDR); /* AHB address for pnp sensor mux selection */
	mt_reg_sync_writel(
	    (__u32)PTPSPARE0_P,
	    offset +
		TEMPADCMUXADDR); /* AHB address for auxadc mux selection */
	mt_reg_sync_writel(
	    (__u32)PTPSPARE1_P,
	    offset + TEMPADCENADDR); /* AHB address for auxadc enable */
	mt_reg_sync_writel(
	    (__u32)PTPSPARE2_P,
	    offset + TEMPADCVALIDADDR); /* AHB address for auxadc valid bit */
	mt_reg_sync_writel(
	...
	mt_reg_sync_writel(
	    0x0, offset + TEMPADCVOLTAGESHIFT); /* do not need to shift */
	mt_reg_sync_writel(
	    0x3, offset + TEMPADCWRITECTRL); /* enable auxadc mux & pnp write
					      * transaction
					      */
	...
	mt_reg_sync_writel(0x0000000F, offset + TEMPMONCTL0); /* enable all
							       * sensing
							       * point(sensing
							       * point 2 is
							       * unused)
...
static void tscpu_thermal_tempADCPNP(int tc_num, int adc, int order)
{
	...
	tscpu_dpr_debug("%s adc %x, order %d\n", __func__, adc, order);
...
void tscpu_thermal_initial_all_tc(void)
{
	...
	mt_reg_sync_writel(
	    temp,
	    AUXADC_CON0_V); /* disable auxadc channel 11 synchronous mode */
	mt_reg_sync_writel(
	    0x800,
	    AUXADC_CON1_CLR_V); /* disable auxadc channel 11 immediate mode */
	...
	for (i = 0; i < ARRAY_SIZE(tscpu_g_tc); i++) {
		...
		for (j = 0; j < tscpu_g_tc[i].ts_number; j++)
			tscpu_thermal_tempADCPNP(
			    i,
			    tscpu_thermal_ADCValueOfMcu(tscpu_g_tc[i].ts[j]),
			    j);
...
void tscpu_config_all_tc_hw_protect(int temperature, int temperature2)
{
	...
	if (wd_api_ret >= 0) {
		wd_api->wd_thermal_direct_mode_config(
		    WD_REQ_DIS, WD_REQ_RST_MODE); /* reset mode */
	} else {
	...
	for (i = 0; i < ARRAY_SIZE(tscpu_g_tc); i++) {
		...
		set_tc_trigger_hw_protect(
		    ...
		    i); /* Move thermal HW protection ahead... */
	}
	...
	if (wd_api_ret >= 0) {
		wd_api->wd_thermal_direct_mode_config(
		    WD_REQ_EN, WD_REQ_RST_MODE); /* reset mode */
	} else {
		pr_debug("%d FAILED TO GET WD API\n", __LINE__);
		WARN_ON_ONCE(1);
	}
...
void tscpu_reset_thermal(void)
{
	...
	/* reset thremal ctrl */
	temp =
	    readl(INFRA_GLOBALCON_RST_0_SET); /* TODO: Is it necessary to read
					       * INFRA_GLOBALCON_RST_0_SET?
					       */
	...
	/* un reset */
	temp =
	    readl(INFRA_GLOBALCON_RST_0_CLR); /* TODO: Is it necessary to read
					       * INFRA_GLOBALCON_RST_0_CLR?
					       */
	temp |=
	    0x00000001; /* 1: Enable reset Disables thermal control software
			 * reset
			 */
...
int tscpu_get_curr_temp(void)
{
	 ...
	 */
	tscpu_curr_cpu_temp =
	    MAX(tscpu_ts_temp[TS_MCU1], tscpu_ts_temp[TS_MCU2]);
 ...
 */
int tscpu_get_curr_max_ts_temp(void) { return tscpu_curr_max_ts_temp; }
...
int get_io_reg_base(void)
{
	...
	if (of_property_read_u32_index(node, "reg", 1, &thermal_phy_base)) {
		pr_debug("[THERM_CTRL] config error thermal_phy_base\n");
		return 0;
	}
	...
	if (of_property_read_u32_index(node, "reg", 1, &auxadc_ts_phy_base)) {
		pr_debug("[THERM_CTRL] config error auxadc_ts_phy_base\n");
		return 0;
	}
	...
	if (of_property_read_u32_index(node, "reg", 1, &apmixed_phy_base)) {
		pr_debug("[THERM_CTRL] config error apmixed_phy_base=\n");
		return 0;
	}
...
{
	sprintf((prefix + offset), "L %d LL %d", get_immediate_cpuL_wrap(),
