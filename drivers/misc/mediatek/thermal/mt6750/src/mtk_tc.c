...
const struct of_device_id mt_thermal_of_match[2] = {
	{.compatible = "mediatek,mt6750-therm_ctrl",},
	{},
};
#endif
...
#if defined(CONFIG_ARCH_MT6750)/*remove Tj=100C shutdown*/
...
int get_io_reg_base(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6750-therm_ctrl");
	BUG_ON(node == 0);
	if (node) {
		/* Setup IO addresses */
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6750-auxadc");
	BUG_ON(node == 0);
	if (node) {
		/* Setup IO addresses */
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6750-infrasys");
	if (!node) {
...
void thermal_get_AHB_clk_info(void)
{
	...
	/*The following rule may change, you need to confirm with DE.
	  These are for mt6750, and confirmed with DE Justin Gu */
