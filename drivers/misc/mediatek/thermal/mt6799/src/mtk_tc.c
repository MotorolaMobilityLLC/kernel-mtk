...
struct thermal_controller tscpu_g_tc[THERMAL_CONTROLLER_NUM] = {
	...
	[2] = {
		.ts = {TS_MCU1, TS_MCU3, TS_MCU2, TS_ABB},
		.ts_number = 4,
		.tc_offset = 0x300,
		.tc_speed = {
			0x0000000C,
...
void get_thermal_slope_intercept(struct TS_PTPOD *ts_info, thermal_bank_name ts_bank)
{
	...
	*/
	...
	case THERMAL_BANK4:
		...
		break;
	case THERMAL_BANK5:
		/* there is no bank5. choose high temp*/
		x_roomt = g_x_roomt[6];
		break;
	case THERMAL_BANK6:
		x_roomt = g_x_roomt[0];
	...
	*   Don't have to change until using next generation thermal sensors.
	...
	if (g_o_slope_sign == 0)
		temp1 = (temp0 * 10) / (1612 + g_o_slope * 10);
	else
		temp1 = (temp0 * 10) / (1612 - g_o_slope * 10);
	...
	if (g_o_slope_sign == 0)
		temp2 = temp1 * 100 / (1612 + g_o_slope * 10);
	else
		temp2 = temp1 * 100 / (1612 - g_o_slope * 10);
...
void eDataCorrector(void)
{
	...
	*   O_VTSMCU4(9b)      Default:260    - 8 ~484
	*   O_VTSMCU5(9b)      Default:260    - 8 ~484
	*   O_VTSMCU6(9b)      Default:260    - 8 ~484
	*   O_VTSMCU7(9b)      Default:260    - 8 ~484
	*   O_VTSMCU8(9b)      Default:260    - 8 ~484
	*   O_VTSMCU9(9b)      Default:260    - 8 ~484
	...
	*/
	...
	if (g_o_vtsmcu9 < -8 || g_o_vtsmcu9 > 484) {
		tscpu_warn("[thermal] Bad efuse data, g_o_vtsmcu9\n");
		g_o_vtsmcu9 = 260;
	}
...
void tscpu_thermal_cal_prepare(void)
{
	...
	*ADDRESS_INDEX_3	  0x11F101AC
	*ADDRESS_INDEX_4	  0x11F101B0
	*/
	...
	temp4 = get_devinfo_with_index(ADDRESS_INDEX_4);
	...
	*   O_VTSMCU6    (9b) *(0x11F101AC)[31:23]
	*   O_VTSMCU7    (9b) *(0x11F101AC)[22:14]
	*   O_VTSMCU8    (9b) *(0x11F101AC)[13:5]
	*   O_VTSMCU9    (9b) *(0x11F101B0)[31:23]
...
void tscpu_thermal_cal_prepare_2(__u32 ret)
{
	...
	if (g_ts_offset == 0) {
		...
		format[1] = (g_o_vtsmcu2 + 3150 - g_oe);
		format[2] = (g_o_vtsmcu3 + 3150 - g_oe);
		format[3] = (g_o_vtsmcu4 + 3150 - g_oe);
	...
	for (i = 1 ; i < TS_ENUM_MAX; i++) {
		...
		if (count == (THERMAL_SLOT_NUM - 2)) {
			while (thermal_to_sspm(THERMAL_IPI_INIT_GRP3 + offset, &thermal_data) != 0)
				udelay(100);
			offset++;
		}
...
static __s32 temperature_to_raw_room(__u32 ret, ts_e ts_name)
{
	...
	if (g_o_slope_sign == 0) {	/* O_SLOPE is Positive. */
		...
		format_2 = format_1 * (1612 + g_o_slope * 10) / 10 * 18 / 15;
...
static __s32 raw_to_temperature_roomt(__u32 ret, ts_e ts_name)
{
	...
	if (g_o_slope_sign == 0)
		format_4 = ((format_3 * 1000) / (1612 + g_o_slope * 10));	/* uint = 0.1 deg */
	else
		format_4 = ((format_3 * 1000) / (1612 - g_o_slope * 10));	/* uint = 0.1 deg */
...
int get_immediate_mcusys_wrap(void)
{
	int curr_temp;

	curr_temp = MAX(tscpu_ts_temp[TS_MCU6], tscpu_ts_temp[TS_MCU7]);
	curr_temp = MAX(curr_temp, tscpu_ts_temp[TS_MCU8]);
...
int get_io_reg_base(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6799-therm_ctrl");
	WARN_ON_ONCE(node == 0);
	if (node) {
		/* Setup IO addresses */
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6799-infrasys"); /**/
	if (!node) {
...
void thermal_get_AHB_clk_info(void)
{
	...
	*  These are for mt6799, and confirmed with DE Justin Gu
	*/
...
{
	sprintf((prefix + offset), "B %d L %d LL %d",
		get_immediate_cpuB_wrap(), get_immediate_cpuL_wrap(), get_immediate_cpuLL_wrap());
