...
struct bank_t tscpu_g_bank[THERMAL_BANK_NUM] = {
	[0] = {.ts = {TS_FILL(TS_MCU1)}, .ts_number = 1},
	[1] = {.ts = {TS_FILL(TS_MCU2)}, .ts_number = 1},
	[2] = {.ts = {TS_FILL(TS_MCU1), TS_FILL(TS_MCU2)}, .ts_number = 2},
	[3] = {.ts = {TS_FILL(TS_MCU3), TS_FILL(TS_MCU4), TS_FILL(TS_MCU5)},
	       .ts_number = 3},
};
...
/*=============================================================*/
void thermal_set_big_core_speed(__u32 tempMonCtl1, __u32 tempMonCtl2,
				__u32 tempAhbPoll)
{
				 ...
				 enum thermal_bank_name ts_bank)
{
	 ...
	 *   the dominant sensor. You can observe it in the thermal doc provided
	 * by Thermal DE.
	 *   For example,
	 *   Bank 1 is for SOC + GPU. Observe all scenarios related to GPU tests
	 * to
...
{
	__s32 format_1 = 0, format_2 = 0, format_3 = 0, format_4 = 0,
	      format_5 = 0, format_6 = 0;
	...
	g_x_roomt[2] = (((format_3 * 10000) / 4096) * 10000) /
		       g_gain; /* x_roomt * 10000 */
	g_x_roomt[3] = (((format_4 * 10000) / 4096) * 10000) /
		       g_gain; /* x_roomt * 10000 */
	g_x_roomt[4] = (((format_5 * 10000) / 4096) * 10000) /
		       g_gain; /* x_roomt * 10000 */
	g_x_roomt[5] = (((format_6 * 10000) / 4096) * 10000) /
		       g_gain; /* x_roomt * 10000 */
...
{
	/* Ycurr = [(Tcurr -
	 * DEGC_cali/2)*(1528+O_slope*10)/10*(18/15)*(1/10000)+X_roomtabb]*Gain*4096
	 * + OE
	 */
...
int get_immediate_gpu_wrap(void)
{
	...
	curr_temp = MAX(tscpu_bank_ts[THERMAL_BANK3][TS_MCU3],
			tscpu_bank_ts[THERMAL_BANK3][TS_MCU4]);
...
int get_immediate_mcucci_wrap(void)
{
	...
	curr_temp = MAX(tscpu_bank_ts[THERMAL_BANK2][TS_MCU1],
			tscpu_bank_ts[THERMAL_BANK2][TS_MCU2]);
	tscpu_dprintk("%s curr_temp=%d\n", __func__, curr_temp);
...
int get_immediate_ts1_wrap(void)
{
	...
	curr_temp = MAX(tscpu_bank_ts[THERMAL_BANK0][TS_MCU1],
			tscpu_bank_ts[THERMAL_BANK2][TS_MCU1]);
	tscpu_dprintk("%s curr_temp=%d\n", __func__, curr_temp);
...
int get_immediate_ts2_wrap(void)
{
	...
	curr_temp = MAX(tscpu_bank_ts[THERMAL_BANK1][TS_MCU2],
			tscpu_bank_ts[THERMAL_BANK2][TS_MCU2]);
	tscpu_dprintk("%s curr_temp=%d\n", __func__, curr_temp);
...
int (*get_immediate_tsX[TS_ENUM_MAX])(void) = {
    ...
    get_immediate_ts4_wrap, get_immediate_ts5_wrap, get_immediate_none_wrap};
...
static void thermal_interrupt_handler(int bank)
{
	...
	tscpu_dprintk(
	    "[tIRQ] %s,bank=0x%08x,ret=0x%08x\n", __func__, bank,
	    ret);

#if THERMAL_INFORM_OTP
	temp = get_immediate_big_wrap();
	tscpu_dprintk("[tIRQ] %s,BIG T=%d\n", __func__, temp);
	...
	if (ret & THERMAL_MON_CINTSTS0)
		tscpu_dprintk("thermal_isr: thermal sensor point 0 - cold "
			      "interrupt trigger\n");

	if (ret & THERMAL_MON_HINTSTS0)
		tscpu_dprintk("<<<thermal_isr>>>: thermal sensor point 0 - hot "
			      "interrupt trigger\n");

	if (ret & THERMAL_MON_HINTSTS1)
		tscpu_dprintk("<<<thermal_isr>>>: thermal sensor point 1 - hot "
			      "interrupt trigger\n");

	if (ret & THERMAL_MON_HINTSTS2)
		tscpu_dprintk("<<<thermal_isr>>>: thermal sensor point 2 - hot "
			      "interrupt trigger\n");
...
static void thermal_reset_and_initial(void)
{
	...
	mt_reg_sync_writel(0x0000030D,
			   TEMPAHBPOLL); /* poll is set to 11.833us */

#if THERMAL_CONTROLLER_HW_FILTER == 2
	mt_reg_sync_writel(
	    0x00000492,
	    TEMPMSRCTL0); /* temperature sampling control, 2 out of 4 samples */
#elif THERMAL_CONTROLLER_HW_FILTER == 4
	mt_reg_sync_writel(
	    0x000006DB,
	    TEMPMSRCTL0); /* temperature sampling control, 4 out of 6 samples */
#elif THERMAL_CONTROLLER_HW_FILTER == 8
	mt_reg_sync_writel(0x00000924, TEMPMSRCTL0);   /* temperature sampling
							*control, 8 out of 10
							*samples
							*/
#elif THERMAL_CONTROLLER_HW_FILTER == 16

  mt_reg_sync_writel(0x00000B6D, TEMPMSRCTL0); /* temperature sampling
	...
	mt_reg_sync_writel(
	    0x00000000,
	    TEMPMSRCTL0); /* temperature sampling control, 1 sample */
#endif
	/* polling time of a bank: 1 sensors 20.155ms, 2 sensors 40.311ms, 3
	 * sensors  60.465ms */

	mt_reg_sync_writel(
	    0xFFFFFFFF,
	    TEMPAHBTO); /* exceed this polling time, IRQ would be inserted */

	mt_reg_sync_writel(0x00000000,
	...
	/* mt_reg_sync_writel(0x1100100C, TEMPADCMUXADDR);// AHB address for
	...
	mt_reg_sync_writel(
	    ...
	    TEMPADCVALIDADDR); /* AHB address for auxadc valid bit */
	mt_reg_sync_writel(
	    (__u32)AUXADC_DAT11_P,
	    TEMPADCVOLTADDR); /* AHB address for auxadc voltage output */
	/* mt_reg_sync_writel(0x11001040, TEMPADCVALIDADDR); // AHB address for
	 * auxadc valid bit */
	/* mt_reg_sync_writel(0x11001040, TEMPADCVOLTADDR);  // AHB address for
	 * auxadc voltage output */

	mt_reg_sync_writel(
	    0x0,
	    TEMPRDCTRL); /* read valid & voltage are at the same register */
	...
	/* mt_reg_sync_writel(0x2, TEMPADCWRITECTRL);                     //
				      ...
				      enum thermal_bank_name bank)
{
	...
	mt_reg_sync_writel(0x20000,
			   TEMPPROTCTL); /* set hot to wakeup event control */

	mt_reg_sync_writel(raw_high,
			   TEMPPROTTC); /* set hot to HOT wakeup event */

	mt_reg_sync_writel(temp | 0x80000000,
			   TEMPMONINT); /* enable trigger Hot SPM interrupt */
}
...
void tscpu_thermal_read_bank_temp(enum thermal_bank_name bank,
				  enum thermal_sensor type, int order)
{
	...
	switch (order) {
	case 0:
		tscpu_bank_ts[bank][type] = read_tc_raw_and_temp(
		    TEMPMSR0, type, &tscpu_bank_ts_r[bank][type]);
		tscpu_dprintk("%s order %d bank %d type %d tscpu_bank_ts %d "
			      "tscpu_bank_ts_r %d\n",
			      __func__, order, bank, type,
			      tscpu_bank_ts[bank][type],
			      tscpu_bank_ts_r[bank][type]);
		break;
	case 1:
		tscpu_bank_ts[bank][type] = read_tc_raw_and_temp(
		    TEMPMSR1, type, &tscpu_bank_ts_r[bank][type]);
		tscpu_dprintk("%s order %d bank %d type %d tscpu_bank_ts %d "
			      "tscpu_bank_ts_r %d\n",
			      __func__, order, bank, type,
			      tscpu_bank_ts[bank][type],
	...
	case 2:
		tscpu_bank_ts[bank][type] = read_tc_raw_and_temp(
		    TEMPMSR2, type, &tscpu_bank_ts_r[bank][type]);
		tscpu_dprintk("%s order %d bank %d type %d tscpu_bank_ts %d "
			      "tscpu_bank_ts_r %d\n",
			      __func__, order, bank, type,
			      tscpu_bank_ts[bank][type],
	...
	case 3:
		tscpu_bank_ts[bank][type] = read_tc_raw_and_temp(
		    TEMPMSR3, type, &tscpu_bank_ts_r[bank][type]);
		tscpu_dprintk("%s order %d bank %d type %d tscpu_bank_ts %d "
			      "tscpu_bank_ts_r %d\n",
			      __func__, order, bank, type,
			      tscpu_bank_ts[bank][type],
...
int tscpu_thermal_fast_init(void)
{
	...
	mt_reg_sync_writel(
	    1, TEMPMONCTL1); /* counting unit is 320 * 31.25us = 10ms */
	mt_reg_sync_writel(
	    1, TEMPMONCTL2); /* sensing interval is 200 * 10ms = 2000ms */
	mt_reg_sync_writel(1, TEMPAHBPOLL);
  /* polling interval to check if temperature sense is ready */
	...
	mt_reg_sync_writel(
	    (__u32)PTPSPARE0_P,
	    TEMPADCMUXADDR); /* AHB address for auxadc mux selection */
	mt_reg_sync_writel((__u32)PTPSPARE1_P,
			   TEMPADCENADDR); /* AHB address for auxadc enable */
	mt_reg_sync_writel(
	    (__u32)PTPSPARE2_P,
	    TEMPADCVALIDADDR); /* AHB address for auxadc valid bit */
	mt_reg_sync_writel(
	...
	mt_reg_sync_writel(
	    0x3,
	    TEMPADCWRITECTRL); /* enable auxadc mux & pnp write transaction */
	...
	mt_reg_sync_writel(0x0000000F,
			   TEMPMONCTL0); /* enable all sensing point (sensing point 2 is unused) */
...
void tscpu_config_tc_sw_protect(int highoffset, int lowoffset)
{
	...
	tscpu_warn("[tIRQ] "
		   "%s,highoffset=%d,lowoffset=%d,bank="
		   "%d,ts_name=%d\n", __func__,
	...
	if (lowoffset != 0) {
		...
		temp = readl(TEMPMONINT);
		mt_reg_sync_writel(temp | 0x00000004,
				   TEMPMONINT); /* set low offset */
	}
	...
	if (highoffset != 0) {
		...
		temp = readl(TEMPMONINT);
		mt_reg_sync_writel(temp | 0x00000008,
				   TEMPMONINT); /* set high offset */
	}
...
int tscpu_get_curr_temp(void)
{
	...
	/* for MT6797 CPU */
	tscpu_curr_cpu_temp =
	    MAX(get_immediate_ts1_wrap(), get_immediate_ts2_wrap());
	tscpu_curr_gpu_temp =
	    MAX(MAX(get_immediate_ts3_wrap(), get_immediate_ts4_wrap()),
...
int get_io_reg_base(void)
{
	...
	node =
	    of_find_compatible_node(NULL, NULL, "mediatek,mt6757-therm_ctrl");
	WARN_ON_ONCE(node == 0);
	if (node) {
		/* Setup IO addresses */
	...
	if (of_property_read_u32_index(node, "reg", 1, &thermal_phy_base)) {
		/*tscpu_printk("[THERM_CTRL] thermal_base
		 * thermal_phy_base=0x%x\n",thermal_phy_base);
		 */
		return 0;
	}
