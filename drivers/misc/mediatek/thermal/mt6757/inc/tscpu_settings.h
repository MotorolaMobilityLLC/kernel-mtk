...
#define THERMAL_GET_PTIME()                                                    \
	{                                                                      \
		thermal_pTime_us = thermal_get_current_time_us()               \
	}

#define THERMAL_GET_CTIME()                                                    \
	{                                                                      \
		thermal_cTime_us = thermal_get_current_time_us()               \
	}
...
/* Thermal controller HW filtering function. Only 1, 2, 4, 8, 16 are valid
*values,
...
/*	Define this in tscpu_settings.h enables this feature. It polls CPU TS in
*hrtimer and
...
#define TS_TURN_ON                                                             \
	0xFFFFFFCF /* turn on TS_CON1[5:4] 2'b 00  11001111 -> 0xCF  ~(0x30)*/
...
#define thermal_setl(addr, val)                                                \
	mt_reg_sync_writel(readl(addr) | (val), ((void *)addr))
...
#define tscpu_warn(fmt, args...) pr_debug(TSCPU_LOG_TAG fmt, ##args)
...
extern void tscpu_thermal_enable_all_periodoc_sensing_point(
...
extern void tscpu_thermal_read_bank_temp(enum thermal_bank_name bank,
					 enum thermal_sensor type, int order);
extern void tscpu_thermal_cal_prepare(void);
extern void tscpu_thermal_cal_prepare_2(unsigned int ret);
extern irqreturn_t tscpu_thermal_all_bank_interrupt_handler(int irq,
							    void *dev_id);
...
#define INFRA_GLOBALCON_RST_0_SET                                              \
	(INFRACFG_AO_BASE_2 + 0x120) /*yes, 0x10001000*/
#define INFRA_GLOBALCON_RST_0_CLR                                              \
	(INFRACFG_AO_BASE_2 + 0x124) /*yes, 0x10001000*/
#define INFRA_GLOBALCON_RST_0_STA                                              \
	(INFRACFG_AO_BASE_2 + 0x128) /*yes, 0x10001000*/
/*******************************************************************************
