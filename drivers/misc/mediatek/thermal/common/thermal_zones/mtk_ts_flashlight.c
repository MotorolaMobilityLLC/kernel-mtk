	...
	if (Res >= FLASHLIGHT_Temperature_Table[0].TemperatureR) {
		...
		for (i = 0; i < asize; i++) {
			if (Res >= FLASHLIGHT_Temperature_Table[i].TemperatureR) {
				RES2 = FLASHLIGHT_Temperature_Table[i]
					.TemperatureR;

				TMP2 = FLASHLIGHT_Temperature_Table[i]
					.FLASHLIGHT_Temp;
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	struct flashlight_data *ptr_flashlight_data = kmalloc(
					sizeof(*ptr_flashlight_data), GFP_KERNEL);

	if (ptr_flashlight_data == NULL)
		return -ENOMEM;

	len = (count < (sizeof(ptr_flashlight_data->desc) - 1)) ?
				count : (sizeof(ptr_flashlight_data->desc) - 1);

	if (copy_from_user(ptr_flashlight_data->desc, buffer, len)) {
		kfree(ptr_flashlight_data);
		return 0;
	}

	ptr_flashlight_data->desc[len] = '\0';

	if (sscanf
	    (ptr_flashlight_data->desc,
	     "%d %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d",
		&num_trip,
		&ptr_flashlight_data->trip[0], &ptr_flashlight_data->t_type[0],
		ptr_flashlight_data->bind0,
		&ptr_flashlight_data->trip[1], &ptr_flashlight_data->t_type[1],
		ptr_flashlight_data->bind1,
		&ptr_flashlight_data->trip[2], &ptr_flashlight_data->t_type[2],
		ptr_flashlight_data->bind2,
		&ptr_flashlight_data->trip[3], &ptr_flashlight_data->t_type[3],
		ptr_flashlight_data->bind3,
		&ptr_flashlight_data->trip[4], &ptr_flashlight_data->t_type[4],
		ptr_flashlight_data->bind4,
		&ptr_flashlight_data->trip[5], &ptr_flashlight_data->t_type[5],
		ptr_flashlight_data->bind5,
		&ptr_flashlight_data->trip[6], &ptr_flashlight_data->t_type[6],
		ptr_flashlight_data->bind6,
		&ptr_flashlight_data->trip[7], &ptr_flashlight_data->t_type[7],
		ptr_flashlight_data->bind7,
		&ptr_flashlight_data->trip[8], &ptr_flashlight_data->t_type[8],
		ptr_flashlight_data->bind8,
		&ptr_flashlight_data->trip[9], &ptr_flashlight_data->t_type[9],
		ptr_flashlight_data->bind9,
		&ptr_flashlight_data->time_msec) == 32) {
		...
		for (i = 0; i < num_trip; i++)
			g_THERMAL_TRIP[i] = ptr_flashlight_data->t_type[i];
		...
		for (i = 0; i < 20; i++) {
			g_bind0[i] = ptr_flashlight_data->bind0[i];
			g_bind1[i] = ptr_flashlight_data->bind1[i];
			g_bind2[i] = ptr_flashlight_data->bind2[i];
			g_bind3[i] = ptr_flashlight_data->bind3[i];
			g_bind4[i] = ptr_flashlight_data->bind4[i];
			g_bind5[i] = ptr_flashlight_data->bind5[i];
			g_bind6[i] = ptr_flashlight_data->bind6[i];
			g_bind7[i] = ptr_flashlight_data->bind7[i];
			g_bind8[i] = ptr_flashlight_data->bind8[i];
			g_bind9[i] = ptr_flashlight_data->bind9[i];
		}
		...
		for (i = 0; i < num_trip; i++)
			trip_temp[i] = ptr_flashlight_data->trip[i];

		interval = ptr_flashlight_data->time_msec / 1000;
 ...
 *				"mtktsflashlighttery-sysrst", NULL,
