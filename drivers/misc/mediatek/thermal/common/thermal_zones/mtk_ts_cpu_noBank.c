...
(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(ptr_mtktscpu_data->desc,
		...
		tscpu_dprintk(
			"tscpu_write trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,",
...
static void tscpu_create_fs(void)
{
	...
	if (!mtktscpu_dir) {
		...
		entry = proc_create("tzcpu_set_temperature", 0644,
				mtktscpu_dir, &mtktscpu_set_temperature_fops);
