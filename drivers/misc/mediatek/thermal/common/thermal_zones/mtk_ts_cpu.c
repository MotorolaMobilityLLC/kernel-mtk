...
static DEFINE_MUTEX(ts_cpu_timer_lock);
...
{
	if (mutex_trylock(&ts_cpu_timer_lock)) {
#ifdef FAST_RESPONSE_ATM
		...
		tscpu_dprintk("[tscpu_workqueue_cancel_timer] trylock fail\n");
	}
}
