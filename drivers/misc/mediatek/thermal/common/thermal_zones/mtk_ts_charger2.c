...
#ifdef  CONFIG_MTK_CHARGER2_THERMAL
#include <tmp_bts.h>
#endif
 ...
 */
 #ifndef  CONFIG_MTK_CHARGER2_THERMAL
...
static __s16 mtkts_charger_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
	...
	/* mtktscharger2_dprintk("mtkts_charger_thermistor_conver_temp() : asize = %d, Res = %d\n",asize,Res); */
	if (Res >=CHARGER_Temperature_Table[0].TemperatureR) {
		...
		/* mtktscharger2_dprintk("%d : RES1 = %d,TMP1 = %d\n",__LINE__,RES1,TMP1); */
...
static __s16 mtk_ts_charger_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__s32 dwVCriAP = 0;
...
static int get_hw_charger_temp(void)
{
	...
	if (IMM_IsAdcInitReady() == 0) {
		mtktscharger2_dprintk("[thermal_auxadc_get_data]: AUXADC is not ready\n");
		return 0;
	}
	...
	mtktscharger2_dprintk("APtery output mV = %d\n", ret);
	output = mtk_ts_charger_volt_to_temp(ret);
	mtktscharger2_dprintk("charger output temperature = %d\n", output);
	return output;
}

static int mtktscharger_get_hw_charger_temp(void)
{
	int t_ret = 0;
...
#define DELTA_TEMP 2000
	...
	if ((t_ret > 100000) || (t_ret < -30000)) {
		mtktscharger2_dprintk("[Power/CHARGER_Thermal] drop this data\n");
		t_ret = pre_temp1;
	} else if ((pre_temp1 != 0)
		   && (((pre_temp1 - t_ret) >= DELTA_TEMP)
		       || ((t_ret - pre_temp1) >= DELTA_TEMP))) {
		mtktscharger2_dprintk
		    ("[Power/CHARGER_Thermal] t_ret=%d,(t_ret-pre_temp1)=%d,(pre_temp1-t_ret)=%d\n",
		     t_ret, (t_ret - pre_temp1), (pre_temp1 - t_ret));
		t_ret = pre_temp1;
	} else {
		/* update previous temp */
		pre_temp1 = t_ret;
		mtktscharger2_dprintk("[Power/CHARGER_Thermal] pre_temp1=%d\n", pre_temp1);
	}
#endif
...
{
#ifdef  CONFIG_MTK_CHARGER2_THERMAL
	*t = mtktscharger_get_hw_charger_temp();
#else
