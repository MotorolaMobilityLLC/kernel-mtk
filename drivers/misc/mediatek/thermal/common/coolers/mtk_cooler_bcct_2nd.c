...
#ifdef CONFIG_MTK_THERMAL_PN
	#define FAST_CHARGER_DEVICE_NAME "usbqc"
#else
	#define FAST_CHARGER_DEVICE_NAME "mtk-slave-charger"
#endif
...
static void x_chrlmt_set_limit_handler(struct work_struct *work)
{
	...
	if (chg_psy == NULL)
		chg_psy = power_supply_get_by_name(FAST_CHARGER_DEVICE_NAME);
	if (chg_psy == NULL || IS_ERR(chg_psy)) {
		mtk_cooler_bcct_2nd_dprintk_always("%s Couldn't get %s\n",
			__func__,FAST_CHARGER_DEVICE_NAME);
		return;
	}
