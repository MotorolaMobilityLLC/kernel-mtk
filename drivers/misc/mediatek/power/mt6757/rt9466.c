...
static int rt9466_hw_init(struct rt9466_info *info)
{
	int ret = 0;
...
/* This function should only used in shutdown function */
...
static int rt_charger_enable_hz(struct mtk_charger_info *mchr_info, void *data)
{
	...
	ret = (enable ? rt9466_set_bit : rt9466_clr_bit)
		(info, RT9466_REG_CHG_CTRL1, RT9466_MASK_CHG_EN);
	...
	const struct i2c_device_id *dev_id)
{
	...
	/* Reset chip */
	ret = rt9466_hw_init(info);
	if (ret < 0)
		battery_log(BAT_LOG_CRTI,
			"%s: set register to default value failed\n", __func__);

	ret = rt9466_sw_init(info);
	if (ret < 0) {
