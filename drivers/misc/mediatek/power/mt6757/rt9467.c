...
#define RT9467_DRV_VERSION "1.0.0_MTK"
...
static const char *rt9467_irq_name[RT9467_IRQIDX_MAX] = {
	"CHG_STATC", "FAULT", "TS_STATC", "IRQ1", "IRQ2", "IRQ3", "DPDM"
};

/* Not to unmask the following IRQs */
static const u8 rt9467_irq_init_data[] = {
	RT9467_MASK_PWR_RDYM | RT9467_MASK_CHG_MIVRM | RT9467_MASK_CHG_AICRM,
	RT9467_MASK_VBUSOVM,
	RT9467_MASK_TS_BAT_HOTM | RT9467_MASK_TS_BAT_WARMM |
	RT9467_MASK_TS_BAT_COOLM | RT9467_MASK_TS_BAT_COLDM |
	RT9467_MASK_TS_STATC_RESERVED,
	RT9467_MASK_CHG_OTPM | RT9467_MASK_CHG_RVPM | RT9467_MASK_CHG_ADPBADM |
	RT9467_MASK_CHG_STATCM | RT9467_MASK_CHG_FAULTM | RT9467_MASK_TS_STATCM,
	RT9467_MASK_IEOCM | RT9467_MASK_TERMM | RT9467_MASK_SSFINISHM |
	RT9467_MASK_CHG_AICLMEASM | RT9467_MASK_IRQ2_RESERVED,
	RT9467_MASK_BST_BATUVM | RT9467_MASK_PUMPX_DONEM |
	RT9467_MASK_ADC_DONEM | RT9467_MASK_IRQ3_RESERVED,
	RT9467_MASK_DCDTM | RT9467_MASK_CHGDETM | RT9467_MASK_HVDCP_DETM |
	RT9467_MASK_DETACHM | RT9467_MASK_ATTACHM |
	RT9467_MASK_DPDMIRQ_RESERVED,
};
...
enum rt9467_usb_status {
	RT9467_USB_STAT_NO_VBUS = 0,
	RT9467_USB_STAT_DETECTING,
	RT9467_USB_STAT_SDP,
	RT9467_USB_STAT_SDP_NSTD,
	RT9467_USB_STAT_DCP,
	RT9467_USB_STAT_CDP,
	RT9467_USB_STAT_MAX,
};
...
static u8 rt9467_find_closest_reg_value(const u32 min, const u32 max,
...
static u32 rt9467_find_closest_real_value(const u32 min, const u32 max,
...
static irqreturn_t rt9467_irq_handler(int irq, void *data)
{
	...
	for (i = 0; i < RT9467_IRQIDX_MAX; i++)
		battery_log(BAT_LOG_CRTI, "%s: %s = 0x%02X\n", __func__,
			rt9467_irq_name[i], irq_data[i]);
...
static int rt9467_irq_register(struct rt9467_info *info)
{
	...
	ret = devm_request_threaded_irq(info->dev, info->irq, NULL,
		rt9467_irq_handler, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
		"rt9467_irq", info);
	if (ret < 0) {
...
static int rt9467_enable_all_irq(struct rt9467_info *info, bool enable)
{
	int ret = 0;
	...
	if (enable)
		ret = rt9467_device_write(info->i2c, RT9467_REG_CHG_STATC_CTRL,
			ARRAY_SIZE(rt9467_irq_init_data), rt9467_irq_init_data);
	else
		ret = rt9467_device_write(info->i2c, RT9467_REG_CHG_STATC_CTRL,
...
static int rt9467_irq_init(struct rt9467_info *info)
{
	...
	/* Disable all irq */
	ret = rt9467_enable_all_irq(info, false);
...
static bool rt9467_is_hw_exist(struct rt9467_info *info)
{
	...
	ret = i2c_smbus_read_byte_data(info->i2c, RT9467_REG_DEVICE_ID);
	if (ret < 0)
		return false;
...
static int rt9467_set_fast_charge_timer(struct rt9467_info *info, u32 hour)
{
	...
	reg_fct = rt9467_find_closest_reg_value(RT9467_WT_FC_MIN, RT9467_WT_FC_MAX,
		RT9467_WT_FC_STEP, RT9467_WT_FC_NUM, hour);
...
static int rt9467_enable_watchdog_timer(struct rt9467_info *info, bool enable)
{
	int ret = 0;

	ret = (enable ? rt9467_set_bit : rt9467_clr_bit)
...
static int rt9467_enable_ilim(struct rt9467_info *info, bool enable)
{
	int ret = 0;
...
static int rt9467_enable_hidden_mode(struct rt9467_info *info, bool enable)
{
	int ret = 0;
	...
	/* Enable hidden mode */
	ret = rt9467_i2c_block_write(info, rt9467_reg_en_hidden_mode[0],
...
static int rt9467_sw_workaround(struct rt9467_info *info)
{
	...
	if (info->mchr_info.device_id <= RT9467_CHIP_REV_E3) {
		mtk_charger_get_platform_boot_mode(&info->mchr_info,
			&boot_mode);
		if (boot_mode == NORMAL_BOOT) {
			ret = rt9467_set_iprec(info, 850);
			if (ret < 0)
				goto out;

			/* Increase Isys drop threshold to 2.5A */
			ret = rt9467_i2c_write_byte(info, 0x26, 0x1C);
			if (ret < 0)
				goto out;
		}
	}
	...
	ret = rt9467_clr_bit(info, 0x2E, 0x01);
	if (ret < 0)
		goto out;
	...
	if (info->mchr_info.device_id > RT9467_CHIP_REV_E1)
		goto out;

	/* ICC: modify sensing node, make it more accurate */
	ret = rt9467_i2c_write_byte(info, 0x27, 0x00);
	if (ret < 0)
		goto out;

	/* DIMIN level */
	ret = rt9467_i2c_write_byte(info, 0x28, 0x86);
	...
	enum rt9467_adc_sel adc_sel, int *adc_val)
{
	...
	for (i = 0; i < max_wait_retry; i++) {
		mdelay(35);
		if (rt9467_i2c_test_bit(info, RT9467_REG_CHG_ADC,
			RT9467_SHIFT_ADC_START) == 0) {
...
static int rt9467_enable_te(struct rt9467_info *info, const u8 enable)
{
	int ret = 0;
...
static int rt9467_set_ieoc(struct rt9467_info *info, const u32 ieoc)
{
	int ret = 0;
...
static int rt9467_enable_jeita(struct rt9467_info *info, bool enable)
{
	int ret = 0;
...
static int rt9467_enable_chgdet_flow(struct rt9467_info *info, bool enable)
{
	int ret = 0;
...
static int rt9467_init_setting(struct rt9467_info *info)
{
	...
	ret = rt9467_enable_te(info, desc->enable_te);
	if (ret < 0)
	...
	ret = rt9467_enable_watchdog_timer(info, desc->enable_wdt);
	if (ret < 0)
...
static int rt9467_is_charging_enable(struct rt9467_info *info, bool *enable)
{
	int ret = 0;

	ret = rt9467_i2c_read_byte(info, RT9467_REG_CHG_CTRL2);
	if (ret < 0)
		return ret;

	*enable = ((ret & RT9467_MASK_CHG_EN) >> RT9467_SHIFT_CHG_EN) > 0 ?
		true : false;

	return ret;
}

static int rt9467_enable_pump_express(struct rt9467_info *info, bool enable)
{
	int ret = 0, i = 0;
	...
	for (i = 0; i < max_retry_times; i++) {
		msleep(2500);
		if (rt9467_i2c_test_bit(info, RT9467_REG_CHG_CTRL17,
			RT9467_SHIFT_PUMPX_EN) == 0)
			break;
	}
...
static int rt9467_set_iin_vth(struct rt9467_info *info, u32 iin_vth)
{
	int ret = 0;
	u8 reg_iin_vth = 0;
	...
	reg_iin_vth = rt9467_find_closest_reg_value(RT9467_IIN_VTH_MIN,
		RT9467_IIN_VTH_MAX, RT9467_IIN_VTH_STEP, RT9467_IIN_VTH_NUM,
		iin_vth);

	ret = rt9467_i2c_update_bits(
		info,
		RT9467_REG_CHG_CTRL14,
		reg_iin_vth << RT9467_SHIFT_IIN_VTH,
		RT9467_MASK_IIN_VTH
	);
...
static void rt9467_discharging_work_handler(struct work_struct *work)
{
	int ret = 0, i = 0;
	const unsigned int max_retry_cnt = 3;
	struct rt9467_info *info = (struct rt9467_info *)container_of(work,
		struct rt9467_info, discharging_work);
	...
	ret = rt9467_set_bit(info, 0x21, 0x04);
	if (ret < 0) {
	...
	for (i = 0; i < max_retry_cnt; i++) {
		/* Disable discharging */
		ret = rt9467_clr_bit(info, 0x21, 0x04);
		if (ret < 0)
			continue;
		if (rt9467_i2c_test_bit(info, 0x21, 2) == 0)
			goto out;
	}
...
static int rt9467_run_discharging(struct rt9467_info *info)
{
	...
	void *data)
{
	...
	if ((Enable_BATDRV_LOG > BAT_LOG_CRTI) ||
	    (chg_status == RT9467_CHG_STATUS_FAULT)) {
		info->i2c_log_level = BAT_LOG_CRTI;
...
static int rt_charger_enable_otg(struct mtk_charger_info *mchr_info, void *data)
{
	...
	if (!enable) {
		...
		ret = rt9467_i2c_write_byte(info, 0x25, 0x0F);
		if (ret < 0)
		...
		ret = rt9467_run_discharging(info);
		if (ret < 0)
	...
	for (i = 0; i < ss_wait_times; i++) {
		ret = rt9467_get_adc(info, RT9467_ADC_VBUS_DIV2, &vbus);
		...
		if (ret == 0 && vbus > 4000) {
			...
			ret = rt9467_i2c_write_byte(info, 0x25, 0x00);
			if (ret < 0)
				battery_log(BAT_LOG_CRTI,
	...
	if (i == ss_wait_times) {
		...
		/* Disable OTG */
		rt9467_clr_bit(info, RT9467_REG_CHG_CTRL1,
			RT9467_MASK_OPA_MODE);
		ret = -EIO;
	}
...
static int rt_charger_enable_power_path(struct mtk_charger_info *mchr_info, void *data)
{
	int ret = 0;
	u8 enable = *((u8 *)data);
	u32 mivr = (enable ? 4500 : RT9467_MIVR_MAX);
	struct rt9467_info *info = (struct rt9467_info *)mchr_info;

	battery_log(BAT_LOG_CRTI, "%s: enable = %d\n", __func__, enable);
	ret = rt9467_set_mivr(info, mivr);
	...
	void *data)
{
	...
	reg_voreg = rt9467_find_closest_reg_value(RT9467_BAT_VOREG_MIN,
		RT9467_BAT_VOREG_MAX, RT9467_BAT_VOREG_STEP,
		RT9467_BAT_VOREG_NUM, voreg);
	...
	ret = rt9467_i2c_update_bits(
		info,
		RT9467_REG_CHG_CTRL4,
		reg_voreg << RT9467_SHIFT_BAT_VOREG,
		RT9467_MASK_BAT_VOREG
	);
	...
	struct mtk_charger_info *mchr_info, void *data)
{
	...
	/* Set Pump Up/Down */
	ret = (is_pump_up ? rt9467_set_bit : rt9467_clr_bit)
	...
	void *data)
{
	...
	*((bool *)data) = ((mivr == RT9467_MIVR_MAX) ? false : true);
...
static int rt_charger_run_aicl(struct mtk_charger_info *mchr_info, void *data)
{
	...
	if (rt9467_i2c_test_bit(info, RT9467_REG_CHG_STATC,
		RT9467_SHIFT_CHG_MIVR) <= 0) {
	...
	ret = rt9467_set_iin_vth(info, iin_vth);
	if (ret < 0)
		goto out;

	ret = rt9467_set_bit(info, RT9467_REG_CHG_CTRL14, RT9467_MASK_IIN_MEAS);
	if (ret < 0)
		goto out;

	for (i = 0; i < max_wait_time; i++) {
		msleep(500);
		if (rt9467_i2c_test_bit(info, RT9467_REG_CHG_CTRL14,
			RT9467_SHIFT_IIN_MEAS) == 0)
			break;
	}
	if (i == max_wait_time) {
...
static int rt_charger_set_ircmp_vclamp(struct mtk_charger_info *mchr_info, void *data)
{
	...
	reg_vclamp = rt9467_find_closest_reg_value(RT9467_IRCMP_VCLAMP_MIN,
		...
		RT9467_IRCMP_VCLAMP_NUM, vclamp);
	...
	void *data)
{
	...
	enum rt9467_usb_status usb_stat = RT9467_USB_STAT_NO_VBUS;
	...
	for (i = 0; i < max_retry_time; i++) {
		...
		if (ret < 0)
			return ret;
		usb_stat = ret & RT9467_MASK_USB_STATUS;
		if (usb_stat != RT9467_USB_STAT_DETECTING)
			break;
		mdelay(500);
	}
	if (i == max_retry_time) {
		pr_err("%s: wait bc12 failed, ret = %d\n", __func__, ret);
		info->chg_type = CHARGER_UNKNOWN;
		goto out;
	}

	switch (usb_stat) {
	case RT9467_USB_STAT_SDP:
		info->chg_type = STANDARD_HOST;
		break;
	case RT9467_USB_STAT_SDP_NSTD:
		info->chg_type = NONSTANDARD_CHARGER;
		break;
	case RT9467_USB_STAT_DCP:
		info->chg_type = STANDARD_CHARGER;
		break;
	case RT9467_USB_STAT_CDP:
...
out:
	*(CHARGER_TYPE *)data = info->chg_type;
	...
	void *data)
{
	...
	ret = rt9467_enable_chgdet_flow(info, enable);
	if (ret < 0)
...
static int rt_charger_sw_reset(struct mtk_charger_info *mchr_info, void *data)
{
	...
	/* Read all irq */
	ret = rt9467_device_read(info->i2c, RT9467_REG_CHG_STATC,
		ARRAY_SIZE(irq_data), irq_data);
	if (ret < 0)
	...
	ret = rt9467_device_write(info->i2c, RT9467_REG_CHG_CTRL1,
...
static const mtk_charger_intf rt9467_mchr_intf[CHARGING_CMD_NUMBER] = {
	...
	const struct i2c_device_id *dev_id)
{
	...
	info = devm_kzalloc(&i2c->dev, sizeof(struct rt9467_info), GFP_KERNEL);
	if (!info) {
		battery_log(BAT_LOG_CRTI, "%s: no enough memory\n", __func__);
		return -ENOMEM;
	}
	...
	if (!rt9467_is_hw_exist(info)) {
		battery_log(BAT_LOG_CRTI, "%s: no rt9467 exists\n", __func__);
		ret = -ENODEV;
		goto err_no_dev;
	}
	i2c_set_clientdata(i2c, info);

	ret = rt9467_parse_dt(info, &i2c->dev);
	if (ret < 0) {
...
{
	int ret = 0;
	struct rt9467_info *info = i2c_get_clientdata(i2c);
...
static void rt9467_shutdown(struct i2c_client *i2c)
{
	int ret = 0;
	struct rt9467_info *info = i2c_get_clientdata(i2c);
