...
static void bq24296_shutdown(struct i2c_client *client)
{
    ...
    printk("[bq24296_shutdown] driver shutdown \n");
...
static ssize_t show_bq24296_id(struct device *dev,struct device_attribute *attr, char *buf)
{
    unsigned char  tmp;

     bq24296_read_byte(0x0A, &tmp);
     tmp &= 0xe0;
     printk("tmp=0x%X\n", tmp);
...
static ssize_t store_bq24296_id(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}

static DEVICE_ATTR(bq24296_id, 0664, show_bq24296_id, store_bq24296_id); //664
...
/*Start lenovo chailu1 add  for usr space read ic ID 20150521*/
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_bq24296_id);	
/*End  lenovo chailu1 add  for usr space read ic ID 20150521*/
