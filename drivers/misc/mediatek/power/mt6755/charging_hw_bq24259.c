 ...
 void lenovo_charging_again_enble(void)
{
	...
	bq24259_set_en_hiz(1);
	bq24259_set_chg_config(0x0); // charger enable
	msleep(10);

	charging_hw_init(&en);
	
	bq24259_set_en_hiz(0x0);	            	
	bq24259_set_chg_config(0x1); // charger enable
}
...
void lenovo_charging_term_set_stage_2(void)
{
	if(lenovo_charging_stage==1)
	{
		...
		bq24259_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_2);
	}
}
...
{
	if(lenovo_charging_stage==1)
		bq24259_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_1);//lenovo_charging_set_iterm(CHARGE_CURRENT_200_00_MA);
	else
		bq24259_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_2);//lenovo_charging_set_iterm(CHARGE_CURRENT_150_00_MA);
}
#endif
...
static unsigned int charging_hw_init(void *data)
{
    ...
    bq24259_set_vindpm(0x8); //VIN DPM check 4.52V ,TI recomand
#else
    bq24259_set_vindpm(0x6); //VIN DPM check 4.36V
#endif
	//lenovo-sw mahj2 modify at 20150514 End
	bq24259_set_reg_rst(0x0);
	bq24259_set_wdt_rst(0x1);	/* Kick watchdog */
	bq24259_set_sys_min(0x5);	/* Minimum system voltage 3.5V */
	bq24259_set_iprechg(0x3);	/* Precharge current 512mA */
	 /*Begin lenovo-sw mahj2 poring for charging terminate as 0.1c */    
#ifdef LENOVO_CHARGING_TERM
	lenovo_charging_term_set_iterm();
#else
	bq24259_set_iterm(0x0);	/* Termination current 128mA */
#endif
 ...
 /*Begin, lenovo-sw mahj2 add 45-50  CV limit fuction,  20150318 */
      ...
      if(lenovo_battery_is_temp_45_to_pos_50()) 
      	{
            bq24259_set_vreg(LENOVO_TEMP_POS_45_TO_POS_50_CV_REG); //4.1v
      	}
       else
#endif
	{
	    ...
	    #if defined(HIGH_BATTERY_VOLTAGE_4400MV_SUPPORT)    
	        bq24259_set_vreg(0x38); // lenovo-sw chailu1 change cv to 4.4v 
	    #else
		  bq24259_set_vreg(0x35); // lenovo-sw zhangrc2 change cv from 4.304 to 4.352  
	    #endif	  
		else		
			bq24259_set_vreg(0x2C);	/* VREG 4.208V */
	}
/*End, lenovo-sw mahj2 add 45-50  CV limit fuction,  20150318 */

	bq24259_set_batlowv(0x1);	/* BATLOWV 3.0V */
	bq24259_set_vrechg(0x0);	/* VRECHG 0.1V (4.108V) */
	bq24259_set_en_term(0x1);	/* Enable termination */
	bq24259_set_watchdog(0x1);	/* WDT 40s */
	bq24259_set_en_timer(0x0);	/* Disable charge timer */
	bq24259_set_int_mask(0x0);	/* Disable fault interrupt */
...
static unsigned int charging_get_current(void *data)
{
	...
	/* Get current level */
	bq24259_read_interface(bq24259_CON2, &ret_val, CON2_ICHG_MASK, CON2_ICHG_SHIFT);

	/* Get Force 20% option */
	bq24259_read_interface(bq24259_CON2, &ret_force_20pct, CON2_FORCE_20PCT_MASK,
...
static unsigned int charging_set_current(void *data)
{
	...
	bq24259_set_ichg(register_value);
...
static unsigned int charging_set_input_current(void *data)
{
	...
	bq24259_set_iinlim(register_value);
...
static unsigned int charging_set_ta_current_pattern(void *data)
{
	...
	if (KAL_FALSE == charging_status) {
		...
		bq24259_set_ichg(0x0);	/* Set charging current 500ma */
		bq24259_set_chg_config(0x1);	/* Enable Charging */
	}

	if (increase == KAL_TRUE) {
		bq24259_set_iinlim(0x0);	/* 100mA */
		msleep(85);

		bq24259_set_iinlim(0x2);	/* 500mA */
