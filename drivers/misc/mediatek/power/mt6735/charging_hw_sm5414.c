...
unsigned int charging_value_to_parameter(const unsigned int *parameter, const unsigned int array_size, const unsigned int val)
{
	...
	{
		battery_log(BAT_LOG_CRTI, "sm5414 Can't find the parameter \r\n");	
		return parameter[0];
	}
}
...
//extern int sm5414_init_for_test (void);
static unsigned int charging_hw_init(void *data)
{
	unsigned int status = STATUS_OK;

	//sm5414_init_for_test ();//add by caozhg for test
#if defined(GPIO_SWCHARGER_SHDN_PIN) //modify by caozhg
...
#endif
	sm5414_set_suspen (SUSPEND_DIS);	// enable power path //add by caozhg
	sm5414_set_aiclth (AICL_THRESHOLD_4_5_V);  	//aiclth 4.5V //add by caozhg
	sm5414_set_topoff(TOPOFF_150mA);
	sm5414_set_prechg (PRECHG_450mA );
#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
	sm5414_set_batreg(BATREG_4_4_0_0_V); //VREG 4.4V
#else
...
static unsigned int charging_get_charging_status(void *data)
{
	...
	ret_val = sm5414_get_topoff_status();
...
signed int chr_control_interface(CHARGING_CTRL_CMD cmd, void *data)
{
	...
	if (cmd < CHARGING_CMD_NUMBER) {
		if (charging_func[cmd] != NULL) {
			...
			battery_log(BAT_LOG_CRTI, "[chr_control_interface] caozhengguang cmd:%d \n", cmd);
		}
		else {
