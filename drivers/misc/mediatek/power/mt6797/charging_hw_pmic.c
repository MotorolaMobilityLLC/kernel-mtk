...
static unsigned int charging_hw_init(void *data)
{
	...
	pmic_set_register_value(PMIC_RG_NORM_CHR_EN, 1);	/* CHR_EN */
    #endif
...
static unsigned int charging_dump_register(void *data)
{
	...
	for (i = MT6351_CHR_CON0 ; i <= MT6351_CHR_CON29 ; i += 2) {
...
static unsigned int charging_set_cv_voltage(void *data)
{
	...
	pmic_set_register_value(PMIC_RG_VBAT_NORM_CV_VTH, register_value);
...
static unsigned int charging_get_current(void *data)
{
	...
	reg_value = pmic_get_register_value(PMIC_RG_NORM_CS_VTH);/*RG_CS_VTH*/
...
static unsigned int charging_set_current(void *data)
{
	...
	pmic_set_register_value(PMIC_RG_NORM_CS_VTH, register_value);
...
static unsigned int charging_set_ta_current_pattern(void *data)
{
	...
	if (increase == KAL_TRUE) {
		...
		pmic_set_register_value(PMIC_RG_NORM_CS_VTH, set_ta_off_current_reg_value);
		msleep(50);

		/* patent start*/
		pmic_set_register_value(PMIC_RG_NORM_CS_VTH, set_ta_on_current_reg_value);
