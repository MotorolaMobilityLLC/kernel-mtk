		...
		unsigned int num_trbs)
{
	...
	if (enq == ring->dequeue) {
		...
		if (num_trbs > left_on_ring) {
			xhci_warn(xhci, "Not enough room on ring; need %u TRBs, %u TRBs left\n",
		...
		struct xhci_dequeue_state *state)
{
	...
	ep_ring = mtktest_xhci_triad_to_transfer_ring(xhci, slot_id,
			ep_index, stream_id);
	if (!ep_ring) {
		xhci_warn(xhci, "WARN can't find new dequeue state for invalid stream ID %u.\n",
				stream_id);
		return;
	}
		...
		struct xhci_td *cur_td)
{
	...
	for (cur_seg = cur_td->start_seg, cur_trb = cur_td->first_trb;
		...
		if ((cur_trb->generic.field[3] & TRB_TYPE_BITMASK) ==
			...
			xhci_dbg(xhci, "Cancel TRB %p (0x%llx dma) in seg %p (0x%llx dma)\n",
					cur_trb,
					(unsigned long long)mtktest_xhci_trb_virt_to_dma(cur_seg, cur_trb),
					cur_seg,
					(unsigned long long)cur_seg->dma);
		...
		struct xhci_dequeue_state *deq_state)
{
	...
	xhci_dbg(xhci, "Set TR Deq Ptr cmd, new deq seg = %p (0x%llx dma), new deq ptr = %p (0x%llx dma), new cycle = %u\n",
			deq_state->new_deq_seg,
			(unsigned long long)deq_state->new_deq_seg->dma,
			deq_state->new_deq_ptr,
			(unsigned long long)mtktest_xhci_trb_virt_to_dma(deq_state->new_deq_seg,
			deq_state->new_deq_ptr),
...
void mtktest_xhci_stop_endpoint_command_watchdog(unsigned long arg)
{
	...
	ep->stop_cmds_pending--;
	if (xhci->xhc_state & XHCI_STATE_DYING) {
		xhci_dbg(xhci, "Stop EP timer ran, but another timer marked xHCI as DYING, exiting.\n");
		spin_unlock(&xhci->lock);
		return;
	}
	if (!(ep->stop_cmds_pending == 0 && (ep->ep_state & EP_HALT_PENDING))) {
		xhci_dbg(xhci, "Stop EP timer ran, but no command pending, exiting.\n");
		spin_unlock(&xhci->lock);
		return;
	}
	...
	for (i = 0; i < MAX_HC_SLOTS; i++) {
		...
		for (j = 0; j < 31; j++) {
			...
			if (!ring)
				continue;
			xhci_dbg(xhci, "Killing URBs for slot ID %u, ep index %u\n", i, j);
		...
		dma_addr_t  suspect_dma)
{
	...
	cur_seg = start_seg;
	pr_debug("start_dma 0x%p\n", (void *)(unsigned long)start_dma);
	pr_debug("start_seg 0x%p\n", start_seg);
	pr_debug("start_trb 0x%p\n", start_trb);
	pr_debug("end_trb 0x%p\n", end_trb);
	pr_debug("suspect_dma 0x%p\n", (void *)(unsigned long)suspect_dma);

	do {
		if (start_dma == 0) {
			pr_debug("return NULL 1\n");
			return NULL;
		}
	...
	struct xhci_virt_ep *ep, int *status)
{
	...
	default:
		xhci_dbg(xhci, "TRB error code %u, halted endpoint index = %u\n",
	...
	case COMP_SUCCESS:
		...
		if (event_trb != td->last_trb) {
			...
			if (usb_endpoint_xfer_bulk(&td->urb->ep->desc))
				...
				xhci_dbg(xhci, "Successful interrupt transfer!\n");
			*status = 0;
		}
		break;
	case COMP_SHORT_TX:
		...
		struct xhci_event_cmd *event)
{
	...
	case TRB_TYPE(TRB_STOP_RING):
		...
		if ((((int)event->cmd_trb) & 0xff0) != g_cmd_ring_pointer1
			&& (((int)event->cmd_trb) & 0xff0) != g_cmd_ring_pointer2) {
...
int rh_get_port_status(struct xhci_hcd *xhci, int port_id)
{
	...
	/*pr_err("[OTG_H][rh_get_port_status] read port status 0x%x\n", temp);*/
...
int port_status;
		...
		if (port_status & USB_PORT_STAT_CONNECTION) {
			...
			if (!(port_status & USB_PORT_STAT_SUPER_SPEED)) {
				if (g_hs_block_reset) {/*  USBIF OTG */
					...
					temp1 = mtktest_xhci_port_state_to_neutral(temp);
					xhci_err(xhci, "Before Reset command, the port status is 0x%x, devctl is 0x%x\n",
						temp1, readl(DEVICE_CONTROL));
	...
	if ((port_status & (USB_PORT_STAT_C_RESET << 16))
		...
		pr_err("[OTG_H] USB_PORT_STAT_C_RESET and\n");
		/*  receiive SSUSB_CHG_A_ROLE_A from U3D test driver */
		if (g_otg_hnp_become_host && g_otg_dev_B) {
			if (!g_otg_wait_con) {
				while (readl(SSUSB_OTG_STS) & SSUSB_XHCI_MAS_DMA_REQ) {
					msleep(20);
					pr_err("[OTG_H] DMA in used\n");
				}
				writel(SSUSB_CHG_B_ROLE_B, SSUSB_OTG_STS);
				pr_err("[OTG_H]alert device done\n");
			}
		}
		g_port_connect = false;
		mb();
...
int mtktest_xhci_handle_event(struct xhci_hcd *xhci)
{
	...
	case TRB_TYPE(TRB_DEV_NOTE):
		...
		xhci_dbg(xhci, "fields 0x%x 0x%x 0x%x 0x%x\n"
			, generic_event->field[0], generic_event->field[1], generic_event->field[2],
			generic_event->field[3]);
		g_dev_notification = TRB_DEV_NOTE_TYPE(generic_event->field[0]);
	...
	if (enqueue_is_link_trb(ep_ring)) {
		...
		while (last_trb(xhci, ring, ring->enq_seg, next)) {
			...
			if (last_trb_on_last_seg(xhci, ring, ring->enq_seg, next)) {
				ring->cycle_state = (ring->cycle_state ? 0 : 1);
				if (!in_interrupt()) {
					xhci_dbg(xhci, "queue_trb: Toggle cycle state for ring %p = %i\n",
...
{
	if (num_trbs != 0)
		dev_dbg(&urb->dev->dev, "%s - ep %#x - Miscalculated number of TRBs, %d left\n", __func__,
				urb->ep->desc.bEndpointAddress, num_trbs);
	if (running_total != urb->transfer_buffer_length)
		dev_dbg(&urb->dev->dev, "%s - ep %#x - Miscalculated tx length, queued %#x (%d), asked for %#x (%d)\n",
				__func__,
				urb->ep->desc.bEndpointAddress,
		...
		struct urb *urb, int slot_id, unsigned int ep_index)
{
	...
	do {
		...
		xhci_dbg(xhci, " sg entry: dma = %#x, len = %#x (%d), 64KB boundary at %#x, end dma = %#x\n",
		...
		struct urb *urb, int slot_id, unsigned int ep_index)
{
	...
	if (!in_interrupt())
		dev_dbg(&urb->dev->dev, "ep %#x - urb len = %#x (%d), addr = %#llx, num_tds = %d\n",
				urb->ep->desc.bEndpointAddress,
				urb->transfer_buffer_length,
				urb->transfer_buffer_length,
				(unsigned long long)urb->transfer_dma,
		...
		u32 field3, u32 field4, bool command_must_succeed)
{
	...
	if (ret < 0) {
		...
		if (command_must_succeed)
			xhci_err(xhci, "[ERROR] Reserved TRB counting for unfailable commands failed.\n");
		return ret;
	}
