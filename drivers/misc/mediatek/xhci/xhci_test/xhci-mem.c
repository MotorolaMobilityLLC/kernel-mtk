		...
		unsigned int num_segs, bool link_trbs, gfp_t flags)
{
	...
	if (link_trbs) {
		...
		xhci_dbg(xhci, "Wrote link toggle flag to segment %p (virtual), 0x%llx (DMA)\n",
		...
		unsigned int ep_index)
{
	...
	if (rings_cached < XHCI_MAX_RINGS_CACHED) {
		...
		xhci_dbg(xhci, "Ring cache full (%d rings), freeing ring\n",
				virt_dev->num_rings_cached);
	}
...
struct xhci_input_control_ctx *mtktest_xhci_get_input_control_ctx(struct xhci_hcd *xhci,
							struct xhci_container_ctx *ctx)
{
	WARN_ON(ctx->type != XHCI_CTX_TYPE_INPUT);
		...
		unsigned int stream_id)
{
	...
	if (stream_id == 0) {
		xhci_warn(xhci, "WARN: Slot ID %u, ep index %u has streams, but URB has no stream ID.\n",
				slot_id, ep_index);
		return NULL;
	}
	...
	xhci_warn(xhci, "WARN: SlotID %u,epindex %u has streamIDs 1 to %u allocated, but streamID %u is requested.\n",
			slot_id, ep_index,
		...
		struct usb_device *udev, gfp_t flags)
{
	...
	xhci_err(xhci, "Slot %d output ctx = 0x%llx (dma)\n", slot_id,
	...
	xhci_err(xhci, "Slot %d input ctx = 0x%llx (dma)\n", slot_id,
...
int mtktest_xhci_setup_addressable_virt_dev(struct xhci_hcd *xhci, struct usb_device *udev)
{
	...
	for (top_dev = udev; top_dev->parent && top_dev->parent->parent; top_dev = top_dev->parent)
	...
	/* Steps 7 and 8 were done in mtktest_xhci_alloc_virt_device() */
	xhci_err(xhci, "slot_ctx 0x%x 0x%x 0x%x 0x%x\n",
		slot_ctx->dev_info, slot_ctx->dev_info2, slot_ctx->tt_info, slot_ctx->dev_state);
	xhci_err(xhci, "ep0_ctx 0x%x 0x%x %llx 0x%x\n",
		ep0_ctx->ep_info, ep0_ctx->ep_info2, ep0_ctx->deq, ep0_ctx->tx_info);
	xhci_err(xhci, "ctrl_ctx 0x%x 0x%x\n",
		...
		gfp_t mem_flags)
{
	...
	case USB_SPEED_LOW:
		...
		max_packet = ep->desc.wMaxPacketSize & 0x7ff;
		/* workaround for maxp size issue of mac */
	...
	/* mtktest_mtk_xhci_endpoint_scheduler_init(xhci, udev, ep, ep_ctx);*/
#if MTK_SCH_NEW
		...
		char *test_name, int test_number)
{
	...
	if (seg != result_seg) {
		...
		xhci_warn(xhci, "Tested TRB math w/ seg %p and input DMA 0x%llx\n",
				input_seg,
				(unsigned long long) input_dma);
		xhci_warn(xhci, "starting TRB %p (0x%llx DMA), ending TRB %p (0x%llx DMA)\n",
				start_trb, start_dma,
				end_trb, end_dma);
...
int mtktest_xhci_mem_init(struct xhci_hcd *xhci, gfp_t flags)
{
	...
	/*
	 * Section 5.4.6 - doorbell array must be
