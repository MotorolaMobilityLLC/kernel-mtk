...
unsigned long long get_mem_bw(void)
{
	...
	/* pr_debug("[get_mem_bw]emi_dcm_disable = %d\n", emi_dcm_disable); */
	...
	if (time_period_ns >= 0xFFFFFFFF) { /* uint32_t overflow */
		...
		pr_debug("[%s] time_period_ns overflow lst\n", __func__);

		if (time_period_ns >= 0xFFFFFFFF) { /* uint32_t overflow */
			do_div(time_period_ns, 1000);
			do_div(throughput, 1000);
			pr_debug("[%s] time_period_ns overflow 2nd\n", __func__);
		}
	}
	...
	/* pr_debug("[get_mem_bw]loop count = %d, last_word_all_count =
...
static struct platform_driver mt_mem_bw_pdrv = {
	...
	.remove = NULL,
	.driver = {
	    .name = "mt-mem_bw", .pm = &mt_mem_bw_pm_ops, .owner = THIS_MODULE,
	},
};
