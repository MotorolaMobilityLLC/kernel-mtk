...
void BM_Init(void)
{
	...
	if (readl(IOMEM(EMI_ARBA)) & 0x00008000) {
		g_cBWL |= 1 << 0;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBA)) & ~0x00008000,
				   EMI_ARBA);
	}

	if (readl(IOMEM(EMI_ARBB)) & 0x00008000) {
		g_cBWL |= 1 << 1;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBB)) & ~0x00008000,
				   EMI_ARBB);
	}

	if (readl(IOMEM(EMI_ARBC)) & 0x00008000) {
		g_cBWL |= 1 << 2;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBC)) & ~0x00008000,
				   EMI_ARBC);
	}

	if (readl(IOMEM(EMI_ARBD)) & 0x00008000) {
		g_cBWL |= 1 << 3;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBD)) & ~0x00008000,
				   EMI_ARBD);
	}

	if (readl(IOMEM(EMI_ARBE)) & 0x00008000) {
		g_cBWL |= 1 << 4;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBE)) & ~0x00008000,
				   EMI_ARBE);
	}
	if (readl(IOMEM(EMI_ARBF)) & 0x00008000) {
		g_cBWL |= 1 << 5;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBF)) & ~0x00008000,
				   EMI_ARBF);
	}

	if (readl(IOMEM(EMI_ARBG_2ND)) & 0x00008000) {
		g_cBWL |= 1 << 6;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBG_2ND)) & ~0x00008000,
				   EMI_ARBG_2ND);
	}
#if defined(CONFIG_MACH_MT6797)
	if (readl(IOMEM(EMI_ARBG)) & 0x00008000) {
		g_cBWL |= 1 << 6;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBG)) & ~0x00008000,
				   EMI_ARBG);
	}
	if (readl(IOMEM(EMI_ARBH)) & 0x00008000) {
		g_cBWL |= 1 << 7;
		mt_reg_sync_writel(readl(IOMEM(EMI_ARBH)) & ~0x00008000,
				   EMI_ARBH);
	}
#endif
}
...
void BM_SetReadWriteType(const unsigned int ReadWriteType)
{
	 ...
	 *                   10 --> only W
	 */
	mt_reg_sync_writel((value & 0xFFFFFFCF) | (ReadWriteType << 4),
			   EMI_BMEN);
}
...
unsigned int BM_GetTransAllCount(void) { return readl(IOMEM(EMI_TACT)); }
...
unsigned int BM_GetBandwidthWordCount(void) { return readl(IOMEM(EMI_BACT)); }

unsigned int BM_GetOverheadWordCount(void) { return readl(IOMEM(EMI_BSCT)); }
			 ...
			 const unsigned int trans_type)
{
	...
	if (counter_num == 1) {
		...
		value |= (((trans_type & 0xFF) << 8) | (master & 0xFF))
			 << ((counter_num % 2) * 16);
	}
...
#if defined(CONFIG_MACH_MT6755) || defined(CONFIG_MACH_MT6797)
unsigned int BM_GetBWST(void) { return readl(IOMEM(EMI_BWST)); }

unsigned int BM_GetBWST1(void) { return readl(IOMEM(EMI_BWST1)); }
...
unsigned int BM_GetBW(void) { return readl(IOMEM(EMI_CONH)) & 0x7FF8; }

unsigned int BM_GetBW1(void) { return readl(IOMEM(EMI_CONO)) & 0xFF7FF8; }
#elif defined(CONFIG_MACH_MT6757)
unsigned int BM_GetBWST(void) { return readl(IOMEM(EMI_BWST0)); }

unsigned int BM_GetBWST1(void) { return readl(IOMEM(EMI_BWST_2ND)); }
...
unsigned int BM_GetBW(void) { return readl(IOMEM(EMI_BWCT0)); }

unsigned int BM_GetBW1(void) { return readl(IOMEM(EMI_BWCT0_2ND)); }
#endif

void *mt_emi_base_get(void) { return EMI_BASE_ADDR; }
