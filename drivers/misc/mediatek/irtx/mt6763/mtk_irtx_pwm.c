...
struct pwm_spec_config irtx_pwm_config = {
	...
	.PWM_MODE_MEMORY_REGS.HDURATION =
	25, /* 1 microseconds, assume clock source is 26M */
...
void switch_irtx_gpio(int mode)
{
	...
	if (mode >= (ARRAY_SIZE(irtx_gpio_cfg))) {
		pr_debug("[IRTX][PinC]%s(%d) fail!! - parameter error!\n",
		       __func__, mode);
		return;
	}

	if (IS_ERR(ppinctrl_irtx)) {
		pr_debug("[IRTX][PinC]%s ppinctrl_irtx:%p is error! err:%ld\n",
	...
	if (mode == IRTX_GPIO_MODE_LED_SET) {
		...
		if (ret < 0) {
			pr_debug("[IRTX] regulator_enable fail!!\n");
			return;
		}
	} else {
		ret = regulator_disable(mt_irtx_dev.buck);
		if (ret < 0) {
			pr_debug("[IRTX] regulator_disable fail!!\n");
			return;
		}
	}
	...
	if (IS_ERR(pins_irtx)) {
		pr_debug(
			"[IRTX][PinC]%s pinctrl_lockup(%p, %s) fail!ppinctrl:%p, err:%ld\n",
			__func__, ppinctrl_irtx, irtx_gpio_cfg[mode], pins_irtx,
			PTR_ERR(pins_irtx));
		return;
	}
...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	ret = put_ir_device();
	if (ret) {
		pr_debug("[IRTX] device close without open\n");
		goto exit;
	}
			   ...
			   unsigned long arg)
{
	...
	case IRTX_IOC_SET_IRTX_LED_EN:
		if (copy_from_user(&para, (void __user *)arg,
				   sizeof(unsigned int))) {
			pr_debug(
				"[IRTX] IRTX_IOC_SET_IRTX_LED_EN:copy_from_user fail!\n");
			ret = -EFAULT;
		} else {
			/* en: bit 12; */
			/* gpio: bit 0-11 */
			...
			en = (para & 0xF);
			pr_info(
				"[IRTX] IRTX_IOC_SET_IRTX_LED_EN: 0x%x,gpio_id:%ul, en:%ul\n",
				para, gpio_id, en);
		...
		break;
	default:
		pr_debug("[IRTX] unknown ioctl cmd 0x%x\n", cmd);
		ret = -ENOTTY;
		break;
	}
	return ret;
			      ...
			      size_t count, loff_t *ppos)
{
	...
	if (!wave_vir) {
		pr_debug("[IRTX] alloc memory fail\n");
		return -ENOMEM;
	}
	ret = copy_from_user(wave_vir, buf, count);
	if (ret) {
		pr_debug("[IRTX] write, copy from user fail %d\n", ret);
		goto exit;
	}
	/* invert bit */
...
static int irtx_probe(struct platform_device *plat_dev)
{
	...
	if (plat_dev->dev.of_node == NULL) {
		pr_debug("[IRTX] irtx OF node is NULL\n");
		return -1;
	}
	...
	pr_debug("[IRTX] device tree info: major=%d pwm=%d invert=%d\n", major,
		 mt_irtx_dev.pwm_ch, mt_irtx_dev.pwm_data_invert);

	mt_irtx_dev.ppinctrl_irtx = devm_pinctrl_get(&plat_dev->dev);
	if (IS_ERR(mt_irtx_dev.ppinctrl_irtx)) {
		pr_debug("[IRTX][PinC]cannot find pinctrl. ptr_err:%ld\n",
	...
	if (mt_irtx_dev.buck == NULL) {
		pr_debug("[IRTX] regulator_get fail!!\n");
		return -1;
	}

	ret = regulator_set_voltage(mt_irtx_dev.buck, 2800000, 2800000);
	if (ret < 0) {
		pr_debug("[IRTX] regulator_set_voltage fail!!\n");
		return -1;
	}
	...
	if (!major) {
		...
		if (ret) {
			pr_debug("[IRTX] alloc_chrdev_region fail ret=%d\n",
			       ret);
			goto exit;
		} else {
		...
		if (ret) {
			pr_debug("[IRTX] register_chrdev_region fail ret=%d\n",
			       ret);
			goto exit;
		}
	}
	...
	if (!c_dev) {
		/* pr_debug("[IRTX] kmalloc cdev fail\n"); */
		goto exit;
	}
	...
	if (ret) {
		pr_debug("[IRTX] cdev_add fail ret=%d\n", ret);
		goto exit;
	}
	...
	if (IS_ERR(dev)) {
		ret = PTR_ERR(dev);
		pr_debug("[IRTX] device_create fail ret=%d\n", ret);
		goto exit;
	}
...
static int __init irtx_init(void)
{
	...
	if (ret) {
		pr_debug("[IRTX] irtx platform driver register fail %d\n", ret);
		goto exit;
	}
