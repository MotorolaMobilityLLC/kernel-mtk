...
#else
#define IRTX_GPIO	(GPIO137 | 0x80000000)
#endif
#ifndef GPIO_IRTX_OUT_PIN_M_IRTX_OUT
			      ...
			      loff_t *ppos)
{
	...
	switch_irtx_gpio(IRTX_GPIO_MODE_LED_SET);
#else				/* !defined(CONFIG_MTK_LEGACY) */
	mt_set_gpio_mode(IRTX_GPIO, GPIO_IRTX_OUT_PIN_M_IRTX_OUT);
...
static int irtx_probe(struct platform_device *plat_dev)
{
	...
	mt_set_gpio_mode(gpio_id, GPIO_IRTX_OUT_PIN_M_IRTX_OUT);
