...
void switch_irtx_gpio(int mode)
{
	...
	if (IS_ERR(pins_irtx)) {
		pr_debug("[IRTX][PinC]%s pinctrl_lockup(%p, %s) fail!! "
			 "ppinctrl:%p, err:%ld\n",
			   ...
			   unsigned long arg)
{
	...
	case IRTX_IOC_SET_IRTX_LED_EN:
		if (copy_from_user(&para, (void __user *)arg,
				   sizeof(unsigned int))) {
			pr_debug("[IRTX] IRTX_IOC_SET_IRTX_LED_EN: "
				 "copy_from_user fail!\n");
			ret = -EFAULT;
		} else {
			/* en: bit 12; */
			...
			if (en) {
				...
				if (ret < 0) {
					pr_debug("[IRTX] regulator_disable "
						 "fail!!\n");
					ret = -1;
				} else {
					/* requlator return 0(no change) or
					 * 1(change) is correct */
					ret = 0;
				}
#else
