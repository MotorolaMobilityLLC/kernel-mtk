...
static int check_proc_write(int *data, const char *ubuf, size_t cnt)
{
...
static void perfmgr_trace_count(int val, const char *fmt, ...)
{
	char log[32];
	va_list args;
...
static void set_cfp_ppm(struct ppm_limit_data *desired_freq, int headroom_opp)
{
	...
	cfp_lockprove(__func__);
	for (clu_idx = 0; clu_idx < nr_ppm_clusters; clu_idx++) {
...
#endif
	mt_ppm_userlimit_cpu_freq(nr_ppm_clusters, cfp_freq);
}
...
void cpu_ctrl_cfp(struct ppm_limit_data *desired_freq)
{
	...
	memcpy(cc_freq, desired_freq,
		nr_ppm_clusters * sizeof(struct ppm_limit_data));
...
int cpu_ctrl_cfp_init(struct proc_dir_entry *parent)
{
	...
	cc_freq = kcalloc(nr_ppm_clusters,
	...
	cfp_freq = kcalloc(nr_ppm_clusters,
		sizeof(struct ppm_limit_data), GFP_KERNEL);
	if (!cfp_freq) {
		ret = -ENOMEM;
	...
	freq_tbl = kcalloc(nr_ppm_clusters,
