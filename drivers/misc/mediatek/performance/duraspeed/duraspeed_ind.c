...
static void calculat_loading_callback(int loading)
{
	...
	perfmgr_trace_printk("cpu_loading", "update cpu_loading");
	...
	perfmgr_trace_log("cpu_loading", "current state:%d\n", state);
...
static ssize_t perfmgr_poltime_ms_proc_write(struct file *filp,
...
static int perfmgr_poltime_ms_proc_show(struct seq_file *m, void *v)
{
	cl_lock(__func__);
	if (m)
		seq_printf(m, "%d\n", polling_ms);
...
int init_duraspeed_ind(struct proc_dir_entry *parent)
{
	...
	lt_dir = proc_mkdir("duraspeed_ind", parent);
