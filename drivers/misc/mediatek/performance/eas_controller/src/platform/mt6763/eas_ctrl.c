...
void update_pwd_tbl(void)
{
	...
	if (max_freq <= 0) {
		...
		pr_debug(
			" max_freq:%lld, ll_0:%lld, ll_1:%lld, l_0:%lld, l_1:%lld\n",
			 max_freq, cpi_ll_boost_threshold[0],
			 cpi_ll_boost_threshold[1], cpi_l_boost_threshold[0],
			 cpi_l_boost_threshold[1]);
		return;
	}
	...
	pr_debug(
	    TAG " max_freq:%lld, ll_0:%lld, ll_1:%lld, l_0:%lld, l_1:%lld\n",
...
int reduce_stall(int boost_value, int cpi_thres, int vcore_high)
{
	...
	if (boost_value < 3000) {
		...
		if (cpi_ll < cpi_thres)
			vcore_opp = -1;
		else if (boost_value - 3100 > cpi_ll_boost_threshold[1] &&
			 vcore_high)
			vcore_opp = 0;
		...
		if (cpi_l < cpi_thres)
			vcore_opp = -1;
		else if (boost_value - 3200 > cpi_l_boost_threshold[1] &&
			 vcore_high)
			vcore_opp = 0;
