...
static long capacity, touch_capacity, current_max_capacity, avg_capacity, chase_capacity;
static long his_cap[2];
...
void notify_frame_complete_legacy(unsigned long frame_time)
{
	...
	if (chase == 1) {
		...
		avg_capacity = (twanted * capacity
				+ (frame_time - twanted) * (chase_capacity))
				/ frame_time;
	} else
...
void notify_frame_complete_eas(unsigned long frame_time)
{
	...
	if (first_frame)
		...
		avg_frame_time = (ema * frame_time + (10 - ema) * avg_frame_time) / 10;
	if (chase == 2) {
		boost_linear = ((avg_frame_time - (twanted-(frame_time-frame_budget))) * 100
