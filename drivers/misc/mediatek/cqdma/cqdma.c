...
int mt_req_gdma(DMA_CHAN chan)
{
	...
	if (clk_cqdma) {
		if (clk_prepare_enable(clk_cqdma)) {
			pr_err("[CQDMA] enable CQDMA clk fail!\n");
...
int mt_polling_gdma(int channel, unsigned long timeout)
{
	...
	do {
		if (time_after(jiffies, timeout)) {
			pr_err("[CQDMA] GDMA_%d polling timeout !!\n", channel);
...
int mt_config_gdma(int channel, struct mt_gdma_conf *config, DMA_CONF_FLAG flag)
{
	...
	if (config->sfix) {
		pr_err("[CQDMA] GMDA fixed address mode doesn't support\n");
		return -DMA_ERR_INV_CONFIG;
	}
	...
	if (config->count > MAX_TRANSFER_LEN1) {
		pr_err("[CQDMA] GDMA transfer length cannot exceeed 0x%x.\n", MAX_TRANSFER_LEN1);
		return -DMA_ERR_INV_CONFIG;
	}

	if (config->limiter > MAX_SLOW_DOWN_CNTER) {
		pr_err("[CQDMA] GDMA slow down counter cannot exceeed 0x%x.\n", MAX_SLOW_DOWN_CNTER);
		return -DMA_ERR_INV_CONFIG;
	}
	...
	case ALL:
		...
		if (enable_4G()) {
			...
			pr_debug("[CQDMA] 2:ADDR2_cfg(4GB):%x %x %x\n",
			...
			pr_debug("[CQDMA] 2:ADDR2_cfg(4GB):SRC=0x%x  DST=0x%x JUMP=0x%x\n",
					readl(DMA_SRC_4G_SUPPORT(channel)),
					readl(DMA_DST_4G_SUPPORT(channel)),
					readl(DMA_JUMP_4G_SUPPORT(channel)));
		}
...
static int cqdma_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < nr_cqdma_channel; i++) {
		...
		if (IS_ERR(env_info[i].base) || (env_info[i].irq <= 0)) {
			pr_err("[CQDMA] unable to map CQDMA%d base registers and irq=%d!!!\n", i, irq);
			return -EINVAL;
		}
	...
	for (i = 0; i < nr_cqdma_channel; i++) {
		...
		if (ret > 0)
			pr_err("[CQDMA] GDMA%d IRQ LINE NOT AVAILABLE,ret 0x%x!!\n", i, ret);
	}

	clk_cqdma = devm_clk_get(&pdev->dev, "cqdma");
	if (IS_ERR(clk_cqdma)) {
		pr_err("[CQDMA] can not get CQDMA clock fail!\n");
...
static int __init init_cqdma(void)
{
	...
	if (ret)
		pr_err("[CQDMA] CQDMA init FAIL, ret 0x%x!!!\n", ret);
