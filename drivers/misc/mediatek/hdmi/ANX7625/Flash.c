...
void command_flash_read(unsigned int addr, unsigned long read_size)
{
	...
	/* == Reads 32 bytes, but only keeps 16 bytes (offset: 1~16) == */
		...
		snprintf(&(pLine[0]), 10, ":%02hhX%04hX%02hhX",
		...
		for (i = 0; i < MAX_BYTE_COUNT_PER_RECORD_FLASH; i++) {
			snprintf(&(pLine[i * 2 + 9]), 3, "%02hhX",
				ReadDataBuf[i]);
			checksum += ReadDataBuf[i];
		}

		snprintf(&(pLine[32 + 9]), 3, "%02hhX", -checksum);
		pLine[32 + 9 + 2] = '\0';
		pr_info("%s\n", pLine);
...
THE_LAST_READ:
			...
			checksum = 0 + (0 >> 8) + (0 & 0xFF) +
				HEX_RECORD_TYPE_EOF;
			snprintf(&(pLine[9]), 3, "%02hhX", -checksum);
			pLine[9 + 2] = '\0';
			pr_info("%s\n", pLine);
