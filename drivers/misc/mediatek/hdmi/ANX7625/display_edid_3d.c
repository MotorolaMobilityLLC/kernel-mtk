	...
	if (limit) {
		for (i = 0 ; i < limit - 1 ; ++i) {
			if ((p_desc->dtd.pixel_clock_low != 0) || (p_desc->dtd.pixel_clock_high != 0)) {
				if ((p_desc->dtd.horz_active_7_0 == 0) && (p_desc->dtd.horz_active_blanking_high.horz_active_11_8 == 0)) {
...
static void si_mhl_tx_prune_edid(edid_3d_data_p mhl_edid_3d_data)
{
	...
	dtd_limit = ARRAY_SIZE(p_EDID_block_0->detailed_timing_descriptors);
	...
	p_3D_u.p_3D = mhl_edid_3d_data->parse_data.p_three_d;
        ...
        SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,"num_3D_structure_bytes_pruned:0x%x byte14: 0x%02x offset to DTDs: 0x%x vsdb header: 0x%x\n"
		...
		*/
		...
		for (vdb_index = mhl_edid_3d_data->parse_data.num_video_data_blocks-1
			...
			if (inner_loop_limit) {
				...
				for (i = 0; i < outer_loop_limit;) {
					if (mhl_edid_3d_data->parse_data.p_video_data_blocks_2d[vdb_index]->short_descriptors[i].VIC == 0) {
						uint8_t j, prev;
				...
				/* check the last one */
				if (mhl_edid_3d_data->parse_data.p_video_data_blocks_2d[vdb_index]->short_descriptors[outer_loop_limit].VIC == 0) {
					num_CEA_VICs_pruned++;
					inner_loop_limit--;
	...
	*/
    ...
    if (si_mhl_tx_drv_set_upstream_edid(mhl_edid_3d_data->drv_context, mhl_edid_3d_data->EDID_block_data, 2*EDID_BLOCK_SIZE) == 0)
#endif /* ) */
	{
			...
			*/
			for (/* burst_index is set to zero above */
					; (burst_index < p_write_burst_data->num_entries_this_burst)
					 ...
					 &&
					 (mhl_edid_3d_data->parse_data.vesa_dtd_index < ARRAY_SIZE(p_EDID_block_0->detailed_timing_descriptors))
			...
			if (mhl_edid_3d_data->parse_data.vesa_dtd_index >= ARRAY_SIZE(p_EDID_block_0->detailed_timing_descriptors)) {
...
static void si_mhl_tx_parse_block_zero_timing_descriptors(edid_3d_data_p mhl_edid_3d_data, PEDID_block0_t p_EDID_block_0)
{
	...
	for (i = 0; i < ARRAY_SIZE(p_EDID_block_0->detailed_timing_descriptors); i++) {
		SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context,
		...
		PEDID_block0_t p_EDID_block_0)
{
	...
	if (p_EDID_block_0->header_data[EDID_OFFSET_HEADER_FIRST_00] != 0) {
	...
	for (i = EDID_OFFSET_HEADER_FIRST_FF; i <= EDID_OFFSET_HEADER_LAST_FF; i++) {
		if (p_EDID_block_0->header_data[i] != 0xFF) {
			DUMP_EDID_BLOCK(1, p_EDID_block_0, sizeof(*p_EDID_block_0))
				SLIMPORT_TX_DBG_ERR(mhl_edid_3d_data->dev_context
							, "EDID -1 check failed\n");
			return false;
	...
	if (p_EDID_block_0->header_data[EDID_OFFSET_HEADER_LAST_00] != 0x00) {
		DUMP_EDID_BLOCK(1, p_EDID_block_0, sizeof(*p_EDID_block_0))
			SLIMPORT_TX_DBG_ERR(mhl_edid_3d_data->dev_context
...
/*#endif*/
			...
			if	(mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15->byte14.HDMI_VIC_len) {
				for (i = 0; i < mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15->byte14.HDMI_VIC_len && i < 5; i++) {
					SP_4k_VIC[i] = mhl_edid_3d_data->parse_data.p_byte_13_through_byte_15->vicList[i];
					if (SP_4k_VIC[i] == 1)
						Notify_AP_MHL_TX_Event(SLIMPORT_TX_EVENT_EDID_UPDATE, HDMI_4k30_DSC, NULL);
						/* update_av_info_edid(true, HDMI_4k30_DSC, 0); */
					else if ((SP_4k_VIC[i] == 3) || (SP_4k_VIC[i] == 4))
						Notify_AP_MHL_TX_Event(SLIMPORT_TX_EVENT_EDID_UPDATE, HDMI_4k24_DSC, NULL);
/* update_av_info_edid(true, HDMI_4k24_DSC, 0); */
					SLIMPORT_TX_EDID_INFO(mhl_edid_3d_data->dev_context, "SP_4k_VIC[%d]=%d\n", i, SP_4k_VIC[i]);
					}
				}
