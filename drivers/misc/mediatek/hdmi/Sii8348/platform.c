...
uint8_t reGetI2cAddress(uint8_t device_ID)
{
	...
	switch(device_ID)
	{
		...
		case 0xA0:
			...
			break;
		default:
			MHL_DBG("Error: invaild device ID\n");
	}
...
uint8_t mhl_i2c_read_len_bytes(struct i2c_client *client, uint8_t offset, uint8_t *buf, uint8_t len)
{
	...
	while(len > 0)
	{
		if(len > MAX_I2C_READ_NUM)
		{
			...
			if(ret < 0)
			{
		        MHL_DBG("[Error]mhl i2c sends command error!\n");
				return 0;
			}
			else
			{
				...
				if(ret < 0)
				{
			        MHL_DBG("[Error]mhl i2c recv data error!\n");
				}
...
uint8_t mhl_i2c_write_len_bytes(struct i2c_client *client, uint8_t offset, uint8_t *buf, uint8_t len)
{
	...
	while(len > 0)
	{
		if(len > MAX_I2C_WRITE_NUM)
		{
			...
			if(ret < 0)
			{
				MHL_DBG("[Error]mhl i2c write command/data error!\n");
				return 0;
			}
		...
		else
		{
			...
			if(ret < 0)
			{
				MHL_DBG("[Error1]mhl i2c write command/data error!\n");
				return 0;
			}
...
int32_t sii_8348_tx_init(void) 
{
	...
	MHL_DBG("mhl sii_8348_init, mClient is %p\n", mClient);
...
void dpi_gpio_ctrl(int enable)
{
    ...
    if (IS_ERR(mhl_pinctrl)) {
        ...
        MHL_DBG("Cannot find MHL RST pinctrl for dpi_gpio_ctrl!\n");
        return;
    }  
    ...
    for(; offset < 32 ;)
    {
        ...
        if (IS_ERR(pin_state)) {
            ...
            MHL_DBG("Cannot find MHL pinctrl--%s!!\n", dpi_gpio_name[offset]);
        }
        else
...
void i2s_gpio_ctrl(int enable)
{
    ...
    if (IS_ERR(mhl_pinctrl)) {
        ret = PTR_ERR(mhl_pinctrl);
        MHL_DBG("Cannot find MHL RST pinctrl for i2s_gpio_ctrl!\n");
        return;
    }  
...
void reset_mhl_board(int hwResetPeriod, int hwResetDelay)
{
    ...
    if (IS_ERR(mhl_pinctrl)) {
        ret = PTR_ERR(mhl_pinctrl);
        MHL_DBG("Cannot find MHL RST pinctrl for reset_mhl_board!\n");
        return;
    }    
