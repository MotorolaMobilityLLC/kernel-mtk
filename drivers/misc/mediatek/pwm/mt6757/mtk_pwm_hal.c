/*******************************************************************************
* mt6757 mtk_pwm_hal.c PWM Drvier
*
...
enum { PWM_CON,
	   ...
	   PWM_THRESH,
	   PWM_SEND_WAVENUM,
	   PWM_VALID } PWM_REG_OFF;

#if defined(CONFIG_MTK_CLKMGR)
int pwm_power_id[] = {MT_CG_PERI_PWM1, MT_CG_PERI_PWM2,      MT_CG_PERI_PWM3,
		      MT_CG_PERI_PWM4, MT_CG_INFRA_PWM_HCLK, MT_CG_PERI_PWM};
...
void mt_pwm_power_on_hal(u32 pwm_no, bool pmic_pad, unsigned long *power_flag)
{
	...
	if (0 == (*power_flag)) {
		...
		if (clk_en_ret) {
			pr_debug("[PWM][CCF]enable clk PWM_CLK failed. ret:%d, "
				 "clk_pwm_main:%p\n",
		...
		if (clk_en_ret) {
			pr_debug("[PWM][CCF]enable clk PWM_HCLK failed. "
				 "ret:%d, clk_pwm_hclk_main:%p\n",
	...
	if (!test_bit(pwm_no, power_flag)) {
		...
		if (clk_en_ret) {
			pr_debug("[PWM][CCF]enable clk_pwm_main failed. "
				 "ret:%d, clk_pwm%d :%p\n",
