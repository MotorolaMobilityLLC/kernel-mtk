/*******************************************************************************
 * mt6763 mtk_pwm_hal.c PWM Drvier
 *
...
enum { PWM_CON,
	   ...
	   PWM_SEND_WAVENUM,
	   PWM_VALID,
	   PWM_BUF_BASE_ADDR2 } PWM_REG_OFF;
...
const char *pwm_clk_name[] = {"PWM1-main", "PWM2-main",     "PWM3-main",
			      "PWM4-main", "PWM-HCLK-main", "PWM-main"};
...
void mt_pwm_power_on_hal(u32 pwm_no, bool pmic_pad, unsigned long *power_flag)
{
	...
	if (0 == (*power_flag)) {
		...
		if (clk_en_ret) {
			pr_debug(
				"[PWM][CCF]enable clk PWM_CLK failed. ret:%d, clk_pwm_main:%p\n",
	...
	if (!test_bit(pwm_no, power_flag)) {
		...
		if (clk_en_ret) {
			pr_debug(
				"[PWM][CCF]enable clk_pwm_main failed. ret:%d, clk_pwm%d :%p\n",
...
void mt_set_pwm_enable_hal(u32 pwm_no) { SETREG32(PWM_ENABLE, 1 << pwm_no); }

void mt_set_pwm_disable_hal(u32 pwm_no) { CLRREG32(PWM_ENABLE, 1 << pwm_no); }

void mt_set_pwm_enable_seqmode_hal(void) {}

void mt_set_pwm_disable_seqmode_hal(void) {}

s32 mt_set_pwm_test_sel_hal(u32 val) { return 0; }
...
void mt_set_pwm_clk_hal(u32 pwm_no, u32 clksrc, u32 div)
{
	...
	if ((clksrc & 0x80000000) != 0) {
		clksrc &= ~(0x80000000);
		if (clksrc ==
		    CLK_BLOCK_BY_1625_OR_32K) { /* old mode: 26M/1625 = 16KHz */
			CLRREG32(
			    reg_con,
			    1 << PWM_CON_CLKSEL_OLD_OFFSET); /* bit 4: 0 */
			SETREG32(reg_con,
...
void pwm_debug_show_hal(void) { mt_pwm_dump_regs_hal(); }
