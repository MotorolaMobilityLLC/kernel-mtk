...
static int usb_driving_capability_show(struct seq_file *s, void *unused)
{
	...
	val = usb20_phy_debugfs_read_val(
	    OFFSET_RG_USB20_TERM_VREF_SEL, SHFT_RG_USB20_TERM_VREF_SEL,
...
static int rg_usb20_hstx_srctrl_show(struct seq_file *s, void *unused)
{
	...
	val = usb20_phy_debugfs_read_val(
	    OFFSET_RG_USB20_HSTX_SRCTRL, SHFT_RG_USB20_HSTX_SRCTRL,
...
static int rg_usb20_intr_en_show(struct seq_file *s, void *unused)
{
	...
	val = usb20_phy_debugfs_read_val(
	    OFFSET_RG_USB20_INTR_EN, SHFT_RG_USB20_INTR_EN,
...
static int rg_usb20_rev6_show(struct seq_file *s, void *unused)
{
	...
	val = usb20_phy_debugfs_read_val(
	    OFFSET_RG_USB20_PHY_REV6, SHFT_RG_USB20_PHY_REV6,
...
{
	return single_open(file, usb_driving_capability_show,
			   inode->i_private);
}

static int rg_usb20_term_vref_sel_open(struct inode *inode, struct file *file)
{
	return single_open(file, rg_usb20_term_vref_sel_show,
			   inode->i_private);
}
...
static ssize_t rg_usb20_rev6_write(struct file *file, const char __user *ubuf,
				   size_t count, loff_t *ppos)
{
	char buf[18];
...
int usb20_phy_init_debugfs(void)
{
	...
	file =
	    debugfs_create_file(FILE_USB_DRIVING_CAPABILITY, 0644,
	...
	file =
	    debugfs_create_file(FILE_RG_USB20_TERM_VREF_SEL, 0644,
	...
	file =
	    debugfs_create_file(FILE_RG_USB20_HSTX_SRCTRL, 0644,
	...
	file =
	    debugfs_create_file(FILE_RG_USB20_VRT_VREF_SEL, 0644,
