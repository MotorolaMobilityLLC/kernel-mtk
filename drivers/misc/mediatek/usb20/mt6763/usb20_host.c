...
static void issue_host_work(int ops, int delay, bool on_st)
{
	...
	if (on_st)
		queue_delayed_work(mtk_musb->st_wq, &work->dwork,
...
static void do_vbus_work(struct work_struct *data)
{
	...
	bool vbus_on = (work->ops == VBUS_OPS_ON ? true : false);
...
static void do_host_plug_test_work(struct work_struct *data)
{
	...
	if (!wake_lock_inited) {
		...
		wake_lock_init(&host_test_wakelock, WAKE_LOCK_SUSPEND,
			       "host.test.lock");
		wake_lock_inited = 1;
	}
	...
	while (1) {
		...
		if (host_on && diff_time >= host_plug_in_test_period_ms) {
			...
			if (!host_test_vbus_only)
			   ...
			   diff_time >= host_plug_out_test_period_ms) {
			host_on = 1;
			DBG(0, "ON\n");
...
static void do_host_work(struct work_struct *data)
{
	 ...
	 */
	while (!inited && !kernel_init_done && !mtk_musb->is_ready &&
	       !timeout) {
		ktime_end = ktime_get();
	...
	host_on = (work->ops == CONNECTION_OPS_CONN ? true : false);
