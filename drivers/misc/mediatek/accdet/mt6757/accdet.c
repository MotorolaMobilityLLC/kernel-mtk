...
unsigned int accdet_eint_type =
	IRQ_TYPE_LEVEL_LOW; /* default low_level trigger */
...
char *accdet_report_string[4] = {
	"No_device", "Headset_mic", "Headset_no_mic", "Headset_five_pole",
	/* "HEADSET_illegal", */
	/* "Double_check" */
};
...
{
	ACCDET_DEBUG("[%s]resister value: R=%u(ohm)\n",
			 __func__, res_value);
...
int accdet_get_cable_type(void) { return cable_type; }
void accdet_auxadc_switch(int enable)
{
	if (enable) {
		pmic_pwrap_write(ACCDET_ADC_REG,
...
static int Accdet_PMIC_IMM_GetOneChannelValue(int deCount)
{
	...
	ACCDET_DEBUG(
		"ACCDET accdet_auxadc_offset: %d mv, MIC_Voltage1 = %d mv!\n\r",
		accdet_auxadc_offset, vol_val);
	return vol_val;
}
...
{
	/* send input event before switch cable_type to NO_device, to ensure
	 * cable_type valid
	 */
...
static void disable_micbias(unsigned long a)
{
	...
	if (!ret)
		ACCDET_DEBUG("[Accdet]disable micbias:accdet_work return:%d!\n",
				 ret);
}
...
/* ACCDET_DEBUG("[Accdet] FSA8049 disable!\n"); */
		...
		pmic_pwrap_write(ACCDET_PWM_THRESH,
				 cust_headset_settings->pwm_thresh);
		ACCDET_DEBUG(
			"[Accdet]pin recog after 5s recover micbias polling!\n");
	}
#endif
}
...
static irqreturn_t accdet_eint_func(int irq, void *data)
{
	...
	ACCDET_DEBUG("[Accdet]Enter accdet eint func !!!!!!\n");
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
/*
 ...
 */
...
#endif
#ifdef CONFIG_ACCDET_EINT_IRQ
		pmic_pwrap_write(ACCDET_EINT_CTL,
				 pmic_pwrap_read(ACCDET_EINT_CTL) &
					 EINT_PLUG_DEB_CLR);
		/* debounce=256ms */
		pmic_pwrap_write(ACCDET_EINT_CTL,
				 pmic_pwrap_read(ACCDET_EINT_CTL) |
		...
		pmic_pwrap_write(ACCDET_DEBOUNCE3,
				 cust_headset_settings->debounce3);
#else
...
#else
		gpio_set_debounce(
		       gpiopin, accdet_dts_data.accdet_plugout_debounce * 1000);
#endif
		/* update the eint status */
...
#endif
	ACCDET_DEBUG("[Accdet]accdet eint func after cur_eint_state=%d\n",
			 cur_eint_state);
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	if (IS_ERR(accdet_pinctrl1)) {
		...
		pr_debug(&accdet_device->dev,
			 "fwq Cannot find accdet accdet_pinctrl1!\n");
		return ret;
	}
	...
	if (IS_ERR(pins_default)) {
		...
		/* pr_debug(&accdet_device->dev, "fwq Cannot find accdet pinctrl
		 * default!\n");
		 */
	}
...
static int key_check(int b)
{
	...
	/* 0.24V ~ */
	/* ACCDET_DEBUG("[accdet] come in key check!!\n"); */
	if ((b < accdet_dts_data.three_key.down_key) &&
		...
		return MD_KEY;
	ACCDET_DEBUG("[accdet] leave key check!!\n");
	return NO_KEY;
}
#else
static int key_check(int b)
...
static void multi_key_detection(int current_status)
{
	...
	mdelay(30);
#ifdef CONFIG_ACCDET_EINT_IRQ
	if (((pmic_pwrap_read(ACCDET_IRQ_STS) & EINT_IRQ_STATUS_BIT) !=
		 EINT_IRQ_STATUS_BIT) ||
	...
	if (((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) !=
		...
		ACCDET_DEBUG(
	     "[Accdet]plug out side effect key press, do not report key = %d\n",
				 cur_key);
		cur_key = NO_KEY;
	}
	if (current_status)
...
int accdet_irq_handler(void)
{
...
#ifdef CONFIG_ACCDET_EINT_IRQ
	ACCDET_DEBUG(
	  "[Accdet %s]clear_accdet_eint_interrupt: ACCDET_IRQ_STS = 0x%x\n",
			 __func__, pmic_pwrap_read(ACCDET_IRQ_STS));
	if ((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) &&
		((pmic_pwrap_read(ACCDET_IRQ_STS) & EINT_IRQ_STATUS_BIT) !=
		 EINT_IRQ_STATUS_BIT)) {
		clear_accdet_interrupt();
		...
		while (((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) &&
			...
			;
		/* clear accdet int, modify	 for fix interrupt trigger twice
		 * error
		 */
	...
	} else if ((pmic_pwrap_read(ACCDET_IRQ_STS) & EINT_IRQ_STATUS_BIT) ==
		   EINT_IRQ_STATUS_BIT) {
		if (cur_eint_state == EINT_PIN_PLUG_IN) {
			if (accdet_eint_type == IRQ_TYPE_LEVEL_HIGH)
				pmic_pwrap_write(
	...
	cur_time = accdet_get_current_time();
	if ((pmic_pwrap_read(ACCDET_IRQ_STS) & NEGV_IRQ_STATUS_BIT) ==
		NEGV_IRQ_STATUS_BIT) {
		ACCDET_DEBUG("[ACCDET NEGV detect]plug in a error Headset\n\r");
		pmic_pwrap_write(ACCDET_IRQ_STS, (IRQ_NEGV_CLR_BIT));
		while (
			((pmic_pwrap_read(ACCDET_IRQ_STS) & NEGV_IRQ_STATUS_BIT)
			&& (accdet_timeout_ns(cur_time, ACCDET_TIME_OUT))))
			;
		pmic_pwrap_write(
		  ACCDET_IRQ_STS,
		  (pmic_pwrap_read(ACCDET_IRQ_STS) & (~IRQ_NEGV_CLR_BIT)));
	}
#endif
	return 1;
}
...
static inline void clear_accdet_eint_interrupt(void)
{
	...
	ACCDET_DEBUG(
		"[Accdet]clear_accdet_eint_interrupt: ACCDET_IRQ_STS = 0x%x\n",
		pmic_pwrap_read(ACCDET_IRQ_STS));
}
	...
	case PLUG_OUT:
		...
		if (current_status == 0) {
			...
			pmic_pwrap_write(ACCDET_PWM_THRESH,
					 cust_headset_settings->pwm_width);
			ACCDET_DEBUG(
				"[Accdet]PIN recognition micbias always on!\n");
			ACCDET_DEBUG(
			"[Accdet]before adc read, pin_adc_value = %d mv!\n",
				pin_adc_value);
			msleep(500);
			current_status =
				((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0) >>
				 6); /* A=bit1; B=bit0 */
			if (current_status == 0 && show_icon_delay != 0) {
				/* accdet_auxadc_switch(1);switch on when need
				 * to use auxadc read voltage
				 */
				pin_adc_value =
					Accdet_PMIC_IMM_GetOneChannelValue(1);
				ACCDET_DEBUG("[Accdet]pin_adc_value = %d mv!\n",
						 pin_adc_value);
				/*accdet_auxadc_switch(0);*/
				if (pin_adc_value < 180 &&
					pin_adc_value >
					90) { /* 90mv illegal headset */
				/* mt_set_gpio_out(GPIO_CAMERA_2_CMRST_PIN,
				 * GPIO_OUT_ONE);
				 */
				/* ACCDET_DEBUG("[Accdet]PIN recognition
				 * change GPIO_OUT!\n");
				 */
					mutex_lock(&accdet_eint_irq_sync_mutex);
					if (eint_accdet_sync_flag == 1) {
			...
			if (eint_accdet_sync_flag == 1) {
				...
				if (pmic_pwrap_read(0x0F46) & 0x01) {
					...
					ACCDET_DEBUG(
					 "[Accdet]check 5 pole headset: YES\n");
...
/* accdet_auxadc_switch(0); */
			...
			if ((0xC3 & (pmic_pwrap_read(0x361E))) == 0x83)
				mtk_audio_reset_input_precharge();
						/* call audio to reset record */
#endif
		} else if (current_status == 3) {
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
			cable_pin_recognition = 0;
	...
	default:
		ACCDET_DEBUG(
		"[Accdet]check_cable_type: accdet current status error!\n");
		break;
	}
	if (!IRQ_CLR_FLAG) {
		mutex_lock(&accdet_eint_irq_sync_mutex);
...
int accdet_get_dts_data(void)
{
	...
	ACCDET_INFO("[ACCDET]Start %s", __func__);
	node = of_find_matching_node(node, accdet_of_match);
	if (node) {
		...
		of_property_read_u32(node, "headset-eint-level-pol",
					 &accdet_dts_data.eint_level_pol);
#ifdef CONFIG_FOUR_KEY_HEADSET
	...
	"[Accdet]mid-Key = %d, voice = %d, up_key = %d, down_key = %d\n",
		...
		ACCDET_INFO(
"[Accdet]pwm_width = %x, pwm_thresh = %x deb0 = %x, deb1 = %x, mic_mode = %d\n",
				cust_headset_settings->pwm_width,
				cust_headset_settings->pwm_thresh,
				cust_headset_settings->debounce0,
				cust_headset_settings->debounce1,
...
void accdet_pmic_Read_Efuse_HPOffset(void)
{
	...
	/* get 8bit from efuse rigister, so need extend to 12bit, shift right
	 * 2
	 */
	/* AD */
	...
	efusevalue =
		efusevalue | ((tmp_val & 0x01) << RG_ACCDET_HIGH_BIT_SHIFT);
	...
	efusevalue =
		(int)((tmp_val >> RG_ACCDET_BIT_SHIFT) & ACCDET_CALI_MASK0);
	...
	accdet_auxadc_offset =
		(int)((efusevalue >> RG_OTP_PA_ACCDET_BIT_SHIFT) & 0xFF);
	if (accdet_auxadc_offset >
		128) /* modify to solve the sign data issue */
		accdet_auxadc_offset -= 256;
	accdet_auxadc_offset = (accdet_auxadc_offset / 2);
...
/* enable INT */
	...
	if (cur_eint_state == EINT_PIN_PLUG_OUT) {
		if (accdet_dts_data.eint_level_pol == IRQ_TYPE_LEVEL_HIGH) {
			...
			pmic_pwrap_write(ACCDET_DEFAULT_STATE_RG,
					 (reg_val | (1 << 14)) & (0x4333));
			mdelay(2);
			reg_val = pmic_pwrap_read(ACCDET_DEFAULT_STATE_RG);
			ACCDET_INFO("[accdet]high:2.[0x%x]=0x%x\n",
	...
	pmic_pwrap_write(ACCDET_IRQ_STS,
			 pmic_pwrap_read(ACCDET_IRQ_STS) & (~IRQ_EINT_CLR_BIT));
#endif
#ifdef CONFIG_ACCDET_EINT
	pmic_pwrap_write(ACCDET_IRQ_STS,
			 pmic_pwrap_read(ACCDET_IRQ_STS) & (~IRQ_CLR_BIT));
	...
	pmic_pwrap_write(ACCDET_ADC_REG,
			 pmic_pwrap_read(ACCDET_ADC_REG) | RG_AUDACCDETPULLLOW);
	pmic_pwrap_write(ACCDET_MICBIAS_REG,
			 pmic_pwrap_read(ACCDET_MICBIAS_REG) |
				 (accdet_dts_data.mic_mode_vol << 4) |
				 RG_AUDMICBIAS1LOWPEN);
	pmic_pwrap_write(ACCDET_RSV, 0x0010);
#ifdef CONFIG_ACCDET_EINT_IRQ
	pmic_pwrap_write(ACCDET_ADC_REG,
			 pmic_pwrap_read(ACCDET_ADC_REG) |
				 RG_EINTCONFIGACCDET); /* Internal connection
							*between ACCDET and EINT
							*/
#endif
#ifdef ACCDET_NEGV_IRQ
	pmic_pwrap_write(ACCDET_EINT_NV,
			 pmic_pwrap_read(ACCDET_EINT_NV) | ACCDET_NEGV_DT_EN);
#endif

	if (accdet_dts_data.accdet_mic_mode == 1) /* ACC mode */
		ACCDET_INFO("[accdet]init mode1\n");
	else if (accdet_dts_data.accdet_mic_mode ==
		 2) /* Low cost mode without internal bias */
		pmic_pwrap_write(ACCDET_ADC_REG,
				 pmic_pwrap_read(ACCDET_ADC_REG) |
					 RG_EINT_ANA_CONFIG);
	else if (accdet_dts_data.accdet_mic_mode ==
		 6) { /* Low cost mode with internal bias */
		pmic_pwrap_write(ACCDET_ADC_REG,
				 pmic_pwrap_read(ACCDET_ADC_REG) |
					 RG_EINT_ANA_CONFIG);
		pmic_pwrap_write(ACCDET_MICBIAS_REG,
	...
	pmic_pwrap_write(ACCDET_HW_MODE_DFF,
			 0x8000 | ACCDET_HWMODE_SEL | ACCDET_EINT_DEB_OUT_DFF);
#else
	...
	if (cur_eint_state == EINT_PIN_PLUG_OUT) /* debounce=256ms */
		pmic_pwrap_write(ACCDET_EINT_CTL,
				 pmic_pwrap_read(ACCDET_EINT_CTL) |
					 EINT_IRQ_DE_IN);
	pmic_pwrap_write(ACCDET_EINT_CTL,
			 pmic_pwrap_read(ACCDET_EINT_CTL) | EINT_PWM_THRESH);
	/* disable ACCDET unit, except CLK of ACCDET */
	pre_state_swctrl = pmic_pwrap_read(ACCDET_STATE_SWCTRL);
	pmic_pwrap_write(ACCDET_CTRL,
			 pmic_pwrap_read(ACCDET_CTRL) | ACCDET_DISABLE);
	pmic_pwrap_write(ACCDET_STATE_SWCTRL,
			 pmic_pwrap_read(ACCDET_STATE_SWCTRL) &
				 (~ACCDET_SWCTRL_EN));
	pmic_pwrap_write(ACCDET_STATE_SWCTRL,
			 pmic_pwrap_read(ACCDET_STATE_SWCTRL) |
				 ACCDET_EINT_PWM_EN);
	pmic_pwrap_write(ACCDET_CTRL,
			 pmic_pwrap_read(ACCDET_CTRL) | ACCDET_EINT_EN);
#else
	/* enable ACCDET unit */
	pmic_pwrap_write(ACCDET_CTRL, ACCDET_ENABLE);
#endif
#ifdef ACCDET_NEGV_IRQ
	pmic_pwrap_write(ACCDET_EINT_PWM_DELAY,
			 pmic_pwrap_read(ACCDET_EINT_PWM_DELAY) & (~0x1F));
	pmic_pwrap_write(ACCDET_EINT_PWM_DELAY,
			 pmic_pwrap_read(ACCDET_EINT_PWM_DELAY) | 0x0F);
	pmic_pwrap_write(ACCDET_CTRL,
			 pmic_pwrap_read(ACCDET_CTRL) | ACCDET_NEGV_EN);
#endif
	/* AUXADC enable auto sample */
	pmic_pwrap_write(
	    ACCDET_AUXADC_AUTO_SPL,
	    (pmic_pwrap_read(ACCDET_AUXADC_AUTO_SPL) | ACCDET_AUXADC_AUTO_SET));
}
...
static DRIVER_ATTR(TS3A225EConnectorType, 0664, show_TS3A225EConnectorType,
		   NULL);
#endif
...
static ssize_t store_accdet_set_register(struct device_driver *ddri,
					 const char *buf, size_t count)
{
	int ret = 0;
...
static DRIVER_ATTR(set_register, 0644, NULL,
		   store_accdet_set_register);
...
#ifdef CONFIG_ACCDET_EINT_IRQ
	pmic_register_interrupt_callback(INT_ACCDET_EINT,
					 accdet_eint_int_handler);
#endif
#else
...
void mt_accdet_pm_restore_noirq(void)
{
	...
	pmic_pwrap_write(ACCDET_RSV,
			 pmic_pwrap_read(ACCDET_RSV) | ACCDET_INPUT_MICP);
	pmic_pwrap_write(ACCDET_ADC_REG,
			 pmic_pwrap_read(ACCDET_ADC_REG) | RG_EINTCONFIGACCDET);
	...
	case 1: /* AB=1 */
		...
		break;
	case 3: /* AB=3 */
		/* send input event before cable_type swutch to no_device, which
		 * is invalid type for input
		 */
		send_accdet_status_event(cable_type, 0);
		cable_type = NO_DEVICE;
		accdet_status = PLUG_OUT;
