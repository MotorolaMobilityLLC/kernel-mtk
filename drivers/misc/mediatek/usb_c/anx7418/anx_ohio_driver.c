...
static bool do_chip_detect = true;
...
*/
	...
	while (retry_count) {
		...
		/* check connect DTB */
		if (do_chip_detect) {
			do_chip_detect = false;
			if (ohio_read_reg(OHIO_SLVAVE_I2C_ADDR, VENDOR_ID_L, (unchar *)(&i)) < 0) {
				/* disable_irq(cbl_det_irq); */
				anx_printk(K_INFO, "Please check DTB connected!\n");
				return;
			}
		}
		...
		if (atomic_read(&ohio_sys_is_ready) != 1) {
			anx_printk(K_INFO, "EEPROM load timeout, retry!\n");
			/*pull down reset chip*/
			pinctrl_select_state(g_exttypec->pinctrl, g_exttypec->pin_cfg->rst_n_low);
			mdelay(1);
			pinctrl_select_state(g_exttypec->pinctrl, g_exttypec->pin_cfg->pwr_en_low);
			mdelay(1);
			continue;
		}
