...
static void fusb301_snk_detected(struct fusb301_chip *chip)
{
	...
	if ((!chip->triedsnk) &&
		 ...
		 */
		//power_supply_set_usb_otg(chip->usb_psy, true);//remove by wengjun1
...
static void fusb301_detach(struct fusb301_chip *chip)
{
	...
	case FUSB_STATE_ATTACHED_SRC:
		//power_supply_set_usb_otg(chip->usb_psy, false);//remove by wengjun1
...
/*
	for (i = 0 ; i < 5; i++) {
		...
		ret += printk("from 0x%02x read 0x%02x\n, from 0x%02x read 0x%02x\n",
							start_reg[(i*2)],
							(rc & 0xFF),
							start_reg[(i*2)+1],
							((rc >> 8) & 0xFF));
