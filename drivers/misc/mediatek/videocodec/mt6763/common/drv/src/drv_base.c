			...
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum;
				...
				if (oal_hw_context[i].u4VCodecThreadID[j] ==
					...
					MODULE_MFV_PR_DEBUG(
						"[VCODEC][%s]\n", __func__);
					MODULE_MFV_PR_DEBUG(
						"Lookup curr HW Locker is ObjId %d in index%d\n",
							    curr_tid, i);
					return i;
				}
			}
...
VAL_INT32_T search_HWLockSlot_ByHandle(VAL_ULONG_T ulpa, VAL_HANDLE_T handle)
{
	...
	/* dump debug info */
	MODULE_MFV_PR_DEBUG("%s", __func__);
			     ...
			     VAL_UINT32_T *a_prIndex)
{
	...
	for (i = 0; i < a_prVcodecThreadID.u4VCodecThreadNum; i++) {
		MODULE_MFV_PR_DEBUG(
			"[VCODEC][%s] VCodecThreadNum = %d, VCodecThreadID = %d\n",
				_func__, a_prVcodecThreadID.u4VCodecThreadNum,
...
struct VAL_VCODEC_OAL_HW_CONTEXT_T *freeCurr_HWLockSlot(VAL_ULONG_T ulpa)
{
	...
	pr_debug(
		"[VCODEC][ERROR] %s can't find pid in HWLockSlot\n",
		__func__);
	return 0;
}
			    ...
			    VAL_UINT32_T *a_pu4VCodecThreadID)
{
	...
	MODULE_MFV_PR_DEBUG(
		"[VCODEC] %s +, KVA = 0x%lx, KPA = 0x%lx, Size = 0x%lx\n",
	...
	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if ((grNonCacheMemoryList[u4I].ulKVA == -1L) &&
			...
			MODULE_MFV_PR_DEBUG(
			    "[VCODEC] %s index = %d, VCodecThreadNum = %d, curr_tid = %d\n",
			    __func__, u4I, a_u4VCodecThreadNum, current->pid);
			...
			for (u4J = 0;
				...
				MODULE_MFV_PR_DEBUG(
				    "[VCODEC][%s]VCodecThreadNum = %d, VCodecThreadID = %d\n",
				    __func__, grNonCacheMemoryList[u4I]
					.u4VCodecThreadNum,
				    grNonCacheMemoryList[u4I]
					.u4VCodecThreadID[u4J]);
			}
...
void Force_Free_NonCacheMemoryList(VAL_UINT32_T a_u4Tid)
{
	...
	MODULE_MFV_PR_DEBUG(
	    "[VCODEC] %s +, curr_id = %d", a_u4Tid, __func__);
	...
	MODULE_MFV_PR_DEBUG(
	    "[VCODEC] %s -, curr_id = %d", __func__, a_u4Tid);
}
...
VAL_ULONG_T Search_NonCacheMemoryList_By_KPA(VAL_ULONG_T a_ulKPA)
{
	...
	MODULE_MFV_PR_DEBUG("[VCODEC] %s +, KPA = 0x%lx, ulVA_Offset = 0x%lx\n",
	...
	if (u4I == VCODEC_MULTIPLE_INSTANCE_NUM_x_10) {
		pr_debug(
			"[VCODEC][ERROR] CAN'T Find %s, Address is not find!!\n",
			__func__);
