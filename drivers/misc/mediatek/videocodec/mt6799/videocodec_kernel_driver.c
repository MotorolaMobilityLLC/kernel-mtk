...
void SendDvfsRequest(int level)
{
	...
	if (level == MMDVFS_VOLTAGE_LOW) {
		...
		MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] SendDvfsRequest(MMDVFS_FINE_STEP_OPP1)");
#ifdef CONFIG_MTK_SMI_EXT
	...
	if (gMMDFVFSMonitorStarts == VAL_TRUE) {
		...
		if (gMMDFVFSMonitorCounts > MONITOR_START_MINUS_1) {
			if (gFirstDvfsLock == VAL_TRUE) {
				...
				/* MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] LOCK 1 start monitor instance = */
...
void vdec_power_on(void)
{
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][vdec_power_on] smi_clk_prepare SMI_LARB_VDECSYS failed, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][vdec_power_on] smi_clk_enable SMI_LARB_VDECSYS failed, ret = %d\n",
		ret);
	}
	...
	ret = clk_prepare_enable(clk_MT_CG_VDEC_LARB1);
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_VDEC_LARB1 not enabled, ret = %d\n", ret);
	}
}
...
void venc_power_on(void)
{
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][venc_power_on] smi_clk_prepare SMI_LARB_VENCSYS failed, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][venc_power_on] smi_clk_enable SMI_LARB_VENCSYS failed, ret = %d\n",
		ret);
	}
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	clk_MT_CG_VDEC_LARB1 = devm_clk_get(&dev->dev, "MT_CG_VDEC_LARB1");
	if (IS_ERR(clk_MT_CG_VDEC_LARB1)) {
		MODULE_MFV_PR_ERR("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VDEC_LARB1\n");
		return PTR_ERR(clk_MT_CG_VDEC_LARB1);
	}
...
static const struct of_device_id vcodec_of_match[] = {
	{ .compatible = "mediatek,mt6799-vdec_gcon", },
	{/* sentinel */}
};
...
static int __init vcodec_driver_init(void)
{
	...
	}
	{
		...
		node = of_find_compatible_node(NULL, NULL, "mediatek,mt6799-vdec_gcon");
...
MODULE_DESCRIPTION("Whitney Vcodec Driver");
MODULE_LICENSE("GPL");
