...
VAL_INT32_T search_HWLockSlot_ByTID(VAL_ULONG_T ulpa, VAL_UINT32_T curr_tid)
{
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM; i++) {
		if (oal_hw_context[i].u4VCodecThreadNum !=
		    VCODEC_THREAD_MAX_NUM) {
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum;
			     j++) {
				if (oal_hw_context[i].u4VCodecThreadID[j] ==
				    curr_tid) {
					MODULE_MFV_PR_DEBUG("[VCODEC][search_"
							    "HWLockSlot_ByTID]"
							    "\n");
					MODULE_MFV_PR_DEBUG("Lookup curr HW "
							    "Locker is ObjId "
							    "%d in index%d\n",
							    curr_tid, i);
					return i;
...
VAL_INT32_T search_HWLockSlot_ByHandle(VAL_ULONG_T ulpa, VAL_HANDLE_T handle)
{
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM / 2; i++) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES: */
		MODULE_MFV_PR_DEBUG("[%d] 0x%lx", i,
				    oal_hw_context[i].pvHandle);
	}
						...
						VAL_UINT32_T tid)
{
		...
		if (oal_hw_context[i].u4VCodecThreadNum !=
			...
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum;
				...
				if (oal_hw_context[i].u4VCodecThreadID[j] ==
					...
					MODULE_MFV_PR_DEBUG("[VCODEC][setCurr_"
							    "HWLockSlot] "
							    "setCurr %d Slot\n",
							    i);
					return &oal_hw_context[i];
	...
	MODULE_MFV_PR_DEBUG(
	    "[VCODEC][ERROR] setCurr_HWLockSlot All %d Slots unavaliable\n",
			     ...
			     VAL_UINT32_T *a_prIndex)
{
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM; i++) {
		if (oal_hw_context[i].u4VCodecThreadNum !=
			...
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum;
				...
				MODULE_MFV_PR_DEBUG(
				    "Dump curr slot %d, ThreadID[%d] = %d\n", i,
				    j, oal_hw_context[i].u4VCodecThreadID[j]);
			}
		}
	}
	...
	for (i = 0; i < a_prVcodecThreadID.u4VCodecThreadNum; i++) {
		if (i < VCODEC_THREAD_MAX_NUM) {
			MODULE_MFV_PR_DEBUG(
			    "[VCODEC][setCurr_HWLockSlot_Thread_ID] "
			    "VCodecThreadNum = %d, VCodecThreadID = %d\n",
			...
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum;
				...
				for (k = 0;
					...
					if ((k < VCODEC_THREAD_MAX_NUM) &&
						...
						MODULE_MFV_PR_DEBUG(
						    "[VCODEC][setCurr_"
						    "HWLockSlot_Thread_ID]\n");
						MODULE_MFV_PR_DEBUG(
						    "Curr Already exist in %d "
						    "Slot\n",
						    i);
						*a_prIndex = i;
						return &oal_hw_context[i];
			...
			for (j = 0; j < a_prVcodecThreadID.u4VCodecThreadNum;
				...
				if (j < VCODEC_THREAD_MAX_NUM) {
					oal_hw_context[i].u4VCodecThreadID[j] =
						...
						.u4VCodecThreadID[j];
					MODULE_MFV_PR_DEBUG(
					    "[VCODEC][setCurr_HWLockSlot_"
					    "Thread_ID] setCurr %d Slot, %d\n",
					    i, oal_hw_context[i]
						   .u4VCodecThreadID[j]);
				}
			}
	...
	{
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] "
				    "setCurr_HWLockSlot_Thread_ID All %d Slots "
				    "unavaliable\n",
				    VCODEC_MULTIPLE_INSTANCE_NUM);
		oal_hw_context[0].u4VCodecThreadNum =
		    a_prVcodecThreadID.u4VCodecThreadNum;
		for (i = 0; i < oal_hw_context[0].u4VCodecThreadNum; i++) {
			if (i < VCODEC_THREAD_MAX_NUM) {
...
struct VAL_VCODEC_OAL_HW_CONTEXT_T *freeCurr_HWLockSlot(VAL_ULONG_T ulpa)
{
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM; i++) {
		if (oal_hw_context[i].ObjId == ulpa) {
			...
			MODULE_MFV_PR_DEBUG(
			    "[VCODEC] freeCurr_HWLockSlot %d Slot\n", i);
			return &oal_hw_context[i];
		}
	}

	MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] freeCurr_HWLockSlot can't find "
			    "pid in HWLockSlot\n");
	return 0;
}
			    ...
			    VAL_UINT32_T *a_pu4VCodecThreadID)
{
	...
	MODULE_MFV_PR_DEBUG("[VCODEC] Add_NonCacheMemoryList +, KVA = 0x%lx, "
			    "KPA = 0x%lx, Size = 0x%lx\n",
	...
	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if ((grNonCacheMemoryList[u4I].ulKVA == -1L) &&
			...
			MODULE_MFV_PR_DEBUG(
			    "[VCODEC] Add_NonCacheMemoryList index = %d, "
			    "VCodecThreadNum = %d, curr_tid = %d\n",
			    u4I, a_u4VCodecThreadNum, current->pid);

			grNonCacheMemoryList[u4I].u4VCodecThreadNum =
			    a_u4VCodecThreadNum;
			for (u4J = 0;
			     u4J < grNonCacheMemoryList[u4I].u4VCodecThreadNum;
			     u4J++) {
				grNonCacheMemoryList[u4I]
				    .u4VCodecThreadID[u4J] =
				    *(a_pu4VCodecThreadID + u4J);
				MODULE_MFV_PR_DEBUG(
				    "[VCODEC][Add_NonCacheMemoryList] "
				    "VCodecThreadNum = %d, VCodecThreadID = "
				    "%d\n",
				    grNonCacheMemoryList[u4I].u4VCodecThreadNum,
				    grNonCacheMemoryList[u4I]
					.u4VCodecThreadID[u4J]);
	...
	if (u4I == VCODEC_MULTIPLE_INSTANCE_NUM_x_10) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES: */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] CAN'T ADD "
				    "Add_NonCacheMemoryList, List is FULL!!\n");
	}
...
void Free_NonCacheMemoryList(VAL_ULONG_T a_ulKVA, VAL_ULONG_T a_ulKPA)
{
	...
	MODULE_MFV_PR_DEBUG(
	    "[VCODEC] Free_NonCacheMemoryList +, KVA = 0x%lx, KPA = 0x%lx\n",
	    a_ulKVA, a_ulKPA);

	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if ((grNonCacheMemoryList[u4I].ulKVA == a_ulKVA) &&
		    (grNonCacheMemoryList[u4I].ulKPA == a_ulKPA)) {
			MODULE_MFV_PR_DEBUG("[VCODEC] Free "
					    "Free_NonCacheMemoryList index = "
					    "%d\n",
					    u4I);
			grNonCacheMemoryList[u4I].u4VCodecThreadNum =
			    VCODEC_THREAD_MAX_NUM;
			for (u4J = 0; u4J < VCODEC_THREAD_MAX_NUM; u4J++) {
				 ...
				 * style, WARNING:BRACES: */
				grNonCacheMemoryList[u4I]
				    .u4VCodecThreadID[u4J] = 0xffffffff;
			}
	...
	if (u4I == VCODEC_MULTIPLE_INSTANCE_NUM_x_10) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES: */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] CAN'T Free "
				    "Free_NonCacheMemoryList, Address is not "
				    "find!!\n");
	}
...
void Force_Free_NonCacheMemoryList(VAL_UINT32_T a_u4Tid)
{
	...
	MODULE_MFV_PR_DEBUG(
	    "[VCODEC] Force_Free_NonCacheMemoryList +, curr_id = %d", a_u4Tid);

	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if (grNonCacheMemoryList[u4I].u4VCodecThreadNum !=
		    VCODEC_THREAD_MAX_NUM) {
			for (u4J = 0;
			     u4J < grNonCacheMemoryList[u4I].u4VCodecThreadNum;
			     u4J++) {
				if (grNonCacheMemoryList[u4I]
					...
					MODULE_MFV_PR_DEBUG(
					    "[VCODEC][WARNING] "
					    "Force_Free_NonCacheMemoryList\n");
					MODULE_MFV_PR_DEBUG(
					    "index = %d, tid = %d, KVA = "
					    "0x%lx, KPA = 0x%lx, Size = %lu\n",
					    u4I, a_u4Tid,
					...
					dma_free_coherent(
					    0, grNonCacheMemoryList[u4I].ulSize,
					    (void *)grNonCacheMemoryList[u4I]
						.ulKVA,
					    (dma_addr_t)
						grNonCacheMemoryList[u4I]
						    .ulKPA);
...
VAL_ULONG_T Search_NonCacheMemoryList_By_KPA(VAL_ULONG_T a_ulKPA)
{
	...
	MODULE_MFV_PR_DEBUG("[VCODEC] Search_NonCacheMemoryList_By_KPA +, KPA "
			    "= 0x%lx, ulVA_Offset = 0x%lx\n",
			    a_ulKPA, ulVA_Offset);

	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if (grNonCacheMemoryList[u4I].ulKPA ==
		    (a_ulKPA - ulVA_Offset)) {
			MODULE_MFV_PR_DEBUG("[VCODEC] Find "
					    "Search_NonCacheMemoryList_By_KPA "
					    "index = %d\n",
					    u4I);
			break;
		}
	...
	if (u4I == VCODEC_MULTIPLE_INSTANCE_NUM_x_10) {
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] CAN'T Find "
				    "Search_NonCacheMemoryList_By_KPA, Address "
				    "is not find!!\n");
		return grNonCacheMemoryList[0].ulKVA + ulVA_Offset;
	}

	MODULE_MFV_PR_DEBUG(
	    "[VCODEC] Search_NonCacheMemoryList_By_KPA, ulVA = 0x%lx -\n",
