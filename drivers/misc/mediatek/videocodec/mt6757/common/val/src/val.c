					...
					unsigned int a_u4ParamSize)
{
	...
	if (pWaitQueue != VAL_NULL) {
		...
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] Event wait Queue failed to create\n");
	}
	if (pFlag != VAL_NULL) {
		a_prParam->pvReserved = (void *)pFlag;
		*((unsigned char *)a_prParam->pvReserved) = VAL_FALSE;
	} else {
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] Event flag failed to create\n");
	}
					...
					unsigned int a_u4ParamSize)
{
	...
	/* MODULE_MFV_PR_DEBUG("[MFV]eVideoWaitEvent,a_prParam->u4TimeoutMs=%d,
	 * timeout = %ld\n",
	...
	i4Ret = wait_event_interruptible_timeout(
		...
		*((unsigned char *)
		...
		timeout_jiff);
	if (i4Ret == 0) {
		MODULE_MFV_PR_DEBUG("[VCODEC] eVideoWaitEvent timeout: %d ms",
				a_prParam->u4TimeoutMs);
		status = VAL_RESULT_INVALID_ISR; /* timeout */
	} else if (-ERESTARTSYS == i4Ret) {
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC] eVideoWaitEvent wake up by ERESTARTSYS");
		status = VAL_RESULT_RESTARTSYS;
	} else if (i4Ret > 0) {
		status = VAL_RESULT_NO_ERROR;
	} else {
		MODULE_MFV_PR_DEBUG("[VCODEC] eVideoWaitEvent wake up by %ld",
				i4Ret);
		status = VAL_RESULT_NO_ERROR;
	}
	*((unsigned char *)a_prParam->pvReserved) = VAL_FALSE;
...
enum VAL_RESULT_T eVideoWaitEventNoTimeout(struct VAL_EVENT_T *a_prParam,
		unsigned int a_u4ParamSize, char a_bKeepCond)
{
	wait_queue_head_t *pWaitQueue;
	long i4Ret;
	enum VAL_RESULT_T status;
	...
	pWaitQueue = (wait_queue_head_t *)a_prParam->pvWaitQueue;
	/* MODULE_MFV_PR_DEBUG("[MFV]eVideoWaitEventNoTimeOut,a_prParam->u4TimeoutMs=%d,
	 * timeout = %ld\n",
	 * a_prParam->u4TimeoutMs);
	 */
	i4Ret = wait_event_interruptible(
	    *pWaitQueue,
	    *((unsigned char *)
		  a_prParam->pvReserved) /*g_mflexvideo_interrupt_handler */);
	if (i4Ret == 0) {
		/* MODULE_MFV_PR_DEBUG("[VCODEC] eVideoWaitEvent timeout: %d
		 * ms", a_prParam->u4TimeoutMs); */
	...
	if (a_bKeepCond == VAL_FALSE) {
		/* Clear condition so next time wait won't be returned
		 * immediately */
		*((unsigned char *)a_prParam->pvReserved) = VAL_FALSE;
	}
	return status;
				...
				unsigned int a_u4ParamSize)
{
	...
	if (a_prParam->pvReserved != VAL_NULL) {
		...
		*((unsigned char *)a_prParam->pvReserved) = VAL_TRUE;
		...
		MODULE_MFV_PR_DEBUG(
			"[VCODEC][ERROR]Event flag should not be null\n");
	}
	if (pWaitQueue != VAL_NULL) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES: */
		...
		MODULE_MFV_PR_DEBUG(
			"[VCODEC][ERROR]Wait Queue should not be null\n");
	}
	return VAL_RESULT_NO_ERROR;
}
					...
					unsigned int a_u4ParamSize)
{
	...
	if (pLock != VAL_NULL) {
		...
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR]Enable to create mutex!\n");
	}
	/* init_MUTEX(pLock); */
	sema_init(pLock, 1);
	return VAL_RESULT_NO_ERROR;
