...
VAL_ULONG_T KVA_VDEC_MISC_BASE, KVA_VDEC_VLD_BASE, KVA_VDEC_BASE,
	KVA_VDEC_GCON_BASE;
...
void SendDvfsRequest(int level)
{
	...
	if (level == MMDVFS_VOLTAGE_LOW_LOW) {
		MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] Send Dvfs Request(MMDVFS_VOLTAGE_LOW_LOW)");
		ret = mmdvfs_set_step(SMI_BWC_SCEN_VP, MMDVFS_VOLTAGE_LOW_LOW);
	} else {
		MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] Send Dvfs Request(MMDVFS_VOLTAGE_DEFAULT_STEP)");
		ret = mmdvfs_set_step(SMI_BWC_SCEN_VP,
	...
	if (ret != 0) {
		 ...
		 */
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][MMDVFS_VDEC] OOPS: mmdvfs_set_step error!");
	}
}
...
/* enable_clock(MT_CG_INFRA_L2C_SRAM, "VDEC"); */
	...
	if (ret) {
		...
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][vdec power on] clk_MT_SCP_SYS_DIS is not enabled, ret = %d \n",
				    ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][vdec power on] clk_MT_CG_DISP0_SMI_COMMON is not enabled, ret = %d\n",
				    ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][vdec_power_ on] clk_MT_SCP_SYS_VDE is not enabled, ret = %d \n",
				    ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][vdec _power_on] clk_MT_CG_VDEC0_VDEC not enabled, ret = %d \n",
				    ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][vdec_ power_on] clk_MT_CG_VDEC1_LARB not enabled, ret = %d \n",
				    ret);
	}
#endif
}
...
void venc_power_on(void)
{
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][venc _power_on] clk_MT_SCP_SYS_DIS is not enabled, ret = %d \n",
				    ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][venc _power_on] clk_MT_CG_DISP0_SMI_COMMON is not enabled, ret = %d \n",
				    ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][venc _power_on] clk_MT_SCP_SYS_VEN is not enabled, ret = %d \n",
				    ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][ve nc_power_on] clk_MT_CG_VENC_VENC is not enabled, ret = %d \n",
				    ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR][venc _power_on] clk_MT_CG_VENC_LARB is not enabled, ret = %d \n",
				    ret);
	}
#endif

	MODULE_MFV_PR_DEBUG("[VCODEC] venc _power_on -\n");
}
...
void venc_power_off(void)
{
	...
	if (gu4VencPWRCounter == 0) {
		...
		gu4VencPWRCounter--;
		MODULE_MFV_PR_DEBUG("[VCODEC] venc_ power_off +\n");
#ifdef CONFIG_MTK_CLKMGR
...
void dec_isr(void)
{
	...
	if ((u4CgStatus & 0x10) != 0) {
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] DEC ISR, VDEC active is not 0x0 (0x%08x)",
		    u4CgStatus);
		return;
	}
	...
	if ((u4DecDoneStatus & (0x1 << 16)) != 0x10000) {
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] DEC ISR, Decode done status is not 0x1 (0x%08x)",
				    u4DecDoneStatus);
		return;
	}
	...
	if (u4TempDecISRCount != u4TempLockDecHWCount) {
		 ...
		 * WARNING:BRACES:
		 */
		/* MODULE_MFV_PR_DEBUG("[INFO] Dec ISRCount: 0x%x,
		 * LockHWCount:0x%x\n",
...
void enc_isr(void)
{
	...
	if (grVcodecEncHWLock.eDriverType ==
		...
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][enc_isr] VAL_DRIVER_TYPE_HEVC_ENC!!\n");
	} else {
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] Invalid lock holder driver type = %d\n",
				   ...
				   VAL_UINT32_T FirstUseDecHW)
{
	...
	if (FirstUseDecHW != 1) {
		...
		if (grVcodecDecHWLock.pvHandle == 0) {
			 ...
			 * WARNING:BRACES:
			 */
			MODULE_MFV_PR_DEBUG("[WARNING] VCODEC_LOCKHW, maybe mediaserver restart before, please check\n");
		} else {
			/* Add one line comment for avoid kernel coding style,
			 * WARNING:BRACES:
			 */
			MODULE_MFV_PR_DEBUG("[WARNING] VCODEC_LOCKHW, someone use HW, and check timeout value\n");
		}
				   ...
				   VAL_UINT32_T FirstUseEncHW)
{
	...
	if (FirstUseEncHW != 1) {
		...
		if (grVcodecEncHWLock.pvHandle == 0) {
			...
			MODULE_MFV_PR_DEBUG("[WARNING] VCODEC_LOCKHW, someone use HW, and check timeout value %d \n",
					    gLockTimeOutCount);
			++gLockTimeOutCount;
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (grVcodecDecHWLock.pvHandle ==
				...
				(VAL_ULONG_T)rHWLock.pvHandle)) {
				MODULE_MFV_PR_DEBUG("[WARNING] VCODEC_LOCKHW, one decoder instance try to lock twice\n");
				MODULE_MFV_PR_DEBUG(
			...
			if (eValRet == VAL_RESULT_INVALID_ISR) {
				...
				MODULE_MFV_PR_DEBUG("[WARNING] VCODEC_LOCKHW, VAL_RESULT_RESTARTSYS return when HWLock!!\n");
				return -ERESTARTSYS;
			}
...
"LockInstance = 0x%lx CurrentTID = %d,rLockedTime(s, us) = %d, %d\n",
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (grVcodecEncHWLock.pvHandle ==
			    (VAL_VOID_T *)pmem_user_v2p_video(
				(VAL_ULONG_T)rHWLock.pvHandle)) {
				MODULE_MFV_PR_DEBUG("[WARNING] VCODEC_LOCKHW, one encoder instance try to lock twice\n");
				MODULE_MFV_PR_DEBUG(
			...
			mutex_lock(&VencHWLock);
			if (grVcodecEncHWLock.pvHandle ==
			    0) { /* No process use HW, so current process can
				  *  use HW
				  */
...
static long vcodec_waitisr(unsigned long arg)
{
	...
	if (val_isr.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		if (bLockedHW == VAL_FALSE) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_WAITISR, DO NOT have HWLock, so return fail\n");
			return -EFAULT;
		}
		...
		if (eValRet == VAL_RESULT_INVALID_ISR) {
			...
			MODULE_MFV_PR_DEBUG("[WARNING] VCODEC_WAITISR, VAL_RESULT_RESTARTSYS return when WAITISR!!\n");
			return -ERESTARTSYS;
		}
		...
		if (val_isr.u4IrqStatusNum > 0) {
			...
			if (ret) {
				MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_WAITISR, copy_to_user failed: %lu\n",
						    ret);
				return -EFAULT;
			}
		}
				  ...
				  unsigned long arg)
{
	...
	case VCODEC_ALLOC_NON_CACHE_BUFFER: {
		MODULE_MFV_PR_DEBUG(
		    "VCODEC_ALLOC_NON_CACHE_BUFFER [EMPTY] + tid = %d\n",
		    current->pid);
		MODULE_MFV_PR_DEBUG(
		    "VCODEC_ALLOC_NON_CACHE_BUFFER [EMPTY] - tid = %d\n",
		    current->pid);
	...
	case VCODEC_FREE_NON_CACHE_BUFFER: {
		MODULE_MFV_PR_DEBUG(
		    "VCODEC_FREE_NON_CACHE_BUFFER [EMPTY] + tid = %d\n",
		    current->pid);
		MODULE_MFV_PR_DEBUG(
		    "VCODEC_FREE_NON_CACHE_BUFFER [EMPTY] - tid = %d\n",
		    current->pid);
	...
	case VCODEC_INC_DEC_EMI_USER: {
		...
		ret = copy_to_user(user_data_addr, &gu4DecEMICounter,
				   sizeof(VAL_UINT32_T));
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_INC_DEC_EMI_USER, copy_to_user failed: %lu\n",
					    ret);
	...
	case VCODEC_DEC_DEC_EMI_USER: {
		...
		ret = copy_to_user(user_data_addr, &gu4DecEMICounter,
				   sizeof(VAL_UINT32_T));
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_DEC_DEC_EMI_USER, copy_to_user failed: %lu\n",
					    ret);
			mutex_unlock(&DecEMILock);
			return -EFAULT;
		}
	...
	case VCODEC_INC_ENC_EMI_USER: {
		...
		ret = copy_to_user(user_data_addr, &gu4EncEMICounter,
				   sizeof(VAL_UINT32_T));
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_INC_ENC_EMI_USER, copy_to_user failed: %lu\n",
					    ret);
	...
	case VCODEC_DEC_ENC_EMI_USER: {
		...
		ret = copy_to_user(user_data_addr, &gu4EncEMICounter,
				   sizeof(VAL_UINT32_T));
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_DEC_ENC_EMI_USER, copy_to_user failed: %lu\n",
					    ret);
			mutex_unlock(&EncEMILock);
			return -EFAULT;
		}
	...
	case VCODEC_INC_PWR_USER: {
		...
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_INC_PWR_USER, "
					    "copy_from_user failed: %lu\n",
					    ret);
			return -EFAULT;
		}
		...
		if (rPowerParam.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {
			...
			if (gu4L2CCounter == 1) {
				if (config_L2(0)) {
					MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] Switch L2C size to 512K failed\n");
	...
	case VCODEC_DEC_PWR_USER: {
		...
		ret = copy_from_user(&rPowerParam, user_data_addr,
				     sizeof(struct VAL_POWER_T));
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_DEC_PWR_USER, copy_from_user failed: %lu\n",
					    ret);
			return -EFAULT;
		}
		...
		if (rPowerParam.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {
			...
			if (gu4L2CCounter == 0) {
				if (config_L2(1)) {
					MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] Switch L2C size to 0K failed\n");
					mutex_unlock(&L2CLock);
					return -EFAULT;
				}
...
/* TODO: */
		...
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_GET_CPU_LOADING_INFO, copy_to_user failed: %lu\n",
					    ret);
			return -EFAULT;
		}
	...
	case VCODEC_GET_CORE_LOADING: {
		...
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_GET_CORE_LOADING, copy_from_user failed: %lu\n",
					    ret);
			return -EFAULT;
		}
		...
		ret = copy_to_user(user_data_addr, &rTempCoreLoading,
				   sizeof(struct VAL_VCODEC_CORE_LOADING_T));
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_GET_CORE_LOADING, copy_to_user failed: %lu\n",
					    ret);
			return -EFAULT;
		}
		MODULE_MFV_PR_DEBUG("VCODEC_GET_CORE_LOADING - - tid = %d\n",
	...
	case VCODEC_GET_CORE_NUMBER: {
		...
		ret =
		    copy_to_user(user_data_addr, &temp_nr_cpu_ids, sizeof(int));
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_GET_CORE_NUMBER, copy_to_user failed: %lu\n",
					    ret);
			return -EFAULT;
		}
		MODULE_MFV_PR_DEBUG("VCODEC_GET_CORE_NUMBER - - tid = %d\n",
	...
	case VCODEC_SET_CPU_OPP_LIMIT: {
		...
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_SET_CPU_OPP_LIMIT, copy_from_user failed: %lu\n",
					    ret);
			return -EFAULT;
		}
		MODULE_MFV_PR_DEBUG(
		 ...
		 *other: FAIL
		 * if (ret) {
		 * MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] cpu_opp_limit failed:
		 *%lu\n", ret);
		 *	return -EFAULT;
		 * }
		 */
	...
	case VCODEC_MB: {
		/* Allow user of kernel driver to issue mb() after register
		 * access*/
		mb();
	} break;
	...
	case VCODEC_SET_LOG_COUNT: {
		...
		ret = copy_from_user(&rIncLogCount, user_data_addr,
				     sizeof(VAL_BOOL_T));
		if (ret) {
			MODULE_MFV_PR_DEBUG("[ERROR] VCODEC_SET_LOG_COUNT, copy_from_user failed: %lu\n",
					    ret);
...
static int vcodec_open(struct inode *inode, struct file *file)
{
	...
	MODULE_MFV_PR_DEBUG("vcodec_open pid = %d, Driver_Open_Count %d\n",
	...
	if (Driver_Open_Count == 0) {
		...
		if (gu4L2CCounter != 0) {
			...
			if (config_L2(1)) {
				 ...
				 * style, WARNING:BRACES:
				 */
				MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] restore L2 settings failed\n");
			}
		}
		gu4L2CCounter = 0;
		mutex_unlock(&L2CLock);
...
static int vcodec_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (((length > VENC_REGION) || (pfn < VENC_BASE) ||
		...
		if (u4I == VCODEC_MULTIPLE_INSTANCE_NUM_x_10) {
			MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] mmap region error: Length(0x%lx), pfn(0x%lx)\n",
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (ret) {
		 ...
		 */
		MODULE_MFV_PR_DEBUG(
		    "[ERROR] Can't Get Major number for VCodec Device\n");
	}
	...
	if (ret) {
		 ...
		 * WARNING:BRACES:
		 */
		MODULE_MFV_PR_DEBUG("[ERROR] Can't add Vcodec Device\n");
	}
	...
	if (IS_ERR(vcodec_class)) {
		...
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] Unable to create class, err = %d", ret);
		return ret;
	}
	...
	if (request_irq(VDEC_IRQ_ID, (irq_handler_t)video_intr_dlr,
		 ...
		 */
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] error to request dec irq\n");
	} else {
	...
	if (request_irq(VENC_IRQ_ID, (irq_handler_t)video_intr_dlr2,
		 ...
		 */
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] error to request enc irq\n");
	} else {
	...
	clk_MT_CG_DISP0_SMI_COMMON =
	    devm_clk_get(&dev->dev, "MT_CG_DISP0_SMI_COMMON");
	if (IS_ERR(clk_MT_CG_DISP0_SMI_COMMON)) {
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_DISP0_SMI_COMMON\n");
	...
	if (IS_ERR(clk_MT_CG_VDEC0_VDEC)) {
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VDEC0_VDEC\n");
	...
	if (IS_ERR(clk_MT_CG_VDEC1_LARB)) {
		MODULE_MFV_PR_DEBUG("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VDEC1_LARB\n");
	...
	if (IS_ERR(clk_MT_CG_VENC_VENC)) {
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VENC_VENC\n");
	...
	if (IS_ERR(clk_MT_CG_VENC_LARB)) {
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VENC_LARB\n");
	...
	if (IS_ERR(clk_MT_SCP_SYS_DIS)) {
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] Unable to devm_clk_get MT_SCP_SYS_DIS\n");
...
static int __init vcodec_driver_init(void)
{
	 ...
	 * (VAL_ULONG_T)ioremap(VENC_IRQ_STATUS_addr, 4); */
	...
	mutex_unlock(&DecHWLockEventTimeoutLock);
	eValHWLockRet =
		eVideoCreateEvent(&DecHWLockEvent, sizeof(struct VAL_EVENT_T));
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES:
		 */
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] create dec hwlock event error\n");
	}
	...
	mutex_unlock(&EncHWLockEventTimeoutLock);
	eValHWLockRet =
		eVideoCreateEvent(&EncHWLockEvent, sizeof(struct VAL_EVENT_T));
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES:
		 */
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] create enc hwlock event error\n");
	}
...
/* [TODO] iounmap the following? */
	...
	eValHWLockRet =
		eVideoCloseEvent(&DecHWLockEvent, sizeof(struct VAL_EVENT_T));
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES:
		 */
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] close dec hwlock event error\n");
	}

	eValHWLockRet =
		eVideoCloseEvent(&EncHWLockEvent, sizeof(struct VAL_EVENT_T));
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES:
		 */
		MODULE_MFV_PR_DEBUG(
		    "[VCODEC][ERROR] close enc hwlock event error\n");
	}
...
MODULE_DESCRIPTION("MT6797 Vcodec Driver");
MODULE_LICENSE("GPL");
