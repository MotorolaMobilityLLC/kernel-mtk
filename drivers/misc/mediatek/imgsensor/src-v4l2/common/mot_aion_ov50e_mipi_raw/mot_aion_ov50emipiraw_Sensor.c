...
static kal_uint16 _i2c_data_ov50e[_I2C_BUF_SIZE];
static unsigned int _size_to_write;
static bool _is_seamless = false;
#define PD_PIX_2_EN 0

#define PFX "mot_aion_ov50e"
static int mot_ov50e_camera_debug = 0;
module_param(mot_ov50e_camera_debug,int, 0644);

static int mot_ov50e_xtalk_en = 0;
module_param(mot_ov50e_xtalk_en,int, 0644);

#define LOG_INF(format, args...)        do { if (mot_ov50e_camera_debug ) { pr_err(PFX "[%s %d] " format, __func__, __LINE__, ##args); } } while(0)
...
static int remosaic_mode_ov50e =0;
static int settledebug_ov50e = 0x10;
module_param(settledebug_ov50e,int, 0644);
MODULE_PARM_DESC(settledebug_ov50e, "settledebug_ov50e");
...
#define OV50E_EEPROM_IIC_ADDR 0xA0
#define OV50E_SENSOR_IIC_ADDR 0x20
#define OV50E_XTLK_EEPROM_OFFSET 0x150F
#define OV50E_XTLK_REG_OFFSET 0x71F0
#define OV50E_XTLK_BYTES 3568
static u8 ov50e_xtlk_buf[OV50E_XTLK_BYTES];
...
static void set_dummy(struct subdrv_ctx *ctx)
{
       ...
       if (!_is_seamless) {
               memset(_i2c_data_ov50e, 0x0, sizeof(_i2c_data_ov50e));
               _size_to_write = 0;
               _i2c_data_ov50e[_size_to_write++] = 0x380e;
               _i2c_data_ov50e[_size_to_write++] = ctx->frame_length >> 8;
               _i2c_data_ov50e[_size_to_write++] = 0x380f;
               _i2c_data_ov50e[_size_to_write++] = ctx->frame_length & 0xFF;
               table_write_cmos_sensor(ctx, _i2c_data_ov50e,
               _size_to_write);
       } else {
               _i2c_data_ov50e[_size_to_write++] = 0x3840;
               _i2c_data_ov50e[_size_to_write++] = ctx->frame_length >> 16;
               _i2c_data_ov50e[_size_to_write++] = 0x380e;
               _i2c_data_ov50e[_size_to_write++] = ctx->frame_length >> 8;
               _i2c_data_ov50e[_size_to_write++] = 0x380f;
               _i2c_data_ov50e[_size_to_write++] = ctx->frame_length & 0xFF;
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	if (!_is_seamless) {
               ...
               _i2c_data_ov50e[_size_to_write++] = 0x380f;
               _i2c_data_ov50e[_size_to_write++] = ctx->frame_length & 0xFF;
               _i2c_data_ov50e[_size_to_write++] = 0x3500;
               _i2c_data_ov50e[_size_to_write++] = (shutter >> 16) & 0xFF;
               _i2c_data_ov50e[_size_to_write++] = 0x3501;
               _i2c_data_ov50e[_size_to_write++] = (shutter >> 8) & 0xFF;
               _i2c_data_ov50e[_size_to_write++] = 0x3502;
               _i2c_data_ov50e[_size_to_write++] = (shutter)  & 0xFF;
               table_write_cmos_sensor(ctx, _i2c_data_ov50e,
               _size_to_write);
...
static kal_uint32 set_gain(struct subdrv_ctx *ctx, kal_uint32 gain)
{
	...
	if (!_is_seamless) {
               memset(_i2c_data_ov50e, 0x0, sizeof(_i2c_data_ov50e));
               _size_to_write = 0;
               _i2c_data_ov50e[_size_to_write++] = 0x03508;
               _i2c_data_ov50e[_size_to_write++] =  reg_gain >> 8;
               _i2c_data_ov50e[_size_to_write++] = 0x03509;
               _i2c_data_ov50e[_size_to_write++] =  reg_gain & 0xff;
               table_write_cmos_sensor(ctx, _i2c_data_ov50e,
               _size_to_write);
...
static void ov50e_qpd_xtalk_read(struct subdrv_ctx *ctx)
{
	int ret = 0;

	memset(ov50e_xtlk_buf, 0x0, sizeof(ov50e_xtlk_buf));
	LOG_INF("%s: xtalk read start",__func__);
	ret = adaptor_i2c_rd_p8(ctx->i2c_client, OV50E_EEPROM_IIC_ADDR >> 1, OV50E_XTLK_EEPROM_OFFSET, ov50e_xtlk_buf, sizeof(ov50e_xtlk_buf));
...
static void ov50e_qpd_calibration_apply(struct subdrv_ctx *ctx)
{
	...
	*/
	...
	#define XTALK_SECOND_SECTION_BYTES (OV50E_XTLK_BYTES-XTALK_FIRST_SECTION_BYTES)
	int ret = 0;

	if (!xtalk_ready) {//Try again if power on reading failed
		LOG_INF("%s: xtalk read retry.", __func__);
		ov50e_qpd_xtalk_read(ctx);
	}

	if (xtalk_ready) {
		ret = adaptor_i2c_wr_seq_p8(ctx->i2c_client, OV50E_SENSOR_IIC_ADDR >> 1, OV50E_XTLK_REG_OFFSET, ov50e_xtlk_buf, XTALK_FIRST_SECTION_BYTES);
		ret |= adaptor_i2c_wr_seq_p8(ctx->i2c_client, OV50E_SENSOR_IIC_ADDR >> 1, XTALK_SECOND_SECTION_START,
		                            &ov50e_xtlk_buf[XTALK_FIRST_SECTION_BYTES], XTALK_SECOND_SECTION_BYTES);
		if (ret < 0) {
...
static void sensor_init(struct subdrv_ctx *ctx)
{
	...
	if (mot_ov50e_xtalk_en) {//Settings that removed xtalk default data.
		...
		ov50e_qpd_calibration_apply(ctx);
	} else {
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	...
	if (!_is_seamless) {
		...
		memcpy((void *) (_i2c_data_ov50e + _size_to_write),
...
static void custom1_setting(struct subdrv_ctx *ctx)
{
	int _length = 0;
	LOG_INF("%s start\n", __func__);
	remosaic_mode_ov50e =1;
	_length = sizeof(addr_data_pair_custom1_ov50e) / sizeof(kal_uint16);
	if (!_is_seamless) {
		table_write_cmos_sensor(ctx,
		addr_data_pair_custom1_ov50e,
		_length);
	} else {
		LOG_INF("%s _is_seamless %d, _size_to_write %d\n",
			__func__, _is_seamless, _size_to_write);
		...
		memcpy((void *) (_i2c_data_ov50e + _size_to_write),
			addr_data_pair_custom1_ov50e,
			sizeof(addr_data_pair_custom1_ov50e));
		_size_to_write += _length;
	}
...
static void custom2_setting(struct subdrv_ctx *ctx)
{
	...
	LOG_INF("%s start\n", __func__);
	remosaic_mode_ov50e =0;
	_length = sizeof(addr_data_pair_custom2_ov50e) / sizeof(kal_uint16);
	if (!_is_seamless) {
		table_write_cmos_sensor(ctx,
		addr_data_pair_custom2_ov50e,
		_length);
	} else {
		LOG_INF("%s _is_seamless %d, _size_to_write %d\n",
			__func__, _is_seamless, _size_to_write);
		...
		memcpy((void *) (_i2c_data_ov50e + _size_to_write),
			addr_data_pair_custom2_ov50e,
			sizeof(addr_data_pair_custom2_ov50e));
		_size_to_write += _length;
	}
	LOG_INF("%s end\n", __func__);
}	/*	custom2_setting  */
...
static void custom3_setting(struct subdrv_ctx *ctx)
{
	...
	LOG_INF("%s start\n", __func__);
	remosaic_mode_ov50e =1;
	_length = sizeof(addr_data_pair_custom3_ov50e) / sizeof(kal_uint16);
	if (!_is_seamless) {
		table_write_cmos_sensor(ctx,
		addr_data_pair_custom3_ov50e,
		_length);
	} else {
		LOG_INF("%s _is_seamless %d, _size_to_write %d\n",
			__func__, _is_seamless, _size_to_write);
		...
		memcpy((void *) (_i2c_data_ov50e + _size_to_write),
			addr_data_pair_custom3_ov50e,
			sizeof(addr_data_pair_custom3_ov50e));
		_size_to_write += _length;
	}
	LOG_INF("%s end\n", __func__);
}	/*	custom2_setting  */
...
static void custom4_setting(struct subdrv_ctx *ctx)
{
	...
	LOG_INF("%s start\n", __func__);
	remosaic_mode_ov50e =1;
	_length = sizeof(addr_data_pair_custom4_ov50e) / sizeof(kal_uint16);
	if (!_is_seamless) {
		table_write_cmos_sensor(ctx,
		addr_data_pair_custom4_ov50e,
		_length);
	} else {
		LOG_INF("%s _is_seamless %d, _size_to_write %d\n",
			__func__, _is_seamless, _size_to_write);
		...
		memcpy((void *) (_i2c_data_ov50e + _size_to_write),
			addr_data_pair_custom4_ov50e,
			sizeof(addr_data_pair_custom4_ov50e));
		_size_to_write += _length;
	}
	LOG_INF("%s end\n", __func__);
}	/*	custom4_setting  */
...
static kal_uint32 return_sensor_id(struct subdrv_ctx *ctx)
{
	return ((read_cmos_sensor_8(ctx, 0x300a) << 16) |
		(read_cmos_sensor_8(ctx, 0x300b) << 8) | read_cmos_sensor_8(ctx, 0x300c)) + 3;
}
	...
	kal_uint32 shutter_2ndframe, kal_uint32 gain_2ndframe)
{
	...
	LOG_INF("%s %d, %d, %d, %d, %d sizeof(_i2c_data_ov50e) %d\n", __func__,
		scenario_id, shutter, gain, shutter_2ndframe, gain_2ndframe, sizeof(_i2c_data_ov50e));
	...
	for (k = 0; k < _size_to_write; k += 2)
		pr_debug("k = %d, 0x%04x , 0x%02x\n", k,  _i2c_data_ov50e[k], _i2c_data_ov50e[k+1]);
	...
	for (k = 0; k < _size_to_write; k += 2)
		LOG_INF("k = %d, 0x%04x , 0x%02x\n",
			k,  _i2c_data_ov50e[k], read_cmos_sensor_8(ctx, _i2c_data_ov50e[k]));
#endif
...
{
	csi_param->cphy_settle = settledebug_ov50e;
	return 0;
}
