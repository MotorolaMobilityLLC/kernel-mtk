#include "imx219mipi_otp.h"

UINT32 temp_CalGain = 0;
UINT32 temp_FacGain = 0; 
UINT16 temp_AFInf = 0;
UINT16 temp_AFMacro = 0;
UINT16 temp_ModuleID = 0;
...
static unsigned char get_otp_AFData(void)
{
	...
	if((AF_Flag&0x30) ==0x10){
		...
		OTP_LOG("IMX219_otp:AF Info is Invalid or empty\n");
		return -1;
	}
...
static unsigned char get_otp_lsc(void)
{
	...
	{
		OTP_LOG("Qtech IM219_OTP:Lsc data Invalid or empty\n");
		return -1;
	}
...
static unsigned char get_otp_wb(void)
{
	...
	if((nWBFlag & 0x30) == 0x10)//group2
	{
		...
		OTP_LOG("IM219_OTP:group2 get_otp_wb() current_r_Gr=0x%x\n",current_r_Gr);
		...
		OTP_LOG("---Qtech group2  current_b_Gr=0x%x\n",current_b_Gr);
		...
		current_r_Gr = (unsigned short)templ + (((unsigned short)temph)  << 8);
		current_r_Gr = current_r_Gr >> 6;
		OTP_LOG("IM219_OTP: group1 current_r_Gr=0x%x\n",current_r_Gr);

		temph = pWBInfoData[3];
		templ = pWBInfoData[4];
		current_b_Gr = (unsigned short)templ + (((unsigned short)temph)  << 8);
		current_b_Gr = current_b_Gr >> 6;
		OTP_LOG("---Qtech group1 current_b_Gr=0x%x\n",current_b_Gr);

		temph = pWBInfoData[5];
		templ = pWBInfoData[6];
		current_Gb_Gr = (unsigned short)templ + (((unsigned short)temph)  << 8);
		current_Gb_Gr = current_Gb_Gr >> 6;
		OTP_LOG("---Qtech group1 current_Gb_Gr=0x%x\n",current_Gb_Gr);

		temph = pWBInfoData[7];
		templ = pWBInfoData[8];
		golden_r_Gr= (unsigned short)templ + (((unsigned short)temph)  << 8);
		golden_r_Gr = golden_r_Gr >> 6;
		OTP_LOG("---Qtech group1 golden_r_Gr=0x%x\n",golden_r_Gr);

		temph = pWBInfoData[9];
		templ = pWBInfoData[10];
		golden_b_Gr= (unsigned short)templ + (((unsigned short)temph)  << 8);
		golden_b_Gr = golden_b_Gr >> 6;
		OTP_LOG("---Qtech group1 golden_b_Gr=0x%x\n",golden_b_Gr);

		temph = pWBInfoData[11];
		templ = pWBInfoData[12];
		golden_Gb_Gr= (unsigned short)templ + (((unsigned short)temph)  << 8);
		golden_Gb_Gr = golden_Gb_Gr >> 6;
		OTP_LOG("---Qtech group1 golden_Gb_Gr=0x%x\n",golden_Gb_Gr);
		current_R = pWBInfoData[13];
		current_B = pWBInfoData[14];
		current_Gr = pWBInfoData[15];
		current_Gb = pWBInfoData[16];
		OTP_LOG("IMX219_OTP:Group1 get_otp_wb:R=0x%x,B=0x%x,Gr=0x%x,Gb=0x%x\n",current_R,current_B,current_Gr,current_Gb);

		golden_R = pWBInfoData[17];
		golden_B = pWBInfoData[18];
		golden_Gr = pWBInfoData[19];
		golden_Gb = pWBInfoData[20];
		OTP_LOG("IMX219_OTP:Group1 get_otp_wb:Golden_R=0x%x,Golden_B=0x%x,Golden_Gr=0x%x,Golden_Gb=0x%x\n",golden_R,golden_B,golden_Gr,golden_Gb);

	}else{
		OTP_LOG("Qtech IM219_OTP:wb data Invalid or empty\n");
		return -1;
	}
	...
	{
         OTP_LOG("WB Data CheckSum is error");
	}
#endif
	return ret;
}

int read_otp_info(void)
{
	int ret = 0;

	if(get_otp_AFData() != 0)
	{
		printk("[%s]:IMX219_OTP:Get AF data Err\n",__func__);
		ret = -1;
	}

	if(get_otp_wb() != 0)
	{
		printk("[%s]:IMX219_OTP:Get WB data Err\n",__func__);
		ret = -1;
	}
...
int read_otp_awb_af_data(void)
{
    unsigned char OTP_Mid = 0x00;
	int ret = 0;

    OTP_Mid = get_otp_module_id();
	printk("[%s]:IMX219_OTP:OTP_Mid = %d\n",__func__,OTP_Mid);
...
int otp_update(void)	//only open Lsc autoload
{
	int ret = 0;
