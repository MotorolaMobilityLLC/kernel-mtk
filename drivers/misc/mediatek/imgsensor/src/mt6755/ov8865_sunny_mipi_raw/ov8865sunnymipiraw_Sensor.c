...
#include "ov8865sunnymipiraw_Sensor.h"
...
#define PFX "OV8865_SUNNY_CAMERA_SENSOR"
#define LOG_1 SENSORDB("OV8865_SUNNY,MIPI CAM\n")
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)
/****************************   Modify end    *******************************************/

//test code
#if 1
extern kal_uint8 Read_OV8865_MID_form_eeprom(void);
#else
static kal_uint8 Read_OV8865_MID_form_eeprom(void)
{
	return 0;
}
#endif
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			data_mid[0] = Read_OV8865_MID_form_eeprom();
			LOG_INF("[huangsh4]ov8865 ofim MID data_mid[0]=%x\n", data_mid[0]);
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			*sensor_id = *sensor_id +data_mid[0];
			LOG_INF("[huangsh4]ov8865 ofilm sensor id = 0x%x,data_mid[0]=0x%x, imgsensor_info.sensor_id=0x%x\n",*sensor_id,data_mid[0], imgsensor_info.sensor_id);
...
UINT32 OV8865_SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
