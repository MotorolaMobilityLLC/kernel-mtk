...
//end
static int Read3P3AWBData(unsigned short ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	int i = 0;
	int addr = ui4_offset;
	unsigned int sum = 0;
	...
	if( AWBdatabuf[0] != 0x1)
	{
		...
		CAM_CALDB("3P3_OTP AWB data invalid!\n");
	}
	...
	{
		CAM_CALDB("3P3_OTP AWB check sum failed!\n");
	}
...
static int Read3P3AFData(unsigned short ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	int i = 0;
	int addr = ui4_offset;
	unsigned int sum = 0;
	...
	if( AFdatabuf[0] != 0x1)
	{
		...
		CAM_CALDB("3P3_OTP AF data invalid!\n");
	}
	...
	if(sum != AFdatabuf[7])
	{
		CAM_CALDB("3P3_OTP AF check sum failed!\n");
	}
...
static int Read3P3LSCData(unsigned short ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	int i = 0;
	int addr = ui4_offset;
	unsigned int sum = 0;

	CAM_CALDB("Read3P3LSCData\n");
	...
	if( LSCdatabuf[0] != 0x1)
	{
		...
		CAM_CALDB("3P3_OTP LSC data invalid!\n");
	}
	...
	{
		CAM_CALDB("3P3_OTP LSC check sum failed!\n");
	}
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            {
            	i4RetValue = Read3P3AWBData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pu1Params);
            }
            else if((ptempbuf->u4Offset == AF_ADDR) && (ptempbuf->u4Length == 4) )
            {
            	i4RetValue = Read3P3AFData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pu1Params);
            }   
            
            else if((ptempbuf->u4Offset == LSC_ADDR) && (ptempbuf->u4Length == LSC_SIZE) )
            {
            	i4RetValue = Read3P3LSCData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pu1Params);
            }   
