...
int iMultiReadReg(u16 a_u2Addr, u8 *a_puBuff, u16 i2cId, u8 number)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		if (i4RetValue != 2) {
			PK_PR_ERR("[CAMERA SENSOR] I2C send failed, addr = 0x%x, data = 0x%x !!\n",
			       a_u2Addr, *a_puBuff);
			return -1;
		}
		/*  */
		...
		if (i4RetValue != 1) {
			PK_PR_ERR("[CAMERA SENSOR] I2C read failed!!\n");
			return -1;
		}
	} else {
		spin_lock(&kdsensor_drv_lock);
		...
		u16 i2cId)
{
	...
	if (g_IsSearchSensor != 1) {
		if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
			...
			/* Remove i2c ack error log during search sensor */
			/* PK_PR_ERR("g_pstI2Cclient->ext_flag: %d", g_IsSearchSensor); */
			if (g_IsSearchSensor == 1)
				g_pstI2Cclient->ext_flag = (g_pstI2Cclient->ext_flag) | I2C_A_FILTER_MSG;
			else
				g_pstI2Cclient->ext_flag = (g_pstI2Cclient->ext_flag) & (~I2C_A_FILTER_MSG);
			...
			if (i4RetValue != a_sizeSendData) {
				PK_PR_ERR("[CAMERA SENSOR] I2C send failed!!, Addr = 0x%x\n",
				       a_pSendData[0]);
				return -1;
			}
...
#ifdef CONFIG_MTK_I2C_EXTENSION
			/* Remove i2c ack error log during search sensor */
			/* PK_PR_ERR("g_pstI2Cclient2->ext_flag: %d", g_IsSearchSensor); */
			if (g_IsSearchSensor == 1)
				g_pstI2Cclient2->ext_flag = (g_pstI2Cclient2->ext_flag) | I2C_A_FILTER_MSG;
			else
				g_pstI2Cclient2->ext_flag = (g_pstI2Cclient2->ext_flag) & (~I2C_A_FILTER_MSG);
		...
		if (i2c_msg_size != ret) {
			PK_PR_ERR("[iReadRegI2C]I2C failed(0x%x)! Data[0]=0x%x, Data[1]=0x%x,timing(0=%d)\n",
		      ...
		      u16 i2cId, u16 timing)
{
	...
	if (i2c_msg_size != ret) {
		PK_PR_ERR("[iReadRegI2CTiming] I2C send failed (0x%x)! timing(0=%d)\n", ret, speed_timing);
		ret = -1;
	}
	return ret;
}
...
int iWriteReg(u16 a_u2Addr, u32 a_u4Data, u32 a_u4Bytes, u16 i2cId)
{
	...
	char puSendCmd[6] = {
		(char)(a_u2Addr >> 8), (char)(a_u2Addr & 0xFF), 0, 0, 0, 0
	};
	...
	if (a_u4Bytes > 2) {
		PK_PR_ERR("[CAMERA SENSOR] exceed 2 bytes\n");
		return -1;
	}
...
int iWriteRegI2C(u8 *a_pSendData, u16 a_sizeSendData, u16 i2cId)
{
	...
	if (g_IsSearchSensor != 1) {
		...
		i2c_msg_size = 1;
		do {
			i4RetValue = mtk_i2c_transfer(pClient->adapter, msg, i2c_msg_size,
						      I2C_A_FILTER_MSG, speed_timing);
			if (i4RetValue != i2c_msg_size) {
				PK_PR_ERR("[%s]I2C failed(0x%x)! Data[0]=0x%x, Data[1]=0x%x,timing(0=%d)\n",
...
int iWriteRegI2CTiming(u8 *a_pSendData, u16 a_sizeSendData, u16 i2cId, u16 timing)
{
	...
	do {
		ret = mtk_i2c_transfer(pClient->adapter, msg, i2c_msg_size, 0, speed_timing);
		/* PK_PR_ERR("[iWriteRegI2CTiming]ret(%d) i2c_msg_size(%d)\n", ret, i2c_msg_size); */
		if (ret != i2c_msg_size) {
			PK_PR_ERR("[%s]i2c failed(0x%x)! Data[0]=0x%x, Data[1]=0x%x,timing(0=%d), retry(0=%d)\n",
...
int kdSetI2CBusNum(u32 i2cBusNum)
{
	...
	if ((i2cBusNum != SUPPORT_I2C_BUS_NUM3) && (i2cBusNum != SUPPORT_I2C_BUS_NUM2)
		...
		PK_PR_ERR("[kdSetI2CBusNum] i2c bus number is not correct(%d)\n", i2cBusNum);
		return -1;
	}
	spin_lock(&kdsensor_drv_lock);
...
int iBurstWriteReg_HW(u8 *pData, u32 bytes, u16 i2cId, u16 transfer_length, u16 timing)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1)
		...
		PK_PR_ERR("iBurstWriteReg_HW only support main cam for now\n");
		return -1;
	}
...
int i2c_buf_mode_en(int enable)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1)
		...
		PK_PR_ERR("i2c_buf_mode_en only support main cam for now\n");
		return -1;
	}
	if (enable)
...
int iBurstWriteReg_HW(u8 *pData, u32 bytes, u16 i2cId, u16 transfer_length, u16 timing)
{
	PK_PR_ERR("plz #define HW_TRIGGER_I2C_SUPPORT 1\n");
	return -1;
}
...
int iBurstWriteReg_multi(u8 *pData, u32 bytes, u16 i2cId, u16 transfer_length, u16 timing)
{
	...
	if (ret != trans_num) {
		PK_PR_ERR("[iBurstWriteReg_multi] I2C send failed (0x%x)! timing(0=%d)\n", ret,
		       speed_timing);
	}
	return ret;
#else
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		if (bytes > MAX_I2C_CMD_LEN) {
			PK_PR_ERR("[iBurstWriteReg] exceed the max write length\n");
			return 1;
		}
		...
		if (buf == NULL) {
			PK_PR_ERR("[iBurstWriteReg] Not enough memory\n");
			return -1;
		}
		memset(buf, 0, bytes);
		...
		do {
			...
			if ((ret & 0xffff) != transfer_length)
				PK_PR_ERR("Error sent I2C ret = %d\n", ret);
...
MUINT32 Get_Camera_Temperature(CAMERA_DUAL_CAMERA_SENSOR_ENUM senDevId, MUINT8 *invalid,
			       INT32 *temp)
{
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (senDevId == g_invokeSocketIdx[i]) {
				...
				if (*invalid == SENSOR_TEMPERATURE_VALID) {
					...
					ret = g_pInvokeSensorFunc[i]->
						SensorFeatureControl(SENSOR_FEATURE_GET_TEMPERATURE_VALUE,
								     (MUINT8 *) temp,
	...
	for (i = (KDIMGSENSOR_MAX_INVOKE_DRIVERS - 1); i >= KDIMGSENSOR_INVOKE_DRIVER_0; i--) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (0 != (g_CurrentSensorIdx & g_invokeSocketIdx[i])) {
				...
				/* State: sensor init but not streaming */
				g_pInvokeSensorFunc[i]->sensorState = SENSOR_STATE_OPEN;

				/* Sensor is power on */
				gSensorTempState[g_invokeSocketIdx[i]] &= ~SENSOR_TEMPERATURE_NOT_POWER_ON;
				if ((gSensorTempState[g_invokeSocketIdx[i]] & TEMPERATURE_INIT_STATE) == 0) {
		      ...
		      MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData[2])
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			...
			if (ret != ERROR_NONE) {
				PK_PR_ERR("[%s] SensorGetInfo fail\n", __func__);
				return ret;
			}
...
MUINT32 kd_MultiSensorGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution[2])
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			...
			if (ret != ERROR_NONE) {
				PK_PR_ERR("[%s] SensorGetResolution fail\n", __func__);
				return ret;
			}
		}
	}
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			...
			if (InvokeCamera == g_invokeSocketIdx[i]) {
				...
				ret = g_pInvokeSensorFunc[i]->SensorFeatureControl(FeatureId, pFeaturePara,
										   pFeatureParaLen);
				if (ret != ERROR_NONE) {
					PK_PR_ERR("[%s] SensorFeatureControl fail, FeatureId %d\n"
						, __func__, FeatureId);
					return ret;
				}
				...
				else if (FeatureId == SENSOR_FEATURE_SET_STREAMING_RESUME)
		...
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (InvokeCamera == g_invokeSocketIdx[i]) {
				...
				ret = g_pInvokeSensorFunc[i]->SensorControl(ScenarioId, pImageWindow,
									    pSensorConfigData);
				if (ret != ERROR_NONE) {
					PK_PR_ERR("ERR:SensorControl(), i =%d\n", i);
					return ret;
				}
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (0 != (g_CurrentSensorIdx & g_invokeSocketIdx[i])) {
				...
				/* sensor power off */
				gSensorTempState[g_invokeSocketIdx[i]] |= SENSOR_TEMPERATURE_NOT_POWER_ON;
				if ((gSensorTempState[g_invokeSocketIdx[i]] & SENSOR_TEMPERATURE_VALID) > 0) {
					/* clear temperature valid */
					gSensorTempState[g_invokeSocketIdx[i]] &= ~SENSOR_TEMPERATURE_VALID;
				}
			}
		}
...
int kdSetDriver(unsigned int *pDrvIndex)
{
	...
	if (kdGetSensorInitFuncList(&pSensorList) != 0) {
		PK_PR_ERR("ERROR:kdGetSensorInitFuncList()\n");
		return -EIO;
	}
...
int kdSensorSyncFunctionPtr(void)
{
	...
	if (g_pSensorFunc == NULL) {
		PK_PR_ERR("ERROR:NULL g_pSensorFunc\n");
		mutex_unlock(&kdCam_Mutex);
		return -EIO;
	}
	...
	*(pRAWGain + 3) = 0x00;
	...
	if (g_NewSensorExpGain.uISPGainDelayFrame == 0) {	/* synchronize the isp gain */
		...
		*(pRAWGain + 3) = g_NewSensorExpGain.u2ISPNewBGain;
		 ...
		 * g_NewSensorExpGain.u2ISPNewGrGain, g_NewSensorExpGain.u2ISPNewGbGain,
...
int kdSensorSetExpGainWaitDone(int *ptime)
{
	...
	if (timeout == 0) {
		PK_PR_ERR("[kd_sensorlist] kdSensorSetExpGainWait: timeout=%d\n", *ptime);
...
static inline int adopt_CAMERA_HW_Open(void)
{
	...
	if (g_pSensorFunc) {
		...
		if (err != ERROR_NONE) {
			 ...
			 */
			PK_PR_ERR("ERROR:SensorOpen(), turn off power\n");
		}
	} else {
	...
	/* } */
	/* else { */
	/* PK_PR_ERR("adopt_CAMERA_HW_Open Fail, g_CamHWOpend = %d\n ",atomic_read(&g_CamHWOpend) ); */
	/* } */
		...
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (g_invokeSocketIdx[i] != DUAL_CAMERA_NONE_SENSOR) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					gSensorTempState[g_invokeSocketIdx[i]] &=
						~SENSOR_TEMPERATURE_CANNOT_SEARCH_SENSOR;
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	if (pSensorGetInfo->SensorId < DUAL_CAMERA_SENSOR_MAX)
	...
	if ((pSensorInfo->TEMPERATURE_SUPPORT == 1) &&
		(pSensorGetInfo->SensorId < DUAL_CAMERA_SENSOR_MAX))
		gSensorTempState[pSensorGetInfo->SensorId] &=
			~SENSOR_TEMPERATURE_NOT_SUPPORT_THERMAL;
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			...
			if (pSensorGetInfo->SensorId == g_invokeSocketIdx[i]) {
				...
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\n\nCAM_Info[%d]:%s;", mtk_ccm_name,
					 g_invokeSocketIdx[IDNum], g_invokeSensorNameStr[IDNum]);
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\nSensorModeNum: %d", mtk_ccm_name,
					 pSensorInfo->SensorModeNum);
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\nPre:  TgGrab_w,h,x_,y=%5d,%5d,%3d,%3d, delay_frm=%2d",
					 mtk_ccm_name, psensorResolution[IDNum]->SensorPreviewWidth,
				...
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 ...
					 mtk_ccm_name, psensorResolution[IDNum]->SensorFullWidth,
				...
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 ...
					 mtk_ccm_name, psensorResolution[IDNum]->SensorVideoWidth,
					 psensorResolution[IDNum]->SensorVideoHeight,
				...
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 ...
					 mtk_ccm_name,
					 psensorResolution[IDNum]->SensorHighSpeedVideoWidth,
					 psensorResolution[IDNum]->SensorHighSpeedVideoHeight,
				...
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 ...
					 mtk_ccm_name,
					 psensorResolution[IDNum]->SensorSlimVideoWidth,
					 psensorResolution[IDNum]->SensorSlimVideoHeight,
					 pSensorInfo->SensorGrabStartX_VD2,
					 pSensorInfo->SensorGrabStartY_VD2,
					 pSensorInfo->SlimVideoDelayFrame);
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\nCST1: TgGrab_w,h,x_,y=%5d,%5d,%3d,%3d, delay_frm=%2d",
					 mtk_ccm_name, psensorResolution[IDNum]->SensorCustom1Width,
					 psensorResolution[IDNum]->SensorCustom1Height,
					 pSensorInfo->SensorGrabStartX_CST1,
					 pSensorInfo->SensorGrabStartY_CST1,
					 pSensorInfo->Custom1DelayFrame);
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\nCST2: TgGrab_w,h,x_,y=%5d,%5d,%3d,%3d, delay_frm=%2d",
					 mtk_ccm_name, psensorResolution[IDNum]->SensorCustom2Width,
					 psensorResolution[IDNum]->SensorCustom2Height,
					 pSensorInfo->SensorGrabStartX_CST2,
					 pSensorInfo->SensorGrabStartY_CST2,
					 pSensorInfo->Custom2DelayFrame);
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\nCST3: TgGrab_w,h,x_,y=%5d,%5d,%3d,%3d, delay_frm=%2d",
					 mtk_ccm_name, psensorResolution[IDNum]->SensorCustom3Width,
					 psensorResolution[IDNum]->SensorCustom3Height,
					 pSensorInfo->SensorGrabStartX_CST3,
					 pSensorInfo->SensorGrabStartY_CST3,
					 pSensorInfo->Custom3DelayFrame);
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\nCST4: TgGrab_w,h,x_,y=%5d,%5d,%3d,%3d, delay_frm=%2d",
					 mtk_ccm_name, psensorResolution[IDNum]->SensorCustom4Width,
					 psensorResolution[IDNum]->SensorCustom4Height,
					 pSensorInfo->SensorGrabStartX_CST4,
					 pSensorInfo->SensorGrabStartY_CST4,
					 pSensorInfo->Custom4DelayFrame);
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\nCST5: TgGrab_w,h,x_,y=%5d,%5d,%3d,%3d, delay_frm=%2d",
					 mtk_ccm_name, psensorResolution[IDNum]->SensorCustom5Width,
					 psensorResolution[IDNum]->SensorCustom5Height,
				...
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 ...
					 mtk_ccm_name, pSensorInfo->PDAF_Support);
				snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
					 "%s\nHDR_Support(0:NO HDR,1: iHDR,2:mvHDR,3:zHDR)=%2d",
					 mtk_ccm_name, pSensorInfo->HDR_Support);
			}
		}
	}
...
static inline int adopt_CAMERA_HW_Control(void *pBuf)
{
	...
	if (pSensorCtrl->pImageWindow == NULL || pSensorCtrl->pSensorConfigData == NULL) {
		PK_DBG("[CAMERA_HW] NULL arg.\n");
		return -EFAULT;
	}
	...
	case SENSOR_FEATURE_GET_SENSOR_HDR_CAPACITY:
		{
			...
			*(pFeaturePara_64 + 1) = (uintptr_t) pValue;
			...
			if (g_pSensorFunc) {
				...
				PK_PR_ERR("[CAMERA_HW]ERROR:NULL g_pSensorFunc\n");
			}
...
static inline int kdSetSensorMclk(int *pBuf)
{
	...
	Check_ccf_clk();
	if (pSensorCtrl->on == 1) {
		if (pSensorCtrl->freq == MCLK_48MHZ_GROUP) {
			/*CAM_PLL_48_GROUP */
...
bool _hwPowerOn(PowerType type, int powerVolt)
{
	...
	if (!IS_ERR(reg)) {
		...
		if (regulator_set_voltage(reg, powerVolt, powerVolt) != 0) {
			PK_PR_ERR("[_hwPowerOn]fail to regulator_set_voltage, powertype:%d powerId:%d\n",
			       type, powerVolt);
			/* return ret; */
		}
		...
		PK_PR_ERR("[_hwPowerOn]IS_ERR_OR_NULL powertype:%d reg %p\n", type, reg);
		return ret;
	}
	...
	case COMPAT_KDIMGSENSORIOC_X_GETRESOLUTION2:
		{
			...
			if (err != 0)
				PK_PR_ERR("[CAMERA SENSOR] compat_get_Acdk_sensor_resolution_info_struct failed\n");
			return ret;
		}
			    ...
			    unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	case KDIMGSENSORIOC_X_GET_ISP_CLK:
		...
		*(unsigned int *)pBuff = mmdvfs_get_stable_isp_clk();
#else
		*(unsigned int *)pBuff = MMSYS_CLK_HIGH;
#endif
		break;
...
static int CAMERA_HW_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	clk_prepare_enable(g_camclk_univpll2_d2);
...
static int CAMERA_HW_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	clk_disable_unprepare(g_camclk_univpll2_d2);
				   ...
				   loff_t *data)
{
	...
	if (sscanf(regBuf, "%x %x", &sensorReg.RegAddr, &sensorReg.RegData) == 2) {
		if (g_pSensorFunc != NULL) {
			g_pSensorFunc->SensorFeatureControl(DUAL_CAMERA_MAIN_SENSOR,
							    SENSOR_FEATURE_SET_REGISTER, (MUINT8 *) &sensorReg,
							    (MUINT32 *) sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
			g_pSensorFunc->SensorFeatureControl(DUAL_CAMERA_MAIN_SENSOR,
							    SENSOR_FEATURE_GET_REGISTER, (MUINT8 *) &sensorReg,
							    (MUINT32 *) sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
				     ...
				     loff_t *data)
{
	...
	if (g_pInvokeSensorFunc[0])
		g_pInvokeSensorFunc[0]->SensorFeatureControl(SENSOR_FEATURE_SET_PDAF_TYPE, regBuf,
							     &u4CopyBufSize);
...
static int __init CAMERA_HW_i2C_init(void)
{
	...
	if (ret) {
		PK_PR_ERR("[camerahw_probe] platform_device_register fail\n");
		return ret;
	}

	ret = platform_device_register(&camerahw2_platform_device);
	if (ret) {
		PK_PR_ERR("[camerahw2_probe] platform_device_register fail\n");
		return ret;
	}
#endif
	...
	if (prEntry) {
		...
		PK_PR_ERR("add /proc/driver/camsensor entry fail\n");
	}
	...
	if (prEntry) {
		...
		PK_PR_ERR("add /proc/driver/camsensor2 entry fail\n");
	}
	...
	if (prEntry) {
		...
		PK_PR_ERR("add /proc/driver/maincam_status entry fail\n");
	}
	...
	if (prEntry) {
		...
		PK_PR_ERR("add /proc/driver/subcam_status entry fail\n");
	}
	...
	if (prEntry) {
		...
		PK_PR_ERR("add /proc/driver/3dcam_status entry fail\n");
	}
