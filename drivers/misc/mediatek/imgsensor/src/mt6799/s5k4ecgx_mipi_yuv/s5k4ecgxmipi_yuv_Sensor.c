...
#define LOG_TAG "[4EC]"
#define SENSORDB(fmt, arg...)    pr_debug(LOG_TAG "[%s] " fmt, __FUNCTION__, ##arg)
#else
#define SENSORDB(x,...)
#endif
...
static void S5K4ECGX_MIPI_AE_Get_Status(UINT32 *pFeatureReturnPara32)
{
    ...
    // AE disable
    if(KAL_FALSE == aeEnable)
    {
        *pFeatureReturnPara32 = SENSOR_AE_IDLE;
    }
    else if(KAL_TRUE  == S5K4ECGX_Driver.userAskAeLock)
    {
...
static void S5K4ECGX_MIPI_AWB_Get_Status(UINT32 *pFeatureReturnPara32)
{
    ...
    // AWB disable
    if(KAL_FALSE == awbEnable)
    {
        *pFeatureReturnPara32 = SENSOR_AWB_IDLE;
    }
    else if(KAL_TRUE == S5K4ECGX_Driver.userAskAwbLock)
    {
               ...
               UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_AE_AWB_LOCK_INFO:
             ...
             S5K4ECGX_MIPI_get_AEAWB_lock((UINT32*)(uintptr_t)(*pFeatureData),(UINT32*)(uintptr_t)(*(pFeatureData+1)));
             break;
