...
#define LOG_TAG "[4EC]"
#define SENSORDB(fmt, arg...)    pr_debug(LOG_TAG "[%s] " fmt, __func__, ##arg)
#else
#define SENSORDB(x, ...)
#endif
...
unsigned int S5K4ECGX_Preview_enabled;
unsigned int s5k4ec_cap_enable;
...
static kal_uint16 S5K4ECGX_table_write_cmos_sensor(const kal_uint16 *para, kal_uint32 len)
{
	...
	while (len > IDX) {
		...
		if (tosend == I2C_BUFFER_LEN || IDX == len || addr != addr_last) {
			BLOCK_I2C_DATA_WRITE(puSendCmd, tosend,
					     S5K4ECGX_MIPI_sensor_write_I2C_address);
			tosend = 0;
		}
	}
	return 0;
...
int SEN_RUN_TEST_PATTERN;

unsigned int S5K4ECGX_MIPI_GetExposureTime(void)
{
...
static void S5K4ECGX_MIPI_AE_Lock(void)
{
...
static void S5K4ECGX_MIPI_AE_UnLock(void)
{
...
static void S5K4ECGX_MIPI_AE_On(void)
{
...
static void S5K4ECGX_MIPI_AE_Off(void)
{
	kal_uint16 Status_3A = 0;
...
static void S5K4ECGX_MIPI_AWB_Lock(void)
{
...
static void S5K4ECGX_MIPI_AWB_UnLock(void)
{
#if 0
	S5K4ECGX_write_cmos_sensor(0x0028, 0x7000);
	S5K4ECGX_write_cmos_sensor(0x002A, 0x2C66);
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0001);
	if (!((S5K4ECGX_Driver.sceneMode == SCENE_MODE_OFF) || (SCENE_MODE_NORMAL ==
								S5K4ECGX_Driver.sceneMode)
	      || (S5K4ECGX_Driver.sceneMode == SCENE_MODE_HDR))) {
		S5K4ECGX_MIPI_set_scene_mode(S5K4ECGX_Driver.sceneMode);
	} else {
		if (!((S5K4ECYX_MIPICurrentStatus.iWB == AWB_MODE_OFF) ||
		      (S5K4ECYX_MIPICurrentStatus.iWB == AWB_MODE_AUTO))) {
...
static void S5K4ECGX_MIPI_AWB_On(void)
{
...
static void S5K4ECGX_MIPI_AWB_Off(void)
{
	kal_uint16 Status_3A = 0;
...
static void S5K4ECGX_MIPI_AE_ExpCurveGain_Config(unsigned int SceneMode)
{
...
static void S5K4ECGX_MIPI_AE_Dump_WeightTbl(void)
{
	/* debug only, readback */
	unsigned int val[32];
	...
	for (offset = 0; offset < 32; offset += 8) {
		SENSORDB("[4EC] AETbl SW_Tbl[%d~%d]: %4x, %4x, %4x, %4x\n", offset, offset + 3,
			 pae_table[offset + 0], pae_table[offset + 1], pae_table[offset + 2],
			 pae_table[offset + 3]);
		SENSORDB("[4EC] AETbl SW_Tbl[%d~%d]: %4x, %4x, %4x, %4x\n", offset + 4, offset + 7,
			 pae_table[offset + 4], pae_table[offset + 5], pae_table[offset + 6],
			 pae_table[offset + 7]);
	}
...
unsigned short S5K4ECGX_MIPI_AE_Get_WeightVal(unsigned int x, unsigned int y)
{
...
static void S5K4ECGX_MIPI_AE_Set_Window2HW(void)
{
...
S5K4ECGX_MIPI_AE_Set_Window(uintptr_t zone_addr, unsigned int prevW, unsigned int prevH)
{
	...
	if ((x0 == x1) && (y0 == y1)) {
		...
		memcpy((unsigned char *)&ae_table[0][0],
		       (unsigned char *)&S5K4ECGX_MIPI_DEFAULT_AE_TABLE[0], 64);
	...
	spin_lock(&s5k4ecgx_mipi_drv_lock);
	if ((x1 == S5K4ECGX_Driver.mapAEWindows[0]) && (y1 == S5K4ECGX_Driver.mapAEWindows[1])) {
...
static void S5K4ECGX_MIPI_AF_Get_Max_Num_Focus_Areas(unsigned int *pFeatureReturnPara32)
{
	*pFeatureReturnPara32 = 1;
	/* SENSORDB("S5K4ECGX *pFeatureReturnPara32 = %d\n",  *pFeatureReturnPara32); */
}

static void S5K4ECGX_MIPI_AE_Get_Max_Num_Metering_Areas(unsigned int *pFeatureReturnPara32)
{
	*pFeatureReturnPara32 = 1;
}

static void S5K4ECGX_MIPI_AF_Get_Inf(unsigned int *pFeatureReturnPara32)
{
	*pFeatureReturnPara32 = 0;
}

static void S5K4ECGX_MIPI_AF_Get_Macro(unsigned int *pFeatureReturnPara32)
{
...
static void S5K4ECGX_MIPI_AF_rollbackWinSet(void)
{
...
S5K4ECGX_AAA_STATUS_ENUM S5K4ECGX_MIPI_AF_Init(void)
{
	unsigned int backupAFWindDone = 1;

	spin_lock(&s5k4ecgx_mipi_drv_lock);
	/* have done in init_function */
	if (S5K4ECGX_Driver.afState == S5K4ECGX_AF_STATE_UNINIT) {
...
S5K4ECGX_AAA_STATUS_ENUM S5K4ECGX_MIPI_AF_Set_Window2HW(void)
{
...
unsigned int orig_inWw;
unsigned int orig_inWh;
unsigned int orig_outWw;
unsigned int orig_outWh;
#define S5K4ECGX_FAF_TOLERANCE    100
#define S5K4ECGX_AF_MIX_ACCURACY  0x200000	/* 0x8000 //Perfer: 0x2x0000 */
static S5K4ECGX_AAA_STATUS_ENUM
S5K4ECGX_MIPI_AF_Set_Window(uintptr_t zone_addr, unsigned int prevW, unsigned int prevH)
{
...
S5K4ECGX_AAA_STATUS_ENUM S5K4ECGX_MIPI_AF_Stop(void)
{
	...
	/* //Lock AE */
	if ((!S5K4ECGX_Driver.userAskAeLock) && (S5K4ECGX_Driver.afMode == S5K4ECGX_AF_MODE_SINGLE)) {
		spin_unlock(&s5k4ecgx_mipi_drv_lock);
...
static S5K4ECGX_AAA_STATUS_ENUM S5K4ECGX_MIPI_AF_CancelFocus(void)
{
	...
	if (S5K4ECGX_Driver.afMode == S5K4ECGX_AF_MODE_RSVD) {
		/* have been aborted */
		return S5K4ECGX_AAA_AF_STATUS_OK;
	}
...
S5K4ECGX_AAA_STATUS_ENUM S5K4ECGX_MIPI_AF_Start(S5K4ECGX_AF_MODE_ENUM mode)
{
	/* unsigned int af_status; */
	unsigned int frameTime;
	...
	if (mode == S5K4ECGX_AF_MODE_SINGLE) {
		...
		if (S5K4ECGX_Driver.afMode == S5K4ECGX_AF_MODE_CONTINUOUS) {
			spin_unlock(&s5k4ecgx_mipi_drv_lock);
...
static S5K4ECGX_AAA_STATUS_ENUM S5K4ECGX_MIPI_AF_Move_to(unsigned int a_u2MovePosition)
{
	return S5K4ECGX_AAA_AF_STATUS_OK;
}
...
static S5K4ECGX_AAA_STATUS_ENUM S5K4ECGX_MIPI_AF_Get_Status(unsigned int *pFeatureReturnPara32)
{
	...
	if (S5K4ECGX_Driver.afState == S5K4ECGX_AF_STATE_ENTERING) {
...
{
	unsigned int exposureTime;	/* to hardware reg */

	/* iShutter is in terms of 32us */
	iShutter *= 32;
	exposureTime = iShutter >> 3;	/* to hardware reg */
...
jpegParserParseImage(unsigned char *srcBuf, unsigned int bufSize, unsigned int *eoiOffset)
{
	...
	while (rdPtr < endPtr) {
		if (rdPtr[0] == JPEG_MARKER_START_CODE) {
			if (rdPtr[1] == JPEG_MARKER_EOI) {
				rdPtr += 2;
				SENSORDB
				    ("[4EC] jpegParserParseImage B:Encounter FFD9: rdPtr=0x%p\n",
				     rdPtr);
				break;
			}
			rdPtr += 1;
			...
			if ((offsetOfEncounter100Zeros == 0) &&
				...
				if (!(memcmp(array100Zeros, rdPtr, 100))) {
					SENSORDB
					    ("[4EC] jpegParserParseImage:C offsetOfEncounter100Zeros = 1: rdPtr=0x%p\n",
					     rdPtr);
					offsetOfEncounter100Zeros = (uintptr_t) rdPtr;
				}
			}
			rdPtr += 1;
		}
	...
	if (rdPtr >= endPtr) {
		...
		rdPtr = (unsigned char *)(uintptr_t) offsetOfEncounter100Zeros;
		rdPtr[0] = 0xFF;
		rdPtr[1] = 0xD9;
		rdPtr += 2;
...
void S5K4ECGX_MIPI_JPEG_Capture_Parser(uintptr_t jpegFileAddr, UINT32 jpegMaxBufSize,
				       ACDK_SENSOR_JPEG_INFO *jpeg_info)
{
	...
	SENSORDB("[4EC] JPEG_Capture_Parser jpegMaxBufSize=0x%x, W=%d, H=%d\n", jpegMaxBufSize,
		 jpeg_info->u4SrcW, jpeg_info->u4SrcH);
#else
...
#define OUTPUT_FMT  5		/* YUV */
	...
	/* ================================================================================== */
	{
		...
		S5K4ECGX_table_write_cmos_sensor(addr_data_pair,
						 sizeof(addr_data_pair) / sizeof(kal_uint16));
	}
...
static void S5K4ECGX_Init_Setting(void)
{
	...
	S5K4ECGX_write_cmos_sensor(0x002A, 0x0722);
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0100);
	/* skl_OTP_usWaitTime This register should be positioned in fornt of D0001000 */
	...
	S5K4ECGX_write_cmos_sensor(0x002A, 0x1A6A);
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x009A);
	/* #senHal_usForbiddenRightOfs    //extend right Forbidden area line */
	...
	S5K4ECGX_write_cmos_sensor(0x002A, 0x08D6);
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0000);
	/* #ash_bUseOTPData If OTP for shading is used,
	*this register should be enable. ( default : disable)
	*/

	S5K4ECGX_write_cmos_sensor(0x002A, 0x146E);
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0001);
	/* #awbb_otp_disable If OTP for AWB is used, this register should be enable.*/

	S5K4ECGX_write_cmos_sensor(0x002A, 0x08DC);
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0000);
	/* #ash_bUseGasAlphaOTP, If OTP alpha for shading is used, this register should be enable.*/
	...
	S5K4ECGX_write_cmos_sensor(0x0F12, 0x0000);
	/* REG_TC_GP_bUse1FrameCaptureMode 0 Continuous mode 1 Single frame mode */
...
static void S5K4ECGX_enb_preview(void)
{
...
void S5K4ECGX_MIPI_GetAutoISOValue(void)
{
	...
	/* ISO 200 : 200 ~ 37F(HEX) */
	/* ISO 400 : over 380(HEX) */
	if (S5K4ECGX_Driver.isoSpeed != AE_ISO_AUTO)
		return;
...
void S5K4ECGX_MIPI_GetActiveConfigNum(unsigned int *pActiveConfigNum)
{
	...
	if (scen == MSDK_SCENARIO_ID_VIDEO_PREVIEW)
		*pActiveConfigNum = S5K4EC_VIDEO_MODE;
	else if (scen == MSDK_SCENARIO_ID_CAMERA_PREVIEW)
...
void S5K4ECGX_MIPI_SetFrameRate(MSDK_SCENARIO_ID_ENUM scen, UINT16 u2FrameRate)
{
	...
	if (u2FrameRate <= 15) {
		...
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			...
			/* REG_xTC_PCFG_usMaxFrTimeMsecMult10 //09C4h:4fps */

			S5K4ECGX_write_cmos_sensor(0x0F12, 0x029A);
			/* REG_xTC_PCFG_usMinFrTimeMsecMult10 //014Ah:30fps */
			break;
...
void S5K4ECGX_MIPI_SceneOffMode(void)
{
...
*
*************************************************************************/
static UINT32 S5K4ECGX_MIPI_Preview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
				    MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	S5K4ECGX_Driver.Period_PixelNum =
	    S5K4ECGX_IMAGE_SENSOR_PV_WIDTH_DRV + S5K4ECGX_Driver.Dummy_Pixels;
	S5K4ECGX_Driver.Period_LineNum =
	    S5K4ECGX_IMAGE_SENSOR_PV_HEIGHT_DRV + S5K4ECGX_Driver.Dummy_Lines;
	...
	if (scene == MSDK_SCENARIO_ID_CAMERA_PREVIEW) {
	...
	image_window->ExposureWindowWidth =
	    S5K4ECGX_IMAGE_SENSOR_PV_WIDTH - 2 * S5K4ECGX_PV_X_START;
	image_window->ExposureWindowHeight =
	    S5K4ECGX_IMAGE_SENSOR_PV_HEIGHT - 2 * S5K4ECGX_PV_Y_START;
...
static UINT32 S5K4ECGX_MIPI_Preview_ZSD(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	unsigned int width, height;
	...
	height = S5K4ECGX_Driver.Preview_Height;
	S5K4ECGX_Driver.Period_PixelNum =
	    S5K4ECGX_IMAGE_SENSOR_FULL_WIDTH_DRV + S5K4ECGX_Driver.Dummy_Pixels;
	S5K4ECGX_Driver.Period_LineNum =
	    S5K4ECGX_IMAGE_SENSOR_FULL_HEIGHT_DRV + S5K4ECGX_Driver.Dummy_Lines;
	...
	image_window->ExposureWindowWidth =
	    S5K4ECGX_IMAGE_SENSOR_FULL_WIDTH_DRV - 2 * S5K4ECGX_PV_X_START;
	image_window->ExposureWindowHeight =
	    S5K4ECGX_IMAGE_SENSOR_FULL_HEIGHT_DRV - 2 * S5K4ECGX_PV_Y_START;
...
UINT32 S5K4ECGX_MIPI_Capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	kal_uint32 shutter = S5K4ECGX_Driver.shutter;
	kal_uint32 gain = 0;

	if (S5K4ECGX_Driver.sceneMode == SCENE_MODE_HDR)
	...
	spin_lock(&s5k4ecgx_mipi_drv_lock);
	S5K4ECGX_Driver.iGrabWidth =
	    S5K4ECGX_IMAGE_SENSOR_FULL_WIDTH_DRV - 2 * S5K4ECGX_FULL_X_START;
	S5K4ECGX_Driver.iGrabheight =
	    S5K4ECGX_IMAGE_SENSOR_FULL_HEIGHT_DRV - 2 * S5K4ECGX_FULL_Y_START;
	image_window->GrabStartX = S5K4ECGX_FULL_X_START;
	image_window->GrabStartY = S5K4ECGX_FULL_Y_START;
...
UINT32 S5K4ECGX_MIPI_GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	default:
		pSensorResolution->SensorPreviewWidth =
		    S5K4ECGX_IMAGE_SENSOR_PV_WIDTH_DRV - 2 * S5K4ECGX_PV_X_START;
		pSensorResolution->SensorPreviewHeight =
		    S5K4ECGX_IMAGE_SENSOR_PV_HEIGHT_DRV - 2 * S5K4ECGX_PV_Y_START;
			     ...
			     MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	spin_lock(&s5k4ecgx_mipi_drv_lock);
	pSensorInfo->CaptureDelayFrame = 100;	/* (0 == S5K4ECGX_Driver.manualAEStart) ? 3 : 2; */
	spin_unlock(&s5k4ecgx_mipi_drv_lock);
...
UINT32 S5K4ECGX_MIPI_Control(MSDK_SCENARIO_ID_ENUM ScenarioId,
 ...
 * Note:
 */
UINT32 S5K4ECGX_MIPI_AF_Infinity(void)
{
...
static void S5K4ECGX_MIPI_AE_Get_Status(UINT32 *pFeatureReturnPara32)
{
	...
	/* [b:1] 1: AE enable, 0: AE disable; [b:3] 1: AWB enable, 0: AWB disable */
	...
	/* AE disable */
	if (aeEnable == KAL_FALSE)
		*pFeatureReturnPara32 = SENSOR_AE_IDLE;
	else if (S5K4ECGX_Driver.userAskAeLock == KAL_TRUE)
...
static void S5K4ECGX_MIPI_AWB_Get_Status(UINT32 *pFeatureReturnPara32)
{
	...
	/* AWB disable */
	if (awbEnable == KAL_FALSE)
		*pFeatureReturnPara32 = SENSOR_AWB_IDLE;
	else if (S5K4ECGX_Driver.userAskAwbLock == KAL_TRUE)
...
BOOL S5K4ECGX_MIPI_set_param_wb(UINT16 para)
{
	...
	while ((Status_3A == 0) && (iter-- > 0)) {
		...
		S5K4ECGX_write_cmos_sensor(0x002E, 0x04E6);
		Status_3A = S5K4ECGX_read_cmos_sensor(0x0F12);
		/* Index number of active capture configuration //Normal capture// */
...
BOOL S5K4ECGX_MIPI_set_param_exposure_for_HDR(UINT16 para)
{
	...
	if (S5K4ECGX_Driver.manualAEStart == 0) {
		S5K4ECGX_write_cmos_sensor(0x0028, 0x7000);
		S5K4ECGX_write_cmos_sensor(0x002A, 0x04E6);
		S5K4ECGX_write_cmos_sensor(0x0F12, 0x0779);	/* Manual AE enable */
		S5K4ECGX_Driver.manualAEStart = 1;
	...
	totalGain =
	    (totalGain > S5K4ECGX_MIPI_MAX_AXD_GAIN) ? S5K4ECGX_MIPI_MAX_AXD_GAIN : totalGain;
	exposureTime =  (exposureTime > S5K4ECGX_MIPI_MAX_EXPOSURE_TIME)
	    ? S5K4ECGX_MIPI_MAX_EXPOSURE_TIME : exposureTime;
...
void S5K4ECGX_MIPI_set_contrast(UINT16 para)
{
	...
	S5K4ECGX_write_cmos_sensor(0xFCFC, 0xD000), S5K4ECGX_write_cmos_sensor(0x0028, 0x7000);
...
void S5K4ECGX_MIPI_set_scene_mode(UINT16 para)
{
	...
	if (((prevSceneMode == SCENE_MODE_PORTRAIT) &&
	     (para != SCENE_MODE_PORTRAIT)) ||
	    ((prevSceneMode == SCENE_MODE_LANDSCAPE) && (para != SCENE_MODE_LANDSCAPE))) {
	...
	if (((prevSceneMode == SCENE_MODE_NIGHTSCENE) && (para != SCENE_MODE_NIGHTSCENE))) {
		S5K4ECGX_write_cmos_sensor(0x002A, 0x06B8);
		S5K4ECGX_write_cmos_sensor(0x0F12, 0x452C);
		S5K4ECGX_write_cmos_sensor(0x0F12, 0x0005);	/* lt_uMaxLei */
		S5K4ECGX_write_cmos_sensor(0x002A, 0x0A1E);
		S5K4ECGX_write_cmos_sensor(0x0F12, 0x0350);
		/* _ccm_oscar_iSaturation   [7:0] AFIT8_RGB2YUV_iYOffset */
	}
	/* Night mode and Sport mode will modfiy AE weighting table */
	/* Here we will determine if we nedd to rollback these setting */
	/* to default or not.... */
	if (((prevSceneMode == SCENE_MODE_NIGHTSCENE) &&
	     (para != SCENE_MODE_NIGHTSCENE)) ||
	    ((prevSceneMode == SCENE_MODE_SPORTS) && (para != SCENE_MODE_SPORTS))) {
		S5K4ECGX_MIPI_AE_ExpCurveGain_Config(para);
	} else if ((para == SCENE_MODE_NIGHTSCENE) || (para == SCENE_MODE_SPORTS)) {
		S5K4ECGX_MIPI_AE_ExpCurveGain_Config(para);
	}
	...
	case SCENE_MODE_NIGHTSCENE:
		...
		/* 0x15C0//_ccm_oscar_iSaturation   [7:0] AFIT8_RGB2YUV_iYOffset */
		break;
	...
	case SCENE_MODE_HDR:
		...
		spin_lock(&s5k4ecgx_mipi_drv_lock);
		if (S5K4ECGX_Driver.manualAEStart == 1) {
			S5K4ECGX_write_cmos_sensor(0x0028, 0x7000);
			S5K4ECGX_write_cmos_sensor(0x002A, 0x04E6);
...
UINT32 S5K4ECGX_MIPI_SetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate)
{
	/* kal_uint32 pclk; */
	/* kal_int16 dummyLine; */
...
UINT32 S5K4ECGX_MIPI_GetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId,
						   MUINT32 *pframeRate)
{
	switch (scenarioId) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
				    ...
				    UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_GET_PERIOD:
		SENSORDB("[4EC] F_GET_PERIOD, pix:%d, line:%d\n",
			 S5K4ECGX_Driver.Period_PixelNum, S5K4ECGX_Driver.Period_LineNum);
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
		SENSORDB("[4EC] F_PIXEL_CLOCK_FREQ, pclk:%d\n", S5K4ECGX_MIPI_sensor_pclk);
	...
	case SENSOR_FEATURE_GET_CONFIG_PARA:
		...
		memcpy(pSensorConfigData, &S5K4ECGXSensorConfigData,
	...
	case SENSOR_FEATURE_GET_EV_AWB_REF:
		...
		S5K4ECGX_MIPI_GetEvAwbRef((PSENSOR_AE_AWB_REF_STRUCT) *pFeatureData);
		break;

	case SENSOR_FEATURE_GET_SHUTTER_GAIN_AWB_GAIN:
		SENSORDB("[4EC] F_GET_SHUTTER_GAIN_AWB_GAIN\n");
		S5K4ECGX_MIPI_GetCurAeAwbInfo((PSENSOR_AE_AWB_CUR_STRUCT) *pFeatureData);
		break;
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		...
		S5K4ECGX_MIPI_SetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData,
							*(pFeatureData + 1));
		break;

	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		SENSORDB("[4EC] F_GET_DEFAULT_FRAME_RATE_BY_SCENARIO\n");
		S5K4ECGX_MIPI_GetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData,
							    (MUINT32
							     *) (uintptr_t) (*(pFeatureData + 1)));
		break;
	...
	case SENSOR_FEATURE_SET_AF_WINDOW:
		...
		S5K4ECGX_MIPI_AF_Set_Window((uintptr_t) *pFeatureData,
					    S5K4ECGX_Driver.Preview_Width,
					    S5K4ECGX_Driver.Preview_Height);
		break;

	case SENSOR_FEATURE_SET_AE_WINDOW:
		SENSORDB("[4EC] F_SET_AE_WINDOW\n");
		S5K4ECGX_MIPI_AE_Set_Window((uintptr_t) *pFeatureData,
					    S5K4ECGX_Driver.Preview_Width,
					    S5K4ECGX_Driver.Preview_Height);
		break;
	...
	case SENSOR_FEATURE_GET_AE_AWB_LOCK_INFO:
		...
		S5K4ECGX_MIPI_get_AEAWB_lock((UINT32 *) (uintptr_t) (*pFeatureData),
	...
	case SENSOR_FEATURE_GET_YUV_JPEG_INFO:
		SENSORDB("[4EC]GET_YUV_JPEG_INFO");
		{
			...
			SENSORDB
			    ("[4EC]GET_YUV_JPEG_INFO: jpegFileAddr=0x%p, maxBufSize=%d, infoAddr=0x%p\n",
			     jpegFileAddr, maxBufSize, jpegInfo);
			S5K4ECGX_MIPI_JPEG_Capture_Parser(jpegFileAddr, maxBufSize,
							  &S5K4ECGX_Driver.jpegSensorInfo);
			memcpy(jpegInfo, &S5K4ECGX_Driver.jpegSensorInfo,
			       sizeof(ACDK_SENSOR_JPEG_INFO));
	...
	case SENSOR_FEATURE_SET_YUV_JPEG_PARA:
		...
		{
			UINT32 *para = (UINT32 *) pFeaturePara;
			ACDK_SENSOR_JPEG_OUTPUT_PARA *jpegPara =
			    (ACDK_SENSOR_JPEG_OUTPUT_PARA *) para[0];
			SENSORDB("[4EC]width=%d, hegith=%d, Quality=%d\n", jpegPara->tgtWidth,
				 jpegPara->tgtHeight, jpegPara->quality);
			if ((jpegPara->tgtHeight == 0) || (jpegPara->tgtWidth == 0)
			    || (jpegPara->quality == 0) || (jpegPara->quality > 100)) {
				SENSORDB("[4EC]SET_JPEG_PARA: Invalid Para!\n");
				return ERROR_INVALID_PARA;
			}
			memcpy(&S5K4ECGX_Driver.jpegSensorPara, jpegPara,
			       sizeof(ACDK_SENSOR_JPEG_OUTPUT_PARA));
			SENSORDB
			    ("[4EC]SET_JPEG_PARA S5K4ECGX_Driver.jpegSensorPara width=%d, hegith=%d, Quality=%d\n",
			     S5K4ECGX_Driver.jpegSensorPara.tgtWidth,
			     S5K4ECGX_Driver.jpegSensorPara.tgtHeight,
			     S5K4ECGX_Driver.jpegSensorPara.quality);
		}
		break;
#endif
	case SENSOR_FEATURE_SET_MIN_MAX_FPS:
		SENSORDB("SENSOR_FEATURE_SET_MIN_MAX_FPS:[%d,%d]\n", (UINT32) *pFeatureData,
			 (UINT32) *(pFeatureData + 1));
