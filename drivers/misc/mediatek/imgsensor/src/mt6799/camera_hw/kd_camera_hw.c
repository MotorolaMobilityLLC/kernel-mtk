...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	if (IS_ERR(cam0_pnd_h)) {
		...
		PK_PR_ERR("%s : pinctrl err, cam0_pnd_h\n", __func__);
	}

	cam0_pnd_l = pinctrl_lookup_state(camctrl, "cam0_pnd0");
	if (IS_ERR(cam0_pnd_l)) {
		ret = PTR_ERR(cam0_pnd_l);
		PK_PR_ERR("%s : pinctrl err, cam0_pnd_l\n", __func__);
	}
	...
	if (IS_ERR(cam0_rst_h)) {
		...
		PK_PR_ERR("%s : pinctrl err, cam0_rst_h\n", __func__);
	}

	cam0_rst_l = pinctrl_lookup_state(camctrl, "cam0_rst0");
	if (IS_ERR(cam0_rst_l)) {
		ret = PTR_ERR(cam0_rst_l);
		PK_PR_ERR("%s : pinctrl err, cam0_rst_l\n", __func__);
	}
	...
	if (IS_ERR(cam1_pnd_h)) {
		...
		PK_PR_ERR("%s : pinctrl err, cam1_pnd_h\n", __func__);
	}

	cam1_pnd_l = pinctrl_lookup_state(camctrl, "cam1_pnd0");
	if (IS_ERR(cam1_pnd_l)) {
		ret = PTR_ERR(cam1_pnd_l);
		PK_PR_ERR("%s : pinctrl err, cam1_pnd_l\n", __func__);
	}
	...
	if (IS_ERR(cam1_rst_h)) {
		...
		PK_PR_ERR("%s : pinctrl err, cam1_rst_h\n", __func__);
	}
	...
	if (IS_ERR(cam1_rst_l)) {
		...
		PK_PR_ERR("%s : pinctrl err, cam1_rst_l\n", __func__);
	}
	...
	if (IS_ERR(cam2_rst_h)) {
		...
		PK_PR_ERR("%s : pinctrl err, cam2_rst_h\n", __func__);
	}
	...
	if (IS_ERR(cam2_rst_l)) {
		...
		PK_PR_ERR("%s : pinctrl err, cam2_rst_l\n", __func__);
	}
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case RST:
		if (PinIdx == 0) {
			if (Val == 0 && !IS_ERR(cam0_rst_l))
				...
				PK_PR_ERR("%s : pinctrl err, PinIdx %d, Val %d, RST\n", __func__,
				       PinIdx, Val);
		} else if (PinIdx == 1) {
	...
	case PDN:
		if (PinIdx == 0) {
			if (Val == 0 && !IS_ERR(cam0_pnd_l))
				...
				PK_PR_ERR("%s : pinctrl err, PinIdx %d, Val %d, PDN\n", __func__,
				       PinIdx, Val);
		} else if (PinIdx == 1) {
	...
	case MAIN2_AVDD:
		...
		if (Val == 0 && !IS_ERR(cam_ldo_vcamd_l)) {
			...
			if (mDVDD_usercounter <= 0) {
				if (mDVDD_usercounter < 0)
					PK_PR_ERR("Please check AVDD pin control\n");
	...
	case AFVDD:
	default:
		PK_PR_ERR("PwrType(%d) is invalid !!\n", PwrType);
		break;
	};
	...
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (_hwPowerOn(MAIN2_AVDD, pwInfo.Voltage) != TRUE) {
					PK_PR_ERR("[CAMERA SENSOR] Fail to enable digital power\n");
					return FALSE;
				}
			} else {
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (mtkcam_gpio_set(pinSetIdx, SUB_AVDD,
					...
					PK_PR_ERR("[CAMERA CUST_AVDD] set gpio failed!!\n");
				}
			}
		} else {
		...
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin ==
				...
				if (mtkcam_gpio_set(pinSetIdx, MAIN2_DVDD,
					...
					PK_PR_ERR("[CAMERA CUST_MAIN2_DVDD] set gpio failed!!\n");
				}
			}
		} else if (pinSetIdx == 1) {
			if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (mtkcam_gpio_set(pinSetIdx, SUB_DVDD,
					...
					PK_PR_ERR("[CAMERA CUST_SUB_DVDD] set gpio failed!!\n");
				}
			}
		} else {
			if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (mtkcam_gpio_set(pinSetIdx, DVDD,
					...
					PK_PR_ERR("[CAMERA CUST_DVDD] set gpio failed!!\n");
				}
			}
		}
		...
		if (PowerCustList.PowerCustInfo[CUST_DOVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (mtkcam_gpio_set(pinSetIdx, DOVDD, PowerCustList.PowerCustInfo[CUST_DOVDD].Voltage))
				PK_PR_ERR("[CAMERA CUST_DOVDD] set gpio failed!!\n");
		}
		...
		if (pwInfo.Voltage == Vol_High) {
			if (mtkcam_gpio_set(pinSetIdx, PDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]))
				PK_PR_ERR("[CAMERA SENSOR] set gpio failed!!\n");
		} else {
	...
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (_hwPowerDown(MAIN2_AVDD) != TRUE) {
					PK_PR_ERR("[CAMERA SENSOR] Fail to disable analog power2\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set(pinSetIdx, MAIN2_AVDD,
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (_hwPowerDown(SUB_AVDD) != TRUE) {
					PK_PR_ERR("[CAMERA SENSOR] Fail to disable analog power1\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set(pinSetIdx, SUB_AVDD,
			...
			if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (_hwPowerDown(AVDD) != TRUE) {
					PK_PR_ERR("[CAMERA SENSOR]Fail to disable analog power0\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set(pinSetIdx, AVDD,
		...
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin ==
				...
				if (_hwPowerDown(MAIN2_DVDD) != TRUE) {
					PK_PR_ERR("[CAMERA SENSOR] Fail to disable digital power2\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set(pinSetIdx, MAIN2_DVDD,
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (_hwPowerDown(SUB_DVDD) != TRUE) {
					PK_PR_ERR("[CAMERA SENSOR] Fail to disable digital power1\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set(pinSetIdx, SUB_DVDD,
			...
			if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (_hwPowerDown(DVDD) != TRUE) {
					PK_PR_ERR("[CAMERA SENSOR] Fail to disable digital power0\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set(pinSetIdx, DVDD,
		...
		if (PowerCustList.PowerCustInfo[CUST_DOVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (_hwPowerDown(pwInfo.PowerType) != TRUE) {
				PK_PR_ERR("[CAMERA SENSOR] Fail to disable io power\n");
				return FALSE;
			}
		} else {
		...
		if (PowerCustList.PowerCustInfo[CUST_AFVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (_hwPowerDown(pwInfo.PowerType) != TRUE) {
				PK_PR_ERR("[CAMERA SENSOR] Fail to disable af power\n");
				return FALSE;
			}
		} else {
