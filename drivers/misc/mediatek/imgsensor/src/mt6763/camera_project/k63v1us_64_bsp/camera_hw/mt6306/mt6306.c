	...
	if (pin == IMGSENSOR_HW_PIN_AVDD ||
	  (pin == IMGSENSOR_HW_PIN_DVDD &&
	   ...
	   sensor_idx == IMGSENSOR_SENSOR_IDX_SUB2)) {
		...
		PK_DBG("%s MIPI_SWITCH_EN pin number %d value %d\n", __func__,
		...
		PK_DBG("%s MIPI_SWITCH_SEL pin number %d value %d\n", __func__,
			 MT6306_PIN_CAM_MIPI_SWITCH_SEL, pin_state);
	} else {
		pin_offset =
			(sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN) ?
			MT6306_PIN_CAM_PDN0 : (sensor_idx ==
			IMGSENSOR_SENSOR_IDX_SUB) ? MT6306_PIN_CAM_PDN1 :
			MT6306_PIN_CAM_PDN2;
