...
static enum IMGSENSOR_RETURN gpio_init(void *pinstance)
{
	...
	if (IS_ERR(pgpio->ppinctrl)) {
		pr_info("Error: %s : Cannot find camera pinctrl!", __func__);
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
...
#endif
	{
		ctrl_state_offset =
			...
			GPIO_CTRL_STATE_CAM0_PDN_H : (sensor_idx ==
			IMGSENSOR_SENSOR_IDX_SUB) ? GPIO_CTRL_STATE_CAM1_PDN_H :
	...
	if (ppinctrl_state != NULL && !IS_ERR(ppinctrl_state))
		...
		pr_info("Error: %s : pinctrl err, PinIdx %d, Val %d\n",
