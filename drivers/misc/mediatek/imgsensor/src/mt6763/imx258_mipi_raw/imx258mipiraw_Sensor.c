...
#define LOG_2                                                                 \
	LOG_INF("preview 1600*1200@30fps,600Mbps/lane; video "                \
		"1600*1200@30fps,600Mbps/lane; "                              \
		"1600*1200@30fps,600Mbps/lane\n")
/****************************	Modify end
 * *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
	    ...
	    .grabwindow_width =
		2100, /* record different mode's width of grabwindow */
	    .grabwindow_height =
		1560, /* record different mode's height of grabwindow */
	    /* ollowing for MIPIDataLowPwr2HighSpeedSettleDelayCount
...
/* full size current fps : 24fps for PIP, 30fps for Normal or ZSD */
	...
	/* test pattern mode or not. KAL_FALSE for in
	 * test pattern mode,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	{4208, 3120, 4, 0, 4200, 3120, 2100, 1560, 0000, 0000, 2100, 1560, 0, 0,
	 2100, 1560}, /* Preview */
	{4208, 3120, 0, 0, 4208, 3120, 4208, 3120, 0000, 0000, 4208, 3120, 0, 0,
	 4208, 3120}, /*capture */
	{4208, 3120, 0, 0, 4208, 3120, 4208, 3120, 0000, 0000, 4208, 3120, 0, 0,
	 4208, 3120}, /*video */
	{4208, 3120, 8, 592, 4192, 1920, 1052, 480, 0000, 0000, 1048, 480, 0, 0,
	 1048, 480}, /*hight speed video */
	{4208, 3120, 4, 0, 4200, 3120, 2100, 1560, 0000, 0000, 2100, 1560, 0, 0,
	 2100, 1560}, /*slim video */
	/*add new mode */
	{4208, 3120, 0, 0, 4208, 3120, 4208, 3120, 0000, 0000, 4208, 3120, 0, 0,
	 4208, 3120}, /*custom1 */
	{4208, 3120, 4, 0, 4200, 3120, 2100, 1560, 0000, 0000, 2100, 1560, 0, 0,
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[3] = {
	...
	{0x03,   0x0a,   0x00, 0x08, 0x40,   0x00,   0x00, 0x2b,
	 0x0834, 0x0618, 0x00, 0x35, 0x0280, 0x0001, 0x00, 0x2f,
	 0x0000, 0x0000, 0x03, 0x00, 0x0000, 0x0000},
	/* Capture mode setting */
	{0x03,   0x0a,   0x00, 0x08, 0x40,   0x00,   0x00, 0x2b,
	 0x1070, 0x0C30, 0x00, 0x35, 0x0280, 0x0001, 0x00, 0x2f,
	 0x00A0, 0x0C00, 0x03, 0x00, 0x0000, 0x0000},
	/* Video mode setting */
	{0x02,   0x0a,   0x00, 0x08, 0x40,   0x00,   0x00, 0x2b,
	 0x1070, 0x0C30, 0x01, 0x00, 0x0000, 0x0000, 0x00, 0x2f,
	 0x00A0, 0x0C00, 0x03, 0x00, 0x0000, 0x0000}
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4PosR = {{26, 33},
	       ...
	       {54, 49} },
	.iMirrorFlip =
	0, /* 0:IMAGE_NORMAL,1:IMAGE_H_MIRROR,2:IMAGE_V_MIRROR,3:IMAGE_HV_MIRROR
	    */
};
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO_Binning[3] = {
	...
	{0x03,   0x0a,   0x00, 0x08, 0x40,   0x00,   0x00, 0x2b,
	 0x0834, 0x0618, 0x00, 0x35, 0x0280, 0x0001, 0x00, 0x2f,
	 0x00A0, 0x0602, 0x03, 0x00, 0x0000, 0x0000},
	/* Capture mode setting */
	{0x03,   0x0a,   0x00, 0x08, 0x40,   0x00,   0x00, 0x2b,
	 0x1070, 0x0C30, 0x00, 0x35, 0x0280, 0x0001, 0x00, 0x2f,
	 0x00A0, 0x0C00, 0x03, 0x00, 0x0000, 0x0000},
...
kal_uint16 IMX258MIPI_sensorGainMapping[IMX258MIPI_MaxGainIndex][2] = {
	...
	{410, 432}, {420, 434}, {431, 436}, {437, 437}, {449, 439}, {455, 440},
	{461, 441}, {468, 442}, {475, 443}, {482, 444}, {489, 445}, {496, 446},
	{504, 447}, {512, 448}, {520, 449}, {529, 450}, {537, 451}, {546, 452},
	{555, 453}, {565, 454}, {575, 455}, {585, 456}, {596, 457}, {607, 458},
	{618, 459}, {630, 460}, {642, 461}, {655, 462}, {669, 463}, {683, 464},
	{697, 465}, {713, 466}, {728, 467}, {745, 468}, {762, 469}, {780, 470},
	{799, 471}, {819, 472}, {840, 473}, {862, 474}, {886, 475}, {910, 476},
	{936, 477}, {964, 478}, {993, 479}, {1024, 480} };
...
static void load_imx258_SPC_Data(void)
{
	if (SPC_data_done == false) {
		if (!read_imx258_eeprom_SPC(0x0F73, IMX258MIPI_SPC_Data,
					    126)) {
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	LOG_INF("Gain mapping don't correctly:%d %d\n", gain,
		IMX258MIPI_sensorGainMapping[i - 1][0]);
...
static void preview_setting(void)
{
	...
	if (test_Pmode) {
		write_cmos_sensor(0x3030, 0x00);
		write_cmos_sensor(0x0B00,
				  0x00); /*default 0, need not to set */
		...
		write_cmos_sensor(
		    0x7BC8, 0x01); /*set in sensor_init(), need not to set */
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	s_DEVINFO_ccm->device_module =
	    "PC0FB0002B"; /* can change if got module id */
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imx258_type == IMX258_HDR_TYPE) {
		/* 0: NO PDAF, 1: PDAF Raw Data mode, 2:PDAF VC mode, 3:PDAF
		 * Binning mode
		 */
	...
	case SENSOR_FEATURE_GET_PDAF_TYPE:
		...
		case 0x00:
			sprintf(feature_para,
				"IMX258 0AQH5-C (BME-HDR version)");
			break;
		case 0x10:
			sprintf(feature_para,
				"IMX258 0APH5-C (Binning version)");
			break;
		case 0x20:
		...
		case 0x30:
			sprintf(feature_para,
				"IMX258 0ATH5-C (Non-PDAF version)");
			break;
		case 0x40:
			sprintf(feature_para,
				"IMX258 0AUH5-C (Horizontal Double Density version)");
