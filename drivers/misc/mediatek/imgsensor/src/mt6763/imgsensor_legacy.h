...
#define iReadRegI2C(a_pSendData, a_sizeSendData, a_pRecvData, a_sizeRecvData, i2cId)\
	iReadRegI2CTiming(a_pSendData, a_sizeSendData, a_pRecvData, a_sizeRecvData, i2cId, IMGSENSOR_I2C_SPEED)

#define iWriteRegI2CTiming(a_pSendData, a_sizeSendData, i2cId, timing)\
	iBurstWriteReg_multi(a_pSendData, a_sizeSendData, i2cId, a_sizeSendData, timing)

#define iBurstWriteReg(pData, bytes, i2cId)\
	iWriteRegI2CTiming(pData, bytes, i2cId, bytes, IMGSENSOR_I2C_SPEED)

#define iWriteRegI2C(a_pSendData, a_sizeSendData, i2cId)\
	iWriteRegI2CTiming(a_pSendData, a_sizeSendData, i2cId, IMGSENSOR_I2C_SPEED)

#define iMultiWriteReg(pData, lens, i2cId)\
	iWriteRegI2CTiming(pData, lens, i2cId, IMGSENSOR_I2C_SPEED)

#define imgsensor_legacy_init(sensor_func)\
do {\
	psensor_func = sensor_func;\
	psensor_func->version = IMGSENSOR_VERSION_V2;\
} while(0)
