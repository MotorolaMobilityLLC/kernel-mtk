...
static int imgsensor_i2c_remove(struct i2c_client *client) { return 0; }
...
enum IMGSENSOR_RETURN imgsensor_i2c_init(struct IMGSENSOR_I2C_CFG *pi2c_cfg,
...
enum IMGSENSOR_RETURN imgsensor_i2c_buffer_mode(int enable)
{
	...
	ret = (enable) ? hw_trig_i2c_enable(pinst->pi2c_client->adapter)
		       : hw_trig_i2c_disable(pinst->pi2c_client->adapter);

	return ret;
}

enum IMGSENSOR_RETURN imgsensor_i2c_read(struct IMGSENSOR_I2C_CFG *pi2c_cfg,
					 u8 *pwrite_data, u16 write_length,
					 u8 *pread_data, u16 read_length,
					 u16 id, int speed)
{
	...
	if (mtk_i2c_transfer(
		pinst->pi2c_client->adapter, pinst->msg,
		IMGSENSOR_I2C_MSG_SIZE_READ,
		(pi2c_cfg->pinst->status.filter_msg) ? I2C_A_FILTER_MSG : 0,
		((speed > 0) && (speed <= 1000))
		    ? speed * 1000
		    : IMGSENSOR_I2C_SPEED * 1000) !=
	    IMGSENSOR_I2C_MSG_SIZE_READ) {
		pr_err("I2C read failed (0x%x)! speed(0=%d)\n", ret, speed);
					  ...
					  int speed)
{
	...
	if (mtk_i2c_transfer(
		pinst->pi2c_client->adapter, pinst->msg, i,
		(pi2c_cfg->pinst->status.filter_msg) ? I2C_A_FILTER_MSG : 0,
		((speed > 0) && (speed <= 1000))
		    ? speed * 1000
		    : IMGSENSOR_I2C_SPEED * 1000) != i) {
		pr_err("I2C write failed (0x%x)! speed(0=%d)\n", ret,
			  speed);
		ret = IMGSENSOR_RETURN_ERROR;
	}
