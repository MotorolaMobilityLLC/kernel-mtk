...
#include "ov02a10macromipiraw_Sensor.h"
#ifndef VENDOR_EDIT
#define VENDOR_EDIT
#endif
#ifdef VENDOR_EDIT
...
#define PFX "OV02A10_MACRO_camera_sensor"
#define OV02A10_MACRO_EEPROM_SIZE 2453
#define EEPROM_DATA_PATH "/data/vendor/camera_dump/ov02a10_macro_eeprom_data.bin"
...
static  imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = OV02A10_MACRO_SENSOR_ID,
...
#if OTP_2A10
#define OV02A10_MACRO_EEPROM_SLAVE_ADD 0xA0
#define OV02A10_MACRO_SENSOR_IIC_SLAVE_ADD 0x7a
#define OV02A10_MACRO_OFILM_MODULE_ID  0x4f46

static uint8_t ov02a10_macro_eeprom[OV02A10_MACRO_EEPROM_SIZE] = {0};
...
static void ov02a10_macro_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_drv_lock);
	...
	//read eeprom data
	for (i = 0; i < size; i ++) {
		ov02a10_macro_eeprom[i] = read_cmos_sensor(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = OV02A10_MACRO_SENSOR_IIC_SLAVE_ADD;
	spin_unlock(&imgsensor_drv_lock);
}

static void ov02a10_macro_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
...
static calibration_status_t ov02a10_macro_check_manufacturing_data(void *data)
{
	struct ov02a10_macro_eeprom_t *eeprom = (struct ov02a10_macro_eeprom_t*)data;

	if (!eeprom_util_check_crc16(data, OV02A10_MACRO_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void ov02a10_macro_eeprom_format_calibration_data(void *data)
{
	if (NULL == data) {
	...
	mnf_status = ov02a10_macro_check_manufacturing_data(data);
	af_status = 0;
	awb_status = 0;
	lsc_status = 0;
	pdaf_status = 0;
...
static void ov02a10_macro_eeprom_get_mnf_data(void *data,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			write_cmos_sensor(0xfd, 0x00);
			*sensor_id = ((read_cmos_sensor(0x0200) << 8) | read_cmos_sensor(0x0300)) + MACRO_MID;
			if (*sensor_id == imgsensor_info.sensor_id) {
				ov02a10_macro_read_data_from_eeprom(OV02A10_MACRO_EEPROM_SLAVE_ADD, 0x0000, OV02A10_MACRO_EEPROM_SIZE);
				ov02a10_macro_eeprom_dump_bin(EEPROM_DATA_PATH, OV02A10_MACRO_EEPROM_SIZE, (void *)ov02a10_macro_eeprom);
				ov02a10_macro_eeprom_format_calibration_data((void *)ov02a10_macro_eeprom);
				LOG_INF("This is ov02a10_macro i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
				...
				MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	ov02a10_macro_eeprom_get_mnf_data((void *)ov02a10_macro_eeprom, &sensor_info->mnf_calibration);
...
{
	LOG_INF("OV02A10_MACRO_MIPI_RAW_SensorInit .....\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL) {
		*pfFunc = &sensor_func;
	}
	return ERROR_NONE;
}	/*	OV02A10_MACRO_MIPI_RAW_SensorInit	*/
