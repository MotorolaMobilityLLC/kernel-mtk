...
#define SBUYA_IMX882_EEPROM_SLAVE_ADDR 0xA0
#define SBUYA_IMX882_SENSOR_IIC_SLAVE_ADDR 0x20
#define SBUYA_IMX882_EEPROM_SIZE  0x27
#define SBUYA_IMX882_EEPROM_CRC_MANUFACTURING_SIZE 39
...
static uint8_t SBUYA_IMX882_eeprom[SBUYA_IMX882_EEPROM_SIZE] = {0};
...
static kal_uint16 SBUYA_IMX882_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void SBUYA_IMX882_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size, kal_uint8* eeprom_data)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		eeprom_data[i] = SBUYA_IMX882_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = SBUYA_IMX882_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}


static calibration_status_t SBUYA_IMX882_check_manufacturing_data(void *data)
{
	struct SBUYA_IMX882_eeprom_t *eeprom = (struct SBUYA_IMX882_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, SBUYA_IMX882_EEPROM_CRC_MANUFACTURING_SIZE-2,
...
static void SBUYA_IMX882_eeprom_get_mnf_data(void *data,
...
static int mot_imx882_get_spc_data()
{
	...
	SBUYA_IMX882_read_data_from_eeprom(SBUYA_IMX882_EEPROM_SLAVE_ADDR, IMX882_SPC_DATA_START, IMX882_SPC_DATA_LEN+2, imx882_spc_data);
...
static int mot_imx882_get_qsc_data()
{
	...
	SBUYA_IMX882_read_data_from_eeprom(SBUYA_IMX882_EEPROM_SLAVE_ADDR, IMX882_QSC_DATA_START, IMX882_QSC_DATA_LEN+2, imx882_qsc_data);
...
void SBUYA_IMX882_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	SBUYA_IMX882_read_data_from_eeprom(SBUYA_IMX882_EEPROM_SLAVE_ADDR, 0x0000, SBUYA_IMX882_EEPROM_CRC_MANUFACTURING_SIZE, SBUYA_IMX882_eeprom);
	calibration_status.mnf = SBUYA_IMX882_check_manufacturing_data(SBUYA_IMX882_eeprom);
	SBUYA_IMX882_eeprom_get_mnf_data((void *)SBUYA_IMX882_eeprom, &mnf_info);
	mot_imx882_get_spc_data();
	mot_imx882_get_qsc_data();
}

mot_calibration_status_t *SBUYA_IMX882_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *SBUYA_IMX882_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
