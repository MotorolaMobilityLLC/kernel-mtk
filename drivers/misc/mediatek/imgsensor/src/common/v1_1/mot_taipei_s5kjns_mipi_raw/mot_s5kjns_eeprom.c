...
typedef struct {
	MUINT16 addr;
	MUINT16 data;
} taipei_xtc_cal_addr_data_t;

#define PFX "MOT_TAIPEI_S5KJNS"
...
#define TAIPEI_S5KJNS_EEPROM_SLAVE_ADDR 0xA0
#define TAIPEI_S5KJNS_SENSOR_IIC_SLAVE_ADDR 0x20
#define TAIPEI_S5KJNS_EEPROM_SIZE  0x1CBF
#define TAIPEI_S5KJNS_EEPROM_CRC_MANUFACTURING_SIZE 37
#define TAIPEI_S5KJNS_EEPROM_CRC_AF_CAL_SIZE 24
#define TAIPEI_S5KJNS_EEPROM_CRC_AWB_CAL_SIZE 43
#define TAIPEI_S5KJNS_EEPROM_CRC_LSC_SIZE 1868
#define TAIPEI_S5KJNS_EEPROM_CRC_PDAF_OUTPUT1_SIZE 496
#define TAIPEI_S5KJNS_EEPROM_CRC_PDAF_OUTPUT2_SIZE 1004
#define TAIPEI_S5KJNS_EEPROM_CRC_XTC_SIZE 346
#define TAIPEI_S5KJNS_EEPROM_CRC_XTC_WRITE_SIZE (TAIPEI_S5KJNS_EEPROM_CRC_XTC_SIZE / 2 + 2)

static taipei_xtc_cal_addr_data_t hw_ggc_data[TAIPEI_S5KJNS_EEPROM_CRC_XTC_WRITE_SIZE] = {{0}};

int xtc_data_valid = 0;

static uint8_t TAIPEI_S5KJNS_eeprom[TAIPEI_S5KJNS_EEPROM_SIZE] = {0};
static mot_calibration_status_t calibration_status = {CRC_FAILURE};
static mot_calibration_mnf_t mnf_info = {0};

extern kal_uint16 mot_taipei_s5kjns_burst_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
static kal_uint16 TAIPEI_S5KJNS_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void TAIPEI_S5KJNS_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		TAIPEI_S5KJNS_eeprom[i] = TAIPEI_S5KJNS_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = TAIPEI_S5KJNS_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static calibration_status_t TAIPEI_S5KJNS_check_manufacturing_data(void *data)
{
	struct TAIPEI_S5KJNS_eeprom_t *eeprom = (struct TAIPEI_S5KJNS_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, TAIPEI_S5KJNS_EEPROM_CRC_MANUFACTURING_SIZE,
...
static calibration_status_t TAIPEI_S5KJNS_check_af_data(void *data)
{
	struct TAIPEI_S5KJNS_eeprom_t *eeprom = (struct TAIPEI_S5KJNS_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->af_data, TAIPEI_S5KJNS_EEPROM_CRC_AF_CAL_SIZE,
...
static calibration_status_t TAIPEI_S5KJNS_check_awb_data(void *data)
{
	struct TAIPEI_S5KJNS_eeprom_t *eeprom = (struct TAIPEI_S5KJNS_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
	...
	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		TAIPEI_S5KJNS_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t TAIPEI_S5KJNS_check_lsc_data_mtk(void *data)
{
	struct TAIPEI_S5KJNS_eeprom_t *eeprom = (struct TAIPEI_S5KJNS_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, TAIPEI_S5KJNS_EEPROM_CRC_LSC_SIZE,
...
static calibration_status_t TAIPEI_S5KJNS_check_pdaf_data(void *data)
{
	struct TAIPEI_S5KJNS_eeprom_t *eeprom = (struct TAIPEI_S5KJNS_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->pdaf_out1_data_mtk, TAIPEI_S5KJNS_EEPROM_CRC_PDAF_OUTPUT1_SIZE,
		convert_crc(eeprom->pdaf_out1_crc16_mtk))) {
		LOG_ERROR("PDAF OUTPUT1 CRC Fails!");
		return CRC_FAILURE;
	}

	if (!eeprom_util_check_crc16(eeprom->pdaf_out2_data_mtk, TAIPEI_S5KJNS_EEPROM_CRC_PDAF_OUTPUT2_SIZE,
...
static void TAIPEI_S5KJNS_eeprom_get_mnf_data(void *data,
...
int get_hw_ggc_data(void *data)
{
	int i;
	struct TAIPEI_S5KJNS_eeprom_t *eeprom = (struct TAIPEI_S5KJNS_eeprom_t*)data;
	if (!eeprom_util_check_crc16(eeprom->hw_ggc_data, TAIPEI_S5KJNS_EEPROM_CRC_XTC_SIZE,
	...
	else
	{
		...
		for (i = 0; i < TAIPEI_S5KJNS_EEPROM_CRC_XTC_SIZE; i += 2)
		{
...
void TAIPEI_S5KJNS_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	TAIPEI_S5KJNS_read_data_from_eeprom(TAIPEI_S5KJNS_EEPROM_SLAVE_ADDR, 0x00, TAIPEI_S5KJNS_EEPROM_SIZE);
	calibration_status.mnf = TAIPEI_S5KJNS_check_manufacturing_data(TAIPEI_S5KJNS_eeprom);
	calibration_status.af = TAIPEI_S5KJNS_check_af_data(TAIPEI_S5KJNS_eeprom);
	calibration_status.awb = TAIPEI_S5KJNS_check_awb_data(TAIPEI_S5KJNS_eeprom);
	calibration_status.lsc = TAIPEI_S5KJNS_check_lsc_data_mtk(TAIPEI_S5KJNS_eeprom);
	calibration_status.pdaf = TAIPEI_S5KJNS_check_pdaf_data(TAIPEI_S5KJNS_eeprom);
	calibration_status.dual = NO_ERRORS;

	TAIPEI_S5KJNS_eeprom_get_mnf_data((void *)TAIPEI_S5KJNS_eeprom, &mnf_info);

	get_hw_ggc_data(TAIPEI_S5KJNS_eeprom);
...
mot_calibration_status_t *TAIPEI_S5KJNS_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *TAIPEI_S5KJNS_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
...
{
	uint16_t write_table[TAIPEI_S5KJNS_EEPROM_CRC_XTC_WRITE_SIZE * 2] = {0};

	pr_debug("E\n");

	memcpy(write_table, &hw_ggc_data[0].addr, sizeof(write_table));

	mot_taipei_s5kjns_burst_write_cmos_sensor(write_table,
