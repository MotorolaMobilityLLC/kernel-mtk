...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] ={
	...
	{ 3264, 2448, 0, 0, 3264, 2448, 3264, 2448, 0, 0, 3264, 2448, 0, 0, 3264, 2448},	/* normal video */
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
    ...
    pr_debug(PFX, "framerate = %d, min framelength should enable?%d \n", framerate,min_framelength_en);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	pr_debug(PFX, "MOT_BOGOTA_SC820_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d, gain = %d\n",
...
{
	pr_debug(PFX, "le: 0x%x, se: 0x%x, gain: 0x%x\n", le, se, gain);
}
...
////min_frame_length
  ...
  if(target_frame_length > 1){
    ...
    write_cmos_sensor8(0x3e01, (shutter >> 4)&0xFF);
    write_cmos_sensor8(0x3e02, (shutter<<4) & 0xF0);
    pr_debug(PFX, "SSSS Exit! shutter = %d, framelength = %d\n", shutter, imgsensor.frame_length);
...
static kal_uint32 open(void)
{
	...
	pr_debug(PFX, "[open]: PLATFORM:MT6750,MIPI 24LANE\n");
	pr_debug(PFX, "preview 1296*972@30fps,360Mbps/lane; capture 2593*1944@30fps,880Mbps/lane\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_debug(PFX, "i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
			pr_debug(PFX, "open:Read sensor id fail open i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
		default:
			pr_debug(PFX, "Error ScenarioId setting");
			preview(image_window, sensor_config_data);
...
{
	pr_debug(PFX, "framerate = %d ", framerate);
...
{
	pr_debug(PFX, "enable = %d, framerate = %d ", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
	if (enable)
		imgsensor.autoflicker_en = KAL_TRUE;
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    pr_debug(PFX, "scenario_id = %d, framerate = %d\n", scenario_id, framerate);
    ...
    switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        	  if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
                ...
                spin_lock(&imgsensor_drv_lock);
                    ...
                    pr_debug(PFX, "Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",framerate,imgsensor_info.cap.max_framerate/10);
        ...
        default:  //coding with  preview scenario by default
			...
			if (imgsensor.frame_length > imgsensor.shutter)
            set_dummy();
            pr_debug(PFX, "error scenario_id = %d, we use preview scenario \n", scenario_id);
            break;
    }
    return ERROR_NONE;
}
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_REGISTER:
		...
		pr_debug(PFX, "adb_i2c_read 0x%x = 0x%x\n", sensor_reg_data->RegAddr,
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		...
		spin_unlock(&imgsensor_drv_lock);
		pr_debug(PFX, "current fps: %d\n", imgsensor.current_fps);
		break;
	case SENSOR_FEATURE_SET_HDR:
		spin_lock(&imgsensor_drv_lock);
		imgsensor.ihdr_en = (UINT16)*feature_data_32;
		spin_unlock(&imgsensor_drv_lock);
		pr_debug(PFX, "ihdr enable: %d\n", imgsensor.ihdr_en);
		break;
	case SENSOR_FEATURE_GET_CROP_INFO:
		pr_debug(PFX, "SENSOR_FEATURE_GET_CROP_INFO scenarioId: %d\n", (UINT32)*feature_data);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		pr_debug(PFX, "SENSOR_SET_SENSOR_IHDR LE = %d, SE = %d, Gain = %d\n",
    ...
    case SENSOR_FEATURE_GET_MIPI_PIXEL_RATE:
    {
        ...
        pr_debug(PFX, "hi556 SENSOR_FEATURE_GET_MIPI_PIXEL_RATE");
