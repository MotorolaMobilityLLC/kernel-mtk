...
#include "mot_nevada_s5kkdsfmmipiraw_Sensor.h"
...
#define PFX "MOT_NEVADA_S5KKDS_FM_"
...
#define NEVADA_S5KKDS_FM_EEPROM_SLAVE_ADDR 0xA2
#define NEVADA_S5KKDS_FM_SENSOR_IIC_SLAVE_ADDR 0x94
#define NEVADA_S5KKDS_FM_EEPROM_SIZE  0x0027
#define NEVADA_S5KKDS_FM_EEPROM_CRC_MANUFACTURING_SIZE 37

static uint8_t NEVADA_S5KKDS_FM_eeprom[NEVADA_S5KKDS_FM_EEPROM_SIZE] = {0};
...
static kal_uint16 NEVADA_S5KKDS_FM_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void NEVADA_S5KKDS_FM_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		NEVADA_S5KKDS_FM_eeprom[i] = NEVADA_S5KKDS_FM_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = NEVADA_S5KKDS_FM_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}


static calibration_status_t NEVADA_S5KKDS_FM_check_manufacturing_data(void *data)
{
	struct NEVADA_S5KKDS_FM_eeprom_t *eeprom = (struct NEVADA_S5KKDS_FM_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, NEVADA_S5KKDS_FM_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void NEVADA_S5KKDS_FM_eeprom_get_mnf_data(void *data,
...
void NEVADA_S5KKDS_FM_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	NEVADA_S5KKDS_FM_read_data_from_eeprom(NEVADA_S5KKDS_FM_EEPROM_SLAVE_ADDR, 0x00, NEVADA_S5KKDS_FM_EEPROM_SIZE);
	calibration_status.mnf = NEVADA_S5KKDS_FM_check_manufacturing_data(NEVADA_S5KKDS_FM_eeprom);
	NEVADA_S5KKDS_FM_eeprom_get_mnf_data((void *)NEVADA_S5KKDS_FM_eeprom, &mnf_info);
}

mot_calibration_status_t *NEVADA_S5KKDS_FM_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *NEVADA_S5KKDS_FM_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
