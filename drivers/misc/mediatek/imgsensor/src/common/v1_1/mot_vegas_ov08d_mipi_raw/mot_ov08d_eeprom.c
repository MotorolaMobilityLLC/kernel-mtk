...
#define VEGAS_OV08D_EEPROM_SLAVE_ADDR 0xA0
#define VEGAS_OV08D_SENSOR_IIC_SLAVE_ADDR 0x20
#define VEGAS_OV08D_EEPROM_SIZE  0x0027
#define VEGAS_OV08D_EEPROM_CRC_MANUFACTURING_SIZE 37

static uint8_t VEGAS_OV08D_eeprom[VEGAS_OV08D_EEPROM_SIZE] = {0};
...
static kal_uint16 VEGAS_OV08D_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void VEGAS_OV08D_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		VEGAS_OV08D_eeprom[i] = VEGAS_OV08D_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = VEGAS_OV08D_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}


static calibration_status_t VEGAS_OV08D_check_manufacturing_data(void *data)
{
	struct VEGAS_OV08D_eeprom_t *eeprom = (struct VEGAS_OV08D_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, VEGAS_OV08D_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void VEGAS_OV08D_eeprom_get_mnf_data(void *data,
		mot_calibration_mnf_t *mnf)
{
	...
	if (eeprom->lens_id[0] == 0x66){
		ret = snprintf(mnf->lens_id, MAX_CALIBRATION_STRING, "38127B-400");
	} else {
	...
	if (eeprom->manufacturer_id[0] == 'T' && eeprom->manufacturer_id[1] == 'S') {
...
void VEGAS_OV08D_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	VEGAS_OV08D_read_data_from_eeprom(VEGAS_OV08D_EEPROM_SLAVE_ADDR, 0x00, VEGAS_OV08D_EEPROM_SIZE);
	calibration_status.mnf = VEGAS_OV08D_check_manufacturing_data(VEGAS_OV08D_eeprom);
	VEGAS_OV08D_eeprom_get_mnf_data((void *)VEGAS_OV08D_eeprom, &mnf_info);
}

mot_calibration_status_t *VEGAS_OV08D_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *VEGAS_OV08D_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
