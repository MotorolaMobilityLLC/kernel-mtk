...
static nevada_ov_cal_addr_data_t ov_pdc_data[NEVADA_OV50D_EEPROM_CRC_PDC_WRITE_SIZE] = {{0}};
...
static kal_uint16 NEVADA_OV50D_read_cmos_sensor_8(kal_uint16 addr)
{
...
int get_ov_pdc_data(void *data)
{
	int i;
	struct NEVADA_OV50D_eeprom_t *eeprom = (struct NEVADA_OV50D_eeprom_t*)data;
	if (!eeprom_util_check_crc16(eeprom->ov_pdc_data, NEVADA_OV50D_EEPROM_CRC_PDC_SIZE,
	...
	else
	{
		...
		pdc_data_valid = 1;
		for (i = 0; i < NEVADA_OV50D_EEPROM_CRC_PDC_WRITE_SIZE; i++)
		{
...
{
	uint16_t write_table[NEVADA_OV50D_EEPROM_CRC_PDC_WRITE_SIZE * 2] = {0};

	pr_debug("E\n");

	memcpy(write_table, &ov_pdc_data[0].addr, sizeof(write_table));

	mot_nevada_ov50d_burst_write_cmos_sensor(write_table,
