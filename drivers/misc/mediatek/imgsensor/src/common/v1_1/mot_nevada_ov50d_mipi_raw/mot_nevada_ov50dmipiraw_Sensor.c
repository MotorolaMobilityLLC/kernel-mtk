 ...
 *============================================================================
 ****************************************************************************/
#define PFX "MOT_NEVADA_OV50D_camera_sensor"
...
#include "mot_nevada_ov50dmipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MOT_NEVADA_OV50D_SENSOR_ID,
...
{
	LOG_INF("MOT NEVADA OV50D preview_setting start\n");

	mot_nevada_ov50d_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_ov50d,
		sizeof(addr_data_pair_preview_mot_nevada_ov50d)/sizeof(kal_uint16));

	LOG_INF("MOT NEVADA OV50D preview_setting end\n");
...
{
	LOG_INF("MOT NEVADA OV50D capture_setting start\n");
	mot_nevada_ov50d_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_ov50d,
		sizeof(addr_data_pair_preview_mot_nevada_ov50d)/sizeof(kal_uint16));

	LOG_INF("MOT NEVADA OV50D capture_setting end\n");
}

static void normal_video_setting(kal_uint16 currefps)
{
	LOG_INF("MOT NEVADA OV50D normal_video_setting start\n");

	mot_nevada_ov50d_table_write_cmos_sensor(addr_data_pair_normal_video_mot_nevada_ov50d,
		sizeof(addr_data_pair_normal_video_mot_nevada_ov50d)/sizeof(kal_uint16));

	LOG_INF("MOT NEVADA OV50D normal_video_setting end\n");
}

static void hs_video_setting(void)
{
	LOG_INF("MOT NEVADA OV50D hs_video_setting start\n");

	mot_nevada_ov50d_table_write_cmos_sensor(addr_data_pair_120fps_ov50d,
		sizeof(addr_data_pair_120fps_ov50d)/sizeof(kal_uint16));

	LOG_INF("MOT NEVADA OV50D hs_video_setting end\n");
}

static void slim_video_setting(void)
{
	LOG_INF("MOT NEVADA OV50D slim_video_setting start\n");

	mot_nevada_ov50d_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_ov50d,
		sizeof(addr_data_pair_preview_mot_nevada_ov50d)/sizeof(kal_uint16));

	LOG_INF("MOT NEVADA OV50D slim_video_setting end\n");
}

static void custom1_setting(void)
{
	LOG_INF("MOT NEVADA OV50D custom1_setting start\n");

	mot_nevada_ov50d_table_write_cmos_sensor(addr_data_pair_30fps_ov50d,
		sizeof(addr_data_pair_30fps_ov50d)/sizeof(kal_uint16));
	LOG_INF("MOT NEVADA OV50D custom1_setting end\n");
}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			LOG_INF("Read sensor id fail,read:0x%x id: 0x%x\n", return_sensor_id(), imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_GAIN_RANGE_BY_SCENARIO:
		...
		*(feature_data + 2) = imgsensor_info.max_gain;
		...
		if(sensor_mode == FPS30_MODE) {
			*(feature_data + 2) = imgsensor_info.max_gain_30fps;
		}
		if(sensor_mode == FPS120_MODE) {
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((*(feature_data + 0)) == 0) {
			*(feature_data + 0) =
				sizeof(mot_nevada_ov50d_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)mot_nevada_ov50d_ana_gain_table,
			sizeof(mot_nevada_ov50d_ana_gain_table));
		}
		break;
...
UINT32 MOT_NEVADA_OV50D_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	return ERROR_NONE;
} /* MOT_NEVADA_OV50D_MIPI_RAW_SensorInit */