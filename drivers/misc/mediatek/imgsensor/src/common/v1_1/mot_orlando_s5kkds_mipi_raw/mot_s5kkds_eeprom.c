...
#define PFX "MOT_ORLANDO_S5KKDS_"
...
#define ORLANDO_S5KKDS_EEPROM_SLAVE_ADDR 0xA2
#define ORLANDO_S5KKDS_SENSOR_IIC_SLAVE_ADDR 0x94
#define ORLANDO_S5KKDS_EEPROM_SIZE  0x0027
#define ORLANDO_S5KKDS_EEPROM_CRC_MANUFACTURING_SIZE 37

static uint8_t ORLANDO_S5KKDS_eeprom[ORLANDO_S5KKDS_EEPROM_SIZE] = {0};
...
static kal_uint16 ORLANDO_S5KKDS_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void ORLANDO_S5KKDS_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		ORLANDO_S5KKDS_eeprom[i] = ORLANDO_S5KKDS_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = ORLANDO_S5KKDS_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}


static calibration_status_t ORLANDO_S5KKDS_check_manufacturing_data(void *data)
{
	struct ORLANDO_S5KKDS_eeprom_t *eeprom = (struct ORLANDO_S5KKDS_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, ORLANDO_S5KKDS_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void ORLANDO_S5KKDS_eeprom_get_mnf_data(void *data,
...
void ORLANDO_S5KKDS_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	ORLANDO_S5KKDS_read_data_from_eeprom(ORLANDO_S5KKDS_EEPROM_SLAVE_ADDR, 0x00, ORLANDO_S5KKDS_EEPROM_SIZE);
	calibration_status.mnf = ORLANDO_S5KKDS_check_manufacturing_data(ORLANDO_S5KKDS_eeprom);
	ORLANDO_S5KKDS_eeprom_get_mnf_data((void *)ORLANDO_S5KKDS_eeprom, &mnf_info);
}

mot_calibration_status_t *ORLANDO_S5KKDS_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *ORLANDO_S5KKDS_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
