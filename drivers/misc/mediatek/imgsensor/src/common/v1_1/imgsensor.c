...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	memset(&info, 0, sizeof(struct ACDK_SENSOR_INFO_STRUCT));
	memset(&sensor_resolution, 0,
	...
	snprintf(mtk_ccm_name,
		 sizeof(mtk_ccm_name),
		 "%s\nCAM[%d]:%s;",
		 mtk_ccm_name, psensor->inst.sensor_idx,
		 psensor->inst.psensor_list->name);
	snprintf(mtk_ccm_name,
		 sizeof(mtk_ccm_name),
		 "%s\nSensor ID = %x;", mtk_ccm_name,
	...
	snprintf(mtk_ccm_name,
		 ...
		 info.SensorGrabStartX_VD2, info.SensorGrabStartY_VD2,
