...
#include "mot_orlando_ov08d_Sensor_setting.h"
...
extern mot_calibration_status_t *ORLANDO_OV08D_eeprom_get_calibration_status(void);
extern mot_calibration_mnf_t *ORLANDO_OV08D_eeprom_get_mnf_info(void);
extern void ORLANDO_OV08D_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor);
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = MOT_ORLANDO_OV08D_SENSOR_ID,
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				ORLANDO_OV08D_eeprom_format_calibration_data(&imgsensor);
				return ERROR_NONE;
			}
			retry--;
		} while (retry > 0);
		      ...
		      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	mdelay(3);
	ov08d_table_write_cmos_sensor(addr_data_pair_preview_mot_orlando_ov08d,
			sizeof(addr_data_pair_preview_mot_orlando_ov08d)/sizeof(kal_uint16));
	LOG_INF("%s X\n", __func__);
	return ERROR_NONE;
}
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	write_cmos_sensor(0x20, 0x0e);
	mdelay(3);
	ov08d_table_write_cmos_sensor(addr_data_pair_normal_video_mot_orlando_ov08d,
			sizeof(addr_data_pair_normal_video_mot_orlando_ov08d)/sizeof(kal_uint16));
	LOG_INF("%s X\n", __func__);
	return ERROR_NONE;
}
		      ...
		      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	memcpy(&sensor_info->mnf_calibration, ORLANDO_OV08D_eeprom_get_mnf_info(), sizeof(mot_calibration_mnf_t));
	memcpy(&sensor_info->calibration_status, ORLANDO_OV08D_eeprom_get_calibration_status(), sizeof(mot_calibration_status_t));
...
UINT32 MOT_ORLANDO_OV08D_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
