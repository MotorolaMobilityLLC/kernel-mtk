...
typedef struct {
	MUINT16 addr;
	MUINT16 data;
} cancunn_ov_cal_addr_data_t;

#define PFX "MOT_CANCUNN_OV50D"
...
#define CANCUNN_OV50D_EEPROM_SLAVE_ADDR 0xA0
#define CANCUNN_OV50D_SENSOR_IIC_SLAVE_ADDR 0x20
#define CANCUNN_OV50D_EEPROM_SIZE  0x1CBF
#define CANCUNN_OV50D_EEPROM_CRC_MANUFACTURING_SIZE 37
#define CANCUNN_OV50D_EEPROM_CRC_AF_CAL_SIZE 24
#define CANCUNN_OV50D_EEPROM_CRC_AWB_CAL_SIZE 43
#define CANCUNN_OV50D_EEPROM_CRC_LSC_SIZE 1868
...
static uint8_t CANCUNN_OV50D_eeprom[CANCUNN_OV50D_EEPROM_SIZE] = {0};
...
static kal_uint16 CANCUNN_OV50D_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void CANCUNN_OV50D_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		CANCUNN_OV50D_eeprom[i] = CANCUNN_OV50D_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = CANCUNN_OV50D_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static calibration_status_t CANCUNN_OV50D_check_manufacturing_data(void *data)
{
	struct CANCUNN_OV50D_eeprom_t *eeprom = (struct CANCUNN_OV50D_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, CANCUNN_OV50D_EEPROM_CRC_MANUFACTURING_SIZE,
...
static calibration_status_t CANCUNN_OV50D_check_af_data(void *data)
{
	struct CANCUNN_OV50D_eeprom_t *eeprom = (struct CANCUNN_OV50D_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->af_data, CANCUNN_OV50D_EEPROM_CRC_AF_CAL_SIZE,
...
static calibration_status_t CANCUNN_OV50D_check_awb_data(void *data)
{
	struct CANCUNN_OV50D_eeprom_t *eeprom = (struct CANCUNN_OV50D_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
	...
	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		CANCUNN_OV50D_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t CANCUNN_OV50D_check_lsc_data_mtk(void *data)
{
	struct CANCUNN_OV50D_eeprom_t *eeprom = (struct CANCUNN_OV50D_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, CANCUNN_OV50D_EEPROM_CRC_LSC_SIZE,
...
static calibration_status_t CANCUNN_OV50D_check_pdaf_data(void *data)
{
	struct CANCUNN_OV50D_eeprom_t *eeprom = (struct CANCUNN_OV50D_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->pdaf_out1_data_mtk, CANCUNN_OV50D_EEPROM_CRC_PDAF_OUTPUT1_SIZE,
		convert_crc(eeprom->pdaf_out1_crc16_mtk))) {
		LOG_ERROR("PDAF OUTPUT1 CRC Fails!");
		return CRC_FAILURE;
	}

	if (!eeprom_util_check_crc16(eeprom->pdaf_out2_data_mtk, CANCUNN_OV50D_EEPROM_CRC_PDAF_OUTPUT2_SIZE,
...
static void CANCUNN_OV50D_eeprom_get_mnf_data(void *data,
...
void CANCUNN_OV50D_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	CANCUNN_OV50D_read_data_from_eeprom(CANCUNN_OV50D_EEPROM_SLAVE_ADDR, 0x00, CANCUNN_OV50D_EEPROM_SIZE);
	calibration_status.mnf = CANCUNN_OV50D_check_manufacturing_data(CANCUNN_OV50D_eeprom);
	calibration_status.af = CANCUNN_OV50D_check_af_data(CANCUNN_OV50D_eeprom);
	calibration_status.awb = CANCUNN_OV50D_check_awb_data(CANCUNN_OV50D_eeprom);
	calibration_status.lsc = CANCUNN_OV50D_check_lsc_data_mtk(CANCUNN_OV50D_eeprom);
	calibration_status.pdaf = CANCUNN_OV50D_check_pdaf_data(CANCUNN_OV50D_eeprom);
	calibration_status.dual = NO_ERRORS;

	CANCUNN_OV50D_eeprom_get_mnf_data((void *)CANCUNN_OV50D_eeprom, &mnf_info);

	get_ov_pdc_data(CANCUNN_OV50D_eeprom);
...
mot_calibration_status_t *CANCUNN_OV50D_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *CANCUNN_OV50D_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
...
{
	uint16_t write_table[CANCUNN_OV50D_EEPROM_CRC_PDC_WRITE_SIZE * 2] = {0};
