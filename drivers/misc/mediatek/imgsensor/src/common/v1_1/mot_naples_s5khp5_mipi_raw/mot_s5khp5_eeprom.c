...
#define NAPLES_S5KHP5_EEPROM_SLAVE_ADDR 0xA0
#define NAPLES_S5KHP5_SENSOR_IIC_SLAVE_ADDR 0x20
#define NAPLES_S5KHP5_EEPROM_SIZE  0x0027
#define NAPLES_S5KHP5_EEPROM_CRC_MANUFACTURING_SIZE 37

static uint8_t NAPLES_S5KHP5_eeprom[NAPLES_S5KHP5_EEPROM_SIZE] = {0};
...
static kal_uint16 NAPLES_S5KHP5_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void NAPLES_S5KHP5_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		NAPLES_S5KHP5_eeprom[i] = NAPLES_S5KHP5_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = NAPLES_S5KHP5_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}


static calibration_status_t NAPLES_S5KHP5_check_manufacturing_data(void *data)
{
	struct NAPLES_S5KHP5_eeprom_t *eeprom = (struct NAPLES_S5KHP5_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, NAPLES_S5KHP5_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void NAPLES_S5KHP5_eeprom_get_mnf_data(void *data,
		mot_calibration_mnf_t *mnf)
{
	...
	if (eeprom->lens_id[0] == 0x65){
		ret = snprintf(mnf->lens_id, MAX_CALIBRATION_STRING, "Sunny39733A-400");
	} else {
...
void NAPLES_S5KHP5_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	NAPLES_S5KHP5_read_data_from_eeprom(NAPLES_S5KHP5_EEPROM_SLAVE_ADDR, 0x00, NAPLES_S5KHP5_EEPROM_SIZE);
	calibration_status.mnf = NAPLES_S5KHP5_check_manufacturing_data(NAPLES_S5KHP5_eeprom);
	NAPLES_S5KHP5_eeprom_get_mnf_data((void *)NAPLES_S5KHP5_eeprom, &mnf_info);
}

mot_calibration_status_t *NAPLES_S5KHP5_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *NAPLES_S5KHP5_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
