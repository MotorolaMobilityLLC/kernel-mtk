...
static int mot_eerpom_debug = 1;
#define OV32BPFX "[imgsensor] mot_lyriq_ov32beeprom"
#define OV32BLOG_INF(format, args...)        do { if (mot_eerpom_debug   ) { pr_err(OV32BPFX"[%s] " format, __func__,##args); } } while(0)
#define LOG_ERROR(format, args...)   pr_err(OV32BPFX "[%s] " format, __func__, ##args)

#define OV32B_SERIAL_NUM_SIZE 16
...
#define LYRIQ_OV32B_EEPROM_SLAVE_ADDR 0xA2
#define LYRIQ_OV32B_SENSOR_IIC_SLAVE_ADDR 0x20
#define LYRIQ_OV32B_EEPROM_SIZE  0x1065
#define EEPROM_DATA_PATH "/data/vendor/camera_dump/mot_gt24p64e_ov32b_eeprom.bin"
#define LYRIQ_OV32B_EEPROM_CRC_AF_CAL_SIZE 24
#define LYRIQ_OV32B_EEPROM_CRC_AWB_CAL_SIZE 43
#define LYRIQ_OV32B_EEPROM_CRC_LSC_SIZE 1868
#define LYRIQ_OV32B_EEPROM_CRC_MANUFACTURING_SIZE 37
#define LYRIQ_OV32B_MANUFACTURE_PART_NUMBER "28D14866"
#define LYRIQ_OV32B_MPN_LENGTH 8

static uint8_t LYRIQ_OV32B_eeprom[LYRIQ_OV32B_EEPROM_SIZE] = {0};
...
static struct imgsensor_struct ov32b_imgsensor = {
	.mirror = IMAGE_NORMAL,
...
#if 1
static kal_uint16 ov32b_read_cmos_sensor_8(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *)&get_byte, 1, ov32b_imgsensor.i2c_write_id);
	return get_byte;
}
#endif
...
static void LYRIQ_OV32B_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_lock);
	ov32b_imgsensor.i2c_write_id = slave;
	spin_unlock(&imgsensor_lock);

	//read eeprom data
	for (i = 0; i < size; i ++) {
		LYRIQ_OV32B_eeprom[i] = ov32b_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	ov32b_imgsensor.i2c_write_id = LYRIQ_OV32B_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static void LYRIQ_OV32B_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
	...
	OV32BLOG_INF("wirte to file(%s)\n", file_name);
p_err:
	if (!IS_ERR_OR_NULL(fp))
		filp_close(fp, NULL);

	set_fs(old_fs);
	OV32BLOG_INF(" end writing file");
}
...
static int32_t eeprom_util_check_crc16(uint8_t *data, uint32_t size, uint32_t ref_crc)
{
	...
	* how CRC should be calculated. */
	...
	OV32BLOG_INF("REF_CRC 0x%x CALC CRC 0x%x CALC Reverse CRC 0x%x matches? %d\n",
		...
		awb_limit_t limit)
{
	...
	OV32BLOG_INF("rg = %d, bg=%d,rgmin=%d,bgmax =%d\n",r_g,b_g,r_g_golden_min,r_g_golden_max);
	OV32BLOG_INF("grg = %d, gbg=%d,bgmin=%d,bgmax =%d\n",golden_rg,golden_bg,b_g_golden_min,b_g_golden_max);
	if (r_g < (golden_rg - r_g_golden_min) || r_g > (golden_rg + r_g_golden_max)) {
		OV32BLOG_INF("Final RG calibration factors out of range!");
		return 1;
	}

	if (b_g < (golden_bg - b_g_golden_min) || b_g > (golden_bg + b_g_golden_max)) {
		OV32BLOG_INF("Final BG calibration factors out of range!");
		return 1;
	}
	return 0;
}

static calibration_status_t LYRIQ_OV32B_check_awb_data(void *data)
{
	struct LYRIQ_OV32B_eeprom_t *eeprom = (struct LYRIQ_OV32B_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
	awb_limit_t golden_limit;

	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		LYRIQ_OV32B_EEPROM_CRC_AWB_CAL_SIZE,
	...
	OV32BLOG_INF("AWB CRC Pass");
	return NO_ERRORS;
}

static calibration_status_t LYRIQ_OV32B_check_lsc_data_mtk(void *data)
{
	struct LYRIQ_OV32B_eeprom_t *eeprom = (struct LYRIQ_OV32B_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, LYRIQ_OV32B_EEPROM_CRC_LSC_SIZE,
		convert_crc(eeprom->lsc_crc16_mtk))) {
		LOG_ERROR("LSC CRC Fails!");
		return CRC_FAILURE;
	}
	OV32BLOG_INF("LSC CRC Pass");
	return NO_ERRORS;
}

static void LYRIQ_OV32B_eeprom_get_mnf_data(void *data,
		mot_calibration_mnf_t *mnf)
{
	...
	if (eeprom->manufacturer_id[0] == 'S' && eeprom->manufacturer_id[1] == 'U') {
		...
		OV32BLOG_INF("unknown manufacturer_id");
	}
	...
	LYRIQ_OV32B_eeprom_dump_bin(FRONT_SERIAL_NUM_DATA_PATH,  OV32B_SERIAL_NUM_SIZE,  eeprom->serial_number);
	if (ret < 0 || ret >= MAX_CALIBRATION_STRING) {
...
static calibration_status_t LYRIQ_OV32B_check_manufacturing_data(void *data)
{
	struct LYRIQ_OV32B_eeprom_t *eeprom = (struct LYRIQ_OV32B_eeprom_t*)data;
        OV32BLOG_INF("Manufacturing eeprom->mpn = %s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(data, LYRIQ_OV32B_EEPROM_CRC_MANUFACTURING_SIZE,
		convert_crc(eeprom->manufacture_crc16))) {
		LOG_ERROR("Manufacturing CRC Fails!");
		return CRC_FAILURE;
	}
	OV32BLOG_INF("Manufacturing CRC Pass");
	return NO_ERRORS;
}

static void LYRIQ_OV32B_eeprom_format_calibration_data(void *data)
{
	if (NULL == data) {
		OV32BLOG_INF("data is NULL");
		return;
	}

	mnf_status = LYRIQ_OV32B_check_manufacturing_data(data);
	af_status = 0;
	awb_status = LYRIQ_OV32B_check_awb_data(data);
	lsc_status = LYRIQ_OV32B_check_lsc_data_mtk(data);
	pdaf_status = 0;
	dual_status = 0;

	OV32BLOG_INF("status mnf:%d, af:%d, awb:%d, lsc:%d, pdaf:%d, dual:%d",
