...
{
	pr_debug("extend_frame_length_en %d fll %d\n",imgsensor.extend_frame_length_en, fll);
...
static void hdr_write_tri_shutter(kal_uint16 le, kal_uint16 me, kal_uint16 se)
{
	...
	kal_uint16 pre_se = read_cmos_sensor_8(0x3541) << 8 | read_cmos_sensor_8(0x3542);
	pr_debug("max! le:0x%x, me:0x%x, se:0x%x pre_se 0x%x, frame_length %d\n",
		le, me, se, pre_se, imgsensor.frame_length);
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable)
		...
		write_cmos_sensor_8(0x3208, 0x02); // reset switch_group2
		write_cmos_sensor_8(0x0000, 0x00);
		write_cmos_sensor_8(0x3208, 0x12);
		write_cmos_sensor_8(0x3208, 0xa2);
		write_cmos_sensor_8(0x3208, 0x01); // reset switch_group2
		write_cmos_sensor_8(0x0000, 0x00);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor_8(0x53c1, 0x01);
		write_cmos_sensor_8(0x53c2, 0xff);
	} else {
		write_cmos_sensor_8(0x50c1, 0x00);
		write_cmos_sensor_8(0x50c2, 0x00);
		write_cmos_sensor_8(0x53c1, 0x00);
		write_cmos_sensor_8(0x53c2, 0x00);
	}
	spin_lock(&imgsensor_drv_lock);
