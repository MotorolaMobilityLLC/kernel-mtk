		...
		enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	if (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN &&
	    !strstr(IMGSENSOR_STRINGIZE(CONFIG_CUSTOM_KERNEL_MAIN_IMGSENSOR), curr_sensor_name))
		return IMGSENSOR_RETURN_ERROR;
#endif
#ifdef CONFIG_CUSTOM_KERNEL_SUB_IMGSENSOR
	if (sensor_idx == IMGSENSOR_SENSOR_IDX_SUB &&
	    !strstr(IMGSENSOR_STRINGIZE(CONFIG_CUSTOM_KERNEL_SUB_IMGSENSOR), curr_sensor_name))
		return IMGSENSOR_RETURN_ERROR;
#endif

#ifdef CONFIG_CUSTOM_KERNEL_MAIN2_IMGSENSOR
	if (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN2 &&
	    !strstr(IMGSENSOR_STRINGIZE(CONFIG_CUSTOM_KERNEL_MAIN2_IMGSENSOR), curr_sensor_name))
		return IMGSENSOR_RETURN_ERROR;
#endif
	...
	if (sensor_idx == IMGSENSOR_SENSOR_IDX_SUB2 &&
	    strncmp(curr_sensor_name,
		    strstr(psensor_name_config, curr_sensor_name), strlen(curr_sensor_name)))
		return IMGSENSOR_RETURN_ERROR;
#endif
