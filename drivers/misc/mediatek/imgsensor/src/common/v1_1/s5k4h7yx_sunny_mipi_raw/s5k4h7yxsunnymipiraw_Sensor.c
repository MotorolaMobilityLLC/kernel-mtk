...
static int s5k4h7yx_read_data_kernel(void)
{
	...
	for (i = 0; i < 30 ; i++ ) {
		if (i == 29) {
			s5k4h7yx_read_otp_page_data(page + i, 0x0a04, s5k4h7yx_sunny_otp_data.lscdata + (64*i), 15);
		} else {
			s5k4h7yx_read_otp_page_data(page + i, 0x0a04, s5k4h7yx_sunny_otp_data.lscdata + (64*i), 64);
		}
...
unsigned int S5K4H7_SUNNY_OTP_Read_Data(unsigned int addr,unsigned char *data, unsigned int size)
{
	if (size == 2) { //read module id
		...
		if (addr == 0x0a04) {
			memcpy(data, (s5k4h7yx_sunny_otp_data.lscdata), size);
		}else {
			memcpy(data, (s5k4h7yx_sunny_otp_data.lscdata + 1), size);
		}
