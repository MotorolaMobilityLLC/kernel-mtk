...
static void table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
	...
	pr_debug(PFX,"write 0x%x = 0x%x\n",addr, data);
	test_val = read_cmos_sensor(addr);
	pr_debug(PFX,"read 0x%x = 0x%x\n",addr,test_val);
	}
}
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	case IMAGE_HV_MIRROR:
		...
		break;
	default:
		pr_debug(PFX,"Error image_mirror setting\n");
		break;
	}
}
...
static void write_shutter(kal_uint32 shutter)
{
	...
	pr_debug(PFX,"shutter =%d, framelength =%d\n",
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < imgsensor_info.min_gain || gain > max_gain) {
		pr_debug(PFX,"Error max gain setting: %d\n", max_gain);
	...
	spin_unlock(&imgsensor_drv_lock);
	pr_debug(PFX,"gain = %d, reg_gain = 0x%x, max_gain:0x%x\n ",
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	pr_debug(PFX,"streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable)
...
static void preview_setting(void)
{
	...
	pr_debug(PFX,"%s preview_End\n", __func__);
}

static void capture_setting(kal_uint16 currefps)
{
	pr_debug(PFX,"[%s] capture_Start, currefps:%d\n", __func__, currefps);
	table_write_cmos_sensor(mot_bogota_gc08a8_3264x2448_addr_data,
		sizeof(mot_bogota_gc08a8_3264x2448_addr_data)/sizeof(kal_uint16));
	pr_debug(PFX,"[%s] capture_End\n", __func__);
}

static void normal_video_setting(kal_uint16 currefps)
{
	pr_debug(PFX,"[%s] normal_video_Start, currefps:%d\n", __func__, currefps);
	table_write_cmos_sensor(mot_bogota_gc08a8_3264x2448_addr_data,
		sizeof(mot_bogota_gc08a8_3264x2448_addr_data)/sizeof(kal_uint16));
	pr_debug(PFX,"[%s] normal_video_End\n", __func__);
}
...
static kal_uint32 set_auto_flicker_mode(kal_bool enable, UINT16 framerate)
{
	...
	if (enable) /*enable auto flicker*/ {
		...
		pr_debug(PFX,"enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		imgsensor.autoflicker_en = KAL_FALSE;
	}
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		pr_debug(PFX,"SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		pr_debug(PFX,"SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		pr_debug(PFX,"SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		pr_debug(PFX,"SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
