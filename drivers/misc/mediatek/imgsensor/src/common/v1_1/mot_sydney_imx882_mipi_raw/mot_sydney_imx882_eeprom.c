...
#include "mot_sydney_imx882mipiraw_Sensor.h"

static int mot_sensor_debug = 1;


#define PFX "MOT_SYDNEY_IMX882"
...
#define SYDNEY_IMX882_EEPROM_SLAVE_ADDR 0xA0
#define SYDNEY_IMX882_SENSOR_IIC_SLAVE_ADDR 0x20
#define SYDNEY_IMX882_EEPROM_SIZE  0x27
#define SYDNEY_IMX882_EEPROM_CRC_MANUFACTURING_SIZE 39
...
static uint8_t SYDNEY_IMX882_eeprom[SYDNEY_IMX882_EEPROM_SIZE] = {0};
static mot_calibration_status_t calibration_status = {CRC_FAILURE};
static mot_calibration_mnf_t mnf_info = {0};

extern kal_uint16 mot_imx882_burst_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
static kal_uint16 SYDNEY_IMX882_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void SYDNEY_IMX882_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size, kal_uint8* eeprom_data)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		eeprom_data[i] = SYDNEY_IMX882_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = SYDNEY_IMX882_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}


static calibration_status_t SYDNEY_IMX882_check_manufacturing_data(void *data)
{
	struct SYDNEY_IMX882_eeprom_t *eeprom = (struct SYDNEY_IMX882_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, SYDNEY_IMX882_EEPROM_CRC_MANUFACTURING_SIZE-2,
...
static void SYDNEY_IMX882_eeprom_get_mnf_data(void *data,
...
static int mot_imx882_get_spc_data()
{
	...
	SYDNEY_IMX882_read_data_from_eeprom(SYDNEY_IMX882_EEPROM_SLAVE_ADDR, IMX882_SPC_DATA_START, IMX882_SPC_DATA_LEN+2, imx882_spc_data);
...
static int mot_imx882_get_qsc_data()
{
	...
	SYDNEY_IMX882_read_data_from_eeprom(SYDNEY_IMX882_EEPROM_SLAVE_ADDR, IMX882_QSC_DATA_START, IMX882_QSC_DATA_LEN+2, imx882_qsc_data);
...
void imx882_qsc_spc_apply(void)
{
	...
	if (imx882_spc_data_ready) {
		mot_imx882_burst_write_cmos_sensor(imx882_SPC_setting, sizeof(imx882_SPC_setting) / sizeof(kal_uint16));
		LOG_INF("SPC apply done.");
	}
	if (imx882_qsc_data_ready) {
		write_cmos_sensor_8(0x3206, 0x01);
		mot_imx882_burst_write_cmos_sensor(imx882_QSC_setting, sizeof(imx882_QSC_setting) / sizeof(kal_uint16));
		LOG_INF("QSC apply done.");
	}
	LOG_INF("imx882_qsc_spc_apply X");
}

void SYDNEY_IMX882_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	SYDNEY_IMX882_read_data_from_eeprom(SYDNEY_IMX882_EEPROM_SLAVE_ADDR, 0x0000, SYDNEY_IMX882_EEPROM_CRC_MANUFACTURING_SIZE, SYDNEY_IMX882_eeprom);
	calibration_status.mnf = SYDNEY_IMX882_check_manufacturing_data(SYDNEY_IMX882_eeprom);
	SYDNEY_IMX882_eeprom_get_mnf_data((void *)SYDNEY_IMX882_eeprom, &mnf_info);
	mot_imx882_get_spc_data();
	mot_imx882_get_qsc_data();
}

mot_calibration_status_t *SYDNEY_IMX882_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *SYDNEY_IMX882_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
