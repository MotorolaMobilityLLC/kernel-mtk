...
#include "mot_devonf_ov50amipiraw_Sensor.h"
...
typedef struct {
	MUINT16 addr;
	MUINT16 data;
} devonf_ov_cal_addr_data_t;

#define PFX "MOT_DEVONF_OV50A"
...
#define DEVONF_OV50A_EEPROM_SLAVE_ADDR 0xA0
#define DEVONF_OV50A_SENSOR_IIC_SLAVE_ADDR 0x20
#define DEVONF_OV50A_EEPROM_SIZE  0x1DEC
#define DEVONF_OV50A_EEPROM_CRC_MANUFACTURING_SIZE 37
#define DEVONF_OV50A_EEPROM_CRC_AF_CAL_SIZE 24
#define DEVONF_OV50A_EEPROM_CRC_AWB_CAL_SIZE 43
#define DEVONF_OV50A_EEPROM_CRC_LSC_SIZE 1868
#define DEVONF_OV50A_EEPROM_CRC_PDAF_OUTPUT1_SIZE 496
#define DEVONF_OV50A_EEPROM_CRC_PDAF_OUTPUT2_SIZE 1004
#define DEVONF_OV50A_EEPROM_CRC_XTALK_SIZE 288
#define DEVONF_OV50A_EEPROM_CRC_PDC_SIZE 732
#define DEVONF_OV50A_EEPROM_CRC_PDC_WRITE_SIZE 720

static devonf_ov_cal_addr_data_t ov_cross_talk_data[DEVONF_OV50A_EEPROM_CRC_XTALK_SIZE] = {{0}};
static devonf_ov_cal_addr_data_t ov_pdc_data[DEVONF_OV50A_EEPROM_CRC_PDC_WRITE_SIZE] = {{0}};

int xtalk_data_valid = 0;
int pdc_data_valid = 0;

static uint8_t DEVONF_OV50A_eeprom[DEVONF_OV50A_EEPROM_SIZE] = {0};
static mot_calibration_status_t calibration_status = {CRC_FAILURE};
static mot_calibration_mnf_t mnf_info = {0};

extern kal_uint16 mot_devonf_ov50a_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
static kal_uint16 DEVONF_OV50A_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void DEVONF_OV50A_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		DEVONF_OV50A_eeprom[i] = DEVONF_OV50A_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = DEVONF_OV50A_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static calibration_status_t DEVONF_OV50A_check_manufacturing_data(void *data)
{
	struct DEVONF_OV50A_eeprom_t *eeprom = (struct DEVONF_OV50A_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, DEVONF_OV50A_EEPROM_CRC_MANUFACTURING_SIZE,
...
static calibration_status_t DEVONF_OV50A_check_af_data(void *data)
{
	struct DEVONF_OV50A_eeprom_t *eeprom = (struct DEVONF_OV50A_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->af_data, DEVONF_OV50A_EEPROM_CRC_AF_CAL_SIZE,
...
static calibration_status_t DEVONF_OV50A_check_awb_data(void *data)
{
	struct DEVONF_OV50A_eeprom_t *eeprom = (struct DEVONF_OV50A_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
	...
	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		DEVONF_OV50A_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t DEVONF_OV50A_check_lsc_data_mtk(void *data)
{
	struct DEVONF_OV50A_eeprom_t *eeprom = (struct DEVONF_OV50A_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, DEVONF_OV50A_EEPROM_CRC_LSC_SIZE,
...
static calibration_status_t DEVONF_OV50A_check_pdaf_data(void *data)
{
	struct DEVONF_OV50A_eeprom_t *eeprom = (struct DEVONF_OV50A_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->pdaf_out1_data_mtk, DEVONF_OV50A_EEPROM_CRC_PDAF_OUTPUT1_SIZE,
		convert_crc(eeprom->pdaf_out1_crc16_mtk))) {
		LOG_ERROR("PDAF OUTPUT1 CRC Fails!");
		return CRC_FAILURE;
	}

	if (!eeprom_util_check_crc16(eeprom->pdaf_out2_data_mtk, DEVONF_OV50A_EEPROM_CRC_PDAF_OUTPUT2_SIZE,
...
static void DEVONF_OV50A_eeprom_get_mnf_data(void *data,
...
int get_ov_cross_talk_data(void *data)
{
	int i;
	struct DEVONF_OV50A_eeprom_t *eeprom = (struct DEVONF_OV50A_eeprom_t*)data;
	if (!eeprom_util_check_crc16(eeprom->ov_xtalk_data, DEVONF_OV50A_EEPROM_CRC_XTALK_SIZE,
	...
	else
	{
		...
		xtalk_data_valid = 1;
		for (i = 0; i < DEVONF_OV50A_EEPROM_CRC_XTALK_SIZE; i++)
		{
...
int get_ov_pdc_data(void *data)
{
	int i;
	struct DEVONF_OV50A_eeprom_t *eeprom = (struct DEVONF_OV50A_eeprom_t*)data;
	if (!eeprom_util_check_crc16(eeprom->ov_pdc_data, DEVONF_OV50A_EEPROM_CRC_PDC_SIZE,
	...
	else
	{
		...
		pdc_data_valid = 1;
		for (i = 0; i < DEVONF_OV50A_EEPROM_CRC_PDC_WRITE_SIZE; i++)
		{
...
void DEVONF_OV50A_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	DEVONF_OV50A_read_data_from_eeprom(DEVONF_OV50A_EEPROM_SLAVE_ADDR, 0x00, DEVONF_OV50A_EEPROM_SIZE);
	calibration_status.mnf = DEVONF_OV50A_check_manufacturing_data(DEVONF_OV50A_eeprom);
	calibration_status.af = DEVONF_OV50A_check_af_data(DEVONF_OV50A_eeprom);
	calibration_status.awb = DEVONF_OV50A_check_awb_data(DEVONF_OV50A_eeprom);
	calibration_status.lsc = DEVONF_OV50A_check_lsc_data_mtk(DEVONF_OV50A_eeprom);
	calibration_status.pdaf = DEVONF_OV50A_check_pdaf_data(DEVONF_OV50A_eeprom);
	calibration_status.dual = NO_ERRORS;

	DEVONF_OV50A_eeprom_get_mnf_data((void *)DEVONF_OV50A_eeprom, &mnf_info);

	get_ov_cross_talk_data(DEVONF_OV50A_eeprom);
	get_ov_pdc_data(DEVONF_OV50A_eeprom);
...
mot_calibration_status_t *DEVONF_OV50A_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *DEVONF_OV50A_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
...
{
	uint16_t write_table[DEVONF_OV50A_EEPROM_CRC_XTALK_SIZE * 2] = {0};

	pr_debug("E\n");

	memcpy(write_table, &ov_cross_talk_data[0].addr, sizeof(write_table));

	mot_devonf_ov50a_table_write_cmos_sensor(write_table,
...
{
	uint16_t write_table[DEVONF_OV50A_EEPROM_CRC_PDC_WRITE_SIZE * 2] = {0};
