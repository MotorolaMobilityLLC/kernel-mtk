 ...
 * Filename:
 * ---------
 *     mot_naples_gc32e1mipi_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "mot_naples_gc32e1mipi_Sensor.h"
#include "mot_naples_gc32e1_sensor_setting.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "mot_naples_gc32e1_camera_sensor"
#define LOG_1 LOG_INF("MOT_NAPLES_GC32E1, MIPI 4LANE\n")
/****************************   Modify end    *******************************************/
#define MOT_NAPLES_GC32E1_DEBUG 1
#if MOT_NAPLES_GC32E1_DEBUG
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)
#else
#define LOG_INF(format, args...)
#endif

extern mot_calibration_status_t *NAPLES_GC32E1_eeprom_get_calibration_status(void);
extern mot_calibration_mnf_t *NAPLES_GC32E1_eeprom_get_mnf_info(void);
extern void NAPLES_GC32E1_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor);

#define CROSSTALK_BUF_SIZE        771
#define CROSSTALK_START_ADDR      0x0773
//static kal_uint8 mot_naples_gc32e1_xtalk_data[CROSSTALK_BUF_SIZE+2];
//extern bool mot_naples_gc32e1_read_crosstalk_data(kal_uint16 addr, BYTE *data, int size);
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MOT_NAPLES_GC32E1_SENSOR_ID,
	...
	.pre = {
		...
		.mipi_pixel_rate = 311040000, //806.4*4/10
		.max_framerate = 300,
	},
	.cap = {
		.pclk = 345600000,
...
{
	table_write_cmos_sensor(mot_naples_gc32e1_init_addr_data,
		sizeof(mot_naples_gc32e1_init_addr_data)/sizeof(kal_uint16));
}

static void preview_setting(void)
{
	table_write_cmos_sensor(mot_naples_gc32e1_pre_addr_data,
	  sizeof(mot_naples_gc32e1_pre_addr_data)/sizeof(kal_uint16));
}

static void capture_setting(void)
{
	table_write_cmos_sensor(mot_naples_gc32e1_cap_addr_data,
		sizeof(mot_naples_gc32e1_cap_addr_data)/sizeof(kal_uint16));
}

static void normal_video_setting(void)
{
	table_write_cmos_sensor(mot_naples_gc32e1_video_addr_data,
		sizeof(mot_naples_gc32e1_video_addr_data)/sizeof(kal_uint16));
}

static void hs_video_setting(void)
{
	table_write_cmos_sensor(mot_naples_gc32e1_hs_video_addr_data,
		sizeof(mot_naples_gc32e1_hs_video_addr_data)/sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
	table_write_cmos_sensor(mot_naples_gc32e1_slim_video_addr_data,
		sizeof(mot_naples_gc32e1_slim_video_addr_data)/sizeof(kal_uint16));
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info("[mot_naples_gc32e1_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				NAPLES_GC32E1_eeprom_format_calibration_data(&imgsensor);
				//mot_naples_gc32e1_read_crosstalk_data(CROSSTALK_START_ADDR, &mot_naples_gc32e1_xtalk_data[2], CROSSTALK_BUF_SIZE);
				return ERROR_NONE;
			}
			pr_info("[mot_naples_gc32e1_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[mot_naples_gc32e1_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_debug("[mot_naples_gc32e1_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	memcpy(&sensor_info->mnf_calibration, NAPLES_GC32E1_eeprom_get_mnf_info(), sizeof(mot_calibration_mnf_t));
	memcpy(&sensor_info->calibration_status, NAPLES_GC32E1_eeprom_get_calibration_status(), sizeof(mot_calibration_status_t));
...
UINT32 MOT_NAPLES_GC32E1_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
