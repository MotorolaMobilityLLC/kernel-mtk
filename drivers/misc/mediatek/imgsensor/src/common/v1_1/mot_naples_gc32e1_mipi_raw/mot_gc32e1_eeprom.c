...
#define NAPLES_GC32E1_EEPROM_SLAVE_ADDR 0xA2
#define NAPLES_GC32E1_SENSOR_IIC_SLAVE_ADDR 0x94
#define NAPLES_GC32E1_EEPROM_SIZE  0x0027
#define NAPLES_GC32E1_EEPROM_CRC_MANUFACTURING_SIZE 37

static uint8_t NAPLES_GC32E1_eeprom[NAPLES_GC32E1_EEPROM_SIZE] = {0};
...
static kal_uint16 NAPLES_GC32E1_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void NAPLES_GC32E1_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		NAPLES_GC32E1_eeprom[i] = NAPLES_GC32E1_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = NAPLES_GC32E1_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}


static calibration_status_t NAPLES_GC32E1_check_manufacturing_data(void *data)
{
	struct NAPLES_GC32E1_eeprom_t *eeprom = (struct NAPLES_GC32E1_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, NAPLES_GC32E1_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void NAPLES_GC32E1_eeprom_get_mnf_data(void *data,
...
void NAPLES_GC32E1_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	NAPLES_GC32E1_read_data_from_eeprom(NAPLES_GC32E1_EEPROM_SLAVE_ADDR, 0x00, NAPLES_GC32E1_EEPROM_SIZE);
	calibration_status.mnf = NAPLES_GC32E1_check_manufacturing_data(NAPLES_GC32E1_eeprom);
	NAPLES_GC32E1_eeprom_get_mnf_data((void *)NAPLES_GC32E1_eeprom, &mnf_info);
}

mot_calibration_status_t *NAPLES_GC32E1_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *NAPLES_GC32E1_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
