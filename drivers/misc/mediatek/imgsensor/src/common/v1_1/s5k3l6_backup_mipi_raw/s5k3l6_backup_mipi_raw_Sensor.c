...
#include "s5k3l6_backup_mipi_raw_Sensor.h"

#define FPT_PDAF_SUPPORT   //for pdaf switch

/****************************Modify Following Strings for Debug****************************/
#define PFX "S5K3L6BACKUP"
#define LOG_INF(format, args...)    pr_err(PFX "[%s] " format, __FUNCTION__, ##args)
//#define LOG_INF printk

#define LOG_1 LOG_INF("S5K3L6BACKUP,MIPI 4LANE\n")
#define S5K3L6_BACKUP_EEPROM_SIZE 3871
#define EEPROM_DATA_PATH "/data/vendor/camera_dump/s5k3l6_backup_eeprom_data.bin"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3L6_BACKUP_SENSOR_ID,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
...
//ihdr_write_shutter_gain not support for s5k3L6BACKUP
...
#if OTP_3L6
#define S5K3L6_BACKUP_EEPROM_SLAVE_ADD 0xB0
#define S5K3L6_BACKUP_SENSOR_IIC_SLAVE_ADD 0x5a
#define S5K3L6_BACKUP_OFILM_MODULE_ID  0x4f46

static uint8_t s5k3l6_backup_eeprom[S5K3L6_BACKUP_EEPROM_SIZE] = {0};
...
static void s5k3l6_backup_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_drv_lock);
	...
	//read eeprom data
	for (i = 0; i < size; i ++) {
		s5k3l6_backup_eeprom[i] = read_cmos_sensor(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = S5K3L6_BACKUP_SENSOR_IIC_SLAVE_ADD;
	spin_unlock(&imgsensor_drv_lock);
}

static void s5k3l6_backup_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
...
static calibration_status_t s5k3l6_backup_check_manufacturing_data(void *data)
{
	struct s5k3l6_backup_eeprom_t *eeprom = (struct s5k3l6_backup_eeprom_t*)data;

	if (!eeprom_util_check_crc16(data, S5K3L6_BACKUP_EEPROM_CRC_MANUFACTURING_SIZE,
...
static calibration_status_t s5k3l6_backup_check_af_data(void *data)
{
	struct s5k3l6_backup_eeprom_t *eeprom = (struct s5k3l6_backup_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->af_infinity_cal_dac, S5K3L6_BACKUP_EEPROM_CRC_AF_CAL_SIZE,
...
static calibration_status_t s5k3l6_backup_check_awb_data(void *data)
{
	struct s5k3l6_backup_eeprom_t *eeprom = (struct s5k3l6_backup_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
	...
	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		S5K3L6_BACKUP_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t s5k3l6_backup_check_lsc_data(void *data)
{
	struct s5k3l6_backup_eeprom_t *eeprom = (struct s5k3l6_backup_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data, S5K3L6_BACKUP_EEPROM_CRC_LSC_SIZE,
...
static calibration_status_t s5k3l6_backup_check_pdaf_data(void *data)
{
	struct s5k3l6_backup_eeprom_t *eeprom = (struct s5k3l6_backup_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->pdaf_output1_data, S5K3L6_BACKUP_EEPROM_CRC_PDAF_OUTPUT1_SIZE,
		convert_crc(eeprom->pdaf_output1_crc16))) {
		LOG_INF("PDAF OUTPUT1 CRC Fails!");
		return CRC_FAILURE;
	}

	if (!eeprom_util_check_crc16(eeprom->pdaf_output2_data, S5K3L6_BACKUP_EEPROM_CRC_PDAF_OUTPUT2_SIZE,
...
static void s5k3l6_backup_eeprom_format_calibration_data(void *data)
{
	if (NULL == data) {
	...
	mnf_status = s5k3l6_backup_check_manufacturing_data(data);
	af_status = s5k3l6_backup_check_af_data(data);
	awb_status = s5k3l6_backup_check_awb_data(data);
	lsc_status = s5k3l6_backup_check_lsc_data(data);
	pdaf_status = s5k3l6_backup_check_pdaf_data(data);
	dual_status = 0;
...
static void s5k3l6_backup_eeprom_get_mnf_data(void *data,
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				s5k3l6_backup_read_data_from_eeprom(S5K3L6_BACKUP_EEPROM_SLAVE_ADD, 0x0000, S5K3L6_BACKUP_EEPROM_SIZE);
				s5k3l6_backup_eeprom_dump_bin(EEPROM_DATA_PATH, S5K3L6_BACKUP_EEPROM_SIZE, (void *)s5k3l6_backup_eeprom);
				s5k3l6_backup_eeprom_format_calibration_data((void *)s5k3l6_backup_eeprom);
				LOG_INF("This is s5k3l6_backup");
				return ERROR_NONE;
			}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("This is s5k3l6_backup, i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	s5k3l6_backup_eeprom_get_mnf_data((void *)s5k3l6_backup_eeprom, &sensor_info->mnf_calibration);
...
{
	LOG_INF("S5K3L6_BACKUP_MIPI_RAW_SensorInit in\n");
