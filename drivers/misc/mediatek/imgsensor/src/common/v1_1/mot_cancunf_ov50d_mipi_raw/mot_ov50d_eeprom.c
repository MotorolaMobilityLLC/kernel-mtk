...
typedef struct {
	MUINT16 addr;
	MUINT16 data;
} cancunf_ov_cal_addr_data_t;

#define PFX "MOT_CANCUNF_OV50D"
...
#define CANCUNF_OV50D_EEPROM_CRC_PDC_WRITE_SIZE 450
...
extern kal_uint16 mot_cancunf_ov50d_burst_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
static kal_uint16 CANCUNF_OV50D_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void CANCUNF_OV50D_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		CANCUNF_OV50D_eeprom[i] = CANCUNF_OV50D_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = CANCUNF_OV50D_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static calibration_status_t CANCUNF_OV50D_check_manufacturing_data(void *data)
{
	struct CANCUNF_OV50D_eeprom_t *eeprom = (struct CANCUNF_OV50D_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, CANCUNF_OV50D_EEPROM_CRC_MANUFACTURING_SIZE,
...
static calibration_status_t CANCUNF_OV50D_check_af_data(void *data)
{
	struct CANCUNF_OV50D_eeprom_t *eeprom = (struct CANCUNF_OV50D_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->af_data, CANCUNF_OV50D_EEPROM_CRC_AF_CAL_SIZE,
...
static calibration_status_t CANCUNF_OV50D_check_awb_data(void *data)
{
	struct CANCUNF_OV50D_eeprom_t *eeprom = (struct CANCUNF_OV50D_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
	...
	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		CANCUNF_OV50D_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t CANCUNF_OV50D_check_lsc_data_mtk(void *data)
{
	struct CANCUNF_OV50D_eeprom_t *eeprom = (struct CANCUNF_OV50D_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, CANCUNF_OV50D_EEPROM_CRC_LSC_SIZE,
...
static calibration_status_t CANCUNF_OV50D_check_pdaf_data(void *data)
{
	struct CANCUNF_OV50D_eeprom_t *eeprom = (struct CANCUNF_OV50D_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->pdaf_out1_data_mtk, CANCUNF_OV50D_EEPROM_CRC_PDAF_OUTPUT1_SIZE,
		convert_crc(eeprom->pdaf_out1_crc16_mtk))) {
		LOG_ERROR("PDAF OUTPUT1 CRC Fails!");
		return CRC_FAILURE;
	}

	if (!eeprom_util_check_crc16(eeprom->pdaf_out2_data_mtk, CANCUNF_OV50D_EEPROM_CRC_PDAF_OUTPUT2_SIZE,
...
static void CANCUNF_OV50D_eeprom_get_mnf_data(void *data,
...
void CANCUNF_OV50D_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	CANCUNF_OV50D_read_data_from_eeprom(CANCUNF_OV50D_EEPROM_SLAVE_ADDR, 0x00, CANCUNF_OV50D_EEPROM_SIZE);
	calibration_status.mnf = CANCUNF_OV50D_check_manufacturing_data(CANCUNF_OV50D_eeprom);
	calibration_status.af = CANCUNF_OV50D_check_af_data(CANCUNF_OV50D_eeprom);
	calibration_status.awb = CANCUNF_OV50D_check_awb_data(CANCUNF_OV50D_eeprom);
	calibration_status.lsc = CANCUNF_OV50D_check_lsc_data_mtk(CANCUNF_OV50D_eeprom);
	calibration_status.pdaf = CANCUNF_OV50D_check_pdaf_data(CANCUNF_OV50D_eeprom);
...
mot_calibration_status_t *CANCUNF_OV50D_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *CANCUNF_OV50D_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
...
{
	uint16_t write_table[CANCUNF_OV50D_EEPROM_CRC_PDC_WRITE_SIZE * 2] = {0};

	pr_debug("E\n");

	memcpy(write_table, &ov_pdc_data[0].addr, sizeof(write_table));

	mot_cancunf_ov50d_burst_write_cmos_sensor(write_table,
