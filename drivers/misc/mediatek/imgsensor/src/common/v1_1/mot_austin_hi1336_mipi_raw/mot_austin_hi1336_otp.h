...
#define AUSTIN_HI1336_EEPROM_CRC_MANUFACTURING_SIZE 37
#define AUSTIN_HI1336_MANUFACTURE_PART_NUMBER "28D14866"
#define AUSTIN_HI1336_MPN_LENGTH 8

static uint8_t AUSTIN_HI1336_eeprom[AUSTIN_HI1336_EEPROM_SIZE] = {0};
...
static void AUSTIN_HI1336_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	//read eeprom data
	for (i = 0; i < size; i ++) {
		AUSTIN_HI1336_eeprom[i] = read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	hi1336_imgsensor.i2c_write_id = AUSTIN_HI1336_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static void AUSTIN_HI1336_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
...
static calibration_status_t AUSTIN_HI1336_check_awb_data(void *data)
{
	struct AUSTIN_HI1336_eeprom_t *eeprom = (struct AUSTIN_HI1336_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
...
static calibration_status_t AUSTIN_HI1336_check_lsc_data_mtk(void *data)
{
	struct AUSTIN_HI1336_eeprom_t *eeprom = (struct AUSTIN_HI1336_eeprom_t*)data;
...
static void AUSTIN_HI1336_eeprom_get_mnf_data(void *data,
...
static calibration_status_t AUSTIN_HI1336_check_manufacturing_data(void *data)
{
	struct AUSTIN_HI1336_eeprom_t *eeprom = (struct AUSTIN_HI1336_eeprom_t*)data;
LOG_INF("Manufacturing eeprom->mpn = %s !",eeprom->mpn);
#if 0
	if(strncmp(eeprom->mpn, AUSTIN_HI1336_MANUFACTURE_PART_NUMBER, AUSTIN_HI1336_MPN_LENGTH) != 0) {
...
static void AUSTIN_HI1336_eeprom_format_calibration_data(void *data)
{
	if (NULL == data) {
	...
	mnf_status = AUSTIN_HI1336_check_manufacturing_data(data);
	af_status = 0;
	awb_status = AUSTIN_HI1336_check_awb_data(data);
	lsc_status = AUSTIN_HI1336_check_lsc_data_mtk(data);
	pdaf_status = 0;
	dual_status = 0;
