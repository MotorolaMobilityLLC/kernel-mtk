...
#define PFX "MOT_VEGAS_OV50D_2ND"
...
#define VEGAS_OV50D_2ND_EEPROM_SLAVE_ADDR 0xA0
#define VEGAS_OV50D_2ND_SENSOR_IIC_SLAVE_ADDR 0x20
#define VEGAS_OV50D_2ND_EEPROM_SIZE  0x1F3
#define VEGAS_OV50D_2ND_EEPROM_CRC_MANUFACTURING_SIZE 39
#define VEGAS_OV50D_2ND_EEPROM_CRC_PDC_SIZE 458
#define VEGAS_OV50D_2ND_EEPROM_CRC_PDC_WRITE_SIZE 450

static vegas_ov_cal_addr_data_t ov_pdc_data[VEGAS_OV50D_2ND_EEPROM_CRC_PDC_WRITE_SIZE] = {{0}};

int pdc_data_valid_2nd = 0;

static uint8_t VEGAS_OV50D_2ND_eeprom[VEGAS_OV50D_2ND_EEPROM_SIZE] = {0};
static mot_calibration_status_t calibration_status = {CRC_FAILURE};
static mot_calibration_mnf_t mnf_info = {0};

extern kal_uint16 mot_vegas_ov50d_2nd_burst_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
static kal_uint16 VEGAS_OV50D_2ND_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void VEGAS_OV50D_2ND_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size, kal_uint8* eeprom_data)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		eeprom_data[i] = VEGAS_OV50D_2ND_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = VEGAS_OV50D_2ND_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static calibration_status_t VEGAS_OV50D_2ND_check_manufacturing_data(void *data)
{
	struct VEGAS_OV50D_2ND_eeprom_t *eeprom = (struct VEGAS_OV50D_2ND_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, VEGAS_OV50D_2ND_EEPROM_CRC_MANUFACTURING_SIZE-2,
...
static void VEGAS_OV50D_2ND_eeprom_get_mnf_data(void *data,
...
int get_ov_pdc_data_2nd(void *data)
{
	int i;
	struct VEGAS_OV50D_2ND_eeprom_t *eeprom = (struct VEGAS_OV50D_2ND_eeprom_t*)data;
	if (!eeprom_util_check_crc16(eeprom->ov_pdc_data, VEGAS_OV50D_2ND_EEPROM_CRC_PDC_SIZE,
		convert_crc(eeprom->ov_pdc_crc)))
	{
		pr_debug("PDC Data CRC Fail!");
		pdc_data_valid_2nd = 0;
	}
	else
	{
		pr_debug("PDC Data CRC Pass");
		pdc_data_valid_2nd = 1;
		for (i = 0; i < VEGAS_OV50D_2ND_EEPROM_CRC_PDC_WRITE_SIZE; i++)
		{
...
void VEGAS_OV50D_2ND_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	VEGAS_OV50D_2ND_read_data_from_eeprom(VEGAS_OV50D_2ND_EEPROM_SLAVE_ADDR, 0x0000, VEGAS_OV50D_2ND_EEPROM_CRC_MANUFACTURING_SIZE, VEGAS_OV50D_2ND_eeprom);
	VEGAS_OV50D_2ND_read_data_from_eeprom(VEGAS_OV50D_2ND_EEPROM_SLAVE_ADDR, 0x19ED, VEGAS_OV50D_2ND_EEPROM_CRC_PDC_SIZE+2, VEGAS_OV50D_2ND_eeprom+VEGAS_OV50D_2ND_EEPROM_CRC_MANUFACTURING_SIZE);
	calibration_status.mnf = VEGAS_OV50D_2ND_check_manufacturing_data(VEGAS_OV50D_2ND_eeprom);
	VEGAS_OV50D_2ND_eeprom_get_mnf_data((void *)VEGAS_OV50D_2ND_eeprom, &mnf_info);
	get_ov_pdc_data_2nd(VEGAS_OV50D_2ND_eeprom);
}

mot_calibration_status_t *VEGAS_OV50D_2ND_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *VEGAS_OV50D_2ND_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}

void write_pdc_data_2nd(void)
{
	uint16_t write_table[VEGAS_OV50D_2ND_EEPROM_CRC_PDC_WRITE_SIZE * 2] = {0};

	pr_debug("E\n");

	memcpy(write_table, &ov_pdc_data[0].addr, sizeof(write_table));

	mot_vegas_ov50d_2nd_burst_write_cmos_sensor(write_table,
