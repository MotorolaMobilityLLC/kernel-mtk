...
#include "mot_naples_ov08f_mipi_raw_Sensor.h"
#include "mot_naples_ov08f_Sensor_setting.h"
...
extern mot_calibration_status_t *NAPLES_OV08F_otp_get_calibration_status(void);
extern mot_calibration_mnf_t *NAPLES_OV08F_otp_get_mnf_info(void);

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = MOT_NAPLES_OV08F_SENSOR_ID,
...
#ifdef Table_write
static kal_uint16 ov08f_table_write_cmos_sensor(
...
static void preview_setting(void)
{
	...
	//3264X2448_30fps
	common_settings();
	ov08f_table_write_cmos_sensor(addr_data_pair_preview_mot_naples_ov08f,
		sizeof(addr_data_pair_preview_mot_naples_ov08f)/sizeof(kal_uint16));
    LOG_INF("%s end\n", __func__);
}
...
static void normal_video_setting(void)
{
	...
	//3264X1836_30fps
	common_settings();
	ov08f_table_write_cmos_sensor(addr_data_pair_normal_video_mot_naples_ov08f,
		sizeof(addr_data_pair_normal_video_mot_naples_ov08f)/sizeof(kal_uint16));
    LOG_INF("%s end\n", __func__);
}
...
{
    write_cmos_sensor(0xfd, 0x00);
	return (((read_cmos_sensor(0x01) << 8) | read_cmos_sensor(0x02))+1);
}
		      ...
		      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	memcpy(&sensor_info->mnf_calibration, NAPLES_OV08F_otp_get_mnf_info(), sizeof(mot_calibration_mnf_t));
	memcpy(&sensor_info->calibration_status, NAPLES_OV08F_otp_get_calibration_status(), sizeof(mot_calibration_status_t));
...
UINT32 MOT_NAPLES_OV08F_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
