...
typedef struct {
	MUINT16 addr;
	MUINT16 data;
} orlando_ov_cal_addr_data_t;

#define PFX "MOT_ORLANDO_OV50D"
...
#define ORLANDO_OV50D_EEPROM_SLAVE_ADDR 0xA0
#define ORLANDO_OV50D_SENSOR_IIC_SLAVE_ADDR 0x20
#define ORLANDO_OV50D_EEPROM_SIZE  0x1F3
#define ORLANDO_OV50D_EEPROM_CRC_MANUFACTURING_SIZE 39
#define ORLANDO_OV50D_EEPROM_CRC_PDC_SIZE 458
#define ORLANDO_OV50D_EEPROM_CRC_PDC_WRITE_SIZE 450

static orlando_ov_cal_addr_data_t ov_pdc_data[ORLANDO_OV50D_EEPROM_CRC_PDC_WRITE_SIZE] = {{0}};

int pdc_data_valid_orlando = 0;

static uint8_t ORLANDO_OV50D_eeprom[ORLANDO_OV50D_EEPROM_SIZE] = {0};
static mot_calibration_status_t calibration_status = {CRC_FAILURE};
static mot_calibration_mnf_t mnf_info = {0};

extern kal_uint16 mot_orlando_ov50d_burst_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
static kal_uint16 ORLANDO_OV50D_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void ORLANDO_OV50D_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size, kal_uint8* eeprom_data)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		eeprom_data[i] = ORLANDO_OV50D_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = ORLANDO_OV50D_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static calibration_status_t ORLANDO_OV50D_check_manufacturing_data(void *data)
{
	struct ORLANDO_OV50D_eeprom_t *eeprom = (struct ORLANDO_OV50D_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, ORLANDO_OV50D_EEPROM_CRC_MANUFACTURING_SIZE-2,
...
static void ORLANDO_OV50D_eeprom_get_mnf_data(void *data,
...
int get_ov_pdc_data_orlando(void *data)
{
	int i;
	struct ORLANDO_OV50D_eeprom_t *eeprom = (struct ORLANDO_OV50D_eeprom_t*)data;
	if (!eeprom_util_check_crc16(eeprom->ov_pdc_data, ORLANDO_OV50D_EEPROM_CRC_PDC_SIZE,
		convert_crc(eeprom->ov_pdc_crc)))
	{
		pr_debug("PDC Data CRC Fail!");
		pdc_data_valid_orlando = 0;
	}
	else
	{
		pr_debug("PDC Data CRC Pass");
		pdc_data_valid_orlando = 1;
		for (i = 0; i < ORLANDO_OV50D_EEPROM_CRC_PDC_WRITE_SIZE; i++)
		{
...
void ORLANDO_OV50D_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	ORLANDO_OV50D_read_data_from_eeprom(ORLANDO_OV50D_EEPROM_SLAVE_ADDR, 0x0000, ORLANDO_OV50D_EEPROM_CRC_MANUFACTURING_SIZE, ORLANDO_OV50D_eeprom);
	ORLANDO_OV50D_read_data_from_eeprom(ORLANDO_OV50D_EEPROM_SLAVE_ADDR, 0x19ED, ORLANDO_OV50D_EEPROM_CRC_PDC_SIZE+2, ORLANDO_OV50D_eeprom+ORLANDO_OV50D_EEPROM_CRC_MANUFACTURING_SIZE);
	calibration_status.mnf = ORLANDO_OV50D_check_manufacturing_data(ORLANDO_OV50D_eeprom);
	ORLANDO_OV50D_eeprom_get_mnf_data((void *)ORLANDO_OV50D_eeprom, &mnf_info);
	get_ov_pdc_data_orlando(ORLANDO_OV50D_eeprom);
}

mot_calibration_status_t *ORLANDO_OV50D_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *ORLANDO_OV50D_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}

void write_pdc_data_orlando(void)
{
	uint16_t write_table[ORLANDO_OV50D_EEPROM_CRC_PDC_WRITE_SIZE * 2] = {0};

	pr_debug("E\n");

	memcpy(write_table, &ov_pdc_data[0].addr, sizeof(write_table));

	mot_orlando_ov50d_burst_write_cmos_sensor(write_table,
