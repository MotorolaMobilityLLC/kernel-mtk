...
extern void ov08f_read_otp_data_2nd(struct imgsensor_struct *pImgsensor);
extern mot_calibration_status_t *NAPLES_OV08F_otp_get_calibration_status_2nd(void);
extern mot_calibration_mnf_t *NAPLES_OV08F_otp_get_mnf_info_2nd(void);

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = MOT_NAPLES_OV08F_2ND_SENSOR_ID,
...
static void preview_setting(void)
{
	...
	//3264X2448_30fps
	common_settings();
	ov08f_table_write_cmos_sensor(addr_data_pair_preview_mot_naples_ov08f_2nd,
		sizeof(addr_data_pair_preview_mot_naples_ov08f_2nd)/sizeof(kal_uint16));
    LOG_INF("%s end\n", __func__);
}
...
static void normal_video_setting(void)
{
	...
	//3264X1836_30fps
	common_settings();
	ov08f_table_write_cmos_sensor(addr_data_pair_normal_video_mot_naples_ov08f_2nd,
		sizeof(addr_data_pair_normal_video_mot_naples_ov08f_2nd)/sizeof(kal_uint16));
    LOG_INF("%s end\n", __func__);
}
...
{
    write_cmos_sensor(0xfd, 0x00);
	return (((read_cmos_sensor(0x01) << 8) | read_cmos_sensor(0x02))+2);
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				ov08f_read_otp_data_2nd(&imgsensor);
				return ERROR_NONE;
			}
			retry--;
		} while (retry > 0);
		      ...
		      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	memcpy(&sensor_info->mnf_calibration, NAPLES_OV08F_otp_get_mnf_info_2nd(), sizeof(mot_calibration_mnf_t));
	memcpy(&sensor_info->calibration_status, NAPLES_OV08F_otp_get_calibration_status_2nd(), sizeof(mot_calibration_status_t));
