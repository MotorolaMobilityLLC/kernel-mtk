...
#define OV02A10_EEPROM_SIZE 2453
#define EEPROM_DATA_PATH "/data/vendor/camera_dump/ov02a10qt_eeprom_data.bin"
...
static uint8_t ov02a10qt_eeprom[OV02A10_EEPROM_SIZE] = {0};
...
static int  ov02a10qt_read_module_id_from_eeprom(kal_uint8 slave, kal_uint32 start_add,kal_uint8 size)
{
	int i = 0;
...
static void ov02a10qt_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = slave;
	...
	//read eeprom data
	for (i = 0; i < size; i ++) {
		ov02a10qt_eeprom[i] = read_cmos_sensor(start_add);
		start_add ++;
	}
...
static void ov02a10qt_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
...
static calibration_status_t ov02a10qt_check_manufacturing_data(void *data)
{
	...
	if (!eeprom_util_check_crc16(data, OV02A10_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void ov02a10qt_eeprom_format_calibration_data(void *data)
{
	if (NULL == data) {
	...
	mnf_status = ov02a10qt_check_manufacturing_data(data);
	af_status = 0;
	awb_status = ov02a10qt_check_awb_data(data);
	lsc_status = ov02a10qt_check_lsc_data(data);
	pdaf_status = 0;
	dual_status = 0;
...
static void ov02a10qt_eeprom_get_mnf_data(void *data,
				...
				if ((ov02a10qt_otp_data.module_id > 0) && (ov02a10qt_otp_data.module_id < 0xFFFF)) {
					...
					if (ov02a10qt_otp_data.module_id != OV02A10QT_MODULE_ID) {
						*sensor_id = 0xFFFFFFFF;
						...
						ov02a10qt_read_data_from_eeprom(OV02A10QT_EEPROM_SLAVE_ADD, 0x0000, OV02A10_EEPROM_SIZE);
						ov02a10qt_eeprom_dump_bin(EEPROM_DATA_PATH, OV02A10_EEPROM_SIZE, (void *)ov02a10qt_eeprom);
						ov02a10qt_eeprom_format_calibration_data((void *)ov02a10qt_eeprom);
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	ov02a10qt_eeprom_get_mnf_data((void *)ov02a10qt_eeprom, &sensor_info->mnf_calibration);
