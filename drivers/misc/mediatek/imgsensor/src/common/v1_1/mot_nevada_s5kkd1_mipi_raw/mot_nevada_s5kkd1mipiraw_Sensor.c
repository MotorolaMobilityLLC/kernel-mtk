 ...
 * Filename:
 * ---------
 *	 mot_nevada_s5kkd1mipi_Sensor.c
 *
 * Project:
 * --------
 *	 MTK SOP
 ...
 *============================================================================
 ****************************************************************************/
#define PFX "MOT_NEVADA_S5KKD1_camera_sensor"
...
#include "mot_nevada_s5kkd1mipiraw_Sensor.h"
#include "mot_nevada_s5kkd1_Sensor_setting.h"

//extern int aw86006_update_fw_sync(void);

static DEFINE_SPINLOCK(imgsensor_drv_lock);

#define S5KKD1_BASEGAIN 128

#define S5KKD1_MAX_GAIN_BINNINGSIZE_PLATFORM 8192    /*64*128, 128 GAINBASE*/
#define S5KKD1_MAX_GAIN_120FPS_PLATFORM 2048          /*16*128, 128 GAINBASE*/
#define S5KKD1_MAX_GAIN_SLIM_PLATFORM 8192          /*64*128, 128 GAINBASE*/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MOT_NEVADA_S5KKD1_SENSOR_ID,
...
#if MULTI_WRITE
kal_uint16 mot_nevada_s5kkd1_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
{
	kal_uint16 reg_gain;
	kal_uint32 max_gain = S5KKD1_MAX_GAIN_BINNINGSIZE_PLATFORM;

	if(sensor_mode == FPS120_MODE) {
		max_gain = S5KKD1_MAX_GAIN_120FPS_PLATFORM;
	}
	if(sensor_mode == SLIM_MODE) {
		max_gain = S5KKD1_MAX_GAIN_SLIM_PLATFORM;
	}

	if (gain < S5KKD1_BASEGAIN || gain > max_gain) {
		pr_debug("Error gain setting");

		if (gain < S5KKD1_BASEGAIN)
			gain = S5KKD1_BASEGAIN;
...
{
	pr_debug("MOT NEVADA S5KKD1 init start\n");
	write_cmos_sensor(0xFCFC, 0x4000);
	write_cmos_sensor(0x0000, 0x0000);
	write_cmos_sensor(0x0000, 0x4841);
	...
	mot_nevada_s5kkd1_table_write_cmos_sensor(addr_data_pair_init_mot_nevada_s5kkd1,
		sizeof(addr_data_pair_init_mot_nevada_s5kkd1)/sizeof(kal_uint16));

/*	if (xtc_data_valid == 1)
		write_xtc_data();
	pr_debug("MOT NEVADA S5KKD1 init end\n");
*/
}	/*	  sensor_init  */

static void preview_setting(void)
{
	pr_debug("MOT NEVADA S5KKD1 preview_setting start\n");

	mot_nevada_s5kkd1_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_s5kkd1,
		sizeof(addr_data_pair_preview_mot_nevada_s5kkd1)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKD1 preview_setting end\n");
...
{
	pr_debug("MOT NEVADA S5KKD1 capture_setting start\n");
	mot_nevada_s5kkd1_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_s5kkd1,
		sizeof(addr_data_pair_preview_mot_nevada_s5kkd1)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKD1 capture_setting end\n");
}

static void normal_video_setting(kal_uint16 currefps)
{
	pr_debug("MOT NEVADA S5KKD1 normal_video_setting start\n");

	mot_nevada_s5kkd1_table_write_cmos_sensor(addr_data_pair_normal_video_mot_nevada_s5kkd1,
		sizeof(addr_data_pair_normal_video_mot_nevada_s5kkd1)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKD1 normal_video_setting end\n");
}

static void hs_video_setting(void)
{
	pr_debug("MOT NEVADA S5KKD1 hs_video_setting start\n");

	mot_nevada_s5kkd1_table_write_cmos_sensor(addr_data_pair_hs_video_mot_nevada_s5kkd1,
		sizeof(addr_data_pair_hs_video_mot_nevada_s5kkd1)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKD1 hs_video_setting end\n");
}

static void slim_video_setting(void)
{
	pr_debug("MOT NEVADA S5KKD1 slim_video_setting start\n");

	mot_nevada_s5kkd1_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_s5kkd1,
		sizeof(addr_data_pair_preview_mot_nevada_s5kkd1)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKD1 slim_video_setting end\n");
}
...
UINT32 MOT_NEVADA_S5KKD1_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	return ERROR_NONE;
} /* MOT_NEVADA_S5KKD1_MIPI_RAW_SensorInit */
