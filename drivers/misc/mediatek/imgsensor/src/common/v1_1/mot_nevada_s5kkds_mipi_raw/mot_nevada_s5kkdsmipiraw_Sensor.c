 ...
 * Filename:
 * ---------
 *	 mot_nevada_s5kkdsmipi_Sensor.c
 *
 * Project:
 * --------
 *	 MTK SOP
 ...
 *============================================================================
 ****************************************************************************/
#define PFX "MOT_NEVADA_S5KKDS_camera_sensor"
...
#include "mot_nevada_s5kkdsmipiraw_Sensor.h"
#include "mot_nevada_s5kkds_Sensor_setting.h"

extern mot_calibration_status_t *NEVADA_S5KKDS_eeprom_get_calibration_status(void);
extern mot_calibration_mnf_t *NEVADA_S5KKDS_eeprom_get_mnf_info(void);
extern void NEVADA_S5KKDS_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor);

//extern int aw86006_update_fw_sync(void);

static DEFINE_SPINLOCK(imgsensor_drv_lock);

#define S5KKDS_BASEGAIN 128

#define S5KKDS_MAX_GAIN_BINNINGSIZE_PLATFORM 16384    /*128*128, 128 GAINBASE*/
#define S5KKDS_MAX_GAIN_120FPS_PLATFORM 2048          /*16*128, 128 GAINBASE*/
#define S5KKDS_MAX_GAIN_SLIM_PLATFORM 16384          /*128*128, 128 GAINBASE*/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MOT_NEVADA_S5KKDS_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	/* Preview*/
    { 3280, 2460,    0,    0, 3280, 2460, 3280, 2460,  0,   0, 3280, 2460,    0,    0, 3280, 2460}, // preveiw
	/* capture */
    { 3280, 2460,    0,    0, 3280, 2460, 3280, 2460,  0,   0, 3280, 2460,    0,    0, 3280, 2460}, // capture
	/* video */
    { 3280, 2460,    0,    0, 3280, 2460, 3280, 2460,  0,   0, 3280, 2460,    0,  306, 3280, 1848}, // video
	/* hs_video */
    { 3280, 2460,    0,    0, 1640, 1230, 1640, 1230,  0,   0, 1640, 1230,  180,  255, 1280,  720}, // hs
	/* slim_video */
    { 3280, 2460,    0,    0, 3280, 2460, 3280, 2460,  0,   0, 3280, 2460,    0,    0, 3280, 2460}, // slim
};
...
int s5kkds_bNeedSetNormalMode = 0;
/*************************************************************************
 * FUNCTION
 *	set_shutter
 *
...
static void set_shutter(kal_uint32 shutter)
{
	...
	if (shutter > 0xFFF0) {
		...
		s5kkds_bNeedSetNormalMode = KAL_TRUE;
		if(shutter >= 4242424){
			shutter = 4242424;
		}
		...
		if (s5kkds_bNeedSetNormalMode == KAL_TRUE) {
			s5kkds_bNeedSetNormalMode = KAL_FALSE;
...
{
	kal_uint16 reg_gain;
	kal_uint32 max_gain = S5KKDS_MAX_GAIN_BINNINGSIZE_PLATFORM;

	if(sensor_mode == FPS120_MODE) {
		max_gain = S5KKDS_MAX_GAIN_120FPS_PLATFORM;
	}
	if(sensor_mode == SLIM_MODE) {
		max_gain = S5KKDS_MAX_GAIN_SLIM_PLATFORM;
	}

	if (gain < S5KKDS_BASEGAIN || gain > max_gain) {
		pr_debug("Error gain setting");

		if (gain < S5KKDS_BASEGAIN)
			gain = S5KKDS_BASEGAIN;
...
{
	pr_debug("MOT NEVADA S5KKDS init start\n");
	write_cmos_sensor(0xFCFC, 0x4000);
	write_cmos_sensor(0x0000, 0x0000);
	write_cmos_sensor(0x0000, 0x484f);
...
{
	pr_debug("MOT NEVADA S5KKDS preview_setting start\n");

	mot_nevada_s5kkds_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_s5kkds,
		sizeof(addr_data_pair_preview_mot_nevada_s5kkds)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKDS preview_setting end\n");
...
{
	pr_debug("MOT NEVADA S5KKDS capture_setting start\n");
	mot_nevada_s5kkds_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_s5kkds,
		sizeof(addr_data_pair_preview_mot_nevada_s5kkds)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKDS capture_setting end\n");
}

static void normal_video_setting(kal_uint16 currefps)
{
	pr_debug("MOT NEVADA S5KKDS normal_video_setting start\n");

	mot_nevada_s5kkds_table_write_cmos_sensor(addr_data_pair_normal_video_mot_nevada_s5kkds,
		sizeof(addr_data_pair_normal_video_mot_nevada_s5kkds)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKDS normal_video_setting end\n");
}

static void hs_video_setting(void)
{
	pr_debug("MOT NEVADA S5KKDS hs_video_setting start\n");

	mot_nevada_s5kkds_table_write_cmos_sensor(addr_data_pair_hs_video_mot_nevada_s5kkds,
		sizeof(addr_data_pair_hs_video_mot_nevada_s5kkds)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKDS hs_video_setting end\n");
}

static void slim_video_setting(void)
{
	pr_debug("MOT NEVADA S5KKDS slim_video_setting start\n");

	mot_nevada_s5kkds_table_write_cmos_sensor(addr_data_pair_preview_mot_nevada_s5kkds,
		sizeof(addr_data_pair_preview_mot_nevada_s5kkds)/sizeof(kal_uint16));

	pr_debug("MOT NEVADA S5KKDS slim_video_setting end\n");
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				NEVADA_S5KKDS_eeprom_format_calibration_data(&imgsensor);
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	memcpy(&sensor_info->calibration_status, NEVADA_S5KKDS_eeprom_get_calibration_status(), sizeof(mot_calibration_status_t));
	memcpy(&sensor_info->mnf_calibration, NEVADA_S5KKDS_eeprom_get_mnf_info(), sizeof(mot_calibration_mnf_t));
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 1744000;//uint is ns
		break;
...
UINT32 MOT_NEVADA_S5KKDS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	return ERROR_NONE;
} /* MOT_NEVADA_S5KKDS_MIPI_RAW_SensorInit */
