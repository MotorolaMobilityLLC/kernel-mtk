...
typedef struct {
	MUINT16 addr;
	MUINT16 data;
} nevada_ov_cal_addr_data_t;

#define PFX "MOT_NEVADA_S5KKDS"
...
#define NEVADA_S5KKDS_EEPROM_SLAVE_ADDR 0xA0
#define NEVADA_S5KKDS_SENSOR_IIC_SLAVE_ADDR 0x20
#define NEVADA_S5KKDS_EEPROM_SIZE  39
#define NEVADA_S5KKDS_EEPROM_CRC_MANUFACTURING_SIZE 39

static uint8_t NEVADA_S5KKDS_eeprom[NEVADA_S5KKDS_EEPROM_SIZE] = {0};
static mot_calibration_status_t calibration_status = {CRC_FAILURE};
static mot_calibration_mnf_t mnf_info = {0};

extern kal_uint16 mot_nevada_s5kkds_burst_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
static kal_uint16 NEVADA_S5KKDS_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void NEVADA_S5KKDS_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size, kal_uint8* eeprom_data)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		eeprom_data[i] = NEVADA_S5KKDS_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = NEVADA_S5KKDS_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static calibration_status_t NEVADA_S5KKDS_check_manufacturing_data(void *data)
{
	struct NEVADA_S5KKDS_eeprom_t *eeprom = (struct NEVADA_S5KKDS_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, NEVADA_S5KKDS_EEPROM_CRC_MANUFACTURING_SIZE-2,
...
static void NEVADA_S5KKDS_eeprom_get_mnf_data(void *data,
		mot_calibration_mnf_t *mnf)
{
	...
	if (eeprom->lens_id[0] == 0x67){
		ret = snprintf(mnf->lens_id, MAX_CALIBRATION_STRING, "Sunny 39395B-400");
	} else {
...
void NEVADA_S5KKDS_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	NEVADA_S5KKDS_read_data_from_eeprom(NEVADA_S5KKDS_EEPROM_SLAVE_ADDR, 0x0000, NEVADA_S5KKDS_EEPROM_CRC_MANUFACTURING_SIZE, NEVADA_S5KKDS_eeprom);
	calibration_status.mnf = NEVADA_S5KKDS_check_manufacturing_data(NEVADA_S5KKDS_eeprom);
	NEVADA_S5KKDS_eeprom_get_mnf_data((void *)NEVADA_S5KKDS_eeprom, &mnf_info);
}

mot_calibration_status_t *NEVADA_S5KKDS_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *NEVADA_S5KKDS_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
