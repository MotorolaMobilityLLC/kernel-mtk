...
#include "mot_lyriq_hi1336mipiraw_Sensor.h"
...
static uint8_t LYRIQ_HI1336_eeprom[LYRIQ_HI1336_EEPROM_SIZE] = {0};
...
static struct imgsensor_struct hi1336_imgsensor = {
	.mirror = IMAGE_NORMAL,
...
static kal_uint16 read_cmos_sensor_8(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, hi1336_imgsensor.i2c_write_id);
...
static void LYRIQ_HI1336_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_lock);
	hi1336_imgsensor.i2c_write_id = slave;
	spin_unlock(&imgsensor_lock);

	//read eeprom data
	for (i = 0; i < size; i ++) {
		LYRIQ_HI1336_eeprom[i] = read_cmos_sensor_8(start_add);
		start_add ++;
	}
...
static calibration_status_t LYRIQ_HI1336_check_awb_data(void *data)
{
	...
	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		LYRIQ_HI1336_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t LYRIQ_HI1336_check_lsc_data_mtk(void *data)
{
	...
	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, LYRIQ_HI1336_EEPROM_CRC_LSC_SIZE,
...
static void LYRIQ_HI1336_eeprom_format_calibration_data(void *data)
{
	if (NULL == data) {
