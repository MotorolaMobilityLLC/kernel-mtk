...
static int write_eeprom_s5k3l6qt(u16 addr, BYTE* data, u32 size )
{
	int i = 0;
	BYTE flag = 0x00;
	int offset = addr;
...
static int read_eeprom_s5k3l6qt(u16 addr, BYTE* data, u32 size )
{
	int i = 0;
	int offset = addr;
...
int dump_dualcam_data_s5k3l6qt(ENUM_DUALCAM_UTILS_EEPROM_OPS_TAG_NAME tag, char *buf, int buf_len_max)
{
    ...
    {
        case verity_flag:
            buf_len = read_eeprom_s5k3l6qt(CALI_FLAG_OFFSET, buf, buf_len_max);
            break;
        case calibration_data:
            buf_len = read_eeprom_s5k3l6qt(CALI_DATA_OFFSET, buf, buf_len_max);
            break;
        case aesync_data:
            buf_len = read_eeprom_s5k3l6qt(AESYNC_DATA_OFFSET, buf, buf_len_max);
            break;
        default:
...
int store_dualcam_data_s5k3l6qt(ENUM_DUALCAM_UTILS_EEPROM_OPS_TAG_NAME tag, char *buf, int buf_len_max)
{
    ...
    {
        case verity_flag:
            buf_len = write_eeprom_s5k3l6qt(CALI_FLAG_OFFSET, buf, buf_len_max);
            mdelay(1);
            break;
        case calibration_data:
            buf_len = write_eeprom_s5k3l6qt(CALI_DATA_OFFSET, buf, buf_len_max);
            mdelay(1);
            break;
        default:
            CALI_ERR("get invalid tag (%d)\n", tag);
