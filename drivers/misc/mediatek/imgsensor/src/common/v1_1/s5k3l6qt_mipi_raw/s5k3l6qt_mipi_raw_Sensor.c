...
#define EEPROM_DATA_PATH "/data/vendor/camera_dump/s5k3l6qt_eeprom_data.bin"
...
static uint8_t s5k3l6qt_eeprom[S5K3L6_EEPROM_SIZE] = {0};
...
static int s5k3l6qt_read_module_id_from_eeprom(kal_uint8 slave, kal_uint32 start_add,kal_uint8 size)
{
	int i = 0;
...
static void s5k3l6qt_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = slave;
	...
	//read eeprom data
	for (i = 0; i < size; i ++) {
		s5k3l6qt_eeprom[i] = read_cmos_sensor(start_add);
		start_add ++;
	}
...
static void s5k3l6qt_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
...
static calibration_status_t s5k3l6qt_check_manufacturing_data(void *data)
{
...
static calibration_status_t s5k3l6qt_check_af_data(void *data)
{
	struct s5k3l6_eeprom_t *eeprom = (struct s5k3l6_eeprom_t*)data;
...
static calibration_status_t s5k3l6qt_check_awb_data(void *data)
{
	struct s5k3l6_eeprom_t *eeprom = (struct s5k3l6_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
...
static calibration_status_t s5k3l6qt_check_lsc_data(void *data)
{
	struct s5k3l6_eeprom_t *eeprom = (struct s5k3l6_eeprom_t*)data;
...
static calibration_status_t s5k3l6qt_check_pdaf_data(void *data)
{
	struct s5k3l6_eeprom_t *eeprom = (struct s5k3l6_eeprom_t*)data;
...
static void s5k3l6qt_eeprom_format_calibration_data(void *data)
{
	if (NULL == data) {
	...
	mnf_status = s5k3l6qt_check_manufacturing_data(data);
	af_status = s5k3l6qt_check_af_data(data);
	awb_status = s5k3l6qt_check_awb_data(data);
	lsc_status = s5k3l6qt_check_lsc_data(data);
	pdaf_status = s5k3l6qt_check_pdaf_data(data);
	dual_status = 0;
...
static void s5k3l6qt_eeprom_get_mnf_data(void *data,
				...
				if ((s5k3l6qt_otp_data.module_id > 0) && (s5k3l6qt_otp_data.module_id < 0xFFFF)) {
					...
					if (s5k3l6qt_otp_data.module_id != S5K3L6QT_MODULE_ID) {
						*sensor_id = 0xFFFFFFFF;
						...
						s5k3l6qt_read_data_from_eeprom(S5K3L6QT_EEPROM_SLAVE_ADD, 0x0000, S5K3L6_EEPROM_SIZE);
						s5k3l6qt_eeprom_dump_bin(EEPROM_DATA_PATH, S5K3L6_EEPROM_SIZE, (void *)s5k3l6qt_eeprom);
						s5k3l6qt_eeprom_format_calibration_data((void *)s5k3l6qt_eeprom);
						LOG_INF("This is Qtech --->s5k3l6 otp data vaild ...");
					}
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	s5k3l6qt_eeprom_get_mnf_data((void *)s5k3l6qt_eeprom, &sensor_info->mnf_calibration);
...
UINT32 S5K3L6QT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
