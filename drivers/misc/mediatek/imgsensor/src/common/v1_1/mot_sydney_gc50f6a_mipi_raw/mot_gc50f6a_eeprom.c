...
#include "mot_sydney_gc50f6a_eeprom.h"
...
#define SYDNEY_GC50F6A_EEPROM_SLAVE_ADDR 0xA0
#define SYDNEY_GC50F6A_SENSOR_IIC_SLAVE_ADDR 0x20
#define SYDNEY_GC50F6A_EEPROM_SIZE  0x1715
#define SYDNEY_GC50F6A_EEPROM_CRC_MANUFACTURING_SIZE 0X0029

static uint8_t SYDNEY_GC50F6A_eeprom[SYDNEY_GC50F6A_EEPROM_SIZE] = {0};
...
static kal_uint16 SYDNEY_GC50F6A_read_cmos_sensor(kal_uint16 addr)
{
...
static void SYDNEY_GC50F6A_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		SYDNEY_GC50F6A_eeprom[i] = SYDNEY_GC50F6A_read_cmos_sensor(start_add);
		start_add ++;
	}
	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = SYDNEY_GC50F6A_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}


static void SYDNEY_GC50F6A_check_manufacturing_data(void *data)
{
	struct SYDNEY_GC50F6A_eeprom_t *eeprom = (struct SYDNEY_GC50F6A_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %.8s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, SYDNEY_GC50F6A_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void SYDNEY_GC50F6A_eeprom_get_mnf_data(void *data,
...
void SYDNEY_GC50F6A_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	SYDNEY_GC50F6A_read_data_from_eeprom(SYDNEY_GC50F6A_EEPROM_SLAVE_ADDR, 0x00, SYDNEY_GC50F6A_EEPROM_SIZE);
	SYDNEY_GC50F6A_check_manufacturing_data(SYDNEY_GC50F6A_eeprom);
	SYDNEY_GC50F6A_eeprom_get_mnf_data((void *)SYDNEY_GC50F6A_eeprom, &mnf_info);
}

mot_calibration_status_t *SYDNEY_GC50F6A_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *SYDNEY_GC50F6A_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}
