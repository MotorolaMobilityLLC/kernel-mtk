...
#define PFX "MOT_CANCUNF_S5KJNS_VCM_YOVA"
...
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_SLAVE_ADDR 0xA0
#define CANCUNF_S5KJNS_VCM_YOVA_SENSOR_IIC_SLAVE_ADDR 0x20
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_SIZE  0x1CBF
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_MANUFACTURING_SIZE 37
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_AF_CAL_SIZE 24
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_AWB_CAL_SIZE 43
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_LSC_SIZE 1868
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_PDAF_OUTPUT1_SIZE 496
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_PDAF_OUTPUT2_SIZE 1004
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_XTC_SIZE 346
#define CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_XTC_WRITE_SIZE (CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_XTC_SIZE / 2 + 2)

static cancunf_xtc_cal_addr_data_t hw_ggc_data[CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_XTC_WRITE_SIZE] = {{0}};

int s5kjns_vcm_yova_xtc_data_valid = 0;

static uint8_t CANCUNF_S5KJNS_VCM_YOVA_eeprom[CANCUNF_S5KJNS_VCM_YOVA_EEPROM_SIZE] = {0};
static mot_calibration_status_t calibration_status = {CRC_FAILURE};
static mot_calibration_mnf_t mnf_info = {0};

extern kal_uint16 mot_cancunf_s5kjns_vcm_yova_burst_write_cmos_sensor(kal_uint16 *para, kal_uint32 len);
...
static kal_uint16 CANCUNF_S5KJNS_VCM_YOVA_read_cmos_sensor_8(kal_uint16 addr)
{
...
static void CANCUNF_S5KJNS_VCM_YOVA_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	...
	for (i = 0; i < size; i ++) {
		CANCUNF_S5KJNS_VCM_YOVA_eeprom[i] = CANCUNF_S5KJNS_VCM_YOVA_read_cmos_sensor_8(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_lock);
	imgsensor->i2c_write_id = CANCUNF_S5KJNS_VCM_YOVA_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_lock);
}

static calibration_status_t CANCUNF_S5KJNS_VCM_YOVA_check_manufacturing_data(void *data)
{
	struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t *eeprom = (struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t*)data;
	LOG_INF("Manufacturing eeprom->mpn = %s !",eeprom->mpn);
	if (!eeprom_util_check_crc16(eeprom->eeprom_table_version, CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_MANUFACTURING_SIZE,
...
static calibration_status_t CANCUNF_S5KJNS_VCM_YOVA_check_af_data(void *data)
{
	struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t *eeprom = (struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->af_data, CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_AF_CAL_SIZE,
...
static calibration_status_t CANCUNF_S5KJNS_VCM_YOVA_check_awb_data(void *data)
{
	struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t *eeprom = (struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
	...
	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t CANCUNF_S5KJNS_VCM_YOVA_check_lsc_data_mtk(void *data)
{
	struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t *eeprom = (struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_LSC_SIZE,
...
static calibration_status_t CANCUNF_S5KJNS_VCM_YOVA_check_pdaf_data(void *data)
{
	struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t *eeprom = (struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->pdaf_out1_data_mtk, CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_PDAF_OUTPUT1_SIZE,
		convert_crc(eeprom->pdaf_out1_crc16_mtk))) {
		LOG_ERROR("PDAF OUTPUT1 CRC Fails!");
		return CRC_FAILURE;
	}

	if (!eeprom_util_check_crc16(eeprom->pdaf_out2_data_mtk, CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_PDAF_OUTPUT2_SIZE,
...
static void CANCUNF_S5KJNS_VCM_YOVA_eeprom_get_mnf_data(void *data,
...
int s5kjns_vcm_yova_get_hw_ggc_data(void *data)
{
	int i;
	struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t *eeprom = (struct CANCUNF_S5KJNS_VCM_YOVA_eeprom_t*)data;
	if (!eeprom_util_check_crc16(eeprom->hw_ggc_data, CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_XTC_SIZE,
		convert_crc(eeprom->xtc_data_crc)))
	{
		pr_debug("XTC Data CRC Fail!");
		s5kjns_vcm_yova_xtc_data_valid = 0;
	}
	else
	{
		pr_debug("XTC Data CRC Pass");
		s5kjns_vcm_yova_xtc_data_valid = 1;

		hw_ggc_data[0].addr = 0x6028;
		hw_ggc_data[0].data = 0x2400;
		hw_ggc_data[1].addr = 0x602A;
		hw_ggc_data[1].data = 0x0CFC;
		for (i = 0; i < CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_XTC_SIZE; i += 2)
		{
...
void CANCUNF_S5KJNS_VCM_YOVA_eeprom_format_calibration_data(struct imgsensor_struct *pImgsensor)
{
	imgsensor = pImgsensor;
	CANCUNF_S5KJNS_VCM_YOVA_read_data_from_eeprom(CANCUNF_S5KJNS_VCM_YOVA_EEPROM_SLAVE_ADDR, 0x00, CANCUNF_S5KJNS_VCM_YOVA_EEPROM_SIZE);
	calibration_status.mnf = CANCUNF_S5KJNS_VCM_YOVA_check_manufacturing_data(CANCUNF_S5KJNS_VCM_YOVA_eeprom);
	calibration_status.af = CANCUNF_S5KJNS_VCM_YOVA_check_af_data(CANCUNF_S5KJNS_VCM_YOVA_eeprom);
	calibration_status.awb = CANCUNF_S5KJNS_VCM_YOVA_check_awb_data(CANCUNF_S5KJNS_VCM_YOVA_eeprom);
	calibration_status.lsc = CANCUNF_S5KJNS_VCM_YOVA_check_lsc_data_mtk(CANCUNF_S5KJNS_VCM_YOVA_eeprom);
	calibration_status.pdaf = CANCUNF_S5KJNS_VCM_YOVA_check_pdaf_data(CANCUNF_S5KJNS_VCM_YOVA_eeprom);
	calibration_status.dual = NO_ERRORS;

	CANCUNF_S5KJNS_VCM_YOVA_eeprom_get_mnf_data((void *)CANCUNF_S5KJNS_VCM_YOVA_eeprom, &mnf_info);

	s5kjns_vcm_yova_get_hw_ggc_data(CANCUNF_S5KJNS_VCM_YOVA_eeprom);
...
mot_calibration_status_t *CANCUNF_S5KJNS_VCM_YOVA_eeprom_get_calibration_status(void)
{
	return &calibration_status;
}

mot_calibration_mnf_t *CANCUNF_S5KJNS_VCM_YOVA_eeprom_get_mnf_info(void)
{
	return &mnf_info;
}


void s5kjns_vcm_yova_write_xtc_data(void)
{
	uint16_t write_table[CANCUNF_S5KJNS_VCM_YOVA_EEPROM_CRC_XTC_WRITE_SIZE * 2] = {0};

	pr_debug("E\n");

	memcpy(write_table, &hw_ggc_data[0].addr, sizeof(write_table));

	mot_cancunf_s5kjns_vcm_yova_burst_write_cmos_sensor(write_table,
