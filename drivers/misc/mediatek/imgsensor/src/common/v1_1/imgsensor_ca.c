...
#define IMGSENSOR_UUID { 0x7b665120, 0x2121, 0x4487, { 0xba, 0x71, 0x0a, 0x51, 0xd7, 0xea, 0x78, 0xfe } }
...
unsigned int imgsensor_ca_open(void)
{
	...
	if (atomic_inc_return(&ca_open_cnt) == 1) {
		...
		memset(&imgsensor_tci_context, 0, sizeof(imgsensor_tci_context));
		memset(&imgsensor_tci_session, 0, sizeof(imgsensor_tci_session));

		ret = TEEC_InitializeContext(NULL, &imgsensor_tci_context);
		if (ret != TEEC_SUCCESS) {
			PK_INFO("teec_initialize_context failed: %x\n", ret);
		} else {
			ret = TEEC_OpenSession(&imgsensor_tci_context, &imgsensor_tci_session,
				&TEEC_IMGSENSOR_UUID, TEEC_LOGIN_PUBLIC, NULL, &teeOperation, NULL);
			if (ret != TEEC_SUCCESS) {
				...
				if (imgsensor_ca_invoke_command(IMGSENSOR_TEE_CMD_GET_I2C_BUS, c_params, &ret) != 0) {
					PK_INFO("Error: SECURE_I2C_BUS is %d", secure_i2c_bus);
				} else {
					PK_DBG("SECURE_I2C_BUS is %d", secure_i2c_bus);
	...
	struct command_params params, MUINT32 *ret)
{
	...
	MSDK_SENSOR_FEATURE_ENUM feature_id = (MSDK_SENSOR_FEATURE_ENUM)params.param0;
	struct TEEC_Operation teeOperation;
	...
	PK_DBG("feature_id = %d %p %p %d %d\n",
		feature_id, teeOperation.params[1].tmpref.buffer, teeOperation.params[2].tmpref.buffer,
	...
	case SENSOR_FEATURE_GET_PERIOD:
		teeOperation.paramTypes =
			TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_OUTPUT,
	...
	case SENSOR_FEATURE_SET_DUAL_GAIN:
		teeOperation.paramTypes =
			TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_INPUT,
					TEEC_MEMREF_TEMP_INPUT, TEEC_NONE);
		teeOperation.params[1].tmpref.size = sizeof(unsigned long long)*2;
		teeOperation.params[2].tmpref.size = sizeof(UINT32);
		break;
#if 0
	case SENSOR_FEATURE_SET_REGISTER:
		teeOperation.paramTypes =
			TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_INPUT,
					TEEC_MEMREF_TEMP_INPUT, TEEC_NONE);
		teeOperation.params[1].tmpref.size = sizeof(MSDK_SENSOR_REG_INFO_STRUCT);
		teeOperation.params[2].tmpref.size = sizeof(UINT32);
		break;
	...
	case SENSOR_FEATURE_GET_SENSOR_HDR_CAPACITY:
		teeOperation.paramTypes =
			TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_INOUT,
					TEEC_MEMREF_TEMP_INPUT, TEEC_MEMREF_TEMP_OUTPUT);
		teeOperation.params[1].tmpref.size = sizeof(unsigned long long)*2;
		teeOperation.params[2].tmpref.size = sizeof(UINT32);
		teeOperation.params[3].tmpref.buffer = (void *)(uintptr_t) (*(feature_data + 1));
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		teeOperation.paramTypes =
			TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_INOUT,
					TEEC_MEMREF_TEMP_INPUT, TEEC_MEMREF_TEMP_OUTPUT);
		teeOperation.params[1].tmpref.size = sizeof(unsigned long long)*3;
		teeOperation.params[2].tmpref.size = sizeof(UINT32);

		teeOperation.params[3].tmpref.buffer = (void *)(uintptr_t) (*(feature_data + 1));
		teeOperation.params[3].tmpref.size = (kal_uint32) (*(feature_data + 2));
		break;
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		teeOperation.params[2].tmpref.size = sizeof(UINT32);
		teeOperation.params[3].tmpref.buffer = (void *)(uintptr_t) (*(feature_data + 1));
		teeOperation.params[3].tmpref.size = sizeof(SENSOR_WINSIZE_INFO_STRUCT);
		break;
	...
	case SENSOR_FEATURE_GET_VC_INFO:
		...
		teeOperation.params[2].tmpref.size = sizeof(UINT32);
		teeOperation.params[3].tmpref.buffer = (void *)(uintptr_t) (*(feature_data + 1));
		teeOperation.params[3].tmpref.size = sizeof(SENSOR_VC_INFO_STRUCT);
		break;
	case SENSOR_FEATURE_SET_AWB_GAIN:
		teeOperation.paramTypes =
			TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_INPUT,
					TEEC_MEMREF_TEMP_INPUT, TEEC_NONE);
		teeOperation.params[1].tmpref.size = sizeof(SET_SENSOR_AWB_GAIN);
		teeOperation.params[2].tmpref.size = sizeof(UINT32);
		break;
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		teeOperation.params[2].tmpref.size = sizeof(UINT32);
		teeOperation.params[3].tmpref.buffer = (void *)(uintptr_t) (*(feature_data + 1));
		teeOperation.params[3].tmpref.size = sizeof(SET_PD_BLOCK_INFO_T);
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		teeOperation.paramTypes =
			TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_INOUT,
					TEEC_MEMREF_TEMP_INPUT, TEEC_NONE);
		teeOperation.params[1].tmpref.size = sizeof(UINT32)*(*feature_para_len);
		teeOperation.params[2].tmpref.size = sizeof(UINT32);
		break;
	...
	ret_tee = TEEC_InvokeCommand(&imgsensor_tci_session, IMGSENSOR_TEE_CMD_FEATURE_CONTROL, &teeOperation, NULL);
	...
	struct command_params params, MUINT32 *ret)
{
	...
	ret_tee = TEEC_InvokeCommand(
		&imgsensor_tci_session, IMGSENSOR_TEE_CMD_GET_RESOLUTION, &teeOperation, NULL);
...
TEEC_Result _tee_cmd_getInfo(struct command_params params, MUINT32 *ret)
{
	...
	TEEC_Result ret_tee = TEEC_SUCCESS;
	struct TEEC_Operation teeOperation;
	MSDK_SCENARIO_ID_ENUM scenario_id = (MSDK_SCENARIO_ID_ENUM)(params.param0);
	...
	ret_tee = TEEC_InvokeCommand(
		&imgsensor_tci_session, IMGSENSOR_TEE_CMD_GET_INFO, &teeOperation, NULL);

	PK_DBG("cagi: params.param1 = %p param2 = %p sizeof(MSDK_SENSOR_CONFIG_STRUCT) %zu %zu\n",
		params.param1, params.param2,
		sizeof(MSDK_SENSOR_CONFIG_STRUCT), sizeof(ACDK_SENSOR_MIPI_LANE_NUMBER_ENUM));
...
TEEC_Result _tee_cmd_Control(struct command_params params, MUINT32 *ret)
{
	...
	ret_tee = TEEC_InvokeCommand(
		&imgsensor_tci_session, IMGSENSOR_TEE_CMD_CONTROL, &teeOperation, NULL);
	...
	struct command_params parms, MUINT32 *ret)
{
	...
	_TRACE("%s\n", __func__);
	memset(&teeOperation, 0, sizeof(struct TEEC_Operation));
	teeOperation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_OUTPUT, TEEC_NONE, TEEC_NONE, TEEC_NONE);
	teeOperation.params[0].value.a = 0xff;

	ret_tee = TEEC_InvokeCommand(&imgsensor_tci_session, IMGSENSOR_TEE_CMD_OPEN, &teeOperation, NULL);
	if (ret_tee == TEEC_SUCCESS)
		*ret = teeOperation.params[0].value.a;
	_TRACE("%s exit\n", __func__);
	return ret_tee;
	...
	struct command_params parms, MUINT32 *ret)
{
	...
	ret_tee = TEEC_InvokeCommand(&imgsensor_tci_session, IMGSENSOR_TEE_CMD_CLOSE, &teeOperation, NULL);
	if (ret_tee == TEEC_SUCCESS)
		*ret = teeOperation.params[0].value.a;
	_TRACE("%s exit\n", __func__);
	return ret_tee;
	...
	struct command_params parms, MUINT32 *ret)
{
	...
	ret_tee = TEEC_InvokeCommand(&imgsensor_tci_session, IMGSENSOR_TEE_CMD_DUMP_REG, &teeOperation, NULL);
	if (ret_tee == TEEC_SUCCESS)
		*ret = teeOperation.params[0].value.a;
	_TRACE("%s exit\n", __func__);
	return ret_tee;
	...
	struct command_params parms, MUINT32 *ret)
{
	...
	ret_tee = TEEC_InvokeCommand(&imgsensor_tci_session, IMGSENSOR_TEE_CMD_GET_I2C_BUS, &teeOperation, NULL);
