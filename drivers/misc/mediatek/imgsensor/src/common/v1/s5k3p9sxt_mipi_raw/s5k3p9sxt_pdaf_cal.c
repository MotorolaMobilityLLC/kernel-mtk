...
#define PFX "s5k3p9sxt_camera_pdaf"
...
static bool s5k3p9sxt_selective_read_eeprom(kal_uint16 addr, BYTE *data)
{
...
bool s5k3p9sxt_read_eeprom(kal_uint16 addr, BYTE *data, kal_uint32 size)
{
	int i = 0;
	/* int offset = addr; */
	int offset = 0x763;
	...
	for (i = 0; i < 1024; i += 2) {
		if (!s5k3p9sxt_selective_read_eeprom(offset, &data[i])) {
...
bool s5k3p9sxt_read_otp_pdaf_data(
	kal_uint16 addr,
	BYTE *data,
	kal_uint32 size)
{
	...
	if (!s5k3p9sxt_read_eeprom(addr, data, size)) {
		get_done = 0;
		last_size = 0;
		last_offset = 0;
