...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.max_frame_length = 0x29b3,           /* max framelength by sensor register's limitation */
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	printk("mjh200811---get_imgsensor_id\n");
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("mjh[gc8034_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
							imgsensor.i2c_write_id, *sensor_id);
				//back_camera_find_success=4;
				//camera_back_probe_ok=1;
				return ERROR_NONE;
			}
			printk("[gc8034_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
						imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("[gc8034_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
							imgsensor.i2c_write_id, sensor_id);
				break;
			}
			LOG_INF("[gc8034_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
						imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
