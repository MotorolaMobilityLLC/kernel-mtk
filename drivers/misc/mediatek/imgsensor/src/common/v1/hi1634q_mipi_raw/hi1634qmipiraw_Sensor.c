...
#include "hi1634qmipiraw_sensor_settings.h"
...
#define HI1634Q_EEPROM_SLAVE_ADDR 0xA2
#define HI1634Q_SENSOR_IIC_SLAVE_ADDR 0x42
#define HI1634Q_EEPROM_SIZE  0x1124
#define HI1634Q_EEPROM_CRC_AWB_CAL_SIZE 43
#define HI1634Q_EEPROM_CRC_LSC_SIZE 1868
#define HI1634Q_SERIAL_NUM_SIZE 16
#define HI1634Q_EEPROM_CRC_MANUFACTURING_SIZE 37
#define FRONT_SERIAL_NUM_DATA_PATH "/data/vendor/camera_dump/serial_number_front.bin"
static uint8_t HI1634Q_eeprom[HI1634Q_EEPROM_SIZE] = {0};
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	{ 4608, 3456,	0,	 0, 4608, 3456,  2304, 1728,  0,  0, 2304, 1728, 0, 0, 2304, 1728},	// preview (2304 x 1728)
	{ 4608, 3456,	0,	 0, 4608, 3456,  2304, 1728,  0,  0, 2304, 1728, 0, 0, 2304, 1728},	// capture (2304 x 1728)
	{ 4608, 3456,	0,	 0, 4608, 3456,  2304, 1728,  0,  0, 2304, 1728, 0, 0, 2304, 1728},	// video (2304 x 1728)
	{ 4608, 3456,	0,	 0, 4608, 3456,  2304, 1728,  0,  0, 2304, 1728, 0, 0, 2304, 1728},	// hs video (2304 x 1728)
	{ 4608, 3456,	0,	 0, 4608, 3456,  2304, 1728,  0,  0, 2304, 1728, 0, 0, 2304, 1728},	// slim video (2304 x 1728)
	{ 4608, 3456,	0,	 0, 4608, 3456,  4608, 3456,  0,  0, 4608, 3456, 0, 0, 4608, 3456},	// custom1 (4608 x 3456)
};
...
{
	pr_err("Hi-1634Q preview_setting start\n");
...
{
	pr_err("Hi-1634Q custom1_setting start\n");
	/*************MIPI output setting************/
...
static void hi1634q_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = slave;
	spin_unlock(&imgsensor_drv_lock);
	LOG_INF("hi1634q_enter_read_data_from_eeprom\n");
	//read eeprom data
	for (i = 0; i < size; i ++) {
		HI1634Q_eeprom[i] = read_cmos_sensor_8(start_add);
		//pr_debug("hi1634q_eeprom[%d]=%x",i,HI1634Q_eeprom[i]);
		start_add ++;
	}

	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = HI1634Q_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_drv_lock);
}

static void hi1634q_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
...
static calibration_status_t hi1634q_check_awb_data(void *data)
{
	struct HI1634Q_eeprom_t *eeprom = (struct HI1634Q_eeprom_t*)data;

	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		HI1634Q_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t hi1634q_check_lsc_data_mtk(void *data)
{
	struct HI1634Q_eeprom_t *eeprom = (struct HI1634Q_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, HI1634Q_EEPROM_CRC_LSC_SIZE,
...
static void hi1634q_eeprom_get_serial_number(void *data){
	//int i = 0;
	struct HI1634Q_eeprom_t *eeprom = (struct HI1634Q_eeprom_t*)data;
	/*
	for(i=0;i<HI1634Q_SERIAL_NUM_SIZE;i++){
			LOG_INF("serial_number[%d]=%x",i,eeprom->serial_number[i]);
	}
	*/
	hi1634q_eeprom_dump_bin(FRONT_SERIAL_NUM_DATA_PATH,  HI1634Q_SERIAL_NUM_SIZE,  eeprom->serial_number);
}
static calibration_status_t hi1634q_check_manufacturing_data(void *data)
{
	struct HI1634Q_eeprom_t *eeprom = (struct HI1634Q_eeprom_t*)data;

	if (!eeprom_util_check_crc16(data, HI1634Q_EEPROM_CRC_MANUFACTURING_SIZE,
...
static void hi1634q_eeprom_format_calibration_data(void *data)
{
	if (NULL == data) {
	...
	mnf_status = hi1634q_check_manufacturing_data(data);
	awb_status = hi1634q_check_awb_data(data);
	lsc_status = hi1634q_check_lsc_data_mtk(data);
    af_status = NO_ERRORS;
...
static void hi1634q_eeprom_get_module_info(void *data)
{
	struct HI1634Q_eeprom_t *eeprom = (struct HI1634Q_eeprom_t*)data;
	if( (eeprom->manufacturer_id[0] ==  'O') && (eeprom->manufacturer_id[1] ==  'F')){//ofilm
		sprintf(module_info_front, "%s", "hi1634q_ofilm_mipi_raw");
	} else if( (eeprom->manufacturer_id[0] ==  'Q') && (eeprom->manufacturer_id[1] ==  'T')){//qtrch
		sprintf(module_info_front, "%s", "hi1634q_qtech_mipi_raw");
	}
	LOG_INF("eeprom->manufacturer_id[0]:%s, eeprom->manufacturer_id[1]:%s", eeprom->manufacturer_id[0], eeprom->manufacturer_id[1]);
}


#define FOUR_CELL_SIZE 480
...
static void read_4cell_from_eeprom(char *data)
{
	...
	if (data != NULL) {
		pr_err("hi1634q  memcpy  xtc to data\n");
		memcpy(data, Four_Cell_Size_Array, 2);	//adress
		memcpy(data+2, &HI1634Q_eeprom[3907], 480);	//QGC
	}
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				hi1634q_read_data_from_eeprom(HI1634Q_EEPROM_SLAVE_ADDR, 0x0000, HI1634Q_EEPROM_SIZE);
				hi1634q_eeprom_get_serial_number((void *)HI1634Q_eeprom);
				hi1634q_eeprom_format_calibration_data((void *)HI1634Q_eeprom);
				hi1634q_eeprom_get_module_info((void *)HI1634Q_eeprom);
	...
	struct SET_SENSOR_PATTERN_SOLID_COLOR *pdata)
{
	...
	if (modes) {
		...
		write_cmos_sensor(0x0C0A, 0x0104);//Solid Color
			...
			write_cmos_sensor_8(0x0C0C, (Color_R >> 8) & 0x3);
			write_cmos_sensor_8(0x0C0D, Color_R & 0xFF);
			write_cmos_sensor_8(0x0C0E, (Color_Gr >> 8) & 0x3);
			write_cmos_sensor_8(0x0C0F, Color_Gr & 0xFF);
			write_cmos_sensor_8(0x0C10, (Color_B >> 8) & 0x3);
			write_cmos_sensor_8(0x0C11, Color_B & 0xFF);
			write_cmos_sensor_8(0x0C12, (Color_Gb >> 8) & 0x3);
			write_cmos_sensor_8(0x0C13, Color_Gb & 0xFF);
	} else{
		write_cmos_sensor(0x0b04, 0x0094);/*No pattern*/
	    write_cmos_sensor(0x0C0A, 0x0000);
	}
	    spin_lock(&imgsensor_drv_lock);
