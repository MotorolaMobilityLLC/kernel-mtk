...
#define PFX "OV02B10_camera_sensor"
#define MACRO_SERIAL_NUM_DATA_PATH "/data/vendor/camera_dump/serial_number_macro.bin"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)
extern kal_uint8 otp_status_macro;
...
static kal_uint16 read_cmos_sensor_ov(kal_uint16 addr)
{
...
#define AWB_GROUP_FLAG_ADDR 0x10
#define AWB_GROUP1_START_ADDR 0x11
#define AWB_GROUP2_START_ADDR 0x18
#define AWB_DATA_SIZE 6
#define AWB_ERR 0x02
static int ov02b10_data_awb[10]= {0};
static void read_ov02b10_awb_info(void)
{
	kal_uint16 i, awb_start_addr = 0, otp_grp_flag = 0;
	//init setting
	write_cmos_sensor(0xfd, 0x06);//page select
	write_cmos_sensor(0x21, 0x00);//otp enable read mode
	write_cmos_sensor(0x2f, 0x01);//otp enable read mode
	write_cmos_sensor(0xfe, 0x02);//trigger
	otp_grp_flag = read_cmos_sensor_ov(AWB_GROUP_FLAG_ADDR); //read value
	LOG_INF("otp grp flag = 0x%x\n", otp_grp_flag);
	...
	ov02b10_data_awb[0] = otp_grp_flag;
	for (i = 0; i < AWB_DATA_SIZE + 1; i++) {
		ov02b10_data_awb[i+1] = read_cmos_sensor_ov(awb_start_addr+i);
		LOG_INF("addr = 0x%x, value = 0x%x\n", (awb_start_addr+i),ov02b10_data_awb[i+1]);
	}
}

static void checksum_cal_ov02b10(void)
{
	kal_uint16 i, otp_grp_flag = 0, CHECK_SUM = 0;
	otp_status_macro = 0x00;
	otp_grp_flag = read_cmos_sensor_ov(AWB_GROUP_FLAG_ADDR); //read valu
	LOG_INF("bitchecksum grp flag = 0x%x\n", otp_grp_flag);
	if (((otp_grp_flag&0xC0) >> 6) == 0x01) { //Bit[7:6] 01:Valid 11:Invalid
		for (i = 0x10; i < 0x17; i++) {
			CHECK_SUM += (read_cmos_sensor_ov(i));
		}
	} else if (((otp_grp_flag & 0x30) >> 4) == 0x01) { //Bit[5:4] 01:Valid 11:Invalid
		for (i = 0x18; i < 0x1e; i++) {
			CHECK_SUM += (read_cmos_sensor_ov(i));
	...
	if ((((CHECK_SUM % 0xff) + 1) == read_cmos_sensor_ov(0x17)) ||
		(((CHECK_SUM % 0xff) + 1) == read_cmos_sensor_ov(0x1e))) {
		LOG_INF("ov02b10 checksum success bit");
	} else {
		otp_status_macro = (otp_status_macro | AWB_ERR);
		LOG_INF("ov02b10 checksum fail bit");
	}
}

#define OV02B10_SERIAL_NUM_SIZE 16
#define SensorID_SIZE 16
#define SensorID_START_ADDR 0x00
static kal_uint8 ov02b10_data_SensorID[SensorID_SIZE];
static void OV02B10_OTP_dump_bin(const char *file_name, uint32_t size, const void *data);
static void read_ov02b10_SensorID_info(void)
{
	kal_uint16 i;
	//init setting
	write_cmos_sensor(0xfd, 0x06);//page select
	...
	write_cmos_sensor(0xfe, 0x02);//trigger
	//read sensor info
	for (i = 0; i < SensorID_SIZE; i++) {
		ov02b10_data_SensorID[i] = read_cmos_sensor_ov(SensorID_START_ADDR + i);
		//sprintf(SensorID_buffer,"%02x",  ov02b10_data_SensorID[i]);
		//strcat(serialNumber,SensorID_buffer);
		LOG_INF("Sensor_id_addr = 0x%x, value = 0x%x\n", (SensorID_START_ADDR + i), ov02b10_data_SensorID[i]);
	}
	OV02B10_OTP_dump_bin(MACRO_SERIAL_NUM_DATA_PATH, OV02B10_SERIAL_NUM_SIZE, ov02b10_data_SensorID);
}
...
unsigned int ov02b10_read_otpdata(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size)
{
	kal_uint16 i;
	for (i = 0; i < size; i++) {
		data[i] = ov02b10_data_awb[i+1];
	}
	return size;
}
#if 0
        ...
        do {
            ...
            *sensor_id = ((read_cmos_sensor(0x0200) << 8) | read_cmos_sensor(0x0300));
            if (*sensor_id == imgsensor_info.sensor_id) {
                //read_module_data();
		read_ov02b10_awb_info();
		read_ov02b10_SensorID_info();
		checksum_cal_ov02b10();
