...
static int32_t gc02m1_check_crc16(int *gc02m1_data_awb,uint32_t crc_data_size,uint32_t ref_crc,uint32_t group_flag)
{
	int32_t crc_match = 0;
	uint16_t crc = 0x0000,crc_reverse = 0x0000;

	uint32_t tmp,tmp_reverse;
	int *check_crc_data = NULL;
	int i,j;
	if (group_flag == 1) {
		check_crc_data = gc02m1_data_awb;
	} else if (group_flag == 2) {
		check_crc_data = &gc02m1_data_awb[1];
	}
	for (i = 0; i < crc_data_size; i++) {
		tmp_reverse = crc_reverse_byte(check_crc_data[i]);
		tmp = check_crc_data[i] & 0xff;
		for (j = 0; j < 8; j++) {
...
//CRC
#define AWB_GROUP1_CRC_START_ADDR 0xb0
#define AWB_GROUP2_CRC_START_ADDR 0xf0
#define CRC_DATA_SIZE 6
#define CRC_FAIL 0x02
static int gc02m1_data_awb[10]= {0};
static void read_gc02m1_awb_info(void)
{
	uint32_t ref_crc_buf[2] = {0};
	uint32_t ref_crc = 0;
	int32_t ret;
	uint32_t crc_data_size = 0, group_flag = 0;
	kal_uint16 awb_crc_start_addr = 0;
	...
	write_cmos_sensor(0xf3, 0x30);//0TP read init set
	...
	if (((otp_grp_flag&0xC0) >> 6) == 0x01) { //Bit[7:6] 01:Valid 11:Invalid
		...
		awb_crc_start_addr = AWB_GROUP1_CRC_START_ADDR;
		crc_data_size = CRC_DATA_SIZE + 1;
		group_flag = 1;
		...
		awb_crc_start_addr = AWB_GROUP2_CRC_START_ADDR;
	...
	for (i = 0;i < 2;i++) {
		write_cmos_sensor(0x17, awb_crc_start_addr+i*0x08);
		write_cmos_sensor(0xf3, 0x34);
		ref_crc_buf[i] = read_cmos_sensor(0x19);
	}
	ref_crc = (ref_crc_buf[0] << 8) | ref_crc_buf[1];
	ret = gc02m1_check_crc16(gc02m1_data_awb, crc_data_size, ref_crc,group_flag);
	otp_status_sub = 0;
	if (ret == 0) {
		otp_status_sub = otp_status_sub | CRC_FAIL;
		LOG_INF("gc02m1 CRC fail!\n");
	} else {
		LOG_INF("gc02m1 CRC success!\n");
	}
}

#define SUB_SERIAL_NUM_DATA_PATH "/data/vendor/camera_dump/serial_number_sub.bin"
#define ID_START_ADDR 0x18
#define ID_DATA_SIZE 11
#define GC02M1_SERIAL_NUM_SIZE 11
static kal_uint8 gc02m1_data_id[GC02M1_SERIAL_NUM_SIZE];
static void gc02m1_OTP_dump_bin(const char *file_name, uint32_t size, const void *data);
static void read_gc02m1_id_info(void)
{
	int i = 0;
	//init setting
	write_cmos_sensor(0xfc, 0x01);
	...
	write_cmos_sensor(0xf3, 0x30);//0TP read init set
	write_cmos_sensor(0xfe, 0x02);//page select
	write_cmos_sensor(0x17,0x18);//set addr
	write_cmos_sensor(0xf3, 0x34); //otp read pulse

	for (i = 0; i < ID_DATA_SIZE; i++) {
		write_cmos_sensor(0x17, (ID_START_ADDR+i*0x08));
		write_cmos_sensor(0xf3, 0x34);
		gc02m1_data_id[i] = read_cmos_sensor(0x19);
		LOG_INF("addr = 0x%x, value = 0x%x\n",(ID_START_ADDR+i*0x08), gc02m1_data_id[i]);
	}
	gc02m1_OTP_dump_bin(SUB_SERIAL_NUM_DATA_PATH,GC02M1_SERIAL_NUM_SIZE, gc02m1_data_id);
}
static void gc02m1_OTP_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
...
unsigned int gc02m1_read_otpdata(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size)
{
	kal_uint16 i;
	for (i = 0; i < size; i++) {
		data[i] = gc02m1_data_awb[i+1];
	}
	return size;
}
