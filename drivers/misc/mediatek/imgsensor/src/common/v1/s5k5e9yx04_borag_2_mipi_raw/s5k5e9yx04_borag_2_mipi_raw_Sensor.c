...
#include "s5k5e9yx04_borag_2_mipi_raw_Sensor.h"
//#include "hwinfo.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K5E9YX04_BORAG_2_SENSOR_ID,
...
{
	LOG_INF("s5k5e9yx04_borag_2_mipi_raw %s:%d E", __func__, __LINE__);
   ...
   // +++++++++++++++++++++++++++//
    ...
    LOG_INF("s5k5e9yx04_borag_2_mipi_raw %s:%d X", __func__, __LINE__);
}	/*	sensor_init  */
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_err("s5k5e9yx04_borag_2[%s](%d)match ok! i2c write id: 0x%x,read sensor id: 0x%x module_id 0x4f46 = : 0x%x \n",
					__FUNCTION__,__LINE__, imgsensor.i2c_write_id,  *sensor_id, module_id);
				memset(front_cam_name,0x00,sizeof(front_cam_name));
				if (module_id == 0x4f46)
				{
					memcpy(front_cam_name,"1_s5k5e9yx04_borag_2",64);
					get_front_cam_efuse_id();
					ontim_get_otp_data(*sensor_id, NULL, 0);
				}
				else
				{
					memcpy(front_cam_name,"1_s5k5e9yx04_borag_no_otp",64);
				}
				return ERROR_NONE;
			}

            pr_err("s5k5e9yx04_borag_2[%s](%d)match fail! i2c write id: 0x%x,read sensor id: 0x%x  module_id 0x4f46 !=: 0x%x \n", 
            		__FUNCTION__,__LINE__, imgsensor.i2c_write_id,  *sensor_id, module_id);
			retry--;
		} while(retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_err("s5k5e9yx04_borag_2_mipi_raw[%s](%d)    match  ok    i2c write id: 0x%x,      read sensor id: 0x%x    need id: 0x%x \n", 
                __FUNCTION__,__LINE__, imgsensor.i2c_write_id,  sensor_id, imgsensor_info.sensor_id);
				break;
			}
				pr_err("s5k5e9yx04_borag_2_mipi_raw[%s](%d)    match  fail    i2c write id: 0x%x,      read sensor id: 0x%x    need id: 0x%x \n", 
                __FUNCTION__,__LINE__, imgsensor.i2c_write_id,  sensor_id, imgsensor_info.sensor_id);
			retry--;
		} while(retry > 0);
		i++;
...
UINT32 S5K5E9YX04_BORAG_2_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
