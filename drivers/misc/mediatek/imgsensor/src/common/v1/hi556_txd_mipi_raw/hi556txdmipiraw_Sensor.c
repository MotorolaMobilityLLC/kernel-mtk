...
#include "hi556txdmipiraw_Sensor.h"

#define PFX "hi556_camera_sensor"
#define LOG_INF(format, args...)    \
	pr_debug(PFX "[%s] " format, __func__, ##args)

extern kal_uint8 otp_status_sub;//otp_state
#define HI556_EEPROM_SLAVE_ADDR 0xA4
#define HI556_SENSOR_IIC_SLAVE_ADDR 0x40
#define HI556_EEPROM_SIZE  0x798
#define HI556_SERIAL_NUM_SIZE 16
#define HI556_EEPROM_MODULE_INFO_SIZE 54
#define HI556_EEPROM_AWB_SIZE 16
#define HI556_EEPROM_LSC_SIZE 1868
#define EEPROM_DATA_PATH "/data/vendor/camera_dump/hi556_eeprom_data.bin"
#define SUB_SERIAL_NUM_DATA_PATH "/data/vendor/camera_dump/serial_number_sub.bin"

static uint8_t HI556_eeprom[HI556_EEPROM_SIZE] = {0};
static calibration_status_t mnf_status = CHECK_FAILURE;
static calibration_status_t awb_status = CHECK_FAILURE;
static calibration_status_t lsc_status = CHECK_FAILURE;
extern char module_info_wide[32];//module_info
...
{ 2592, 1944,    0,    0, 2592, 1944, 2592, 1944,
	 0,    0, 2592, 1944,    0,    0, 2592, 1944},
	// video
{ 2592, 1944,    0,    243, 2592, 1458, 2592, 1458,
	 0,    0, 2592, 1458,    0,    0, 2592, 1458},
};
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0e0a, 0x0001); //odin
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x0030, 0x1111);
	write_cmos_sensor(0x0032, 0x1111);
	write_cmos_sensor(0x0006, 0x080e);
	write_cmos_sensor(0x0a22, 0x0000);
	write_cmos_sensor(0x0a12, 0x0a20);
	write_cmos_sensor(0x0a14, 0x0798);
	write_cmos_sensor(0x003e, 0x0000);
	write_cmos_sensor(0x0074, 0x080c);
	write_cmos_sensor(0x0070, 0x0405);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_video_hi556[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
#if MULTI_WRITE
	hi556_table_write_cmos_sensor(
	addr_data_pair_video_hi556,
		sizeof(addr_data_pair_video_hi556) /
		sizeof(kal_uint16));
#else
		LOG_INF("%s fps = 300\n", __func__);
		write_cmos_sensor(0x0b0a, 0x8252);
		...
		write_cmos_sensor(0x0022, 0x0008);
		write_cmos_sensor(0x0028, 0x0017);
		write_cmos_sensor(0x0024, 0x011b);
		write_cmos_sensor(0x002a, 0x0120);
		write_cmos_sensor(0x0026, 0x0122);
		write_cmos_sensor(0x002c, 0x06d3);
		write_cmos_sensor(0x002e, 0x1111);
		write_cmos_sensor(0x0030, 0x1111);
		write_cmos_sensor(0x0032, 0x1111);
		write_cmos_sensor(0x0006, 0x080e);
		write_cmos_sensor(0x0a22, 0x0000);
		write_cmos_sensor(0x0a12, 0x0a20);
		write_cmos_sensor(0x0a14, 0x05b2);
		write_cmos_sensor(0x003e, 0x0000);
		write_cmos_sensor(0x0074, 0x080c);
		write_cmos_sensor(0x0070, 0x0405);
		write_cmos_sensor(0x0804, 0x0200);
...
static void HI556_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_drv_lock);
	...
	//read eeprom data
	for (i = 0; i < size; i ++) {
		HI556_eeprom[i] = read_cmos_sensor(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = HI556_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_drv_lock);
}

static void HI556_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
...
static int32_t eeprom_util_checksum(uint8_t *data, uint32_t size, uint8_t *check_data)
{
	int32_t check_match = 0;
	uint32_t checksum = 0;
	uint16_t i = 0;
	...
	//LOG_INF(" 0x%04x, 0x%02x, checksum[0x%04x]=0x%04x", size, data[i], size, checksum);
	if (((checksum % 0xFF) + 1) == check_data[0]) {
...
static calibration_status_t HI556_check_manufacturing_data(void *data)
{
	struct hi556_eeprom_t *eeprom = (struct hi556_eeprom_t*)data;

	if (!eeprom_util_checksum(eeprom->module_inf_id, HI556_EEPROM_MODULE_INFO_SIZE,
		eeprom->module_checksum)) {
		LOG_INF("Manufacturing Checksum Fails!");
		return CHECK_FAILURE;
	}
	LOG_INF("Manufacturing Checksum Pass");
	return NO_ERRORS;
}

static calibration_status_t HI556_check_awb_data(void *data)
{
	struct hi556_eeprom_t *eeprom = (struct hi556_eeprom_t*)data;

	if(!eeprom_util_checksum(eeprom->awb_data,
		HI556_EEPROM_AWB_SIZE,
		eeprom->awb_checksum)) {
		LOG_INF("AWB Checksum Fails!");
		return CHECK_FAILURE;
	}

	LOG_INF("AWB Checksum Pass");
	return NO_ERRORS;
}

static calibration_status_t HI556_check_lsc_data_mtk(void *data)
{
	struct hi556_eeprom_t *eeprom = (struct hi556_eeprom_t*)data;

	if (!eeprom_util_checksum(eeprom->lsc_data, HI556_EEPROM_LSC_SIZE,
		eeprom->lsc_checksum)) {
		LOG_INF("LSC Checksum Fails!");
		return CHECK_FAILURE;
	}
	LOG_INF("LSC Checksum Pass");
	return NO_ERRORS;
}

static void HI556_eeprom_get_serial_number(void *data)
{
	struct hi556_eeprom_t *eeprom = (struct hi556_eeprom_t*)data;

	HI556_eeprom_dump_bin(SUB_SERIAL_NUM_DATA_PATH, HI556_SERIAL_NUM_SIZE, eeprom->module_serial_number);
}

static void HI556_eeprom_format_calibration_data(void *data)
{
	otp_status_sub = 0x00;
	if (NULL == data) {
	...
	mnf_status = HI556_check_manufacturing_data(data);
	awb_status = HI556_check_awb_data(data);
	lsc_status = HI556_check_lsc_data_mtk(data);
	otp_status_sub = otp_status_sub | mnf_status | awb_status << 1 | lsc_status << 2;

	LOG_INF("status mnf:%d, awb:%d, lsc:%d",
		mnf_status, awb_status, lsc_status);
}

static void HI556_eeprom_get_module_info(void *data)
{
	struct hi556_eeprom_t *eeprom = (struct hi556_eeprom_t*)data;
	if (eeprom->module_inf_id[0] == 0x58) { // txd
		sprintf(module_info_wide, "%s", "hi556_txd_mipi_raw");
	} else if (eeprom->module_inf_id[0] == 0x55) { // hlt
		sprintf(module_info_wide, "%s", "hi556_hlt_mipi_raw");
	}
	LOG_INF("eeprom->module_inf_id[0]:0x%x", eeprom->module_inf_id[0]);
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				HI556_read_data_from_eeprom(HI556_EEPROM_SLAVE_ADDR, 0x0000, HI556_EEPROM_SIZE);
				//HI556_eeprom_dump_bin(EEPROM_DATA_PATH, HI556_EEPROM_SIZE, (void *)HI556_eeprom);
				HI556_eeprom_get_serial_number((void *)HI556_eeprom);
				HI556_eeprom_format_calibration_data((void *)HI556_eeprom);
				HI556_eeprom_get_module_info((void *)HI556_eeprom);
				LOG_INF(
