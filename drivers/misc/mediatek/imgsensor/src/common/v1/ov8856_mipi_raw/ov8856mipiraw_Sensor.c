...
#define PFX "ov8856mipiraw_Sensor.c"

#define LOG_INF(fmt, args...)   pr_err(PFX "[%s](%d) " fmt, __func__, __LINE__, ##args)
typedef enum {
	OV8856R2A,
	OV8856R1A
}OV8856_VERSION;
...
#ifdef OV8856OTP
#define RG_TYPICAL   	0x12c//0x14d	//modified when write WB !!!
#define BG_TYPICAL		0x17d//0x16f	//modified when write WB !!!
struct otp_struct {
//	flag bit[7]:[6] = 0:1 group1   bit[5]:[4] = 0:1 group2 
...
//	lsc_flag bit[7]:[6] = 0:1 group1   bit[5]:[4] = 0:1 group2 	
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	addr = 0;
	if(((*otp_ptr).flag & 0xc0) == 0x40) {// group1 
		addr = 0x7011; // base address of info group 1
	}
	else if(((*otp_ptr).flag & 0x30) == 0x10) {// group2 
