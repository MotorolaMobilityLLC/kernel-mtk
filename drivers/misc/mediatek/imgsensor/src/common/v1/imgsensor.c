...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
	{
		...
		if (buf_size == 8008){//for s5kjn1sq03
                        pr_err("check: buf_size is 8008\n");
			if (buf_size > 2*PDAF_DATA_SIZE) {
				kfree(pFeaturePara);
				PK_DBG("check: buf_size > PDAF_DATA_SIZE\n");
				return -EINVAL;
			}
			pPdaf_data = kmalloc(
				    sizeof(char) * 2*PDAF_DATA_SIZE,
				    GFP_KERNEL);
			...
			memset(pPdaf_data, 0xff, sizeof(char) *2* PDAF_DATA_SIZE);
		}else{
