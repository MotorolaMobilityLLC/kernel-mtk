...
bool curSensorIs50M = false;/*Judge current sensor type*/
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		err = ERROR_SENSOR_CONNECT_FAIL;
		 ...
		 if (S5KJN1_QT_P410AE_SENSOR_ID == sensorID) {
            curSensorIs50M = true;
    }
			pr_info(" Sensor found ID = 0x%x, curSensorIs50M = %d\n", sensorID, curSensorIs50M);
			err = ERROR_NONE;
	}
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	//strncpy(pSensorInfo->TinnoModuleSn,pInfo->TinnoModuleSn,sizeof(pInfo->TinnoModuleSn));//Tinno add for read module sn
	memcpy((void *)pSensorInfo->TinnoModuleSn,(void *)pInfo->TinnoModuleSn,sizeof(pInfo->TinnoModuleSn));
