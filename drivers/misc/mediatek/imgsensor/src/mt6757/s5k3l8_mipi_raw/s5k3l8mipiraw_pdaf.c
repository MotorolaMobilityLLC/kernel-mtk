...
#define S5K3L8_EEPROM_WRITE_ID   0xA0
...
static bool selective_read_eeprom(kal_uint16 addr, BYTE* data)
{
	...
	if(iReadRegI2C(pu_send_cmd, 2, (u8*)data, 1, S5K3L8_EEPROM_WRITE_ID)<0)
		return false;
    return true;
}
...
static bool _read_3L8_eeprom(kal_uint16 addr, BYTE* data, kal_uint32 size ){
	...
	for(i = 0; i < size; i++) {
		...
		LOG_INF("read_eeprom 0x%0x %d=== %0x\n",offset, data[i],data[i]);
		offset++;
	}
	get_done = true;
	last_size = size;
...
//	size = 1404;
	//BYTE header[9]= {0};
	//_read_3L8_eeprom(0x0000, header, 9);
...
//if((s5k3L8_eeprom_data[0] != 1) || (s5k3L8_eeprom_data[498] != 1) || (s5k3L8_eeprom_data[1306] != 1))
	if((s5k3L8_eeprom_data[0] != 1) || (s5k3L8_eeprom_data[498] != 1))
	{
	...
	{
		data[i] = s5k3L8_eeprom_data[i+1];
	}

	for(i = 496; i < 1404; i++)  // proc2 data
	{
		data[i] = s5k3L8_eeprom_data[i+3];
	}	
	...
	//memcpy(data, s5k3L8_eeprom_data, size); //huangsh4 make pdaf buffer correct
    return true;
}
