...
{
	kal_uint16 Length =1;
	 LOG_INF("[%s] ofim mid  read_s5k4h5_otp \n",__func__);
	write_cmos_sensor(0x6028,0x4000);
	...
	if(!read_otp(AWB_PAGE, OTP_MID_GROUP_1,&MID,Length))
	{
	 ...
	 LOG_INF("[%s] group 1 ofilm -mid = %d   read_s5k4h5_otp \n",__func__,MID);
		 if(MID== S5K4H8_OFILM_MID)
		 {
		       return 0;
		 }else {
		read_otp(AWB_PAGE, OTP_MID_GROUP_2,&MID,Length);
		 LOG_INF("[%s] group 2 ofilm  mid = %d   read_s5k4h5_otp \n",__func__,MID);
		 }
...
static BOOL read_data_from_otp(void)
{
	...
	calculate_awb_data(GOLD_RG_VALUE,  GOLD_BG_VALUE , GOLD_RG_VALUE, GOLD_BG_VALUE) ; 
	} else {
