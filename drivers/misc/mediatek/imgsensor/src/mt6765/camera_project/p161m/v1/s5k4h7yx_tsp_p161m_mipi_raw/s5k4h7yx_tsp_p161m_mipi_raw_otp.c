...
OTP tsp_4h7_otp_data_info = {0};

/**********************************************************
 * get_tsp_4h7_page_data
 * get page data
...
void get_tsp_4h7_page_data(int pageidx, unsigned char *pdata)
{
	...
	{
		mdelay(1);
		get_byte = otp_tsp_4h7_read_cmos_sensor(0x0A01);
	}while((get_byte & 0x01) != 1);

	for(i = 0; i < 20; i++){
		pdata[i] = otp_tsp_4h7_read_cmos_sensor(addr);
		addr++;
        printk("OTP  otp_tsp_4h7_read_cmos_sensor pdata[%d]=0x%x",i,pdata[i]);
	}
...
void get_tsp_4h7_page_data_lsc(int pageidx, unsigned char *pdata)
{
	...
	for(i = 0; i < 64; i++){
		pdata[i] = otp_tsp_4h7_read_cmos_sensor(addr);
		addr++;
               printk("OTP  otp_tsp_4h7_read_cmos_sensor_lsc pdata[%d]=0x%x",i,pdata[i]);
	}
...
unsigned short tsp_4h7_selective_read_region(int pageidx,unsigned int addr)
{
	unsigned short get_byte = 0;
	otp_tsp_4h7_write_cmos_sensor_8(0x0A02,pageidx);
	otp_tsp_4h7_write_cmos_sensor_8(0x0A00,0x01);
	...
	get_byte = otp_tsp_4h7_read_cmos_sensor(addr);
	otp_tsp_4h7_write_cmos_sensor_8(0x0A00,0x00);
...
unsigned short tsp_4h7_selective_read_region_16(int pageidx,unsigned int addr)
{
	unsigned short get_byte = 0;
	otp_tsp_4h7_write_cmos_sensor_8(0x0A02,pageidx);
	otp_tsp_4h7_write_cmos_sensor_8(0x0A00,0x01);
	...
	get_byte = ((otp_tsp_4h7_read_cmos_sensor(addr) << 8) | otp_tsp_4h7_read_cmos_sensor(addr+1));
	printk("--OTP  tsp_4h7_selective_read_region_16  addr=0x%x  addr+1=0x%x   get_byte=0x%x  addr=0x%x pageidx=%d" ,otp_tsp_4h7_read_cmos_sensor(addr),otp_tsp_4h7_read_cmos_sensor(addr+1),get_byte,addr, pageidx);
...
#if 0
unsigned int tsp_4h7_selective_read_region_16(int pageidx,unsigned int addr)
{
...
/*****************************************************
 * tsp_4h7_cal_rgb_gain
 * **************************************************/
void tsp_4h7_cal_rgb_gain(int* r_gain, int* g_gain, int* b_gain, unsigned int r_ration, unsigned int b_ration)
{
...
//Chenyee camera pangfei 20180608 modify for CSW1802A-9 begin
	...
	tsp_4h7_otp_data_info.frgcur = (unsigned int)(tsp_4h7_otp_data_info.nrcur*1000 / tsp_4h7_otp_data_info.ngcur + 500);
	tsp_4h7_otp_data_info.fbgcur = (unsigned int)(tsp_4h7_otp_data_info.nbcur*1000 / tsp_4h7_otp_data_info.ngcur + 500);

	tsp_4h7_otp_data_info.nggolden = G_G_GOLDEN;//(unsigned int)((tsp_4h7_otp_data_info.ngrgolden + tsp_4h7_otp_data_info.ngbgolden)*1000 / 2 + 500);

	tsp_4h7_otp_data_info.frggolden = R_G_GOLDEN;//(unsigned int)(tsp_4h7_otp_data_info.nrgolden*1000/ tsp_4h7_otp_data_info.nggolden +500);
	tsp_4h7_otp_data_info.fbggolden = B_G_GOLDEN;//(unsigned int)(tsp_4h7_otp_data_info.nbgolden*1000/ tsp_4h7_otp_data_info.nggolden +500);
#else
	//tsp_4h7_otp_data_info.ngcur = (unsigned int)((tsp_4h7_otp_data_info.ngrcur + tsp_4h7_otp_data_info.ngbcur)*1000/2 + 500);
...
//Chenyee camera pangfei 20180608 modify for CSW1802A-9 end
	...
	otp_tsp_4h7_write_cmos_sensor_8(0x0210,r_gain_h);
	otp_tsp_4h7_write_cmos_sensor_8(0x0211,r_gain_l);

	otp_tsp_4h7_write_cmos_sensor_8(0x020E,g_gain_h);
	otp_tsp_4h7_write_cmos_sensor_8(0x020F,g_gain_l);

	otp_tsp_4h7_write_cmos_sensor_8(0x0214,g_gain_h);
	otp_tsp_4h7_write_cmos_sensor_8(0x0215,g_gain_l);

	otp_tsp_4h7_write_cmos_sensor_8(0x0212,b_gain_h);
	otp_tsp_4h7_write_cmos_sensor_8(0x0213,b_gain_l);
	printk("OTP    apply_tsp_4h7_otp_awb  r_gain_h=0x%x  r_gain_l=0x%x  g_gain_h=0x%x  g_gain_l=0x%x  b_gain_h=0x%x  b_gain_l=0x%x \n",r_gain_h,r_gain_l,g_gain_h,g_gain_l,b_gain_h,b_gain_l);
}
...
void apply_tsp_4h7_otp_enb_lsc(void)
{
	printk("OTP enable lsc\n");
	otp_tsp_4h7_write_cmos_sensor_8(0x0B00,0x01);
	otp_tsp_4h7_write_cmos_sensor_8(0x3400,0x00);
}

/*********************************************************
 * otp_group_info_tsp_4h7
 * *****************************************************/
//Chenyee camera pangfei 20180608 modify for CSW1802A-9 begin
#ifdef ORIGINAL
int otp_group_info_tsp_4h7(void)
{
	memset(&tsp_4h7_otp_data_info,0,sizeof(OTP));

	tsp_4h7_otp_data_info.lsc_infoflag =
		tsp_4h7_selective_read_region(S5K4H7SUB_LSC_PAGE,S5K4H7SUB_OTP_LSCFLAG_ADDR);

	if( tsp_4h7_otp_data_info.lsc_infoflag == 0x01 ){
		tsp_4h7_otp_data_info.lsc_offset = 0;
		tsp_4h7_otp_data_info.lsc_group = 1;
		tsp_4h7_otp_data_info.lsc_sum = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A32);
                printk("4H7 OTP tsp_4h7_otp_data_info.lsc_group = 1;\n");
	}
	else if ( tsp_4h7_otp_data_info.lsc_infoflag == 0x03 ){
		tsp_4h7_otp_data_info.lsc_offset = 1;
		tsp_4h7_otp_data_info.lsc_group = 2;
		tsp_4h7_otp_data_info.lsc_sum = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A34);
                printk("tsp_4h7_otp_data_info.lsc_group = 2;\n");
	}
	else{
	...
	tsp_4h7_otp_data_info.flag_infoflag =
		tsp_4h7_selective_read_region(S5K4H7SUB_FLAG_PAGE,S5K4H7SUB_OTP_FLAGFLAG_ADDR);
	...
	tsp_4h7_otp_data_info.flag_module_integrator_id =
		otp_tsp_4h7_read_cmos_sensor(S5K4H7SUB_OTP_FLAGFLAG_ADDR + tsp_4h7_otp_data_info.flag_offset + 1);

	tsp_4h7_otp_data_info.infoflag = tsp_4h7_selective_read_region(S5K4H7SUB_AWB_PAGE,S5K4H7SUB_OTP_AWBFLAG_ADDR);

	tsp_4h7_otp_data_info.nrcur = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A0E);
	tsp_4h7_otp_data_info.nbcur = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A10);
	tsp_4h7_otp_data_info.ngrcur = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A12);
	tsp_4h7_otp_data_info.ngbcur = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A14);

	tsp_4h7_otp_data_info.nrgolden = 0x020d;//tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A16);
	tsp_4h7_otp_data_info.nbgolden = 0x0297;//tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A18);
	tsp_4h7_otp_data_info.ngrgolden = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A1A);
	tsp_4h7_otp_data_info.ngbgolden = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A1c);

	if( (tsp_4h7_otp_data_info.infoflag>>4 & 0x0c) == 0x04 ){
		tsp_4h7_otp_data_info.awb_offset = 0;
		tsp_4h7_otp_data_info.group = 1;
		tsp_4h7_otp_data_info.awb_flag_sum = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A2E);
	}
	else if ( (tsp_4h7_otp_data_info.infoflag>>4 & 0x03) == 0x01 ){
		tsp_4h7_otp_data_info.awb_offset = 0X10;
		tsp_4h7_otp_data_info.group = 2;
		tsp_4h7_otp_data_info.awb_flag_sum = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A30);
	}
	else{
		printk("4h7 OTP read data fail tsp_4h7_otp_data_info.empty!!!\n");
		//goto error;
	}
...
int otp_group_info_tsp_4h7(void)
{
	...
	if( (tsp_4h7_otp_data_info.flag_group ==1) || (tsp_4h7_otp_data_info.flag_group == 2)){
		if(tsp_4h7_otp_data_info.flag_group ==1){
			...
			tsp_4h7_otp_data_info.fbgcur = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A07);//0x179;
			...
			tsp_4h7_otp_data_info.fbgcur = tsp_4h7_selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A1C);
...
unsigned int tsp_4h7_sum_awb_flag(unsigned int sum_start, unsigned int sum_end, unsigned char *pdata)
{
	int i = 0;
	unsigned int start;
...
unsigned int tsp_4h7_sum_awb_flag_lsc(unsigned int sum_start, unsigned int sum_end, unsigned char *pdata)
{
	int i = 0;
	unsigned int start;
	unsigned int re_sum = 0;
...
bool tsp_4h7_check_sum_flag_awb(void)
{
...
bool  tsp_4h7_check_sum_flag_lsc(void)
{
	...
	if(tsp_4h7_otp_data_info.lsc_group == 1){
		...
		for(page_start = 6, page_end = 12; page_start <= page_end; page_start++){
			...
			if(page_start == 6){
				...
				sum_slc += tsp_4h7_sum_awb_flag_lsc(0x0A04, 0X0A13, data_p[page_start-1]);
			}
		}
	}

	#if 0
	if(sum_slc == tsp_4h7_otp_data_info.lsc_sum){
                printk("OTP 4h7 check lsc sum ok !!!");
		apply_tsp_4h7_otp_enb_lsc();
		tsp_4h7_otp_data_info.lsc_check_flag = 1;
	}
	else{
		printk("OTP 4h7 check lsc sum fail!!!");
		bresult &= 0;
		tsp_4h7_otp_data_info.lsc_check_flag = 0;
	}
        #endif
...
bool update_tsp_otp(void)
{
	int result = 1;
	if(otp_group_info_tsp_4h7() == -1){
		printk("OTP read data fail  empty!!!\n");
		result &= 0;
	}
	else{
		if(tsp_4h7_check_sum_flag_awb() == 0 && tsp_4h7_otp_data_info.lsc_check_flag){
