...
#include "s5k4h7wide_qt_p410ae_mipi_raw_Sensor.h"
...
#define PFX "s5k4h7wide_qt_p410ae_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7WIDE_QT_P410AE_SENSOR_ID,
...
#define DUMP_DATA_PATH "/data/vendor/camera_dump/s5k4h7_otp_data.bin"
OTP_DATA otp_data;
...
static void write_cmos_sensor_16(kal_uint16 addr, kal_uint16 para)
{
 ...
 static int read_otp_page_data(int page, int start_add, unsigned char *Buff, int size)
 {
 	...
 	for ( i = 0; i < size; i++ ) {
 		...
 		LOG_INF("cur page = %d, Buff[%d] = 0x%x\n",page,i,Buff[i]);
 	}
 ...
 static bool read_valid_data(int page, int start_add)
 {
 	...
 	if (page_flag[0] != 0 || page_flag[1] != 0) {
 		otp_data.page_flag = ((page_flag[0] > page_flag[1])? page_flag[0] : page_flag[1]);
 		if (!((getbit(otp_data.page_flag,6)) && !(getbit(otp_data.page_flag,7)))) {
 			LOG_INF("valid bit 7 = %d,bit 6 = %d\n",getbit(otp_data.page_flag,7),getbit(otp_data.page_flag,6));
 ...
 static int read_otp_data(void)
 {
 	...
 	if(!page) return -1;
 	LOG_INF("moduleid vaild page = %d", page);
	read_otp_page_data(page ,0x0a1a, otp_data.serial_number,8);
	read_otp_page_data(page ,0x0a12, otp_data.name,2);
 	//read module id
 
 	read_otp_page_data(page, MODULE_ID_START_ADD, otp_data.module_id, MODULE_ID_LENGTH);
 	for (i = 0;i < MODULE_ID_LENGTH ; i++ ) {
 		LOG_INF ("+++4h7 6 page = %d modulue_id[%d] = 0x%x",page,i,otp_data.module_id[i]);
 		sum += otp_data.module_id[i];
 	}
 	if (!sum) return -1;
 	...
 	read_otp_page_data(page, 0x0a3d, otp_data.awb_gloden, 7);
 	read_otp_page_data(page + 1, 0x0a04, otp_data.awb_gloden + 7, 1);
 	read_otp_page_data(page + 1, 0x0a19, otp_data.awb_unint, 8);
	...
	if(page == 33) {
 		for (i = 0; i < 30 ; i++ ) {
 			if (i == 29) {
 				read_otp_page_data(page + i, lsc_addr, otp_data.lscdata + (64*i), 15);
 			} else {
 				read_otp_page_data(page + i, lsc_addr, otp_data.lscdata + (64*i), 64);
 			}
 		}
	} else if(page == 62) {
		lsc_addr = 0x0a13;
 		for (i = 0; i < 30 ; i++ ) {
			if( (i!=0) && (i!=29)) {
 				...
 				read_otp_page_data(page + i, lsc_addr, otp_data.lscdata + (64*i), 49);
 			} else if(i == 29){
 				read_otp_page_data(page + i, lsc_addr, otp_data.lscdata + (49 + 64*(i-1)), 30);
 			}
 		}
	} else if(page == 91) {
		lsc_addr = 0x0a22;
 		for (i = 0; i < 30 ; i++ ) {
			if( (i!=0) && (i!=29)) {
 				...
 				read_otp_page_data(page + i, lsc_addr, otp_data.lscdata + (64*i), 34);
 			} else if(i == 29){
 				read_otp_page_data(page + i, lsc_addr, otp_data.lscdata + (34 + 64*(i-1)), 45);
 			}
 		}
	}
 	return 0;
 ...
 unsigned int s5k4h7_read_region(struct i2c_client *client,unsigned int addr,unsigned char *data, unsigned int size)
 {
 	if (size == 4) { //read module id
 		memcpy(data, otp_data.module_id, size);
 		LOGE("add = 0x%x,read module id\n",addr);
 	} else if (size == 8) { //read single awb data
 		if (addr == 0x05) {
 			memcpy(data, (otp_data.awb_gloden), size);
 			LOGE("add = 0x%x, read golden\n",addr);
 		} else {
 			memcpy(data,(otp_data.awb_unint), size);
 			LOGE("add = 0x%x, read awb_unint\n",addr);
 		}
 	} else if (size >=1868 && size < 2048){
 		if (addr == 0x15) {
 			memcpy(data, (otp_data.lscdata), size);
 		}else {
 			memcpy(data, (otp_data.lscdata + 1), size);
 		}
 	} else if (size > 2048 && size < 6400){ //read all data
 			read_otp_data_all(data, size);
 	}else{
 		LOGE("add = 0x%x, size = %d ,read error !!!\n",addr,size); 		
 	}
...
#endif
#ifdef CONFIG_TINNO_PRODUCT_INFO
			if('Q' ==otp_data.name[0] && 'T' == otp_data.name[1])
			FULL_PRODUCT_DEVICE_INFO_CAMERA(S5K4H7WIDE_QT_P410AE_SENSOR_ID,2,"s5k4h7wide_qt_p410ae_mipi_raw",imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height);
                         else if('O' ==otp_data.name[0] && 'F' == otp_data.name[1])
			FULL_PRODUCT_DEVICE_INFO_CAMERA(S5K4H7WIDE_QT_P410AE_SENSOR_ID,2,"s5k4h7wide_of_p410ae_mipi_raw",imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height);
#endif
                   break;
			}
			retry--;
...
/*#ifdef CONFIG_TINNO_PRODUCT_INFO
        FULL_PRODUCT_DEVICE_INFO_CAMERA(S5K4H7WIDE_QT_P410_SENSOR_ID, 1, "s5k4h7wide_qt_p410ae_mipi_raw",
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	LOG_INF("enable: %d\n", enable);
        ...
        if (enable) {
                write_cmos_sensor_8(0x0601, 0x01);  //Black
                ...
                write_cmos_sensor_8(0x0601, 0x00);  //Default
                write_cmos_sensor_8(0x0602, 0x00);
                write_cmos_sensor_8(0x0604, 0x00);
                write_cmos_sensor_8(0x0606, 0x00);
                write_cmos_sensor_8(0x0608, 0x00);
