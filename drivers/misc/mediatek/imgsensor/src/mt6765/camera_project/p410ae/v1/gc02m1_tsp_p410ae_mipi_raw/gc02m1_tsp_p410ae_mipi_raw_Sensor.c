...
#include "gc02m1_tsp_p410ae_mipi_raw_Sensor.h"
...
#define PFX "gc02m1_camera_sensor_kd"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02M1_TSP_P410AE_SENSOR_ID,
...
#define READ_OTP 1
#if READ_OTP
static kal_uint8 otp_data[24];
#define OTP_SIZE 17
#define OTP_OFFSET 0x78
...
static void read_otp_data()
{
	...
	for(i = 0; i < OTP_SIZE; i++){
		...
		otp_data[i] = read_cmos_sensor(0x19);
		LOG_INF("otp_data[%d] = 0x%x\n", i, otp_data[i]);
	}
	write_cmos_sensor(0xf7, 0x00);
	write_cmos_sensor(0xfe, 0x00);
}
unsigned int gc02m1_read_region(struct i2c_client *client,unsigned int addr,unsigned char *data, unsigned int size)
{
	if(size <= 8) {
		if(0x40 == otp_data[0]) {
			memcpy(data, otp_data+1 + addr, size);
		} else if(0x10 == otp_data[0]) {
			memcpy(data, otp_data+9 + addr, size);
		} else {
			size = 0;
		}
	} else if(size > 17){
		memcpy(data,otp_data,17);
		return 17;
	}else{
		memcpy(data,otp_data,size);
	}
	return size;
}
#endif
		...
		do {
			*sensor_id = return_sensor_id()+1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("gc02m1 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
#if READ_OTP
				read_otp_data();
#endif
#ifdef CONFIG_TINNO_PRODUCT_INFO
#ifdef HAWAIIPL_P411AE
        FULL_PRODUCT_DEVICE_INFO_CAMERA(GC02M1_TSP_P410AE_SENSOR_ID, 4, "gc02m1_p411be_mipi_raw",
                imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height);
#else
        FULL_PRODUCT_DEVICE_INFO_CAMERA(GC02M1_TSP_P410AE_SENSOR_ID, 4, "gc02m1_p410ae_mipi_raw",
                imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height);
#endif
#endif		
				return ERROR_NONE;
			}
			LOG_INF("gc02m1 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
