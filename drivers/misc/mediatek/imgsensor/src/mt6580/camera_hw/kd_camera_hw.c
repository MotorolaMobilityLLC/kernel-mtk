...
/*#if 0
	...
	if ((0 == pinSetIdx) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC5005_MIPI_RAW, currSensorName)))
        {
	    ...
	    //VCAMA
            ...
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power (VCAMA), power id = %d \n", VCAMA);
                 goto _kdCISModulePowerOn_exit_;
            }
            udelay(100);
	...
	//enable active sensor
		...
		mtkcam_gpio_set(pinSetIdx, CAMRST,pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
        ...
        }else if ((1 == pinSetIdx) && currSensorName
			   && (0 == strcmp(SENSOR_DRVNAME_C2590_MIPI_RAW, currSensorName))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
			...
			mdelay(10);
		}else  if ((0 == pinSetIdx) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI553_MIPI_RAW, currSensorName)))
        {
	    //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
		mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
	...
	//enable active sensor
		...
		mtkcam_gpio_set(pinSetIdx, CAMRST,pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
			...
			mdelay(10);
			   ...
			   && (0 == strcmp(SENSOR_DRVNAME_GC2375_MIPI_RAW, currSensorName))) {
			/* VCAM_IO */
			if (TRUE != _hwPowerOn(VCAMIO, VOL_1800)) {
				PK_DBG
				    ("[CAMERA SENSOR] Fail to enable IO power (VCAM_IO),power id = %d\n",
			...
			if (TRUE != _hwPowerOn(VCAMD, VOL_1800)) {
				PK_DBG
				    ("[CAMERA SENSOR] Fail to enable D power (VCAM_D),power id = %d\n",
				     VCAMD);
				goto _kdCISModulePowerOn_exit_;
			}
			mdelay(1);
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST,pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
		}else if ((0 == pinSetIdx) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC5005W_MIPI_RAW, currSensorName)))
        {
            //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
		mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
	...
	if ((0 == pinSetIdx) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC5005_MIPI_RAW, currSensorName)))
		...
		mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
			...
			if (TRUE != _hwPowerDown(VCAMD)) {
				PK_DBG
				    ("[CAMERA SENSOR] Fail to OFF digital power (VCAM_D), power id = %d\n",
				     VCAMD);
				goto _kdCISModulePowerOn_exit_;
			}
			/* VCAM_IO */
		...
		}if ((0 == pinSetIdx) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC5005W_MIPI_RAW, currSensorName)))
        {
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
		mtkcam_gpio_set(pinSetIdx, CAMPDN,pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
