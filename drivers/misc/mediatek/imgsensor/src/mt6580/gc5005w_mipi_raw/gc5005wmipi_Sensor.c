...
#include "gc5005wmipi_Sensor.h"



/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5005W_camera_sensor"
#define LOG_1 LOG_INF("GC5005W,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define GC5005W_MIPI_USE_OTP

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC5005WDuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id =  GC5005W_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#ifdef GC5005W_MIPI_USE_OTP

#define DD_PARAM_QTY 			200
#define WINDOW_WIDTH  			0x0a30 //2608 max effective pixels
#define WINDOW_HEIGHT 			0x07a0 //1952
#define RG_TYPICAL    			0xb5
#define BG_TYPICAL    			0xb1

typedef struct otp_gc5005w{
	kal_uint16 module_id;
	kal_uint16 module_info;
	kal_uint16 lens_vcmid;
	kal_uint16 rg_gain;
	kal_uint16 bg_gain;
	kal_uint16 wb_flag;
...
static gc5005w_otp gc5005w_otp_info;
...
static kal_uint8 gc5005w_read_otp(kal_uint8 addr)
{
	kal_uint8 value;
...
static void gc5005w_select_page_otp(otp_page otp_select_page)
{
	kal_uint8 page;
...
static void gc5005w_gcore_read_otp_info(void)
{
	kal_uint8 flag0,flag_group;
	kal_uint16 checksum1=0;
	...
	memset(&gc5005w_otp_info,0,sizeof(gc5005w_otp));
	/*TODO*/
	gc5005w_select_page_otp(otp_page0);
	flag0 = gc5005w_read_otp(0x00);
	total_number0 = gc5005w_read_otp(0x01) + gc5005w_read_otp(0x02);
	LOG_INF("GC5005W_OTP : flag0 = 0x%x , total_number0 = %d\n",flag0,total_number0);
	
	gc5005w_select_page_otp(otp_page1);
	total_number1 = gc5005w_read_otp(0x28) + gc5005w_read_otp(0x29);
	LOG_INF("GC5005W_OTP : total_number1 = %d\n",total_number1);

//For GC
	gc5005w_otp_info.gc_flag = (flag0>>2)&0x03;
	LOG_INF("GC5005W_OTP : gc5005w_otp_info.gc_flag = 0x%x \n",gc5005w_otp_info.gc_flag);

	if(gc5005w_otp_info.gc_flag==0x01) 
	{
		gc5005w_otp_info.Val32=0x78;
		gc5005w_otp_info.Vald1=0xb0;
		gc5005w_otp_info.Val21=0x08;
		gc5005w_otp_info.Val29=0x14;
	}
	else
	{
		gc5005w_otp_info.Val32=0xfc;
		gc5005w_otp_info.Vald1=0xa0;
		gc5005w_otp_info.Val21=0x0f;
		gc5005w_otp_info.Val29=0x22;
	}
...
//DD
	...
	{
	case 0x00:
		LOG_INF("GC5005W_OTP_DD is Empty !!\n");
		gc5005w_otp_info.dd_flag = 0x00;
		break;
	case 0x01:	
		LOG_INF("GC5005W_OTP_DD is Valid!!\n");
		...
		LOG_INF("GC5005W_OTP_DD total_number=%d!!\n",total_number);
		...
		for(i=0; i<total_number; i++)
		{
			...
			check_dd_flag = gc5005w_read_otp(dd_rom_start + 4*(i-offset) + 3);
				
			if(check_dd_flag&0x10)
			{//Read OTP
				type = check_dd_flag&0x0f;
		
				dd0 = gc5005w_read_otp(dd_rom_start + 4*(i-offset));
				dd1 = gc5005w_read_otp(dd_rom_start + 4*(i-offset) + 1); 	
				dd2 = gc5005w_read_otp(dd_rom_start + 4*(i-offset) + 2);
				x = ((dd1&0x0f)<<8) + dd0;
				y = (dd2<<4) + ((dd1&0xf0)>>4);
				LOG_INF("GC5005W_OTP_DD : type = %d , x = %d , y = %d \n",type,x,y);
				LOG_INF("GC5005W_OTP_DD : dd0 = %d , dd1 = %d , dd2 = %d \n",dd0,dd1,dd2);
				
				if(type == 3)
				{
					for(j=0; j<4; j++)
					{
						gc5005w_otp_info.dd_param_x[cnt] = x;
						gc5005w_otp_info.dd_param_y[cnt] = y + j;
						gc5005w_otp_info.dd_param_type[cnt++] = 2;
					}
				}
				else if(type == 2)
				{
				...
				{
					gc5005w_otp_info.dd_param_x[cnt] = x;
					gc5005w_otp_info.dd_param_y[cnt] = y;
					gc5005w_otp_info.dd_param_type[cnt++] = type;
				}
			}
			else
			{
				LOG_INF("GC5005W_OTP_DD:check_id[%d] = %x,checkid error!!\n",i,check_dd_flag);
			}
		}
		gc5005w_otp_info.dd_cnt = cnt;
		gc5005w_otp_info.dd_flag = 0x01;
		break;
	case 0x02:
	case 0x03:
		LOG_INF("GC5005W_OTP_DD is Invalid !!\n");
		gc5005w_otp_info.dd_flag = 0x02;
		break;
	default :
		break;
	}
	...
	{
		start_add = index*8;
		flag_group= gc5005w_read_otp(start_add);
		if(flag_group==0x01)
		{       
                              for(i=1;i<7;i++)
			     {
				checksum1 += gc5005w_read_otp(start_add + i);
			     }
			     gc5005w_otp_info.module_id= gc5005w_read_otp(start_add + 1);
			     gc5005w_otp_info.module_info = gc5005w_read_otp(start_add + 2);
			     gc5005w_otp_info.rg_gain = gc5005w_read_otp(start_add + 3);
			     gc5005w_otp_info.bg_gain = gc5005w_read_otp(start_add + 4);
			     gc5005w_otp_info.golden_rg = gc5005w_read_otp(start_add + 5);
			     gc5005w_otp_info.golden_bg = gc5005w_read_otp(start_add + 6);
                             gc5005w_otp_info.checksum = gc5005w_read_otp(start_add + 7);
			    if(gc5005w_otp_info.checksum == (checksum1%0xff+1))
			    {
				LOG_INF("GC5005W_OTP_INFO_bolixin group%d is Valid !!\n", index + 1);
				gc5005w_otp_info.wb_flag = 0x01;
				break;
			    }
			    else
			    {
				LOG_INF("GC5005W_OTP_INFO_bolixin group%d checksum error,checksum1=0x%x !!\n", index + 1,(checksum1%0xff+1));
				gc5005w_otp_info.wb_flag = 0x00;
			    }
		        
		}
		else
		{
			LOG_INF("GC5005W_OTP_INFO&WB_BOLIXIN group%d is Invalid !!\n", index + 1);
			gc5005w_otp_info.wb_flag = 0x00;
		}
	}
	/*print otp information*/
	LOG_INF("GC5005W_OTP_INFO&WB_BOLIXIN:module_id=0x%x\n",gc5005w_otp_info.module_id);
	LOG_INF("GC5005W_OTP_INFO&WB_BOLIXIN:module_info=0x%x\n",gc5005w_otp_info.module_info);
	LOG_INF("GC5005W_OTP_INFO&WB_BOLIXIN:r/g=0x%x\n",gc5005w_otp_info.rg_gain);
	LOG_INF("GC5005W_OTP_INFO&WB_BOLIXIN:b/g=0x%x\n",gc5005w_otp_info.bg_gain);
	LOG_INF("GC5005W_OTP_INFO&WB_BOLIXIN:golden r/g=0x%x\n",gc5005w_otp_info.golden_rg);
	LOG_INF("GC5005W_OTP_INFO&WB_BOLIXIN:golden b/g=0x%x\n",gc5005w_otp_info.golden_bg);
	LOG_INF("GC5005W_OTP_INFO&WB_BOLIXIN:checksum=0x%x\n",gc5005w_otp_info.checksum);
}

static void gc5005w_gcore_update_dd(void)
{
	...
	if(0x01 ==gc5005w_otp_info.dd_flag)
	{
#if defined(IMAGE_NORMAL_MIRROR)
		for(i=0; i<gc5005w_otp_info.dd_cnt; i++)
		{
			if(gc5005w_otp_info.dd_param_type[i]==0)
			{	gc5005w_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005w_otp_info.dd_param_x[i] + 1;	}
			else if(gc5005w_otp_info.dd_param_type[i]==1)
			{	gc5005w_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005w_otp_info.dd_param_x[i] - 1;	}
			else
			{	gc5005w_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005w_otp_info.dd_param_x[i];	}
		}
		...
		for(i=0; i<gc5005w_otp_info.dd_cnt; i++)
		{
			...
			{
				gc5005w_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005w_otp_info.dd_param_x[i]+1;
				gc5005w_otp_info.dd_param_y[i]= WINDOW_HEIGHT - gc5005w_otp_info.dd_param_y[i]+1;
			}
			else if(gc5005w_otp_info.dd_param_type[i]==1)
			{
				gc5005w_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005w_otp_info.dd_param_x[i]-1;
		...
		//y
		for(i=0 ; i< gc5005w_otp_info.dd_cnt-1; i++)
		{
			for(j = i+1; j < gc5005w_otp_info.dd_cnt; j++)
			{
				if(gc5005w_otp_info.dd_param_y[i] > gc5005w_otp_info.dd_param_y[j])
				{
					temp_x = gc5005w_otp_info.dd_param_x[i] ; gc5005w_otp_info.dd_param_x[i] = gc5005w_otp_info.dd_param_x[j] ;  gc5005w_otp_info.dd_param_x[j] = temp_x;
					temp_y = gc5005w_otp_info.dd_param_y[i] ; gc5005w_otp_info.dd_param_y[i] = gc5005w_otp_info.dd_param_y[j] ;  gc5005w_otp_info.dd_param_y[j] = temp_y;
					temp_type = gc5005w_otp_info.dd_param_type[i] ; gc5005w_otp_info.dd_param_type[i] = gc5005w_otp_info.dd_param_type[j]; gc5005w_otp_info.dd_param_type[j]= temp_type;
				}
			}
		...
		for(i=0; i<gc5005w_otp_info.dd_cnt; i++)
		{
			if(gc5005w_otp_info.dd_param_y[i]==gc5005w_otp_info.dd_param_y[i+1])
			{
				s=i++;
				while((gc5005w_otp_info.dd_param_y[s] == gc5005w_otp_info.dd_param_y[i+1])&&(i<gc5005w_otp_info.dd_cnt-1))
					i++;
				e=i;
				...
				for(n=s; n<e; n++)
				{
					for(m=n+1; m<e+1; m++)
					{
						if(gc5005w_otp_info.dd_param_x[n] > gc5005w_otp_info.dd_param_x[m])
						{
							temp_x = gc5005w_otp_info.dd_param_x[n] ; gc5005w_otp_info.dd_param_x[n] = gc5005w_otp_info.dd_param_x[m] ;  gc5005w_otp_info.dd_param_x[m] = temp_x;
							temp_y = gc5005w_otp_info.dd_param_y[n] ; gc5005w_otp_info.dd_param_y[n] = gc5005w_otp_info.dd_param_y[m] ;  gc5005w_otp_info.dd_param_y[m] = temp_y;
							temp_type = gc5005w_otp_info.dd_param_type[n] ; gc5005w_otp_info.dd_param_type[n] = gc5005w_otp_info.dd_param_type[m]; gc5005w_otp_info.dd_param_type[m]= temp_type;
						}
					}
				}
		...
		for(i=0; i<gc5005w_otp_info.dd_cnt; i++)
		{
			temp_val0 = gc5005w_otp_info.dd_param_x[i]& 0x00ff;
			temp_val1 = ((gc5005w_otp_info.dd_param_y[i]<<4)& 0x00f0) + ((gc5005w_otp_info.dd_param_x[i]>>8)&0X000f);
			temp_val2 = (gc5005w_otp_info.dd_param_y[i]>>4) & 0xff;
			write_cmos_sensor(0xaa,i);
			write_cmos_sensor(0xac,temp_val0);
			write_cmos_sensor(0xac,temp_val1);
			write_cmos_sensor(0xac,temp_val2);
			write_cmos_sensor(0xac,gc5005w_otp_info.dd_param_type[i]);
			LOG_INF("GC5005W_OTP_GC val0 = 0x%x , val1 = 0x%x , val2 = 0x%x \n",temp_val0,temp_val1,temp_val2);
			LOG_INF("GC5005W_OTP_GC x = %d , y = %d \n",((temp_val1&0x0f)<<8) + temp_val0,(temp_val2<<4) + ((temp_val1&0xf0)>>4));
		}
...
static void gc5005w_gcore_update_awb(void)
{
	...
	if(0x01==gc5005w_otp_info.wb_flag)
	{
		if((0!=gc5005w_otp_info.golden_rg)&&(0!=gc5005w_otp_info.golden_bg))
		{
			rg_typical=gc5005w_otp_info.golden_rg;
			bg_typical=gc5005w_otp_info.golden_bg;
		}
		else
		{
		...
		LOG_INF("GC5005W_OTP_UPDATE_AWB:rg_typical = 0x%x , bg_typical = 0x%x\n",rg_typical,bg_typical);

		if((0==gc5005w_otp_info.rg_gain)||(0==gc5005w_otp_info.bg_gain))
		{
			gc5005w_otp_info.rg_gain = rg_typical;
			gc5005w_otp_info.bg_gain = bg_typical;
		}

		r_gain_current = 256 * rg_typical/gc5005w_otp_info.rg_gain;
		b_gain_current = 256 * bg_typical/gc5005w_otp_info.bg_gain;
		g_gain_current = 256;
		...
		LOG_INF("GC5005W_OTP_UPDATE_AWB:r_gain_current = 0x%x , b_gain_current = 0x%x , base_gain = 0x%x \n",r_gain_current,b_gain_current,base_gain);
		r_gain = 0x80 * r_gain_current / base_gain;
		g_gain = 0x80 * g_gain_current / base_gain;
		b_gain = 0x80 * b_gain_current / base_gain;
		LOG_INF("GC5005W_OTP_UPDATE_AWB:r_gain = 0x%x , g_gain = 0x%x , b_gain = 0x%x \n",r_gain,g_gain,b_gain);
		/*TODO*/
		write_cmos_sensor(0xfe,0x00);
...
static void gc5005w_gcore_update_otp(void)
{
	gc5005w_gcore_update_dd();
	gc5005w_gcore_update_awb();
}


static void gc5005w_gcore_enable_otp(otp_state state)
{
	...
	if(state)
	{
		...
		LOG_INF("GC5005W_OTP: Enable OTP!\n");
	}
	else
	{
		otp_en = otp_en & 0x7f;
		...
		LOG_INF("GC5005W_OTP: Disable OTP!\n");
	}

}

static void gc5005w_gcore_identify_otp(void)
{	
	...
	gc5005w_gcore_enable_otp(otp_open);
	gc5005w_gcore_read_otp_info();
	gc5005w_gcore_enable_otp(otp_close);
}
#if 1
static void gc5005w_gcore_read_otp_moduleid_info(void)
{
	kal_uint8 flag_group;
	kal_uint8 index;
	kal_uint8 start_add;
...
static void gc5005w_gcore_identify_moduleid_otp(void)
{
	write_cmos_sensor(0xfe, 0x00); //reset related
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00); //modify 1117
	...
	gc5005w_gcore_enable_otp(otp_open);
	gc5005w_gcore_read_otp_moduleid_info();
	gc5005w_gcore_enable_otp(otp_close);
}
#endif
	...
	if(shutter <= 500)
  	{
		write_cmos_sensor(0xfe, 0x00);
#ifdef GC5005W_MIPI_USE_OTP
		write_cmos_sensor(0x32, gc5005w_otp_info.Val32);
#else
...
{	
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		write_cmos_sensor(0xfe, 0x00);          
#ifdef GC5005W_MIPI_USE_OTP
		write_cmos_sensor(0x21, gc5005w_otp_info.Val21);
		write_cmos_sensor(0x29, gc5005w_otp_info.Val29);
#else
		...
		LOG_INF("GC5005WMIPI analogic gain 1x, GC5005WMIPI add pregain = %d\n",temp);
	}
	...
	{                                    
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005WMIPI analogic gain 1.469x , GC5005WMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005WMIPI analogic gain 2.165x , GC5005WMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005WMIPI analogic gain 3.105x , GC5005WMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005WMIPI analogic gain 3.632x , GC5005WMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005WMIPI analogic gain 4.750x , GC5005WMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005WMIPI analogic gain 8.569x , GC5005WMIPI add pregain = %d\n",temp);
	}
	else
	{                                                                                                                                                                                  
		write_cmos_sensor(0xfe, 0x00);                                                         
		...
		write_cmos_sensor(0xb1, temp>>6);                                                       
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);                                                
		LOG_INF("GC5005WMIPI analogic gain 11.69x , GC5005WMIPI add pregain = %d\n",temp);
	}                                                                                         
...
#ifdef GC5005W_MIPI_USE_OTP
	write_cmos_sensor(0xd1, gc5005w_otp_info.Vald1);
#else
...
#ifdef GC5005W_MIPI_USE_OTP
				gc5005w_gcore_identify_moduleid_otp();
				LOG_INF("[gc5005] sangong gc5005w_otp_info.module_id:0x%x\n",gc5005w_otp_info.module_id);
				if(gc5005w_otp_info.module_id != 0xe)
				{
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
#endif
			      hardwareinfo_set_prop(HARDWARE_BACK_CAM_ID, "bolixin");
                LOG_INF("lifeng i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("lifeng Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC5005WDuringTestPattern = KAL_FALSE; 
...
UINT32 GC5005WMIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC5005WMIPI_RAW_SensorInit    */
