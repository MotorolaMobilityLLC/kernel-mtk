...
static imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0xbc25e25f,		//checksum value for Camera Auto Test
...
/*************enter otp w/r mode **********************/
		...
		printk("[hi553], seg1 0x501 flag:%x\n",flag);
...
static int read_otp_info(int group, hi553_otp_info *otp_info)
{
	...
	for(i = 0; i <= checksum_addr_info-start_addr_info;i++)
	{
		...
		if((*otp_info).checksum_info == (temp%0xFF + 1))
			{
				printk("[hi553],checksum_info otp_info success\n");
			}
		else
			{
				printk("[hi553],checksum_info otp_info fail,checksun_info %x,temp %x\n",(*otp_info).checksum_info,((temp%0xFF) + 1));
				return -1;			
			}
		return 1;
}
...
static int read_otp_awb(unsigned int group,hi553_otp_data* otp_data)
{
	...
	printk("[hi553],read_otp_awb,group:%d,start_addr %x,end_addr %x,checksum_addr %x\n",group,start_addr,end_addr,checksum_addr);
...
/*************exit otp w/r mode **********************/
		...
		printk("[hi553],otp_data: otp_data.R_Gr_Ratio:%x,otp_data.B_Gb_Ratio:%x,otp_data.Gb_Gr_Ratio:%x,checksum:%x\n",otp_data->R_Gr_Ratio,otp_data->B_Gb_Ratio,otp_data->Gb_Gr_Ratio,data[29]);
		...
		if(otp_data->checksun_data == ((temp%0xFF) + 1))
			{
				printk("[hi553],checksum_data otp_awb success\n");
			}
		else{
			printk("[hi553],checksum_data otp_awb fail,checksun_data %x,temp %x\n",otp_data->checksun_data,((temp%0xFF) + 1));
			return -1;			
			}
	return 0;
}
...
int check_otp_awb(void)
{
	...
	write_cmos_sensor_byte(0x0102,0x01);
	flag = read_cmos_sensor_byte(0x0108);
	printk("[hi553], seg2 0x535 flag:%x\n",flag);
...
static int hi553_gcore_update_awb(void)
{
	...
	int RG_golden = 316;//= 0.5 * 512 + 0.5;
	int BG_golden = 321;//= 0.6 * 512 + 0.5;
	...
	for(i=1;i<=3;i++)
	{
		...
		if(group)
		{
			printk("[hi553],check_otp_awb,i:%d,group:%d\n",i,group);
			break;
		}
		printk("[hi553],check_otp_awb,i %d, fail, again \n",i);
	}
	if(i>3 || group == 0)
	{
		printk("[hi553],[hi553-OTP]no valid OTP awb data\n");
		return -1;
	}
	...
	printk("[hi553],hi553_gcore_update_awb-> nR_gain:%x,nB_gain:%x,nG_gain;%x\n",nR_gain,nB_gain,nG_gain);
	...
	printk("[hi553],hi553_gcore_update_awb-> R_gain:%x,B_gain:%x,G_gain;%x\n",R_gain,B_gain,G_gain);
...
static void hi553_gcore_update_otp(void)
{
	...
	if(ret)
	{
		printk("[hi553],hi553_gcore_update_awb fail\n");
	}
}
...
static int otp_module_id(void)
{
    ...
    for(i=1;i<=3;i++)
    {
        ...
        if(group)
		{
			printk("[hi553],otp_module_id,i:%d,group:%d\n",i,group);
			break;
		}
		printk("[hi553],check_otp_info,i %d, fail, again \n",i);
    }
    if (i>3 || group == 0)
    {
        LOG_INF("[hi553],[hi553-OTP]no valid module_id OTP data\n");
        return -1;
    }
	ret = read_otp_info(group, &otp_info);
	if(!ret)
	{
		printk("[hi553],otp_module_id, read_otp_info fail\n");
		return -1;
	}
...
static kal_uint32 open(void)
{
	...
	{
		printk("[hi553] read otp_module_id ok is 0x02\n");
		//return ERROR_NONE;
	}
	else 
	{
		printk("[hi553] read otp_module_id fail\n");
	}
