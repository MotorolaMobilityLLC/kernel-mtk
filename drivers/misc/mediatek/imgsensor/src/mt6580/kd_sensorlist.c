...
struct platform_device camerahw_platform_device = {//Paul
	.name = "image_sensor",
	.id = 0,
	.dev = {
		...
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
		            ...
		            {
		                hardwareinfo_set_prop(HARDWARE_BACK_CAM, g_invokeSensorNameStr[i]);
		            }
		            else if(g_invokeSocketIdx[i] == DUAL_CAMERA_SUB_SENSOR)
		            {
		                hardwareinfo_set_prop(HARDWARE_FRONT_CAM, g_invokeSensorNameStr[i]);
		            }
