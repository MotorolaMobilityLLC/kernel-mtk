...
extern bool is_ofilm_camera(void);
...
#ifdef AR1335_OTP
kal_uint16 awb_R_Gr_ratio=1024;
kal_uint16 awb_B_Gr_ratio= 1024;
kal_uint16 awb_Gb_Gr_ratio= 1024;

static void AR1335_otp_read(kal_uint16 otp_type){   // here type 1100 \BB\F2\D5\DF\CA\C7\C6\E4\CB\FBtype 3700
...
static void AR1335_OTP_AUTO_LOAD_LSC(void)
{
	kal_uint16 temp;
	AR1335_otp_read(0x11);

	// enable sc
	temp = read_cmos_sensor_2(0x3780);  //
	write_cmos_sensor_2byte(0x3780,temp|0x8000);  
}
static void AR1335_otp_read_awb(void)
{
	kal_uint16 temp;
	...
	if(temp&0x0008==0x0008)
	{
		...
		if((temp ==0xFFFF)&&((0x304A&0x0040)==0x0040))  // if the flag == 0xffff&0x304A[6]==1 then read AWB
		{
			...
			Checksum1 = (type37_R3800+type37_R3802+type37_R3804+type37_R3806)%65536+1;
...
#if 1
	//// end init
	if(OTP_KEY == 1 || OTP_KEY == 3) //OTP_KEY = 1: do lsc  OTP_KEY = 2:do AWB OTP_KEY = 3:do lsc and AWB
	{
		if(OTP_KEY & 1)
		{
...
static void preview_setting(void)
{
	...
	write_cmos_sensor_2byte(0x0306, 0x7D7D); 	// PLL_MULTIPLIER 0x7878
	write_cmos_sensor_2byte(0x0308, 0x000A); 	// OP_PIX_CLK_DIV
	write_cmos_sensor_2byte(0x030A, 0x0001); 	// OP_SYS_CLK_DIV
	write_cmos_sensor_2byte(0x0112, 0x0A0A); 	// CCP_DATA_FORMAT
	write_cmos_sensor_2byte(0x3016, 0x0101); 	// ROW_SPEED
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor_2byte(0x0302, 0x0001); 	// VT_SYS_CLK_DIV
	write_cmos_sensor_2byte(0x0304, 0x0404); 	// PRE_PLL_CLK_DIV
	write_cmos_sensor_2byte(0x0306, 0x9292); 	// PLL_MULTIPLIER
	write_cmos_sensor_2byte(0x0308, 0x000A); 	// OP_PIX_CLK_DIV
	write_cmos_sensor_2byte(0x030A, 0x0001); 	// OP_SYS_CLK_DIV
	write_cmos_sensor_2byte(0x0112, 0x0A0A); 	// CCP_DATA_FORMAT
	write_cmos_sensor_2byte(0x3016, 0x0101); 	// ROW_SPEED
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			LOG_INF("ar1335 huangsh4 read sensor id =0x%x\n",*sensor_id);
			*sensor_id = return_sensor_id();
			LOG_INF("ar1335 read sensor id =0x%x\n",*sensor_id);
			*sensor_id = *sensor_id;
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				if(!is_ofilm_camera())/*can change if got OTP module*/
				{      
#ifdef CONFIG_LCT_DEVINFO_SUPPORT	
...
static kal_uint32 open(void)
{
	...
	//if ((imgsensor_info.sensor_id != (sensor_id +1 ))||(!is_ofilm_camera()))
	//return ERROR_SENSOR_CONNECT_FAIL;
