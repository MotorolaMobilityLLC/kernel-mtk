...
#define AWB_ADDR	0x00B9
#define LSC_ADDR	0x00DE
#define LSC_SIZE	0xD4
...
*
********************************************************************************/
#define GT24c32_CAM_CAL_DRVNAME "GT24c32_CAM_CAL_DRV"
#define CAM_CAL_I2C_GROUP_ID 0
/*******************************************************************************
*
********************************************************************************/
//static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(GT24c32_CAM_CAL_DRVNAME, (GT24c32_DEVICE_ID >> 1))};
...
static int selective_read_region(u32 addr, unsigned char* data,u16 i2c_id,u32 size)
{
  //  u32 page = addr/PAGE_SIZE; /* size of page was 256 */
    ...
    while(size_to_read>0) {
		if(selective_read_byte(addr,(u8*)buff,GT24c32_DEVICE_ID)){
			addr+=1;
			buff+=1;
			size_to_read-=1;
    		ret+=1;
...
static long GT24c32_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
    CAM_CALDB("[CAMERA SENSOR] GT24c32_Ioctl_Compat,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );


    if (!filp->f_op || !filp->f_op->unlocked_ioctl)
        return -ENOTTY;
CAM_CALDB("GT24c32_Ioctl_Compat compat cmd = %ud , cmd = %lu,cmd =%lu\n",cmd,COMPAT_CAM_CALIOC_G_READ,CAM_CALIOC_G_READ);
    switch (cmd) {
    ...
    case COMPAT_CAM_CALIOC_G_READ:
    {
		...
		CAM_CALDB("GT24c32_Ioctl_Compat 222222\n");
    ...
    default:
		CAM_CALDB("GT24c32_Ioctl_Compat 1111111\n");
        return -ENOIOCTLCMD;
    }
}
...
static bool selective_read_eeprom(unsigned short addr, unsigned char * data)
{
	...
	if(iReadRegI2C(pu_send_cmd, 2, (u8*)data, 1, GT24c32_DEVICE_ID)<0)
		return false;
    return true;
}
#if 0
...
//end
#endif
static int ReadAR1335AWBData(unsigned short ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	int i = 0;
	int addr = ui4_offset;
	u8 AWBdatabuf[37];

	CAM_CALDB("ReadAR1335AWBData\n");
...
#if 0
static int ReadAR1335AFData(unsigned short ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	int i = 0;
	int addr = ui4_offset;
	u8 AFdatabuf[7];

	CAM_CALDB("ReadAR1335AFData\n");
	...
	if( AFdatabuf[0] != 0x1)
	{
		...
		CAM_CALDB("AR1335_OTP AF data invalid!\n");
	}
	for(i = 3; i < 7; i++)
	{
...
static int iReadData(unsigned short ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	...
	for(i = 0; i < ui4_length; i++) {
		...
		CAM_CALDB("huangsh4 read_eeprom 0x%0x %d\n",addr, pinputdata[i]);
		addr++;
	}	
...
#endif
{
    ...
    else
    {
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                CAM_CALDB("[GT24c32_CAL] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:
            CAM_CALDB("[GT24c32_CAL] Write CMD \n");
#ifdef CAM_CALGETDLT_DEBUG
        ...
        case CAM_CALIOC_G_READ:
            CAM_CALDB("[GT24c32_CAL] Read CMD \n");
#ifdef CAM_CALGETDLT_DEBUG
            do_gettimeofday(&ktv1);
#endif
            ...
            {
            	i4RetValue = ReadAR1335LSCData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pu1Params);
            }   
            ...
            CAM_CALDB("[GT24c32_CAL] After read Working buffer data  0x%x \n", *pu1Params);
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        CAM_CALDB("[GT24c32_CAL] to user length %d \n", ptempbuf->u4Length);
        CAM_CALDB("[GT24c32_CAL] to user  Working buffer address 0x%p \n", pu1Params);
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pu1Params , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pu1Params);
            CAM_CALDB("[GT24c32_CAL] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
{
    CAM_CALDB("[GT24c32_CAL] CAM_CAL_Open\n");
    spin_lock(&g_CAM_CALLock);
    if(g_u4Opened)
    {
        spin_unlock(&g_CAM_CALLock);
		CAM_CALDB("[GT24c32_CAL] Opened, return -EBUSY\n");
        return -EBUSY;
    }
    else
    {
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = GT24c32_Ioctl_Compat,
#endif
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    if( alloc_chrdev_region(&g_CAM_CALdevno, 0, 1,GT24c32_CAM_CAL_DRVNAME) )
    {
        CAM_CALDB("[GT24c32_CAL] Allocate device no failed\n");

        return -EAGAIN;
    }
#else
    if( register_chrdev_region(  g_CAM_CALdevno , 1 , GT24c32_CAM_CAL_DRVNAME) )
    {
        CAM_CALDB("[GT24c32_CAL] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        CAM_CALDB("[GT24c32_CAL] Allocate mem for kobject failed\n");
    ...
    {
        CAM_CALDB("[GT24c32_CAL] Attatch file operation failed\n");
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "GT24c32_CAM_CALdrv");
    if (IS_ERR(CAM_CAL_class)) {
        int ret = PTR_ERR(CAM_CAL_class);
        CAM_CALDB("Unable to create class, err = %d\n", ret);
        return ret;
    }
    CAM_CAL_device = device_create(CAM_CAL_class, NULL, g_CAM_CALdevno, NULL, GT24c32_CAM_CAL_DRVNAME);
...
static const struct i2c_device_id CAM_CAL_i2c_id[] = {{GT24c32_CAM_CAL_DRVNAME,0},{}};

 

static const struct of_device_id GT24c32_of_match[] = {
...
static struct i2c_driver CAM_CAL_i2c_driver = {
    .driver = {
		 .name = GT24c32_CAM_CAL_DRVNAME,

		 .of_match_table = GT24c32_of_match,
...
#ifndef CAM_CAL_ICS_REVISION
static int CAM_CAL_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info) {
    strcpy(info->type, GT24c32_CAM_CAL_DRVNAME);
    return 0;
}
#endif
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {
int i4RetValue = 0;
    CAM_CALDB("[GT24c32_CAL] Attach I2C \n");
//    spin_lock_init(&g_CAM_CALLock);

    //get sensor i2c client
    spin_lock(&g_CAM_CALLock); //for SMP
    g_pstI2Cclient = client;
    g_pstI2Cclient->addr = GT24c32_DEVICE_ID>>1;
    ...
    if(i4RetValue){
        CAM_CALDB("[GT24c32_CAL] register char device failed!\n");
        return i4RetValue;
    }


    CAM_CALDB("[GT24c32_CAL] Attached!! \n");
    return 0;
}
...
static int __init CAM_CAL_i2C_init(void)
{
   // i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        CAM_CALDB("failed to register GT24c32_CAL driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        CAM_CALDB("failed to register GT24c32_CAL driver, 2nd time\n");
        return -ENODEV;
    }
