...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	if (PwrType == PDN) {
			...
			if (mt_set_gpio_mode
				(pinSet[PinIdx][IDX_PS_CMPDN],
				 pinSet[PinIdx][IDX_PS_CMPDN + IDX_PS_MODE])) {
				PK_DBG("[CAMERA SENSOR] set gpio mode failed!!\n");
			}
			if (mt_set_gpio_dir(pinSet[PinIdx][IDX_PS_CMPDN], GPIO_DIR_OUT)) {
			...
			if (Val == Vol_High) {
				...
				if (mt_set_gpio_out
					(pinSet[PinIdx][IDX_PS_CMPDN],
					 pinSet[PinIdx][IDX_PS_CMPDN + IDX_PS_OFF])) {
					PK_DBG("[CAMERA SENSOR] set gpio failed!!\n");
				}
			}
		} else if (PwrType == RST) {
			...
			if (PinIdx == 0) {
				if (mt_set_gpio_mode
					(pinSet[PinIdx][IDX_PS_CMRST],
					 pinSet[PinIdx][IDX_PS_CMRST + IDX_PS_MODE])) {
					PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (CMRST)\n");
				}
				if (mt_set_gpio_dir(pinSet[PinIdx][IDX_PS_CMRST], GPIO_DIR_OUT)) {
					PK_DBG("[CAMERA SENSOR] set gpio dir failed!! (CMRST)\n");
				}
				if (mt_set_gpio_out
					(pinSet[PinIdx][IDX_PS_CMRST],
					 pinSet[PinIdx][IDX_PS_CMRST + IDX_PS_OFF])) {
					PK_DBG("[CAMERA SENSOR] set gpio failed!! (CMRST)\n");
				}
				if (Val == Vol_High) {
					if (mt_set_gpio_out
						(pinSet[PinIdx][IDX_PS_CMRST],
						 pinSet[PinIdx][IDX_PS_CMRST + IDX_PS_ON])) {
