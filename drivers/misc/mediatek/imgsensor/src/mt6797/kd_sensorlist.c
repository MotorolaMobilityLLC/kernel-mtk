	...
	if (1)
	{
		...
		if(node){
			...
			if (name == NULL){
				...
				sensor_device->of_node = of_find_compatible_node(NULL,NULL,"mediatek,regulator_supply");
				/* 若你需要sub也定義的話，需要自己加上
				if (regVCAMA == NULL) {
			    ...
			    */
			    if (regVCAMA == NULL) {
			 	    regVCAMA = regulator_get(sensor_device, "MAIN_CAMERA_POWER_A");
			    }
			    if (regVCAMD == NULL) {
			 	    regVCAMD = regulator_get(sensor_device, "MAIN_CAMERA_POWER_D");
			    }
			    if (regVCAMIO == NULL) {
			 	    regVCAMIO = regulator_get(sensor_device, "MAIN_CAMERA_POWER_IO");
			    }
			    if (regVCAMAF == NULL) {
			 	    regVCAMAF = regulator_get(sensor_device, "MAIN_CAMERA_POWER_AF");
			    }
