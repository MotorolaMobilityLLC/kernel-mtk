...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX258_SENSOR_ID,	/* IMX258MIPI_SENSOR_ID,
	sensor_id = 0x2680 record sensor id defined in Kd_imgsensor.h */

	.checksum_value = 0xdb783615,	/* checksum value for Camera Auto Test£¬last value is 0x16d8abef */

	.pre = {
		/* @20151214 refer to mode "B3" */
		.pclk = 259200000,	/* record different mode's pclk */
		.linelength = 5352,	/* record different mode's linelength */
		.framelength = 1612,	/* record different mode's framelength */
...
static SET_PD_BLOCK_INFO_T imgsensor_pd_info_Binning = {
	...
	/* {{30,29},{46,29},{30,31},{46,31},{37,52},{53,52},{37,54},{53,54}},//{{27,30},{43,30},{34,49},{50,49}}, */
	.i4PosR = {{29, 36}, {45, 36}, {38, 47}, {54, 47} },
	/* {{29,36},{45,36},{29,38},{45,38},{38,45},{54,45},{38,47},{54,47}},//{{26,33},{42,33},{35,46},{51,46}}, */
};
	...
	if (write_cmos_sensor(0x0A02, 0x0F) == 0) {
		...
		while (retry--) {
			if (read_cmos_sensor(0x0A01) == 0x01) {
				...
				LOG_INF("imx258 type = 0x%x(0x00=HDR,0x10=binning,0x30=No PDAF)",
					imx258_type);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x4E21, 0x14);	/* @20151216 referred to LG and Sony */
...
{
	/* @20151221 referred to LG B3 Mode */
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			if ((imx258_type == IMX258_HDR_TYPE) || (imx258_type == IMX258_NOPDAF_TYPE))
				...
				memcpy((void *)wininfo, (void *)&imgsensor_winsize_info_Binning[1],
				       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			if ((imx258_type == IMX258_HDR_TYPE) || (imx258_type == IMX258_NOPDAF_TYPE))
				...
				memcpy((void *)wininfo, (void *)&imgsensor_winsize_info_Binning[2],
				       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
			if ((imx258_type == IMX258_HDR_TYPE) || (imx258_type == IMX258_NOPDAF_TYPE))
				...
				memcpy((void *)wininfo, (void *)&imgsensor_winsize_info_Binning[3],
				       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			if ((imx258_type == IMX258_HDR_TYPE) || (imx258_type == IMX258_NOPDAF_TYPE))
				...
				memcpy((void *)wininfo, (void *)&imgsensor_winsize_info_Binning[4],
				       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		default:
			if ((imx258_type == IMX258_HDR_TYPE) || (imx258_type == IMX258_NOPDAF_TYPE))
				...
				memcpy((void *)wininfo, (void *)&imgsensor_winsize_info_Binning[0],
				       sizeof(SENSOR_WINSIZE_INFO_STRUCT));
			break;
		}
		break;
