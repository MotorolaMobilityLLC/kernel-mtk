...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV64B40_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{ 9248, 6944,   0, 0,  9248, 6944, 4624, 3472,   0,     0, 4624, 3472,     0, 0, 4624, 3472}, // custom2 as 120fps
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[4] = {
	/* Preview mode setting 1136(pxiel)*860*/
	{
		...
		0x01, 0x02b, 0x058c, 0x035c, 0x00, 0x00, 0x0000, 0x0000
	},
	...
	/* Video mode setting 496(pxiel)*1496 */
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
		0x00, 0x2b, 0x1210, 0x0D90, 0x00, 0x00, 0x0000, 0x0000,
		0x01, 0x02b, 0x058c, 0x035c, 0x00, 0x0000, 0x0000
	},
...
#if MULTI_WRITE
#if 0 /*for debug*/
	printk("[ov64b40-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d start", len);
	for (i = 0; i < len/2; i++)
		LOG_INF("[ov64b40-debug]readback addr 0x%04x, 0x%02x,\n",
			para[2*i], read_cmos_sensor(para[2*i]) & 0xFF);
	printk("[ov64b40-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d end", len);
#endif
#endif
	return 0;
}
...
static void sensor_init(void)
{
	...
	//mdelay(10);
	table_write_cmos_sensor(ov64b40_init_setting,
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(ov64b40_hs_video_setting,
...
static void custom2_setting(void)
{
	...
	table_write_cmos_sensor(ov64b40_custom2_setting,
			sizeof(ov64b40_custom2_setting) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	table_write_cmos_sensor(ov64b40_custom3_setting,
			sizeof(ov64b40_custom3_setting) / sizeof(kal_uint16));
}
...
static void custom4_setting(void)
{
	...
	table_write_cmos_sensor(ov64b40_custom4_setting,
			sizeof(ov64b40_custom4_setting) / sizeof(kal_uint16));
}


#define OV64B40_EEPROM_SLAVE_ADDR 0xA0
#define OV64B40_SENSOR_IIC_SLAVE_ADDR 0x44
#define OV64B40_EEPROM_SIZE  0x1D1B
#define EEPROM_DATA_PATH "/data/vendor/camera_dump/ov64b10_eeprom_data.bin"
#define OV64B40_EEPROM_CRC_AF_CAL_SIZE 24
#define OV64B40_EEPROM_CRC_AWB_CAL_SIZE 43
...
static void ov64b40_read_data_from_eeprom(kal_uint8 slave, kal_uint32 start_add, uint32_t size)
{
	int i = 0;
	spin_lock(&imgsensor_drv_lock);
	...
	//read eeprom data
	for (i = 0; i < size; i ++) {
		ov64b40_eeprom[i] = read_cmos_sensor(start_add);
		start_add ++;
	}

	spin_lock(&imgsensor_drv_lock);
	imgsensor.i2c_write_id = OV64B40_SENSOR_IIC_SLAVE_ADDR;
	spin_unlock(&imgsensor_drv_lock);
}

static void ov64b40_eeprom_dump_bin(const char *file_name, uint32_t size, const void *data)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
...
static calibration_status_t ov64b40_check_awb_data(void *data)
{
	struct ov64b40_eeprom_t *eeprom = (struct ov64b40_eeprom_t*)data;
	awb_t unit;
	awb_t golden;
	...
	if(!eeprom_util_check_crc16(eeprom->cie_src_1_ev,
		OV64B40_EEPROM_CRC_AWB_CAL_SIZE,
...
static calibration_status_t ov64b40_check_lsc_data_mtk(void *data)
{
	struct ov64b40_eeprom_t *eeprom = (struct ov64b40_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->lsc_data_mtk, OV64B40_EEPROM_CRC_LSC_SIZE,
...
static calibration_status_t ov64b40_check_af_data(void *data)
{
	struct ov64b40_eeprom_t *eeprom = (struct ov64b40_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->af_data, OV64B40_EEPROM_CRC_AF_CAL_SIZE,
...
static calibration_status_t ov64b40_check_pdaf_data(void *data)
{
	struct ov64b40_eeprom_t *eeprom = (struct ov64b40_eeprom_t*)data;

	if (!eeprom_util_check_crc16(eeprom->pdaf_out1_data_mtk, OV64B40_EEPROM_CRC_PDAF_OUTPUT1_SIZE,
		convert_crc(eeprom->pdaf_out1_crc16_mtk))) {
		LOG_INF("PDAF OUTPUT1 CRC Fails!");
		return CRC_FAILURE;
	}

	if (!eeprom_util_check_crc16(eeprom->pdaf_out2_data_mtk, OV64B40_EEPROM_CRC_PDAF_OUTPUT2_SIZE,
...
static void ov64b40_eeprom_format_calibration_data(void *data)
{
	...
	pdaf_status = ov64b40_check_pdaf_data(data);
	dual_status = 0;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("OV64B40 get_imgsensor_id in\n");
...
UINT32 MOT_LISBON_OV64B40_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    LOG_INF("MOT_LISBON_OV64B40_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	MOT_LISBON_OV64B40_MIPI_RAW_SensorInit	*/
