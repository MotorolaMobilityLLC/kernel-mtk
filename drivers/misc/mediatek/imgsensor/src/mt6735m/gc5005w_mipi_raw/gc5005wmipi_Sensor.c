 ...
 ****************************************************************************/
//////////////////////////////////////////////////////////////////////////////
//bug 270334,tiantian,20170621 modify by GC,20170623,make preview to binning
//////////////////////////////////////////////////////////////////////////////
...
#define RG_TYPICAL    			0xad
#define BG_TYPICAL				0xb3
...
//For GC
	...
	{
		gc5005_otp_info.Val32=0xfc;
		gc5005_otp_info.Vald1=0xa0;
		gc5005_otp_info.Val21=0x0f;
...
//INFO&WB
	...
	{
		start_add = index*8;
		flag_group = gc5005_read_otp(start_add);
		if(flag_group==0x01)
		{
			for(i=1;i<7;i++)
			{
				checksum1 += gc5005_read_otp(start_add + i);
			}
			gc5005_otp_info.module_id = gc5005_read_otp(start_add + 1) & 0xff;
			gc5005_otp_info.lens_vcmid = gc5005_read_otp(start_add + 2) & 0xff;
			gc5005_otp_info.rg_gain = gc5005_read_otp(start_add + 3) & 0xff;
			gc5005_otp_info.bg_gain = gc5005_read_otp(start_add + 4) & 0xff;
			gc5005_otp_info.golden_rg =gc5005_read_otp(start_add + 5) & 0xff;
			gc5005_otp_info.golden_bg = gc5005_read_otp(start_add + 6) & 0xff;
			gc5005_otp_info.checksum = gc5005_read_otp(start_add + 7);
			if(gc5005_otp_info.checksum == ((checksum1%0xff+1)&0xff))
			{
				LOG_INF("GC5005_OTP_INFO&WB group%d is Valid !!\n", index + 1);
				gc5005_otp_info.wb_flag = 0x01;
			break;
			}
			else
			{
				LOG_INF("GC5005_OTP_INFO&WB group%d checksum error,checksum1=0x%x !!\n", index + 1,(checksum1%0xff+1));
			...
			LOG_INF("GC5005_OTP_INFO&WB group%d is Invalid !!\n", index + 1);
			gc5005_otp_info.wb_flag = 0x00;
		}		
	}

	/*print otp information*/
	LOG_INF("GC5005_OTP_INFO&WB:module_id=0x%x\n",gc5005_otp_info.module_id);
	LOG_INF("GC5005_OTP_INFO&WB:lens_vcmid=0x%x\n",gc5005_otp_info.lens_vcmid);
	LOG_INF("GC5005_OTP_INFO&WB:r/g=0x%x\n",gc5005_otp_info.rg_gain);
	LOG_INF("GC5005_OTP_INFO&WB:b/g=0x%x\n",gc5005_otp_info.bg_gain);
	LOG_INF("GC5005_OTP_INFO&WB:golden r/g=0x%x\n",gc5005_otp_info.golden_rg);
	LOG_INF("GC5005_OTP_INFO&WB:golden b/g=0x%x\n",gc5005_otp_info.golden_bg);
	LOG_INF("GC5005_OTP_INFO&WB:checksum=0x%x\n",gc5005_otp_info.checksum);
}
...
static void gc5005_gcore_update_awb(void)
{
	...
	if(0x01==gc5005_otp_info.wb_flag)
	{
		...
		{	
			gc5005_otp_info.rg_gain = rg_typical;
			gc5005_otp_info.bg_gain = bg_typical;
		}
...
#if 1
static void gc5005_gcore_read_otp_moduleid_info(void)
{
	...
	//module id info
		for(index=0;index<3;index++)
		{
			...
			{
				gc5005_otp_info.module_id = gc5005_read_otp(start_add + 1) & 0xff;
				LOG_INF("GC5005_OTP_INFO&WB group%d is Valid !! module_id:0x%x\n", index + 1,gc5005_otp_info.module_id);
			}
			else
			{
				LOG_INF("GC5005_OTP_INFO&WB group%d is Invalid !!\n", index + 1);
...
static void gc5005_gcore_identify_moduleid_otp(void)
{
	write_cmos_sensor(0xfe, 0x00); //reset related
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00); //modify 1117
	...
	gc5005_gcore_enable_otp(otp_open);
	gc5005_gcore_read_otp_moduleid_info();
	gc5005_gcore_enable_otp(otp_close);
}
#endif
  ...
  //  kal_uint32 frame_length = 0;
	...
	shutter_temp=shutter*4;
		...
		if(shutter_temp <= 30)
  	{
		write_cmos_sensor(0xfe, 0x00);
#ifdef GC5005_MIPI_USE_OTP
		write_cmos_sensor(0x32, gc5005_otp_info.Val32);
#else
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x92, CAP_STARTX);//Don't Change Here!!!   crop_win_y1[7:0]                                     
	write_cmos_sensor(0x94, CAP_STARTY);//Don't Change Here!!!   crop_win_x1[7:0]                       
...
static void preview_setting(void)
{
	...
	/*ISP*/  
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, PV_STARTX); //crop y                                       
	write_cmos_sensor(0x94, PV_STARTY); //crop x  
...
static void capture_setting(kal_uint16 currefps)
{
	...
	/*ISP*/  
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, CAP_STARTX); //crop y                                       
	write_cmos_sensor(0x94, CAP_STARTY); //crop x  
	write_cmos_sensor(0x95, 0x07); 
	write_cmos_sensor(0x96, 0x98);
	write_cmos_sensor(0x97, 0x0a); 
	write_cmos_sensor(0x98, 0x20); 
...
#ifdef GC5005_MIPI_USE_OTP
			gc5005_gcore_identify_moduleid_otp();
			LOG_INF("[gc5005] sangong gc5005_otp_info.module_id:0x%x\n",gc5005_otp_info.module_id);
				if(gc5005_otp_info.module_id != 0x0e)
				{
                ...
                LOG_INF("[gc5005] sangong,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("[gc5005] ,sangong failed ,Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
			//if(gc5005_otp_info.module_id != 0x0e) return -1;
            sensor_id = return_sensor_id()+1;
            if (sensor_id == imgsensor_info.sensor_id) {
