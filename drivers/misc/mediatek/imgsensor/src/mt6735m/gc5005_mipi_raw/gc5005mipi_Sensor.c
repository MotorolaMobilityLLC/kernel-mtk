...
#define RG_TYPICAL    			0x0150//0x015e
#define BG_TYPICAL				0x0150//0x0157
...
//DD
	...
	for(index=0;index<3;index++)
	{
		...
		if(flag_group==0x55)
		{
			...
			gc5005_otp_info.module_id = gc5005_read_otp(start_add + 1) & 0x1f;
			gc5005_otp_info.lens_vcmid = (gc5005_read_otp(start_add + 1) & 0xE0)>>5;

			gc5005_otp_info.rg_gain = ((gc5005_read_otp(start_add + 6) & 0x03)<< 8)|gc5005_read_otp(start_add + 2);
			gc5005_otp_info.bg_gain = ((gc5005_read_otp(start_add + 6) & 0x0c)<< 6)|gc5005_read_otp(start_add + 3);

			gc5005_otp_info.golden_rg =((gc5005_read_otp(start_add + 6) & 0x30)<< 4)|gc5005_read_otp(start_add + 4);
			gc5005_otp_info.golden_bg = ((gc5005_read_otp(start_add + 6) & 0xc0)<< 2)|gc5005_read_otp(start_add + 5);
...
static void gc5005_gcore_read_otp_moduleid_info(void)
{
		...
		for(index=0;index<3;index++)
		{
			...
			{
				gc5005_otp_info.module_id = gc5005_read_otp(start_add + 1) & 0x1f;
				LOG_INF("GC5005_OTP_INFO&WB group%d is Invalid !! module_id:0x%x\n", index + 1,gc5005_otp_info.module_id);
			}
			else
			{
				LOG_INF("GC5005_OTP_INFO&WB group%d is Invalid !!\n", index + 1);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			...
			LOG_INF("[gc5005] yigong gc5005_otp_info.module_id:0x%x\n",gc5005_otp_info.module_id);
				if(gc5005_otp_info.module_id != 0x0d)
				{
...
#endif
			    hardwareinfo_set_prop(HARDWARE_BACK_CAM_ID, "xuanyang");
                LOG_INF("[gc5005] yigong, i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("[gc5005],yigong failed Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
