...
#include "s5k5e8yxmipiraw_Sensor_xy.h"

/****************************Modify following Strings for debug****************************/
#define PFX "s5k5e8yx_camera_sensor_xy"
...
kal_uint16 R_gain_xy=0,B_gain_xy=0;
kal_uint8  vendor_id_xy=0;
extern int read_s5k5e8_eeprom_mtk_fmt_xy(void);
#endif
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K5E8YX_SENSOR_ID_XY,
...
static int read_otp_s5k5e8(struct S5K5E8_otp_struct *otp_ptr)
{
	...
	if(addr != 0)
	{
	 ...
	 R_gain_xy = (otp_ptr->R_gain_h<<8)|(otp_ptr->R_gain_l) ;
	 B_gain_xy = (otp_ptr->B_gain_h<<8)|(otp_ptr->B_gain_l) ;
	 LOG_INF("wangkangmin s5k5e8 read R_G_B gain= %x,%x,%x,%x,%x,%x,%x,%x \n",otp_ptr->R_gain_h,otp_ptr->R_gain_l,otp_ptr->B_gain_h,otp_ptr->B_gain_l,otp_ptr->G_gain_h,otp_ptr->G_gain_l,R_gain_xy,B_gain_xy);
	}
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(otp_ptr1->MID == 0x39)
				{
					read_s5k5e8_eeprom_mtk_fmt_xy();
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					kfree(otp_ptr1);
					return ERROR_NONE;
				}
				else{
					LOG_INF("Read sensor is zq 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					kfree(otp_ptr1);
