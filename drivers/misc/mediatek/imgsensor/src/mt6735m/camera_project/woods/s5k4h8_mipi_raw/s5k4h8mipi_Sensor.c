...
kal_uint16 R_gain_of=0,B_gain_of=0,gd_R_gain=0,gd_B_gain=0,in_af,ma_af;
kal_uint8  vendor_id_of=0;
...
static int read_otp_s5k4h8(struct S5K4H8_otp_struct *otp_ptr)
{
    ...
    if((otp_flag & 0xc0) == 0x40) 
	{
	  ...
	  vendor_id_of = otp_ptr->MID;//vendor id group 1
	...
	else if((otp_flag & 0x30) == 0x10) 
	{
	  ...
	  vendor_id_of = otp_ptr->MID;//vendor id group 2
	...
	if(addr != 0)
	{
	 ...
	 R_gain_of = (otp_ptr->R_gain_h<<8)|(otp_ptr->R_gain_l) ;
	 B_gain_of = (otp_ptr->B_gain_h<<8)|(otp_ptr->B_gain_l) ;
	 gd_R_gain = (otp_ptr->gd_R_gain_h<<8)|(otp_ptr->gd_R_gain_l) ;
	 gd_B_gain = (otp_ptr->gd_B_gain_h<<8)|(otp_ptr->gd_B_gain_l) ;
	 LOG_INF("luminjie s5k5e8 read R_G_B gain= %x,%x,%x,%x,%x,%x,%x,%x \n",otp_ptr->R_gain_h,otp_ptr->R_gain_l,otp_ptr->B_gain_h,otp_ptr->B_gain_l,otp_ptr->G_gain_h,otp_ptr->G_gain_l,R_gain_of,B_gain_of);
	 LOG_INF("luminjie s5k5e8 read R_G_B gain= %x,%x,%x,%x,%x,%x,%x,%x \n",otp_ptr->gd_R_gain_h,otp_ptr->gd_R_gain_l,otp_ptr->gd_B_gain_h,otp_ptr->gd_B_gain_l,otp_ptr->gd_G_gain_h,otp_ptr->gd_G_gain_l,gd_R_gain,gd_B_gain);
	}
	else{
	 otp_ptr->R_gain_h = 0;
	 otp_ptr->R_gain_l = 0;
	...
	if(af_addr !=0){
		in_af = ((read_cmos_sensor_8(af_addr)&0xff)<<2) | ((read_cmos_sensor_8(af_addr+1)&0xff)>>6);
		ma_af = ((read_cmos_sensor_8(af_addr+2)&0xff)<<2) | ((read_cmos_sensor_8(af_addr+3)&0xff)>>6);
