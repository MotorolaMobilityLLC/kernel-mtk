...
static void sensor_init(void)
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
      ...
      write_cmos_sensor(0xfb,0x73);//9.29canshu 63
			write_cmos_sensor(0xfd,0x01);// yewei,wt,add,begin,20161031
			write_cmos_sensor(0x3f,0x03);// yewei,wt,add,end,20161031
			write_cmos_sensor(0xfd,0x01);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			printk("yw,enter,sp2509->get_imgsensor_id->sensor_id %d\n",*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
			    hardwareinfo_set_prop(HARDWARE_FRONT_CAM_ID, "qunhui");
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
			printk("yw,enter,sp2509->open sensor_id =0x%x\n",sensor_id);
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("gpw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
...
UINT32 SP2509MIPISensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    ...
    printk("yw,enter,SP2509MIPISensorInit");
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
