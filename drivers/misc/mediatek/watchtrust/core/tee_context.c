...
int tee_context_dump(struct tee *tee, char *buff, size_t len)
{
	...
	list_for_each(ptr_ctx, &tee->list_ctx) {
		...
		list_for_each(ptr_sess, &ctx->list_sess) {
			sess = list_entry(ptr_sess, struct tee_session, entry);

			pos += sprintf(buff + pos,
				       "[%02d.%d] sess=%p sessid=%08x\n",
				       i, j, sess, sess->sessid);
		...
		list_for_each(ptr_shm, &ctx->list_shm) {
			...
			pos += sprintf(buff + pos,
				       ...
				       i, j, shm, &shm->paddr, shm->kaddr);
			pos += sprintf(buff + pos,
				       " s=%zu(%zu)\n",
				       shm->size_req, shm->size_alloc);
			if ((len - pos) < 80) {
				pos = 0;
				goto out;
			}
...
struct tee_context *tee_context_create(struct tee *tee)
{
	...
	WD_DEV_DBG(_DEV(ctx->tee), "%s: < ctx=%p is created\n", __func__, ctx);
	return ctx;
}
...
static void _tee_context_do_release(struct kref *kref)
{
	...
	WD_DEV_DBG(_DEV(tee), "%s: < ctx=%p is destroyed\n", __func__, ctx);
}
...
void tee_context_get(struct tee_context *ctx)
{
	...
	WD_DEV_DBG(_DEV(ctx->tee), "%s: ctx=%p, kref=%d\n", __func__,
...
{
	struct tee_context *_ctx __attribute__((unused)) = ctx;
	struct tee *tee;
	...
	WD_DEV_DBG(_DEV(tee), "%s: ctx=%p, kref=%d\n", __func__,
				 ...
				 void *dest, const void *src, size_t size)
{
	...
	WD_DEV_DBG(_DEV(ctx->tee), "%s: dest %p, src %p, size %ld\n", __func__,
		dest, src, size);
