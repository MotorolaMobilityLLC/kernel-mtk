...
#define INMSG WD_DEV_DBG(_DEV_TEE, "%s: >\n", __func__)
#define OUTMSG(val) WD_DEV_DBG(_DEV_TEE, "%s: < %d\n", __func__, (int)val)
...
static int tee_copy_to_user(struct tee_context *ctx, void *to, void *from,
			    size_t size)
{
	WD_DEV_DBG(_DEV(ctx->tee),
		"%s: to %p, from %p, size %ld\n", __func__, to,
		from, size);
	if ((!to) || (!from) || (!size))
		return 0;
	if (ctx->usr_client)
			       ...
			       struct tee_cmd_io *cmd_io)
{
	...
	WD_DEV_DBG(_DEV(tee), "%s: > open a new session", __func__);

	sess->sessid = 0;
	ret = _init_tee_cmd(sess, cmd_io, &cmd);
	if (ret) {
		cmd_io->err = TEEC_ERROR_BAD_PARAMETERS;
		goto out;
	}

	if (cmd.uuid) {
		WD_DEV_DBG(_DEV(tee), "%s: UUID=%s\n", __func__,
...
out:
	...
	WD_DEV_DBG(_DEV(tee), "%s: < ret=%d, sessid=%08x", __func__, ret,
		sess->sessid);
	return ret;
}
...
int tee_session_invoke_be(struct tee_session *sess, struct tee_cmd_io *cmd_io)
{
	...
	WD_DEV_DBG(_DEV(tee), "%s: > sessid=%08x, cmd=0x%08x\n", __func__,
...
out:
	_release_tee_cmd(sess, &cmd);
	WD_DEV_DBG(_DEV(tee), "%s: < ret=%d", __func__, ret);
	return ret;
}
...
static int tee_session_close_be(struct tee_session *sess)
{
	...
	WD_DEV_DBG(_DEV(tee), "%s: > sessid=%08x", __func__, sess->sessid);
				 ...
				 struct tee_cmd_io __user *u_cmd)
{
	...
	WD_DEV_DBG(_DEV(tee), "%s: > sessid=%08x\n", __func__, sess->sessid);
			     ...
			     struct tee_cmd_io __user *u_cmd)
{
	...
	WD_DEV_DBG(sess->ctx->tee->dev, "%s: > sessid=%08x\n", __func__,
		sess->sessid);
			      ...
			      unsigned long arg)
{
	...
	WD_DEV_DBG(_DEV(tee), "%s: > cmd nr=%d\n", __func__, _IOC_NR(cmd));
						...
						struct tee_cmd_io *cmd_io)
{
	...
	if (ret || !sess->sessid || cmd_io->err) {
		dev_err(_DEV(tee),
			"%s: ERROR ret=%d (err=0x%08x, org=%d,  sessid=0x%08x)\n",
...
int tee_session_create_fd(struct tee_context *ctx, struct tee_cmd_io *cmd_io)
{
	...
	if (IS_ERR_OR_NULL(sess)) {
		ret = PTR_ERR(sess);
		WD_DEV_DBG(_DEV(tee),
			"%s: ERROR can't create the session (ret=%d, err=0x%08x, org=%d)\n",
			__func__, ret, cmd_io->err, cmd_io->origin);
		cmd_io->fd_sess = -1;
		goto out;
	}
...
out:
	WD_DEV_DBG(_DEV(tee), "%s: < ret=%d, sess=%p, fd=%d\n", __func__,
...
static bool to_memref_type(int flags, int *type)
{
			 ...
			 struct tee_cmd *cmd)
{
	...
	ret2 = tee_context_copy_from_client(ctx, &op, cmd_io->op, sizeof(op));
	if (ret2) {
	...
	for (idx = 0; idx < TEEC_CONFIG_PAYLOAD_REF_COUNT; ++idx) {
		...
		case TEEC_MEMREF_WHOLE:
			...
			WD_DEV_DBG(_DEV_TEE,
				...
				idx, type, param->c_shm[idx].buffer, offset,
				size);

			if (!to_memref_type(param->c_shm[idx].flags, &type)) {
				dev_err(_DEV(tee),
					"MEMREE type %d can not match flag %d\n",
					type, param->c_shm[idx].flags);
				goto out;
			}
			...
			if (IS_ERR_OR_NULL(param->params[idx].shm)) {
				...
				if (IS_ERR_OR_NULL(param->params[idx].shm)) {
					dev_err(_DEV_TEE, "tee shm ERROR %d\n",
						__LINE__);
					goto out;
				}
			}

			WD_DEV_DBG(_DEV_TEE, "< %s %d %p:%zd\n", __func__, idx,
				(void *)param->params[idx].shm->paddr,
				param->params[idx].shm->size_req);
			break;

		default:
			dev_err(_DEV_TEE, "%s: %s...\n", __func__,
				"unknown param type");
			goto out;
		}
	...
	if (cmd_io->uuid != NULL) {
		WD_DEV_DBG(_DEV_TEE, "%s: copy UUID value...\n", __func__);
		cmd->uuid = tee_context_alloc_shm_tmp(sess->ctx,
						      sizeof(*cmd_io->uuid),
						      cmd_io->uuid,
						      TEEC_MEM_INPUT);
		if (IS_ERR_OR_NULL(cmd->uuid)) {
			dev_err(_DEV_TEE, "tee uuid ERROR %d\n", __LINE__);
			ret = -EINVAL;
			goto out;
		}
	}
				   ...
				   struct tee_cmd *cmd)
{
	...
	WD_DEV_DBG(_DEV_TEE, "%s: returned err=0x%08x (origin=%d)\n", __func__,
		cmd->err, cmd->origin);
	...
	for (idx = 0; idx < TEEC_CONFIG_PAYLOAD_REF_COUNT; ++idx) {
		...
		WD_DEV_DBG(_DEV_TEE,
			"%s: id %d type %d\n", __func__, idx, type);
		if (tee_session_is_supported_type(sess, type) == false) {
			dev_err(_DEV_TEE, "%s: %s\n", __func__,
				"unknown para type");
			return;
		}
		...
		case TEEC_VALUE_OUTPUT:
		case TEEC_VALUE_INOUT:
			WD_DEV_DBG(_DEV_TEE, "%s: a=%08x, b=%08x\n",
				__func__,
		...
		case TEEC_MEMREF_TEMP_INOUT:
			...
			WD_DEV_DBG(_DEV_TEE, "%s: tmpref %p\n", __func__,
			...
			if (size_new > op.params[idx].tmpref.size)
				dev_err(_DEV_TEE,
				 ...
				 (ctx,
				  ...
				  cmd->param.params[idx].shm->kaddr, size_new))
				dev_err(_DEV_TEE,
					"%s:%d: can't update %d result to user\n",
					__func__, __LINE__, idx);
		...
		case TEEC_MEMREF_WHOLE:
			...
			WD_DEV_DBG(_DEV_TEE,
				"%s: offset %d, size %ld, newsize %ld, parent->size %ld\n",
				__func__, offset, size, size_new, parent->size);
			WD_DEV_DBG(_DEV_TEE,
				"%s: mem type %d, shm->flags %08x\n",
				__func__, type,
				cmd->param.params[idx].shm->flags);
			/*
...
static void _release_tee_cmd(struct tee_session *sess, struct tee_cmd *cmd)
{
	...
	WD_DEV_DBG(_DEV_TEE, "%s: > free the temporary objects...\n", __func__);
