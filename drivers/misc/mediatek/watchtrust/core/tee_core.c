...
int tee_get(struct tee *tee)
{
	...
	if (atomic_inc_return(&tee->refcount) == 1) {
		WD_DEV_DBG(_DEV(tee), "%s: refcount=1 call %s::start()...\n",
			__func__, tee->name);
		get_device(tee->dev);
		if (tee->ops->start)
	...
	if (ret) {
		...
		WD_DEV_DBG(_DEV(tee), "%s: refcount=%d\n", __func__, count);
...
int tee_put(struct tee *tee)
{
	...
	if (atomic_dec_and_test(&tee->refcount)) {
		WD_DEV_DBG(_DEV(tee), "%s: refcount=0 call %s::stop()...\n",
			__func__, tee->name);
		if (tee->ops->stop)
...
static int tee_supp_open(struct tee *tee)
{
	...
	WD_DEV_DBG(_DEV(tee), "%s: appclient=\"%s\" pid=%d\n", __func__,
	...
	if (strncmp(_tee_supp_app_name, current->comm,
		...
		if (atomic_add_return(1, &tee->rpc->used) > 1) {
			ret = -EBUSY;
			dev_err(tee->dev,
				"%s: ERROR Only one Supplicant is allowed\n",
				__func__);
			atomic_sub(1, &tee->rpc->used);
		}
		WD_DEV_DBG(_DEV(tee), "%s: tee->rpc->used %p\n", __func__,
			&tee->rpc->used);
	}
...
static void tee_supp_release(struct tee *tee)
{
	...
	if ((atomic_read(&tee->rpc->used) == 1) &&
	    (strncmp(_tee_supp_app_name, current->comm,
		     strlen(_tee_supp_app_name)) == 0))
		atomic_sub(1, &tee->rpc->used);
	WD_DEV_DBG(_DEV(tee), "%s > : rpc->used %p sub 1\n", __func__,
		&tee->rpc->used);
}
...
exit:
	WD_DEV_DBG(_DEV(tee), "%s: < ret=%d, sessfd=%d\n", __func__, ret,
		k_cmd.fd_sess);
	return ret;
}
...
exit:
	WD_DEV_DBG(_DEV(tee), "%s: < ret=%d, shmfd=%d\n", __func__, ret,
		k_shm.fd_shm);
	return ret;
}
...
static long tee_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	WD_DEV_DBG(_DEV(ctx->tee), "%s: > cmd nr=%d\n", __func__, _IOC_NR(cmd));
	...
	WD_DEV_DBG(_DEV(ctx->tee), "%s: < ret=%d\n", __func__, ret);
