				  ...
				  uint32_t id, void *data, size_t datalen)
{
	...
	size_t size;
	struct task_struct *task __attribute__((unused)) = current;

	WD_DEV_DBG(tee->dev, "> tgid:[%d] id:[0x%08x]\n", task->tgid, id);
		      ...
		      size_t length, loff_t *offset)
{
	...
	tee = ctx->tee;
	WD_DEV_DBG(tee->dev, "%s > ctx %p\n", __func__, ctx);

	rpc = tee->rpc;

	if (atomic_read(&rpc->used) == 0) {
		dev_err(tee->dev,
			"%s: rpc->used %p, ERROR Supplicant application NOT ready\n",
			__func__, &rpc->used);
		ret = -EPERM;
		goto out;
	}
...
int tee_supp_init(struct tee *tee)
{
	...
	WD_DEV_DBG(tee->dev, "%s rpc->used %p is set zero\n", __func__,
		&rpc->used);
	tee->rpc = rpc;
	return 0;
}
