				     ...
				     void *shm_vaddr, unsigned long shm_paddr)
{
	...
	WD_DEV_DBG(dev, "> vaddr=0x%p, paddr=0x%p, size=%zuKiB\n",
	...
	WD_DEV_DBG(dev, "wdtee cache line size %d\n", cl_size);
...
#endif
	WD_DEV_DBG(dev, "< poolH(0x%p) chunkH=0x%p\n",
...
void tee_shm_pool_destroy(struct device *dev, struct shm_pool *pool)
{
	...
	WD_DEV_DBG(dev, "free chunkH=0x%p\n", (void *)chunk);
	_KFREE(chunk);
	_KFREE(pool);
...
/**
 * \brief check the pool empty size
 *
 * \param
 *
 * \return size
 ...
 *
 */
size_t tee_shm_pool_check(struct device *dev, struct shm_pool *pool)
{
	size_t size = 0;
	...
	WD_DEV_DBG(dev, "> %s pool totle size %zu, used %zu\n", __func__,
		pool->size, pool->used);

	size = pool->size - pool->used;

	WD_DEV_DBG(dev, "< %s pool remainder %zu\n", __func__, size);
				 ...
				 size_t size, size_t alignment)
{
	...
	WD_DEV_DBG(dev, "> poolH(%p:%p:%x) size=0x%zx align=0x%zx\n",
	...
	if (ALIGN(size, cl_size) < size)
		goto failed_out;

	if (alignment == 0)
		alignment = 1;

	size = ALIGN(size, cl_size);

	alignment = ALIGN(alignment, cl_size);
		      ...
		      unsigned long paddr, size_t *size)
{
	...
	WD_DEV_DBG(dev, "> Try to free ... poolH(0x%p) paddr=0x%p\n",
