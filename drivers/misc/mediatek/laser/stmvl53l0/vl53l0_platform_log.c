...
int32_t VL53L0_trace_config(char *filename, uint32_t modules, uint32_t level, uint32_t functions)
{
#if 0
    int STATUS = 0;
...
void trace_print_module_function(uint32_t module, uint32_t level, uint32_t function, const char *format, ...)
{
    ...
    {
        va_list arg_list;
        char message[VL53L0_MAX_STRING_LENGTH_PLT];

        va_start(arg_list, format);
        vsnprintf(message, VL53L0_MAX_STRING_LENGTH_PLT, format, arg_list);
        va_end(arg_list);
