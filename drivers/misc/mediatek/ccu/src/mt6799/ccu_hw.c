...
static ccu_id_t curr_algo;
static int32_t g_ccu_sensor_current_fps = -1;

uint8_t *pI2CCmdBuf[MAX_I2CBUF_NUM];
uint8_t *pLogBuf[MAX_LOG_BUF_NUM];
uint8_t *pLogBuf_base[MAX_LOG_BUF_NUM];

uint32_t I2CCmdBuf_mva[MAX_I2CBUF_NUM];
uint32_t MailBox_mva[MAX_MAILBOX_NUM];
uint32_t LogBuf_mva[MAX_LOG_BUF_NUM];
...
#define _KCCU_MAILBOX_USE_DMEM_

/*#define _KCCU_LOG_USE_KERNEL_BUFFER_*/

/*#define _ccu_mem_use_get_free_pages_*/
#define _ccu_mem_use_dma_api_
#if defined(_ccu_mem_use_dma_api_)
static struct sg_table table;
#endif

dma_addr_t dma_handle_mbt;	/* mbt:mailbox transmitter */
dma_addr_t dma_handle_mbr;	/* mbt:mailbox receiver */
dma_addr_t dma_handle_log[MAX_LOG_BUF_NUM];
...
static inline int wait_command(void)
{
	return (wait_event_interruptible_timeout(cmd_wait, cmd_done,
						    msecs_to_jiffies(10 * 1000)) > 0) ? 0 : -ETIMEDOUT;
}
...
int ccu_init_hw(ccu_device_t *device)
{
	...
	/*ccu_request_emulator_irq(device->irq_num, ccu_isr_handler);*/
#else
	...
	LOG_DBG("create working buffers(0x)\n");
	/*working buffers*/
	pI2CCmdBuf[0] = kzalloc(SIZE_1MB, GFP_KERNEL);

	LOG_DBG("allocate for mailbox");

#if defined(_KCCU_MAILBOX_USE_DMEM_)
#else
	pMailBox[MAILBOX_SEND] =
	    dma_zalloc_coherent(device->dev, sizeof(ccu_mailbox_t), &dma_handle_mbt,
				GFP_KERNEL | GFP_DMA);
	pMailBox[MAILBOX_GET] =
	    dma_zalloc_coherent(device->dev, sizeof(ccu_mailbox_t), &dma_handle_mbr,
				GFP_KERNEL | GFP_DMA);
	LOG_DBG("pMailBox[MAILBOX_SEND]: (%p), (%p)\n", &(pMailBox[MAILBOX_SEND]),
		(void *)(dma_handle_mbt));
	LOG_DBG("pMailBox[MAILBOX_GET]:  (%p), (%p)\n", &(pMailBox[MAILBOX_GET]),
		(void *)(dma_handle_mbr));
#endif

#if defined(_ccu_mem_use_get_free_pages_)
	/*
	* Allocate 2^order pages and return a pointer to the first page's logical address
	* (2^8)*4*1024  = 1MB
	*/
	pLogBuf[0] = (uint8_t *) __get_free_pages(GFP_KERNEL, SIZE_1MB_PWR2PAGE);
	if (!pLogBuf[0]) {
		LOG_ERR("insufficient memory: you must handle this error!\n");
		goto out;
	}
	pLogBuf[1] = (uint8_t *) __get_free_pages(GFP_KERNEL, SIZE_1MB_PWR2PAGE);
	if (!pLogBuf[1]) {
		LOG_ERR("insufficient memory: you must handle this error!\n");
		goto out;
	}
	pLogBuf[2] = (uint8_t *) __get_free_pages(GFP_KERNEL, SIZE_1MB_PWR2PAGE);
	if (!pLogBuf[2]) {
		LOG_ERR("insufficient memory: you must handle this error!\n");
		goto out;
	}
#elif defined(_ccu_mem_use_dma_api_)
	pLogBuf_base[0] =
	    dma_zalloc_coherent(device->dev, SIZE_1MB + PAGE_SIZE, &dma_handle_log[0],
				GFP_KERNEL | GFP_DMA);
	pLogBuf[0] = (uint8_t *) (((unsigned long)pLogBuf_base[0] + PAGE_SIZE - 1) & PAGE_MASK);

	pLogBuf_base[1] =
	    dma_zalloc_coherent(device->dev, SIZE_1MB + PAGE_SIZE, &dma_handle_log[1],
				GFP_KERNEL | GFP_DMA);
	pLogBuf[1] = (uint8_t *) (((unsigned long)pLogBuf_base[1] + PAGE_SIZE - 1) & PAGE_MASK);

	pLogBuf_base[2] =
	    dma_zalloc_coherent(device->dev, SIZE_1MB + PAGE_SIZE, &dma_handle_log[2],
				GFP_KERNEL | GFP_DMA);
	pLogBuf[2] = (uint8_t *) (((unsigned long)pLogBuf_base[2] + PAGE_SIZE - 1) & PAGE_MASK);
#else
	pLogBuf_base[0] = kmalloc(SIZE_1MB, GFP_KERNEL);
	pLogBuf_base[1] = kmalloc(SIZE_1MB, GFP_KERNEL);
	pLogBuf_base[2] = kmalloc(SIZE_1MB, GFP_KERNEL);

	pLogBuf[0] = pLogBuf_base[0];
	pLogBuf[1] = pLogBuf_base[1];
	pLogBuf[2] = pLogBuf_base[2];
#endif


	I2CCmdBuf_mva[0] = 0x10000000;
	ret = m4u_alloc_mva(m4u_client, CCUG_OF_M4U_PORT, (unsigned long)pI2CCmdBuf[0], 0,
			    SIZE_1MB, M4U_PROT_READ | M4U_PROT_WRITE,
			    M4U_FLAGS_START_FROM, &I2CCmdBuf_mva[0]);
	if (ret) {
		LOG_ERR("fail to allocate mva for I2CCmdBuf_mva, ret=%d\n", ret);
		goto out;
	}
#if defined(_KCCU_MAILBOX_USE_DMEM_)
#else
	/*Mapping MVA of mailbox must use flag M4U_FLAGS_START_FROM to force MVA mapped beyond 0x10000000 */
	MailBox_mva[MAILBOX_SEND] = 0x10000000;
	ret = m4u_alloc_mva(m4u_client, CCUG_OF_M4U_PORT, (unsigned long)pMailBox[MAILBOX_SEND], 0,
			    sizeof(ccu_mailbox_t), M4U_PROT_READ | M4U_PROT_WRITE,
			    M4U_FLAGS_START_FROM, &MailBox_mva[MAILBOX_SEND]);
	if (ret) {
		LOG_ERR("fail to allocate mva for MailBox_mva[MAILBOX_SEND], ret=%d\n", ret);
		goto out;
	}
	LOG_DBG("dma_alloc_mva for MailBox_mva[MAILBOX_SEND] done: %x\n",
		MailBox_mva[MAILBOX_SEND]);

	MailBox_mva[MAILBOX_GET] = 0x10000000;
	ret = m4u_alloc_mva(m4u_client, CCUG_OF_M4U_PORT, (unsigned long)pMailBox[MAILBOX_GET], 0,
			    sizeof(ccu_mailbox_t), M4U_PROT_READ | M4U_PROT_WRITE,
			    M4U_FLAGS_START_FROM, &MailBox_mva[MAILBOX_GET]);
	if (ret) {
		LOG_ERR("fail to allocate mva for MailBox_mva[MAILBOX_GET], ret=%d\n", ret);
		goto out;
	}
	LOG_DBG("dma_alloc_mva for MailBox_mva[MAILBOX_GET] done: %x\n", MailBox_mva[MAILBOX_GET]);
#endif

	/*Mapping MVA for Log buffer */
#if defined(_ccu_mem_use_dma_api_)
	sg_table = &table;
	sg_alloc_table(sg_table, 1, GFP_KERNEL);
	sg_dma_address(sg_table->sgl) = dma_handle_log[0];
	sg_dma_len(sg_table->sgl) = SIZE_1MB + PAGE_SIZE;
#endif
	LogBuf_mva[0] = 0x10000000;
	ret = m4u_alloc_mva(m4u_client, CCUG_OF_M4U_PORT, (unsigned long)pLogBuf[0], 0,
			    SIZE_1MB, M4U_PROT_READ | M4U_PROT_WRITE,
			    M4U_FLAGS_START_FROM, &LogBuf_mva[0]);
	if (ret) {
		LOG_ERR("fail to allocate mva for LogBuf_mva[0], ret=%d\n", ret);
		goto out;
	}
	LOG_DBG("dma_alloc_mva for LogBuf_mva[0] done: %x\n", LogBuf_mva[0]);

	LogBuf_mva[1] = 0x10000000;
	ret = m4u_alloc_mva(m4u_client, CCUG_OF_M4U_PORT, (unsigned long)pLogBuf[1], 0,
			    SIZE_1MB, M4U_PROT_READ | M4U_PROT_WRITE,
			    M4U_FLAGS_START_FROM, &LogBuf_mva[1]);
	if (ret) {
		LOG_ERR("fail to allocate mva for LogBuf_mva[1], ret=%d\n", ret);
		goto out;
	}
	LOG_DBG("dma_alloc_mva for LogBuf_mva[1] done: %x\n", LogBuf_mva[1]);
	LogBuf_mva[2] = 0x10000000;
	ret = m4u_alloc_mva(m4u_client, CCUG_OF_M4U_PORT, (unsigned long)pLogBuf[2], 0,
			    SIZE_1MB, M4U_PROT_READ | M4U_PROT_WRITE,
			    M4U_FLAGS_START_FROM, &LogBuf_mva[2]);
	if (ret) {
		LOG_ERR("fail to allocate mva for LogBuf_mva[2], ret=%d\n", ret);
		goto out;
	}
	LOG_DBG("dma_alloc_mva for LogBuf_mva[2] done: %x\n", LogBuf_mva[2]);

#if defined(_KCCU_MAILBOX_USE_DMEM_)
	LOG_DBG("(0x%p)(0x%p)(0x%p)(0x%p)\n", pI2CCmdBuf[0], pLogBuf[0], pLogBuf[1], pLogBuf[2]);
	LOG_DBG("(0x%x)(0x%x)(0x%x)(0x%x)\n", I2CCmdBuf_mva[0], LogBuf_mva[0], LogBuf_mva[1],
		LogBuf_mva[2]);
#else

	LOG_DBG("(0x%p)(0x%p)(0x%p)(0x%p)(0x%p)(0x%p)\n", pI2CCmdBuf[0], pMailBox[MAILBOX_SEND],
		pMailBox[MAILBOX_GET], pLogBuf[0], pLogBuf[1], pLogBuf[2]);
	LOG_DBG("(0x%x)(0x%x)(0x%x)(0x%x)(0x%x)(0x%x)\n", I2CCmdBuf_mva[0],
		MailBox_mva[MAILBOX_SEND], MailBox_mva[MAILBOX_GET], LogBuf_mva[0], LogBuf_mva[1],
		LogBuf_mva[2]);
#endif
...
int ccu_uninit_hw(ccu_device_t *device)
{
	...
	/*release m4u mapping*/
	/*--todo: remove this, i2c buffer allocation move to user space now*/
	/*if (I2CCmdBuf_mva[0])*/
	/*  m4u_dealloc_mva(m4u_client, CCUG_OF_M4U_PORT, I2CCmdBuf_mva[0]); */

#if defined(_KCCU_MAILBOX_USE_DMEM_)
#else
	if (MailBox_mva[MAILBOX_SEND])
		m4u_dealloc_mva(m4u_client, CCUG_OF_M4U_PORT, MailBox_mva[MAILBOX_SEND]);
	if (MailBox_mva[MAILBOX_GET])
		m4u_dealloc_mva(m4u_client, CCUG_OF_M4U_PORT, MailBox_mva[MAILBOX_GET]);
#endif

	if (LogBuf_mva[0])
		m4u_dealloc_mva(m4u_client, CCUG_OF_M4U_PORT, LogBuf_mva[0]);
	if (LogBuf_mva[1])
		m4u_dealloc_mva(m4u_client, CCUG_OF_M4U_PORT, LogBuf_mva[1]);
	if (LogBuf_mva[2])
		m4u_dealloc_mva(m4u_client, CCUG_OF_M4U_PORT, LogBuf_mva[2]);
...
#if defined(_KCCU_MAILBOX_USE_DMEM_)
#else
	dma_free_coherent(device->dev, sizeof(ccu_mailbox_t), (void *)pMailBox[MAILBOX_SEND], dma_handle_mbt);
	/*kfree((void*)pMailBox[MAILBOX_GET]);*/
	dma_free_coherent(device->dev, sizeof(ccu_mailbox_t), (void *)pMailBox[MAILBOX_GET], dma_handle_mbr);
#endif

#if defined(_ccu_mem_use_get_free_pages_)
	free_pages((unsigned long)pLogBuf[0], SIZE_1MB_PWR2PAGE);
	free_pages((unsigned long)pLogBuf[1], SIZE_1MB_PWR2PAGE);
	free_pages((unsigned long)pLogBuf[2], SIZE_1MB_PWR2PAGE);
#elif defined(_ccu_mem_use_dma_api_)
	dma_free_coherent(device->dev, SIZE_1MB + PAGE_SIZE, (void *)pLogBuf_base[0], dma_handle_log[0]);
	dma_free_coherent(device->dev, SIZE_1MB + PAGE_SIZE, (void *)pLogBuf_base[1], dma_handle_log[1]);
	dma_free_coherent(device->dev, SIZE_1MB + PAGE_SIZE, (void *)pLogBuf_base[2], dma_handle_log[2]);
#else
	kfree(pLogBuf[0]);
	kfree(pLogBuf[1]);
	kfree(pLogBuf[2]);
...
int ccu_get_i2c_dma_buf_addr(uint32_t *mva)
{
	...
	ret = m4u_alloc_mva(m4u_client, CCUG_OF_M4U_PORT, (unsigned long)va, 0,
			    4096, M4U_PROT_READ | M4U_PROT_WRITE, M4U_FLAGS_SEQ_ACCESS, mva);
...
#if defined(_KCCU_MAILBOX_USE_DMEM_)
#else
		ccu_write_reg(ccu_base, CCU_DATA_REG_MAILBOX_APMCU, MailBox_mva[MAILBOX_SEND]);
		ccu_write_reg(ccu_base, CCU_DATA_REG_MAILBOX_CCU, MailBox_mva[MAILBOX_GET]);
		/**/
		LOG_DBG("pMailBox[0](0x%p)\n", pMailBox[MAILBOX_SEND]);
		LOG_DBG("pMailBox[1](0x%p)\n", pMailBox[MAILBOX_GET]);
		LOG_DBG("MailBox_mva[0](0x%x)\n", MailBox_mva[MAILBOX_SEND]);
		LOG_DBG("MailBox_mva[1](0x%x)\n", MailBox_mva[MAILBOX_GET]);
#endif

#if defined(_KCCU_LOG_USE_KERNEL_BUFFER_)
		ccu_write_reg(ccu_base, CCU_DATA_REG_LOG_BUF0, LogBuf_mva[0]);
		ccu_write_reg(ccu_base, CCU_DATA_REG_LOG_BUF1, LogBuf_mva[1]);

		LOG_DBG("pLogBuf_base[0](0x%p)\n", pLogBuf_base[0]);
		LOG_DBG("pLogBuf_base[1](0x%p)\n", pLogBuf_base[1]);
		LOG_DBG("pLogBuf_base[2](0x%p)\n", pLogBuf_base[2]);
		LOG_DBG("pLogBuf[0](0x%p)\n", pLogBuf[0]);
		LOG_DBG("pLogBuf[1](0x%p)\n", pLogBuf[1]);
		LOG_DBG("pLogBuf[2](0x%p)\n", pLogBuf[2]);
		LOG_DBG("LogBuf_mva[0](0x%x)\n", LogBuf_mva[0]);
		LOG_DBG("LogBuf_mva[1](0x%x)\n", LogBuf_mva[1]);
		LOG_DBG("LogBuf_mva[2](0x%x)\n", LogBuf_mva[2]);

#else				/*use user space buffer*/
