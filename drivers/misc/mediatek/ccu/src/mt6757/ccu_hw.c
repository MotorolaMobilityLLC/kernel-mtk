...
static struct ccu_sensor_info g_ccu_sensor_info_main = {-1, -1, NULL};
static char g_ccu_sensor_name_main[SENSOR_NAME_MAX_LEN];
static struct ccu_sensor_info g_ccu_sensor_info_main2 = {-1, -1, NULL};
static char g_ccu_sensor_name_main2[SENSOR_NAME_MAX_LEN];
static struct ccu_sensor_info g_ccu_sensor_info_sub = {-1, -1, NULL};
static char g_ccu_sensor_name_sub[SENSOR_NAME_MAX_LEN];

struct ccu_mailbox_s *pMailBox[MAX_MAILBOX_NUM];
...
static inline int wait_command(void)
{
	return (wait_event_interruptible_timeout(
		    cmd_wait, cmd_done, msecs_to_jiffies(1 * 1000)) > 0)
		   ? 0
		   : -ETIMEDOUT;
}
...
irqreturn_t ccu_isr_handler(int irq, void *dev_id)
{
	...
	while (1) {
		...
		case MSG_TO_APMCU_FLUSH_LOG: {
			/*for ccu_waitirq();*/
			LOG_DBG("got MSG_TO_APMCU_FLUSH_LOG:%d , wakeup "
		...
		case MSG_TO_APMCU_CCU_ASSERT: {
			LOG_ERR("got MSG_TO_APMCU_CCU_ASSERT:%d, wakeup "
				"ccuInfo.WaitQueueHead\n",
				receivedCcuCmd.in_data_ptr);
		...
		case MSG_TO_APMCU_CCU_WARNING: {
			LOG_ERR("got MSG_TO_APMCU_CCU_WARNING:%d, wakeup "
				"ccuInfo.WaitQueueHead\n",
				receivedCcuCmd.in_data_ptr);
...
int ccu_get_i2c_dma_buf_addr(uint32_t *mva)
{
	...
	*mva = 0x10000000;
	...
	if (ret)
		LOG_ERR("fail to allocate mva for the pointer to "
			"i2c_dma_buf_addr, ret=%d\n",
			ret);
...
int ccu_force_powerdown(void)
{
	...
	if (ccuInfo.IsCcuPoweredOn == 1) {
		LOG_WRN("CCU kernel drv released on CCU running, try to force "
			"shutdown\n");
...
{
	int32_t timeout = 10;
	struct ccu_mailbox_s *ccuMbPtr = NULL;
	struct ccu_mailbox_s *apMbPtr = NULL;
	...
	* Driver wait CCU main initialize done and query INFO00 & INFO01 as
	* mailbox address
	*/
	pMailBox[MAILBOX_SEND] = (struct ccu_mailbox_s *)(uintptr_t)(
	    dmem_base + ccu_read_reg(ccu_base, CCU_DATA_REG_MAILBOX_CCU));
	pMailBox[MAILBOX_GET] = (struct ccu_mailbox_s *)(uintptr_t)(
	    dmem_base + ccu_read_reg(ccu_base, CCU_DATA_REG_MAILBOX_APMCU));

	ccuMbPtr = (struct ccu_mailbox_s *)pMailBox[MAILBOX_SEND];
	apMbPtr = (struct ccu_mailbox_s *)pMailBox[MAILBOX_GET];
