...
static struct ccu_mailbox_s *_ccu_mailbox;
static struct ccu_mailbox_s *_apmcu_mailbox;
...
* Public functions
********************************************************************************/
mb_result mailbox_init(
	struct ccu_mailbox_s *apmcu_mb_addr, struct ccu_mailbox_s *ccu_mb_addr)
{
	...
	LOG_DBG("memclr _ccu_mailbox: %d\n",
		ccu_memclr(_ccu_mailbox, sizeof(struct ccu_mailbox_s)));
	LOG_DBG("memclr _apmcu_mailbox: %d\n",
		ccu_memclr(_apmcu_mailbox, sizeof(struct ccu_mailbox_s)));
...
* Private functions
********************************************************************************/
static int ccu_msg_copy(ccu_msg_t *dest, ccu_msg_t *src)
{
...
static mb_result _mailbox_write_to_buffer(ccu_msg_t *task)
{
	...
	if (nextWriteSlot == _ccu_mailbox->front) {
		...
		LOG_DBG("incming cmd: cmd(%d), in(%x), out(%x)\n", task->msg_id,
			task->in_data_ptr, task->out_data_ptr);
