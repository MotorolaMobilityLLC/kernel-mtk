...
static struct ion_handle *_ccu_ion_alloc(struct ion_client *client,
					 ...
					 size_t align, unsigned int size)
{
			     ...
			     struct ion_handle **handle)
{
	...
	*handle =
	    _ccu_ion_alloc(ccu_ion_client, ION_HEAP_MULTIMEDIA_MAP_MVA_MASK,
...
void ccu_set_sensor_info(int32_t sensorType, struct ccu_sensor_info *info)
{
	if (sensorType == IMGSENSOR_SENSOR_IDX_NONE) {
		...
		LOG_DBG_MUST(
		    "ccu catch Main sensor i2c slave address : 0x%x\n",
		    info->slave_addr);
