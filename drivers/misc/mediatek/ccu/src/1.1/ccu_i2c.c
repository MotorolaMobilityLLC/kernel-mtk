...
int ccu_i2c_frame_reset(void)
{
	...
	i2c_writew(I2C_HS_NACKERR | I2C_ACKERR | I2C_TRANSAC_COMP, i2c,
		OFFSET_INTR_MASK);
...
int ccu_trigger_i2c(int transac_len, MBOOL do_dma_en)
{
	...
	/*LOG_DBG("i2c_dma_buf_content: %x,%x,%x,%x,",*/
	/*	dmaBufVa[0], dmaBufVa[1], dmaBufVa[2], dmaBufVa[3]);*/
	/*LOG_DBG("%x,%x,%x,%x,",*/
	/*	dmaBufVa[4], dmaBufVa[5], dmaBufVa[18], dmaBufVa[19]);*/
	/*LOG_DBG("%x,%x,%x,%x\n",*/
	/*	dmaBufVa[20], dmaBufVa[21], dmaBufVa[22], dmaBufVa[23]);*/
...
static int ccu_i2c_release_controller(void)
{
	...
	if (ccu_i2c_locked == MTRUE) {
		...
		/*LOG_INF_MUST("ccu i2c controller not ");*/
		/*LOG_INF_MUST("in locked status, skip unlock\n");*/
		LOG_INF_MUST("i2c controller irq is not ");
		LOG_INF_MUST("in disable status, skip enable\n");
	}
