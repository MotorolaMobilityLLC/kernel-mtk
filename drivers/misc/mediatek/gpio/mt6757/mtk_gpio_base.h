...
/* #define GPIO_SET_BITS(BIT,REG)   ((*(volatile unsigned long*)(REG)) =
 * (unsigned long)(BIT)) */
/* #define GPIO_CLR_BITS(BIT,REG)   ((*(volatile unsigned long*)(REG)) &=
 * ~((unsigned long)(BIT))) */
...
#define IOCFG_WR32(addr, data)                                                 \
	mt_reg_sync_writel(data, (IOCFG_BASE + (unsigned long)addr))
#define IOCFG_RD32(addr) __raw_readl((IOCFG_BASE + (unsigned long)addr))
#define IOCFG_SW_SET_BITS(BIT, REG)                                            \
	IOCFG_WR32(REG, IOCFG_RD32(REG) | ((unsigned long)(BIT)))
#define IOCFG_SET_BITS(BIT, REG) IOCFG_WR32(REG, (unsigned long)(BIT))
#define IOCFG_CLR_BITS(BIT, REG)                                               \
	IOCFG_WR32(REG, IOCFG_RD32(REG) & ~((unsigned long)(BIT)))
