...
int mt_get_clock_output(unsigned long num, unsigned long *src,
			unsigned long *div)
{
	GPIOERR("GPIO CLKM module not be implement any more!\n");
	return RSUCCESS;
...
void mt_gpio_dump_base(GPIO_REGS *regs)
{
	...
	GPIOMSG("%s\n", __func__);
	if (regs ==
	    NULL) { /*if arg is null, load & dump; otherwise, dump only */
	 ...
	 */
	/* for (idx = 0; idx < sizeof(regs->pullen)/sizeof(regs->pullen[0]);
	 * idx++) { */
	 ...
	 */
	/* for (idx = 0; idx < sizeof(regs->pullsel)/sizeof(regs->pullsel[0]);
	 * idx++) { */
...
void gpio_dump_regs(void)
{
	...
	for (idx = MT_GPIO_BASE_START; idx < MT_GPIO_BASE_MAX; idx++) {
		pr_debug("idx = %3d: %d %d %d %d %d %d %d\n", idx,
			 mt_get_gpio_mode_base(idx),
			 mt_get_gpio_pull_select_base(idx),
			 mt_get_gpio_in_base(idx), mt_get_gpio_out_base(idx),
			 mt_get_gpio_pull_enable_base(idx),
			 mt_get_gpio_dir_base(idx), mt_get_gpio_ies_base(idx));
	}
}
...
static void mt_gpio_read_pin_base(GPIO_CFG *cfg, int method)
{
	if (method == 0) {
		...
		if (cfg->no < MT_GPIO_BASE_MAX) {
			/* cfg->pullsel= (cur->pullsel[num].val & (1L << bit)) ?
			 * (1) : (0); */
			cfg->din =
			    (cur->din[num].val & (1L << bit)) ? (1) : (0);
			cfg->dout =
			    (cur->dout[num].val & (1L << bit)) ? (1) : (0);
			/* cfg->pullen = (cur->pullen[num].val & (1L << bit)) ?
			 * (1) : (0); */
			cfg->dir =
			    (cur->dir[num].val & (1L << bit)) ? (1) : (0);
			/* cfg->dinv   = (cur->dinv[num].val & (1L << bit)) ?
			 * (1) : (0); */
			num = cfg->no / MAX_GPIO_MODE_PER_REG;
			bit = cfg->no % MAX_GPIO_MODE_PER_REG;
			cfg->mode =
			    (cur->mode[num].val >> (GPIO_MODE_BITS * bit)) &
			    mask;
		}
	} else if (method == 1) {
...
static ssize_t mt_gpio_dump_addr_base(void)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(reg->dir); idx++)
		GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx,
			&reg->dir[idx].val, idx, &reg->dir[idx].set, idx,
			&reg->dir[idx].rst);
	/* GPIOMSG("# ies\n"); */
   ...
   */
	/* GPIOMSG("# pull enable\n"); */
	/* for (idx = 0; idx < sizeof(reg->pullen)/sizeof(reg->pullen[0]);
	 * idx++) */
	/* GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx,
   *&reg->pullen[idx].val,
   ...
   */
	/* GPIOMSG("# pull select\n"); */
	/* for (idx = 0; idx < sizeof(reg->pullsel)/sizeof(reg->pullsel[0]);
	 * idx++) */
	/* GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx,
   *&reg->pullsel[idx].val,
   ...
   */
	...
	for (idx = 0; idx < ARRAY_SIZE(reg->dout); idx++)
		GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx,
			&reg->dout[idx].val, idx, &reg->dout[idx].set, idx,
			&reg->dout[idx].rst);
	GPIOMSG("# data input\n");
	...
	for (idx = 0; idx < ARRAY_SIZE(reg->mode); idx++)
		GPIOMSG("val[%2d] %p\nset[%2d] %p\nrst[%2d] %p\n", idx,
			&reg->mode[idx].val, idx, &reg->mode[idx].set, idx,
			&reg->mode[idx].rst);
	return 0;
}
...
*reg->dir[idx].val, cur->dir[idx].val);
...
*reg->pullen[idx].val, cur->pullen[idx].val);
...
*reg->pullsel[idx].val, cur->pullsel[idx].val);
*    for (idx = 0; idx < sizeof(reg->dinv)/sizeof(reg->dinv[0]); idx++)
*	if (reg->dinv[idx].val != cur->dinv[idx].val)
*	    GPIOERR("GPIOEXT mismatch dinv[%2d]: %x <> %x\n", idx,
*reg->dinv[idx].val, cur->dinv[idx].val);
...
*reg->mode[idx].val, cur->mode[idx].val);
*
*    kfree(cur);
*    return;
*}
...
static ssize_t mt_gpio_dump_regs(char *buf, ssize_t bufLen)
{
	...
	for (idx = MT_GPIO_BASE_START; idx < MT_GPIO_BASE_MAX; idx++) {
		len += snprintf(buf + len, bufLen - len, "%3d:%d%d%d\n", idx,
				mt_get_gpio_in_base(idx),
				mt_get_gpio_out_base(idx),
	...
	for (idx = MT_GPIO_BASE_START; idx < MT_GPIO_BASE_MAX; idx++) {
		len += snprintf(
		    buf + len, bufLen - len, "%3d:%d%d%d%d%d%d%d%d\n", idx,
		    mt_get_gpio_mode_base(idx),
		    mt_get_gpio_pull_select_base(idx), mt_get_gpio_in_base(idx),
		    mt_get_gpio_out_base(idx),
		    mt_get_gpio_pull_enable_base(idx),
		    mt_get_gpio_dir_base(idx), mt_get_gpio_ies_base(idx),
		    mt_get_gpio_smt_base(idx));
...
/*---------------------------------------------------------------------------*/
ssize_t mt_gpio_show_pin(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
			  ...
			  const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "-h", 2)) {
		...
		GPIOMSG("echo -wpen num x > pin  #x: 1,pull enable; 0 pull "
			"disable\n");
		GPIOMSG("echo -wies num x > pin  #x: 1,ies enable; 0 ies "
			"disable\n");
		GPIOMSG("echo -wdir num x > pin  #x: 1, output; 0, input\n");
		/*GPIOMSG("echo -wdinv num x > pin #x: 1, inversion enable; 0,
		 * disable\n"); */
		GPIOMSG("echo -w=num x x x x x x > pin #set all property one "
			"time\n");
		GPIOMSG("PIN: [MODE] [PSEL] [DIN] [DOUT] [PEN] [DIR] [IES]\n");
	} else if (!strncmp(buf, "-r0", 3) &&
		   (sscanf(buf + 3, "%d", &pin) == 1)) {
		GPIO_CFG cfg = {.no = pin};
		/*if pmic */
		mt_gpio_read_pin_base(&cfg, 0);
		GPIOMSG("%3d: %d %d %d %d %d %d\n", cfg.no, cfg.mode,
			cfg.pullsel, cfg.din, cfg.dout, cfg.pullen, cfg.dir);
	} else if (!strncmp(buf, "-r1", 3) &&
		   (sscanf(buf + 3, "%d", &pin) == 1)) {
		GPIO_CFG cfg = {.no = pin};
 ...
 * dinv)); */
			...
			GPIOMSG("set dir (%3d, %d)=%d\n", pin, dir,
				mt_set_gpio_dir(pin, dir));
#else
		else if (sscanf(buf, "=%d:%d %d %d %d %d %d %d", &pin, &mode,
				&pullsel, &dout, &pullen, &dir, &ies,
				&smt) == 8) {
			GPIOMSG("set mode(%3d, %d)=%d\n", pin, mode,
				mt_set_gpio_mode(pin, mode));
			GPIOMSG("set psel(%3d, %d)=%d\n", pin, pullsel,
