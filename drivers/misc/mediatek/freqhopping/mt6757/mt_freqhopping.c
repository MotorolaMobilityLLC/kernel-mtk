...
static DEFINE_SPINLOCK(g_mt6757_0x1001AXXX_lock);
...
static int __init mt6757_0x1001AXXX_sw_protect_init(void)
{
	...
	/* DVFSP HW semaphore init. */
	{
		...
		hs_write32(g_reg_cspm_poweron_en, 0x0b160001);	/* mt6757-dvfsp enable internal CG bit */
	}
...
core_initcall(mt6757_0x1001AXXX_sw_protect_init);

/* HW semaphore3 M0 get.
 *  For ATF, SPM and kernel protecting 0x1001AXXX access
 */

static int mt6757_0x1001AXXX_get_semaphore(void)
{
	int i;
	int n = DIV_ROUND_UP(SEMA_GET_TIMEOUT, 10);

	FH_MSG_DEBUG("mt6757_0x1001AXXX_get_semaphore+");

	/* mt6757-dvfsp enable internal CG bit */
 ...
 *  For ATF, SPM and kernel protecting 0x1001AXXX access
 */
static void mt6757_0x1001AXXX_release_semaphore(void)
{
 ...
 */
void mt6757_0x1001AXXX_reg_write(unsigned long reg_offset, unsigned int value)
{
	...
	mt6757_0x1001AXXX_lock();	/* To protect between ATF and SPM. */

	mt_reg_sync_writel(value, reg);
	ndelay(200);		/* DE workaround, for first read after sequential write. */

	mt6757_0x1001AXXX_unlock();	/* To protect between ATF and SPM. */
 ...
 *  Provide to MCUMIXEDSYS access.  (Clock related driver)
 */
unsigned int mt6757_0x1001AXXX_reg_read(unsigned long reg_offset)
{
...
void mt6757_0x1001AXXX_reg_set(unsigned long reg_offset, unsigned int field, unsigned int val)
{
	volatile unsigned long reg;
 ...
 */
void mt6757_0x1001AXXX_lock(void)
{
	spin_lock(&g_mt6757_0x1001AXXX_lock);


	if (0 != mt6757_0x1001AXXX_get_semaphore()) {
		if (0 != mt6757_0x1001AXXX_get_semaphore()) {
...
void mt6757_0x1001AXXX_unlock(void)
{
	mt6757_0x1001AXXX_release_semaphore();

	spin_unlock(&g_mt6757_0x1001AXXX_lock);
}
