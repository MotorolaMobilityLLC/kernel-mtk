...
int fhctl_slt_test_result = -2;

static int freqhopping_slt_test_proc_read(struct seq_file *m, void *v)
{
	pr_err("%s()\n", __func__);
	pr_err("fhctl slt result info\n");
	pr_err("return 0 => [SLT]FHCTL Test Pass!\n");
	pr_err("return -1 => [SLT]FHCTL Test Fail!\n");
	pr_err("return (result != 0 && result != -1) => [SLT]FHCTL Test Untested!\n");
...
static ssize_t freqhopping_slt_test_proc_write(struct file *file, const char *buffer, size_t count,
					     loff_t *data)
{
	...
	if (!strcmp(cmd, "slt_start"))
		fhctl_slt_test_result = g_p_fh_hal_drv->mt_fh_slt_start();
	else
...
static int freqhopping_debug_proc_init(void)
{
	...
	prUserdefEntry =
	    proc_create("userdef", 0664, fh_proc_dir, &userdef_fops);
	if (prUserdefEntry) {
	...
	if (prSltTestEntry) {
		/* prUserdefEntry->read_proc  = freqhopping_slt_test_proc_read; */
		/* prUserdefEntry->write_proc = freqhopping_slt_test_proc_write; */
		FH_MSG("[%s]: successfully create /proc/freqhopping/slt_test", __func__);
	} else {
		FH_MSG("[%s]: failed to create /proc/freqhopping/slt_test", __func__);
		return 1;
	}
