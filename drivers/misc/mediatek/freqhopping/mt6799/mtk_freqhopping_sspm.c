...
static int mt_fh_hal_general_pll_dfs(enum FH_PLL_ID pll_id, unsigned int target_dds)
{
	...
	/* [MT6799 Only] All new platform should confirm again!!! */
	switch (pll_id) {
	case FH_MEM_PLLID: /* MEMPLL Confirmed with DRAM SA that MEMPLL hopping will only control by clk DIV*/
		FH_MSG("ERROR! The [PLL_ID]:%d was forbidden hopping by MT6799 FHCTL.", pll_id);
		WARN_ON(1);
		break;
	default:
		break;
...
/* [For MT6799] GPU CLK hopping, MFGPLL */
