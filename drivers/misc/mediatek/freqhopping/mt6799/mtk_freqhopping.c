...
static int mt_fh_hal_hopping(enum FH_PLL_ID pll_id, unsigned int dds_value)
{
	...
	if (pll_id == FH_MEM_PLLID) {
		...
		FH_MSG_DEBUG("Do not support MEMPLL  on MT6799");
		return 0;
	}
...
/* [For Elbrus] GPU CLK hopping, MFGPLL */
static int mt_fh_hal_dfs_mmpll(unsigned int target_dds)
{
	/* unsigned long flags = 0; */
	/* [For Elbrus] MFGPLL, ToDo: Need to confirmed with GPU clk owner */
...
static int mt_fh_hal_general_pll_dfs(enum FH_PLL_ID pll_id, unsigned int target_dds)
{
	...
	/* [Elbrus Only] All new platform should confirm again!!! */
	switch (pll_id) {
	case FH_PLL0:
	case FH_PLL1:
	case FH_PLL2:
