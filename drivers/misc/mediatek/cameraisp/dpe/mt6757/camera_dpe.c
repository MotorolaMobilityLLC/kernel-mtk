...
#endif
/* !defined(USE_DEPRECATED_CONFIG) && defined(CONFIG_COMMON_CLK)  */
#endif
/*  */
#ifndef MTRUE
#define MTRUE               1
...
LOG_ERR("WaitIRQ Err,type:%d,urNum:%d, status:%d,whichReq:%d,ProcessID:0x%x\n",
...
#ifdef __DPE_KERNEL_PERFORMANCE_MEASURE__
	mt_kernel_trace_begin("Config DVEHW");
#endif
...
#ifdef __DPE_KERNEL_PERFORMANCE_MEASURE__
	mt_kernel_trace_begin("Config WMFEHW");
#endif
	...
	(pRegIo->Count) * sizeof(struct DPE_REG_STRUCT)) == 0) {
		for (i = 0; i < pRegIo->Count; i++) {
			if ((ISP_DPE_BASE + pData->Addr >= ISP_DPE_BASE)
				...
				pData->Val = DPE_RD32(ISP_DPE_BASE +
				pData->Addr);
			} else {
...
LOG_ERR("No Such Stats can be waited!!irq Type/User/Sts/Pid(0x%x/%d/0x%x/%d)\n",
	...
	LOG_ERR("ERRRR Timeout!Timeout(%d)\n", WaitIrq->Timeout);
	LOG_ERR("Clear(%d),Type(%d),IrqSta(0x%08X), WaitSta(0x%08X)\n",
		WaitIrq->Clear, WaitIrq->Type, irqStatus, WaitIrq->Status);

	LOG_ERR("urKey(%d),whReq(%d),PID(%d)\n",
		WaitIrq->UserKey, whichReq, WaitIrq->ProcessID);
	LOG_ERR("DveIrqCnt(0x%08X),WmfeIrqCnt(0x%08X)\n",
		DPEInfo.IrqInfo.DveIrqCnt,
		DPEInfo.IrqInfo.WmfeIrqCnt);
	LOG_ERR("WriteReqIdx(0x%08X),ReadReqIdx(0x%08X)\n",
		DPEInfo.WriteReqIdx,
		DPEInfo.ReadReqIdx);
...
static signed int DPE_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	if (length > DPE_REG_RANGE) {
		LOG_ERR("mmap range error :module:0x%x length(0x%lx)",
		pfn,
		length);
		LOG_ERR("mmap range error :DPE_REG_RANGE(0x%x)!",
		DPE_REG_RANGE);
...
"Unable to request IRQ, request_irq fail,nr_DPE_devs=%d, devnode(%s), irq=%d, ISR: %s\n",
	...
	if (nr_DPE_devs == 1) {
		...
		dpe_clk.CG_SCP_SYS_DIS =
			devm_clk_get(&pDev->dev, "DPE_CG_SCP_SYS_DIS");
		dpe_clk.CG_MM_SMI_COMMON =
			devm_clk_get(&pDev->dev, "DPE_CG_MM_SMI_COMMON");
		dpe_clk.CG_SCP_SYS_ISP =
			devm_clk_get(&pDev->dev, "DPE_CG_SCP_SYS_ISP");
		dpe_clk.CG_IMGSYS_LAB5 =
			devm_clk_get(&pDev->dev, "DPE_CG_IMG_LARB5");
...
static irqreturn_t ISP_Irq_DPE(signed int Irq, void *DeviceId)
{
	...
	if (DVE_INT_ST == (DVE_INT_ST & DveStatus)) {
		...
		if (DPEInfo.WriteReqIdx == DPEInfo.ReadReqIdx) {
			IRQ_LOG_KEEPER(DPE_IRQ_TYPE_INT_DPE_ST, m_CurrentPPB,
			_LOG_ERR,
			"ISP_Irq DPE Err!!, WriteReqIdx:0x%x,ReadReqIdx:0x%x\n",
			DPEInfo.WriteReqIdx, DPEInfo.ReadReqIdx);
		}
#endif
	...
	"ISP_Irq DPE:%d, reg 0x%x : 0x%x, bResulst:%d, DveHWSta:0x%x\n",
	Irq,
	DPE_INT_STATUS_HW,
	DpeIntStatus,
	bResulst,
