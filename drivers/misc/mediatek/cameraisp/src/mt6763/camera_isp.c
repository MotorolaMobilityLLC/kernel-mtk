...
#define LOG_AST(format, args...)                                              \
	pr_alert(MyTag "[%s] " format, __func__, ##args)
...
static struct T_ION_TBL gION_TBL[ISP_DEV_NODE_NUM] = {
	...
	{ISP_DEV_NODE_NUM, NULL, NULL, NULL, NULL},
	{ISP_DEV_NODE_NUM, NULL, NULL, NULL, NULL},
	{ISP_CAM_A_IDX, (int *)G_WRDMA_IonCt[0],
	 (int *)G_WRDMA_IonFd[0],
	 (struct ion_handle **)G_WRDMA_IonHnd[0],
	 (spinlock_t *)SpinLock_IonHnd[0]},
	{ISP_CAM_B_IDX, (int *)G_WRDMA_IonCt[1],
	 (int *)G_WRDMA_IonFd[1],
...
/* currently, _pr_info only used at ipl_buf_ctrl to protect critical section */
enum eLOG_TYPE {
	_pr_info = 0,
	_pr_info = 1,
	_pr_err = 2,
...
{
	if (enter) {
		int imgo_en = 0, rrzo_en = 0, imgo_bpp, rrzo_bpp, imgo_xsize,
		    imgo_ysize;
		...
		trace_ISP_Pass1_CAM_enter(
		    imgo_en, rrzo_en, imgo_bpp, rrzo_bpp, imgo_xsize,
		    imgo_ysize, rrzo_xsize, rrzo_ysize, rrz_src_w, rrz_src_h,
		    rrz_dst_w, rrz_dst_h, rrz_hori_step, rrz_vert_step);
	} else {
...
static void ISP_DumpDmaDeepDbg(enum ISP_IRQ_TYPE_ENUM module)
{
	...
	IRQ_LOG_KEEPER_PR_ERR(module, m_CurrentPPB, _pr_err,
...
static int ISP_DumpDIPReg(void)
{
	...
	if (g_bDumpPhyISPBuf == MFALSE) {
		...
		g_dumpInfo.tdri_baseaddr =
		    ISP_RD32(ISP_DIP_A_BASE + 0x204); /* 0x15022204 */
		g_dumpInfo.imgi_baseaddr =
		    ISP_RD32(ISP_DIP_A_BASE + 0x400); /* 0x15022400 */
		g_dumpInfo.dmgi_baseaddr =
		    ISP_RD32(ISP_DIP_A_BASE + 0x520); /* 0x15022520 */
		g_tdriaddr = g_dumpInfo.tdri_baseaddr;
		for (cmdqidx = 0; cmdqidx < 32; cmdqidx++) {
			if (ctrl_start & (0x1 << cmdqidx)) {
				g_cmdqaddr = ISP_RD32(ISP_DIP_A_BASE + 0x108 +
						      (cmdqidx * 12));
				break;
			}
		}
...
static int ISP_ReadReg(struct ISP_REG_IO_STRUCT *pRegIo)
{
	...
	for (i = 0; i < pRegIo->Count; i++) {
		...
		/*  */
		/* pr_debug("[KernelRDReg]addr(0x%x),value()0x%x\n",
	...
	case ISP_P2_BUFQUE_LIST_TAG_UNIT:
		...
		case ISP_P2_BUF_STATE_DEQUE_SUCCESS:
			do { /* to find the newest cur index */
				tmpIdx =
				    (tmpIdx + 1) % _MAX_SUPPORT_P2_FRAME_NUM_;
				switch (P2_FrameUnit_List[property][tmpIdx]
					    .bufSts) {
				case ISP_P2_BUF_STATE_ENQUE:
...
static int ISP_FLUSH_IRQ(struct ISP_WAIT_IRQ_STRUCT *irqinfo)
{
	...
	pr_info("type(%d)userKey(%d)St_type(%d)St(0x%x)", irqinfo->Type,
...
static int ISP_WaitIrq(struct ISP_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	if (WaitIrq->Type >= ISP_IRQ_TYPE_AMOUNT) {
		pr_debug("WaitIrq: type error(%d)", WaitIrq->Type);
		return -EFAULT;
	}

	if (WaitIrq->EventInfo.St_type >= ISP_IRQ_ST_AMOUNT) {
		pr_debug("WaitIrq: st_type error(%d)",
	...
	if (WaitIrq->EventInfo.UserKey >= IRQ_USER_NUM_MAX ||
		...
		pr_debug("WaitIrq: userkey error(%d)",
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_GET_SUPPORTED_ISP_CLOCKS:
		...
		ispclks.clklevelcnt = mmdvfs_qos_get_thres_count(
		    &isp_qos, MMDVFS_PM_QOS_SUB_SYS_CAMERA);
	...
	case ISP_GET_CUR_ISP_CLOCK:
		{
			...
			curclk = mmdvfs_qos_get_cur_thres(&isp_qos,
						MMDVFS_PM_QOS_SUB_SYS_CAMERA);
			pr_debug("Get current clock level:%d", curclk);

			if (copy_to_user((void *)Param, &curclk,
			    sizeof(unsigned int)) != 0) {
				pr_debug("copy_to_user failed");
				Ret = -EFAULT;
			}
	...
	struct file *pFile)
{
	...
	if (g_bIonBufferAllocated == MTRUE) {
		g_pPhyISPBuffer =
		    (unsigned int *)(uintptr_t)(g_isp_p2_imem_buf.va);
		g_pTuningBuffer =
		    (unsigned int *)(((uintptr_t)g_pPhyISPBuffer) +
				     ISP_DIP_REG_SIZE);
		g_pTpipeBuffer =
		...
		g_pVirISPBuffer =
		    (unsigned int *)(((uintptr_t)g_pTpipeBuffer) +
				     MAX_ISP_TILE_TDR_HEX_NO);
		g_pCmdqBuffer = (unsigned int *)(((uintptr_t)g_pVirISPBuffer) +
						 ISP_DIP_REG_SIZE);
		/* Kernel Exception */
		g_pKWTpipeBuffer =
...
/* Get platform_device parameters */
	...
	if (nr_isp_devs == 1) {
		...
		/*  */
		IspInfo.IrqInfo
		    .ErrMask[ISP_IRQ_TYPE_INT_CAM_A_ST][SIGNAL_INT] =
		    INT_ST_MASK_CAM_ERR;
		IspInfo.IrqInfo
		    .ErrMask[ISP_IRQ_TYPE_INT_CAM_B_ST][SIGNAL_INT] =
		    INT_ST_MASK_CAM_ERR;
		IspInfo.IrqInfo
...
)
{
	...
	if (!G_u4EnableClockCount) {
		spin_unlock(&(IspInfo.SpinLockClock));
		/* Only print cama log */
		if (strcmp(moduleName, IRQ_CB_TBL[ISP_IRQ_TYPE_INT_CAM_A_ST]
					   .device_name) == 0) {
...
/*  */
	...
	do {
		...
		if (comp_str == NULL) {
			/*pr_debug("kmalloc failed for finding compatible\n");*/
			break;
		}
	...
	if (!node) {
		pr_debug("find mediatek,apmixed node failed!!!\n");
		return -ENODEV;
	}
	...
	if (!isp_p2_dir) {
		pr_debug("[%s]: fail to mkdir /proc/isp_p2\n", __func__);
		return 0;
	}
...
irqreturn_t ISP_Irq_CAMSV_5(int Irq, void *DeviceId)
{
	...
	if (IrqStatus & SV_SOF_INT_ST) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			if (pstRTBuf[module]->ring_buf[_camsv_imgo_].active)
				magic_num = ISP_RD32(
				CAMSV_REG_IMGO_FH_SPARE_3(reg_module));
			else
				magic_num = ISP_RD32(
					CAMSV_REG_IMGO_FH_SPARE_3(reg_module));
#endif
...
irqreturn_t ISP_Irq_CAM_A(int Irq, void *DeviceId)
{
	...
	if (IrqStatus & SOF_INT_ST) {
		...
		if (frmPeriod == 0) {
			IRQ_LOG_KEEPER(module, m_CurrentPPB, _pr_err,
	...
	if (IrqStatus & SOF_INT_ST) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
...
#if (TIMESTAMP_QUEUE_EN == 1)
			{
				...
				/* Prevent WCNT increase after
				 * ISP_CompensateMissingSofTime around P1_DON
				 * and FBC_CNT decrease to 0, following drop
				 * frame is checked becomes true,
				 * then SOF timestamp will missing for current
				 * frame
				 */
				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_aao_])
				...
				if (IspInfo.TstpQInfo[module]
					...
					ISP_CompensateMissingSofTime(
					    reg_module, module, _imgo_, sec,
					    usec, frmPeriod);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_rrzo_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _rrzo_, sec,
					    usec, frmPeriod);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_ufeo_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _ufeo_, sec,
					    usec, frmPeriod);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_eiso_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _eiso_, sec,
					    usec, frmPeriod);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_lcso_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _lcso_, sec,
					    usec, frmPeriod);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_aao_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _aao_, sec,
					    usec, 1);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_afo_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _afo_, sec,
					    usec, 1);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_flko_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _flko_, sec,
					    usec, 1);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_pdo_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _pdo_, sec,
					    usec, 1);
				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_pso_])
					ISP_CompensateMissingSofTime(
					    reg_module, module, _pso_, sec,
					    usec, 1);
...
/* must after current ISP_PushBufTimestamp() */
			...
			if ((frmPeriod > 1) &&
				...
				if (IspInfo.TstpQInfo[module]
							.DmaEnStatus[_imgo_])
					ISP_PatchTimestamp(module, _imgo_,
						frmPeriod, cur_timestp,
						gPrevSofTimestp[module]);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_rrzo_])
					ISP_PatchTimestamp(module, _rrzo_,
						frmPeriod, cur_timestp,
						gPrevSofTimestp[module]);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_ufeo_])
					ISP_PatchTimestamp(module, _ufeo_,
						frmPeriod, cur_timestp,
						gPrevSofTimestp[module]);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_eiso_])
					ISP_PatchTimestamp(module, _eiso_,
						frmPeriod, cur_timestp,
						gPrevSofTimestp[module]);

				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_lcso_])
					ISP_PatchTimestamp(module, _lcso_,
						frmPeriod, cur_timestp,
						gPrevSofTimestp[module]);
			}
#endif
			...
			IRQ_LOG_KEEPER(
				module, m_CurrentPPB, _pr_info,
				"(0x%x_0x%x_0x%x)af(0x%x_0x%x_0x%x),pd(0x%x_0x%x_0x%x),ps(0x%x_0x%x_0x%x)\n",
			    ISP_RD32(CAM_REG_AAO_BASE_ADDR(reg_module)),
			    (unsigned int)(ISP_RD32(
...
#ifdef ENABLE_STT_IRQ_LOG
	if (DmaStatus &
	    (AAO_DONE_ST | AFO_DONE_ST | PDO_DONE_ST | PSO_DONE_ST)) {
		IRQ_LOG_KEEPER(module, m_CurrentPPB, _pr_info,
			       "CAMA_STT_Done_%d_0x%x\n",
	...
	if (IrqStatus & SOF_INT_ST) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			if (FrameStatus[module] != CAM_FST_DROP_FRAME) {
				...
				if (IspInfo.TstpQInfo[module]
					.DmaEnStatus[_eiso_])
					ISP_PushBufTimestamp(module, _eiso_,
						sec, usec, frmPeriod);
...
/* must after current ISP_PushBufTimestamp() */
			...
			IRQ_LOG_KEEPER(module, m_CurrentPPB, _LOG_INF,
"CAMB P1_SOF_%d_%d(0x%x_0x%x, 0x%x_0x%x, 0x%x, 0x%x, 0x%x),int_us:u%d, cq:0x%x\n",
				   sof_count[module], cur_v_cnt,
				   ISP_RD32(CAM_REG_FBC_IMGO_CTL1(reg_module)),
				   ISP_RD32(CAM_REG_FBC_IMGO_CTL2(reg_module)),
				   ISP_RD32(CAM_REG_FBC_RRZO_CTL1(reg_module)),
