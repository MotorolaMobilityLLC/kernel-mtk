...
struct MT6573FDVTDBuffRegMap {
	UINT32 u4Addr[MT6573FDVT_DBUFFREGCNT];
	UINT32 u4Data[MT6573FDVT_DBUFFREGCNT];
	UINT32 u4Counter;
};
#define MT6573FDVTDBuffRegMap struct MT6573FDVTDBuffRegMap
...
static int MT6573FDVT_ReadRegHW(MT6573FDVTRegIO *a_pstCfg)
{
	...
	for (i = 0; i < a_pstCfg->u4Count; i++) {
		if ((FDVT_ADDR + pMT6573FDVTRDBuff.u4Addr[i]) >= FDVT_ADDR &&
			...
			/*pMT6573FDVTRDBuff.u4Addr[i]),*/
			/*(u32) pMT6573FDVTRDBuff.u4Data[i]);*/
		} else {
...
static int MT6573FDVT_WaitIRQ(u32 *u4IRQMask)
{
	...
	timeout = wait_event_interruptible_timeout(g_MT6573FDVTWQ, (g_u4MT6573FDVTIRQMSK & g_u4MT6573FDVTIRQ),
		ms_to_jiffies(500));
...
static int FDVT_probe(struct platform_device *dev)
{
	...
	for (i = 0; i < FDVT_IRQ_IDX_NUM; i++) {
		...
		if (i == FDVT_IRQ_IDX) {
			...
			ret = request_irq(fdvt_dev->irq[i], (irq_handler_t)MT6573FDVT_irq, IRQF_TRIGGER_NONE,
			FDVT_DEVNAME,  NULL);
