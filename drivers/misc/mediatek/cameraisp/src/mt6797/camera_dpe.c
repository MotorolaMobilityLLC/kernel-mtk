...
typedef struct {
	...
	MINT32 RrameRDIdx;	/* Frame read Index */
...
static WMFE_REQUEST_RING_STRUCT g_WMFE_RequestRing;
...
#define DPE_WMFE_DPO_BASE_ADDR_0_HW   (DPE_BASE_HW + 0x250)
#define DPE_WMFE_DPO_STRIDE_0_HW      (DPE_BASE_HW + 0x254)
...
#define DPE_WMFE_DPO_BASE_ADDR_1_HW   (DPE_BASE_HW + 0x290)
#define DPE_WMFE_DPO_STRIDE_1_HW      (DPE_BASE_HW + 0x294)
...
#define DPE_WMFE_DPO_BASE_ADDR_2_HW   (DPE_BASE_HW + 0x2D0)
#define DPE_WMFE_DPO_STRIDE_2_HW      (DPE_BASE_HW + 0x2D4)

#define DPE_WMFE_DBG_INFO_00_HW       (DPE_BASE_HW + 0x2F4)
#define DPE_WMFE_DBG_INFO_01_HW       (DPE_BASE_HW + 0x2F8)
#define DPE_WMFE_DBG_INFO_02_HW       (DPE_BASE_HW + 0x2FC)
#define DPE_WMFE_DBG_INFO_03_HW       (DPE_BASE_HW + 0x300)
#define DPE_WMFE_DBG_INFO_04_HW       (DPE_BASE_HW + 0x304)
#define DPE_WMFE_DBG_INFO_05_HW       (DPE_BASE_HW + 0x308)
...
#define DPE_WMFE_DPO_BASE_ADDR_0_REG   (ISP_DPE_BASE + 0x250)
#define DPE_WMFE_DPO_STRIDE_0_REG      (ISP_DPE_BASE + 0x254)
...
#define DPE_WMFE_DPO_BASE_ADDR_1_REG   (ISP_DPE_BASE + 0x290)
#define DPE_WMFE_DPO_STRIDE_1_REG      (ISP_DPE_BASE + 0x294)
...
#define DPE_WMFE_DPO_BASE_ADDR_2_REG   (ISP_DPE_BASE + 0x2D0)
#define DPE_WMFE_DPO_STRIDE_2_REG      (ISP_DPE_BASE + 0x2D4)

#define DPE_WMFE_DBG_INFO_00_REG       (ISP_DPE_BASE + 0x2F4)
#define DPE_WMFE_DBG_INFO_01_REG       (ISP_DPE_BASE + 0x2F8)
#define DPE_WMFE_DBG_INFO_02_REG       (ISP_DPE_BASE + 0x2FC)
#define DPE_WMFE_DBG_INFO_03_REG       (ISP_DPE_BASE + 0x300)
#define DPE_WMFE_DBG_INFO_04_REG       (ISP_DPE_BASE + 0x304)
#define DPE_WMFE_DBG_INFO_05_REG       (ISP_DPE_BASE + 0x308)
				      ...
				      DPE_PROCESS_ID_ENUM whichReq, int ProcessID)
{
	...
	if (stus & DPE_DVE_INT_ST) {
		...
		LOG_ERR
		    (" WaitIRQ Status Error, type:%d, userNumber:%d, status:%d, whichReq:%d,ProcessID:0x%x, ReadReqIdx:%d\n",
...
static MBOOL ConfigDVERequest(MINT32 ReqIdx)
{
	...
	if (DPE_REQUEST_STATE_PENDING == g_DVE_RequestRing.DVEReq_Struct[ReqIdx].RequestState) {
...
static MBOOL UpdateDVE(MUINT32 DpeDveSta0, pid_t *ProcessID)
{
	...
	for (i = g_DVE_RequestRing.HWProcessIdx; i < _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_; i++) {
		if (DPE_REQUEST_STATE_RUNNING == g_DVE_RequestRing.DVEReq_Struct[i].RequestState) {
			for (j = 0; j < _SUPPORT_MAX_DPE_FRAME_REQUEST_; j++) {
...
static MBOOL ConfigWMFERequest(MINT32 ReqIdx)
{
	...
	spin_lock_irqsave(&(DPEInfo.SpinLockIrq[DPE_IRQ_TYPE_INT_DPE_ST]), flags);
	if (DPE_REQUEST_STATE_PENDING == g_WMFE_RequestRing.WMFEReq_Struct[ReqIdx].RequestState) {
		g_WMFE_RequestRing.WMFEReq_Struct[ReqIdx].RequestState = DPE_REQUEST_STATE_RUNNING;
		for (j = 0; j < _SUPPORT_MAX_DPE_FRAME_REQUEST_; j++) {
			if (DPE_FRAME_STATUS_ENQUE ==
			    g_WMFE_RequestRing.WMFEReq_Struct[ReqIdx].WmfeFrameStatus[j]) {
				g_WMFE_RequestRing.WMFEReq_Struct[ReqIdx].WmfeFrameStatus[j] =
				    DPE_FRAME_STATUS_RUNNING;
				spin_unlock_irqrestore(&
						       (DPEInfo.
							SpinLockIrq[DPE_IRQ_TYPE_INT_DPE_ST]),
						       flags);
				ConfigWMFEHW(&g_WMFE_RequestRing.WMFEReq_Struct[ReqIdx].
					     WmfeFrameConfig[j]);
				spin_lock_irqsave(&(DPEInfo.SpinLockIrq[DPE_IRQ_TYPE_INT_DPE_ST]),
						  flags);
			}
		...
		LOG_ERR("ConfigWMFERequest state machine error!!, ReqIdx:%d, RequestState:%d\n",
			ReqIdx, g_WMFE_RequestRing.WMFEReq_Struct[ReqIdx].RequestState);
	}
	spin_unlock_irqrestore(&(DPEInfo.SpinLockIrq[DPE_IRQ_TYPE_INT_DPE_ST]), flags);
...
static MINT32 ConfigDVEHW(DPE_DVEConfig *pDveConfig)
{
	...
	cmdqRecWrite(handle, DPE_INT_CTL_HW, 0x1, CMDQ_REG_MASK);	/* DPE Interupt read-clear mode */
	cmdqRecWrite(handle, DPE_DVE_INT_CTRL_HW, 0x1, CMDQ_REG_MASK);	/* DVE Interupt read-clear mode */
	...
	cmdqRecWrite(handle, DPE_DVE_START_HW, 0x1, CMDQ_REG_MASK);	/* DPE Interupt read-clear mode */

	cmdqRecWait(handle, CMDQ_EVENT_DPE_EOF);
	cmdqRecWrite(handle, DPE_DVE_START_HW, 0x0, CMDQ_REG_MASK);	/* DPE Interupt read-clear mode */
	...
	DPE_WR32(DPE_INT_CTL_REG, 0x1);	/* DPE Interupt read-clear mode */
	DPE_WR32(DPE_DVE_INT_CTRL_REG, 0x1);	/* DVE Interupt read-clear mode */
	...
	DPE_WR32(DPE_DVE_START_REG, 0x1);	/* DPE Interupt read-clear mode */
...
static MBOOL UpdateWMFE(pid_t *ProcessID)
{
	...
	for (i = g_WMFE_RequestRing.HWProcessIdx; i < _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_; i++) {
		if (DPE_REQUEST_STATE_RUNNING == g_WMFE_RequestRing.WMFEReq_Struct[i].RequestState) {
			for (j = 0; j < _SUPPORT_MAX_DPE_FRAME_REQUEST_; j++) {
				if (DPE_FRAME_STATUS_RUNNING ==
				    g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[j]) {
					break;
			...
			if (j != _SUPPORT_MAX_DPE_FRAME_REQUEST_) {
				...
				if ((_SUPPORT_MAX_DPE_FRAME_REQUEST_ == (next_idx))
				    || ((_SUPPORT_MAX_DPE_FRAME_REQUEST_ > (next_idx))
					&& (DPE_FRAME_STATUS_EMPTY ==
					    g_WMFE_RequestRing.WMFEReq_Struct[i].
					    WmfeFrameStatus[next_idx]))) {
					bFinishRequest = MTRUE;
					(*ProcessID) =
					    g_WMFE_RequestRing.WMFEReq_Struct[i].processID;
					g_WMFE_RequestRing.WMFEReq_Struct[i].RequestState =
					    DPE_REQUEST_STATE_FINISHED;
					g_WMFE_RequestRing.HWProcessIdx =
					    (g_WMFE_RequestRing.HWProcessIdx +
					...
					IRQ_LOG_KEEPER(DPE_IRQ_TYPE_INT_DPE_ST, m_CurrentPPB,
						       ...
						       i, j, g_WMFE_RequestRing.HWProcessIdx);
				} else {
					IRQ_LOG_KEEPER(DPE_IRQ_TYPE_INT_DPE_ST, m_CurrentPPB,
						       _LOG_DBG,
		...
		if (DPE_REQUEST_STATE_PENDING == g_WMFE_RequestRing.WMFEReq_Struct[i].RequestState) {
			...
			if (j != _SUPPORT_MAX_DPE_FRAME_REQUEST_) {
				...
				if ((_SUPPORT_MAX_DPE_FRAME_REQUEST_ == (next_idx))
				    || ((_SUPPORT_MAX_DPE_FRAME_REQUEST_ > (next_idx))
					&& (DPE_FRAME_STATUS_EMPTY ==
					    g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[next_idx]))) {
					bFinishRequest = MTRUE;
					(*ProcessID) =
					    g_WMFE_RequestRing.WMFEReq_Struct[i].processID;
					g_WMFE_RequestRing.WMFEReq_Struct[i].RequestState =
...
static MINT32 ConfigWMFEHW(DPE_WMFEConfig *pWmfeConfig)
{
	...
	/* Use command queue to write register */
	cmdqRecWrite(handle, DPE_INT_CTL_HW, 0x1, CMDQ_REG_MASK);	/* DPE Interupt read-clear mode */
	cmdqRecWrite(handle, DPE_WMFE_INT_CTRL_HW, 0x1, CMDQ_REG_MASK);	/* WMFE Interupt read-clear mode */

	if (WMFE_ENABLE == (pWmfeConfig->DPE_WMFE_CTRL_0 & WMFE_ENABLE)) {
		cmdqRecWrite(handle, DPE_WMFE_CTRL_0_HW, pWmfeConfig->DPE_WMFE_CTRL_0,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_SIZE_0_HW, pWmfeConfig->DPE_WMFE_SIZE_0,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_IMGI_BASE_ADDR_0_HW,
			     pWmfeConfig->DPE_WMFE_IMGI_BASE_ADDR_0, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_IMGI_STRIDE_0_HW, pWmfeConfig->DPE_WMFE_IMGI_STRIDE_0,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPI_BASE_ADDR_0_HW,
			     pWmfeConfig->DPE_WMFE_DPI_BASE_ADDR_0, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPI_STRIDE_0_HW, pWmfeConfig->DPE_WMFE_DPI_STRIDE_0,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_TBLI_BASE_ADDR_0_HW,
			     pWmfeConfig->DPE_WMFE_TBLI_BASE_ADDR_0, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_TBLI_STRIDE_0_HW, pWmfeConfig->DPE_WMFE_TBLI_STRIDE_0,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPO_BASE_ADDR_0_HW,
			     pWmfeConfig->DPE_WMFE_DPO_BASE_ADDR_0, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPO_STRIDE_0_HW, pWmfeConfig->DPE_WMFE_DPO_STRIDE_0,
			     CMDQ_REG_MASK);
	}

	if (WMFE_ENABLE == (pWmfeConfig->DPE_WMFE_CTRL_1 & WMFE_ENABLE)) {
		cmdqRecWrite(handle, DPE_WMFE_CTRL_1_HW, pWmfeConfig->DPE_WMFE_CTRL_1,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_SIZE_1_HW, pWmfeConfig->DPE_WMFE_SIZE_1,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_IMGI_BASE_ADDR_1_HW,
			     pWmfeConfig->DPE_WMFE_IMGI_BASE_ADDR_1, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_IMGI_STRIDE_1_HW, pWmfeConfig->DPE_WMFE_IMGI_STRIDE_1,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPI_BASE_ADDR_1_HW,
			     pWmfeConfig->DPE_WMFE_DPI_BASE_ADDR_1, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPI_STRIDE_1_HW, pWmfeConfig->DPE_WMFE_DPI_STRIDE_1,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_TBLI_BASE_ADDR_1_HW,
			     pWmfeConfig->DPE_WMFE_TBLI_BASE_ADDR_1, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_TBLI_STRIDE_1_HW, pWmfeConfig->DPE_WMFE_TBLI_STRIDE_1,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPO_BASE_ADDR_1_HW,
			     pWmfeConfig->DPE_WMFE_DPO_BASE_ADDR_1, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPO_STRIDE_1_HW, pWmfeConfig->DPE_WMFE_DPO_STRIDE_1,
			     CMDQ_REG_MASK);
	}

	if (WMFE_ENABLE == (pWmfeConfig->DPE_WMFE_CTRL_2 & WMFE_ENABLE)) {
		cmdqRecWrite(handle, DPE_WMFE_CTRL_2_HW, pWmfeConfig->DPE_WMFE_CTRL_2,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_SIZE_2_HW, pWmfeConfig->DPE_WMFE_SIZE_2,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_IMGI_BASE_ADDR_2_HW,
			     pWmfeConfig->DPE_WMFE_IMGI_BASE_ADDR_2, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_IMGI_STRIDE_2_HW, pWmfeConfig->DPE_WMFE_IMGI_STRIDE_2,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPI_BASE_ADDR_2_HW,
			     pWmfeConfig->DPE_WMFE_DPI_BASE_ADDR_2, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPI_STRIDE_2_HW, pWmfeConfig->DPE_WMFE_DPI_STRIDE_2,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_TBLI_BASE_ADDR_2_HW,
			     pWmfeConfig->DPE_WMFE_TBLI_BASE_ADDR_2, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_TBLI_STRIDE_2_HW, pWmfeConfig->DPE_WMFE_TBLI_STRIDE_2,
			     CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPO_BASE_ADDR_2_HW,
			     pWmfeConfig->DPE_WMFE_DPO_BASE_ADDR_2, CMDQ_REG_MASK);
		cmdqRecWrite(handle, DPE_WMFE_DPO_STRIDE_2_HW, pWmfeConfig->DPE_WMFE_DPO_STRIDE_2,
			     CMDQ_REG_MASK);
	}

	cmdqRecWrite(handle, DPE_WMFE_START_HW, 0x1, CMDQ_REG_MASK);	/* DPE Interupt read-clear mode */

	cmdqRecWait(handle, CMDQ_EVENT_WMF_EOF);
	cmdqRecWrite(handle, DPE_WMFE_START_HW, 0x0, CMDQ_REG_MASK);	/* DPE Interupt read-clear mode */
	...
	DPE_WR32(DPE_INT_CTL_REG, 0x1);	/* DPE Interupt read-clear mode */
	DPE_WR32(DPE_WMFE_INT_CTRL_REG, 0x1);	/* WMFE Interupt read-clear mode */

	if (WMFE_ENABLE == (pWmfeConfig->DPE_WMFE_CTRL_0 & WMFE_ENABLE)) {
		DPE_WR32(DPE_WMFE_CTRL_0_REG, pWmfeConfig->DPE_WMFE_CTRL_0);
		DPE_WR32(DPE_WMFE_SIZE_0_REG, pWmfeConfig->DPE_WMFE_SIZE_0);
		DPE_WR32(DPE_WMFE_IMGI_BASE_ADDR_0_REG, pWmfeConfig->DPE_WMFE_IMGI_BASE_ADDR_0);
		DPE_WR32(DPE_WMFE_IMGI_STRIDE_0_REG, pWmfeConfig->DPE_WMFE_IMGI_STRIDE_0);
		DPE_WR32(DPE_WMFE_DPI_BASE_ADDR_0_REG, pWmfeConfig->DPE_WMFE_DPI_BASE_ADDR_0);
		DPE_WR32(DPE_WMFE_DPI_STRIDE_0_REG, pWmfeConfig->DPE_WMFE_DPI_STRIDE_0);
		DPE_WR32(DPE_WMFE_TBLI_BASE_ADDR_0_REG, pWmfeConfig->DPE_WMFE_TBLI_BASE_ADDR_0);
		DPE_WR32(DPE_WMFE_TBLI_STRIDE_0_REG, pWmfeConfig->DPE_WMFE_TBLI_STRIDE_0);
		DPE_WR32(DPE_WMFE_DPO_BASE_ADDR_0_REG, pWmfeConfig->DPE_WMFE_DPO_BASE_ADDR_0);
		DPE_WR32(DPE_WMFE_DPO_STRIDE_0_REG, pWmfeConfig->DPE_WMFE_DPO_STRIDE_0);
	}

	if (WMFE_ENABLE == (pWmfeConfig->DPE_WMFE_CTRL_1 & WMFE_ENABLE)) {
		DPE_WR32(DPE_WMFE_CTRL_1_REG, pWmfeConfig->DPE_WMFE_CTRL_1);
		DPE_WR32(DPE_WMFE_SIZE_1_REG, pWmfeConfig->DPE_WMFE_SIZE_1);
		DPE_WR32(DPE_WMFE_IMGI_BASE_ADDR_1_REG, pWmfeConfig->DPE_WMFE_IMGI_BASE_ADDR_1);
		DPE_WR32(DPE_WMFE_IMGI_STRIDE_1_REG, pWmfeConfig->DPE_WMFE_IMGI_STRIDE_1);
		DPE_WR32(DPE_WMFE_DPI_BASE_ADDR_1_REG, pWmfeConfig->DPE_WMFE_DPI_BASE_ADDR_1);
		DPE_WR32(DPE_WMFE_DPI_STRIDE_1_REG, pWmfeConfig->DPE_WMFE_DPI_STRIDE_1);
		DPE_WR32(DPE_WMFE_TBLI_BASE_ADDR_1_REG, pWmfeConfig->DPE_WMFE_TBLI_BASE_ADDR_1);
		DPE_WR32(DPE_WMFE_TBLI_STRIDE_1_REG, pWmfeConfig->DPE_WMFE_TBLI_STRIDE_1);
		DPE_WR32(DPE_WMFE_DPO_BASE_ADDR_1_REG, pWmfeConfig->DPE_WMFE_DPO_BASE_ADDR_1);
		DPE_WR32(DPE_WMFE_DPO_STRIDE_1_REG, pWmfeConfig->DPE_WMFE_DPO_STRIDE_1);
	}

	if (WMFE_ENABLE == (pWmfeConfig->DPE_WMFE_CTRL_2 & WMFE_ENABLE)) {
		DPE_WR32(DPE_WMFE_CTRL_2_REG, pWmfeConfig->DPE_WMFE_CTRL_2);
		DPE_WR32(DPE_WMFE_SIZE_2_REG, pWmfeConfig->DPE_WMFE_SIZE_2);
		DPE_WR32(DPE_WMFE_IMGI_BASE_ADDR_2_REG, pWmfeConfig->DPE_WMFE_IMGI_BASE_ADDR_2);
		DPE_WR32(DPE_WMFE_IMGI_STRIDE_2_REG, pWmfeConfig->DPE_WMFE_IMGI_STRIDE_2);
		DPE_WR32(DPE_WMFE_DPI_BASE_ADDR_2_REG, pWmfeConfig->DPE_WMFE_DPI_BASE_ADDR_2);
		DPE_WR32(DPE_WMFE_DPI_STRIDE_2_REG, pWmfeConfig->DPE_WMFE_DPI_STRIDE_2);
		DPE_WR32(DPE_WMFE_TBLI_BASE_ADDR_2_REG, pWmfeConfig->DPE_WMFE_TBLI_BASE_ADDR_2);
		DPE_WR32(DPE_WMFE_TBLI_STRIDE_2_REG, pWmfeConfig->DPE_WMFE_TBLI_STRIDE_2);
		DPE_WR32(DPE_WMFE_DPO_BASE_ADDR_2_REG, pWmfeConfig->DPE_WMFE_DPO_BASE_ADDR_2);
		DPE_WR32(DPE_WMFE_DPO_STRIDE_2_REG, pWmfeConfig->DPE_WMFE_DPO_STRIDE_2);
	}

	DPE_WR32(DPE_WMFE_START_REG, 0x1);	/* DPE Interupt read-clear mode */
	...
	for (i = 0; i < _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_; i++) {
		LOG_INF
		    ("WMFE:RequestState:%d, processID:0x%08X, callerID:0x%08X, enqueReqNum:%d,\
		     ...
		     g_WMFE_RequestRing.WMFEReq_Struct[i].RequestState,\
		     g_WMFE_RequestRing.WMFEReq_Struct[i].processID,\
		     g_WMFE_RequestRing.WMFEReq_Struct[i].callerID,\
		     g_WMFE_RequestRing.WMFEReq_Struct[i].enqueReqNum,\
		     g_WMFE_RequestRing.WMFEReq_Struct[i].FrameWRIdx,\
		...
		for (j = 0; j < _SUPPORT_MAX_DPE_FRAME_REQUEST_;) {
			LOG_INF
			    ("WMFE:FrameStatus[%d]:%d, FrameStatus[%d]:%d, FrameStatus[%d]:%d, FrameStatus[%d]:%d\n",
			     ...
			     j + 2, g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[j + 2]
			     , j + 3, g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[j + 3]);
			j = j + 4;
		}
...
static MINT32 DPE_WaitIrq(DPE_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	if (WaitIrq->Clear == DPE_IRQ_CLEAR_STATUS) {
		...
		,WaitIrq->EventInfo.Clear,WaitIrq->Type,DPEInfo.IrqInfo.Status[WaitIrq->Type]); */
		/* DPEInfo.IrqInfo.Status[WaitIrq->Type][WaitIrq->EventInfo.UserKey] &=
	...
	if ((Timeout != 0)
		...
		LOG_DBG\
		    ("interrupted by system signal,return value(%d),\
		     irq Type/User/Sts/whichReq/Pid(0x%x/%d/0x%x/%d/%d)\n",\
	...
	if (Timeout == 0) {
		...
		if (WaitIrq->Clear == DPE_IRQ_WAIT_CLEAR) {
			...
			if (WaitIrq->Status & DPE_DVE_INT_ST) {
				...
				if (0 == DPEInfo.IrqInfo.DveIrqCnt)
					DPEInfo.IrqInfo.Status[WaitIrq->Type] &= (~WaitIrq->Status);
			} else if (WaitIrq->Status & DPE_WMFE_INT_ST) {
				DPEInfo.IrqInfo.WmfeIrqCnt--;
				if (0 == DPEInfo.IrqInfo.WmfeIrqCnt)
					DPEInfo.IrqInfo.Status[WaitIrq->Type] &= (~WaitIrq->Status);
			} else {
...
static long DPE_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case DPE_DVE_ENQUE_REQ:
		if (copy_from_user(&dpe_DveReq, (void *)Param, sizeof(DPE_DVERequest)) == 0) {
			...
			if (_SUPPORT_MAX_DPE_FRAME_REQUEST_ < dpe_DveReq.m_ReqNum) {
			...
			if (DPE_REQUEST_STATE_EMPTY ==
				...
				g_DVE_RequestRing.DVEReq_Struct[g_DVE_RequestRing.WriteIdx].
				    processID = pUserInfo->Pid;
				g_DVE_RequestRing.DVEReq_Struct[g_DVE_RequestRing.WriteIdx].
				    enqueReqNum = dpe_DveReq.m_ReqNum;
				...
				g_DVE_RequestRing.DVEReq_Struct[g_DVE_RequestRing.WriteIdx].
				    RequestState = DPE_REQUEST_STATE_PENDING;
				DveWriteIdx = g_DVE_RequestRing.WriteIdx;
				g_DVE_RequestRing.WriteIdx =
				    (g_DVE_RequestRing.WriteIdx +
				     1) % _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_;
				LOG_DBG("DVE request enque done!!\n");
			...
			if (copy_from_user(&dpe_DveReq, (void *)Param, sizeof(DPE_DVERequest)) == 0) {
				...
				for (idx = 0; idx < dequeNum; idx++) {
					if (DPE_FRAME_STATUS_FINISHED ==
						...
						LOG_ERR\
						    ("DPE Err! idx(%d), dequeNum(%d), ReadIdx,(%d), RrameRDIdx(%d),\
						     DveFrameStatus(%d)\n",\
						     idx, dequeNum, g_DVE_RequestRing.ReadIdx,\
						     g_DVE_RequestRing.\
						     DVEReq_Struct[g_DVE_RequestRing.ReadIdx].RrameRDIdx,\
						     g_DVE_RequestRing.DVEReq_Struct[g_DVE_RequestRing.ReadIdx].\
						     DveFrameStatus[g_DVE_RequestRing.\
								    DVEReq_Struct[g_DVE_RequestRing.ReadIdx].\
								    RrameRDIdx]);
	...
	case DPE_WMFE_ENQUE_REQ:
		{
			if (copy_from_user(&dpe_WmfeReq, (void *)Param, sizeof(DPE_WMFERequest)) ==
				...
				LOG_DBG("WMFE_ENQNUE_NUM:%d, pid:%d\n", dpe_WmfeReq.m_ReqNum,
					pUserInfo->Pid);
				if (_SUPPORT_MAX_DPE_FRAME_REQUEST_ < dpe_WmfeReq.m_ReqNum) {
				...
				if (DPE_REQUEST_STATE_EMPTY ==
					...
					WmfeWriteIdx = g_WMFE_RequestRing.WriteIdx;
					g_WMFE_RequestRing.WriteIdx =
					    (g_WMFE_RequestRing.WriteIdx +
					     1) % _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_;
			...
			if (copy_from_user(&dpe_WmfeReq, (void *)Param, sizeof(DPE_WMFERequest)) ==
				...
				for (idx = 0; idx < dequeNum; idx++) {
					if (DPE_FRAME_STATUS_FINISHED ==
						...
						LOG_ERR
						    ("WMFE Err! idx(%d), dequeNum(%d), ReadIdx,(%d),\
						    RrameRDIdx(%d), DveFrameStatus(%d)\n",\
						     idx, dequeNum, g_WMFE_RequestRing.ReadIdx,\
						     g_WMFE_RequestRing.\
						     WMFEReq_Struct[g_WMFE_RequestRing.ReadIdx].RrameRDIdx,\
						     g_WMFE_RequestRing.\
						     WMFEReq_Struct[g_WMFE_RequestRing.ReadIdx].\
						     WmfeFrameStatus[g_WMFE_RequestRing.WMFEReq_Struct\
								     [g_WMFE_RequestRing.ReadIdx].\
								     RrameRDIdx]);
					}
				}
...
static MINT32 DPE_open(struct inode *pInode, struct file *pFile)
{
	...
	for (i = 0; i < _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_; i++) {
		...
		g_DVE_RequestRing.DVEReq_Struct[i].RrameRDIdx = 0x0;
		/* WMFE */
		g_WMFE_RequestRing.WMFEReq_Struct[i].processID = 0x0;
		g_WMFE_RequestRing.WMFEReq_Struct[i].callerID = 0x0;
		g_WMFE_RequestRing.WMFEReq_Struct[i].enqueReqNum = 0x0;
		/* g_WMFE_RequestRing.WMFEReq_Struct[i].enqueIdx = 0x0; */
		g_WMFE_RequestRing.WMFEReq_Struct[i].RequestState = DPE_REQUEST_STATE_EMPTY;
		g_WMFE_RequestRing.WMFEReq_Struct[i].FrameWRIdx = 0x0;
		g_WMFE_RequestRing.WMFEReq_Struct[i].RrameRDIdx = 0x0;
		for (j = 0; j < _SUPPORT_MAX_DPE_FRAME_REQUEST_; j++) {
			g_DVE_RequestRing.DVEReq_Struct[i].DveFrameStatus[j] =
			    DPE_FRAME_STATUS_EMPTY;
			g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[j] =
	...
	g_WMFE_RequestRing.WriteIdx = 0x0;
	g_WMFE_RequestRing.ReadIdx = 0x0;
	g_WMFE_RequestRing.HWProcessIdx = 0x0;
...
static MINT32 DPE_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	LOG_INF\
	    ("DPE_mmap: pVma->vm_pgoff(0x%lx),pfn(0x%x),phy(0x%lx),\
	     pVmapVma->vm_start(0x%lx),pVma->vm_end(0x%lx),length(0x%lx)",\
	     pVma->vm_pgoff, pfn, pVma->vm_pgoff << PAGE_SHIFT, pVma->vm_start, pVma->vm_end,\
	     length);
...
static MINT32 DPE_probe(struct platform_device *pDev)
{
	...
	nr_DPE_devs += 1;
	DPE_devs = krealloc(DPE_devs, sizeof(struct DPE_device) * nr_DPE_devs, GFP_KERNEL);
	if (!DPE_devs) {
	...
	if (DPE_dev->irq > 0) {
		...
		for (i = 0; i < DPE_IRQ_TYPE_AMOUNT; i++) {
			if (0 == strcmp(pDev->dev.of_node->name, DPE_IRQ_CB_TBL[i].device_name)) {
				Ret =
...
static int dpe_dump_read(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_; i++) {
		seq_printf(m,
			   "DVE:RequestState:%d, processID:0x%08X, callerID:0x%08X, enqueReqNum:%d, FrameWRIdx:%d, RrameRDIdx:%d\n",
			   g_DVE_RequestRing.DVEReq_Struct[i].RequestState,
			   g_DVE_RequestRing.DVEReq_Struct[i].processID,
			   g_DVE_RequestRing.DVEReq_Struct[i].callerID,
			   g_DVE_RequestRing.DVEReq_Struct[i].enqueReqNum,
	...
	seq_printf(m, "WMFE:HWProcessIdx:%d, WriteIdx:%d, ReadIdx:%d\n",
		   g_WMFE_RequestRing.HWProcessIdx, g_WMFE_RequestRing.WriteIdx,
		   g_WMFE_RequestRing.ReadIdx);

	for (i = 0; i < _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_; i++) {
		seq_printf(m,
			   "WMFE:RequestState:%d, processID:0x%08X, callerID:0x%08X, enqueReqNum:%d, FrameWRIdx:%d, RrameRDIdx:%d\n",
			   g_WMFE_RequestRing.WMFEReq_Struct[i].RequestState,
			   g_WMFE_RequestRing.WMFEReq_Struct[i].processID,
			   g_WMFE_RequestRing.WMFEReq_Struct[i].callerID,
			   g_WMFE_RequestRing.WMFEReq_Struct[i].enqueReqNum,
		...
		for (j = 0; j < _SUPPORT_MAX_DPE_FRAME_REQUEST_;) {
			seq_printf(m,
				   ...
				   j, g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[j]
				   , j + 1,
				   g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[j + 1],
				   j + 2,
				   g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[j + 2]
				   , j + 3,
				   g_WMFE_RequestRing.WMFEReq_Struct[i].WmfeFrameStatus[j + 3]);
			j = j + 4;
		}
	}
...
static irqreturn_t ISP_Irq_DPE(MINT32 Irq, void *DeviceId)
{
	...
	IRQ_LOG_KEEPER(DPE_IRQ_TYPE_INT_DPE_ST, m_CurrentPPB, _LOG_INF,
		       ...
		       DPEInfo.WriteReqIdx, DPEInfo.ReadReqIdx);
	/* IRQ_LOG_KEEPER(DPE_IRQ_TYPE_INT_DPE_ST, m_CurrentPPB, _LOG_INF, "DveHWSta:0x%x, WmfeHWSta:0x%x,
	DpeDveSta0:0x%x\n", DveStatus, WmfeStatus, DpeDveSta0); */
