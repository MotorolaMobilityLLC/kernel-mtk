...
static inline void FD_Prepare_ccf_clock(void)
{
	...
	/* CG_MM_SMI_COMMON -> CG_SCP_SYS_ISP -> FD clk */
...
{
	/* must keep this clk close order: FD clk -> CG_SCP_SYS_ISP ->*/
...
static long FDVT_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FDVT_IOC_G_WAITIRQ:
		...
		/* BinChang 20120516 Close Interrupt */
		FDVT_WR32(0x00000000, FDVT_INT_EN);
		break;
	case FDVT_IOC_T_SET_FDCONF_CMD:
		/* LOG_DBG("[FDVT] FDVT set FD config\n"); */
		/* LDVT Disable, Due to the different */
		/* feature number between FD/SD/BD/REC */
		FaceDetecteConfig();
...
static int FDVT_release(struct inode *inode, struct file *file)
{
	...
	/* BinChang 20120517 Close Interrupt */
	FDVT_WR32(0x00000000, FDVT_INT_EN);
	g_FDVTIRQ = ioread32((void *)FDVT_INT);
 ...
 *#if 0
 *		// NEED_TUNING_BY_CHIP.
 *      // 1: Only one IORESOURCE_MEM type resource in
 ...
 */
	...
	/* For Linux 3.0 migration */
	/* replace mt65xx_irq_unmask() to enable_irq() */
	...
	fd_clk.CG_SCP_SYS_DIS =
		devm_clk_get(&dev->dev, "FD_CG_SCP_SYS_DIS");
	fd_clk.CG_MM_SMI_COMMON =
		devm_clk_get(&dev->dev, "FD_CG_MM_SMI_COMMON");
	fd_clk.CG_SCP_SYS_ISP =
		devm_clk_get(&dev->dev, "FD_CG_SCP_SYS_ISP");
	fd_clk.CG_IMGSYS_LAB5 =
		devm_clk_get(&dev->dev, "FD_CG_IMG_LARB5");
	fd_clk.CG_IMGSYS_FD =
		devm_clk_get(&dev->dev, "FD_CG_IMG_FD");
