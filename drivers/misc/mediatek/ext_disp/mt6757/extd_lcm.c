 ...
 * If we support dual lcm, please modify the dts, dtsi, configs and dws files as
 ...
 *
 * configs:
 *	CONFIG_CUSTOM_KERNEL_LCM="nt35595_fhd_dsi_cmd_truly_nt50358_extern
...
unsigned int lcm_get_width(void) { return extd_interface_params.width; }

unsigned int lcm_get_height(void) { return extd_interface_params.height; }
...
int lcm_get_dev_info(int is_sf, void *info)
{
	...
	if (is_sf == SF_GET_INFO) {
		...
		/*
		 *	LCM_LOG("DEV_INFO configuration displayHeight:%u,
		 *displayWidth:%u, mode:%u\n",
		 *			dispif_info->displayHeight,
		 *dispif_info->displayWidth, dispif_info->displayMode);
		 */
	}
...
int lcm_post_init(void)
{
	 ...
	 *rdma_irq_handler);
	 *		fence_release_task =
	 *kthread_create(fence_release_kthread,
	 *							NULL,
	 *"fence_release_kthread");
