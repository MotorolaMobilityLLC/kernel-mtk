...
enum HDMI_CAPABILITY {
	...
	/*bit3-bit6:channalcount;bit7-bit9:sample rate; bit10-bit11: bitwidth*/
...
struct _HDMI_EDID_T {
  ...
  /* use EDID_VIDEO_RES_T, there are many resolution */
	...
	/* use EDID_VIDEO_RES_T, only one NTSC resolution, Zero means none
	 * native NTSC resolution is available
	 */
	unsigned int ui4_sink_native_ntsc_resolution;
	/* use EDID_VIDEO_RES_T, only one resolution, Zero means none native PAL
	...
	unsigned int
	    ui4_sink_1st_dtd_ntsc_resolution;	 /* use EDID_VIDEO_RES_T */
	...
	unsigned int
	    ui4_hdmi_pcm_ch3ch4ch5ch7_type; /* use EDID_A_FMT_CH_TYPE1 */
	...
	unsigned char
	    ui1_sink_year_of_manufacture; /* (11H)  base on year 1990 */
};
