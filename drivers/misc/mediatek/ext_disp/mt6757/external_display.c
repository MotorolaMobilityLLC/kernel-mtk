...
static int _should_config_ovl_input(void)
{
	...
	/* if(pgc->mode == EXTD_SINGLE_LAYER_MODE ||pgc->mode ==
	 * EXTD_RDMA_DPI_MODE)
	 */
	/* return 0; */
	/* else */
			     ...
			     unsigned int userdata)
{
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER) &&
		...
		dpmgr_path_mutex_get(pgc->dpmgr_handle,
				     pgc->cmdq_handle_config);
	} else {
...
int ext_disp_init(char *lcm_name, unsigned int session)
{
	...
	if (ext_disp_use_cmdq == CMDQ_ENABLE) {
		if (DISP_SESSION_DEV(session) != DEV_EINK + 1) {
			...
			EXT_DISP_LOG("ext_disp display BUILD cmdq trigger loop "
				     "finished\n");
...
int ext_disp_suspend(unsigned int session)
{
	...
	if (pgc->state == EXTD_DEINIT || pgc->state == EXTD_SUSPEND ||
	    session != pgc->session) {
		EXT_DISP_ERR(
		    "status is not EXTD_RESUME or session is not match\n");
		goto done;
	}
...
int ext_fence_release_callback(unsigned long userdata)
{
	...
	external_display_util.hdmi_video_format_config(input_config_info &
						       0xff);
#endif
...
int ext_disp_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	if (layer_cnt == 1) {
		...
		if (ext_disp_get_ovl_req_status(session) ==
			...
			ret = m4u_config_port(&sPort);
			if (ret != 0)
				EXT_DISP_LOG("config M4U Port "
					     "DISP_MODULE_OVL1_2L FAIL\n");
		}
	}
...
int ext_disp_get_width(void) { return 0; }

int ext_disp_get_height(void) { return 0; }
...
int ext_disp_path_change(enum EXTD_OVL_REQ_STATUS action, unsigned int session)
{
	...
	break;
		...
		case EXTD_OVL_REMOVE_REQ:
			 ...
			 *pgc->cmdq_handle_config);
			 *ext_disp_path_set_mode(EXTD_RDMA_DPI_MODE,
			 *session);
			 *pgc->ovl_req_state =
			 *EXTD_OVL_REMOVE_REQ;
			 *}
		...
		case EXTD_OVL_INSERT_REQ:
			...
			*pgc->cmdq_handle_config);
			*ext_disp_path_set_mode(EXTD_DIRECT_LINK_MODE,
			*session);
			*pgc->ovl_req_state =
			*EXTD_OVL_INSERT_REQ;
			*}
