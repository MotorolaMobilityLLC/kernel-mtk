...
static unsigned int hdmi_resolution_param_table[][3] = {
	{720, 480, 60}, {1280, 720, 60}, {1920, 1080, 30}, {1920, 1080, 60},
};
...
static void hdmi_enable_dvfs(int enable) { mmdvfs_mhl_enable(enable); }
#else
...
int hdmi_allocate_hdmi_buffer(void)
{
	 ...
	 *hdmi_mva_r, hdmiBufferSize);
	 *#endif
	 */
	return 0;
}

int hdmi_free_hdmi_buffer(void) { return 0; }
...
int hdmi_wait_vsync_debug(int enable)
{
	...
	if (enable != 0 && hdmi_wait_vsync_task == NULL) {
		hdmi_wait_vsync_task = kthread_create(
		    hdmi_wait_vsync_kthread, NULL, "hdmi_wait_vsync_kthread");
...
unsigned int hdmi_get_width(void) { return p->hdmi_width; }

unsigned int hdmi_get_height(void) { return p->hdmi_height; }
		      ...
		      enum HDMI_VIDEO_OUTPUT_FORMAT vout)
{
	...
	if (vformat == HDMI_VIDEO_2160p_DSC_30Hz ||
		...
		wait_event_interruptible(
		    hdmi_video_config_wq,
		    atomic_read(&dcs_4ch_switch_done_event));
				   ...
				   unsigned int param)
{
	...
	if (param & 0x2) { /* start */
		...
		if (hdmi_params->cabletype ==
		    MHL_SMB_CABLE) { /* rdma1 register updated */
			hdmi_vsync_flag = 1;
...
static int hdmi_dcs_switch_to_4ch_kthread(void *data)
{
	...
	for (;;) {
		...
		/* dcs switch fail */
		HDMI_ERR("ERROR!!!!! dsc switch fail!!! channel=%d "
			 "dcs_status=%d ret=%d\n",
...
static enum HDMI_STATUS hdmi_drv_init(void)
{
	...
	if (!hdmi_3d_config_task) {
		hdmi_3d_config_task = kthread_create(
		    hdmi_3d_config_kthread, NULL, "hdmi_3d_config_kthread");
...
static void hdmi_resolution_setting(int arg)
{
	...
	if (hdmi_drv && hdmi_drv->get_params) {
		...
		if (dst_is_dsi) {
			...
			p->bg_height =
			    ((hdmi_params->height * p->scaling_factor) / 100 >>
			     2)
			    << 2;
			p->bg_width =
			    ((hdmi_params->width * p->scaling_factor) / 100 >>
			     2)
			    << 2;
			...
			if ((arg == HDMI_VIDEO_2160p_DSC_24Hz) ||
				...
				memset(&(extd_dpi_params.dispif_config.dsi
					     .dsc_params),
				...
				extd_dpi_params.dispif_config.dsi.dsc_params
				    .nfl_bpg_offset = 0xdb7; /*0x667*/
				extd_dpi_params.dispif_config.dsi.dsc_params
				    .slice_bpg_offset = 0x394; /*0x1ca*/
				extd_dpi_params.dispif_config.dsi.dsc_params
				    .final_offset = 0x10f0;
				extd_dpi_params.dispif_config.dsi.dsc_params
				    .line_bpg_offset = 0xc;
			...
			if ((arg == HDMI_VIDEO_2160p_DSC_24Hz) ||
			    (arg == HDMI_VIDEO_2160p_DSC_30Hz)) {
				memset(&(extd_dpi_params.dispif_config.dpi
					     .dsc_params),
				       0, sizeof(LCM_DSC_CONFIG_PARAMS));
...
int hdmi_recompute_bg(int src_w, int src_h)
{
	 ...
	 * src_h, p->output_video_resolution);
	 *	int physical_w = 0, physical_h = 0;
	 *	int temp = 0;
	 *	int ret = 0;	 *
	 *	int extd_path_state = ext_disp_is_alive();	 *
	 *	if ((extd_path_state == EXTD_DEINIT || extd_path_state ==
	 * EXTD_SUSPEND) || is_hdmi_active() == false) {
	 ...
	 *	default:
	 *		{
	 *			HDMI_LOG("hdmi_recompute_bg, resolution
	 * fail\n");
	 *			break;
	 *		}
	 *	}	 *
	 *	ret = hdmi_check_resolution(src_w, src_h, physical_w,
	 * physical_h);
	 *	if (ret >= 0) {
	 *		extd_dpi_params.dispif_config.dpi.bg_width = physical_w
	 * - src_w;
	 *		extd_dpi_params.dispif_config.dpi.bg_height = physical_h
	 * - src_h;	 *
...
int hdmi_set_resolution(int res)
{
	...
	HDMI_LOG("video resolution configuration, res:%d, old res:%ld, "
		 "video_on:%d\n",
	...
	if (res == HDMI_VIDEO_2160p_DSC_30Hz ||
		...
		HDMI_LOG("Notify the DCS switch to 4ch, and wake up "
			 "hdmi_dcs_switch_to_4ch_kthread\n");
	...
	/*if ddp path already start, stop ddp path, and release all fence of
	 * external session
	 */
...
int hdmi_get_dev_info(int is_sf, void *info)
{
	...
	if (is_sf == AP_GET_INFO) {
		 ...
		 * displayType-%d, HDMI support version:%u, 3d support %d\n",
		 *			dispif_info->displayType,
		 * dispif_info->isHDCPSupported, dispif_info->is3DSupport);
		 *		HDMI_LOG("DEV_INFO configuration
		 * displayHeight:%u, displayWidth:%u\n",
		 *			dispif_info->displayHeight,
...
void hdmi_udelay(unsigned int us) { udelay(us); }

void hdmi_mdelay(unsigned int ms) { msleep(ms); }
...
int hdmi_post_init(void)
{
	...
	if (hdmi_drv == NULL) {
		HDMI_ERR("[hdmi]%s, hdmi_init fail, can not get hdmi driver "
			 "handle\n",
			 __func__);
		return -1;
	}
