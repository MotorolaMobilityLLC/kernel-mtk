 ...
 *		ret = extd_driver[DEV_MHL]->ioctl(RECOMPUTE_BG_CMD, src_w,
 *src_h, NULL);
 *	else if (device_id == DEV_EINK && extd_driver[DEV_EINK]->ioctl)
 *		ret = extd_driver[DEV_EINK]->ioctl(RECOMPUTE_BG_CMD, src_w,
 *src_h, NULL);
 *
 *	return ret;
 *}
...
static int disp_switch_mode_kthread(void *data)
{
	...
	for (;;) {
		...
		MULTI_COTRL_LOG("switch mode, create or change path, mode:%d, "
				"session:0x%x\n",
				    ...
				    unsigned int device_id)
{
	...
	if ((session_id > 0 &&
		...
		if (path_info.old_session[device_id] == DISP_SESSION_EXTERNAL &&
		    mode >= DISP_SESSION_DIRECT_LINK_MIRROR_MODE) {
			/*1 layer cab be used, it is to say that the path is
			 * RDMA-DPI
			 */
