 ...
 *
 *	if (device_id >= DEV_MAX_NUM) {
 *		EXT_MGR_ERR("external_display_force_disable, device id is
 *invalid!");
 *		return;
 *	}
 *
			       ...
			       unsigned long arg)
{
	...
	case MTK_HDMI_AUDIO_VIDEO_ENABLE: {
		/* 0xXY
		 * the low 16 bits(Y) are for disable and enable, and the high
		 * 16 bits(X) are for device id
		 * the device id:
		 * X = 0 - mhl
		 * X = 1 - wifi display
		 */
	...
	case MTK_HDMI_FORCE_FULLSCREEN_ON:
		...
		{
			/* 0xXY
			 * the low 16 bits(Y) are for disable and enable, and
			 * the high 16 bits(X) are for device id
			 * the device id:
			 * X = 0 - mhl
			 * X = 1 - wifi display
			 */
...
static int mtk_extd_mgr_probe(struct platform_device *pdev)
{
	...
	/* For character driver register to system, device number binded to file
	 * operations
	 */
	...
	/* For device number binded to device name(hdmitx), one class is
	 * corresponeded to one node
	 */
	extd_class = class_create(THIS_MODULE, EXTD_DEVNAME);
	/* mknod /dev/hdmitx */
	class_dev = (struct class_device *)device_create(
	    extd_class, NULL, extd_devno, NULL, EXTD_DEVNAME);
...
const struct dev_pm_ops extd_pm_ops = {
	.suspend = extd_pm_suspend, .resume = extd_pm_resume,
};
#endif
