...
static int extd_get_device_type(unsigned int session)
{
	...
	if (dev_index >= 0 && extd_driver[dev_index] &&
		...
		ret = extd_driver[dev_index]->ioctl(GET_DEV_TYPE_CMD, 0, 0,
						    NULL);
	...
	if (DISP_SESSION_TYPE(session) == DISP_SESSION_MEMORY &&
		...
		if (path_info.old_session[device_id] ==
			...
			if (EXTD_OVERLAY_CNT < 1) {
				/*external display has no OVL to use, so no
		...
		if (extd_type != DISP_IF_EPD &&
			...
			if (ext_disp_wait_ovl_available(0) > 0 &&
				...
				if (path_info.old_session[device_id] ==
					...
					/*create external display path with
					 * OVL
					 */
...
static int disp_switch_mode_kthread(void *data)
{
	...
	for (;;) {
		...
		EXTDMSG(
			"switch mode, create or change path, mode:%d, session:0x%x\n",
				       ...
				       unsigned int device_id)
{
	...
	if (session_id == 0 &&
		...
		if (device_id == DEV_WFD) {
			...
			session = MAKE_DISP_SESSION(DISP_SESSION_EXTERNAL,
						    device_id);
		} else {
				    ...
				    unsigned int device_id)
{
	...
	if ((session_id > 0 &&
		...
		/*workaroud for HWC, get layers and set input layers not
		 * match
		 */
		if (path_info.old_session[device_id] ==
			DISP_SESSION_EXTERNAL &&
			     ...
			     unsigned int session)
{
	...
	if (ovl_status == EXTD_OVL_REMOVING) {
		...
		/*the new buffer configured, ovl already is inserted in the
		 * path
		 */
...
int external_display_suspend(unsigned int session)
{
	...
	if (session == 0) {
		for (i = DEV_MHL; i < DEV_MAX_NUM; i++) {
			if (i <= DEV_EINK)
				session_id = MAKE_DISP_SESSION(
				    DISP_SESSION_EXTERNAL, i + 1);
			else if (i == DEV_WFD)
				session_id =
				    MAKE_DISP_SESSION(DISP_SESSION_MEMORY, i);
			else
				session_id = MAKE_DISP_SESSION(
				    DISP_SESSION_EXTERNAL, i);
...
#ifndef OVL_CASCADE_SUPPORT
		switching =
		    path_change_without_cascade(mode, session_id[j], j);
#else
