...
void hdmi_force_resolution(int params)
{
	...
	EXTDMSG("%s params:0x%lx, 3d:%d\n", force_reschange,
		__func__, hdmi_params->is_3d_support);
}
...
void hdmi_cable_fake_plug_in(void)
{
	...
	if (p->is_force_disable == false) {
		if (IS_HDMI_STANDBY()) {
			...
			if (project_is_bsp == 1) {
				EXTDMSG(
					"[HDMIFake]set resolution for BSP project\n");
	...
	for (;;) {
		...
		while (i--) {
			...
			if (dcs_channel == 4 && dcs_status == 0) {
				...
				/* wake up hdmi_video_config_wq for video
				 * config
				 */
		...
		/* dcs switch fail */
		EXTDERR(
			"ERROR!!!!! dsc switch fail!!! channel=%d dcs_status=%d ret=%d\n",
...
int hdmi_set_resolution(int res)
{
	...
	EXTDFUNC();
	EXTDMSG(
		"video resolution configuration, res:%d, old res:%ld, video_on:%d\n",
	...
	if (res == HDMI_VIDEO_2160p_DSC_30Hz ||
		...
		EXTDMSG(
			"Notify the DCS switch to 4ch, and wake up hdmi_dcs_switch_to_4ch_kthread\n");
...
int hdmi_post_init(void)
{
	...
	static const struct EXTERNAL_DISPLAY_UTIL_FUNCS
	    external_display_utils = {
