...
void extd_disp_drv_set_util_funcs(
	const struct EXTERNAL_DISPLAY_UTIL_FUNCS *util)
{
...
static int _build_path_decouple(void) { return 0; }

static int _build_path_single_layer(void) { return 0; }
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (pgc->cmdq_handle_trigger == NULL) {
		ret = cmdqRecCreate(CMDQ_SCENARIO_TRIGGER_LOOP,
	...
	if (ext_disp_is_video_mode()) {
		...
		/* for operations before frame transfer, such as waiting for
		 * DSI
		 * TE
		 */
		dpmgr_path_build_cmdq(pgc->dpmgr_handle,
...
static int ext_disp_init_hdmi(unsigned int session)
{
	...
	ret =
	    cmdqRecCreate(CMDQ_SCENARIO_MHL_DISP, &(pgc->cmdq_handle_config));
	if (ret) {
...
static int ext_disp_init_lcm(char *lcm_name, unsigned int session)
{
	...
	if (ext_disp_use_cmdq == CMDQ_ENABLE) {
		ret = cmdqRecCreate(CMDQ_SCENARIO_MHL_DISP,
				    &(pgc->cmdq_handle_config));
		if (ret) {
			EXTDERR("cmdqRecCreate FAIL, ret=%d\n", ret);
			ret = EXT_DISP_STATUS_ERROR;
...
void ext_disp_esd_check_lock(void) { mutex_lock(&esd_check_lock); }

void ext_disp_esd_check_unlock(void) { mutex_unlock(&esd_check_lock); }
...
int ext_disp_esd_recovery(void)
{
	...
	if (pgc->state != EXTD_RESUME) {
		EXTDERR(
			"[ESD]esd recovery but external display path is slept??\n");
		goto done;
	}
...
void ext_disp_probe(void)
{
	...
	if (plcm_interface == NULL) {
		plcm_interface = disp_ext_lcm_probe(
		    ext_mtkfb_lcm_name, LCM_INTERFACE_NOTDEFINED, 0);
		if (plcm_interface == NULL)
...
static int ext_disp_suspend_release_fence(unsigned int session)
{
	...
	for (i = 0; i < EXTERNAL_SESSION_INPUT_LAYER_COUNT; i++) {
		DISPPR_FENCE("%s session=0x%x,layerid=%d\n",
			     __func__, session, i);
...
int ext_disp_resume(unsigned int session)
{
	...
	if (DISP_SESSION_DEV(session) == DEV_LCM) {
		...
		if (disp_lcm_is_video_mode(pgc->plcm)) {
			 ...
			 * DPREC_EVENT_CMDQ_SET_EVENT_ALLOW when trigger loop
			 * start
			 */
			if (_should_insert_wait_frame_done_token())
				_ext_cmdq_insert_wait_frame_done_token(
		     ...
		     unsigned int session)
{
	...
	if (pgc->mode == EXTD_DECOUPLE_MODE)
		ret = dpmgr_path_trigger(pgc->ovl2mem_path_handle, NULL,
					 ext_disp_use_cmdq);
	else if (DISP_SESSION_TYPE(session) == DISP_SESSION_EXTERNAL &&
		 DISP_SESSION_DEV(session) == DEV_MHL + 1)
		ret = _ext_disp_trigger(blocking, callback,
					atomic_read(&g_extd_trigger_ticket));
	else if (DISP_SESSION_TYPE(session) == DISP_SESSION_EXTERNAL &&
		 DISP_SESSION_DEV(session) == DEV_EINK + 1)
		ret = _ext_disp_trigger_EPD(
		    blocking, callback, atomic_read(&g_extd_trigger_ticket));
	else if (DISP_SESSION_TYPE(session) == DISP_SESSION_EXTERNAL &&
		 DISP_SESSION_DEV(session) == DEV_LCM)
		ret = _ext_disp_trigger_LCM(
		    blocking, callback, atomic_read(&g_extd_trigger_ticket));
	else
		goto done;
			     ...
			     unsigned int session)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->Extd_State,
			 MMPROFILE_FLAG_PULSE, Suspend, 0);
	...
	mmprofile_log_ex(ddp_mmp_get_events()->Extd_State,
			 MMPROFILE_FLAG_PULSE, Suspend, 1);
	return ret;
}
...
int ext_disp_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	if (layer_cnt == 1) {
		...
		if (ext_disp_get_ovl_req_status(session) ==
			...
			/* dpmgr_remove_ovl1_sub(pgc->dpmgr_handle,
			 * pgc->cmdq_handle_config);
			 */
		...
		if (ext_disp_get_ovl_req_status(session) ==
			...
			/* dpmgr_insert_ovl1_sub(pgc->dpmgr_handle,
			 * pgc->cmdq_handle_config);
			 */
	...
	if (_should_config_ovl_input()) {
		...
		ret = _convert_disp_input_to_rdma(
		    &(data_config->rdma_config), &(cfg->input_cfg[0]),
	...
	memcpy(&(data_config->dispif_config), &extd_lcm_params,
	       sizeof(struct LCM_PARAMS));
	ret = dpmgr_path_config(
	    pgc->dpmgr_handle, data_config,
	...
	cmdqRecBackupUpdateSlot(
	    pgc->cmdq_handle_config, pgc->ext_input_config_info, 0,
	    (input_source << 8) | (cfg->input_cfg[0].layer_type));
	...
	if (_should_config_ovl_input())
		cmdqRecBackupRegisterToSlot(
		    pgc->cmdq_handle_config, pgc->ext_ovl_rdma_status_info, 0,
		    disp_addr_convert(DDP_REG_BASE_DISP_OVL1 +
				      DISP_REG_OVL_STA));
	else
		cmdqRecBackupRegisterToSlot(
		    pgc->cmdq_handle_config, pgc->ext_ovl_rdma_status_info, 0,
		    disp_addr_convert(DISP_REG_RDMA_GLOBAL_CON +
...
enum CMDQ_SWITCH ext_disp_cmdq_enabled(void) { return ext_disp_use_cmdq; }
...
enum EXTD_POWER_STATE ext_disp_get_state(void) { return pgc->state; }
...
void *ext_disp_get_dpmgr_handle(void) { return pgc->dpmgr_handle; }
