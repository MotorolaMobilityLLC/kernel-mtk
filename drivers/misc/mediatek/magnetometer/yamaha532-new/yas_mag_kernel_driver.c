...
static struct i2c_board_info i2c_yamaha530 __initdata = { I2C_BOARD_INFO("yamaha530", (0x5c>>1))};
#endif
...
static int yamaha532_init_flag =  -1;
...
#endif
/*----------------------------------------------------------------------------*/
static const struct file_operations yamaha530_fops = {
	.owner = THIS_MODULE,
	.open = yamaha530_open,
...
static int yamaha530_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, yamaha530_attr_list[idx]);
		if (err) {
...
static int yamaha530_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	rt = misc_register(&yamaha530_device);
	if (rt) {
...
static int yamaha530_i2c_remove(struct i2c_client *client)
{
	...
	if (data != NULL) {
		...
		err = yamaha530_delete_attr(&(yamaha532_init_info.platform_diver_addr->driver));
		if (err)
...
{
	const char *name = "mediatek,yamaha532";
