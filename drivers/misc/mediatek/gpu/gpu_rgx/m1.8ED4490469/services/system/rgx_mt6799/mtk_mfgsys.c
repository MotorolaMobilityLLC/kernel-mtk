...
static IMG_VOID MTKEnableMfgClock(void)
{
	...
	buck_state = mt_gpufreq_voltage_enable_set(BUCK_ON);
	if (buck_state != 0x3) {
		PVR_DPF((PVR_DBG_ERROR, "MTKEnableMfgClock BUCK_ON failed"));
	...
	ged_log_buf_print2(_mtk_ged_log, GED_LOG_ATTR_TIME, "mfg0[+]");
	MTKCLK_prepare_enable(mtcmos_mfg0);
	ged_log_buf_print2(_mtk_ged_log, GED_LOG_ATTR_TIME, "mfg1[+]");
...
static IMG_VOID MTKDisableMfgClock(IMG_BOOL bForce)
{
	...
	if (bForce) /* Force buck off */
		buck_state = mt_gpufreq_voltage_enable_set(BUCK_ENFORCE_OFF);
...
{
	struct device *pdev;
	_mtk_ged_log = ged_log_buf_alloc(1024, 1024 * 32, GED_LOG_BUF_TYPE_RINGBUFFER, "PowerLog", "ppL");
