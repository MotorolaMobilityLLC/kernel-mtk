...
static int kbase_jit_allocate_process(struct kbase_jd_atom *katom)
{
	...
	if (!reg) {
		...
		if (kctx->jit_pending_free) {
			/* There are pending frees, so we should wait to
			 * see whether they free the memory. Add to the
			 * beginning of the list to ensure that the atom is
			 * processed only once in kbase_jit_free_finish
			 */
			list_add(&katom->queue, &kctx->jit_pending_alloc);

			/* Free the JIT ID so the reallocation later will
			 * succeed
			 */
