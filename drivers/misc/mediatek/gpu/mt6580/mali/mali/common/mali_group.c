...
static void mali_group_complete_pp_and_unlock(struct mali_group *group, mali_bool success, mali_bool in_upper_half)
{
	...
	if (!success) {
		MALI_DEBUG_PRINT(1, ("Mali group: Executing recovery reset due to PP job failure\n"));
...
static void mali_group_complete_gp_and_unlock(struct mali_group *group, mali_bool success)
{
	...
	if (!success) {
		MALI_DEBUG_PRINT(1, ("Mali group: Executing recovery reset due to GP job failure\n"));
		mali_group_recovery_reset(group);
...
static void mali_group_mmu_page_fault_and_unlock(struct mali_group *group)
{
	...
	if (NULL != group->pp_core) {
		...
		MALI_DEBUG_PRINT(1, ("Mali group: Executing PP reset due to MMU page fault\n"));
		...
		MALI_DEBUG_PRINT(1, ("Mali group: Executing GP reset due to MMU page fault\n"));
