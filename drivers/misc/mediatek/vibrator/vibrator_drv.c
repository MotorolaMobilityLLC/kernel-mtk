...
static void off_vibrator(struct work_struct *work)
{
	...
	if (vib_flag) {
		...
		pr_err(VIB_TAG "vibrator_notifier_call_chain CAM_EVENT_OFF\n");
		vib_flag = 0;
	}
}
...
static void vibrator_enable(unsigned int dur, unsigned int activate)
{
	...
	if (activate) {
		if (dur > VIBR_LONG_DUR && camera_detect_flag == CAM_EVENT_ON) {
			activate = 0;
		} else if (dur > VIBR_SHORT_DUR && camera_detect_flag == CAM_EVENT_OFF) {
                        cam_vcm_notifier_call_chain(CAM_VCM_EVENT_VIB_ON, NULL);
			vib_flag = 1;
			pr_err(VIB_TAG "vibrator_notifier_call_chain CAM_EVENT_ON\n");
		}
	}
...
static int vib_probe(struct platform_device *pdev)
{
	...
	if (rets) {
		printk("Unable to register vibrator_notifier: %d\n",ret);
	}
