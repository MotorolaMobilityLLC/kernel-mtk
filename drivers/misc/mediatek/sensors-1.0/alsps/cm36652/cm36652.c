...
#define APS_PR_ERR(fmt, args...)	pr_err(APS_TAG fmt, ##args)
...
EXIT_ERR:
	mutex_unlock(&cm36652_mutex);
	APS_PR_ERR("CM36652_i2c_master_operate fail\n");
	return res;
}
...
static ssize_t cm36652_show_config(struct device_driver *ddri, char *buf)
{
	...
	if (!cm36652_obj) {
		APS_PR_ERR("cm36652_obj is null!!\n");
		return 0;
	}
...
static int cm36652_check_intr(struct i2c_client *client)
{
	...
	if (databuf[1]&0x02)
		...
		res = -1;
		APS_PR_ERR("cm36652_check_intr fail databuf[1]&0x01: %d\n", res);
		goto EXIT_ERR;
	}

	return 0;
EXIT_ERR:
	APS_PR_ERR("cm36652_check_intr dev: %d\n", res);
	return res;
}
...
static void cm36652_eint_work(struct work_struct *work)
{
	...
	if (res != 0)
		APS_PR_ERR("cm36652_eint_work err: %d\n", res);
...
static int cm36652_irq_handler(void *data, uint len)
{
	...
	APS_LOG("len = %d, type = %d, action = %d, errCode = %d\n",
	...
	case SENSOR_HUB_NOTIFY:
		...
		case SCP_NOTIFY:
			if (rsp->notify_rsp.data[0] == CM36652_NOTIFY_PROXIMITY_CHANGE) {
				...
				APS_PR_ERR("Unknown notify");
			break;
		default:
			APS_PR_ERR("Error sensor hub notify");
			break;
		}
		break;
	default:
		APS_PR_ERR("Error sensor hub action");
		break;
	}
...
static int als_enable_nodata(int en)
{
	...
	if (atomic_read(&cm36652_obj->init_done)) {
		...
		APS_PR_ERR("sensor hub has not been ready!!\n");
...
static int als_get_data(int *value, int *status)
{
	...
	if (err)
		APS_PR_ERR("SCP_sensorHub_req_send fail!\n");
	else {
	...
	if (atomic_read(&cm36652_obj->trace) & CMC_TRC_PS_DATA)
		...
		APS_PR_ERR("sensor hub hat not been ready!!\n");
		err = -1;
	}
#else /* #ifdef CUSTOM_KERNEL_SENSORHUB */
	if (!cm36652_obj) {
...
static int cm36652_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		APS_PR_ERR("cm36652_delete_attr fail: %d\n", err);
