					 ...
					 unsigned long arg)
{
	...
	uint32_t pdata = 0;
	int als_cali = 0;
	int als_target_lux = 0;
#ifdef PENANG_4G_FACTORY
	uint32_t alsdata[3] = {0};
#endif
	...
	case ALSPS_GET_ALS_RAW_DATA:
		if (alsps_factory.fops != NULL &&
			...
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
#else
			err = alsps_factory.fops->als_get_raw_data(alsdata);
			if (err < 0) {
				pr_err(
					"ALSPS_GET_ALS_RAW_DATA read alsdata fail!\n");
				return -EINVAL;
			}
			if (copy_to_user(ptr, alsdata, sizeof(alsdata)))
				return -EFAULT;
			pr_err("ALSPS_GET_ALS_RAW_DATA read alsdata lux:%d,CH0:%d,CH1:%d\n",
						alsdata[0], alsdata[1], alsdata[2]);
#endif
		} else {
	...
	case ALSPS_PS_ENABLE_CALI:
		...
		return 0;
	case ALSPS_ALS_SET_TARGET_LUX:
		if (copy_from_user(&als_target_lux, ptr, sizeof(als_target_lux)))
			return -EFAULT;
		if (alsps_factory.fops != NULL &&
			alsps_factory.fops->als_set_target_lux != NULL) {
			err = alsps_factory.fops->als_set_target_lux(als_target_lux);
			if (err < 0) {
				pr_err("ALSPS_ALS_SET_TARGET_LUX FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_err("ALSPS_ALS_SET_TARGET_LUX NULL\n");
			return -EINVAL;
		}
		return 0;
	case ALSPS_PS_SET_CALI_DATA:
		if (copy_from_user(&pdata, ptr, sizeof(pdata)))
			return -EFAULT;
		if (alsps_factory.fops != NULL &&
			alsps_factory.fops->ps_set_calidata != NULL) {
			err = alsps_factory.fops->ps_set_calidata(pdata);
			if (err < 0) {
				pr_err("ALSPS_PS_SET_CALI_DATA FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_err("ALSPS_PS_SET_CALI_DATA NULL\n");
			return -EINVAL;
		}
		return 0;
	default:
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_ALSPS_ALS_SET_TARGET_LUX:
	case CMOPAT_ALSPS_PS_SET_CALI_DATA:
