...
#ifndef PENANG_4G_FACTORY
	*als = data_t.light;
#else
	*als = data_t.light_t.lux;
#endif
...
static int als_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	if (event->flush_action == FLUSH_ACTION)
		...
		err = als_data_report_t(event->light,
				...
				(int64_t)event->time_stamp);
#else
		err = als_data_report_t(event->light_t.lux,
				SENSOR_STATUS_ACCURACY_MEDIUM,
				(int64_t)event->time_stamp);
#endif
	else if (event->flush_action == CALI_ACTION) {
...
static int alshub_factory_get_data(int32_t *data)
{
	...
	*data = data_t.light;
#else
	data[0] = data_t.light_t.lux;
	data[1] = data_t.light_t.photopic;
	data[2] = data_t.light_t.als_ir;
	pr_err("alshub_factory_get_data lux:%d, CH0:%d, CH1:%d\n",data[0], data[1], data[2]);
#endif
	return 0;
}
...
static int alshub_factory_set_target_lux(int32_t target_lux)
{
	int err = 0;
	int32_t cfg_data[2] = {0};

	if (target_lux <= 0) {
		pr_err("PS set target lux fail! invalid value:[%d]\n", target_lux);
		return -1;
	}
	cfg_data[1] = target_lux;
	pr_err("PS set target lux:[%d]\n", target_lux);
	err = sensor_cfg_to_hub(ID_LIGHT,
		(uint8_t *)&cfg_data, sizeof(cfg_data));
	if (err < 0)
		pr_err("sensor_cfg_to_hub fail\n");
...
static int pshub_factory_set_calidata(uint32_t value)
{
	int err = 0;
	uint32_t cfg_data[4] = {0};
	uint32_t rawdata = value;
	static uint32_t cover = 0;
	static uint32_t uncover = 0;
	uint32_t pdata[2] = {0};

	if (rawdata >> 31) {
		cover = rawdata & 0x0000FFFF; // 2.2cm cover rawdata
		pr_err("pshub revcive 2.2cm cover rawdata:%d %d\n", cover, uncover);
	} else {
		uncover = rawdata & 0x0000FFFF; // uncover rawdata
		pr_err("pshub recive uncover rawdata:%d %d\n", uncover, cover);
	}
	if ( cover > 0 &&  uncover > 0 && cover > uncover) {
		pdata[0] = cover;
		pdata[1] = uncover;
		cover = 0;
		uncover = 0;
		pr_err("pshub set cover:%d, uncover:%d send to scp...\n",
				pdata[0], pdata[1]);
	} else {
		if (rawdata >> 31)
			pr_err("miss uncover parameter!\n");
		else
			pr_err("miss cover parameter!\n");
		return 0;
	}

	if (pdata[0] < pdata[1] || pdata[0] <= 0 || pdata[1] <= 0) {
		pr_err("PS set pdata fail! invalid value:[%d, %d]\n",
			pdata[0], pdata[1]);
		return -1;
	}

	cfg_data[2] = pdata[0]; // 2.2cm cover raw data
	cfg_data[3] = pdata[1]; // uncover raw data
	pr_err("PS set pdata value:[%d, %d]\n", pdata[0], pdata[1]);
	err = sensor_cfg_to_hub(ID_PROXIMITY,
		(uint8_t *)cfg_data, sizeof(cfg_data));
	if (err < 0)
		pr_err("sensor_cfg_to_hub fail\n");
...
static struct alsps_factory_fops alspshub_factory_fops = {
	...
	.als_set_target_lux = alshub_factory_set_target_lux,
	...
	.ps_set_calidata = pshub_factory_set_calidata,
};
