...
static int CM36558_check_intr(struct i2c_client *client)
{
	...
	pr_debug("CM36558_REG_PS_DATA value value_low = %x, value_reserve = %x\n", databuf[0], databuf[1]);
...
static long CM36558_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_SET_PS_THRESHOLD:
		...
		APS_PR_ERR("%s set threshold high: 0x%x, low: 0x%x\n", __func__, threshold[0], threshold[1]);
	...
	case ALSPS_GET_PS_THRESHOLD_HIGH:
		...
		APS_PR_ERR("%s get threshold high: 0x%x\n", __func__, threshold[0]);
	...
	case ALSPS_GET_PS_THRESHOLD_LOW:
		...
		APS_PR_ERR("%s get threshold low: 0x%x\n", __func__, threshold[0]);
		if (copy_to_user(ptr, &threshold[0], sizeof(threshold[0]))) {
			err = -EFAULT;
			goto err_out;
		}
...
static int CM36558_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err) {
		APS_PR_ERR("register light batch support err = %d\n", err);
		goto exit_sensor_obj_attach_fail;
	}

	err = batch_register_support_info(ID_PROXIMITY, ps_ctl.is_support_batch, 100, 0);
	if (err) {
		APS_PR_ERR("register proximity batch support err = %d\n", err);
		goto exit_sensor_obj_attach_fail;
	}
#endif
	CM36558_init_flag = 0;
