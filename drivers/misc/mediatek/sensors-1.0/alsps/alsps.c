...
static int als_get_screen_id(void)
{
	int screen_id = 0;
	/* match screen id */
	if (strstr(saved_command_line, "ili9883a_hdp_dsi_vdo_txd")) {
		screen_id = TXD_ID;
	} else if (strstr(saved_command_line, "icnl9916_hdp_dsi_vdo_tm"))  {
		screen_id = TM_ID;
	} else if (strstr(saved_command_line, "td4160_hdp_dsi_vdo_boe")) {
		screen_id = BOE_ID;
	} else if (strstr(saved_command_line, "ili7807s_hdp_dsi_vdo_tm")) {
		screen_id = TM4_ID;
	} else {
		screen_id = MAX_ID;
		pr_err("als match screen id fail");
	}
	return screen_id;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cali_buf = vzalloc(count + sizeof(int));
	if (!cali_buf)
		return -ENOMEM;
	memcpy(cali_buf, buf, count);
	screen_id = als_get_screen_id();
	memcpy(cali_buf+count, &screen_id, sizeof(int));
	pr_err("match screen id:%d\n", screen_id);

	mutex_lock(&alsps_context_obj->alsps_op_mutex);
	cxt = alsps_context_obj;
	if (cxt->als_ctl.set_cali != NULL)
		err = cxt->als_ctl.set_cali(cali_buf, count + sizeof(int));
	if (err < 0)
