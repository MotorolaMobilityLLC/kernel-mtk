...
#define HMDYHUB_FUN(f)               pr_debug(HMDYHUB_TAG"%s\n", __func__)
#define HMDYHUB_PR_ERR(fmt, args...) \
	pr_err(HMDY_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
#define HMDYHUB_LOG(fmt, args...)    pr_debug(HMDYHUB_TAG fmt, ##args)
...
static int hmdyhub_set_powermode(bool enable)
{
	...
	if (!atomic_read(&obj->scp_init_done)) {
		HMDYHUB_PR_ERR("sensor hub has not been ready!!\n");
		return -1;
	}
	err = sensor_enable_to_hub(ID_RELATIVE_HUMIDITY, enable);
	if (err < 0)
		HMDYHUB_PR_ERR("sensor_enable_to_hub fail!\n");
...
static int hmdyhub_get_humidity(char *buf, int bufsize)
{
	...
	if (err < 0) {
		HMDYHUB_PR_ERR("sensor_get_data_from_hub fail!\n");
		return err;
	}
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	if (err < 0) {
		HMDYHUB_PR_ERR("hmdyhub_set_powermode fail!!\n");
		return 0;
	}
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj == NULL) {
		HMDYHUB_PR_ERR("pointer is null\n");
		return 0;
	}
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (res == 0) {
		...
		if (res < 0) {
			HMDYHUB_PR_ERR("sensor_set_cmd_to_hub fail, (ID: %d),(action: %d)\n",
				ID_RELATIVE_HUMIDITY, CUST_ACTION_SET_TRACE);
			return 0;
		}
	} else {
		HMDYHUB_PR_ERR("invalid content: '%s', length = %d\n", buf, (int)count);
	}
	return count;
}
...
static int hmdyhub_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		if (err) {
			HMDYHUB_PR_ERR("driver_create_file (%s) = %d\n", hmdyhub_attr_list[idx]->attr.name, err);
			break;
		}
	}
	return err;
...
static long hmdyhub_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (err) {
		HMDYHUB_PR_ERR("access error: %08X, (%2d, %2d)\n", cmd, _IOC_DIR(cmd), _IOC_SIZE(cmd));
		return -EFAULT;
	}
	...
	case HUMIDITY_IOCTL_INIT:
		...
		if (err) {
			HMDYHUB_PR_ERR("sensor_set_delay_to_hub failed!\n");
			break;
		}
		break;
	...
	case HUMIDITY_GET_HMDY_DATA:
		...
		err = hmdyhub_get_humidity(strbuf, HMDYHUB_BUFSIZE);
		if (err < 0) {
			HMDYHUB_PR_ERR("hmdyhub_get_humidity fail\n");
			break;
		}
	...
	case HUMIDITY_GET_TEMP_DATA:
		...
		if (err < 0) {
			HMDYHUB_PR_ERR("hmdyhub_get_temperature fail\n");
			break;
		}
		dat = 0;
		if (copy_to_user(data, &dat, sizeof(dat))) {
	...
	default:
		HMDYHUB_PR_ERR("unknown IOCTL: 0x%08x\n", cmd);
		err = -ENOIOCTLCMD;
		break;
	}
...
static int hmdyhub_set_delay(u64 ns)
{
	...
	if (atomic_read(&obj->scp_init_done)) {
		...
		if (err < 0) {
			HMDYHUB_PR_ERR("als_set_delay fail!\n");
			return err;
		}
	} else {
		HMDYHUB_PR_ERR("sensor hub has not been ready!!\n");
...
static int hmdyhub_get_data(int *value, int *status)
{
	...
	if (err) {
		HMDYHUB_PR_ERR("get compensated humidity value failed, err = %d\n", err);
		return -1;
	}
...
static int hmdyhub_probe(struct platform_device *pdev)
{
	...
	if (err) {
		HMDYHUB_PR_ERR("misc device register failed, err = %d\n", err);
	...
	if (err) {
		HMDYHUB_PR_ERR("create attribute failed, err = %d\n", err);
	...
	if (err) {
		HMDYHUB_PR_ERR("register hmdy control path err\n");
		goto exit_create_attr_failed;
	}
	...
	if (err) {
		HMDYHUB_PR_ERR("hmdy_register_data_path failed, err = %d\n", err);
		goto exit_create_attr_failed;
	}
...
static int hmdyhub_remove(struct platform_device *pdev)
{
	...
	if (err)
		HMDYHUB_PR_ERR("hmdyhub_delete_attr failed, err = %d\n", err);
...
static int hmdyhub_local_init(void)
{
	if (platform_driver_register(&hmdyhub_driver)) {
		HMDYHUB_PR_ERR("add driver error\n");
		return -1;
	}
...
static int __init hmdyhub_init(void)
{
	...
	if (platform_device_register(&hmdyhub_device)) {
		HMDYHUB_PR_ERR("hmdy platform device error\n");
		return -1;
	}
