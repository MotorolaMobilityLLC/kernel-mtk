...
#endif
		{
			GYRO_PR_ERR("read power ctl 2 register err!\n");
	} else {
		if (atomic_read(&obj->trace) & GYRO_TRC_INFO)
...
static int MPU6515_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if ((databuf[0] & 0x1f) != 0x1)
		GYRO_PR_ERR("MPU6515 PWR_MGMT_1 = %x\n", databuf[0]);
...
static int MPU6515_ReadFifoData(struct i2c_client *client, s16 *data, int *datalen)
{
	...
	*datalen = packet_cnt;
	...
	if (packet_cnt && (abs(packet_thresh - packet_cnt) < 4)) {
...
#endif
				{
					GYRO_PR_ERR("MPU6515 read data from FIFO error: %d\n", err);
					return -2;
			} else {
		...
		GYRO_PR_ERR("MPU6515 Incorrect packet count: %d\n", packet_cnt);
		return -3;
	}
...
#endif
		{
			GYRO_PR_ERR("MPU6515 read temperature data  error\n");
			return -2;
	} else {
...
#endif
		{
			GYRO_PR_ERR("MPU6515 read gyroscope data  error\n");
			return -2;
	} else {
...
static int MPU6515_SMTReadSensorData(struct i2c_client *client, s16 *buf, int bufsize)
{
	...
	for (i = 0; i < MPU6515_AXES_NUM; i++) {
		...
		if (res) {
			GYRO_PR_ERR("MPU6515_FIFOConfig error:%d!\n", res);
			return -3;
		}
		...
		if (res) {
			if (res == (-3)) {
				...
				GYRO_PR_ERR("MPU6515_ReadData error:%d!\n", res);
				return -3;
			}
		} else {
			buf[0] = datalen;
...
#else
	pinctrl = devm_pinctrl_get(&mpu6515_i2c_client->dev);
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
...
static int gyroscope_enable_nodata(int en)
{
	...
	if (err != MPU6515_SUCCESS) {
		GYRO_PR_ERR("gsensor_enable_nodata fail!\n");
		return -1;
	}
...
/*----------------------------------------------------------------------------*/
static int mpu6515g_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err = 0;
...
#endif
	err = mpu6515_init_client(mpu6515_i2c_client, false);
	if (err)
...
static int mpu6515g_factory_get_raw_data(int32_t data[3])
{
	char *strbuf;
#if 0
	...
	MPU6515_ReadGyroDataRaw(mpu6515_i2c_client, strbuf, MPU6515_BUFSIZE);
...
static int mpu6515g_factory_enable_calibration(void)
{
	return 0;
}
static int mpu6515g_factory_clear_cali(void)
{
	int err = 0;
	...
	if (err) {
		GYRO_INFO("mpu6515g_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int mpu6515g_factory_set_cali(int32_t data[3])
{
	int err = 0;
	int cali[3] = { 0 };
#if 0
...
#endif
	cali[MPU6515_AXIS_X] = data[0] * MPU6515_DEFAULT_LSB / MPU6515_FS_MAX_LSB;
	cali[MPU6515_AXIS_Y] = data[1] * MPU6515_DEFAULT_LSB / MPU6515_FS_MAX_LSB;
	cali[MPU6515_AXIS_Z] = data[2] * MPU6515_DEFAULT_LSB / MPU6515_FS_MAX_LSB;
	GYRO_LOG("gyro set cali:[%5d %5d %5d]\n", cali[MPU6515_AXIS_X],
		 cali[MPU6515_AXIS_Y], cali[MPU6515_AXIS_Z]);
	err = MPU6515_WriteCalibration(mpu6515_i2c_client, cali);
	if (err) {
		GYRO_INFO("mpu6515g_WriteCalibration failed!\n");
		return -1;
	}

	return 0;
}
static int mpu6515g_factory_get_cali(int32_t data[3])
{
	int err = 0;
	int cali[3] = { 0 };
#if 0
	...
	if (err) {
		GYRO_INFO("mpu6515g_ReadCalibration failed!\n");
		return -1;
	}
	data[0] = cali[MPU6515_AXIS_X] * MPU6515_FS_MAX_LSB / MPU6515_DEFAULT_LSB;
	data[1] = cali[MPU6515_AXIS_Y] * MPU6515_FS_MAX_LSB / MPU6515_DEFAULT_LSB;
	data[2] = cali[MPU6515_AXIS_Z] * MPU6515_FS_MAX_LSB / MPU6515_DEFAULT_LSB;

	return 0;
}
static int mpu6515g_factory_do_self_test(void)
{
	return 0;
}

static struct gyro_factory_fops mpu6515g_factory_fops = {
	.enable_sensor = mpu6515g_factory_enable_sensor,
	.get_data = mpu6515g_factory_get_data,
	.get_raw_data = mpu6515g_factory_get_raw_data,
	.enable_calibration = mpu6515g_factory_enable_calibration,
	.clear_cali = mpu6515g_factory_clear_cali,
	.set_cali = mpu6515g_factory_set_cali,
	.get_cali = mpu6515g_factory_get_cali,
	.do_self_test = mpu6515g_factory_do_self_test,
};

static struct gyro_factory_public mpu6515g_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &mpu6515g_factory_fops,
};
/*----------------------------------------------------------------------------*/
static int mpu6515_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
...
static int mpu6515_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = gyro_factory_device_register(&mpu6515g_factory_device);
	if (err) {
		GYRO_PR_ERR("mpu6515_device misc register failed!\n");
		goto exit_misc_device_register_failed;
	}

	err = mpu6515_create_attr(&gyroscope_init_info.platform_diver_addr->driver);
	if (err) {
		GYRO_PR_ERR("mpu6515 create attribute err = %d\n", err);
	...
	if (err) {
		GYRO_PR_ERR("register acc data path err\n");
		goto exit_kfree;
	}
...
static int mpu6515_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		GYRO_PR_ERR("mpu6515_delete_attr fail: %d\n", err);


	/* err = misc_deregister(&mpu6515_device); */
	err = gyro_factory_device_deregister(&mpu6515g_factory_device);
	if (err)
