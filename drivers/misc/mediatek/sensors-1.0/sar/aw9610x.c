...
#define AW9610x_DEV_NAME "sar"

static struct sensorInfo_NonHub_t sar_devinfo;
...
#define GET_CHX_STATUS(value, move) ((value >> move) & 0x01)

#ifdef AW_USB_PLUG_CAIL
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
#define USB_POWER_SUPPLY_NAME   "sgm4154x-charger"
#else
#define USB_POWER_SUPPLY_NAME   "usb"
#endif
#endif

static uint32_t report_data[3] = {0};
                        ...
                        const char *buf, size_t count)
{
    ...
    if (state != 0) {
        aw9610x_i2c_write_bits(aw9610x, REG_SCANCTRL0, ~(0x3f << 8),
                            (data_en & 0x3f) << 8);
        if (report_data[0] == 0) {
            sar_data_report_t(report_data, 0);
            AWLOGD(aw9610x->dev, "cali report_data = 0x%02x",report_data[0]);
        }
        AWLOGE(aw9610x->dev, "auto cali start ...");
    } else {
        AWLOGE(aw9610x->dev, "fail to set aot cali");
    }
    ...
    if (aw9610x->channels_arr == NULL) {
     ...
     for (i = 0; i < AW_CHANNEL_MAX; i++) {
         curr_status = curr_status |
                         (((uint8_t)(curr_status_val >> 24 ) & 0xffff));
     }

    report_data[0] = GET_CHX_STATUS(curr_status, 1) << 0;  //get channel 1 status
    report_data[0] |= GET_CHX_STATUS(curr_status, 3) << 1; //get channel 3 status
    report_data[0] |= GET_CHX_STATUS(curr_status, 4) << 2; //get channel 4 status
    AWLOGD(aw9610x->dev, "report_data = 0x%02x",report_data[0]);
               ...
               struct device_node *np)
{
    ...
    val = of_property_read_u32(np, "ca-num", &aw9610x->ca_num);
    if (val != 0) {
        AWLOGE(aw9610x->dev, "multiple sar failed!");
        return -AW_MULTIPLE_SAR_FAILED;
    } else {
        AWLOGI(aw9610x->dev, "ca num = %d", aw9610x->ca_num);
    }
...
static int sar_ap_open_report_data(int open)
{
    return 0;
}

static int sar_ap_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
    return 0;
}

static int   sar_ap_flush(void)
{
    int ret = 0;
    ret = situation_flush_report(ID_SAR);
    return ret;
}

static int  sar_ap_get_data(int *probability, int *status)
{
    return 0;
}

static int32_t aw9610x_situation_init(void)
{
    struct situation_control_path ctl = {0};
    struct situation_data_path data = {0};
    int err = 0;

    ctl.open_report_data =  sar_ap_open_report_data;
    ctl.batch =  sar_ap_batch;
    ctl.flush =  sar_ap_flush;
    ctl.is_support_wake_lock = true;
    ctl.is_support_batch =  false;
    err = situation_register_control_path(&ctl, ID_SAR);
    if (err) {
       pr_err("registry sar control path err\n");

    }
    data.get_data =  sar_ap_get_data;
    err = situation_register_data_path(&data, ID_SAR);
    if (err) {
       pr_err("registry sar data path err\n");
    }
    return err;
}
...
aw9610x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    */
    strncpy(sar_devinfo.name, AW9610x_DEV_NAME, sizeof(AW9610x_DEV_NAME));
    sensorlist_register_deviceinfo(ID_SAR, &sar_devinfo);
    AWLOGD(aw9610x->dev, "%s: OK\n", __func__);
...
static int32_t __init aw9610x_init(void)
{
