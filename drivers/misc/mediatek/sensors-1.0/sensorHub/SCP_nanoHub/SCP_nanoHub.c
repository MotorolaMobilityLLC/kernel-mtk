...
static void SCP_sensorHub_init_sensor_state(void)
{
	...
	mSensorState[SENSOR_TYPE_CHOP_GESTURE].sensorType = SENSOR_TYPE_CHOP_GESTURE;
	mSensorState[SENSOR_TYPE_CHOP_GESTURE].rate = SENSOR_RATE_ONCHANGE;
	mSensorState[SENSOR_TYPE_CHOP_GESTURE].timestamp_filter = false;

	mSensorState[SENSOR_TYPE_CAMERA_GESTURE].sensorType = SENSOR_TYPE_CAMERA_GESTURE;
	mSensorState[SENSOR_TYPE_CAMERA_GESTURE].rate = SENSOR_RATE_ONCHANGE;
	mSensorState[SENSOR_TYPE_CAMERA_GESTURE].timestamp_filter = false;

	mSensorState[SENSOR_TYPE_SIGNIFICANT_MOVE].sensorType = SENSOR_TYPE_SIGNIFICANT_MOVE;
	mSensorState[SENSOR_TYPE_SIGNIFICANT_MOVE].rate = SENSOR_RATE_ONCHANGE;
	...
	struct data_unit_t *data)
{
	...
	case ID_LIGHT:
		data->time_stamp = data_t->time_stamp;
		data->light_t.lux = data_t->light_t.lux;
		data->light_t.photopic = data_t->light_t.photopic;
		data->light_t.als_ir = data_t->light_t.als_ir;
		break;
#endif
	case ID_PROXIMITY:
		data->time_stamp = data_t->time_stamp;
