...
#define ACCEL_TAG				"[ACCELEROMETER]"
#define ACCEL_FUN(f)			pr_err(ACCEL_TAG"%s\n", __FUNCTION__)
#define ACCEL_ERR(fmt, args...)	pr_err(ACCEL_TAG"[ERROR][%s][%d]: "fmt, __FUNCTION__, __LINE__, ##args)
#define ACCEL_LOG(fmt, args...)	pr_err(ACCEL_TAG"[INFO][%s][%d]:  "fmt, __FUNCTION__, __LINE__, ##args)
#define ACCEL_DBG(fmt, args...)	ontim_dev_dbg(7, ACCEL_TAG"[DEBUG][%s][%d]:  "fmt, __FUNCTION__, __LINE__, ##args)
...
static int MC3XXX_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	if (!client) {
		...
		ACCEL_ERR(" length %d exceeds %d\n", len, C_I2C_FIFO_SIZE);
		mutex_unlock(&MC3XXX_i2c_mutex);
		return -EINVAL;
	}
	err = i2c_transfer(client->adapter, msgs, ARRAY_SIZE(msgs));
	if (err != 2) {
		ACCEL_ERR("i2c_transfer error: (%d %p %d) %d\n", addr, data, len, err);
		err = -EIO;
	} else
		err = 0;
...
static struct file *openFile(char *path, int flag, int mode)
{
	...
	if (IS_ERR(fp) || !fp->f_op) {
		ACCEL_LOG("Calibration File filp_open return NULL\n");
		return NULL;
	} else
		return fp;
...
static int mcube_write_log_data(struct i2c_client *client, u8 data[0x3f])
{
	...
	if (fd_file == NULL)
		ACCEL_LOG("mcube_write_log_data fail to open\n");
	else {
	...
	if (_pszBuffer == NULL) {
		ACCEL_ERR("fail to allocate memory for buffer\n");
		return -1;
	}
...
static int mcube_read_cali_file(struct i2c_client *client)
{
	...
	if (fd_file == NULL) {
		ACCEL_LOG("%s:fail to open calibration file: %s\n", __func__, file_path);
		...
		ACCEL_LOG("Open backup calibration file successfully: %s\n", backup_file_path);
	} else
		ACCEL_LOG("Open calibration file successfully: %s\n", file_path);
...
static void MC3XXX_rbm(struct i2c_client *client, int enable)
{
	...
	/* ACCEL_LOG("[%s] REG(0x04): 0x%X, enable: %d\n", __FUNCTION__, _baDataBuf[0], enable); */
	...
	/* ACCEL_LOG("RBM CONTROL DONE - REG(0x04): 0x%X\n", _baDataBuf[0]); */
...
{
	ACCEL_DBG("[%s] *pbPCode: 0x%02X, *pbHwID: 0x%02X\n", __func__, *pbPCode, *pbHwID);
...
static int MC3XXX_Read_Reg_Map(struct i2c_client *p_i2c_client, u8 *pbUserBuf)
{
	...
	for (_nIndex = 0; _nIndex < MC3XXX_REGMAP_LENGTH; _nIndex++) {
		...
		ACCEL_LOG("[Gsensor] REG[0x%02X] = 0x%02X\n", _nIndex, _baData[_nIndex]);
	}
...
static void MC3XXX_SaveDefaultOffset(struct i2c_client *p_i2c_client)
{
	...
	ACCEL_DBG("s_baOTP_OffsetData: 0x%02X - 0x%02X - 0x%02X - 0x%02X - 0x%02X - 0x%02X\n",
		...
		if (atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend)) {
			...
			if (priv->fir.num < firlen) {
				...
				if (atomic_read(&priv->trace) & MCUBE_TRC_FILTER) {
					ACCEL_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n", priv->fir.num,
				...
				if (atomic_read(&priv->trace) & MCUBE_TRC_FILTER) {
					ACCEL_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d] : [%5d %5d %5d]\n", idx,
...
_LRF_RETURN:
	...
	/*
	* ACCEL_LOG(">>>>> [_MC3XXX_LowResFilter][%d] _nCurrDiff: %4d _nSumDiff: %4d _nCurrData:%4d	 Rep:%4d\n",
...
static void	_MC3XXX_ReadData_RBM2RAW(s16 waData[MC3XXX_AXES_NUM])
{
	...
	if (atomic_read(&_pt_i2c_obj->trace) & MCUBE_TRC_INFO)
		ACCEL_LOG("RBM->RAW->LPF <<<<<[%08d %08d %08d]\n", waData[MC3XXX_AXIS_X],
			waData[MC3XXX_AXIS_Y], waData[MC3XXX_AXIS_Z]);
	...
	if (atomic_read(&_pt_i2c_obj->trace) & MCUBE_TRC_INFO)
		ACCEL_LOG("RBM->RAW->LPF->RAW <<<<<[%08d %08d %08d]\n", waData[MC3XXX_AXIS_X],
			waData[MC3XXX_AXIS_Y], waData[MC3XXX_AXIS_Z]);
...
static int	MC3XXX_ReadData(struct i2c_client *pt_i2c_client, s16 waData[MC3XXX_AXES_NUM])
{
	...
	if (atomic_read(&_pt_i2c_obj->trace) & MCUBE_TRC_INFO)
		ACCEL_LOG("[%s] s_nIsRBM_Enabled: %d\n", __func__, s_nIsRBM_Enabled);

	if (pt_i2c_client == NULL) {
		ACCEL_ERR("ERR: Null Pointer\n");
	...
	if (!s_nIsRBM_Enabled) {
		if (s_bResolution == MC3XXX_RESOLUTION_LOW) {
			...
			MC3XXX_LOW_REOLUTION_DATA_SIZE)) {
				ACCEL_ERR("ERR: fail to read data via I2C!\n");
			...
			if (atomic_read(&_pt_i2c_obj->trace) & MCUBE_TRC_INFO)
				ACCEL_LOG("[%s][high] X: %d, Y: %d, Z: %d\n",
					__func__, waData[MC3XXX_AXIS_X], waData[MC3XXX_AXIS_Y], waData[MC3XXX_AXIS_Z]);

		}

		if (atomic_read(&_pt_i2c_obj->trace) & MCUBE_TRC_INFO)
			ACCEL_LOG("RAW<<<<<[%04d %04d %04d]\n",
...
static int MC3XXX_ReadCalibration(struct i2c_client *client, int dat[MC3XXX_AXES_NUM])
{
	...
	ACCEL_LOG("MC3XXX_ReadCalibration %d %d %d\n",
...
static int MC3XXX_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if (enable == mc3xxx_sensor_power)
		ACCEL_LOG("Sensor power status should not be set again!!!\n");

	if (MC3XXX_i2c_read_block(client, addr, databuf, 1)) {
		ACCEL_ERR("read power ctl register err!\n");
		return MC3XXX_RETCODE_ERROR_I2C;
	}

	ACCEL_DBG("set power read MC3XXX_REG_MODE_FEATURE =%x\n", databuf[0]);
	...
	if (res < 0) {
		ACCEL_LOG("fwq set power mode failed!\n");
		return MC3XXX_RETCODE_ERROR_I2C;
	} else if (atomic_read(&obj->trace) & MCUBE_TRC_INFO)
		ACCEL_LOG("fwq set power mode ok %d!\n", databuf[1]);
...
#endif
	default:
		ACCEL_ERR("ERR: no resolution assigned!\n");
	}

	ACCEL_DBG("[%s] s_bResolution: %d\n", __func__, s_bResolution);
}
...
static void MC3XXX_SetSampleRate(struct i2c_client *pt_i2c_client)
{
	...
	if (IS_MCFM12() || IS_MCFM3X()) {
		...
		case 0xC0:
			...
			break;
		default:
			ACCEL_ERR("[%s] no chance to get here... check code!\n", __func__);
			break;
		}
	}
	else if(IS_MESA())
...
static void MC3XXX_ConfigRegRange(struct i2c_client *pt_i2c_client)
{
	...
	if (res < 0)
		ACCEL_ERR("MC3XXX_ConfigRegRange fail\n");

	ACCEL_DBG("[%s] set 0x%X\n", __func__, _baDataBuf[1]);
}
...
static int MC3XXX_Init(struct i2c_client *client, int reset_cali)
{
	...
	/* ACCEL_LOG("[%s] confirmed i2c addr: 0x%X\n", __FUNCTION__, client->addr); */
	#endif
...
static int MC3XXX_ReadSensorData(struct i2c_client *pt_i2c_client, char *pbBuf, int nBufSize)
{
	...
	if (false == mc3xxx_sensor_power) {
		if (MC3XXX_SetPowerMode(pt_i2c_client, true) != MC3XXX_RETCODE_SUCCESS)
			ACCEL_ERR("ERR: fail to set power mode!\n");
	}
	...
	if (MC3XXX_ReadData(pt_i2c_client, _pt_i2c_obj->data) != MC3XXX_RETCODE_SUCCESS) {
		ACCEL_ERR("ERR: fail to read data!\n");
...
static int _MC3XXX_ReadAverageData(struct i2c_client *client, char *buf)
{
	...
	ACCEL_LOG("MC3XXX_ReadRawData MC3XXX_ReadData: %d, %d, %d!\n",
	...
	ACCEL_LOG("MC3XXX_ReadRawData sensor_data_max: %d, %d, %d!\n",
		sensor_data_max[MC3XXX_AXIS_X], sensor_data_max[MC3XXX_AXIS_Y], sensor_data_max[MC3XXX_AXIS_Z]);
	ACCEL_LOG("MC3XXX_ReadRawData sensor_data_mini: %d, %d, %d!\n",
	...
	ACCEL_LOG("MC3XXX_ReadRawData sensor_data: %d, %d, %d!\n",
		sensor_data[MC3XXX_AXIS_X], sensor_data[MC3XXX_AXIS_Y], sensor_data[MC3XXX_AXIS_Z]);
	...
	ACCEL_LOG("MC3XXX_ReadRawData mapdata: %d, %d, %d!\n",
	...
	ACCEL_LOG("MC3XXX_ReadRawData mapdata1: %d, %d, %d!\n",
		acc[MC3XXX_AXIS_X], acc[MC3XXX_AXIS_Y], acc[MC3XXX_AXIS_Z]);
...
static int MC3XXX_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	if (mc3xxx_sensor_power == false) {
		...
		if (res)
			ACCEL_ERR("Power on mc3xxx error %d!\n", res);
	}
	...
	if (res) {
		ACCEL_ERR("I2C error: ret value=%d", res);
		return -EIO;
	}
...
static int MC3XXX_JudgeTestResult(struct i2c_client *client)
{
	...
	if ((self_result > 475923) && (self_result < 2185360)) {
		ACCEL_ERR("MC3XXX_JudgeTestResult successful\n");
		return MC3XXX_RETCODE_SUCCESS;
	}

	ACCEL_ERR("MC3XXX_JudgeTestResult failt\n");
	return -EINVAL;
}
...
static int	MC3XXX_GetOpenStatus(void)
{
	...
	/* ACCEL_LOG("[%s] pass wait_event_interruptible: %d\n", __FUNCTION__, atomic_read(&s_t_mc3xxx_open_status)); */
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	ACCEL_LOG("fwq show_chipinfo_value\n");
	if (client == NULL) {
		ACCEL_ERR("i2c client is null!!\n");
		return 0;
	}
...
static ssize_t store_cali_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "rst", 3)) {
		...
		mc3xxx_mutex_unlock();
		if (err)
			ACCEL_ERR("write calibration err = %d\n", err);
	} else
		ACCEL_ERR("invalid format\n");
...
{   /*write anything to this register will trigger the process*/
	...
	if (ret != 0) {
		ACCEL_ERR("parse number fail\n");
		return count;
	} else if (num == 0) {
		ACCEL_ERR("invalid data count\n");
		return count;
	}
	...
	if (!MC3XXX_JudgeTestResult(client)) {
		ACCEL_LOG("SELFTEST : PASS\n");
		strcpy(selftestRes, "y");
	} else {
		ACCEL_LOG("SELFTEST : FAIL\n");
...
static ssize_t store_firlen_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	ACCEL_LOG("fwq store_firlen_value\n");

	ret = kstrtoint(buf, 10, &firlen);
	if (ret != 0)
		ACCEL_ERR("invallid format\n");
	else if (firlen > C_MAX_FIR_LENGTH)
		ACCEL_ERR("exceeds maximum filter length\n");
	else {
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	ACCEL_LOG("fwq show_trace_value\n");

	if (obj == NULL) {
		ACCEL_ERR("i2c_data obj is null!!\n");
		return 0;
	}
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	ACCEL_LOG("fwq store_trace_value\n");
	...
	if (sscanf(buf, "0x%x", &trace) == 1)
		...
		ACCEL_ERR("invalid content: '%s', length = %zu\n", buf, count);
...
static ssize_t show_status_value(struct device_driver *ddri, char *buf)
{
	...
	ACCEL_LOG("fwq show_status_value\n");
...
static ssize_t show_chip_orientation(struct device_driver *ptDevDrv, char *pbBuf)
{
	...
	ACCEL_LOG("[%s] default direction: %d\n", __func__, _ptAccelHw->direction);
...
static ssize_t store_chip_orientation(struct device_driver *ptDevDrv, const char *pbBuf, size_t tCount)
{
	...
	if (ret == 0) {
		if (hwmsen_get_convert(_nDirection, &_pt_i2c_obj->cvt))
			ACCEL_ERR("ERR: fail to set direction\n");
	}

	ACCEL_LOG("[%s] set direction: %d\n", __func__, _nDirection);
...
static ssize_t store_accuracy_status(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (ret != 0) {
		ACCEL_ERR("incorrect argument\n");
		return count;
	}

	if (_nAccuracyStatus > SENSOR_STATUS_ACCURACY_HIGH) {
		ACCEL_ERR("illegal accuracy status\n");
		return count;
	}
...
static int mc3xxx_suspend(struct device *dev)
{
	...
	ACCEL_LOG("mc3xxx_suspend\n");

	if (obj == NULL) {
		ACCEL_ERR("null pointer!!\n");
		return -EINVAL;
	}
	...
	mc3xxx_mutex_unlock();
	if (err) {
		ACCEL_ERR("write power control fail!!\n");
		return err;
	}
...
static int mc3xxx_resume(struct device *dev)
{
	...
	if (err) {
		mc3xxx_mutex_unlock();
		ACCEL_ERR("initialize client fail!!\n");
		return err;
	}
	...
	for (_nCount = 0; _nCount < _MC3XXX_I2C_PROBE_ADDR_COUNT_; _nCount++) {
		...
		if (MC3XXX_ValidateSensorIC(&_baData1Buf[0], &_baData2Buf[0]) == MC3XXX_RETCODE_SUCCESS) {
			...
			if (s_bPCODE == MC3XXX_PCODE_3416) {
				ACCEL_LOG("The chip is 3416, pard_id:0x%x.\n", s_bPCODE);
				sprintf(&mc3433_prox_vendor_name[0], "MC3416-P-Mcube");
			}
			else
			{
				ACCEL_LOG("The chip is 3433, pard_id:0x%x.\n", s_bPCODE);
...
static int mc3xxx_enable_nodata(int en)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		if (res == 0) {
			ACCEL_LOG("MC3XXX_SetPowerMode done\n");
			break;
		}
		ACCEL_LOG("MC3XXX_SetPowerMode fail\n");
	}
	...
	ACCEL_LOG("mc3xxx_enable_nodata OK!\n");
	return 0;
}
...
static int mc3xxx_set_delay(u64 ns)
{
	...
	ACCEL_LOG("mc3xxx_set_delay (%d), chip only use 1024HZ\n", value);
	return 0;
}
...
*
*	if (err < 0) {
*		ACCEL_ERR("set delay parameter error!\n");
*		return -1;
*	}
*	ACCEL_LOG("mc3xxx_batch acc set delay = (%d) ok.\n", value);
*/
	ACCEL_DBG("mc3xxx_batch (%d), chip only use 1024HZ\n", value);
	return 0;
}
...
static int mc3xxx_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	...
	if (err) {
		ACCEL_ERR("%s enable sensor failed!\n", __func__);
		return -1;
	}
	err = mc3xxx_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
		ACCEL_ERR("%s enable set batch failed!\n", __func__);
		return -1;
	}
	return 0;
}
...
static int mc3xxx_factory_get_raw_data(int32_t data[3])
{
	...
	ACCEL_LOG("support mc3xxx_factory_get_raw_data!\n");
	return 0;
}
...
static int mc3xxx_factory_clear_cali(void)
{
	...
	if (err) {
		ACCEL_ERR("mc3xxx_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int mc3xxx_factory_set_cali(int32_t data[3])
{
	...
	if (err) {
		ACCEL_ERR("mc3xxx_WriteCalibration failed!\n");
		return -1;
	}
	return 0;
}
...
//-add by hzb for ontim debug
	...
	if (err < 0) {
		ACCEL_ERR("get cust_baro dts info fail\n");
		goto exit;
	}
	ACCEL_DBG("i2c_num=0x%x,I2C_ADDRS=0x%x;0x%x;\n",hw->i2c_num,hw->i2c_addr[0],client->addr);

	if (_mc3xxx_i2c_auto_probe(client) != MC3XXX_RETCODE_SUCCESS)
	{
		err = -ENODEV;
		ACCEL_ERR("_mc3xxx_i2c_auto_probe fail\n");
		goto exit;
	}
	...
	if (err) {
		ACCEL_ERR("register acc control path err\n");
		goto exit_kfree;
	}
	...
	if (err) {
		ACCEL_ERR("register acc data path err= %d\n", err);
		goto exit_kfree;
	}
	ACCEL_LOG("%s: OK\n", __func__);
...
static int mc3xxx_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		ACCEL_ERR("mc3xxx_delete_attr fail: %d\n", err);
