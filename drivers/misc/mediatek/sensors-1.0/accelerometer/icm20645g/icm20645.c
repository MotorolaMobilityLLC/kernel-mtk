...
{
	if (icm20645_i2c_client == NULL) {
		GSE_PR_ERR("ICM20645_hwmsen_read_block null ptr!!\n");
...
static int icm20645_set_bank(struct i2c_client *client, u8 bank)
{
	...
	if (res < 0) {
		GSE_PR_ERR("icm20645_set_bank fail at %x", bank);
		return ICM20645_ERR_I2C;
	}
...
static int icm20645_turn_on(struct i2c_client *client, u8 status, bool on)
{
	...
	if (res < 0) {
		GSE_PR_ERR("icm20645_turn_on fail at %x", on);
		return ICM20645_ERR_I2C;
	}
	if (on == true) {
		databuf[0] &= ~status;
...
static int icm20645_lp_mode(struct i2c_client *client, bool on)
{
	...
	if (res < 0) {
		GSE_PR_ERR("icm20645_lp_mode fail at %x", on);
		return ICM20645_ERR_I2C;
	}
	if (on == true) {
...
static int ICM20645_lowest_power_mode(struct i2c_client *client, unsigned int on)
{
	...
	if (res < 0) {
		GSE_PR_ERR("ICM20645_lowest_power_mode1 err!\n");
		return ICM20645_ERR_I2C;
	}
	if (on == true) {
		databuf[0] |= BIT_LP_EN;
		...
		if (res < 0) {
			GSE_PR_ERR("ICM20645_lowest_power_mode2 err!\n");
			return ICM20645_ERR_I2C;
		}
	} else {
		databuf[0] &= ~BIT_LP_EN;
		res = mpu_i2c_write_block(client, ICM20645_REG_POWER_CTL, databuf, 0x1);
		if (res < 0) {
			GSE_PR_ERR("ICM20645_lowest_power_mode3 err!\n");
			return ICM20645_ERR_I2C;
		}
	}
...
static int ICM20645_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if (enable == false) {
		if (ICM20645_gyro_mode() == false) {
			...
			if (res != ICM20645_SUCCESS) {
				GSE_PR_ERR("ICM20645_lowest_power_mode error\n");
				return res;
			}
		}
	}
	...
	res = mpu_i2c_write_block(client, ICM20645_REG_POWER_CTL, databuf, 0x1);
	if (res < 0) {
		GSE_PR_ERR("set power mode failed!\n");
		return ICM20645_ERR_I2C;
...
static int ICM20645_SetDataResolution(struct icm20645_i2c_data *obj)
{
	...
	if (err) {
		GSE_PR_ERR("write data format fail!!\n");
		return err;
	}
	...
	if (reso < ARRAY_SIZE(icm20645_data_resolution)) {
...
static int ICM20645_Dev_Reset(struct i2c_client *client)
{
	...
	if (res < 0) {
		GSE_PR_ERR("ICM20645_Dev_Reset fail\n");
		return ICM20645_ERR_I2C;
	}
...
static int ICM20645_selclk(struct i2c_client *client)
{
	...
	res = mpu_i2c_read_block(client, ICM20645_REG_POWER_CTL, databuf, 0x1);
	if (res < 0) {
		GSE_PR_ERR("ICM20645_REG_POWER_CTL read fail\n");
		return ICM20645_ERR_I2C;
	}
	databuf[0] |= (BIT_CLK_PLL | BIT_TEMP_DIS);
	res = mpu_i2c_write_block(client, ICM20645_REG_POWER_CTL, databuf, 0x1);
	if (res < 0) {
		GSE_PR_ERR("ICM20645_REG_POWER_CTL write fail\n");
		return ICM20645_ERR_I2C;
	}
	return ICM20645_SUCCESS;
}
...
static int icm20645_init_client(struct i2c_client *client, int reset_cali)
{
	...
	if (res != ICM20645_SUCCESS) {
		GSE_PR_ERR("ICM20645_selclk error\n");
		return res;
	}
	res = ICM20645_CheckDeviceID(client);
	if (res != ICM20645_SUCCESS) {
		GSE_PR_ERR("Check ID error\n");
		return res;
	}
	...
	for (; retry_fail < 10; retry_fail++) {
		...
		if (res != ICM20645_SUCCESS) {
			GSE_PR_ERR("set power error\n");
			udelay(5);
			continue;
		}
		if (retry_fail == 9) {
			GSE_PR_ERR("set power error, retry 10 times\n");
			return res;
		}
		break;
	}
	...
	if (res != ICM20645_SUCCESS) {
		GSE_PR_ERR("icm20645_turn_on error\n");
		return res;
	}
	res = icm20645_lp_mode(client, true);
	if (res != ICM20645_SUCCESS) {
		GSE_PR_ERR("icm20645_lp_mode error\n");
		return res;
	}
	...
	if (res != ICM20645_SUCCESS) {
		GSE_PR_ERR("set data format error\n");
		return res;
	}
	res = ICM20645_Setfilter(client, ACCEL_AVGCFG_8X);
	if (res != ICM20645_SUCCESS) {
		GSE_PR_ERR("ICM20645_Setfilter error\n");
		return res;
	}
	res = ICM20645_SetSampleRate(client, 125);
	if (res != ICM20645_SUCCESS) {
		GSE_PR_ERR("ICM20645_SetSampleRate error\n");
		return res;
	}
	...
	if (res != ICM20645_SUCCESS) {
		GSE_PR_ERR("icm20645_SetIntEnable error\n");
		return res;
	}
...
static int ICM20645_ReadAllReg(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (sensor_power == false) {
		...
		if (err)
			GSE_PR_ERR("Power on mpu6050 error %d!\n", err);
		msleep(50);
	}
	...
	for (i = 0; i <= total_len; i++)
		GSE_LOG("ICM20645 bank0 reg=0x%x, data=0x%x\n", (addr + i), buff[i]);
	icm20645_set_bank(client, BANK_SEL_2);
	addr = 0X10;
	res = mpu_i2c_read_block(client, addr, buff, total_len);
	if (res < 0)
	...
	for (i = 0; i <= total_len; i++)
		GSE_LOG("ICM20645 bank2 reg=0x%x, data=0x%x\n", (addr + i), buff[i]);
	icm20645_set_bank(client, BANK_SEL_0);
...
static int ICM20645_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (sensor_power == false) {
		...
		res = icm20645_turn_on(client, BIT_PWR_ACCEL_STBY, true);
		if (res)
			GSE_PR_ERR("Power on icm20645 error %d!\n", res);
		msleep(50);
	}
	res = ICM20645_ReadData(client, obj->data);
	if (res) {
		GSE_PR_ERR("I2C error: ret value=%d\n", res);
		return -3;
	}
...
static ssize_t store_selftest_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (obj == NULL) {
		GSE_PR_ERR("i2c data obj is null!!\n");
		return 0;
	}
...
static int icm20645_enable_nodata(int en)
{
	...
	if (power == true) {
		for (retry = 0; retry < 3; retry++) {
			...
			GSE_PR_ERR("ICM20645_SetPowerMode fail\n");
		}
...
static int icm20645_set_delay(u64 ns)
{
	...
	if (err != ICM20645_SUCCESS) {
		GSE_PR_ERR("icm20645_set_delay Set delay parameter error!\n");
		return -1;
	}
...
static int icm20645_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err) {
		GSE_PR_ERR("icm20645_device register failed\n");
...
static int icm20645_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		GSE_PR_ERR("icm20645_delete_attr fail: %d\n", err);
	err = misc_deregister(&icm20645_device);
	if (err)
		GSE_PR_ERR("misc_deregister fail: %d\n", err);
	err = hwmsen_detach(ID_ACCELEROMETER);
	if (err)
		GSE_PR_ERR("hwmsen_detach fail: %d\n", err);
