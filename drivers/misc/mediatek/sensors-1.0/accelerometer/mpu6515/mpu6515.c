...

	{
		...
		if ((atomic_read(&priv->trace) & MPU6515_TRC_DUMP) && (num == 0)) {
			...
			for (i = 0; i < MPU6515_DUMP_LEN; i += 8) {
				...
				GSE_LOG("[%02X %02X %02X %02X %02X %02X %02X %02X]",
...
#endif
	err = mpu6515_init_client(mpu6515_i2c_client, 0);
	if (err) {
		GSE_ERR("initialize client fail!!\n");
		return -1;
	}
...
static int bmi160_factory_get_raw_data(int32_t data[3])
{
	...
	MPU6515_ReadRawData(mpu6515_i2c_client, strbuf);
...
static int bmi160_factory_clear_cali(void)
{
	...
	err = MPU6515_ResetCalibration(mpu6515_i2c_client);
	if (err) {
		GSE_ERR("mpu6515a_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int bmi160_factory_set_cali(int32_t data[3])
{
	...
	err = MPU6515_WriteCalibration(mpu6515_i2c_client, cali);
	if (err) {
		GSE_ERR("mpu6515a_WriteCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int bmi160_factory_get_cali(int32_t data[3])
{
	...
	data[2] = cali[BMI160_ACC_AXIS_Z]
		* GRAVITY_EARTH_1000 / obj_data->reso->sensitivity;
#endif
	err = MPU6515_ReadCalibration(mpu6515_i2c_client, cali);
	if (err) {
		GSE_ERR("mpu6515a_ReadCalibration failed!\n");
		return -1;
	}
	data[0] = cali[MPU6515_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[1] = cali[MPU6515_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[2] = cali[MPU6515_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	return 0;
}
...
static struct accel_factory_fops mpu6515a_factory_fops = {
...
static struct accel_factory_public mpu6515a_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &mpu6515a_factory_fops,
};
/*----------------------------------------------------------------------------*/
static int mpu6515_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
...
static int mpu6515_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = accel_factory_device_register(&mpu6515a_factory_device);
	if (err) {
...
static int mpu6515_i2c_remove(struct i2c_client *client)
{
	...
	accel_factory_device_deregister(&mpu6515a_factory_device);
