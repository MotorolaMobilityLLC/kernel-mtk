...
static int LSM6DS3H_Int_Ctrl(struct i2c_client *client, LSM6DS3H_ACC_GYRO_INT_ACTIVE_t int_act,
...
static int LSM6DS3H_set_bank(struct i2c_client *client, u8 bank)
{
	...
	if (res < 0) {
		GSE_PR_ERR("LSM6DS3H_set_bank fail at %x", bank);
		return LSM6DS3H_ERR_I2C;
	}
...
static int LSM6DS3H_write_rel_calibration(struct lsm6ds3h_i2c_data *obj, int dat[LSM6DS3H_AXES_NUM])
{
	...
	if (atomic_read(&obj->trace) & ACCEL_TRC_CALI) {
		GSE_LOG("test  (%5d, %5d, %5d) ->(%5d, %5d, %5d)->(%5d, %5d, %5d))\n",
			...
			obj->cvt.sign[LSM6DS3H_AXIS_Z], dat[LSM6DS3H_AXIS_X], dat[LSM6DS3H_AXIS_Y],
		...
		GSE_LOG("write calibration data  (%5d, %5d, %5d)\n", obj->cali_sw[LSM6DS3H_AXIS_X],
			obj->cali_sw[LSM6DS3H_AXIS_Y], obj->cali_sw[LSM6DS3H_AXIS_Z]);
	}
#endif
	return 0;
}
...
static int LSM6DS3H_SetFullScale(struct i2c_client *client, u8 acc_fs)
{
	...
	if (mpu_i2c_read_block(client, LSM6DS3H_CTRL1_XL, databuf, 0x01)) {
		GSE_PR_ERR("read LSM6DS3H_CTRL1_XL err!\n");
		return LSM6DS3H_ERR_I2C;
	}
	...
	if (mpu_i2c_read_block(client, LSM6DS3H_CTRL9_XL, databuf, 0x01)) {
		GSE_PR_ERR("read LSM6DS3H_CTRL9_XL err!\n");
		return LSM6DS3H_ERR_I2C;
	}
	...
	databuf[0] |=
	    LSM6DS3H_ACC_ENABLE_AXIS_X | LSM6DS3H_ACC_ENABLE_AXIS_Y | LSM6DS3H_ACC_ENABLE_AXIS_Z;
...
static int LSM6DS3H_Enable_Func(struct i2c_client *client, LSM6DS3H_ACC_GYRO_FUNC_EN_t newValue)
{
	...
	if (mpu_i2c_read_block(client, LSM6DS3H_CTRL10_C, databuf, 0x01)) {
		GSE_PR_ERR("%s read LSM6DS3H_CTRL10_C register err!\n", __func__);
		return LSM6DS3H_ERR_I2C;
	}
	...
	if (res < 0) {
		GSE_PR_ERR("%s write LSM6DS3H_CTRL10_C register err!\n", __func__);
		return LSM6DS3H_ERR_I2C;
	}
...
static int LSM6DS3H_ReadAccData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (res < 0) {
		...
		sprintf(buf, "%04x %04x %04x", acc[LSM6DS3H_AXIS_X], acc[LSM6DS3H_AXIS_Y],
			acc[LSM6DS3H_AXIS_Z]);
...
static int LSM6DS3H_ReadAccRawData(struct i2c_client *client, s16 data[LSM6DS3H_AXES_NUM])
{
	...
	if (client == NULL)
		...
		if (hwmsen_read_block(client, LSM6DS3H_OUTX_L_XL, databuf, 6)) {
			GSE_PR_ERR("LSM6DS3H read acc data  error\n");
			return -2;
		}
...
static int LSM6DS3H_Set_RegInc(struct i2c_client *client, bool inc)
{
	...
	if (mpu_i2c_read_block(client, LSM6DS3H_CTRL3_C, databuf, 0x01)) {
		GSE_PR_ERR("read LSM6DS3H_CTRL3_XL err!\n");
		return LSM6DS3H_ERR_I2C;
	}
...
static int gsensor_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	GSE_LOG("lsm6ds3h acc set delay = (%d) ok.\n", value);
...
/*----------------------------------------------------------------------------*/
static long lsm6ds3h_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#if 0
		sensor_data.x =
		    (s64) (cali[LSM6DS3H_AXIS_X]) * obj->sensitivity * GRAVITY_EARTH_1000 / (1000 * 1000);	/*NTC*/
		sensor_data.y =
		    (s64) (cali[LSM6DS3H_AXIS_Y]) * obj->sensitivity * GRAVITY_EARTH_1000 / (1000 *
											     1000);
		sensor_data.z =
		    (s64) (cali[LSM6DS3H_AXIS_Z]) * obj->sensitivity * GRAVITY_EARTH_1000 / (1000 *
											     1000);
#else
...
#ifdef CONFIG_COMPAT
static long lsm6ds3h_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long err = 0;
...
static int lsm6ds3h_resume(struct device *dev)
{
	...
	if (err) {
		GSE_PR_ERR("initialize client fail! err code %d!\n", err);
		return err;
	}
...
static int lsm6ds3h_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		GSE_PR_ERR("lsm6ds3h_i2c_remove fail: %d\n", err);
