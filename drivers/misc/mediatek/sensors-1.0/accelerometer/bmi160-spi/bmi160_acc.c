...
int BMP160_spi_read_bytes(struct spi_device *spi, u16 addr, u8 *rx_buf, u32 data_len)
{
	...
	if (xfer == NULL) {
		GSE_PR_ERR("%s, no memory for SPI transfer\n", __func__);
		return -ENOMEM;
	}
...
s8 bmi_read_wrapper(u8 dev_addr, u8 reg_addr, u8 *data, u8 len)
{
	...
	if (err < 0)
		GSE_PR_ERR("read bmi160 i2c failed.\n");
	return err;
}
...
static int BMI160_ACC_ResetCalibration(struct bmi160_acc_data *obj)
{
	...
	if (err)
		GSE_PR_ERR("write offset failed.\n");
#endif
...
static int BMI160_ACC_WriteCalibration(struct bmi160_acc_data *obj, int dat[BMI160_ACC_AXES_NUM])
{
	...
	err =
	    BMP160_spi_write_bytes(obj->spi, BMI160_ACC_REG_OFSX, obj->offset, BMI160_ACC_AXES_NUM);
	if (err) {
		GSE_PR_ERR("write offset fail: %d\n", err);
		return err;
	}
#endif
...
static int BMI160_ACC_CheckDeviceID(struct bmi160_acc_data *obj)
{
	...
	if (err < 0) {
		GSE_PR_ERR("read chip id failed.\n");
...
static int BMI160_ACC_SetPowerMode(struct bmi160_acc_data *obj, bool enable)
{
	...
	if (err < 0) {
		GSE_PR_ERR("write power mode value to register failed.\n");
...
static int BMI160_ACC_SetBWRate(struct bmi160_acc_data *obj, u8 bwrate)
{
	...
	mdelay(20);
	if (err < 0) {
		GSE_PR_ERR("set bandwidth failed, res = %d\n", err);
		mutex_unlock(&obj->lock);
		return BMI160_ACC_ERR_SPI;
	}
...
static int BMI160_ACC_CompassReadData(struct bmi160_acc_data *obj, char *buf, int bufsize)
{
	...
	if (sensor_power == false) {
		...
		if (res)
			GSE_PR_ERR("Power on bmi160_acc error %d!\n", res);
	}
	res = BMI160_ACC_ReadData(obj, databuf);
	if (res) {
		GSE_PR_ERR("read acc data failed.\n");
		return -3;
	}
...
static int BMI160_ACC_ReadSensorData(struct bmi160_acc_data *obj, char *buf, int bufsize)
{
	...
	if (sensor_power == false) {
		...
		if (err) {
			GSE_PR_ERR("set power on acc failed.\n");
			return err;
		}
	}
...
static int BMI160_ACC_ReadRawData(struct bmi160_acc_data *obj, char *buf)
{
	...
	err = BMI160_ACC_ReadData(obj, databuf);
	if (err) {
		GSE_PR_ERR("read acc raw data failed.\n");
		return -EIO;
	}
...
static ssize_t show_acc_op_mode_value(struct device_driver *ddri, char *buf)
{
	...
	if (err < 0) {
		GSE_PR_ERR("get acc op mode failed.\n");
		return err;
	}
...
static ssize_t store_acc_op_mode_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (err < 0) {
		GSE_PR_ERR("set acc op mode = %d failed.\n", (int)data);
		return err;
	}
	return count;
}
...
static ssize_t show_acc_range_value(struct device_driver *ddri, char *buf)
{
	...
	if (err < 0) {
		GSE_PR_ERR("get acc range failed.\n");
		return err;
	}
	return sprintf(buf, "%d\n", data);
}
...
static ssize_t store_acc_range_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (err < 0) {
		GSE_PR_ERR("set acc range = %d failed.\n", (int)data);
		return err;
	}
	return count;
}
...
static ssize_t store_acc_odr_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (err < 0) {
		GSE_PR_ERR("set acc bandwidth failed.\n");
		return err;
	}
...
static ssize_t store_cali_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "rst", 3)) {
		...
		GSE_PR_ERR("set calibration value by invalid format.\n");
	}
	return count;
}
...
static ssize_t bmi160_fifo_data_out_frame_show(struct device_driver *ddri, char *buf)
{
	...
	if (err < 0) {
		GSE_PR_ERR("read fifo length error.\n");
		return -EINVAL;
	}
	if (fifo_bytecount == 0)
		return 0;
...
#endif
	if (err < 0) {
		GSE_PR_ERR("read fifo data error.\n");
...
static ssize_t bmi_register_store(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (!err) {
		GSE_LOG("write reg 0x%2x, value= 0x%2x\n", reg_addr, data);
	} else {
		GSE_PR_ERR("write reg fail\n");
		return err;
	}
	return count;
}
...
static int bmi160_acc_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		if (err) {
			GSE_PR_ERR("create driver file (%s) failed.\n",
...
static int bmi160_acc_open(struct inode *inode, struct file *file)
{
	...
	if (file->private_data == NULL) {
		GSE_PR_ERR("file->private_data is null pointer.\n");
		return -EINVAL;
	}
...
static long bmi160_acc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_SET_CALI:
		...
		if (atomic_read(&obj->suspend)) {
			GSE_PR_ERR("can't perform calibration in suspend state.\n");
			err = -EINVAL;
		} else {
	...
	case GSENSOR_IOCTL_GET_CALI:
		...
		if (err) {
			GSE_PR_ERR("read calibration failed.\n");
			break;
		}
...
static int bmi160_acc_suspend(void)
{
	...
	if (op_mode != BMI160_ACC_MODE_SUSPEND && sig_flag != 1)
		err = BMI160_ACC_SetPowerMode(obj, false);
	if (err) {
		GSE_PR_ERR("write power control failed.\n");
		return err;
	}
...
static int bmi160_acc_enable_nodata(int en)
{
	...
	if (err < 0) {
		GSE_PR_ERR("BMI160_ACC_SetPowerMode failed.\n");
		return -1;
	}
...
static int bmi160_acc_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	if (err < 0) {
		GSE_PR_ERR("set delay parameter error!\n");
		return -1;
	}
...
static int bmi160_acc_set_delay(u64 ns)
{
	...
	bmi160_acc_batch(0, ns, 0);

	/* sensor API 1.0, active -> setdelay , so we need to set sensor rate here */
	err = BMI160_ACC_SetBWRate(obj_data, obj_data->bwrate);
	if (err < 0) {
		GSE_PR_ERR("set delay parameter error!\n");
		return -1;
	}
...
static int bmi160_acc_get_data(int *x, int *y, int *z, int *status)
{
	...
	if (err < 0) {
		GSE_PR_ERR("bmi160_acc_get_data failed.\n");
		return err;
	}
...
static int bmi160_factory_get_raw_data(int32_t data[3])
{
	...
	GSE_LOG("don't support bmi160_factory_get_raw_data!\n");
	return 0;
}
...
static int bmi160_factory_clear_cali(void)
{
	...
	if (err) {
		GSE_PR_ERR("bmi160_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int bmi160_factory_set_cali(int32_t data[3])
{
	...
	if (err) {
		GSE_PR_ERR("bmi160_WriteCalibration failed!\n");
		return -1;
	}
	return 0;
}
...
static int sm_init_interrupts(u8 sig_map_int_pin)
{
	...
	if (ret)
		GSE_PR_ERR("bmi160 sig motion setting failed.\n");
	return ret;
}

#ifdef CUSTOM_KERNEL_SIGNIFICANT_MOTION_SENSOR
static void bmi_std_interrupt_handle(struct bmi160_acc_data *client_data)
{
	...
	if (err < 0) {
		GSE_PR_ERR("get step detect enable failed.\n");
		return;
	}
	if (step_detect_enable == ENABLE)
		err = bmi160_step_notify(TYPE_STEP_DETECTOR);
	if (err < 0)
		GSE_PR_ERR("notify step detect failed.\n");
...
static void bmi_sm_interrupt_handle(struct bmi160_acc_data *client_data)
{
	...
	if (err < 0) {
		GSE_PR_ERR("get significant motion failed.\n");
		return;
	}
	if (sig_sel == ENABLE) {
		sig_flag = 1;
	...
	if (err < 0)
		GSE_PR_ERR("notify significant motion failed.\n");
...
static int bmi160acc_setup_eint(struct spi_device *spi)
{
	...
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
		GSE_PR_ERR("Cannot find step pinctrl!\n");
		return ret;
	}
	pins_default = pinctrl_lookup_state(pinctrl, "pin_default");
	if (IS_ERR(pins_default)) {
		ret = PTR_ERR(pins_default);
		GSE_PR_ERR("Cannot find step pinctrl default!\n");
		return ret;
	}
	pins_cfg = pinctrl_lookup_state(pinctrl, "pin_cfg");
	if (IS_ERR(pins_cfg)) {
		ret = PTR_ERR(pins_cfg);
		GSE_PR_ERR("Cannot find step pinctrl pin_cfg!\n");
		return ret;
	}
	...
	if (node) {
		...
		GSE_LOG("obj->IRQ = %d\n", obj->IRQ);
		if (!obj->IRQ) {
			GSE_PR_ERR("irq_of_parse_and_map fail!!\n");
			return -EINVAL;
		}
		if (request_irq
		    (obj->IRQ, bmi_irq_handler, IRQF_TRIGGER_RISING, "mediatek,GSE_1-eint", NULL)) {
			GSE_PR_ERR("IRQ LINE NOT AVAILABLE!!\n");
			return -EINVAL;
		}
		enable_irq(obj->IRQ);
	} else {
		GSE_PR_ERR("null irq node!!\n");
		return -EINVAL;
	}
	return 0;
}
...
static int bmi160_acc_spi_probe(struct spi_device *spi)
{
	...
	hw = get_accel_dts_func(spi->dev.of_node, hw);
	if (!hw) {
		GSE_PR_ERR("device tree configuration error!\n");
		return 0;
	}
	...
	if (obj->spi_buffer == NULL) {
		GSE_PR_ERR("kzalloc spi_buffer fail.\n");
		goto err_buf;
	}
	...
	if (err)
		GSE_PR_ERR("could not request irq\n");
#else
	...
	if (err) {
		GSE_PR_ERR("create attribute failed.\n");
	...
	if (err) {
		GSE_PR_ERR("register acc control path error.\n");
		goto exit_kfree;
	}
	...
	if (err) {
		GSE_PR_ERR("register acc data path error.\n");
		goto exit_kfree;
	}
#ifdef CONFIG_PM
	err = register_pm_notifier(&pm_notifier_func);
	if (err) {
		GSE_PR_ERR("Failed to register PM notifier.\n");
		goto exit_kfree;
	}
#endif				/* CONFIG_PM */
...
static int bmi160_acc_spi_remove(struct spi_device *spi)
{
	...
	if (err)
		GSE_PR_ERR("delete device attribute failed.\n");
