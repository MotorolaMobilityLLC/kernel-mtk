...
static struct i2c_board_info __initdata i2c_kxtj2_1009_board_info = { I2C_BOARD_INFO(KXTJ2_1009_DEV_NAME, 0x0e) }; //modify 10.15
...
static const struct dev_pm_ops kxtj2_1009_pm_ops = {
...
static char kxtj2_1009_prox_version[]="kxtj3-1058-1.0";
static char kxtj2_1009_prox_vendor_name[20]="kxtj3-1058-ROHM";
    DEV_ATTR_DECLARE(gsensor)
    DEV_ATTR_DEFINE("version",kxtj2_1009_prox_version)
    DEV_ATTR_DEFINE("vendor",kxtj2_1009_prox_vendor_name)
...
static int kxtj2_1009_suspend(struct device *dev) 
{
	...
	mutex_lock(&kxtj2_1009_mutex);
	err = KXTJ2_1009_SetPowerMode(client, false);
        mutex_unlock(&kxtj2_1009_mutex);
	if (err) {
...
*/
	GSE_LOG("kxtj2_1009_batch (%d), chip only use 1024HZ\n", value);
	return 0;
}
...
static int kxtj2_1009_factory_get_raw_data(int32_t data[3])
{
	...
	GSE_ERR("support kxtj2_1009_factory_get_raw_data!\n");
	return 0;
}
...
{
	int err;
	const char *name = "mediatek,gsensor_kxtj2";
	struct device_node *node =of_find_compatible_node(NULL,NULL,name);

	GSE_FUN();
	err = get_accel_dts_func(node, hw);
	if (err<0){
		GSE_LOG("%s get accel dts fail!!\n",__func__);
	}

	i2c_register_board_info(hw->i2c_num,&i2c_kxtj2_1009_board_info,1);
