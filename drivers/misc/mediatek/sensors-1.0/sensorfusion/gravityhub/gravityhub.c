...
#define GRVTY_PR_ERR(fmt, args...)    pr_err(GRVTY_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
static int grav_get_data(int *x, int *y, int *z, int *scalar, int *status)
{
	...
	if (err < 0) {
		GRVTY_PR_ERR("sensor_get_data_from_hub fail!!\n");
		return -1;
	}
...
static int gravityhub_local_init(void)
{
	...
	if (err) {
		GRVTY_PR_ERR("register gravity control path err\n");
		goto exit;
	}
	...
	if (err) {
		GRVTY_PR_ERR("register gravity data path err\n");
		goto exit;
	}
	err = scp_sensorHub_data_registration(ID_GRAVITY, gravity_recv_data);
	if (err < 0) {
		GRVTY_PR_ERR("SCP_sensorHub_data_registration failed\n");
		goto exit;
	}
	return 0;
exit:
