...
#define ROTVEC_PR_ERR(fmt, args...)    pr_err(ORIENT_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
static int orienthub_local_init(void)
{
	...
	if (err) {
		ROTVEC_PR_ERR("register orientation control path err\n");
		goto exit;
	}
	...
	if (err) {
		ROTVEC_PR_ERR("register orientation data path err\n");
		goto exit;
	}
