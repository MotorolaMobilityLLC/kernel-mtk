...
#define UNMAGHUB_PR_ERR(fmt, args...)    pr_err(UNMAGHUB_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
static int uncali_maghub_local_init(void)
{
	...
	if (err) {
		UNMAGHUB_PR_ERR("register uncali_mag control path err\n");
		goto exit;
	}
	...
	if (err) {
		UNMAGHUB_PR_ERR("register uncali_mag data path err\n");
		goto exit;
	}
	err = scp_sensorHub_data_registration(ID_MAGNETIC_UNCALIBRATED, uncali_mag_recv_data);
	if (err < 0) {
		UNMAGHUB_PR_ERR("SCP_sensorHub_data_registration failed\n");
		goto exit;
	}
	return 0;
exit:
