...
static int otpMatrix[3] = {1000, 1000,1350};
...
static DEFINE_MUTEX(mmc3680x_suspend_mutex);

//+add by hzb for ontim debug
#include <ontim/ontim_dev_dgb.h>
static char mc3410_prox_version[]="mmc3680_mtk_1.0";
static char mc3410_prox_vendor_name[20]="MEMSIC-MMC3680";
...
static int mmc3680x_read_otp(void)
{
	...
	otpMatrix[2] = (stemp1 * 6 + 1000) * 135 / 100;
...
static int mmc3680x_suspend(struct device *dev)
{
    ...
    if(obj == NULL) {
        MEMSIC_ERR("mmc3680x null pointer\n");
        return -EINVAL;
    }

//+ add by fanxzh
    atomic_set(&obj->suspend, 1);
    mutex_lock(&mmc3680x_suspend_mutex);
    mmc3680x_power(obj->hw, 0);
    mutex_unlock(&mmc3680x_suspend_mutex);
//- add by fanxzh
	return 0;
}
static int mmc3680x_resume(struct device *dev)
...
static int mmc3680x_factory_get_data(int32_t data[3], int *status)
{
	/* get raw data */
//+add by fxz ut
	mmc3680x_get_data(&data[0], &data[1], &data[2], status);
	data[0]  /= 10240;
	data[1]  /= 10240;
	data[2]  /= 10240;
//-add by fxz
...
//-add by hzb for ontim debug
	...
	get_mag_dts_func(client->dev.of_node, hw);
	if (!hw) {
...
//- add by fanxzh
	...
	ctl.is_support_batch = data->hw->is_batch_supported;
        #if 1
	strlcpy(ctl.libinfo.libname, "memsicd3680x", sizeof(ctl.libinfo.libname));
        //ctl.libinfo.layout = AKECS_setCert();
        ctl.libinfo.deviceid = MMC3680x_DEVICE_ID;
        #else
