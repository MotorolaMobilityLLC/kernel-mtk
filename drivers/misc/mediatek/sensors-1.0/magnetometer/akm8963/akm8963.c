...
#define AKMDBG(format, ...)	pr_err("AKM8963 " format "\n", ## __VA_ARGS__)
#else
#define AKMDBG(format, ...)
#endif

#if AKM8963_DEBUG_FUNC
#define AKMFUNC(func) pr_err("AKM8963 " func " is called\n")
#else
#define AKMFUNC(func)
#endif
...
#ifdef CONFIG_MTK_LEGACY
static struct i2c_board_info i2c_akm8963 __initdata = { I2C_BOARD_INFO("akm8963", (0x0F)) };
#endif
...
static long AKI2C_RxData(char *rxData, int length)
{
	...
	if (atomic_read(&data->trace) & AMK_I2C_DEBUG) {
		pr_info("RxData: len=%02x, addr=%02x\n  data=", length, addr);
		for (i = 0; i < length; i++)
...
#if DEBUG
	if (atomic_read(&data->trace) & AMK_I2C_DEBUG) {
		pr_info("TxData: len=%02x, addr=%02x\n  data=", length, txData[0]);
...
static int TEST_DATA(const char testno[],
		     const char testname[],
		     const int testdata, const int lolimit, const int hilimit, int *pf_total)
{
...
static int FctShipmntTestProcess_Body(void)
{
	...
	TEST_DATA(TLIMIT_NO_SNG_HZ, TLIMIT_TN_SNG_HZ, hdata[2], TLIMIT_LO_SNG_HZ, TLIMIT_HI_SNG_HZ,
		  &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_ST2, TLIMIT_TN_SNG_ST2, (int)i2cData[7], TLIMIT_LO_SNG_ST2,
	...
	/* TEST */
	TEST_DATA(TLIMIT_NO_SLF_ST2, TLIMIT_TN_SLF_ST2, (int)i2cData[7], TLIMIT_LO_SLF_ST2,
...
static int akm8963_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, akm8963_attr_list[idx]);
		if (err) {
			pr_err("driver_create_file (%s) = %d\n", akm8963_attr_list[idx]->attr.name,
			       err);
			break;
		}
	}
...
static long akm8963_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	/* pr_info("akm8963 akm8963_unlocked_ioctl arg:0x%lx\n", arg); */
	switch (cmd) {
	case ECS_IOCTL_WRITE:
		...
		/*pr_info("ECS_IOCTL_WRITE:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,
	...
	case ECS_IOCTL_READ:
		...
		/*pr_info("ECS_IOCTL_READ:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,
			0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x.\n",
		   rwbuf[0],
		   rwbuf[1],
		   rwbuf[2],
	...
	case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
		...
		if (copy_to_user(argp, &osensor_data, sizeof(struct hwm_sensor_data)))
			return -EFAULT;
...
static long akm8963_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	/*pr_info("akm8963_compat_ioctl arg: 0x%lx, arg32: 0x%p\n",arg, arg32); */

	switch (cmd) {
	case COMPAT_ECS_IOCTL_WRITE:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_WRITE\n"); */
		if (arg32 == NULL) {
			AKMDBG("invalid argument.");
			return -EINVAL;
		}
	...
	case COMPAT_ECS_IOCTL_RESET:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_RESET\n"); */
	...
	case COMPAT_ECS_IOCTL_READ:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_READ\n"); */
		if (arg32 == NULL) {
			AKMDBG("invalid argument.");
			return -EINVAL;
		}
	...
	case COMPAT_ECS_IOCTL_SET_MODE:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_SET_MODE\n"); */
		if (arg32 == NULL) {
			AKMDBG("invalid argument.");
			return -EINVAL;
		}
	...
	case COMPAT_ECS_IOCTL_GETDATA:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_GETDATA\n"); */
	...
	case COMPAT_ECS_IOCTL_SET_YPR:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_SET_YPR\n"); */
		if (arg32 == NULL) {
			AKMDBG("invalid argument.");
			return -EINVAL;
		}

		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_SET_YPR, (unsigned long)(arg32));
		if (ret) {
	...
	case COMPAT_ECS_IOCTL_GET_OPEN_STATUS:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_GET_OPEN_STATUS\n"); */
	...
	case COMPAT_ECS_IOCTL_GET_CLOSE_STATUS:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_GET_CLOSE_STATUS\n"); */
	...
	case COMPAT_ECS_IOCTL_GET_OSENSOR_STATUS:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_GET_OSENSOR_STATUS\n"); */
	...
	case COMPAT_ECS_IOCTL_GET_DELAY:
		/*pr_info("akm8963_compat_ioctl COMPAT_ECS_IOCTL_GET_DELAY\n"); */
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_DELAY, (unsigned long)(arg32));
		if (ret) {
	...
	case COMPAT_ECS_IOCTL_GET_LAYOUT:
		/*pr_info("akm8963 COMPAT_ECS_IOCTL_GET_LAYOUT\n"); */
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_LAYOUT, (unsigned long)arg32);
		if (ret) {
	...
	case COMPAT_MSENSOR_IOCTL_READ_CHIPINFO:
		/*pr_info("akm8963_compat_ioctl COMPAT_MSENSOR_IOCTL_READ_CHIPINFO\n"); */
	...
	case COMPAT_MSENSOR_IOCTL_READ_SENSORDATA:
		/*pr_info("akm8963_compat_ioctl COMPAT_MSENSOR_IOCTL_READ_SENSORDATA\n"); */
	...
	case COMPAT_MSENSOR_IOCTL_SENSOR_ENABLE:
		/*pr_info("akm8963_compat_ioctl COMPAT_MSENSOR_IOCTL_SENSOR_ENABLE\n"); */
		if (arg32 == NULL) {
			AKMDBG("invalid argument.");
			return -EINVAL;
		}
	...
	case COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
		/*pr_info("akm8963_compat_ioctl COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA\n"); */
		if (arg32 == NULL) {
			AKMDBG("invalid argument.");
			return -EINVAL;
		}
...
#endif
/*----------------------------------------------------------------------------*/
static const struct file_operations akm8963_fops = {
	.owner = THIS_MODULE,
	.open = akm8963_open,
...
static int akm8963_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	if (akm8963_onoff) {
		if (msg.event == PM_EVENT_SUSPEND) {
			...
			err = AKECS_SetMode(AK8963_MODE_POWERDOWN);
			if (err < 0) {
				AKMDBG("%s:%d Error.\n", __func__, __LINE__);
				return err;
			}
...
static int akm8963_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strlcpy(info->type, AKM8963_DEV_NAME, sizeof(info->type));
	return 0;
}
...
static int akm8963_open_report_data(int open)
{
	return 0;
}
...
static int akm8963_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	int value = 0;
	...
	AKMDBG("akm8963 mag set delay = (%d) ok.\n", value);
	return 0;
}

static int akm8963_flush(void)
{
...
static int akm8963_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_err(" AKM8963 akm8963_probe: before init prob\n");
	/* Check connection */
...
#if 0
	err = misc_register(&akm8963_device);
	if (err) {
		AKMDBG(KERN_ERR "akm8963_device register failed\n");
		goto exit_misc_device_register_failed;
	}
#endif
	ctl.is_use_common_factory = true /*false */;


	ctl.enable = akm8963_enable;
	ctl.set_delay = akm8963_set_delay;
	ctl.open_report_data = akm8963_open_report_data;
#if 0
	...
	mag_data.get_data = akm8963_m_get_data;
	...
	data->early_drv.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 2,
	    ...
	    data->early_drv.resume = akm8963_late_resume, register_early_suspend(&data->early_drv);
#endif
	akm8963_init_flag = 1;
