...
static int bmp_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	while (left > 0) {
		...
		while (i2c_transfer(client->adapter, &msg[0], 2) != 2) {
			...
			if (retry == 20) {
				BAR_PR_ERR("i2c read register=%#x length=%d failed\n", addr + offset,
					len);
				return -EIO;
			}
		}
...
static int bmp_i2c_write_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	while (left > 0) {
		...
		while (i2c_transfer(client->adapter, &msg, 1) != 1) {
			...
			if (retry == 20) {
				BAR_PR_ERR("i2c write register=%#x length=%d failed\n", buffer[0],
					len);
				return -EIO;
			}
	...
	if (hw->power_id != POWER_NONE_MACRO) {	/* have externel LDO */
		...
		if (power_on == on) {	/* power status not change */
			...
			if (!hwPowerOn(hw->power_id, hw->power_vol, BMP_DEV_NAME))
				BAR_PR_ERR("power on failed\n");
		} else {	/* power off */
			if (!hwPowerDown(hw->power_id, BMP_DEV_NAME))
				BAR_PR_ERR("power off failed\n");
		}
	}
#endif
	power_on = on;
...
static int bmp_get_chip_type(struct i2c_client *client)
{
	...
	if (obj->sensor_type == INVALID_TYPE) {
		BAR_PR_ERR("unknown pressure sensor\n");
		return -1;
	}
	return 0;
}
...
static int bmp_set_powermode(struct i2c_client *client, enum BMP_POWERMODE_ENUM power_mode)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {	/* BMP280 */
		if (power_mode == BMP_SUSPEND_MODE) {
			...
			err = -EINVAL;
			BAR_PR_ERR("invalid power mode = %d\n", power_mode);
	...
	if (err < 0)
		BAR_PR_ERR("set power mode failed, err = %d, sensor name = %s\n",
			err, obj->sensor_name);
	else
...
static int bmp_set_filter(struct i2c_client *client, enum BMP_FILTER_ENUM filter)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {	/* BMP280 */
		if (filter == BMP_FILTER_OFF)
			...
			err = -EINVAL;
			BAR_PR_ERR("invalid hw filter = %d\n", filter);
			mutex_unlock(&obj->lock);
			return err;
		}
	...
	if (err < 0)
		BAR_PR_ERR("set hw filter failed, err = %d, sensor name = %s\n",
			err, obj->sensor_name);
	else
				  ...
				  enum BMP_OVERSAMPLING_ENUM oversampling_p)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {	/* BMP280 */
		if (oversampling_p == BMP_OVERSAMPLING_SKIPPED)
			...
			err = -EINVAL;
			BAR_PR_ERR("invalid oversampling_p = %d\n", oversampling_p);
			mutex_unlock(&obj->lock);
			return err;
		}
	...
	if (err < 0)
		BAR_PR_ERR("set pressure oversampling failed, err = %d,sensor name = %s\n", err,
			obj->sensor_name);
	else
				  ...
				  enum BMP_OVERSAMPLING_ENUM oversampling_t)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {	/* BMP280 */
		if (oversampling_t == BMP_OVERSAMPLING_SKIPPED)
			...
			err = -EINVAL;
			BAR_PR_ERR("invalid oversampling_t = %d\n", oversampling_t);
			mutex_unlock(&obj->lock);
			return err;
		}
	...
	if (err < 0)
		BAR_PR_ERR("set temperature oversampling failed, err = %d, sensor name = %s\n", err,
			obj->sensor_name);
	else
...
static int bmp_read_raw_temperature(struct i2c_client *client, s32 *temperature)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {	/* BMP280 */
		...
		if (err < 0) {
			BAR_PR_ERR("read raw temperature failed, err = %d\n", err);
			mutex_unlock(&obj->lock);
			return err;
		}
...
static int bmp_read_raw_pressure(struct i2c_client *client, s32 *pressure)
{
	...
	if (priv->sensor_type == BMP280_TYPE) {	/* BMP280 */
		...
		if (err < 0) {
			BAR_PR_ERR("read raw pressure failed, err = %d\n", err);
...
static int bmp_get_pressure(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (time_before_eq((unsigned long)(obj->last_temp_measurement +
		...
		if (kstrtos32(temp_buf, 16, &temperature) != 1)
			BAR_PR_ERR("sscanf parsing fail\n");
	}
...
static int bmp_init_client(struct i2c_client *client)
{
	...
	if (err < 0) {
		BAR_PR_ERR("get chip type failed, err = %d\n", err);
		return err;
	}

	err = bmp_get_calibration_data(client);
	if (err < 0) {
		BAR_PR_ERR("get calibration data failed, err = %d\n", err);
		return err;
	}

	err = bmp_set_powermode(client, BMP_SUSPEND_MODE);
	if (err < 0) {
		BAR_PR_ERR("set power mode failed, err = %d\n", err);
		return err;
	}

	err = bmp_set_filter(client, BMP_FILTER_8);
	if (err < 0) {
		BAR_PR_ERR("set hw filter failed, err = %d\n", err);
		return err;
	}

	err = bmp_set_oversampling_p(client, BMP_OVERSAMPLING_8X);
	if (err < 0) {
		BAR_PR_ERR("set pressure oversampling failed, err = %d\n", err);
		return err;
	}

	err = bmp_set_oversampling_t(client, BMP_OVERSAMPLING_1X);
	if (err < 0) {
		BAR_PR_ERR("set temperature oversampling failed, err = %d\n", err);
		return err;
	}
...
static int bmp280_verify_i2c_disable_switch(struct bmp_i2c_data *obj)
{
	...
	BAR_PR_ERR("verification of i2c interface is failure\n");
	return -1;		/* Failure */
}
...
static int bmp_check_calib_param(struct bmp_i2c_data *obj)
{
	...
	if (cali->dig_T1 == 0 && cali->dig_T2 == 0 && cali->dig_T3 == 0
		...
		BAR_PR_ERR("all calibration parameters are zero\n");
		return -2;
	}
...
static int bmp_check_pt(struct bmp_i2c_data *obj)
{
	...
	if (temperature <= -40 * 100 || temperature >= 85 * 100) {
		BAR_PR_ERR("temperature value is out of range:%d*0.01degree\n", temperature);
		return -16;
	}
	...
	if (pressure <= 800 * 100 || pressure >= 1100 * 100) {
		BAR_PR_ERR("pressure value is out of range:%d Pa\n", pressure);
		return -17;
	}
...
static int bmp_do_selftest(struct bmp_i2c_data *obj)
{
	...
	if (err) {
		selftest = 0;
		BAR_PR_ERR("bmp280_verify_i2c_disable_switch:err=%d\n", err);
		goto exit;
	}
	...
	if (err) {
		selftest = 0;
		BAR_PR_ERR("bmp_check_calib_param:err=%d\n", err);
		goto exit;
	}
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj == NULL) {
		BAR_PR_ERR("bmp i2c data pointer is null\n");
		return 0;
	}
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (obj == NULL) {
		BAR_PR_ERR("i2c_data obj is null\n");
		return 0;
	}
			...
			int size_in, void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_ENABLE:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			BAR_PR_ERR("enable sensor parameter error\n");
			err = -EINVAL;
		} else {
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			BAR_PR_ERR("get sensor data parameter error\n");
			err = -EINVAL;
		} else {
			temperature_data = (hwm_sensor_data *) buff_out;
			err = bmp_get_temperature(priv->client, buff, BMP_BUFSIZE);
			if (err) {
				BAR_PR_ERR("get compensated temperature value failed,err = %d\n", err);
				return -1;
			}
	...
	default:
		BAR_PR_ERR("temperature operate function no this parameter %d\n", command);
		err = -1;
		break;
	}
...
static long bmp_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (err) {
		BAR_PR_ERR("access error: %08X, (%2d, %2d)\n", cmd, _IOC_DIR(cmd), _IOC_SIZE(cmd));
		return -EFAULT;
	}
	...
	default:
		BAR_PR_ERR("unknown IOCTL: 0x%08x\n", cmd);
		err = -ENOIOCTLCMD;
		break;
	}
...
static long compat_bmp_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (!filp->f_op || !filp->f_op->unlocked_ioctl) {
		BAR_PR_ERR("compat_ion_ioctl file has no f_op or no f_op->unlocked_ioctl.\n");
		return -ENOTTY;
	}
	...
	default:
		BAR_PR_ERR("compat_ion_ioctl : No such command!! 0x%x\n", cmd);
		return -ENOIOCTLCMD;
	}
}
#endif
...
static int bmp_suspend(struct device *dev)
{
	...
	if (err) {
		BAR_PR_ERR("bmp set suspend mode failed, err = %d\n", err);
		return err;
	}
	bmp_power(obj->hw, 0);
	return err;
...
static int bmp_resume(struct device *dev)
{
	...
	if (err) {
		BAR_PR_ERR("initialize client fail\n");
		return err;
	}

	err = bmp_set_powermode(obj->client, BMP_NORMAL_MODE);
	if (err) {
		BAR_PR_ERR("bmp set normal mode failed, err = %d\n", err);
		return err;
	}
#ifdef CONFIG_BMP_LOWPASS
...
static int bmp_enable_nodata(int en)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		BAR_PR_ERR("bmp_set_powermode fail\n");
	}
...
static int bmp_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	...
	if (err) {
		BAR_PR_ERR("%s enable sensor failed!\n", __func__);
		return -1;
	}
	err = bmp_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
		BAR_PR_ERR("%s enable set batch failed!\n", __func__);
		return -1;
	}
	return 0;
}
...
static int bmp_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!hw) {
		BAR_PR_ERR("get cust_baro dts info fail\n");
		return 0;
	}
	bmp_power(hw, 1);
	...
	if (err) {
		BAR_PR_ERR("baro_factory device register failed, err = %d\n", err);
	...
	err =
	    batch_register_support_info(ID_PRESSURE, obj->hw->is_batch_supported, data.vender_div,
					0);
	if (err) {
		BAR_PR_ERR("register baro batch support err = %d\n", err);
		goto exit_hwmsen_attach_pressure_failed;
	}
#endif
#ifdef CONFIG_ID_TEMPERATURE
	...
	if (err) {
		BAR_PR_ERR("hwmsen attach failed, err = %d\n", err);
...
static int bmp_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		BAR_PR_ERR("hwmsen_detach ID_TEMPRERATURE failed, err = %d\n", err);
#endif

	err = bmp_delete_attr(&(bmp_init_info.platform_diver_addr->driver));
	if (err)
		BAR_PR_ERR("bmp_delete_attr failed, err = %d\n", err);
