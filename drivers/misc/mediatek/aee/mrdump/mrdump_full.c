...
static u32 *append_elf_note(u32 *buf, char *name, unsigned type, void *data, size_t data_len)
{
...
static int __init mrdump_init(void)
{
	...
	if ((0 != memcmp(mrdump_cb_init->sig, MRDUMP_VERSION, 8))
	    && (0 != memcmp(mrdump_cb_init->sig, MRDUMP_LK_DUMP, 8))) {
		mrdump_enable = 0;
		pr_err("%s: MT-RAMDUMP init failed. Version not matched.\n", __func__);
		return -EINVAL;
	}
	...
	machdesc_p->nr_cpus = mrdump_enable ? num_possible_cpus() : 0;
	...
	if (!crash_notes) {
		pr_notice("Kexec: Memory allocation for saving cpu register" " states failed\n");
		return -ENOMEM;
	}
...
static int param_set_mrdump_enable(const char *val, const struct kernel_param *kp)
{
	...
	if ((mrdump_cb != NULL) && (mrdump_plat != NULL)
	    && (0 == memcmp(mrdump_cb->sig, MRDUMP_GO_DUMP, 8))) {
		retval = param_set_bool(val, kp);
		if (retval == 0) {
			mrdump_plat->hw_enable(mrdump_enable);
			mrdump_cb->machdesc.nr_cpus = mrdump_enable ? num_possbile_cpus() : 0;
			__inner_flush_dcache_all();
		}
	}
	return retval;
...
static int param_set_mrdump_lbaooo(const char *val, const struct kernel_param *kp)
{
	...
	if ((retval == 0) && (mrdump_cb != NULL)
	    && (mrdump_cb->machdesc.output_fstype == MRDUMP_FS_EXT4)) {
