...
static ssize_t udi_jtag_clock_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if ((recv_buf[0] < 0) || (recv_buf[0] > 3)) {
		udi_info("ERROR: Sub-Chains  ont 1~3\n");
		goto out1;
	} else {
	...
	if (recv_buf[1] == 0)
		...
		udi_info("ERROR: IR string too short (%zu characters not match with %u bits)\n",
	...
	if (recv_buf[2] == 0)
		...
		udi_info("ERROR: DR string too short (%zu hex characters not match with %u bits)\n",
