...
static void start_devapc(void)
{
	...
	DEVAPC_MSG(
	    "[DEVAPC] INFRA VIO_MASK 0:0x%x 1:0x%x 2:0x%x 3:0x%x 4:0x%x 5:0x%x 6:0x%x 7:0x%x 8:0x%x 9:0x%x\n",
	...
	DEVAPC_MSG(
		   "[DEVAPC] INFRA VIO_STA 0:0x%x 1:0x%x 2:0x%x 3:0x%x 4:0x%x 5:0x%x 6:0x%x 7:0x%x 8:0x%x 9:0x%x\n",
	...
	DEVAPC_MSG(
	    "[DEVAPC] Clear INFRA VIO_STA and unmask INFRA VIO_MASK...\n");
...
static irqreturn_t devapc_violation_irq(int irq_number, void *dev_id)
{
	...
	if (irq_number == devapc_infra_irq) {
		...
		DEVAPC_VIO_MSG(
		    ...
		    "INFRA VIO_MASK", readl(DEVAPC_PD_INFRA_VIO_MASK(0)),
		    readl(DEVAPC_PD_INFRA_VIO_MASK(1)),
		    readl(DEVAPC_PD_INFRA_VIO_MASK(2)),
		    readl(DEVAPC_PD_INFRA_VIO_MASK(3)),
		    readl(DEVAPC_PD_INFRA_VIO_MASK(4)),
		...
		DEVAPC_VIO_MSG(
		    "[DEVAPC] %s 0:0x%x 1:0x%x 2:0x%x 3:0x%x 4:0x%x 5:0x%x 6:0x%x 7:0x%x 8:0x%x 9:0x%x\n",
		    "INFRA VIO_STA", readl(DEVAPC_PD_INFRA_VIO_STA(0)),
		    readl(DEVAPC_PD_INFRA_VIO_STA(1)),
		    readl(DEVAPC_PD_INFRA_VIO_STA(2)),
		    readl(DEVAPC_PD_INFRA_VIO_STA(3)),
		    readl(DEVAPC_PD_INFRA_VIO_STA(4)),
		...
		for (i = 0; i <= PD_INFRA_VIO_SHIFT_MAX_BIT; ++i)
			if (readl(DEVAPC_PD_INFRA_VIO_SHIFT_STA) &
			    (0x1 << i)) {
				mt_reg_sync_writel(
				    0x1 << i, DEVAPC_PD_INFRA_VIO_SHIFT_SEL);
				mt_reg_sync_writel(
				    0x1, DEVAPC_PD_INFRA_VIO_SHIFT_CON);
				for (shift_done = 0;
				     (shift_done < 100) &&
				     ...
				     ++shift_done)
					DEVAPC_VIO_MSG(
					    "[DEVAPC] Syncing INFRA DBG0 & DBG1 (%d, %d)\n",
					    i, shift_done);
#if DEVAPC_VIO_DEBUG
				DEVAPC_VIO_MSG(
				    "[DEVAPC] VIO_SHIFT_SEL=0x%X, VIO_SHIFT_CON=0x%X\n",
				    readl(DEVAPC_PD_INFRA_VIO_SHIFT_SEL),
				    readl(DEVAPC_PD_INFRA_VIO_SHIFT_CON));
#endif
				if ((readl(DEVAPC_PD_INFRA_VIO_SHIFT_CON) &
				     0x3) == 0x3) {
					DEVAPC_VIO_MSG(
						"[DEVAPC] Sync INFRA DBG0 & DBG1 (%d) SUCCESS\n",
						i);
					shift_done = 1;
				} else {
					DEVAPC_VIO_MSG(
						"[DEVAPC] Sync INFRA DBG0 & DBG1 (%d) FAIL\n",
						i);
					shift_done = 0;
				}

				mt_reg_sync_writel(
				    0x0, DEVAPC_PD_INFRA_VIO_SHIFT_CON);
				if (shift_done) {
					dbg0 = readl(DEVAPC_PD_INFRA_VIO_DBG0);
					dbg1 = readl(DEVAPC_PD_INFRA_VIO_DBG1);
					master_id =
					    (dbg0 & INFRA_VIO_DBG_MSTID) >>
					    INFRA_VIO_DBG_MSTID_START_BIT;
					domain_id =
					    (dbg0 & INFRA_VIO_DBG_DMNID) >>
					    INFRA_VIO_DBG_DMNID_START_BIT;
					write_violation =
					    (dbg0 & INFRA_VIO_DBG_W_VIO) >>
					    INFRA_VIO_DBG_W_VIO_START_BIT;
					read_violation =
					    (dbg0 & INFRA_VIO_DBG_R_VIO) >>
					    INFRA_VIO_DBG_R_VIO_START_BIT;
					vio_addr_high =
					    (dbg0 & INFRA_VIO_ADDR_HIGH) >>
					    INFRA_VIO_ADDR_HIGH_START_BIT;
				}
				mt_reg_sync_writel(
				    0x0, DEVAPC_PD_INFRA_VIO_SHIFT_SEL);
				mt_reg_sync_writel(
				    0x1 << i, DEVAPC_PD_INFRA_VIO_SHIFT_STA);
#if DEVAPC_VIO_DEBUG
				DEVAPC_VIO_MSG(
				...
				if (shift_done)
					DEVAPC_VIO_MSG(
					    ...
					    read_violation == 1 ? "R" : " ",
					    write_violation == 1 ? "W)" : " )",
					    "Process:", current->comm, "PID:",
					    current->pid, "Vio Addr:0x", dbg1,
		...
		/* No need to check violation of EMI & EMI MPU slaves for Infra
		 * because they will not be unmasked
		 */
		...
		DEVAPC_VIO_MSG(
		    "[DEVAPC] (ERROR) irq_number %d is not registered!\n",
		    irq_number);
	}
	...
	if ((DEVAPC_ENABLE_ONE_CORE_VIOLATION_DEBUG) ||
		...
		DEVAPC_VIO_MSG(
			"[DEVAPC] ====== Start dumping Device APC violation tracing ======\n");

		DEVAPC_VIO_MSG(
			"[DEVAPC] **************** [All IRQ Registers] ****************\n");
		regs = get_irq_regs();
		show_regs(regs);

		DEVAPC_VIO_MSG(
			"[DEVAPC] **************** [All Current Task Stack] ****************\n");
		show_stack(current, NULL);

		DEVAPC_VIO_MSG(
			"[DEVAPC] ====== End of dumping Device APC violation tracing ======\n");
	}
...
static int devapc_probe(struct platform_device *pdev)
{
	...
	if (devapc_pd_infra_base == NULL) {
		if (node) {
			devapc_pd_infra_base = of_iomap(
			    node, DAPC_DEVICE_TREE_NODE_PD_INFRA_INDEX);
			devapc_infra_irq = irq_of_parse_and_map(
			    node, DAPC_DEVICE_TREE_NODE_PD_INFRA_INDEX);
			DEVAPC_MSG("[DEVAPC] PD_INFRA_ADDRESS: %p, IRQ: %d\n",
				   devapc_pd_infra_base, devapc_infra_irq);
		} else {
			pr_debug("[DEVAPC] can't find DAPC_INFRA_PD compatible node\n");
			return -1;
		}
	}
...
#if DEVAPC_TURN_ON
	ret = request_irq(
	    devapc_infra_irq, (irq_handler_t)devapc_violation_irq,
...
/* CCF */
	...
	if (IS_ERR(dapc_infra_clk)) {
		pr_debug(
			"[DEVAPC] (Infra) Cannot get devapc clock from common clock framework.\n");
