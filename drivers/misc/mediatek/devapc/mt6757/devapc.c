...
int mt_devapc_emi_initial(void)
{
	...
	if (devapc_pd_base != NULL) {
		mt_reg_sync_writel(readl(IOMEM(DEVAPC0_PD_APC_CON)) &
				       (0xFFFFFFFF ^ (1 << 2)),
				   DEVAPC0_PD_APC_CON);
		mt_reg_sync_writel(ABORT_EMI, DEVAPC0_D0_VIO_STA_7);
		mt_reg_sync_writel(ABORT_EMI_MPU, DEVAPC0_D0_VIO_STA_7);

		/* Notice: You cannot unmask Type B slave (e.g. EMI, EMI_MPU)
		 * unless unregistering IRQ
		 */
		/* EMI and EMI_MPU are moved from Type B to Type 2 slave */

		pr_debug("[DEVAPC] EMI_MPU Init done\n");
		return 0;
	}
...
static int devapc_ioremap(void)
{
	if (devapc_pd_base == NULL) {
		...
		if (node) {
			...
			pr_debug(
			    "[DEVAPC] can't find DAPC_PD compatible node\n");
			return -1;
		}
	}
...
static void start_devapc(void)
{
	...
	mt_reg_sync_writel(readl(DEVAPC0_PD_APC_CON) & (0xFFFFFFFF ^ (1 << 2)),
			   DEVAPC0_PD_APC_CON);
	...
	pr_debug("[DEVAPC] %s: Current VIO_STA 0:0x%x, 1:0x%x, 2:0x%x, 3:0x%x, 4:0x%x, 5:0x%x, 6:0x%x, 7:0x%x\n",
		 __func__,
	...
	pr_debug("[DEVAPC] %s: Current VIO_MASK 0:0x%x, 1:0x%x, 2:0x%x, 3:0x%x, 4:0x%x, 5:0x%x, 6:0x%x, 7:0x%x\n",
		 __func__,
	...
	if (devapc_vio_aee_shown[i] == 0) {
		if (devapc_vio_count[i] < DEVAPC_VIO_AEE_TRIGGER_TIMES) {
			...
			if (devapc_vio_count[i] == 1) {
				/* violation for this slave is triggered the
				 * first time
				 */
		...
		if (devapc_vio_count[i] >= DEVAPC_VIO_AEE_TRIGGER_TIMES) {
		  ...
		  /* get current time from start-up in ns */
			...
			if (((current_time - devapc_vio_first_trigger_time[i]) /
			  ...
			  /* diff time by ms */
				/* Mark the flag for showing AEE (AEE should be
				 * shown only once)
				 */
				devapc_vio_aee_shown[i] = 1;

				DEVAPC_VIO_MSG(
				    "[DEVAPC] Populating AEE Warning...\n");
				...
				if (devapc_vio_current_aee_trigger_times <
					...
					aee_kernel_warning(
					    ...
					    devapc_devices[i].device, dbg1,
					    "CRDISPATCH_KEY:Device APC "
					    "Violation Issue/",
...
static irqreturn_t devapc_violation_irq(int irq, void *dev_id)
{
	...
	if (r_w_violation == 1) {
		DEVAPC_VIO_MSG("[DEVAPC] Violation(W) - Process:%s, PID:%i,Vio Addr:0x%x, Bus ID:0x%x, Dom ID:0x%x, DBG0:0x%x\n",
			       current->comm, current->pid, dbg1, master_id,
			       domain_id, (*DEVAPC0_VIO_DBG0));
	} else {
		DEVAPC_VIO_MSG("[DEVAPC] Violation(R) - Process:%s, PID:%i, Vio Addr:0x%x, Bus ID:0x%x, Dom ID:0x%x,DBG0:0x%x\n",
			       current->comm, current->pid, dbg1, master_id,
			       domain_id, (*DEVAPC0_VIO_DBG0));
	}
	...
	DEVAPC_VIO_MSG("[DEVAPC] VIO_STA 0:0x%x, 1:0x%x, 2:0x%x, 3:0x%x,4:0x%x, 5:0x%x, 6:0x%x, 7:0x%x\n",
		       ...
		       readl(DEVAPC0_D0_VIO_STA_2), readl(DEVAPC0_D0_VIO_STA_3),
		       readl(DEVAPC0_D0_VIO_STA_4), readl(DEVAPC0_D0_VIO_STA_5),
		       readl(DEVAPC0_D0_VIO_STA_6),
		       readl(DEVAPC0_D0_VIO_STA_7));
	...
	if (check_vio_status(INDEX_EMI))
		DEVAPC_VIO_MSG(
		    "[DEVAPC] Access Violation Slave: EMI (index=%d)\n",
		    INDEX_EMI);

	if (check_vio_status(INDEX_EMI_MPU))
		DEVAPC_VIO_MSG(
		    "[DEVAPC] Access Violation Slave: EMI_MPU (index=%d)\n",
		    INDEX_EMI_MPU);
	...
	for (i = 0; i < device_count; i++) {
		...
		if ((check_vio_status(i)) &&
		    (devapc_devices[i].enable_vio_irq == true)) {
			DEVAPC_VIO_MSG(
			    "[DEVAPC] Access Violation Slave: %s (index=%d)\n",
			    devapc_devices[i].device, i);

#if defined(CONFIG_MTK_AEE_FEATURE) && defined(DEVAPC_ENABLE_AEE)
			/* Frequency-based Violation AEE Warning (Under the
			 * condition that the violation
			 */
			/* for the module is not shown, it will trigger the AEE
			 * if "x" violations in "y" ms)
			 */
...
/*
 * Interrupts of vilation (including SPC in SMI, or EMI MPU) are triggered by
 * the device APC.
 * need to share the interrupt with the SPC driver.
 */
#if DEVAPC_TURN_ON
	ret = request_irq(devapc_irq, (irq_handler_t)devapc_violation_irq,
			  IRQF_TRIGGER_LOW | IRQF_SHARED, "devapc",
			  &g_devapc_ctrl);
	if (ret) {
		pr_debug("[DEVAPC] Failed to request irq! (%d)\n", ret);
		return ret;
	}
#endif
...
/* CCF */
	...
	if (IS_ERR(dapc_clk)) {
		pr_debug("[DEVAPC] cannot get devapc clock from common clock framework.\n");
...
#ifdef CONFIG_MTK_HIBERNATION
	register_swsusp_restore_noirq_func(ID_M_DEVAPC, devapc_pm_restore_noirq,
					   NULL);
#endif
...
static int devapc_remove(struct platform_device *dev) { return 0; }
				...
				size_t count, loff_t *data)
{
	...
	if (input_type == DAPC_INPUT_TYPE_DEBUG_ON) {
		...
		pr_debug("[DEVAPC] One-Core Debugging: Enabled\n");
	} else if (input_type == DAPC_INPUT_TYPE_DEBUG_OFF) {
		enable_dynamic_one_core_violation_debug = 0;
		pr_debug("[DEVAPC] One-Core Debugging: Disabled\n");
	}

	return count;
}

static int devapc_dbg_open(struct inode *inode, struct file *file) { return 0; }
