...
/*
 * PD9402VAF voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "PD9402VAF_DRV"
...
static int initAF(void)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		ret = s4AF_WriteReg(0, 0x02, 0x01); //Power down mode
		s4AF_WriteReg(0, 0x02, 0x00); //Normal operation mode
		mdelay(1);
		s4AF_WriteReg(0, 0x02, 0x02); //enable SDC mode
		s4AF_WriteReg(0, 0x06, 0x80);
		s4AF_WriteReg(0, 0x07, 0x66); //setting SDC_C mode Tvib=10.13ms
...
int MOT_SYDNEY_PD9402VAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	int Ret = 0;
...
int MOT_SYDNEY_PD9402VAF_PowerDown(struct i2c_client *pstAF_I2Cclient,
			int *pAF_Opened)
{
...
int MOT_SYDNEY_PD9402VAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int MOT_SYDNEY_PD9402VAF_GetFileName(unsigned char *pFileName)
{
