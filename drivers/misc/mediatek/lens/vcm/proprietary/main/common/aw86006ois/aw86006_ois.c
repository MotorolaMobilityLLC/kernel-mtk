...
#define AW86006VCMYOVA_FW_NAME      "mot_aw86006vcmyova.prog"
								...
								uint8_t *pactuator_id)
{
	...
	temp_addr = o_ctrl->client->addr;
	o_ctrl->client->addr = OV50D_EEPROM_I2C_ADDR;
	ret = aw86006_i2c_reads(o_ctrl, OV50D_EEPROM_ACTUATOR_ID_POSITION-1, AW_SIZE_BYTE_2, &actuator_id,
						...
						const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d %d %d", &r_um, &step, &cycles, &accuracy_um) == 4) {
		...
		aw86006_ois_drawcircle_test(r_um, step, cycles, accuracy_um, 150, 30);
	}
