...
static int initdrv(void)
{
	...
	{0x02, 0x02}, {0x06, 0x80}, {0x07, 0x62},
	};
	unsigned char cmd_number;

	LOG_INF("InitDrv debug\n");


	for (cmd_number = 0; cmd_number < 6; cmd_number++) {
		if ((puSendCmdArray[cmd_number][0] != 0xFE) && (g_pstAF_I2Cclient != NULL)) {
			g_pstAF_I2Cclient->addr = AF_I2C_SLAVE_ADDR;
			g_pstAF_I2Cclient->addr = g_pstAF_I2Cclient->addr >> 1;
...
/* ////////////////////////////////////////////////////////////// */
long MOT_DEVONN_DW9800VAF_Ioctl_Main(struct file *a_pstFile,
...
int MOT_DEVONN_DW9800VAF_Release_Main(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
int MOT_DEVONN_DW9800VAF_SetI2Cclient_Main(struct i2c_client *pstAF_I2Cclient,
...
int MOT_DEVONN_DW9800VAF_GetFileName_Main(unsigned char *pFileName)
{
