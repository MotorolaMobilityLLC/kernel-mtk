...
static int initAF(void)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		for (regIdx=0; regIdx<5; regIdx++) {
			...
			if (gt6764_init_setting[regIdx][1] == 0x00) {
				mdelay(1);
			}
		}
...
/* ////////////////////////////////////////////////////////////// */
long MOT_BOGOTA_GT9764AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int MOT_BOGOTA_GT9764AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	int Ret = 0;
...
int MOT_BOGOTA_GT9764AF_PowerDown(struct i2c_client *pstAF_I2Cclient,
			int *pAF_Opened)
{
...
int MOT_BOGOTA_GT9764AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int MOT_BOGOTA_GT9764AF_GetFileName(unsigned char *pFileName)
{
