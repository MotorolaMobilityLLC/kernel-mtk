...
static bool project_p410ae_motor;
...
static	struct mutex vibrate_lock;
...
static long AF_SetMotorName(__user struct stAF_MotorName *pstMotorName)
{
	...
	/* Distinguish project by motor name start. */
	if (false == project_p410ae_motor && !strcmp("DW9800VAF", stMotorName.uMotorName))
	{
		LOG_INF("Motor name :%s by p410ae project.", stMotorName.uMotorName);
		project_p410ae_motor = true;
	}
	/* Distinguish project by motor name end. */
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (!project_p410ae_motor)
	{
		return -1;
		LOG_INF("Other project return -1");
	}
