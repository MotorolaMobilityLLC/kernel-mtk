...
void dw9800MoveAFToZero(unsigned long a_u4Position)
{
	unsigned long currPosition = 512;
	LOG_INF("dw9800MoveAFToZero start %d\n",currPosition);

	while (currPosition > 17) {
		currPosition -= 17;
		moveAF(currPosition);
		mdelay(3);
	}
	LOG_INF("dw9800MoveAFToZero end %d\n",currPosition);
	moveAF(a_u4Position);
}
	...
	if (*g_pAF_Opened == 2) {
            ...
            if (g_u4CurrPosition > endPos) {
                if (g_u4CurrPosition > 800) {
                    nextPosition = 800;
                    LOG_INF(":nextPosition = %d g_u4CurrPosition = %d g_u4AF_MACRO = %d", nextPosition, g_u4CurrPosition,g_u4AF_MACRO);
                    if (moveAF(nextPosition) == 0) {
                        g_u4CurrPosition = nextPosition;
                        ret = 0;
                    } else {
                        LOG_INF("move motor error!\n");
                        ret = -1;
                    }
                }
                mdelay(20);
                ...
                while (g_u4CurrPosition > endPos + af_step) {
                    ...
                    LOG_INF(":af_step = %d g_u4CurrPosition = %d nextPosition = %d\n", af_step, g_u4CurrPosition, nextPosition);
                    if (moveAF(nextPosition) == 0) {
                         g_u4CurrPosition = nextPosition;
                        ret = 0;
                    } else {
                ...
                // <512 multiple times
                while (g_u4CurrPosition < endPos - 5) {
                    nextPosition = g_u4CurrPosition + 5;//af_step;
                    if (moveAF(nextPosition) == 0) {
                        g_u4CurrPosition = nextPosition;
                        ret = 0;
                    } else {
