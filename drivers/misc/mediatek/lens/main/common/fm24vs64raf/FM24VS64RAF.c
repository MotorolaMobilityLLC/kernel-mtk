...
static int initAF(void)
{
	...
	//设置TIMING_SCALE  X2
	puSendCmd[0] = (char)(REG_TIMING_SCALE);
	...
	//设置DMC_TIMING  0X3B
	puSendCmd[0] = (char)(REG_DAC_TIMING);
	puSendCmd[1] = (char)(0x3B);
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
		if (i4RetValue < 0) {
		LOG_INF("I2C send failed!!\n");
		return -1;
...
/* ////////////////////////////////////////////////////////////// */
long FM24VS64RAF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
int FM24VS64RAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
int FM24VS64RAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
...
int FM24VS64RAF_GetFileName(unsigned char *pFileName)
{
