...
static ssize_t sgm7220_regdump_show(struct sgm7220_chip *info)
{
        int i, rc, ret = 0;
	u8 reg_val;

        for (i = 0; i <= 0x0A; i++) {
                rc = sgm7220_read_reg(info->client, (uint8_t)i, &reg_val);
                if (rc < 0) {
                        pr_err("cannot read 0x%02x\n", i);
                        rc = 0;
                }
		pr_err("sgm7220 reg %d value : %d\n", i, reg_val);
        }
        return ret;
}
/***********************************************************
...
static void sgm7220_irq_work_handler(struct kthread_work *work)
{
    ...
    rc = sgm7220_read_reg(chip->client, REG_INT, &reg_val);
    if (rc < 0) {
        pr_err("cannot read 0x%02x\n", reg_val);
        return;
    }
    int_sts = (reg_val & INT_INTERRUPT_STATUS) >> INT_INTERRUPT_STATUS_SHIFT;
    type = reg_val >> INT_ATTACHED_STATE_SHIFT;
    pr_err("%s read int_sts =  0x%0x, type = 0x%x\n",__func__, int_sts, type);
    switch (type) {
        case SET_MODE_SELECT_SNK:
...
static int sgm7220_parse_dt(struct sgm7220_chip *chip, struct device *dev)
{
	...
	if (ret < 0) {
		pr_err("%s no sgm7220,irq_gpio info\n", __func__);
		return ret;
	}
	chip->irq_gpio = ret;
#else
