...
static int wusb3801_i2c_wr_rd( struct tcpc_device *tcpc, uint8_t reg, uint8_t val)
{
    int i, ret;
    struct wusb3801_chip *chip = tcpc_get_dev_data(tcpc);

    for (i = 0; i < I2C_RETRY_CNT; i++) {
        wusb3801_i2c_write8(chip->tcpc, reg, val);
        msleep(2);
        ret = wusb3801_i2c_read8(chip->tcpc, reg);
        if ((ret >= 0) && (val == ret)) {
            return 0;
        }
    }

    pr_err("wusb3801_i2c_wr_rd is timeout more than five\n");
    return 1;
}
...
static int test_cc_patch(struct wusb3801_chip *chip)
{
    ...
    ret =  wusb3801_i2c_wr_rd(chip->tcpc, WUSB3801_REG_TEST_02, 0x82);
    if (ret) {
        pr_err("%s write & read WUSB3801_REG_TEST_02 fail \n", __func__);
        return (chip->cc_sts);
    }

    ret =  wusb3801_i2c_wr_rd(chip->tcpc, WUSB3801_REG_TEST_09, 0xC0);
    if (ret) {
        pr_err("%s write & read WUSB3801_REG_TEST_09 fail \n", __func__);
        return (chip->cc_sts);
    }

    rc_reg_08 = wusb3801_i2c_read8(chip->tcpc, WUSB3801_REG_TEST0);

    ret =  wusb3801_i2c_wr_rd(chip->tcpc, WUSB3801_REG_TEST_09, 0x00);
    if (ret) {
        pr_err("%s write & read WUSB3801_REG_TEST_09 fail \n", __func__);
        return (chip->cc_sts);
    }

    ret = wusb3801_i2c_wr_rd(chip->tcpc, WUSB3801_REG_TEST_02, 0x80);
    if (ret) {
        pr_err("%s write & read WUSB3801_REG_TEST_02 fail \n", __func__);
        return (chip->cc_sts);
    }

    ret = wusb3801_i2c_wr_rd(chip->tcpc, WUSB3801_REG_TEST_02, 0x00);
    if (ret) {
        pr_err("%s write & read WUSB3801_REG_TEST_02 fail \n", __func__);
        return (chip->cc_sts);
    }
    ...
    dev_err(cdev, "%s reg.06 = [0x%02x] \n", __func__, rc_reg_08);
    msleep(80);
    return BITS_GET(rc_reg_08, 0x40);
}
#endif /* __TEST_CC_PATCH__ */
...
static void wusb3801_irq_work_handler(struct kthread_work *work)
{
    ...
    uint8_t status, type;
    struct tcpc_device *tcpc;
    static bool otg_serivce_flag = 0;
        ...
        /*OTG MODE NOT ENTRY TEST_XX_PATH*/
        if ((rc == 0x88) && (otg_serivce_flag == 1)) {
                otg_serivce_flag = 0;
                pr_err("WUSB3801 OTG MODE NOT ENTRY TEST_XX_PATH\n");
...
static inline int wusb3801_software_reset(struct tcpc_device *tcpc)
{
        struct wusb3801_chip *chip = tcpc_get_dev_data(tcpc);
        int ret;
        int nvalue = 0;
...
static int wusb3801_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
        ...
        pr_info("%s enter \n",__func__);
        if (sw_reset) {
                ret = wusb3801_software_reset(tcpc);
                if (ret < 0)
                        return ret;
        }
        /*for REG 2 init*/
                ...
                struct tcpc_device *tcpc, uint16_t *pwr_status)
{
        ...
        pr_err("%s WUSB3801_REG_STATUS : rc = 0x%02x\n", __func__, rc);
        rc &= WUSB3801_ATTACH;
        *pwr_status = 0;

        if (rc & WUSB3801_TYPE_SNK) {
                /* wusb3801 does not supprt vbus detection,
                assume vbus is detected if act as UFP */
                *pwr_status |= TCPC_REG_POWER_STATUS_VBUS_PRES;
        }

        if (rc & WUSB3801_TYPE_SRC) {
...
static int wusb3801_get_cc(struct tcpc_device *tcpc, int *cc1, int *cc2)
{
        ...
        *cc2 = TYPEC_CC_VOLT_OPEN;
        ...
        pr_info("%s enter \n",__func__);
        active_cc = (typec_cc_orientation - 1)? cc2 : cc1;
        *active_cc = typec_cc_orientation;
...
/*      wakeup_source_init(&chip->irq_wake_lock,
        ...
        pr_info("wusb3801_chipID11111 = 0x%0x\n", chip_id);
...
static bool is_or_not_adv_configuration(void)
{
    ...
    ptr = strstr(saved_command_line, "androidboot.boardid=");
    if (ptr) {
        ptr += strlen("androidboot.boardid=");
        boardid_index = simple_strtol(ptr, NULL, 10);
        pr_err("%s:%d, boardid: %d", __func__,__LINE__,boardid_index);
    } else {
        pr_err("%s:%d, cannot find androidboot.boardid", __func__,__LINE__);
        return false;
    }
