...
#define MT6370_DRV_VERSION	"1.1.8_MTK"
...
static int mt6370_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	mt6370_i2c_write16(tcpc, MT6370_REG_DRP_DUTY_CTRL, 400);
...
static int mt_parse_dt(struct mt6370_chip *chip, struct device *dev)
{
	...
	chip->irq_gpio = ret;
#else
	ret = of_property_read_u32(np, "mt6370pd,intr_gpio_num",
		&chip->irq_gpio);
	if (ret < 0)
		pr_err("%s no intr_gpio info\n", __func__);
#endif
...
/* 1.1.1_MTK
 *	-- first release version, sync to mt6370 pd driver revision 13
 * 1.1.2_MTK
 *	-- sync to mt6370 pd driver v014
 * 1.1.3_MTK
 *	-- sync to mt6370 pd driver v015
 * 1.1.4_MTK
 *	-- modify dws name mt6370_pd->usb_type_c
 * 1.1.5_MTK
 *	-- sync to mt6370 pd driver v017
 * 1.1.6_MTK
 *	-- sync to mt6370 pd driver v018
 * 1.1.7_MTK
 *	-- sync to mt6370 pd driver v019
 * 1.1.8_MTK
 *	-- sync to mt6370 pd driver v020
 */
