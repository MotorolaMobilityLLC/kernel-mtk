...
static int hmac_sha256(u8 *key, u8 ksize, char *plaintext, u8 psize, u8 *output)
{
	...
	if (IS_ERR(tfm)) {
		pr_info("crypto_alloc_ahash failed: err %ld", PTR_ERR(tfm));
		return PTR_ERR(tfm);
	}

	ret = crypto_shash_setkey(tfm, key, ksize);
	if (ret) {
		pr_info("crypto_ahash_setkey failed: err %d", ret);
		goto failed;
	}
		...
		struct svdm_svid_data *svid_data, bool ack)
{
    ...
    case MMI_REQUEST_CAP:
        pr_info("mmi_dfp_notify_uvdm  MMI_REQUEST_CAP \n");
    ...
    case MMI_REQUEST_TEMP:
        if (mmi_uvdm_ack(pd_port,svid_data)) {
            pr_info("now adapter temp = %d\n",pd_port->uvdm_data[1]);
            mmi_adapter_set_state(pd_port,MMI_ENCRYPT);
            return true;
        }
        break;
    case MMI_ENCRYPT:
        pr_info("mmi_dfp_notify_uvdm  MMI_ENCRYPT \n");
        if (mmi_uvdm_ack(pd_port,svid_data)) {
            /**
        ...
        break;
    case MMI_VERIFY:
        pr_info("mmi_dfp_notify_uvdm  MMI_VERIFY \n");
        if (mmi_uvdm_ack(pd_port,svid_data) &&
...
int mmi_notify_pe_ready(struct pd_port *pd_port,
		struct svdm_svid_data *svid_data)
{
    pr_info("mmi_notify_pe_ready:  %d \n",pd_port->mmi_adapter_state);
    switch(pd_port->mmi_adapter_state) {
    case MMI_REQUEST_CAP:
