				       ...
				       int brightness_sel)
{
	...
	return fled_dev->hal->
		rt_hal_fled_set_torch_current_sel(fled_dev, brightness_sel);
}
					...
					int brightness_sel)
{
	...
	return fled_dev->hal->
		rt_hal_fled_set_strobe_current_sel(fled_dev, brightness_sel);
}
				      ...
				      int selector)
{
	...
	return fled_dev->hal->rt_hal_fled_strobe_timeout_list(fled_dev, selector);
}
					...
					int min_uA, int max_uA, int *selector)
{
	...
	for (sel = 0; ; sel++) {
		rc = fled_dev->hal->rt_hal_fled_torch_current_list(fled_dev, sel);
		if (rc < 0)
			return rc;
		if (rc >= min_uA && rc <= max_uA) {
			*selector = sel;
			return fled_dev->hal->
				rt_hal_fled_set_torch_current_sel(fled_dev, sel);
		}
	}
	return -EINVAL;
}
		...
		int min_uA, int max_uA, int *selector)
{
	...
	for (sel = 0; ; sel++) {
		rc = fled_dev->hal->rt_hal_fled_strobe_current_list(fled_dev, sel);
		if (rc < 0)
			return rc;
		if (rc >= min_uA && rc <= max_uA) {
			*selector = sel;
			return fled_dev->hal->
				rt_hal_fled_set_strobe_current_sel(fled_dev, sel);
		}
	}
	return -EINVAL;
}
					...
					int min_uA, int max_uA, int *selector)
{
	...
	for (sel = 0; ; sel++) {
		rc = fled_dev->hal->rt_hal_fled_timeout_level_list(fled_dev, sel);
		if (rc < 0)
			return rc;
		if (rc >= min_uA && rc <= max_uA) {
			*selector = sel;
			return fled_dev->hal->
				rt_hal_fled_set_timeout_level_sel(fled_dev, sel);
		}
	}
	return -EINVAL;
}
					...
					int min_mV, int max_mV, int *selector)
{
	...
	for (sel = 0; ; sel++) {
		rc = fled_dev->hal->rt_hal_fled_lv_protection_list(fled_dev, sel);
		if (rc < 0)
			return rc;
		if (rc >= min_mV && rc <= max_mV) {
			*selector = sel;
			return fled_dev->hal->
				rt_hal_fled_set_lv_protection_sel(fled_dev, sel);
		}
	}
	return -EINVAL;
}
		...
		int min_ms, int max_ms, int *selector)
{
	...
	for (sel = 0; ; sel++) {
		rc = fled_dev->hal->rt_hal_fled_strobe_timeout_list(fled_dev, sel);
		if (rc < 0)
			return rc;
		if (rc >= min_ms && rc <= max_ms) {
			*selector = sel;
			return fled_dev->hal->
				rt_hal_fled_set_strobe_timeout_sel(fled_dev, sel);
		}
	}
	return -EINVAL;
}
...
static int rtfled_check_hal_implement(struct rt_fled_hal *hal)
{
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_set_torch_current))
		hal->rt_hal_fled_set_torch_current = rtled_impl_set_torch_current;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_set_strobe_current))
		hal->rt_hal_fled_set_strobe_current = rtled_impl_set_strobe_current;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_set_timeout_level))
		hal->rt_hal_fled_set_timeout_level = rtled_impl_set_timeout_level;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_set_lv_protection))
		hal->rt_hal_fled_set_lv_protection = rtled_impl_set_lv_protection;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_set_strobe_timeout))
		hal->rt_hal_fled_set_strobe_timeout = rtled_impl_set_strobe_timeout;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_get_torch_current))
		hal->rt_hal_fled_get_torch_current = rtled_impl_get_torch_current;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_get_strobe_current))
		hal->rt_hal_fled_get_strobe_current = rtled_impl_get_strobe_current;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_get_timeout_level))
		hal->rt_hal_fled_get_timeout_level = rtled_impl_get_timeout_level;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_get_lv_protection))
		hal->rt_hal_fled_get_lv_protection = rtled_impl_get_lv_protection;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_get_strobe_timeout))
		hal->rt_hal_fled_get_strobe_timeout = rtled_impl_get_strobe_timeout;
	if (HAL_NOT_IMPLEMENTED(rt_hal_fled_get_is_ready))
		hal->rt_hal_fled_get_is_ready = rtled_impl_get_is_ready;
	check_hal_implemented(hal->rt_hal_fled_set_mode);
	check_hal_implemented(hal->rt_hal_fled_get_mode);
	check_hal_implemented(hal->rt_hal_fled_strobe);
	check_hal_implemented(hal->rt_hal_fled_torch_current_list);
	check_hal_implemented(hal->rt_hal_fled_strobe_current_list);
	check_hal_implemented(hal->rt_hal_fled_timeout_level_list);
	check_hal_implemented(hal->rt_hal_fled_lv_protection_list);
	check_hal_implemented(hal->rt_hal_fled_strobe_timeout_list);
	check_hal_implemented(hal->rt_hal_fled_set_torch_current_sel);
	check_hal_implemented(hal->rt_hal_fled_set_strobe_current_sel);
	check_hal_implemented(hal->rt_hal_fled_set_timeout_level_sel);
	check_hal_implemented(hal->rt_hal_fled_set_lv_protection_sel);
	check_hal_implemented(hal->rt_hal_fled_set_strobe_timeout_sel);
	check_hal_implemented(hal->rt_hal_fled_get_torch_current_sel);
	check_hal_implemented(hal->rt_hal_fled_get_strobe_current_sel);
	check_hal_implemented(hal->rt_hal_fled_get_timeout_level_sel);
	check_hal_implemented(hal->rt_hal_fled_get_lv_protection_sel);
	check_hal_implemented(hal->rt_hal_fled_get_strobe_timeout_sel);
	return 0;
}
