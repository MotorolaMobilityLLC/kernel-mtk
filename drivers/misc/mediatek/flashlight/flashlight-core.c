...
static int is_torch =0;//add by hq for lowpower torch
...
static int fl_set_level(struct flashlight_dev *fdev, int level)
{
	...
	/* if pt is low */
#ifdef CONFIG_MTK_FLASHLIGHT_PT
	if (pt_is_low(pt_low_vol, pt_low_bat, pt_over_cur) && (is_torch == 0))//add by hq for lowpower torch
...
#endif
#ifdef CONFIG_MTK_FLASHLIGHT_PT
	if (pt_is_low(pt_low_vol, pt_low_bat, pt_over_cur) == 2 && is_torch == 0)//add by hq for lowpower torch
		if (enable) {
			enable = 0;
...
static int pt_trigger(void)
{
	...
	list_for_each_entry(fdev, &flashlight_list, node) {
		...
		if (pt_strict && is_torch == 0) {//add by hq for lowpower torch
...
//torch sysfs format:AB A:torch_flag 1:on 0:off  B:torch_duty
static ssize_t flashlight_torch_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	pr_debug("flashlight_torch_show\n");
		...
		struct device_attribute *attr, const char *buf,size_t size)
{
	...
	flashlight_state = cmd;
	if(ret <= 0){
		pr_debug("wrong cmd,ret:%d\n",ret);
		return size;
	}

	if(cmd > 4032){
		pr_debug("wrong cmd,cmd:%d\n",cmd);
		return size;
	}
	if(cmd > 4000){
		torch_duty = cmd - 4000;
	...
	pr_debug("torch_duty:d%,torch_flag:%d\n",torch_duty,torch_flag);

	if(cmd > 4015){
		if(torch_duty > 31){
		torch_duty = 31;
		}
	sprintf(cmd_buf,"0 0 0 %d 800",torch_duty);
	pr_debug("cmd_buf:%s\n",cmd_buf);
	flashlight_strobe_store(dev,attr,cmd_buf,strlen(cmd_buf));
	return size;
	}

	//find flashlight device led1
	mutex_lock(&fl_mutex);
	...
	pr_debug("flashlight_ioctl fl_dev_arg.arg=%d,fl_dev_arg.channel=%d,type=%d,ct=%d,part=%d\n",
	...
	pr_debug("flashlight_get_part_id:%d\n",part_id);
	if(fdev->ops){
		mutex_lock(&fl_mutex);
		...
		fl_dev_arg.arg = 0;
		if(fdev->ops->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS,(unsigned long)&fl_dev_arg)){
...
static DEVICE_ATTR(torch1,0644,flashlight_torch_show,flashlight_torch_store);
//
static int flashlight_probe(struct platform_device *dev)
{
	...
	if (device_create_file(flashlight_device, &dev_attr_torch1)) {
...
err_create_torch_device_file:
	device_remove_file(flashlight_device, &dev_attr_torch1);
