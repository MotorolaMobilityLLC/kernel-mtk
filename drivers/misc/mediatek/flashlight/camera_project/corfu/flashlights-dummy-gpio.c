...
/* define pinctrl */
/* TODO: define pinctrl */
#define DUMMY_PINCTRL_PIN_EN 1
#define DUMMY_PINCTRL_PIN_CE 0
#define DUMMY_PINCTRL_PINSTATE_LOW 0
#define DUMMY_PINCTRL_PINSTATE_HIGH 1
#define DUMMY_PINCTRL_STATE_EN_HIGH "flashlight_en_high"
#define DUMMY_PINCTRL_STATE_EN_LOW  "flashlight_en_low"
#define DUMMY_PINCTRL_STATE_CE_HIGH "flashlight_ce_high"
#define DUMMY_PINCTRL_STATE_CE_LOW  "flashlight_ce_low"
static struct pinctrl *dummy_pinctrl;
static struct pinctrl_state *dummy_en_high;
static struct pinctrl_state *dummy_en_low;
static struct pinctrl_state *dummy_ce_high;
static struct pinctrl_state *dummy_ce_low;
...
static int dummy_pinctrl_init(struct platform_device *pdev)
{
	...
	dummy_ce_high = pinctrl_lookup_state(
			dummy_pinctrl, DUMMY_PINCTRL_STATE_CE_HIGH);
	if (IS_ERR(dummy_ce_high)) {
		pr_err("Failed to init (%s)\n", DUMMY_PINCTRL_STATE_CE_HIGH);
		ret = PTR_ERR(dummy_ce_high);
	}
	dummy_ce_low = pinctrl_lookup_state(
			dummy_pinctrl, DUMMY_PINCTRL_STATE_CE_LOW);
	if (IS_ERR(dummy_ce_low)) {
		pr_err("Failed to init (%s)\n", DUMMY_PINCTRL_STATE_CE_LOW);
		ret = PTR_ERR(dummy_ce_low);
	}
...
static int dummy_pinctrl_set(int pin, int state)
{
	...
	case DUMMY_PINCTRL_PIN_EN:
		...
		break;
	case DUMMY_PINCTRL_PIN_CE:
		if (state == DUMMY_PINCTRL_PINSTATE_LOW &&
				!IS_ERR(dummy_ce_low))
			pinctrl_select_state(dummy_pinctrl, dummy_ce_low);
		else if (state == DUMMY_PINCTRL_PINSTATE_HIGH &&
				!IS_ERR(dummy_ce_high))
			pinctrl_select_state(dummy_pinctrl, dummy_ce_high);
		else
			pr_err("set err, pin(%d) state(%d)\n", pin, state);
		break;
	default:
...
static int dummy_enable(void)
{
	...
	if(flash_mode)
		return dummy_pinctrl_set(DUMMY_PINCTRL_PIN_EN, DUMMY_PINCTRL_PINSTATE_HIGH);
	else
		return dummy_pinctrl_set(DUMMY_PINCTRL_PIN_CE, DUMMY_PINCTRL_PINSTATE_HIGH);
}
...
static int dummy_disable(void)
{
	...
	return dummy_pinctrl_set(DUMMY_PINCTRL_PIN_CE, DUMMY_PINCTRL_PINSTATE_LOW);
}
...
static int dummy_init(void)
{
	...
	dummy_pinctrl_set(DUMMY_PINCTRL_PIN_CE,0);
	return dummy_pinctrl_set(DUMMY_PINCTRL_PIN_EN, 0);
}
