...
/* ======================================================================== */
/* TEST */
static ssize_t show_flashlight_dvt(struct device *dev, struct device_attribute *attr, char *buf)
{
	pr_err("show");
	return sprintf(buf, "%u\n", 1);
}

static ssize_t store_flashlight_dvt(struct device *dev, struct device_attribute *attr, const char *buf,
				size_t size)
{
	int ret = 0;
	char *pvalue = NULL;
	...
	if (buf != NULL && size != 0) {
		...
		/*flashlight_dvt_entry(test_item);*/
		flashlight_dvt_test(test_item, test_item);
	}
	return size;
}
static DEVICE_ATTR(flashlight_dvt, 0770, show_flashlight_dvt, store_flashlight_dvt);
...
#define FLASHLIGHT_MAJOR 242
	...
	if (flashlight_device == NULL) {
		logI("[flashlight_probe] device_create fail ~");
		goto flashlight_probe_error;
	}

	/* create file for flashlight_dvt;*/
	device_create_file(flashlight_device, &dev_attr_flashlight_dvt);
	/* initialize members */
...
static const struct of_device_id FLASHLIGHT_of_match[] = {
	{.compatible = "mediatek,mt6799-flashlight"},
	{},
};
#endif
