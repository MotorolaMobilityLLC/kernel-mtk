 ...
 */
 //lenovo.sw START huangsh4 20170225 bringup INDRIYAPRO camera Flash
...
#include "../imgsensor/src/mt6797/camera_hw/kd_camera_hw.h"
#include <asm/io.h>
...
#define aw3644_NAME "leds-aw3644-1"
...
#define FLASH_TORCH_ENABLE  (GPIO255| 0x80000000)
#define FLASH_STROBE_ENABLE  (GPIO91 | 0x80000000)
...
static struct i2c_client *aw3644_i2c_client = NULL;
...
static int aw3644_write_reg(struct i2c_client *client, u8 reg, u8 val)
{
	int ret=0;
...
static int aw3644_read_reg(struct i2c_client *client, u8 reg)
{
	int val=0;
	struct aw3644_chip_data *chip = i2c_get_clientdata(client);
	mutex_lock(&chip->lock);
...
int readReg(int reg)
{
    ...
    int val=0;
    buf[0]=reg;
    val=aw3644_read_reg(aw3644_i2c_client,buf[0]);
...
int writeReg(int reg, int data)
{
    ...
    buf[0]=reg;
    buf[1]=data;
    aw3644_write_reg(aw3644_i2c_client,buf[0],buf[1]);
...
int setDuty_aw3644_1(int duty)
{
	if(duty<0)
		...
		duty=e_DutyNum-1;
	m_duty1=duty;
	PK_DBG(" setDuty_aw3644_1 line=%d, m_duty1=%d\n",__LINE__, m_duty1);
	return 0;
}
...
//	mt_set_gpio_out(FLASH_TX, 1);
	...
	else
	{
		if((isMovieMode[m_duty1] == 1) & (isMovieMode[m_duty2] == 1))
			...
			PK_DBG("FLASH mode\n");
			//mt_set_gpio_out(FLASH_STROBE_ENABLE, GPIO_OUT_ONE);
			//writeReg(aw3644_REG_ENABLE, (temp&0xF0) | 0xFF);//flash mode
			//writeReg(aw3644_REG_LED1_FLASH, 33);//duty5 400mA
			//writeReg(aw3644_REG_LED2_FLASH, 33);//duty5 400mA
			
			writeReg(aw3644_REG_ENABLE, 0x0F);
			}
	}
	return 0;
}
...
int flashDisable_aw3644_2(void)
{
	...
	flashEnable_aw3644_2();
	PK_DBG("flashDisable_aw3644_2 Exit!");
	return 0;
}
...
int setDuty_aw3644_2(int duty)
{
	...
	PK_DBG("setDuty_aw3644_2:m_duty = %d, m_duty2 = %d \n", m_duty1, m_duty2);
...
#if 0
static int aw3644_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strcpy(info->type, aw3644_NAME);
	return 0;
}
#endif
			...
			const struct i2c_device_id *id)
{
	...
	PK_DBG("aw3644_probe start--->.\n");

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		err = -ENODEV;
		PK_DBG("aw3644 i2c functionality check fail.\n");
		return err;
	}
	...
	if (pdata == NULL) {	/* values are set to Zero. */
		PK_DBG("aw3644 Platform data does not exist\n");
	...
	chip->pdata  = pdata;
	if(aw3644_chip_init(chip)<0)
		goto err_chip_init;

	aw3644_i2c_client = client;
	if(aw3644_i2c_client != NULL)
		aw3644_i2c_client->addr=aw3644_i2c_client->addr-0x0c;
	PK_DBG("aw3644 Initializing is done \n");
	return 0;

err_chip_init:
	i2c_set_clientdata(client, NULL);
	kfree(chip);
	PK_DBG("aw3644 probe is failed\n");
	return -ENODEV;
}
...
static const struct i2c_device_id aw3644_id[] = {
	{aw3644_NAME, 0},
	{}
};
...
static struct i2c_driver aw3644_i2c_driver = {
	...
	.probe	= aw3644_probe,
	//.detect = aw3644_detect,
	.remove   = aw3644_remove,
	.id_table = aw3644_id,
};

#if 0
struct aw3644_platform_data aw3644_pdata = {0, 0, 0, 0, 0};
static struct i2c_board_info __initdata i2c_aw3644={ I2C_BOARD_INFO(aw3644_NAME, 0x6f), \
    .platform_data = &aw3644_pdata,};
#endif
static int __init aw3644_init(void)
{
	int ret;
	printk("aw3644_init\n");
	ret=i2c_add_driver(&aw3644_i2c_driver);
	printk("aw3644_init ret=%d\n",ret);
	return ret;
}

static void __exit aw3644_exit(void)
{
	i2c_del_driver(&aw3644_i2c_driver);
}
module_init(aw3644_init);
module_exit(aw3644_exit);
#define GPIO_I2C0_SDA_PIN_FLASH	                   (GPIO38 | 0x80000000)
#define GPIO_I2C0_SCL_PIN_FLASH	                   (GPIO37 | 0x80000000)
#define GPIO_I2C2_SDA_PIN_FLASH	                   (GPIO113 | 0x80000000)
#define GPIO_I2C2_SCL_PIN_FLASH	                   (GPIO112 | 0x80000000)
void flash_set_i2c_gpio(void)
  {
    //set i2c gpio mode
   mt_set_gpio_mode(GPIO_I2C0_SDA_PIN_FLASH, 1);
   mt_set_gpio_mode(GPIO_I2C0_SCL_PIN_FLASH, 1);
...
int init_aw3644(void)
{
	...
	err =  writeReg(aw3644_REG_ENABLE,0x00);
	err =  writeReg(aw3644_REG_TIMING, 0x1F);
	return err;
}
//digital io
...
#endif
#endif
//lenovo.sw END huangsh4 20170225 bringup INDRIYAPRO camera Flash
