...
static int rt5081_enable(void)
{
	...
	if ((rt5081_en_ht == RT5081_ENABLE_FLASH)
		...
		ret = flashlight_set_mode(
				flashlight_dev, FLASHLIGHT_MODE_FLASH);
	else
		ret = flashlight_set_mode(
				flashlight_dev, FLASHLIGHT_MODE_TORCH);
...
static int rt5081_disable(void)
{
	...
	return flashlight_set_mode(flashlight_dev, FLASHLIGHT_MODE_OFF);
}
...
static int rt5081_set_level_ht(int level)
{
	...
	/* TODO: need separate channel */
	if (!rt5081_is_torch(level))
		flashlight_set_torch_brightness(
				flashlight_dev, rt5081_torch_level[level]);
	flashlight_set_strobe_brightness(
			flashlight_dev, rt5081_strobe_level[level]);
...
static int rt5081_init(void)
{
	...
	flashlight_dev = find_flashlight_by_name(RT_FLED_DEVICE);
	if (!flashlight_dev) {
		fl_err("Failed to get flashlight device.\n");
		return -1;
	}

	/* setup strobe mode timeout */
	if (flashlight_set_strobe_timeout(flashlight_dev, 400, 600)) {
