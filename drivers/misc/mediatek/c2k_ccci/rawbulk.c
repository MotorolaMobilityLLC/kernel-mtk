...
static ssize_t rawbulk_attr_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	switch (idx) {
	case ATTR_ENABLE:
		count = sprintf(buf, "%d", enab);
		break;
#ifdef SUPPORT_LEGACY_CONTROL
	case ATTR_ENABLE_C:
		count = sprintf(buf, "%s", enab ? "gadget" : "tty");
		break;
#endif
	case ATTR_AUTORECONN:
		count = sprintf(buf, "%d", fn->autoreconn);
		break;
	...
	case ATTR_NUPS:
		...
		break;
	case ATTR_NDOWNS:
		count = sprintf(buf, "%d", enab ? fn->ndowns : -1);
		break;
	case ATTR_UPSIZE:
		count = sprintf(buf, "%d", enab ? fn->upsz : -1);
		break;
	case ATTR_DOWNSIZE:
		count = sprintf(buf, "%d", enab ? fn->downsz : -1);
		break;
	default:
		break;
	}
