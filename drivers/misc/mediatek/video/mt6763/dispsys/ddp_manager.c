...
static int module_power_on(enum DISP_MODULE_ENUM module)
{
	...
	if (ddp_get_module_driver(module) != 0) {
		if (ddp_get_module_driver(module)->power_on != 0)
			ddp_get_module_driver(module)->power_on(
			    module, NULL); /* now just 0; */
	}
	return 0;
}
	...
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->init != 0)
				ddp_get_module_driver(module_name)
				    ->init(module_name, cmdq_handle);

			if (ddp_get_module_driver(module_name)->set_listener !=
			    0)
				ddp_get_module_driver(module_name)
				    ->set_listener(module_name,
						   dpmgr_module_notify);
		}
	}
	...
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->deinit != 0)
				ddp_get_module_driver(module_name)
				    ->deinit(module_name, cmdqHandle);

			if (ddp_get_module_driver(module_name)->set_listener !=
			    0)
				ddp_get_module_driver(module_name)
				    ->set_listener(module_name, NULL);
		}
	}
	...
	for (i = module_num - 1; i >= 0; i--) {
		...
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->ioctl != 0) {
				ret += ddp_get_module_driver(module_name)
					   ->ioctl(module_name, cmdq_handle,
						   ioctl_cmd, params);
			}
		}
	}
	...
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->config != 0) {
				ddp_get_module_driver(module_name)
				    ->config(module_name, config, cmdq_handle);
			}
...
int dpmgr_path_power_off(disp_path_handle dp_handle, enum CMDQ_SWITCH encmdq)
{
	...
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) &&
		    ddp_get_module_driver(module_name)->power_off) {
			ddp_get_module_driver(module_name)
...
int dpmgr_path_power_on(disp_path_handle dp_handle, enum CMDQ_SWITCH encmdq)
{
	...
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) &&
		    ddp_get_module_driver(module_name)->power_on) {
			ddp_get_module_driver(module_name)
...
/* TODO: just for verify rootcause, will be removed soon */
		...
		if (is_module_in_path(DISP_MODULE_AAL0, handle)) {
			if (ddp_get_module_driver(DISP_MODULE_AAL0)->cmd !=
			    NULL)
				ret = ddp_get_module_driver(DISP_MODULE_AAL0)
					  ->cmd(DISP_MODULE_AAL0, msg, arg,
						cmdqhandle);
		}
#endif
		break;
	case DISP_IOCTL_SET_GAMMALUT:
		if (ddp_get_module_driver(DISP_MODULE_GAMMA0)->cmd != NULL)
			ret = ddp_get_module_driver(DISP_MODULE_GAMMA0)
				  ->cmd(DISP_MODULE_GAMMA0, msg, arg,
					cmdqhandle);
		break;
	...
	case DISP_IOCTL_READ_SW_REG:
		...
		ret =
		    disp_color_ioctl(DISP_MODULE_COLOR0, msg, arg, cmdqhandle);
		break;
	default:
		...
		if (ddp_get_module_driver(module_name) != 0) {
			if ((ddp_get_module_driver(module_name)
				...
				DDPDBG("%s set lcm utils\n",
				       ddp_get_module_name(module_name));
				ddp_get_module_driver(module_name)
				    ->set_lcm_utils(module_name, lcm_drv);
			}
		}
	}
	return 0;
...
int dpmgr_signal_event(disp_path_handle dp_handle, enum DISP_PATH_EVENT event)
{
	...
	if (handle->wq_list[event].init) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->event_signal,
				 MMPROFILE_FLAG_PULSE, event, wq_handle->data);
	}
	return 0;
}
...
int dpmgr_factory_mode_test(int module_name, void *cmdqhandle, void *config)
{
	if (ddp_get_module_driver(module_name) != 0) {
		if (ddp_get_module_driver(module_name)->ioctl != 0) {
			...
			ddp_get_module_driver(DISP_MODULE_DPI)
			    ->ioctl(module_name, cmdqhandle,
			    ...
			    int module_name, const char *caller)
{
	...
	if (module_name == DISP_MODULE_NUM) {
		...
		if (ddp_get_module_driver(module_name)->switch_to_nonsec != 0)
			ddp_get_module_driver(module_name)
			    ->switch_to_nonsec(module_name, cmdqhandle);
	}
