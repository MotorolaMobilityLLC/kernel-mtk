...
/**
 * create disp path handle , it will assign mutex to this handle,
...
dpmgr_path_get_dst_module_type(disp_path_handle dp_handle);
...
/* init path , it will set mutex according to modules on this path
...
/* connect path , it will set mutex according to modules on this path
 * and sof sorce.
...
int dpmgr_path_build_cmdq(disp_path_handle dp_handle,
			  void *trigger_loop_handle, enum CMDQ_STATE state,
			  int reverse);
 ...
 * event: path event.
 */
int dpmgr_disable_event(disp_path_handle dp_handle,
...
/* dpmgr_ioctl , it will call ioctl of ddp modules ioctl() to do some
