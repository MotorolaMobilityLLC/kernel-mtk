		      ...
		      struct golden_setting_context *p_golden_setting)
{
	...
	consume_rate_div =
	    DIV_ROUND_UP((unsigned int)consume_rate_div_tmp, 10);
	...
	output_valid_fifo_threshold =
	    preultra_low < temp ? preultra_low : temp;
	...
	if (primary_display_is_video_mode()) {
		/* video mode*/
		DISP_REG_SET(
		    handle, idx * DISP_RDMA_INDEX_OFFSET +
			...
			MEM_GMC_SETTING_0_FLD_RG_VALID_THRESHOLD_FORCE_PREULTRA,
			    0) |
			REG_FLD_VAL(
			    MEM_GMC_SETTING_0_FLD_RG_VDE_FORCE_PREULTRA, 1));
		...
		DISP_REG_SET(
			...
			MEM_GMC_SETTING_0_FLD_RG_VALID_THRESHOLD_FORCE_PREULTRA,
			    1) |
			REG_FLD_VAL(
			    MEM_GMC_SETTING_0_FLD_RG_VDE_FORCE_PREULTRA, 0));
	...
	DISP_REG_SET(
	    handle, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_FIFO_CON,
	...
	dvfs_preultra_low =
	    (preultra_low_us + fifo_off_dvfs) * consume_rate_div;

	dvfs_preultra_high =
	    (preultra_high_us + fifo_off_dvfs) * consume_rate_div;
		      ...
		      golden_setting_context *p_golden_setting)
{
	...
	if (is_large_resolution)
		regval |=
		    REG_FLD_VAL(FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, 312);
	else
		regval |=
		    REG_FLD_VAL(FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, 175);
	...
	if (is_large_resolution) {
		...
		if (is_wrot_sram)
			...
			regval |=
			    REG_FLD_VAL(RDMA_THRESHOLD_FOR_SODI_FLD_HIGH, 363);
		else
			regval |=
			    REG_FLD_VAL(RDMA_THRESHOLD_FOR_SODI_FLD_HIGH, 606);
	} else {
		...
		if (is_wrot_sram)
			...
			regval |=
			    REG_FLD_VAL(RDMA_THRESHOLD_FOR_SODI_FLD_HIGH, 205);
		else
			regval |=
			    REG_FLD_VAL(RDMA_THRESHOLD_FOR_SODI_FLD_HIGH, 578);
	}
	...
	unsigned int bpp)
{
	...
	DISP_REG_SET_FIELD(
	    handle, SIZE_CON_1_FLD_OUTPUT_FRAME_HEIGHT,
...
void rdma_dump_reg(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("(0x03c)R_M_GMC_SET2=0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_MEM_GMC_SETTING_2 +
			     DISP_RDMA_INDEX_OFFSET * idx));
...
void rdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP(
	    "rdma%d: en=%d,memory_mode=%d,smi_busy=%d,w=%d,h=%d,pitch=%d,addr=0x%x,fmt=%s,fifo_min=%d,\n",
	    idx, REG_FLD_VAL_GET(GLOBAL_CON_FLD_ENGINE_EN +
				     DISP_RDMA_INDEX_OFFSET * idx,
				 global_ctrl),
	    REG_FLD_VAL_GET(GLOBAL_CON_FLD_MODE_SEL +
				DISP_RDMA_INDEX_OFFSET * idx,
			    global_ctrl),
	    REG_FLD_VAL_GET(GLOBAL_CON_FLD_SMI_BUSY +
				DISP_RDMA_INDEX_OFFSET * idx,
			    global_ctrl),
	    DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 +
			 DISP_RDMA_INDEX_OFFSET * idx) &
	...
	DDPDUMP(
	    ...
	    DISP_REG_GET(DISP_REG_RDMA_OUT_LINE_CNT +
			 DISP_RDMA_INDEX_OFFSET * idx),
	    REG_FLD_VAL_GET(RDMA_BG_CON_1_TOP + DISP_RDMA_INDEX_OFFSET * idx,
			    bg1),
	    REG_FLD_VAL_GET(
		RDMA_BG_CON_1_BOTTOM + DISP_RDMA_INDEX_OFFSET * idx, bg1),
	    REG_FLD_VAL_GET(RDMA_BG_CON_0_LEFT + DISP_RDMA_INDEX_OFFSET * idx,
			    bg0),
	    REG_FLD_VAL_GET(RDMA_BG_CON_0_RIGHT + DISP_RDMA_INDEX_OFFSET * idx,
			    bg0),
			    ...
			    struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	rdma_config(module, mode,
		    ...
		    &(r_config->bg_ctrl), handle, p_golden_setting,
		    pConfig->lcm_bpp);
		       ...
		       struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	cmdq_engine = rdma_to_cmdq_engine(module);
	/* rdma is in sec stat, we need to switch it to nonsec */
	ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &(wait_handle));
	if (ret)
		DDPAEE("[SVP]fail to create disable handle %s ret=%d\n",
		       __func__, ret);
	cmdqRecReset(wait_handle);
	cmdqRecSetSecure(wait_handle, 1);
	cmdqRecSecureEnablePortSecurity(wait_handle, (1LL << cmdq_engine));
	if (handle != NULL) {
		/*Async Flush method*/
		enum CMDQ_EVENT_ENUM cmdq_event_nonsec_end;
		/*cmdq_event_nonsec_end =
...
{
	unsigned int idx = rdma_index(module);
	UINT32 value = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 +
				    DISP_RDMA_INDEX_OFFSET * idx);
			   ...
			   struct rdma_color_post *post)
{
	...
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C00,
		     matrix->C00);
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C01,
		     matrix->C01);
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C02,
		     matrix->C02);
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C10,
		     matrix->C10);
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C11,
		     matrix->C11);
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C12,
		     matrix->C12);
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C20,
		     matrix->C20);
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C21,
		     matrix->C21);
	DISP_REG_SET(NULL, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_C22,
		     matrix->C22);

	DISP_REG_SET(NULL,
		     idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_PRE_ADD_0,
		     pre->ADD0);
	DISP_REG_SET(NULL,
		     idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_PRE_ADD_1,
		     pre->ADD1);
	DISP_REG_SET(NULL,
		     idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_PRE_ADD_2,
		     pre->ADD2);

	DISP_REG_SET(NULL,
		     idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_POST_ADD_0,
		     post->ADD0);
	DISP_REG_SET(NULL,
		     idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_POST_ADD_1,
		     post->ADD1);
	DISP_REG_SET(NULL,
		     idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_POST_ADD_2,
		     post->ADD2);
}
