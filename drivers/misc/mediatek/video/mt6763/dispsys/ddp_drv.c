...
static enum mc_result late_open_mobicore_device(void)
{
	...
	if (opened_device == 0) {
		...
		if (mcRet == MC_DRV_ERR_INVALID_OPERATION) {
			/* skip false alarm when the
			 * mc_open_device(mc_deviceId)
...
static int open_tplay_driver_connection(void)
{
	...
	do {
		...
		mcRet = mc_malloc_wsm(mc_deviceId, 0,
				      sizeof(struct tplay_tciMessage_t),
				      (uint8_t **)&pTplayTci, 0);
		if (mcRet != MC_DRV_OK) {
		...
		mcRet = mc_open_session(
		    &tplaySessionHandle, &MC_UUID_TPLAY, (uint8_t *)pTplayTci,
...
/* disp_node_struct *pNode = (disp_node_struct *) file->private_data; */
	...
	if (cmd == DISP_IOCTL_SET_TPLAY_HANDLE) {
		...
		if (copy_from_user(&value, (void *)arg,
				   sizeof(unsigned int))) {
			DDPERR(
			"DISP_IOCTL_SET_TPLAY_HANDLE,copy_from_user failed\n");
			return -EFAULT;
		}
...
struct device *disp_get_device(void) { return &(mydev.dev); }
...
static int __init disp_probe_1(void)
{
	...
	"[ERR]DT, i=%d, module=%s, map_irq=%d,virtohw_irq=%d, check_irq=%d\n",
			...
			if (ret) {
				DDPERR(
			"[ERR]DT, i=%d, module=%s,request_irq(%d) fail\n",
				       i, ddp_get_module_name(i),
				       ddp_get_module_irq(i));
				continue;
			}
