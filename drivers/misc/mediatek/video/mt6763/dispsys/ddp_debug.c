...
int get_lp_cust_mode(void) { return low_power_cust_mode; }

void backup_vfp_for_lp_cust(unsigned int vfp) { vfp_backup = vfp; }

unsigned int get_backup_vfp(void) { return vfp_backup; }
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "regr:", 5) == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(para); i++) {
			/* make fmt like:
			 * "set_dsi_cmd:0x%x,0x%hhx,0x%hhx,0x%hhx,0x%hhx\n"
			 */
		...
		DSI_set_cmdq_V2(DISP_MODULE_DSI0, NULL, cmd, para_cnt, para,
				1);
		...
		DSI_dcs_read_lcm_reg_v2(DISP_MODULE_DSI0, NULL, cmd, para,
					size);
...
{
	char *str =
	    "idlemgr disable mtcmos now, all the regs may 0x00000000\n";
...
void ddp_debug_init(void)
{
	...
	if (!debug_init) {
		debug_init = 1;
		debugfs = debugfs_create_file("dispsys", S_IFREG | 0444,
		...
		if (debugDir) {
			...
			debugfs_dump = debugfs_create_file(
			    ...
			    &debug_fops_dump);
			debug_cmd_help = debugfs_create_file(
			    "cmd_help", S_IFREG | 0444, debugDir, NULL,
			    &cmd_help_fops);
			d = debugfs_create_file("lowpowermode",
						S_IFREG | 0444, debugDir,
...
unsigned int ddp_debug_analysis_to_buffer(void) { return dump_to_buffer; }

unsigned int ddp_debug_dbg_log_level(void) { return dbg_log_level; }

unsigned int ddp_debug_irq_log_level(void) { return irq_log_level; }

int ddp_debug_force_roi(void) { return dbg_force_roi; }

int ddp_debug_partial_statistic(void) { return dbg_partial_statis; }

int ddp_debug_force_roi_x(void) { return dbg_partial_x; }

int ddp_debug_force_roi_y(void) { return dbg_partial_y; }

int ddp_debug_force_roi_w(void) { return dbg_partial_w; }

int ddp_debug_force_roi_h(void) { return dbg_partial_h; }
...
int ddp_mem_test(void) { return -1; }

int ddp_lcd_test(void) { return -1; }
