			    ...
			    void *handle)
{
	...
	if (!is_engine_sec) {
		...
		if (cfg->security != DISP_SECURE_BUFFER) {
			...
			/* we need to pass this handle and offset to cmdq
			 * driver
			 */
...
int _ovl_get_info(enum DISP_MODULE_ENUM module, void *data, int *layer_idx)
{
	...
	unsigned int src_on = DISP_REG_GET(DISP_REG_OVL_SRC_CON + ovl_base);
	struct OVL_BASIC_STRUCT *p = NULL;
	unsigned int src_ext_on =
	    DISP_REG_GET(DISP_REG_OVL_DATAPATH_EXT_CON + ovl_base);
	int ext_en[3] = {0};
	int ext_sel[3] = {-1, -1, -1};
		...
		if (p->layer_en) {
			...
			for (j = 0; j < 3; j++) {
				if ((i == ext_sel[j]) && ext_en[j]) {
					...
					layer_num++;
					/* get ext layer data */
					__ovl_get_info(module, p, ext_en[j], j,
						       0);
		    ...
		    int ext_layer, int layer)
{
	...
	unsigned long ovl_base = ovl_base_addr(module);
	unsigned long ext_layer_off_con =
	    ext_en ? (DISP_REG_OVL_EL0_CON - DISP_REG_OVL_L0_CON) : 0;
	unsigned long ext_layer_off_size =
	    ext_en ? (DISP_REG_OVL_EL0_SRC_SIZE - DISP_REG_OVL_L0_SRC_SIZE)
		   : 0;
	unsigned long ext_layer_off_pitch =
	    ext_en ? (DISP_REG_OVL_EL0_PITCH - DISP_REG_OVL_L0_PITCH) : 0;
	unsigned long ext_layer_off_addr =
	    ext_en ? (DISP_REG_OVL_EL0_ADDR - DISP_REG_OVL_L0_ADDR) : 0;
	unsigned long ext_layer_off_path =
	    ext_en
		? (DISP_REG_OVL_DATAPATH_EXT_CON - DISP_REG_OVL_DATAPATH_CON)
		: 0;

	unsigned long layer_off_con =
	    ovl_base + ext_layer_off_con +
	    (ext_en ? ext_layer : layer) * OVL_LAYER_OFFSET;
	unsigned long layer_off_size =
	    ovl_base + ext_layer_off_size +
	    (ext_en ? ext_layer : layer) * OVL_LAYER_OFFSET;
	unsigned long layer_off_pitch =
	    ovl_base + ext_layer_off_pitch +
	    (ext_en ? ext_layer : layer) * OVL_LAYER_OFFSET;
	unsigned long layer_off_addr =
	    ovl_base + ext_layer_off_addr +
	    (ext_en ? ext_layer * 4 : layer * OVL_LAYER_OFFSET);
	unsigned long layer_off_path = ovl_base + ext_layer_off_path;

	data->fmt = display_fmt_reg_to_unified_fmt(
	    DISP_REG_GET_FIELD(L_CON_FLD_CFMT,
	...
	data->src_h =
	    (DISP_REG_GET(layer_off_size + DISP_REG_OVL_L0_SRC_SIZE) >> 16) &
	    0xfff;
	data->src_pitch =
	    DISP_REG_GET(layer_off_pitch + DISP_REG_OVL_L0_PITCH) & 0xffff;
	data->bpp = UFMT_GET_bpp(data->fmt) / 8;

	if (ext_en) {
		data->alpha =
		    (DISP_REG_GET(layer_off_con + DISP_REG_OVL_L0_CON) &
		     (0x1 << 8))
			? 1
			: 0;
		data->gpu_mode =
		    (DISP_REG_GET(layer_off_path + DISP_REG_OVL_DATAPATH_CON) &
		     (0x1 << (8 + ext_layer)))
			? 1
			: 0;
		data->adobe_mode = 0;
					     ...
					     struct disp_rect *roi)
{
	...
	DDPDBG(
		"sur_aen=%d,sur_alpha=0x%x,yuv_range=%d,sec=%d,const_bld=%d\n",
		ovl_cfg->sur_aen,
...
int ovl_switch_to_nonsec(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	if (ovl_is_sec[ovl_idx] == 1) {
		...
		/* we should disable ovl before new (nonsec) setting take
		 * effect
			    ...
			    struct disp_ddp_path_config *pConfig)
{
	...
	for (local_layer = 0; global_layer < total_layer_num_ovl;
			...
			struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	DDPDBG(
		"ovl%d enabled_layers=0x%01x, enabled_ext_layers=0x%01x, ext_sel_layers=0x%04x\n",
...
void ovl_dump_reg(enum DISP_MODULE_ENUM module)
{
	if (disp_helper_get_option(DISP_OPT_REG_PARSER_RAW_DUMP)) {
		...
		if (src_on & 0x1) {
			DDPDUMP(
			    "0x030: 0x%08x 0x%08x 0x%08x 0x%08x\n",
		...
		if (src_on & 0x2) {
			DDPDUMP(
			    "0x050: 0x%08x 0x%08x 0x%08x 0x%08x\n",
		...
		if (src_on & 0x4) {
			DDPDUMP(
			    "0x070: 0x%08x 0x%08x 0x%08x 0x%08x\n",
		...
		if (src_on & 0x8) {
			DDPDUMP(
			    "0x090: 0x%08x 0x%08x 0x%08x 0x%08x\n",
			...
			DDPDUMP(
			    "0xfa0=0x%08x,0x0a4=0x%08x,0x120=0x%08x,0x128=0x%08x,0x130=0x%08x\n",
...
static void ovl_dump_ext_layer_info(int layer, unsigned long layer_offset)
{
	...
	layer_addr_offset =
	    layer_offset - layer * OVL_LAYER_OFFSET + layer * 4;
	fmt = display_fmt_reg_to_unified_fmt(
	    DISP_REG_GET_FIELD(L_CON_FLD_CFMT,
			      ...
			      enum dst_module_type dst_mod_type, void *cmdq)
{
	...
	if (dst_mod_type == DST_MOD_REAL_TIME) {
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES, 127);
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES_URG, 63);
	} else {
		/* decouple */
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES, 63);
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES_URG, 63);
	}
	...
	regval =
	    REG_FLD_VAL(FLD_OVL_RDMA_GREQ_LAYER0_GREQ_NUM, layer_greq_num);
	if (layer_num > 1)
