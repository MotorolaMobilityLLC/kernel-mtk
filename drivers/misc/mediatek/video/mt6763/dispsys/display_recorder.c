	...
	while (1) {
		...
		if (dbg_disp.show_fps_en) {
			...
			if (!debug_layer_update_flag) {
				...
				memset(&dbg_disp.fps_info_dbg, 0,
				       sizeof(struct fps_debug));
			  ...
			  struct dprec_logger_fps *logger)
{
	...
	*time = 500 * 1000 * 1000;
	...
	for (i = 0; i < OVL_NUM; i++) {
		...
		layer_pos += layer_num;
		layer_num = _ovl_get_info(ovl_index, &(ovlInfo[layer_pos]),
					  &layer_max_idx);
...
void caculate_fps(void)
{
	...
	for (i = 0; i < 8; i++) {
		if (l->layer_fps[i] >= 1 && old_update_layer[i] == 1)
			fps_high_tmp =
			    (l->layer_fps[i] - 1) * 1000 * 1000 * 1000;
		else
...
void init_log_buffer(void)
{
	...
	if (!err_buffer)
		goto err;
	fence_buffer =
	    kzalloc(sizeof(char *) * FENCE_BUFFER_COUNT, GFP_KERNEL);
	if (!fence_buffer)
		goto err;
	...
	if (!dump_buffer)
		goto err;
	status_buffer =
	    kzalloc(sizeof(char *) * DUMP_BUFFER_COUNT, GFP_KERNEL);
	if (!status_buffer)
		goto err;
