...
int wdma_stop(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(handle, idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_EN,
		     0x00);
	DISP_REG_SET(
	    handle, idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_INTSTA, 0x00);
...
int wdma_start(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(handle,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_INTEN, 0x03);
...
static int wdma_config_yuv420(enum DISP_MODULE_ENUM module,
			      ...
			      unsigned int dstPitch, unsigned int Height,
	...
	enum DISP_BUFFER_TYPE sec, void *handle)
{
	...
	DISP_REG_SET(handle, idx_offst + DISP_REG_WDMA_DST_W_IN_BYTE,
		     dstPitch);
...
void wdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP(
		...
		(DISP_REG_GET(DISP_REG_WDMA_CFG + idx_offst) >> 10) & 0x1,
		0)));
	DDPDUMP(
...
void wdma_dump_reg(enum DISP_MODULE_ENUM module)
{
	if (disp_helper_get_option(DISP_OPT_REG_PARSER_RAW_DUMP)) {
		...
		unsigned long module_base =
		    DISPSYS_WDMA0_BASE + idx * DISP_WDMA_INDEX_OFFSET;
		...
		DDPDUMP(
		    "0x020=0x%08x,0x028=0x%08x,0x02c=0x%08x,0x038=0x%08x\n",
		...
		DDPDUMP(
		    "0x03c=0x%08x,0x058=0x%08x,0x05c=0x%08x,0x060=0x%08x\n",
		...
		DDPDUMP(
		    "0x064=0x%08x,0x078=0x%08x,0x080=0x%08x,0x084=0x%08x\n",
		...
		DDPDUMP(
		    "0x088=0x%08x,0x0a0=0x%08x,0x0a4=0x%08x,0x0a8=0x%08x\n",
			       ...
			       unsigned int is_primary_flag, void *cmdq)
{
	...
	if (is_primary_flag) {
		...
		fifo_off_dvfs = 2;
		res =
		    p_golden_setting->dst_width * p_golden_setting->dst_height;
	} else {
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_SMI_CON,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON1,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON3,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON4,
		     regval);
	...
	preultra_low = (preultra_low_us + fifo_off_drs_enter) * consume_rate *
		       bytes_per_sec;
	preultra_low_UV =
	    (preultra_low_us + fifo_off_drs_enter) * consume_rate;
	...
	preultra_high_UV =
	    (preultra_high_us + fifo_off_drs_enter) * consume_rate;
	do_div(preultra_high, 1000);
	do_div(preultra_high_UV, 1000);

	ultra_high = (ultra_high_us + fifo_off_drs_enter) * consume_rate *
		     bytes_per_sec;
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON5,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON6,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON7,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON8,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON9,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON10,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_DRS_CON0,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_DRS_CON1,
		     regval);

	ultra_low =
	    (ultra_low_us + fifo_off_drs_leave) * consume_rate * bytes_per_sec;
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_DRS_CON2,
		     regval);
	...
	/*DVFS*/
	preultra_low =
	    (preultra_low_us + fifo_off_dvfs) * consume_rate * bytes_per_sec;
	preultra_low_UV = (preultra_low_us + fifo_off_dvfs) * consume_rate;
	do_div(preultra_low, 1000);
	do_div(preultra_low_UV, 1000);

	preultra_high =
	    (preultra_high_us + fifo_off_dvfs) * consume_rate * bytes_per_sec;
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON11,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON13,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON14,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON15,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON16,
		     regval);
	...
	DISP_REG_SET(cmdq,
		     idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_BUF_CON18,
		     regval);
				     ...
				     void *handle)
{
	...
	cmdq_engine =
	    wdma_idx == 0 ? CMDQ_ENG_DISP_WDMA0 : CMDQ_ENG_DISP_WDMA1;
