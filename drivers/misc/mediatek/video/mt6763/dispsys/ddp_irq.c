...
void disp_invoke_irq_callbacks(enum DISP_MODULE_ENUM module,
			       unsigned int param)
{
	int i;
...
irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
	...
	if (irq == ddp_get_module_irq(DISP_MODULE_DSI0) ||
		...
		/* rd_rdy don't clear and wait for ESD & Read LCM will clear
		 * the
		 * bit.
		 */
		...
		if (reg_val & 0x2)
			mmprofile_log_ex(
			    ...
			    (cnt_wdma_underflow[index] << 24) | (index << 16) |
				reg_val,
			    module);
		...
		/* for debug */
		pr_debug("[DDP/IRQ] mutex_status = 0x%08x\n",
	...
	} else if (irq == ddp_get_module_irq(
			      DISP_MODULE_CONFIG)) { /* MMSYS error intr */
	...
	while (1) {
		...
		if (disp_irq_rdma_underflow) {
			...
			if (disp_helper_get_option(
				...
				/* Just count the underflow which happens more
				 * frequently
				 */
				if (last_timer != 0) {
					...
					do_div(freq,
					       sched_clock() - last_timer);
					if (freq > 0)
						considerable_cnt++;
					else
						considerable_cnt = 0;
				}
				/* Should trigger AEE as more than 5 times
				 * continuous underflow happens
				 */
