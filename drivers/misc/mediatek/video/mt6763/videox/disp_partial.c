...
static void _update_layer_dirty(struct OVL_CONFIG_STRUCT *old,
					  ...
					  struct disp_rect *out)
{
	...
	DISPDBG("pic to ovl dirty(%d,%d,%d,%d)->(%d,%d,%d,%d)\n", pic_roi.x,
		pic_roi.y, pic_roi.width, pic_roi.height, out->x, out->y,
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (input_cfg->dirty_roi_num) {
			...
			if (!rect_isEmpty(&layer_total_roi))
				_convert_picture_to_ovl_dirty(
				    input_cfg, &layer_total_roi,
				    &layer_total_roi);
		}
