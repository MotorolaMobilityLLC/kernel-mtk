...
#define MTKFB_LOG(fmt, arg...)                                                \
	do {                                                                  \
		if (mtkfb_log_on)                                             \
			DISP_LOG_PRINT(ANDROID_LOG_WARN, "MTKFB", fmt,        \
				       ##arg);                                \
	} while (0)
...
static int mtkfb_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		...
		if (bypass_blank) {
			pr_debug("FB_BLANK_UNBLANK bypass_blank %d\n",
				bypass_blank);
			break;
		}
	...
	case FB_BLANK_POWERDOWN:
		...
		if (bypass_blank) {
			pr_debug("FB_BLANK_POWERDOWN bypass_blank %d\n",
				bypass_blank);
			break;
		}
					   ...
					   struct disp_input_config *dst)
{
	...
	default:
		pr_debug("Invalid color format: 0x%x\n", src->src_fmt);
		return -1;
	}
				  ...
				  struct fb_info *info)
{
	...
	if (!session_input) {
		pr_debug("session input allocat fail\n");
		ASSERT(0);
		return -1;
	}
	...
	case 32:
		...
		break;
	default:
		pr_debug("Invalid color format bpp: %d\n", var->bits_per_pixel);
		kfree(session_input);
		return -1;
	}
...
static void set_fb_fix(struct mtkfb_device *fbdev)
{
	...
	case 8:
		...
		break;
	default:
		pr_debug("set fb fix fail,bits per pixel=%d\n",
			var->bits_per_pixel);
		return;
	}
		       ...
		       unsigned long arg)
{
	...
	case MTKFB_SET_AOD_POWER_MODE: {
		...
		if (aod_pm == MTKFB_AOD_DOZE_SUSPEND) {
			...
			pr_debug("AOD: error: unknown AOD power mode %d\n",
				aod_pm);
		}
		if (ret < 0)
			pr_debug("AOD: set %s failed\n",
	...
	case MTKFB_POWEROFF: {
		...
		/*ret = primary_display_suspend();*/
		if (ret < 0)
			pr_debug("primary display suspend failed\n");
	...
	case MTKFB_ERROR_INDEX_UPDATE_TIMEOUT: {
		pr_debug(
		    "[DDP] %s:MTKFB_ERROR_INDEX_UPDATE_TIMEOUT\n", __func__);
	...
	case MTKFB_META_RESTORE_SCREEN: {
		...
		if (info->var.yres + info->var.yoffset >
...
int __init __parse_tag_ext_videolfb(unsigned long node)
{
	...
	tag_ext_videolfb = (struct tag_ext_videolfb *)of_get_flat_dt_prop(
	    node, "atag,ext_videolfb", (int *)&size);
	if (tag_ext_videolfb) {
		memset((void *)ext_mtkfb_lcm_name, 0,
		       sizeof(ext_mtkfb_lcm_name));
		strcpy((char *)ext_mtkfb_lcm_name,
		       tag_ext_videolfb->ext_lcmname);
		ext_mtkfb_lcm_name[strlen(tag_ext_videolfb->ext_lcmname)] =
		    '\0';
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	if (r) {
		pr_debug("mtkfb_register_sysfs fail, r = %d\n", r);
		goto cleanup;
	}
	init_state++; /* 5 */
...
static void mtkfb_late_resume(void)
{
	...
	if (ret) {
		pr_debug("primary display resume failed\n");
		return;
	}
