...
int ovl2mem_get_info(void *info)
{
	...
	/* FIXME,  for decouple mode, should dynamic return 4 or 8, please
	 * refer
				      ...
				      struct disp_input_config *src)
{
	...
	if (!src || !dst) {
		pr_debug("%s src(0x%p) or dst(0x%p) is null\n", __func__, src,
			dst);
		return -1;
	}
...
static int ovl2mem_callback(unsigned int userdata)
{
	...
	for (layid = 0; layid < (MEMORY_SESSION_INPUT_LAYER_COUNT); layid++) {
		cmdqBackupReadSlot(pgcl->ovl2mem_cur_config_fence, layid,
				   &fence_idx);
		cmdqBackupReadSlot(pgcl->ovl2mem_subtractor_when_free, layid,
				   &subtractor);
...
int ovl2mem_init(unsigned int session)
{
	...
	if (pgcl->state > 0) {
		pr_debug("path has created, state%d\n", pgcl->state);
		goto Exit;
	}
	...
	/* dpmgr_path_set_dst_module(pgcl->dpmgr_handle,DISP_MODULE_ENUM
	 * dst_module)
	 */
...
static int ovl2mem_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		cmdqRecBackupUpdateSlot(pgcl->cmdq_handle_config,
					pgcl->ovl2mem_subtractor_when_free, i,
					ext_sub);
	}
...
int ovl2mem_frame_cfg(struct disp_frame_cfg_t *cfg)
{
	...
	if (pgcl->state == 0) {
		pr_debug("ovl2mem is already slept\n");
		_ovl2mem_path_unlock(__func__);
		return 0;
	}
