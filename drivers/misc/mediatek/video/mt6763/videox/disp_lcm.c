...
int _is_lcm_inited(struct disp_lcm_handle *plcm)
{
	...
	pr_debug("WARNING, invalid lcm handle: %p\n", plcm);
	return 0;
}
...
void parse_lcm_params_dt_node(struct device_node *np,
			      struct LCM_PARAMS *struct LCM_PARAMS)
{
	if (!lcm_params) {
	...
	disp_of_getprop_u32(
	    np, "lcm_params-dsi-vertical_frontporch_for_low_power",
	...
	disp_of_getprop_u32(
	    np, "lcm_params-dsi-horizontal_sync_active_word_count",
			   ...
			   unsigned char *dts)
{
	...
	for (i = 0; i < INIT_SIZE; i++) {
		...
		case LCM_FUNC_GPIO:
			memcpy(&(lcm_dts->init[i].data_t1), tmp + 3,
			       lcm_dts->init[i].size);
			break;

		case LCM_FUNC_I2C:
			memcpy(&(lcm_dts->init[i].data_t2), tmp + 3,
			       lcm_dts->init[i].size);
			break;
		...
		case LCM_FUNC_CMD:
			...
			case LCM_UTIL_WRITE_CMD_V1:
				memcpy(&(lcm_dts->init[i].data_t5), tmp + 3,
				       lcm_dts->init[i].size);
				break;

			case LCM_UTIL_WRITE_CMD_V2:
				memcpy(&(lcm_dts->init[i].data_t3), tmp + 3,
				       lcm_dts->init[i].size);
				break;
	...
	if (len <= 0) {
		...
		for (i = 0; i < COMPARE_ID_SIZE; i++) {
			...
			case LCM_FUNC_GPIO:
				memcpy(&(lcm_dts->compare_id[i].data_t1),
				       tmp + 3, lcm_dts->compare_id[i].size);
				break;

			case LCM_FUNC_I2C:
				memcpy(&(lcm_dts->compare_id[i].data_t2),
				       tmp + 3, lcm_dts->compare_id[i].size);
				break;
	...
	if (len <= 0) {
		pr_debug(
		    "%s:%d: Cannot find LCM suspend table, cannot skip it!\n",
		    __FILE__, __LINE__);
		return;
	}
	...
	for (i = 0; i < SUSPEND_SIZE; i++) {
		...
		case LCM_FUNC_GPIO:
			memcpy(&(lcm_dts->suspend[i].data_t1), tmp + 3,
			       lcm_dts->suspend[i].size);
			break;

		case LCM_FUNC_I2C:
			memcpy(&(lcm_dts->suspend[i].data_t2), tmp + 3,
			       lcm_dts->suspend[i].size);
			break;
	...
	if (len <= 0) {
		...
		for (i = 0; i < BACKLIGHT_SIZE; i++) {
			...
			case LCM_FUNC_GPIO:
				memcpy(&(lcm_dts->backlight[i].data_t1),
				       tmp + 3, lcm_dts->backlight[i].size);
				break;

			case LCM_FUNC_I2C:
				memcpy(&(lcm_dts->backlight[i].data_t2),
				       tmp + 3, lcm_dts->backlight[i].size);
				break;
	...
	if (len <= 0) {
		pr_debug(
		    "%s:%d: Cannot find LCM backlight cmdq table, skip it!\n",
		    __FILE__, __LINE__);
	} else {
		...
		for (i = 0; i < BACKLIGHT_CMDQ_SIZE; i++) {
			...
			case LCM_FUNC_GPIO:
				memcpy(&(lcm_dts->backlight_cmdq[i].data_t1),
				       tmp + 3,
				       lcm_dts->backlight_cmdq[i].size);
				break;

			case LCM_FUNC_I2C:
				memcpy(&(lcm_dts->backlight_cmdq[i].data_t2),
				       tmp + 3,
				       lcm_dts->backlight_cmdq[i].size);
				break;
			...
			default:
				pr_debug("%s:%d: %d\n", __FILE__, __LINE__,
				       (unsigned int)lcm_dts->backlight_cmdq[i]
					   .func);
				return;
			}
				       ...
				       int is_lcm_inited)
{
...
#endif
	if (_lcm_count() == 0) {
		pr_debug("no lcm driver defined in linux kernel driver\n");
		return NULL;
	...
	if (isLCMFound == false) {
		pr_debug("FATAL ERROR!!!No LCM Driver defined\n");
		return NULL;
	}
	...
	plcm = kzalloc(sizeof(uint8_t *) * sizeof(struct disp_lcm_handle),
		       GFP_KERNEL);
	lcm_param =
	    kzalloc(sizeof(uint8_t *) * sizeof(struct LCM_PARAMS), GFP_KERNEL);
	if (plcm && lcm_param) {
					   ...
					   int is_lcm_inited)
{
	...
	if (_lcm_count() < 2) {
		pr_debug("no ext lcm driver defined in linux kernel driver\n");
		return NULL;
	...
	(CONFIG_MTK_DUAL_DISPLAY_SUPPORT == 2)
			...
			for (i = 0; i < _lcm_count(); i++) {
				if (strcmp(lcm_driver_list[i]->name,
					...
					lcm_drv = lcm_driver_list[i];
					DISPCHECK(
					    "EXT_LCM Driver name is %s\n",
					    lcm_drv->name);
					break;
				}
			}
		if (lcm_drv == NULL) {
			pr_debug("NO EXT_LCM,lk ext lcm is(%s)\n", plcm_name);
				return NULL;
			}
	...
	if (isLCMFound == false) {
		pr_debug("FATAL ERROR!!!No EXT LCM Driver defined\n");
		return NULL;
	}
...
int disp_lcm_init(struct disp_lcm_handle *plcm, int force)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->init) {
			...
			pr_debug("FATAL ERROR, lcm_drv->init is null\n");
			return -1;
		}
		    ...
		    int force)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->update) {
			...
			pr_debug("lcm is cmd mode lcm_drv->update is null\n");
			return -1;
		}
...
int disp_lcm_esd_check(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		pr_debug("FATAL ERROR, lcm_drv->esd_check is null\n");
		return 0;
...
int disp_lcm_suspend(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->suspend) {
			...
			pr_debug("FATAL ERROR, lcm_drv->suspend is null\n");
			return -1;
		}
...
int disp_lcm_resume(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->resume) {
			...
			pr_debug("FATAL ERROR, lcm_drv->resume is null\n");
			return -1;
		}
...
int disp_lcm_aod(struct disp_lcm_handle *plcm, int enter)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->aod) {
			...
			pr_debug("FATAL ERROR, lcm_drv->aod is null\n");
			return -1;
		}
		return 0;
	}
...
unsigned int disp_lcm_ATA(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->ata_check) {
			...
			pr_debug("FATAL ERROR, lcm_drv->ata_check is null\n");
			return 0;
		}
...
void *disp_lcm_switch_mode(struct disp_lcm_handle *plcm, int mode)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		pr_debug("Not enable switch in lcm_get_params function\n");
			return NULL;
		}
		lcm_drv = plcm->drv;
		if (lcm_drv->switch_mode) {
			...
			pr_debug("FATAL ERROR, lcm_drv->switch_mode is null\n");
			return NULL;
		}
			 ...
			 unsigned int *lcm_value)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->set_lcm_cmd) {
			...
			pr_debug("FATAL ERROR, lcm_drv->set_lcm_cmd is null\n");
			return -1;
		}
			  ...
			  int *h)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		pr_debug("Not support partial roi\n");
		return -1;
	}
	pr_debug("validate roi lcm_drv is null\n");
	return -1;
}
