...
void disp_init_ged_log_handle(void)
{
	...
	if (ret != 0)
		pr_debug("ged_log_buf_get_early Failed!\n");
}
...
static int frame_wait_all_fence(struct disp_frame_cfg_t *cfg)
{
	...
	if (cfg->prev_present_fence_struct) {
		...
		if (tmp) {
			pr_debug("wait present fence fail! cnt=%d\n", cnt);
			cnt++;
			ret = -1;
		}
	}
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		tmp = _do_wait_fence(
		    (struct sync_fence **)&cfg->input_cfg[i].src_fence_struct,
		    session_id, cfg->input_cfg[i].layer_id,
		    cfg->input_cfg[i].src_fence_fd,
		    cfg->input_cfg[i].next_buff_idx, present_fence_idx);
		if (tmp) {
	...
	if (cfg->output_en && cfg->output_cfg.src_fence_struct) {
		...
		if (tmp) {
			pr_debug("wait output fence fail!\n");
			ret = -1;
		}
		disp_sync_buf_cache_sync(session_id,
				 ...
				 int session_id)
{
	...
	head->worker = kthread_run(
	    fence_wait_worker_func, head, "disp_queue_%s%d",
...
static int frame_queue_size(struct frame_queue_head_t *head)
{
	...
	list_for_each(list, &head->queue) cnt++;
		     ...
		     struct frame_queue_t *node)
{
	...
	if (frame_queue_sz >= 5) {
		 ...
		 */
		mutex_unlock(&head->lock);
		pr_debug("block HWC because jobs=%d >=5\n", frame_queue_sz);
