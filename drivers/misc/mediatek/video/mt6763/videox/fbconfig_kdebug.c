...
static void *pm_get_handle(void) { return (void *)&pm_params; }
			   ...
			   uint8_t *buffer, uint32_t num)
{
	...
	if ((dsi_id == PM_DSI0) || (dsi_id == PM_DSI_DUAL))
		ret = DSI_dcs_read_lcm_reg_v2(pm_get_dsi_handle(PM_DSI0), NULL,
					      cmd, buffer, num);
	else if (dsi_id == PM_DSI1)
		ret = DSI_dcs_read_lcm_reg_v2(pm_get_dsi_handle(PM_DSI1), NULL,
					      cmd, buffer, num);
			   ...
			   unsigned long arg)
{
	...
	case LCM_GET_ESD: {
		...
		ret = fbconfig_get_esd_check_test(dsi_id, esd_para.addr,
			       ...
			       struct ESD_PARA *data)
{
	...
	err |=
	    put_user((unsigned char *)(unsigned long)d, &data->esd_ret_buffer);
...
{
	ConfigPara_dbgfs = debugfs_create_file(
	    "fbconfig", S_IFREG | 0444, NULL, (void *)0, &fbconfig_fops);

	INIT_LIST_HEAD(&head_list.list);
}

void PanelMaster_Deinit(void) { debugfs_remove(ConfigPara_dbgfs); }
