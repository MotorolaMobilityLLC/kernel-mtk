...
#define ASSERT(expr)                                                          \
	do {                                                                  \
		...
		aee_kernel_exception("fence", "[FENCE]error:", __FILE__,      \
				     __LINE__);                               \
	} while (0)
#endif
...
_get_session_sync_info(unsigned int session_id)
{
	...
	if (DISP_SESSION_TYPE(session_id) != DISP_SESSION_PRIMARY &&
		...
		pr_debug("invalid session id:0x%08x\n", session_id);
		return NULL;
	}
	...
	for (i = 0; i < ARRAY_SIZE(_disp_fence_context); i++) {
		...
		session_info = &(_disp_fence_context[i]);
			...
			sprintf(name, "%s%d_frame_cfg",
				...
				DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(
			    &session_info->event_frame_cfg, name,
			    DPREC_LOGGER_LEVEL_DEFAULT,
			...
			sprintf(name, "%s%d_wait_fence",
				...
				DISP_SESSION_DEV(session_id));
			dprec_logger_event_init(
			    &session_info->event_wait_fence, name,
			    DPREC_LOGGER_LEVEL_DEFAULT,
			    &session_info->event_prepare.mmp);
			...
			for (j = 0;
				...
				if (DISP_SESSION_TYPE(session_id) ==
					...
					sprintf(
					    name, "%s-primary-%d-%d", prefix,
					    DISP_SESSION_DEV(session_id), j);
				else if (DISP_SESSION_TYPE(session_id) ==
					 DISP_SESSION_EXTERNAL)
					sprintf(
					    name, "%s-external-%d-%d", prefix,
					    DISP_SESSION_DEV(session_id), j);
				else if (DISP_SESSION_TYPE(session_id) ==
					 DISP_SESSION_MEMORY)
					sprintf(
					    name, "%s-memory-%d-%d", prefix,
					    DISP_SESSION_DEV(session_id), j);
				else
					sprintf(
					    name, "%s-unknown-%d-%d", prefix,
					    DISP_SESSION_DEV(session_id), j);
	...
	if (session_info == NULL)
		pr_debug("wrong session_id:%d, 0x%08x\n", session_id,
			session_id);
				      ...
				      unsigned int timeline_id)
{
	...
	if (session_info) {
		if (timeline_id >=
			...
			pr_debug("invalid timeline_id:%d\n", timeline_id);
			goto done;
		} else {
			layer_info =
	...
	if (layer_info->inited == 0) {
		pr_debug("layer_info[%d] not inited\n", timeline_id);
		goto done;
	}
			       ...
			       struct disp_input_config *buf_info)
{
	...
	DISPMSG(
	    ...
	    session_id, buf_info->layer_id, buf_info->next_buff_idx, buf->mva,
	    buf_info->layer_type);
...
static struct mtkfb_fence_buf_info *mtkfb_get_buf_info(void)
{
	...
	/* we must use another mutex for buffer list because it will be
	 * operated
	 * by ALL layer info.
	 */
	...
	if (!list_empty(&info_pool_head)) {
		...
		info =
		    kzalloc(sizeof(struct mtkfb_fence_buf_info), GFP_KERNEL);
		mtkfb_init_buf_info(info);
			 ...
			 int fence)
{
	...
	if (layer_info->timeline != NULL) {
		...
		if (ion_release_count != num_fence)
			pr_debug("released %d fence but %d ion handle freed\n",
...
int disp_sync_get_present_timeline_id(unsigned int session_id)
{
	...
	pr_debug("session id is wrong, session=0x%x!!\n", session_id);
	return -1;
}
				       ...
				       unsigned long mva)
{
	...
	disp_sync_convert_input_to_fence_layer_info_v2(
	    &(layer_info->cached_config), timeline_idx, fence_id, layer_en,
	    mva);
			   ...
			   struct mtkfb_fence_buf_info *buf_info)
{
	...
	if (handle)
		...
		pr_debug("can't import ion handle for fd:%d\n",
			     buf->ion_fd);
...
disp_sync_prepare_buf(struct disp_buffer_info *buf)
{
	...
	if (buf == NULL) {
		pr_debug("Prepare Buffer, buf is NULL!!\n");
		return NULL;
	}
	...
	if (ret != 0) {
		...
		pr_debug("%s%d,layer%d create Fence Object failed ret=%d!\n",
			disp_session_mode_spy(session_id),
				     ...
				     unsigned long phy_addr)
{
	...
	if (layer_info->fence_idx == 0)
		/* pr_debug("layer_info->fence_idx == 0!\n"); */
		return -2;
				      ...
				      unsigned int idx)
{
	...
	if (!found) {
		pr_debug(
			...
			disp_session_mode_spy(session_id),
			DISP_SESSION_DEV(session_id), timeline_id);
		pr_debug("idx=%d, fence_idx=%d, timeline_idx=%d, cur_idx=%d!\n",
					 ...
					 int need_sync)
{
	...
	if (layer_info == NULL || mva == NULL || size == NULL) {
		pr_debug("layer_info is null, layer_info=%p, mva=%p, size=%p\n",
			layer_info, mva, size);
		return 0;
	}
			...
			if (va) {
				if (buf->va)
					*va = (void *)buf->va;
					...
					tmp_va = ion_map_kernel(ion_client,
								buf->hnd);
	...
	if (dst_mva != 0x0) {
		...
		*size = dst_size;
		...
		/* FIXME: non-ion buffer need cache sync here? */
		pr_debug("cannot find this buf, session:%s%d, layer=%d,\n",
			disp_session_mode_spy(session_id),
			DISP_SESSION_DEV(session_id), timeline_id);
		pr_debug("idx=%d, fence_idx=%d, timeline_idx=%d, cur_idx=%d!\n",
			idx, layer_info->fence_idx, layer_info->timeline_idx,
...
int disp_sync_get_debug_info(char *stringbuf, int buf_len)
{
	...
	/* for (i = 0; i < ARRAY_SIZE(_disp_fence_context) /
	 * sizeof(_disp_fence_context[0]); i++) {
	 */
