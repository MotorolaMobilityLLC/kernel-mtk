...
/* wfd connected(session is existing whereas ext mode or dcm mode), or
 * screenrecord
 */
...
int disp_mgr_has_mem_session(void) { return has_memory_session; }
...
int disp_create_session(struct disp_session_config *config)
{
	...
	if (idx != -1) {
		...
		if (session ==
			...
			/* disable dynamic switch for screen idle, avoid
			 * conflict
			 */
		...
		pr_debug("Invalid session creation request\n");
		ret = -1;
	}
done:
...
int disp_destroy_session(struct disp_session_config *config)
{
	...
	if (ret == 0) {
		...
		pr_debug("session(0x%x) does not exists\n", session);
...
int _ioctl_prepare_present_fence(unsigned long arg)
{
	...
	timeline_id =
	    disp_sync_get_present_timeline_id(preset_fence_struct.session_id);
	layer_info =
	    _get_sync_info(preset_fence_struct.session_id, timeline_id);
	if (layer_info == NULL) {
		pr_debug("layer_info is null\n");
		ret = -EFAULT;
		return ret;
	}
	...
	data.value =
	    ++prepare_present_fence_idx[DISP_SESSION_TYPE(
...
int disp_validate_ioctl_params(struct disp_frame_cfg_t *cfg)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++)
		if (disp_validate_input_params(
			&cfg->input_cfg[i], _get_max_layer(cfg->session_id)) !=
		    0)
			return -1;
...
static int disp_input_get_dirty_roi(struct disp_frame_cfg_t *cfg)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (copy_from_user(addr, cfg->input_cfg[i].dirty_roi_addr,
				   size)) {
			pr_debug(
			    "[drity roi]: copy_from_user failed! line:%d\n",
			    __LINE__);
			pr_debug("to=%p, from=%p, size=0x%lx\n", addr,
...
static int input_config_preprocess(struct disp_frame_cfg_t *cfg)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (layer_id >= _get_max_layer(session_id)) {
			pr_debug("set_%s_buffer, invalid layer_id = %d!\n",
...
/* get src fence */
			...
			if (!src_fence &&
				...
				pr_debug("error to get src_fence from fd %d\n",
					cfg->input_cfg[i].src_fence_fd);
				is_err = 1;
			}
			cfg->input_cfg[i].src_fence_struct = src_fence;
#endif
			/* OVL addr is not the start address of buffer, which
			 * is
...
static int output_config_preprocess(struct disp_frame_cfg_t *cfg)
{
	...
	if (!dst_mva) {
		pr_debug("%s output mva=0!!, skip it\n", __func__);
		cfg->output_en = 0;
		goto out;
	}
/* get src fence */
	...
	if (!src_fence && cfg->output_cfg.src_fence_fd != -1) {
		pr_debug("error to get src_fence from output fd %d\n",
...
/* get present fence */
	...
	if (frame_cfg->prev_present_fence_fd != -1) {
		...
		if (!present_fence) {
			pr_debug("error to get prev_present_fence from fd %d\n",
...
static long _ioctl_frame_config(unsigned long arg)
{
	...
	if (copy_from_user(frame_cfg, (void __user *)arg,
			   sizeof(*frame_cfg))) {
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
	 ...
	 (defined(CONFIG_MTK_DUAL_DISPLAY_SUPPORT) &&                         \
	  (CONFIG_MTK_DUAL_DISPLAY_SUPPORT == 2)))
		...
		pr_debug("session type is wrong:0x%08x\n", session_id);
		return -1;
	}
...
int _ioctl_set_vsync(unsigned long arg)
{
	...
	if ((fps < 50) || (fps > 60)) {
		pr_debug(
		    "%s fps setting is out of range, fps=%d\n",
		    __func__, fps);
		return -EFAULT;
	}
	DISPMSG("%s fps setting is %d\n", __func__, fps);
	ret = primary_display_force_set_vsync_fps(
	    fps, 1); /* second parameter means APP set FPS */
...
int _ioctl_set_scenario(unsigned long arg)
{
	...
	if (ret) {
		pr_debug("session(0x%x) set scenario (%d) fail, ret=%d\n",
...
int set_session_mode(struct disp_session_config *config_info, int force)
{
	...
	if (config_info->mode == DISP_SESSION_DIRECT_LINK_MIRROR_MODE ||
		...
		if (has_memory_session)
			primary_display_switch_mode_blocked(
			    config_info->mode, config_info->session_id, 0);
		else if (DISP_SESSION_TYPE(config_info->session_id) ==
			 DISP_SESSION_PRIMARY)
			primary_display_switch_mode(
			    config_info->mode, config_info->session_id, 0);
		else
			pr_debug("[FB]: session(0x%x) swith mode(%d) fail\n",
				config_info->session_id, config_info->mode);
	} else {

		if (has_memory_session)
			primary_display_switch_mode_blocked(
			    config_info->mode, config_info->session_id, 1);
		else if (DISP_SESSION_TYPE(config_info->session_id) ==
			 DISP_SESSION_PRIMARY)
			primary_display_switch_mode(
			    config_info->mode, config_info->session_id, 0);
		...
		/* DCm -> Ext mode, switch to Directlink first, then create Ext
		 * path
		 */
		external_display_switch_mode(config_info->mode, session_config,
					     config_info->session_id);
...
long mtk_disp_mgr_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	default: {
		pr_debug("[session]ioctl not supported, 0x%08x\n", cmd);
	}
	}
				      ...
				      unsigned long arg)
{
	...
	default: {
		pr_debug("[%s]ioctl not supported, 0x%08x\n", __func__, cmd);
		return -ENOIOCTLCMD;
	}
	}
...
static int mtk_disp_mgr_suspend(struct platform_device *pdev,
				pm_message_t mesg)
{
	return 0;
}
