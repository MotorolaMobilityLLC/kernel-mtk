...
enum DAL_STATUS DAL_SetColor(unsigned int fgColor, unsigned int bgColor)
{
	...
	DAL_CHECK_MFC_RET(
	    MFC_SetColor(mfc_handle, dal_fg_color, dal_bg_color));
	DAL_UNLOCK();
...
{
	/* const uint32_t BG_COLOR = MAKE_TWO_RGB565_COLOR(DAL_BG_COLOR,
	 * DAL_BG_COLOR);
	 */
	...
	if (MFC_ResetCursor(mfc_handle) != MFC_STATUS_OK) {
		pr_debug("mfc_handle = %p\n", mfc_handle);
		goto End;
	}
	ctxt->screen_color = 0;
	DAL_SetScreenColor(DAL_COLOR_RED);

	/* TODO: if dal_shown=false, and 3D enabled, mtkfb may disable UI
	 * layer,
...
enum DAL_STATUS DAL_Printf(const char *fmt, ...)
{
	...
	if (i >= ARRAY_SIZE(dal_print_buffer)) {
		pr_debug("[AEE]dal print buffer no space, i=%d\n", i);
		return -1;
	}
...
enum DAL_COLOR color_wdma[24] = {
	DAL_COLOR_PINK,      DAL_COLOR_GREEN,       DAL_COLOR_BLUE,
	DAL_COLOR_RED,       DAL_COLOR_MAROON,      DAL_COLOR_STEEL_BLUE,
	DAL_COLOR_DARK_CYAN, DAL_COLOR_OLIVE_GREEN, DAL_COLOR_CORNSILK,
	DAL_COLOR_TURQUOISE, DAL_COLOR_YELLOW,      DAL_COLOR_BLACK,
};
...
int show_layers_draw_wdma(struct Layer_draw_info *draw_info)
{
	...
	for (i = layer_num - 1; i >= 0; i--) {
		width_x = draw_info->frame_lower_right_x -
			  draw_info->frame_upper_left_x;
		height_y = draw_info->frame_lower_right_y -
			   draw_info->frame_upper_left_y;
		for (j = draw_info->frame_upper_left_y[i];
		     j < draw_info->frame_upper_left_y[i] +
			     draw_info->frame_width[i];
		     j++) {
			buffer_pos = show_layers_va + j * DAL_WIDTH * 3;
			for (k = draw_info->frame_upper_left_x[i];
			     k <= draw_info->frame_lower_right_x[i]; k++) {
				*(char *)(buffer_pos + 3 * k + 2) =
				    (color_wdma[i] & 0xff0000) >> 16;
				*(char *)(buffer_pos + 3 * k + 1) =
				    (color_wdma[i] & 0x00ff00) >> 8;
				*(char *)(buffer_pos + 3 * k) =
				    color_wdma[i] & 0x0000ff;
			}
		}
		for (j = draw_info->frame_lower_right_y[i] -
			 draw_info->frame_width[i] + 1;
		     j < draw_info->frame_lower_right_y[i] + 1; j++) {
			buffer_pos =
			    (void *)(show_layers_va + j * DAL_WIDTH * 3);
			for (k = draw_info->frame_upper_left_x[i];
			     k <= draw_info->frame_lower_right_x[i]; k++) {
				*(char *)(buffer_pos + 3 * k + 2) =
				    (color_wdma[i] & 0xff0000) >> 16;
		...
		for (j = draw_info->frame_upper_left_y[i];
		     j < draw_info->frame_lower_right_y[i]; j++) {
			buffer_pos = show_layers_va + j * DAL_WIDTH * 3;
			for (k = draw_info->frame_upper_left_x[i];
			     k < draw_info->frame_upper_left_x[i] +
				     draw_info->frame_width[i];
			     k++) {
				*(char *)(buffer_pos + 3 * k + 2) =
				    (color_wdma[i] & 0xff0000) >> 16;
			...
			for (k = draw_info->frame_lower_right_x[i] -
				 draw_info->frame_width[i] + 1;
	...
	for (i = 0; i < layer_num; i++) {
		MFC_SetCursor(show_mfc_handle, draw_info->digit_x[i] / 8,
			      draw_info->digit_y[i] / 16);
...
enum DAL_STATUS DAL_Clean(void)
{
	pr_debug("[MTKFB_DAL] %s is not implemented\n", __func__);
	return DAL_STATUS_OK;
}
EXPORT_SYMBOL(DAL_Clean);
...
int is_DAL_Enabled(void) { return 0; }
