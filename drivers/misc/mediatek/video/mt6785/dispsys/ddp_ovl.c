...
enum CMDQ_EVENT_ENUM ovl_to_cmdq_event_nonsec_end(enum DISP_MODULE_ENUM module)
{
	...
	default:
		DDP_PR_ERR(
			"invalid ovl module=%d, get cmdq event nonsecure fail\n",
			   module);
		ASSERT(0);
...
int ovl_start(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(handle, baddr + DISP_REG_OVL_GDRDY_PRD, 0xffffffff);
		 ...
		 const struct disp_rect *const layer_partial_roi, void *handle)
{
	...
	value = REG_FLD_VAL(L_PITCH_FLD_SA_SEL, cfg->src_alpha & 0x3) |
		...
		REG_FLD_VAL(L_PITCH_FLD_SRC_PITCH, cfg->src_pitch & 0xFFFF);
	...
	value = 0;
	value = (REG_FLD_VAL((L_PITCH_MSB_FLD_YUV_TRANS), (0)) |
		REG_FLD_VAL((L_PITCH_MSB_FLD_2ND_SUBBUF), (0)) |
		REG_FLD_VAL((L_PITCH_MSB_FLD_SRC_PITCH_MSB),
		(cfg->src_pitch >> 16) & 0xF));
	DISP_REG_SET(handle, DISP_REG_OVL_L0_PITCH_MSB + Lx_base, value);
				...
				void *handle)
{
	...
	unsigned long Lx_clr_base = 0;
	unsigned long Lx_addr_base = 0;
	unsigned long Lx_comp_hdr_base = 0;
	...
	unsigned int tile_header_size = OVL_HEADER_SIZE_PER_TILE_BYTES;
	unsigned int tile_body_size = tile_w * tile_h * Bpp;
	unsigned int comp_header_align = OVL_HEADER_ALIGN_BYTES;
	bool half_tile_align = OVL_HALF_TILE_ALIGN;
	unsigned int Lx_2nd_subbuf = 0;
	unsigned int src_y_half_align = 0;
	unsigned int src_y_end_half_align = 0;
	unsigned int src_y_end_align = 0;
	unsigned int src_h_half_align = 0;
	...
	/*rgb only support RGB888 ARGB8888 ARGB2101010, not support 16FP*/
	if (!is_rgb || (Bpp != 4 && Bpp != 3)) {
	...
	if (cfg->ext_layer == -1) {
		...
		Lx_clr_base = baddr + phy_layer * 0x4;
		Lx_addr_base = baddr + phy_layer * OVL_LAYER_OFFSET;
		Lx_comp_hdr_base = baddr + phy_layer *
		...
		Lx_comp_hdr_base = baddr + cfg->ext_layer *
		    (DISP_REG_OVL_EL1_HDR_ADDR - DISP_REG_OVL_EL0_HDR_ADDR);
		Lx_comp_hdr_base += (DISP_REG_OVL_EL0_HDR_ADDR -
	...
	src_buf_tile_num = ALIGN_TO(cfg->src_pitch / Bpp, tile_w) *
	...
	if (half_tile_align) {
		...
		src_y_align = (src_y / tile_h) * tile_h;
		src_h_align =
			(1 + (src_y + src_h - 1) / tile_h) * tile_h -
			src_y_align;
	}

	if (rotate) {
		tile_offset = (src_x_align + src_w_align - tile_w) / tile_w +
			(src_pitch / tile_w / Bpp) *
			(src_y_align + src_h_align - tile_h) /
			tile_h;

		if (src_y_end_half_align != src_y_end_align)
			Lx_2nd_subbuf = 1;
	} else {
		tile_offset = src_x_align / tile_w +
			(src_pitch / tile_w / Bpp) * src_y_align / tile_h;

		if (src_y_half_align != src_y_align)
			Lx_2nd_subbuf = 1;
	}
	...
	/*bit per chanel 0:8bit, 1:10bit, 3:16bit*/
	value = 0;
	if (format == UFMT_RGBA1010102 || format == UFMT_PRGBA1010102)
		value = 1;
	...
	DISP_REG_SET(handle, DISP_REG_OVL_L0_SRC_SIZE + Lx_base,
		     src_h_half_align << 16 | src_w_align);
	...
	if (rotate)
		src_addr_offset = tile_offset * tile_body_size +
		(tile_body_size >> 1);
	else
		src_addr_offset = tile_offset * tile_body_size;

	/* sbch can use the variable */
	cfg->real_addr = buf_addr + src_addr_offset;
	cfg->real_dst_w = dst_w;
	cfg->real_dst_h = dst_h;
	...
	value = REG_FLD_VAL(L_PITCH_FLD_SA_SEL, cfg->src_alpha & 0x3) |
		...
		REG_FLD_VAL(L_PITCH_FLD_DRGB_SEL, cfg->dst_alpha & 0x3) |
		REG_FLD_VAL(L_PITCH_FLD_SURFL_EN, cfg->src_alpha & 0x1) |
		REG_FLD_VAL(L_PITCH_FLD_SRC_PITCH,
		(cfg->src_pitch * tile_h) & 0xFFFF);
	...
	value = 0;
	/*config OVL_Lx_PITCH_MSB Lx_2ND_SUBBUF 0x40*/
	value = (REG_FLD_VAL((L_PITCH_MSB_FLD_YUV_TRANS), (1)) |
		REG_FLD_VAL((L_PITCH_MSB_FLD_2ND_SUBBUF), (Lx_2nd_subbuf)) |
		REG_FLD_VAL((L_PITCH_MSB_FLD_SRC_PITCH_MSB),
		((cfg->src_pitch * tile_h) >> 16) & 0xF));
	DISP_REG_SET(handle, DISP_REG_OVL_L0_PITCH_MSB + Lx_base, value);
	...
	if (!rotate) {
		...
		if (half_tile_align) {
			if (src_y > src_y_half_align)
				value |= REG_FLD_VAL(OVL_L_CLIP_FLD_BOTTOM,
					src_y - src_y_half_align);
			if (src_y + src_h < src_y_half_align +
				src_h_half_align)
				value |= REG_FLD_VAL(OVL_L_CLIP_FLD_TOP,
					src_y_half_align + src_h_half_align -
					src_y - src_h);
	...
	/* 5. config compress header register */
	/* OVL_Lx_HDR_ADDR 0xF44 */
	DISP_REG_SET(handle, DISP_REG_OVL_L0_HDR_ADDR + Lx_comp_hdr_base,
		cfg->addr + tile_offset * OVL_HEADER_SIZE_PER_TILE_BYTES);

	value = cfg->src_pitch / Bpp / tile_w * OVL_HEADER_SIZE_PER_TILE_BYTES;

	/* OVL_Lx_HDR_PITCH 0xF48 */
	DISP_REG_SET(handle, DISP_REG_OVL_L0_HDR_PITCH + Lx_comp_hdr_base,
		value);
...
static int ovl_check_input_param(struct OVL_CONFIG_STRUCT *cfg)
{
	if ((cfg->addr == 0 && cfg->source == 0) ||
		...
		DDP_PR_ERR(
			"ovl parameter invalidate, addr=0x%08lx, w=%d, h=%d\n",
...
void ovl_dump_reg(enum DISP_MODULE_ENUM module)
{
	if (disp_helper_get_option(DISP_OPT_REG_PARSER_RAW_DUMP)) {
		...
		DDPDUMP(
			"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
