...
void init_ddp_mmp_events(void)
{
	...
	DDP_MMP_Events.dvfs = mmprofile_register_event(DDP_MMP_Events.DDP,
						       "dvfs");
	...
	DDP_MMP_Events.tmp_debug =
		mmprofile_register_event(DDP_MMP_Events.DDP, "tmp_debug");
			...
			unsigned int down_sample_y)
{
	...
	if (wdma_num > 1) {
		DDP_PR_ERR("dprec_mmp_dump_wdma_layer is error %d\n", wdma_num);
		return;
	}
	...
	case UFMT_YVYU:
		...
		break;
	default:
		DDP_PR_ERR("dprec_mmp_dump_wdma_layer(), unknown fmt=%d, dump raw\n",
	...
	if (!raw) {
		...
		if (disp_mva_map_kernel(DISP_MODULE_WDMA0,
			...
			DDP_PR_ERR("dprec_mmp_dump_wdma_layer(),fail to dump rgb(0x%x)\n",
				   wdma_layer->outputFormat);
		}
	} else {
		meta.data_type = MMPROFILE_META_RAW;
		...
		if (disp_mva_map_kernel(DISP_MODULE_WDMA0,
			...
			DDP_PR_ERR("dprec_mmp_dump_wdma_layer(),fail to dump raw(0x%x)\n",
				   wdma_layer->outputFormat);
		}
	}
}
			...
			unsigned int down_sample_y)
{
	...
	if (rdma_num > 1) {
		DDP_PR_ERR("dump_rdma_layer is error %d\n", rdma_num);
		return;
	}
	...
	case UFMT_YVYU:
		...
		break;
	default:
		DDP_PR_ERR("dump_rdma_layer(), unknown fmt=%d, dump raw\n",
	...
	if (!raw) {
		...
		if (disp_mva_map_kernel(module, rdma_layer->address,
			...
			DDP_PR_ERR("dump_rdma_layer(),fail to dump rgb(0x%x)\n",
				   rdma_layer->inputFormat);
		}
	} else {
		meta.data_type = MMPROFILE_META_RAW;
		...
		if (disp_mva_map_kernel(module, rdma_layer->address, meta.size,
			...
			DDP_PR_ERR("dprec_mmp_dump_rdma_layer(),fail to dump raw(0x%x)\n",
				   rdma_layer->inputFormat);
		}
	}
}
