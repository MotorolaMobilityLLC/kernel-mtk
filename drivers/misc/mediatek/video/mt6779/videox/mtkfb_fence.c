				 ...
				 unsigned int idx)
{
	...
	if (!mva) {
		...
		DISP_PR_ERR("cannot find session(0x%x) buf, layer=%d, idx=%d, fence_idx=%d, timeline_idx=%d, cur_idx=%d!\n",
			...
			unsigned int idx, unsigned int ticket)
{
	...
	if (layer_id >= DISP_SESSION_TIMELINE_COUNT) {
		DISP_PR_ERR("mtkfb_update_buf_state return MVA=0x0 mtkfb_query_buf_mva layer_id %d !(Warning)\n",
			    layer_id);
		return mva;
	}
...
*disp_sync_prepare_buf(struct disp_buffer_info *disp_buf)
{
	...
	if (!l_info->inited) {
		DISP_PR_ERR("FATAL ERROR, sync info not inited, session_id=0x%08x|layer_id=%d\n",
			    session, timeline_id);
		return NULL;
	}
		...
		list_for_each_entry(buf, &l_info->buf_list, list) {
			...
			if ((buf->mva + buf->mva_offset) == phy_addr) {
				if (phy_addr >= buf->mva &&
					...
					DISP_PR_ERR("wrong addr:pa0x%08lx,mva0x%08lx,sz0x%08x\n",
						    phy_addr, buf->mva,
						    buf->size);
				      ...
				      unsigned int idx)
{
	...
	mutex_unlock(&l_info->sync_lock);
	if (!found) {
		DISP_PR_ERR("cannot find buf for cache sync, session:%s%d, layer=%d,\n",
			    disp_session_type_str(session_id),
			    DISP_SESSION_DEV(session_id), timeline_id);
