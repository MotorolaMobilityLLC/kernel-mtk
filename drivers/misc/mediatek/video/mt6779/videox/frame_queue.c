...
static int disp_dump_fence_info(struct sync_fence *fence, int is_err)
{
	...
	_DISP_PRINT_FENCE_OR_ERR(is_err, "fence[%p] %s: stat=%d ==>\n", fence,
				 fence->name, atomic_read(&fence->status));
			  ...
			  unsigned int present_idx)
{
	...
	if (ret == -ETIME) {
		DISP_PR_ERR("== display fence wait timeout for 1000ms. ret%d,layer%d,fd%d,idx%d ==>\n",
			    ret, timeline, fence_fd, buf_idx);
	} else if (ret != 0) {
		DISP_PR_ERR("== display fence wait status error. ret%d,layer%d,fd%d,idx%d ==>\n",
			    ret, timeline, fence_fd, buf_idx);
	} else {
