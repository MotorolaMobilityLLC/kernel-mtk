...
static unsigned int __maybe_unused cmdqDdpClockOn(uint64_t engineFlag)
{
	return 0;
}

static unsigned int __maybe_unused cmdqDdpResetEng(uint64_t engineFlag)
{
	return 0;
}

static unsigned int __maybe_unused cmdqDdpClockOff(uint64_t engineFlag)
{
	return 0;
}

static unsigned int __maybe_unused cmdqDdpDumpInfo(uint64_t engineFlag,
						   char *pOutBuf,
				      ...
				      struct disp_input_config *src)
{
	...
	if (!src || !dst) {
		DISP_PR_ERR("%s src(0x%p) or dst(0x%p) is null\n",
			    __func__, src, dst);
		return -1;
	}
	...
	if (src->buffer_source == DISP_BUFFER_ALPHA) {
		...
		DISP_PR_ERR("unknown source = %d", src->buffer_source);
...
int ovl2mem_init(unsigned int session)
{
	...
	if (pgc->state > 0) {
		DISP_PR_ERR("path has created, state%d\n", pgc->state);
		goto exit;
	}
...
#ifdef CONFIG_MTK_M4U
	sPort.ePortID = M4U_PORT_DISP_OVL1_2L;
...
int ovl2mem_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (pgc->need_trigger_path == 0) {
		DISPINFO("%s do not trigger\n", __func__);
		if ((atomic_read(&g_trigger_ticket) -
		     atomic_read(&g_release_ticket)) == 1) {
			DISPDBG("%s(0x%08x), configure input, but does not configure output!!\n",
				__func__, pgc->session);

			for (layid = 0;
			     layid < (MEMORY_SESSION_INPUT_LAYER_COUNT + 1);
			     layid++) {
				fence_idx = mtkfb_query_idx_by_ticket(
						pgc->session, layid,
						atomic_read(&g_trigger_ticket));
				if (fence_idx >= 0)
					mtkfb_release_fence(pgc->session,
							    layid, fence_idx);
			}
		}
		return ret;
...
int ovl2mem_frame_cfg(struct disp_frame_cfg_t *cfg)
{
	...
	if (pgc->state == 0) {
		DISP_PR_ERR("ovl2mem is already slept\n");
		_ovl2mem_path_unlock(__func__);
		return 0;
	}
...
int ovl2mem_deinit(void)
{
	...
	/* [SVP]switch ddp module to nonsec when deinit the extension path */
