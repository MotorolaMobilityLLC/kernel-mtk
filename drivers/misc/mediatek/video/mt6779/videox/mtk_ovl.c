...
static void ovl2mem_path_lock(const char *caller)
{
...
static void ovl2mem_path_unlock(const char *caller)
{
...
static int ovl2mem_callback(unsigned int userdata)
{
	...
	/* Because WDMA frame done would be clear when WDMA stop,
	 * print trace log here instead.
	 */
	MMPathTraceSecondOvl2Mem();
...
int ovl2mem_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (pgcl->need_trigger_path == 0) {
		...
		DISPMSG("%s(%x), no config output!!\n",
			__func__, pgcl->session);
		return ret;
	}
...
static int ovl2mem_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		cmdqBackupReadSlot(pgcl->ovl2mem_cur_config_fence, i,
			&ext_last_fence);
...
void ovl2mem_wait_done(void)
{
	...
	while ((atomic_read(&g_trigger_ticket) -
		    atomic_read(&g_release_ticket)) > 1) {
		dpmgr_wait_event_timeout(pgcl->dpmgr_handle,
		DISP_PATH_EVENT_FRAME_COMPLETE, HZ / 30);
...
int ovl2mem_deinit(void)
{
	...
	if (loop_cnt >= 10)
		DISPMSG("%s loop_cnt >= 10, g_trigger_t=%d, g_release_t=%d\n",
