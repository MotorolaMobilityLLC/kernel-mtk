...
int mtkfb_check_bootmode(void)
{
	...
	tag = (struct tag_bootmode *)of_get_property(np_chosen, "atag,boot",
						    NULL);
	if (!tag) {
		DISPDBG("%s: fail to get atag,boot\n", __func__);
	} else {
		DISPDBG("bootmode: 0x%x boottype: 0x%x\n",
...
static int mtkfb_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_POWERDOWN:
        ...
        if (mtkfb_check_bootmode() == 1)
		{
        //printk("prev_pm is %d\n",prev_pm);
            if (prev_pm != FB_SUSPEND) {//ALPS06431259:recovery backlight add
...
/* tinno add for LCD dev_info */
#ifdef CONFIG_TINNO_PRODUCT_INFO
//	tinno_get_product_code(lcd_product_code);
    FULL_PRODUCT_DEVICE_INFO(ID_LCD, mtkfb_lcm_name);
//    FULL_PRODUCT_DEVICE_INFO(ID_PANEL, lcd_product_code);
#endif
	return 0;
