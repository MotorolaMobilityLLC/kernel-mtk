	...
	struct cmdqRecStruct *cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		0x3FFF0080); /* BG_LPF_EN=0 BG_CORE_EN=1 */
		mdelay(1);
		MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_LANE_CON,
			0x3FFF00C0); /* BG_LPF_EN=1 */
	...
	struct cmdqRecStruct *cmdq, int on)
{
	...
	i <= DSI_MODULE_END(module); i++) {
		...
		/* step 2 */
		MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_LANE_CON,
		0x3FFF0080); /* BG_LPF_EN=0 */
		MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_LANE_CON,
			0x3FFF0000); /* BG_CORE_EN=1 */
	...
	UINT8 buffer_size)
{
	...
	do {
		...
		if (packet_type == 0x1A || packet_type == 0x1C) {
			...
			if (recv_data_cnt > 10) {
				DISPCHECK(
					"DSI read long packet data exceeds 4 bytes,size:%d\n",
					recv_data_cnt);
				recv_data_cnt = 10;
			}
	...
	unsigned int enable)
{
	if (module == DISP_MODULE_DSI0)
		DSI_OUTREGBIT(handle, struct DSI_INT_ENABLE_REG,
			DSI_REG[0]->DSI_INTEN, TE_RDY, enable);
	...
	void *cmdq_trigger_handle, enum CMDQ_STATE state)
{
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < 3; i++) {
			...
			DISPDBG("[DSI]para_list[0]=0x%x,para_list[1]=0x%x\n",
