...
static atomic_t roi_dirty = ATOMIC_INIT(0);
...
int primary_display_is_directlink_mode(void)
{
	...
	if (mode == DISP_SESSION_DIRECT_LINK_MIRROR_MODE || mode == DISP_SESSION_DIRECT_LINK_MODE ||
		mode == DISP_SESSION_DUAL_DIRECT_LINK_MODE)
		return 1;
	else
		return 0;
}
...
int primary_display_is_rdma_mode(void)
{
	enum DISP_MODE mode = pgc->session_mode;

	if (mode == DISP_SESSION_DUAL_RDMA_MODE || mode == DISP_SESSION_RDMA_MODE)
		return 1;
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		/* if (_need_lfr_check()) */
		/* ret = disp_cmdq_wait_event(pgc->cmdq_handle_trigger, CMDQ_EVENT_DISP_DSI0_EOF); */
		...
		if (need_wait_esd_eof()) {
			...
			ret = disp_cmdq_wait_event_no_clear(pgc->cmdq_handle_trigger, CMDQ_SYNC_TOKEN_ESD_EOF);
		}
		/* ret = disp_cmdq_wait_event(pgc->cmdq_handle_trigger, CMDQ_EVENT_MDP_DSI0_TE_SOF); */
...
#endif
		ret = disp_cmdq_wait_event_no_clear(pgc->cmdq_handle_trigger, CMDQ_SYNC_TOKEN_CABC_EOF);
		...
		ret = disp_cmdq_clear_event(pgc->cmdq_handle_trigger, CMDQ_SYNC_TOKEN_STREAM_EOF);

		ret = disp_cmdq_clear_event(pgc->cmdq_handle_trigger, CMDQ_SYNC_TOKEN_CONFIG_DIRTY);

		/* clear dsi EOF token before wait */
		ret = disp_cmdq_clear_event(pgc->cmdq_handle_trigger, CMDQ_EVENT_DISP_DSI0_EOF);
 ...
 */
		...
		/* polling DSI idle */
		/* ret = disp_cmdq_poll_reg(pgc->cmdq_handle_trigger, 0x1401b00c, 0, 0x80000000); */
		/* polling wdma frame done */
		/* ret = disp_cmdq_poll_reg(pgc->cmdq_handle_trigger, 0x140060A0, 1, 0x1); */

		/* now frame done, config thread is allowed to config register now */
		ret = disp_cmdq_set_event(pgc->cmdq_handle_trigger, CMDQ_SYNC_TOKEN_STREAM_EOF);
		ret = disp_cmdq_set_event(pgc->cmdq_handle_trigger, CMDQ_SYNC_TOKEN_CABC_EOF);
		/* RUN forever!!!! */
		if (ret < 0)
...
static void _cmdq_build_monitor_loop(void)
{
	...
	ret = disp_cmdq_wait_event(g_cmdq_handle_monitor, CMDQ_EVENT_DISP_RDMA0_UNDERRUN);
	...
	ret = disp_cmdq_start_loop(g_cmdq_handle_monitor);
}
#endif
...
void _cmdq_start_trigger_loop(void)
{
	...
	/* disp_cmdq_dump_command(pgc->cmdq_handle_trigger);*/
...
static void _cmdq_set_config_handle_dirty(void)
{
	if (!primary_display_is_video_mode()) {
		...
		disp_cmdq_set_event(pgc->cmdq_handle_config, CMDQ_SYNC_TOKEN_CONFIG_DIRTY);
...
static void _cmdq_handle_clear_dirty(struct cmdqRecStruct *cmdq_handle)
{
	if (!primary_display_is_video_mode()) {
		...
		disp_cmdq_clear_event(cmdq_handle, CMDQ_SYNC_TOKEN_CONFIG_DIRTY);
	}
}
...
{
	if (primary_display_is_video_mode()) {
		/*disp_cmdq_wait_event_no_clear(handle, CMDQ_EVENT_DISP_RDMA0_EOF);*/
		disp_cmdq_wait_event_no_clear(handle, CMDQ_EVENT_MUTEX0_STREAM_EOF);
			       ...
			       disp_path_handle disp_handle, struct cmdqRecStruct *cmdq_handle)
{
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		...
		DISPINFO("%s:addr:0x%lx,src(%ux%u),clip(%u,%u,%ux%u),fmt:%s,p:%u\n",
		       ...
		       pconfig->wdma_config.clipWidth, pconfig->wdma_config.clipHeight,
				   ...
				   disp_path_handle disp_handle, struct cmdqRecStruct *cmdq_handle)
{
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		DISPINFO("%s:addr:0x%lx,src(%u,%u,%ux%u),dst(%u,%u,%ux%u),bg(%ux%u),fmt:%s,p:%u\n",
		       __func__, pconfig->rdma_config.address,
		       pconfig->rdma_config.src_x, pconfig->rdma_config.src_y,
		       pconfig->rdma_config.src_w, pconfig->rdma_config.src_h,
		       pconfig->rdma_config.dst_x, pconfig->rdma_config.dst_y,
		       pconfig->rdma_config.width, pconfig->rdma_config.height,
		       pconfig->rdma_config.dst_w, pconfig->rdma_config.dst_h,
		       unified_color_fmt_name(pconfig->rdma_config.inputFormat),
...
int _DL_switch_to_DL_dual_fast(struct cmdqRecStruct *handle, int block)
{
	...
	dpmgr_path_ioctl(pgc->dpmgr_handle, handle, DDP_RDMA_GOLDEN_SETTING, data_config_dl);
...
int _DL_dual_switch_to_DL_fast(struct cmdqRecStruct *handle, int block)
{
	...
	disp_cmdq_write_slot(handle, pgc->rdma_buff_info, 0, 0);
...
int _DL_dual_switch_to_DC_fast(void)
{
	int ret = 0;
	struct disp_ddp_path_config *data_config_dl = NULL;
	...
	dpmgr_path_ioctl(pgc->dpmgr_handle, pgc->cmdq_handle_config, DDP_RDMA_GOLDEN_SETTING, data_config_dl);
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		disp_cmdq_write_slot(pgc->cmdq_handle_config, pgc->rdma_buff_info,
				     0, data_config_dl->rdma_config.address);
		disp_cmdq_write_slot(pgc->cmdq_handle_config, pgc->rdma_buff_info,
				     1, data_config_dl->rdma_config.pitch);
		disp_cmdq_write_slot(pgc->cmdq_handle_config, pgc->rdma_buff_info,
				     2, data_config_dl->rdma_config.inputFormat);
		disp_cmdq_write_slot(pgc->cmdq_handle_config, pgc->rdma_buff_info,
				     3, data_config_dl->rdma_config.src_w);
		disp_cmdq_write_slot(pgc->cmdq_handle_config, pgc->rdma_buff_info,
				     4, data_config_dl->rdma_config.src_h);

		disp_cmdq_write_slot(pgc->cmdq_handle_config, pgc->hrt_info,
				     0, data_config_dl->hrt_path);
		disp_cmdq_write_slot(pgc->cmdq_handle_config, pgc->hrt_info,
				     1, data_config_dl->hrt_scale);
	} else {
...
int DC_switch_to_DL_dual(void)
{
	_DC_switch_to_DL_fast();
	_DL_switch_to_DL_dual_fast(NULL, 1);
	return 0;
}

int DL_dual_switch_to_DC_dual(void)
{
	int ret = 0;
	enum DDP_SCENARIO_ENUM old_scenario, new_scenario;
	struct RDMA_CONFIG_STRUCT rdma_config = decouple_rdma_config;
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		...
		data_config_dc->ovl_pma_enable = data_config_dl->ovl_pma_enable;
...
int DC_dual_switch_to_DL_dual(void)
{
	int ret = 0;
	int layer = 0;
	struct disp_ddp_path_config *data_config_dl = NULL;
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		...
		data_config_dl->ovl_pma_enable = data_config_dc->ovl_pma_enable;
	}
	...
	disp_cmdq_clear_event(pgc->cmdq_handle_ovl1to2_config, CMDQ_EVENT_DISP_WDMA0_SOF);
	...
	disp_cmdq_write_slot(pgc->cmdq_handle_config, pgc->rdma_buff_info, 0, 0);
...
static int DL_switch_to_rdma_mode(struct cmdqRecStruct *handle, int block)
{
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		...
		pconfig->hrt_path = disp_rsz_map_dual_to_single(pconfig->hrt_path);
		new_scenario = primary_get_RDMA_scenario(pconfig);
...
static int rdma_mode_switch_to_DL(struct cmdqRecStruct *handle, int block)
{
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		...
		pconfig->hrt_path = disp_rsz_map_dual_to_single(pconfig->hrt_path);
		new_scenario = primary_get_DL_scenario(pconfig);
		pconfig->is_dual = ddp_path_is_dual(new_scenario);
...
static int DL_dual_switch_to_dual_rdma_mode(struct cmdqRecStruct *handle, int block)
{
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		...
		pconfig->hrt_path = disp_rsz_map_single_to_dual(pconfig->hrt_path);
		new_scenario = primary_get_RDMA_scenario(pconfig);
		pconfig->is_dual = ddp_path_is_dual(new_scenario);
...
{
	int ret;
	enum DDP_SCENARIO_ENUM old_scenario = DDP_SCENARIO_MAX, new_scenario = DDP_SCENARIO_MAX;
...
static int config_display_m4u_port(void)
{
	...
	if (ret) {
		DISPERR("config M4U Port %s to %s FAIL(ret=%d)\n",
			  ddp_get_module_name(DISP_MODULE_OVL1), m4u_usage, ret);
		return -1;
	}
				  ...
				  CmdqAsyncFlushCB callback, unsigned int data)
{
	...
	disp_cmdq_wait_event_no_clear(cmdq_handle, CMDQ_EVENT_DISP_WDMA0_EOF);
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		...
		disp_cmdq_write_slot(cmdq_handle, pgc->rdma_buff_info, 3, mem_config.w);
		disp_cmdq_write_slot(cmdq_handle, pgc->rdma_buff_info, 4, mem_config.h);
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER) &&
		...
		disp_cmdq_wait_event(cmdq_handle, CMDQ_EVENT_DISP_WDMA0_EOF);
	}
...
static int _decouple_update_rdma_config_nolock(void)
{
	...
	if (primary_display_is_decouple_mode()) {
		...
		if (disp_helper_get_option(DISP_OPT_RSZ)) {
			cmdqBackupReadSlot(pgc->rdma_buff_info, 3, &tmpConfig.src_w);
			cmdqBackupReadSlot(pgc->rdma_buff_info, 4, &tmpConfig.src_h);
		}
...
int _display_set_lcm_refresh_rate(int fps)
{
	...
	disp_cmdq_reset(cmdq_handle);
	_cmdq_insert_wait_frame_done_token_mira(cmdq_handle);
	ret = disp_cmdq_clear_event(cmdq_handle, CMDQ_EVENT_DSI_TE);
	ret = disp_cmdq_wait_event(cmdq_handle, CMDQ_EVENT_DSI_TE);
...
int primary_display_suspend(void)
{
	...
	if (disp_helper_get_option(DISP_OPT_USE_CMDQ)) {
		DISPINFO("[POWER]display cmdq trigger loop stop\n");
		_cmdq_stop_trigger_loop();
	}
	mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_PULSE, 0, 3);

	DISPINFO("[POWER]primary display path stop[begin]\n");
	dpmgr_path_stop(pgc->dpmgr_handle, CMDQ_DISABLE);
	DISPINFO("[POWER]primary display path stop[end]\n");
	...
	if (primary_display_get_power_mode_nolock() == DOZE_SUSPEND) {
		...
		DISPINFO("[POWER]lcm suspend[end]\n");
		DISP_SYSTRACE_END();
	...
	DISPINFO("[POWER]dpmanager path power off[begin]\n");
	dpmgr_path_power_off(pgc->dpmgr_handle, CMDQ_DISABLE);
	if (disp_helper_get_option(DISP_OPT_MET_LOG))
		set_enterulps(1);

	DISPINFO("[POWER]dpmanager path power off[end]\n");
...
int primary_display_resume(void)
{
	...
	if (is_ipoh_bootup) {
		DISPINFO("[primary display path] leave primary_display_resume -- IPOH\n");
		DISPINFO("ESD check start[begin]\n");
		primary_display_esd_check_enable(1);
		DISPINFO("ESD check start[end]\n");
		is_ipoh_bootup = false;
		DISPINFO("[POWER]start cmdq[begin]--IPOH\n");
		if (disp_helper_get_option(DISP_OPT_USE_CMDQ))
			_cmdq_start_trigger_loop();
		enable_idlemgr(1);
		DISPINFO("[POWER]start cmdq[end]--IPOH\n");
		/* pgc->state = DISP_ALIVE; */
		goto done;
	}
	...
	DISPINFO("dpmanager path power on[begin]\n");
	dpmgr_path_power_on(pgc->dpmgr_handle, CMDQ_DISABLE);
	if (disp_helper_get_option(DISP_OPT_MET_LOG))
		set_enterulps(0);

	DISPINFO("dpmanager path power on[end]\n");
	...
	DISPINFO("[POWER]dpmanager re-init[begin]\n");
	...
	DISPINFO("[POWER]dpmanager re-init[end]\n");
	...
	if (primary_display_get_power_mode_nolock() == DOZE) {
		...
		if (primary_display_get_lcm_power_state_nolock() != LCM_ON) {
			DISPINFO("[POWER]lcm resume[begin]\n");
			if (primary_display_get_lcm_power_state_nolock() != LCM_ON_LOW_POWER) {
				disp_lcm_resume(pgc->plcm);
			} else {
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE ||
		...
		if (pgc->need_trigger_ovl1to2 == 0) {
			...
			pgc->need_trigger_ovl1to2 = 0;
				...
				pgc->session_mode == DISP_SESSION_DUAL_DECOUPLE_MODE) {
			     ...
			     disp_path_handle disp_handle, struct cmdqRecStruct *cmdq_handle)
{
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		if (cfg->overlap_layer_num) {
			if (data_config->hrt_scale != HRT_GET_SCALE_SCENARIO(cfg->overlap_layer_num) ||
				...
				data_config->ovl_pma_enable =
					HRT_is_pma_enabled(data_config->hrt_path);
	...
	hrt_path = HRT_GET_PATH_SCENARIO(cfg->overlap_layer_num);
	...
	if (disp_helper_get_option(DISP_OPT_RSZ)) {
		if (data_config->dst_dirty) {
			...
			else if (primary_display_is_rdma_mode())
				new_scn = primary_get_RDMA_scenario(data_config);
...
#ifdef DEBUG_OVL_CONFIG_TIME
	disp_cmdq_read_reg_to_slot(cmdq_handle, pgc->ovl_config_time, 1, 0x10008028);
#endif
	if (!cmdq_handle)
		goto done;
...
static int primary_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	if (primary_display_is_decouple_mode() && !primary_display_is_mirror_mode()) {
		...
		if (disp_helper_get_option(DISP_OPT_RSZ)) {
			mem_config.w = wconfig->clipWidth;
			mem_config.h = wconfig->clipHeight;
			mem_config.pitch = wconfig->dstPitch;
		}
		mmprofile_log_ex(ddp_mmp_get_events()->primary_wdma_config, MMPROFILE_FLAG_PULSE,
					...
					struct cmdqRecStruct *handle, int block)
{
	...
	DISPINFO("%s: require sess_mode %s(from:%s)\n",
		__func__, session_mode_spy(sess_mode), session_mode_spy(pgc->session_mode));
	if (need_lock)
		_primary_path_lock(__func__);
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE
		...
		DL_switch_to_DC_fast(0);
			...
			sess_mode == DISP_SESSION_DUAL_DIRECT_LINK_MODE) {
		ret = _DL_switch_to_DL_dual_fast(NULL, 1);
		if (ret)
			goto err;
	} else if (pgc->session_mode == DISP_SESSION_DUAL_DIRECT_LINK_MODE &&
			sess_mode == DISP_SESSION_DIRECT_LINK_MODE) {
		...
		if (ret)
			...
			sess_mode == DISP_SESSION_DUAL_DECOUPLE_MODE) {
		ret = DL_dual_switch_to_DC_dual();
		if (ret)
			goto err;
	} else if (pgc->session_mode == DISP_SESSION_DUAL_DECOUPLE_MODE &&
			sess_mode == DISP_SESSION_DUAL_DIRECT_LINK_MODE) {
...
int _set_backlight_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		...
		_cmdq_insert_wait_frame_done_token_mira(cmdq_handle_backlight);
		disp_lcm_set_backlight(pgc->plcm, cmdq_handle_backlight, level);
		disp_cmdq_set_event(cmdq_handle_backlight, CMDQ_SYNC_TOKEN_CONFIG_DIRTY);
		disp_cmdq_set_event(cmdq_handle_backlight, CMDQ_SYNC_TOKEN_CABC_EOF);
...
int _set_lcm_cmd_by_cmdq(unsigned int *lcm_cmd, unsigned int *lcm_count, unsigned int *lcm_value)
{
	...
	if (primary_display_is_video_mode()) {
		...
		disp_lcm_set_lcm_cmd(pgc->plcm, cmdq_handle_lcm_cmd, lcm_cmd, lcm_count, lcm_value);
		disp_cmdq_set_event(cmdq_handle_lcm_cmd, CMDQ_SYNC_TOKEN_CONFIG_DIRTY);
...
int primary_display_mipi_clk_change(unsigned int clk_value)
{
	...
	disp_cmdq_clear_event(cmdq_handle, CMDQ_EVENT_MUTEX0_STREAM_EOF);
