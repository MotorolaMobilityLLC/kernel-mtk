...
static unsigned long rsz_base_addr(enum DISP_MODULE_ENUM module)
{
	...
	case DISP_MODULE_RSZ0:
		return DDP_REG_BASE_DISP_RSZ0;
	case DISP_MODULE_RSZ1:
		return DDP_REG_BASE_DISP_RSZ1;
	default:
...
static int rsz_config(enum DISP_MODULE_ENUM module, struct disp_ddp_path_config *pconfig, void *qhandle)
{
	...
	if (pconfig->is_dual) {
		if (module == DISP_MODULE_RSZ0)
			tile_idx = 0;
		else if (module == DISP_MODULE_RSZ1)
			tile_idx = 1;
	}

	in_w = pconfig->rsz_config.tw[tile_idx].in_len;
	in_h = pconfig->rsz_config.th.in_len;
	out_w = pconfig->rsz_config.tw[tile_idx].out_len;
	out_h = pconfig->rsz_config.th.out_len;
...
void rsz_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("power_saving:%d,rgb_bit_mode:%d,frm_start:%d,frm_end:%d,size_err:%d,sof_rst:%d\n",
		REG_FLD_VAL_GET(FLD_RSZ_POWER_SAVING, con2), REG_FLD_VAL_GET(FLD_RSZ_RGB_BIT_MODE, con2),
