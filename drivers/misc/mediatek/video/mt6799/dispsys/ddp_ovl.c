...
static u32 ovl_bg_w[2][2], ovl_bg_h[2][2];
...
static inline unsigned long ovl_to_m4u_port(enum DISP_MODULE_ENUM module)
{
	...
	case DISP_MODULE_OVL0_2L:
		return M4U_PORT_DISP_2L_OVL0_LARB1;
	case DISP_MODULE_OVL1_2L:
...
int ovl_start(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(handle, ovl_base + DISP_REG_OVL_INTEN,
		     0x1E | REG_FLD_VAL(INTEN_FLD_ABNORMAL_SOF, 1) | REG_FLD_VAL(INTEN_FLD_OVL_START_INTEN, 1));
...
int disable_ovl_layers(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DDPMSG("[SVP] switch ovl%d to nonsec: disable all the layers first!\n", ovl_idx);
	return 0;
}
...
static void _store_roi(enum DISP_MODULE_ENUM module, struct disp_ddp_path_config *pconfig)
{
	switch (module) {
	case DISP_MODULE_OVL0:
		ovl_bg_w[0][0] = pconfig->dst_w;
		ovl_bg_h[0][0] = pconfig->dst_h;
		break;
	case DISP_MODULE_OVL0_2L:
		ovl_bg_w[0][1] = pconfig->dst_w;
		ovl_bg_h[0][1] = pconfig->dst_h;
		break;
	case DISP_MODULE_OVL1:
		ovl_bg_w[1][0] = pconfig->dst_w;
		ovl_bg_h[1][0] = pconfig->dst_h;
		break;
	case DISP_MODULE_OVL1_2L:
		ovl_bg_w[1][1] = pconfig->dst_w;
		ovl_bg_h[1][1] = pconfig->dst_h;
		break;
	default:
		break;
	}
...
static void _get_roi(enum DISP_MODULE_ENUM module, u32 *bg_w, u32 *bg_h)
{
	switch (module) {
	case DISP_MODULE_OVL0:
		*bg_w = ovl_bg_w[0][0];
