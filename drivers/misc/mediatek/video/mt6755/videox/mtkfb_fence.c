...
do { \
	if (mtkfb_fence_on) \
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "fence", fmt, ##arg); \
} while (0)
			       ...
			       disp_input_config *buf_info)
{
	...
	DISPMSG("mva updaten:session_id=0x%08x, layer_id=%d, %x, mva=0x%lx-%x\n", session_id,
		buf_info->layer_id, buf_info->next_buff_idx, buf->mva, buf_info->layer_type);
				      ...
				      unsigned int idx, unsigned long *mva, unsigned int *size)
{
	...
	if (dst_mva != 0x0) {
		...
		*size = dst_size;
		...
		/* FIXME: non-ion buffer need cache sync here? */
		DISPERR
		    ("cannot find buf,session:%s%d,layer=%d,idx=%d,fence_idx=%d,timeline_idx=%d,cur_idx=%d!\n",
		     disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id), timeline_id,
