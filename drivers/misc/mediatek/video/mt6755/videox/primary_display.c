...
#endif
#define mt_gpio_set_debounce(eint_num, ms) (void)0
...
static int do_primary_display_switch_mode(int sess_mode, unsigned int session, int need_lock,
				...
				unsigned int x_pos, unsigned int block_sz)
{
	...
	tmp = map_va + (unsigned long)(mva - real_mva);
	buf_va = (unsigned char *)tmp;
...
int primary_display_dsi_vfp_change(int state)
{
	...
	if (state == 1)
		dpmgr_path_ioctl(pgc->dpmgr_handle, handle,
...
void primary_display_idlemgr_leave_idle(int need_lock)
{
	...
	if (primary_display_is_video_mode() && disp_helper_get_option(DISP_OPT_IDLEMGR_SWTCH_DECOUPLE)) {
		...
		if (disp_helper_get_option(DISP_OPT_IDLEMGR_SWTCH_DECOUPLE) &&
			...
			do_primary_display_switch_mode(session_mode_before_enter_idle, pgc->session_id,
						      need_lock, NULL, 0);
		}
...
#if 0
		/* //use separate api for sodi control flow later */
...
check_again:
		...
		if (recovery_done == 1) {
			if (!ret) {
				...
				DISPCHECK("[ESD]after esd recovery %d times, still fail check\n",
						esd_try_cnt);
...
static int32_t _Interface_fence_release_callback(unsigned long userdata)
{
	int layer = disp_sync_get_output_interface_timeline_id();

	if (userdata > 0) {
		mtkfb_release_fence(primary_session_id, layer, (int)userdata);
		MMProfileLogEx(ddp_mmp_get_events()->primary_wdma_fence_release, MMProfileFlagPulse,
			       layer, (unsigned int)userdata);
	}
	return 0;
}
...
static int _decouple_update_rdma_config_nolock(void)
{
	...
	if (primary_display_is_decouple_mode()) {
		...
		if (ret == 0) {
			...
			cmdqRecReset(cmdq_handle);
			_cmdq_insert_wait_frame_done_token_mira(cmdq_handle);
			cmdqBackupReadSlot(pgc->rdma_buff_info, 0, (unsigned int *)&(tmpConfig.address));
...
unsigned long get_dim_layer_mva_addr(void)
{
	...
	if (dim_layer_mva == 0) {
		...
		DISPMSG
		    ("init dim layer va_start=0x%lx, va_end=0x%lx,size %d,fb_va start=0x%lx, end=0x%lx\n",
...
int primary_display_user_cmd(unsigned int cmd, unsigned long arg)
{
	...
	MMProfileLogEx(ddp_mmp_get_events()->primary_display_cmd, MMProfileFlagStart, cmd, 0);
	...
	MMProfileLogEx(ddp_mmp_get_events()->primary_display_cmd, MMProfileFlagEnd, cmd,
		       cmdqsize);
...
int primary_display_get_virtual_width(void)
{
	...
	if (pgc->plcm->params) {
		ret = pgc->plcm->params->virtual_width;
	} else {
		DISPERR("lcm_params is null!\n");
		ret = 0;
	}
...
int primary_display_get_virtual_height(void)
{
	...
	if (pgc->plcm->params) {
		ret = pgc->plcm->params->virtual_height;
	} else {
		DISPERR("lcm_params is null!\n");
		ret = 0;
	}
...
int primary_display_get_original_width(void)
{
	...
	if (pgc->plcm->params) {
		ret = pgc->plcm->lcm_original_width;
	} else {
		DISPERR("lcm_params is null!\n");
		ret = 0;
	}
...
int primary_display_get_original_height(void)
{
	...
	if (pgc->plcm->params) {
		ret = pgc->plcm->lcm_original_height;
	} else {
		DISPERR("lcm_params is null!\n");
		ret = 0;
	}
...
uint32_t DISP_GetActiveHeight(void)
{
	...
	if (pgc->plcm->params) {
		ret = pgc->plcm->params->physical_height;
	} else {
		DISPERR("lcm_params is null!\n");
		ret = 0;
	}
...
uint32_t DISP_GetActiveWidth(void)
{
	...
	if (pgc->plcm->params) {
		ret = pgc->plcm->params->physical_width;
	} else {
		DISPERR("lcm_params is null!\n");
		ret = 0;
	}
...
static int _screen_cap_by_cmdq(unsigned int mva, enum UNIFIED_COLOR_FMT ufmt, DISP_MODULE_ENUM after_eng)
{
	...
	buffer_size = h_yres * w_xres * pixel_byte;
	/*create config thread */
	ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &cmdq_handle);
	if (ret != 0) {
