...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	if (!strcmp(mtkfb_find_lcm_driver(),"icnl9916_hdp_dsi_vdo_tm")) {
		register_ccci_sys_call_back(MD_SYS1,
			MD_DISPLAY_DYNAMIC_MIPI, mipi_clk_change);
	}

        if (!strcmp(mtkfb_find_lcm_driver(),"ili9883a_hdp_dsi_vdo_txd")) {
		register_ccci_sys_call_back(MD_SYS1,
			MD_DISPLAY_DYNAMIC_MIPI, mipi_clk_change);
	}

        if (!strcmp(mtkfb_find_lcm_driver(),"td4160_hdp_dsi_vdo_boe")) {
		register_ccci_sys_call_back(MD_SYS1,
		        MD_DISPLAY_DYNAMIC_MIPI, mipi_clk_change);
	}

	if (!strcmp(mtkfb_find_lcm_driver(),"ili7807s_hdp_dsi_vdo_tm")) {
		register_ccci_sys_call_back(MD_SYS1,
			MD_DISPLAY_DYNAMIC_MIPI, mipi_clk_change);
	}
