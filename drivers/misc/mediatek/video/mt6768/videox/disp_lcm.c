...
#include <linux/of.h>
#endif
#define BRIGHETNESS	2040

char lcm_name[64] = {0};
int g_backlight_level = BRIGHETNESS;
unsigned int disp_lcm_get_id(void);
	...
	if (_lcm_count() == 0) {
		...
		if (plcm_name == NULL) {
			...
			for (i = 0; i < _lcm_count(); i++) {
				...
				if (!strcmp(lcm_drv->name, plcm_name)) {
					strncpy(lcm_name, lcm_drv->name, (sizeof(lcm_name)-1));
					isLCMFound = true;
					isLCMInited = true;
					lcmindex = i;
					break;
...
int disp_lcm_esd_recover(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->esd_recover) {
			...
			disp_lcm_set_backlight(plcm, NULL, g_backlight_level);
		}
...
unsigned int disp_lcm_get_id(void)
{
	...
	if (!strcmp(lcm_name, "ili9883a_hdp_dsi_vdo_txd")) {
		lcd_id = 0x05;
		return lcd_id;
	}

	if (!strcmp(lcm_name, "td4160_hdp_dsi_vdo_boe")) {
		lcd_id = 0x0D;
		return lcd_id;
	}

	if (!strcmp(lcm_name, "icnl9916_hdp_dsi_vdo_tm")) {
		lcd_id = 0x0A;
		return lcd_id;
	}

	if (!strcmp(lcm_name, "ili7807s_hdp_dsi_vdo_tm")) {
		lcd_id = 0x07;
		return lcd_id;
	}
