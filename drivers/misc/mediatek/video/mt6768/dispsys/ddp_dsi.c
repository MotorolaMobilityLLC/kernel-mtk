...
int mipi_clk_change(int msg, int en)
{
	...
	if (en) {
		if (!strcmp(mtkfb_lcm_name,"ili9883a_hdp_dsi_vdo_txd")) {
			def_data_rate = 884;
			if (dsi_params->mode != CMD_MODE) {
				def_dsi_hbp = 4; /* adaptive HBP value */
			}
		} else if (!strcmp(mtkfb_lcm_name,"icnl9916_hdp_dsi_vdo_tm")) {
			def_data_rate = 884;
			if (dsi_params->mode != CMD_MODE) {
				def_dsi_hbp = 68; /* adaptive HBP value */
			}
		} else if (!strcmp(mtkfb_lcm_name,"td4160_hdp_dsi_vdo_boe")) {
			def_data_rate = 884;
			if (dsi_params->mode != CMD_MODE) {
				def_dsi_hbp = 120; /* adaptive HBP value */
			}
		} else if (!strcmp(mtkfb_lcm_name,"ili7807s_hdp_dsi_vdo_tm")) {
			def_data_rate = 884;
			if (dsi_params->mode != CMD_MODE) {
				def_dsi_hbp = 250;
			}
	...
	if (_is_power_on_status(DISP_MODULE_DSI0)) {
		...
		ddp_dsi_porch_setting(DISP_MODULE_DSI0, handle, DSI_HBP, def_dsi_hbp);
