...
inline enum MFC_STATUS DAL_CHECK_MFC_RET(enum MFC_STATUS expr)
{
	...
	if (ret != MFC_STATUS_OK) {
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DAL",
			       "Warning: call MFC_XXX function failed in %s(), "
			       "line: %d, ret: %d\n",
...
inline enum DISP_STATUS DAL_CHECK_DISP_RET(enum DISP_STATUS expr)
{
	...
	if (ret != DISP_STATUS_OK) {
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DAL",
			       "Warning: call DISP_XXX function failed in "
			       "%s(), line: %d, ret: %d\n",
			       __func__, __LINE__, ret);
		return ret;
	}
...
enum DAL_STATUS DAL_Clean(void)
{
	...
	if (isAEEEnabled == 1) {
		...
		DAL_Dynamic_Change_FB_Layer(
		    isAEEEnabled); /* restore UI layer to DEFAULT_UI_LAYER */
	}
...
enum DAL_STATUS DAL_Printf(const char *fmt, ...)
{
	...
	DAL_LOCK();
	if (isAEEEnabled == 0) {
		pr_debug("[DDP] isAEEEnabled from 0 to 1, ASSERT_LAYER=%d, "
			 "dal_fb_pa 0x%lx\n",
			 primary_display_get_option("ASSERT_LAYER"), dal_fb_pa);

		isAEEEnabled = 1;
		DAL_Dynamic_Change_FB_Layer(isAEEEnabled); /* default_ui_layer
							      config to
							      changed_ui_layer
							      */
