...
#define CHECK_RET(expr)                                                        \
	do {                                                                   \
		int ret = (expr);                                              \
		aee_kernel_exception("mtkfb", "[DISP]error:%s,%d", __FILE__,   \
				     __LINE__);                                \
	} while (0)
			   ...
			   u_int transp, struct fb_info *info)
{
	...
	case 32:
		/* ARGB8888 */
		((u32 *)(info->pseudo_palette))[regno] =
		    (0xff000000) | ((red & 0xFF00) << 8) | ((green & 0xFF00)) |
		    ((blue & 0xFF00) >> 8);
		break;
...
static int mtkfb_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		DISPDBG("mtkfb_ blank mtkfb _late_resume\n");
		if (bypass_blank) {
	...
	case FB_BLANK_POWERDOWN:
		DISPDBG("mtkfb_ blank mtkfb_ early_suspend\n");
		if (bypass_blank) {
...
{
	MTKFB_FUNC();
	DISPDBG("mtkfb_ set _backlight_level:%d Start\n", level);
	primary_display_setbacklight(level);
	DISPDBG("mtkfb_ set_ backlight_level End\n");
	return 0;
}
					   ...
					   struct disp_input_config *dst)
{
	...
	DISPDBG("_convert_ fb _layer_ to _disp_input, dst->addr=0x%p\n",
		dst->src_phy_addr);
...
unsigned int mtkfb_fm_auto_test(void)
{
	...
	if (r != 0)
		PRNERR("failed to mtkfb _check_ var\n");
	...
	if (r != 0)
		PRNERR("failed to mtkfb _set_ par\n");
#endif
	if (color == 0)
		color = 0xFF00FF00;
		       ...
		       unsigned long arg)
{
	...
	case MTKFB_GET_DISPLAY_IF_INFORMATION: {
		...
		if (displayid == 0) {
			...
			DISPERR("information for displayid: %d is not "
				"available now\n",
				displayid);
			return -EFAULT;
		}
	...
	case MTKFB_SET_AOD_POWER_MODE: {
		...
		if (aod_pm == MTKFB_AOD_DOZE_SUSPEND) {
			/*
			 * First DOZE to power on dispsys and LCM(low power
			 * mode);
	...
	case MTKFB_CAPTURE_FRAMEBUFFER: {
		...
		unsigned int fbsize =
		    DISP_GetScreenHeight() * DISP_GetScreenWidth() * pixel_bpp;
	...
	case MTKFB_SLT_AUTO_CAPTURE: {
		...
		if (copy_from_user(&capConfig, (void __user *)arg,
			...
			if (copy_to_user(
				(unsigned long *)capConfig.outputBuffer,
				src_pbuf, fbsize)) {
				MTKFB_LOG(
	...
	case MTKFB_SET_OVERLAY_LAYER: { /* no function */
		...
		if (copy_from_user(layerInfo, (void __user *)arg,
			...
			if (layerInfo->layer_id >= TOTAL_OVL_LAYER_NUM) {
				DISPERR(
				"[FB] error, layer_id invalid=%d, line:%d\n",
			...
			if (r < 0) {
				DISPERR(
			"[FB] error, _convert_fb_layer failed! line:%d\n",
					__LINE__);
				kfree(layerInfo);
				return -EFAULT;
			}
			r |= primary_display_config_input_multiple(
			    &session_input);
	...
	case MTKFB_SET_VIDEO_LAYERS: {
		...
		struct fb_overlay_layer *layerInfo;
		int layerInfo_size =
		    sizeof(struct fb_overlay_layer) * VIDEO_LAYER_COUNT;
		...
		if (copy_from_user(layerInfo, (void __user *)arg,
			...
			for (i = 0; i < VIDEO_LAYER_COUNT; ++i) {
				...
				if (r < 0) {
					DISPERR(
		"[FB] error, _convert_fb_layer_to_disp_input failed!\n");
					kfree(layerInfo);
					return -EFAULT;
				}
			}
	...
	case MTKFB_AEE_LAYER_EXIST: {
		...
		return copy_to_user(argp, &dal_en, sizeof(dal_en)) ? -EFAULT
								   : 0;
	}
	...
	case MTKFB_META_SHOW_BOOTLOGO: {
		...
		input->src_phy_addr =
		    (void *)(unsigned long)(fbdev->fb_pa_base +
					    (ALIGN_TO(MTK_FB_XRES,
						      MTK_FB_ALIGNMENT) *
					     ALIGN_TO(MTK_FB_YRES,
						      MTK_FB_ALIGNMENT) *
					     4));
...
"[FB Driver] mtkfb_compat_ioctl, cmd=0x%08x, cmd nr=0x%08x, cmd size=0x%08x\n",
	...
	case COMPAT_MTKFB_GET_DISPLAY_IF_INFORMATION: {
		...
		if (get_user(displayid, data32)) {
			pr_debug(
			    "COMPAT_MTKFB_GET_DISPLAY_IF_INFORMATION failed\n");
			return -EFAULT;
		}
	...
	case COMPAT_MTKFB_SET_OVERLAY_LAYER: {
		...
		compat_layerInfo =
		    kmalloc(sizeof(*compat_layerInfo), GFP_KERNEL);
		if (!compat_layerInfo)
			return -ENOMEM;
	...
	case COMPAT_MTKFB_SET_VIDEO_LAYERS: {
		struct compat_fb_overlay_layer *compat_layerInfo;
		int compat_layerInfo_size =
		    sizeof(struct compat_fb_overlay_layer) * VIDEO_LAYER_COUNT;
		...
		if (copy_from_user(compat_layerInfo, (void __user *)arg,
			...
			/* mmprofile_logStructure(MTKFB_MMP_Events.SetOverlayLayers,
			 * MMPROFILE_FLAG_END,
			 */
...
static int mtkfb_fbinfo_init(struct fb_info *info)
{
	...
	var.xres_virtual = MTK_FB_XRESV;
	var.yres_virtual = MTK_FB_YRESV;
	pr_debug("mtkfb_fbinfo_init "
	"var.xres=%d,var.yres=%d,var.xres_virtual=%d,var.yres_virtual=%d\n",
		 var.xres, var.yres, var.xres_virtual, var.yres_virtual);
...
{
	void *buffer =
	    info->screen_base + info->var.yoffset * info->fix.line_length;
	int size = info->var.xres_virtual * info->var.yres *
...
static void _mtkfb_draw_point(unsigned int addr, unsigned int x,
			      ...
			      unsigned int color)
{
	...
	for (j = 0; j < h; j++) {
		for (i = 0; i < w; i++)
			mt_reg_sync_writel(
			    color, (start_addr + i * 4 + j * MTK_FB_XRESV * 4));
	}
}
...
int _mtkfb_internal_test(unsigned long va, unsigned int w, unsigned int h)
{
	...
	for (i = 0;
		...
		color = (i & 0x1) * 0xff;
		color += 0xff000000U;
		_mtkfb_draw_block(va, i % (w / _internal_test_block_size) *
...
static int __parse_tag_videolfb_extra(struct device_node *node)
{
	...
	prop =
	    (void *)of_get_property(node, "atag,videolfb-islcm_inited", NULL);
	if (!prop)
		is_lcm_inited = 1;
	else
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	pr_debug("mtkfb_probe name [%s]  = [%s][%p]\n", pdev->name,
		 pdev->dev.init_name, (void *)&pdev->dev);
	...
	if (dts_gpio_state != 0)
		dev_dbg(&pdev->dev, "retrieve GPIO DTS failed.");
	...
	DISPCHECK(
"FB_XRES=%d, FB_YRES=%d, FB_BPP=%d, FB_PAGES=%d, FB_LINE=%d, FB_SIZEV=%d\n",
...
#ifdef FPGA_DEBUG_PAN
	test_task =
	    kthread_create(update_test_kthread, NULL, "update_test_kthread");
	...
	/*this function will get fb_heap base address to ion for management
	 * frame buffer
	 */
...
int mtkfb_ipoh_restore(struct notifier_block *nb, unsigned long val, void *ign)
{
	...
	case PM_HIBERNATION_PREPARE:
		DISPDBG(
		    "[FB Driver] mtkfb_ipoh_restore PM_HIBERNATION_PREPARE\n");
		return NOTIFY_DONE;
