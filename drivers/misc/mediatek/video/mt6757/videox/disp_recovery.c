...
int primary_display_switch_esd_mode(int mode)
{
	...
	if (mode == GPIO_EINT_MODE) {
		...
		if (node) {
			...
			DISPERR("[ESD][%s] can't find DSI_TE eint compatible "
				"node\n",
				__func__);
	}
	...
	while (1) {
		...
		if (esd_check_enable) {
			...
			do {
				...
				if (ret == 1) {
					DISPERR("[ESD]esd check fail, will do "
						"esd recovery. try=%d\n",
						i);
			...
			if (ret == 1) {
				DISPERR("[ESD]after esd recovery %d times, "
...
int primary_display_esd_recovery(void)
{
	...
	if (dpmgr_path_is_busy(primary_get_dpmgr_handle())) {
		DISPERR("[ESD]Fatal error, we didn't trigger display path but "
			"it's already busy\n");
		ret = -1;
		/* goto done; */
	}
