...
enum CMDQ_SWITCH ovl2mem_cmdq_enabled(void) { return ovl2mem_use_cmdq; }
...
void ovl2mem_setlayernum(int layer_num) { ovl2mem_layer_num = layer_num; }
...
int get_ovl2mem_ticket(void) { return atomic_read(&g_trigger_ticket); }
...
int ovl2mem_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (pgc->need_trigger_path == 0) {
		...
		if ((atomic_read(&g_trigger_ticket) -
			...
			DISPMSG("ovl2mem_trigger(%x), configue input, but does "
				"not config output!!\n",
				pgc->session);
			for (layid = 0;
...
static int ovl2mem_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle))
		dpmgr_wait_event_timeout(
		    pgc->dpmgr_handle, DISP_PATH_EVENT_FRAME_COMPLETE, HZ / 5);
...
/* set_overlay will not use fence+ion handle */
	...
	data_config->wdma_config.dstPitch =
	    cfg->output_cfg.pitch *
	    UFMT_GET_Bpp(data_config->wdma_config.outputFormat);
...
int ovl2mem_get_max_layer(void) { return MEMORY_SESSION_INPUT_LAYER_COUNT; }
...
void ovl2mem_wait_done(void)
{
	...
	while ((atomic_read(&g_trigger_ticket) -
		atomic_read(&g_release_ticket)) > 1) {
		dpmgr_wait_event_timeout(
		    pgc->dpmgr_handle, DISP_PATH_EVENT_FRAME_COMPLETE, HZ / 30);
	...
	DISPINFO(
	    "ovl2mem_wait_done loop %d, trigger tick:%d, release tick:%d\n",
...
int ovl2mem_deinit(void)
{
	...
	switch_module_to_nonsec(pgc->dpmgr_handle, NULL, DISP_MODULE_NUM,
				__func__);
