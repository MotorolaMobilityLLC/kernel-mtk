...
int _ioctl_prepare_present_fence(unsigned long arg)
{
	...
	if (copy_from_user(&preset_fence_struct, (void __user *)arg,
	...
	if (copy_to_user(argp, &preset_fence_struct,
...
int _ioctl_prepare_buffer(unsigned long arg, enum PREPARE_FENCE_TYPE type)
{
	...
	if (info.layer_en) {
			...
			if (primary_display_is_decouple_mode() &&
				...
				if (buf2 != NULL) {
					...
					info.interface_fence_fd =
					    MTK_FB_INVALID_FENCE_FD; /* invalid
									fd */
		...
		DISP_ERROR("P+ FAIL /%s%d/l%d/e%d/ion%d/c%d/id%d/ffd%d\n",
			   disp_session_mode_spy(info.session_id),
			   DISP_SESSION_DEV(info.session_id), info.layer_id,
			   info.layer_en, info.ion_fd, info.cache_sync,
			   info.index, info.fence_fd);
				      ...
				      int layer_num)
{
	...
	if (cfg->layer_enable) {
		if ((cfg->src_fmt <= 0) || ((cfg->src_fmt >> 8) == 15) ||
			...
			DISPERR("layer_id=%d,src_fmt=0x%x is invalid color "
				"format\n",
...
static int disp_input_get_dirty_roi(struct disp_frame_cfg_t *cfg)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (copy_from_user(addr, cfg->input_cfg[i].dirty_roi_addr,
				   size)) {
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (cfg->input_cfg[i].layer_enable) {
			...
			if (cfg->input_cfg[i].src_phy_addr) {
				dst_mva = (unsigned long)(cfg->input_cfg[i]
							      .src_phy_addr);
			} else {
				disp_sync_query_buf_info_nosync(
				    session_id, layer_id,
				    (unsigned int)cfg->input_cfg[i]
					.next_buff_idx,
				    &dst_mva, &dst_size);
			}
			...
			if (dst_mva == 0) {
				DISP_ERROR("disable layer %d because of no "
					   "valid mva\n",
					   cfg->input_cfg[i].layer_id);
			...
			/* get src fence */
			src_fence =
			    sync_fence_fdget(cfg->input_cfg[i].src_fence_fd);
			if (!src_fence &&
			...
			mtkfb_update_buf_info(
			    cfg->session_id, cfg->input_cfg[i].layer_id,
			...
			disp_sync_put_cached_layer_info(
			    session_id, layer_id, &cfg->input_cfg[i], dst_mva);
...
static int output_config_preprocess(struct disp_frame_cfg_t *cfg)
{
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		/* must be mirror mode */
		if (primary_display_is_decouple_mode()) {
			disp_sync_put_cached_layer_info_v2(
			    session_id,
			    disp_sync_get_output_interface_timeline_id(),
	...
	DISPPR_FENCE(
	    "S+O/%s%d/L%d(id%d)/L%d(id%d)/%dx%d(%d,%d)/%s/%d/0x%08x/0x%p/"
	    "mva0x%08lx/t%d/sec%d\n",
	...
	DISPDBG("_ioctl_set_output_buffer done idx 0x%x, mva %lx, fmt %x, w "
		"%x, h %x (%x %x), p %x\n",
...
static int __frame_config(struct frame_queue_t *frame_node)
{
	...
	if (session_info)
		dprec_start(&session_info->event_frame_cfg, present_fence_idx,
			    0);
	...
	if (session_info)
		dprec_done(&session_info->event_frame_cfg, present_fence_idx,
			   0);
...
static int _ioctl_frame_config(unsigned long arg)
{
	...
	if (IS_ERR_OR_NULL(frame_node)) {
		...
		DISPERR(
		    "[FB Driver]: frame queue node create failed! line:%d\n",
		    __LINE__);
...
int _ioctl_wait_vsync(unsigned long arg)
{
	...
	if (copy_from_user(&vsync_config, argp, sizeof(vsync_config))) {
		DISP_ERROR("[FB]: copy_from_user failed! line:%d\n", __LINE__);
		return -EFAULT;
	}
	...
	if (copy_to_user(argp, &vsync_config, sizeof(vsync_config))) {
		DISP_ERROR("[FB]: copy_to_user failed! line:%d\n", __LINE__);
		return -EFAULT;
	}
	return ret;
}
				      ...
				      unsigned long arg)
{
	...
	case DISP_IOCTL_SET_SCENARIO: {
		/* arg of this ioctl is all unsigned int, don't need special
		 * compat ioctl */
