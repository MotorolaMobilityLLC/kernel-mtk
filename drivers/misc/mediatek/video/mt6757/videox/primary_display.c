...
long primary_display_wait_state(enum DISP_POWER_STATE state, long timeout)
{
	...
	ret =
	    __primary_display_wait_state(primary_get_state() == state, timeout);
	return ret;
}
			    ...
			    int bytes_per_pix)
{
	...
	if (disp_helper_get_option(DISP_OPT_SHOW_VISUAL_DEBUG_INFO)) {
		...
		ret =
		    m4u_mva_map_kernel(real_mva, real_size, &kva, &mapped_size);
		if (ret < 0) {
		...
		ret = MFC_Open(&mfc_handle, (void *)kva, pitch, h,
			       bytes_per_pix, DAL_COLOR_WHITE, DAL_COLOR_RED);
...
static int primary_show_basic_debug_info(struct disp_frame_cfg_t *cfg)
{
	...
	snprintf(disp_tmp, sizeof(disp_tmp), "PQ_trigger:%lld.%02lld,", fps.fps,
		 fps.fps_low);
...
#if 0 /* defined but not used */
static int primary_dynamic_debug(unsigned int mva, unsigned int pitch,
				...
				unsigned int x_pos, unsigned int block_sz)
{
	...
	if (ret) {
		pr_debug("%s error to query mva = 0x%x\n", __func__, mva);
		return -1;
	}
	ret = m4u_mva_map_kernel(real_mva, real_size, &map_va, &map_size);
	if (ret) {
		pr_debug("%s error to map mva = 0x%x\n", __func__, real_mva);
		return -1;
	}
...
static int fps_ctx_init(struct fps_ctx_t *fps_ctx, int wnd_sz)
{
	...
	/* fps_monitor = kthread_create(fps_monitor_thread, NULL,
	 * "fps_monitor");
	 */
...
#if 0 /* defined but not used */
static int _fps_ctx_check_drastic_change(struct fps_ctx_t *fps_ctx,
							unsigned int abs_fps)
{
	unsigned int avg_fps;
...
#if 0 /* defined but not used */
static int fps_ctx_get_wnd_sz(struct fps_ctx_t *fps_ctx, unsigned int *cur_wnd,
				unsigned int *max_wnd)
{
	mutex_lock(&fps_ctx->lock);
	if (cur_wnd)
	...
	if (disp_helper_get_option(DISP_OPT_USE_M4U)) {
		...
		if (ret == 0) {
			DISPDBG("config M4U Port %s to %s SUCCESS\n",
				...
				disp_helper_get_option(DISP_OPT_USE_M4U)
				    ? "virtual"
				    : "physical");
		} else {
		...
		if (disp_helper_get_option(DISP_OPT_ARR_PHASE_1)) {
			...
			if (dsi_vfp_addr[0]) {
				cmdqRecBackupWriteRegisterFromSlot(
				    pgc->cmdq_handle_trigger, pgc->dsi_vfp_line,
				    0, (unsigned int)(dsi_vfp_addr[0] &
						      0x0FFFFFFFF));
			}
...
/* for operations before frame transfer, such as waiting for DSI TE */
		...
		/*
		 *		if
		 *(disp_helper_get_option(DISP_OPT_SODI_SUPPORT))
...
{
	mmprofile_log_ex(ddp_mmp_get_events()->cg_mode, MMPROFILE_FLAG_PULSE, 0,
			 0);
}
				       ...
				       enum DISP_MODULE_ENUM after_engine)
{
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER)) {
		if (disp_helper_get_option(DISP_OPT_SHADOW_MODE) == 0)
			dpmgr_path_mutex_release(pgc->dpmgr_handle,
						 cmdq_handle);
	}
...
static int sec_buf_ion_alloc(int buf_size)
{
	...
	/*2. query sec hnd*/
	if (ion_phys(ion_client, sec_ion_handle, (unsigned long int *)&sec_hnd,
		     (size_t *)&mva_size)) {
...
static int _DL_switch_to_DC_fast(void)
{
	...
	/* mva for 1.ovl->wdma and2.Rdma->dsi , */
	unsigned int mva;
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER) &&
		...
		/* for vdo mode, we should wait for next SOF for MUTEX0 setting
		 * take effect
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER) &&
	    disp_helper_get_option(DISP_OPT_SHADOW_MODE) == 0) {
		/* If full shadow mode, get/release_mutex to surround all
		 * configs
		 */
		dpmgr_path_mutex_get(pgc->ovl2mem_path_handle,
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER) &&
	    disp_helper_get_option(DISP_OPT_SHADOW_MODE) == 0) {
		dpmgr_path_mutex_release(pgc->ovl2mem_path_handle,
					 pgc->cmdq_handle_ovl1to2_config);
	}
...
static int _DC_switch_to_DL_fast(void)
{
	...
	/* if blocking flush won't cause UX issue, we should simplify this code:
	 * remove callback
	 */
...
static int DL_switch_to_rdma_mode(struct cmdqRecStruct *handle, int block)
{
	...
	dpmgr_modify_path_power_on_new_modules(pgc->dpmgr_handle, new_scenario,
					       0);
	dpmgr_modify_path(
	    pgc->dpmgr_handle, new_scenario, handle,
	    primary_display_is_video_mode() ? DDP_VIDEO_MODE : DDP_CMD_MODE, 0);
	dpmgr_modify_path_power_off_old_modules(old_scenario, new_scenario, 0);
...
static int config_display_m4u_port(void)
{
	...
	if (ret) {
		DISPERR("config M4U Port %s to %s FAIL(ret=%d)\n",
			ddp_get_module_name(DISP_MODULE_OVL0_2L), m4u_usage,
			ret);
		return -1;
	}
			   ...
			   CmdqAsyncFlushCB callback, unsigned int data)
{
	...
	/* If full shadow mode: enable mutex in dpmgr_path_trigger() will signal
	 * sof
	...
	if (real_hrt_level > HRT_LEVEL_LOW &&
		...
		if (dvfs_last_ovl_req == HRT_LEVEL_EXTREME_LOW)
			primary_display_request_dvfs_perf(
			    MMDVFS_SCEN_DISP, HRT_LEVEL_EXTREME_LOW);
	}
	_primary_path_unlock(__func__);
			 ...
			 int is_lcm_inited)
{
	...
	DISPCHECK("primary _display _init begin lcm=%s, inited=%d\n", lcm_name,
		  is_lcm_inited);
	...
	if (init_decouple_buffer_thread == NULL) {
		init_decouple_buffer_thread =
		    kthread_create(_init_decouple_buffers_thread, NULL,
				   "init_decouple_buffer");
	...
	dpmgr_path_set_video_mode(pgc->dpmgr_handle,
				  primary_display_is_video_mode());
	DISPDBG("primary_ display _init->dpmgr_path_init\n");
	...
	DISPINFO("primary_ display _init ->dpmgr_path_config\n");
	...
	/* path start must after lcm init for video mode, because dsi_start will
	 * set mode
	 */
	...
	DISPCHECK("primary_ display _init done\n");
	...
	for (i = 0; i < PRIMARY_SESSION_INPUT_LAYER_COUNT; i++) {
		if (i == primary_display_get_option("ASSERT_LAYER") &&
			...
			disp_sync_get_cached_layer_info(
			    ...
			    &fence_idx);
			if (fence_idx == -1) {
				DISP_ERROR("find fence for layer %d,addr 0x%08x fail, unregistered\n",
					   i, addr);
			} else if (fence_idx < 0) {
				DISP_ERROR("find fence idx for layer %d,addr 0x%08x fail,unknown\n",
					   i, addr);
			} else {
				if (layer_en)
		...
		if (g_keep == pgc->force_fps_keep_count) {
			...
			while (g_skip != pgc->force_fps_skip_count) {
				...
				ret = dpmgr_wait_event_timeout(
				    ...
				    HZ / 10);
				if (ret == -2) {
					DISP_ERROR("vsync for primary display path not enabled yet\n");
					return -1;
				} else if (ret == 0) {
...
/* primary_display_idlemgr_kick(__func__, 1); */
	...
	return dpmgr_wait_event_timeout(pgc->dpmgr_handle,
					DISP_PATH_EVENT_IF_VSYNC, timeout);
}
...
int primary_display_suspend(void)
{
	...
	DISPCHECK("primary_ display_ suspend begin\n");
	...
	while (primary_get_state() == DISP_BLANK) {
		_primary_path_unlock(__func__);
		DISPCHECK("primary _display_ suspend wait tui finish!!\n");
#ifdef CONFIG_TRUSTONIC_TRUSTED_UI
		switch_set_state(&disp_switch_data, DISP_SLEPT);
#endif
		primary_display_wait_state(DISP_ALIVE, MAX_SCHEDULE_TIMEOUT);
		_primary_path_lock(__func__);
		DISPCHECK("primary _display_ suspend wait tui done stat=%d\n",
			  primary_get_state());
	}
...
/* pgc->state = DISP_SLEPT; */
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend,
			 MMPROFILE_FLAG_END, 0, 0);
	DISPCHECK("primary _display_ suspend end\n");
...
int primary_display_resume(void)
{
	...
	DISPCHECK("primary _display_ resume begin\n");
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume,
			 MMPROFILE_FLAG_START, 0, 0);
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume,
			 MMPROFILE_FLAG_PULSE, 0, 1);
	if (is_ipoh_bootup) {
		DISPCHECK("[primary display path] leave primary _display_ resume -- IPOH\n");
	...
	/* reinit fake timer for debug, we can enable option then press powerkey
...
{
	DISPMSG("primary _display_ ipoh_ restore In\n");
	...
	DISPMSG("primary _display_ ipoh _restore Out\n");
	return 0;
}
...
static int trigger_decouple_mirror(void)
{
	if (pgc->need_trigger_dcMirror_out == 0) {
		/* DISP_ERROR("There is no output config when decouple
		 * mirror!!\n");
		 */
	} else {
					  ...
					  int need_merge)
{
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE ||
		...
		if (pgc->need_trigger_ovl1to2 == 0) {
			DISP_ERROR("There is no output config when directlink mirror!!\n");
		} else {
...
static enum DISP_HELPER_OPT opt_backup_name[OPT_BACKUP_NUM] = {
    DISP_OPT_SMART_OVL, DISP_OPT_IDLEMGR_SWTCH_DECOUPLE, DISP_OPT_BYPASS_OVL};

static int opt_backup_value[OPT_BACKUP_NUM];
static unsigned int idlemgr_flag_backup;

static int svp_inited;
/* svp_inited: make sure the opt_backup & restore_opt functions be used in
 * pairs.
 */
			     ...
			     struct cmdqRecStruct *cmdq_handle)
{
	...
	if (hrt_level > HRT_LEVEL_LOW && primary_display_is_directlink_mode()) {
		primary_display_request_dvfs_perf(MMDVFS_SCEN_DISP,
						  HRT_LEVEL_HIGH);
	...
	if (bypass) {
		...
		if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE) {
			...
			do_primary_display_switch_mode(DISP_SESSION_RDMA_MODE,
						       pgc->session_id, 0,
						       cmdq_handle, 0);
		}
	} else {
...
int _set_backlight_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		...
		DISPMSG("[BL]_ set_ backlight_ by_cmdq ret=%d\n", ret);
	} else {
		mmprofile_log_ex(ddp_mmp_get_events()->primary_set_bl,
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_set_bl,
				 MMPROFILE_FLAG_PULSE, 1, 6);
		DISPMSG("[BL]_ set_ backlight_ by_ cmdq ret=%d\n", ret);
	}
			 ...
			 unsigned int *lcm_value)
{
	...
	if (primary_display_is_video_mode()) {
		...
		DISPCHECK("[CMD]_ set_ lcm_ cmd_ by_ cmdq ret=%d\n", ret);
	} else {
		mmprofile_log_ex(ddp_mmp_get_events()->primary_set_bl,
				 MMPROFILE_FLAG_PULSE, 1, 3);
		cmdqRecReset(cmdq_handle_lcm_cmd);
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_set_cmd,
				 MMPROFILE_FLAG_PULSE, 1, 6);
		DISPCHECK("[CMD]_ set_ lcm_ cmd_ by _cmdq ret=%d\n", ret);
	}
...
int primary_display_mipi_clk_change(unsigned int clk_value)
{
	...
	DISPCHECK("primary _display_ mipi _clk_ change return\n");
					    ...
					    enum UNIFIED_COLOR_FMT ufmt)
{
	...
	ret = m4u_cache_sync(m4uClient, M4U_PORT_DISP_WDMA0, pbuf, buffer_size,
			     mva, M4U_CACHE_INVALID_BY_RANGE);
out:
	if (mva > 0)
...
UINT32 DISP_GetVRamSize(void)
{
	...
	if (vramSize == 0) {
		...
		DISP_LOG("DISP _ GetVRamSize: %u bytes\n", vramSize);
	}
...
int primary_display_switch_dst_mode(int mode)
{
	...
	if (pgc->state == DISP_SLEPT) {
		...
		DISPCHECK("[primary _display_ switch _dst_ mode], primary display path is already sleep, skip\n");
		goto done;
	}
	...
	if (mode == primary_display_cur_dst_mode) {
		...
		DISPCHECK("[primary _display_ switch _dst_ mode]not need switch,cur_mode:%d, switch_mode:%d\n",
	...
	if (lcm_cmd == NULL) {
		...
		DISPCHECK("[primary _display_ switch_ dst_ mode]get lcm cmd fail primary _display_ cur _dst_ mode=%d mode=%d\n",
			  primary_display_cur_dst_mode, mode);
		goto done;
	}

	/* When switch to vdo mode, go back to DL mode if display path change to
	 * DC mode by SMART OVL
	 */
	if (disp_helper_get_option(DISP_OPT_SMART_OVL) &&
	    !primary_display_is_video_mode()) {
...
struct OPT_BACKUP tui_opt_backup[4] = {{DISP_OPT_SHARE_SRAM, 0},
...
/*Now the normal display vsync is DDP_IRQ_RDMA0_DONE in vdo mode, but when enter
 *TUI,
