...
void _dump_lcm_info(struct disp_lcm_handle *plcm)
{
	...
	if (p->type == LCM_TYPE_DSI) {
		DISPCHECK("[LCM] LANE_NUM: %d,data_format\n",
			  (int)p->dsi.LANE_NUM);
		DISPCHECK(
		    "[LCM] vact: %u, vbp: %u, vfp: %u, vact_line: %u, hact: "
		    "%u, hbp: %u, hfp: %u, hblank: %u\n",
		...
		DISPCHECK("[LCM] lcm_ext_te_enable: %d, noncont_clock: %d, "
		...
		for (i = 0; i < COMPARE_ID_SIZE; i++) {
			...
			case LCM_FUNC_CMD:
				...
				default:
					pr_debug(
					    "%s:%d: %d\n", __FILE__, __LINE__,
					    (unsigned int)lcm_dts->compare_id[i]
						.type);
					return;
				}
				break;
	...
	if (len <= 0) {
		...
		for (i = 0; i < BACKLIGHT_SIZE; i++) {
			...
			case LCM_FUNC_CMD:
				...
				case LCM_UTIL_WRITE_CMD_V2:
					memcpy(&(lcm_dts->backlight[i].data_t3),
					       tmp + 3,
		...
		for (i = 0; i < BACKLIGHT_CMDQ_SIZE; i++) {
			...
			case LCM_FUNC_CMD:
				...
				case LCM_UTIL_WRITE_CMD_V23:
					memcpy(&(lcm_dts->backlight_cmdq[i]
						     .data_t3),
					       tmp + 3,
					       lcm_dts->backlight_cmdq[i].size);
					break;
				       ...
				       int is_lcm_inited)
{
	...
	if (check_lcm_node_from_DT() == 0) {
		...
		if (strcmp(lcm_drv->name, plcm_name)) {
			DISPERR("FATAL ERROR!!!LCM Driver defined in "
				"kernel(%s) is different with LK(%s)\n",
		...
		if (plcm_name == NULL) {
			...
			if (!isLCMFound) {
				DISPERR("Fatal error: can't found lcm "
					"driver:%s in linux kernel driver\n",
					plcm_name);
...

	{
		...
		DISPERR("the specific LCM Interface [%d] didn't define any lcm "
			"driver\n",
			lcm_id);
		goto FAIL;
	}
...
void *disp_lcm_switch_mode(struct disp_lcm_handle *plcm, int mode)
{
	...
	if (_is_lcm_inited(plcm)) {
		if (plcm->params->dsi.switch_mode_enable == 0) {
			DISPERR(" ERROR, Not enable switch in lcm_get_params "
				"function\n");
			return NULL;
		}
		lcm_drv = plcm->drv;
		if (lcm_drv->switch_mode) {
			lcm_cmd =
			    (struct LCM_DSI_MODE_SWITCH_CMD *)lcm_drv->switch_mode(
				mode);
			lcm_cmd->cmd_if =
