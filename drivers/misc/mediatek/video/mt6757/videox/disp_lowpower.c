...
static struct disp_idlemgr_context *_get_idlemgr_context(void)
{
	...
	if (!is_inited) {
		...
		g_idlemgr_context.primary_display_idlemgr_task = kthread_create(
		    _primary_path_idlemgr_monitor_thread, NULL, "disp_idlemgr");
...
int primary_display_dsi_vfp_change(int state)
{
	...
	/* wait rdma0_sof: only used for video mode & trigger loop need wait and
	 * clear rdma0 sof */
...
void _acquire_wrot_resource_nolock(enum CMDQ_EVENT_ENUM resourceEvent)
{
	...
	acquireResult = cmdqRecWriteForResource(
	    handle, resourceEvent, disp_addr_convert(DISP_REG_RDMA_SRAM_SEL), 1,
	    ~0);
...
void _release_wrot_resource_nolock(enum CMDQ_EVENT_ENUM resourceEvent)
{
	...
	/* In cmd mode, after release wrot resource, primary display maybe enter
	 * idle */
	/* mode, so the RDMA0 register only be written into shadow register. It
	 * will bring */
	/* about RDMA0 and WROT1 use sram in the same time. Fix this bug by
	 * bypass */
	/* shadow register. */
...
int _switch_mmsys_clk(int mmsys_clk_old, int mmsys_clk_new)
{
	...
	if (primary_get_state() != DISP_ALIVE || is_mipi_enterulps()) {
		DISPERR("[disp_lowpower]_switch_mmsys_clk when display suspend "
			"old = %d & new = %d.\n",
...
void _primary_display_disable_mmsys_clk(void)
{
	...
	if (primary_display_is_decouple_mode()) {
		...
		if (primary_get_ovl2mem_handle())
			...
			DISPERR("display is decouple mode, but "
				"ovl2mem_path_handle is null\n");
...
void _primary_display_enable_mmsys_clk(void)
{
	...
	if (dpmgr_path_is_busy(primary_get_dpmgr_handle()))
		DISPERR("[LP]3.Fatal error, we didn't trigger display path but "
			"it's already busy\n");
...
void _vdo_mode_enter_idle(void)
{
	...
	if (!primary_is_sec() &&
		...
		/*if
		 * (disp_helper_get_option(DISP_OPT_DYNAMIC_RDMA_GOLDEN_SETTING))*/
	...
	while (1) {
...
#endif
		if (((local_clock() - idlemgr_pgc->idlemgr_last_kick_time) /
		     1000) < 100 * 1000) {
		...
		if (get_ddr_type() == TYPE_LPDDR3)
			...
			primary_display_request_dvfs_perf(
			    SMI_BWC_SCEN_UI_IDLE, HRT_LEVEL_EXTREME_LOW);
...
void primary_display_idlemgr_kick(const char *source, int need_lock)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->idlemgr, MMPROFILE_FLAG_PULSE, 1,
			 0);
...
void set_enterulps(unsigned flag) { idlemgr_pgc->enterulps = flag; }
