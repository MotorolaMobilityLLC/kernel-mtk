...
static int alloc_buffer_from_dma(size_t size, struct test_buf_info *buf_info)
{
	...
	if (!(buf_info->buf_va)) {
		DISPMSG("dma_alloc_coherent error!  dma memory not available. "
			"size=%zu\n",
			size);
		return -1;
	}
	...
	if (disp_helper_get_option(DISP_OPT_USE_M4U)) {
		...
		ret = m4u_alloc_mva(buf_info->m4u_client, M4U_PORT_DISP_OVL0, 0,
				    sg_table, size_align,
				      ...
				      unsigned int b, unsigned int a)
{
	...
	DISPMSG("%s: layer_num=%u,w=%d,h=%d,fmt=%s,frame_num=%d,vsync=%d, "
		"size=%lu\n",
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "primary_basic_test:", 19) == 0) {
		...
		ret = sscanf(
		    ...
		    &layer_num, &w, &h, &fmt, &frame_num, &vsync_num, &x, &y,
		    &r, &g, &b, &a);
		if (ret != 12) {
...
int debug_get_info(unsigned char *stringbuf, int buf_len)
{
	...
	n +=
	    dprec_logger_get_buf(DPREC_LOGGER_DUMP, stringbuf + n, buf_len - n);
			  ...
			  loff_t *ppos)
{
	 ...
	 * need massive
	 * iteration. We only copy ringbuffer content to debugfs buffer at first
	 * time (*ppos = 0)
	 */
...
void DBG_Init(void)
{
	...
	mtkfb_dbgfs = debugfs_create_file("mtkfb", S_IFREG | S_IRUGO, NULL,
					  (void *)0, &debug_fops);

	d_folder = debugfs_create_dir("displowpower", NULL);
	if (d_folder) {
		d_file = debugfs_create_file("kickdump", S_IFREG | S_IRUGO,
					     d_folder, NULL, &kickidle_fops);
		d_file = debugfs_create_file("partial", S_IFREG | S_IRUGO,
