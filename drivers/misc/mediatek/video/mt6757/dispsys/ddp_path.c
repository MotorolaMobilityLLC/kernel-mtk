...
unsigned int module_list_scenario[DDP_SCENARIO_MAX][DDP_ENING_NUM] = {
	...
	{DISP_MODULE_OVL0, DISP_MODULE_OVL0_2L, DISP_MODULE_OVL0_VIRTUAL,
	 ...
	 DISP_MODULE_GAMMA0, /*DISP_MODULE_OD,*/
	 DISP_MODULE_DITHER0, DISP_MODULE_RDMA0, DISP_PATH0, DISP_MODULE_UFOE,
	...
	{DISP_MODULE_RDMA0, DISP_MODULE_COLOR0, DISP_MODULE_CCORR0,
	 ...
	 /*DISP_MODULE_OD,*/ DISP_MODULE_DITHER0, DISP_PATH0, DISP_MODULE_UFOE,
	 DISP_MODULE_PWM0, DISP_MODULE_DSI0, -1},
	...
	{DISP_MODULE_OVL0, DISP_MODULE_OVL0_2L, DISP_MODULE_OVL0_VIRTUAL,
	 DISP_MODULE_COLOR0, DISP_MODULE_CCORR0, DISP_MODULE_AAL0,
	 DISP_MODULE_GAMMA0, /*DISP_MODULE_OD,*/
	 DISP_MODULE_DITHER0, DISP_PATH0, DISP_MODULE_UFOE, DISP_MODULE_PWM0,
	 DISP_MODULE_DSI0, -1},
	...
	{DISP_MODULE_OVL0, DISP_MODULE_OVL0_2L, DISP_MODULE_OVL0_VIRTUAL,
	 ...
	 DISP_MODULE_WDMA0, -1},
	/*SUB_DISP */
	{DISP_MODULE_OVL1, DISP_MODULE_OVL1_2L, DISP_MODULE_COLOR1,
	 DISP_MODULE_CCORR1, DISP_MODULE_AAL1, DISP_MODULE_GAMMA1,
	 DISP_MODULE_DITHER1, DISP_MODULE_RDMA1, DISP_MODULE_DPI, -1},
	/*SUB_RDMA1_DISP */
	...
	/*PRIMARY_DISP ALL*/
	{DISP_MODULE_OVL0, DISP_MODULE_OVL0_2L, DISP_MODULE_OVL0_VIRTUAL,
	 DISP_MODULE_WDMA0, DISP_MODULE_COLOR0, DISP_MODULE_CCORR0,
	 DISP_MODULE_AAL0, DISP_MODULE_GAMMA0, /*DISP_MODULE_OD,*/
	 DISP_MODULE_DITHER0, DISP_MODULE_RDMA0, DISP_PATH0, DISP_MODULE_UFOE,
	 DISP_MODULE_PWM0, DISP_MODULE_DSI0, -1},
	/*SUB_ALL */
	{DISP_MODULE_OVL1, DISP_MODULE_OVL1_2L, DISP_MODULE_WDMA1,
	 DISP_MODULE_COLOR1, DISP_MODULE_CCORR1, DISP_MODULE_AAL1,
	 DISP_MODULE_DITHER1, DISP_MODULE_RDMA1, DISP_MODULE_DPI, -1},
	/*DDP_SCENARIO_DITHER_1TO2*/
	{DISP_MODULE_OVL0, DISP_MODULE_OVL0_2L, DISP_MODULE_OVL0_VIRTUAL,
	 DISP_MODULE_COLOR0, DISP_MODULE_CCORR0, DISP_MODULE_AAL0,
	 DISP_MODULE_GAMMA0, /*DISP_MODULE_OD,*/
	 DISP_MODULE_DITHER0, DISP_MODULE_WDMA0, DISP_MODULE_RDMA0, DISP_PATH0,
	...
	{DISP_MODULE_OVL0, DISP_MODULE_OVL0_2L, DISP_MODULE_OVL0_VIRTUAL,
	 ...
	 DISP_MODULE_GAMMA0, /*DISP_MODULE_OD,*/
	 DISP_MODULE_DITHER0, DISP_MODULE_RDMA0, DISP_PATH0, DISP_MODULE_UFOE,
	 DISP_MODULE_WDMA0, DISP_MODULE_PWM0, DISP_MODULE_DSI0, -1},
};
...
static struct mout_s mout_map[DDP_MOUT_NUM] = {
	...
	{DISP_MODULE_OVL1,
	 {{DISP_MODULE_OVL1_2L, 1 << 0},
	  {DISP_MODULE_OVL0_VIRTUAL, 1 << 1},
	  {DISP_MODULE_WDMA1, 1 << 2},
	  {DISP_MODULE_COLOR1, 1 << 3},
	  {-1, 0} },
	 0,
	 0},
	{DISP_MODULE_OVL1_2L,
	...
	{DISP_MODULE_UFOE,
	 {{DISP_MODULE_DSI0, 1 << 0},
	  {DISP_MODULE_SPLIT0, 1 << 1},
	  {DISP_MODULE_DPI, 1 << 2},
	  {DISP_MODULE_WDMA0, 1 << 3},
...
static struct sel_s sel_out_map[DDP_SEL_OUT_NUM] = {
	...
	/* RDMA0_SOUT */
	{DISP_MODULE_RDMA0,
	 {DISP_PATH0, DISP_MODULE_COLOR0, DISP_MODULE_DSI0, DISP_MODULE_DSI1,
	  DISP_MODULE_DPI},
	 0,
	 0},
	/* RDMA1_SOUT */
	{DISP_MODULE_RDMA1,
	 {DISP_PATH1, DISP_MODULE_COLOR1, DISP_MODULE_DSI0, DISP_MODULE_DSI1,
	  DISP_MODULE_DPI},
	 0,
	 0},
	/* OVL0_SOUT */
	{DISP_MODULE_OVL0_2L,
	 {DISP_MODULE_OVL0_VIRTUAL, DISP_MODULE_OVL1, DISP_MODULE_OVL1_2L, -1},
	 0,
	 0},
};
...
static struct sel_s sel_in_map[DDP_SEL_IN_NUM] = {
	...
	{DISP_MODULE_COLOR0,
	 ...
	 0},
	{DISP_MODULE_COLOR1,
	 {DISP_MODULE_RDMA1, DISP_MODULE_OVL1_2L, DISP_MODULE_OVL1, -1},
	 0,
	 0},

	/* WDMA_SEL */
	{DISP_MODULE_WDMA0,
	 {DISP_MODULE_OVL0_VIRTUAL, DISP_MODULE_DITHER0, DISP_MODULE_UFOE,
	  DISP_MODULE_OVL0, -1},
	 0,
	 0},
	{DISP_MODULE_WDMA1,
	 {DISP_MODULE_OVL1_2L, DISP_MODULE_DITHER1, DISP_MODULE_DSC,
	  DISP_MODULE_OVL1, -1},
	 0,
	 0},
	...
	/* DSI_SEL */
	{DISP_MODULE_DSI0,
	 {DISP_MODULE_UFOE, DISP_MODULE_SPLIT0, DISP_MODULE_RDMA0, DISP_MODULE_DSC,
	  DISP_MODULE_RDMA1, -1},
	 0,
	 0},
	{DISP_MODULE_DSI1,
	 {DISP_MODULE_SPLIT0, DISP_MODULE_RDMA0, DISP_MODULE_DSC, DISP_MODULE_RDMA1,
	  DISP_MODULE_UFOE, -1},
	 0,
	 0},
	...
	/* DPI0_SEL */
	{DISP_MODULE_DPI,
	 {DISP_MODULE_UFOE, DISP_MODULE_RDMA0, DISP_MODULE_DSC, DISP_MODULE_RDMA1,
	  DISP_MODULE_RDMA2, -1},
	 0,
	 0},
	...
	/* OVL0_SEL */
	{DISP_MODULE_OVL0_VIRTUAL,
	 {DISP_MODULE_OVL0_2L, DISP_MODULE_OVL1_2L, DISP_MODULE_OVL1, -1},
	 0,
	 0},
...
int ddp_path_init(void)
{
	...
	/* sel_out */
	sel_out_map[0].reg =
	    (unsigned long *)DISP_REG_CONFIG_DISP_PATH0_SOUT_SEL_IN;
	sel_out_map[1].reg =
	    (unsigned long *)DISP_REG_CONFIG_DISP_PATH1_SOUT_SEL_IN;
	sel_out_map[2].reg =
	    (unsigned long *)DISP_REG_CONFIG_DISP_RDMA0_SOUT_SEL_IN;
	sel_out_map[3].reg =
	    (unsigned long *)DISP_REG_CONFIG_DISP_RDMA1_SOUT_SEL_IN;
	sel_out_map[4].reg =
	    (unsigned long *)DISP_REG_CONFIG_DISP_OVL0_SOUT_SEL_IN;
	...
	sel_in_map[8].reg =
	    (unsigned long *)DISP_REG_CONFIG_DSI0_SEL_IN; /* DISDUAL for DSI0 */
	sel_in_map[9].reg =
	    (unsigned long *)DISP_REG_CONFIG_DSI1_SEL_IN; /* DISDUAL for DSI1 */
	...
	sel_in_map[14].reg =
	    (unsigned long *)DISP_REG_CONFIG_DISP_OVL1_2L_SEL_IN;
	...
	for (i = 0; i < module_num - 1; i++) {
		for (j = 0; j < DDP_MOUT_NUM; j++) {
			if (module_list[i] == mout_map[j].id) {
				...
				step = i + 1;
				while (
				    module_can_connect[module_list[step]].bit ==
					0 &&
				    step < module_num) {
					step++;
				}
				...
				for (k = 0;
				     k < ARRAY_SIZE(mout_map[j].out_id_bit_map);
				     k++) {
	...
	for (i = 0; i < module_num - 1; i++) {
		for (j = 0; j < DDP_SEL_OUT_NUM; j++) {
			if (module_list[i] == sel_out_map[j].id) {
				...
				ASSERT(step < module_num);
				for (k = 0;
				     k < ARRAY_SIZE(sel_out_map[j].id_bit_map);
				     k++) {
					if (sel_out_map[j].id_bit_map[k] == -1)
						break;
					if (sel_out_map[j].id_bit_map[k] ==
					    module_list[step]) {
						DDPDBG("connect out_s %s to "
						       "%s, value=%d\n",
						       ddp_get_module_name(
							   module_list[i]),
						       ddp_get_module_name(
	...
	for (i = 1; i < module_num; i++) {
		for (j = 0; j < DDP_SEL_IN_NUM; j++) {
			if (module_list[i] == sel_in_map[j].id) {
				...
				ASSERT(step >= 0);
				for (k = 0;
				     k < ARRAY_SIZE(sel_in_map[j].id_bit_map);
				     k++) {
					if (sel_in_map[j].id_bit_map[k] == -1)
						break;
					if (sel_in_map[j].id_bit_map[k] ==
					    module_list[step]) {
						DDPDBG("connect in_s %s to %s, "
						       "value=%d\n",
						       ddp_get_module_name(
							   module_list[step]),
						       ddp_get_module_name(
			...
			for (k = 0; k < 4; k++) {
				...
				if (sel_in_map[j].id_bit_map[k] ==
					...
					if (DISP_REG_GET(sel_in_map[j].reg) !=
						...
						path_error += 1;
						DDPDUMP(
						    "error:in_s %s not conn to "
						    "%s,expect0x%x,real0x%x\n",
						    ddp_get_module_name(
							module_list[step]),
						    ddp_get_module_name(
							module_list[i]),
			     ...
			     unsigned int *EOF_src)
{
	...
	if (dest_module == DISP_MODULE_DSI0 ||
		...
		DDPERR("get mutex sof, invalid param dst module = %s(%d), dis "
		       "mode %s\n",
			   ...
			   enum DDP_MODE ddp_mode, void *handle)
{
	...
	for (i = 0; i < module_num; i++) {
		if (module_mutex_map[module_list[i]].bit != -1) {
			...
			} else if (module_mutex_map[module_list[i]].mod_num ==
				   1) {
				/* DISP_MODULE_DSIDUAL is special */
				if (module_mutex_map[module_list[i]].module ==
				    DISP_MODULE_DSIDUAL) {
					value1 |= (1 << module_mutex_map
							    [DISP_MODULE_DSI0]
								.bit);
					...
					value1 |=
					    (1
					     << module_mutex_map[module_list[i]]
						    .bit);
				}
			}
		} else {
			      ...
			      enum DDP_MODE ddp_mode)
{
	...
	if (expect_value1 != real_value1)
		DDPDUMP("error:mutex %d error: expect1 0x%x, real1 0x%x\n",
			mutex_id, expect_value1, real_value1);
		      ...
		      enum DISP_MODULE_ENUM place, enum DISP_MODULE_ENUM module)
{
	...
	if (idx < 0) {
		DDPERR("error: ddp_insert_module , place=%s is not in scenario "
		       "%s!\n",
		      ...
		      enum DISP_MODULE_ENUM module)
{
	...
	if (idx < 0) {
		DDPERR("ddp_remove_module, can not find module %s in scenario "
		       "%s\n",
		       ddp_get_module_name(module),
		       ddp_get_scenario_name(ddp_scenario));
		return -1;
...
void ddp_connect_path(enum DDP_SCENARIO_ENUM scenario, void *handle)
{
	...
	if (scenario == DDP_SCENARIO_PRIMARY_ALL) {
		...
		ddp_connect_path_l(
		    module_list_scenario[DDP_SCENARIO_PRIMARY_DISP], handle);
		ddp_connect_path_l(
		    module_list_scenario[DDP_SCENARIO_PRIMARY_DITHER_MEMOUT],
		    handle);
	} else if (scenario == DDP_SCENARIO_UFOE_1TO2) {
		ddp_connect_path_l(
		    module_list_scenario[DDP_SCENARIO_PRIMARY_DISP], handle);
		ddp_connect_path_l(
		    module_list_scenario[DDP_SCENARIO_PRIMARY_UFOE_MEMOUT],
		    handle);
	} else {
...
int ddp_is_moudule_in_mutex(int mutex_id, enum DISP_MODULE_ENUM module)
{
	...
	if (mutex_id < DISP_MUTEX_DDP_FIRST || mutex_id > DISP_MUTEX_DDP_LAST) {
		DDPDUMP("error:check_moudule_in_mute fail:exceed mutex max (0 "
			"~ %d)\n",
			DISP_MUTEX_DDP_LAST);
		return ret;
	}
	...
	if (module < DISP_MODULE_UNKNOWN) {
		if (module_mutex_map[module].bit != -1) {
			...
			if (module_mutex_map[module].mod_num == 0)
				DISP_REG_MASK(
				    handle,
				    DISP_REG_CONFIG_MUTEX_MOD0(mutex_id), value,
				    value);
			else if (module_mutex_map[module].mod_num == 1)
				DISP_REG_MASK(
				    handle,
				    DISP_REG_CONFIG_MUTEX_MOD1(mutex_id), value,
				    value);
		}
	}
	return 0;
	...
	if (module < DISP_MODULE_UNKNOWN) {
		if (module_mutex_map[module].bit != -1) {
			...
			if (module_mutex_map[module].mod_num == 0)
				...
				DISP_REG_MASK(
				    handle,
				    DISP_REG_CONFIG_MUTEX_MOD1(mutex_id), 0,
				    value);
		}
	}
	return 0;
