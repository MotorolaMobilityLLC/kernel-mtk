...
static enum DISP_MODULE_ENUM ovl_index_module[OVL_NUM] = {
	DISP_MODULE_OVL0, DISP_MODULE_OVL1, DISP_MODULE_OVL0_2L,
	DISP_MODULE_OVL1_2L};
...
int ovl_start(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET_FIELD(handle, EN_FLD_OVL_EN, ovl_base + DISP_REG_OVL_EN,
			   0x1);
...
int ovl_is_idle(enum DISP_MODULE_ENUM module)
{
	...
	if (((DISP_REG_GET(ovl_base + DISP_REG_OVL_FLOW_CTRL_DBG) & 0x3ff) !=
	     0x1) &&
	    ((DISP_REG_GET(ovl_base + DISP_REG_OVL_FLOW_CTRL_DBG) & 0x3ff) !=
	     0x2))
		return 0;
	else
		return 1;
...
#define OVL_IDLE (0x3)
	...
	if (handle == NULL) {
		while (!(DISP_REG_GET(ovl_base + DISP_REG_OVL_FLOW_CTRL_DBG) &
			 OVL_IDLE)) {
			delay_cnt++;
			udelay(10);
			if (delay_cnt > 2000) {
	    ...
	    unsigned int bg_color, void *handle)
{
	...
	if ((bg_w > OVL_MAX_WIDTH) || (bg_h > OVL_MAX_HEIGHT)) {
		DDPERR("ovl roi,exceed OVL max size, w=%d, h=%d\n", bg_w, bg_h);
		ASSERT(0);
	}
...
int disable_ovl_layers(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DDPSVPMSG(
	    "[SVP] switch ovl%d to nonsec: disable all the layers first!\n",
	    ovl_idx);
	return 0;
}

int ovl_layer_switch(enum DISP_MODULE_ENUM module, unsigned layer,
			    ...
			    void *handle)
{
	...
	unsigned long ovl_base = ovl_base_addr(module);
	unsigned long ext_layer_offset =
	    is_ext_layer ? (DISP_REG_OVL_EL0_CON - DISP_REG_OVL_L0_CON) : 0;
	unsigned long ext_layer_offset_clr =
	    is_ext_layer ? (DISP_REG_OVL_EL0_CLEAR - DISP_REG_OVL_L0_CLR) : 0;
	unsigned long ext_layer_offset_addr =
	    is_ext_layer ? (DISP_REG_OVL_EL0_ADDR - DISP_REG_OVL_L0_ADDR) : 0;
	unsigned long layer_offset =
	    ovl_base + ext_layer_offset +
	    (is_ext_layer ? cfg->ext_layer : layer) * OVL_LAYER_OFFSET;
	unsigned long layer_offset_clr =
	    ovl_base + ext_layer_offset_clr +
	    (is_ext_layer ? cfg->ext_layer : layer) * 4;
	unsigned long layer_offset_rdma_ctrl =
	    ovl_base + layer * OVL_LAYER_OFFSET;
	unsigned long layer_offset_addr =
	    ovl_base + ext_layer_offset_addr +
	...
	if (ovl_partial_roi != NULL) {
		...
		DDPDBG(
		"layer partial (%d,%d)(%d,%d,%d,%d) to (%d,%d)(%d,%d,%d,%d)\n",
	...
	if (format == UFMT_UYVY || format == UFMT_VYUY || format == UFMT_YUYV ||
	    format == UFMT_YVYU) {
	...
	DISP_REG_SET(handle, DISP_REG_OVL_RDMA0_CTRL + layer_offset_rdma_ctrl,
		     0x1);
		...
		if (cfg->security != DISP_SECURE_BUFFER) {
			...
			cmdqRecWriteSecure(
			    handle, disp_addr_convert(DISP_REG_OVL_L0_ADDR +
						      layer_offset_addr),
			    CMDQ_SAM_NMVA_2_MVA, cfg->addr + offset, 0, size,
			    m4u_port);
			...
			cmdqRecWriteSecure(
			    handle, disp_addr_convert(DISP_REG_OVL_L0_ADDR +
						      layer_offset_addr),
			    CMDQ_SAM_H_2_MVA, cfg->addr, offset, size,
			    m4u_port);
		}
	}
...
int ovl_clock_on(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	case DISP_MODULE_OVL0:
#ifdef CONFIG_MTK_CLKMGR
		enable_clock(MT_CG_DISP0_DISP_OVL0,
			     ddp_get_module_name(module));
#else
	...
	case DISP_MODULE_OVL1:
#ifdef CONFIG_MTK_CLKMGR
		enable_clock(MT_CG_DISP0_DISP_OVL1,
			     ddp_get_module_name(module));
#else
	...
	case DISP_MODULE_OVL0_2L:
#ifdef CONFIG_MTK_CLKMGR
		enable_clock(MT_CG_DISP0_DISP_OVL0_2L,
			     ddp_get_module_name(module));
#else
	...
	case DISP_MODULE_OVL1_2L:
#ifdef CONFIG_MTK_CLKMGR
		enable_clock(MT_CG_DISP0_DISP_OVL1_2L,
			     ddp_get_module_name(module));
#else
...
int ovl_clock_off(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	case DISP_MODULE_OVL0:
#ifdef CONFIG_MTK_CLKMGR
		disable_clock(MT_CG_DISP0_DISP_OVL0,
			      ddp_get_module_name(module));
#else
	...
	case DISP_MODULE_OVL1:
#ifdef CONFIG_MTK_CLKMGR
		disable_clock(MT_CG_DISP0_DISP_OVL1,
			      ddp_get_module_name(module));
#else
	...
	case DISP_MODULE_OVL0_2L:
#ifdef CONFIG_MTK_CLKMGR
		disable_clock(MT_CG_DISP0_DISP_OVL0_2L,
			      ddp_get_module_name(module));
#else
	...
	case DISP_MODULE_OVL1_2L:
#ifdef CONFIG_MTK_CLKMGR
		disable_clock(MT_CG_DISP0_DISP_OVL1_2L,
			      ddp_get_module_name(module));
#else
...
void ovl_get_info(enum DISP_MODULE_ENUM module, void *data)
{
	...
	for (i = 0; i < ovl_layer_num(module); i++) {
		...
		if (p->layer_en) {
			p->fmt = display_fmt_reg_to_unified_fmt(
			    ...
			    DISP_REG_GET_FIELD(L_CON_FLD_RGB_SWAP,
						   ...
						   DISP_REG_OVL_L0_CON));
			p->addr =
			    DISP_REG_GET(layer_off + DISP_REG_OVL_L0_ADDR);
			p->src_w =
			    DISP_REG_GET(layer_off + DISP_REG_OVL_L0_SRC_SIZE) &
			    0xfff;
			p->src_h = (DISP_REG_GET(layer_off +
				   ...
				   0xfff;
			p->src_pitch =
			    DISP_REG_GET(layer_off + DISP_REG_OVL_L0_PITCH) &
			    0xffff;
			p->bpp = UFMT_GET_bpp(p->fmt) / 8;
			p->gpu_mode = (DISP_REG_GET(layer_off +
		...
		DDPMSG("ovl get info:layer%d,en %d,w %d,h %d,bpp %d,addr %lu\n",
...
static int ovl_check_input_param(struct OVL_CONFIG_STRUCT *config)
{
	...
	if (config->addr != 0)
		ret = m4u_query_mva_info(config->addr,
					 config->src_pitch * config->dst_h,
					 &mva, &size);
	if ((config->addr == 0 && config->source == 0) || config->dst_w == 0 ||
	    config->dst_h == 0 || (config->addr != 0 && size == 0 &&
				   config->security == DISP_NORMAL_BUFFER)) {
		DDPERR(
		    "ovl parameter invalidate, addr=%lu, w=%d, h=%d, size=%d\n",
...
{
	DDPDBG(
"%s, layer=%d(%d), source=%s, off(x=%d, y=%d), dst(%d, %d, %d, %d),pitch=%d,",
...
static int setup_ovl_sec(enum DISP_MODULE_ENUM module, void *handle, int is_engine_sec)
{
	...
	/*_cmdq_insert_wait_frame_done_token_mira(nonsec_switch_handle);*/
			...
			* setting take effect or translation fault may happen,
...
static inline int ovl_switch_to_sec(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	cmdq_engine = ovl_to_cmdq_engine(module);
	cmdqRecSetSecure(handle, 1);
	/* set engine as sec port, it will to access the sec memory EMI_MPU
	 * protected
	 */
	cmdqRecSecureEnablePortSecurity(handle, (1LL << cmdq_engine));
...
/* 1. check if it has been prepared, just only prepare once for each frame */
	...
	for (local_layer = 0; global_layer < PRIMARY_SESSION_INPUT_LAYER_COUNT;
		...
		/* skip disabled layers, but need to decrease local_layer to
		 * make layout continuously
		 * all layers layout continuously by HRT Calc, so this is not
		 * necessary
		 */
		...
		if (is_DAL_Enabled() &&
			...
			ovl_cfg->phy_layer =
			    ovl_layer_num(DISP_MODULE_OVL0_2L) - 1;
		...
		if (disp_helper_get_option(DISP_OPT_OVL_EXT_LAYER)) {
			if (ovl_cfg->ext_sel_layer != -1) {
				 ...
				 * TODO: remove this if we dispatch layer id not
				 * always start from idx=0
				 */
				if (phy_layer != ovl_cfg->ext_sel_layer) {
					DDPERR(
					"L%d layout not match: cur=%d, in=%d\n",
			...
			struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	for (layer_id = 0; layer_id < TOTAL_REAL_OVL_LAYER_NUM; layer_id++) {
		...
		if (pConfig->ovl_partial_dirty) {
			...
			if (rect_intersect(&layer_roi,
					   ...
					   &layer_partial_roi)) {
				print_layer_config_args(
				    module, ovl_cfg->phy_layer, ovl_cfg,
				    &layer_partial_roi);
	...
	DDPDBG(
"ovl%d enabledlayers=0x%01x enabledextlayers=0x%01x, ext_sel_layers=0x%04x\n",
...
void ovl_dump_reg(enum DISP_MODULE_ENUM module)
{
	...
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			...
			INREG32(module_base + 0x14), 0x20,
			INREG32(module_base + 0x20), 0x24,
			INREG32(module_base + 0x24));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x28, INREG32(module_base + 0x28), 0x2C,
			INREG32(module_base + 0x2C), 0x30,
			INREG32(module_base + 0x30), 0x34,
			INREG32(module_base + 0x34));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x38, INREG32(module_base + 0x38), 0x3C,
			INREG32(module_base + 0x3C), 0xF40,
			INREG32(module_base + 0xF40), 0x44,
			INREG32(module_base + 0x44));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x48, INREG32(module_base + 0x48), 0x4C,
			INREG32(module_base + 0x4C), 0x50,
			INREG32(module_base + 0x50), 0x54,
			INREG32(module_base + 0x54));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x58, INREG32(module_base + 0x58), 0x5C,
			INREG32(module_base + 0x5C), 0xF60,
			INREG32(module_base + 0xF60), 0x64,
			INREG32(module_base + 0x64));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x68, INREG32(module_base + 0x68), 0x6C,
			INREG32(module_base + 0x6C), 0x70,
			INREG32(module_base + 0x70), 0x74,
			INREG32(module_base + 0x74));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x78, INREG32(module_base + 0x78), 0x7C,
			INREG32(module_base + 0x7C), 0xF80,
			INREG32(module_base + 0xF80), 0x84,
			INREG32(module_base + 0x84));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x88, INREG32(module_base + 0x88), 0x8C,
			INREG32(module_base + 0x8C), 0x90,
			INREG32(module_base + 0x90), 0x94,
			INREG32(module_base + 0x94));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x98, INREG32(module_base + 0x98), 0x9C,
			INREG32(module_base + 0x9C), 0xFa0,
			INREG32(module_base + 0xFa0), 0xa4,
			INREG32(module_base + 0xa4));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0xa8, INREG32(module_base + 0xa8), 0xAC,
			INREG32(module_base + 0xAC), 0xc0,
			INREG32(module_base + 0xc0), 0xc8,
			INREG32(module_base + 0xc8));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0xcc, INREG32(module_base + 0xcc), 0xd0,
			INREG32(module_base + 0xd0), 0xe0,
			INREG32(module_base + 0xe0), 0xe8,
			INREG32(module_base + 0xe8));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0xec, INREG32(module_base + 0xec), 0xf0,
			INREG32(module_base + 0xf0), 0x100,
			INREG32(module_base + 0x100), 0x108,
			INREG32(module_base + 0x108));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x10c, INREG32(module_base + 0x10c), 0x110,
			INREG32(module_base + 0x110), 0x120,
			INREG32(module_base + 0x120), 0x128,
			INREG32(module_base + 0x128));
		DDPDUMP("OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x12c, INREG32(module_base + 0x12c), 0x130,
			INREG32(module_base + 0x130), 0x134,
			INREG32(module_base + 0x134), 0x138,
			INREG32(module_base + 0x138));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x13c, INREG32(module_base + 0x13c), 0x140,
			INREG32(module_base + 0x140), 0x144,
			INREG32(module_base + 0x144), 0x148,
			INREG32(module_base + 0x148));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x14c, INREG32(module_base + 0x14c), 0x150,
			INREG32(module_base + 0x150), 0x154,
			INREG32(module_base + 0x154), 0x158,
			INREG32(module_base + 0x158));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x15c, INREG32(module_base + 0x15c), 0x160,
			INREG32(module_base + 0x160), 0x164,
			INREG32(module_base + 0x164), 0x168,
			INREG32(module_base + 0x168));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x16c, INREG32(module_base + 0x16c), 0x170,
			INREG32(module_base + 0x170), 0x174,
			INREG32(module_base + 0x174), 0x178,
			INREG32(module_base + 0x178));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x17c, INREG32(module_base + 0x17c), 0x180,
			INREG32(module_base + 0x180), 0x184,
			INREG32(module_base + 0x184), 0x188,
			INREG32(module_base + 0x188));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x18c, INREG32(module_base + 0x18c), 0x190,
			INREG32(module_base + 0x190), 0x194,
			INREG32(module_base + 0x194), 0x198,
			INREG32(module_base + 0x198));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x19c, INREG32(module_base + 0x19c), 0x1a0,
			INREG32(module_base + 0x1a0), 0x1a4,
			INREG32(module_base + 0x1a4), 0x1a8,
			INREG32(module_base + 0x1a8));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x1ac, INREG32(module_base + 0x1ac), 0x1b0,
			INREG32(module_base + 0x1b0), 0x1b4,
			INREG32(module_base + 0x1b4), 0x1b8,
			INREG32(module_base + 0x1b8));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x1bc, INREG32(module_base + 0x1bc), 0x1c0,
			INREG32(module_base + 0x1c0), 0x1c4,
			INREG32(module_base + 0x1c4), 0x1c8,
			INREG32(module_base + 0x1c8));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x1cc, INREG32(module_base + 0x1cc), 0x1d0,
			INREG32(module_base + 0x1d0), 0x1d4,
			INREG32(module_base + 0x1d4), 0x1e0,
			INREG32(module_base + 0x1e0));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x1e4, INREG32(module_base + 0x1e4), 0x1e8,
			INREG32(module_base + 0x1e8), 0x1ec,
			INREG32(module_base + 0x1ec), 0x1F0,
			INREG32(module_base + 0x1F0));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x1F4, INREG32(module_base + 0x1F4), 0x1F8,
			INREG32(module_base + 0x1F8), 0x1FC,
			INREG32(module_base + 0x1FC), 0x200,
			INREG32(module_base + 0x200));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x208, INREG32(module_base + 0x208), 0x20C,
			INREG32(module_base + 0x20C), 0x210,
			INREG32(module_base + 0x210), 0x214,
			INREG32(module_base + 0x214));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x218, INREG32(module_base + 0x218), 0x21C,
			INREG32(module_base + 0x21C), 0x230,
			INREG32(module_base + 0x230), 0x234,
			INREG32(module_base + 0x234));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x238, INREG32(module_base + 0x238), 0x240,
			INREG32(module_base + 0x240), 0x244,
			INREG32(module_base + 0x244), 0x24c,
			INREG32(module_base + 0x24c));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x250, INREG32(module_base + 0x250), 0x254,
			INREG32(module_base + 0x254), 0x258,
			INREG32(module_base + 0x258), 0x25c,
			INREG32(module_base + 0x25c));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x260, INREG32(module_base + 0x260), 0x264,
			INREG32(module_base + 0x264), 0x268,
			INREG32(module_base + 0x268), 0x26C,
			INREG32(module_base + 0x26C));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x270, INREG32(module_base + 0x270), 0x280,
			INREG32(module_base + 0x280), 0x284,
			INREG32(module_base + 0x284), 0x288,
			INREG32(module_base + 0x288));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x28C, INREG32(module_base + 0x28C), 0x290,
			INREG32(module_base + 0x290), 0x29C,
			INREG32(module_base + 0x29C), 0x2A0,
			INREG32(module_base + 0x2A0));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x2A4, INREG32(module_base + 0x2A4), 0x2B0,
			INREG32(module_base + 0x2B0), 0x2B4,
			INREG32(module_base + 0x2B4), 0x2B8,
			INREG32(module_base + 0x2B8));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x2BC, INREG32(module_base + 0x2BC), 0x2C0,
			INREG32(module_base + 0x2C0), 0x2C4,
			INREG32(module_base + 0x2C4), 0x2C8,
			INREG32(module_base + 0x2C8));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x324, INREG32(module_base + 0x324), 0x330,
			INREG32(module_base + 0x330), 0x334,
			INREG32(module_base + 0x334), 0x338,
			INREG32(module_base + 0x338));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x33C, INREG32(module_base + 0x33C), 0xFB0,
			INREG32(module_base + 0xFB0), 0x344,
			INREG32(module_base + 0x344), 0x348,
			INREG32(module_base + 0x348));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x34C, INREG32(module_base + 0x34C), 0x350,
			INREG32(module_base + 0x350), 0x354,
			INREG32(module_base + 0x354), 0x358,
			INREG32(module_base + 0x358));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x35C, INREG32(module_base + 0x35C), 0xFB4,
			INREG32(module_base + 0xFB4), 0x364,
			INREG32(module_base + 0x364), 0x368,
			INREG32(module_base + 0x368));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x36C, INREG32(module_base + 0x36C), 0x370,
			INREG32(module_base + 0x370), 0x374,
			INREG32(module_base + 0x374), 0x378,
			INREG32(module_base + 0x378));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x37C, INREG32(module_base + 0x37C), 0xFB8,
			INREG32(module_base + 0xFB8), 0x384,
			INREG32(module_base + 0x384), 0x388,
			INREG32(module_base + 0x388));
		DDPDUMP(
	"OVL0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			0x38C, INREG32(module_base + 0x38C), 0x390,
			INREG32(module_base + 0x390), 0x394,
			INREG32(module_base + 0x394), 0x398,
			INREG32(module_base + 0x398));
		DDPDUMP("OVL0: 0x%04x=0x%08x\n", 0xFC0,
			INREG32(module_base + 0xFC0));
		...
		if (src_on & 0x1) {
			...
			DDPDUMP(
			"0xf40=0x%08x,0x044=0x%08x,0x0c0=0x%08x,0x0c8=0x%08x\n",
			...
			DDPDUMP(
			    "0x0d0=0x%08x,0x1e0=0x%08x,0x24c=0x%08x\n",
		...
		if (src_on & 0x2) {
			...
			DDPDUMP(
	"0xf60=0x%08x,0x064=0x%08x,0x0e0=0x%08x,0x0e8=0x%08x,0x0f0=0x%08x\n",
		...
		if (src_on & 0x4) {
			...
			DDPDUMP(
	"0xf80=0x%08x,0x084=0x%08x,0x100=0x%08x,0x108=0x%08x,0x110=0x%08x\n",
...
static void ovl_printf_status(unsigned int status)
{
	...
	DDPDUMP("addcon_idle:%d,blend_idle:%d,out_valid:%d,out_ready:%d,out_"
		"idle:%d\n",
	...
	DDPDUMP(
	    ...
	    (status >> 20) & (0x1));
	DDPDUMP("trig:%d,frame_hwrst_done:%d,frame_swrst_done:%d,frame_"
		"underrun:%d,frame_done:%d\n",
		(status >> 21) & (0x1), (status >> 23) & (0x1),
		(status >> 24) & (0x1), (status >> 25) & (0x1),
		(status >> 26) & (0x1));
	DDPDUMP("ovl_running:%d,ovl_start:%d,ovl_clr:%d,reg_update:%d,ovl_upd_"
		"reg:%d\n",
...
static void ovl_dump_layer_info(int layer, unsigned long layer_offset)
{
	...
	DDPDUMP("layer%d: "
		"w=%d,h=%d,off(x=%d,y=%d),pitch=%d,addr=0x%x,fmt=%s,source=%s,"
		"aen=%d,alpha=%d\n",
		layer,
		DISP_REG_GET(layer_offset + DISP_REG_OVL_L0_SRC_SIZE) & 0xfff,
		(DISP_REG_GET(layer_offset + DISP_REG_OVL_L0_SRC_SIZE) >> 16) &
		    0xfff,
		DISP_REG_GET(layer_offset + DISP_REG_OVL_L0_OFFSET) & 0xfff,
		(DISP_REG_GET(layer_offset + DISP_REG_OVL_L0_OFFSET) >> 16) &
		    0xfff,
		...
		(DISP_REG_GET_FIELD(L_CON_FLD_LARC,
				    DISP_REG_OVL_L0_CON + layer_offset) == 0)
		    ? "memory"
		    : "constant_color",
...
static void ovl_dump_ext_layer_info(int layer, unsigned long layer_offset)
{
	...
	DDPDUMP(
"ext layer%d: w=%d,h=%d,off(x=%d,y=%d),pitch=%d,addr=0x%x,fmt=%s,source=%s,aen=%d,alpha=%d\n",
		layer,
		...
		unified_color_fmt_name(fmt),
		(DISP_REG_GET_FIELD(L_CON_FLD_LARC,
				    DISP_REG_OVL_EL0_CON + layer_offset) == 0)
		    ? "memory"
		    : "constant_color",
		DISP_REG_GET_FIELD(L_CON_FLD_AEN,
				   DISP_REG_OVL_EL0_CON + layer_offset),
...
void ovl_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("ovl_en=%d,layer_enable(%d,%d,%d,%d),bg(w=%d, h=%d)\n",
		DISP_REG_GET(DISP_REG_OVL_EN + offset) & 0x1, src_con & 0x1,
	...
	DDPDUMP("ext layer: layer_enable(%d,%d,%d), attach_layer(%d,%d,%d)\n",
		...
		(ext_con >> 24) & 0xf);
	DDPDUMP(
	    "cur_pos(x=%d,y=%d),layer_hit(%d,%d,%d,%d),bg_mode=%s,sta=0x%x\n",
			      ...
			      enum dst_module_type dst_mod_type, void *cmdq)
{
	...
	if (dst_mod_type == DST_MOD_REAL_TIME)
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC_PRE_ULTRA_THRESHOLD, 0xff);
	else
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC_PRE_ULTRA_THRESHOLD, 0xe0);
	...
	if (dst_mod_type == DST_MOD_REAL_TIME) {
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES, 191);
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES_URG, 95);
	} else {
		/* decouple */
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES, 31);
		regval |=
		    REG_FLD_VAL(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES_URG, 15);
	}
	...
	regval =
	    REG_FLD_VAL(FLD_OVL_RDMA_GREQ_LAYER0_GREQ_URG_NUM, layer_greq_num);
	if (layer_num > 0)
