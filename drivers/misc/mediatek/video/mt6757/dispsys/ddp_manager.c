...
static struct DDP_IRQ_EVENT_MAPPING
    ddp_irq_event_list[DEFAULT_IRQ_EVENT_SCENARIO][DISP_PATH_EVENT_NUM] = {
	{
	    ...
	    {DDP_IRQ_UNKNOWN},
	    /*TRIGER*/ {DDP_IRQ_AAL_OUT_END_FRAME}, /*AAL_OUT_END_EVENT */
	},
	{
	    /* ovl1 path */
...
static int _dpmgr_path_connect(enum DDP_SCENARIO_ENUM scenario, void *handle)
{
	...
	for (i = 0; i < module_num; i++) {
		module = modules[i];
		if (ddp_modules_driver[module] &&
		    ddp_modules_driver[module]->connect) {
			int prev =
			    i == 0 ? DISP_MODULE_UNKNOWN : modules[i - 1];
			int next = i == module_num - 1 ? DISP_MODULE_UNKNOWN
						       : modules[i + 1];

			ddp_modules_driver[module]->connect(module, prev, next,
							    1, handle);
		}
	}
					   ...
					   int sw_only)
{
	...
	for (i = 0; i < new_module_num; i++) {
		...
		if (content->module_usage_table[module_name] ==
		    0) { /* new module's count =0 */
			content->module_usage_table[module_name]++;
...
int dpmgr_modify_path(disp_path_handle dp_handle,
		      enum DDP_SCENARIO_ENUM new_scenario,
		      struct cmdqRecStruct *cmdq_handle, enum DDP_MODE mode,
		      int sw_only)
{
			  ...
			  enum DISP_MODULE_ENUM engine, void *cmdq_handle)
{
	...
	if (ddp_is_module_in_scenario(handle->scenario, wdma) == 1) {
		DDPERR("dpmgr_path_add_memout error, wdma is already in "
		       "scenario=%s\n",
		       ddp_get_scenario_name(handle->scenario));
		return -1;
	}
...
int dpmgr_path_remove_memout(disp_path_handle dp_handle, void *cmdq_handle)
{
	...
	if (ddp_is_module_in_scenario(handle->scenario, wdma) == 0) {
		DDPERR("dpmgr_path_remove_memout error, wdma is not in "
		       "scenario=%s\n",
		       ddp_get_scenario_name(handle->scenario));
		return -1;
	}
...
int dpmgr_insert_ovl1_sub(disp_path_handle dp_handle, void *cmdq_handle)
{
	...
	ret = ddp_mutex_set(handle->hwmutexid, DDP_SCENARIO_SUB_DISP,
...
int dpmgr_remove_ovl1_sub(disp_path_handle dp_handle, void *cmdq_handle)
{
	...
	if (ddp_is_module_in_scenario(handle->scenario, DISP_MODULE_OVL1) ==
	    0) {
		DISP_LOG_I(
		    "dpmgr_remove_ovl1_sub, OVL1 already not in the path\n");
		return -1;
	}
	...
	ret = ddp_mutex_set(handle->hwmutexid, DDP_SCENARIO_SUB_RDMA1_DISP,
			    handle->mode, cmdq_handle);
		...
		if (ddp_modules_driver[module_name] != 0) {
			if (ddp_modules_driver[module_name]->ioctl != 0) {
				ret += ddp_modules_driver[module_name]->ioctl(
				    module_name, cmdq_handle, ioctl_cmd,
				    params);
			}
		}
	}
			  ...
			  enum DDP_IRQ_LEVEL irq_level)
{
	...
	for (i = module_num - 1; i >= 0; i--) {
		...
		if (ddp_modules_driver[module_name] &&
			...
			ret += ddp_modules_driver[module_name]->enable_irq(
			    module_name, cmdq_handle, irq_level);
		}
	}
	return ret;
}
		      ...
		      struct disp_ddp_path_config *config, void *cmdq_handle)
{
	...
	DISP_LOG_V("path config ovl %d, rdma %d, wdma %d, dst %d on handle %p "
		   "scenario %s\n",
		       ...
		       int encmdq)
{
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER)) {
		if (disp_helper_get_option(DISP_OPT_SHADOW_MODE) == 0) {
			dpmgr_path_mutex_release(dp_handle,
						 trigger_loop_handle);
...
int dpmgr_path_power_off(disp_path_handle dp_handle, enum CMDQ_SWITCH encmdq)
{
	...
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_modules_driver[module_name] &&
			...
			ddp_modules_driver[module_name]->power_off(
			    module_name, encmdq ? handle->cmdqhandle : NULL);
		}
	}
	handle->power_state = 0;
	path_top_clock_off();
...
/* TODO: just for verify rootcause, will be removed soon */
		...
		if (is_module_in_path(DISP_MODULE_AAL0, handle)) {
			if (ddp_modules_driver[DISP_MODULE_AAL0]->cmd != NULL)
				ret = ddp_modules_driver[DISP_MODULE_AAL0]->cmd(
				    DISP_MODULE_AAL0, msg, arg, cmdqhandle);
		}
#endif
		break;
	case DISP_IOCTL_SET_GAMMALUT:
		if (ddp_modules_driver[DISP_MODULE_GAMMA0]->cmd != NULL)
			ret = ddp_modules_driver[DISP_MODULE_GAMMA0]->cmd(
			    DISP_MODULE_GAMMA0, msg, arg, cmdqhandle);
		break;
	case DISP_IOCTL_SET_CCORR:
	case DISP_IOCTL_CCORR_EVENTCTL:
	case DISP_IOCTL_CCORR_GET_IRQ:
		if (ddp_modules_driver[DISP_MODULE_CCORR0]->cmd != NULL)
			ret = ddp_modules_driver[DISP_MODULE_CCORR0]->cmd(
			    DISP_MODULE_CCORR0, msg, arg, cmdqhandle);
		break;
			     ...
			     enum DISP_PATH_EVENT event, int timeout)
{
	...
	if (wq_handle->init) {
		...
		if (ret == 0) {
			...
			DISP_LOG_E("wait %s interrupt by other timeleft %d on "
				   "scenario %s\n",
		      ...
		      unsigned long long *event_ts)
{
	...
	ret =
	    wait_event_interruptible(wq_handle->wq, cur_time < wq_handle->data);
	if (ret < 0) {
		...
		if (regvalue & (0x1 << i)) {
			...
			for (j = 0; j < DISP_PATH_EVENT_NUM; j++) {
				if (handle->wq_list[j].init &&
					...
					DDPIRQ("irq signal event %s on cycle "
					       "%llu on scenario %s\n",
					       path_event_name(j),
					       handle->wq_list[j].data,
...
int dpmgr_factory_mode_test(int module_name, void *cmdqhandle, void *config)
{
	if (ddp_modules_driver[module_name] != 0) {
		if (ddp_modules_driver[module_name]->ioctl != 0) {
			...
			ddp_modules_driver[DISP_MODULE_DPI]->ioctl(
	...
	if (ddp_modules_driver[module_name] != 0) {
		if (ddp_modules_driver[module_name]->ioctl != 0) {
			DISP_LOG_I(" %s factory_mode_test\n",
				   ddp_get_module_name(DISP_MODULE_DPI));
			ddp_modules_driver[DISP_MODULE_DPI]->ioctl(
			    module_name, cmdqhandle, DDP_DPI_FACTORY_RESET,
			    config);
		}
	}
