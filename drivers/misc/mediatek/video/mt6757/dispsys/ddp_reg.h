...
/* 0~1 TYPE ,2 BTA,3 HS, 4 CL,5 TE,6~7 RESV, 8~15 DATA_ID,16~23 DATA_0,24~31
 * DATA_1 */
struct DSI_CMDQ {
	unsigned char byte0;
	unsigned char byte1;
...
#define DISP_REG_MASK(handle, reg32, val, mask)                                \
	do {                                                                   \
		if (handle == NULL) {                                          \
			mt_reg_sync_writel(                                    \
			    (unsigned int)(INREG32(reg32) & ~(mask)) | (val),  \
			    (reg32));                                          \
		} else {                                                       \
			cmdqRecWrite(                                          \
			    handle, disp_addr_convert((unsigned long)(reg32)), \
			    val, mask);                                        \
		}                                                              \
	} while (0)
...
/* ------------------------------------------------------------- */
/* MUTEX */
#define DISP_OVL_SEPARATE_MUTEX_ID                                             \
	(DISP_MUTEX_DDP_LAST + 1) /* other disp will not see mutex 4 */
