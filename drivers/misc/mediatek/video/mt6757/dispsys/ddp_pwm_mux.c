...
#define FQMTR_CK_EN                                                            \
	(disp_pmw_mux_base + 0x220) /* Enable fqmeter reference clk */
#define SET_DIV_CNT (disp_pmw_mux_base + 0x104) /* Set fqmeter div count */
#define AD_OSC_CLK_DBG                                                         \
	(disp_pmw_mux_base + 0x10c) /* Select debug for AD_OSC_CLK */
...
void disp_pwm_ulposc_query(char *debug_output)
{
	...
	buf_offset = snprintf(temp_buf, buf_max_len, "0x10006458: (0x%08x)\n",
			      clk_readl(OSC_ULPOSC_ADDR));
	temp_buf += buf_offset;
	buf_offset = snprintf(temp_buf, buf_max_len, "0x10006000: (0x%08x)\n",
			      clk_readl(OSC_ULPOSC_DBG));
	temp_buf += buf_offset;

	osc_con = clk_readl(PLL_OSC_CON0);
	buf_offset =
	    snprintf(temp_buf, buf_max_len, "0x10001B00: (0x%08x)\n", osc_con);
	temp_buf += buf_offset;

	buf_offset = snprintf(temp_buf, buf_max_len, "0x10001B04: (0x%08x)\n",
			      clk_readl(PLL_OSC_CON1));
	temp_buf += buf_offset;
	buf_offset = snprintf(temp_buf, buf_max_len, "0x10000220: (0x%08x)\n",
	...
	buf_offset =
	    snprintf(temp_buf, buf_max_len, "current meter value: (0x%08x)\n",
		     current_meter_value);
	temp_buf += buf_offset;
}
