...
static void ddp_disp_refresh_tag_start(unsigned int index)
{
	...
	if (dpp_disp_is_decouple() == 1) {
		...
		static struct OVL_BASIC_STRUCT
		    old_ovlInfo[OVL_NUM * OVL_LAYER_NUM_PER_OVL];
		static struct OVL_BASIC_STRUCT
		    ovlInfo[OVL_NUM * OVL_LAYER_NUM_PER_OVL];
		int ovl_index;
		int b_layer_changed;
		int i, j;
	...
	for (i = 0; i < layerCnt; i++) {
		...
		if (ovlInfo[i].layer_en) {
			...
			if (sLayerBufAddr[index][idx] != ovlInfo[i].addr) {
				...
				met_tag_oneshot(DDP_IRQ_LAYER_FPS_ID,
						tag_name, i+1);
			...
			if (sLayerBufFmt[index][idx] != ovlInfo[i].fmt) {
				...
				sprintf(tag_name, "OVL%dL%d_Fmt_%s",
					index, idx, fmtStr);
			...
			if (sLayerBufHeight[index][idx] != ovlInfo[i].src_h) {
				...
				sprintf(tag_name, "OVL%dL%d_Height",
					index, idx);
				met_tag_oneshot(DDP_IRQ_LAYER_SIZE_ID, tag_name,
						ovlInfo[i].src_h);
				layer_change_bits |= 1 << i;
	...
	/*CLS:met mmsys profile*/
	{
		...
		for (i = 0; i < OVL_LAYER_NUM; i++) {
			if (layer_change_bits & (1 << i)) {
				MET_UDTL_GET_PROP(OVL_LAYER_Props).layer
				  = i;
				MET_UDTL_GET_PROP(OVL_LAYER_Props).layer_en
				  = layer_enable_bits;
				MET_UDTL_GET_PROP(OVL_LAYER_Props).fmt
				  = ovlInfo[i].fmt;
				MET_UDTL_GET_PROP(OVL_LAYER_Props).addr
				  = ovlInfo[i].addr;
				MET_UDTL_GET_PROP(OVL_LAYER_Props).src_w
				  = ovlInfo[i].src_w;
				MET_UDTL_GET_PROP(OVL_LAYER_Props).src_h
				  = ovlInfo[i].src_h;
				MET_UDTL_GET_PROP(OVL_LAYER_Props).src_pitch
				  = ovlInfo[i].src_pitch;
				MET_UDTL_GET_PROP(OVL_LAYER_Props).bpp
				  = ovlInfo[i].bpp;

				MET_UDTL_TRACELINE_PROP(MMSYS,
							OVL_LAYERS__LAYER,
							OVL_LAYER_Props);
			}
		}
