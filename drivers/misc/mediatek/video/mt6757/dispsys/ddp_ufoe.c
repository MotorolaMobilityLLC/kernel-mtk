...
{
	if (ufoe_enable)
		DISP_REG_SET_FIELD(cmdq, START_FLD_DISP_UFO_START,
		       ...
		       struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (lcm_config->ufoe_enable == 1 && pConfig->dst_dirty) {
		...
		DISP_REG_SET_FIELD(handle, START_FLD_DISP_UFO_BYPASS,
				   DISP_REG_UFO_START, 0);
		/* DISP_REG_SET_FIELD(handle, START_FLD_DISP_UFO_START,
		 * DISP_REG_UFO_START, 1); */
		if (lcm_config->ufoe_params.lr_mode_en == 1) {
			...
			if (lcm_config->ufoe_params.compress_ratio == 3) {
				...
				DISP_REG_SET_FIELD(
				    handle, CFG_0B_FLD_DISP_UFO_CFG_COM_RATIO,
			...
			if (lcm_config->ufoe_params.vlc_disable) {
				...
				DISP_REG_SET(
				    ...
				    (lcm_config->ufoe_params.vlc_config == 0)
					? 5
...
static int _ufoe_partial_update(enum DISP_MODULE_ENUM module, void *arg,
				void *handle)
{
