...
#define MAKE_UNIFIED_COLOR_FMT(rgb, bpp, block, vdo, format, byteswap,         \
			       rgbswap, id)                                    \
	(((rgb) << _UFMT_RGB_SHIFT) | ((bpp) << _UFMT_bpp_SHIFT) |             \
	 ((block) << _UFMT_BLOCK_SHIT) | ((vdo) << _UFMT_VDO_SHIFT) |          \
	 ((format) << _UFMT_FORMAT_SHIFT) |                                    \
	 ((byteswap) << _UFMT_BYTESWAP_SHIFT) |                                \
	 ((rgbswap) << _UFMT_RGBSWAP_SHIFT) | ((id) << _UFMT_ID_SHIFT))
...
enum UNIFIED_COLOR_FMT
display_fmt_reg_to_unified_fmt(int fmt_reg_val, int byteswap, int rgbswap);
...
struct OVL_CONFIG_STRUCT {
	...
	int is_configured; /* is this layer configured to OVL HW, for multiply
			      OVL sync */
	int const_bld;
	int ext_sel_layer;
	int ext_layer;
...
struct disp_ddp_path_config {
	...
	int ovl_layer_scanned; /*each bit reprsent one layer, used for ovl
				  engines */
