...
int disp_irq_esd_cust_get(void) { return atomic_read(&esd_check_bycmdq); }
...
int disp_unregister_irq_callback(DDP_IRQ_CALLBACK cb)
{
	...
	if (i == DISP_MAX_IRQ_CALLBACK) {
		DDPERR("Try to unregister callback function %p which was not "
		       "registered\n",
		       cb);
		return -1;
	}
					...
					DDP_IRQ_CALLBACK cb)
{
	...
	if (i == DISP_MAX_IRQ_CALLBACK) {
		DDPERR("Try to unregister callback function with was not "
		       "registered. module=%d cb=%p\n",
		       module, cb);
		return -1;
	}
	return 0;
...
irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
	...
	DDPIRQ("disp_irq_handler, irq=%d, module=%s\n", irq,
	       ddp_get_module_name(disp_irq_module(irq)));

	if (irq == dispsys_irq[DISP_REG_DSI0] ||
	    irq == dispsys_irq[DISP_REG_DSI1]) {
		index = (irq == dispsys_irq[DISP_REG_DSI0]) ? 0 : 1;
		module = (irq == dispsys_irq[DISP_REG_DSI0]) ? DISP_MODULE_DSI0
							     : DISP_MODULE_DSI1;
		reg_val = (DISP_REG_GET(dsi_reg_va[index] + 0xC) & 0xff);
		reg_temp_val = reg_val;
		/* rd_rdy don't clear and wait for ESD & Read LCM will clear the
		 * bit. */
		...
		mmprofile_log_ex(ddp_mmp_get_events()->DSI_IRQ[index],
				 ...
				 DISP_REG_GET(dsi_reg_va[index] + 0x4));
		DDPIRQ("DSI irq=%d, regval=0x%x\n", irq, reg_val);
	} else if (irq == dispsys_irq[DISP_REG_OVL0] ||
		   irq == dispsys_irq[DISP_REG_OVL1] ||
		   irq == dispsys_irq[DISP_REG_OVL0_2L] ||
		   irq == dispsys_irq[DISP_REG_OVL1_2L]) {

		module = disp_irq_module(irq);
		index = ovl_to_index(module);
		reg_val =
		    DISP_REG_GET(DISP_REG_OVL_INTSTA + ovl_base_addr(module));
		if (reg_val & (1 << 0))
		...
		for (mutexID = 0; mutexID < 5; mutexID++) {
			if (reg_val & (0x1 << mutexID)) {
				...
				mmprofile_log_ex(
				    ddp_mmp_get_events()->MUTEX_IRQ[mutexID],
				    MMPROFILE_FLAG_PULSE, reg_val, 0);
			}
		...
		if (reg_val & (1 << 0))
			DDPERR("MMSYS to MFG APB TX Error, MMSYS clock off but "
			       "MFG clock on!\n");

		if (reg_val & (1 << 1))
			DDPERR("MMSYS to MJC APB TX Error, MMSYS clock off but "
			       "MJC clock on!\n");
...
static int disp_irq_log_kthread_func(void *data)
{
	...
	while (1) {
		...
		DDPMSG("disp_irq_log_kthread_func dump intr register: "
		       "disp_irq_log_module=%d\n",
		       disp_irq_log_module);
