...
enum DSI_STATUS DSI_DumpRegisters(enum DISP_MODULE_ENUM module, int level)
{
	...
	if (disp_helper_get_option(DISP_OPT_REG_PARSER_RAW_DUMP)) {
		if (module == DISP_MODULE_DSI0 ||
			...
			DDPDUMP(
	"DSI0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
	...
	"DSI0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			...
			DDPDUMP("DSI0: 0x%04x=0x%08x\n", 0x200,
				INREG32(DDP_REG_BASE_DSI0 + 0x200));
		...
		if (module == DISP_MODULE_DSI1 ||
			...
			DDPDUMP(
	"DSI1: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
	...
	"DSI1: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
			...
			DDPDUMP(
	"DSI1: 0x%04x=0x%08x\n", 0x200,
				INREG32(DDP_REG_BASE_DSI1 + 0x200));
		...
		if (level >= 1) {
			for (i = DSI_MODULE_BEGIN(module);
				...
				DDPDUMP("== DSI%d REGS ==\n", i);
				for (k = 0; k < sizeof(struct DSI_REGS);
				     k += 16) {
					DDPDUMP(
					    "0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
					    k, INREG32(dsi_base_addr + k),
				...
				for (k = 0; k < 32;
				     k +=
				     16) { /* only dump first 32 bytes cmd */
					DDPDUMP("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
						k, INREG32((dsi_base_addr +
							    0x200 + k)),
						INREG32((dsi_base_addr + 0x200 +
							 k + 0x4)),
						INREG32((dsi_base_addr + 0x200 +
							 k + 0x8)),
						INREG32((dsi_base_addr + 0x200 +
				...
				for (k = 0; k < sizeof(struct DSI_PHY_REGS);
				     k += 16) {
					DDPDUMP(
					    "0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
				      ...
				      unsigned int param)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (status.RD_RDY) {
			...
			/* / write clear RD_RDY interrupt must be before
			 * DSI_RACK
			 */
			/* / because CMD_DONE will raise after DSI_RACK, */
			/* / so write clear RD_RDY after that will clear */
			/* CMD_DONE too */
			/*do*/
			/*{*/
			/*/send read ACK*/
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enter) {
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LD0CON_REG,
				      DSI_REG[i]->DSI_PHY_LD0CON, L0_RM_TRIG_EN,
				      0);
			mdelay(1);
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LD0CON_REG,
				      DSI_REG[i]->DSI_PHY_LD0CON, Lx_ULPM_AS_L0,
				      1);
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LD0CON_REG,
...
int DSI_WaitVMDone(enum DISP_MODULE_ENUM module)
{
	...
	if (_dsi_is_video_mode(module)) {
		DDPMSG(
		"DSI WaitVMDone error: should set DSI to CMD mode firstly\n");
		return -1;
	}

	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		ret = wait_event_interruptible_timeout(
		    _dsi_wait_vm_done_queue[i], !(DSI_REG[i]->DSI_INTSTA.BUSY),
		    WAIT_TIMEOUT);
		if (ret == 0) {
			       ...
			       struct cmdqRecStruct *cmdq)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (i == 0) {
			...
			if (ret == 0) {
				DISPERR(
				    "dsi%d wait event for not busy timeout\n",
				    i);
				DSI_DumpRegisters(module, 1);
				DSI_Reset(module, NULL);
			} else if (ret < 0) {
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		while (1) {
			...
			if (count++ > 1000000000) {
				DISPERR(
				    "polling dsi%d wait for not busy timeout\n",
				    i);
				DSI_DumpRegisters(module, 1);
				DSI_Reset(module, NULL);
				break;
			   ...
			   struct cmdqRecStruct *cmdq)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (ret == 0) {
			...
			DISPERR(
			"dsi%d wait_sleepout is woken up by signal ret %d\n",
				i, ret);
			mdelay(5);
		}
	}
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (i == 1) {
			...
			while (1) {
				...
				if (cnt++ > 1000000000) {
					DISPERR(
					    "polling dsi%d sleepout timeout\n",
					    i);
					DSI_DumpRegisters(module, 1);
					DSI_Reset(module, NULL);
					break;
			...
			if (cnt)
				DISPDBG(
				    "%s:dsi%d polling sleepout bit %d times\n",
				    __func__, i, cnt);
		}
	}
				     ...
				     struct cmdqRecStruct *cmdq)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DDPMSG("DSI RestoreRegisters VM_CMD_EN %d TS_VFP_EN %d\n",
				      ...
				      unsigned int color)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enable) {
			...
			/* DSI_OUTREGBIT(struct
			 * DSI_START_REG,DSI_REG->DSI_START,DSI_START,0);
			 */
			DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_START, 0);
				DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_START, 1);
			/* DSI_OUTREGBIT(struct
			 * DSI_START_REG,DSI_REG->DSI_START,DSI_START,1);
			 */
			}
		} else {
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->mode == SYNC_EVENT_VDO_MODE ||
			...
			ASSERT(dsi_params->horizontal_sync_active *
			       ...
			       9);
			horizontal_sync_active_byte =
			    (dsi_params->horizontal_sync_active * dsiTmpBufBpp -
			     10);
...
unsigned int dsi_phy_get_clk(enum DISP_MODULE_ENUM module)
{
	...
	int j = 0;
	unsigned int pcw =
	    (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2.RG_DSI_MPPLL_SDM_PCW >> 24) &
	    0x7f;
	unsigned int prediv =
	    (1 << (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_PREDIV));
	unsigned int posdiv =
	    (1 << (DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON0.RG_DSI0_MPPLL_POSDIV));
	unsigned int S2Qdiv =
	    (1 << (DSI_PHY_REG[i]->MIPITX_DSI_PLL_TOP.RG_MPPLL_S2QDIV));

	DISPERR("%s, pcw: %d, prediv: %d, posdiv: %d, S2Qdiv: %d\n", __func__,
		pcw, prediv, posdiv, S2Qdiv);
			...
			struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (data_Rate != 0) {
			...
			/*  pcw = (data_rate/2) * 2^(preDIV+postDIV+S2Q) /
			 * (26*2*(2^24))
			 */
			...
			tmp =
			    ...
			    (((256 * (data_Rate * pcw_ratio % 26) / 26) & 0xFF)
			     << 16) |
			    (((256 * (256 * (data_Rate * pcw_ratio % 26) % 26) /
			       26) &
			      0xFF)
			     << 8) |
			    ((256 *
			...
			if (dsi_params->ssc_disable != 1) {
				DSI_OUTREGBIT(
				    cmdq, struct MIPITX_DSI_PLL_CON1_REG,
				...
				ASSERT(delta1 <= 8);
				pdelta1 =
				    (delta1 * data_Rate * pcw_ratio * 262144 +
				     281664) /
				    563329;
				DSI_OUTREGBIT(
				    cmdq, struct MIPITX_DSI_PLL_CON3_REG,
				...
				DISPINFO(
	"PLL config:data_rate=%d,pcw_ratio=%d,pcw=%d,delta1=%d,pdelta1=0x%x\n",
				    data_Rate, pcw_ratio,
				    DSI_INREG32(
			...
			DSI_OUTREGBIT(cmdq, struct MIPITX_DSI_PLL_CON0_REG,
				      ...
				      dsi_params->pll_posdiv);
			DSI_OUTREGBIT(cmdq, struct MIPITX_DSI_PLL_CON0_REG,
				      DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON0,
				      RG_DSI0_MPPLL_PREDIV,
				      dsi_params->pll_prediv);
			/*2. PLL PCW Config*/
			 ...
			 struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->lane_swap_en) {
			...
			MIPITX_OUTREGBIT(
			    ...
			    DSI_PHY_REG[i]->MIPITX_DSI_PHY_SEL,
			    MIPI_TX_PHY0_SEL,
			    dsi_params->lane_swap[i][MIPITX_PHY_LANE_0]);
			MIPITX_OUTREGBIT(
			    struct MIPITX_DSI_PHY_SEL_REG,
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DISPINFO(
		"mipi_tx%d: clk=0x%x,lan0=0x%x,lan1=0x%x,lan2=0x%x,lan3=0x%x\n",
	...
	"PLL config:data_rate=%d,pcw_ratio=%d,pcw=%d,delta1=%d,pdelta1=0x%x\n",
		...
		if (dsi_params->lane_swap_en) {
			if (dsi_params->lane_swap[i][MIPITX_PHY_LANE_CK] ==
			    MIPITX_PHY_LANE_CK)
				MIPITX_OUTREGBIT(
		       ...
		       struct LCM_DSI_PARAMS *dsi_params)
{
	...
	"[DISP] - kernel - DSI PHY_TIMCONFIG, Cycle Time = %d(ns), Unit Interval = %d(ns). , lane# = %d\n",
...
#define NS_TO_CYCLE(n, c) ((n) / (c))
	...
	DISP_LOG_PRINT(
	    ANDROID_LOG_INFO, "DSI",
	    "[DISP] - kernel - DSI PHY_TIMCONFIG, HS_TRAIL = %d, HS_ZERO = %d, HS_PRPR = %d, LPX = %d, TA_GET = %d, TA_SURE = %d, TA_GO = %d, CLK_TRAIL = %d, CLK_ZERO = %d, CLK_HS_PRPR = %d\n",
	...
	if (module != DISP_MODULE_DSIDUAL) {
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			DSI_OUTREGBIT(cmdq, struct DSI_START_REG,
				      DSI_REG[i]->DSI_START, DSI_START, 0);
			DSI_OUTREGBIT(cmdq, struct DSI_START_REG,
				      DSI_REG[i]->DSI_START, DSI_START, 1);
		}
	} else {
			       ...
			       UINT8 *buffer, UINT8 buffer_size)
{
		...
		do {
			...
			if (DSI_REG[d]->DSI_INTSTA.RD_RDY != 0 ||
				...
				/* dump cmdq & rxdata */
				{
					...
					DISPCHECK(
					"Last DSI Read Why not clear irq???\n");
					DISPCHECK(
					    "DSI CMDQ_SIZE  : %d\n",
					    AS_UINT32(
			...
			if (ret > 0) {
				...
				if (timeout == 1000) {
					...
					DISPERR(
				"DSI Read Fail: dsi wait cmd done timeout\n");
					DSI_DumpRegisters(module, 2);
					...
					DSI_OUTREGBIT(
					    cmdq, struct DSI_INT_ENABLE_REG,
			...
			"DSI Read Fail: dsi wait read ready timeout\n");
					...
					/* wake up by other interrupt, need try
					 * again???
					 */
					DDPERR(
	"DSI Read Fail: dsi wait read ready wake up by other interrupt\n");
			...
			"DSI read begin i = %d --------------------\n",
					  5 - max_try_count);
				DISPCHECK("DSI RX_STA     : 0x%08x\n",
					  AS_UINT32(&DSI_REG[d]->DSI_TRIG_STA));
				DISPCHECK(
				    "DSI CMDQ_SIZE  : %d\n",
				    AS_UINT32(&DSI_REG[d]->DSI_CMDQ_SIZE));
				for (i = 0;
				     i < DSI_REG[d]->DSI_CMDQ_SIZE.CMDQ_SIZE;
				     i++) {
					DISPCHECK(
					    "DSI CMDQ_DATA%d : 0x%08x\n", i,
					    AS_UINT32(
						&DSI_CMDQ_REG[d]->data[i]));
				}
				DISPCHECK("DSI RX_DATA0   : 0x%08x\n",
					  AS_UINT32(&DSI_REG[d]->DSI_RX_DATA0));
				DISPCHECK("DSI RX_DATA1   : 0x%08x\n",
					  AS_UINT32(&DSI_REG[d]->DSI_RX_DATA1));
				DISPCHECK("DSI RX_DATA2   : 0x%08x\n",
					  AS_UINT32(&DSI_REG[d]->DSI_RX_DATA2));
				DISPCHECK("DSI RX_DATA3   : 0x%08x\n",
					  AS_UINT32(&DSI_REG[d]->DSI_RX_DATA3));
				DISPCHECK(
				"DSI read end ----------------------------\n");
			}
		  ...
		  unsigned char force_update)
{
	...
	/* DISPCHECK("DSI set_cmdq, module=%s, cmdq=0x%08x\n",
		 ...
		 unsigned int y, unsigned int width, unsigned int height)
{
	...
	data_array[2] = (y1_LSB);
	DSI_set_cmdq(module, handle, data_array, 3, 1);
	DDPMSG("DSI Send_ROI(%d,%d,%d,%d)Done!\n", x, y, width, height);
...
/* DSI_OUTREG32(MMSYS_CONFIG_BASE+0xC08, 0xffffffff); */
	...
	if (MIPITX_IsEnabled(module, cmdq)) {
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
		     i++) {
			unsigned long mipi_tx_reg_base =
			    (unsigned long)DSI_PHY_REG[i];

			if (i == 0) { /* only enable DSI0's INT */
				DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG,
					      DSI_REG[i]->DSI_INTEN, CMD_DONE,
					      1);
				DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG,
					      DSI_REG[i]->DSI_INTEN, RD_RDY, 1);
				DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG,
					      DSI_REG[i]->DSI_INTEN, VM_DONE,
					      1);
			/* enable te_rdy when need, not here (both cmd
			 * mode & vdo mode)
			 */
			/* DSI_OUTREGBIT(NULL, struct
			 * DSI_INT_ENABLE_REG,DSI_REG[i]->DSI_INTEN,TE_RDY,1);
			 */
				DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG,
					      DSI_REG[i]->DSI_INTEN,
					      VM_CMD_DONE, 1);
			...
			clock_lane[i] =
			    (INREG32(mipi_tx_reg_base +
				     0x4)); /* MIPITX_DSI_CLOCK_LANE */
			data_lane0[i] =
			    (INREG32(mipi_tx_reg_base +
				     0x8)); /* MIPITX_DSI_DATA_LANE0 */
			data_lane1[i] =
			    (INREG32(mipi_tx_reg_base +
				     0xc)); /* MIPITX_DSI_DATA_LANE1 */
			data_lane2[i] =
			    (INREG32(mipi_tx_reg_base +
				     0x10)); /* MIPITX_DSI_DATA_LANE2 */
			data_lane3[i] =
			    (INREG32(mipi_tx_reg_base +
				     0x14)); /* MIPITX_DSI_DATA_LANE3 */
			DISPINFO(
		"mipi_tx%d: clk=0x%x,lan0=0x%x,lan1=0x%x,lan2=0x%x,lan3=0x%x\n",
				 i, clock_lane[i], data_lane0[i], data_lane1[i],
				 data_lane2[i], data_lane3[i]);
		}
	}
#endif
		   ...
		   struct disp_ddp_path_config *config, void *cmdq)
{
...
#endif
	{
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			/* enable te_rdy when need, not here (both cmd mode &
			 * vdo mode)
			 */
			/* DSI_OUTREGBIT(NULL, struct
			 * DSI_INT_ENABLE_REG,DSI_REG[i]->DSI_INTEN,TE_RDY,1);
	...
	/* DSI_OUTREGBIT(NULL, struct
	 * DSI_INT_ENABLE_REG,DSI_REG[i]->DSI_INTEN,FRAME_DONE_INT_EN,0);
	 */
			DSI_BackupRegisters(module, NULL);
		}
#endif
...
int ddp_dsi_start(enum DISP_MODULE_ENUM module, void *cmdq)
{
		...
		if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER)) {
			if (disp_helper_get_option(DISP_OPT_SHADOW_MODE) == 0) {
				/* full shadow mode */
				       ...
				       DISP_OPT_SHADOW_MODE) == 1) {
				/* force commit */
				DSI_OUTREGBIT(cmdq, struct DSI_SHADOW_DBG_REG,
					      DSI_REG[i]->DSI_SHADOW_DBG,
					      FORCE_COMMIT, 1);
			} else if (disp_helper_get_option(
			...
			/* read shadow */
			DSI_OUTREGBIT(cmdq, struct DSI_SHADOW_DBG_REG,
				      DSI_REG[i]->DSI_SHADOW_DBG, READ_WRK_REG,
				      0);
		}
	}
	...
	if (module == DISP_MODULE_DSIDUAL) {
		...
		/* must set DSI_START to 0 before set dsi_dual_en, don't know
		 * why.2014.02.15
		 */
		DSI_OUTREGBIT(cmdq, struct DSI_START_REG, DSI_REG[0]->DSI_START,
			      DSI_START, 0);
...
int ddp_dsi_switch_lcm_mode(enum DISP_MODULE_ENUM module, void *params)
{
	...
	if (dsi_currect_mode == mode) {
		DDPMSG(
		"[ddp _dsi_ switch _mode] not need switch mode, current mode = %d, switch to %d\n",
			...
			void *params)
{
	...
	if (mode == 0) { /* V2C */
		...
		DSI_MASKREG32(cmdq_handle,
		DISP_REG_CONFIG_MUTEX0_RST, 0x1, 0x1); /* reset mutex for V2C */
		DSI_MASKREG32(cmdq_handle,
		DISP_REG_CONFIG_MUTEX0_RST, 0x1, 0x0);
		DSI_MASKREG32(cmdq_handle,
		DISP_REG_CONFIG_MUTEX0_SOF, 0x7, 0x0); /* mutex to cmd  mode */
		if (disp_helper_get_option(DISP_OPT_MUTEX_EOF_EN_FOR_CMD_MODE))
			DSI_MASKREG32(cmdq_handle,
		DISP_REG_CONFIG_MUTEX0_SOF, 0x1c0, 0x40); /* eof */
		...
		DSI_POLLREG32(cmdq_handle, &DSI_REG[i]->DSI_INTSTA, 0x80000000,
			      0x0);
		DSI_OUTREGBIT(cmdq_handle, struct MIPITX_DSI_PLL_CON1_REG,
			      DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1,
			      RG_DSI0_MPPLL_SDM_SSC_EN, 1);
		...
		/*0x00001500 |(lcm_cmd.addr << 16)|(lcm_cmd.val[0]<<
		 * 24));
		 */
		  ...
		  unsigned int ioctl_cmd, unsigned long *params)
{
	...
	case DDP_BACK_LIGHT: {
		...
		DDPMSG("[ddp _dsi_ ioctl] level = %d\n", level);
...
int ddp_dsi_trigger(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	if (_dsi_context[i].dsi_params.mode == CMD_MODE) {
		...
		if (module == DISP_MODULE_DSIDUAL) {
			/*
			 * DSI1 is only used for triggering video data; thus
			 * pull up DSI_DUAL_EN,
			 * and pull down DSI_DUAL_EN after triggering video data
			 * is done.
			 */
			DSI_OUTREGBIT(cmdq, struct DSI_START_REG,
...
#endif
		if (is_ipoh_bootup) {
			pr_debug("IPOH bootup ??!! This feature is back again??!!\n");
			ASSERT(0);
		}
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		if (module == DISP_MODULE_DSI0 ||
			...
			if (disp_helper_get_option(DISP_OPT_USE_CMDQ)) {
				ret = cmdqRecClearEventToken(
				    cmdq_trigger_handle, CMDQ_EVENT_DSI_TE);
		...
		if (module == DISP_MODULE_DSI0 ||
			...
			DSI_POLLREG32(cmdq_trigger_handle,
				      &DSI_REG[dsi_i]->DSI_INTSTA, 0x80000000,
				      0);
		}
		...
		/* DSI_OUTREGBIT(cmdq_trigger_handle, struct
...
void PanelMaster_DSI_set_timing(UINT32 dsi_index, struct MIPI_TIMING timing)
{
	...
	case HS_ZERO:
		...
		/* OUTREGBIT(DSI_PHY_TIMCON0_REG,
		 * DSI_REG->DSI_PHY_TIMECON0,HS_ZERO,timing.value);
		 */
		break;
	case HS_TRAIL:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON0_REG,
		 * DSI_REG->DSI_PHY_TIMECON0,HS_TRAIL,timing.value);
		 */
		break;
	case TA_GO:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON1_REG,
		 * DSI_REG->DSI_PHY_TIMECON1,TA_GO,timing.value);
		 */
		break;
	case TA_SURE:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON1_REG,
		 * DSI_REG->DSI_PHY_TIMECON1,TA_SURE,timing.value);
		 */
		break;
	case TA_GET:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON1_REG,
		 * DSI_REG->DSI_PHY_TIMECON1,TA_GET,timing.value);
		 */
		break;
	case DA_HS_EXIT:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON1_REG,
		 * DSI_REG->DSI_PHY_TIMECON1,DA_HS_EXIT,timing.value);
		 */
		break;
	case CONT_DET:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON2_REG,
		 * DSI_REG->DSI_PHY_TIMECON2,CONT_DET,timing.value);
		 */
		break;
	case CLK_ZERO:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON2_REG,
		 * DSI_REG->DSI_PHY_TIMECON2,CLK_ZERO,timing.value);
		 */
		break;
	case CLK_TRAIL:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON2_REG,
		 * DSI_REG->DSI_PHY_TIMECON2,CLK_TRAIL,timing.value);
		 */
		break;
	case CLK_HS_PRPR:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON3_REG,
		 * DSI_REG->DSI_PHY_TIMECON3,CLK_HS_PRPR,timing.value);
		 */
		break;
	case CLK_HS_POST:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON3_REG,
		 * DSI_REG->DSI_PHY_TIMECON3,CLK_HS_POST,timing.value);
		 */
		break;
	case CLK_HS_EXIT:
		if (dsi_index == PM_DSI0) {
		...
		/* OUTREGBIT(DSI_PHY_TIMCON3_REG,
		 * DSI_REG->DSI_PHY_TIMECON3,CLK_HS_EXIT,timing.value);
		 */
		break;
	case HPW:
		if (!(dsi_params->mode == SYNC_EVENT_VDO_MODE ||
