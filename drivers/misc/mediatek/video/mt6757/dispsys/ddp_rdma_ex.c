...
void rdma_set_ultra_l(unsigned int idx, unsigned int bpp, void *handle, struct golden_setting_context *p_golden_setting)
{
	...
	if (!p_golden_setting) {
		DDPAEE("RDMA golden setting fail\n");
		return;
	}
	...
	Bytes_per_sec = bpp / 8;
	if (!Bytes_per_sec) {
		DDPAEE("byte per second is NULL!\n");
		return;
	}
...
void rdma_set_ultra_l(unsigned int idx, unsigned int bpp, void *handle, struct golden_setting_context *p_golden_setting)
{
	...
	if (!p_golden_setting) {
		DDPAEE("RDMA godlen setting fail in rmda_set_ultral_l\n");
		return;
	}
...
{
	unsigned int idx = rdma_index(module);
	unsigned int value = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 + DISP_RDMA_INDEX_OFFSET * idx);
