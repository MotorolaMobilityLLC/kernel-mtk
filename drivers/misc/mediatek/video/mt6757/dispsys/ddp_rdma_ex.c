...
int rdma_stop(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(
	    handle, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_INT_ENABLE, 0);
	DISP_REG_SET(
	    handle, idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_INT_STATUS, 0);
	return 0;
}
...
int rdma_reset(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	while ((DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPERR("rdma%d_reset timeout, stage 1! "
	...
	while ((DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPERR("rdma%d_reset timeout, stage 2! "
			       "DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
			       idx, DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
						 DISP_REG_RDMA_GLOBAL_CON));
			break;
		      ...
		      struct golden_setting_context *p_golden_setting)
{
	...
	if (idx == 0) {
		consume_rate =
		    ...
		    rdma_golden_setting->dst_height * frame_rate *
		    Bytes_per_sec;
		...
		consume_rate =
		    ...
		    rdma_golden_setting->ext_dst_height * frame_rate *
		    Bytes_per_sec;
		do_div(consume_rate, 1000);
	}
	consume_rate *= 1250;
	...
	issue_req_threshold =
	    min(fifo_valid_size - preultra_low, (unsigned int)255);

	/* output valid should < total rdma data size, or hang will happen */
	temp = (unsigned long long)rdma_golden_setting->rdma_width *
	...
	if (rdma_golden_setting->dst_width == 0 ||
	    rdma_golden_setting->dst_height == 0 || bpp == 0 ||
	    frame_rate == 0) {
		      ...
		      golden_setting_context *p_golden_setting)
{
	...
	if (is_large_resolution) {
		regval |=
		    REG_FLD_VAL(MEM_GMC_SETTING_1_FLD_ULTRA_THRESHOLD_LOW, 208);
		regval |= REG_FLD_VAL(
		    MEM_GMC_SETTING_1_FLD_ULTRA_THRESHOLD_HIGH, 312);
	} else {
		regval |=
		    REG_FLD_VAL(MEM_GMC_SETTING_1_FLD_ULTRA_THRESHOLD_LOW, 117);
		regval |= REG_FLD_VAL(
	...
	if (is_large_resolution) {
		...
		if (is_wrot_sram)
			regval |=
			    REG_FLD_VAL(RDMA_THRESHOLD_FOR_SODI_FLD_HIGH, 1742);
		...
		if (is_wrot_sram)
			regval |=
			    REG_FLD_VAL(RDMA_THRESHOLD_FOR_SODI_FLD_HIGH, 1715);
		else if (p_golden_setting->is_dc)
			regval |=
		       ...
		       unsigned int bpp)
{
	...
	DDPDBG("RDMAConfig idx %d, mode %d, address 0x%lx, inputformat %s, "
	       "pitch %u, width %u, height %u,sec%d\n",
	...
	if (input_is_yuv == 1 && output_is_yuv == 0) {
		...
		DISP_REG_SET_FIELD(
		    handle, SIZE_CON_0_FLD_MATRIX_ENABLE,
		...
		DISP_REG_SET_FIELD(
		    handle, SIZE_CON_0_FLD_MATRIX_INT_MTX_SEL,
		    idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_SIZE_CON_0, 0);
	}
...
void rdma_dump_reg(enum DISP_MODULE_ENUM module)
{
	if (disp_helper_get_option(DISP_OPT_REG_PARSER_RAW_DUMP)) {
		...
		unsigned long module_base =
		    DDP_REG_BASE_DISP_RDMA0 + idx * DISP_RDMA_INDEX_OFFSET;

		DDPDUMP("== START: DISP RDMA0 REGS ==\n");
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x000, INREG32(module_base + 0x000), 0x004,
			INREG32(module_base + 0x004), 0x010,
			INREG32(module_base + 0x010), 0x014,
			INREG32(module_base + 0x014));
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x018, INREG32(module_base + 0x018), 0x01c,
			INREG32(module_base + 0x01c), 0x024,
			INREG32(module_base + 0x024), 0x02C,
			INREG32(module_base + 0x02C));
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x030, INREG32(module_base + 0x030), 0x034,
			INREG32(module_base + 0x034), 0x038,
			INREG32(module_base + 0x038), 0x03C,
			INREG32(module_base + 0x03C));
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x040, INREG32(module_base + 0x040), 0x044,
			INREG32(module_base + 0x044), 0x054,
			INREG32(module_base + 0x054), 0x058,
			INREG32(module_base + 0x058));
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x05C, INREG32(module_base + 0x05C), 0x060,
		...
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			...
			INREG32(module_base + 0x070), 0x074,
			INREG32(module_base + 0x074), 0x078,
			INREG32(module_base + 0x078));
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x07C, INREG32(module_base + 0x07C), 0x080,
			INREG32(module_base + 0x080), 0x084,
			INREG32(module_base + 0x084), 0x088,
			INREG32(module_base + 0x088));
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x08C, INREG32(module_base + 0x08C), 0x090,
			INREG32(module_base + 0x090), 0x094,
			INREG32(module_base + 0x094), 0xF00,
			INREG32(module_base + 0xF00));
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x0a0, INREG32(module_base + 0x0a0), 0x0a4,
			INREG32(module_base + 0x0a4), 0x0a8,
			INREG32(module_base + 0x0a8), 0x0ac,
			INREG32(module_base + 0x0ac));
		DDPDUMP("RDMA0: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, "
			"0x%04x=0x%08x\n",
			0x0b0, INREG32(module_base + 0x0b0), 0x0b4,
			INREG32(module_base + 0x0b4), 0x0b8,
			INREG32(module_base + 0x0b8), 0x0c0,
			INREG32(module_base + 0x0c0));
		...
		DDPDUMP("(0x100)R_DBG_OUT=0x%x\n",
			DISP_REG_GET(DISP_REG_RDMA_DBG_OUT +
				     DISP_RDMA_INDEX_OFFSET * idx));
	}
}
...
void rdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	unsigned int global_ctrl = DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON +
						DISP_RDMA_INDEX_OFFSET * idx);
	unsigned int bg0 =
	    DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_BG_CON_0);
	unsigned int bg1 =
	    DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_BG_CON_1);

	DDPDUMP("== DISP RDMA%d ANALYSIS ==\n", idx);
	DDPDUMP("rdma%d: "
		"en=%d,memory_mode=%d,smi_busy=%d,w=%d,h=%d,pitch=%d,addr=0x%x,"
		"fmt=%s,fifo_min=%d,\n",
		...
		DISP_REG_GET(DISP_REG_RDMA_FIFO_LOG +
			     DISP_RDMA_INDEX_OFFSET * idx));
	DDPDUMP(
	    "in_p=%d,in_l=%d,out_p=%d,out_l=%d,bg(t%d,b%d,l%d,r%d),start=%lld "
	    "ns,end=%lld ns\n",
	    ...
	    REG_FLD_VAL_GET(RDMA_BG_CON_0_RIGHT, bg0), rdma_start_time[idx],
	    rdma_end_time[idx]);
			    ...
			    struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	DDPDBG("top=%d,bottom=%d,left=%d,right=%d,r.dst_x=%d,r.dst_y=%d,r.dst_"
	       "w=%d,r.dst_h=%d,width=%d,height=%d\n",
	       pConfig->rdma_config.bg_ctrl.top,
	       pConfig->rdma_config.bg_ctrl.bottom,
	       pConfig->rdma_config.bg_ctrl.left,
	       pConfig->rdma_config.bg_ctrl.right, r_config->dst_x,
	       r_config->dst_y, r_config->dst_w, r_config->dst_h, width,
	       height);
			  ...
			  struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (rdma_is_sec[rdma_idx] == 1) {
		...
		cmdqRecSetSecure(nonsec_switch_handle, 1);
		/*ugly work around by kzhang !!. will remove when cmdq delete
		 *disable scenario.
		 * To avoid translation fault like ovl (see notes in ovl.c)
		 *check the mode now, bypass the frame during DL->DC(), avoid
		 *hang when vdo mode.
		 */
		if (get_rdma_mode(module) == RDMA_MODE_MEMORY)
			do_rdma_config_l(module, pConfig, nonsec_switch_handle);
			  ...
			  struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (mode == RDMA_MODE_MEMORY) {
		if (is_engine_sec == 1)
			...
			ret = rdma_switch_to_nonsec(
			    module, pConfig,
			    NULL); /*hadle = NULL, use the sync flush method*/
		if (ret)
			   ...
			   enum CMDQ_STATE state)
{
	...
	if (state == CMDQ_RESET_AFTER_STREAM_EOF) {
		/* if rdma frame done with underflow, rdma will hold dvfs
		 * request forever */
