...
static void dprec_to_mmp(unsigned int type_logsrc, enum mmp_log_type mmp_log,
			 unsigned int data1, unsigned data2)
{
...
void dprec_event_op(enum DPREC_EVENT event)
{
	...
	len +=
	    scnprintf(dprec_string_buffer + len, dprec_string_max_length - len,
		      "[%s]", _get_event_string(event));
	len += scnprintf(dprec_string_buffer + len,
			  ...
			  unsigned int val2)
{
	...
	if (source == DPREC_LOGGER_DSI_EXT_TE) {
		 ...
		 * max=%llu,min=%llu,start=%llu,trigger=%llu\n", l->count,*/
		/* time, l->period_frame, l->period_max_frame,
...
{
	if (p) {
		/* scnprintf(p->name, ARRAY_SIZE(p->name) / sizeof(p->name[0]),
		 * name); */
...
static void mmp_kernel_trace_end(void) { DISP_SYSTRACE_END(); }

void dprec_logger_frame_seq_begin(unsigned int session_id,
				  unsigned frm_sequence)
{
...
unsigned long disp_get_tracing_mark(void) { return 0UL; }
	...
	if (event) {
		...
		if (event->level & DPREC_LOGGER_LEVEL_SYSTRACE &&
			...
			scnprintf(name, ARRAY_SIZE(name) / sizeof(name[0]),
				  "K_%s_0x%x_0x%x", event->name, val1, val2);
				   ...
				   char *stringbuf, int strlen)
{
	...
	len += scnprintf(
	    stringbuf + len, strlen - len, "|%-24s|%8llu |%8lld.%02ld "
					   "|%8llu.%02ld |%8llu.%02ld "
					   "|%8llu.%02ld |\n",
...
int dprec_logger_get_result_string_all(char *stringbuf, int strlen)
{
	...
	n +=
	    scnprintf(stringbuf + n, strlen - n,
...
unsigned int dprec_get_vsync_count(void) { return vsync_cnt; }
		  ...
		  unsigned int mask)
{
	...
	len +=
	    scnprintf(dprec_string_buffer + len, dprec_string_max_length - len,
		      "[%s]", _find_module_by_reg_addr(reg));
	len +=
	    scnprintf(dprec_string_buffer + len, dprec_string_max_length - len,
		      "[%s]", cmdq ? "CMDQ" : "CPU");
	...
	len +=
	    scnprintf(dprec_string_buffer + len, dprec_string_max_length - len,
		      "0x%08x=0x%08x", reg, val);

	if (mask)
		len +=
		    scnprintf(dprec_string_buffer + len,
	...
	if (_control.cmm_dump) {
		/*pr_debug("[CMM]D.S SD:0x%08x %\LE %\LONG 0x%08x; write
		 * %s\n",*/
...
void dprec_logger_dump(char *string) { dprec_logger_vdump(string); }
...
char *dprec_logger_get_dump_addr() { return dprec_string_buffer_analysize; }
...
/* return true if overall_switch is set. this will dump all register setting by
 * default. */
/* other functions outside of this display_recorder.c*/
/* could use this api to determine whether to enable debug funciton */
int dprec_option_enabled(void) { return _control.overall_switch; }
...
char *get_dprec_status_ptr(int buffer_idx)
{
	if (buffer_idx < dprec_logger_buffer[DPREC_LOGGER_STATUS].count)
		return dprec_logger_buffer[DPREC_LOGGER_STATUS]
...
char *dprec_logger_get_dump_addr() { return NULL; }

unsigned int dprec_logger_get_dump_len(void) { return 0; }

int dprec_handle_option(unsigned int option) { return 0; }

int dprec_option_enabled(void) { return 0; }
...
char *get_dprec_status_ptr(int buffer_idx) { return NULL; }
char *debug_buffer;
bool is_buffer_init;
void init_log_buffer(void) {}
#endif
