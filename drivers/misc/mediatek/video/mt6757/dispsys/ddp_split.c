...
{
	DISP_REG_SET_FIELD(handle, DEBUG_FLD_PIXEL_CNT_EN, DISP_REG_SPLIT_DEBUG,
			   1);
...
static int split_config(enum DISP_MODULE_ENUM module,
			struct disp_ddp_path_config *pConfig, void *handle)
{
	DISP_REG_SET(handle, DISP_REG_SPLIT_CFG,
		     0x00); /* 0: even pixel first, 1: odd pexel first */
...
int split_reset(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	while ((DISP_REG_GET_FIELD(DEBUG_FLD_SPLIT_FSM, DISP_REG_SPLIT_DEBUG) &
		0x3) == 0) {
		delay_cnt++;
		udelay(10);
		if (delay_cnt > 2000) {
