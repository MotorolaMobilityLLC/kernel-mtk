...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "set_reg:", 8) == 0) {
	...
	} else if (strncmp(opt, "get_reg:", 8) == 0) {
		char *p = (char *)opt + 8;

		ret = kstrtoul(p, 16, &addr);

		SMIMSG("get register: 0x%lx = 0x%x\n", addr, COM_ReadReg32(addr));
	} else if (strncmp(opt, "set_pa:", 7) == 0) {
		unsigned long val = 0;
		...
		iounmap((void *)reg_va);
	} else if (strncmp(opt, "get_pa:", 7) == 0) {
		char *p = (char *)opt + 7;
		unsigned long reg_va;
		...
		iounmap((void *)reg_va);
	} else if (strncmp(opt, "always_on_clk:", 14) == 0) {
		unsigned long val = 0;
		char *p = (char *)opt + 14;
