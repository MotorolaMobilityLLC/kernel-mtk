	...
	if (mmdvfs_step	== current_step) {
		...
		<	current_step)))	{
			if (self->apply_vcore_hw_configurtion_by_step) {
				...
				MMDVFSDEBUG(3,
					"apply_vcore_hw_configurtion_by_step	is NULL. No	vocre	change.\n");
			}
			...
			if (self->apply_clk_hw_configurtion_by_step) {
				...
				MMDVFSDEBUG(3,
					"apply_clk_hw_configurtion_by_step is NULL. No	clk	change.\n");
			}
		}	else {
			if (self->apply_clk_hw_configurtion_by_step) {
				MMDVFSDEBUG(3, "Apply	CLK	setting:\n");
...
void mmdvfs_pm_qos_update_request(struct mmdvfs_pm_qos_request *req, int mmdvfs_pm_qos_class,
int	new_value)
{
	...
	step =
		g_mmdvfs_thresholds_dvfs_handler->get_step(g_mmdvfs_thresholds_dvfs_handler,
