...
static struct pm_qos_request smi_freq_request;
...
static s32 comm_port_limit[MAX_LARB_COUNT] = {};
static s32 comm_port_hrt[MAX_LARB_COUNT] = {};
static s32 force_comm_bwl[MAX_LARB_COUNT] = {};
...
void mm_qos_update_larb_bwl(u32 larb_update, bool bw_change)
{
	...
	s32 freq = mmdvfs_qos_get_freq(PM_QOS_DISP_FREQ);
	...
	for (i = 0; i < SMI_COMM_MASTER_NR; i++) {
		if (!(larb_update & (1 << i)))
			continue;

		larb_bw = 0;
		if (force_comm_bwl[i] != 0) {
			larb_bw = force_comm_bwl[i];
			if (log_level & 1 << log_bw)
				pr_notice("force LARB(%d) BWL with 0x%x\n",
				i, larb_bw);
		} else if (comm_port_limit[i]) {
			larb_bw = (comm_port_limit[i] << 8) / freq;
			if (log_level & 1 << log_bw)
				pr_notice("config LARB(%d) with 0x%x bw:%u\n",
				i, larb_bw, comm_port_limit[i]);
		}
		if (larb_bw) {
			smi_bwl_update(i, larb_bw,
...
static s32 channel_bw[MAX_CH_COUNT] = {};
static s32 channel_hrt_bw[MAX_CH_COUNT] = {};
static s32 channel_disp_hrt_cnt[MAX_CH_COUNT] = {};
...
static bool is_camera_larb(u32 master_id)
{
	...
	for (i = 0; i < cam_larb_size; i++) {
		if (SMI_LARB_ID_GET(master_id) == cam_larb_ids[i]) {
			result = true;
			break;
		}
	}
...
static s32 get_total_used_hrt_bw(void)
{
	...
	s32 disp_hrt_bw =
		larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_DISP)].total_hrt_data;
	s32 md_hrt_bw =
		larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_MD)].total_hrt_data;
...
static void log_hrt_bw_info(u32 master_id)
{
...
#ifdef MMDVFS_MMP
	u32 param1 = (SMI_LARB_ID_GET(master_id) << 24) |
	...
	if (log_level & 1 << log_bw)
		pr_notice("%s larb=%d p1=%d ccu=%d disp=%d ddr_opp=%d\n",
			__func__, SMI_LARB_ID_GET(master_id), p1_hrt_bw,
...
#ifdef DVFSRC_HRT
	u32 md_larb_id = SMI_LARB_ID_GET(PORT_VIRTUAL_MD);
	s32 mm_used_hrt_bw =
	...
	struct mm_qos_request *req, u32 smi_master_id)
{
	...
	larb_id = SMI_LARB_ID_GET(smi_master_id);
	port_id = smi_master_id & SMI_MASTER_PORT_MASK;
	if (!req) {
	...
	u32 hrt_value, u32 comp_type)
{
	...
	larb = SMI_LARB_ID_GET(req->master_id);
	port = req->master_id & SMI_MASTER_PORT_MASK;
	if (!req->init ||
	...
	if (req->hrt_value) {
		...
		if (larb < MAX_LARB_COUNT &&
			larb_req[larb].comm_port < SMI_COMM_MASTER_NR)
			comm_port_hrt[larb_req[larb].comm_port] -=
				req->hrt_value;
		if (larb < MAX_LARB_COUNT &&
			larb_req[larb].channel < MAX_CH_COUNT) {
			if (is_disp_larb(larb))
				channel_disp_hrt_cnt[larb_req[larb].channel]--;
			else
				channel_hrt_bw[larb_req[
	...
	if (hrt_value) {
		...
		if (larb < MAX_LARB_COUNT &&
			larb_req[larb].comm_port < SMI_COMM_MASTER_NR)
			comm_port_hrt[larb_req[larb].comm_port] += hrt_value;
		if (larb < MAX_LARB_COUNT &&
			larb_req[larb].channel < MAX_CH_COUNT) {
			if (is_disp_larb(larb))
				channel_disp_hrt_cnt[larb_req[larb].channel]++;
				...
				channel_hrt_bw[
					larb_req[larb].channel] += hrt_value;
		}
	} else
		larb_req[larb].total_mix_limit += new_comp_limit;

	if (larb < MAX_LARB_COUNT && larb_req[larb].channel < MAX_CH_COUNT) {
		channel_bw[larb_req[larb].channel] -= old_larb_mix_value;
		channel_bw[larb_req[larb].channel] +=
			larb_req[larb].total_mix_limit;
	}
	if (larb < MAX_LARB_COUNT &&
		larb_req[larb].comm_port < SMI_COMM_MASTER_NR) {
		comm_port_limit[larb_req[larb].comm_port] -= old_larb_mix_value;
		comm_port_limit[larb_req[larb].comm_port] +=
			larb_req[larb].total_mix_limit;
	}
	...
	mmdvfs_update_plat_ostd(larb, hrt_port, &req->ostd);
	list_for_each_entry(enum_req, &(req->port_node), port_node)
...
void mm_qos_update_all_request(struct plist_head *owner_list)
{
	...
	if (is_camera_larb(req->master_id)) {
		...
		if (cam_bw > camera_max_bw) {
			pr_notice("cam_bw(%d) > camera_max_bw(%d) error\n",
			...
			aee_kernel_warning("mmdvfs",
				"cam_bw(%d) > camera_max_bw(%d) error\n",
				cam_bw, camera_max_bw);
#endif
		}
		if (cam_scen_change) {
		...
		if (atomic_read(&lock_cam_count) > 0)
			blocking_camera();
#endif
		if (get_total_used_hrt_bw() > total_hrt_bw)
	...
	plist_for_each_entry(req, owner_list, owner_node) {
		if (!req->updated)
			continue;
		i++;
		larb_id = SMI_LARB_ID_GET(req->master_id);
		port_id = req->master_id & SMI_MASTER_PORT_MASK;
		larb_update |= 1 << larb_req[larb_id].comm_port;
		if (log_level & 1 << log_bw)
	...
	for (i = 0; i < MAX_CH_COUNT; i++) {
		final_chn_hrt_bw[i] = channel_disp_hrt_cnt[i] > 0 ?
			channel_hrt_bw[i] + larb_req[SMI_LARB_ID_GET(
	...
	for (i = 0; i < MAX_CH_COUNT; i++) {
		if (log_level & 1 << log_smi_freq)
			pr_notice("channel:%d srt_bw:%d hrt_bw:%d\n",
				i, channel_bw[i], final_chn_hrt_bw[i]);
#ifdef MMDVFS_MMP
		mmprofile_log_ex(
			mmdvfs_mmp_events.smi_freq,
			MMPROFILE_FLAG_PULSE,
			((i+1) << 28) | min_t(s32, channel_bw[i], 0xffff),
			((i+1) << 28) | min_t(s32,
					final_chn_hrt_bw[i], 0xffff));
#endif
	}
	...
	for (i = 0; i < MAX_LARB_COUNT; i++)
		mm_bw += (larb_req[i].comm_port != SMI_COMM_MASTER_NR) ?
...
s32 mm_hrt_get_available_hrt_bw(u32 master_id)
{
	...
	s32 src_hrt_bw = larb_req[SMI_LARB_ID_GET(master_id)].total_hrt_data;
	s32 cam_bw;
	s32 result;
	...
	if (SMI_LARB_ID_GET(master_id) ==
			SMI_LARB_ID_GET(PORT_VIRTUAL_DISP)) {
...
static s32 get_total_hrt_bw(void)
{
	...
	s32 result = MULTIPLY_BW_THRESH_HIGH(max_freq * ch_num * io_width);
	...
	const char *name, struct mm_freq_step_config *step_config)
{
	...
	if (likely(!result)) {
		...
		pr_notice("read freq steps %s not found (%d)\n", name, result);
	}
}
...
{
	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_DISP)].port_count = 1;
	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_DISP)].ratio[0] = 1;
	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_DISP)].channel = MAX_CH_COUNT;
	larb_req[SMI_LARB_ID_GET(
		PORT_VIRTUAL_DISP)].comm_port = SMI_COMM_MASTER_NR;
	init_larb_list(SMI_LARB_ID_GET(PORT_VIRTUAL_DISP));

	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_CCU_COMMON)].port_count = 1;
	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_CCU_COMMON)].ratio[0] = 1;
	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_CCU_COMMON)].channel =
		SMI_BUS_SEL_MASTER[mmdvfs_get_ccu_smi_common_port()];
	larb_req[SMI_LARB_ID_GET(
		PORT_VIRTUAL_CCU_COMMON)].comm_port =
		mmdvfs_get_ccu_smi_common_port();
	init_larb_list(SMI_LARB_ID_GET(PORT_VIRTUAL_CCU_COMMON));

	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_MD)].port_count = 1;
	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_MD)].ratio[0] = 1;
	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_MD)].channel = MAX_CH_COUNT;
	larb_req[SMI_LARB_ID_GET(
		PORT_VIRTUAL_MD)].comm_port = SMI_COMM_MASTER_NR;
	init_larb_list(SMI_LARB_ID_GET(PORT_VIRTUAL_MD));
	larb_req[SMI_LARB_ID_GET(PORT_VIRTUAL_MD)].total_hrt_data = 1600;
}
	...
	const char *freq_name, struct mm_freq_step_config step_configs[])
{
	...
	of_property_for_each_string(dev->of_node, freq_name, prop, name) {
		...
		plat_name = mmdvfs_get_plat_name();
		if (plat_name) {
			strncpy(ext_name, name, sizeof(ext_name)-1);
			strncat(ext_name, plat_name,
				sizeof(ext_name)-strlen(name)-1);
			mmdvfs_get_step_node(dev,
				ext_name, &step_configs[count]);
		}
	...
	struct mm_freq_limit_config *limit_config)
{
	...
	limit_config->limit_steps = kzalloc(
		sizeof(*limit_config->limit_steps) * limit_size,
		GFP_KERNEL);
	for (i = 0; i < limit_size; i++) {
		limit_config->limit_steps[i] = kzalloc(
			sizeof(*limit_config->limit_steps[i]) * MAX_FREQ_STEP,
			GFP_KERNEL);
...
static int mmdvfs_probe(struct platform_device *pdev)
{
	...
	pm_qos_add_request(&smi_freq_request, PM_QOS_DISP_FREQ,
	...
	if (SMI_LARB_MMDVFS_NR != 0)
		init_virtual_larbs();

	for (i = 0; i < SMI_LARB_MMDVFS_NR; i++) {
		value = SMI_L1ARB_LARB[i];
		larb_req[i].comm_port = value;
		if (value != SMI_COMM_MASTER_NR)
			larb_req[i].channel =
				SMI_BUS_SEL_MASTER[value & 0xffff];
...
static int mmdvfs_remove(struct platform_device *pdev)
{
	...
	pm_qos_remove_request(&smi_freq_request);
...
static int make_cam_hrt_bw(void *data)
{
	...
	mm_qos_add_request(&cam_req_list,
		&cam_req, SMI_MASTER_ID(cam_larb_ids[0], 0));
...
int mmdvfs_qos_ut_set(const char *val, const struct kernel_param *kp)
{
	...
	/* smi_debug_bus_hang_detect(0x1ff, true, false, false); */
...
int set_force_bwl(const char *val, const struct kernel_param *kp)
{
	...
	result = sscanf(val, "%d %d", &port, &bwl);
	if (result != 2) {
		pr_notice("invalid input: %s, result(%d)\n", val, result);
		return -EINVAL;
	}

	if (port >= 0 && port < MAX_LARB_COUNT)
		force_comm_bwl[port] = bwl;
