...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*///wt add for gs cali start
static int BMA2XX_remap_to_gravity(struct i2c_client *client, int data[BMA2XX_AXES_NUM]);
...
/*----------------------------------------------------------------------------*/
/*--------------------------------wt add for gs cali start--------------------------------------------*/
static int BMA2XX_WriteCalibration_New(struct i2c_client *client, struct GS_CALI_DATA_STRUCT *data_cali)
{
...
static int BMA2XX_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		//wt add for gs cali start
        if(gs_cali.valid == 1){
            acc[BMA2XX_AXIS_X] -= gs_cali.x;
            acc[BMA2XX_AXIS_Y] -= gs_cali.y;
            acc[BMA2XX_AXIS_Z] -= gs_cali.z;
        }
        //wt add for gs cali end
...
static int BMA2XX_remap_to_gravity(struct i2c_client *client, int data[BMA2XX_AXES_NUM])
{
	...
	/*remap coordinate*/
	acc_tmp[obj->cvt.map[BMA2XX_AXIS_X]] = obj->cvt.sign[BMA2XX_AXIS_X]* data[BMA2XX_AXIS_X];
	acc_tmp[obj->cvt.map[BMA2XX_AXIS_Y]] = obj->cvt.sign[BMA2XX_AXIS_Y]* data[BMA2XX_AXIS_Y];
	acc_tmp[obj->cvt.map[BMA2XX_AXIS_Z]] = obj->cvt.sign[BMA2XX_AXIS_Z]* data[BMA2XX_AXIS_Z];

	//Out put the mg
	data[BMA2XX_AXIS_X] = acc_tmp[BMA2XX_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMA2XX_AXIS_Y] = acc_tmp[BMA2XX_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMA2XX_AXIS_Z] = acc_tmp[BMA2XX_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
...

{
	...
	/*-----wt add for gs cali start ---*/
	int acc_tmp[BMA2XX_AXES_NUM] = {0};
	s16 raw_data[BMA2XX_AXES_NUM] = {0};
	uint32_t enable;
	...
	switch(cmd)
	{
		...
		case GSENSOR_SET_ACC_MODE:
			...
			if(enable == 1)
				err = BMA2XX_SetPowerMode(client,true);
			else
				err = BMA2XX_SetPowerMode(client,false);
			if (err)
				GSE_ERR("set power mode failed, err = %ld\n", err);
			break;
		/*------------------------------wt add for gs cali end----------------------------------------------*/
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			...
			/*------------------------------wt add for gs cali start----------------------------------------------*/
			BMA2XX_ReadData(client, raw_data);
			acc_tmp[0] = (int)raw_data[0];
			acc_tmp[1] = (int)raw_data[1];
			acc_tmp[2] = (int)raw_data[2];
			err = BMA2XX_remap_to_gravity(client, acc_tmp);
			if(err){
				err = -EFAULT;
				break;
			}
		...
		case GSENSOR_IOCTL_SET_CALI_NEW:
			...
			{/* get remapped value in NV (mg) */
				err = BMA2XX_WriteCalibration_New(client, &gs_cali_temp);
				if(err){
					err = -EFAULT;
					break;
				}
		...
		unsigned long arg)
{
	...
	void __user *arg32 = (void __user *)(unsigned long)(arg);
	...
	switch (cmd)
	{
		...
		case COMPAT_GSENSOR_IOCTL_SET_CALI:
			...
			break;
	    case COMPAT_GSENSOR_IOCTL_SET_CALI_NEW:
			if (arg32 == NULL) {
				err = -EINVAL;
				break;
			}
			err =
				file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_SET_CALI_NEW, (unsigned long)arg32);
			if (err) {
				GSE_ERR("GSENSOR_IOCTL_SET_CALI unlocked_ioctl failed.");
				return err;
			}
...
//	mdelay(1200);
//	gsensor_init_flag = -3;
	GSE_ERR(" huangzifan invalid direction: bma2xx_i2c_probe0\n");
...
//	gsensor_init_flag = -4;
	...
	GSE_ERR(" huangzifan invalid direction: bma2xx_i2c_probe1\n");
...
//	gsensor_init_flag = -5;
	...
	GSE_ERR("invalid direction: bma2xx_i2c_probe2\n");
...
//	gsensor_init_flag = -6;
	...
	GSE_ERR("invalid direction: bma2xx_i2c_probe3\n");
	...
	GSE_ERR("invalid direction: bma2xx_i2c_probe4\n");
	...
	GSE_ERR("invalid direction: bma2xx_i2c_probe5\n");
	...
	GSE_ERR("invalid direction: bma2xx_i2c_probe6\n");
	...
	gsensor_init_flag =0;
		...
		hardwareinfo_set_prop(HARDWARE_ACCELEROMETER, "BMA2xx");
	}
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
...
{
	GSE_FUN();
	GSE_ERR("huangzifan gsensor_local_init\n");
	BMA2XX_power(hw, 1);
	if(i2c_add_driver(&bma2xx_i2c_driver))
	{
		GSE_ERR("huangzifan add driver error\n");
		return -1;
	}
	GSE_ERR("huangzifan gsensor_local_init22, gsensor_init_flag =%d!!\n",gsensor_init_flag);	
...
static int __init bma2xx_init(void)
{
	...
	if (!hw){
		GSE_ERR("huangzifan get dts info fail\n");
		return -ENODEV;
	}
	GSE_LOG("%s: huangzifan i2c_number=%d\n", __func__,hw->i2c_num);
	acc_driver_add(&bma2xx_init_info);
		GSE_LOG(" huangzifan i2c_number333\n");
	return 0;    
}
