...
/*----------------------------------------------------------------------------*/
static struct i2c_client *bma2xx_i2c_client;
static struct bma2xx_i2c_data *obj_i2c_data;
...
static int BMA2XX_SetDataResolution(struct bma2xx_i2c_data *obj)
{
	...
	*/
	...
	/*
	* so bma2xx_data_resolution[0] set value as {{ 3, 9}, 256} when declaration,
	* and assign the value to obj->reso here
	*/
...
static int BMA2XX_ReadData(struct i2c_client *client, s16 data[BMA2XX_AXES_NUM])
{
	...
	if (client == NULL) {
		err = -EINVAL;
	} else if (bma_i2c_read_block(client, addr, buf, 6)) {
		GSE_ERR("error: %d\n", err);
	} else {
			...
			if (atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend)) {
				...
				if (priv->fir.num < firlen) {
					...
					if (atomic_read(&priv->trace) & ADX_TRC_FILTER) {
						...
						priv->fir.raw[priv->fir.num][BMA2XX_AXIS_X],
						priv->fir.raw[priv->fir.num][BMA2XX_AXIS_Y],
						priv->fir.raw[priv->fir.num][BMA2XX_AXIS_Z],
						priv->fir.sum[BMA2XX_AXIS_X], priv->fir.sum[BMA2XX_AXIS_Y],
						priv->fir.sum[BMA2XX_AXIS_Z]);
					}
					priv->fir.num++;
					priv->fir.idx++;
				} else {
					...
					if (atomic_read(&priv->trace) & ADX_TRC_FILTER) {
						...
						idx, priv->fir.raw[idx][BMA2XX_AXIS_X],
						priv->fir.raw[idx][BMA2XX_AXIS_Y],
						priv->fir.raw[idx][BMA2XX_AXIS_Z], priv->fir.sum[BMA2XX_AXIS_X],
						priv->fir.sum[BMA2XX_AXIS_Y], priv->fir.sum[BMA2XX_AXIS_Z],
						data[BMA2XX_AXIS_X], data[BMA2XX_AXIS_Y], data[BMA2XX_AXIS_Z]);
					}
				}
			}
...
#else
	err = bma_i2c_read_block(client, BMA2XX_REG_OFSX, ofs, BMA2XX_AXES_NUM);
	if (err)
		GSE_ERR("error: %d\n", err);
#endif
...
static int BMA2XX_ReadCalibration(struct i2c_client *client, int dat[BMA2XX_AXES_NUM])
{
...
#else
	err = BMA2XX_ReadOffset(client, obj->offset);
	if (err) {
	...
	dat[obj->cvt.map[BMA2XX_AXIS_X]] =
	obj->cvt.sign[BMA2XX_AXIS_X]*(obj->offset[BMA2XX_AXIS_X]*mul + obj->cali_sw[BMA2XX_AXIS_X]);
	dat[obj->cvt.map[BMA2XX_AXIS_Y]] =
	obj->cvt.sign[BMA2XX_AXIS_Y]*(obj->offset[BMA2XX_AXIS_Y]*mul + obj->cali_sw[BMA2XX_AXIS_Y]);
	dat[obj->cvt.map[BMA2XX_AXIS_Z]] =
	obj->cvt.sign[BMA2XX_AXIS_Z]*(obj->offset[BMA2XX_AXIS_Z]*mul + obj->cali_sw[BMA2XX_AXIS_Z]);
...
static int BMA2XX_WriteCalibration(struct i2c_client *client, int dat[BMA2XX_AXES_NUM])
{
	...
	err = BMA2XX_ReadCalibrationEx(client, cali, raw);
	if (err != 0)	/*offset will be updated in obj->offset*/ {
		GSE_ERR("read offset fail, %d\n", err);
		return err;
	}
	...
	GSE_LOG("NEWOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n",
			...
			obj->offset[BMA2XX_AXIS_X], obj->offset[BMA2XX_AXIS_Y], obj->offset[BMA2XX_AXIS_Z],
			obj->cali_sw[BMA2XX_AXIS_X], obj->cali_sw[BMA2XX_AXIS_Y], obj->cali_sw[BMA2XX_AXIS_Z]);
	err = hwmsen_write_block(obj->client, BMA2XX_REG_OFSX, obj->offset, BMA2XX_AXES_NUM);
	if (err) {
...
static int BMA2XX_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	res = BMA2XX_ReadData(client, obj->data);
	if (res != 0) {
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
	...
	if (err != 0)
		return -EINVAL;
	err = BMA2XX_ReadCalibration(client, tmp);
	if (err != 0)
		return -EINVAL;
...
static ssize_t store_cali_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "rst", 3)) {
		...
		dat[BMA2XX_AXIS_Y] = y;
		dat[BMA2XX_AXIS_Z] = z;
		err = BMA2XX_WriteCalibration(client, dat);
		if (err != 0)
...
static ssize_t show_firlen_value(struct device_driver *ddri, char *buf)
{
	...
	if (atomic_read(&obj->firlen)) {
		...
		for (idx = 0; idx < len; idx++)
			GSE_LOG("[%5d %5d %5d]\n", obj->fir.raw[idx][BMA2XX_AXIS_X],
			obj->fir.raw[idx][BMA2XX_AXIS_Y], obj->fir.raw[idx][BMA2XX_AXIS_Z]);


		GSE_LOG("sum = [%5d %5d %5d]\n", obj->fir.sum[BMA2XX_AXIS_X],
		obj->fir.sum[BMA2XX_AXIS_Y], obj->fir.sum[BMA2XX_AXIS_Z]);
		GSE_LOG("avg = [%5d %5d %5d]\n", obj->fir.sum[BMA2XX_AXIS_X]/len,
		obj->fir.sum[BMA2XX_AXIS_Y]/len, obj->fir.sum[BMA2XX_AXIS_Z]/len);
	}
...
{
	int idx, err = 0;
	int num = (int)(ARRAY_SIZE(bma2xx_attr_list));

	if (driver == NULL)
		return -EINVAL;

	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, bma2xx_attr_list[idx]);
		if (err != 0) {
...
static void gsensor_irq_work(struct work_struct *work)
{
	...
	max_cust_data_size_per_packet = sizeof(data.set_cust_req.custData) - offsetof(struct BMA2XX_SET_CUST, data);

	GSE_ERR("sizeOfCustData = %d, max_cust_data_size_per_packet = %d\n",
	sizeOfCustData, max_cust_data_size_per_packet);
	GSE_ERR("offset %d\n", offsetof(struct BMA2XX_SET_CUST, data));
	...
	for (i = 0; sizeOfCustData > 0; i++) {
		...
		len += offsetof(SCP_SENSOR_HUB_SET_CUST_REQ, custData) + offsetof(struct BMA2XX_SET_CUST, data);
...

{
	...
	case GSENSOR_IOCTL_GET_CALI:
			...
			err = BMA2XX_ReadCalibration(client, cali);
			if (err != 0)
				break;
...
#endif
/*----------------------------------------------------------------------------*/
static const struct file_operations bma2xx_fops = {
	.owner = THIS_MODULE,
	.open = bma2xx_open,
...
#ifdef CUSTOM_KERNEL_SENSORHUB
		err = BMA2XX_SCP_SetPowerMode(false, ID_ACCELEROMETER);
#else
...
#ifndef CUSTOM_KERNEL_SENSORHUB
	err = bma2xx_init_client(client, 0);
#else
...
static int bma2xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for (retry = 0; retry < 3; retry++) {
		err = bma2xx_init_client(new_client, 1);
		if (err != 0) {
	...
	err = misc_register(&bma2xx_device);
	if (err != 0) {
		GSE_ERR("bma2xx_device register failed\n");
		goto exit_misc_device_register_failed;
	}
	err = bma2xx_create_attr(&bma2xx_init_info.platform_diver_addr->driver);
	if (err != 0) {
...
static int bma2xx_i2c_remove(struct i2c_client *client)
{
	...
	err = bma2xx_delete_attr(&bma2xx_init_info.platform_diver_addr->driver);
	if (err != 0)
