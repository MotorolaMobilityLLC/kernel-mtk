...
/* pinctrl-names from dtsi.
 *GPIO operations: rx set, rx clear, tx set, tx clear
 */
...
static void save_rx_raw_data(struct mtk_uart *uart,
...
static struct mtk_uart_setting mtk_uart_default_settings[] = {
	{
	  // .tx_mode = UART_NON_DMA,
	  //.rx_mode = UART_RX_VFIFO_DMA,
	  //.dma_mode = UART_DMA_MODE_0,
	 .tx_mode = UART_TX_VFIFO_DMA,
	 .rx_mode = UART_RX_VFIFO_DMA,
	 .dma_mode = UART_DMA_MODE_0,
	 // .tx_mode = UART_NON_DMA,
	 //.rx_mode = UART_NON_DMA,
	 //.dma_mode = UART_DMA_MODE_0,
	 .tx_trig = UART_FCR_TXFIFO_1B_TRI,
	 .rx_trig = UART_FCR_RXFIFO_12B_TRI,

	 //.uart_base = AP_UART0_BASE,
	 //.irq_num = UART0_IRQ_BIT_ID,
	 //.irq_sen = MT_LEVEL_SENSITIVE,
...
#if 0
	{
	 ...
	 //.irq_num = UART2_IRQ_BIT_ID,
	 //.irq_sen = MT_LEVEL_SENSITIVE,
	 .set_bit = PDN_FOR_UART3,
	 .clr_bit = PDN_FOR_UART3,
	 .pll_id = PDN_FOR_UART3,
	 .sysrq = FALSE,
	 .hw_flow = FALSE,
	 .vff = TRUE,	/* UART3 */
	 },
	{
	 ...
	 //.irq_num = UART3_IRQ_BIT_ID,
	 //.irq_sen = MT_LEVEL_SENSITIVE,
	 .set_bit = PDN_FOR_UART4,
	 .clr_bit = PDN_FOR_UART4,
	 .pll_id = PDN_FOR_UART4,
	 .sysrq = FALSE,
	 .hw_flow = FALSE,
	 .vff = FALSE,	/* UART4 */
	 },
...
void set_uart_default_settings(int idx)
{
	...
	if (node) {
		/* iomap registers */
		mtk_uart_default_settings[idx].uart_base =
				 (unsigned long)of_iomap(node, 0);
		/* get IRQ ID */
		mtk_uart_default_settings[idx].irq_num =
	...
	pr_debug("[UART%d] phys_regs=0x%lx, regs=0x%lx",
		 idx,
		 mtk_uart_default_settings[idx].uart_phys_base,
		 mtk_uart_default_settings[idx].uart_base);
	pr_debug(" irq=%d, irq_flags=0x%lx\n",
		 mtk_uart_default_settings[idx].irq_num,
...
static inline void dump_reg(struct mtk_uart *uart, const char *caller)
{
	...
	MSG(CFG, "%s: RATEFIX(%02X); HSPEED(%02X); CNT(%02X);"
	     "PNT(%02X); DLH(%02X), DLL(%02X), IER(%02X)\n",
		caller, uratefix, uhspeed, usamplecnt,
		usamplepnt, udlh, udll, ier);
#endif
}
...
int mtk_uart_vfifo_enable(struct mtk_uart *uart, struct mtk_uart_vfifo *vfifo)
{
	...
	if (vfifo->type == UART_RX_VFIFO)
		UART_SET_BITS(UART_RX_DMA_EN
			| UART_TO_CNT_AUTORST, UART_DMA_EN);
	else if (vfifo->type == UART_TX_VFIFO)
		UART_SET_BITS(UART_TX_DMA_EN, UART_DMA_EN);
	mb();/* make sure UART FCR to be written */
	return 0;
}
...
{
	reg_sync_writel(VFF_TX_INT_EN_B,
		 VFF_INT_EN(uart->tx_vfifo->base));
}
...
{
	reg_sync_writel(VFF_RX_INT_EN0_B | VFF_RX_INT_EN1_B,
			VFF_INT_EN(uart->rx_vfifo->base));
}
...
unsigned int mtk_uart_vfifo_read_byte(struct mtk_uart *uart)
{
	...
	if (UART_READ16(VFF_RPT(base)) == (UART_READ32(VFF_LEN(base)) - 1))
		reg_sync_writel(~(UART_READ32(VFF_RPT(base))) & 0x10000,
						VFF_RPT(base));
	else
		reg_sync_writel(UART_READ32(VFF_RPT(base)) + 1,
						VFF_RPT(base));
...
static void mtk_uart_dma_vfifo_tx_tasklet_byte(unsigned long arg)
{
	...
	if (UART_DEBUG_EVT(DBG_EVT_DAT) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		...
		pr_debug("[UART%d_TX] %4d bytes:",
			uart->nport, vfifo->cur->idx);
...
/*---------------------------------------------------------------------------*/
/*static int mtk_uart_vfifo_write_string(struct mtk_uart *uart,
 *		const unsigned char *chars, size_t size)
 *{
 *    void *addr, *base = uart->tx_vfifo->base;
 *    unsigned int wpt = UART_READ32(VFF_WPT(base));
 ...
 *	memcpy(uart->tx_vfifo->addr, &chars[num_to_end],
 *                           (unsigned int)size - num_to_end);
 *	mb(); //make sure write point updated after VFIFO written.
 ...
 *		str[idx] = 0;
 *	}
 *	MSG(DMA, "RX[%4d]: %4d bytes from VFIFO [%4d]"
 *                  "(%02X %02X .. %02X %02X) %d\n",
...
/*---------------------------------------------------------------------------*/
static int mtk_uart_tty_insert_flip_string(struct mtk_uart *uart,
			const unsigned char *chars, size_t size)
{
	struct tty_struct *tty = uart->port.state->port.tty;
	struct uart_port *port = &uart->port;
	...
	if (flag == TTY_NORMAL) {
		...
		copied += tty_insert_flip_char(tty->port,
			chars[size - 1], flag);
	}
...
static void mtk_uart_dma_vfifo_rx_tasklet_str(unsigned long arg)
{
	...
	count = left = ((rxreg ^ txreg) & 0x00010000) ?
		(txptr + vfifo->size - rxptr) : (txptr - rxptr);
	...
	if (UART_DEBUG_EVT(DBG_EVT_DMA) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		...
		MSG(DMA, "RX[%4d]: [%5X..%5X] [%5X..%5X] [%d] [%4X.%4X]\n",
			...
			UART_READ32(VFF_WPT(base)),
		    UART_READ32(VFF_FLUSH(base)),
			UART_READ32(VFF_VALID_SIZE(base)),
			 UART_READ32(VFF_LEFT_SIZE(base)));
	}
#endif
	spin_unlock_irqrestore(&port->lock, flags);
...
void mtk_uart_dma_setup(struct mtk_uart *uart, struct mtk_uart_dma *dma)
{
	...
	if (dma->mode == UART_RX_VFIFO_DMA || dma->mode == UART_TX_VFIFO_DMA) {
		...
		/* reg_sync_writel(VFF_RX_INT_EN0_B, VFF_INT_EN(base)); */
			reg_sync_writel(VFF_RX_INT_EN0_B
				| VFF_RX_INT_EN1_B, VFF_INT_EN(base));
		mb();/* make sure write point updated after VFIFO written */
	}
}
	...
	if (dma->mode == UART_RX_VFIFO_DMA || dma->mode == UART_TX_VFIFO_DMA) {
		...
		while (UART_READ32(VFF_FLUSH(base))) {
			...
			if (polling_cnt > 10000) {
				pr_debug("%s:polling VFF_FLUSH fail.",
							__func__);
				pr_debug("VFF_DEBUG_STATUS=0x%x\n",
		...
		while (UART_READ32(VFF_EN(base))) {
			polling_cnt++;
			if (polling_cnt > 10000) {
				pr_debug("%s: polling VFF_EN fail.",
						__func__);
				pr_debug("VFF_DEBUG_STATUS=0x%x\n",
					UART_READ32(VFF_DEBUG_STATUS(base)));
				break;
...
void mtk_uart_set_mode(struct mtk_uart *uart, int mode)
{
	...
	mb();/* make sure FCR register is updated */
}
...
/*---------------------------------------------------------------------------*/
static void mtk_uart_cal_baud(struct mtk_uart *uart,
		int baudrate, int highspeed)
{
	unsigned long base = uart->base;
	...
	if (highspeed == 3)/* Enable UART DCM */
		UART_SET_BITS(UART_MCR_DCM_EN, UART_MCR);
	else    /* Disable UART DCM */
		UART_CLR_BITS(UART_MCR_DCM_EN, UART_MCR);
	...
	if (highspeed == 0) {
		...
		/* reg_sync_writel(0x00, UART_RATE_FIX_AD); */
		/*divider is 16 */
		reg_sync_writel(0x00, UART_HIGHSPEED);
		...
		/* reg_sync_writel(0x00, UART_RATE_FIX_AD); */
		/*divider is 8 */
		reg_sync_writel(0x01, UART_HIGHSPEED);
		...
		/* reg_sync_writel(0x00, UART_RATE_FIX_AD); */
		/*divider is 4 */
		reg_sync_writel(0x02, UART_HIGHSPEED);
		...
		/*
		 * NOTICE: We found some chip, that is using lower clock,
		 * may not have enough time to check stop bit.
...
void mtk_uart_set_flow_ctrl(struct mtk_uart *uart, int mode)
{
	...
	case UART_FC_SW:	/*MTK software flow control */
		...
		reg_sync_writel(START_CHAR(uart->port.state->port.tty),
				UART_XON1);
		reg_sync_writel(STOP_CHAR(uart->port.state->port.tty),
				UART_XOFF1);
		reg_sync_writel(tmp, UART_LCR);
	...
	if (uart->poweron_count > 0) {
		...
		if (clk_en_ret) {
			...
			if ((uart != console_port)
			    && (uart->tx_mode == UART_TX_VFIFO_DMA
			    || uart->rx_mode == UART_RX_VFIFO_DMA)) {
				clk_en_ret = clk_enable(clk_uart_dma);
				if (clk_en_ret) {
					pr_notice("[UART%d]enable clk_uart_main failed.",
					       uart->nport);
					pr_notice("ret:%d, clk_dma:%p\n",
						clk_en_ret, clk_uart_dma);
				} else {
					pr_debug("[UART%d]enabled clk_uart_dma:%p\n",
...
void mtk_uart_rx_pre_handler(struct mtk_uart *uart, int intrs)
{
	...
	if (intrs == UART_IIR_CTI) {
		 ...
		 * the RX data timeout interrupt will be
		 * triggered again and again.Hence,
...
void mtk_uart_break_ctl(struct uart_port *port, int break_state)
{
	...
	mb();/* make sure UART_LCR is updated */
	spin_unlock_irqrestore(&port->lock, flags);
}
...
void mtk_uart_is_ate_factory_mode(struct mtk_uart *uart)
{
	if ((uart->nport == 0) && (get_boot_mode() == ATE_FACTORY_MODE)) {
		...
		UART_CLR_BITS(UART_RX_DMA_EN
			| UART_TO_CNT_AUTORST | UART_TX_DMA_EN, UART_DMA_EN);
		mb();/* make sure DMA register is updated */
	}
}
	...
	if (uart->nport == 2) {
		...
		if (!stop_update) {
			if (curr_idx < RECORD_LENGTH) {
				uart_history[curr_record][curr_idx] =
						UART_READ8(addr);
				curr_idx++;
...
static void reset_rx_raw_data(struct mtk_uart *uart)
{
	...
	if (uart->nport == 2) {
		...
		if (!stop_update) {
			...
			for (curr_rx_idx = 0; curr_rx_idx < RECORD_LENGTH;
					 curr_rx_idx++)
				uart_rx_history[curr_rx_record][curr_rx_idx]
						 = 0;
			curr_rx_idx = 0;
		}
	...
	const unsigned char *chars, size_t size)
{
	...
	if (uart->nport == 2) {
		spin_lock_irqsave(&rx_history_lock, flags);
		if (!stop_update) {
			for (i = 0; (curr_rx_idx < RECORD_LENGTH) && (i < size);
						 i++, curr_rx_idx++)
				uart_rx_history[curr_rx_record][curr_rx_idx] =
							 chars[i];
...
void mtk_uart_restore(void)
{
	...
	for (i = 0; i < UART_NR; i++) {
		...
		reg_sync_writel(uart->registers.lcr, UART_LCR);
		reg_sync_writel(uart->registers.sample_count,
			UART_SAMPLE_COUNT);
		reg_sync_writel(uart->registers.sample_point,
			UART_SAMPLE_POINT);
...
void switch_uart_gpio(int uartport, int gpioopid)
{
	...
	pins_uart = pinctrl_lookup_state(ppinctrl,
...
void mtk_uart_switch_tx_to_gpio(struct mtk_uart *uart)
{
	...
	!defined(CONFIG_FPGA_EARLY_PORTING) && \
	!defined(CONFIG_MTK_LEGACY)
 ...
 */
	...
	if (uartport > 3) {
		pr_notice("[UART%d] %s fail!! port:%d",
				uartport, __func__, uartport);
		return;
	}
#ifdef CONFIG_PM
#ifndef CONFIG_FPGA_EARLY_PORTING
 ...
 */
	...
	case 0:
#ifdef GPIO_UART_UTXD0_PIN
	mt_set_gpio_mode(GPIO_UART_UTXD0_PIN,
			 GPIO_UART_UTXD0_PIN_M_UTXD);
#else
	...
	case 1:
#ifdef GPIO_UART_UTXD1_PIN
	mt_set_gpio_mode(GPIO_UART_UTXD1_PIN,
			 GPIO_UART_UTXD1_PIN_M_UTXD);
#else
	...
	case 2:
#ifdef GPIO_UART_UTXD2_PIN
	mt_set_gpio_mode(GPIO_UART_UTXD2_PIN,
			GPIO_UART_UTXD2_PIN_M_UTXD);
#else
	...
	case 3:
#ifdef GPIO_UART_UTXD3_PIN
	mt_set_gpio_mode(GPIO_UART_UTXD3_PIN,
			GPIO_UART_UTXD3_PIN_M_UTXD);
#else
...
void mtk_uart_switch_rx_to_gpio(struct mtk_uart *uart)
{
	...
	mt_set_gpio_mode(GPIO_UART_URXD0_PIN,
			GPIO_UART_URXD0_PIN_M_GPIO);
#else
	...
	mt_set_gpio_mode(GPIO_UART_URXD1_PIN,
			GPIO_UART_URXD1_PIN_M_GPIO);
#else
	...
	mt_set_gpio_mode(GPIO_UART_URXD2_PIN,
			GPIO_UART_URXD2_PIN_M_GPIO);
#else
	...
	mt_set_gpio_mode(GPIO_UART_URXD3_PIN,
			GPIO_UART_URXD3_PIN_M_GPIO);
#else
...
#ifndef CONFIG_FPGA_EARLY_PORTING
	spm_resource_req(SPM_RESOURCE_USER_UART,
