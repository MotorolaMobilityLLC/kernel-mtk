...
/* the size must be 8-byte alignment */
...
#define MSGERR(fmt, args...)   \
		pr_notice("  [UART%d]:E:%4d: " fmt,\
		uart->nport, __LINE__, ##args)
...
#define MSG_ERR(fmt, args...)  \
		pr_notice("[UARTX]:E:%4d: " fmt, __LINE__, ##args)
...
/*---------------------------------------------------------------------------*/
/* For MT6589, both RX
 * and TX will not use port to send or receive data
 */
/* IER */
...
#define UART_IER_HW_NORMALINTS   \
	(UART_IER_ERBFI|UART_IER_ELSI	\
	|UART_IER_EDSSI|UART_IER_VFF_FC_EN)
...
#define UART_FCR_FIFO_INIT       \
	(UART_FCR_FIFOE|UART_FCR_CLRR|UART_FCR_CLRT)
...
#define UART_EFR_AUTO_RTSCTS      \
		(UART_EFR_AUTO_RTS|UART_EFR_AUTO_CTS)
...
/* #define UART_AUTOBADUSAM_52M        29 */
/* CHECKME! 28 or 29 ? */
/* CHECKME! 31 or 32 ? */
...
/*when UART issues flush to DMA and all
 *data in UART VFIFO is transferred to VFF
 */
...
/*when UART issues flush to DMA
 *and all data in UART VFIFO is transferred to VFF
 */
