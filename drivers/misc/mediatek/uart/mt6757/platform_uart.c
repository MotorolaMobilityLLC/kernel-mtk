...
static void save_rx_raw_data(struct mtk_uart *uart, const unsigned char *chars,
			     size_t size);
...
static struct mtk_uart_setting mtk_uart_default_settings[] = {
    {
	/* .tx_mode = UART_NON_DMA, .rx_mode = UART_RX_VFIFO_DMA, .dma_mode =
	   UART_DMA_MODE_0, */
	.tx_mode = UART_TX_VFIFO_DMA,
	.rx_mode = UART_RX_VFIFO_DMA,
	.dma_mode = UART_DMA_MODE_0,
	/* .tx_mode = UART_NON_DMA, .rx_mode = UART_NON_DMA, .dma_mode =
	   UART_DMA_MODE_0, */
	.tx_trig = UART_FCR_TXFIFO_1B_TRI,
	.rx_trig = UART_FCR_RXFIFO_12B_TRI,

	/* .uart_base = AP_UART0_BASE, .irq_num = UART0_IRQ_BIT_ID, .irq_sen =
	   MT_LEVEL_SENSITIVE, */
	.sysrq = FALSE,
	.hw_flow = TRUE,
	.vff = TRUE,
	...
	/* .uart_base = AP_UART1_BASE, .irq_num = UART1_IRQ_BIT_ID, .irq_sen =
	   MT_LEVEL_SENSITIVE, */
	.sysrq = FALSE,
	.hw_flow = TRUE,
	.vff = TRUE,
...
static unsigned long mtk_uart_evt_mask[] = {
    DBG_EVT_NONE, DBG_EVT_NONE, DBG_EVT_NONE, DBG_EVT_NONE,
};
...
void set_uart_default_settings(int idx)
{
	...
	pr_debug(
	    "[UART%d] phys_regs=0x%lx, regs=0x%lx, irq=%d, irq_flags=0x%lx\n",
...
/*---------------------------------------------------------------------------*/
unsigned long get_uart_evt_mask(int idx) { return mtk_uart_evt_mask[idx]; }

/*---------------------------------------------------------------------------*/
void set_uart_evt_mask(int idx, int value) { mtk_uart_evt_mask[idx] = value; }

/*---------------------------------------------------------------------------*/
unsigned long get_uart_lsr_status(int idx) { return mtk_uart_lsr_status[idx]; }
...
static inline void dump_reg(struct mtk_uart *uart, const char *caller)
{
	...
	MSG(CFG, "%s: RATEFIX(%02X); HSPEED(%02X); CNT(%02X); PNT(%02X); "
		 "DLH(%02X), DLL(%02X), IER(%02X)\n",
...
int mtk_uart_vfifo_enable(struct mtk_uart *uart, struct mtk_uart_vfifo *vfifo)
{
	...
	if (vfifo->type == UART_RX_VFIFO)
		UART_SET_BITS(UART_RX_DMA_EN | UART_TO_CNT_AUTORST,
			      UART_DMA_EN);
...
int mtk_uart_vfifo_disable(struct mtk_uart *uart, struct mtk_uart_vfifo *vfifo)
{
	...
	if (!vfifo) {
		...
		UART_CLR_BITS(UART_RX_DMA_EN | UART_TO_CNT_AUTORST,
			      UART_DMA_EN);
	} else if (vfifo->type == UART_TX_VFIFO) {
...
static void mtk_uart_dma_vfifo_tx_tasklet_byte(unsigned long arg)
{
	...
	if (UART_DEBUG_EVT(DBG_EVT_DMA) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		...
		MSG(DMA,
		    ...
		    UART_READ32(VFF_RPT(vfifo->base)), str[0], str[1], str[2],
		    str[3]);
	} else {
		MSG(DMA, "TX[%4d]: %4d/%4d [%05X-%05X]\n", size, count, left,
		    UART_READ32(VFF_WPT(vfifo->base)),
	...
	if (UART_DEBUG_EVT(DBG_EVT_DAT) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		...
		pr_debug("[UART%d_TX] %4d bytes:", uart->nport,
			 vfifo->cur->idx);
...
/*---------------------------------------------------------------------------*/
/*static int mtk_uart_vfifo_write_string(struct mtk_uart *uart, const unsigned
...
*	mb();                                                  //make sure write
*point updated after VFIFO written.
*	reg_sync_writel( wpt+(unsigned int)size, VFF_WPT(base));
*    }else{
*	memcpy(addr, chars, num_to_end);
*	memcpy(uart->tx_vfifo->addr, &chars[num_to_end], (unsigned int)size -
*num_to_end);
*	mb();                                                  //make sure write
*point updated after VFIFO written.
...
*		str[idx] = 0;
*	}
*	MSG(DMA, "RX[%4d]: %4d bytes from VFIFO [%4d] (%02X %02X .. %02X %02X)
*%d\n",
*	    left, left - count, mtk_uart_vfifo_get_counts(vfifo), str[0],
*str[1], str[2], str[3],
					   ...
					   size_t size)
{
	...
	if (flag == TTY_NORMAL) {
		...
		copied +=
		    tty_insert_flip_char(tty->port, chars[size - 1], flag);
	}
...
static void mtk_uart_dma_vfifo_rx_tasklet_str(unsigned long arg)
{
	...
	if (UART_DEBUG_EVT(DBG_EVT_DMA) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		...
		MSG(DMA, "RX[%4d]: [%5X..%5X] [%5X..%5X] (%02X %02X .. %02X "
			 "%02X) [%d]\n",
		    left, rxreg, txreg, UART_READ32(VFF_RPT(base)),
		    UART_READ32(VFF_WPT(base)), str[0], str[1], str[2], str[3],
		    UART_READ32(VFF_FLUSH(base)));
	} else {
		MSG(DMA, "RX[%4d]: [%5X..%5X] [%5X..%5X] [%d] [%4X.%4X]\n",
		    left, rxreg, txreg, UART_READ32(VFF_RPT(base)),
		    UART_READ32(VFF_WPT(base)), UART_READ32(VFF_FLUSH(base)),
	...
	if (UART_DEBUG_EVT(DBG_EVT_DAT) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		...
		pr_debug("[UART%d_RX] %4d bytes:", uart->nport,
			 vfifo->cur->idx);
...
{ /*the function will be called through dma irq or tasklet_schedule */
	...
	MSG(DMA, "%d, %x, %x\n", uart->read_allow(uart),
	    UART_READ32(VFF_VALID_SIZE(vfifo->base)), vfifo->trig);
	spin_lock_irqsave(&vfifo->iolock, flags);
	atomic_inc(&vfifo->entry);
	...
	if (dma->mode == UART_RX_VFIFO_DMA || dma->mode == UART_TX_VFIFO_DMA) {
		...
		while (UART_READ32(VFF_FLUSH(base))) {
			...
			if (polling_cnt > 10000) {
				pr_debug("%s: polling VFF_FLUSH "
					 "fail VFF_DEBUG_STATUS=0x%x\n",
					 __func__,
...
/*---------------------------------------------------------------------------*/
static void mtk_uart_cal_baud(struct mtk_uart *uart, int baudrate,
			      int highspeed)
{
	unsigned long base = uart->base;
	...
	if (highspeed == 0) {
		...
		u32 fraction;
		u16 fraction_L_mapping[] = {0,    1,    0x5,  0x15, 0x55, 0x57,
...
#if defined(ENABLE_FRACTIONAL)
		fraction = ((uartclk * 10) / baudrate) %
			   10; /*count fraction to set fractoin register */
		...
		/*
		 * NOTICE: We found some chip, that is using lower clock, may
		 * not have enough time to check stop bit.
	...
	MSG(CFG, "BaudRate = %d, SysClk = %d, Divisor = %d, %04X/%04X\n",
	    baudrate, uartclk, divisor, UART_READ32(UART_IER),
...
void mtk_uart_set_flow_ctrl(struct mtk_uart *uart, int mode)
{
	...
	case UART_FC_NONE:
		...
		mtk_uart_disable_intrs(uart, UART_IER_XOFFI | UART_IER_RTSI |
						 UART_IER_CTSI);
		break;
	case UART_FC_HW:
		reg_sync_writel(UART_ESCAPE_CH, UART_ESCAPE_DAT);
...
void mtk_uart_power_up(struct mtk_uart *uart)
{
	...
	if (uart->poweron_count > 0) {
		...
		if (clk_en_ret) {
			pr_notice("[UART%d][CCF]enable clk_uart_main failed. "
				  "ret:%d, clk_main:%p\n",
				  uart->nport, clk_en_ret,
				  setting->clk_uart_main);
		} else {
			if (__ratelimit(&ratelimit)) {
				pr_debug(
				    "[UART%d][CCF]enabled clk_uart_main:%p\n",
				    uart->nport, setting->clk_uart_main);
			}
			if ((uart != console_port) &&
				...
				if (clk_en_ret) {
					pr_notice("[UART%d][CCF]enable "
						  "clk_uart_main failed. "
						  "ret:%d, clk_dma:%p\n",
						  uart->nport, clk_en_ret,
						  clk_uart_dma);
				} else {
					pr_debug("[UART%d][CCF]enabled "
						 "clk_uart_dma:%p\n",
...
void mtk_uart_get_modem_status(struct mtk_uart *uart)
{
	...
	MSG(INFO, "MSR: DCD(%d), RI(%d), DSR(%d), (%d)\n",
	    status & UART_MSR_DCD ? 1 : 0, status & UART_MSR_RI ? 1 : 0,
...
void mtk_uart_rx_pre_handler(struct mtk_uart *uart, int intrs)
{
	...
	if (intrs == UART_IIR_CTI) {
		 ...
		 * Without the function call, the RX data timeout interrupt will
		 * be
		 * triggered again and again.Hence, the purpose of this function
		 * call
		...
		MSG(DMA, "rx timeout: %x, %4d\n", tmp,
		    mtk_uart_vfifo_get_counts(uart->rx_vfifo));
#endif
...
void mtk_uart_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	...
	MSG(CFG, "MCR: DTR(%d), RTS(%d), OUT1(%d), OUT2(%d), LOOP(%d)\n",
	    val & UART_MCR_DTR ? 1 : 0, val & UART_MCR_RTS ? 1 : 0,
	    val & UART_MCR_OUT1 ? 1 : 0, val & UART_MCR_OUT2 ? 1 : 0,
...
#else
void stop_log(void) { /* dummy API */ }

void dump_uart_history(void) { /* dummy API */ }
#endif
...
void switch_uart_gpio(int uartport, int gpioopid)
{
	...
	pins_uart =
	    pinctrl_lookup_state(ppinctrl, uart_gpio_cmds[uartport][gpioopid]);

	if (IS_ERR(pins_uart)) {
		if (__ratelimit(&ratelimit)) {
			pr_notice("[UART%d]%s pinctrl_lockup(%d, %s) fail!! "
				  "pctrl:%p, err:%ld\n",
				  uartport, __func__, uartport,
				  uart_gpio_cmds[uartport][gpioopid], ppinctrl,
				  PTR_ERR(pins_uart));
		}
		return;
	}
...
void mtk_uart_switch_tx_to_gpio(struct mtk_uart *uart)
{
...
#endif /* defined(CONFIG_PM) && !defined(CONFIG_FPGA_EARLY_PORTING) &&         \
	  !defined(CONFIG_MTK_LEGACY) */
	...
	if (uartport > 3) {
		pr_notice("[UART%d] %s fail!! port:%d", uartport, __func__,
			  uartport);
		return;
	}
#ifdef CONFIG_PM
...
/*---------------------------------------------------------------------------*/
int mtk_uart_plat_info_query(const char str[]) { return 0; }
