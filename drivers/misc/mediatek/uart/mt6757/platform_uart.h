	...
	do {                                                                   \
		if ((DBG_EVT_##evt) & uart->evt_mask) {                        \
			...
			if (DBG_EVT_##evt & DBG_EVT_ERR)                       \
				...
				pr_debug("  [UART%d]:%c: " fmt, uart->nport,   \
					 s[0], ##args);                        \
		}                                                              \
	} while (0)
/*---------------------------------------------------------------------------*/
...
/* When set "1", enable flow control triggered by RX FIFO full when VFIFO_EN is
 * set. */
...
#define UART_IER_ALL_INTS                                                      \
	(UART_IER_ERBFI | UART_IER_ETBEI | UART_IER_ELSI | UART_IER_EDSSI |    \
	 UART_IER_XOFFI | UART_IER_RTSI | UART_IER_CTSI)
...
#define UART_FCR_NORMAL                                                        \
	(UART_FCR_FIFO_INIT | UART_FCR_TXFIFO_4B_TRI | UART_FCR_RXFIFO_12B_TRI)
/*---------------------------------------------------------------------------*/
/* LCR */
...
#define UART_IIR_HW_FLOW_CTRL (0x20) /*  or RTS Rising Edge */
...
#define UART_RATE_FIX_ALL_13M                                                  \
	(UART_RATE_FIX_13M | UART_AUTORATE_FIX_13M | UART_FREQ_SEL_13M)
