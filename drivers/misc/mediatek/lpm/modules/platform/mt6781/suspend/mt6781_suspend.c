...
struct mtk_lpm_model mt6781_model_suspend;
...
static inline int mt6781_suspend_common_enter(unsigned int *susp_status)
{
...
static inline int mt6781_suspend_common_resume(unsigned int susp_status)
{
	/* Implement suspend common flow here */
	return 0;
}

static int __mt6781_suspend_prompt(int type, int cpu,
				   const struct mtk_lpm_issuer *issuer)
{
	int ret = 0;
	unsigned int spm_res = 0;

	mt6781_suspend_status = 0;

	printk_deferred("[name:spm&][%s:%d] - prepare suspend enter\n",
			__func__, __LINE__);

	ret = mt6781_suspend_common_enter(&mt6781_suspend_status);

	if (ret)
		goto PLAT_LEAVE_SUSPEND;

	/* Legacy SSPM flow, spm sw resource request flow */
	mt6781_do_mcusys_prepare_pdn(mt6781_suspend_status, &spm_res);
...
static void __mt6781_suspend_reflect(int type, int cpu,
					const struct mtk_lpm_issuer *issuer)
{
	printk_deferred("[name:spm&][%s:%d] - prepare suspend resume\n",
			__func__, __LINE__);

	mt6781_suspend_common_resume(mt6781_suspend_status);
	mt6781_do_mcusys_prepare_on();

	printk_deferred("[name:spm&][%s:%d] - resume\n",
			__func__, __LINE__);

	/* do not call issuer when prepare fail */
	if (mt6781_model_suspend.flag & MTK_LP_PREPARE_FAIL)
		return;
...
int mt6781_suspend_system_prompt(int cpu,
					const struct mtk_lpm_issuer *issuer)
{
	return __mt6781_suspend_prompt(MTK_LPM_SUSPEND_S2IDLE,
				       cpu, issuer);
}

void mt6781_suspend_system_reflect(int cpu,
					const struct mtk_lpm_issuer *issuer)
{
	return __mt6781_suspend_reflect(MTK_LPM_SUSPEND_S2IDLE,
					cpu, issuer);
}

int mt6781_suspend_s2idle_prompt(int cpu,
					const struct mtk_lpm_issuer *issuer)
{
	int ret = 0;
...
#endif
		if (ret < 0)
			mt6781_model_suspend.flag |= MTK_LP_PREPARE_FAIL;

		ret = __mt6781_suspend_prompt(MTK_LPM_SUSPEND_S2IDLE,
					      cpu, issuer);
	}
	return ret;
}

int mt6781_suspend_s2idle_prepare_enter(int prompt, int cpu,
					const struct mtk_lpm_issuer *issuer)
{
	int ret = 0;
...
void mt6781_suspend_s2idle_reflect(int cpu,
					const struct mtk_lpm_issuer *issuer)
{
	if (cpumask_weight(&s2idle_cpumask) == num_online_cpus()) {
		__mt6781_suspend_reflect(MTK_LPM_SUSPEND_S2IDLE,
					 cpu, issuer);
...
#define MT6781_SUSPEND_OP_INIT(_prompt, _enter, _resume, _reflect) ({\
	mt6781_model_suspend.op.prompt = _prompt;\
	mt6781_model_suspend.op.prepare_enter = _enter;\
	mt6781_model_suspend.op.prepare_resume = _resume;\
	mt6781_model_suspend.op.reflect = _reflect; })


struct mtk_lpm_model mt6781_model_suspend = {
	.flag = MTK_LP_REQ_NONE,
	.op = {
		.prompt = mt6781_suspend_system_prompt,
		.reflect = mt6781_suspend_system_reflect,
	}
};
...
static struct notifier_block mt6781_spm_suspend_pm_notifier_func = {
	.notifier_call = mt6781_spm_suspend_pm_event,
	.priority = 0,
};

#endif /* CONFIG_PM */

int __init mt6781_model_suspend_init(void)
{
	int ret;
	int i;
	...
	if (suspend_type == MTK_LPM_SUSPEND_S2IDLE) {
		MT6781_SUSPEND_OP_INIT(mt6781_suspend_s2idle_prompt,
					mt6781_suspend_s2idle_prepare_enter,
					NULL,
					mt6781_suspend_s2idle_reflect);
		mtk_lpm_suspend_registry("s2idle", &mt6781_model_suspend);
		mtk_s2idle_state_enable(0);
	} else {
		MT6781_SUSPEND_OP_INIT(mt6781_suspend_system_prompt,
					NULL,
					NULL,
					mt6781_suspend_system_reflect);
		mtk_lpm_suspend_registry("suspend", &mt6781_model_suspend);
	}
...
#ifdef CONFIG_PM
	ret = register_pm_notifier(&mt6781_spm_suspend_pm_notifier_func);
	if (ret) {
