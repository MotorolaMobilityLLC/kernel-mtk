		      ...
		      int pages_no, struct page **pages)
{
	...
	locked_pages = gup_local(task->mm, (unsigned long)virt_start_page_addr,
				 pages_no, 1, pages);
...
static int alloc_mmu_table_store(void)
{
	...
	for (i = 0; i < MMU_TABLES_PER_PAGE; i++) {
		...
		mmutable->phys = mmutable_set->phys +
				 i * sizeof(struct mmutable);
...
static struct mc_mmu_table *alloc_mmu_table(struct mc_instance *instance)
{
	...
	table  = list_first_entry(&mem_ctx.free_mmu_tables, struct mc_mmu_table,
				  list);
	if (!table) {
		MCDRV_DBG_ERROR(mcd, "out of memory");
		return ERR_PTR(-ENOMEM);
		      ...
		      unsigned int wsm_len, struct mc_mmu_table *table)
{
	...
	if (task && !is_vmalloc_addr(wsm_buffer)) {
		/*
	...
	else if (!task && !is_vmalloc_addr(wsm_buffer)) {
...
static void unmap_buffers(struct mc_mmu_table *table)
{
	...
	for (i = 0; i < table->pages; i++) {
		...
		pte = mmutable->table_entries[i];
#else
		pte.pte = mmutable->table_entries[i];
#endif
		page = pte_page(pte);
...
static void unmap_mmu_table(struct mc_mmu_table *table)
{
	...
	if (!atomic_dec_and_test(&table->usage)) {
		MCDRV_DBG_VERBOSE(mcd,
				  ...
				  (u64)table->phys, atomic_read(&table->usage));
		return;
	}
...
int mc_free_mmu_table(struct mc_instance *instance, u32 handle)
{
	struct mc_mmu_table *table;
	...
	if (table->type & MC_MMU_TABLE_TYPE_WSM_FAKE_L1) {
		 ...
		 */
#ifdef LPAE_SUPPORT
		u64 *pte = (u64 *)table->virt;
#else
		u32 *pte = (u32 *)table->virt;
#endif /* LPAE_SUPPORT */
...
int mc_lock_mmu_table(struct mc_instance *instance, u32 handle)
{
	int ret = 0;
	struct mc_mmu_table *table = NULL;
...
phys_addr_t mc_find_mmu_table(u32 handle, s32 fd)
{
	phys_addr_t ret = 0;
	struct mc_mmu_table *table = NULL;
