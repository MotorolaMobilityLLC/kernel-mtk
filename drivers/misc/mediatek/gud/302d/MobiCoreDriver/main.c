...
static u32 mc_find_cont_wsm_addr(struct mc_instance *instance, void *uaddr,
				 void **addr, u32 len)
{
	int ret = 0;
	struct mc_buffer *buffer;
...
bool mc_check_owner_fd(struct mc_instance *instance, s32 fd)
{
...
static u32 mc_find_cont_wsm(struct mc_instance *instance, u32 handle,
			    s32 fd, phys_addr_t *phys, u32 *len)
{
	int ret = 0;
	struct mc_buffer *buffer;
 ...
 *
 */
static int __free_buffer(struct mc_instance *instance, u32 handle,
			 bool unlock)
{
	int ret = 0;
	struct mc_buffer *buffer;
...
found_buffer:
	 ...
	 */
	if (!unlock && buffer->uaddr) {
#ifndef MC_VM_UNMAP
		/* do_munmap must be done with mm->mmap_sem taken */
		down_write(&mm->mmap_sem);
		ret = do_munmap(mm, (unsigned long)buffer->uaddr, buffer->len);
		up_write(&mm->mmap_sem);

#else
		ret = vm_munmap((unsigned long)buffer->uaddr, buffer->len);
#endif
		if (ret < 0) {
			/*
...
int mc_free_buffer(struct mc_instance *instance, u32 handle)
{
	int ret = 0;
...
int mc_get_buffer(struct mc_instance *instance, struct mc_buffer **buffer,
		  unsigned long len)
{
	...
	MCDRV_DBG_VERBOSE(mcd,
			  ...
			  (u64)phys, (u64)(phys + allocated_size), addr,
			  cbuffer->handle,
 ...
 */
static int __lock_buffer(struct mc_instance *instance, u32 handle)
{
	int ret = 0;
	struct mc_buffer *buffer;
 ...
 */
int mc_register_wsm_mmu(struct mc_instance *instance, void *buffer,
			u32 len, u32 *handle, phys_addr_t *phys)
{
	int ret = 0;
	...
	/* since for both non-LPAE and LPAE cases we use u64 records
	 *  for the fake table we don't support more than 512 MB TA size
	 */
	if (nb_of_1mb_section > SZ_4K / sizeof(u64)) {
	...
	if (nb_of_1mb_section > 1) {
		...
		MCDRV_DBG_VERBOSE(mcd, "allocate %d L2 table",
				  nb_of_1mb_section);
		mmu_table = (u64 *)get_zeroed_page(GFP_KERNEL);
	...
	handles = kcalloc(nb_of_1mb_section, sizeof(unsigned int), GFP_KERNEL);
	if (!handles) {
	...
	if (mmu_table) {
		MCDRV_DBG_VERBOSE(mcd, "fake L1 buffer: %p, len=%zu",
				  mmu_table,
				  nb_of_1mb_section * sizeof(u64));

		table = mc_alloc_mmu_table(instance, NULL, mmu_table,
					   nb_of_1mb_section * sizeof(u64),
...
int mc_unregister_wsm_mmu(struct mc_instance *instance, u32 handle)
{
	int ret = 0;
...
static int mc_lock_handle(struct mc_instance *instance, u32 handle)
{
	int ret = 0;
...
static int mc_unlock_handle(struct mc_instance *instance, u32 handle)
{
	int ret = 0;
			     ...
			     unsigned long arg)
{
	...
	switch (cmd) {
	case MC_IO_FREE:
		ret = mc_free_buffer(instance, (u32)arg);
		break;

	/*
	 * 32/64 bit interface compatibility notice:
	...
	case MC_IO_UNREG_WSM:
		ret = mc_unregister_wsm_mmu(instance, (u32)arg);
		break;
			      ...
			      unsigned long arg)
{
	...
	case MC_IO_INFO: {
		...
		ret = mc_info(info.ext_info_id, &info.state, &info.ext_info);
		if (!ret) {
	...
	case MC_IO_LOCK_WSM: {
		ret = mc_lock_handle(instance, (u32)arg);
		break;
	}
	case MC_IO_UNLOCK_WSM:
		ret = mc_unlock_handle(instance, (u32)arg);
		break;
