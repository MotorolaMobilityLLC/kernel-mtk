...
int mc_switch_core(u32 core_num)
{
	...
	MCDRV_DBG(mcd, "<- cmd=0x%08x, core_id=0x%08x\n",
		  fc_switch_core.as_in.cmd, fc_switch_core.as_in.core_id);
	MCDRV_DBG(mcd, "<- core_num=0x%08x, active_cpu=0x%08x\n",
		  core_num, active_cpu);
	    ...
	    u32 mcp_length)
{
	...
	u64 base_addr = (u64)base;
	u32 base_high = (u32)(base_addr >> 32);
	...
	fc_init.as_in.base = (u32)base_addr;
