...
static u32 g_cmd_id = TLC_TUI_CMD_NONE;
...
static bool tlc_open_driver(void)
{
	...
	mc_ret = mc_open_session(&dr_session_handle, &dr_uuid, (u8 *)dci,
...
static u32 send_cmd_to_user(u32 command_id)
{
	...
	if (g_user_rsp.id != command_id) {
		pr_debug("ERROR %s: Wrong response id 0x%08x iso 0x%08lx\n",
...
int tlc_wait_cmd(u32 *cmd_id)
{
	/*
	 * Create the TlcTui Main thread and start secure driver
	 * (only 1st time)
	 */
