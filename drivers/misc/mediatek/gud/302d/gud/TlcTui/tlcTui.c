...
static void tlc_process_cmd(void)
{
	...
	if (dci == NULL) {
		pr_debug("ERROR %s: DCI not set up properly, exiting\n",
			 __func__);
		return;
	}
...
bool tlc_notify_event(uint32_t event_type)
{
	...
	if (dci == NULL) {
		pr_notice("ERROR tlc_notify_event: DCI not set up, exiting\n");
		return false;
	}
...
int tlc_wait_cmd(uint32_t *cmd_id)
{
	...
	if (dr_session_handle.session_id == 0) {
		...
		if (!thread_id) {
			pr_notice("Unable to start Trusted UI main thread\n");
			return -EFAULT;
		}
	}
