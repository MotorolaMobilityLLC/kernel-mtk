...
size_t connection_read_data(struct connection *conn, void *buffer, u32 len,
			    int32_t timeout)
{
	size_t ret = 0;

	MCDRV_ASSERT(buffer);
	MCDRV_ASSERT(conn->socket_descriptor);
...
int connection_write_data(struct connection *conn, void *buffer, u32 len)
{
