...
static struct mcore_device_t *resolve_device_id(u32 device_id)
{
...
static bool remove_device(u32 device_id)
{
	...
	do {
		...
		struct mc_drv_cmd_open_session_t cmd_open_session = {
			...
			},
			{
				session->device_id,
				*uuid,
				(u32)((uintptr_t)(wsm->virt_addr) & 0xFFF),
				wsm->handle,
				tci_len
			}
		};
...
enum mc_result mc_malloc_wsm(u32 device_id, u32 align, u32 len,
			     u8 **wsm, u32 wsm_flags)
{
...
enum mc_result mc_map(struct mc_session_handle *session_handle, void *buf,
		      u32 buf_len, struct mc_bulk_map *map_info)
{
	enum mc_result mc_result = MC_DRV_ERR_UNKNOWN;
	...
	do {
		...
		/* Get session */
		u32 session_id = session_handle->session_id;
			...
			struct mc_bulk_map *map_info)
{
	...
	do {
		...
		u32 handle = session_find_bulk_buf(session, buf);
					 ...
					 int32_t *last_error)
{
	...
	do {
		...
		/* Get session */
		u32 session_id = session->session_id;
		struct session *nqsession =
				mcore_device_resolve_session_id(device,
								session_id);
		if (!nqsession) {
