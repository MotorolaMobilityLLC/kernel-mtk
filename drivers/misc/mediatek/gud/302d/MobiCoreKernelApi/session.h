...
struct bulk_buffer_descriptor {
	...
	u32	len;		/* Length of the Bulk buffer */
	u32	handle;
...
struct session_information {
	...
	s32		last_error;	/* Last error of session */
};
...
struct bulk_buffer_descriptor *session_add_bulk_buf(
	struct session *session, void *buf, u32 len);
 ...
 */
void session_set_error_info(struct session *session, s32 err);
 ...
 */
s32 session_get_last_err(struct session *session);
