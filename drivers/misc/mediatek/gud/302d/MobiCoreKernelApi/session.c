...
struct bulk_buffer_descriptor *bulk_buffer_descriptor_create(
	void *virt_addr, u32 len, u32 handle)
{
...
struct session *session_create(
	u32 session_id, void *instance, struct connection *connection)
{
...
u32 session_find_bulk_buf(struct session *session, void *virt_addr)
{
	struct bulk_buffer_descriptor *tmp;
	struct list_head *pos, *q;
	u32 handle = 0;
