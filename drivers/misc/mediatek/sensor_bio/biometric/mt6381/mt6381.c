	...
	for (;;) {
		...
		if (s_info->filp != NULL) {
			...
			while (rc != wc && s_info->numOfData) {
				vsm_driver_read_register(&data);
				//mt2511_read(s_info->upsram_rd_data, buf);
				//len = sprintf(str_buf, "%d, %d, %d, %x, %lld\n", count++, rc, wc, *(int *)buf, sched_clock());
				if (atomic_read(&bio_trace) != 0)
					BIO_ERR("%d, %d, %x, %x, %lld\n", rc, wc, *(int *)buf, s_info->upsram_rd_data, sched_clock());
...
static ssize_t store_io_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	u32 address[4] = {0};
	struct bus_data_t data;
	int num = sscanf(buf, "%x %x %x %x %x %x %x %x ", &address[0], &value[0], &address[1], &value[1], &address[2], &value[2], &address[3], &value[3]);

	BIO_ERR("(%d, %x, %x, %x, %x, %x, %x, %x, %x)\n", num, address[0], value[0], address[1], value[1], address[2], value[2], address[3], value[3]);
	if(1 == num)
	{
...
static ssize_t show_init(struct device_driver *ddri, char *buf)
{
	...
	if (modify_array_len != 0) {
		for (i = 0; i < new_init_array_len; i++) {
			sprintf(tmp_buf, "0x%x, 0x%08x\n", VSM_SIGNAL_NEW_INIT_array[i].addr, VSM_SIGNAL_NEW_INIT_array[i].value);
			strcat(buf, tmp_buf);
		}
	} else {
		for (i = 0; i < ARRAY_SIZE(VSM_SIGNAL_INIT_array); i++) {
			sprintf(tmp_buf, "0x%x, 0x%08x\n", VSM_SIGNAL_INIT_array[i].addr, VSM_SIGNAL_INIT_array[i].value);
			strcat(buf, tmp_buf);
		}
	}
					  ...
					  uint32_t *enable_data)
{
	...
	for (i = 0; i < len; i++) {
		//if ((reg_addr + i)->addr != DIGITAL_START_ADDR) {
			...
			/* ms_delay(5); */
			if (DBG) {
				pr_warn("%d:vsm_driver_read1, addr:0x%x, reg:0x%x, data 0x%x \r\n",
			...
			err = vsm_driver_read_register(&data);
			if (DBG) {
				pr_warn("%d:vsm_driver_read2, addr:0x%x, reg:0x%x, data 0x%x \r\n",
					i, data.addr, data.reg, *(uint32_t *) (data.data_buf));
			}
#endif
	...
	if (sram_len > 0) {
		...
		if (atomic_read(&bio_trace) != 0)
			BIO_LOG("%s =====> sram_type: %d, sram_len,%d,%d,%d,%lld,delta : %lld, %lld, %lld, %lld\n", __func__, sram_type,
					sram_len, read_counter, write_counter, rate[sram_type], sched_clock() - current_timestamp,
