		...
		for (idx = 0; idx <= MAX_ANDROID_SENSOR_NUM; idx++) {
			if (ID_ACCELEROMETER == idx || ID_MAGNETIC == idx || ID_ORIENTATION == idx
				...
				if (obj_data.sensors_data[idx].values[0] == SENSOR_INVALID_VALUE ||
				    obj_data.sensors_data[idx].values[1] == SENSOR_INVALID_VALUE ||
				    obj_data.sensors_data[idx].values[2] == SENSOR_INVALID_VALUE) {
					event_type &= ~(1LL << idx);
...
static int hwmsen_enable(struct hwmdev_object *obj, int sensor, int enable)
{
	...
	if (enable == 1) {
		...
		if ((obj->dc->polling_running == 0) && (obj->active_data_sensor != 0)) {
		...
		if ((obj->dc->polling_running == 1) && (obj->active_data_sensor == 0)) {
...
{
/*
 *	pr_debug("sensor test: hwmsen_store_active function!\n");
...
#if 0
	struct hwmdev_object *devobj = (struct hwmdev_object *)dev_get_drvdata(dev);
	/* pr_debug("sensor test: hwmsen_show_delay function!\n"); */
	if (!devobj || !devobj->dc) {
		HWM_ERR("null pointer!!\n");
		return 0;
	}
...
{
#if 0
	/* pr_debug("sensor test: hwmsen_show_wake function!\n"); */
	struct hwmdev_object *devobj = (struct hwmdev_object *)dev_get_drvdata(dev);
...
/*	struct hwmdev_object *devobj = (struct hwmdev_object*)dev_get_drvdata(dev);
 *	int wake, err;
 *	pr_debug("sensor test: hwmsen_store_wake function!\n");
 *	if (!devobj || !devobj->dc) {
 *	    HWM_ERR("null pointer!!\n");
 *	    return count;
 *	}
...
{
	int idx, err = 0;
	int num = (int)(ARRAY_SIZE(hwmsen_attr_list));
...
int hwmsen_msensor_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_CHOOSE_G_NUM; i++) {
		if (strcmp(msensor_name, msensor_init_list[i]->name) == 0) {
			if (msensor_init_list[i]->uninit == NULL) {
...
int hwmsen_gsensor_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_CHOOSE_G_NUM; i++) {
		if (strcmp(gsensor_name, gsensor_init_list[i]->name) == 0) {
			if (gsensor_init_list[i]->uninit == NULL) {
...
int hwmsen_alsps_sensor_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_CHOOSE_G_NUM; i++) {
		if (strcmp(alsps_name, alsps_init_list[i]->name) == 0) {
			if (alsps_init_list[i]->uninit == NULL) {
