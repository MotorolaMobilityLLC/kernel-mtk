...
static struct LCM_setting_table lcm_hbm_setting = {0x51, 2, {0xFF, 0x0F} };//100% PWM
...
{
	pr_info("%s,txd_icnl9911c cmd:%d, value = %d\n", __func__, *lcm_cmd, *lcm_value);
	...
	switch(*lcm_cmd) {
		...
		case PARAM_CABC:
			...
			break;
		default:
			pr_err("%s,txd_icnl9911c cmd:%d, unsupport\n", __func__, *lcm_cmd);
			break;
	}

	pr_info("%s,txd_icnl9911c cmd:%d, value = %d done\n", __func__, *lcm_cmd, *lcm_value);
}
struct LCM_DRIVER mipi_mot_vid_hlt_ft8006s_652_hdp_lcm_drv = {
	.name = "mipi_mot_vid_hlt_ft8006s_652_hdp",
	.supplier = "hlt_ft8006s_hdp_vid",
