...
static void lcm_suspend(void)
{
	...
	//    SET_RESET_PIN(0);
	//	SET_RESET_PIN_TS(0);
	LCM_LOGI("%s,ili9881h lcm_suspend power shut start\n", __func__);
	MDELAY(1);
	set_gpio_lcd_enn(0);
	MDELAY(1);
	...
	set_gpio_lcd_enp(0);
#endif
	LCM_LOGI("%s,ili9881h lcm_suspend power shutted over\n", __func__);
...
{
	LCM_LOGI("%s start: ili9881h",__func__);
	lcm_init();
	LCM_LOGI("%s end: ili9881h\n",__func__);
}
...
struct LCM_DRIVER ontim_ili9881h_hdplus_dsi_vdo_truly_lcm_drv = {
	.name = "ontim_ili9881h_hdplus_dsi_vdo_truly",
