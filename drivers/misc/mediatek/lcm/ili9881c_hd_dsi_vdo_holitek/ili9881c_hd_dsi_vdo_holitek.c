...
static void lcm_init(void)
{
    ...
    if (ret < 0)
        LCM_LOGI("holitek-ili9881c----cmd=%0x--i2c write error----\n", cmd);
    else
        LCM_LOGI("holitek-ili9881c---cmd=%0x--i2c write success----\n", cmd);
    cmd = 0x01;
    data = 0x0F;
   ret = NT50358A_write_byte(cmd, data);
    if (ret < 0)
...
//    printk(KERN_ERR "yzwyzw %s %d\n",__func__,__LINE__);
    ...
    MDELAY(30);
//    SET_RESET_PIN(0);
    LCM_LOGI("%s,holitek-ili9881c lcm_suspend power shut start\n", __func__);
    MDELAY(1);
    set_gpio_lcd_enn(0);
    MDELAY(1);
//    set_gpio_lcd_enn(0);
...
//    MDELAY(1);
    set_gpio_lcd_enp(0);
#endif
    LCM_LOGI("%s,holitek-ili9881clcm_suspend power shutted over\n", __func__);
...
{
    LCM_LOGI("%s start: tholitek-ili9881cn",__func__);
    lcm_init();
    LCM_LOGI("%s end: holitek-ili9881c\n",__func__);
}
...
struct LCM_DRIVER ili9881c_hd_dsi_vdo_holitek_lcm_drv = {
    .name = "ili9881c_hd_dsi_vdo_holitek",
