...
#define LCM_BL_MAX_LEVEL_MIN			LCM_BL_MAX_LEVEL_DEFAULT	//bl max level min check
...
#if (LCM_DSI_CMD_MODE)
	params->dsi.PLL_CLOCK = 364;	/* this value must be in MTK suggested table */
#else
	params->dsi.PLL_CLOCK = 364;	/* this value must be in MTK suggested table */
#endif
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	if (bl_lvl == level)
		LCM_LOGI("%s:tm_ili:level=bl_lvl=%d, para[0]=0x%x,para[1]=0x%x\n",__func__, bl_lvl, bl_level[1].para_list[0],bl_level[1].para_list[1]);
	else
		LCM_LOGI("%s:tm_ili:level=%d, bl_lvl=%d, para[0]=0x%x,para[1]=0x%x\n",__func__, level, bl_lvl, bl_level[1].para_list[0],bl_level[1].para_list[1]);
