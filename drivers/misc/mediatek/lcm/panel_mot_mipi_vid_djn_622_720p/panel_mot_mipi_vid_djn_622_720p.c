...
extern void nvt_lcm_update_firmware_work(void);
extern void wait_nvt_update_firmware(void);
...
static void lcm_init(void)
{
	...
	if (ret < 0)
		LCM_LOGI("nt36525B----ocp2131----cmd=%0x--i2c write error----\n", cmd);
	else
		LCM_LOGI("nt36525B----ocp2131----cmd=%0x--i2c write success----\n", cmd);
	...
	nvt_lcm_update_firmware_work();

	push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
	LCM_LOGI("nt36525B----djn----lcm mode = vdo mode ----\n");

	wait_nvt_update_firmware();
}
...
LCM_DRIVER mipi_mot_vid_djn_720p_622_lcm_drv = {
