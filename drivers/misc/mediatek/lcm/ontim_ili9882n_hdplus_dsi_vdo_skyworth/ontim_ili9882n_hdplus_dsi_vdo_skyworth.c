...
static const unsigned char LCD_MODULE_ID = 0x03;
...
static void lcm_reset(void)
{
	...
	LCM_LOGI("ili9881X lcm reset end.\n");
}
...
static void lcm_setbacklight(void *handle, unsigned int level)
{
	...
	LCM_LOGI("%s, WWW----988Xbacklight set level = %d \n", __func__, level);
...
static unsigned int lcm_compare_id(void)
{
	...
	LCM_LOGI("%s,ili9881X vendor id=0x%08x, id_flashed=%08x\n", __func__, id, id_flashed);


	if (id == LCM_ID && id_flashed == 0x03)
		return 1;
	else
		return 0;
#endif
...
struct LCM_DRIVER ontim_ili9882n_hdplus_dsi_vdo_skyworth_lcm_drv = {
	.name = "ontim_ili9882n_hdplus_dsi_vdo_skyworth",
	.lcm_id = 1,
