...
static void lcm_reset(void)
{
	...
	LCM_LOGI("%s:ft8006p lcm reset end\n",__func__);
}
...
static void lcm_suspend(void)
{
	...
	MDELAY(30);
	//    SET_RESET_PIN(0);
	LCM_LOGI("%s,ft8006P lcm_suspend power shut start\n", __func__);
	MDELAY(1);
	set_gpio_lcd_enn(0);
	MDELAY(1);
	...
	set_gpio_lcd_enp(0);
#endif
	LCM_LOGI("%s,ft8006p lcm_suspend power shutted over\n", __func__);
...
{
	LCM_LOGI("%s start: ft8006P \n",__func__);
	lcm_init();
	LCM_LOGI("%s end: ft8006P \n",__func__);
}
...
struct LCM_DRIVER ontim_ft8006p_hdplus_dsi_vdo_skyworth_lcm_drv = {
	.name = "ontim_ft8006p_hdplus_dsi_vdo_skyworth",
