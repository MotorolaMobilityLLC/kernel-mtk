...
{
	unsigned int id = 0, id1 = 0, version_id = 0,version_id1 = 0;
	...
	version_id = buffer[2];
	version_id1 = buffer[3];

	LCM_LOGI("%s,djn id=0x%02x, id1=0x%02x,version_id=0x%02x,version_id1=0x%02x\n", __func__, id, id1, version_id,version_id1);

	if (id == 0x0d && id1 == 0x04 && version_id == 0x94 && version_id1 == 0x92)
		return 1;
	else
		return 0;
}
...
LCM_DRIVER mipi_mot_vid_djn_720p_571_lcm_drv = {
	.name = "mipi_mot_vid_djn_720p_571",
