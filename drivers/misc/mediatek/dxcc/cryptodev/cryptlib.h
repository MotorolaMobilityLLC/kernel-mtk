...
int cryptodev_cipher_init(struct cipher_data *out, const char *alg_name,
              uint8_t *key, size_t keylen, int stream, int aead, int secure, int is_sync);
                ...
                void *iv, size_t iv_size)
{
    if (cdata->is_sync == 0)
        ...
        crypto_blkcipher_set_iv(cdata->sync.s, iv, crypto_blkcipher_ivsize(cdata->sync.s));
}
                ...
                void *iv, size_t iv_size)
{
    if (cdata->is_sync == 0)
        ...
        crypto_blkcipher_get_iv(cdata->sync.s, iv, min(iv_size, (size_t)crypto_blkcipher_ivsize(cdata->sync.s)));
}
