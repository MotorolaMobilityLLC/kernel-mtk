...
void my_kdump_hex(const char *title, uint8_t *buffer, size_t len)
{
    int i;

    if (title != NULL)
        printk("%s, size=%zu", title, len);
    for (i = 0; i < len; i++){
      if ((i%32 == 0) )
        printk("\n\t");
...
void my_kdump_address(const char *title, void *address, int type)
{
    if (title != NULL)
        printk("%s: ", title);
    printk("%p \n", address);
        ...
        uint32_t len)
{
    ...
    if (cop->op == COP_ENCRYPT) {
        dinfo(0, "hash_n_crypt COP_ENCRYPT: len=%d hdata.init=%d cdata.init=%d", len, ses_ptr->hdata.init, ses_ptr->cdata.init);
        ...
        dinfo(0, "hash_n_crypt COP_DECRYPT: len=%d hdata.init=%d cdata.init=%d", len, ses_ptr->hdata.init, ses_ptr->cdata.init);
        if (ses_ptr->cdata.init != 0) {
...
__crypto_run_std(struct csession *ses_ptr, struct crypt_op *cop)
{
    ...
    dinfo(0, "__crypto_run_std nbytes=%zu", nbytes);
    if (unlikely(!data)) {
    ...
    while (nbytes > 0) {
        ...
        dinfo(0, "copy_from_user current_len=%zu", current_len);
...
__crypto_run_zc(struct csession *ses_ptr, struct kernel_crypt_op *kcop)
{
    ...
    dinfo(0, "__crypto_run_zc cop->len=%d", cop->len);

    my_kdump_address("cop->src address", cop->src, 0);
    //my_kdump_hex("cop->src", cop->src, cop->len);
    
    my_kdump_address("cop->dst address", cop->dst, 0);
    //my_kdump_hex("cop->dst", cop->dst, cop->len);

    if (ses_ptr->cdata.secure == 0)
        ret = get_userbuf(ses_ptr, cop->src, cop->len, cop->dst, cop->len,
                          kcop->task, kcop->mm, &src_sg, &dst_sg);
    else
        ret = get_userbuf_and_securebuf(ses_ptr, cop->op, cop->src, cop->len, cop->dst, cop->len,
                          kcop->task, kcop->mm, &src_sg, &dst_sg);
        
    if (unlikely(ret)) {
        dinfo(0, "Error getting user pages ret=0x%08x. Falling back to non zero copy.",ret);
        derr(1, "Error getting user pages. Falling back to non zero copy.");
        return __crypto_run_std(ses_ptr, cop);
    }

    my_kdump_address("src_sg address", src_sg, 0);
    my_kdump_address("dst_sg address", dst_sg, 0);
...
int crypto_run(struct fcrypt *fcr, struct kernel_crypt_op *kcop)
{
    ...
    if (unlikely(cop->op != COP_ENCRYPT && cop->op != COP_DECRYPT)) {
        ddebug(0, "invalid operation op=%u", cop->op);
        return -EINVAL;
    }
    ...
    if (unlikely(!ses_ptr)) {
        derr(0, "invalid session ID=0x%08X", cop->ses);
        return -EINVAL;
    }

    if (ses_ptr->hdata.init != 0 && (cop->flags == 0 || cop->flags & COP_FLAG_RESET)) {
        dinfo(0, "cryptodev_hash_reset ");
    ...
    if (ses_ptr->cdata.init != 0) {
        ...
        dinfo(0, "cryptodev_cipher_set_iv len=%d ",min(ses_ptr->cdata.ivsize, kcop->ivlen));
    ...
    if (ses_ptr->cdata.init != 0) {
        dinfo(0, "cryptodev_cipher_get_iv ");
