...
crypto_create_session(struct fcrypt *fcr, struct session_op *sop)
{
    ...
    int hmac_mode = 1, stream = 0, aead = 0, secure = 0, is_sync = 0;
    /*
    ...
    if (unlikely(!sop->cipher && !sop->mac)) {
        dinfo(0, "Both 'cipher' and 'mac' unset.");
        return -EINVAL;
    }
    ...
    case CRYPTO_AES_XTS:
        alg_name = "xts(aes)";
        break;
    case CRYPTO_AES_CBC_CTS:
        alg_name = "cts1(cbc(aes))";
        break;
    case CRYPTO_AES_OFB:
        alg_name = "ofb(aes)";
        break;
    case CRYPTO_AES_CCM:
        ...
        aead = 1;
        break;
    case CRYPTO_AES_CCM_RFC4309:
        alg_name = "rfc4309(ccm(aes))";
        aead = 1;
        break;
    ...
    case CRYPTO_AES_GCM:
        ...
        aead = 1;
        break;
    case CRYPTO_AES_GCM_RFC4543:
        alg_name = "rfc4543(gcm(aes))";
        stream = 1;
        aead = 1;
        break;
    case CRYPTO_AES_GCM_RFC4106:
    ...
    case CRYPTO_NULL:
        ...
        stream = 1;
        break;
    case CRYPTO_SAES_CBC:
        alg_name = "cbc(saes)";
        secure = 1;
        break;
    case CRYPTO_SAES_CTR:
        alg_name = "ctr(saes)";
        secure = 1;
        break;
    case CRYPTO_SAES_CTS:
        alg_name = "cts1(cbc(saes))";
        secure = 1;
        break;
    case CRYPTO_SMULTI2_OFB:
        alg_name = "ofb(smulti2)";
        secure = 1;
        break;
    case CRYPTO_SMULTI2_CBC:
        alg_name = "cbc(smulti2)";
        secure = 1;
        break;
    case CRYPTO_SBYPASS:
        alg_name = "sbypass";
        secure = 1;
        break;
    case CRYPTO_SMULTI2_OFB_SYNC:
        alg_name = "ofb(smulti2)";
        secure = 1;
        is_sync = 1;
        break;
    case CRYPTO_SBYPASS_SYNC:
        alg_name = "sbypass";
        secure = 1;
        is_sync = 1;
        break;
    case CRYPTO_SMULTI2_CBC_SYNC:
        alg_name = "cbc(smulti2)";
        secure = 1;
        is_sync = 1;
        break;
    case CRYPTO_SAES_CTR_SYNC:
        alg_name = "ctr(saes)";
        secure = 1;
        is_sync = 1;
        break;
    case CRYPTO_SAES_CTS_SYNC:
        alg_name = "cts1(cbc(saes))";
        secure = 1;
        is_sync = 1;
        break;
    case CRYPTO_SAES_CBC_SYNC:
        alg_name = "cbc(saes)";
        secure = 1;
        is_sync = 1;
        break;
    default:
        dinfo(0, "bad cipher: %d", sop->cipher);
        return -EINVAL;
    }
    ...
    case CRYPTO_MD5_HMAC_SYNC:
        hash_name = "hmac(md5)";
        is_sync = 1;
        break;
    case CRYPTO_SHA2_224_HMAC_SYNC:
        hash_name = "hmac(sha224)";
        is_sync = 1;
        break;
    case CRYPTO_SHA2_256_HMAC_SYNC:
        hash_name = "hmac(sha256)";
        is_sync = 1;
        break;
    case CRYPTO_SHA1_HMAC_SYNC:
        hash_name = "hmac(sha1)";
        is_sync = 1;
        break;
    ...
    case CRYPTO_AES_CMAC:
        hash_name = "cmac(aes)";
        break;
    case CRYPTO_MD5_SYNC:
        hash_name = "md5";
        hmac_mode = 0;
        is_sync = 1;
        break;
    case CRYPTO_SHA2_224_SYNC:
        hash_name = "sha224";
        hmac_mode = 0;
        is_sync = 1;
        break;
    case CRYPTO_SHA2_256_SYNC:
        hash_name = "sha256";
        hmac_mode = 0;
        is_sync = 1;
        break;
    case CRYPTO_SHA1_SYNC:
        hash_name = "sha1";
        hmac_mode = 0;
        is_sync = 1;
        break;
    default:
        dinfo(0, "bad mac: %d", sop->mac);
        return -EINVAL;
    }
    ...
    if (alg_name) {
        ...
        dinfo(0, "setup crypto transform keylen=%d",keylen);
        if (unlikely(ret < 0)) {
            dinfo(0, "Setting key failed for %s-%zu.",
        ...
        if (unlikely(ret < 0)) {
            dinfo(0, "cryptodev_get_cipher_key failed for %s", alg_name);
            goto error_cipher;
        }

        ret = cryptodev_cipher_init(&ses_new->cdata, alg_name, keys.ckey,
                        keylen, stream, aead, secure, is_sync);
        if (ret < 0) {
            dinfo(0, "Failed to load cipher for %s", alg_name);
            ret = -EINVAL;
            goto error_cipher;
        }
    }
    ...
    if (hash_name && aead == 0) {
        dinfo(0, "setup hash transform sop->mackeylen=%d",sop->mackeylen);
        ...
        if (sop->mackey && unlikely(copy_from_user(keys.mkey, sop->mackey,
                        sop->mackeylen))) {
            dinfo(0, "sop->mackey  -EFAULT");
            ret = -EFAULT;
            goto error_hash;
        }

        ret = cryptodev_hash_init(&ses_new->hdata, hash_name, hmac_mode,
                            keys.mkey, sop->mackeylen, is_sync);
        if (ret != 0) {
            dinfo(0, "Failed to load hash for %s", hash_name);
            ret = -EINVAL;
            goto error_hash;
        }
    ...
    dinfo(0, "got alignmask %d", ses_new->alignmask);

    ses_new->array_size = DEFAULT_PREALLOC_PAGES;
    dinfo(0, "preallocating for %d user pages", ses_new->array_size);
            ...
            struct fcrypt *fcr, void __user *arg)
{
    ...
    dinfo(0, "kcop_from_user op=%d, len=%d, src=%p, dst=%p",kcop->cop.op, kcop->cop.len, kcop->cop.src, kcop->cop.dst );
...
static int get_session_info(struct fcrypt *fcr, struct session_info_op *siop)
{
    ...
    if (ses_ptr->cdata.init) {
        if (ses_ptr->cdata.aead == 0)
            if (ses_ptr->cdata.is_sync == 0)
                tfm = crypto_ablkcipher_tfm(ses_ptr->cdata.async.s);
            else
                tfm = crypto_blkcipher_tfm(ses_ptr->cdata.sync.s);
        else
    ...
    if (ses_ptr->hdata.init) {
        if (ses_ptr->hdata.is_sync == 0)
            tfm = crypto_ahash_tfm(ses_ptr->hdata.async.s);
        else
            tfm = crypto_hash_tfm(ses_ptr->hdata.sync.s);
...
cryptodev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg_)
{
    ...
    case CRIOGET:
        ...
        return ret;
    case CIOCGSESSION:
        dinfo(0, "copy_from_user ");
        if (unlikely(copy_from_user(&sop, arg, sizeof(sop))))
            return -EFAULT;

        ret = crypto_create_session(fcr, &sop);
        dinfo(0, "crypto_create_session return %d(0x%08x)",ret, ret);
        if (unlikely(ret))
            return ret;
        dinfo(0, "copy_to_user ");
        ret = copy_to_user(arg, &sop, sizeof(sop));
        if (unlikely(ret)) {
            dinfo(0, "crypto_finish_session ");
            crypto_finish_session(fcr, sop.ses);
            return -EFAULT;
        }
        dinfo(0, "end of CIOCGSESSION ");
        return ret;
    case CIOCFSESSION:
    ...
    case CIOCCRYPT:
        ...
        if (unlikely(ret)) {
            ...
            dinfo(0, "Error in crypto_run");
            return ret;
        }

        dinfo(0, "kcop_to_user ");
...
cryptodev_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg_)
{
    ...
    dinfo(0, "cryptodev_compat_ioctl ");
