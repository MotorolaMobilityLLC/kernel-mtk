...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
static void ssi_aead_exit(struct crypto_tfm *tfm)
#else
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
	struct ssi_aead_ctx *ctx = crypto_tfm_ctx(tfm);
...
static int ssi_aead_init(struct crypto_tfm *tfm)
#else
...
ssi_aead_setkey(struct crypto_aead *tfm, const u8 *key, unsigned int keylen)
{
	...
	if (seq_len > 0) { /* For CCM there is no sequence to setup the key */
		...
		ssi_req.op_type = STAT_OP_TYPE_SETKEY;
#endif
		rc = ssi_send_request(ctx->drvdata, &ssi_req, desc, seq_len, 0);
		if (unlikely(rc != 0)) {
			SSI_LOG_ERR("send_request() failed (rc=%d)\n", rc);
			goto setkey_error;
		}
	...
	unsigned int *seq_size)
{
	...
	case SSI_DMA_BUF_DLLI:
		...
		if (ctx->auth_mode == SEP_HASH_XCBC_MAC)
#else
		if (ctx->auth_mode == SEP_HASH_XCBC_MAC && (areq_ctx->cryptlen > 0) )
#endif
...
static int ssi_aead_process(struct aead_request *req, enum sep_crypto_direction direct)
{
	int rc = 0;
	int seq_len = 0;
...
#ifdef ENABLE_CYCLE_COUNT
	ssi_req.op_type = (direct == SEP_CRYPTO_DIRECTION_DECRYPT) ?
	...
	rc = ssi_send_request(ctx->drvdata, &ssi_req, desc, seq_len, 1);
...
static int ssi_aead_encrypt(struct aead_request *req)
{
	...
	rc = ssi_aead_process(req, SEP_CRYPTO_DIRECTION_ENCRYPT);
	if (rc != -EINPROGRESS)
...
static int ssi_aead_decrypt(struct aead_request *req)
{
	...
	rc = ssi_aead_process(req, SEP_CRYPTO_DIRECTION_DECRYPT);
	if (rc != -EINPROGRESS)
		req->iv = areq_ctx->backup_iv;
...
#endif /* SSI_CC_HAS_AES_GCM */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
static int ssi_aead_givencrypt(struct aead_givcrypt_request *req)
{
	...
	rc = ssi_aead_process(&req->areq, SEP_CRYPTO_DIRECTION_ENCRYPT);
	if (rc != -EINPROGRESS)
...
#if SSI_CC_HAS_AES_CCM
static int ssi_rfc4309_ccm_givencrypt(struct aead_givcrypt_request *req)
{
	/* Very similar to ssi_aead_givencrypt() above. */
	...
	ssi_rfc4309_ccm_process(&req->areq);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)			
			.givencrypt = ssi_aead_givencrypt,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
			.givencrypt = ssi_rfc4309_ccm_givencrypt,
#else
            .init = ssi_aead_init,
            .exit = ssi_aead_exit,
#endif
...
static struct ssi_crypto_alg *ssi_aead_create_alg(struct ssi_alg_template *template)
{
	...
	alg->cra_alignmask = 0;
	alg->cra_ctxsize = sizeof(struct ssi_aead_ctx);
	alg->cra_flags = CRYPTO_ALG_ASYNC | CRYPTO_ALG_KERN_DRIVER_ONLY |
			 template->type;
	alg->cra_init = ssi_aead_init;
	alg->cra_exit = ssi_aead_exit;
