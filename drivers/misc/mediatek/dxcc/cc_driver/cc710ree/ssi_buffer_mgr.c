...
dma_addr_t ssi_buff_mgr_update_dma_addr(dma_addr_t orig_addr, uint32_t data_len)
{
	...
	if((((orig_addr >> 16) & 0xFFFF) != 0xFFFF) && 
		(data_len <= SASI_MAX_MLLI_ENTRY_SIZE)) {
#else
	...
	enum ssi_sg_cpy_direct direct)
{
	...
	while (sg_index <= to_skip) {
		current_sg = sg_next(current_sg);
		//if have reached the end of the sgl, then there is nothing to do. just return back
		if (current_sg == NULL) 
			return;
		sg_index += current_sg->length;
		nents--;
	...
	uint32_t **mlli_entry_pp)
{
	...
	new_nents = (*curr_nents + buff_size/SASI_MAX_MLLI_ENTRY_SIZE + 1);
	if (new_nents > MAX_NUM_OF_TOTAL_MLLI_ENTRIES ) {
		return -ENOMEM;
	}

	/*handle buffer longer than 64 kbytes */
	while (buff_size > SASI_MAX_MLLI_ENTRY_SIZE ) {
		SSI_UPDATE_DMA_ADDR_TO_48BIT(buff_dma, SASI_MAX_MLLI_ENTRY_SIZE);
		LLI_SET_ADDR(mlli_entry_p,buff_dma);
		LLI_SET_SIZE(mlli_entry_p, SASI_MAX_MLLI_ENTRY_SIZE);
		SSI_LOG_DEBUG("entry[%d]: single_buff=0x%08X size=%08X\n",*curr_nents,
			   mlli_entry_p[LLI_WORD0_OFFSET],
			   mlli_entry_p[LLI_WORD1_OFFSET]);
		SSI_RESTORE_DMA_ADDR_TO_48BIT(buff_dma);
		buff_dma += SASI_MAX_MLLI_ENTRY_SIZE;
		buff_size -= SASI_MAX_MLLI_ENTRY_SIZE;
	...
	struct scatterlist *dst)
{
	...
	if (src != dst) {
		if (req_ctx->sec_dir != SSI_DST_DMA_IS_SECURE) {
	...
	struct scatterlist *dst)
{
	...
	if (sg_is_last(src) &&
		...
		req_ctx->sec_dir = SSI_SRC_DMA_IS_SECURE;
		req_ctx->in_nents = 1;
		SSI_UPDATE_DMA_ADDR_TO_48BIT(sg_dma_address(src),
					    nbytes);
	} else {
	...
	if (unlikely(src == dst)) {
		if (req_ctx->sec_dir == SSI_SRC_DMA_IS_SECURE) {
			SSI_LOG_ERR("Inplace operation for Secure key "
				   "is un-supported\n");
		...
		if (sg_is_last(dst) && (sg_page(dst) == NULL) &&
		    sg_dma_address(dst)) {
			if (req_ctx->sec_dir == SSI_SRC_DMA_IS_SECURE) {
				SSI_LOG_ERR("Secure key in both sides is"
					   "un-supported \n");
				/* both sides are secure */
				rc = -ENOMEM;
				goto ablkcipher_exit;
			...
			req_ctx->sec_dir = SSI_DST_DMA_IS_SECURE;
			req_ctx->out_nents = 1;
			SSI_UPDATE_DMA_ADDR_TO_48BIT(sg_dma_address(dst),
						    nbytes);
		} else {
			/* Map the dst sg */
	...
	struct device *dev, struct aead_request *req)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
	if (req->assoclen != 0) {
		SSI_LOG_DEBUG("Unmapping sgl assoc: req->assoc=%pK\n", 
			sg_virt(req->assoc));
		SSI_RESTORE_DMA_ADDR_TO_48BIT(sg_dma_address(req->assoc));
    ...
    if(areq_ctx->gen_ctx.op_type == SEP_CRYPTO_DIRECTION_ENCRYPT){
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
	rc = ssi_buffer_mgr_map_scatterlist(dev, req->assoc,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,3,0)
			req->assoc, req->assoclen, 0,is_last,
#else
	...
	bool is_last_table, bool do_chain)
{
	...
	rc = ssi_buffer_mgr_map_scatterlist(dev, req->src, size_for_map,
...
void ssi_buffer_mgr_update_aead_mlli_nents( struct ssi_drvdata *drvdata,
					   struct aead_request *req)
{
	struct aead_req_ctx *areq_ctx = aead_request_ctx(req);
...
/**
 * ssi_buffer_mgr_init_scatterlist_one() - This function should be used by kerenl user
...
EXPORT_SYMBOL(ssi_buffer_mgr_init_scatterlist_one);
