...
do { \
	...
	if ((is_monitored) == true) { \
		elapsed_cycles = READ_REGISTER((cc_base_addr) + SASI_REG_OFFSET(CRY_KERNEL, DSCRPTR_MEASURE_CNTR)); \
...
int request_mgr_init(struct ssi_drvdata *drvdata)
{
	...
	req_mgr_h->hw_queue_size = READ_REGISTER(drvdata->cc_base +
		SASI_REG_OFFSET(CRY_KERNEL, DSCRPTR_QUEUE_SRAM_SIZE));
	...
	rc = ssi_send_request(drvdata, &monitor_req, monitor_desc, 2, 0);
	if (unlikely(rc != 0))
		goto req_mgr_init_err;

	drvdata->monitor_null_cycles = READ_REGISTER(drvdata->cc_base +
		SASI_REG_OFFSET(CRY_KERNEL, DSCRPTR_MEASURE_CNTR));
	...
	HwDesc_s *desc, unsigned int len, bool is_dout)
{
	...
	SSI_LOG_DEBUG("send_request trace-1\n");
	...
	struct ssi_drvdata *drvdata, HwDesc_s *desc, unsigned int len)
{
	...
	/* Update the free slots in HW queue */
	req_mgr_h->q_free_slots = SASI_HAL_READ_REGISTER(
					SASI_REG_OFFSET(CRY_KERNEL,
					 DSCRPTR_QUEUE0_CONTENT));
...
static void proc_completions(struct ssi_drvdata *drvdata)
{
	...
	while(request_mgr_handle->axi_completed) {
		...
		/* Delay */
		{
			...
			SSI_LOG_DEBUG("Delay\n");
			for (i=0;i<1000000;i++) {
				axi_err = READ_REGISTER(drvdata->cc_base + SASI_REG_OFFSET(CRY_KERNEL, AXIM_MON_ERR));
			}
		}
...
static void comp_handler(unsigned long devarg)
{
	...
	WRITE_REGISTER(cc_base + SASI_REG_OFFSET(HOST_RGF, HOST_ICR), SSI_COMP_IRQ_MASK);

	/* Avoid race with above clear: Test completion counter once more */
	request_mgr_handle->axi_completed += SASI_REG_FLD_GET(CRY_KERNEL, AXIM_MON_COMP8, VALUE, 
	...
	WRITE_REGISTER(cc_base + SASI_REG_OFFSET(HOST_RGF, HOST_IMR), 
		...
		SASI_REG_OFFSET(HOST_RGF, HOST_IMR)) & ~(SSI_COMP_IRQ_MASK));
