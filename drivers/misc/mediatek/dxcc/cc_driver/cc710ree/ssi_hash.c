								...
								unsigned int digestsize)
{
	state->digest_result_dma_addr =
		dma_map_single(dev, (void *)state->heap_buff->digest_result_buff,
			       digestsize,
			       DMA_BIDIRECTIONAL);
	...
	SSI_LOG_DEBUG("Mapped digest result buffer %u B "
		...
		digestsize, state->heap_buff->digest_result_buff,
								...
								struct ssi_hash_ctx *ctx)
{
	...
	state->heap_buff = kzalloc(sizeof(*state->heap_buff), GFP_KERNEL|GFP_DMA);
	if (!state->heap_buff) {
		SSI_LOG_ERR("Allocating heap_buff in context failed\n");
		goto fail0;
	}
	...
	if (is_hmac) {
...
#if (DX_DEV_SHA_MAX > 256)
			if (unlikely((ctx->hash_mode == SEP_HASH_SHA512) || (ctx->hash_mode == SEP_HASH_SHA384))) {
		...
		if (ctx->hash_mode != SEP_HASH_NULL) {
		...
		rc = ssi_send_request(ctx->drvdata, &ssi_req, &desc, 1, 0);
		if (unlikely(rc != 0)) {
	...
	if (is_hmac && ctx->hash_mode != SEP_HASH_NULL) {
...
fail1:
	if (state->heap_buff != NULL) {
		kfree(state->heap_buff);
		state->heap_buff = NULL;
	}
fail0:
	return rc;
}
									...
									unsigned int digestsize, u8 *result)
{
	if (state->digest_result_dma_addr != 0) {
		...
		SSI_LOG_DEBUG("unmpa digest result buffer "
			     "va (%pK) pa (%llx) len %u\n",
			     state->heap_buff->digest_result_buff,
			     (unsigned long long)state->digest_result_dma_addr,
			     digestsize);
		memcpy(result,
		       state->heap_buff->digest_result_buff,
							...
							void *async_req)
{
	...
	HW_DESC_INIT(&desc[idx]);
	HW_DESC_SET_CIPHER_MODE(&desc[idx], ctx->hw_mode); 
	HW_DESC_SET_DOUT_DLLI(&desc[idx], state->digest_result_dma_addr, digestsize, AXI_ID, NS_BIT, async_req? 1:0);   /*TODO*/
	HW_DESC_SET_FLOW_MODE(&desc[idx], S_HASH_to_DOUT);
	HW_DESC_SET_SETUP_MODE(&desc[idx], SETUP_WRITE_STATE0);
	...
	if (async_req) {
		rc = ssi_send_request(ctx->drvdata, &ssi_req, desc, idx, 1);
		...
		rc = ssi_send_request(ctx->drvdata, &ssi_req, desc, idx, 0);
		if (rc != 0) {
			SSI_LOG_ERR("send_request() failed (rc=%d)\n", rc);
			ssi_buffer_mgr_unmap_hash_request(dev, state, src, true);
		} else {
							...
							void *async_req)
{
	...
	HW_DESC_INIT(&desc[idx]);
	HW_DESC_SET_CIPHER_MODE(&desc[idx], ctx->hw_mode);
	HW_DESC_SET_DOUT_DLLI(&desc[idx], state->digest_bytes_len_dma_addr, HASH_LEN_SIZE, AXI_ID, NS_BIT, async_req? 1:0);
	HW_DESC_SET_FLOW_MODE(&desc[idx], S_HASH_to_DOUT);
	HW_DESC_SET_SETUP_MODE(&desc[idx], SETUP_WRITE_STATE1);
	idx++;
							...
							void *async_req)
{
	...
	/* Get final MAC result */
	HW_DESC_INIT(&desc[idx]);
	HW_DESC_SET_DOUT_DLLI(&desc[idx], state->digest_result_dma_addr, digestsize, AXI_ID, NS_BIT, async_req? 1:0);
	HW_DESC_SET_FLOW_MODE(&desc[idx], S_HASH_to_DOUT);
	HW_DESC_SET_CIPHER_CONFIG1(&desc[idx], HASH_PADDING_DISABLED);
	HW_DESC_SET_SETUP_MODE(&desc[idx], SETUP_WRITE_STATE0);
	ssi_set_hash_endianity(ctx->hash_mode,&desc[idx]);
							...
							bool synchronize)
{
	...
	if (unlikely(dma_mapping_error(&ctx->drvdata->plat_dev->dev,
		...
		ctx->key_params.key_dma_addr = 0x0;
	}
...
static struct ssi_hash_template driver_hash[] = {
	//Synchronize hash template
	{
		.name = "sha1",
		.driver_name = "sha1-sync-dx",
		.hmac_name = "hmac(sha1)",
		.hmac_driver_name = "hmac-sha1-sync-dx",
		.blocksize = SHA1_BLOCK_SIZE,
		.synchronize = true,
		.template_shash = {
			.init = ssi_shash_init,
			.update = ssi_shash_update,
			.final = ssi_shash_final,
			.finup = ssi_shash_finup,
			.digest = ssi_shash_digest,
			.export = ssi_shash_export,
			.import = ssi_shash_import,
			.setkey = ssi_shash_setkey,
			.digestsize = SHA1_DIGEST_SIZE,
			.descsize = sizeof(struct ahash_req_ctx),
            .statesize =  sizeof(struct sha1_state),
			},		
		.hash_mode = SEP_HASH_SHA1,
		.hw_mode = SEP_HASH_HW_SHA1,
		.inter_digestsize = SHA1_DIGEST_SIZE,
	},
	{
		.name = "sha256",
		.driver_name = "sha256-sync-dx",
		.hmac_name = "hmac(sha256)",
		.hmac_driver_name = "hmac-sha256-sync-dx",
		...
		.hash_mode = SEP_HASH_SHA256,
		.hw_mode = SEP_HASH_HW_SHA256,
		.inter_digestsize = SHA256_DIGEST_SIZE,
	},
	{
		.name = "sha224",
		.driver_name = "sha224-sync-dx",
		.hmac_name = "hmac(sha224)",
		.hmac_driver_name = "hmac-sha224-sync-dx",
	...
	{
		.name = "sha384",
		.driver_name = "sha384-sync-dx",
		.hmac_name = "hmac(sha384)",
		.hmac_driver_name = "hmac-sha384-sync-dx",
	...
	{
		.name = "sha512",
		.driver_name = "sha512-sync-dx",
		.hmac_name = "hmac(sha512)",
		.hmac_driver_name = "hmac-sha512-sync-dx",
	...
	{
		.name = "md5",
		.driver_name = "md5-sync-dx",
		.hmac_name = "hmac(md5)",
		.hmac_driver_name = "hmac-md5-sync-dx",
...
int ssi_hash_alloc(struct ssi_drvdata *drvdata)
{
	...
	for (alg = 0; alg < ARRAY_SIZE(driver_hash); alg++) {
		if (driver_hash[alg].synchronize)
			continue;

		/* register hmac version */

		if ((((struct ssi_hash_template)driver_hash[alg]).hw_mode != SEP_CIPHER_XCBC_MAC) &&
			(((struct ssi_hash_template)driver_hash[alg]).hw_mode != SEP_CIPHER_CMAC)) {
