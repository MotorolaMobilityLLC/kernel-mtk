...
__weak void *mt_spm_base_get(void) { return 0; }
...
#ifdef SW_TX_TRACKING
static unsigned int read_dram_mode_reg(unsigned int mr_index,
	...
	for (iChannelIdx = 0; iChannelIdx < get_ch_num(); iChannelIdx++) {
		if ((DRAM_TYPE == TYPE_LPDDR4) ||
			...
			if (iChannelIdx == 0) { /*Channel-A*/
				...
				u4rg_88 =
				    IOMEM(DRAMC_NAO_CHA_BASE_ADDR + 0x88);
			} else { /*Channel-B*/
				...
				u4rg_88 =
				    IOMEM(DRAMC_NAO_CHB_BASE_ADDR + 0x88);
			}
...
unsigned int get_dram_data_rate(void)
{
	...
	u4SDM_PCW = readl(IOMEM(DDRPHY_AO_CHA_BASE_ADDR + 0xd94 +
				0x500 * u4ShuLevel)) >>
		    16;
	u4PREDIV = (readl(IOMEM(DDRPHY_AO_CHA_BASE_ADDR + 0xda0 +
				0x500 * u4ShuLevel)) &
		    0x000c0000) >>
		   18;
	u4POSDIV = readl(IOMEM(DDRPHY_AO_CHA_BASE_ADDR + 0xda0 +
			       0x500 * u4ShuLevel)) &
		   0x00000007;
	u4CKDIV4 = (readl(IOMEM(DDRPHY_AO_CHA_BASE_ADDR + 0xd18 +
				0x500 * u4ShuLevel)) &
		    0x08000000) >>
		   27;
...
static ssize_t read_dram_data_rate_show(struct device_driver *driver,
					char *buf)
{
...
static int dram_probe(struct platform_device *pdev)
{
	...
	pr_debug("[DRAMC]get DDRPHY_AO_CHA_BASE_ADDR @ %p\n",
		DDRPHY_AO_CHA_BASE_ADDR);
	pr_debug("[DRAMC]get DDRPHY_AO_CHB_BASE_ADDR @ %p\n",
	...
	pr_debug("[DRAMC]get DDRPHY_NAO_CHA_BASE_ADDR @ %p\n",
		DDRPHY_NAO_CHA_BASE_ADDR);
	pr_debug("[DRAMC]get DDRPHY_NAO_CHB_BASE_ADDR @ %p\n",
	...
	CH_NUM = get_ch_num();
	pr_debug("[DRAMC Driver] Channel num =%d\n", CH_NUM);

	if (!CH_NUM) {
		pr_debug("[DRAMC Driver] channel number error !!\n");
		return -1;
	}
