...
void __iomem *DRAMC_AO_CHC_BASE_ADDR;
void __iomem *DRAMC_AO_CHD_BASE_ADDR;

void __iomem *DRAMC_NAO_CHA_BASE_ADDR;
void __iomem *DRAMC_NAO_CHB_BASE_ADDR;
void __iomem *DRAMC_NAO_CHC_BASE_ADDR;
void __iomem *DRAMC_NAO_CHD_BASE_ADDR;
...
void *mt_dramc_chc_base_get(void)
{
	return DRAMC_AO_CHC_BASE_ADDR;
}
EXPORT_SYMBOL(mt_dramc_chc_base_get);

void *mt_dramc_chd_base_get(void)
{
	return DRAMC_AO_CHD_BASE_ADDR;
}
EXPORT_SYMBOL(mt_dramc_chd_base_get);
...
void *mt_dramc_nao_chc_base_get(void)
{
	return DRAMC_NAO_CHC_BASE_ADDR;
}
EXPORT_SYMBOL(mt_dramc_nao_chc_base_get);

void *mt_dramc_nao_chd_base_get(void)
{
	return DRAMC_NAO_CHD_BASE_ADDR;
}
EXPORT_SYMBOL(mt_dramc_nao_chd_base_get);
...
void *mt_ddrphy_chc_base_get(void)
{
	return DDRPHY_CHC_BASE_ADDR;
}
EXPORT_SYMBOL(mt_ddrphy_chc_base_get);

void *mt_ddrphy_chd_base_get(void)
{
	return DDRPHY_CHD_BASE_ADDR;
}
...
static tx_result dramc_tx_tracking(int channel)
{
	...
	if (channel == 0) {
		...
		dramc_nao_chx_base = DRAMC_NAO_CHC_BASE_ADDR;
		ddrphy_chx_base = DDRPHY_CHC_BASE_ADDR;
	} else {
		dramc_ao_chx_base = DRAMC_AO_CHD_BASE_ADDR;
		dramc_nao_chx_base = DRAMC_NAO_CHD_BASE_ADDR;
...
int enter_pasr_dpd_config(unsigned char segment_rank0, unsigned char segment_rank1)
{
	...
	for (iChannelIdx = 0; iChannelIdx < CHANNEL_NUMBER; iChannelIdx++) {
		...
		for (iRankIdx = 0; iRankIdx < dram_rank_num; iRankIdx++) {
	...
	for (RankCounter = 0; RankCounter < dram_rank_num; RankCounter++) {
		for (CHCounter = 0; CHCounter < dram_channel_num; CHCounter++) {
			...
			if (CHCounter == 0) {
				...
				u4rg_24 = IOMEM(DRAMC_AO_CHC_BASE_ADDR + 0x24);
				u4rg_38 = IOMEM(DRAMC_AO_CHC_BASE_ADDR + 0x38);
				u4rg_5C = IOMEM(DRAMC_AO_CHC_BASE_ADDR + 0x5C);
				u4rg_60 = IOMEM(DRAMC_AO_CHC_BASE_ADDR + 0x60);
				u4rg_88 = IOMEM(DRAMC_NAO_CHC_BASE_ADDR + 0x88);
			} else if (CHCounter == 3) {
				u4rg_24 = IOMEM(DRAMC_AO_CHD_BASE_ADDR + 0x24);
				u4rg_38 = IOMEM(DRAMC_AO_CHD_BASE_ADDR + 0x38);
				u4rg_5C = IOMEM(DRAMC_AO_CHD_BASE_ADDR + 0x5C);
				u4rg_60 = IOMEM(DRAMC_AO_CHD_BASE_ADDR + 0x60);
				u4rg_88 = IOMEM(DRAMC_NAO_CHD_BASE_ADDR + 0x88);
