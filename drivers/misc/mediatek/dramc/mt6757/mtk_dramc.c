...
#define dramc_rsv_aee_warn(string, args...)                                    \
	do {                                                                   \
...
static int __init dt_scan_dram_info(unsigned long node, const char *uname,
				    int depth, void *data)
{
...
void *mt_dramc_cha_base_get(void) { return DRAMC_AO_CHA_BASE_ADDR; }
EXPORT_SYMBOL(mt_dramc_cha_base_get);

void *mt_dramc_chb_base_get(void) { return DRAMC_AO_CHB_BASE_ADDR; }
EXPORT_SYMBOL(mt_dramc_chb_base_get);

void *mt_dramc_nao_cha_base_get(void) { return DRAMC_NAO_CHA_BASE_ADDR; }
EXPORT_SYMBOL(mt_dramc_nao_cha_base_get);

void *mt_dramc_nao_chb_base_get(void) { return DRAMC_NAO_CHB_BASE_ADDR; }
EXPORT_SYMBOL(mt_dramc_nao_chb_base_get);

void *mt_ddrphy_cha_base_get(void) { return DDRPHY_CHA_BASE_ADDR; }
EXPORT_SYMBOL(mt_ddrphy_cha_base_get);

void *mt_ddrphy_chb_base_get(void) { return DDRPHY_CHB_BASE_ADDR; }
EXPORT_SYMBOL(mt_ddrphy_chb_base_get);

unsigned int get_dram_type(void) { return DRAM_TYPE; }
EXPORT_SYMBOL(get_dram_type);

#ifdef SW_TX_TRACKING
static tx_result read_dram_mode_reg(unsigned int mr_index,
...
#if 0
	if (time_cnt == 0)
		pr_debug("[DRAMC] read mode reg time out 2\n");
#endif
...
static tx_result start_dram_dqs_osc(void __iomem *dramc_ao_chx_base,
				    void __iomem *dramc_nao_chx_base)
{
	unsigned int response;
	unsigned int time_cnt;
				   ...
				   void __iomem *dramc_nao_chx_base)
{
	...
	temp = Reg_Readl(DRAMC_AO_CKECTRL) & ~((0x1 << 5) | (0x1 << 7));
	Reg_Sync_Writel(DRAMC_AO_CKECTRL, temp | (0x1 << 4) | (0x1 << 6));
	...
	res = read_dram_mode_reg(18, &mr18_cur, dramc_ao_chx_base,
				 dramc_nao_chx_base);
	if (res != TX_DONE)
		goto ret_auto_dram_dqs_osc;
	res = read_dram_mode_reg(19, &mr19_cur, dramc_ao_chx_base,
				 dramc_nao_chx_base);
	if (res != TX_DONE)
		goto ret_auto_dram_dqs_osc;
	...
	pr_debug("[DRAMC] Rank %d, (LSB)MR18= 0x%x, (MSB)MR19= 0x%x, tDQSOscB0 = %d ps tDQSOscB1 = %d ps\n",
...
static tx_result dramc_tx_tracking(int channel)
{
	...
	dqsosc_inc =
	    (Reg_Readl(DRAMC_AO_DQSOSC_PRD + shu_offset_dramc) >> 16) & 0xFF;
	dqsosc_dec =
	    (Reg_Readl(DRAMC_AO_DQSOSC_PRD + shu_offset_dramc) >> 24) & 0xFF;

	/* mr1819_base[rank][byte] */
	mr1819_base[0][0] =
	    (Reg_Readl(DRAMC_AO_SHU1RK0_DQSOSC + shu_offset_dramc) >> 0) &
	    0xFFFF;
	mr1819_base[1][0] =
	    (Reg_Readl(DRAMC_AO_SHU1RK1_DQSOSC + shu_offset_dramc) >> 0) &
	    0xFFFF;
	if (CBT_MODE == BYTE_MODE) {
		mr1819_base[0][1] =
		    (Reg_Readl(DRAMC_AO_SHU1RK0_DQSOSC + shu_offset_dramc) >>
		     16) &
		    0xFFFF;
		mr1819_base[1][1] =
		    (Reg_Readl(DRAMC_AO_SHU1RK1_DQSOSC + shu_offset_dramc) >>
		     16) &
		    0xFFFF;
	...
	for (rank = 0; rank < 2; rank++) {
		res = auto_dram_dqs_osc(rank, dramc_ao_chx_base,
					dramc_nao_chx_base);
		if (res != TX_DONE)
			...
			if (mr1819_cur[byte] >= mr1819_base[rank][byte]) {
				...
				for (shu_index = 0; shu_index < 3;
					...
					dqm_new[shu_index][rank][byte] =
					    ...
					    0x3F;
#if 0 /* print message for debugging */
pr_debug("[DRAMC], CH%d RK%d B%d, shu=%d base=%X cur=%X delta=%d INC=%d PI=0x%x Adj=%d newPI=0x%x\n",
...
pi_new[shu_index][rank][byte]);
				...
				for (shu_index = 0; shu_index < 3;
					...
					dqm_new[shu_index][rank][byte] =
					    ...
					    0x3F;
#if 0 /* print message for debugging */
pr_debug("[DRAMC], CH%d RK%d B%d, shu=%d base=%X cur=%X delta=%d DEC=%d PI=0x%x Adj=%d newPI=0x%x\n",
channel, rank, byte, shu_index, mr1819_base[rank][byte], mr1819_cur[byte],
...
pi_new[shu_index][rank][byte]);
	...
	if (time_cnt == 0) {
		pr_debug("[DRAMC] write DDRPHY time out\n");
...
unsigned int mt_dramc_ta_support_ranks(void) { return dram_rank_num; }

unsigned int read_dram_mode_reg_by_rank(unsigned int mr_index,
					unsigned int *mr_value,
					unsigned int rank, unsigned int channel)
{
	...
	res = read_dram_mode_reg(mr_index, mr_value, dramc_ao_chx_base,
				 dramc_nao_chx_base);
	if (res != TX_DONE)
		ret = -1;
...
static int __init set_single_channel_test_angent(int channel)
{
	...
	if (emi_base == NULL) {
		pr_debug("[LastDRAMC] can't find EMI base\n");
		return -1;
	}
	...
	/* pr_debug("[LastDRAMC] reserved address before emi: %llx\n",
	 * test_agent_base);
	 */
	 ...
	 */
	if ((emi_cona & 0x300) != 0) {
		/* pr_debug("[LastDRAMC] two channels\n"); */
		temp = (test_agent_base &
			(0x1ffffffff << (channel_position + 1))) >>
		       1;
		test_agent_base =
		    temp | (test_agent_base &
			    (0x1ffffffff >> (33 - channel_position)));
	}
	/* pr_debug("[LastDRAMC] reserved address after emi: %llx\n",
	 * test_agent_base);
	 */
	...
	if ((DRAM_TYPE == TYPE_LPDDR4) || (DRAM_TYPE == TYPE_LPDDR4X))
		...
		pr_debug("[LastDRAMC] undefined DRAM type\n");
		return -1;
	}
...
static int __init last_dramc_test_agent_init(void)
{
	...
	if (get_emi_base == NULL) {
		pr_debug("[LastDRAMC] mt_emi_base_get is NULL\n");
		return 0;
	}
			  ...
			  unsigned char segment_rank1)
{
	...
	if ((get_dram_info->rank_info[1].start >=
		 (get_max_DRAM_size() + DRAM_BASE) &&
	...
	local_irq_save(save_flags);
	if (acquire_dram_ctrl() != 0) {
		pr_debug("[DRAMC0] can NOT get SPM HW SEMAPHORE!\n");
		local_irq_restore(save_flags);
		return -1;
	}
	...
	for (iChannelIdx = 0; iChannelIdx < 2; iChannelIdx++) {
		if ((DRAM_TYPE == TYPE_LPDDR4) || (DRAM_TYPE == TYPE_LPDDR4X)) {
			if (iChannelIdx == 0) { /*Channel-A*/
			   ...
			   TYPE_LPDDR3) {
		 /* #else PASR_TEST_SCENARIO == PASR_SUPPORT_1_CHANNEL LPDDR3 */
			if (iChannelIdx == 1)
				break;
			u4rg_24 = IOMEM(DRAMC_AO_CHA_BASE_ADDR + 0x24);
			u4rg_64 = IOMEM(DRAMC_AO_CHA_BASE_ADDR + 0x64);
		...
		writel(readl(u4rg_38) & 0xBFFFFFFF,
		       u4rg_38); /* PHYCLKDYNGEN = 0 */
		writel(readl(u4rg_38) | 0x04000000,
		       u4rg_38); /* MIOCKCTRLOFF = 1 */
		...
		for (iRankIdx = 0; iRankIdx < 2; iRankIdx++) {
			...
			do {
				if (cnt-- == 0) {
					if (iRankIdx == 0)
						pr_debug("[DRAMC0] CHA PASR "
							 "MRW fail!\n");
					else
						pr_debug("[DRAMC0] CHB PASR "
							 "MRW fail!\n");
#if !__ETT__
					if (release_dram_ctrl() != 0)
						pr_debug("[DRAMC0] release SPM "
							 "HW SEMAPHORE "
							 "fail!\n");
					/* pr_debug("[DRAMC0] release SPM HW
					 * SEMAPHORE success!\n");
					 */
					local_irq_restore(save_flags);
#endif
...
{
	unsigned int u4ShuLevel, u4SDM_PCW, u4PREDIV, u4POSDIV, u4CKDIV4,
	    u4VCOFreq, u4DataRate = 0;

	u4ShuLevel = get_shuffle_status();

	u4SDM_PCW =
	    readl(IOMEM(DDRPHY_CHA_BASE_ADDR + 0xd94 + 0x500 * u4ShuLevel)) >>
	    16;
	u4PREDIV =
	    (readl(IOMEM(DDRPHY_CHA_BASE_ADDR + 0xda0 + 0x500 * u4ShuLevel)) &
	     0x000c0000) >>
	    18;
	u4POSDIV =
	    readl(IOMEM(DDRPHY_CHA_BASE_ADDR + 0xda0 + 0x500 * u4ShuLevel)) &
	    0x00000007;
	u4CKDIV4 =
	    (readl(IOMEM(DDRPHY_CHA_BASE_ADDR + 0xd18 + 0x500 * u4ShuLevel)) &
	     0x08000000) >>
	    27;
	...
	/* pr_debug("[DRAMC Driver] PCW=0x%X, u4PREDIV=%d, u4POSDIV=%d,
	 * CKDIV4=%d, DataRate=%d\n",
	 */
...
int get_ddr_type(void) { return DRAM_TYPE; }
int get_emi_ch_num(void) { return get_ch_num(); }
...
static int dram_probe(struct platform_device *pdev)
{
	...
	if (!_reg_VCORE)
		pr_debug("[DRAMC] Regulator_get _reg_VCORE fail\n");
	_reg_VDRAM1 = regulator_get(&pdev->dev, "vdram1");
	if (!_reg_VDRAM1)
		pr_debug("[DRAMC] Regulator_get _reg_VDRAM1 fail\n");
	_reg_VDRAM2 = regulator_get(&pdev->dev, "vdram2");
	if (!_reg_VDRAM2)
		pr_debug("[DRAMC] Regulator_get _reg_VDRAM2 fail\n");
#endif

	return ret;
}

static int dram_remove(struct platform_device *dev) { return 0; }
...
static int dram_dt_init(void)
{
	...
	if (node) {
		...
		pr_debug("[DRAMC]can't find DRAMC0 CHA compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_debug("[DRAMC]get DRAMC_AO_CHB_BASE_ADDR @ %p\n",
			 DRAMC_AO_CHB_BASE_ADDR);
	} else {
		pr_debug("[DRAMC]can't find DRAMC0 CHB compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_debug("[DRAMC]can't find DDRPHY CHA compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_debug("[DRAMC]get DDRPHY_CHB_BASE_ADDR @ %p\n",
			 DDRPHY_CHB_BASE_ADDR);
	} else {
		pr_debug("[DRAMC]can't find DDRPHY CHB compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_debug("[DRAMC]can't find DRAMCNAO CHA compatible node\n");
		return -1;
	}
	...
	if (node) {
		...
		pr_debug("[DRAMC]get DRAMC_NAO_CHB_BASE_ADDR @ %p\n",
			 DRAMC_NAO_CHB_BASE_ADDR);
	} else {
		pr_debug("[DRAMC]can't find DRAMCNAO CHB compatible node\n");
		return -1;
	}
...
void zqcs_timer_callback(unsigned long data)
{
	...
	for (RankCounter = 0; RankCounter < 2; RankCounter++) {
		for (CHCounter = 0; CHCounter < 2; CHCounter++) {
			...
			writel(readl(u4rg_38) & 0xFFFFFFFD,
			       u4rg_38); /* DCMEN2 */
			writel(readl(u4rg_38) & 0xBFFFFFFF,
			       u4rg_38); /* DMPHYCLKDYNGEN */
			writel(readl(u4rg_38) | 0x04000000,
			       u4rg_38); /* DMMIOCKCTRLOFF */
			writel(readl(u4rg_24) | 0x40, u4rg_24); /* DMCKEFIXON */
			writel(readl(u4rg_24) | 0x10,
			       u4rg_24); /* DMCKE1FIXON */

			if (RankCounter == 0)
				writel(readl(u4rg_5C) & 0xCFFFFFFF,
				       u4rg_5C); /* Rank 0 */
			...
			writel(readl(u4rg_60) | 0x10,
			       u4rg_60); /* for ZQCal Start */
			...
			writel(readl(u4rg_60) & 0xFFFFFFEF,
			       u4rg_60); /* ZQCal Stop */

			if (TimeCnt == 0) { /* time out */
				writel(readl(u4rg_24) & 0xFFFFFFEF,
				       u4rg_24); /* DMCKE1FIXON */
				writel(readl(u4rg_24) & 0xFFFFFFBF,
				       u4rg_24); /* DMCKEFIXON */
				writel(readl(u4rg_38) & 0xFBFFFFFF,
				       u4rg_38); /* DMMIOCKCTRLOFF */
				writel(readl(u4rg_38) | 0x40000000,
				       u4rg_38); /* DMPHYCLKDYNGEN */
				writel(readl(u4rg_38) | 0x00000002,
				       u4rg_38); /* DCMEN2 */
				if (release_dram_ctrl() != 0)
					pr_debug("[DRAMC] release SPM HW "
						 "SEMAPHORE fail!\n");
			...
			TimeCnt = 100;
			writel(readl(u4rg_60) | 0x40,
			       u4rg_60); /* for ZQCal latch */
			...
			writel(readl(u4rg_60) & 0xFFFFFFBF,
			       u4rg_60); /* ZQ latch Stop*/
...
void del_zqcs_timer(void) { del_timer_sync(&zqcs_timer); }

void add_zqcs_timer(void)
{
	mod_timer(&zqcs_timer,
		  jiffies +
		      msecs_to_jiffies(280)); /* add_timer(&zqcs_timer); */
}

#ifdef DRAM_HQA
static unsigned int hqa_vcore;

int calculate_voltage(unsigned int x) { return (600 + ((625 * x) / 100)); }
...
static int __init dram_hqa_init(void)
{
	...
	if (ret) {
		pr_debug("fail to create dram_test platform driver\n");
		return ret;
	}
	...
	if (ret) {
		pr_debug(
		    "fail to create the read dram data rate sysfs files\n");
		return ret;
	}
...
static int __init dram_test_init(void)
{
	...
	ret = dram_dt_init();
	if (ret) {
		pr_debug("[DRAMC] Device Tree Init Fail\n");
		return ret;
	}
	...
	if ((DRAM_TYPE == TYPE_LPDDR4) || (DRAM_TYPE == TYPE_LPDDR4X)) {
		low_freq_counter = 10;
		setup_deferrable_timer_on_stack(&zqcs_timer,
						zqcs_timer_callback, 0);
