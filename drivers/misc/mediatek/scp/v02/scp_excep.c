...
void scp_aee_last_reg(void)
{
	...
	scp_aee_status.pc = SCP_A_DEBUG_PC_REG;
	scp_aee_status.lr = SCP_A_DEBUG_LR_REG;
	scp_aee_status.psp = SCP_A_DEBUG_PSP_REG;
	scp_aee_status.sp = SCP_A_DEBUG_SP_REG;
	scp_aee_status.m2h = SCP_A_TO_HOST_REG;
	scp_aee_status.h2m = GIPC_TO_SCP_REG;
...
#define neg_force_zero(x)	((((ssize_t)(x)) > 0) ? (x) : 0)
	...
	len += snprintf(buf + len, neg_force_zero(buf_len - len), "wdt en=%d, count=0x%08x\n",
			(SCP_A_WDT_REG & 0x10000000) ? 1 : 0,
			(SCP_A_WDT_REG & 0xFFFFF));
...
void scp_aed(scp_excep_id type)
{
	...
	if (type == EXCEP_RUNTIME)
		type = (is_scp_A_ready()) ? EXCEP_RUNTIME : EXCEP_BOOTUP;
...
struct bin_attribute bin_attr_scp_A_dump = {
	.attr = {
		.name = "scp_A_dump",
		.mode = 0444,
	},
