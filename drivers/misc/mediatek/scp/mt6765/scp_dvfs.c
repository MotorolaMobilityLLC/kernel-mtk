...
int scp_set_pmic_vcore(unsigned int cur_freq)
{
	...
	if (ret_vc != 0 || ret_vs != 0) {
		ret = -1;
		pr_notice(DVFS_TAG "%s: scp vcore/vsram setting error, (%d, %d)\n",
...
int scp_request_freq(void)
{
	...
	if (scp_dvfs_flag != 1) {
		pr_info(DVFS_TAG "warning: SCP DVFS is OFF\n");
		return 0;
	}
	...
	if (scp_current_freq != scp_expected_freq) {
		...
		do {
			ret = scp_ipi_send(IPI_DVFS_SET_FREQ,
								...
								SCP_A_ID);
			if (ret != SCP_IPI_DONE)
				pr_info(DVFS_TAG "%s: SCP send IPI fail - %d\n",
						__func__, ret);
			...
			if (timeout <= 0) {
				pr_notice(
		DVFS_TAG "%s: set freq fail, current(%d) != expect(%d)\n",
					__func__,
					scp_current_freq,
					scp_expected_freq);
				goto set_freq_fail;
		...
		if (is_increasing_freq == 0)
			mt_secure_call(MTK_SIP_KERNEL_SCP_DVFS_CTRL,
							scp_expected_freq,
							0, 0, 0);
		...
		if (ret != 0) {
			pr_notice(
	DVFS_TAG "%s: scp_set_pmic_vcore(%d) fail - %d\n",
					__func__,
					scp_current_freq,
					ret);
			goto fatal_error;
...
set_freq_fail:
	mt_secure_call(MTK_SIP_KERNEL_SCP_DVFS_CTRL, scp_current_freq,
					0, 0, 0);
	scp_A_dump_regs();
	__pm_relax(&scp_suspend_lock);
	WARN_ON(1);
...
void wait_scp_dvfs_init_done(void)
{
	...
	while (g_scp_dvfs_init_flag != 1) {
		...
		count++;
		if (count > 3000) {
			pr_notice(DVFS_TAG "ERROR: %s: SCP dvfs driver init fail\n",
						__func__);
			WARN_ON(1);
		}
	}
...
int scp_pll_ctrl_set(unsigned int pll_ctrl_flag, unsigned int pll_sel)
{
	...
	pr_info(DVFS_TAG "%s(%d, %d)\n", __func__, pll_ctrl_flag, pll_sel);

	if (pll_ctrl_flag == PLL_ENABLE) {
		ret = clk_prepare_enable(mt_scp_pll->clk_mux);
		if (ret) {
			pr_notice(DVFS_TAG
					"ERROR: %s: scp dvfs cannot enable clk mux, %d\n",
					__func__, ret);
			WARN_ON(1);
		}
...
{
	if (scp_sleep_flag == -1)
		seq_puts(m, "Warning: SCP sleep is not configured by command yet.\n");
	...
	if (n == 1 || n == 2) {
		if (!strcmp(cmd, "on")) {
			...
			if (req >= 0 && req <= 5) {
				...
				pr_info("echo req <0~5> > /proc/scp_dvfs/scp_dvfs_ctrl\n");
			}
#endif
		} else {
...
static int mt_scp_dvfs_create_procfs(void)
{
	...
	if (!dir) {
		pr_notice(DVFS_TAG "fail to create /proc/scp_dvfs @ %s()\n",
					__func__);
		return -ENOMEM;
	}
	...
	for (i = 0; i < ARRAY_SIZE(entries); i++) {
		if (!proc_create(entries[i].name,
						...
						dir,
						entries[i].fops))
			pr_notice(DVFS_TAG "ERROR: %s: create /proc/scp_dvfs/%s failed\n",
...
static long _set_state(const char *name)
{
	...
	if (IS_ERR(pState)) {
		pr_notice(DVFS_TAG "lookup state '%s' failed\n", name);
		ret = PTR_ERR(pState);
		goto exit;
	}
...
static int mt_scp_dvfs_pdrv_probe(struct platform_device *pdev)
{
	...
	if (!node) {
		dev_notice(&pdev->dev, "%s: fail to find SCPDVFS node\n",
					__func__);
		WARN_ON(1);
	}
	...
	if (!node) {
		pr_notice(DVFS_TAG "error: can't find GPIO node\n");
		WARN_ON(1);
		return 0;
	}
	...
	if (!gpio_base) {
		pr_notice(DVFS_TAG "error: iomap fail for GPIO\n");
		WARN_ON(1);
		return 0;
	}
	...
	if (gpio_mode == 1)
		pr_info(DVFS_TAG "V_REQ muxpin setting is correct\n");
	else
		pr_notice(DVFS_TAG "error: V_REQ muxpin setting is wrong - %d\n",
					gpio_mode);
...
void mt_pmic_sshub_init(void)
{
	...
	pr_info(DVFS_TAG "Before init vcore and vsram_core:\n");
	pr_info(DVFS_TAG "vcore: on, mode, vosel = 0x%x, 0x%x, 0x%x\n",
			ret[0], ret[1], ret[2]);
	pr_info(DVFS_TAG "vsram: on, mode, vosel = 0x%x, 0x%x, 0x%x\n",
			ret[3], ret[4], ret[5]);
	...
	ret[4] = pmic_get_register_value(PMIC_RG_LDO_VSRAM_CORE_SSHUB_MODE);
	ret[5] = pmic_get_register_value(PMIC_RG_LDO_VSRAM_CORE_SSHUB_VOSEL);
	pr_info(DVFS_TAG "After init vcore and vsram_core:\n");
	pr_info(DVFS_TAG "vcore: on, mode, vosel = 0x%x, 0x%x, 0x%x\n",
			ret[0], ret[1], ret[2]);
	pr_info(DVFS_TAG "vsram: on, mode, vosel = 0x%x, 0x%x, 0x%x\n",
			ret[3], ret[4], ret[5]);
...
int __init scp_dvfs_init(void)
{
	...
	if (mt_scp_dvfs_create_procfs()) {
		pr_notice(DVFS_TAG "mt_scp_dvfs_create_procfs fail..\n");
		WARN_ON(1);
		return -1;
	}
#endif /* CONFIG_PROC_FS */
	...
	if (ret) {
		pr_notice(DVFS_TAG "fail to register scp dvfs device @ %s()\n",
					__func__);
		WARN_ON(1);
		return -1;
	}
	...
	if (ret) {
		pr_notice(DVFS_TAG "fail to register scp dvfs driver @ %s()\n",
					__func__);
