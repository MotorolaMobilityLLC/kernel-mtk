...
static size_t scp_A_get_last_log(size_t b_len)
{
	...
	unsigned char *scp_last_log_buf = (unsigned char *)(SCP_TCM +
						scp_A_log_buf_addr_last);
	...
	if (log_end_idx >= b_len)
		...
		update_start_idx = scp_A_log_buf_maxlen_last -
						(b_len - log_end_idx) + 1;
...
static ssize_t scp_A_log_if_read(struct file *file, char __user *data
...
static unsigned int scp_A_log_wakeup_set(unsigned int enable)
{
	...
	if (scp_A_logger_inited) {
		...
		retrytimes = SCP_IPI_RETRY_TIMES;
		do {
			ret = scp_ipi_send(IPI_LOGGER_WAKEUP,
					&enable, sizeof(enable), 0, SCP_A_ID);
			if (ret == SCP_IPI_DONE)
				break;
			retrytimes--;
...
DEVICE_ATTR(scp_mobile_log, 0644, scp_A_mobile_log_show
					, scp_A_mobile_log_store);
...
DEVICE_ATTR(scp_A_logger_wakeup_AP, 0644, scp_A_wakeup_show
					, scp_A_wakeup_store);
...
static ssize_t scp_A_last_log_store(struct device *kobj
		, struct device_attribute *attr, const char *buf, size_t n)
{
	scp_A_get_last_log(scp_A_log_buf_maxlen_last);
	return n;
...
DEVICE_ATTR(scp_A_get_last_log, 0644, scp_A_last_log_show
					, scp_A_last_log_store);
...
DEVICE_ATTR(scp_A_mobile_log_UT, 0644, scp_A_mobile_log_UT_show
					, scp_A_mobile_log_UT_store);
#endif
 ...
 * NOTE: this function may be blocked and should not be called
 * in interrupt context
 * @param ws:   work struct
 */
static void scp_logger_notify_ws(struct work_struct *ws)
{
	struct scp_work_struct *sws = container_of(ws, struct scp_work_struct
								, work);
...
int scp_logger_init(phys_addr_t start, phys_addr_t limit)
{
	...
	SCP_A_buf_info = (struct buffer_info_s *) (((unsigned char *)
				SCP_A_log_ctl) + SCP_A_log_ctl->info_ofs);
...
void scp_crash_log_move_to_buf(enum scp_core_id scp_id)
{
	...
	ret = 0;
	if (scp_last_logger) {
		ret += snprintf(scp_last_logger,
				strlen(crash_message), crash_message);
		ret--;
		while ((log_start_idx != log_end_idx) &&
			...
			if (log_start_idx >= scp_log_buf_maxlen_last[scp_id])
				log_start_idx = log_start_idx -
					scp_log_buf_maxlen_last[scp_id];
