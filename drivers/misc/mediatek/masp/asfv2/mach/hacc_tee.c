...
static char *hacc_secure_request(enum hacc_user user,
				 unsigned char *buf,
				 ...
				 unsigned int seed_size)
{
	...
	if (buf_size != 0) {
		...
		pr_debug("[HACC] hacc_secure_request - buffer size is 0,
			 no encryption or decyrption is performed\n");
	}
...
exit:
	...
	if (ret)
		pr_debug("[HACC] hacc_secure_request fail (0x%x)\n", ret);
			  ...
			  unsigned char *ResText)
{
	...
	case TRUE:
		/* encrypt */
		dst = hacc_secure_request(HACC_USER3,
					  (unsigned char *)src,
					  ContentLen,
					  TRUE,
					  FALSE,
...
wrong_direction:
	...
	if (err)
		pr_debug("[HACC] masp_hal_secure_algo error (0x%x)\n", err);
}

/*
 * For SECRO (user1), this function will help to get hacc lock
 * For SECCFG (user1-sbchk), it should get hacc lock via ioctl command
 * before using this function
 * For MD NVRAM (user3), it should get hacc lock before using this function
 * For AP NVRAM (user2), it should get hacc lock via ioctl command
 * before using this function
 */
unsigned char *masp_hal_sp_hacc_enc(unsigned char *buf,
				    unsigned int size,
				    unsigned char bAC,
				    enum hacc_user user,
...
unsigned char *masp_hal_sp_hacc_dec(unsigned char *buf,
				    unsigned int size,
				    unsigned char bAC,
				    enum hacc_user user,
				    unsigned char bDoLock)
