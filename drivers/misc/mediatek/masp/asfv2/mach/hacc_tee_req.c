...
int open_sdriver_connection(void)
{
	...
	int retryCnt = 0;
	size_t dapc_tci_msg_sz = sizeof(struct dapc_tciMessage_t);
	...
	do {
		...
		if (mcRet != MC_DRV_OK) {
			retryCnt++;
			pr_debug("NWD HACC: mc_open_device failed: %d,
				 retry count( % d)\n",
				 mcRet,
				 retryCnt);
			continue;
		       ...
		       unsigned int seed_size)
{
	...
	if (seed_size != 0) {
		...
		ret = mc_map(&drSessionHandle,
			     seedbuf,
			     seed_size,
			     &seedMapInfo);
		if (ret != MC_DRV_OK) {
			pr_debug("NWD HACC: mcMap failed of seed buffer: %d",
				 ret);
			ret = ERR_HACC_MCMAP_BUFFER_FAIL;
	...
	mcRet = mc_wait_notification(&drSessionHandle,
				     /* MC_INFINITE_TIMEOUT */ 20000);
	if (mcRet != MC_DRV_OK) {
		pr_debug("NWD HACC IRQ fail: mc_wait_notification 20s timeout:
				%d\n", mcRet);
