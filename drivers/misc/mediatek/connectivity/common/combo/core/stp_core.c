...
static VOID stp_trace32_dump(VOID)
{
	if (STP_IS_ENABLE_DBG(stp_core_ctx) && (stp_core_ctx.parser.type == STP_TASK_INDX) &&
		...
		if (0 == osal_strncmp("coredump end", stp_core_ctx.rx_buf + stp_core_ctx.rx_counter -
			...
			/*put one dump opId into btm's opId queue to
			 * makesure coredump complete before whole chip reset */
...
static VOID stp_process_header_only_packet(VOID)
{
	if (stp_core_ctx.parser.length == 0) {
		...
		if (stp_core_ctx.inband_rst_set == 0) {
			/* STP type id */
			stp_dbg_pkt_log(STP_TASK_INDX, stp_core_ctx.parser.ack, stp_core_ctx.parser.seq, 5,
					PKT_DIR_RX, NULL, 0);
...
static VOID stp_process_packet(VOID)
{
	...
	if ((stp_core_ctx.sequence.expected_rxseq == stp_core_ctx.parser.seq) &&
		...
		stp_ctx_unlock(&stp_core_ctx);
		STP_ERR_FUNC("sequence not match && previous packet enqueue successfully\n");
		STP_ERR_FUNC("send the previous ACK (ack no =%d)\n", txAck);
	} else {
	/*sequence not match && previous packet enqueue failed, do nothing,
	 * make the other side timeout */
		stp_process_packet_fail_count++;
		STP_ERR_FUNC("sequence not match && previous packet enqueue failed, do nothing\n");
		STP_ERR_FUNC("make the other side timeout\n");
	}
				...
				if (i >= remain_length) {
					...
					*(stp_core_ctx.rx_buf + stp_core_ctx.rx_counter) = '\0';
					...
					if ((stp_core_ctx.rx_counter == 1)
						...
						/* For MT6620, enable/disable coredump function
						 * is controlled by firmware
						 * for the moment, we need to set coredump enable flag to be 1 after */
						/* see firmware send a pariticallar character(0xff) before any coredump
						 * packet is sent */
					...
					*(stp_core_ctx.rx_buf + stp_core_ctx.rx_counter) = '\0';
					...
					if (stp_core_ctx.parser.type == STP_TASK_INDX) {
						...
						osal_dbg_print("[len=%d][type=%d]\n%s\n",
							       ...
							       stp_core_ctx.parser.type,
							       stp_core_ctx.rx_buf);
						stp_dbg_log_pkt(g_mtkstp_dbg, STP_DBG_FW_DMP
		...
		while (i > 0) {
			...
			case MTKSTP_SYNC:	/* b'10 */
				...
				if (((*p_data & 0x80) == 0x80) && ((*p_data & 0x40) == 0x00)) {
					...
					/* a start of 0x7f, maybe this is resync pattern */
					stp_change_rx_state(MTKSTP_RESYNC2);
					osal_assert(0);
					STP_ERR_FUNC
...
#endif
{
	...
	if (MTK_WCN_BOOL_TRUE == stp_psm_is_quick_ps_support()) {
		...
		/* if(stp_is_apply_powersaving()) */
		{
			if (type == WMT_TASK_INDX)
				goto DONT_MONITOR;
			if ((type == BT_TASK_INDX) && (STP_SDIO_IF_TX == wmt_plat_get_comm_if_type())) {
				if (stp_psm_is_to_block_traffic(STP_PSM_CORE(stp_core_ctx)))
