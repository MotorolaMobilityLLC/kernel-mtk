...
/* iRet = (*kal_stp_tx)((PUINT8)&rWmtPktCmd, u4WmtCmdPduLen, &u4WrittenSize); */
		...
		if (PKT_TYPE_EVENT != rWmtPktEvent.eType) {
			WMT_ERR_FUNC
			    ("WMT-CORE: wmt_func_ctrl_cmd PKT_TYPE_EVENT != rWmtPktEvent.eType %d\n",
			     rWmtPktEvent.eType);
			break;
		}
...
static INT32 wmt_core_stp_init(VOID)
{
	...
	if ((NULL != pctx->p_ic_ops->co_clock_ctrl) && (pWmtGenConf != NULL)) {
		(*(pctx->p_ic_ops->co_clock_ctrl)) (pWmtGenConf->co_clock_flag);
	} else {
		WMT_INFO_FUNC("pctx->p_ic_ops->co_clock_ctrl(%p), pWmtGenConf(%p)\n",
...
static INT32 opfunc_pwr_sv(P_WMT_OP pWmtOp)
{
	...
	if (SLEEP == pWmtOp->au4OpData[0]) {
		...
		ret =
		    wmt_core_tx((const PUINT8)WMT_WAKEUP_CMD, sizeof(WMT_WAKEUP_CMD), &u4_result,
				1);
