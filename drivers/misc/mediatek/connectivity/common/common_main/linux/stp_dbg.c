...
static struct WCN_CORE_DUMP_T *g_core_dump;
static struct STP_DBG_CPUPCR_T *g_stp_dbg_cpupcr;
/* just show in log at present */
static struct STP_DBG_DMAREGS_T *g_stp_dbg_dmaregs;

static VOID stp_dbg_core_dump_timeout_handler(ULONG data);
static _osal_inline_ struct WCN_CORE_DUMP_T *stp_dbg_core_dump_init(UINT32 timeout);
static _osal_inline_ INT32 stp_dbg_core_dump_deinit(struct WCN_CORE_DUMP_T *dmp);
static _osal_inline_ INT32 stp_dbg_core_dump_check_end(PUINT8 buf, INT32 len);
static _osal_inline_ INT32 stp_dbg_core_dump_in(struct WCN_CORE_DUMP_T *dmp, PUINT8 buf, INT32 len);
static _osal_inline_ INT32 stp_dbg_core_dump_post_handle(struct WCN_CORE_DUMP_T *dmp);
static _osal_inline_ INT32 stp_dbg_core_dump_out(struct WCN_CORE_DUMP_T *dmp, PPUINT8 pbuf, PINT32 plen);
static _osal_inline_ INT32 stp_dbg_core_dump_reset(struct WCN_CORE_DUMP_T *dmp, UINT32 timeout);
static _osal_inline_ INT32 stp_dbg_core_dump_nl(struct WCN_CORE_DUMP_T *dmp, PUINT8 buf, INT32 len);
...
static _osal_inline_ struct WCN_COMPRESSOR_T *stp_dbg_compressor_init(PUINT8 name, INT32 L1_buf_sz, INT32 L2_buf_sz);
static _osal_inline_ INT32 stp_dbg_compressor_deinit(struct WCN_COMPRESSOR_T *cprs);
static _osal_inline_ INT32 stp_dbg_compressor_in(struct WCN_COMPRESSOR_T *cprs, PUINT8 buf, INT32 len, INT32 finish);
static _osal_inline_ INT32 stp_dbg_compressor_out(struct WCN_COMPRESSOR_T *cprs, PPUINT8 pbuf, PINT32 plen);
static _osal_inline_ INT32 stp_dbg_compressor_reset(struct WCN_COMPRESSOR_T *cprs, UINT8 enable, enum WCN_COMPRESS_ALG_T type);
static _osal_inline_ VOID stp_dbg_dump_data(PUINT8 pBuf, PINT8 title, INT32 len);
static _osal_inline_ INT32 stp_dbg_dmp_in(struct MTKSTP_DBG_T *stp_dbg, PINT8 buf, INT32 len);
static _osal_inline_ INT32 stp_dbg_notify_btm_dmp_wq(struct MTKSTP_DBG_T *stp_dbg);
static _osal_inline_ INT32 stp_dbg_get_avl_entry_num(struct MTKSTP_DBG_T *stp_dbg);
static _osal_inline_ INT32 stp_dbg_fill_hdr(struct STP_DBG_HDR_T *hdr, INT32 type, INT32 ack, INT32 seq,
			      INT32 crc, INT32 dir, INT32 len, INT32 dbg_type);
static _osal_inline_ INT32 stp_dbg_add_pkt(struct MTKSTP_DBG_T *stp_dbg, struct STP_DBG_HDR_T *hdr, const PUINT8 body);
...
static _osal_inline_ INT32 stp_dbg_parser_assert_str(PINT8 str, enum ENUM_ASSERT_INFO_PARSER_TYPE type);
static _osal_inline_ struct STP_DBG_CPUPCR_T *stp_dbg_cpupcr_init(VOID);
static _osal_inline_ VOID stp_dbg_cpupcr_deinit(struct STP_DBG_CPUPCR_T *pCpupcr);
static _osal_inline_ struct STP_DBG_DMAREGS_T *stp_dbg_dmaregs_init(VOID);
static _osal_inline_ VOID stp_dbg_dmaregs_deinit(struct STP_DBG_DMAREGS_T *pDmaRegs);
...
static _osal_inline_ struct WCN_CORE_DUMP_T *stp_dbg_core_dump_init(UINT32 timeout)
{
	struct WCN_CORE_DUMP_T *core_dmp = NULL;

	core_dmp = (struct WCN_CORE_DUMP_T *) osal_malloc(sizeof(struct WCN_CORE_DUMP_T));
	if (!core_dmp) {
		STP_DBG_ERR_FUNC("alloc mem failed!\n");
		goto fail;
	}

	osal_memset(core_dmp, 0, sizeof(struct WCN_CORE_DUMP_T));
 ...
 */
static _osal_inline_ struct WCN_COMPRESSOR_T *stp_dbg_compressor_init(PUINT8 name, INT32 L1_buf_sz,
		INT32 L2_buf_sz)
{
	z_stream *pstream = NULL;
	struct WCN_COMPRESSOR_T *compress = NULL;

	compress = (struct WCN_COMPRESSOR_T *) osal_malloc(sizeof(struct WCN_COMPRESSOR_T));
	if (!compress) {
		STP_DBG_ERR_FUNC("alloc compressor failed!\n");
		goto fail;
	}

	osal_memset(compress, 0, sizeof(struct WCN_COMPRESSOR_T));
 ...
 * Retunr 0 if success, else NULL
 */
static _osal_inline_ INT32 stp_dbg_compressor_in(struct WCN_COMPRESSOR_T *cprs, PUINT8 buf, INT32 len,
		INT32 finish)
{
	INT32 tmp_len = 0;
	INT32 ret = 0;
 ...
 * Retunr 0 if success, else NULL
 */
static _osal_inline_ INT32 stp_dbg_compressor_reset(struct WCN_COMPRESSOR_T *cprs, UINT8 enable,
		enum WCN_COMPRESS_ALG_T type)
{
	if (!cprs) {
		STP_DBG_ERR_FUNC("invalid para!\n");
		return -1;
...
INT32 stp_dbg_disable(struct MTKSTP_DBG_T *stp_dbg)
{
	...
	spin_lock_irqsave(&(stp_dbg->logsys->lock), flags);
	stp_dbg->pkt_trace_no = 0;
	memset(stp_dbg->logsys, 0, sizeof(struct MTKSTP_LOG_SYS_T));
	stp_dbg->is_enable = 0;
	spin_unlock_irqrestore(&(stp_dbg->logsys->lock), flags);
...
{
	ULONG flags;
	struct STP_DBG_HDR_T *pHdr = NULL;
	PINT8 pBuf = NULL;
	UINT32 length = 0;
	...
	if (internalFlag) {
		...
		if (gStpDbgLogOut != 0) {
			pHdr = (struct STP_DBG_HDR_T *) &(stp_dbg->logsys->queue[stp_dbg->logsys->in].buffer[0]);
			pBuf = (PINT8)&(stp_dbg->logsys->queue[stp_dbg->logsys->in].buffer[0]) +
				sizeof(struct STP_DBG_HDR_T);
			length = stp_dbg->logsys->queue[stp_dbg->logsys->in].len - sizeof(struct STP_DBG_HDR_T);
...
/* #ifndef CONFIG_LOG_STP_INTERNAL */
	if (stp_dbg->btm != NULL)
		retval += stp_btm_notify_wmt_dmp_wq((struct MTKSTP_BTM_T *) stp_dbg->btm);
/* #endif */
...
#define MAX_DMP_NUM 80
	...
	while (dumpSize > 0) {
		pHdr = (struct STP_DBG_HDR_T *) &(stp_dbg->logsys->queue[outIndex].buffer[0]);
		pBuf = &(stp_dbg->logsys->queue[outIndex].buffer[0]) + sizeof(struct STP_DBG_HDR_T);
		len = stp_dbg->logsys->queue[outIndex].len - sizeof(struct STP_DBG_HDR_T);
...
static _osal_inline_ INT32 stp_dbg_add_pkt(struct MTKSTP_DBG_T *stp_dbg, struct STP_DBG_HDR_T *hdr, const PUINT8 body)
{
	...
	static struct STP_PACKET_T stp_pkt;
	UINT32 hdr_sz = sizeof(struct STP_DBG_HDR_T);
	UINT32 body_sz = 0;
	ULONG flags;
	UINT32 avl_num;
...
static _osal_inline_ struct STP_DBG_CPUPCR_T *stp_dbg_cpupcr_init(VOID)
{
	struct STP_DBG_CPUPCR_T *pSdCpupcr = NULL;

	pSdCpupcr = (struct STP_DBG_CPUPCR_T *) osal_malloc(osal_sizeof(struct STP_DBG_CPUPCR_T));
	if (!pSdCpupcr) {
		STP_DBG_ERR_FUNC("stp dbg cpupcr allocate memory fail!\n");
		return NULL;
	}

	osal_memset(pSdCpupcr, 0, osal_sizeof(struct STP_DBG_CPUPCR_T));
...
static _osal_inline_ struct STP_DBG_DMAREGS_T *stp_dbg_dmaregs_init(VOID)
{
	struct STP_DBG_DMAREGS_T *pDmaRegs = NULL;

	pDmaRegs = (struct STP_DBG_DMAREGS_T *) osal_malloc(osal_sizeof(struct STP_DBG_DMAREGS_T));
	if (!pDmaRegs) {
		STP_DBG_ERR_FUNC("stp dbg dmareg allocate memory fail!\n");
		return NULL;
	}

	osal_memset(pDmaRegs, 0, osal_sizeof(struct STP_DBG_DMAREGS_T));
...
static VOID stp_dbg_dmaregs_deinit(struct STP_DBG_DMAREGS_T *pDmaRegs)
{
	if (pDmaRegs) {
...
{
	enum ENUM_ASSERT_INFO_PARSER_TYPE type_index;
	PUINT8 tempbuf = NULL;
	UINT32 i = 0;
	INT32 iRet = 0;
...
{
	struct MTKSTP_DBG_T *stp_dbg = NULL;

	STP_DBG_INFO_FUNC("stp-dbg init\n");

	stp_dbg = kzalloc(sizeof(struct MTKSTP_DBG_T), GFP_KERNEL);
	if (stp_dbg == NULL)
		goto ERR_EXIT1;
	if (IS_ERR(stp_dbg)) {
	...
	stp_dbg->logsys = vmalloc(sizeof(struct MTKSTP_LOG_SYS_T));
