...
static INT32 stp_sdio_irq(const UINT32 clt_ctx);
static INT32 stp_sdio_probe(const UINT32 clt_ctx,
		const struct MTK_WCN_HIF_SDIO_FUNCINFO *sdio_func_infop);
static INT32 stp_sdio_remove(const UINT32 clt_ctx);
...
static _osal_inline_ enum SDIO_PS_OP stp_sdio_get_own_state(VOID);
static _osal_inline_ INT32 stp_sdio_do_own_set(struct MTK_WCN_STP_SDIO_HIF_INFO *p_info);
static _osal_inline_ INT32 stp_sdio_do_own_clr(INT32 wait);
static _osal_inline_ VOID stp_sdio_check_tx_sanity(const struct MTK_WCN_STP_SDIO_HIF_INFO *p_info, const UINT32 id);
static _osal_inline_ VOID stp_sdio_tx_wkr_comp(struct MTK_WCN_STP_SDIO_HIF_INFO * const p_info);
static _osal_inline_ INT32 stp_sdio_ownback_poll(const UINT32 clt_ctx, UINT32 retry,
		UINT32 delay_us);
...
static const struct MTK_WCN_HIF_SDIO_FUNCINFO mtk_stp_sdio_id_tbl[] = {
...
const struct MTK_WCN_HIF_SDIO_CLTINFO g_stp_sdio_cltinfo = {
	.func_tbl = mtk_stp_sdio_id_tbl,
	.func_tbl_size = (sizeof(mtk_stp_sdio_id_tbl) / sizeof(struct MTK_WCN_HIF_SDIO_FUNCINFO) - 1),
...
struct MTK_WCN_STP_SDIO_HIF_INFO g_stp_sdio_host_info;	/*[STP_SDIO_HOST_COUNT]; */
/* STP SDIO host information pointer (for stp if_tx() function) */
struct MTK_WCN_STP_SDIO_HIF_INFO *const gp_info = &g_stp_sdio_host_info;
 ...
 * \retval !=0 fail
 */
INT32 stp_sdio_own_ctrl(enum SDIO_PS_OP op)
{
	INT32 ret = -1;
	struct OSAL_SIGNAL *pOsalSignal = NULL;
...
static INT32 stp_sdio_wait_for_msg(PVOID pvData)
{
	struct MTK_WCN_STP_SDIO_HIF_INFO *pInfo = (struct MTK_WCN_STP_SDIO_HIF_INFO *) pvData;
...
static VOID stp_sdio_tx_rx_handling(PVOID pData)
{
	struct MTK_WCN_STP_SDIO_HIF_INFO *pInfo = (struct MTK_WCN_STP_SDIO_HIF_INFO *) pData;
	UINT32 iRet = 0;
	UINT32 chisr = 0;
 ...
 * \retval none.
 */
static VOID stp_sdio_tx_wkr_comp(struct MTK_WCN_STP_SDIO_HIF_INFO * const p_info)
{
	INT32 comp_count;
	UINT32 idx;
	INT32 i;
...
static VOID stp_sdio_tx_wkr(struct work_struct *work)
{
	struct MTK_WCN_STP_SDIO_HIF_INFO *p_info;
	UINT32 bus_txlen;
	UINT32 four_byte_align_len;
	PUINT8 buf_tx;
	INT32 ret;
	UINT32 idx;
	struct MTK_WCN_STP_SDIO_PKT_BUF *pb;
	struct timeval now;
	UINT64 ts;
	ULONG nsec;

	p_info = container_of(work, struct MTK_WCN_STP_SDIO_HIF_INFO, tx_work);
	ret = HIF_SDIO_ERR_SUCCESS;
	pb = &p_info->pkt_buf;
...
static VOID stp_sdio_rx_wkr(struct work_struct *work)
{
	...
	p_info = container_of(work, struct MTK_WCN_STP_SDIO_HIF_INFO, rx_work);
...
static INT32 stp_sdio_irq(const UINT32 clt_ctx)
{
	...
	UINT32 chlcpr_value = 0;
	UINT32 write_value = 0;
	struct MTK_WCN_STP_SDIO_HIF_INFO *p_info = gp_info;
 ...
 *  ret:    Probe result
 *****************************************************************************/
static INT32 stp_sdio_ownback_poll(const UINT32 clt_ctx, UINT32 retry, UINT32 delay_us)
{
	INT32 ret;
	UINT32 chlpcr = 0;
 ...
 *  ret:    Probe result
 *****************************************************************************/
/* typedef INT32 (*MTK_WCN_HIF_SDIO_PROBE)(UINT32, const MTK_WCN_HIF_SDIO_FUNCINFO *); */
static INT32 stp_sdio_probe(const UINT32 clt_ctx,
		const struct MTK_WCN_HIF_SDIO_FUNCINFO *sdio_func_infop)
{
...
INT32 stp_sdio_rw_retry(enum ENUM_STP_SDIO_HIF_TYPE_T type, UINT32 retry_limit,
		UINT32 clt_ctx, UINT32 offset, PUINT32 pData, UINT32 len)
{
	INT32 ret = -1;
	INT32 ret_1 = -1;
	UINT32 value = 0;
...
INT32 stp_sdio_wake_up_ctrl(UINT32 ctx)
{
	INT32 ret;
