...
static _osal_inline_ INT32 hif_sdio_clt_probe_func(struct MTK_WCN_HIF_SDIO_REGISTINFO *registinfo_p,
		INT8 probe_idx);
...
static _osal_inline_ INT32 hif_sdio_add_clt_list(PINT32 clt_index_p, const struct MTK_WCN_HIF_SDIO_CLTINFO *pinfo,
		UINT32 tbl_index);
...
static _osal_inline_ INT32 hif_sdio_deep_sleep_info_set_act(UINT32 chipid, UINT16 func_num,
		UINT32 ctx, UINT8 act_flag);

static INT32 _hif_sdio_wake_up_ctrl(UINT32 ctx);

static _osal_inline_ INT32 wmt_tra_sdio_update(VOID);

static _osal_inline_ INT32 hif_sdio_deep_sleep_info_dmp(struct MTK_WCN_HIF_SDIO_DS_INFO *p_ds_info);
...
static struct MTK_WCN_HIF_SDIO_REGISTINFO g_hif_sdio_clt_drv_list[CFG_CLIENT_COUNT];

/* MMC probed function list */
/* static list g_hif_sdio_probed_func_list */
static struct MTK_WCN_HIF_SDIO_PROBEINFO g_hif_sdio_probed_func_list[CFG_CLIENT_COUNT];

/* spin lock info for g_hif_sdio_clt_drv_list and g_hif_sdio_probed_func_list */
static struct MTK_WCN_HIF_SDIO_LOCKINFO g_hif_sdio_lock_info;
...
struct MTK_WCN_HIF_SDIO_DS_INFO g_hif_sdio_ds_info_list[] = {
	{
	 .chip_id = 0x6630,
	 .reg_offset = 0xF1,
	 .value = 0x1,
 ...
 */
static _osal_inline_ struct sdio_func *hif_sdio_ctx_to_func(UINT32 ctx)
{
	UINT32 probe_index;
...
{
	UINT32 i = 0;
	struct MTK_WCN_HIF_SDIO_DS_CLT_INFO *ctl_info = NULL;
	...
	for (i = 0; i < array_size; i++) {
		...
		for (j = 0; j < clt_info_size; j++)
			memset(&g_hif_sdio_ds_info_list[i].clt_info[j],
					0, sizeof(struct MTK_WCN_HIF_SDIO_DS_CLT_INFO));
					       ...
					       UINT32 ctx, UINT8 act_flag)
{
	...
	UINT32 clt_info_size = 0;
	UINT32 idx = 0;
	struct MTK_WCN_HIF_SDIO_DS_CLT_INFO *p_ds_clt_info = NULL;
...
static INT32 _hif_sdio_wake_up_ctrl(UINT32 ctx)
{
	...
	INT32 i = 0;
	UINT32 delay_us = 500;
	struct WMT_GPIO_STATE_INFO gpio_state_list[2];
...
#endif
INT32 hif_sdio_wake_up_ctrl(UINT32 ctx)
{
	UINT32 i = 0;
	UINT32 j = 0;
	INT32 ret = 0;
	...
	UINT32 clt_info_size = 0;
	struct MTK_WCN_HIF_SDIO_DS_CLT_INFO *p_ds_clt_info = NULL;
	struct MTK_WCN_HIF_SDIO_DS_INFO *p_ds_info = NULL;
 ...
 */
INT32 mtk_wcn_hif_sdio_client_reg(const struct MTK_WCN_HIF_SDIO_CLTINFO *pinfo)
{
	INT32 ret = -HIF_SDIO_ERR_FAIL;
	INT32 clt_index = -1;
	UINT32 i = 0;
	UINT32 j = 0;
	struct MTK_WCN_HIF_SDIO_CLT_PROBE_WORKERINFO *clt_probe_worker_info = 0;
		...
		for (j = 0; j < CFG_CLIENT_COUNT; j++) {
			...
			if ((g_hif_sdio_clt_drv_list[clt_index].func_info->manf_id ==
				...
				clt_probe_worker_info =
				    vmalloc(sizeof(struct MTK_WCN_HIF_SDIO_CLT_PROBE_WORKERINFO));
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_client_unreg(const struct MTK_WCN_HIF_SDIO_CLTINFO *pinfo)
{
	INT32 ret = -HIF_SDIO_ERR_FAIL;
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_readb(UINT32 ctx, UINT32 offset, PUINT8 pvb)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_writeb(UINT32 ctx, UINT32 offset, UINT8 vb)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_readl(UINT32 ctx, UINT32 offset, PUINT32 pvl)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_writel(UINT32 ctx, UINT32 offset, UINT32 vl)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_read_buf(UINT32 ctx,
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_write_buf(UINT32 ctx,
				 UINT32 offset, PUINT32 pbuf, UINT32 len)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_abort(UINT32 ctx)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
 ...
 *
 *
 * \param clent's UINT32.
 *
 * \retval none.
 */
VOID mtk_wcn_hif_sdio_set_drvdata(UINT32 ctx, PVOID private_data_p)
{
 ...
 */
PVOID mtk_wcn_hif_sdio_get_drvdata(UINT32 ctx)
{
	UINT8 probed_idx = CLTCTX_IDX(ctx);
 ...
 */
INT32 mtk_wcn_hif_sdio_wmt_control(enum WMT_SDIO_FUNC_TYPE func_type, MTK_WCN_BOOL is_on)
{
 ...
 * \retval none
 */
VOID mtk_wcn_hif_sdio_get_dev(UINT32 ctx, struct device **dev)
{
#if HIF_SDIO_UPDATE
	struct sdio_func *func;
#else
 ...
 * \retval none.
 */
static _osal_inline_ INT32 hif_sdio_clt_probe_func(struct MTK_WCN_HIF_SDIO_REGISTINFO *registinfo_p, INT8 probe_idx)
{
	UINT16 card_id = 0;
	UINT16 func_num = 0;
	UINT16 blk_sz = 0;
...
static VOID hif_sdio_clt_probe_worker(struct work_struct *work)
{
	struct MTK_WCN_HIF_SDIO_CLT_PROBE_WORKERINFO *clt_worker_info_p = 0;
	UINT16 card_id = 0;
	UINT16 func_num = 0;
	UINT16 blk_sz = 0;
	INT8 prob_idx = 0;
	...
	clt_worker_info_p = container_of(work, struct MTK_WCN_HIF_SDIO_CLT_PROBE_WORKERINFO, probe_work);
...
static _osal_inline_ INT32 hif_sdio_add_clt_list(PINT32 clt_index_p,
				   const struct MTK_WCN_HIF_SDIO_CLTINFO *pinfo, UINT32 tbl_index)
{
	INT32 i = 0;
...
static INT32 hif_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
	INT32 ret = 0;
	INT32 i = 0;
	struct MTK_WCN_HIF_SDIO_PROBEINFO *hif_sdio_probed_funcp = 0;
	INT32 probe_index = -1;
	INT32 idx;
#if 0
	INT32 clt_index = -1;
...
static _osal_inline_ INT32 hif_sdio_stp_on(VOID)
{
	...
	const struct MTK_WCN_HIF_SDIO_FUNCINFO *func_info = NULL;
 ...
 */
INT32 mtk_wcn_hif_sdio_bus_set_power(UINT32 ctx, UINT32 pwrState)
{
	INT32 probe_index = -1;
	struct sdio_func *func = 0;
...
VOID mtk_wcn_hif_sdio_enable_irq(UINT32 ctx, MTK_WCN_BOOL enable)
{
	UINT8 probed_idx = CLTCTX_IDX(ctx);
...
static _osal_inline_ INT32 hif_sdio_is_autok_support(struct sdio_func *func)
{
	...
	if ((func->vendor == 0x037A) && (func->device == 0x6630) && (func->num == 1))
		iRet = 0;
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_f0_readb(UINT32 ctx, UINT32 offset, PUINT8 pvb)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
 ...
 * \retval < 0  list error code here
 */
INT32 mtk_wcn_hif_sdio_f0_writeb(UINT32 ctx, UINT32 offset, UINT8 vb)
{
#if HIF_SDIO_UPDATE
	INT32 ret;
	struct sdio_func *func;
