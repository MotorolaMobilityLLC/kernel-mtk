...
struct MTK_WCN_STP_SDIO_PKT_BUF {
...
struct MTK_WCN_STP_SDIO_Tx_Pkt_LIST {
	UINT32 pkt_rd_cnt;
	UINT32 pkt_wr_cnt;
...
struct MTK_WCN_STP_SDIO_FIRMWARE_INFO {
...
struct MTK_WCN_STP_SDIO_PRIVATE_INFO {
	UINT8 stp_sdio_host_idx;
};

/* STP SDIO host information */
struct MTK_WCN_STP_SDIO_HIF_INFO {
	UINT32 sdio_cltctx;
	struct MTK_WCN_STP_SDIO_PKT_BUF pkt_buf;
	struct MTK_WCN_STP_SDIO_Tx_Pkt_LIST tx_pkt_list;
	UINT32 rx_pkt_len;	/* George: use 32-bit for efficiency. Correct name to pkt for packet */
	struct MTK_WCN_STP_SDIO_FIRMWARE_INFO firmware_info;
	struct MTK_WCN_STP_SDIO_PRIVATE_INFO private_info;
#if STP_SDIO_OWN_THREAD
	/* struct tasklet_struct tx_rx_job; */
	struct OSAL_THREAD tx_rx_thread;
	INT32 irq_pending;
	INT32 sleep_flag;
	INT32 wakeup_flag;
	INT32 awake_flag;
	INT32 txwkr_flag;
	struct OSAL_EVENT tx_rx_event;
	struct OSAL_SIGNAL isr_check_complete;
	INT32 dump_flag;
#endif
...
********************************************************************************
*/
extern struct MTK_WCN_STP_SDIO_HIF_INFO g_stp_sdio_host_info;
