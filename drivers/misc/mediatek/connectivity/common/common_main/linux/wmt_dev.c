...
#endif
struct WMT_PATCH_INFO *pPatchInfo;
UINT32 pAtchNum;
...
struct OSAL_SLEEPABLE_LOCK g_es_lr_lock;
...
INT32 wmt_dev_rx_timeout(struct OSAL_EVENT *pEvent)
{
...
INT32 wmt_dev_patch_get(PUINT8 pPatchName, struct firmware **ppPatch)
{
	INT32 iRet = -1;
...
INT32 wmt_dev_patch_put(struct firmware **ppPatch)
{
	if (*ppPatch != NULL) {
...
{
	UINT32 mask = 0;
	struct OSAL_EVENT *pEvent = wmt_lib_get_cmd_event();
...
LONG WMT_unlocked_ioctl(struct file *filp, UINT32 cmd, ULONG arg)
{
	...
	case WMT_IOCTL_SET_STP_MODE:	/* stp/hif/fm mode */
		...
		do {
			...
			osal_memcpy(&pOp->op.au4OpData[0], pHif, sizeof(struct WMT_HIF_CONF));
	...
	case WMT_IOCTL_SET_PATCH_NUM:
		...
		pPatchInfo = kcalloc(pAtchNum, sizeof(struct WMT_PATCH_INFO), GFP_ATOMIC);
		if (!pPatchInfo) {
	...
	case WMT_IOCTL_SET_PATCH_INFO:
		do {
			struct WMT_PATCH_INFO wMtPatchInfo;
			struct WMT_PATCH_INFO *pTemp = NULL;
			UINT32 dWloadSeq;
			...
			if (copy_from_user(&wMtPatchInfo, (PVOID)arg, sizeof(struct WMT_PATCH_INFO))) {
				WMT_ERR_FUNC("copy_from_user failed at %d\n", __LINE__);
				iRet = -EFAULT;
				break;
			}
			...
			osal_memcpy(pPatchInfo + dWloadSeq - 1, &wMtPatchInfo, sizeof(struct WMT_PATCH_INFO));
...
module_param(gWmtMajor, uint, 0000);
