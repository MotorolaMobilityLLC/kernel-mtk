...
INT32 osal_thread_wait_for_event(struct OSAL_THREAD *pThread, struct OSAL_EVENT *pEvent, P_OSAL_EVENT_CHECKER pChecker)
{
	/*  P_DEV_WMT pDevWmt;*/
...
INT32 osal_trigger_event(struct OSAL_EVENT *pEvent)
{
	INT32 ret = 0;
...
INT32 osal_wait_for_event(struct OSAL_EVENT *pEvent, INT32(*condition) (PVOID), PVOID cond_pa)
{
	if (pEvent)
...
INT32 osal_wait_for_event_timeout(struct OSAL_EVENT *pEvent, INT32(*condition) (PVOID), PVOID cond_pa)
{
	if (pEvent)
...
INT32 osal_bit_op_lock(struct OSAL_UNSLEEPABLE_LOCK *pLock)
{

	return 0;
}

INT32 osal_bit_op_unlock(struct OSAL_UNSLEEPABLE_LOCK *pLock)
{
...
INT32 osal_timer_start(struct OSAL_TIMER *pTimer, UINT32 ms)
{
...
INT32 osal_timer_modify(struct OSAL_TIMER *pTimer, UINT32 ms)
{
...
INT32 _osal_fifo_init(struct OSAL_FIFO *pFifo, PUINT8 buf, UINT32 size)
{
...
INT32 _osal_fifo_deinit(struct OSAL_FIFO *pFifo)
{
	struct kfifo *fifo = NULL;
...
INT32 _osal_fifo_size(struct OSAL_FIFO *pFifo)
{
	struct kfifo *fifo = NULL;
	INT32 ret = 0;
...
INT32 _osal_fifo_avail_size(struct OSAL_FIFO *pFifo)
{
	struct kfifo *fifo = NULL;
	INT32 ret = 0;
...
INT32 _osal_fifo_len(struct OSAL_FIFO *pFifo)
{
	struct kfifo *fifo = NULL;
	INT32 ret = 0;
...
INT32 _osal_fifo_is_empty(struct OSAL_FIFO *pFifo)
{
	struct kfifo *fifo = NULL;
	INT32 ret = 0;
...
INT32 _osal_fifo_is_full(struct OSAL_FIFO *pFifo)
{
	struct kfifo *fifo = NULL;
	INT32 ret = 0;
...
INT32 _osal_fifo_data_in(struct OSAL_FIFO *pFifo, const PVOID buf, UINT32 len)
{
	struct kfifo *fifo = NULL;
	INT32 ret = 0;
...
INT32 _osal_fifo_data_out(struct OSAL_FIFO *pFifo, PVOID buf, UINT32 len)
{
	struct kfifo *fifo = NULL;
	INT32 ret = 0;
...
INT32 _osal_fifo_reset(struct OSAL_FIFO *pFifo)
{
	struct kfifo *fifo = NULL;
...
INT32 osal_fifo_init(struct OSAL_FIFO *pFifo, UINT8 *buffer, UINT32 size)
{
	if (!pFifo) {
