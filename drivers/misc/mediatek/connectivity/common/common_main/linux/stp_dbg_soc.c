...
static _osal_inline_ INT32 stp_dbg_soc_paged_dump(INT32 dump_sink)
{
	...
	UINT32 loop_cnt1 = 0;
	UINT32 loop_cnt2 = 0;
	enum ENUM_HOST_DUMP_STATE host_state;
	enum ENUM_CHIP_DUMP_STATE chip_state;
	UINT32 dump_phy_addr = 0;
	PUINT8 dump_vir_addr = NULL;
	UINT32 dump_len = 0;
	UINT32 isEnd = 0;
	struct CONSYS_EMI_ADDR_INFO *p_ecsi;
	...
	do {
		...
		host_state = (enum ENUM_HOST_DUMP_STATE)wmt_plat_get_dump_info(
		...
		while (1) {
			chip_state = (enum ENUM_CHIP_DUMP_STATE)wmt_plat_get_dump_info(
...
UINT32 stp_dbg_soc_read_debug_crs(enum ENUM_CONNSYS_DEBUG_CR cr)
{
