...
struct mtkstp_parser_context_struct {
	enum mtkstp_parser_state state;
	UINT8 seq;
	UINT8 ack;
	UINT8 nak;
	UINT8 type;
...
struct mtkstp_sequence_context_struct {
...
struct mtkstp_ring_buffer_struct {
	/* MTK_WCN_MUTEX           mtx; */
	struct OSAL_UNSLEEPABLE_LOCK mtx;
	UINT8 buffer[MTKSTP_BUFFER_SIZE];
	UINT32 read_p;
	UINT32 write_p;
};

struct mtkstp_context_struct {
	...
	struct mtkstp_ring_buffer_struct ring[MTKSTP_MAX_TASK_NUM];	/* ring buffers for each function driver */
	struct mtkstp_parser_context_struct parser;	/* current rx pkt's content */
	struct mtkstp_sequence_context_struct sequence;	/* state machine's current status */
	/* MTK_WCN_MUTEX stp_mutex; */
#if CFG_STP_CORE_CTX_SPIN_LOCK
	struct OSAL_UNSLEEPABLE_LOCK stp_mutex;
#else
	struct OSAL_SLEEPABLE_LOCK stp_mutex;
#endif
	/* MTK_WCN_TIMER tx_timer; // timer for tx timeout handling */
	struct OSAL_TIMER tx_timer;

	struct MTKSTP_PSM_T *psm;
	struct MTKSTP_BTM_T *btm;
...
*****************************************************************************/
extern INT32 mtk_wcn_stp_init(const struct mtkstp_callback * const cb_func);
...
extern INT32 mtk_wcn_stp_set_psm_state(enum MTKSTP_PSM_STATE_T state);
...
extern VOID mtk_wcn_stp_set_if_tx_type(enum ENUM_STP_TX_IF_TYPE stp_if_type);
...
extern INT32 mtk_wcn_stp_dpidle_ctrl(enum ENUM_BTIF_DPIDLE_CTRL en_flag);
extern INT32 mtk_wcn_stp_lpbk_ctrl(enum ENUM_BTIF_LPBK_MODE mode);
extern INT32 mtk_wcn_stp_logger_ctrl(enum ENUM_BTIF_DBG_ID flag);
