...
enum MTKSTP_PSM_MONSTATE_T {
	MON = 0,
	UNMON,
};

typedef INT32(*wmt_notify_t) (enum MTKSTP_PSM_ACTION_T action);
...
/* OSAL_UNSLEEPABLE_LOCK   flagSpinlock; */
	...
	struct OSAL_WAKE_LOCK wake_lock;
	struct OSAL_TIMER psm_timer;	/*monitor if active */
	struct OSAL_EVENT wait_wmt_q;
	struct OSAL_FIFO hold_fifo;

	/* PSM_FIFO_LOCK hold_fifo_lock; */
	struct OSAL_SLEEPABLE_LOCK hold_fifo_spinlock_global;

	struct OSAL_UNSLEEPABLE_LOCK wq_spinlock;
	struct OSAL_SLEEPABLE_LOCK user_lock;
	struct OSAL_SLEEPABLE_LOCK stp_psm_lock;
	INT32 (*wmt_notify)(enum MTKSTP_PSM_ACTION_T action);
...
struct STP_PSM_RECORD_T {
	struct STP_PSM_ENTRY_T queue[STP_PSM_DBG_SIZE];
	UINT32 in;
	UINT32 out;
	UINT32 size;
	struct OSAL_UNSLEEPABLE_LOCK lock;
};

struct STP_PSM_OPID_RECORD {
	struct STP_PSM_ENTRY_T queue[STP_PSM_DBG_SIZE];
	UINT32 in;
	UINT32 out;
	UINT32 size;
...
INT32 stp_psm_hold_data(struct MTKSTP_PSM_T *stp_psm,
