...
typedef INT32(*WMT_OPID_FUNC) (struct OSAL_OP_DAT *);
...
typedef INT32(*SW_INIT) (struct WMT_HIF_CONF *pWmtHifConf);
typedef INT32(*SW_DEINIT) (struct WMT_HIF_CONF *pWmtHifConf);
typedef INT32(*IC_PIN_CTRL) (enum WMT_IC_PIN_ID id, enum WMT_IC_PIN_STATE state, UINT32 flag);
typedef INT32(*IC_VER_CHECK) (VOID);
typedef INT32(*CO_CLOCK_CTRL) (enum WMT_CO_CLOCK on);
...
struct WMT_CTX {
	enum ENUM_DRV_STS eDrvStatus[WMTDRV_TYPE_MAX];	/* Controlled driver status */
	UINT32 wmtInfoBit;	/* valid info bit */
	struct WMT_HIF_CONF wmtHifConf;	/* HIF information */
...
*****************************************************************************/
extern INT32 wmt_core_opid(struct OSAL_OP_DAT *pWmtOp);

extern INT32 wmt_core_ctrl(enum ENUM_WMT_CTRL_T ctrId, PULONG pPa1, PULONG pPa2);

extern INT32 wmt_core_func_ctrl_cmd(enum ENUM_WMTDRV_TYPE_T type, MTK_WCN_BOOL fgEn);
...
#endif
extern VOID wmt_core_set_coredump_state(enum ENUM_DRV_STS state);
...
extern enum ENUM_DRV_STS wmt_core_get_drv_status(enum ENUM_WMTDRV_TYPE_T type);
#endif
#if CFG_WMT_LTE_COEX_HANDLING
...
extern struct WMT_IC_OPS wmt_ic_ops_mt6620;
#endif

#if CFG_CORE_MT6628_SUPPORT
extern struct WMT_IC_OPS wmt_ic_ops_mt6628;
#endif

#if CFG_CORE_MT6630_SUPPORT
extern struct WMT_IC_OPS wmt_ic_ops_mt6630;
#endif

#if CFG_CORE_MT6632_SUPPORT
extern struct WMT_IC_OPS wmt_ic_ops_mt6632;
#endif

#if CFG_CORE_SOC_SUPPORT
extern struct WMT_IC_OPS wmt_ic_ops_soc;
#endif
...
static _osal_inline_ MTK_WCN_BOOL wmt_core_ic_ops_check(struct WMT_IC_OPS *p_ops)
{
	if (!p_ops)
