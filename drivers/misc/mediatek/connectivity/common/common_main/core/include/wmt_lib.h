...
struct DEV_WMT {
	...
	struct OSAL_SLEEPABLE_LOCK psm_lock;
	struct OSAL_SLEEPABLE_LOCK idc_lock;
	...
	struct OSAL_EVENT rWmtdWq;	/* rename */
	/* ULONG state; *//* bit field of WMT_STAT */
	struct OSAL_BIT_OP_VAR state;

	/* STP context information */
	/* wait_queue_head_t rWmtRxWq;  *//* STP Rx wait queue */
	struct OSAL_EVENT rWmtRxWq;	/* rename */
	/* WMT_STP_FUNC rStpFunc; *//* STP functions */
	struct WMT_FDRV_CB rFdrvCb;

	/* WMT Configurations */
	struct WMT_HIF_CONF rWmtHifConf;
	struct WMT_GEN_CONF rWmtGenConf;
	...
	struct OSAL_OP arQue[WMT_OP_BUF_SIZE];	/* real op instances */
...
/* struct completion cmd_comp; */
	...
	struct OSAL_SIGNAL cmdResp;
	struct OSAL_EVENT cmdReq;
...
/* WMT_CMB_VER combo_ver; */
	...
	struct WMT_PATCH_INFO *pWmtPatchInfo;
};
...
/* LXOP functions: */
extern struct OSAL_OP *wmt_lib_get_free_op(VOID);
extern INT32 wmt_lib_put_op_to_free_queue(struct OSAL_OP *pOp);
extern MTK_WCN_BOOL wmt_lib_put_act_op(struct OSAL_OP *pOp);

/* extern ENUM_WMTHWVER_TYPE_T wmt_lib_get_hwver (VOID); */
extern UINT32 wmt_lib_get_icinfo(enum ENUM_WMT_CHIPINFO_TYPE_T type);
...
extern struct OSAL_EVENT *wmt_lib_get_cmd_event(VOID);
...
extern struct WMT_HIF_CONF *wmt_lib_get_hif(VOID);
...
#endif
extern INT32 wmt_lib_msgcb_reg(enum ENUM_WMTDRV_TYPE_T eType, PF_WMT_CB pCb);

extern INT32 wmt_lib_msgcb_unreg(enum ENUM_WMTDRV_TYPE_T eType);
enum ENUM_WMTRSTRET_TYPE_T wmt_lib_cmb_rst(enum ENUM_WMTRSTSRC_TYPE_T src);
...
extern VOID wmt_lib_set_patch_info(struct WMT_PATCH_INFO *pPatchinfo);
extern MTK_WCN_BOOL wmt_lib_stp_is_btif_fullset_mode(VOID);

extern INT32 wmt_lib_set_current_op(struct DEV_WMT *pWmtDev, struct OSAL_OP *pOp);
extern struct OSAL_OP *wmt_lib_get_current_op(struct DEV_WMT *pWmtDev);
