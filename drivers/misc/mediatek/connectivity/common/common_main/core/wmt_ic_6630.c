...
static const struct WMT_IC_INFO_S *gp_mt6630_info;
static struct WMT_PATCH gp_mt6630_patch_info;
static enum WMT_CO_CLOCK gCoClockEn = WMT_CO_CLOCK_DIS;
...
static const struct WMT_IC_INFO_S mt6630_info_table[] = {
	{
	 .u4HwVer = 0x8A00,
...
static INT32 mt6630_sw_init(struct WMT_HIF_CONF *pWmtHifConf);

static INT32 mt6630_sw_deinit(struct WMT_HIF_CONF *pWmtHifConf);

static INT32 mt6630_pin_ctrl(enum WMT_IC_PIN_ID id, enum WMT_IC_PIN_STATE state, UINT32 flag);

static INT32 mt6630_aif_ctrl(enum WMT_IC_PIN_STATE state, UINT32 flag);

static INT32 mt6630_ver_check(VOID);

static const struct WMT_IC_INFO_S *mt6630_find_wmt_ic_info(const UINT32 hw_ver);
...
static INT32 mt6630_co_clock_ctrl(enum WMT_CO_CLOCK on);
static enum WMT_CO_CLOCK mt6630_co_clock_get(VOID);
...
struct WMT_IC_OPS wmt_ic_ops_mt6630 = {
	.icId = 0x6630,
...
static INT32 mt6630_gps_sync_ctrl(enum WMT_IC_PIN_STATE state, UINT32 flag)
{
	INT32 iRet = -1;
	UINT32 uVal = 0;
...
INT32 mt6630_co_clock_ctrl(enum WMT_CO_CLOCK on)
{
	INT32 iRet = 0;
...
enum WMT_CO_CLOCK mt6630_co_clock_get(VOID)
{
	return gCoClockEn;
}
...
static INT32 mt6630_ver_check(VOID)
{
	...
	UINT32 fw_ver;
	INT32 iret;
	const struct WMT_IC_INFO_S *p_info;
	ULONG ctrlPa1;
	ULONG ctrlPa2;
...
static INT32 wmt_stp_init_coex(VOID)
{
	INT32 iRet;
	ULONG addr;
	struct WMT_GEN_CONF *pWmtGenConf;
...
#define COEX_MISC 4
	...
	pWmtGenConf = (struct WMT_GEN_CONF *) addr;
...
{
	INT32 iRet = -1;
	struct WMT_PATCH *patchHdr = NULL;
	PUINT8 pBuf = NULL;
	...
	patchHdr = (struct WMT_PATCH *) pPatchBuf;
	...
	osal_memcpy(&gp_mt6630_patch_info, patchHdr, osal_sizeof(struct WMT_PATCH));
	 ...
	 */
	patchSize -= sizeof(struct WMT_PATCH);
	pPatchBuf += sizeof(struct WMT_PATCH);
...
static INT32 mt6630_patch_dwn(VOID)
{
	 ...
	 * |<-patch header: 28 Bytes->|<-patch body: X Bytes ----->|
	 */
	patchHdr = (struct WMT_PATCH) pbuf;
	/* check patch file information */
