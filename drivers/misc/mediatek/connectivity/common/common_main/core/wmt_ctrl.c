...
static INT32 wmt_ctrl_stp_conf_ex(enum WMT_STP_CONF_TYPE type, UINT32 value);

static INT32 wmt_ctrl_hw_pwr_off(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_hw_pwr_on(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_hw_rst(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_stp_close(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_stp_open(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_stp_conf(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_free_patch(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_get_patch(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_host_baudrate_set(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_sdio_hw(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_sdio_func(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_hwidver_set(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_stp_rst(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_get_wmt_conf(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_others(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_tx(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_rx(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_patch_search(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_crystal_triming_put(struct WMT_CTRL_DATA *pWmtCtrlData);
static INT32 wmt_ctrl_crystal_triming_get(struct WMT_CTRL_DATA *pWmtCtrlData);
static INT32 wmt_ctrl_hw_state_show(struct WMT_CTRL_DATA *pWmtCtrlData);
static INT32 wmt_ctrl_get_patch_num(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_get_patch_info(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_rx_flush(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_soc_paldo_ctrl(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_soc_wakeup_consys(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_set_stp_dbg_info(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_bgw_desense_ctrl(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_evt_err_trg_assert(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_evt_parser(struct WMT_CTRL_DATA *pWmtCtrlData);
#if CFG_WMT_LTE_COEX_HANDLING
static INT32 wmt_ctrl_get_tdm_req_antsel(struct WMT_CTRL_DATA *);
#endif

static INT32 wmt_ctrl_gps_sync_set(struct WMT_CTRL_DATA *pData);

static INT32 wmt_ctrl_gps_lna_set(struct WMT_CTRL_DATA *pData);


static INT32 wmt_ctrl_get_patch_name(struct WMT_CTRL_DATA *pWmtCtrlData);
static INT32 wmt_ctrl_set_stp_dbg_info(struct WMT_CTRL_DATA *);
static INT32 wmt_ctrl_evt_err_trg_assert(struct WMT_CTRL_DATA *pWmtCtrlData);
...
INT32 wmt_ctrl(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	UINT32 ctrlId = 0;
...
INT32 wmt_ctrl_tx(struct WMT_CTRL_DATA *pWmtCtrlData /*UINT8 *pData, UINT32 size, UINT32 *writtenSize */)
{
...
INT32 wmt_ctrl_rx(struct WMT_CTRL_DATA *pWmtCtrlData /*UINT8 *pBuff, UINT32 buffLen, UINT32 *readSize */)
{
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
	INT32 readLen;
	INT32 waitRet = -1;
...
INT32 wmt_ctrl_rx_flush(struct WMT_CTRL_DATA *pWmtCtrlData)
{
...
INT32 wmt_ctrl_hw_pwr_off(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iret;
	/*psm should be disabled before wmt_ic_deinit*/
	struct DEV_WMT *pDev = &gDevWmt;
...
INT32 wmt_ctrl_hw_pwr_on(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iret;
...
INT32 wmt_ctrl_ul_cmd(struct DEV_WMT *pWmtDev, const PUINT8 pCmdStr)
{
	INT32 waitRet = -1;
	struct OSAL_SIGNAL *pCmdSignal;
	struct OSAL_EVENT *pCmdReq;
...
INT32 wmt_ctrl_hw_rst(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	wmt_plat_pwr_ctrl(FUNC_RST);
	return 0;
}

INT32 wmt_ctrl_hw_state_show(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	wmt_plat_pwr_ctrl(FUNC_STAT);
	return 0;
}

INT32 wmt_ctrl_stp_close(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
	INT32 iRet = 0;
...
INT32 wmt_ctrl_stp_open(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
	INT32 iRet;
	UINT8 cmdStr[NAME_MAX + 1] = { 0 };
...
INT32 wmt_ctrl_patch_search(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
	INT32 iRet;
	UINT8 cmdStr[NAME_MAX + 1] = { 0 };
...
INT32 wmt_ctrl_get_patch_num(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
...
INT32 wmt_ctrl_get_patch_info(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
	UINT32 downLoadSeq = 0;
	struct WMT_PATCH_INFO *pPatchinfo = NULL;
	PUINT8 pNbuf = NULL;
	PUINT8 pAbuf = NULL;
...
INT32 wmt_ctrl_soc_paldo_ctrl(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iRet = 0;
	enum ENUM_PALDO_TYPE ept = pWmtCtrlData->au4CtrlData[0];
	enum ENUM_PALDO_OP epo = pWmtCtrlData->au4CtrlData[1];
...
INT32 wmt_ctrl_soc_wakeup_consys(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iRet = 0;
...
static INT32 wmt_ctrl_bgw_desense_ctrl(struct WMT_CTRL_DATA *pWmtCtrlData)
{
...
#if CFG_WMT_LTE_COEX_HANDLING
static INT32 wmt_ctrl_get_tdm_req_antsel(struct WMT_CTRL_DATA *pWmtCtrlData)
{
...
INT32 wmt_ctrl_stp_conf_ex(enum WMT_STP_CONF_TYPE type, UINT32 value)
{
	INT32 iRet = -1;
...
INT32 wmt_ctrl_stp_conf(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iRet = -1;
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
	UINT32 type;
...
INT32 wmt_ctrl_free_patch(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	UINT32 patchSeq = pWmtCtrlData->au4CtrlData[0];

	WMT_DBG_FUNC("BF free patch, gDevWmt.pPatch(%p)\n", gDevWmt.pPatch);
	if (gDevWmt.pPatch != NULL)
		wmt_dev_patch_put((struct firmware **) &(gDevWmt.pPatch));
...
INT32 wmt_ctrl_get_patch_name(struct WMT_CTRL_DATA *pWmtCtrlData)
{
...
INT32 wmt_ctrl_crystal_triming_put(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	WMT_DBG_FUNC("BF free patch, gDevWmt.pPatch(%p)\n", gDevWmt.pPatch);
	if (gDevWmt.pNvram != NULL)
		wmt_dev_patch_put((struct firmware **) &(gDevWmt.pNvram));
	WMT_DBG_FUNC("AF free patch, gDevWmt.pNvram(%p)\n", gDevWmt.pNvram);
	return 0;
}


INT32 wmt_ctrl_crystal_triming_get(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iRet = 0x0;
...
INT32 wmt_ctrl_get_patch(struct WMT_CTRL_DATA *pWmtCtrlData)
{
...
INT32 wmt_ctrl_host_baudrate_set(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iRet = -1;
...
INT32 wmt_ctrl_sdio_hw(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iRet = 0;
	UINT32 statBit = WMT_STAT_SDIO1_ON;
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */

	enum WMT_SDIO_SLOT_NUM sdioSlotNum = pWmtCtrlData->au4CtrlData[0];
	enum ENUM_FUNC_STATE funcState = pWmtCtrlData->au4CtrlData[1];
...
INT32 wmt_ctrl_sdio_func(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	INT32 iRet = -1;
	UINT32 statBit = WMT_STAT_SDIO_WIFI_ON;
	INT32 retry = 10;
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
	enum WMT_SDIO_FUNC_TYPE sdioFuncType = pWmtCtrlData->au4CtrlData[0];
...
INT32 wmt_ctrl_hwidver_set(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
	...
	pDev->eWmtHwVer = (enum ENUM_WMTHWVER_TYPE_T) (pWmtCtrlData->au4CtrlData[1] & 0xFFFF0000) >> 16;
...
INT32 wmt_ctrl_stp_rst(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	return 0;
}

INT32 wmt_ctrl_get_wmt_conf(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	struct DEV_WMT *pDev = &gDevWmt;	/* single instance */
...
INT32 wmt_ctrl_others(struct WMT_CTRL_DATA *pWmtCtrlData)
{
...
INT32 wmt_ctrl_set_stp_dbg_info(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	PUINT8 pRomVer = NULL;
	struct WMT_PATCH *pPatch = NULL;
	UINT32 chipID = 0;

	chipID = pWmtCtrlData->au4CtrlData[0];
	pRomVer = (PUINT8) (pWmtCtrlData->au4CtrlData[1]);
	pPatch = (struct WMT_PATCH *)(pWmtCtrlData->au4CtrlData[2]);
	if (!pRomVer) {
...
static INT32 wmt_ctrl_evt_err_trg_assert(struct WMT_CTRL_DATA *pWmtCtrlData)
{
	...
	enum ENUM_WMTDRV_TYPE_T drv_type;
	UINT32 reason = 0;
