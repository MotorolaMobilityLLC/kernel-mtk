...
struct MTKSTP_PSM_T stp_psm_i;
struct MTKSTP_PSM_T *stp_psm = &stp_psm_i;

struct STP_PSM_RECORD_T *g_stp_psm_dbg;
static UINT32 g_record_num;

struct STP_PSM_OPID_RECORD *g_stp_psm_opid_dbg;
...
static inline INT32 _stp_psm_notify_wmt(struct MTKSTP_PSM_T *stp_psm, const enum MTKSTP_PSM_ACTION_T action);
static INT32 _stp_psm_thread_lock_aquire(struct MTKSTP_PSM_T *stp_psm);
static INT32 _stp_psm_thread_lock_release(struct MTKSTP_PSM_T *stp_psm);
static INT32 _stp_psm_dbg_dmp_in(struct STP_PSM_RECORD_T *stp_psm_dbg, UINT32 flag, UINT32 line_num);
static INT32 _stp_psm_dbg_out_printk(struct STP_PSM_RECORD_T *stp_psm_dbg);
static INT32 _stp_psm_opid_dbg_dmp_in(struct STP_PSM_OPID_RECORD *p_opid_dbg, UINT32 opid, UINT32 line_num);
static INT32 _stp_psm_opid_dbg_out_printk(struct STP_PSM_OPID_RECORD *p_opid_dbg);
...
static INT32 _stp_psm_release_data(struct MTKSTP_PSM_T *stp_psm);

static inline INT32 _stp_psm_get_state(struct MTKSTP_PSM_T *stp_psm);

static INT32 _stp_psm_is_redundant_active_op(struct OSAL_OP *pOp, struct OSAL_OP_Q *pOpQ);

static INT32 _stp_psm_clean_up_redundant_active_op(struct OSAL_OP_Q *pOpQ);
static MTK_WCN_BOOL _stp_psm_is_quick_ps_support(VOID);

enum ENUM_STP_TX_IF_TYPE __weak wmt_plat_get_comm_if_type(VOID)
{
...
#endif
static INT32 _stp_psm_handler(struct MTKSTP_PSM_T *stp_psm, struct OSAL_OP_DAT *pStpOp)
{
	INT32 ret = -1;
...
static struct OSAL_OP *_stp_psm_get_op(struct MTKSTP_PSM_T *stp_psm, struct OSAL_OP_Q *pOpQ)
{
	struct OSAL_OP *pOp;
...
static INT32 _stp_psm_dump_active_q(struct OSAL_OP_Q *pOpQ)
{
	UINT32 read_idx;
	UINT32 write_idx;
	UINT32 opId;
...
static INT32 _stp_psm_clean_up_redundant_active_op(struct OSAL_OP_Q *pOpQ)
{
	...
	struct OSAL_OP *pOp;
	struct OSAL_OP_Q *pFreeOpQ = &stp_psm->rFreeOpQ;
...
static INT32 _stp_psm_put_op(struct MTKSTP_PSM_T *stp_psm, struct OSAL_OP_Q *pOpQ, struct OSAL_OP *pOp)
{
	INT32 ret;
...
struct OSAL_OP *_stp_psm_get_free_op(struct MTKSTP_PSM_T *stp_psm)
{
	struct OSAL_OP *pOp;
...
INT32 _stp_psm_put_act_op(struct MTKSTP_PSM_T *stp_psm, struct OSAL_OP *pOp)
{
	INT32 bRet = 0;		/* MTK_WCN_BOOL_FALSE; */
	INT32 bCleanup = 0;	/* MTK_WCN_BOOL_FALSE; */
	INT32 wait_ret = -1;
	struct OSAL_SIGNAL *pSignal = NULL;
	INT32 ret = 0;
...
{
	struct MTKSTP_PSM_T *stp_psm = (struct MTKSTP_PSM_T *)pvData;
...
static inline INT32 _stp_psm_set_state(struct MTKSTP_PSM_T *stp_psm, const enum MTKSTP_PSM_STATE_T state)
{
	if (stp_psm == NULL)
		return STP_PSM_OPERATION_FAIL;
	if (stp_psm->work_state < STP_PSM_MAX_STATE) {
...
static inline INT32 _stp_psm_start_monitor(struct MTKSTP_PSM_T *stp_psm)
{
...
static inline INT32 _stp_psm_stop_monitor(struct MTKSTP_PSM_T *stp_psm)
{
	if (!stp_psm)
		return STP_PSM_OPERATION_FAIL;
...
INT32
_stp_psm_hold_data(struct MTKSTP_PSM_T *stp_psm, const PUINT8 buffer, const UINT32 len, const UINT8 type)
{
...
INT32 _stp_psm_has_pending_data(struct MTKSTP_PSM_T *stp_psm)
{
	return osal_fifo_len(&stp_psm->hold_fifo);
}

INT32 _stp_psm_release_data(struct MTKSTP_PSM_T *stp_psm)
{
...
static inline INT32 _stp_psm_notify_wmt_host_awake_wq(struct MTKSTP_PSM_T *stp_psm)
{
...
static inline INT32 _stp_psm_notify_wmt_wakeup_wq(struct MTKSTP_PSM_T *stp_psm)
{
	struct OSAL_OP *pOp;
	INT32 bRet;
	INT32 retval;
...
static inline INT32 _stp_psm_notify_wmt_sleep_wq(struct MTKSTP_PSM_T *stp_psm)
{
	struct OSAL_OP *pOp;
	INT32 bRet;
	INT32 retval;
...
static inline INT32 _stp_psm_reset(struct MTKSTP_PSM_T *stp_psm)
{
	INT32 i = 0;
	struct OSAL_OP_Q *pOpQ;
	struct OSAL_OP *pOp;
	INT32 ret = 0;
...
static inline INT32 _stp_psm_wait_wmt_event_wq(struct MTKSTP_PSM_T *stp_psm)
{
...
static inline INT32 _stp_psm_notify_stp(struct MTKSTP_PSM_T *stp_psm, const enum MTKSTP_PSM_ACTION_T action)
{
...
{
	struct MTKSTP_PSM_T *stp_psm = (struct MTKSTP_PSM_T *) data;
...
static inline INT32 _stp_psm_init_monitor(struct MTKSTP_PSM_T *stp_psm)
{
	if (!stp_psm)
		return STP_PSM_OPERATION_FAIL;
...
static inline INT32 _stp_psm_deinit_monitor(struct MTKSTP_PSM_T *stp_psm)
{
	if (!stp_psm)
		return STP_PSM_OPERATION_FAIL;
...
static inline INT32 _stp_psm_is_to_block_traffic(struct MTKSTP_PSM_T *stp_psm)
{
	INT32 iRet = -1;
	/* osal_lock_unsleepable_lock(&stp_psm->flagSpinlock); */
...
static inline INT32 _stp_psm_is_disable(struct MTKSTP_PSM_T *stp_psm)
{
	if (osal_test_bit(STP_PSM_WMT_EVENT_DISABLE_MONITOR, &stp_psm->flag))
		return 1;
	return 0;
...
static inline INT32 _stp_psm_do_wait(struct MTKSTP_PSM_T *stp_psm, enum MTKSTP_PSM_STATE_T state)
{
...
static inline INT32 _stp_psm_do_wakeup(struct MTKSTP_PSM_T *stp_psm)
{
	INT32 ret = 0;
	INT32 retry = 10;
	struct OSAL_OP_Q *pOpQ;
...
static inline INT32 _stp_psm_disable(struct MTKSTP_PSM_T *stp_psm)
{
...
static inline INT32 _stp_psm_enable(struct MTKSTP_PSM_T *stp_psm, INT32 idle_time_to_sleep)
{
	INT32 ret = STP_PSM_OPERATION_FAIL;
...
INT32 _stp_psm_thread_lock_aquire(struct MTKSTP_PSM_T *stp_psm)
{
	return osal_lock_sleepable_lock(&stp_psm->stp_psm_lock);
}

INT32 _stp_psm_thread_lock_release(struct MTKSTP_PSM_T *stp_psm)
{
...
#if PSM_USE_COUNT_PACKAGE
INT32 stp_psm_disable_by_tx_rx_density(struct MTKSTP_PSM_T *stp_psm, INT32 dir)
{
...
INT32 stp_psm_disable_by_tx_rx_density(struct MTKSTP_PSM_T *stp_psm, INT32 dir, INT32 length)
{
	if (sample_start) {
		if (dir)
			rx_sum_len += length;
...
INT32 stp_psm_set_state(struct MTKSTP_PSM_T *stp_psm, enum MTKSTP_PSM_STATE_T state)
{
	return _stp_psm_set_state(stp_psm, state);
}


INT32 stp_psm_thread_lock_aquire(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_thread_lock_aquire(stp_psm);
}


INT32 stp_psm_thread_lock_release(struct MTKSTP_PSM_T *stp_psm)
{
...
INT32 stp_psm_do_wakeup(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_do_wakeup(stp_psm);
}

INT32 stp_psm_notify_stp(struct MTKSTP_PSM_T *stp_psm, const enum MTKSTP_PSM_ACTION_T action)
{

	return _stp_psm_notify_stp(stp_psm, action);
}

INT32 stp_psm_notify_wmt_wakeup(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_notify_wmt_wakeup_wq(stp_psm);
}

INT32 stp_psm_notify_wmt_sleep(struct MTKSTP_PSM_T *stp_psm)
{

	return _stp_psm_notify_wmt_sleep_wq(stp_psm);
}

INT32 stp_psm_start_monitor(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_start_monitor(stp_psm);
}

INT32 stp_psm_is_to_block_traffic(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_is_to_block_traffic(stp_psm);
}

INT32 stp_psm_is_disable(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_is_disable(stp_psm);
}

INT32 stp_psm_has_pending_data(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_has_pending_data(stp_psm);
}

INT32 stp_psm_release_data(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_release_data(stp_psm);
}

INT32
stp_psm_hold_data(struct MTKSTP_PSM_T *stp_psm, const PUINT8 buffer, const UINT32 len, const UINT8 type)
{
	return _stp_psm_hold_data(stp_psm, buffer, len, type);
}

INT32 stp_psm_disable(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_disable(stp_psm);
}

INT32 stp_psm_enable(struct MTKSTP_PSM_T *stp_psm, INT32 idle_time_to_sleep)
{
	return _stp_psm_enable(stp_psm, idle_time_to_sleep);
}

INT32 stp_psm_reset(struct MTKSTP_PSM_T *stp_psm)
{
...
INT32 stp_psm_sleep_for_thermal(struct MTKSTP_PSM_T *stp_psm)
{
	return _stp_psm_notify_wmt_sleep_wq(stp_psm);
}


INT32 stp_psm_set_sleep_enable(struct MTKSTP_PSM_T *stp_psm)
{
	INT32 ret = 0;
...
INT32 stp_psm_set_sleep_disable(struct MTKSTP_PSM_T *stp_psm)
{
	INT32 ret = 0;
 ...
 */
INT32 stp_psm_check_sleep_enable(struct MTKSTP_PSM_T *stp_psm)
{
	INT32 ret = 0;
...
struct MTKSTP_PSM_T *stp_psm_init(VOID)
{
	INT32 err = 0;
	INT32 i = 0;
	INT32 ret = -1;
...
/* osal_unsleepable_lock_init(&stp_psm->flagSpinlock); */
	...
	g_stp_psm_dbg = (struct STP_PSM_RECORD_T *) osal_malloc(osal_sizeof(struct STP_PSM_RECORD_T));
	if (!g_stp_psm_dbg) {
		STP_PSM_ERR_FUNC("stp psm dbg allocate memory fail!\n");
		return NULL;
	}
	osal_memset(g_stp_psm_dbg, 0, osal_sizeof(struct STP_PSM_RECORD_T));
	osal_unsleepable_lock_init(&g_stp_psm_dbg->lock);

	g_stp_psm_opid_dbg = (struct STP_PSM_OPID_RECORD *) osal_malloc(osal_sizeof(struct STP_PSM_OPID_RECORD));
	if (!g_stp_psm_opid_dbg) {
		STP_PSM_ERR_FUNC("stp psm dbg allocate memory fail!\n");
		return NULL;
	}
	osal_memset(g_stp_psm_opid_dbg, 0, osal_sizeof(struct STP_PSM_OPID_RECORD));
...
INT32 stp_psm_deinit(struct MTKSTP_PSM_T *stp_psm)
{
	INT32 ret = -1;
