...
static const enum WMT_IC_PIN_STATE cmb_aif2pin_stat[] = {
...
#if CFG_WMT_PS_SUPPORT
static MTK_WCN_BOOL wmt_lib_ps_action(enum MTKSTP_PSM_ACTION_T action);
...
static INT32 wmt_lib_ps_handler(enum MTKSTP_PSM_ACTION_T action);
#endif

static MTK_WCN_BOOL wmt_lib_put_op(struct OSAL_OP_Q *pOpQ, struct OSAL_OP *pLxOp);

static struct OSAL_OP *wmt_lib_get_op(struct OSAL_OP_Q *pOpQ);

static INT32 wmtd_thread(PVOID pvData);

static INT32 wmt_lib_pin_ctrl(enum WMT_IC_PIN_ID id, enum WMT_IC_PIN_STATE stat, UINT32 flag);
...
********************************************************************************
*/
INT32 __weak mtk_wcn_consys_stp_btif_dpidle_ctrl(enum ENUM_BTIF_DPIDLE_CTRL en_flag)
{
...
INT32 wmt_lib_init(VOID)
{
	INT32 iRet;
	UINT32 i;
	struct DEV_WMT *pDevWmt;
	struct OSAL_THREAD *pThread;
	...
	if (gDevWmt.rWmtGenConf.cfgExist != 0) {
		struct PWR_SEQ_TIME pwrSeqTime;
...
{
	struct OSAL_SIGNAL *pSignal = &gDevWmt.cmdResp;
...
struct OSAL_EVENT *wmt_lib_get_cmd_event(VOID)
{
...
{
	UINT32 val;
	struct WMT_HIF_CONF *pHif = &gDevWmt.rWmtHifConf;
...
struct WMT_HIF_CONF *wmt_lib_get_hif(VOID)
{
...
INT32 wmt_lib_ps_stp_cb(enum MTKSTP_PSM_ACTION_T action)
{
#if CFG_WMT_PS_SUPPORT
...
UINT32 wmt_lib_wait_event_checker(struct OSAL_THREAD *pThread)
{
	struct DEV_WMT *pDevWmt;

	if (pThread) {
		pDevWmt = (struct DEV_WMT *) (pThread->pThreadData);
...
{
	struct DEV_WMT *pWmtDev = (struct DEV_WMT *) pvData;
	struct OSAL_EVENT *pEvent = NULL;
	struct OSAL_OP *pOp;
	INT32 iResult;
...
static MTK_WCN_BOOL wmt_lib_put_op(struct OSAL_OP_Q *pOpQ, struct OSAL_OP *pOp)
{
	INT32 iRet;
...
INT32 wmt_lib_put_op_to_free_queue(struct OSAL_OP *pOp)
{
	struct DEV_WMT *pWmtDev = &gDevWmt;
...
struct OSAL_OP *wmt_lib_get_free_op(VOID)
{
	struct OSAL_OP *pOp = NULL;
	struct DEV_WMT *pDevWmt = &gDevWmt;
...
MTK_WCN_BOOL wmt_lib_put_act_op(struct OSAL_OP *pOp)
{
	struct DEV_WMT *pWmtDev = &gDevWmt;
	MTK_WCN_BOOL bRet = MTK_WCN_BOOL_FALSE;
...
enum ENUM_WMTHWVER_TYPE_T wmt_lib_get_hwver(VOID)
{
/*
...
UINT32 wmt_lib_get_icinfo(enum ENUM_WMT_CHIPINFO_TYPE_T index)
{
...
MTK_WCN_BOOL wmt_lib_btm_cb(enum MTKSTP_BTM_WMT_OP_T op)
{
	MTK_WCN_BOOL bRet = MTK_WCN_BOOL_FALSE;
...
MTK_WCN_BOOL wmt_cdev_rstmsg_snd(enum ENUM_WMTRSTMSG_TYPE_T msg)
{
	...
	for (i = 0; i <= WMTDRV_TYPE_ANT; i++) {
		...
		if (pDevWmt->rFdrvCb.fDrvRst[i]) {
			...
			pDevWmt->rFdrvCb.fDrvRst[i] (WMTDRV_TYPE_WMT, i, WMTMSG_TYPE_RESET, &msg,
						     sizeof(enum ENUM_WMTRSTMSG_TYPE_T));
...
#define RETRYTIMES 10
	MTK_WCN_BOOL bRet;
	enum ENUM_WMTRSTRET_TYPE_T retval = WMTRSTRET_MAX;
	enum ENUM_WMTRSTMSG_TYPE_T rstMsg = WMTRSTMSG_RESET_MAX;
...
MTK_WCN_BOOL wmt_lib_msgcb_reg(enum ENUM_WMTDRV_TYPE_T eType, PF_WMT_CB pCb)
{
...
MTK_WCN_BOOL wmt_lib_msgcb_unreg(enum ENUM_WMTDRV_TYPE_T eType)
{
	MTK_WCN_BOOL bRet = MTK_WCN_BOOL_FALSE;
	struct DEV_WMT *pWmtDev = &gDevWmt;
...
VOID wmt_lib_set_patch_info(struct WMT_PATCH_INFO *pPatchinfo)
{
	struct DEV_WMT *pWmtDev = &gDevWmt;
...
INT32 wmt_lib_set_current_op(struct DEV_WMT *pWmtDev, struct OSAL_OP *pOp)
{
	if (pWmtDev) {
		pWmtDev->pCurOP = pOp;
...
struct OSAL_OP *wmt_lib_get_current_op(struct DEV_WMT *pWmtDev)
{
	if (pWmtDev)
...
UINT8 *wmt_lib_get_fwinfor_from_emi(UINT8 section, UINT32 offset, UINT8 *buf, UINT32 len)
{
	...
	UINT32 sublen1 = 0;
	UINT32 sublen2 = 0;
	struct CONSYS_EMI_ADDR_INFO *p_consys_info;
...
{
	return wmt_core_get_drv_status((enum ENUM_WMTDRV_TYPE_T) type);
}
#endif
