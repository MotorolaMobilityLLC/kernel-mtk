...
static const struct WMT_IC_INFO_S *gp_soc_info;
static struct WMT_PATCH gp_soc_patch_info;
...
static const struct WMT_IC_INFO_S mtk_wcn_soc_info_table[] = {
	{
	 .u4HwVer = 0x8A00,
...
static INT32 mtk_wcn_soc_sw_init(struct WMT_HIF_CONF *pWmtHifConf);

static INT32 mtk_wcn_soc_sw_deinit(struct WMT_HIF_CONF *pWmtHifConf);

static INT32 mtk_wcn_soc_pin_ctrl(enum WMT_IC_PIN_ID id, enum WMT_IC_PIN_STATE state, UINT32 flag);

static INT32 mtk_wcn_soc_aif_ctrl(enum WMT_IC_PIN_STATE state, UINT32 flag);

static INT32 mtk_wcn_soc_ver_check(VOID);

static const struct WMT_IC_INFO_S *mtk_wcn_soc_find_wmt_ic_info(const UINT32 hw_ver);
...
static INT32 mtk_wcn_soc_co_clock_ctrl(enum WMT_CO_CLOCK on);
static enum WMT_CO_CLOCK mtk_wcn_soc_co_clock_get(VOID);
...
struct WMT_IC_OPS wmt_ic_ops_soc = {
...
static INT32 mtk_wcn_soc_gps_sync_ctrl(enum WMT_IC_PIN_STATE state, UINT32 flag)
{
	INT32 iRet = -1;
	UINT32 uVal = 0;
...
INT32 mtk_wcn_soc_co_clock_ctrl(enum WMT_CO_CLOCK on)
{
	INT32 iRet = 0;
...
enum WMT_CO_CLOCK mtk_wcn_soc_co_clock_get(VOID)
{
	return gCoClockEn;
}
...
static INT32 mtk_wcn_soc_patch_dwn(UINT32 index)
{
	...
	osal_memcpy(&gp_soc_patch_info, patchHdr, osal_sizeof(struct WMT_PATCH));
