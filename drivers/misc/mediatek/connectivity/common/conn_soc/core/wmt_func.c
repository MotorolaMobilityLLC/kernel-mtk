...
INT32 _osal_inline_ wmt_func_bt_ctrl(ENUM_FUNC_STATE funcState)
{
	...
	return wmt_core_func_ctrl_cmd(WMTDRV_TYPE_BT, (funcState == FUNC_ON) ? MTK_WCN_BOOL_TRUE : MTK_WCN_BOOL_FALSE);
}
...
INT32 _osal_inline_ wmt_func_gps_ctrl(ENUM_FUNC_STATE funcState)
{
	...
	return wmt_core_func_ctrl_cmd(WMTDRV_TYPE_GPS, (funcState == FUNC_ON) ? MTK_WCN_BOOL_TRUE : MTK_WCN_BOOL_FALSE);
}
...
INT32 wmt_func_gps_pre_ctrl(P_WMT_IC_OPS pOps, P_WMT_GEN_CONF pConf, ENUM_FUNC_STATE funcStatus)
{
	...
	if ((pOps->ic_pin_ctrl == NULL) ||
		(pOps->ic_pin_ctrl(
				WMT_IC_PIN_GSYNC,
				funcStatus == FUNC_ON ? WMT_IC_PIN_MUX : WMT_IC_PIN_GPIO,
				1) < 0)) {	/*WMT_IC_PIN_GSYNC */
		pCmbPinCtrl = &gCmbPinCtrl[CMB_PIN_GSYNC_ID];
		regNum = pCmbPinCtrl->regNum;
		for (i = 0; i < regNum; i++) {
			if (funcStatus == FUNC_ON)
	...
	if (pConf != NULL) {
		if (pConf->wmt_gps_lna_enable == 0) {
			...
			wmtIcPinId = pConf->wmt_gps_lna_pin == 0 ? WMT_IC_PIN_EEDI : WMT_IC_PIN_EEDO;
			if ((pOps->ic_pin_ctrl == NULL) ||
				(pOps->ic_pin_ctrl(
					wmtIcPinId,
					funcStatus == FUNC_ON ? WMT_IC_PIN_GPIO_HIGH : WMT_IC_PIN_GPIO_LOW,
					1) < 0)) {	/*WMT_IC_PIN_GSYNC */
...
INT32 _osal_inline_ wmt_func_fm_ctrl(ENUM_FUNC_STATE funcState)
{
	...
	return wmt_core_func_ctrl_cmd(WMTDRV_TYPE_FM, (funcState == FUNC_ON) ? MTK_WCN_BOOL_TRUE : MTK_WCN_BOOL_FALSE);
}
...
INT32 wmt_func_wifi_ctrl(ENUM_FUNC_STATE funcState)
{
	...
	unsigned long ctrlPa2 = (funcState == FUNC_ON) ? 1 : 0;	/* turn on Wi-Fi driver */
