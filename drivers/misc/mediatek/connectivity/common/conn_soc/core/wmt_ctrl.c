...
INT32 wmt_ctrl_crystal_triming_get(P_WMT_CTRL_DATA pWmtCtrlData)
{
	...
	if (wmt_dev_patch_get(pFileName, &pNvram, 0) == 0) {
...
INT32 wmt_ctrl_get_patch(P_WMT_CTRL_DATA pWmtCtrlData)
{
	...
	if ((pFullPatchName != NULL)
	    && (wmt_dev_patch_get(pFullPatchName, &pPatch, BCNT_PATCH_BUF_HEADROOM) == 0)) {
	...
	if ((pDefPatchName != NULL)
	    && (wmt_dev_patch_get(pDefPatchName, &pPatch, BCNT_PATCH_BUF_HEADROOM) == 0)) {
...
static INT32 wmt_ctrl_gps_sync_set(P_WMT_CTRL_DATA pData)
{
	...
	iret = wmt_plat_gpio_ctrl(PIN_GPS_SYNC, (pData->au4CtrlData[0] == 0) ? PIN_STA_DEINIT : PIN_STA_MUX);
...
static INT32 wmt_ctrl_gps_lna_set(P_WMT_CTRL_DATA pData)
{
	...
	iret = wmt_plat_gpio_ctrl(PIN_GPS_LNA, (pData->au4CtrlData[0] == 0) ? PIN_STA_DEINIT : PIN_STA_OUT_H);
