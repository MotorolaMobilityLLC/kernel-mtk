...
#endif
static void stp_dbg_nl_send_retry(INT32 rc, PINT8 p, INT32 len)
{
	INT32 nl_retry = 0, retryc = rc;
	...
	while (retryc) {
		...
		osal_sleep_ms(3);
		retryc = stp_dbg_nl_send(p, 2, len);
		if (!retryc)
		...
		if (buf_len > 0) {
			...
			if (hdr->dbg_type == STP_DBG_FW_DMP) {
				...
				if (len <= 1500) {
					...
					stp_dbg_nl_send_retry(rc, (PINT8)&tmp, len+5);
					/* schedule(); */
				} else {
...
static INT32 _stp_btm_handler(MTKSTP_BTM_T *stp_btm, P_STP_BTM_OP pStpOp)
{
	...
	case STP_OPID_BTM_PAGED_DUMP:
		...
		do {
			...
			if (page_counter == 0) {	/* do fw assert infor paser in first paged dump */
