...
{
	MTK_WCN_BOOL bRet = (g_es_lr_flag_for_quick_sleep == 0) ? MTK_WCN_BOOL_FALSE : MTK_WCN_BOOL_TRUE;
...
INT32 wmt_dbg_psm_ctrl(INT32 par1, INT32 par2, INT32 par3)
{
	...
	if (par2 == 0) {
		...
		par2 = (par2 < 1 || par2 > 20000) ? STP_PSM_IDLE_TIME_SLEEP : par2;
...
{
	if (par2 >= WMTDSNS_FM_DISABLE && par2 < WMTDSNS_MAX) {
...
INT32 wmt_dbg_cmd_test_api(ENUM_WMTDRV_CMD_T cmd)
{
	...
	case WMTDRV_CMD_FWTRACE_TEST:
		...
		break;
	default:
		if (cmd >= WMTDRV_CMD_COEXDBG_00 && cmd <= WMTDRV_CMD_COEXDBG_15) {
...
{
	par2 = (par2 >= WMT_LOG_ERR && par2 <= WMT_LOG_LOUD) ? par2 : WMT_LOG_INFO;
...
static ssize_t wmt_dev_dbg_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if (osal_array_size(wmt_dev_dbg_func) > x && wmt_dev_dbg_func[x] != NULL)
...
INT32 wmt_dev_read_file(PUINT8 pName, const PPUINT8 ppBufPtr, INT32 offset, INT32 padSzBuf)
{
	...
	*ppBufPtr = NULL;
	...
	do {
		...
		/*read_len = fd->f_op->read(fd, p + padSzBuf, file_len, &fd->f_pos);*/
