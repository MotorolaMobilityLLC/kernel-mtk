...
INT32 stp_dbg_set_fw_info(UINT8 *issue_info, UINT32 len, ENUM_STP_FW_ISSUE_TYPE issue_type)
{
	...
	if ((issue_type == STP_FW_ASSERT_ISSUE) ||
	    (issue_type == STP_HOST_TRIGGER_FW_ASSERT) || (issue_type == STP_HOST_TRIGGER_ASSERT_TIMEOUT)) {
		...
		if ((issue_type == STP_HOST_TRIGGER_FW_ASSERT) || (issue_type == STP_HOST_TRIGGER_ASSERT_TIMEOUT)) {
			...
			case 4:
				...
				if ((g_stp_dbg_cpupcr->host_assert_info.reason == 38) ||
...
INT32 stp_dbg_cpupcr_infor_format(PPUINT8 buf, PUINT32 str_len)
{
	...
	if ((g_stp_dbg_cpupcr->issue_type == STP_FW_NOACK_ISSUE) ||
		...
		len +=
		    osal_sprintf(*buf + len, "<client>\n\t\t\t<task>%s</task>\n\t\t\t",
				 _stp_dbg_id_to_task(g_stp_dbg_cpupcr->fwTaskId));
		if (g_stp_dbg_cpupcr->host_assert_info.reason == 32 || g_stp_dbg_cpupcr->host_assert_info.reason == 33
