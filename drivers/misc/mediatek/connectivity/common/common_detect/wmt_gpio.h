...
struct GPIO_CTRL_STATE {
	INT32 gpio_num;
	struct pinctrl_state *gpio_state[GPIO_STATE_MAX];
};

struct WMT_GPIO_STATE_INFO {
	INT32 gpio_num;
	INT32 mode;
	INT32 pull_sel;
	INT32 in;
...
struct GPIO_CTRL_INFO {
	...
	struct GPIO_CTRL_STATE gpio_ctrl_state[GPIO_PIN_ID_MAX];
};
...
extern struct GPIO_CTRL_INFO gpio_ctrl_info;
