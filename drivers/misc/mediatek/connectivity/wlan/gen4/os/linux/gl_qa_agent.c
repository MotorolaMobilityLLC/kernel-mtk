	...
	if (prGlueInfo->prAdapter->fgIsSupportQAAccessEfuse == TRUE) {
		...
		if (Len == 2) {
			...
			if (!g_BufferDownload) {
				...
				P_GLUE_INFO_T prGlueInfo = NULL;
				PARAM_CUSTOM_EFUSE_BUFFER_MODE_T rSetEfuseBufModeInfo;
				PARAM_CUSTOM_EFUSE_BUFFER_MODE_1_T rSetEfuseBufModeInfo_1;

				prGlueInfo = *((P_GLUE_INFO_T *) netdev_priv(prNetDev));
				kalMemZero(&rSetEfuseBufModeInfo, sizeof(PARAM_CUSTOM_EFUSE_BUFFER_MODE_T));
				kalMemZero(&rSetEfuseBufModeInfo_1, sizeof(PARAM_CUSTOM_EFUSE_BUFFER_MODE_1_T));
				  ...
				  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	*/
	...
	MT_ATESetDBDCBandIndex(prNetDev, u4Band_idx);
	if ((u4Central_ch0 >= 7 && u4Central_ch0 <= 12) && u4Ch_band == 1) {
