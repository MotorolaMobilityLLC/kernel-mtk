...
VOID dumpMemory8IEOneLine(IN PUINT_8 pString, IN PUINT_8 pucStartAddr, IN UINT_32 u4Length)
{
	...
	if (pString == NULL)
		offsetMsg += kalSnprintf(
			pucMsg + offsetMsg, BUF_MSG_LENGTH - offsetMsg, "Len:%u:[", u4Length);
	else
		offsetMsg += kalSnprintf(
			pucMsg + offsetMsg, BUF_MSG_LENGTH - offsetMsg, "[%s],Len:%u:[", pString, u4Length);
