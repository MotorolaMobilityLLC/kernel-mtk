...
#define STP_DBG_LOUD_FUNC(fmt, arg...) pr_debug(PFX_STP_DBG "%s: " fmt, __func__ , ##arg)

#define STP_DBG_DBG_FUNC(fmt, arg...) pr_debug(PFX_STP_DBG "%s: " fmt, __func__ , ##arg)

#define STP_DBG_INFO_FUNC(fmt, arg...) pr_warn(PFX_STP_DBG fmt, ##arg)

#define STP_DBG_WARN_FUNC(fmt, arg...) pr_err(PFX_STP_DBG "%s: "  fmt, __func__ , ##arg)

#define STP_DBG_ERR_FUNC(fmt, arg...) pr_err(PFX_STP_DBG "%s: "   fmt, __func__ , ##arg)

#define STP_DBG_TRC_FUNC(f) pr_debug(PFX_STP_DBG "<%s> <%d>\n", __func__, __LINE__)
...
static struct genl_ops stp_dbg_gnl_ops[] = {
	{
				 ...
				 PINT32 out_sz, INT32 finish)
{
	...
	STP_DBG_DBG_FUNC("out buf 0x%p, out sz %d\n", out_buf, *out_sz);
...
static INT32 _stp_dbg_dmp_in(MTKSTP_DBG_T *stp_dbg, PINT8 buf, INT32 len)
{
	...
	if (internalFlag) {
		...
		if (0 != gStpDbgLogOut) {
			...
			pHdr =
			    (STP_DBG_HDR_T *) &(stp_dbg->logsys->
						 queue[stp_dbg->logsys->in].buffer[0]);
			pBuf =
...
static INT32 stp_dbg_fill_hdr(struct stp_dbg_pkt_hdr *hdr, INT32 type, INT32 ack, INT32 seq,
...
static VOID stp_dbg_nl_init(VOID)
{
	if (genl_register_family(&stp_dbg_gnl_family) != 0) {
		...
		if (genl_register_family_with_ops(&stp_dbg_gnl_family, stp_dbg_gnl_ops) != 0)
			STP_DBG_ERR_FUNC("%s(): operation registration fail\n", __func__);
	}
}
