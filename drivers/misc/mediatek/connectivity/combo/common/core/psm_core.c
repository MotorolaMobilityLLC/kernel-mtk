...
static inline INT32 _stp_psm_notify_stp(MTKSTP_PSM_T *stp_psm, const MTKSTP_PSM_ACTION_T action)
{
	...
	case ACT_INACT:
		if (action == SLEEP) {
			...
			STP_PSM_ERR_FUNC
			    ("Action = %d, ACT_INACT state, the case should not happens\n\r",
			     action);
			STP_PSM_ERR_FUNC("state = %d, flag = %ld\n", stp_psm->work_state,
					 stp_psm->flag.data);
	...
	case INACT_ACT:
		if (action == WAKEUP) {
			...
			STP_PSM_ERR_FUNC
			    ("Action = %d, INACT_ACT state, the case should not happens\n\r",
			     action);
			STP_PSM_ERR_FUNC("state = %d, flag = %ld\n", stp_psm->work_state,
					 stp_psm->flag.data);
			retval = STP_PSM_OPERATION_FAIL;
	...
	case INACT:
		STP_PSM_ERR_FUNC
		    ("Action = %d, INACT state, the case should not happens\n\r", action);
		STP_PSM_ERR_FUNC("state = %d, flag = %ld\n", stp_psm->work_state,
				 stp_psm->flag.data);
		retval = -1;
		break;
	case ACT:
		STP_PSM_ERR_FUNC("Action = %d, ACT state, the case should not happens\n\r", action);
		STP_PSM_ERR_FUNC("state = %d, flag = %ld\n", stp_psm->work_state,
				 stp_psm->flag.data);
		retval = STP_PSM_OPERATION_FAIL;
		break;
	default:
		/*invalid */
		STP_PSM_ERR_FUNC
		    ("Action = %d, Invalid state, the case should not happens\n\r", action);
		STP_PSM_ERR_FUNC("state = %d, flag = %ld\n", stp_psm->work_state,
				 stp_psm->flag.data);
		retval = STP_PSM_OPERATION_FAIL;
		break;
