...
#define PWM_NOTICE(fmt, arg...) pr_notice("[PWM] " fmt "\n", ##arg)
...
int disp_pwm_is_enabled(disp_pwm_id_t id)
{
	...
	int pwm_is_enabled = DISP_REG_GET(reg_base + DISP_PWM_EN_OFF) & 0x1;
...
{
	int ret = disp_pwm_set_max_backlight(disp_pwm_get_main(), level_1024);
	return ret;
}
...
{
	int ret = disp_pwm_set_backlight(disp_pwm_get_main(), level_1024);
	return ret;
}
