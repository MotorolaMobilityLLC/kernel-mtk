...
static int gyro_recv_interrupt_ipidata(void *data, unsigned int len)
{
	struct gyrohub_ipi_data *obj = obj_ipi_data;
	SCP_SENSOR_HUB_DATA_P rsp = (SCP_SENSOR_HUB_DATA_P)data;

	GYROS_ERR("len = %d, type = %d, action = %d, errCode = %d\n", len, rsp->rsp.sensorType, rsp->rsp.action,
		rsp->rsp.errCode);
	...
	case SENSOR_HUB_NOTIFY:
		...
		case SCP_NOTIFY:
			/* need add interrupt mode recv data later */
			break;
		default:
			GYROS_ERR("Error sensor hub notify");
			break;
		}
		break;
	default:
		GYROS_ERR("Error sensor hub action");
		break;
	}
...
static int gyrohub_probe(struct platform_device *pdev)
{
	...
	if (err < 0) {
		GYROS_ERR("SCP_sensorHub_rsp_registration failed\n");
		goto exit_kfree;
	}
