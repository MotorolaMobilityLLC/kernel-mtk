...
static int bmg_set_powermode(struct bmg_gyro_data *obj,
		enum BMG_POWERMODE_ENUM power_mode);
...
static int bmg_read_raw_data(struct bmg_gyro_data *obj, s16 data[BMG_AXES_NUM])
{
	...
	if (priv->sensor_type == BMI160_GYRO_TYPE) {
		...
		err = BMP160_spi_read_bytes(NULL, BMI160_USER_DATA_8_GYR_X_LSB__REG,
				buf_tmp, 6);
		if (err) {
...
static int bmg_get_hw_offset(struct bmg_gyro_data *obj,
...
static int bmg_set_hw_offset(struct bmg_gyro_data *obj,
		s8 offset[BMG_AXES_NUM + 1])
{
	int err = 0;
	/* HW calibration is under construction */
...
static int bmg_write_calibration(struct bmg_gyro_data *obj,
		...
		enum BMG_POWERMODE_ENUM power_mode)
{
	...
	if (obj->sensor_type == BMI160_GYRO_TYPE) {
		...
		err += BMP160_spi_write_bytes(NULL,
...
static int bmg_set_datarate(struct bmg_gyro_data *obj,
		int datarate)
{
	int err = 0;
	u8 data = 0;
	...
	mutex_lock(&obj->lock);
	if (obj->sensor_type == BMI160_GYRO_TYPE) {
		err = BMP160_spi_read_bytes(NULL,
...
*/
static int bmg_read_sensor_data(struct bmg_gyro_data *obj,
