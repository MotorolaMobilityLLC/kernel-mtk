...
void ioremap_debug_hook_func(phys_addr_t phys_addr, size_t size, pgprot_t prot)
{
	...
	if ((phys_addr + size <= target) || (phys_addr >= (target + PAGE_SIZE)))
		return;
	...
	pr_debug("ioremap to 0x1001AXXX more than once (%pa, %zx)\n",
		 &phys_addr, size);
	BUG();
}
...
int arm_undefinstr_retry(struct pt_regs *regs, unsigned int instr)
{
	...
	if ((user_mode(regs)) || processor_mode(regs) == PSR_MODE_EL1h) {
		...
		int *prev_undefinstr_counter =
		    &get_cpu_var(__prev_undefinstr_counter);

		pr_debug("USR_MODE / SVC_MODE Undefined Instruction Address "
			 "curr:%p pc=%p:%p, instr: 0x%x compat: %s\n",
			 (void *)current, (void *)pc,
			 (void *)*prev_undefinstr_pc, instr,
			 is_compat_task() ? "yes" : "no");
		if (*prev_undefinstr_pc != pc) {
			/*
			 * If the current process or program counter is
			 * changed...
			 * renew the counter.
			 */
			pr_debug("First Time Recovery curr:%p pc=%p:%p\n",
				 (void *)current, (void *)pc,
			...
			*prev_undefinstr_counter = 0;
			...
			pr_debug("%dth Time Recovery curr:%p pc=%p:%p\n",
				 *prev_undefinstr_counter + 1, (void *)current,
				 (void *)pc, (void *)*prev_undefinstr_pc);
		...
		*prev_undefinstr_counter += 1;
		...
		put_cpu_var(__prev_undefinstr_pc);
		put_cpu_var(__prev_undefinstr_counter);
		pr_debug("Go to ARM Notify Die curr:%p pc=%p:%p\n",
			 (void *)current, (void *)pc,
			 (void *)*prev_undefinstr_pc);
	}
	return 1;
