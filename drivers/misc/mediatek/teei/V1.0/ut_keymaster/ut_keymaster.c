...
static long keymaster_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case CMD_MEM_CLEAR:
			...
			break;
		case CMD_MEM_SEND:
			//IMSG_DEBUG("microturst ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
			//IMSG_DEBUG("microtrust kernel keymaster CMD_MEM_SEND\n");
			/*
			fp_cid = virt_to_phys(&fp_fid);
			IMSG_DEBUG("chx share mem: fp_fid=%d,fp_cid = %d, len=%d\n",fp_fid, fp_cid,sizeof(unsigned int));
			...
			*/
			...
			//IMSG_DEBUG("microtrust keymaster ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~end\n");
			break;
		case CMD_NOTIFY_UTD:
		...
		case CMD_FIRST_TIME_BOOT:
			//IMSG_DEBUG("teei daemon tell the keymaster this is first time boot start\n");
			...
			//IMSG_DEBUG("teei daemon tell the keymaster this is first time boot end\n");
			break;
		default:
			up(&keymaster_api_lock);
			return -EINVAL;
