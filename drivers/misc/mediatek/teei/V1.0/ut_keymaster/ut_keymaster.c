...
static long keymaster_ioctl(struct file *filp, unsigned int cmd,
			    unsigned long arg)
{
	...
	case CMD_MEM_SEND:
		if (!keymaster_buff_addr) {
			IMSG_ERROR(
				"microtrust keymaster fp_buiff_addr is invalid!.\n");
		...
		if (copy_from_user((void *)keymaster_buff_addr, (void *)arg,
			...
			IMSG_ERROR(
				KERN_ERR
				"microturst keymaster copy from user failed.\n");
			up(&keymaster_api_lock);
			return -EFAULT;
		}
		...
		if (copy_to_user((void *)arg, (void *)keymaster_buff_addr,
				 KEYMASTER_SIZE)) {
			IMSG_ERROR(
				"microtrust keymaster copy from user failed.\n");
			up(&keymaster_api_lock);
			return -EFAULT;
		}
...
static ssize_t keymaster_read(struct file *filp, char __user *buf, size_t size,
			      loff_t *ppos)
{
	int ret = 0;
	return ret;
