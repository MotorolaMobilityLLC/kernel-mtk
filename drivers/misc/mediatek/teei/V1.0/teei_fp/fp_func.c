...
int fp_open(struct inode *inode, struct file *filp)
{
	...
	if (ret <= 0) {
		IMSG_INFO(
			"[TEE] error, tee load not finished yet,and wait timeout\n");
		return -1;
	}
...
static long fp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case CMD_MEM_CLEAR:
		...
		break;
	case CMD_FP_CMD:
		if (copy_from_user((void *)args, (void *)arg,
				   FP_BUFFER_OFFSET)) {
...
#ifdef FP_DEBUG
		IMSG_DEBUG(
			"invoke fp cmd CMD_FP_CMD: arg's address is %x, args's length %d\n",
		...
		IMSG_DEBUG("[%s][%d] fp_buff_addr 88 - 91 = %d\n", __func__,
			   args_len, *((unsigned int *)(fp_buff_addr + 88)));
#endif
...
#endif
		if (copy_from_user((void *)gatekeeper_buff_addr, (void *)arg,
				   0x1000)) {
			IMSG_ERROR("copy from user failed.\n");
			up(&fp_api_lock);
			return -EFAULT;
...
#endif
		if (copy_to_user((void *)arg, (void *)gatekeeper_buff_addr,
				 0x1000)) {
			IMSG_ERROR("copy from user failed.\n");
			up(&fp_api_lock);
			return -EFAULT;
...
static ssize_t fp_write(struct file *filp, const char __user *buf, size_t size,
			loff_t *ppos)
{
	return 0;
}
