        ...
        unsigned long arg)
{
    ...
    case UT_TUI_NOTICE_COMMAND_U32:
		...
		if (1 !=lockNum && 2 != lockNum) {
			pr_err("[%s][%d] No channel can be used!\n", __func__, __LINE__);
			retVal = -EINVAL;
			return retVal;
        }
		...
		break;
    default:
		pr_err("[%s][%d] command(%x) not found!\n", __func__, __LINE__,cmd);
		retVal = -EINVAL;
		return retVal;
    }
    ...
    if (copy_from_user((void*)message_buff + sizeof(data.datalen), pt, data.datalen)) {
		pr_err("copy buffer from user failed. \n");
		return -EFAULT;
    }
...
static const struct file_operations ut_tui_fops =
{
	...
	.unlocked_ioctl = tz_ut_tui_u64_ioctl,
	.compat_ioctl = tz_ut_tui_u32_ioctl,
	.open = tz_ut_tui_open,
	.mmap = tz_ut_tui_map,
...
int ut_tui_init(void)
{
    ...
    if (result < 0) {
        pr_err("Err: failed in alloc dev %x\n", result);
        return result;
    }
    ...
    if (IS_ERR(driver_class)) {
        result = -ENOMEM;
        pr_err("Err: failed in creating class %x.\n", result);
    ...
    if (!class_dev) {
        result = -ENOMEM;
        pr_err("Err: failed in creating device %x.\n", result);
        goto class_destroy;
    }
    ...
    if (add_cdev_result < 0) {
        result = -ENOMEM;
        pr_err("Err: failed add char_reg_setup_cdev %x\n", add_cdev_result);
