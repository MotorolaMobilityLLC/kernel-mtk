...
{
	free_pages(ut_tui_client_buf, get_order(ROUND_UP(ut_tui_client_buf_len, SZ_4K)));
	ut_tui_client_buf = 0;
...
#ifdef UT_DMA_ZONE
	ut_tui_client_buf =  (unsigned long) __get_free_pages(GFP_KERNEL | GFP_DMA, get_order(ROUND_UP(ut_tui_client_buf_len, SZ_4K)));
#else
	ut_tui_client_buf =  (unsigned long) __get_free_pages(GFP_KERNEL, get_order(ROUND_UP(ut_tui_client_buf_len, SZ_4K)));
#endif
	retVal = remap_pfn_range(vma, vma->vm_start, ((virt_to_phys((void *)ut_tui_client_buf)) >> PAGE_SHIFT),
