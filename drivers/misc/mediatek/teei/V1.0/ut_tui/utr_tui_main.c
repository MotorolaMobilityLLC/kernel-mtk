...
extern int get_primary_state(void);

static dev_t devno;
struct ut_tui_dev *ut_tui_devp = NULL;
        ...
        unsigned long arg)
{
	...
	if(get_primary_state() == disp_sleeped){
		pr_debug("[%s][%d] disp_sleeped!\n", __func__, __LINE__);
        return -EIO;
	}
    switch (cmd)
    {
    ...
    case UT_TUI_NOTICE_COMMAND_U32:
    {
       ...
       if(1 !=lockNum && 2 != lockNum){
         pr_debug("[%s][%d] No channel can be used!\n", __func__, __LINE__);
         retVal = -EINVAL;
         return retVal;
        }
    ...
    break;
    default:
    pr_debug("[%s][%d] command(%x) not found!\n", __func__, __LINE__,cmd);
    retVal = -EINVAL;
    return retVal;
    }
	...
	pr_debug(KERN_INFO "copy data from user failed. \n");
	return -EFAULT;
    }
	...
	pr_debug(KERN_INFO "copy buffer from user failed. \n");
	return -EFAULT;
    }
    ...
    if (copy_to_user(argp,(void*)message_buff, sizeof(data.datalen))){
        pr_debug(KERN_INFO "copy datalen to user failed. \n");
	return -EFAULT;
    }

    if(copy_to_user(pt, (void*)message_buff + sizeof(data.datalen), *((uint32_t*)message_buff))){
        pr_debug(KERN_INFO "copy data to user failed. \n");
	return -EFAULT;
    }
...
int ut_tui_init(void)
{
    ...
    if (result < 0)
    {
        pr_debug("Err: failed in alloc dev %x\n", result);
        return result;
    }
    ...
    {
        result = -ENOMEM;
        pr_debug("Err: failed in creating class %x.\n", result);
        goto unregister_chrdev_region;
    }

    class_dev = device_create(driver_class, NULL, devno, NULL,
    UT_TUI_CLIENT_DEV);
    if (!class_dev)
    {
        result = -ENOMEM;
        pr_debug("Err: failed in creating device %x.\n", result);
        goto class_destroy;
    }
    ...
    {
        result = -ENOMEM;
        pr_debug("Err: failed add char_reg_setup_cdev %x\n", add_cdev_result);
    ...
    ut_tui_devp_destroy: kfree(ut_tui_devp);
    device_destroy: device_destroy(driver_class, devno);
