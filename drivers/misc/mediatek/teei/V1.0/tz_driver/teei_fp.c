...
unsigned long create_fp_fdrv(int buff_size)
{
	...
	if (buff_size > VDRV_MAX_SIZE) {
		IMSG_ERROR(
			"[%s][%d]: FP Drv buffer is too large, Can NOT create it.\n",
			__FILE__, __LINE__);
		return (unsigned long)NULL;
	}
...
int send_fp_command(unsigned long share_memory_size)
{
	...
	Invalidate_Dcache_By_Area((unsigned long)fp_buff_addr,
				  (unsigned long)fp_buff_addr + FP_BUFF_SIZE);
