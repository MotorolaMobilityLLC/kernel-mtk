...
irqreturn_t tlog_handler(void)
{
	...
	if (-1 != pos) {
		...
		memcpy(tlog_ent[pos].context, (char *)tlog_message_buff,
		       TLOG_CONTEXT_LEN);
...
long init_tlog_buff_head(unsigned long tlog_virt_addr, unsigned long buff_size)
{
	...
	Flush_Dcache_By_Area((unsigned long)tlog_virt_addr,
			     (unsigned long)tlog_virt_addr + buff_size);
...
int tlog_print(unsigned long log_start)
{
	...
	/*tlog_pos = (tlog_pos + sizeof(struct ut_log_entry)) % (((struct
	 *ut_log_buf_head *)tlog_buf)->length
	 *- sizeof(struct ut_log_buf_head));
	 */
	tlog_pos = (tlog_pos + sizeof(struct ut_log_entry))
		   % (LOG_BUF_LEN - sizeof(struct ut_log_buf_head));
...
{
	int retVal = 0;
	unsigned long shared_buff_write_pos =
		((struct ut_log_buf_head *)tlog_buf)->write_pos;
	unsigned long tlog_cont_pos =
		(unsigned long)tlog_buf + sizeof(struct ut_log_buf_head);
	unsigned long last_log_pointer = tlog_cont_pos + shared_buff_write_pos;
	unsigned long start_log_pointer = tlog_cont_pos + tlog_pos;

	if ((shared_buff_write_pos < 0)
	    || (shared_buff_write_pos
		>= (LOG_BUF_LEN - sizeof(struct ut_log_buf_head)))) {
	...
	while (last_log_pointer != start_log_pointer) {
		...
		if (retVal != 0) {
			IMSG_ERROR(
				"[%s][%d]fail to print tlog last_log_pointer = %lx, start_log_pointer = %lx!\n",
				__func__, __LINE__, last_log_pointer,
				start_log_pointer);
...
int tlog_worker(void *p)
{
	...
	while (!kthread_should_stop()) {
		if (((struct ut_log_buf_head *)tlog_buf)->write_pos
		    == tlog_pos) {
 ...
 **************************************************/
long init_utgate_log_buff_head(unsigned long log_virt_addr,
			       unsigned long buff_size)
{
	...
	Flush_Dcache_By_Area((unsigned long)log_virt_addr,
			     (unsigned long)log_virt_addr + buff_size);
...
int utgate_log_print(unsigned long log_start)
{
	...
	utgate_log_pos = (utgate_log_pos + 1)
			 % (((struct utgate_log_head *)utgate_log_buff)->length
...
{
	unsigned long utgate_log_cont_pos =
		(unsigned long)utgate_log_buff + sizeof(struct utgate_log_head);
	unsigned long utgate_last_log_pos =
		utgate_log_cont_pos
		+ ((struct utgate_log_head *)utgate_log_buff)->write_pos;
	unsigned long utgate_start_log_pos =
		utgate_log_cont_pos + utgate_log_pos;
	int retVal = 0;
...
int utgate_log_worker(void *p)
{
	...
	while (!kthread_should_stop()) {
		Invalidate_Dcache_By_Area(utgate_log_buff,
					  utgate_log_buff
						  + MESSAGE_LENGTH * 128);

		if (((struct utgate_log_head *)utgate_log_buff)->write_pos
		    == utgate_log_pos) {
			schedule_timeout_interruptible(1 * HZ);
			continue;
		}
		...
		default:
			IMSG_ERROR("[%s][%d] utgate tlog VERSION is wrong !\n",
				   __func__, __LINE__);
			utgate_log_pos =
				((struct utgate_log_head *)utgate_log_buff)
					->write_pos;
			ret = -EFAULT;
		}
	}
			      ...
			      unsigned long buff_size)
{
	...
	utgate_log_thread =
		kthread_create(utgate_log_worker, NULL, "utgate_tlog");
