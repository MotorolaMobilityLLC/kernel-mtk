...
static void tz_driver_dump_logs(struct tz_log_state *s)
{
	...
	while ((put = log->put) != get) {
		 ...
		 * It can prevent from printing message to uart by using KERN_DEBUG
		 * log level if with default printk setting.
		 * (default setting is print message to uart if log level >= KERN_INFO)
		 */
		...
		measure_log_perf(IMSG_TAG, read_chars, *(uint32_t *)(s->line_buffer));
#endif
	}
	s->get = get;
}
...
#if 0
static const struct of_device_id tz_log_of_match[] = {
...
static struct platform_driver tz_log_driver = {
	...
	.driver = {
		.name = "tz-log",
		.owner = THIS_MODULE,
		.of_match_table = tz_log_of_match,
	},
};
