...
static void tz_driver_dump_logs(struct tz_log_state *s)
{
	...
	while ((put = log->put) != get) {
		/*
		 * Make sure that the read of put
		 * occurs before the read of log data
		 */
		rmb();
		 ...
		 * It can prevent from printing message to uart
		 * by using KERN_DEBUG
		 * log level if with default printk setting.
		...
		/*
		 * Dump early log to boot log buffer until
		 * boot log buffer is full
		 */
		...
		/*
		 * Print warning message if log output frequency
		 * is over rate limit
		 */
		__ratelimit(&_rs);
...
int tz_log_probe(struct platform_device *pdev)
{
	...
	s->boot_log->put = 0;
	s->boot_log->sz =
		rounddown_pow_of_two(TZ_LOG_SIZE - sizeof(struct boot_log_rb));
...
/*TODO: support auto probe by device tree */
