...
static long register_shared_param_buf(struct service_handler *handler)
{
	...
	if (handler->size > VDRV_MAX_SIZE) {
		IMSG_ERROR(
			"[%s][%d]: The vDrv buffer is too large, DO NOT Allow to create it.\n",
			__FILE__, __LINE__);
		return -EINVAL;
	}

#ifdef UT_DMA_ZONE
	handler->param_buf = (void *)__get_free_pages(
		GFP_KERNEL | GFP_DMA,
		get_order(ROUND_UP(handler->size, SZ_4K)));
#else
	handler->param_buf = (void *)__get_free_pages(
		GFP_KERNEL, get_order(ROUND_UP(handler->size, SZ_4K)));
#endif
...
int vfs_thread_function(unsigned long virt_addr, unsigned long para_vaddr,
			unsigned long buff_vaddr)
{
...
static int vfs_handle(struct service_handler *handler)
{
	...
	vfs_thread_function((unsigned long)(handler->param_buf), para_vaddr,
			    buff_vaddr);
	...
	Flush_Dcache_By_Area((unsigned long)vfs_bdrv_ent,
			     (unsigned long)vfs_bdrv_ent
				     + sizeof(struct bdrv_call_struct));
