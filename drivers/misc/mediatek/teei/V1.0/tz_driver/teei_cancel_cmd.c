...
int __send_cancel_command(unsigned long share_memory_size)
{
	...
	Flush_Dcache_By_Area((unsigned long)cancel_message_buff, cancel_message_buff + CANCEL_MESSAGE_SIZE);
...
static void secondary_send_cancel_command(void *info)
{
	struct cancel_command_struct *cd = (struct cancel_command_struct *)info;

	/* with a rmb() */
	rmb();

	cd->retVal = __send_cancel_command(cd->mem_size);
...
int send_cancel_command(unsigned long share_memory_size)
{
...
#if 0
	cancel_command_entry.mem_size = share_memory_size;
	dd
#else
	...
	Invalidate_Dcache_By_Area((unsigned long)cancel_message_buff, cancel_message_buff + CANCEL_MESSAGE_SIZE);
	rmb();
...
#if 0
	return cancel_command_entry.retVal;
#else
