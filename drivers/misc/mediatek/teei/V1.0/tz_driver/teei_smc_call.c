...
static u32 teei_smc(u32 cmd_addr, int size, int valid_flag)
{
	...
	set_sch_nq_cmd();
	Flush_Dcache_By_Area((unsigned long)t_nt_buffer,
			     (unsigned long)t_nt_buffer + 0x1000);
 ...
 * @return
 */
int __teei_smc_call(unsigned long local_smc_cmd, u32 teei_cmd_type,
		    u32 dev_file_id, u32 svc_id, u32 cmd_id, u32 context,
		    u32 enc_id, const void *cmd_buf, size_t cmd_len,
		    void *resp_buf, size_t resp_len, const void *meta_data,
		    const void *info_data, size_t info_len, int *ret_resp_len,
		    int *error_code, struct semaphore *psema)
{
	u32 ret = 50;
	void *smc_cmd_phys = 0;
	...
	ret = teei_smc((u32)(unsigned long)smc_cmd_phys,
...
int teei_smc_call(u32 teei_cmd_type, u32 dev_file_id, u32 svc_id, u32 cmd_id,
		  u32 context, u32 enc_id, void *cmd_buf, size_t cmd_len,
		  void *resp_buf, size_t resp_len, void *meta_data,
		  void *info_data, size_t info_len, int *ret_resp_len,
		  int *error_code, struct semaphore *psema)
{
	int retVal = 0;

	struct teei_smc_cmd *local_smc_cmd =
		(struct teei_smc_cmd *)tz_malloc_shared_mem(
	...
	Flush_Dcache_By_Area((unsigned long)&smc_call_entry,
			     (unsigned long)&smc_call_entry
				     + sizeof(smc_call_entry));
