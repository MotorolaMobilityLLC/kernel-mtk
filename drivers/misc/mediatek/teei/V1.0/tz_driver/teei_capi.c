...
int teei_client_context_init(void *private_data, void *argp)
{
	...
	if (dev_found) {
		...
		retVal = teei_smc_call(TEEI_CMD_TYPE_INITIALIZE_CONTEXT,
				       dev_file_id, 0, 0, 0, 0, name, 255,
				       resp_flag, 4, NULL, NULL, 0, NULL,
				       &error_code, &(temp_cont->cont_lock));
	}
...
int teei_client_context_close(void *private_data, void *argp)
{
	...
	if (dev_found) {
		strncpy(temp_cont->tee_name, ctx.name, TEE_NAME_SIZE);
		retVal = teei_smc_call(TEEI_CMD_TYPE_FINALIZE_CONTEXT,
				       dev_file_id, 0, 0, 0, 0, NULL, 0,
				       resp_flag, 4, NULL, NULL, 0, NULL,
				       &error_code, &(temp_cont->cont_lock));
	}
...
int teei_client_session_init(void *private_data, void *argp)
{
	...
	ses_new = (struct teei_session *)tz_malloc(sizeof(struct teei_session),
						   GFP_KERNEL);
 ...
 *
 * @return		0: success
 * EFAULT: copy data from user space OR copy data back to
 *the user space error EINVLA: can not find the context OR session structure.
 */
int teei_client_session_open(void *private_data, void *argp)
{
	...
	ses_open = (struct ser_ses_id *)tz_malloc_shared_mem(
		sizeof(struct ser_ses_id), GFP_KERNEL);

	if (ses_open == NULL) {
		IMSG_ERROR(
			"[%s][%d] ========== ses_open is NULL ============\n",
			__func__, __LINE__);
		return -EFAULT;
	}
	...
	if (enc_found) {
		...
		retVal = teei_smc_call(
			TEEI_CMD_TYPE_OPEN_SESSION, dev_file_id, 0, 0, 0, 0,
			enc_temp->ker_req_data_addr, enc_temp->enc_req_offset,
			enc_temp->ker_res_data_addr, enc_temp->enc_res_offset,
			enc_temp->meta, ses_open, sizeof(struct ser_ses_id),
			NULL, NULL, &(temp_cont->cont_lock));

	} else {
		/* This session didn't have been encoded */
		retVal = teei_smc_call(TEEI_CMD_TYPE_OPEN_SESSION, dev_file_id,
				       0, 0, 0, 0, NULL, 0, NULL, 0, NULL,
				       ses_open, sizeof(struct ser_ses_id),
				       NULL, NULL, &(temp_cont->cont_lock));
	}
...
int teei_client_session_close(void *private_data, void *argp)
{
	...
	list_for_each_entry(temp_cont, &teei_contexts_head.context_list, link) {
		if (temp_cont->cont_id == dev_file_id) {
			list_for_each_entry(temp_ses, &temp_cont->sess_link,
				link) {
 ...
 * @return		EINVAL: Invalid parament
 *                      EFAULT: copy data from user space OR copy data back to
 * the user space error 0: success
 */
int teei_client_send_cmd(void *private_data, void *argp)
{
	...
	if (enc.encode_id != -1) {
		...
		retVal = teei_client_prepare_encode(private_data, &enc,
						    &enc_temp, &temp_ses);
	...
	retVal = teei_smc_call(
		TEEI_CMD_TYPE_INVOKE_COMMAND, dev_file_id, 0, enc.cmd_id,
		enc.session_id, enc.encode_id, enc_temp->ker_req_data_addr,
		enc_temp->enc_req_offset, enc_temp->ker_res_data_addr,
		enc_temp->enc_res_offset, enc_temp->meta, return_Origin, 4,
		&enc.return_value, NULL, &(temp_cont->cont_lock));
...
int teei_client_encode_uint32(void *private_data, void *argp)
{
	...
	retVal = teei_client_prepare_encode(private_data, &enc, &enc_context,
					    &session);
	...
	if (enc.param_type == TEEIC_PARAM_IN) {
		if (enc_context->ker_req_data_addr == NULL) {
			...
			if (enc_context->ker_req_data_addr == NULL) {
				IMSG_ERROR(
					"[%s][%d] enc_context->ker_req_data_addr is NULL!\n",
					__func__, __LINE__);
				retVal = -ENOMEM;
				goto ret_encode_u32;
			}
		...
		if ((enc_context->enc_req_offset + sizeof(u32) <= TEEI_1K_SIZE)
			...
			*(u32 *)((char *)enc_context->ker_req_data_addr
			...
			enc_context->meta[enc_context->enc_req_pos].len =
				sizeof(u32);
			enc_context->meta[enc_context->enc_req_pos].value_flag =
				enc.value_flag;
			enc_context->meta[enc_context->enc_req_pos].param_pos =
				enc.param_pos;
		...
		if (!enc_context->ker_res_data_addr) {
			...
			if (!enc_context->ker_res_data_addr) {
				IMSG_ERROR(
					"[%s][%d] enc_context->ker_res_data_addr is NULL\n",
					__func__, __LINE__);
				retVal = -ENOMEM;
				goto ret_encode_u32;
			}
		...
		if ((enc_context->enc_res_offset + sizeof(u32) <= TEEI_1K_SIZE)
			...
			< (TEEI_MAX_RES_PARAMS + TEEI_MAX_REQ_PARAMS))) {
			if ((unsigned char *)((unsigned long)enc.data)
			    != NULL) {
				/*
				 *u64 addr = enc.data;
				 *void __user *pt = compat_ptr((unsigned int
				 **)addr); u32 value = 0; copy_from_user(&value,
				 *pt, 4); IMSG_DEBUG("value %x", value);
				 */
			...
			enc_context->meta[enc_context->enc_res_pos].len =
				sizeof(u32);
			enc_context->meta[enc_context->enc_res_pos].value_flag =
				enc.value_flag;
			enc_context->meta[enc_context->enc_res_pos].param_pos =
				enc.param_pos;
			enc_context->meta[enc_context->enc_res_pos]
				.param_pos_type = enc.param_pos_type;
			enc_context->enc_res_pos++;
		...
		if ((enc_context->enc_req_offset + enc.len <= TEEI_1K_SIZE)
			...
			if (copy_from_user(
				    (char *)enc_context->ker_req_data_addr
					    + enc_context->enc_req_offset,
				    (void __user *)(unsigned long)enc.data,
				    enc.len)) {
				IMSG_ERROR("[%s][%d] copy from user failed.\n",
					   __func__, __LINE__);
				retVal = -EFAULT;
...
int teei_client_encode_mem_ref(void *private_data, void *argp)
{
	...
	list_for_each_entry(temp_shared_mem, &session->shared_mem_list,
		s_head) {
		u64 addr = enc.data;
 ...
 * @param argp
 *
 * @return		EFAULT:	fail to copy data from user space OR to user
 *space. 0:	success
 */
int teei_client_decode_uint32(void *private_data, void *argp)
{
	...
	if ((dec_context->dec_res_pos <= dec_context->enc_res_pos)
		...
		if (dec_context->meta[dec_context->dec_res_pos].usr_addr)
			dec.data =
				(unsigned long long)(dec_context
							->meta[dec_context
							->dec_res_pos]
							.usr_addr);


		/* *(u32 *)dec.data = *((u32 *)((char
		 ...
		 *value =  *((u32 *)((char *)dec_context->ker_res_data_addr +
		 *dec_context->dec_offset)); copy_to_user(pt, &value, 4);
		 */

		if (((u32 *)((unsigned long)dec.data)) == NULL)
			IMSG_DEBUG(
				"[%s][%d] error decode dec.data addr11111 is NULL!\n",
				__func__, __LINE__);
		...
		if (((u32 *)((char *)dec_context->ker_res_data_addr
		     ...
		     == NULL)) {
			IMSG_DEBUG(
				"[%s][%d] decode data decode addr11111 is NULL!\n",
				__func__, __LINE__);
		} else {
			value1 =
				*((u32 *)((char *)dec_context->ker_res_data_addr
					  + dec_context->dec_offset));
...
int teei_client_decode_array_space(void *private_data, void *argp)
{
	...
	if ((dec_context->dec_res_pos <= dec_context->enc_res_pos)
		...
		if (dec_context->meta[dec_context->dec_res_pos].len
			...
			if (dec_context->meta[dec_context->dec_res_pos]
				    .usr_addr)
				dec.data =
					dec_context
			...
			if (copy_to_user(
				    (void __user *)((unsigned long)dec.data),
				    (char *)dec_context->ker_res_data_addr
					    + dec_context->dec_offset,
				    dec_context->meta[dec_context->dec_res_pos]
					    .ret_len)) {
				IMSG_ERROR(
					"[%s][%d] copy from user failed while copying array!\n",
					__func__, __LINE__);
				retVal = -EFAULT;
				goto return_func;
			}
			...
			IMSG_ERROR(
				"[%s][%d] buffer length is small. Length required %x and supplied length %x,pos %x ",
				__func__, __LINE__,
				dec_context->meta[dec_context->dec_res_pos]
					.ret_len,
			...
			retVal = -EFAULT;
			dec.len = dec_context->meta[dec_context->dec_res_pos]
					  .ret_len;
			goto return_func;
		}
		...
		if (dec_context->meta[dec_context->dec_res_pos].len
			...
			dec.data = dec_context->meta[dec_context->dec_res_pos]
					   .usr_addr;
			mem = (char *)addr;
			...
			IMSG_WARN(
				"[%s][%d] buffer length is small. Length required %x and supplied length %x",
				__func__, __LINE__,
				dec_context->meta[dec_context->dec_res_pos]
					.ret_len,
				dec_context->meta[dec_context->dec_res_pos]
		...
		IMSG_ERROR(
			"[%s][%d] invalid data type or decoder at wrong position!\n",
			__func__, __LINE__);
		retVal = -EINVAL;
		goto return_func;
	}
...
static int
teei_client_close_session_for_service_plus(void *private_data,
					  ...
					  struct teei_session *temp_ses)
{
	...
	retVal = teei_smc_call(TEEI_CMD_TYPE_CLOSE_SESSION, dev_file_id, 0,
			       TEEI_GLOBAL_CMD_ID_CLOSE_SESSION, 0, 0,
			       ses_close, sizeof(struct ser_ses_id), res,
			       sizeof(int), NULL, NULL, 0, NULL, &error_code,
			       &(curr_cont->cont_lock));
...
int teei_client_service_exit(void *private_data)
{
	...
	list_for_each_entry_safe(temp_context, temp_context_pos,
		...
		if (temp_context->cont_id == dev_file_id) {
			...
			if (!list_empty(&temp_context->sess_link)) {
				list_for_each_entry_safe(
					temp_ses, temp_ses_pos,
					&temp_context->sess_link, link)
				teei_client_close_session_for_service_plus(
						private_data, temp_ses);
			}
