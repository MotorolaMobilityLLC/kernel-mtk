...
static int init_switch_call_struct(struct switch_call_struct *ent,
				   int work_type, unsigned char *buff)
{
	if (ent == NULL) {
...
int handle_capi_call(void *buff)
{
	...
	cd->retVal = __teei_smc_call(
		cd->local_cmd, cd->teei_cmd_type, cd->dev_file_id, cd->svc_id,
		cd->cmd_id, cd->context, cd->enc_id, cd->cmd_buf, cd->cmd_len,
		cd->resp_buf, cd->resp_len, cd->meta_data, cd->info_data,
		cd->info_len, cd->ret_resp_len, cd->error_code, cd->psema);
...
int handle_fdrv_call(void *buff)
{
	...
	case TUI_DISPLAY_SYS_NO:
		cd->retVal =
			__send_tui_display_command(cd->fdrv_call_buff_size);
		break;
...
int handle_bdrv_call(void *buff)
{
	...
	case VFS_SYS_NO:
		cd->retVal =
			__vfs_handle((struct service_handler *)cd->handler);
		kfree(buff);
		break;

	case REETIME_SYS_NO:
		cd->retVal =
			__reetime_handle((struct service_handler *)cd->handler);
		kfree(buff);
		break;
