...
#define TEEI_BOOT_ERROR_ITEMS_SIZE                                             \
	(sizeof(teei_boot_error_items) / sizeof(struct teei_boot_error_item))
...
void secondary_boot_stage1(void *info)
{
	...
	n_init_t_boot_stage1((uint64_t)(cd->vfs_phy_addr),
			     (uint64_t)(cd->tlog_phy_addr), &smc_type);
	while (smc_type == 0x54)
		nt_sched_t(&smc_type);
...
static void boot_stage1(unsigned long vfs_addr, unsigned long tlog_addr)
{
	...
	retVal = add_work_entry(BOOT_STAGE1,
				(unsigned char *)(&boot_stage1_entry));
	...
	case CPU_DOWN_PREPARE_FROZEN:
		if (cpu == sched_cpu) {
			...
			add_work_entry(SWITCH_CORE,
				       (unsigned char *)(unsigned long)cpu);
		}
...
void secondary_init_cmdbuf(void *info)
{
	...
	IMSG_DEBUG(
		"[%s][%d] message = %lx,  fdrv message = %lx, bdrv_message = %lx, tlog_message = %lx.\n",
		__func__, __LINE__, (unsigned long)cd->phy_addr,
		(unsigned long)cd->fdrv_phy_addr,
		(unsigned long)cd->bdrv_phy_addr,
		(unsigned long)cd->tlog_phy_addr);

	n_init_t_fc_buf((uint64_t)cd->phy_addr, (uint64_t)cd->fdrv_phy_addr,
			&smc_type);
	while (smc_type == 0x54)
		nt_sched_t(&smc_type);

	n_init_t_fc_buf((uint64_t)cd->bdrv_phy_addr,
			(uint64_t)cd->tlog_phy_addr, &smc_type);
	while (smc_type == 0x54)
		nt_sched_t(&smc_type);
...
#ifdef UT_DMA_ZONE
	message_buff = (unsigned long)__get_free_pages(
		GFP_KERNEL | GFP_DMA,
		get_order(ROUND_UP(MESSAGE_LENGTH, SZ_4K)));
#else
	message_buff = (unsigned long)__get_free_pages(
		GFP_KERNEL, get_order(ROUND_UP(MESSAGE_LENGTH, SZ_4K)));
#endif
...
#ifdef UT_DMA_ZONE
	tlog_message_buff = (unsigned long)__get_free_pages(
		GFP_KERNEL | GFP_DMA,
		get_order(ROUND_UP(MESSAGE_LENGTH * 128, SZ_4K)));
#else
	tlog_message_buff = (unsigned long)__get_free_pages(
		GFP_KERNEL, get_order(ROUND_UP(MESSAGE_LENGTH * 128, SZ_4K)));
#endif
...
long teei_service_init_second(void)
{
	...
	IMSG_DEBUG("[%s][%d] begin to tui display command buffer!\n", __func__,
		   __LINE__);
	tui_display_message_buff =
		create_tui_buff(TUI_DISPLAY_BUFFER, TUI_DISPLAY_SYS_NO);
	...
	IMSG_DEBUG("[%s][%d] begin to tui notice command buffer!\n", __func__,
		   __LINE__);
	tui_notice_message_buff =
		create_tui_buff(TUI_NOTICE_BUFFER, TUI_NOTICE_SYS_NO);
...
static int init_teei_framework(void)
{
	...
	struct tz_log_state *s =
		dev_get_platdata(&tz_drv_state->tz_log_pdev->dev);
...
#ifndef CONFIG_MICROTRUST_TZ_LOG
	tlog_buff = (unsigned long)__get_free_pages(
		GFP_KERNEL | GFP_DMA, get_order(ROUND_UP(LOG_BUF_LEN, SZ_4K)));

	if ((void *)tlog_buff == NULL) {
		IMSG_ERROR(
			"[%s][%d]ERROR: There is no enough memory for TLOG!\n",
			__func__, __LINE__);
...
#ifdef UT_DMA_ZONE
	boot_vfs_addr = (unsigned long)__get_free_pages(
		GFP_KERNEL | GFP_DMA, get_order(ROUND_UP(VFS_SIZE, SZ_4K)));
#else
	boot_vfs_addr = (unsigned long)__get_free_pages(
		GFP_KERNEL, get_order(ROUND_UP(VFS_SIZE, SZ_4K)));
#endif

	if ((void *)boot_vfs_addr == NULL) {
		IMSG_ERROR(
			"[%s][%d]ERROR: There is no enough memory for booting Soter!\n",
			__func__, __LINE__);
...
#else
	boot_stage1((unsigned long)virt_to_phys((void *)boot_vfs_addr),
		    (unsigned long)virt_to_phys((void *)tlog_buff));
#endif
	...
	/* waiting for keymaster share memory ready and anable the keymaster
	 * IOCTL
	 */
...
static long teei_config_ioctl(struct file *file, unsigned int cmd,
			      unsigned long arg)
{
	int retVal = 0;
...
static int teei_config_init(void)
{
	...
	retVal = alloc_chrdev_region(&teei_config_device_no, 0, 1,
				     TEEI_CONFIG_DEV);
 ...
 * @return
 */
static long teei_client_ioctl(struct file *file, unsigned int cmd,
			      unsigned long arg)
{
	int retVal = 0;
	...
	if (teei_config_flag == 0) {
		IMSG_ERROR(
			"Error: soter is NOT ready, Can not support IOCTL!\n");
		return -ECANCELED;
	}
	...
	if (cmd == TEEI_CANCEL_COMMAND) {
		...
		if (copy_from_user((void *)cancel_message_buff, (void *)argp,
				   MAX_BUFF_SIZE)) {
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_INITCONTEXT beginning .....\n",
			__func__, __LINE__);
#endif
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_CLOSECONTEXT beginning .....\n",
			__func__, __LINE__);
#endif
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_CLOSECONTEXT end .....\n",
			__func__, __LINE__);
#endif
		break;
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_SES_INIT beginning .....\n",
			__func__, __LINE__);
#endif
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_SES_OPEN beginning .....\n",
			__func__, __LINE__);
#endif
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_SES_CLOSE beginning .....\n",
			__func__, __LINE__);
#endif
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_OPERATION_RELEASE beginning .....\n",
			__func__, __LINE__);
#endif
		retVal =
			teei_client_operation_release(file->private_data, argp);
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_OPERATION_RELEASE end .....\n",
			__func__, __LINE__);
#endif
		break;
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_SEND_CMD beginning .....\n",
			__func__, __LINE__);
#endif
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_GET_DECODE beginning .....\n",
			__func__, __LINE__);
#endif
	...
	case TEEI_CLIENT_IOCTL_ENC_UINT32:
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_ENC_UINT32 beginning .....\n",
			__func__, __LINE__);
#endif
		...
		if (retVal != 0)
			IMSG_ERROR(
				"[%s][%d] failed teei_client_encode_cmd: %x.\n",
				__func__, __LINE__, retVal);
	...
	case TEEI_CLIENT_IOCTL_DEC_UINT32:
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_DEC_UINT32 beginning .....\n",
			__func__, __LINE__);
#endif
		...
		if (retVal != 0)
			IMSG_ERROR(
				"[%s][%d] failed teei_client_decode_cmd: %x.\n",
				__func__, __LINE__, retVal);
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_ENC_ARRAY beginning .....\n",
			__func__, __LINE__);
#endif
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_DEC_ARRAY_SPACE beginning .....\n",
			__func__, __LINE__);
#endif
		retVal = teei_client_decode_array_space(file->private_data,
							argp);
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_DEC_ARRAY_SPACE end .....\n",
			__func__, __LINE__);
#endif
		break;
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_DEC_MEM_REF beginning .....\n",
			__func__, __LINE__);
#endif
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_ENC_ARRAY_SPACE beginning .....\n",
			__func__, __LINE__);
#endif
		retVal = teei_client_encode_mem_ref(file->private_data, argp);
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_ENC_ARRAY_SPACE end .....\n",
			__func__, __LINE__);
#endif
		break;
	...
	case TEEI_CLIENT_IOCTL_SHR_MEM_ALLOCATE_REQ:
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_SHR_MEM_ALLOCATE beginning .....\n",
			__func__, __LINE__);
#endif
		...
		if (retVal != 0)
			IMSG_ERROR(
				"[%s][%d] failed teei_client_shared_mem_alloc: %x.\n",
				__func__, __LINE__, retVal);

#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_SHR_MEM_ALLOCATE end .....\n",
			__func__, __LINE__);
#endif
		break;
	...
	case TEEI_CLIENT_IOCTL_SHR_MEM_FREE_REQ:
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_SHR_MEM_FREE beginning .....\n",
			__func__, __LINE__);
#endif
		...
		if (retVal != 0)
			IMSG_ERROR(
				"[%s][%d] failed teei_client_shared_mem_free: %x.\n",
				__func__, __LINE__, retVal);


#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_SHR_MEM_FREE end .....\n",
			__func__, __LINE__);
#endif
		break;
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_GET_TEEI_CONFIG_STAT beginning .....\n",
			__func__, __LINE__);
#endif
 ...
 */
static long teei_client_unioctl(struct file *file, unsigned int cmd,
				unsigned long arg)
{
	/*IMSG_DEBUG("[%s][%d] TEEI_CLIENT_IOCTL_INITCONTEXT chengxin test
	 * unioctl11111.....\n", __func__, __LINE__);
	 */
	int retVal = 0;
	void *argp = (void __user *)arg;
	...
	case TEEI_CLIENT_IOCTL_ENC_UINT32:
		...
		IMSG_DEBUG(
			...
			__func__, __LINE__);
#endif
		retVal = teei_client_encode_uint32_64bit(file->private_data,
							 argp);
	...
	case TEEI_CLIENT_IOCTL_ENC_ARRAY:
		...
		IMSG_DEBUG(
			...
			__func__, __LINE__);
#endif
		retVal = teei_client_encode_array_64bit(file->private_data,
							argp);
	...
	case TEEI_CLIENT_IOCTL_ENC_MEM_REF:
		...
		IMSG_DEBUG(
			...
			__func__, __LINE__);
#endif
		retVal = teei_client_encode_mem_ref_64bit(file->private_data,
							  argp);
...
#ifdef UT_DEBUG
		IMSG_DEBUG(
			"[%s][%d] TEEI_CLIENT_IOCTL_DEC_MEM_REF end  .....\n",
			__func__, __LINE__);
#endif
		break;
...
static ssize_t teei_client_dump(struct file *filp, char __user *buf,
...
static int teei_client_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	list_for_each_entry(cont, &(teei_contexts_head.context_list),
		link)   {
...
#ifdef UT_DMA_ZONE
	alloc_addr = (unsigned long)__get_free_pages(
		GFP_KERNEL | GFP_DMA, get_order(ROUND_UP(length, SZ_4K)));
#else
	alloc_addr = (unsigned long)__get_free_pages(
		GFP_KERNEL, get_order(ROUND_UP(length, SZ_4K)));
#endif
	...
	retVal = remap_pfn_range(
		vma, vma->vm_start,
		((virt_to_phys((void *)alloc_addr)) >> PAGE_SHIFT), length,
		vma->vm_page_prot);
...
static int teei_client_init(void)
{
	...
	ret_code = alloc_chrdev_region(&teei_client_device_no, 0, 1,
				       TEEI_CLIENT_DEV);
	...
	class_dev = device_create(driver_class, NULL, teei_client_device_no,
				  NULL, TEEI_CLIENT_DEV);
	...
	teei_switch_task = kthread_create(
		kthread_worker_fn, &ut_fastcall_worker, "teei_switch_thread");
