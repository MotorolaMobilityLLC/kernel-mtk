...
char *teei_boot_error_to_string(uint32_t id)
{
	...
	for (i = 0; i < (sizeof(teei_boot_error_items) / sizeof(struct teei_boot_error_item)); i++) {
...
struct workqueue_struct *secure_wq = NULL;
...
void secondary_boot_stage1(void *info)
{
	...
	n_init_t_boot_stage1(cd->vfs_phy_addr, cd->tlog_phy_addr, (uint64_t *)(&smc_type));
	while (smc_type == 0x54) {
		nt_sched_t((uint64_t *)(&smc_type));
	}
...
static void boot_stage1(unsigned long vfs_addr, unsigned long tlog_addr)
{
	...
	retVal = add_work_entry(BOOT_STAGE1, (unsigned long)(&boot_stage1_entry));
		...
		unsigned long action, void *hcpu)
{
	...
	case CPU_DOWN_PREPARE_FROZEN:
		if (cpu == sched_cpu) {
			...
			add_work_entry(SWITCH_CORE, (unsigned long)(unsigned long)cpu);
		}
		break;
	default:
		break;
...
void secondary_init_cmdbuf(void *info)
{
	...
	n_init_t_fc_buf(cd->phy_addr, cd->fdrv_phy_addr, (uint64_t *)(&smc_type));
	while (smc_type == 0x54) {
		nt_sched_t((uint64_t *)(&smc_type));
	}
	n_init_t_fc_buf(cd->bdrv_phy_addr, cd->tlog_phy_addr, (uint64_t *)(&smc_type));
	while (smc_type == 0x54) {
		nt_sched_t((uint64_t *)(&smc_type));
	}
...
#ifdef CONFIG_MICROTRUST_DCIH_SUPPORT
	IMSG_DEBUG("[%s][%d] begin to init DCIH service!\n", __func__, __LINE__);
	init_dcih_service();
#endif
...
static int init_teei_framework(void)
{
	...
	if ((unsigned char *)tlog_buff == NULL)
