...
ut_rmqueue_smallest(struct zone *zone, unsigned int order, int migratetype)
{
	...
	for (current_order = order; current_order < MAX_ORDER;
		...
		do {
			...
			if ((unsigned long)(page_to_phys(page)) >= UT_MAX_MEM) {
				IMSG_DEBUG(
					"[%s][%d] FAILED! ORDER = %d page_to_phys(page) = %lx\n",
					__func__, __LINE__, current_order,
				...
				IMSG_DEBUG(
					"[%s][%d] SUCCESS! ORDER = %d page_to_phys(page) = %lx\n",
					__func__, __LINE__, current_order,
					(unsigned long)page_to_phys(page));
				page_found = 1;
				break;
 ...
 * or two.
 */
static inline struct page *ut_buffered_rmqueue(struct zone *preferred_zone,
	...
	if (likely(order == 0)) {
		...
		if (cold) {
			...
			do {
				...
				if ((unsigned long)(page_to_phys(page))
				    >= UT_MAX_MEM) {
					page_found = 0;
				} else {
					page_found = 1;
 ...
 */
static struct page *
ut_get_page_from_freelist(gfp_t gfp_mask, nodemask_t *nodemask,
...
zonelist_scan:
	...
	for_each_zone_zonelist_nodemask(zone, z, zonelist, high_zoneidx,
		...
		if ((alloc_flags & ALLOC_WMARK_LOW) && (gfp_mask & __GFP_WRITE)
		    && !zone_dirty_ok(zone))
			goto this_zone_full;
		...
		if (!(alloc_flags & ALLOC_NO_WATERMARKS)) {
			...
			if (IS_ENABLED(CONFIG_NUMA) && !did_zlc_setup
			    && nr_online_nodes > 1) {
				/*
	...
	if (page) {
		if (_forbid_cma_alloc) {
			...
			if (svp_is_in_range(vpfn)) {
				IMSG_ERROR(
					"%s %d: pfn: %lu in _forbid_cma_alloc %d\n",
					__func__, __LINE__, vpfn,
					_forbid_cma_alloc);
				dump_stack();
			}
		}
 ...
 */
struct page *ut_alloc_pages_nodemask(gfp_t gfp_mask, unsigned int order,
...
retry_cpuset:
	...
	page = ut_get_page_from_freelist(
		gfp_mask | __GFP_HARDWALL, nodemask, order, zonelist,
		high_zoneidx, alloc_flags, preferred_zone, migratetype);
	...
	if (unlikely(!atomic_read(&system_freezing_cnt))) {
		...
		if (order >= page_alloc_dump_order_threshold) {
			...
			trace_dump_allocate_large_pages(page, order, gfp_mask,
							entries);
...
static inline struct page *ut_alloc_pages(gfp_t gfp_mask, unsigned int order,
					  struct zonelist *zonelist)
{
