...
#define pr_debug(fmt, args...) pr_debug("\033[;34m[TEEI][TZDriver]"fmt"\033[0m", ##args)
...
unsigned long create_tui_buff(int buff_size,unsigned int fdrv_type)
{
	...
	if (message_buff == NULL) {
		pr_debug("[%s][%d]: There is NO command buffer!.\n", __func__, __LINE__);
		return NULL;
	}
	...
	if (temp_addr == NULL) {
		pr_debug("[%s][%d]: kmalloc fp drv buffer failed.\n", __FILE__, __LINE__);
		return NULL;
	}
...
int wait_for_power_down(void)
{
	...
	do {
		...
		if ((power_down_flag == 1) && (enter_tui_flag)) {
			...
			send_power_down_cmd();
			pr_debug("[%s][%d]catch a power_down_flag!!!\n", __func__, __LINE__ );
		}
		power_down_flag = 0;
...
int tui_notify_reboot(struct notifier_block* this,unsigned long code,void *x)
{
	if ((code == SYS_RESTART) && enter_tui_flag) {
		mtkfb_set_backlight_level(0);
		send_power_down_cmd();
		pr_debug("[%s][%d]catch a ree_reboot_signal!!!\n", __func__, __LINE__ );
	}
	return NOTIFY_OK;
}
