...
unsigned long create_tui_buff(int buff_size, unsigned int fdrv_type)
{
...
#ifdef UT_DMA_ZONE
	temp_addr = (unsigned long)__get_free_pages(
		GFP_KERNEL | GFP_DMA, get_order(ROUND_UP(buff_size, SZ_4K)));
#else
	temp_addr = (unsigned long)__get_free_pages(
		GFP_KERNEL, get_order(ROUND_UP(buff_size, SZ_4K)));
#endif
	...
	memcpy((void *)(message_buff + sizeof(struct message_head)), &msg_body,
...
void set_tui_display_command(unsigned long type)
{
	...
	memcpy((void *)fdrv_message_buff, &fdrv_msg_head,
...
int send_tui_display_command(unsigned long type)
{
	...
	Flush_Dcache_By_Area((unsigned long)&fdrv_ent,
			     (unsigned long)&fdrv_ent
				     + sizeof(struct fdrv_call_struct));
