...
static long create_notify_queue(unsigned long msg_buff, unsigned long size)
{
	...
	if (size > MAX_BUFF_SIZE) {
		IMSG_ERROR(
			"[%s][%d]: The NQ buffer size is too large, DO NOT Allow to create it.\n",
			__FILE__, __LINE__);
		retVal = -EINVAL;
		goto return_fn;
	}
...
#ifdef UT_DMA_ZONE
	nt_t_buffer = (unsigned long)__get_free_pages(
		GFP_KERNEL | GFP_DMA, get_order(ROUND_UP(size, SZ_4K)));
#else
	nt_t_buffer = (unsigned long)__get_free_pages(
		GFP_KERNEL, get_order(ROUND_UP(size, SZ_4K)));
#endif
...
#ifdef UT_DMA_ZONE
	t_nt_buffer = (unsigned long)__get_free_pages(
		GFP_KERNEL | GFP_DMA, get_order(ROUND_UP(size, SZ_4K)));
#else
	t_nt_buffer = (unsigned long)__get_free_pages(
		GFP_KERNEL, get_order(ROUND_UP(size, SZ_4K)));
...
int add_nq_entry(u32 command_buff, int command_length, int valid_flag)
{
	...
	temp_entry =
		(struct NQ_entry *)(nt_t_buffer + NQ_BLOCK_SIZE
...
unsigned char *get_nq_entry(unsigned char *buffer_addr)
{
	...
	if (temp_head->start_index == temp_head->end_index) {
		IMSG_DEBUG(
			"[cache] temp_head->start_index = %d  temp_head->end_index = %d\n ",
			temp_head->start_index, temp_head->end_index);
		return NULL;
	}

	temp_entry =
		(struct NQ_entry *)(buffer_addr + NQ_BLOCK_SIZE
				    + temp_head->start_index * NQ_BLOCK_SIZE);
	temp_head->start_index =
		(temp_head->start_index + 1) % temp_head->Max_count;
...
long create_nq_buffer(void)
{
	...
	if (retVal < 0) {
		IMSG_ERROR(
			"[%s][%d]:create_notify_queue failed with errno %ld.\n",
