...
void add_sched_queue(void)
{
	...
	if (curr_entry == NULL) {
		IMSG_ERROR("[%s][%d] Can NOT get unused schedule work_entry!",
			   __func__, __LINE__);
		return;
	}
...
{
	return request_irq(irq, nt_soter_irq_handler, 0, "tz_drivers_service",
			   NULL);
}
...
void load_func(struct work_struct *entry)
{
	...
	vfs_thread_function(boot_vfs_addr, (unsigned long)NULL,
			    (unsigned long)NULL);
	down(&smc_lock);
	...
	if (boot_soter_flag == START_STATUS) {
		...
		if (msg_head->message_type == FAST_CALL_TYPE) {
			...
			if (msg_head->child_type == VDRV_CALL_TYPE) {
				...
				/* IMSG_DEBUG("[%s][%d] ==== STANDARD_CALL_TYPE
				 * ACK ========\n", __func__, __LINE__);
				 */
				...
				Invalidate_Dcache_By_Area(
					...
					(unsigned long)command
						+ MESSAGE_LENGTH);
				/* Get the semaphore */
				cmd_sema = (struct semaphore
						    *)(command->teei_sema);
				/*
				 *	IMSG_DEBUG("[%s][%d] After smc_call
				 *cmd_sema = %p\n", __func__, __LINE__,
				 *cmd_sema);
				 */
				/* Up the semaphore */
...
{
	return request_irq(irq, ut_drv_irq_handler, 0, "tz_drivers_service",
			   NULL);
}
