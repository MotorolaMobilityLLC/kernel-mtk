...
static struct platform_driver mem_bw_ctrl = {
	.driver = {
	    .name = "mem_bw_ctrl", .owner = THIS_MODULE,
	},
};

static struct platform_driver ddr_type = {
	.driver = {
	    .name = "ddr_type", .owner = THIS_MODULE,
	},
};
...
int mtk_mem_bw_ctrl(int sce, int op)
{
	...
	if (highest != cur_con_sce) {
		if (dram_type == TYPE_LPDDR3) {
			...
			mt_reg_sync_writel(emi_conm_lpddr3_1866_val[highest],
					   EMI_CONM);
			...
			mt_reg_sync_writel(emi_conm_lpddr4_2667_val[highest],
					   EMI_CONM);
		}
 ...
 */
static ssize_t ddr_type_store(struct device_driver *driver, const char *buf,
			      size_t count)
{
	/*do nothing*/
	return count;
 ...
 * Return the number of write bytes.
 */
static ssize_t con_sce_store(struct device_driver *driver, const char *buf,
			     size_t count)
{
	int i;
...
static int __init emi_bwl_mod_init(void)
{
	...
	if (ret)
		pr_debug("[EMI/BWL] fail to register EMI_BW_LIMITER driver\n");
	...
	if (ret)
		pr_debug("[EMI/BWL] fail to register DRAM_TYPE driver\n");

	ret = driver_create_file(&ddr_type.driver, &driver_attr_ddr_type);
	if (ret)
		pr_debug("[EMI/BWL] fail to create DRAM_TYPE sysfs file\n");
