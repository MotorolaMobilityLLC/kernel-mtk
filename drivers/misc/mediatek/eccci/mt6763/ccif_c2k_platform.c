			      ...
			      struct md_hw_info *hw_info)
{
	...
	case 2: /*MD_SYS3 */
		...
		node =
		    of_find_compatible_node(NULL, NULL, "mediatek,md_ccif1");
		...
		node = of_find_compatible_node(NULL, NULL,
					       "mediatek,mt6799-toprgu");
		hw_info->toprgu_base = of_iomap(node, 0);

		node = of_find_compatible_node(NULL, NULL,
					       "mediatek,mt6799-apmixedsys");
		...
		node =
		    of_find_compatible_node(NULL, NULL, "mediatek,infracfg");
	...
	CCCI_INIT_LOG(dev_cfg->index, TAG,
		...
		dev_cfg->major, dev_cfg->minor_base,
		dev_cfg->capability,
		(void *)hw_info->ap_ccif_base,
...
int md_ccif_io_remap_md_side_register(struct ccci_modem *md)
{
	...
	case MD_SYS3:
		...
		md_info->ccirq_base[0] =
		    ioremap_nocache(MD1_C2K_CCIRQ_BASE, 0x100);
		md_info->ccirq_base[1] =
		    ioremap_nocache(C2K_MD1_CCIRQ_BASE, 0x100);
...
static int reset_ccirq_hardware(struct ccci_modem *md)
{
	...
	for (i = 0; i < 2; i++) {
		/* config MD1_C2K/C2K_MD1 CC_IRQ_CLEAR_IRQ and
		 * CC_IRQ_CLEAR_AUTH_EXEC register to reset CC IRQ
		 */
	...
	CCCI_NORMAL_LOG(md->index, TAG, "%s end\n", __func__);
	return 0;
}
...
int md_ccif_let_md_go(struct ccci_modem *md)
{
	...
	case MD_SYS3:
		...
		/* step 5: wake up C2K */
		/* step 5.1: switch MDPLL1(208M) Control to hw mode: [28] =
		 * 1'b0
		 */
		ccif_write32(apmixed_base, MDPLL_CON3,
		...
		ccif_write32(
		    hw_info->infra_ao_base, C2K_SPM_CTRL,
		    ccif_read32(hw_info->infra_ao_base, C2K_SPM_CTRL) |
			(0x1 << 1));
		/* step 5.5: polling C2K_STATUS[1] is high - C2KSYS has enter
		 * idle state
		 */
		CCCI_BOOTUP_LOG(
		    md->index, TAG, "[C2K] C2K_STATUS before = 0x%x\n",
		...
		while (
			...
			;
		CCCI_BOOTUP_LOG(
		    md->index, TAG, "[C2K] C2K_STATUS after = 0x%x\n",
		    ccif_read32(hw_info->infra_ao_base, C2K_STATUS));
		/* step 5.6 */
		ccif_write32(
		    hw_info->infra_ao_base, C2K_SPM_CTRL,
...
int md_ccif_power_on(struct ccci_modem *md)
{
	...
	if (ret == 0 && md->index != MD_SYS3) {
		/*disable MD WDT */
		ccif_write32(md_info->md_rgu_base, WDT_MD_MODE,
			     WDT_MD_MODE_KEY);
	}
	return ret;
}
