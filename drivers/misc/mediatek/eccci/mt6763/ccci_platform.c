...
int Is_MD_EMI_voilation(void)
{
	...
	if ((violation_port == MASTER_MDMCU) ||
	    (violation_port == MASTER_MDHW))
		return 1;
	return 0;
#else
	return 1;
...
#define MPU_ACCESS_PERMISSON_AP_MD1_RO_ATTR                                   \
...
#define MPU_ACCESS_PERMISSON_AP_ATTR                                          \
	SET_ACCESS_PERMISSON(FORBIDDEN, NO_PROTECTION, FORBIDDEN,             \
			     NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,  \
			     NO_PROTECTION)
/* #define MPU_REGION_ID_MD1_SMEM	  8  infact: in LK*/
#define MPU_ACCESS_PERMISSON_MD1_SMEM_ATTR                                    \
	SET_ACCESS_PERMISSON(NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,  \
			     NO_PROTECTION, FORBIDDEN, NO_PROTECTION,         \
			     NO_PROTECTION)
/* #define MPU_REGION_ID_MD3_ROM       19 infact: in LK
 * note that D3 was set to NO_PROTECTION, but the mpu indicate
 * it be SEC_R_NSEC_R
 */
#define MPU_ACCESS_PERMISSON_MD3_ROM_ATTR                                     \
	SET_ACCESS_PERMISSON(FORBIDDEN, FORBIDDEN, SEC_R_NSEC_R, FORBIDDEN,   \
			     FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_R_NSEC_R)
/* #define MPU_REGION_ID_MD3_RW		20 infact: in LK */
#define MPU_ACCESS_PERMISSON_MD3_RW_ATTR                                      \
	SET_ACCESS_PERMISSON(FORBIDDEN, FORBIDDEN, NO_PROTECTION, FORBIDDEN,  \
			     FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_R_NSEC_R)
/* #define MPU_REGION_ID_MD3_SMEM	  9 infact: in LK */
#define MPU_ACCESS_PERMISSON_MD3_SMEM_ATTR                                    \
	SET_ACCESS_PERMISSON(FORBIDDEN, FORBIDDEN, NO_PROTECTION, FORBIDDEN,  \
			     NO_PROTECTION, FORBIDDEN, FORBIDDEN,             \
			     NO_PROTECTION)
/* #define MPU_REGION_ID_MD1MD3_SMEM   10
 * infact: in LK , AP need to clear smem, so set it to NO_PROTECTION
 */
#define MPU_ACCESS_PERMISSON_MD1MD3_SMEM_ATTR                                 \
	SET_ACCESS_PERMISSON(FORBIDDEN, FORBIDDEN, NO_PROTECTION, FORBIDDEN,  \
			     FORBIDDEN, FORBIDDEN, NO_PROTECTION,             \
			     NO_PROTECTION)

static const unsigned int
	MPU_ATTR_DEFAULT[MPU_REGION_ID_TOTAL_NUM][MPU_DOMAIN_ID_TOTAL_NUM] = {
/*==========================================================================*/
...
/*---------+----------------------------------------------------------------*/
	...
	{SEC_RW, FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_RW, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN},
	/* 3*/
	{FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_RW, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN},
	/* 4*/
	{NO_PROTECTION, FORBIDDEN, FORBIDDEN, NO_PROTECTION, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN, FORBIDDEN},
	/* 5*/
	{SEC_RW, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN},
	/* 6*/
	{SEC_RW, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN},
	/* 7*/
	{SEC_RW, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN},
	/*8*/
	{NO_PROTECTION, NO_PROTECTION, FORBIDDEN, NO_PROTECTION, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN, FORBIDDEN},
	/*9*/
	{NO_PROTECTION, FORBIDDEN, FORBIDDEN, NO_PROTECTION, FORBIDDEN,
	 NO_PROTECTION, FORBIDDEN, FORBIDDEN},
	...
	{SEC_R_NSEC_R, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN, NO_PROTECTION},
	/*12*/
	{SEC_R_NSEC_R, SEC_R_NSEC_R, FORBIDDEN, FORBIDDEN, FORBIDDEN,
...
static const unsigned int CHEAD_MPU_DOMAIN_ID[] = {
	CHEAD_MPU_DOMAIN_0, CHEAD_MPU_DOMAIN_1, CHEAD_MPU_DOMAIN_2,
	CHEAD_MPU_DOMAIN_3};
...
/* region_id: 0~7 0=MPU_DOMAIN_INFO_ID_MD1
 * ->CHEAD_MPU_DOMAIN_ID[MPU_DOMAIN_INFO_ID_MD1]-- domain 1 MD1
 */
	...
	for (domain_id = 0; domain_id < 8; domain_id++) {
		...
		case CHEAD_MPU_DOMAIN_3:
			...
			if ((extract_value >= NO_PROTECTION) &&
				...
				region_dom[domain_id] = MPU_ATTR_DEFAULT
		...
		default:
			region_dom[domain_id] =
			    MPU_ATTR_DEFAULT[MPU_REGION_INFO_ID[region_id]]
					    [domain_id];
			CCCI_DEBUG_LOG(md->index, TAG,
				       "2. region_dom[%d] = %X\n", domain_id,
				       region_dom[domain_id]);
	...
	case MD_SYS1:
		if (modem_run_env_ready(MD_SYS1)) {
			...
			if (mpu_cfg_inf) {
				CCCI_BOOTUP_LOG(
				    md->index, TAG,
				    "Start MPU protect region <%d:%u:%u> %X\n",
				    region_mpu_id, mpu_cfg_inf->start,
				    mpu_cfg_inf->end, region_mpu_attr);
				emi_mpu_set_region_protection(
				    mpu_cfg_inf->start,  /*START_ADDR */
			...
			while (next != 0) {
				related_mpu_cfg_inf =
				    get_mpu_region_cfg_info(next);

				if (related_mpu_cfg_inf) {
					has_relate = 1;
					emi_mpu_set_region_protection(
						/*START_ADDR */
						related_mpu_cfg_inf->start,
						/*END_ADDR */
						related_mpu_cfg_inf->end,
						/*region */
						related_mpu_cfg_inf->region,
						region_mpu_attr);
					CCCI_BOOTUP_LOG(md->index, TAG,
					"MPU <%d> -- <%d> using same setting(@1st)\n",
					    mpu_cfg_inf->region, next);
					/* goto next */
					next =
					related_mpu_cfg_inf->relate_region;
				} else {
					CCCI_ERROR_LOG(md->index, TAG,
					"MPU <%d> -- <%d> setting not found(@1st)\n",
					mpu_cfg_inf->region, next);
					break;
				}
			}
	...
	case MD_SYS1:
		...
		if (mpu_cfg_inf) {
			CCCI_BOOTUP_LOG(
			    md->index, TAG,
			    "Start MPU protect region <%d:%u:%u> %X\n",
			    mpu_cfg_inf->region, mpu_cfg_inf->start,
			    mpu_cfg_inf->end, mpu_cfg_inf->permission);
			emi_mpu_set_region_protection(
			    mpu_cfg_inf->start,  /*START_ADDR */
			    mpu_cfg_inf->end,    /*END_ADDR */
			    mpu_cfg_inf->region, /*region */
		...
		while (next != 0) {
			...
			if (related_mpu_cfg_inf) {
				has_relate = 1;
				emi_mpu_set_region_protection(
				    related_mpu_cfg_inf->start, /*START_ADDR */
				    related_mpu_cfg_inf->end,   /*END_ADDR */
				    related_mpu_cfg_inf->region, /*region */
				    mpu_cfg_inf->permission);
				CCCI_BOOTUP_LOG(md->index, TAG,
						"MPU <%d> -- <%d> using same setting(@2nd)\n",
						mpu_cfg_inf->region, next);
				next = related_mpu_cfg_inf
					   ->relate_region; /* goto next */
			} else {
				CCCI_ERROR_LOG(md->index, TAG,
					       "MPU <%d> -- <%d> setting not found(@2nd)\n",
					       mpu_cfg_inf->region, next);
				break;
			}
		}
		if (!has_relate)
			CCCI_BOOTUP_LOG(
			    md->index, TAG,
			    "No relate region for region<%d>(@2nd)\n",
			    region_mpu_id);
...
/*
 * when MD attached its codeviser for debuging,
 * this bit will be set. so CCCI should disable some
...
unsigned int ccci_get_md_debug_mode(struct ccci_modem *md)
{
	...
	/* this function does NOT distinguish modem ID,
	 * may be a risk point
	 */
				    ...
				    LOW_POEWR_NOTIFY_TYPE type, int level)
{
	...
	case LOW_BATTERY:
		if (level == LOW_BATTERY_LEVEL_0)
			...
			reserve = (1 << 6); /* 64 */
		ret = port_proxy_send_msg_to_md(md->port_proxy, CCCI_SYSTEM_TX,
						MD_LOW_BATTERY_LEVEL, reserve,
						1);
		if (ret)
			CCCI_ERROR_LOG(
			    md->index, TAG,
			...
			void __iomem *baseB)
{
	...
	for (i = 0; i < PCCIF_SRAM_SIZE / sizeof(unsigned int); i++) {
		ccci_write32(baseA, PCCIF_CHDATA + i * sizeof(unsigned int),
			     0);
		ccci_write32(baseB, PCCIF_CHDATA + i * sizeof(unsigned int),
			     0);
	}
}
