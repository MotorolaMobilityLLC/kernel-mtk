...
const struct of_device_id swtp_of_match[] = {
	...
	//BEGIN HUAQIN,<ZHANGZHAO>,<2022-9-20>,<EKPENANG4G-3078>,[BUG],/Modify the SWTP function power boost method.
...
static int swtp_switch_state(int irq, struct swtp_t *swtp)
{
	...
	if (swtp->gpio_state[i] == SWTP_EINT_PIN_PLUG_IN)
        ...
        if (boardid_index < 0) {
            CCCI_LEGACY_ERR_LOG(-1, SYS, "%s:BoardID Invalid\n", __func__);
            return -1;
        }

        switch ((BoardID)boardid_index) {
        //BEGIN NO CA(SWTP)
            case BoardID_0:
            case BoardID_1:
            case BoardID_2:
        ...
        //BEGIN CA(SWTP)
            default:
            {
                CCCI_LEGACY_ERR_LOG(-1, SYS, "%s Added BoardID\n", __func__);
                break;
            }
        }
        //END <EKPENANG4G-3199>
