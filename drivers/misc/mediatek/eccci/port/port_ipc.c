...
static int port_ipc_parse_gf_port(GF_IP_TYPE ip_type,
	GF_PROTOCOL_TYPE prot_type,
	struct garbage_filter_item *list, int number)
{
	int port_number = -1;
	/*
	 * [Porting notes]
	 * if really need, please find suitable net
	 * interface to get opened port list
	 */
	CCCI_NORMAL_LOG(-1, IPC,
		"IP:%d Protocol:%d port number:%d\n",
...
int ccci_ipc_set_garbage_filter(int md_id, int reg)
{
	...
	if (reg) {
		ret = port_ipc_parse_gf_port(GF_IPV4, GF_TCP,
			gf_port_list, GF_PORT_LIST_MAX - count);
		if (ret > 0)
			count += ret;
		ret = port_ipc_parse_gf_port(GF_IPV4, GF_UDP,
			gf_port_list + count, GF_PORT_LIST_MAX - count);
		if (ret > 0)
			count += ret;
		ret = port_ipc_parse_gf_port(GF_IPV6, GF_TCP,
			gf_port_list + count, GF_PORT_LIST_MAX - count);
		if (ret > 0)
			count += ret;
		ret = port_ipc_parse_gf_port(GF_IPV6, GF_UDP,
			gf_port_list + count, GF_PORT_LIST_MAX - count);
		if (ret > 0)
			count += ret;
		CCCI_NORMAL_LOG(md->index, IPC,
			"register garbage filer port number %d\n", count);
		...
		if (count == 0)
			...
			gf_header.filter_cnt = count;
		CCCI_NORMAL_LOG(md->index, IPC,
			"unregister garbage filer port number %d\n", count);
	}
	gf_header.uplink = 0;

	actual_count = sizeof(struct ccci_header) +
		sizeof(struct ccci_ipc_ilm) + sizeof(struct local_para);
	if (reg)
		actual_count += (sizeof(struct garbage_filter_header) +
			count * sizeof(struct garbage_filter_item));
	else
		actual_count += (sizeof(struct garbage_filter_header) +
			count * sizeof(int));
	...
	if (skb) {
		...
		local_para_ptr = (struct local_para *)skb_put(skb,
			sizeof(struct local_para));
		local_para_ptr->ref_count = 0;
		local_para_ptr->_stub = 0;
		if (reg)
			garbage_length = count *
				sizeof(struct garbage_filter_item);
		else
			garbage_length = count * sizeof(int);
		local_para_ptr->msg_len =
		    garbage_length + sizeof(struct garbage_filter_header) +
				sizeof(struct local_para);
		/* copy gf header */
		...
		/* copy gf items */
		if (reg)
			memcpy(skb_put(skb, garbage_length), gf_port_list,
				garbage_length);
		else
			memcpy(skb_put(skb, garbage_length), gf_port_list_unreg,
				garbage_length);
		CCCI_NORMAL_LOG(md->index, IPC,
			"garbage filer data length %d/%d\n", garbage_length,
			actual_count);
