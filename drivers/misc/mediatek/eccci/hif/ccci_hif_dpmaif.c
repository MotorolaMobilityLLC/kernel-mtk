...
static int dpmaifq_set_rx_bat(unsigned char q_num, unsigned int bat_cnt)
{
	...
	if ((rxq->que_started != true) ||
		...
		CCCI_ERROR_LOG(-1, TAG, "dpmaifq_set_rx_bat: %d, 0x%x\n",
	...
	if (old_sw_rl_idx > old_sw_wr_idx) {
		...
		if (new_sw_wr_idx >= old_sw_rl_idx) {
			CCCI_HISTORY_LOG(-1, TAG,
				"dpmaifq_set_rx_bat: new_sw_wr_idx >= old_sw_rl_idx\n");
			ret = FLOW_CHECK_ERR;
		}
	} else {
...
#endif
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_LOG(-1, TAG, "dpmaifq_set_rx_bat: 0x%x\n", bat_cnt);
#endif
	return ret;
}
		...
		unsigned char q_num, unsigned int buf_cnt, int blocking)
{
	...
	alloc_cnt = buf_cnt;
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_LOG(-1, TAG, "dpmaif_alloc_rx_buf 1: 0x%x, 0x%x\n",
	...
	bat_cnt = ring_buf_writeable(bat_max_cnt, bat_req->bat_rel_rd_idx,
					bat_req->bat_wr_idx);
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_LOG(-1, TAG, "dpmaif_alloc_rx_buf 2: 0x%x, 0x%x\n",
		bat_max_cnt, bat_cnt);
#endif
			...
			unsigned short rel_entry_num)
{
	...
	if (rel_entry_num >= rxq->pit_size_cnt) {
		CCCI_ERROR_LOG(-1, TAG,
			"dpmaifq_rel_rx_pit_entry: (num >= rxq->pit_size_cnt)\n");
		return -1;
	}
	...
	if (old_hw_wr_idx == old_sw_rel_idx) {
		CCCI_HISTORY_LOG(-1, TAG,
			"dpmaifq_rel_rx_pit_entry: (old_hw_wr_idx == old_sw_rel_idx)\n");
	}

	if (old_hw_wr_idx > old_sw_rel_idx) {
		if (new_sw_rel_idx > old_hw_wr_idx) {
			CCCI_HISTORY_LOG(-1, TAG,
				"dpmaifq_rel_rx_pit_entry: (new_rel_idx > old_hw_wr_idx)\n");
		}
		...
		if (new_sw_rel_idx >= rxq->pit_size_cnt) {
			...
			if (new_sw_rel_idx > old_hw_wr_idx) {
				CCCI_HISTORY_LOG(-1, TAG,
					"dpmaifq_rel_rx_pit_entry: (new_rel_idx > old_wr_idx)\n");
			}
		}
	}
