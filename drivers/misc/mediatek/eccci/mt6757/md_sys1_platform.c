...
static struct ccci_clk_node clk_table[] = {
	{NULL, "scp-sys-md1-main"},    {NULL, "infra-cldma-ap"},
	{NULL, "infra-ccif-ap"},       {NULL, "infra-ccif-md"},
	{NULL, "infra-ap-c2k-ccif-0"}, {NULL, "infra-ap-c2k-ccif-1"},
	{NULL, "infra-md2md-ccif-0"},  {NULL, "infra-md2md-ccif-1"},
	{NULL, "infra-md2md-ccif-2"},  {NULL, "infra-md2md-ccif-3"},
	{NULL, "infra-md2md-ccif-4"},  {NULL, "infra-md2md-ccif-5"},
};

static void __iomem *md_sram_pms_base;
static void __iomem *md_sram_pd_psmcusys_base;

/* cancel unused pinctrl code */
#if 0 && defined(CONFIG_PINCTRL_MT6757)
...
int __weak spm_get_md1_bulk_option(void) { return 0; }
...
/* cancel unused pinctrl code */
	...
	if (cldma_hw->cldma_ap_ao_base == 0 ||
		...
		CCCI_ERROR_LOG(dev_cfg->index, TAG, "ap_cldma: ao_base=0x%p, pdn_base=0x%p,md_cldma: ao_base=0x%p,pdn_base=0x%p\n",
			       (void *)cldma_hw->cldma_ap_ao_base,
			       (void *)cldma_hw->cldma_ap_pdn_base,
			       (void *)cldma_hw->cldma_md_ao_base,
			       (void *)cldma_hw->cldma_md_pdn_base);
		return -1;
	}
	...
	if (cldma_hw->cldma_irq_id == 0 || hw_info->ap_ccif_irq_id == 0 ||
	    hw_info->md_wdt_irq_id == 0) {
		CCCI_ERROR_LOG(dev_cfg->index, TAG,
			       "cldma_irq:%d,ccif_irq:%d,md_wdt_irq:%d\n",
			       cldma_hw->cldma_irq_id, hw_info->ap_ccif_irq_id,
			       hw_info->md_wdt_irq_id);
		return -1;
	}
	CCCI_DEBUG_LOG(dev_cfg->index, TAG, "ap_cldma: ao_base=0x%p,pdn_base=0x%p,md_cldma: ao_base=0x%p, pdn_base=0x%p\n",
		       (void *)cldma_hw->cldma_ap_ao_base,
		       (void *)cldma_hw->cldma_ap_pdn_base,
		       (void *)cldma_hw->cldma_md_ao_base,
		       (void *)cldma_hw->cldma_md_pdn_base);
			       ...
			       unsigned int stop_type)
{
	...
	if (!is_clk_buf_from_pmic()) {
		CCCI_NORMAL_LOG(md->index, TAG,
				"%s: clk buf is not from pmic, exist!\n",
				__func__);
		return;
	}
	...
	if (power_on) {
		...
		if (cnt > 0) {
			if (atomic_dec_and_test(&md_power_on_off_cnt) &&
			    stop_type == MD_FLIGHT_MODE_ENTER) {
				reg_value = ccci_read32(ap_sleep_base, 0x370);
				reg_value |= 0x7F;
				ccci_write32(ap_sleep_base, 0x370, reg_value);
				...
				CCCI_NORMAL_LOG(
				    md->index, TAG,
				    "%s: power off done, clk:0x%x sleep:0x%x\n",
				    __func__, reg_value,
				    ccci_read32(ap_sleep_base, 0x370));
			}
		} else
...
int md_cd_io_remap_md_side_register(struct ccci_modem *md)
{
	...
	md_info->md_bus_status =
	    ioremap_nocache(MD_BUS_STATUS_BASE, MD_BUS_STATUS_LENGTH);
	md_info->md_topsm_status =
	    ioremap_nocache(MD_TOPSM_STATUS_BASE, MD_TOPSM_STATUS_LENGTH);
	...
	md_reg->md_pc_mon1 =
	    ioremap_nocache(MD_PC_MONITOR_BASE, MD_PC_MONITOR_LENGTH);
	md_info->md_pc_monitor = md_reg->md_pc_mon1;
	md_reg->md_pc_mon2 =
	    ioremap_nocache(MD_PC_MONITORL1_BASE, MD_PC_MONITORL1_LENGTH);
	md_reg->md_clkSW = ioremap_nocache(MD_CLKSW_BASE, MD_CLKSW_LENGTH);
	md_reg->md_dcm =
	    ioremap_nocache(MD_GLOBAL_CON_DCM_BASE, MD_GLOBAL_CON_DCM_LEN);
	md_reg->md_peri_misc =
	...
	md_reg->md_busreg1 =
	    ioremap_nocache(MD_BUS_DUMP_ADDR1, MD_BUS_DUMP_LEN1);
	md_reg->md_busreg2 =
	    ioremap_nocache(MD_BUS_DUMP_ADDR2, MD_BUS_DUMP_LEN2);
	md_reg->md_busrec =
	    ioremap_nocache(MD_BUSREC_DUMP_ADDR, MD_BUSREC_DUMP_LEN);
	md_reg->md_busrec_L1 =
	    ioremap_nocache(MD_BUSREC_L1_DUMP_ADDR, MD_BUSREC_L1_DUMP_LEN);
	md_reg->md_dbgsys_clk =
	    ioremap_nocache(MD_DBGSYS_CLK_ADDR, MD_DBGSYS_CLK_LEN);
	...
	md_reg->md_clk_ctl01 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR01, MD_Clkctrl_DUMP_LEN01);
	md_reg->md_clk_ctl02 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR02, MD_Clkctrl_DUMP_LEN02);
	md_reg->md_clk_ctl03 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR03, MD_Clkctrl_DUMP_LEN03);
	md_reg->md_clk_ctl04 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR04, MD_Clkctrl_DUMP_LEN04);
	md_reg->md_clk_ctl05 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR05, MD_Clkctrl_DUMP_LEN05);
	md_reg->md_clk_ctl06 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR06, MD_Clkctrl_DUMP_LEN06);
	md_reg->md_clk_ctl07 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR07, MD_Clkctrl_DUMP_LEN07);
	md_reg->md_clk_ctl08 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR08, MD_Clkctrl_DUMP_LEN08);
	md_reg->md_clk_ctl09 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR09, MD_Clkctrl_DUMP_LEN09);
	md_reg->md_clk_ctl10 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR10, MD_Clkctrl_DUMP_LEN10);
	md_reg->md_clk_ctl11 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR11, MD_Clkctrl_DUMP_LEN11);
	md_reg->md_clk_ctl12 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR12, MD_Clkctrl_DUMP_LEN12);
	md_reg->md_clk_ctl13 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR13, MD_Clkctrl_DUMP_LEN13);
	md_reg->md_clk_ctl14 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR14, MD_Clkctrl_DUMP_LEN14);
	md_reg->md_clk_ctl15 =
	    ioremap_nocache(MD_Clkctrl_DUMP_ADDR15, MD_Clkctrl_DUMP_LEN15);
	md_reg->md_boot_stats0 =
	    (void __iomem *)(infra_cfg_base + MD1_CFG_BOOT_STATS0);
	md_reg->md_boot_stats1 =
	    (void __iomem *)(infra_cfg_base + MD1_CFG_BOOT_STATS1);
...
void md_cd_dump_debug_register(struct ccci_modem *md)
{
	...
	if (per_md_data->md_dbg_dump_flag & (1 << MD_DBG_DUMP_TOPSM)) {
		...
		ccci_write32(md_reg->md_busreg1, R_DBGSYS_CLK_PMS_1,
			     0xE7C5); /* pre-action: permission */
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_info->md_topsm_status,
				   MD_TOPSM_STATUS_LENGTH);
	}
	/* 3. PC Monitor */
	if (per_md_data->md_dbg_dump_flag & (1 << MD_DBG_DUMP_PCMON)) {
		...
		while (!(reg_value = ccci_read32(md_addr, 0x20) & (1 << 3))) {
			...
			if (cnt > 100) {
				CCCI_ERROR_LOG(md->index, TAG,
					       "[pre-action]read 0x%p=  0x%x\n",
					       (md_addr + 0x20),
					       ccci_read32(md_addr, 0x20));
				break;
			}
		}
		...
		ccci_write32(md_reg->md_pc_mon1, 4,
			     0x80000000); /* stop MD PCMon */
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_pc_mon1, 0x48);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   (md_reg->md_pc_mon1 + 0x100), 0x280);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   (md_reg->md_pc_mon1 + 0x400), 0x100);
		ccci_write32(md_reg->md_pc_mon1, 4, 0x1); /* restart MD PCMon */

		ccci_write32(md_reg->md_pc_mon2, 4,
			     0x80000000); /* stop MD PCMon:L1 */
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_pc_mon2, 0x48);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   (md_reg->md_pc_mon2 + 0x100), 0x280);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   (md_reg->md_pc_mon2 + 0x400), 0x100);
	...
	if (per_md_data->md_dbg_dump_flag & (1 << MD_DBG_DUMP_BUS)) {
		...
		ccci_write32(md_reg->md_busreg1, R_DBGSYS_CLK_PMS_1,
			     0xE7C5); /* pre-action: permission */
		ccci_write32(md_reg->md_busreg1, R_DBGSYS_CLK_PMS_2,
			     0x65); /* pre-action: permission */
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_info->md_bus_status, 0x38);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   (md_info->md_bus_status + 0x100), 0x30);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_busreg1, MD_BUS_DUMP_LEN1);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_busreg2, MD_BUS_DUMP_LEN2);
	}
	/* 7. dump PLL */
	if (per_md_data->md_dbg_dump_flag & (1 << MD_DBG_DUMP_PLL)) {
		...
		ccci_write32(md_reg->md_busreg1, R_DBGSYS_CLK_PMS_2,
			     0x65); /* pre-action: permission */
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clkSW, 0x4C);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl01, MD_Clkctrl_DUMP_LEN01);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl02, MD_Clkctrl_DUMP_LEN02);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl03, MD_Clkctrl_DUMP_LEN03);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl04, MD_Clkctrl_DUMP_LEN04);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl05, MD_Clkctrl_DUMP_LEN05);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl06, MD_Clkctrl_DUMP_LEN06);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl07, MD_Clkctrl_DUMP_LEN07);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl08, MD_Clkctrl_DUMP_LEN08);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl09, MD_Clkctrl_DUMP_LEN09);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl10, MD_Clkctrl_DUMP_LEN10);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl11, MD_Clkctrl_DUMP_LEN11);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl12, MD_Clkctrl_DUMP_LEN12);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl13, MD_Clkctrl_DUMP_LEN13);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl14, MD_Clkctrl_DUMP_LEN14);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_clk_ctl15, MD_Clkctrl_DUMP_LEN15);
	}
	/* 8. Bus REC */
	...
	if (per_md_data->md_dbg_dump_flag & (1 << MD_DBG_DUMP_ECT)) {
		...
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_ect_1, MD_ECT_DUMP_LEN1);
		CCCI_MEM_LOG_TAG(md->index, TAG, "Dump MD ECT 0x%x,len=0x%x\n",
				 MD_ECT_DUMP_ADDR2, MD_ECT_DUMP_LEN2);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_ect_2, MD_ECT_DUMP_LEN2);
		CCCI_MEM_LOG_TAG(md->index, TAG, "Dump MD ECT 0x%x,len=0x%x\n",
				 MD_ECT_DUMP_ADDR3, MD_ECT_DUMP_LEN3);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				   md_reg->md_ect_3, MD_ECT_DUMP_LEN3);
	}
...
static void md1_pcore_sram_turn_on(struct ccci_modem *md)
{
	...
	if (val != golden_val) {
		CCCI_ERROR_LOG(
		    md->index, TAG,
		    "MD_SRAM_PD_PSMCUSYS_SRAM_1 = 0x%X!= 0xFFFFFFFF\n", val);
		error_val = -1;
	}
	...
	if (val != golden_val) {
		CCCI_ERROR_LOG(
		    md->index, TAG,
		    "MD_SRAM_PD_PSMCUSYS_SRAM_2 = 0x%X!= 0xFFFFFFFF\n", val);
		error_val = -2;
	}
	...
	if (val != golden_val) {
		CCCI_ERROR_LOG(
		    md->index, TAG,
		    "MD_SRAM_PD_PSMCUSYS_SRAM_3 = 0x%X!= 0xFFFFFFFF\n", val);
		error_val = -3;
	}
	...
	if (val != golden_val) {
		CCCI_ERROR_LOG(
		    md->index, TAG,
		    "MD_SRAM_PD_PSMCUSYS_SRAM_4 = 0x%X!= 0xFFFFFFFA\n", val);
		error_val = -4;
	}

	if (error_val != 0) {
		CCCI_ERROR_LOG(md->index, TAG,
			       "%s: error_code=%d, trigger AEE db\n", __func__,
			       error_val);
#if defined(CONFIG_MTK_AEE_FEATURE)
...
static void md1_pcore_sram_on(struct ccci_modem *md)
{
	...
	CCCI_NORMAL_LOG(md->index, TAG, "%s exit\n", __func__);
}
...
void md1_pll_on_1(struct ccci_modem *md)
{
	...
	ROr2W(md_pll->md_top_Pll, 0x10,
	      (0x1 << 16)); /* let mdpll on ctrl into software mode */
	ROr2W(md_pll->md_top_Pll, 0x14,
	      (0x1 << 16)); /* let mdpll enable into software mode */
}
...
void md1_pll_on_2(struct ccci_modem *md)
{
	...
	RAnd2W(md_pll->md_top_Pll, 0x10,
	       ~(0x1 << 16)); /* let mdpll on ctrl into hardware mode */
	RAnd2W(md_pll->md_top_Pll, 0x14,
	       ~(0x1 << 16)); /* let mdpll enable into hardware mode */
...
void md1_pll_on(struct ccci_modem *md)
{
	...
	CCCI_DEBUG_LOG(md->index, TAG, "%s:reset0(0x%p)0x%X\n", __func__,
		       map_addr + MDPLL1_CON0,
		       ccci_read32(map_addr, MDPLL1_CON0));

	/* reset MDPLL_CON2 to default value */
	ccci_write32(map_addr, MDPLL1_CON2, 0x4800);
	CCCI_DEBUG_LOG(md->index, TAG, "%s:reset2(0x%p)0x%X\n", __func__,
		       map_addr + MDPLL1_CON2,
		       ccci_read32(map_addr, MDPLL1_CON2));
	...
	CCCI_DEBUG_LOG(md->index, TAG, "%s, (0x%p)0x%X\n", __func__, map_addr,
		       ccci_read32(map_addr, MDPLL1_CON0));
...
void md1_pll_init(struct ccci_modem *md)
{
	...
	/*
	 * From MT6797 and then, PSMCU2EMI bus divider need to be 3,
	 * if it used old value 4 as MT6755, it can be failed in corner case.
	 */
	 ...
	 */
	/* L2DCM L1BUS div 16 */
	ccci_write32(md_pll->md_L1_a0, R_BUSL2DCM_CON3,
		     0x0000FDE7); /* <= 1FDE7 */
	ccci_write32(md_pll->md_L1_a0, R_BUSL2DCM_CON3,
		     0x1000FDE7); /* toggle setting */
	/*
	...
	CCCI_BOOTUP_LOG(md->index, TAG,
			"Wait R_FLEXCKGEN_STS1 & 0x80808080 ready\n");
	...
	CCCI_BOOTUP_LOG(md->index, TAG,
			"Wait R_FLEXCKGEN_STS2 & 0x8080 ready\n");
...
int md_cd_power_on(struct ccci_modem *md)
{
...
#if defined(CONFIG_MTK_MD3_SUPPORT) && (CONFIG_MTK_MD3_SUPPORT > 0)
	reg_value &= ~(0x92); /* md1 set 0x29: bit 0/3/4/7, bit1/5: VRF18 control for MT6755 */
	...
	CCCI_BOOTUP_LOG(
	    md->index, CORE,
	    "%s: set md1_srcclkena bit(0x1000_1F0C)=0x%x\n", __func__,
	...
	ccci_write32(md_info->l1_rgu_base, REG_L1RSTCTL_WDT_MODE,
		     L1_WDT_MD_MODE_KEY);
...
int md_cd_let_md_go(struct ccci_modem *md)
{
	...
	ccci_write32(md_info->md_boot_slave_En, 0,
...
void cldma_dump_register(struct md_cd_ctrl *md_ctrl)
{
	...
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG,
			 ...
			 md_ctrl->txq_active);
	ccci_util_mem_dump(
	    md_ctrl->md_id, CCCI_DUMP_MEM_DUMP,
	...
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG,
			 "dump MD CLDMA Tx pdn register, active=%x\n",
			 md_ctrl->txq_active);
	ccci_util_mem_dump(
	    md_ctrl->md_id, CCCI_DUMP_MEM_DUMP,
	    ...
	    CLDMA_AP_UL_LAST_UPDATE_ADDR_4MSB - CLDMA_AP_UL_START_ADDR_0 + 4);
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG,
			 "dump MD CLDMA Tx ao register, active=%x\n",
			 md_ctrl->txq_active);
	ccci_util_mem_dump(
	    md_ctrl->md_id, CCCI_DUMP_MEM_DUMP,
	...
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG,
			 "dump MD CLDMA Rx pdn register, active=%x\n",
			 md_ctrl->rxq_active);
	ccci_util_mem_dump(md_ctrl->md_id, CCCI_DUMP_MEM_DUMP,
			   md_ctrl->cldma_md_pdn_base + CLDMA_AP_SO_ERROR,
			   CLDMA_AP_SO_STOP_CMD - CLDMA_AP_SO_ERROR + 4);
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG,
			 "dump MD CLDMA Rx ao register, active=%x\n",
			 md_ctrl->rxq_active);
	ccci_util_mem_dump(md_ctrl->md_id, CCCI_DUMP_MEM_DUMP,
	...
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG,
			 "dump MD CLDMA MISC pdn register\n");
	ccci_util_mem_dump(md_ctrl->md_id, CCCI_DUMP_MEM_DUMP,
			   md_ctrl->cldma_md_pdn_base + CLDMA_AP_L2TISAR0,
			   CLDMA_AP_L3TIMSR2 - CLDMA_AP_L2TISAR0 + 4);
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG,
			 "dump MD CLDMA MISC ao register\n");
	ccci_util_mem_dump(md_ctrl->md_id, CCCI_DUMP_MEM_DUMP,
			   md_ctrl->cldma_md_ao_base + CLDMA_AP_L2RIMR0,
			   CLDMA_AP_DUMMY - CLDMA_AP_L2RIMR0 + 4);
}
...
void ccci_modem_restore_reg(struct ccci_modem *md)
{
	...
	if (ccci_read32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_TQSAR(0)) ||
		...
		/* ccci_write32(md_ctrl->cldma_ap_pdn_base,
		 * CLDMA_AP_SO_START_CMD, CLDMA_BM_ALL_QUEUE);
		 */
		/* ccci_read32(md_ctrl->cldma_ap_pdn_base,
		...
		ccci_write32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_L3TIMCR0,
			     CLDMA_BM_INT_ALL);
		ccci_write32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_L3TIMCR1,
			     CLDMA_BM_INT_ALL);
		ccci_write32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_L3RIMCR0,
			     CLDMA_BM_INT_ALL);
		ccci_write32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_L3RIMCR1,
			     CLDMA_BM_INT_ALL);
