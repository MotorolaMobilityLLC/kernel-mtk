...
#ifdef ENABLE_EMI_PROTECTION
/*
 * (D7(MDHW),	D6(MFG),	D5(MD3),	D4(MM),	D3(Resv),D2(CONN),
 * D1(MD1), D0(AP))
 */
#define MPU_ACCESS_PERMISSON_CLEAR                                             \
	SET_ACCESS_PERMISSON(NO_PROTECTION, NO_PROTECTION, NO_PROTECTION,      \
			     NO_PROTECTION, NO_PROTECTION, NO_PROTECTION,      \
			     NO_PROTECTION, NO_PROTECTION)

static const unsigned int
	mpu_attr_default_table[MPU_REGION_ID_TOTAL_NUM][MPU_DOMAIN_ID_TOTAL_NUM] = {
	/*====================================================================*/
	/* No |  | D0(AP)    | D1(MD1)      | D2(CONN) | D3(Res)  | D4(MM)
	 *  | D5(MD3 )      | D6(MFG)    | D7(MDHW)|
	 */
	/*--------------+-----------------------------------------------------*/
	/* 0: set in preloder*/
	{SEC_RW, FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_RW, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN},
	/*1: Preloader*/
	{SEC_RW, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN},
	...
	{SEC_RW, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN, NO_PROTECTION,
	 FORBIDDEN, FORBIDDEN},
	/* 5: CCCI_MD1, set in LK*/
	{NO_PROTECTION, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	...
	{NO_PROTECTION, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 NO_PROTECTION, FORBIDDEN, FORBIDDEN},
	/* 8:   MD1_MCURW_HWRW*/
	{SEC_R_NSEC_R, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN, NO_PROTECTION},
	/* 9: MD1_ROM_DSP, set in LK*/
	{SEC_R_NSEC_R, SEC_R_NSEC_R, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, SEC_R_NSEC_R},
	/*10: MD1_MCURW_HWRO set in LK*/
	{SEC_R_NSEC_R, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN, SEC_R_NSEC_R},
	/*11: MD1_MCURO_HWRW set in kernel*/
	{SEC_R_NSEC_R, SEC_R_NSEC_R, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, NO_PROTECTION},
	/*12: WIFI FW, set in kernel*/
	{FORBIDDEN, FORBIDDEN, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN},
	/*13: WMT set in kernel*/
	{NO_PROTECTION, FORBIDDEN, NO_PROTECTION, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN, FORBIDDEN},
	...
	{SEC_R_NSEC_R, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 SEC_R_NSEC_R, FORBIDDEN, FORBIDDEN},
	/*16: MD3 ROM set in LK*/
	{SEC_R_NSEC_R, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN, SEC_R_NSEC_R,
	 FORBIDDEN, FORBIDDEN},
	/*17: MD3 R/W, set in LK*/
	{SEC_R_NSEC_R, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 NO_PROTECTION, FORBIDDEN, FORBIDDEN},
	/*18: MD Padding1, set in LK*/
	{NO_PROTECTION, NO_PROTECTION, FORBIDDEN, FORBIDDEN, NO_PROTECTION,
	 FORBIDDEN, FORBIDDEN, FORBIDDEN},
	/*19: MD Padding2, set in LK*/
	{NO_PROTECTION, NO_PROTECTION, FORBIDDEN, FORBIDDEN, NO_PROTECTION,
	 FORBIDDEN, FORBIDDEN, FORBIDDEN},
	/*20: MD Protect, set in LK*/
	{NO_PROTECTION, SEC_R_NSEC_R, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN, FORBIDDEN},
	/*21: MD dynamic set in LK*/
	{FORBIDDEN, NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN},
	/*22: GPS EMI, set in Kernel*/
	{NO_PROTECTION, FORBIDDEN, NO_PROTECTION, FORBIDDEN, FORBIDDEN,
	 FORBIDDEN, FORBIDDEN, FORBIDDEN},
	/*23: AP, set in Kernel */
	{NO_PROTECTION, SEC_R_NSEC_R, FORBIDDEN, FORBIDDEN, NO_PROTECTION,
	 FORBIDDEN, NO_PROTECTION, FORBIDDEN},
};
/*=========================================================================*/
...
static const unsigned int mpu_region_info_id[] = {
	MPU_REGION_INFO_ID0, MPU_REGION_INFO_ID1, MPU_REGION_INFO_ID2,
	MPU_REGION_INFO_ID3, MPU_REGION_INFO_ID4, MPU_REGION_INFO_ID5,
	MPU_REGION_INFO_ID6, MPU_REGION_INFO_ID7};
...
void __iomem *ap_sleep_base;
void __iomem *ap_topckgen_base;
atomic_t md_power_on_off_cnt;
#endif

#ifdef ENABLE_EMI_PROTECTION
inline unsigned int EXTRACT_REGION_VALUE(unsigned int domain_set,
					 int region_num)
{
	unsigned int ret;
...
unsigned int CheckHeader_region_attr_paser(struct ccci_modem *md,
					   unsigned region_id)
{
	...
	for (domain_id = 0; domain_id < 8; domain_id++) {
		...
		case CHEAD_MPU_DOMAIN_3:
			/* different domain attr value  is in different
			 * domain_attr[]
			 */
			temp_attr = md->per_md_data.img_info[IMG_MD]
					.rmpu_info
					.domain_attr[domain_attr_id[domain_id]];
			extract_value =
			    EXTRACT_REGION_VALUE(temp_attr, region_id);
			CCCI_DEBUG_LOG(
			    md->index, TAG,
			    "%d,  temp_attr = %X, extract_value= %X\n",
			    domain_attr_id[domain_id], temp_attr,
			    extract_value);
			if ((extract_value >= NO_PROTECTION) &&
			    (extract_value <= FORBIDDEN))
				region_dom[domain_id] = extract_value;
			else
				region_dom[domain_id] = mpu_attr_default_table
				    [mpu_region_info_id[region_id]][domain_id];
	...
	region_attr = SET_ACCESS_PERMISSON(
	    region_dom[7], region_dom[6], region_dom[5], region_dom[4],
...
/* For HW DE Error: initial value of domain is wrong, we add protection 1st
 * stage
 */
void ccci_set_mem_access_protection_1st_stage(struct ccci_modem *md)
{
	...
	case MD_SYS1:
		if (modem_run_env_ready(MD_SYS1)) {
			CCCI_BOOTUP_LOG(
			    md->index, TAG,
			    "Has protected, only MDHW bypass other 1st step\n");
			img_info = &md->per_md_data.img_info[IMG_MD];
			md_layout = &md->mem_layout;
			region_mpu_id = MPU_REGION_ID_MD1_MCURO_HWRW;
			region_mpu_attr = SET_ACCESS_PERMISSON(
			    NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN,
			    FORBIDDEN, FORBIDDEN, NO_PROTECTION, SEC_R_NSEC_R);
			region_mpu_start =
			    md_layout->md_bank0.base_ap_view_phy +
			    img_info->rmpu_info.region_info[2]
				.region_offset; /* Note here!!!!!, 2 */
			region_mpu_end = ((region_mpu_start +
					   img_info->rmpu_info.region_info[2]
					       .region_size /* Note here, 2 */
					   + 0xFFFF) &
					  (~0xFFFF)) -
					 0x1;
			...
			if (mpu_cfg_inf && (mpu_cfg_inf->relate_region != 0)) {
				...
				CCCI_BOOTUP_LOG(md->index, TAG,
						"Relate region<%d> using same "
						"setting(@1st)\n",
						mpu_cfg_inf->relate_region);
			} else
				CCCI_BOOTUP_LOG(
				    md->index, TAG,
				    "No relate region for region<%d>(@1st)\n",
				    region_mpu_id);
			break;
		}
		break;
	case MD_SYS3:
		CCCI_DEBUG_LOG(md->index, CORE,
			       "[error]invalid when MPU protect\n");
		return;
	default:
		CCCI_DEBUG_LOG(md->index, CORE,
			       "[error]invalid when MPU protect\n");
...
void ccci_set_mem_access_protection_second_stage(int md_id)
{
	...
	case MD_SYS1:
		...
		md_layout = &md->mem_layout;
		region_id =
		    MD_SET_REGION_MD1_MCURO_HWRW; /*MPU_REGION_ID_MD1_MCURO_HWRW;*/
		/* set 11 */
		...
		region_mpu_end =
		    ((region_mpu_start +
		      img_info->rmpu_info.region_info[region_id].region_size +
		      0xFFFF) &
		     (~0xFFFF)) -
		    0x1;
...
/*
 * when MD attached its codeviser for debuging, this bit will be set. so CCCI
 * should disable some
...
unsigned int ccci_get_md_debug_mode(struct ccci_modem *md)
{
	...
	if ((debug_setting_flag & DBG_FLAG_JTAG) == 0) {
		...
		if (dbg_spare & MD_DBG_JTAG_BIT) {
			...
			mt_reg_sync_writel(dbg_spare & (~MD_DBG_JTAG_BIT),
					   (dbgapb_base + 0x10));
		}
	}
...
void ccci_get_platform_version(char *ver) { sprintf(ver, "MTxxxx_S00"); }

#ifdef FEATURE_LOW_BATTERY_SUPPORT
static int ccci_md_low_power_notify(struct ccci_modem *md,
				    LOW_POEWR_NOTIFY_TYPE type, int level)
{
	...
	case LOW_BATTERY:
		if (level == LOW_BATTERY_LEVEL_0)
			...
			reserve = (1 << 6); /* 64 */
		ret =
		    port_proxy_send_msg_to_md(md->port_proxy, CCCI_SYSTEM_TX,
					      MD_LOW_BATTERY_LEVEL, reserve, 1);
		if (ret)
			CCCI_ERROR_LOG(
			    md->index, TAG,
	...
	case BATTERY_PERCENT:
		...
		if (ret)
			CCCI_ERROR_LOG(
			    md->index, TAG,
			    "send battery percent notification fail, ret=%d\n",
			    ret);
		break;
	default:
		break;
...
void ccci_reset_ccif_hw(unsigned char md_id, int ccif_id, void __iomem *baseA,
			void __iomem *baseB)
{
	int i;
#if 0
...
#endif
#ifdef FEATURE_LOW_BATTERY_SUPPORT
	register_low_battery_notify(&ccci_md_low_battery_cb,
				    LOW_BATTERY_PRIO_MD);
	register_battery_percent_notify(&ccci_md_battery_percent_cb,
					BATTERY_PERCENT_PRIO_MD);
#endif
	return 0;
}
