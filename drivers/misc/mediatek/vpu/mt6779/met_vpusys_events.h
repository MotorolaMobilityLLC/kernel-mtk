...
TRACE_EVENT(VPU__D2D_enter,
	TP_PROTO(int core, int algo_id, int vcore_opp, int dsp_freq,
		int ipu_if_freq, int dsp1_freq, int dsp2_freq),
	TP_ARGS(core, algo_id, vcore_opp, dsp_freq, ipu_if_freq,
		dsp1_freq, dsp2_freq),
	TP_STRUCT__entry(
		__field(int, core)
		__field(int, algo_id)
	...
	TP_printk("_id=c%da%d, %s=%d, %s=%d, %s=%d, %s=%d, %s=%d",
			__entry->core, __entry->algo_id,
			"vcore_opp", __entry->vcore_opp,
			"dsp_freq", __entry->dsp_freq,
			"ipu_if_freq", __entry->ipu_if_freq,
			"dsp1_freq", __entry->dsp1_freq,
			"dsp2_freq", __entry->dsp2_freq)
);
