				...
				size_t n)
{
	...
	case '9':
		...
		th = kthread_run(chunk_memory_test_inMTEE, NULL, "Chunk Memory test");
		break;
#endif
	case 'B':
...
static int _test_chmem_normalFun(void)
{
	...
	TEST_BEGIN("====> start to test:_test_chmem_normalFun");
	RESET_UNITTESTS;

	KREE_DEBUG("====> start to test:_test_chmem_normalFun\n");



	allocatechm_handle = kmalloc(numOfAllocChm *
						sizeof(KREE_SECUREMEM_HANDLE), GFP_KERNEL);
	...
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("mem_srv CreateSession (appendchm_session:0x%x) Error: %d\n"
	...
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("mem_srv CreateSession (allocatechm_session:0x%x) Error: %d\n"
	...
	for (i = 0; i < numOfAllocChm; i++) {
		...
		/* allocate chm */
		ret = KREE_ION_AllocChunkmem(allocatechm_session, appendchm_handle,
		...
		KREE_DEBUG("[OK]Alloc chunk memory [chm_handle = 0x%x, mem_handle = 0x%x (alignment=%d, size=%d)]\n",
					appendchm_handle, allocatechm_handle[i], alignment, alloc_size);


		/*reference chm*/
		ret = KREE_ION_ReferenceChunkmem(allocatechm_session, allocatechm_handle[i]);
		CHECK_EQ(TZ_RESULT_SUCCESS, ret, "reference chunk memory");
		if (ret != TZ_RESULT_SUCCESS) {
			KREE_ERR("[%s] KREE_ION_ReferenceChunkmem() returns Fail. [Stop!]. ret=0x%x\n"
					, __func__, ret);
			break;
		}
		...
		/*query chm*/
		ret = KREE_ION_QueryChunkmem_TEST(echo_session, allocatechm_handle[i], 0x9995);
		...
		/*unreference chm*/
		ret = KREE_ION_UnreferenceChunkmem(allocatechm_session, allocatechm_handle[i]);
	...
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE_CloseSession Error:appendchm_session=0x%x, ret=%d\n"
		, (uint32_t) appendchm_session, ret);
	}
	...
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE_CloseSession Error:allocatechm_session=0x%x, ret=%d\n"
		, (uint32_t) allocatechm_session, ret);
	}
...
chunkmem_out5:
	...
	KREE_DEBUG("[%s] ====> _test_chmem_normalFun ends ======.\n", __func__);
...
static int _test_chmem_API_in_MTEE(uint32_t cmd)
{
	...
	TEST_BEGIN("====> start to test: _test_chmem_API_in_MTEE");
	RESET_UNITTESTS;

	KREE_DEBUG("====> start to test: _test_chmem_API_in_MTEE\n");

	/*create session for appendCM*/
	ret = KREE_CreateSession(mem_srv_name, &appendchm_session);
	CHECK_EQ(TZ_RESULT_SUCCESS, ret, "create appendchm_session");
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("CreateSession (appendchm_session:0x%x) Error. ret=0x%x\n"
			, (uint32_t) appendchm_session, ret);
		goto chunkmem_out5;
	}
	KREE_DEBUG("[OK] create appendchm_session = 0x%x.\n",
	...
	CHECK_EQ(TZ_RESULT_SUCCESS, ret, "create echo_session");
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("echo_srv CreateSession (echo_session) Error: 0x%x, %d\n"
			, (uint32_t) echo_session, ret);
		goto chunkmem_out5;
	}
	KREE_DEBUG("[OK] create echo_session = 0x%x.\n",
...
chunkmem_out5:
	...
	KREE_DEBUG("====> _test_chmem_API_in_MTEE ends ======.\n");
...
static int _test_chmem_saturation_body(KREE_SESSION_HANDLE allocatechm_session,
							KREE_SECUREMEM_HANDLE appendchm_handle,
							uint32_t chm_size, int alloc_size, int alignment,
							int flags)
{
	/*limit: current chmem impl. need continuous pages*/
	...
	TEST_BEGIN("====> start to test:_test_chmem_saturation_body");
	/*RESET_UNITTESTS;*/

	KREE_DEBUG("====> start to test:_test_chmem_saturation_body\n");
	...
	for (i = 0; i < numOfAllocChm; i++) {
		...
		/* allocate chm */
		if (flags == 0)
			ret = KREE_ION_AllocChunkmem(allocatechm_session,
						appendchm_handle, &allocatechm_handle[i],
						alignment, alloc_size);
		else
			ret = KREE_ION_ZallocChunkmem(allocatechm_session,
						appendchm_handle, &allocatechm_handle[i],
						alignment, alloc_size);
		...
		/*
		 * KREE_DEBUG("[OK]Alloc chmem [chm_handle = 0x%x, mem_handle = 0x%x (alignment=0x%x, size=0x%x)]\n",
		 *		appendchm_handle, allocatechm_handle[i], alignment, alloc_size);
		 */
	...
	/*
	 * KREE_DEBUG("====> _test_chmem_saturation_body ends ======.\n");
	 */
...
static int _test_chmem_saturation(void)
{
	...
	uint32_t chm_size = (10*4096);	/*default test chm size: 10 pages=40960*/
	int alignment = 4096;				/*PAGE_SIZE*/
	int alloc_size = 4096;
	...
	TEST_BEGIN("====> start to test:_test_chmem_saturation");
	RESET_UNITTESTS;

	KREE_DEBUG("====> start to test:_test_chmem_saturation\n");
	...
	_test_chmem_saturation_body(allocatechm_session, appendchm_handle,
				chm_size, alloc_size, alignment, 0);	/*0:no_init0*/
#endif
...
/*ion's test data*/
	...
	for (i = 0; i < param_size; i++) {
		if (alloc_test_params[i].size <= chm_size) {
			KREE_DEBUG("Case1[%d]params: size=0x%x, align=0x%x, init0=0x%x\n",
				i, alloc_test_params[i].size, alloc_test_params[i].alignment,
				alloc_test_params[i].flags);

			_test_chmem_saturation_body(allocatechm_session, appendchm_handle,
				chm_size, alloc_test_params[i].size,
	...
	for (i = 0; i < param_size; i++) {
		if (alloc_zero_test_params[i].size <= chm_size) {
			KREE_DEBUG("Case2[%d]params: size=0x%x, align=0x%x, init0=0x%x\n",
						i, alloc_zero_test_params[i].size,
						alloc_zero_test_params[i].alignment,
						alloc_zero_test_params[i].flags);

			_test_chmem_saturation_body(allocatechm_session, appendchm_handle,
				chm_size, alloc_zero_test_params[i].size,
				alloc_zero_test_params[i].alignment,
				alloc_zero_test_params[i].flags);
		} else
			break;
...
chunkmem_sat:
	...
	KREE_DEBUG("[%s] ====> _test_chmem_saturation ends ======.\n", __func__);
				...
				int flags)
{
	...
	TEST_BEGIN("====> start to test:_test_chmem_boundary_body");
	/*RESET_UNITTESTS;*/

	KREE_DEBUG("====> start to test:_test_chmem_boundary_body\n");

	allocatechm_handle = kmalloc((numOfAllocChm+1) *
						sizeof(KREE_SECUREMEM_HANDLE), GFP_KERNEL);
	...
	/*
	 * KREE_DEBUG("====> _test_chmem_boundary_body ends ======.\n");
	 */
...
static int _test_chmem_boundary(void)
{
	...
	TEST_BEGIN("====> start to test:_test_chmem_boundary");
	RESET_UNITTESTS;

	KREE_DEBUG("====> start to test:_test_chmem_boundary\n");
...
chunkmem_sat:
	...
	KREE_DEBUG("[%s] ====> _test_chmem_boundary ends ======.\n", __func__);
							...
							int flags)
{
	...
	TEST_BEGIN("====> start to test:_test_chmem_alignment_body");
	/*RESET_UNITTESTS;*/

	KREE_DEBUG("====> start to test:_test_chmem_alignment_body\n");
	...
	if (flags == 0)
		...
		ret = KREE_ION_ZallocChunkmem(allocatechm_session, appendchm_handle,
					&allocatechm_handle1, alignment, alloc_size);
	 ...
	 * else
	 *	KREE_DEBUG("[OK]Alloc chmem [chm_handle = 0x%x, mem_handle = 0x%x (alignment=0x%x, size=0x%x)]\n",
	 *			appendchm_handle, allocatechm_handle1, alignment, alloc_size);
	 */
	 ...
	 * else
	 *	KREE_DEBUG("[OK]Alloc chmem [chm_handle = 0x%x, mem_handle = 0x%x (alignment=0x%x, size=0x%x)]\n",
	 *			appendchm_handle, allocatechm_handle2, alignment, alloc_size);
	 */

	/*unreference chm1: free*/
	ret = KREE_ION_UnreferenceChunkmem(allocatechm_session, allocatechm_handle1);
	CHECK_EQ(TZ_RESULT_SUCCESS, ret, "unreference chunk memory");
	if (ret != TZ_RESULT_SUCCESS) {
	...
	ret = KREE_ION_UnreferenceChunkmem(allocatechm_session, allocatechm_handle2);
	CHECK_EQ(TZ_RESULT_SUCCESS, ret, "unreference chunk memory");
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("[%s] KREE_ION_UnreferenceChunkmem() returns Fail. [Stop!]. ret=0x%x\n"
				, __func__, ret);
	}
	/*
	 * else
	...
	/*
	 * KREE_DEBUG("====> _test_chmem_alignment_body ends ======.\n");
	 */
...
static int _test_chmem_alignment(void)
{
	...
	TEST_BEGIN("====> start to test:_test_chmem_alignment");
	RESET_UNITTESTS;

	KREE_DEBUG("====> start to test:_test_chmem_alignment\n");
...
chunkmem_sat:
	...
	KREE_DEBUG("====> _test_chmem_alignment ends ======.\n");
