...
static void test_hw_cnt(void)
{
	...
	EXPECT_NEQ(0, f, "MTEE_GetSystemCntFreq");

	cnt = KREE_GetSystemCnt();
	KREE_DEBUG("KREE_GetSystemCnt: %llu\n", cnt);
	EXPECT_NEQ(0, cnt, "MTEE_GetSystemCnt");
...
static void test_gz_syscall(void)
{
	...
	EXPECT_EQ(TZ_RESULT_ERROR_COMMUNICATION, ret, "connect to unknown server");

	/* null checking */
	ret = KREE_CreateSession(echo_srv_name, NULL);
	EXPECT_EQ(TZ_RESULT_ERROR_BAD_PARAMETERS, ret, "create session null checking");

	/* connect to the same server multiple times*/
	ret = KREE_CreateSession(echo_srv_name, &session2);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "create echo srv session 2");
	...
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "test TA syscall");

	if (ret != TZ_RESULT_SUCCESS) {
		KREE_DEBUG("KREE_TeeServiceCall Error: handle 0x%x, ret %d\n", (uint32_t)session, ret);
	} else {
		tmp = strcmp((char *)param[1].mem.buffer, "sample data 1!!");
		EXPECT_EQ(0, tmp, "check gp param: mem output");
		tmp = check_gp_inout_mem(buf2);
		EXPECT_EQ(0, tmp, "check gp param: mem inout");
		EXPECT_EQ(99, param[3].value.a, "check gp param: value output");
	}
	ret = KREE_CloseSession(session);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "close echo srv session");
	ret = KREE_CloseSession(session2);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "close echo srv session 2");
...
static void test_gz_mem_api(void)
{
	...
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "alloc secure mem 128");
	KREE_DEBUG("[GZTEST]KREE_AllocSecuremem handle = %d.\n", mem_handle[0]);

	ret = KREE_AllocSecuremem(mem_session, &mem_handle[1], 0, 512);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "alloc secure mem 512");
	KREE_DEBUG("[GZTEST]KREE_AllocSecuremem handle = %d.\n", mem_handle[1]);

	ret = KREE_AllocSecuremem(mem_session, &mem_handle[2], 0, 1024);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "alloc secure mem 1024");
	KREE_DEBUG("[GZTEST]KREE_AllocSecuremem handle = %d.\n", mem_handle[2]);

	ret = KREE_ZallocSecurememWithTag(mem_session, &mem_handle[3], 0, 2048);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "zero alloc secure mem 2048");
	KREE_DEBUG("[GZTEST]KREE_ZallocSecuremem handle = %d.\n", mem_handle[3]);


	ret = KREE_ReferenceSecuremem(mem_session, mem_handle[0]);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "reference secure mem 1");
	ret = KREE_ReferenceSecuremem(mem_session, mem_handle[0]);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "reference secure mem 2");

	ret = KREE_UnreferenceSecuremem(mem_session, mem_handle[0]);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "unreference secure mem 1");
	ret = KREE_UnreferenceSecuremem(mem_session, mem_handle[0]);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "unreference secure mem 2");
	ret = KREE_UnreferenceSecuremem(mem_session, mem_handle[0]);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "unreference secure mem 3");
...
static int sync_test(void)
{
	...
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "create echo srv session");
	EXPECT_GT_ZERO(sessionHandle, "check echo srv session value");

	/* Connect to sync-ut service */
	ret = KREE_CreateSession(APP_NAME2, &sessionHandle2);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "create sync-ut srv session");
	EXPECT_GT_ZERO(sessionHandle, "check echo sync-ut session value");
	...
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "get mutex handle from TA1");

	EXPECT_GT_ZERO(param[0].value.a, "check mutex value");
	...
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "send mutex handle to TA2");

	/* start mutex test */
	ret = KREE_TeeServiceCall(sessionHandle, TZCMD_TEST_MUTEX, types, param);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "send start cmd to TA1");
	ret = KREE_TeeServiceCall(sessionHandle2, TZCMD_TEST_MUTEX, types, param);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "send start cmd to TA2");

	ret = KREE_CloseSession(sessionHandle);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "close echo srv session");
	ret = KREE_CloseSession(sessionHandle2);
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "close echo sync-ut session");
...
int shm_mem_service_process(KREE_SHAREDMEM_PARAM *shm_param, int numOfPA)
{
...
int _registerSharedmem_body(KREE_SESSION_HANDLE shm_session,
...
static uint64_t init_shm_test(char **buf_p, int size)
{
	...
	*buf_p = kmalloc(size, GFP_KERNEL);
	...
	EXPECT_EQ((size-1), stat[0], "input string a");
	EXPECT_EQ(0, stat[1], "input string b");
	TEST_END;
	return pa;
}
...
static void verify_shm_result(char *buf, int size)
{
	...
	EXPECT_EQ(0, stat[0], "input string a");
	EXPECT_EQ((size-1), stat[1], "input string b");

	TEST_END;
}

int gz_test_shm_case1(void *arg)
{
	uint64_t pa;
	char *buf = NULL;
	int numOfPA = 0;
	...
	EXPECT_EQ(TZ_RESULT_SUCCESS, ret, "shared memory process sys call. ");
	if (ret != TZ_RESULT_SUCCESS) {
...
int gz_test_shm_case2(void *arg)
{
	TZ_RESULT ret;
	KREE_SHAREDMEM_PARAM shm_param;
...
int gz_test_shm_case3(void *arg)
{
	...
	EXPECT_EQ(numOfChar, stat[0], "input string a");
	EXPECT_EQ(0, stat[1], "input string b");
	...
	EXPECT_EQ(0, stat[0], "input string a");
	EXPECT_EQ(numOfChar, stat[1], "input string b");
