...
signed int use_chip_trim_value(signed int not_trim_val)
{
	...
	bm_debug("[use_chip_trim_value] %d -> %d\n", not_trim_val, ret_val);
...
static signed int fgauge_read_current(void *data)
{
	...
	*(signed int *) (data) = 0;
	...
	bm_debug("[fgauge_read_current] : FG_CURRENT = %x\r\n", uvalue16);
	/*
	 *(5)    (Read other data)
	 *(6)    Clear status to 0
	 */
	...
	if (g_fg_is_charging == true)
		bm_debug("[fgauge_read_current] current(charging) = %d mA\r\n",
			 dvalue);
	else
		bm_debug("[fgauge_read_current] current(discharging) = %d mA\r\n",
			 dvalue);
		...
		if (fg_cust_data.r_fg_value != 100) {
			...
			bm_info("[fgauge_read_current] new current=%d\n", dvalue);
		}
...
signed int fgauge_read_IM_current(void *data)
{
		...
		*(signed int *) (data) = 0;
		...
		bm_info("[fgauge_read_IM_current] : FG_CURRENT = %x\r\n",
			 uvalue16);
		...
		if (fg_cust_data.r_fg_value != 100) {
			...
			bm_info("[fgauge_read_IM_current] new current=%d\n",
			dvalue);
		}

		bm_info("[fgauge_read_IM_current] ori current=%d\n", dvalue);

		dvalue = ((dvalue * fg_cust_data.car_tune_value) / 1000);

		bm_info("[fgauge_read_IM_current] final current=%d (ratio=%d)\n",
			 dvalue, fg_cust_data.car_tune_value);
...
static signed int fg_get_current_iavg(void *data)
{
	...
	if (pmic_get_register_value(PMIC_FG_IAVG_VLD) == 1) {
		...
		bm_notice("[fg_get_current_iavg] fg_iavg_ma %lld fg_iavg_reg %lld fg_iavg_reg_tmp %lld\n",
			fg_iavg_ma, fg_iavg_reg, fg_iavg_reg_tmp);

		do_div(fg_iavg_ma, 1000000);
		bm_notice("[fg_get_current_iavg] fg_iavg_ma %lld\n", fg_iavg_ma);
		...
		bm_notice("[fg_get_current_iavg] fg_iavg_ma %lld fg_iavg_reg %lld r_fg_value %d 27_16 0x%x 15_00 0x%x\n",
		...
		bm_notice("[fg_get_current_iavg] PMIC_FG_IAVG_VLD == 1\n");
	} else {
...
signed int fgauge_set_columb_interrupt_internal2(void *data, int is_ht)
{
	...
	if (is_ht == 1)
		bm_notice("fgauge_set_columb_interrupt_internal2 ht_thr %d\n", thr);
	if (is_ht == 0)
		bm_notice("fgauge_set_columb_interrupt_internal2 lt_thr %d\n", thr);
...
signed int fgauge_set_columb_interrupt_internal1(void *data, int reset)
{
	...
	bm_info("fgauge_set_columb_interrupt_internal1 car=%d\n", car);
...
*/
	...
	bm_info("[fgauge_set_columb_interrupt_internal1] upper = 0x%x:%d  low=0x%x:%d  diff_car=0x%x:%d\r\n",
...
*/
	...
	bm_debug("[fgauge_read_columb_internal] temp_CAR_15_0 = 0x%x  temp_CAR_31_16 = 0x%x\n",
...
static signed int fgauge_hw_reset(void *data)
{
	...
	bm_debug("[fgauge_hw_reset] : Start \r\n");
...
static signed int read_fg_hw_info(void *data)
{
	...
	*(signed int *) (data) = 0;
	pr_err("[FGADC_intr_end][%s][read_fg_hw_info] curr_1 %d curr_2 %d Iavg %d sign %d car %d ncar %d time %d\n",
...
static signed int read_nafg_vbat(void *data)
{
	...
	*(signed int *) (data) = nag_vbat_mv;
	...
	pr_err("[FGADC_intr_end][read_nafg_vbat] nag_vbat_reg 0x%x nag_vbat_mv %d\n",
...
static signed int fg_get_nafg_cnt(void *data)
{
	...
	*(signed int *) (data) = (NAG_C_DLTV_CNT & 0xffff) + ((NAG_C_DLTV_CNT_H & 0x3ff) << 16);
	...
	bm_info("[fg_bat_nafg][fgauge_get_nafg_cnt] %d [25_16 %d 15_0 %d]\n",
...
static signed int fg_get_nafg_dltv(void *data)
{
	...
	*(signed int *) (data) = NAG_DLTV_mV_value;
	...
	bm_info("[fg_bat_nafg][fgauge_get_nafg_dltv] mV:Reg [%d:%d]\n", NAG_DLTV_mV_value, NAG_DLTV_reg_value);
...
static signed int fg_get_nafg_c_dltv(void *data)
{
	...
	*(signed int *) (data) = NAG_C_DLTV_mV_value;
	...
	bm_info("[fg_bat_nafg][fgauge_get_nafg_c_dltv] mV:Reg [%d:%d] [26_16 %d 15_00 %d]\n",
...
void fg_set_nafg_intr_internal(int _prd, int _zcv_mv, int _thr_mv)
{
	...
	bm_warn("[fg_bat_nafg][fgauge_set_nafg_interrupt_internal] time[%d] zcv[%d:%d] thr[%d:%d] 26_16[0x%x] 15_00[0x%x]\n",
...
void fg_set_zcv_intr_internal(int fg_zcv_det_time, int fg_zcv_car_th)
{
	...
	bm_notice("[FG_ZCV_INT][fg_set_zcv_intr_internal] det_time %d mv %d reg %lld 30_16 0x%x 15_00 0x%x\n",
...
static signed int fg_set_cycle_interrupt(void *data)
{
	...
	pr_err("car:%d carR:%lld r:%lld current:low:0x%x high:0x%x target:low:0x%x high:0x%x",
...
{
	int fg_reset_rtc_status = *(signed int *) (data);
	int before_reset_val;
	int after_reset_val;
	int value;

	if (fg_reset_rtc_status == 1) {
		before_reset_val = hal_rtc_get_spare_register(RTC_FG_INIT);
		value = before_reset_val | (1<<7);

		hal_rtc_set_spare_register(RTC_FG_INIT, value);
		mdelay(1);
		...
		pr_err("[fg_set_fg_reset_rtc_status][RTC_func] 0x%x 0x%x\n", before_reset_val, after_reset_val);
	}

	return STATUS_OK;
}

static signed int fg_set_fg_reset_status(void *data)
{
...
static signed int fg_get_fg_reset_status(void *data)
{
	int fg_reset_status = pmic_get_register_value(PMIC_FG_RSTB_STATUS);
...
{
	unsigned int is_bat_plugout = 0;
	int hw_id = upmu_get_reg_value(0x0200);
	unsigned int temp_data;
	unsigned int temp_data2;
	unsigned int temp_bit_13;
	unsigned int rtc_invalid;

	temp_data = hal_rtc_get_spare_register(RTC_FG_INIT);
	temp_data2 = get_rtc_spare_fg_value();
	rtc_invalid = temp_data2 >> 7;

	if (rtc_invalid == 0) {
		temp_bit_13 = (temp_data & 0x20) >> 5;

		if ((hw_id & 0xff00) == 0x3500)
			is_bat_plugout = temp_bit_13;
		else
			is_bat_plugout = ~temp_bit_13;

	} else {
		/* if rtc invalid, battery was plugouted */
		is_bat_plugout = 1;
		set_rtc_spare_fg_value(0);	/* Clear RTC */
	}

	*(unsigned int *) (data) = is_bat_plugout;
	bm_err("[read_boot_battery_plug_out_status][RTC_func] is_bat_plugout %d rtc_invalid %d temp_data 0x%x temp_data2 0x%x\n",
			is_bat_plugout, rtc_invalid, temp_data, temp_data2);
...
{
	unsigned int bat_plug_out_time = 32;
	unsigned int is_bat_plugout;
	unsigned int temp_data;
	unsigned int temp_data2;
	unsigned int rtc_invalid;
	...
	if (rtc_invalid == 0) {
		is_bat_plugout = (temp_data & 0x20) >> 5;
		bat_plug_out_time = temp_data & 0x1f;
	} else {
		is_bat_plugout = 1;
		bat_plug_out_time = 31;	/*[12:8], 5 bits*/
		/*set_rtc_spare_fg_value(0);*/	/* Reset in read_boot_battery_plug_out_status() */

	}

	*(unsigned int *) (data) = bat_plug_out_time;

	bm_err("[read_bat_plug_out_time][RTC_func] bat_plug_out_time %d rtc_invalid %d is_bat_plugout %d temp_data 0x%x temp_data2 0x%x\n",
			bat_plug_out_time, rtc_invalid, is_bat_plugout, temp_data, temp_data2);
...
static signed int fgauge_get_zcv(void *data)
{
		...
		adc_result = REG_to_MV_value(adc_result_reg);
		bm_warn("[oam] fgauge_get_zcv BATSNS (swchr) : adc_result_reg=%d, adc_result=%d\n",
			 adc_result_reg, adc_result);
#else
		adc_result_reg = pmic_get_register_value(PMIC_AUXADC_ADC_OUT_FGADC1);
		adc_result = REG_to_MV_value(adc_result_reg);
		bm_warn("[oam] fgauge_get_zcv BATSNS  (pchr) : adc_result_reg=%d, adc_result=%d\n",
			 adc_result_reg, adc_result);
#endif
		adc_result += g_hw_ocv_tune_value;
...
signed int bm_ctrl_cmd(BATTERY_METER_CTRL_CMD cmd, void *data)
{
	...
	if (init == -1) {
		...
		bm_func[BATTERY_METER_CMD_GET_FG_RESET_STATUS] = fg_get_fg_reset_status;
