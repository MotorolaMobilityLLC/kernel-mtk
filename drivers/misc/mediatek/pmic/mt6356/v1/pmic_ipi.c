	...
	case MAIN_PMIC_REGULATOR:
		if (ret_val) {
			if (ret_val == IPI_BUSY ||
				 ...
				 */
				pr_debug(
				    "%s ap_ret_mreg = %d ipi_ret_mreg =%d\n",
				    __func__, ret_val, ipi_ret);
		} else {
			if (ipi_ret != 0)
				pr_debug(
	...
	case MT6311_FPWM:
		if (ret_val) {
			if (ret_val == IPI_BUSY ||
			    ret_val == IPI_TIMEOUT_ACK) {
				if (ipi_ret != 0)
					pr_debug("%s ap_ret_mt6311 = %d ipi_ret_mt6311 =%d\n",
					       __func__, ret_val, ipi_ret);
			} else
				/* Real PMIC service execution result, by each
				 * PMIC service
...
static unsigned int pmic_interface_test_code(void)
{
	...
	for (i = 0; i < 30; i++) {
		ret_val = pmic_ipi_config_interface(
		    PMIC_DEW_WRITE_TEST_ADDR, test_data[i], 0xffff, 0, 1);
		...
		if (ret_val != 0 || rdata != test_data[i]) {
			pr_debug(
			       "%s read failed: test_data[%d]=%x rdata =%x ret_val=%x\n",
