...
static int pmic_auxadc_notify_call(struct notifier_block *nb, unsigned long id,
				   void *unused)
{
#if 0
	...
	case NOTIFY_SOIDLE3_ENTER:
		/* Turn off AUXADC BAT TEMP Background detection,
		 * because VBIF28 will be disabled
		 */
...
void pmic_auxadc_resume(void) { wk_auxadc_bgd_ctrl(1); }
...
int wk_auxadc_battmp_dbg(int bat_temp)
{
	...
	pr_notice(
		  "BAT_TEMP1: %d, BAT_TEMP2:%d, VBIF:%d, BAT_TEMP3:%d, BATID:%d, DA_VBIF28_STB:%d\n",
		  bat_temp, bat_temp2, vbif, bat_temp3, bat_id,
		  pmic_get_register_value(PMIC_DA_VBIF28_STB));

	if (bat_temp < 200 || (battmp != 0 && (bat_temp - battmp > 100 ||
					       battmp - bat_temp > 100))) {
		wk_auxadc_dbg_dump();
		for (i = 0; i < 5; i++)
			arr_bat_temp[i] =
		...
		pr_notice("%d,%d,%d,%d,%d, BAT_TEMP_NEW:%d\n", arr_bat_temp[0],
			  arr_bat_temp[1], arr_bat_temp[2], arr_bat_temp[3],
			  arr_bat_temp[4], bat_temp);
...
void lockadcch3(void) { mutex_lock(&auxadc_ch3_mutex); }

void unlockadcch3(void) { mutex_unlock(&auxadc_ch3_mutex); }
...
void mt6356_auxadc_monitor_mts_regs(void)
{
	...
	if (mts_count >= 7 && mts_count < 9) {
		...
		udelay(10);
		while (pmic_get_register_value(
			   PMIC_AUXADC_ADC_RDY_CH7_BY_GPS) != 1) {
		...
		udelay(10);
		while (pmic_get_register_value(
			   PMIC_AUXADC_ADC_RDY_CH7_BY_MD) != 1) {
			usleep_range(1300, 1500);
			if ((count++) > count_time_out)
				break;
		}
...
int mts_kthread(void *x)
{
	...
	while (1) {
		...
		while (mts_adc == ktrhead_mts_adc) {
			while (pmic_get_register_value(
				   PMIC_AUXADC_ADC_RDY_MDRT) == 1) {
				i++;
				mdelay(1);
			}
			while (pmic_get_register_value(
				   PMIC_AUXADC_ADC_RDY_MDRT) == 0) {
				i++;
				mdelay(1);
			}
			ktrhead_mts_adc =
			...
			if (polling_cnt % 20 == 0) {
				pr_notice(
				    "[MTS_ADC] RDY=1 at %dms, AUXADC_ADC36=0x%x, MDRT_OUT=%d\n",
				    i, upmu_get_reg_value(MT6356_AUXADC_ADC36),
				    ktrhead_mts_adc);
			}
...
int mt6356_get_auxadc_value(u8 channel)
{
	...
	if (channel - AUXADC_LIST_START < 0 || channel - AUXADC_LIST_END > 0) {
		pr_debug("[%s] Invalid channel(%d)\n", __func__, channel);
		return -EINVAL;
	}
	auxadc_channel = &mt6356_auxadc_channel[channel - AUXADC_LIST_START];
	...
	if (channel == AUXADC_LIST_VBIF) {
		if (pmic_get_register_value(PMIC_BATON_TDET_EN)) {
			tdet_tmp = 1;
			pr_notice(
			    "pmic auxadc baton_tdet_en should be zero\n");
	...
	while (pmic_get_register_value(auxadc_channel->channel_rdy) != 1) {
		...
		if ((count++) > count_time_out) {
			...
			pr_debug(
			    "[%s] Reg[0x%x]=0x%x, RG_AUXADC_CK_PDN_HWEN=%d, RG_AUXADC_CK_TSTSEL=%d, RG_SMPS_CK_TSTSEL=%d\n",
			    __func__, MT6356_STRUP_CON6,
	...
	if (auxadc_channel->resolution == 12)
		adc_result =
		    (reg_val * auxadc_channel->r_val * VOLTAGE_FULL_RANGE) /
		    4096;
	...
	if (channel != AUXADC_LIST_MT6356_BUCK1_TEMP &&
	    channel != AUXADC_LIST_MT6356_BUCK2_TEMP) {
		if (__ratelimit(&ratelimit)) {
			if (channel == AUXADC_LIST_BATTEMP) {
				...
				if (boot_time)
					pr_notice(
					    "[%s] ch_idx = %d, channel = %d, bat_cur = %d, reg_val = 0x%x, adc_result = %d\n",
					    __func__, channel,
					    auxadc_channel->ch_num, bat_cur,
					    reg_val, adc_result);
			} else {
				if (boot_time)
					pr_notice(
					    "[%s] ch_idx = %d, channel = %d, reg_val = 0x%x, adc_result = %d\n",
					    __func__, channel,
					    auxadc_channel->ch_num, reg_val,
					    adc_result);
			}
		}
	} else {
		PMICLOG(
			"[%s] ch_idx = %d, channel = %d, reg_val = 0x%x, adc_result = %d\n",
			__func__, channel, auxadc_channel->ch_num, reg_val,
			adc_result);
	}
...
{
	mts_thread_handle =
	    kthread_create(mts_kthread, (void *)NULL, "mts_thread");
	if (IS_ERR(mts_thread_handle)) {
		mts_thread_handle = NULL;
		pr_debug(PMICTAG "[adc_kthread] creation fails\n");
	} else {
	...
	if (g_pmic_pad_vbif28_vol < 2500) {
		for (i = 0; i < 5; i++) {
			...
			if (pmic_get_register_value(
				PMIC_RG_INT_RAW_STATUS_VBIF28_OC) != 1)
				break;
			pmic_set_register_value(PMIC_RG_INT_STATUS_VBIF28_OC,
						1);
			mdelay(1);
		}
		if (i >= 5) {
