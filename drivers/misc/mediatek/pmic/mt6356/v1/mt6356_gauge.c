...
{
	/*int ret = (_reg * VOLTAGE_FULL_RANGE * 10 * R_VAL_TEMP_3) /
	 * ADC_PRECISE;
	 */
...
#else
	_reg64 = div_s64(_reg64 * VOLTAGE_FULL_RANGE * 10 * R_VAL_TEMP_3,
			 ADC_PRECISE);
#endif
	ret = _reg64;
...
#if defined(__LP64__) || defined(_LP64)
	_reg64 =
	    (_reg64 * ADC_PRECISE) / (VOLTAGE_FULL_RANGE * 10 * R_VAL_TEMP_3);
#else
...
static int fgauge_set_info(struct gauge_device *gauge_dev,
			   enum gauge_info ginfo, int value)
{
	int ret = 0;

	if (ginfo == GAUGE_2SEC_REBOOT)
		pmic_config_interface(PMIC_RG_SYSTEM_INFO_CON0_ADDR, value,
				      0x0001, 0x0);
...
static int fgauge_get_info(struct gauge_device *gauge_dev,
			   enum gauge_info ginfo, int *value)
{
	int ret = 0;

	if (ginfo == GAUGE_2SEC_REBOOT)
		pmic_read_interface(PMIC_RG_SYSTEM_INFO_CON0_ADDR, value,
				    0x0001, 0x0);
	else if (ginfo == GAUGE_PL_CHARGING_STATUS)
		pmic_read_interface(PMIC_RG_SYSTEM_INFO_CON0_ADDR, value,
				    0x0001, 0x1);
...
static unsigned int fg_get_data_ready_status(void)
{
	...
	ret = pmic_read_interface(PMIC_FG_LATCHDATA_ST_ADDR, &temp_val, 0xFFFF,
				  0x0);

	temp_val = (temp_val & (PMIC_FG_LATCHDATA_ST_MASK
...
static void read_fg_hw_info_Iavg(struct gauge_device *gauge_dev,
				 int *is_iavg_valid)
{
	...
	*is_iavg_valid = valid_bit;
	...
	bm_debug("[%s] fg_iavg_reg 0x%llx fg_iavg_reg_tmp 0x%llx 27_16 0x%x 15_00 0x%x\n",
		 __func__, fg_iavg_reg, fg_iavg_reg_tmp, fg_iavg_reg_27_16,
		 fg_iavg_reg_15_00);
	bm_debug(
...
static signed int fg_get_current_iavg(struct gauge_device *gauge_dev,
				      int *data)
{
	long long fg_iavg_reg = 0;
	long long fg_iavg_reg_tmp = 0;
	...
	if (pmic_get_register_value(PMIC_FG_IAVG_VLD) == 1) {
		...
		bm_debug(
		    "[%s] PMIC_FG_IAVG_VLD != 1, avg %d, current_1 %d\n",
		    __func__,
		    gauge_dev->fg_hw_info.current_avg,
		    gauge_dev->fg_hw_info.current_1);
	}
...
static signed int fg_set_iavg_intr(struct gauge_device *gauge_dev, void *data)
{
	...
	fg_iavg_reg_ht =
	    iavg_ht * 1000 * 1000 * gauge_dev->fg_cust_data->r_fg_value;
	if (fg_iavg_reg_ht < 0) {
		sign_bit_ht = 1;
	...
	fg_iavg_reg_lt =
	    iavg_lt * 1000 * 1000 * gauge_dev->fg_cust_data->r_fg_value;
	if (fg_iavg_reg_lt < 0) {
		sign_bit_lt = 1;
	...
	bm_debug(
		 "[FG_IAVG_INT][%s] iavg %d iavg_gap %d iavg_ht %lld iavg_lt %lld fg_iavg_reg_ht %lld fg_iavg_reg_lt %lld\n",
		 __func__, iavg, iavg_gap, iavg_ht, iavg_lt, fg_iavg_reg_ht,
		 fg_iavg_reg_lt);
	bm_debug(
		 "[FG_IAVG_INT][%s] lt_28_16 0x%x lt_15_00 0x%x ht_28_16 0x%x ht_15_00 0x%x\n",
...
/* 0.1 mAh */
	...
	if (uvalue32_NCAR_MSB == 0x1)
		dvalue_NCAR =
		    (signed int)(Temp_Value -
				 (Temp_Value * 2)); /* keep negative value */
	else
	...
	if (gauge_dev->fg_cust_data->r_fg_value != 100)
		dvalue_NCAR =
		    (dvalue_NCAR * 100) / gauge_dev->fg_cust_data->r_fg_value;

	gauge_dev->fg_hw_info.ncar =
	    ((dvalue_NCAR * gauge_dev->fg_cust_data->car_tune_value) / 1000);
}
			       ...
			       bool *fg_is_charging, int *data)
{
	 ...
	 */
	uvalue16 = pmic_get_register_value(
	    PMIC_FG_CURRENT_OUT); /*mt6325_upmu_get_fg_current_out(); */
...
static int fgauge_get_average_current(struct gauge_device *gauge_dev,
				      int *data, bool *valid)
{
	long long fg_iavg_reg = 0;
	long long fg_iavg_reg_tmp = 0;
...
static int fgauge_get_coulomb(struct gauge_device *gauge_dev, int *data)
{
	...
	*data = bq27531_get_remaincap();
	...
	if (reset == 0)
		ret = pmic_config_interface(MT6356_FGADC_CON1, 0x0001, 0x1F05,
					    0x0);
	else {
		ret = pmic_config_interface(MT6356_FGADC_CON1, 0x0705, 0x1F05,
					    0x0); /*weiching0803*/
		bm_err("[%s] reset fgadc 0x0705\n", __func__);
	}
	...
	while (fg_get_data_ready_status() == 0) {
		m++;
		if (m > 1000) {
			bm_err("[%s] fg_get_data_ready_status timeout 1 !\r\n",
			__func__);
			break;
		}
	}
	...
	bm_trace("[%s] FG_CAR = 0x%x\r\n", __func__,
		 uvalue32_CAR);
	bm_trace("[%s] uvalue32_CAR_MSB = 0x%x\r\n", __func__,
		 uvalue32_CAR_MSB);
	...
	while (fg_get_data_ready_status() != 0) {
		m++;
		if (m > 1000) {
			bm_err("[%s] fg_get_data_ready_status timeout 2 !\r\n",
			__func__);
			break;
		}
	}
...
/* 0.1 mAh */
	...
	if (gauge_dev->fg_cust_data->r_fg_value != 100) {
		bm_trace(
			 ...
			 __func__, gauge_dev->fg_cust_data->r_fg_value,
			 dvalue_CAR);

		dvalue_CAR =
		    (dvalue_CAR * 100) / gauge_dev->fg_cust_data->r_fg_value;

		bm_trace("[%s] new CAR=%d\n", __func__,
			 dvalue_CAR);
	}

	dvalue_CAR =
	    ((dvalue_CAR * gauge_dev->fg_cust_data->car_tune_value) / 1000);
...
static int fgauge_reset_hw(struct gauge_device *gauge_dev)
{
	...
	while (val_car != 0x0) {
		ret = pmic_config_interface(MT6356_FGADC_CON1, 0x0600, 0x1F00,
					    0x0);
...
static int read_hw_ocv_6356_plug_in(void)
{
	...
	adc_rdy =
	    pmic_get_register_value(PMIC_AUXADC_ADC_RDY_BAT_PLUGIN_SWCHR);
	adc_result_reg =
	    pmic_get_register_value(PMIC_AUXADC_ADC_OUT_BAT_PLUGIN_SWCHR);
	...
	adc_result_reg =
	    pmic_get_register_value(PMIC_AUXADC_ADC_OUT_BAT_PLUGIN_PCHR);
	adc_result = REG_to_MV_value(adc_result_reg);
...
#if defined(SWCHR_POWER_PATH) || defined(CONFIG_MTK_SWCHR_SUPPORT)
	adc_result_rdy =
	    pmic_get_register_value(PMIC_AUXADC_ADC_RDY_PWRON_SWCHR);
	adc_result_reg =
	    pmic_get_register_value(PMIC_AUXADC_ADC_OUT_PWRON_SWCHR);
	adc_result = REG_to_MV_value(adc_result_reg);
	bm_debug(
	    "[oam] %s (swchr) : adc_result_reg=%d, adc_result=%d, start_sel=%d, rdy=%d\n",
	    __func__, adc_result_reg, adc_result,
	    pmic_get_register_value(PMIC_RG_STRUP_AUXADC_START_SEL),
	    adc_result_rdy);
...
int read_hw_ocv_6336_charger_in(void)
{
	...
	zcv_36_rdy =
	    mt6336_get_flag_register_value(MT6336_AUXADC_ADC_RDY_WAKEUP1);
	zcv_chrgo_1_rdy =
	    mt6336_get_flag_register_value(MT6336_AUXADC_ADC_RDY_CHRGO1);
	zcv_fgadc1_rdy =
	    mt6336_get_flag_register_value(MT6336_AUXADC_ADC_RDY_FGADC1);
	...
	if (1) {
		...
		bm_err("[%s] zcv_36_rdy %d hw_ocv_36_1 %d [0x%x:0x%x]\n",
		       __func__, zcv_36_rdy, hw_ocv_36_1, zcv_36_low,
		       zcv_36_high);
	...
	if (1) {
		...
		bm_err("[%s] zcv_chrgo_1_rdy %d hw_ocv_36_2 %d [0x%x:0x%x]\n",
		       __func__, zcv_chrgo_1_rdy, hw_ocv_36_2, zcv_chrgo_1_lo,
		       zcv_chrgo_1_hi);
		mt6336_set_flag_register_value(MT6336_AUXADC_ADC_RDY_CHRGO_CLR,
					       1);
		mdelay(1);
		mt6336_set_flag_register_value(MT6336_AUXADC_ADC_RDY_CHRGO_CLR,
					       0);
	...
	if (1) {
		...
		bm_err(
		    "[%s] FGADC1 %d hw_ocv_36_3 %d [0x%x:0x%x]\n", __func__,
		    zcv_fgadc1_rdy, hw_ocv_36_3, zcv_fgadc1_lo, zcv_fgadc1_hi);
		mt6336_set_flag_register_value(MT6336_AUXADC_ADC_RDY_FGADC_CLR,
					       1);
		mdelay(1);
		mt6336_set_flag_register_value(MT6336_AUXADC_ADC_RDY_FGADC_CLR,
					       0);
...
int read_hw_ocv(struct gauge_device *gauge_dev, int *data)
{
	...
	if (_hw_ocv < 30000) {
		bm_err("[%s] ERROR, _hw_ocv=%d, force use swocv\n",
		       __func__, _hw_ocv);
		_hw_ocv = _sw_ocv;
		_hw_ocv_src = FROM_SW_OCV;
	}
	...
	*data = _hw_ocv;
	...
	bm_err(
	       "[%s] _hw_ocv %d _hw_ocv_src %d _prev_hw_ocv %d _prev_hw_ocv_src %d _flag_unreliable %d\n",
	       __func__, _hw_ocv, _hw_ocv_src, _prev_hw_ocv, _prev_hw_ocv_src,
	       _flag_unreliable);
	bm_debug(
	    "[%s] _hw_ocv_35_pon_rdy %d _hw_ocv_35_pon %d _hw_ocv_35_plugin %d _hw_ocv_chgin %d _sw_ocv %d now_temp %d now_thr %d\n",
	    __func__, _hw_ocv_35_pon_rdy, _hw_ocv_35_pon, _hw_ocv_35_plugin,
	    _hw_ocv_chgin, _sw_ocv, now_temp, now_thr);

	return 0;
}

int fgauge_set_coulomb_interrupt1_ht(struct gauge_device *gauge_dev,
				     int car_value)
{
	unsigned int uvalue32_CAR = 0;
	unsigned int uvalue32_CAR_MSB = 0;
	 ...
	 */
	if (reset == 0) {
		ret = pmic_config_interface(MT6356_FGADC_CON1, 0x0001, 0x1F0F,
					    0x0);
	} else {
		ret = pmic_config_interface(MT6356_FGADC_CON1, 0x1F05, 0xFF0F,
					    0x0);
		bm_err(
	...
	value32_CAR |= ((pmic_get_register_value(PMIC_FG_CAR_31_16)) & 0xffff)
		       << 16;
	uvalue32_CAR_MSB =
...
/* gap to register-base */
	...
	bm_trace(
		 "[fgauge_set_columb_interrupt_internal1] upper = 0x%x:%d diff_car=0x%llx:%lld\r\n",
	...
	bm_trace(
	    "[fgauge_set_columb_interrupt_internal1]final upper = 0x%x:%d car=0x%llx:%lld\r\n",
	    upperbound, upperbound, car, car);

	bm_trace(
	    "[fgauge_set_columb_interrupt_internal1]final upper 0x%x 0x%x 0x%x car=0x%llx\n",
				     ...
				     int car_value)
{
	...
	bm_trace("[%s] FG_CAR = 0x%x	uvalue32_CAR_MSB:0x%x 0x%x 0x%x\r\n",
		 __func__, uvalue32_CAR, uvalue32_CAR_MSB,
		 (pmic_get_register_value(PMIC_FG_CAR_15_00)),
		 (pmic_get_register_value(PMIC_FG_CAR_31_16)));
...
/* gap to register-base */
	...
	bm_trace("[%s] low:[0xcae]=0x%x 0x%x lcar:%d\r\n", __func__,
...
static int
fgauge_read_boot_battery_plug_out_status(struct gauge_device *gauge_dev,
					 int *is_plugout, int *plutout_time)
{
	*is_plugout = is_bat_plugout;
	*plutout_time = bat_plug_out_time;
	bm_err(
	       "[%s] rtc_invalid %d plugout %d bat_plug_out_time %d sp0:0x%x sp3:0x%x %d %d\n",
	       __func__, rtc_invalid, is_bat_plugout, bat_plug_out_time,
	       gspare0_reg, gspare3_reg, moniter_plchg_bit, pl_charging_status);

	return 0;
}

static int fgauge_get_ptim_current(struct gauge_device *gauge_dev,
				   int *ptim_current, bool *is_charging)
{
	unsigned short uvalue16 = 0;
	signed int dvalue = 0;
	/*int m = 0;*/
...
static int fgauge_get_zcv_current(struct gauge_device *gauge_dev,
				  int *zcv_current)
{
	unsigned short uvalue16 = 0;
	signed int dvalue = 0;
...
#if defined(SWCHR_POWER_PATH) || defined(CONFIG_MTK_SWCHR_SUPPORT)
	adc_result_reg =
	    pmic_get_register_value(PMIC_AUXADC_ADC_OUT_FGADC_SWCHR);
	adc_result = REG_to_MV_value(adc_result_reg);
...
static int fgauge_is_gauge_initialized(struct gauge_device *gauge_dev,
				       int *init)
{
...
static int fgauge_set_gauge_initialized(struct gauge_device *gauge_dev,
					int init)
{
...
static void fgauge_set_nafg_intr_internal(int _prd, int _zcv_mv, int _thr_mv)
{
	...
	bm_debug(
		 "[fg_bat_nafg][fgauge_set_nafg_interrupt_internal] time[%d] zcv[%d:%d] thr[%d:%d] 26_16[0x%x] 15_00[0x%x]\n",
...
static int fgauge_set_nag_c_dltv(struct gauge_device *gauge_dev, int c_dltv_mv)
{
	...
	fgauge_set_nafg_intr_internal(
	    gauge_dev->fg_cust_data->nafg_time_setting, nag_zcv_mv,
	    nag_c_dltv_mv);
	return 0;
}
...
static int fgauge_get_nag_cnt(struct gauge_device *gauge_dev, int *nag_cnt)
{
	...
	*nag_cnt =
	    (NAG_C_DLTV_CNT & 0xffff) + ((NAG_C_DLTV_CNT_H & 0x3ff) << 16);
...
static int fgauge_get_nag_c_dltv(struct gauge_device *gauge_dev,
				 int *nag_c_dltv)
{
	...
	*nag_c_dltv = NAG_C_DLTV_mV_value;
	...
	bm_debug(
		 "[fg_bat_nafg][fgauge_get_nafg_c_dltv] mV:Reg[%d:%d][b10:%d][26_16(0x%04x) 15_00(0x%04x)]\n",
		 NAG_C_DLTV_mV_value, NAG_C_DLTV_reg_value, bcheckbit10,
		 NAG_C_DLTV_value_H, NAG_C_DLTV_value);

	return 0;
}

static void fgauge_set_zcv_intr_internal(struct gauge_device *gauge_dev,
					 int fg_zcv_det_time,
					 int fg_zcv_car_th)
{
	...
	/*fg_zcv_car_th_reg = (fg_zcv_car_th * 100 * 3600 * 1000) /
	 * UNIT_FGCAR_ZCV;
	 */
...
#if defined(__LP64__) || defined(_LP64)
		fg_zcv_car_th_reg =
		    (fg_zcv_car_th_reg * gauge_dev->fg_cust_data->r_fg_value) /
		    100;
#else
		fg_zcv_car_th_reg = div_s64(
		    fg_zcv_car_th_reg * gauge_dev->fg_cust_data->r_fg_value,
		    100);
#endif
	...
	bm_debug(
	    "[FG_ZCV_INT][%s] det_time %d mv %d reg %lld 30_16 0x%x 15_00 0x%x\n",
...
static int fgauge_set_zcv_interrupt_threshold(struct gauge_device *gauge_dev,
					      int threshold)
{
	int fg_zcv_det_time = gauge_dev->fg_cust_data->zcv_suspend_time;
	int fg_zcv_car_th = threshold;

	fgauge_set_zcv_intr_internal(gauge_dev, fg_zcv_det_time,
				     fg_zcv_car_th);
...
void battery_dump_nag(void)
{
	...
	do {
		nag_vbat_reg =
		    upmu_get_reg_value(PMIC_AUXADC_ADC_OUT_NAG_ADDR);
	...
	bm_err(
	       "[read_nafg_vbat] i:%d nag_vbat_reg 0x%x nag_vbat_mv %d:%d %d,nag_zcv:%d,_zcv_reg:0x%x,thr:%d,_thr_reg:0x%x\n",
	       i, nag_vbat_reg, nag_vbat_mv, vbat_val,
	       pmic_get_battery_voltage(), nag_zcv_mv, _zcv_reg, nag_c_dltv_mv,
	       _thr_reg);
...
static int
fgauge_enable_battery_tmp_lt_interrupt(struct gauge_device *gauge_dev, bool en,
				       int threshold)
{
	int tmp_int_lt = 0;
	...
	bm_debug(
	    "[gauge_enable_battery_tmp_lt_interrupt] en:%d mv:%d reg:%d\n", en,
	    threshold, tmp_int_lt);

	return 0;
}

static int
fgauge_enable_battery_tmp_ht_interrupt(struct gauge_device *gauge_dev, bool en,
				       int threshold)
{
	int tmp_int_ht = 0;
	...
	bm_debug(
	    "[gauge_enable_battery_tmp_ht_interrupt] en:%d mv:%d reg:%d\n", en,
	    threshold, tmp_int_ht);
...
int fgauge_get_time(struct gauge_device *gauge_dev, unsigned int *ptime)
{
	...
	bm_trace("[%s] low:0x%x high:0x%x rtime:0x%llx 0x%x!\r\n", __func__,
...
int fgauge_set_time_interrupt(struct gauge_device *gauge_dev, int threshold)
{
	...
	do {
		...
		bm_debug("[%s] now:%lld time:%lld\r\n", __func__,
			 now / 2, time / 2);
		ret = pmic_config_interface(MT6356_FGADC_CON1, 0x0008, 0x000F,
					    0x0);
		...
		/*(8)	 Recover original settings */
		ret = pmic_config_interface(MT6356_FGADC_CON1, 0x0000, 0x000F,
					    0x0);

		pmic_enable_interrupt(FG_TIME_NO, 0, "GM30");
		pmic_set_register_value(PMIC_FG_TIME_HTH_15_00,
					(time & 0xffff));
		...
		bm_trace("[%s] low:0x%x high:0x%x time:%lld %lld\r\n", __func__,
			 pmic_get_register_value(PMIC_FG_TIME_HTH_15_00),
			 pmic_get_register_value(PMIC_FG_TIME_HTH_29_16), time,
			 time2);
	} while (time2 >= time);
	return 0;
}
...
static void Intr_Number_to_Name(char *intr_name, int intr_no)
{
	...
	case FG_INTR_BAT_TIME_INT:
		...
		sprintf(
		    intr_name, "FG_INTR_BAT_TIME_INT");
		break;
...
int fgauge_get_hw_status(struct gauge_device *gauge_dev,
			 struct gauge_hw_status *gauge_status, int intr_no)
{
	...
	if ((is_iavg_valid == 1) && (gauge_status->iavg_intr_flag == 0)) {
		bm_trace(
		    "[read_fg_hw_info] set first fg_set_iavg_intr %d %d\n",
	...
	bm_err(
	    "[FGADC_intr_end][%s][read_fg_hw_info] curr_1 %d curr_2 %d Iavg %d sign %d car %d ncar %d time %d\n",
	    intr_name, gauge_dev->fg_hw_info.current_1,
	    gauge_dev->fg_hw_info.current_2, gauge_dev->fg_hw_info.current_avg,
	    gauge_dev->fg_hw_info.current_avg_sign, gauge_dev->fg_hw_info.car,
...
int fgauge_enable_iavg_interrupt(struct gauge_device *gauge_dev, bool ht_en,
				 int ht_th, bool lt_en, int lt_th)
{
...
{
#if defined(SWCHR_POWER_PATH) || defined(CONFIG_MTK_SWCHR_SUPPORT)
	pmic_set_register_value(PMIC_AUXADC_SOURCE_LBAT2_SEL,
				1); /*0:Batsns, 1:Isense*/
#else
...
int (*gauge_enable_vbat_low_threshold)(struct gauge_device *gauge_dev,
				       int threshold);
int (*gauge_enable_vbat_high_threshold)(struct gauge_device *gauge_dev,
					int threshold);

int fgauge_set_vbat_low_threshold(struct gauge_device *gauge_dev,
				  int threshold)
{
	int vbat2_l_th_mv = threshold;
	int vbat2_l_th_reg = MV_to_REG_12_value(vbat2_l_th_mv);
	int vbat2_det_time_15_0 =
	    ((1000 * gauge_dev->fg_cust_data->vbat2_det_time) & 0xffff);
	int vbat2_det_time_19_16 =
	    ((1000 * gauge_dev->fg_cust_data->vbat2_det_time) & 0xffff0000) >>
	    16;
	...
	bm_debug(
	    "[fg_set_vbat2_l_th] set [0x%x 0x%x 0x%x 0x%x] get [0x%x 0x%x 0x%x 0x%x]\n",
	    vbat2_l_th_reg, vbat2_det_time_15_0, vbat2_det_time_19_16,
	    vbat2_det_counter,
...
int fgauge_set_vbat_high_threshold(struct gauge_device *gauge_dev,
				   int threshold)
{
	...
	bm_debug(
		 "[fg_set_vbat2_h_th] set %d [0x%x 0x%x 0x%x 0x%x] get [0x%x 0x%x 0x%x 0x%x]\n",
...
static int
fgauge_enable_car_tune_value_calibration(struct gauge_device *gauge_dev,
					 ...
					 int *car_tune_value)
{
	...
	if (meta_input_cali_current != 0) {
		for (i = 0; i < CALI_CAR_TUNE_AVG_NUM; i++) {
			if (!fgauge_get_AUXADC_current_rawdata(&uvalue16)) {
				...
				if (uvalue32 <= 0x8000) {
					...
					bm_err(
					       "[111]uvalue16 %d uvalue32 %d Temp_Value1 %lld\n",
					       uvalue16, uvalue32,
					       Temp_Value1);
					...
					bm_err(
					       "[222]uvalue16 %d uvalue32 %d Temp_Value1 %lld\n",
					       uvalue16, uvalue32,
					       Temp_Value1);
				}
				sum_all += Temp_Value1;
				...
				/*****************/
				bm_err(
				       "[333]uvalue16 %d uvalue32 %d Temp_Value1 %lld sum_all %lld\n",
		...
		temp_sum = sum_all;
		bm_err(
		       "[444]sum_all %lld temp_sum %lld avg_cnt %d current_from_ADC %lld\n",
		...
		bm_err(
		       "[555]sum_all %lld temp_sum %lld avg_cnt %d current_from_ADC %lld\n",
		       sum_all, temp_sum, avg_cnt, current_from_ADC);

		Temp_Value2 = current_from_ADC * UNIT_FGCURRENT;

		bm_err(
		       "[555]Temp_Value2 %lld current_from_ADC %lld UNIT_FGCURRENT %d\n",
		       Temp_Value2, current_from_ADC, UNIT_FGCURRENT);
		...
		if (dvalue != 0) {
			...
			bm_err(
			       "[777]dvalue %d fg_cust_data.r_fg_value %d cali_car_tune %d\n",
...
static int fgauge_set_rtc_ui_soc(struct gauge_device *gauge_dev,
				 int rtc_ui_soc)
{
...
int fgauge_set_reset_status(struct gauge_device *gauge_dev, int reset)
{
	...
	bm_err(
	       "[fgauge_read_RTC_boot_status] spare0 0x%x 0x%x, spare3 0x%x 0x%x\n",
		       ...
		       int type)
{
	if (m != NULL) {
		...
		seq_printf(
		    m, "AUXADC_LBAT2_DEBT_MIN  :%x\n",
		    pmic_get_register_value(PMIC_AUXADC_LBAT2_DEBT_MIN));
		seq_printf(
		    m, "AUXADC_LBAT2_DEBT_MAX  :%x\n",
		    pmic_get_register_value(PMIC_AUXADC_LBAT2_DEBT_MAX));

		seq_printf(
		    m, "AUXADC_LBAT2_DET_PRD_15_0  :%x\n",
		    pmic_get_register_value(PMIC_AUXADC_LBAT2_DET_PRD_15_0));
		seq_printf(
		    m, "AUXADC_LBAT2_DET_PRD_19_16	:%x\n",
		    pmic_get_register_value(PMIC_AUXADC_LBAT2_DET_PRD_19_16));

		seq_printf(
		    m, "AUXADC_LBAT2_MAX_IRQ_B  :%x\n",
		...
		seq_printf(
		    m, "AUXADC_LBAT2_IRQ_EN_MAX  :%x\n",
		    pmic_get_register_value(PMIC_AUXADC_LBAT2_IRQ_EN_MAX));
		seq_printf(
		    m, "AUXADC_LBAT2_VOLT_MAX	:%x\n",
		    pmic_get_register_value(PMIC_AUXADC_LBAT2_VOLT_MAX));

		seq_printf(
		    m, "AUXADC_LBAT2_MIN_IRQ_B  :%x\n",
		...
		seq_printf(
		    m, "AUXADC_LBAT2_IRQ_EN_MIN  :%x\n",
		    pmic_get_register_value(PMIC_AUXADC_LBAT2_IRQ_EN_MIN));
		seq_printf(
		    m, "AUXADC_LBAT2_VOLT_MIN	:%x\n",
		    pmic_get_register_value(PMIC_AUXADC_LBAT2_VOLT_MIN));
		...
		seq_printf(m,
			   ...
			   charger_zcv, pmic_rdy, pmic_zcv, pmic_in_zcv, swocv,
			   zcv_from, zcv_tmp);
	}
	...
	bm_debug(
		 "chr_zcv:%d pmic_zcv:%d %d pmic_in_zcv:%d swocv:%d zcv_from:%d tmp:%d\n",
		 charger_zcv, pmic_rdy, pmic_zcv, pmic_in_zcv, swocv, zcv_from,
		 zcv_tmp);
	return 0;
}
...
int fgauge_set_battery_cycle_interrupt(struct gauge_device *gauge_dev,
				       int threshold)
{
	...
	bm_debug(
		 "car:%d carR:%lld r:%lld current:low:0x%x high:0x%x target:low:0x%x high:0x%x",
...
static int mt6356_parse_dt(struct mt6356_gauge *info, struct device *dev)
{
	...
	if (of_property_read_string(np, "gauge_name", &info->gauge_dev_name) <
	    0) {
...
static int mt6356_gauge_probe(struct platform_device *pdev)
{
	...
	info =
	    devm_kzalloc(&pdev->dev, sizeof(struct mt6356_gauge), GFP_KERNEL);
	if (!info)
		return -ENOMEM;
	...
	info->gauge_dev =
	    gauge_device_register(info->gauge_dev_name, &pdev->dev, info,
				  &mt6356_gauge_ops, &info->gauge_prop);
...
static struct platform_driver mt6356_gauge_driver = {
	...
	.driver = {
	    .name = "mt6356_gauge", .of_match_table = mt6356_gauge_of_match,
	},
};
