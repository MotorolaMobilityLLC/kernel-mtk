...
{
	PMICLOG("[wake_up_mt6336]\r\n");
	wake_up_process(mt6336_thread_handle);

#if !defined CONFIG_HAS_WAKELOCKS
	__pm_stay_awake(&mt6336Thread_lock);
#else
	wake_lock(&mt6336Thread_lock);
#endif
}
EXPORT_SYMBOL(wake_up_mt6336);

irqreturn_t mt6336_eint_irq(int irq, void *desc)
{
	PMICLOG("[mt6336_eint_irq]\r\n");
	wake_up_mt6336();
...
void mt6336_enable_interrupt(unsigned int intNo, char *str)
{
	...
	if (shift >= ARRAY_SIZE(mt6336_interrupts)) {
		PMICLOG("[mt6336_enable_interrupt] fail intno=%d \r\n", intNo);
		return;
	}
...
void mt6336_mask_interrupt(unsigned int intNo, char *str)
{
	...
	if (shift >= ARRAY_SIZE(mt6336_interrupts)) {
		PMICLOG("[mt6336_mask_interrupt] fail intno=%d \r\n", intNo);
		return;
	}
...
void mt6336_register_interrupt_callback(unsigned int intNo, void (EINT_FUNC_PTR) (void))
{
	...
	if (shift >= ARRAY_SIZE(mt6336_interrupts)) {
		PMICLOG("[mt6336_register_interrupt_callback] fail intno=%d\r\n", intNo);
		return;
	}
...
static void mt6336_int_handler(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6336_interrupts); i++) {
		...
		PMICLOG("[CHR_INT] %d status[0x%x]=0x%x [0x%x]=0x%x en[0x%x]=0x%x mask[0x%x]=0x%x\n", i,
...
#if !defined CONFIG_HAS_WAKELOCKS
		__pm_relax(&mt6336Thread_lock);
#else
		wake_unlock(&mt6336Thread_lock);
#endif
		set_current_state(TASK_INTERRUPTIBLE);
