...
.va = 0, .mva = 0, .size = 0, .port = M4U_PORT_UNKNOWN, };
...
static phys_addr_t m4u_user_v2p(unsigned long va)
{
	...
	if (NULL == current->mm) {
		M4UMSG(
				"warning: m4u_user_v2p, current->mm is NULL! tgid=0x%x, name=%s\n",
...
static int m4u_fill_sgtable_user(unsigned long va, int page_num,
	...
	for (i = 0; i < page_num; i++) {
		...
		if (!pa || !sg) {
			M4UMSG(
					...
					__func__, va, page_num, va_align+i*PAGE_SIZE, &pa, sg, i);
...
int m4u_alloc_mva(m4u_client_t *client, M4U_PORT_ID port, unsigned long va,
...
int m4u_alloc_mva_sg(int eModuleID, struct sg_table *sg_table,
		const unsigned int BufSize, int security, int cache_coherent,
	...
	return m4u_alloc_mva(ion_m4u_client, eModuleID, 0, sg_table, BufSize, prot,
			0, pRetMVABuf);
}
...
int m4u_dealloc_mva(m4u_client_t *client, M4U_PORT_ID port, unsigned int mva)
{
	...
	if (is_err) {
		m4u_aee_print("%s fail: port=%s, mva=0x%x, size=0x%x, sg=0x%p\n",
				__func__, m4u_get_port_name(port), mva, pMvaInfo->size, pMvaInfo->sg_table);
		ret = -EINVAL;
	} else
		ret = 0;
 ...
 **/
int m4u_cache_sync(m4u_client_t *client, M4U_PORT_ID port, unsigned long va,
		unsigned int size, unsigned int mva, M4U_CACHE_SYNC_ENUM sync_type) {
	int ret = 0;
