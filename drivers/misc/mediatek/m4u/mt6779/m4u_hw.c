...
char *smi_clk_name[] = {
	...
	"m4u_smi_larb3", "m4u_smi_larb5", "m4u_smi_larb7",
	"m4u_smi_larb8", "m4u_smi_larb9", "m4u_smi_larb10"
};
...
void m4u_print_port_status(struct seq_file *seq, int only_print_active)
{
	...
	for (port = 0; port < gM4u_port_num; port++) {
		...
		if (m4u_index == 0) {
			...
			mmu_en =
				m4uHw_get_field_by_mask(larb_base,
					SMI_LARB_NON_SEC_CONx(larb_port),
					F_SMI_NON_SEC_MMU_EN(1));
			...
			mmu_en =
				m4uHw_get_field_by_mask(gPericfgBaseAddr,
...
irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
...

	{
		...
		if (IntrSrc & F_INT_TRANSLATION_FAULT(m4u_slave_id)) {
			...
			M4UMSG("fault: port=%s, mva=0x%x, pa=0x%x\n",
			       m4u_get_port_name(m4u_port),
			       fault_mva, fault_pa);
			M4UMSG("fault: layer=%d, wr=%d, fault_id=0x%x\n",
			       layer, write, regval);
