...
static M4U_PROG_DIST_T gM4U0_prog_pfh[M4U0_PROG_PFH_NR * M4U_SLAVE_NUM(0)] = {
    {0} };

/* static M4U_PROG_DIST_T gM4u1_prog_pfh[M4U1_PROG_PFH_NR * M4U_SLAVE_NUM(1)] =
 * {{0} }; */
			      ...
			      unsigned int st[], unsigned int pte[])
{
	...
	M4ULOG_MID("id mva        valid   port-id       pa         pte     "
		   "larb  w/r  other-status\n");
...
static int m4u_dump_rs_info(int m4u_index, int m4u_slave_id)
{
	...
	for (i = 0; i < MMU_TOTAL_RS_NR; i++) {
		va[i] =
		    COM_ReadReg32((m4u_base + REG_MMU_RSx_VA(m4u_slave_id, i)));
		pa[i] =
		    COM_ReadReg32((m4u_base + REG_MMU_RSx_PA(m4u_slave_id, i)));
		st[i] =
		    COM_ReadReg32((m4u_base + REG_MMU_RSx_ST(m4u_slave_id, i)));
		pte[i] = COM_ReadReg32(
		      ...
		      unsigned int larb_mask)
{
	...
	M4ULOG_HIGH(
	  "%s [%d] start=%x end=%x write %d portmask=%x larbmask=%x\n",
		    __func__,
		    mau_set, start, end, wr, port_mask, larb_mask);

	M4U_WriteReg32(m4u_base, REG_MMU_MAU_START(m4u_slave_id, mau_set),
		       start);
	M4U_WriteReg32(m4u_base, REG_MMU_MAU_START_BIT32(m4u_slave_id, mau_set),
		       !!(bit32));
	...
	for (i = 0; i < M4U0_MAU_NR; i++) {
		if (gM4u0_mau[i].Enabled != 0) {
			if (MVAStart >= gM4u0_mau[i].MVAStart &&
			    MVAEnd <= gM4u0_mau[i].MVAEnd) { /* no overlap */
				if (mau.enable == 0) {
...
int mau_get_config_info(struct mau_config_info *cfg)
{
	...
	cfg->end_bit32 = M4U_ReadReg32(
	    m4u_base, REG_MMU_MAU_START_BIT32(m4u_slave_id, mau_set));
	cfg->port_mask =
	    M4U_ReadReg32(m4u_base, REG_MMU_MAU_PORT_EN(m4u_slave_id, mau_set));
	cfg->larb_mask =
	    m4uHw_get_field_by_mask(m4u_base, REG_MMU_MAU_LARB_EN(m4u_slave_id),
				    F_MAU_LARB_MSK(mau_set));

	cfg->io = !!(m4uHw_get_field_by_mask(
...
int __mau_dump_status(int m4u_id, int m4u_slave_id, int mau)
{
	...
	if (status & (1 << mau)) {
		...
		M4ULOG_HIGH("mau_cfg: "
			    "start=0x%x,end=0x%x,virt(%d),io(%d),wr(%d),s_b32(%"
			    "d),e_b32(%d)\n",
...
int m4u_dump_reg(int m4u_index, unsigned int start, unsigned int end)
{
	...
	for (i = 0; i < 368 / 8; i += 4) {
		M4UINFO(
		    ...
		    start + 8 * i, M4U_ReadReg32(gM4UBaseAddr[m4u_index],
						 start + 8 * i + 4 * 0),
			     ...
			     mmu_tlb_t *pTlb)
{
	...
	regValue = F_READ_ENTRY_EN | F_READ_ENTRY_PFH |
		   F_READ_ENTRY_PFH_IDX(set) | F_READ_ENTRY_PFH_PAGE_IDX(page) |
	...
	for (way = 0; way < way_nr; way++) {
		for (set = 0; set < set_nr; set++) {
			...
			if (regval & F_MMU_PFH_VLD_BIT(set, way)) {
				...
				tag_s =
				    imu_pfh_tag_to_va(m4u_index, set, way, tag);
...
const char *smi_clk_name[] = {
    ...
    "m4u_smi_larb3",  "m4u_smi_larb3_2", "m4u_smi_larb4",  "m4u_smi_larb5",
    "m4u_mtcmos_ven", "m4u_mtcmos_vde",  "m4u_mtcmos_isp", "m4u_mtcmos_dis",
    "m4u_mtcmos_cam"};
#endif
...
#else
	M4UMSG("error: %s not support", __func__);
	return;
#endif
}
...
int m4u_enable_prog_dist_by_id(int port, int id)
{
	...
	spin_lock(&gM4u_reg_lock);
	m4uHw_set_field_by_mask(m4u_base, REG_MMU_PROG_DIST(id, m4u_slave_id),
				F_PF_EN(1), 1);
	spin_unlock(&gM4u_reg_lock);
...
int m4u_config_prog_dist(M4U_PORT_ID port, int dir, int dist, int en, int mm_id,
			 int sel)
{
	int i, free_id = -1;
	...
	for (i = 0; i < M4U_PROG_PFH_NUM(m4u_index); i++) {
		if (pProgPfh[i].Enabled == 1) {
			if (port == pProgPfh[i].port &&
				...
				M4UMSG("m4u warning: cannot set two direction or difference distance in the same port.\n");
				M4UMSG("original value: module = %s, mm_id = %d, dir = %d, dist = %d, sel = %d.\n",
				       m4u_get_port_name(port), mm_id, dir,
				       dist, sel);
				M4UMSG("new value: module = %s, mm_id = %d, dir = %d, dist = %d, sel = %d.\n",
				       m4u_get_port_name(pProgPfh[i].port),
				       pProgPfh[i].mm_id, pProgPfh[i].dir,
	...
	m4uHw_set_field_by_mask(m4u_base,
				...
				F_PF_ID_COMP_SEL(1), F_PF_ID_COMP_SEL(!!(sel)));

	m4uHw_set_field_by_mask(m4u_base,
				REG_MMU_PROG_DIST(free_id, m4u_slave_id),
				F_PF_ID_COMP_SEL(1), F_PF_DIR(!!(dir)));
			 ...
			 unsigned int MVAEnd)
{
	...
	if (MVAEnd - MVAStart < PAGE_SIZE) {
		M4ULOG_MID("too small size, skip to insert! module:%s, "
			   "MVAStart:0x%x, size:%d\n",
	...
	for (i = 0; i < M4U_SEQ_NUM(m4u_index); i++) {
		if (pSeq[i].Enabled == 1) {
			if (MVAEnd < pSeq[i].MVAStart ||
				...
				M4ULOG_HIGH("warning: tlb range overlapped with previous ranges, current process=%s,!\n",
					    current->comm);
				M4ULOG_HIGH(
...
static int _m4u_config_port(int port, int virt, int sec, int dis, int dir)
{
	...
	if (m4u_index == 0) {
		...
		M4U_WriteReg32(larb_base, SMI_LARB_NON_SEC_CONx(larb_port),
			       F_SMI_NON_SEC_MMU_EN(!!(virt)));
		...
		if (!!(mmu_en) != virt) {
			M4ULOG_HIGH(
			    "m4u_config_port error, port=%s, Virtuality=%d, "
			    "mmu_en=%x (%x, %x)\n",
...
int m4u_config_port(M4U_PORT_STRUCT *pM4uPort) /* native */
{
	 ...
	 */
	M4ULOG_HIGH("m4u_config_port: %s, m4u_tee_en:%d, mmu_en: %d -> %d, "
		    "sec_en:%d -> %d\n",
	...
	for (port = 0; port < gM4u_port_num; port++) {
		...
		if (m4u_index == 0) {
			...
			if (orig_value != regNew[larb][larb_port]) {
				spin_lock(&gM4u_reg_lock);
				M4U_WriteReg32(gLarbBaseAddr[larb],
					       SMI_LARB_NON_SEC_CONx(larb_port),
...
int m4u_monitor_stop(int m4u_id)
{
	...
	M4ULOG_MID("[M4U%d-%d] total:%u, main miss:%u, pfh miss(walk):%u, auto "
		   "pfh:%u\n",
	...
	for (m4u_id = 0; m4u_id < TOTAL_M4U_NUM; m4u_id++) {
		...
		for (m4u_slave = 0; m4u_slave < M4U_SLAVE_NUM(m4u_id);
			...
			for (dist = 0; dist < MMU_TOTAL_PROG_DIST_NR; dist++)
				__M4U_BACKUP(m4u_base,
					     REG_MMU_PROG_DIST(dist, m4u_slave),
					     *(pReg++));
...
int m4u_reg_restore(void)
{
	...
	for (m4u_id = 0; m4u_id < TOTAL_M4U_NUM; m4u_id++) {
		...
		for (m4u_slave = 0; m4u_slave < M4U_SLAVE_NUM(m4u_id);
			...
			for (dist = 0; dist < MMU_TOTAL_PROG_DIST_NR; dist++)
				__M4U_RESTORE(m4u_base, REG_MMU_PROG_DIST(
							    dist, m4u_slave),
					      *(pReg++));
			...
			for (mau = 0; mau < MAU_NR_PER_M4U_SLAVE; mau++) {
				__M4U_RESTORE(m4u_base,
					      REG_MMU_MAU_START(m4u_slave, mau),
					      *(pReg++));
				__M4U_RESTORE(m4u_base, REG_MMU_MAU_START_BIT32(
							    m4u_slave, mau),
					      *(pReg++));
				__M4U_RESTORE(m4u_base,
					      REG_MMU_MAU_END(m4u_slave, mau),
...

	{
		...
		if (IntrSrc & F_INT_TRANSLATION_FAULT(m4u_slave_id)) {
			...
			if (M4U_PORT_DISP_OVL0 == m4u_port ||
				...
				m4u_query_mva_info(fault_mva - 1, 0, &valid_mva,
						   &valid_size);
		 ...
		 int m4u_id)
{
...

	{
		...
		M4U_WriteReg32(gM4UBaseAddr[m4u_id], REG_MMU_MMU_COHERENCE_EN,
			       0x3);
		M4U_WriteReg32(gM4UBaseAddr[m4u_id], REG_MMU_MMU_TABLE_WALK_DIS,
			       0);
		...
		if (!gM4U_4G_DRAM_Mode) {
			M4U_WriteReg32(
			    gM4UBaseAddr[m4u_id], REG_MMU_IVRP_PADDR,
	...
	if (m4u_id == 0) {
		...
		m4uHw_set_field_by_mask(m4u_base, REG_MMU_WR_LEN, F_BIT_SET(5),
					0);
		m4uHw_set_field_by_mask(m4u_base, REG_MMU_DUMMY,
					F_REG_MMU_IDLE_ENABLE, 1);
	}
...
int m4u_hw_init(struct m4u_device *m4u_dev, int m4u_id)
{
	...
	/*	gM4uDev->infra_m4u = devm_clk_get(gM4uDev->pDev[m4u_id],
	 * "infra_m4u");*/
	...
	for (i = SMI_COMMON; i < SMI_CLK_NUM; i++) {
		gM4uDev->smi_clk[i] =
		    devm_clk_get(gM4uDev->pDev[m4u_id], smi_clk_name[i]);
