...
extern int mtk_gpufreq_register(struct mt_gpufreq_power_table_info *freqs,
				int num);
...
extern void thermal_set_big_core_speed(unsigned int tempMonCtl1,
				       unsigned int tempMonCtl2,
				       unsigned int tempAhbPoll);

/* return value(1): cooler of abcct/abcct_lcmoff is deactive, and no thermal
 * current limit.
 */
