...
/* PTP-OD */
extern unsigned int mt_cpufreq_get_freq_by_idx(enum mt_cpu_dvfs_id id,
					int idx);
extern unsigned int mt_cpufreq_get_volt_by_idx(enum mt_cpu_dvfs_id id,
					int idx);
...
extern void notify_cpu_volt_sampler(enum mt_cpu_dvfs_id id, unsigned int volt,
				    int up, int event);
...
extern unsigned int
mt_cpufreq_get_cur_phy_freq_no_lock(enum mt_cpu_dvfs_id id);
...
typedef void (*cpuFreqsampler_func)(unsigned int cluster_id,
				    unsigned int freq);
