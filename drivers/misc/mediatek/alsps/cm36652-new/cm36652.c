...
struct mutex	cm36652_cs_mutex;
ulong           cs_enable;         /*enable mask*/

enum CMC_CS_BIT CS_BIT;
/*-----------------------------CMC for debugging-------------------------------*/
enum {
...
EXPORT_SYMBOL(CM36652_WRAP_i2c_master_operate);
/*----------------------------------------------------------------------------*/
...
static int als_enable_nodata(int en)
{
	...
	if (en)	/* mean RGB want to en */
		set_bit(CMC_BIT_AS, &cs_enable);
	else
		clear_bit(CMC_BIT_AS, &cs_enable);
	mutex_unlock(&cm36652_cs_mutex);
	if (!cm36652_obj) {
		APS_ERR("cm36652_obj is null!!\n");
		return -1;
