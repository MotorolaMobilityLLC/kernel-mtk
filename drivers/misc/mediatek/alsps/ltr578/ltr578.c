 ...
 * Steven		  steven.zhao@liteon.com	   may/1/2016
 *
 *******************************************************************************/
...
struct platform_device *alspsPltFmDev_ltr;
...
int ltr578_setup_eint(struct i2c_client *client)
{
	...
	alspsPltFmDev_ltr = get_alsps_platformdev();

	/* gpio setting */

	pinctrl = devm_pinctrl_get(&alspsPltFmDev_ltr->dev);
	if (IS_ERR(pinctrl)) {
	ret = PTR_ERR(pinctrl);
...
const struct i2c_device_id *id)
{
	...
	obj->irq_node = of_find_compatible_node(NULL,
	NULL, "mediatek,als_ps");
	obj->enable = 0;
	...
	err = batch_register_support_info(ID_LIGHT,
		als_ctl.is_support_batch, 100, 0);
	if (err)
		APS_ERR("register light batch support err = %d\n", err);

	err = batch_register_support_info(ID_PROXIMITY,
		ps_ctl.is_support_batch, 100, 0);
	if (err)
