...
#else
			pr_info("%s:MODE_I2C: CONFIG_BACKLIGHT_CLASS_DEVICE undefined.\n", __func__);
#endif
		}
		break;
...
unsigned int mt_get_bl_max_brightness(void)
{
	...
	if(cust_led_list[TYPE_LCD].max_brightness)
		max_level = cust_led_list[TYPE_LCD].max_brightness;
...
int mt_bl_esd_recover(int level)
{
	...
	ret = mt_mt65xx_led_set_cust(&cust_led_list[TYPE_LCD], bl_level);
