...
u16 pmic_set_register_value(u32 flagname, u32 val) { return 0; }
#endif
	...
	if (pled_dtsi == NULL) {
		...
		for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
			...
			led_node = of_find_compatible_node(
			    NULL, NULL, strncat(node_name, leds_name[i],
						sizeof(node_name) -
						    strlen(node_name) - 1));
			if (!led_node) {
				...
				isSupportDTS = true;
				ret = of_property_read_u32(led_node,
							   "led_mode", &mode);
				if (!ret) {
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_PMIC:
		/* for button baclight used SINK channel, when set button
		 * ISINK,
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev,
	...
	if (disp_aal_is_support() == true) {
		if (led_data->level != level) {
			...
			if (strcmp(led_data->cust.name, "lcd-backlight") !=
			    0) {
				LEDS_DEBUG(
				    "Set NLED directly %d at time %lu\n",
