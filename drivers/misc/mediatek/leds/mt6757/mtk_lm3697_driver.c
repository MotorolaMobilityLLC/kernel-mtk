...
#define CPD_ERR(fmt, args...)                                                  \
	pr_debug("[ChargePump] %s %d : " fmt, __func__, __LINE__, ##args)
...
#define MIN_MAX_SCALE(x)                                                       \
	(((x) < LM3697_MIN_VALUE_SETTINGS)                                     \
	     ? LM3697_MIN_VALUE_SETTINGS                                       \
	     : (((x) > LM3697_MAX_VALUE_SETTINGS) ? LM3697_MAX_VALUE_SETTINGS  \
						  : (x)))
...
static char lm3697_i2c_write(struct i2c_client *client, u8 reg_addr, u8 *data,
			     u8 len)
{
	s32 dummy;
...
unsigned int lm3697_set_brightness_level(unsigned int level_a)
{
	...
	data0 = 0;
	ret_code = lm3697_smbus_write_byte(
	    new_client, LM3697_CTL_A_BRIGHTNESS_LSB_REG, &data0);
	if (ret_code != 0) {
	...
	data1 = level_a;
	ret_code = lm3697_smbus_write_byte(
	    new_client, LM3697_CTL_A_BRIGHTNESS_MSB_REG, &data1);
	if (ret_code != 0) {
	...
	data0 = 0;
	ret_code = lm3697_smbus_write_byte(
	    new_client, LM3697_CTL_B_BRIGHTNESS_LSB_REG, &data0);
	if (ret_code != 0) {
	...
	data1 = level_a;
	ret_code = lm3697_smbus_write_byte(
	    new_client, LM3697_CTL_B_BRIGHTNESS_MSB_REG, &data1);
	if (ret_code != 0) {
...
int chargepump_set_backlight_level(unsigned int level)
{
	...
	if (level == 0) {
		...
		data = 0x0;
		lm3697_smbus_write_byte(new_client, LM3697_CTL_B_BANK_EN_REG,
					&data);
		...
		if (is_suspend == 1) {
			...
			data = 0x3;
			lm3697_smbus_write_byte(
			    new_client, LM3697_CTL_B_BANK_EN_REG, &data);
...
lm3697_detect(struct i2c_client *client, int kind, struct i2c_board_info *info)
{
	return 0;
}
...
static void __exit lm3697_exit(void) { i2c_del_driver(&lm3697_i2c_driver); }
