...
s32 pwm_set_spec_config(struct pwm_spec_config *conf) { return 0; }
...
static int time_array_hal[PWM_DIV_NUM] = {256,  512,  1024,  2048,
					  4096, 8192, 16384, 32768};
static unsigned int div_array_hal[PWM_DIV_NUM] = {1, 2, 4, 8, 16, 32, 64, 128};

static unsigned int backlight_PWM_div_hal =
    CLK_DIV1; /* this para come from cust_leds. */
...
#else
	    wake_lock_init(&leds_suspend_lock, "leds wakelock");
#endif
}

unsigned int mt_get_bl_brightness(void) { return bl_brightness_hal; }

unsigned int mt_get_bl_duty(void) { return bl_duty_hal; }

unsigned int mt_get_bl_div(void) { return bl_div_hal; }

unsigned int mt_get_bl_frequency(void) { return bl_frequency_hal; }

unsigned int *mt_get_div_array(void) { return &div_array_hal[0]; }

void mt_set_bl_duty(unsigned int level) { bl_duty_hal = level; }

void mt_set_bl_div(unsigned int div) { bl_div_hal = div; }

void mt_set_bl_frequency(unsigned int freq) { bl_frequency_hal = freq; }
...
struct cust_mt65xx_led *get_cust_led_dtsi(void)
{
	...
	/*LEDS_DEBUG("get_cust_led_dtsi: get the leds info from device
	 * tree\n");
	 */
	...
	if (pled_dtsi == NULL) {
		...
		for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
			...
			led_node = of_find_compatible_node(
			    NULL, NULL,
			...
			if (!led_node) {
				...
				ret = of_property_read_u32(led_node, "data",
							   &data);
				if (!ret) {
				...
				ret = of_property_read_u32_array(
				    led_node, "pwm_config", pwm_config,
				    ARRAY_SIZE(pwm_config));
				if (!ret) {
					LEDS_DEBUG("The %s's pwm config data "
						   "is %d %d %d %d %d\n",
						   pled_dtsi[i].name,
						   pwm_config[0], pwm_config[1],
						   pwm_config[2], pwm_config[3],
						   pwm_config[4]);
					...
					LEDS_DEBUG("led dts can not get pwm "
						   "config data.\n");
				...
				case MT65XX_LED_MODE_CUST_LCM:
					...
					pled_dtsi[i].data = (long)
					    chargepump_set_backlight_level;
					LEDS_DEBUG("backlight set by "
						   "chargepump_set_backlight_"
						   "level\n");
#else
					pled_dtsi[i].data =
					    (long)mtkfb_set_backlight_level;
#endif /* CONFIG_BACKLIGHT_SUPPORT_LM3697 */
					LEDS_DEBUG("kernel:the backlight hw "
						   "mode is LCM.\n");
					break;
...
int mt_led_set_pwm(int pwm_num, struct nled_setting *led)
{
	...
	/* We won't choose 32K to be the clock src of old mode because of system
	 * performance.
	 */
	/* The setting here will be clock src = 26MHz, CLKSEL = 26M/1625 (i.e.
	 * 16K)
	 */
	...
	case NLED_BLINK:
		...
		pwm_setting.PWM_MODE_OLD_REGS.DATA_WIDTH =
		    (led->blink_on_time + led->blink_off_time) *
		    MIN_FRE_OLD_PWM / div_array_hal[time_index];
		pwm_setting.PWM_MODE_OLD_REGS.THRESH =
		    (led->blink_on_time * 100) /
		    (led->blink_on_time + led->blink_off_time);
		break;
	default:
			 ...
			 struct PWM_config *config_data)
{
	...
	if (BacklightLevelSupport == BACKLIGHT_LEVEL_PWM_256_SUPPORT)
		...
		pwm_setting.mode =
		    PWM_MODE_FIFO; /* New mode fifo and periodical mode */
	...
	if (BacklightLevelSupport == BACKLIGHT_LEVEL_PWM_256_SUPPORT) {
		/* PWM_CLK_OLD_MODE_32K is block/1625 = 26M/1625 = 16KHz @
		 * MT6571
		 */
		...
		if (level > 0 && level < 256) {
			pwm_set_spec_config(&pwm_setting);
			LEDS_DEBUG("[LEDS][%d]backlight_set_pwm: old mode: "
				   "thres/data_width is %d/%d\n",
		...
		LEDS_DEBUG("[LEDS]backlight_set_pwm:clk_src/div/high/low is "
			   "%d%d%d%d\n",
...
unsigned int mt_show_pwm_register(unsigned int addr) { return 0; }
...
int mt_brightness_set_pmic_duty_store(u32 level, u32 div) { return -1; }
	...
	if (disp_aal_is_support() == true) {
		if (led_data->level != level) {
			...
			if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
				...
				disp_pq_notify_backlight_changed(
...
int mt_mt65xx_blink_set(struct led_classdev *led_cdev, unsigned long *delay_on,
			unsigned long *delay_off)
{
	...
	if (*delay_on != led_data->delay_on ||
	    *delay_off != led_data->delay_off) {
		...
		if (led_data->delay_on &&
		    led_data->delay_off) { /* enable blink */
			led_data->level =
			    255; /* when enable blink  then to set the level(255) */
		...
		} else if (!led_data->delay_on &&
			   !led_data->delay_off) { /* disable blink */
			/* AP PWM all support OLD mode */
			if (led_data->cust.mode == MT65XX_LED_MODE_PWM) {
