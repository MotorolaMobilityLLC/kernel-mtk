...
int setMaxbrightness(int max_level, int enable)
{
	...
	if (enable == 1) {
		limit_flag = 1;
		if(cust_led_list[TYPE_LCD].max_brightness)
			limit = max_level*cust_led_list[TYPE_LCD].max_brightness/255;
		else
			limit = max_level;
		...
		if (current_level != 0 && limit < min_level) {
		...
		limit_flag = 0;
		if(cust_led_list[TYPE_LCD].max_brightness)
			limit = cust_led_list[TYPE_LCD].max_brightness;
		else
			limit = 255;
		mutex_unlock(&bl_level_limit_mutex);
...
int hbm_brightness_set(int enable)
{
	...
	if(enable) {
		level = cust_led_list[TYPE_LCD].max_brightness;
	} else {
		level = current_level;
	}
	...
	return mt_mt65xx_led_set_cust(&cust_led_list[TYPE_LCD], level);
}
