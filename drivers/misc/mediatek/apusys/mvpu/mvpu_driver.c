...
static int apusys_mvpu_handler_lite(int type, void *hnd, struct apusys_device *dev)
{
	...
	case APUSYS_CMD_VALIDATE:
		...
		if (copy_from_user(sec_chk_addr, (void __user *)mvpu_req->sec_chk_addr,
		...
		if (copy_from_user(sec_buf_size, (void __user *)mvpu_req->sec_buf_size,
							buf_num*sizeof(uint32_t))) {
		...
		if (copy_from_user(mem_is_kernel, (void __user *)mvpu_req->mem_is_kernel,
							buf_num*sizeof(uint32_t))) {
...
#ifdef MVPU_SEC_DEBUG
			for (i = 0; i < buf_num; i++)
				pr_info("[MVPU][Sec] copy mem_is_kernel success 0x%llx, attr[%d]: %d\n",
						mvpu_req->mem_is_kernel,
						i, mem_is_kernel[i]);
#endif
		}
