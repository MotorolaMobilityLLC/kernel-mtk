...
bool get_ker_info(uint32_t hash, uint32_t *ker_bin_offset, uint32_t *ker_bin_num)
{
	...
	//pr_info("[SEC_IMG] get KER HASH: 0x%08x, img addr 0x%08x,
...
void map_base_buf_id(uint32_t buf_num, uint32_t *sec_chk_addr, uint32_t rp_num,
							uint32_t *target_map, uint32_t *target_base)
{
	int i = 0, j = 0;
