...
static ssize_t ccci_util_pin_bc_read(struct file *filp, char __user *buf,
					size_t size, loff_t *ppos)
{
	...
	if (filp->f_flags & O_NONBLOCK) {
		...
		if (user_ctrl->pin_update == 0)
			return 0;
		memcpy(&user_ctrl->pin_event, pin_event,
			sizeof(struct pin_status_event));
		spin_unlock(&pin_event_update_lock);
		if (copy_to_user(buf, &user_ctrl->pin_event,
				 sizeof(struct pin_status_event)))
			return -EFAULT;
	} else {
		wait_event_interruptible(pin_event_wait,
					 user_ctrl->pin_update == 1);
		spin_lock(&pin_event_update_lock);
		user_ctrl->pin_update = 0;
		memcpy(&user_ctrl->pin_event, pin_event,
...
static unsigned int ccci_util_pin_bc_poll(struct file *filp,
...
int ccci_util_pin_broadcast_init(void)
{
	...
	device_create(s_ccci_pin_class, NULL, s_pin_status_dev, NULL,
		      "ccci_pin_sta");
