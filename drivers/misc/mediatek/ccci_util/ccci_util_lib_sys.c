...
static void aat_write(unsigned long base,
...
static int dump_line_with_8(unsigned long addr, char buf[], int num, int size)
{
	...
	unsigned long tag_addr =
	    addr - ((unsigned long)aat_runtime_map_base_vir)
		+ ((unsigned long)aat_runtime_map_base_phy);

	ch_in_line += snprintf(&buf[ch_in_line],
		size - ch_in_line, "[0x%016lX] ", tag_addr);
	for (i = 0; i < num; i++) {
		tmp = read8((unsigned char *)(addr + i));
		ch_in_line += snprintf(&buf[ch_in_line],
			size - ch_in_line, "%02x ", tmp);
	}
	ch_in_line += snprintf(&buf[ch_in_line],
					size - ch_in_line, "\n");
	return ch_in_line;
...
static int runtime_reg_dump(unsigned long start_addr,
	unsigned int size, unsigned int access_width, char out_buf[])
{
	unsigned int i, buf_idx;
	unsigned int dump_num_in_line, has_dumped;
	...
	for (i = 0; i < size; i += 16) {
		...
		if (dump_num_in_line > 16)
			dump_num_in_line = 16;
		buf_idx += rd_func(curr_addr, &out_buf[buf_idx],
			dump_num_in_line, 4096 - buf_idx);
...
static int command_parser(const char *input_str, size_t count,
	char cmd[], unsigned long out_put[])
{
	int i = 0, j, k;
	char tmp_buf[32];
	char filter_str[256];
	...
	if (string_count > 255) {
		CCCI_UTIL_ERR_MSG("error: input String length = %u>255\n",
...
static void cmd_process(char cmd[], unsigned long para[], int para_num)
{
	if ((para_num == 2) && (strcmp(cmd, "IOR") == 0)) {
		...
		if (aat_runtime_map_base_vir == NULL) {
			aat_runtime_map_base_vir
				= ioremap_nocache((phys_addr_t) para[0], 4096);
			if (aat_runtime_map_base_vir == NULL) {
				snprintf(aat_err_str, 64,
					"Map phy addr:0x%p fail\n",
					(void *)para[0]);
				aat_err_no = 0x00002100;
			} else {
				snprintf(aat_err_str, 64,
					"Map phy addr:0x%p OK\n",
					(void *)para[0]);
		...
		aat_show_case = 0;	/*Error info */
				...
				&& (strcmp(cmd, "WR") == 0)) {
		CCCI_UTIL_DBG_MSG(
			"Command:%s, Offset:0x%p, value:0x%p, type:0x%p\n",
			cmd, (void *)para[0],
		...
		if (aat_runtime_map_base_vir != NULL) {
			if (para[0] >= 4096L) {
				snprintf(aat_err_str, 64,
				"Offset should less than 4096\n");
				aat_err_no = 0x00002100;
			} else {
				aat_write(((unsigned long)aat_runtime_map_base_vir)
					+ para[0], para[1], para[2]);
				snprintf(aat_err_str, 64,
					"Write phy addr:0x%p OK\n",
					(void *)((unsigned long)aat_runtime_map_base_phy
					+ para[0]));
				aat_err_no = 0x00001000;
			}
		} else {
		...
		if (aat_runtime_map_base_vir != NULL) {
			if (para[0] >= 4096L) {
				...
				snprintf(aat_err_str, 64,
					"Offset + Size should less than 4096\n");
				aat_err_no = 0x00002100;
				aat_show_case = 0;	/*Error info */
			} else {
				snprintf(aat_err_str, 64,
					"Query offset:0x%p size:0x%p OK\n",
					(void *)para[0],
					(void *)para[1]);
				ast_dump_start_addr
				= (unsigned long)aat_runtime_map_base_vir
					+ para[0];
		...
		/*MD Force Unlock */
		CCCI_UTIL_DBG_MSG("Command:%s\n", cmd);
		/* Do Un-lock operation here,
		 * unlock sleep/clock here
		 */
		...
		CCCI_UTIL_ERR_MSG(
		"In-valid command:%s and parameter number:%d\n",
		cmd, para_num);
		snprintf(aat_err_str, 64,
		"In-valid command:%s and parameter number:%d\n",
		cmd, para_num);
...
static ssize_t aat_show(char *buf)
{
	...
	if (aat_show_case == 1) {
		...
		curr += runtime_reg_dump(ast_dump_start_addr,
					dump_size,
					(unsigned int)ast_dump_width,
					&buf[curr]);
	}
...
static ssize_t kcfg_setting_show(char *buf)
{
	...
	actual_write = snprintf(&buf[curr],
		4096 - curr, "[EVDO_DT_SUPPORT]:1\n");
#else
	actual_write = snprintf(&buf[curr],
		4096 - curr, "[EVDO_DT_SUPPORT]:0\n");
#endif
	curr += actual_write;
#ifdef CONFIG_MTK_LTE_DC_SUPPORT
	actual_write = snprintf(&buf[curr],
		4096 - curr, "[MTK_LTE_DC_SUPPORT]:1\n");
#else
	actual_write = snprintf(&buf[curr],
		4096 - curr, "[MTK_LTE_DC_SUPPORT]:0\n");
#endif
	curr += actual_write;
