...
#define get_max_real_power_by_segment(seg)                                     \
	((ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_SB)                   \
	     ? DLPT_MAX_REAL_POWER_SB                                          \
...
struct ppm_cobra_data {
	struct ppm_cobra_basic_pwr_data basic_pwr_tbl[TOTAL_CORE_NUM]
						     [DVFS_OPP_NUM];
...
extern void ppm_cobra_update_limit(enum ppm_power_state new_state,
				   void *user_req);
