...
struct ppm_pwr_idx_ref_tbl_data ppm_get_pwr_idx_ref_tbl(void)
{
	...
	if (ppm_main_info.is_turbo_core)
		return (ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_SB)
			   ? pwr_idx_ref_tbl_SB_v3
			   : pwr_idx_ref_tbl_FY_v3;
	else
		return (ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_SB)
			   ? pwr_idx_ref_tbl_SB_v2
			   : pwr_idx_ref_tbl_FY_v2;
#else
	return (ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_SB)
		   ? pwr_idx_ref_tbl_SB
		   : pwr_idx_ref_tbl_FY;
...
int *ppm_get_perf_idx_ref_tbl(enum ppm_cluster cluster)
{
	...
	if (ppm_main_info.is_turbo_core)
		return (ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_SB)
			   ? cpu_perf_idx_ref_tbl_SB_v3[cluster]
			   : cpu_perf_idx_ref_tbl_FY_v3[cluster];
	else
		return (ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_SB)
			   ? cpu_perf_idx_ref_tbl_SB_v2[cluster]
			   : cpu_perf_idx_ref_tbl_FY_v2[cluster];
#else
	return (ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_SB)
		   ? cpu_perf_idx_ref_tbl_SB[cluster]
		   : cpu_perf_idx_ref_tbl_FY[cluster];
#endif
}
...
/* TODO: HOW to interpolate power_idx? */
			...
			p_FF =
			    ref_tbl.pwr_idx_ref_tbl[i].core_dynamic_power_FF[j];
			p_TT =
			    ref_tbl.pwr_idx_ref_tbl[i].core_dynamic_power_TT[j];
			ref_tbl.pwr_idx_ref_tbl[i].core_dynamic_power[j] =
			    (big_lkg_efuse >= BIG_LKG_EFUSE_TT)
				? p_TT -
				      (abs(p_FF - p_TT) * lkg_ratio +
				       (100 - 1)) /
					  100
				: p_TT +
				      (abs(p_FF - p_TT) * lkg_ratio +
			...
			p_FF =
			    ref_tbl.pwr_idx_ref_tbl[i].core_total_power_FF[j];
			p_TT =
			    ref_tbl.pwr_idx_ref_tbl[i].core_total_power_TT[j];
			ref_tbl.pwr_idx_ref_tbl[i].core_total_power[j] =
			    (big_lkg_efuse >= BIG_LKG_EFUSE_TT)
