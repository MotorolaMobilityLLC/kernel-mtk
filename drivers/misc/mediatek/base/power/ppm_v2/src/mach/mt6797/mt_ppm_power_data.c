...
int *ppm_get_perf_idx_ref_tbl(enum ppm_cluster cluster)
{
	...
	return (ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_SB) ? cpu_perf_idx_ref_tbl_SB[cluster]
		: (ppm_main_info.dvfs_tbl_type == DVFS_TABLE_TYPE_MB) ? cpu_perf_idx_ref_tbl_MB[cluster]
...
void ppm_power_data_init(void)
{
	...
	unsigned int lkg_ratio, p_TT, p_FF, diff = BIG_LKG_EFUSE_FF - BIG_LKG_EFUSE_TT;

	/* get big leakage efuse and calculate ratio */
