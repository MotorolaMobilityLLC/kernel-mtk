...
void ppm_update_req_by_pwr(enum ppm_power_state new_state,
...
int ppm_find_pwr_idx(struct ppm_cluster_status *cluster_status)
{
	...
	for_each_ppm_clusters(i) {
		...
		if (core != 0 && opp >= 0 && opp < DVFS_OPP_NUM) {
			pwr_idx +=
			    (ref_tbl.pwr_idx_ref_tbl[i].core_total_power[opp] *
			     core) +
...
ppm_judge_state_by_user_limit(enum ppm_power_state cur_state,
			      struct ppm_userlimit_data user_limit)
{
	...
	ppm_ver("Judge: input --> [%s] (%d)(%d)(%d)(%d) [(%d)(%d)]\n",
		...
		L_core_min, L_core_max, LL_freq_min, L_freq_max);
	...
	case PPM_POWER_STATE_LL_ONLY:
		new_state =
			...
			: (L_core_min <= 0 || L_core_max == 0)
			      ? cur_state
			       /* should not go to L only due to root cluster is
				* fixed at LL
				*/
			      : (L_core_min > 0 &&
				 ppm_main_info.fixed_root_cluster == 0)
	...
	case PPM_POWER_STATE_L_ONLY:
		new_state =
			...
			: (LL_core_min <= 0 || LL_core_max == 0)
			      ? cur_state
			      /* keep current if for only LL min is set */
			      : (LL_core_min > 0 && L_core_min == -1 &&
				 L_freq_max >= LL_freq_min)
				    ? cur_state
				    /* will return NONE if LL min */
				    /* is set but L min is not */
				    : (L_core_min == 4)
					  ? PPM_POWER_STATE_4L_LL
					  : (LL_core_min == 4)
						? PPM_POWER_STATE_4LL_L
...
void ppm_limit_check_for_user_limit(enum ppm_power_state cur_state,
				    struct ppm_policy_req *req,
				    struct ppm_userlimit_data user_limit)
{
	if (req && cur_state == PPM_POWER_STATE_L_ONLY) {
		...
		if (LL_min_core > 0 && L_max_freq >= LL_min_freq) {
			...
			} else if (ppm_main_info.fix_state_by_segment ==
				   PPM_POWER_STATE_L_ONLY) {
				req->limit[0].min_cpu_core = 0;
				req->limit[1].min_cpu_core =
				    min(sum, get_cluster_max_cpu_core(1));
			...
			if (LL_min_freq > L_min_freq) {
				req->limit[1].min_cpufreq_idx =
				    ppm_main_freq_to_idx(1, LL_min_freq,
						CPUFREQ_RELATION_L);
