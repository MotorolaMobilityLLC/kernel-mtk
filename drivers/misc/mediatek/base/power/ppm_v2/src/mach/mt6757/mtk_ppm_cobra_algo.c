...
static int freq_idx_mapping_tbl_fy_r[DVFS_OPP_NUM] = {0, 1, 1, 2, 2, 3, 3, 3,
						      4, 4, 5, 5, 6, 6, 7, 7};
static int freq_idx_mapping_tbl_sb[COBRA_OPP_NUM] = {0, 2, 4, 7, 9, 11, 13, 15};
static int freq_idx_mapping_tbl_sb_r[DVFS_OPP_NUM] = {0, 1, 1, 2, 2, 3, 3, 3,
						      4, 4, 5, 5, 6, 6, 7, 7};
			      ...
			      unsigned int opp)
{
	...
	cur_pwr =
	    (L_core)
		? cobra_tbl.basic_pwr_tbl[idx_L + L_core - 1][cur_opp].power_idx
		: 0; /* L */
	cur_pwr += (LL_core)
		       ? cobra_tbl.basic_pwr_tbl[idx_LL + LL_core - 1][cur_opp]
			     .power_idx
		       : 0; /* L+LL */
	...
	if (opp == COBRA_OPP_NUM - 1) {
		prev_pwr =
			...
			? ((L_core > 1)
			       ? (cobra_tbl
				      .basic_pwr_tbl[idx_L + L_core - 2]
						    [cur_opp]
				      .power_idx +
				  ((LL_core)
				       ? cobra_tbl
					     .basic_pwr_tbl[idx_LL + LL_core -
							    1][cur_opp]
					     .power_idx
				       : 0))
			       : ((LL_core)
				      ? cobra_tbl
					    .basic_pwr_tbl[idx_LL + LL_core - 1]
							  [cur_opp]
					    .power_idx
				      : 0))
			: ((LL_core > 1)
			       ? cobra_tbl
				     .basic_pwr_tbl[idx_LL + LL_core - 2]
						   [cur_opp]
				     .power_idx
			       : 0);
	} else {
		...
		prev_pwr =
		    (L_core)
			? cobra_tbl.basic_pwr_tbl[idx_L + L_core - 1][prev_opp]
			      .power_idx
			: 0;
		prev_pwr +=
		    (LL_core)
			? cobra_tbl
			      .basic_pwr_tbl[idx_LL + LL_core - 1][prev_opp]
			      .power_idx
			: 0;
	}
...
void ppm_cobra_update_limit(enum ppm_power_state new_state, void *user_req)
{
	...
	ppm_dbg(COBRA_ALGO,
		"[PREV]Core_Limit=%d%d, policy_limit=%d%d, state=%s\n",
		Core_limit[PPM_CLUSTER_LL], Core_limit[PPM_CLUSTER_L],
	...
	for_each_ppm_clusters(i) {
		...
		if (!cl_status[i].core_num)
			...
			cl_status[i].freq_idx = ppm_main_freq_to_idx(
			    i, mt_cpufreq_get_cur_phy_freq_no_lock(i),
			    CPUFREQ_RELATION_L);

		ppm_ver("[%d] core = %d, freq_idx = %d\n", i,
			cl_status[i].core_num, cl_status[i].freq_idx);
	}
	...
	for_each_ppm_clusters(i) {
		...
		opp[i] = (cl_status[i].freq_idx >= 0)
			     ? freq_idx_mapping_tbl_r[cl_status[i].freq_idx]
			     : -1;
#else
		...
		active_core[i] =
		    (cl_status[i].core_num >= 0) ? cl_status[i].core_num : 0;
	...
	ppm_dbg(COBRA_ALGO, "[IN](bgt/delta/cur)=(%d/%d/%d), (opp/act/c_lmt)=(%d,%d/%d%d/%d%d)\n",
		...
		opp[PPM_CLUSTER_L], ACT_CORE(LL), ACT_CORE(L), CORE_LIMIT(LL),
		CORE_LIMIT(L));
		...
		while (1) {
			...
			if (opp[LxLL] > 0) {
				target_delta_pwr = get_delta_pwr_LxLL(
				    ACT_CORE(L), ACT_CORE(LL), opp[LxLL] - 1);
			...
			if (new_state != PPM_POWER_STATE_L_ONLY) {
				while (
					...
					target_delta_pwr = get_delta_pwr_LxLL(
					    ACT_CORE(L), CORE_LIMIT(LL) + 1,
					    COBRA_OPP_NUM - 1);
					if (delta_power < target_delta_pwr)
						break;

					delta_power -= target_delta_pwr;
					req->limit[PPM_CLUSTER_LL]
					    .max_cpu_core = ++CORE_LIMIT(LL);
				}
			}
			...
			ppm_dbg(COBRA_ALGO, "[+]ChoosenCl=-1! delta=%d, (opp/c_lmt)=(%d,%d/%d%d)\n",
				...
				opp[PPM_CLUSTER_L], CORE_LIMIT(LL),
				CORE_LIMIT(L));
...
prepare_next_round:
		...
		while (delta_power < 0) {
			...
			if (opp[LxLL] >= 0 &&
				...
				ChoosenCl = 1;
				ChoosenPwr = get_delta_pwr_LxLL(
				    ACT_CORE(L), ACT_CORE(LL), opp[LxLL]);
			...
			if (opp[LxLL] == PPM_COBRA_MAX_FREQ_IDX) {
				if (ACT_CORE(L) > 1 ||
				    (ACT_CORE(LL) > 0 && ACT_CORE(L) > 0))
					req->limit[PPM_CLUSTER_L].max_cpu_core =
					    --ACT_CORE(L);
				else if (ACT_CORE(LL) > 1)
					req->limit[PPM_CLUSTER_LL]
					    .max_cpu_core = --ACT_CORE(LL);
				if (ACT_CORE(L) > 0)
					opp[PPM_CLUSTER_L] =
...
if (opp[PPM_CLUSTER_L] >= 0 && ACT_CORE(L) > 0)
	...
	ppm_dbg(
	    ...
	    delta_power, opp[PPM_CLUSTER_LL], opp[PPM_CLUSTER_L], ACT_CORE(LL),
	    ACT_CORE(L), req->limit[PPM_CLUSTER_LL].max_cpu_core,
	    req->limit[PPM_CLUSTER_L].max_cpu_core,
	    req->limit[PPM_CLUSTER_LL].max_cpufreq_idx,
	    req->limit[PPM_CLUSTER_L].max_cpufreq_idx);
...
void ppm_cobra_init(void)
{
	...
	for (i = 0; i < TOTAL_CORE_NUM; i++) {
		for (j = 0; j < DVFS_OPP_NUM; j++) {
			...
			cobra_tbl.basic_pwr_tbl[i][j].power_idx =
			    pwr_ref_tbl.pwr_idx_ref_tbl[i / 4]
				    .core_total_power[j] *
				core +
	...
	for_each_ppm_power_state(i) {
		for_each_ppm_clusters(j) {
			int max_core = state_info[i]
					   .cluster_limit->state_limit[j]
					   .max_cpu_core;
			int min_core = state_info[i]
					   .cluster_limit->state_limit[j]
					   .min_cpu_core;
			int idx = 4 * j;
			...
			state_info[i].min_pwr_idx +=
			    (min_core)
				? cobra_tbl
				      .basic_pwr_tbl[idx + min_core - 1]
						    [DVFS_OPP_NUM - 1]
				      .power_idx
				: 0;
			state_info[i].max_perf_idx +=
			    (max_core)
				? cobra_tbl.basic_pwr_tbl[idx + max_core - 1][0]
				      .perf_idx
				: 0;
		}
		ppm_info("%s: min_pwr_idx = %d, max_perf_idx = %d\n",
			 state_info[i].name, state_info[i].min_pwr_idx,
			 state_info[i].max_perf_idx);
	}
	...
	for (i = 0; i <= get_cluster_max_cpu_core(PPM_CLUSTER_L); i++) {
		for (j = 0; j <= get_cluster_max_cpu_core(PPM_CLUSTER_LL);
			...
			for (k = 0; k < COBRA_OPP_NUM; k++) {
				...
				if (i == 0 && j == 0) {
					cobra_tbl.delta_tbl_LxLL[i][j][k]
					    .delta_pwr = 0;
				...
				cur_pwr =
				    (i)
					? cobra_tbl
					      .basic_pwr_tbl[idx_L + i - 1][opp]
					      .power_idx
					: 0; /* L */
				cur_pwr += (j)
					       ? cobra_tbl
						     .basic_pwr_tbl[idx_LL + j -
								    1][opp]
						     .power_idx
					       : 0; /* L+LL */
...
static unsigned int get_limit_opp_and_budget(void)
{
	...
	for (i = 0; i <= get_cluster_min_cpufreq_idx(PPM_CLUSTER_LL);
	     i++) { /* LLxL */
...
static void ppm_cobra_lookup_by_budget(struct seq_file *m)
{
	...
	for (i = get_cluster_max_cpu_core(PPM_CLUSTER_L); i >= 0; i--) {
		for (j = get_cluster_max_cpu_core(PPM_CLUSTER_LL); j >= 0;
		     j--) {
			if (!i && !j)
				continue;
			...
			if (power) {
				seq_printf(
				    m, "(%d, %2d, %d, %2d) = %4d\n", j,
				    cobra_lookup_data.limit[PPM_CLUSTER_LL].opp,
				    i,
...
static void ppm_cobra_lookup_by_limit(struct seq_file *m)
{
	...
	for_each_ppm_clusters(i) {
		...
		if (core)
			budget += cobra_tbl.basic_pwr_tbl[4 * i + core - 1][opp]
				      .power_idx;
