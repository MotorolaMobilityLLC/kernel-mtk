...
int mt_ocp_set_volt(enum ocp_cluster cluster, unsigned int volt_mv)
{
	int ret = -1;
...
static int ocp_status_dump_proc_show(struct seq_file *m, void *v)
{
	...
	for_each_ocp_cluster(i) {
		...
		seq_printf(m, "Cluster %d total_lkg = %dmA\n", i, ocp_val_status(i, TOTAL_LKG));
...
static int ocp_auto_capture_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < ocp_info.auto_capture_total_cnt; i++) {
		for_each_ocp_cluster(j) {
			...
			seq_printf(m, "Cluster_%d_TotLkg      = %d mA\n",
					j, ocp_info.cl_setting[j].status[i].total_lkg);
						...
						size_t count, loff_t *pos)
{
	...
	if (sscanf(buf, "%d %d %d", &cluster, &enable, &mode) == 3) {
		...
		if (enable > 0) {
			...
			if (ocp_is_available(cluster)) {
				/* turn off lkgmon for online cores */
				while (--cpus) {
					mt_secure_call_ocp(MTK_SIP_KERNEL_OCPLKGMONENDIS,
						cluster, cpus, false);
				}
...
#if defined(CONFIG_CPU_FREQ) && !defined(OCP_SSPM_SUPPORT)
static int ocp_cpu_freq_callback(struct notifier_block *nb,
...
static struct notifier_block ocp_cpu_freq_notifier = {
	.notifier_call = ocp_cpu_freq_callback,
};

static void ocp_cpu_volt_callback(enum mt_cpu_dvfs_id id, unsigned int mv, int up, int event)
{
...
#if defined(CONFIG_CPU_FREQ) && !defined(OCP_SSPM_SUPPORT)
	cpufreq_register_notifier(&ocp_cpu_freq_notifier, CPUFREQ_TRANSITION_NOTIFIER);
	mt_cpufreq_setvolt_ocp_registerCB(ocp_cpu_volt_callback);
#endif
...
#if defined(CONFIG_CPU_FREQ) && !defined(OCP_SSPM_SUPPORT)
	cpufreq_unregister_notifier(&ocp_cpu_freq_notifier, CPUFREQ_TRANSITION_NOTIFIER);
#endif
