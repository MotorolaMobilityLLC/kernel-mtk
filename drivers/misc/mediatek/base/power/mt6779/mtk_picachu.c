...
#define PICACHU_ENABLE_LOO (0)

#if PICACHU_ENABLE_LOO
/*
...
#if PICACHU_ENABLE_LOO
	[MT_PICACHU_LITTLE_VPROC] = {EEM_CTRL_L_HI, EEM_CTRL_L, EEM_CTRL_CCI},
	[MT_PICACHU_BIG_VPROC] = {EEM_CTRL_B_HI, EEM_CTRL_B, -1},
#else
	[MT_PICACHU_LITTLE_VPROC] = {EEM_CTRL_L, EEM_CTRL_CCI},
	[MT_PICACHU_BIG_VPROC] = {EEM_CTRL_B, -1},
#endif
};
				       ...
				       struct picachu_info *p)
{
	...
	for (i = 0; i < NR_EEM_EFUSE_PER_VPROC; i++) {
		...
		if (p->pi_dvtfixed == PICACHU_DVTFIXED)
			eem_set_pi_dvtfixed(eem_ctrl_id[id][i], p->pi_dvtfixed);
	}
}
#else
static void picachu_apply_efuse_to_eem(enum mt_picachu_vproc_id id,
...
static void picachu_get_data(enum mt_picachu_vproc_id vproc_id)
{
	...
	if (tmp == PICACHU_SIGNATURE) {
		p->vmin = val & 0xFF;
		p->pi_offset = (val >> 8) & 0xFF;
		p->pi_dvtfixed = (val >> 16) & 0xF;
	}
