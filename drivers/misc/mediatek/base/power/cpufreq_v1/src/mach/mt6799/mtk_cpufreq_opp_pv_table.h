/*
* Copyright (C) 2016 MediaTek Inc.
*
* This program is free software; you can redistribute it and/or modify
* it under the terms of the GNU General Public License version 2 as
* published by the Free Software Foundation.
*
* This program is distributed in the hope that it will be useful,
* but WITHOUT ANY WARRANTY; without even the implied warranty of
* MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
* See http://www.gnu.org/licenses/gpl-2.0.html for more details.
*/

#include "mtk_cpufreq_config.h"

#define NR_FREQ		16
#define ARRAY_COL_SIZE	4

static unsigned int fyTbl[NR_FREQ * NR_MT_CPU_DVFS][ARRAY_COL_SIZE] = {
/* Freq, Vproc, post_div, clk_div */
	{1248, 0x32A, 2, 1},/* (LL) */
	{1209, 0x316, 2, 1},
	{1170, 0x307, 2, 1},
	{1118, 0x2F3, 2, 1},
	{1066, 0x2DF, 2, 1},
	{1027, 0x2CB, 2, 1},
	{975, 0x2B7, 4, 1},
	{923, 0x2A3, 4, 1},
	{858, 0x28A, 4, 1},
	{806, 0x276, 4, 1},
	{741, 0x270, 4, 1},
	{663, 0x270, 4, 1},
	{585, 0x270, 4, 1},
	{520, 0x270, 4, 1},
	{442, 0x270, 4, 2},
	{351, 0x270, 4, 2},

	{1378, 0x32A, 2, 1},/* (L) */
	{1313, 0x316, 2, 1},
	{1274, 0x307, 2, 1},
	{1209, 0x2F3, 2, 1},
	{1144, 0x2DF, 2, 1},
	{1079, 0x2CB, 2, 1},
	{1014, 0x2B7, 2, 1},
	{962, 0x2A3, 4, 1},
	{884, 0x28A, 4, 1},
	{819, 0x276, 4, 1},
	{741, 0x270, 4, 1},
	{663, 0x270, 4, 1},
	{585, 0x270, 4, 1},
	{520, 0x270, 4, 1},
	{442, 0x270, 4, 2},
	{338, 0x270, 4, 2},

	{1638, 0x32A, 2, 1},/* (B) */
	{1534, 0x316, 2, 1},
	{1469, 0x307, 2, 1},
	{1378, 0x2F3, 2, 1},
	{1287, 0x2DF, 2, 1},
	{1196, 0x2CB, 2, 1},
	{1105, 0x2B7, 2, 1},
	{1014, 0x2A3, 2, 1},
	{897, 0x28A, 4, 1},
	{806, 0x276, 4, 1},
	{715, 0x270, 4, 1},
	{650, 0x270, 4, 1},
	{572, 0x270, 4, 1},
	{507, 0x270, 4, 1},
	{429, 0x270, 4, 2},
	{338, 0x270, 4, 2},

	{754, 0x32A, 4, 1},/* (CCI) */
	{715, 0x316, 4, 1},
	{689, 0x307, 4, 1},
	{650, 0x2F3, 4, 1},
	{611, 0x2DF, 4, 1},
	{585, 0x2CB, 4, 1},
	{546, 0x2B7, 4, 1},
	{507, 0x2A3, 4, 1},
	{468, 0x28A, 4, 2},
	{429, 0x276, 4, 2},
	{390, 0x270, 4, 2},
	{338, 0x270, 4, 2},
	{299, 0x270, 4, 2},
	{247, 0x270, 4, 4},
	{208, 0x270, 4, 4},
	{156, 0x270, 4, 4},
};

static unsigned int fyaTbl[NR_FREQ * NR_MT_CPU_DVFS][ARRAY_COL_SIZE] = {
/* Freq, Vproc, post_div, clk_div */
	{1586, 0x32A, 2, 1},/* (LL) */
	{1495, 0x316, 2, 1},
	{1430, 0x307, 2, 1},
	{1352, 0x2F3, 2, 1},
	{1261, 0x2DF, 2, 1},
	{1183, 0x2CB, 2, 1},
	{1092, 0x2B7, 2, 1},
	{1014, 0x2A3, 2, 1},
	{910, 0x28A, 4, 1},
	{819, 0x276, 4, 1},
	{741, 0x270, 4, 1},
	{663, 0x270, 4, 1},
	{585, 0x270, 4, 1},
	{520, 0x270, 4, 1},
	{442, 0x270, 4, 2},
	{351, 0x270, 4, 2},

	{1690, 0x32A, 2, 1},/* (L) */
	{1586, 0x316, 2, 1},
	{1521, 0x307, 2, 1},
	{1417, 0x2F3, 2, 1},
	{1326, 0x2DF, 2, 1},
	{1235, 0x2CB, 2, 1},
	{1131, 0x2B7, 2, 1},
	{1040, 0x2A3, 2, 1},
	{923, 0x28A, 4, 1},
	{832, 0x276, 4, 1},
	{741, 0x270, 4, 1},
	{663, 0x270, 4, 1},
	{585, 0x270, 4, 1},
	{520, 0x270, 4, 1},
	{442, 0x270, 4, 2},
	{338, 0x270, 4, 2},

	{1976, 0x32A, 2, 1},/* (B) */
	{1846, 0x316, 2, 1},
	{1742, 0x307, 2, 1},
	{1612, 0x2F3, 2, 1},
	{1495, 0x2DF, 2, 1},
	{1365, 0x2CB, 2, 1},
	{1235, 0x2B7, 2, 1},
	{1105, 0x2A3, 2, 1},
	{949, 0x28A, 4, 1},
	{819, 0x276, 4, 1},
	{715, 0x270, 4, 1},
	{650, 0x270, 4, 1},
	{572, 0x270, 4, 1},
	{507, 0x270, 4, 1},
	{429, 0x270, 4, 2},
	{338, 0x270, 4, 2},

	{949, 0x32A, 4, 1},/* (CCI) */
	{884, 0x316, 4, 1},
	{845, 0x307, 4, 1},
	{793, 0x2F3, 4, 1},
	{728, 0x2DF, 4, 1},
	{676, 0x2CB, 4, 1},
	{624, 0x2B7, 4, 1},
	{559, 0x2A3, 4, 1},
	{494, 0x28A, 4, 2},
	{442, 0x276, 4, 2},
	{390, 0x270, 4, 2},
	{338, 0x270, 4, 2},
	{299, 0x270, 4, 2},
	{247, 0x270, 4, 4},
	{208, 0x270, 4, 4},
	{156, 0x270, 4, 4},
};

static unsigned int fybTbl[NR_FREQ * NR_MT_CPU_DVFS][ARRAY_COL_SIZE] = {
/* Freq, Vproc, post_div, clk_div */
	{1755, 0x32A, 2, 1},/* (LL) */
	{1651, 0x316, 2, 1},
	{1573, 0x307, 2, 1},
	{1469, 0x2F3, 2, 1},
	{1365, 0x2DF, 2, 1},
	{1261, 0x2CB, 2, 1},
	{1157, 0x2B7, 2, 1},
	{1053, 0x2A3, 2, 1},
	{936, 0x28A, 4, 1},
	{832, 0x276, 4, 1},
	{741, 0x270, 4, 1},
	{663, 0x270, 4, 1},
	{585, 0x270, 4, 1},
	{520, 0x270, 4, 1},
	{442, 0x270, 4, 2},
	{351, 0x270, 4, 2},

	{1872, 0x32A, 2, 1},/* (L) */
	{1755, 0x316, 2, 1},
	{1664, 0x307, 2, 1},
	{1547, 0x2F3, 2, 1},
	{1430, 0x2DF, 2, 1},
	{1326, 0x2CB, 2, 1},
	{1209, 0x2B7, 2, 1},
	{1092, 0x2A3, 2, 1},
	{949, 0x28A, 4, 1},
	{832, 0x276, 4, 1},
	{741, 0x270, 4, 1},
	{663, 0x270, 4, 1},
	{585, 0x270, 4, 1},
	{520, 0x270, 4, 1},
	{442, 0x270, 4, 2},
	{338, 0x270, 4, 2},

	{2197, 0x32A, 1, 1},/* (B) */
	{2041, 0x316, 1, 1},
	{1924, 0x307, 2, 1},
	{1768, 0x2F3, 2, 1},
	{1625, 0x2DF, 2, 1},
	{1469, 0x2CB, 2, 1},
	{1313, 0x2B7, 2, 1},
	{1170, 0x2A3, 2, 1},
	{975, 0x28A, 4, 1},
	{819, 0x276, 4, 1},
	{715, 0x270, 4, 1},
	{650, 0x270, 4, 1},
	{572, 0x270, 4, 1},
	{507, 0x270, 4, 1},
	{429, 0x270, 4, 2},
	{338, 0x270, 4, 2},

	{1053, 0x32A, 2, 1},/* (CCI) */
	{975, 0x316, 4, 1},
	{923, 0x307, 4, 1},
	{858, 0x2F3, 4, 1},
	{793, 0x2DF, 4, 1},
	{728, 0x2CB, 4, 1},
	{663, 0x2B7, 4, 1},
	{598, 0x2A3, 4, 1},
	{507, 0x28A, 4, 1},
	{442, 0x276, 4, 2},
	{390, 0x270, 4, 2},
	{338, 0x270, 4, 2},
	{299, 0x270, 4, 2},
	{247, 0x270, 4, 4},
	{208, 0x270, 4, 4},
	{156, 0x270, 4, 4},
};

static unsigned int sbTbl[NR_FREQ * NR_MT_CPU_DVFS][ARRAY_COL_SIZE] = {
/* Freq, Vproc, post_div, clk_div */
	{1898, 0x359, 2, 1},/* (LL) */
	{1863, 0x345, 2, 1},
	{1828, 0x331, 2, 1},
	{1769, 0x31D, 2, 1},
	{1678, 0x304, 2, 1},
	{1604, 0x2F0, 2, 1},
	{1512, 0x2D7, 2, 1},
	{1421, 0x2BE, 2, 1},
	{1347, 0x2AA, 2, 1},
	{1237, 0x28C, 2, 1},
	{1145, 0x273, 2, 1},
	{1005, 0x25A, 2, 1},
	{830, 0x23C, 4, 1},
	{648, 0x21D, 4, 1},
	{449, 0x1FB, 4, 2},
	{249, 0x1D9, 4, 4},

	{2197, 0x359, 1, 1},/* (L) */
	{2157, 0x345, 1, 1},
	{2117, 0x331, 1, 1},
	{2040, 0x31D, 1, 1},
	{1917, 0x304, 2, 1},
	{1818, 0x2F0, 2, 1},
	{1694, 0x2D7, 2, 1},
	{1571, 0x2BE, 2, 1},
	{1472, 0x2AA, 2, 1},
	{1324, 0x28C, 2, 1},
	{1200, 0x273, 2, 1},
	{1053, 0x25A, 2, 1},
	{873, 0x23C, 4, 1},
	{687, 0x21D, 4, 1},
	{484, 0x1FB, 4, 2},
	{279, 0x1D9, 4, 2},

	{2496, 0x359, 1, 1},/* (B) */
	{2452, 0x345, 1, 1},
	{2408, 0x331, 1, 1},
	{2318, 0x31D, 1, 1},
	{2173, 0x304, 1, 1},
	{2057, 0x2F0, 1, 1},
	{1911, 0x2D7, 2, 1},
	{1766, 0x2BE, 2, 1},
	{1649, 0x2AA, 2, 1},
	{1475, 0x28C, 2, 1},
	{1329, 0x273, 2, 1},
	{1169, 0x25A, 2, 1},
	{974, 0x23C, 4, 1},
	{771, 0x21D, 4, 1},
	{551, 0x1FB, 4, 1},
	{328, 0x1D9, 4, 2},

	{1196, 0x359, 2, 1},/* (CCI) */
	{1172, 0x345, 2, 1},
	{1149, 0x331, 2, 1},
	{1108, 0x31D, 2, 1},
	{1043, 0x304, 2, 1},
	{991, 0x2F0, 4, 1},
	{927, 0x2D7, 4, 1},
	{862, 0x2BE, 4, 1},
	{810, 0x2AA, 4, 1},
	{733, 0x28C, 4, 1},
	{668, 0x273, 4, 1},
	{587, 0x25A, 4, 1},
	{488, 0x23C, 4, 2},
	{385, 0x21D, 4, 2},
	{272, 0x1FB, 4, 2},
	{159, 0x1D9, 4, 4},
};

static unsigned int sbaTbl[NR_FREQ * NR_MT_CPU_DVFS][ARRAY_COL_SIZE] = {
/* Freq, Vproc, post_div, clk_div */
	{1898, 0x369, 2, 1},/* (LL) */
	{1863, 0x345, 2, 1},
	{1828, 0x331, 2, 1},
	{1769, 0x31D, 2, 1},
	{1678, 0x304, 2, 1},
	{1604, 0x2F0, 2, 1},
	{1512, 0x2D7, 2, 1},
	{1421, 0x2BE, 2, 1},
	{1347, 0x2AA, 2, 1},
	{1237, 0x28C, 2, 1},
	{1145, 0x273, 2, 1},
	{1005, 0x25A, 2, 1},
	{830, 0x23C, 4, 1},
	{648, 0x21D, 4, 1},
	{449, 0x1FB, 4, 2},
	{249, 0x1D9, 4, 4},

	{2197, 0x369, 1, 1},/* (L) */
	{2157, 0x345, 1, 1},
	{2117, 0x331, 1, 1},
	{2040, 0x31D, 1, 1},
	{1917, 0x304, 2, 1},
	{1818, 0x2F0, 2, 1},
	{1694, 0x2D7, 2, 1},
	{1571, 0x2BE, 2, 1},
	{1472, 0x2AA, 2, 1},
	{1324, 0x28C, 2, 1},
	{1200, 0x273, 2, 1},
	{1053, 0x25A, 2, 1},
	{873, 0x23C, 4, 1},
	{687, 0x21D, 4, 1},
	{484, 0x1FB, 4, 2},
	{279, 0x1D9, 4, 2},

	{2600, 0x369, 1, 1},/* (B) */
	{2452, 0x345, 1, 1},
	{2408, 0x331, 1, 1},
	{2318, 0x31D, 1, 1},
	{2173, 0x304, 1, 1},
	{2057, 0x2F0, 1, 1},
	{1911, 0x2D7, 2, 1},
	{1766, 0x2BE, 2, 1},
	{1649, 0x2AA, 2, 1},
	{1475, 0x28C, 2, 1},
	{1329, 0x273, 2, 1},
	{1169, 0x25A, 2, 1},
	{974, 0x23C, 4, 1},
	{771, 0x21D, 4, 1},
	{551, 0x1FB, 4, 1},
	{328, 0x1D9, 4, 2},

	{1196, 0x369, 2, 1},/* (CCI) */
	{1172, 0x345, 2, 1},
	{1149, 0x331, 2, 1},
	{1108, 0x31D, 2, 1},
	{1043, 0x304, 2, 1},
	{991, 0x2F0, 4, 1},
	{927, 0x2D7, 4, 1},
	{862, 0x2BE, 4, 1},
	{810, 0x2AA, 4, 1},
	{733, 0x28C, 4, 1},
	{668, 0x273, 4, 1},
	{587, 0x25A, 4, 1},
	{488, 0x23C, 4, 2},
	{385, 0x21D, 4, 2},
	{272, 0x1FB, 4, 2},
	{159, 0x1D9, 4, 4},
};

static unsigned int sbbTbl[NR_FREQ * NR_MT_CPU_DVFS][ARRAY_COL_SIZE] = {
/* Freq, Vproc, post_div, clk_div */
	{2000, 0x32A, 1, 1},/* (LL) */
	{1912, 0x316, 2, 1},
	{1847, 0x307, 2, 1},
	{1759, 0x2F3, 2, 1},
	{1672, 0x2DF, 2, 1},
	{1585, 0x2CB, 2, 1},
	{1497, 0x2B7, 2, 1},
	{1410, 0x2A3, 2, 1},
	{1301, 0x28A, 2, 1},
	{1214, 0x276, 2, 1},
	{1071, 0x25D, 2, 1},
	{917, 0x244, 4, 1},
	{764, 0x22B, 4, 1},
	{610, 0x212, 4, 1},
	{457, 0x1F9, 4, 2},
	{261, 0x1DA, 4, 2},

	{2200, 0x32A, 1, 1},/* (L) */
	{2093, 0x316, 1, 1},
	{2014, 0x307, 1, 1},
	{1907, 0x2F3, 2, 1},
	{1801, 0x2DF, 2, 1},
	{1695, 0x2CB, 2, 1},
	{1589, 0x2B7, 2, 1},
	{1482, 0x2A3, 2, 1},
	{1349, 0x28A, 2, 1},
	{1243, 0x276, 2, 1},
	{1095, 0x25D, 2, 1},
	{941, 0x244, 4, 1},
	{788, 0x22B, 4, 1},
	{634, 0x212, 4, 1},
	{480, 0x1F9, 4, 2},
	{285, 0x1DA, 4, 2},

	{2665, 0x32A, 1, 1},/* (B) */
	{2522, 0x316, 1, 1},
	{2418, 0x307, 1, 1},
	{2262, 0x2F3, 1, 1},
	{2119, 0x2DF, 1, 1},
	{1976, 0x2CB, 2, 1},
	{1833, 0x2B7, 2, 1},
	{1690, 0x2A3, 2, 1},
	{1508, 0x28A, 2, 1},
	{1365, 0x276, 2, 1},
	{1196, 0x25D, 2, 1},
	{1027, 0x244, 2, 1},
	{871, 0x22B, 4, 1},
	{702, 0x212, 4, 1},
	{533, 0x1F9, 4, 1},
	{325, 0x1DA, 4, 2},

	{1250, 0x32A, 2, 1},/* (CCI) */
	{1189, 0x316, 2, 1},
	{1144, 0x307, 2, 1},
	{1084, 0x2F3, 2, 1},
	{1023, 0x2DF, 2, 1},
	{963, 0x2CB, 4, 1},
	{903, 0x2B7, 4, 1},
	{842, 0x2A3, 4, 1},
	{767, 0x28A, 4, 1},
	{707, 0x276, 4, 1},
	{623, 0x25D, 4, 1},
	{536, 0x244, 4, 1},
	{450, 0x22B, 4, 2},
	{363, 0x212, 4, 2},
	{276, 0x1F9, 4, 2},
	{166, 0x1DA, 4, 4},
};

unsigned int *xrecordTbl[NUM_CPU_LEVEL] = {
	[CPU_LEVEL_0] = &fyTbl[0][0],
	[CPU_LEVEL_1] = &sbTbl[0][0],
	[CPU_LEVEL_2] = &fyaTbl[0][0],
	[CPU_LEVEL_3] = &fybTbl[0][0],
	[CPU_LEVEL_4] = &sbaTbl[0][0],
	[CPU_LEVEL_5] = &sbbTbl[0][0],
};
