...
unsigned int __spm_get_vcore_volt_pmic_val(bool is_vcore_volt_lower, int ch)
{
	...
	vcore_volt_pmic_val =
		(is_vcore_volt_lower) ?
		VOLT_TO_PMIC_VAL(60000) : get_vcore_ptp_volt(opp);
...
void __spm_get_wakeup_status(struct wake_status *wakesta)
{
	...
	/* get wakeup event */
	wakesta->r12 =
		spm_read(SPM_SW_RSV_0);        /* backup of PCM_REG12_DATA */
	...
	wakesta->wake_misc =
		spm_read(SPM_BSI_D0_SR);	/* backup of SPM_WAKEUP_MISC */

	/* get sleep time */
	wakesta->timer_out =
		spm_read(SPM_BSI_D1_SR);	/* backup of PCM_TIMER_OUT */
...
enum wk_reason __spm_output_wake_reason(const struct wake_status *wakesta,
	const struct pcm_desc *pcmdesc, bool suspend, const char *scenario)
{
	...
	log_size += sprintf(log_buf, "wake up by %s, timer_out = %u\
		,r13 = 0x%x, debug_flag = 0x%x, ch = %u\n",
		buf, wakesta->timer_out, wakesta->r13,
		wakesta->debug_flag, wakesta->dcs_ch);
	...
	log_size += sprintf(log_buf + log_size,
		"raw_ext_sta = 0x%x\
		,wake_misc = 0x%x\
		,pcm_flag = 0x%x 0x%x 0x%x\
		,req = 0x%x, resource = 0x%x\n",
		wakesta->raw_ext_sta,
		wakesta->wake_misc,
...
void spm_set_dummy_read_addr(int debug)
{
	...
	if (debug)
		spm_crit(
		"dummy read addr(4GB: %d): rank0: 0x%llx, rank1: 0x%llx\n",
...
void __spm_set_pcm_wdt(int en)
{
	...
	if (en) {
		...
		spm_write(PCM_WDT_VAL,
			spm_read(PCM_TIMER_VAL) + PCM_WDT_TIMEOUT);
		spm_write(PCM_CON1,
			con1 | SPM_REGWR_CFG_KEY | RG_PCM_WDT_EN_LSB);
	} else {
...
u32 _spm_get_wake_period(int pwake_time, enum wk_reason last_wr)
{
...
int get_channel_lock(bool blocking)
{
	...
	if (ret) {
		aee_kernel_warning("DCS Warring",
			"dcs_get_dcs_status_lock busy");
		return -1;
	}
