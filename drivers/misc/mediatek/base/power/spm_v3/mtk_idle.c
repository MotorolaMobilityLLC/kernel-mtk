...
#define SODI_PATH			"/sys/kernel/debug/cpuidle/soidle_state"
#define SODI3_PATH		"/sys/kernel/debug/cpuidle/soidle3_state"
#define DPIDLE_PATH		"/sys/kernel/debug/cpuidle/dpidle_state"
#define MCDI_PATH			"/sys/kernel/debug/cpuidle/mcidle_state"
#define SLIDLE_PATH		"/sys/kernel/debug/cpuidle/slidle_state"
#define RGIDLE_PATH		"/sys/kernel/debug/cpuidle/rgidle_state"
#define IDLE_STAT_PATH		"/sys/kernel/debug/cpuidle/idle_state"
...
enum wk_reason __attribute__((weak)) spm_go_to_dpidle(u32 spm_flags,
	u32 spm_data, u32 log_cond, u32 operation_cond)
{
	go_to_wfi();
...
enum wk_reason __attribute__((weak)) spm_go_to_sodi3(u32 spm_flags,
	u32 spm_data, u32 sodi_flags, u32 operation_cond)
{
	return WR_UNKNOWN;
}

enum wk_reason __attribute__((weak)) spm_go_to_sodi(u32 spm_flags,
	u32 spm_data, u32 sodi_flags, u32 operation_cond)
{
	return WR_UNKNOWN;
}
...
void __attribute__((weak)) spm_go_to_mcsodi(u32 spm_flags,
	u32 spm_data, u32 sodi_flags, u32 operation_cond)
{
}
...
void __attribute__((weak))
	spm_mcdi_switch_on_off(enum spm_mcdi_lock_id id, int mcdi_en)
{
...
/* DeepIdle */
static unsigned int     dpidle_time_criteria = 1200 * 13; /* 1200us */
...
int mtk_idle_notifier_register(struct notifier_block *n)
{
	...
	if (symname) {
		pr_info("[mt_idle_ntf] <%02d>%08lx (%s)\n",
			index++, (unsigned long)n->notifier_call, symname);
	} else {
		pr_info("[mt_idle_ntf] <%02d>%08lx\n",
...
static bool soidle3_can_enter(int cpu, int reason)
{
	...
	if (sodi3_by_uptime_count != -1) {
		...
		if (val <= SODI3_UPTIME) {
			...
			idle_info("SODI3: blocking by uptime, count = %d\n",
				sodi3_by_uptime_count);
...
unsigned int ufs_cb_before_xxidle(void)
{
	...
	op_cond =
		(ufs_in_hibernate == UFS_H8) ? DEEPIDLE_OPT_XO_UFS_ON_OFF : 0;
	op_cond |=
		(ufs_in_hibernate == UFS_H8) ?
				DEEPIDLE_OPT_UFSCARD_MUX_SWITCH : 0;
#endif
...
static unsigned int dpidle_pre_process(int cpu)
{
	...
	clkmux_cond[IDLE_TYPE_DP] =
		mtk_idle_check_clkmux(IDLE_TYPE_DP, clkmux_block_mask);
...
int dpidle_enter(int cpu)
{
	...
	operation_cond |=
		dpidle_force_vcore_lp_mode ? DEEPIDLE_OPT_VCORE_LP_MODE :
			(clkmux_cond[IDLE_TYPE_DP] ?
				DEEPIDLE_OPT_VCORE_LP_MODE : 0);

	spm_go_to_dpidle(slp_spm_deepidle_flags, (u32)cpu,
...
int soidle3_enter(int cpu)
{
	...
	/* clkmux for sodi3 */
	memset(clkmux_block_mask[IDLE_TYPE_SO3], 0,
		NF_CLK_CFG * sizeof(unsigned int));
	clkmux_cond[IDLE_TYPE_SO3] =
		mtk_idle_check_clkmux(IDLE_TYPE_SO3, clkmux_block_mask);
...
int soidle_enter(int cpu)
{
	...
	spm_go_to_sodi(slp_spm_SODI_flags, (u32)cpu,
		sodi_flags, operation_cond);
			...
			char __user *userbuf, size_t count, loff_t *f_pos)
{
	...
	for (i = 0; i < nr_cpu_ids; i++) {
		mt_idle_log(
			"dpidle_cnt[%d]=%lu, dpidle_26m[%d]=%lu\
			, soidle3_cnt[%d]=%lu, soidle_cnt[%d]=%lu, ",
			i, dpidle_cnt[i], i, dpidle_f26m_cnt[i],
			i, soidle3_cnt[i], i, soidle_cnt[i]);
		mt_idle_log(
			"mcidle_cnt[%d]=%lu, mcsodi_cnt[%d]=%lu\
			, slidle_cnt[%d]=%lu, rgidle_cnt[%d]=%lu\n",
			i, mcidle_cnt[i], i, mcsodi_cnt[i],
	...
	mt_idle_log("\n********** idle command help **********\n");
	mt_idle_log("status help:\tcat %s\n", IDLE_STAT_PATH);
	mt_idle_log("switch on/off: echo switch mask > %s\n", IDLE_STAT_PATH);
	mt_idle_log("idle ratio profile: echo ratio 1/0 > %s\n",
						IDLE_STAT_PATH);

	mt_idle_log("soidle3 help:\tcat %s\n", SODI3_PATH);
	mt_idle_log("soidle help:\tcat %s\n", SODI_PATH);
	mt_idle_log("dpidle  help:\tcat %s\n", DPIDLE_PATH);
	mt_idle_log("mcidle help:\tcat %s\n", MCDI_PATH);
	mt_idle_log("slidle help:\tcat %s\n", SLIDLE_PATH);
	mt_idle_log("rgidle help:\tcat %s\n", RGIDLE_PATH);
	...
	size_t count, loff_t *f_pos)
{
	...
	mt_idle_log("\n********** mcidle command help **********\n");
	mt_idle_log("mcidle help:\tcat %s\n", MCDI_PATH);
	mt_idle_log("switch on/off:\techo [mcidle] 1/0 > %s\n", MCDI_PATH);
	mt_idle_log("modify tm_cri:\techo time value(dec) > %s\n", MCDI_PATH);
	...
	size_t count, loff_t *f_pos)
{
	...
	for (i = 0; i < NR_GRPS; i++) {
		mt_idle_log(
			"[%02d]dpidle_condition_mask[%-8s]=0x%08x\t\t\
	...
	mt_idle_log("\n*********** dpidle command help  ************\n");
	mt_idle_log("dpidle help:\tcat %s\n", DPIDLE_PATH);
	mt_idle_log("switch on/off:\techo [dpidle] 1/0 > %s\n", DPIDLE_PATH);
	mt_idle_log("cpupdn on/off:\techo cpupdn 1/0 > %s\n", DPIDLE_PATH);
	mt_idle_log("en_dp_by_bit:\techo enable id > %s\n", DPIDLE_PATH);
	mt_idle_log("dis_dp_by_bit:\techo disable id > %s\n", DPIDLE_PATH);
	mt_idle_log("modify tm_cri:\techo time value(dec) > %s\n", DPIDLE_PATH);
	mt_idle_log("bypass cg:\techo bypass 1/0 > %s\n", DPIDLE_PATH);
	...
	size_t count, loff_t *f_pos)
{
	...
	for (i = 0; i < NR_PLLS; i++) {
		mt_idle_log(
			...
			soidle3_pll_block_mask[%-8s]=0x%08x\n",
			i,
	...
	for (i = 0; i < NR_GRPS; i++) {
		mt_idle_log(
			"[%02d]soidle3_condition_mask[%-8s]=0x%08x\t\t\
			soidle3_block_mask[%-8s]=0x%08x\n",
			i,
			mtk_get_cg_group_name(i),
	...
	mt_idle_log("soidle3 help:\tcat %s\n", SODI3_PATH);
	mt_idle_log("switch on/off:\techo [soidle3] 1/0 > %s\n", SODI3_PATH);
	mt_idle_log("en_dp_by_bit:\techo enable id > %s\n", SODI3_PATH);
	mt_idle_log("dis_dp_by_bit:\techo disable id > %s\n", SODI3_PATH);
	mt_idle_log("modify tm_cri:techo time value(dec) > %s\n", SODI3_PATH);
	mt_idle_log("bypass pll:\techo bypass_pll 1/0 > %s\n", SODI3_PATH);
	mt_idle_log("bypass cg:\techo bypass 1/0 > %s\n", SODI3_PATH);
	mt_idle_log("bypass en:\techo bypass_en 1/0 > %s\n", SODI3_PATH);
	mt_idle_log("sodi3 flags:\techo sodi3_flags value > %s\n", SODI3_PATH);
	...
	size_t count, loff_t *f_pos)
{
	...
	mt_idle_log("\n*********** soidle command help  ************\n");
	mt_idle_log("soidle help:\tcat %s\n", SODI_PATH);
	mt_idle_log("switch on/off:\techo [soidle] 1/0 > %s\n", SODI_PATH);
	mt_idle_log("en_dp_by_bit:\techo enable id > %s\n", SODI_PATH);
	mt_idle_log("dis_dp_by_bit:\techo disable id > %s\n", SODI_PATH);
	mt_idle_log("modify tm_cri:techo time value(dec) > %s\n", SODI_PATH);
	mt_idle_log("bypass cg:\techo bypass 1/0 > %s\n", SODI_PATH);
	mt_idle_log("bypass en:\techo bypass_en 1/0 > %s\n", SODI_PATH);
	mt_idle_log("sodi flags:\techo sodi_flags value > %s\n", SODI_PATH);
	mt_idle_log("\t[0] reduce log, [1] residency, [2] resource usage\n");
...
static ssize_t mcsodi_state_read(struct file *filp, char __user *userbuf,
	size_t count, loff_t *f_pos)
{
	int i, len = 0;
	char *p = dbg_buf;
	...
	for (i = 0; i < NR_GRPS; i++) {
		mt_idle_log(
			"[%02d]mcsodi_condition_mask[%-8s]=0x%08x\t\t\
			mcsodi_block_mask[%-8s]=0x%08x\n",
			i,
			mtk_get_cg_group_name(i),
			idle_condition_mask[IDLE_TYPE_MCSO][i],
			mtk_get_cg_group_name(i),
			idle_block_mask[IDLE_TYPE_MCSO][i]);
	}
	...
	mt_idle_log("\n*********** mcsodi command help  ************\n");
	mt_idle_log("mcsodi help:\tcat %s\n", MCDI_PATH);
	mt_idle_log("switch on/off:\techo [mcsodi] 1/0 > %s\n", MCDI_PATH);
	mt_idle_log("en_dp_by_bit:\techo enable id > %s\n", MCDI_PATH);
	mt_idle_log("dis_dp_by_bit:\techo disable id > %s\n", MCDI_PATH);
	mt_idle_log("modify tm_cri:\techo time value(dec) > %s\n", MCDI_PATH);
	mt_idle_log("bypass cg:\techo bypass 1/0 > %s\n", MCDI_PATH);
	mt_idle_log("bypass en:\techo bypass_en 1/0 > %s\n", MCDI_PATH);
	mt_idle_log("bypass pg:\techo bypass_pg 1/0 > %s\n", MCDI_PATH);
...
static ssize_t slidle_state_read(struct file *filp,
	char __user *userbuf, size_t count, loff_t *f_pos)
{
	int i, len = 0;
	char *p = dbg_buf;
	...
	for (i = 0; i < NR_GRPS; i++) {
		mt_idle_log(
			"[%02d]slidle_condition_mask[%-8s]=0x%08x\t\t\
			slidle_block_mask[%-8s]=0x%08x\n",
			i,
			mtk_get_cg_group_name(i),
			idle_condition_mask[IDLE_TYPE_SL][i],
			mtk_get_cg_group_name(i),
			idle_block_mask[IDLE_TYPE_SL][i]);
	}

	mt_idle_log("\n********** slidle command help **********\n");
	mt_idle_log("slidle help:\tcat %s\n", SLIDLE_PATH);
	mt_idle_log("switch on/off:\techo [slidle] 1/0 > %s\n", SLIDLE_PATH);
...
static int mtk_cpuidle_debugfs_init(void)
{
	...
	debugfs_create_file("idle_state",
		0644, root_entry, NULL, &idle_state_fops);
	debugfs_create_file("dpidle_state",
		0644, root_entry, NULL, &dpidle_state_fops);
	debugfs_create_file("soidle3_state",
		0644, root_entry, NULL, &soidle3_state_fops);
	debugfs_create_file("soidle_state",
		0644, root_entry, NULL, &soidle_state_fops);
	debugfs_create_file("mcidle_state",
		0644, root_entry, NULL, &mcidle_state_fops);
	debugfs_create_file("mcsodi_state",
		0644, root_entry, NULL, &mcsodi_state_fops);
	debugfs_create_file("slidle_state",
		0644, root_entry, NULL, &slidle_state_fops);
	debugfs_create_file("reg_dump",
		0644, root_entry, NULL, &reg_dump_fops);
...
{
	mtk_idle_twam_init();
	mtk_idle_block_setting(IDLE_TYPE_DP,
			dpidle_cnt, dpidle_block_cnt,
			idle_block_mask[IDLE_TYPE_DP]);
	mtk_idle_block_setting(IDLE_TYPE_SO3,
			soidle3_cnt, soidle3_block_cnt,
			idle_block_mask[IDLE_TYPE_SO3]);
	mtk_idle_block_setting(IDLE_TYPE_SO,
			soidle_cnt, soidle_block_cnt,
			idle_block_mask[IDLE_TYPE_SO]);
	mtk_idle_block_setting(IDLE_TYPE_MCSO,
			mcsodi_cnt,
			mcsodi_block_cnt,
			idle_block_mask[IDLE_TYPE_MCSO]);
	mtk_idle_block_setting(IDLE_TYPE_SL,
			slidle_cnt,
			slidle_block_cnt,
			idle_block_mask[IDLE_TYPE_SL]);
	mtk_idle_block_setting(IDLE_TYPE_RG,
			rgidle_cnt,
			NULL,
			NULL);
}
