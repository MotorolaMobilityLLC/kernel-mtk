...
#define idle_prof_info(fmt, args...)    pr_info(IDLE_PROF_TAG fmt, ##args)
#define idle_prof_dbg(fmt, args...)     pr_info(IDLE_PROF_TAG fmt, ##args)
...
{
	idle_prof_notice("spm twam (sel%d: %d) ratio: %5u/1000\n",
...
{
	if (idle_ratio_en && type >= 0 && type < NR_TYPES)
		idle_prof[type].ratio.value += idle_get_current_time_ms() -
	...
	if (type < IDLE_TYPE_MC) {
		...
		"XXIDLE %llu, %llu, %llu, %llu, %llu, %llu, %llu, %llu, %d\n",
			...
			ratio->value, ratio->value_dp, ratio->value_so3,
			ratio->value_so, last_ratio, type);
#endif
...
static void mtk_idle_dump_cnt(int type)
{
	...
	if (enter_idle)
		append_log("%s Total = %lu --- ",
			get_idle_buf(buf), total_cnt);
	else
		append_log("No enter --- ");
}
...
bool mtk_idle_select_state(int type, int reason)
{
	...
	if (dump_block_info) {
		...
		idle_buf_append(log, "CNT(%s,rgidle): ", p_idle->name);
		for (i = 0; i < nr_cpu_ids; i++)
			idle_buf_append(log, "[%d] = (%lu,%lu), ",
				i, p_idle->cnt[i],
		...
		idle_buf_append(log, "%s_block_cnt: ", p_idle->name);
		for (i = 0; i < NR_REASONS; i++)
			idle_buf_append(log, "[%s] = %lu, ",
				mtk_get_reason_name(i), p_idle->block_cnt[i]);
		idle_prof_warn("%s\n", get_idle_buf(log));

		reset_idle_buf(log);

		idle_buf_append(log, "%s_block_mask: ", p_idle->name);
		for (i = 0; i < NR_GRPS; i++)
			idle_buf_append(log, "0x%08x, ", p_idle->block_mask[i]);
		idle_prof_warn("%s\n", get_idle_buf(log));
