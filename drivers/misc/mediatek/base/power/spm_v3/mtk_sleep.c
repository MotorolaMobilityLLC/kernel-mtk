...
#define slp_error(fmt, args...)     pr_info("[SLP] " fmt, ##args)
...
static enum wk_reason slp_wake_reason = WR_NONE;

static bool slp_ck26m_on;
bool slp_dump_gpio;
bool slp_dump_golden_setting;
int slp_dump_golden_setting_type = GS_ALL;
...
static int slp_suspend_ops_begin(suspend_state_t state)
{
	...
	/* legacy log */
	slp_notice("%s\tChip_pm_begin(%u)(%u)\t%s\n",
		MARK_LINE,
...
#if !defined(CONFIG_FPGA_EARLY_PORTING)
		slp_wake_reason =
			spm_go_to_sleep_dpidle(slp_spm_deepidle_flags,
						slp_spm_data);
#endif
	else
#endif
...
enum wk_reason slp_get_wake_reason(void)
{
...
void slp_module_init(void)
{
	...
	slp_notice(
		"SLEEP_DPIDLE_EN:%d, REPLACE_DEF_WAKESRC:%d\
		, SUSPEND_LOG_EN:%d\n",
