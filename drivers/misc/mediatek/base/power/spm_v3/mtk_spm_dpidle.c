...
static void spm_dpidle_notify_sspm_before_wfi(bool sleep_dpidle,
	u32 operation_cond, struct pwr_ctrl *pwrctrl)
{
	int ret;
	...
	spm_opt |= ((operation_cond & DEEPIDLE_OPT_XO_UFS_ON_OFF) &&
		!sleep_dpidle) ? SPM_OPT_XO_UFS_OFF : 0;
	spm_opt |= ((operation_cond & DEEPIDLE_OPT_CLKBUF_BBLPM) &&
		!sleep_dpidle) ? SPM_OPT_CLKBUF_ENTER_BBLPM : 0;
...
static void spm_dpidle_pcm_setup_before_wfi(bool sleep_dpidle, u32 cpu,
	struct pcm_desc *pcmdesc, struct pwr_ctrl *pwrctrl, u32 operation_cond)
{
...
defined(CONFIG_MTK_PSCI) || defined(CONFIG_MTK_HW_FDE)
	mt_secure_call(MTK_SIP_KERNEL_SPM_DPIDLE_ARGS,
		pwrctrl->pcm_flags, resource_usage, 0);
	mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
		SPM_PWR_CTRL_DPIDLE, PWR_OPP_LEVEL, pwrctrl->opp_level);
...
static enum wk_reason spm_output_wake_reason(struct wake_status *wakesta,
	struct pcm_desc *pcmdesc, u32 log_cond, u32 operation_cond)
{
	...
	if (log_cond & DEEPIDLE_LOG_FULL) {
		wr = __spm_output_wake_reason(wakesta,
						pcmdesc, false, "dpidle");
		...
		if (log_print == true) {
			dpidle_dbg("dpidle_log_discard_cnt = %d\
				, timer_out_too_short = %d, oper_cond = %x\n",
...
enum wk_reason spm_go_to_dpidle(u32 spm_flags, u32 spm_data, u32 log_cond,
	u32 operation_cond)
{
 ...
 */
enum wk_reason spm_go_to_sleep_dpidle(u32 spm_flags, u32 spm_data)
{
	u32 sec = 0;
...
#endif
#endif
	static enum wk_reason last_wr = WR_NONE;
	...
	spm_dpidle_notify_sspm_before_wfi(true,
				DEEPIDLE_OPT_VCORE_LP_MODE, pwrctrl);
