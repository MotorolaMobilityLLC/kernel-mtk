...
defined(CONFIG_MTK_PSCI) || defined(CONFIG_MTK_HW_FDE)
	...
	mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS, SPM_PWR_CTRL_SUSPEND,
		PWR_OPP_LEVEL, pwrctrl->opp_level);
#endif
}
...
#if defined(CONFIG_MACH_MT6758)
	spm_crit2("suspend dormant state = %d, ddr = %d, vcore = %d\
		,spm_sleep_count = %d, sleep ddr = %d, debug = 0x%x\n",
		spm_dormant_sta, ddr_status, vcore_status, spm_sleep_count,
		sleep_ddr, sleep_vcorefs_debug);
#else
	spm_crit2("suspend dormant state = %d, ddr = %d, vcore = %d\
		, spm_sleep_count = %d\n",
		spm_dormant_sta, ddr_status, vcore_status,
		spm_sleep_count);
#endif
	if (spm_ap_mdsrc_req_cnt != 0)
		spm_crit2("warning: spm_ap_mdsrc_req_cnt = %d\
			, r7[ap_mdsrc_req] = 0x%x\n",
...
enum wk_reason spm_go_to_sleep(u32 spm_flags, u32 spm_data)
{
	u32 sec = 2;
	unsigned long flags;
...
RESTORE_IRQ:
#endif
	spm_crit2("0x10060040 = 0x%x, 0x10060044 = 0x%x, 0x10060048 = 0x%x\
		,0x1006004c = 0x%x, 0x10060100= 0x%x,\n",
		spm_read(DEVICE_MPU_AO_BASE + 0x0040),
		spm_read(DEVICE_MPU_AO_BASE + 0x0044),
		spm_read(DEVICE_MPU_AO_BASE + 0x0048),
		spm_read(DEVICE_MPU_AO_BASE + 0x004c),
		spm_read(DEVICE_MPU_AO_BASE + 0x0100));
	/* record last wakesta */
...
void spm_ap_mdsrc_req(u8 set)
{
	...
	if (set) {
		...
		if (spm_ap_mdsrc_req_cnt < 0) {
			spm_crit2("warning: set = %d\
				, spm_ap_mdsrc_req_cnt = %d\n",
