...
void __iomem *device_mpu_ao_base;
u32 spm_irq_0;
...
static void spm_register_init(void)
{
	...
	device_mpu_ao_base = of_iomap(node, 2);
	if (!device_mpu_ao_base)
		spm_err("base device_mpu_ao failed\n");
...
defined(CONFIG_MTK_PSCI) || defined(CONFIG_MTK_HW_FDE)
	if (local_spm_load_firmware_status == -1)
		local_spm_load_firmware_status =
			mt_secure_call(MTK_SIP_KERNEL_SPM_FIRMWARE_STATUS,
			0, 0, 0);
#endif
...
#endif
	ret = spm_to_sspm_command(SPM_SUSPEND_NOIRQ, &spm_d);
	if (ret < 0) {
		pr_info("#@# %s(%d) SPM_SUSPEND_NOIRQ return %d!!!\n",
			__func__, __LINE__, ret);
	}
#endif /* CONFIG_MTK_TINYSYS_SSPM_SUPPORT */
}
...
int __init spm_module_init(void)
{
	...
	spm_file = debugfs_create_file("spm_sleep_count", 0444, spm_dir,
				NULL, &spm_sleep_count_fops);
	spm_file = debugfs_create_file("spm_last_wakeup_src", 0444, spm_dir,
				NULL, &spm_last_wakeup_src_fops);
...
void spm_twam_disable_monitor(void)
{
	...
	spin_lock_irqsave(&__spm_lock, flags);
	spm_write(SPM_TWAM_CON,
		spm_read(SPM_TWAM_CON) & ~REG_TWAM_ENABLE_LSB);
	spm_write(SPM_IRQ_MASK, spm_read(SPM_IRQ_MASK) | ISRM_TWAM);
...
int spm_golden_setting_cmp(bool en)
{
	...
	for (i = 0; i < ddrphy_num; i++) {
		...
		if ((value & ddrphy_setting[i].mask) !=
			...
			spm_err(
				"dram err addr: 0x%.2x, offset: 0x%.3x\
...
int spm_to_sspm_command_async(u32 cmd, struct spm_data *spm_d)
{
	...
	case SPM_LEAVE_SODI3:
		...
		if (ret != 0)
			pr_info(
				"#@# %s(%d) sspm_ipi_send_async(cmd:0x%x\
				) ret %d\n",
...
int spm_to_sspm_command_async_wait(u32 cmd)
{
	...
	case SPM_LEAVE_SODI3:
		...
		if (ret != 0) {
			pr_info(
				"#@# %s(%d) sspm_ipi_send_async_wait(cmd:0x%x\
				) ret %d\n",
				__func__, __LINE__, cmd, ret);
		} else if (ack_data < 0) {
			ret = ack_data;
...
int spm_to_sspm_command(u32 cmd, struct spm_data *spm_d)
{
	...
	case SPM_LEAVE_SODI3:
		...
		if (ret != 0) {
			pr_info(
				"#@# %s(%d) sspm_ipi_send_sync(cmd:0x%x\
				) ret %d\n",
				__func__, __LINE__, cmd, ret);
		} else if (ack_data < 0) {
			ret = ack_data;
