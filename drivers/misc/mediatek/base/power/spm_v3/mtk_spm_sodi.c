...
static void spm_sodi_pcm_setup_before_wfi(u32 cpu, struct pcm_desc *pcmdesc,
...
defined(CONFIG_MTK_PSCI) || defined(CONFIG_MTK_HW_FDE)
...
static bool spm_sodi_is_not_gpt_event(struct wake_status *wakesta,
	long int curr_time)
{
	bool logout = false;
...
{
	return (curr_time - sodi_logout_prev_time)
		> SODI_LOGOUT_INTERVAL_CRITERIA;
}
...
enum wk_reason spm_sodi_output_log(struct wake_status *wakesta,
	struct pcm_desc *pcmdesc, u32 flags, u32 operation_cond)
{
	enum wk_reason wr = WR_NONE;
	...
	if (spm_read(SPM_SW_RSV_3) & 0x1)
		so_info(flags, "SPM CHECK EMI IDLE FAIL\n");
#endif

	if (!(flags & SODI_FLAG_REDUCE_LOG) ||
			(flags & SODI_FLAG_RESIDENCY)) {
		so_notice(flags, "self_refresh = 0x%x, sw_flag = 0x%x, 0x%x\
			,oper_cond = 0x%0x\n", spm_read(SPM_PASR_DPD_0),
			spm_read(SPM_SW_FLAG), spm_read(DUMMY1_PWR_CON),
			operation_cond);
		wr = __spm_output_wake_reason(wakesta, pcmdesc, false,
			(flags&SODI_FLAG_3P0) ? "sodi3" : "sodi");
		...
		if (need_log_out != SODI_LOGOUT_NONE) {
			...
			if (need_log_out == SODI_LOGOUT_ASSERT) {
				if (wakesta->assert_pc != 0) {
					so_err(flags, "Warn:WR_PCM_ASSERT!\n");
					wr = WR_PCM_ASSERT;
				} else if (wakesta->r12 == 0) {
					so_err(flags, "Warn:WR_UNKNOWN!\n");
					wr = WR_UNKNOWN;
				}
				so_err(flags, "SELF_REFRESH = 0x%x\
					SW_FLAG = 0x%x, 0x%x\
					SODI_CNT = %d\
					SELF_REFRESH_CNT = 0x%x\
					ASSERT_PC = 0x%0x, R13 = 0x%x\
					DEBUG_FLAG = 0x%x, R12 = 0x%x\
					R12_E = 0x%x, RAW_STA = 0x%x\
					IDLE_STA = 0x%x, EVENT_REG = 0x%x\
					ISR = 0x%x\n",
					spm_read(SPM_PASR_DPD_0),
					spm_read(SPM_SW_FLAG),
					spm_read(DUMMY1_PWR_CON),
					logout_sodi_cnt, logout_selfrefresh_cnt,
					wakesta->assert_pc, wakesta->r13,
					wakesta->debug_flag, wakesta->r12,
					wakesta->r12_ext, wakesta->raw_sta,
					wakesta->idle_sta, wakesta->event_reg,
					wakesta->isr);
				wr = WR_PCM_ASSERT;
			} else {
				...
				so_notice(flags, "wake up by %s\
					, self_refresh = 0x%x\
					, sw_flag = 0x%x, 0x%x, %d, 0x%x\
					, timer_out = %u, r13 = 0x%x\
					, debug_flag = 0x%x, 0x%x, 0x%x, 0x%x\
					, 0x%x, 0x%x, 0x%x, res_usage = 0x%x\
					, op_cond = 0x%x, %d\n",
					...
					wakesta->timer_out, wakesta->r13,
					wakesta->debug_flag, wakesta->r12,
					wakesta->r12_ext, wakesta->raw_sta,
					wakesta->idle_sta, wakesta->event_reg,
					wakesta->isr,
					spm_get_resource_usage(),
					operation_cond, need_log_out);
			}
			logout_sodi_cnt = 0;
...
enum wk_reason spm_go_to_sodi(u32 spm_flags, u32 spm_data, u32 sodi_flags,
	u32 operation_cond)
{
...
RESTORE_IRQ:
	...
	if (wr == WR_UART_BUSY)
		sodi_info("request uart sleep: fail\n");
	else
