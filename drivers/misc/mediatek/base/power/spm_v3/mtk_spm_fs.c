...
static char *pwr_ctrl_str[PWR_MAX_COUNT] = {
	...
	[PWR_REG_CONN_SRCCLKENA_CKSEL2_MASK_B]
		= "reg_conn_srcclkena_cksel2_mask_b",
	[PWR_REG_SSPM_APSRC_REQ_DDREN_MASK_B]
		= "reg_sspm_apsrc_req_ddren_mask_b",
	[PWR_REG_SCP_APSRC_REQ_DDREN_MASK_B]
		= "reg_scp_apsrc_req_ddren_mask_b",
	...
	[PWR_REG_MD_SRCCLKENA_0_VRF18_MASK_B] =
		"reg_md_srcclkena_0_vrf18_mask_b",
#if defined(CONFIG_MACH_MT6759) || defined(CONFIG_MACH_MT6758)
	...
	[MCU0_WFI_ENABLE] = "mcu0_wfi_en",
	[MCU1_WFI_ENABLE] = "mcu1_wfi_en",
	[MCU2_WFI_ENABLE] = "mcu2_wfi_en",
	[MCU3_WFI_ENABLE] = "mcu3_wfi_en",
	[MCU4_WFI_ENABLE] = "mcu4_wfi_en",
	[MCU5_WFI_ENABLE] = "mcu5_wfi_en",
	[MCU6_WFI_ENABLE] = "mcu6_wfi_en",
	[MCU7_WFI_ENABLE] = "mcu7_wfi_en",
	[MCU8_WFI_ENABLE] = "mcu8_wfi_en",
	[MCU9_WFI_ENABLE] = "mcu9_wfi_en",
	[MCU10_WFI_ENABLE] = "mcu10_wfi_en",
	[MCU11_WFI_ENABLE] = "mcu11_wfi_en",
	[MCU12_WFI_ENABLE] = "mcu12_wfi_en",
	[MCU13_WFI_ENABLE] = "mcu13_wfi_en",
	[MCU14_WFI_ENABLE] = "mcu14_wfi_en",
	[MCU15_WFI_ENABLE] = "mcu15_wfi_en",
	[MCU16_WFI_ENABLE] = "mcu16_wfi_en",
	[MCU17_WFI_ENABLE] = "mcu17_wfi_en",
#if defined(CONFIG_MACH_MT6759) || defined(CONFIG_MACH_MT6758)
...
#ifdef SUP_MCSODI_FS
static ssize_t mcsodi_ctrl_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
...
defined(CONFIG_MTK_PSCI) || defined(CONFIG_MTK_HW_FDE)
	...
	if (!strcmp(cmd, pwr_ctrl_str[PWR_PCM_FLAGS])) {
		...
		pwr_ctrl_str[PWR_REG_SSPM_SRCCLKENA_INFRA_MASK_B])) {
		...
		pwr_ctrl_str[PWR_REG_SCP_SRCCLKENA_INFRA_MASK_B])) {
		...
		pwr_ctrl_str[PWR_REG_CCIF1_MD_EVENT_MASK_B])) {
		...
		pwr_ctrl_str[PWR_REG_MD_SRCCLKENA_0_VRF18_MASK_B])) {
	...
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU0_WFI_EN])) {
		pwrctrl->mcu0_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU0_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU1_WFI_ENABLE])) {
		pwrctrl->mcu1_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU1_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU2_WFI_ENABLE])) {
		pwrctrl->mcu2_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU2_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU3_WFI_ENABLE])) {
		pwrctrl->mcu3_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU3_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU4_WFI_ENABLE])) {
		pwrctrl->mcu4_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU4_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU5_WFI_ENABLE])) {
		pwrctrl->mcu5_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU5_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU6_WFI_ENABLE])) {
		pwrctrl->mcu6_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU6_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU7_WFI_ENABLE])) {
		pwrctrl->mcu7_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU7_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU8_WFI_ENABLE])) {
		pwrctrl->mcu8_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU8_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU9_WFI_ENABLE])) {
		pwrctrl->mcu9_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU9_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU10_WFI_ENABLE])) {
		pwrctrl->mcu10_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU10_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU11_WFI_ENABLE])) {
		pwrctrl->mcu11_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU11_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU12_WFI_ENABLE])) {
		pwrctrl->mcu12_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU12_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU13_WFI_ENABLE])) {
		pwrctrl->mcu13_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU13_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU14_WFI_ENABLE])) {
		pwrctrl->mcu14_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU14_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU15_WFI_ENABLE])) {
		pwrctrl->mcu15_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU15_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU16_WFI_ENABLE])) {
		pwrctrl->mcu16_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU16_WFI_ENABLE, val);
	} else if (!strcmp(cmd, pwr_ctrl_str[MCU17_WFI_ENABLE])) {
		pwrctrl->mcu17_wfi_en = val;
		mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS,
				id, MCU17_WFI_ENABLE, val);
#if defined(CONFIG_MACH_MT6759) || defined(CONFIG_MACH_MT6758)
...
#ifdef SUP_MCSODI_FS
static ssize_t mcsodi_ctrl_store(struct kobject *kobj,
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	return store_pwr_ctrl(SPM_PWR_CTRL_MCSODI,
		__spm_mcsodi.pwrctrl, buf, count);
}
#endif
