...
static inline void spm_dpidle_footprint(enum spm_deepidle_step step)
{
	...
	aee_rr_rec_deepidle_val(
			aee_rr_curr_deepidle_val() | step | cpu_footprint);
#endif
}
...
static void spm_dpidle_notify_sspm_before_wfi(
		bool sleep_dpidle, u32 operation_cond, struct pwr_ctrl *pwrctrl)
{
	int ret;
...
static void spm_dpidle_notify_sspm_after_wfi(
		bool sleep_dpidle, u32 operation_cond)
{
	int ret;
	struct spm_data spm_d;
	unsigned int spm_opt = 0;
	...
	spm_opt |= ((operation_cond & DEEPIDLE_OPT_XO_UFS_ON_OFF)
			&& !sleep_dpidle) ? SPM_OPT_XO_UFS_OFF : 0;
	spm_opt |= ((operation_cond &
			DEEPIDLE_OPT_CLKBUF_BBLPM) && !sleep_dpidle) ?
...
static void spm_dpidle_pcm_setup_after_wfi(
		bool sleep_dpidle, u32 operation_cond)
{
...
static unsigned int spm_output_wake_reason(
		...
		u32 spm_flags, u32 spm_data, u32 log_cond, u32 operation_cond)
{
	...
	spm_dpidle_pcm_setup_before_wfi(
			false, cpu, pcmdesc, pwrctrl, operation_cond);
...
RESTORE_IRQ:
	...
	wr = spm_output_wake_reason(
			&wakesta, pcmdesc, log_cond, operation_cond);
...
unsigned int spm_go_to_sleep_dpidle(u32 spm_flags, u32 spm_data)
{
	...
	spm_dpidle_notify_sspm_before_wfi(
			true, DEEPIDLE_OPT_VCORE_LP_MODE, pwrctrl);
	...
	spm_dpidle_footprint(
			SPM_DEEPIDLE_SLEEP_DPIDLE
	...
	spm_dpidle_footprint(
			SPM_DEEPIDLE_SLEEP_DPIDLE | SPM_DEEPIDLE_ENTER_WFI);
	...
	spm_dpidle_footprint(
			SPM_DEEPIDLE_SLEEP_DPIDLE | SPM_DEEPIDLE_LEAVE_WFI);
...
RESTORE_IRQ:
	...
	last_wr = __spm_output_wake_reason(
			&wakesta, pcmdesc, true, "sleep_dpidle");
