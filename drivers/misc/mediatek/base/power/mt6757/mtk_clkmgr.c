...
int clkmgr_is_locked(void) { return clkmgr_locked(); }
...
static struct pll plls[NR_PLLS] = {
	{
	.name = __stringify(MAINPLL), .en_mask = 0xF0000101,
	},
	{
	.name = __stringify(UNIVPLL), .en_mask = 0xFE000001,
	},
	{
	.name = __stringify(MSDCPLL), .en_mask = 0x00000101,
	},
	{
	.name = __stringify(VENCPLL), .en_mask = 0x00000101,
	},
	{
	.name = __stringify(MMPLL), .en_mask = 0x00000101,
	},
	{
	.name = __stringify(TVDPLL), .en_mask = 0xc0000101,
	},
	{
	.name = __stringify(APLL1), .en_mask = 0x00000101,
	},
	{
	.name = __stringify(APLL2), .en_mask = 0x00000101,
	}
};
...
static struct subsys syss[NR_SYSS] = {
	{
	.name = __stringify(SYS_MD1), .sta_mask = 1U << 0,
	},
	{
	.name = __stringify(SYS_CONN), .sta_mask = 1U << 1,
	},
	{
	.name = __stringify(SYS_DIS), .sta_mask = 1U << 3,
	},
	{
	.name = __stringify(SYS_MFG), .sta_mask = 1U << 4,
	},
	{
	.name = __stringify(SYS_ISP), .sta_mask = 1U << 5,
	},
	{
	.name = __stringify(SYS_VDE), .sta_mask = 1U << 7,
	},
	{
	.name = __stringify(SYS_VEN), .sta_mask = 1U << 21,
	},
	{
	.name = __stringify(SYS_MFG_ASYNC), .sta_mask = 1U << 23,
	},
	{
	.name = __stringify(SYS_AUDIO), .sta_mask = 1U << 24,
	},
	{
	.name = __stringify(SYS_CAM), .sta_mask = 1U << 27,
	},
	{
	.name = __stringify(SYS_C2K), .sta_mask = 1U << 28,
	},
	{
	.name = __stringify(SYS_MDSYS_INTF_INFRA), .sta_mask = 1U << 29,
	},
	{
	.name = __stringify(SYS_MFG_CORE1), .sta_mask = 1U << 30,
	},
	{
	.name = __stringify(SYS_MFG_CORE0), .sta_mask = 1U << 31,
	}
};
...
void clk_dump(void)
{
	pr_debug("********** PLL register dump *********\n");

	pr_debug("[MAINPLL_CON0]=0x%08x\n", clk_readl(MAINPLL_CON0));
	pr_debug("[MAINPLL_CON1]=0x%08x\n", clk_readl(MAINPLL_CON1));
	pr_debug("[MAINPLL_PWR_CON0]=0x%08x\n", clk_readl(MAINPLL_PWR_CON0));

	pr_debug("[UNIVPLL_CON0]=0x%08x\n", clk_readl(UNIVPLL_CON0));
	pr_debug("[UNIVPLL_CON1]=0x%08x\n", clk_readl(UNIVPLL_CON1));
	pr_debug("[UNIVPLL_PWR_CON0]=0x%08x\n", clk_readl(UNIVPLL_PWR_CON0));

	pr_debug("[MSDCPLL_CON0]=0x%08x\n", clk_readl(MSDCPLL_CON0));
	pr_debug("[MSDCPLL_CON1]=0x%08x\n", clk_readl(MSDCPLL_CON1));
	pr_debug("[MSDCPLL_PWR_CON0]=0x%08x\n", clk_readl(MSDCPLL_PWR_CON0));

	pr_debug("[VENCPLL_CON0]=0x%08x\n", clk_readl(VENCPLL_CON0));
	pr_debug("[VENCPLL_CON1]=0x%08x\n", clk_readl(VENCPLL_CON1));
	pr_debug("[VENCPLL_PWR_CON0]=0x%08x\n", clk_readl(VENCPLL_PWR_CON0));
	...
	pr_debug("[TVDPLL_CON0]=0x%08x\n", clk_readl(TVDPLL_CON0));
	pr_debug("[TVDPLL_CON1]=0x%08x\n", clk_readl(TVDPLL_CON1));
	pr_debug("[TVDPLL_PWR_CON0]=0x%08x\n", clk_readl(TVDPLL_PWR_CON0));

	pr_debug("[APLL1_CON0]=0x%08x\n", clk_readl(APLL1_CON0));
	pr_debug("[APLL1_CON1]=0x%08x\n", clk_readl(APLL1_CON1));
	pr_debug("[APLL1_PWR_CON0]=0x%08x\n", clk_readl(APLL1_PWR_CON0));

	pr_debug("[APLL2_CON0]=0x%08x\n", clk_readl(APLL2_CON0));
	pr_debug("[APLL2_CON1]=0x%08x\n", clk_readl(APLL2_CON1));
	pr_debug("[APLL2_PWR_CON0]=0x%08x\n", clk_readl(APLL2_PWR_CON0));

	pr_debug("[AP_PLL_CON3]=0x%08x\n", clk_readl(AP_PLL_CON3));
	pr_debug("[AP_PLL_CON4]=0x%08x\n", clk_readl(AP_PLL_CON4));

	pr_debug("********** subsys pwr sts register dump *********\n");
	pr_debug("PWR_STATUS=0x%08x, PWR_STATUS_2ND=0x%08x\n",
		 clk_readl(PWR_STATUS), clk_readl(PWR_STATUS_2ND));

	pr_debug("********** mux register dump *********\n");

	pr_debug("[CLK_CFG_0]=0x%08x\n", clk_readl(CLK_CFG_0));
	pr_debug("[CLK_CFG_1]=0x%08x\n", clk_readl(CLK_CFG_1));
	pr_debug("[CLK_CFG_2]=0x%08x\n", clk_readl(CLK_CFG_2));
	pr_debug("[CLK_CFG_3]=0x%08x\n", clk_readl(CLK_CFG_3));
	pr_debug("[CLK_CFG_4]=0x%08x\n", clk_readl(CLK_CFG_4));
	pr_debug("[CLK_CFG_5]=0x%08x\n", clk_readl(CLK_CFG_5));
	pr_debug("[CLK_CFG_6]=0x%08x\n", clk_readl(CLK_CFG_6));
	pr_debug("[CLK_CFG_7]=0x%08x\n", clk_readl(CLK_CFG_7));
	pr_debug("[CLK_CFG_8]=0x%08x\n", clk_readl(CLK_CFG_8));
	pr_debug("[CLK_CFG_9]=0x%08x\n", clk_readl(CLK_CFG_9));

	pr_debug("********** CG register dump *********\n");
	pr_debug("[INFRA_SUBSYS][%p]\n", clk_infracfg_ao_base); /* 0x10001000 */

	pr_debug("[INFRA_PDN_STA0]=0x%08x\n", clk_readl(INFRA_SW_CG0_STA));
	pr_debug("[INFRA_PDN_STA1]=0x%08x\n", clk_readl(INFRA_SW_CG1_STA));
	pr_debug("[INFRA_PDN_STA2]=0x%08x\n", clk_readl(INFRA_SW_CG2_STA));

	pr_debug("[INFRA_TOPAXI_PROTECTSTA1]=0x%08x\n",
		 clk_readl(INFRA_TOPAXI_PROTECTSTA1));

	pr_debug("[MMSYS_SUBSYS][%p]\n", clk_mmsys_config_base);
	pr_debug("[DISP_CG_CON0]=0x%08x\n", clk_readl(DISP_CG_CON0));
	pr_debug("[DISP_CG_CON1]=0x%08x\n", clk_readl(DISP_CG_CON1));
	pr_debug("[DISP_CG_CON0_DUMMY]=0x%08x\n",
		 clk_readl(DISP_CG_CON0_DUMMY));
	pr_debug("[DISP_CG_CON1_DUMMY]=0x%08x\n",
		 clk_readl(DISP_CG_CON1_DUMMY));

	/**/
	pr_debug("[MFG_SUBSYS][%p]\n", clk_mfgcfg_base);
	pr_debug("[MFG_CG_CON]=0x%08x\n", clk_readl(MFG_CG_CON));

	pr_debug("[AUDIO_SUBSYS][%p]\n", clk_audio_base);
	pr_debug("[AUDIO_TOP_CON0]=0x%08x\n", clk_readl(AUDIO_TOP_CON0));

	pr_debug("[IMG_SUBSYS][%p]\n", clk_imgsys_base);
	pr_debug("[IMG_CG_CON]=0x%08x\n", clk_readl(IMG_CG_CON));

	pr_debug("[VDEC_SUBSYS][%p]\n", clk_vdec_gcon_base);
	pr_debug("[VDEC_CKEN_SET]=0x%08x\n", clk_readl(VDEC_CKEN_SET));
	pr_debug("[LARB_CKEN_SET]=0x%08x\n", clk_readl(LARB_CKEN_SET));

	pr_debug("[VENC_SUBSYS][%p]\n", clk_venc_gcon_base);
	pr_debug("[VENC_CG_CON]=0x%08x\n", clk_readl(VENC_CG_CON));

	pr_debug("[CAM_SUBSYS][%p]\n", clk_camsys_base);
	pr_debug("[CAM_CG_CON]=0x%08x\n", clk_readl(CAM_CG_CON));
}

void clk_misc_cfg_ops(bool flag)
{
	pr_debug("[%s]: warning! warning! warning! to use this function\n",
		 __func__);
}
...
{
	pr_debug("[%s]: warning!! real function doesn't exist\n", __func__);
}
...
static void pll_force_off(void)
{
	...
	clk_writel(ARMPLL_L_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					  PLL_PWR_CON0_ISO_EN_VALUE);

	temp = clk_readl(CCIPLL_PWR_CON0);
	clk_writel(CCIPLL_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					PLL_PWR_CON0_ISO_EN_VALUE);

	temp = clk_readl(UNIVPLL_PWR_CON0);
	clk_writel(UNIVPLL_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					 PLL_PWR_CON0_ISO_EN_VALUE);

	temp = clk_readl(MSDCPLL_PWR_CON0);
	clk_writel(MSDCPLL_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					 PLL_PWR_CON0_ISO_EN_VALUE);

	temp = clk_readl(VENCPLL_PWR_CON0);
	clk_writel(VENCPLL_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					 PLL_PWR_CON0_ISO_EN_VALUE);

	temp = clk_readl(MMPLL_PWR_CON0);
	clk_writel(MMPLL_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					   PLL_PWR_CON0_ISO_EN_VALUE);

	temp = clk_readl(TVDPLL_PWR_CON0);
	clk_writel(TVDPLL_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					PLL_PWR_CON0_ISO_EN_VALUE);

	temp = clk_readl(APLL1_PWR_CON0);
	clk_writel(APLL1_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					   PLL_PWR_CON0_ISO_EN_VALUE);

	temp = clk_readl(APLL2_PWR_CON0);
	clk_writel(APLL2_PWR_CON0, (temp & PLL_PWR_CON0_ISO_EN_MASK) |
					   PLL_PWR_CON0_ISO_EN_VALUE);
...
static int armlpll_fsel_read(struct seq_file *m, void *v) { return 0; }

static int armllpll_fsel_read(struct seq_file *m, void *v) { return 0; }

static int armccipll_fsel_read(struct seq_file *m, void *v) { return 0; }

static int mmpll_fsel_read(struct seq_file *m, void *v) { return 0; }

static int vencpll_fsel_read(struct seq_file *m, void *v) { return 0; }

static int _clk_debugf_open(struct seq_file *s, void *data) { return 0; }

static ssize_t clk_debugf_read(struct file *filp, char __user *userbuf,
				   size_t count, loff_t *f_pos)
{
	...
	for (i = 0; i < NR_PLLS; i++) {
		if (pll_is_on(i))
			p += snprintf(p, buff_end - p,
					  "suspend warning: %s is on!!!\n",
					  plls[i].name);
	}
	for (i = 0; i < NR_SYSS; i++) {
		if (subsys_is_on(i)) {
			if ((i != SYS_MD1) && (i != SYS_C2K) &&
				(i != SYS_MDSYS_INTF_INFRA))
				p += snprintf(p, buff_end - p,
						  "suspend warning: %s is on!!!\n",
						  syss[i].name);
			else
	...
	p += snprintf(p, buff_end - p,
			  "********** subsys pwr sts register dump *********\n");
	p += snprintf(p, buff_end - p,
			  "PWR_STATUS=0x%08x, PWR_STATUS_2ND=0x%08x\n",
			  clk_readl(PWR_STATUS), clk_readl(PWR_STATUS_2ND));
...
static ssize_t armccipll_fsel_write(struct file *file,
				  ...
				  size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%x %x", &con0, &con1) == 2) {
		...
		/*pr_debug("vencpll con0=0x%x, con1=0x%x\n",value,con1);*/
	...
	for (i = 0; i < NR_PLLS; i++) {
		if (pll_is_on(i)) {
			...
			pr_debug("suspend warning: %s is on!!!\n",
				 plls[i].name);
			/*pr_debug("warning! warning! warning! it may cause
			 * resume fail\n");
			 */
		}
	}
	for (i = 0; i < NR_SYSS; i++) {
		if (subsys_is_on(i)) {
			if ((i != SYS_MD1) && (i != SYS_C2K) &&
				...
				pr_debug("suspend warning: %s is on!!!\n",
					 syss[i].name);
				/*pr_debug("warning! warning! warning! it may
				 * cause resume fail\n");
				 */
			} else
				pr_debug("%s: on\n", syss[i].name);
...
static int proc_slp_chk_mtcmos_pll_stat_open(struct inode *inode,
						 struct file *file)
{
...
{
	int output = 0, i = 0;
	unsigned int temp, clk26cali_0, clk_dbg_cfg, clk_misc_cfg_0,
		clk26cali_1;
	...
	clk_writel(
		CLK_MISC_CFG_0,
		(clk_misc_cfg_0 & 0x00FFFFFF)); /*select divider?dvt set zero*/
...
unsigned int abist_meter_32k(void)
{
	...
	clk_writel(CLK_MISC_CFG_0,
		   clk_misc_cfg_0 &
			   0x00FFFFFF); /* select divider, WAIT CONFIRM*/

	clk26cali_1 = clk_readl(CLK26CALI_1);
	/*clk_writel(CLK26CALI_1, 0x00ff0000); // cycle count default
	 * 1024,[25:16]=3FF
	 */
...
unsigned int abist_meter(int ID)
{
	...
	clk_writel(CLK_MISC_CFG_0,
		   (clk_misc_cfg_0 & 0x00FFFFFF) |
			   0x01000000); /* select divider, WAIT CONFIRM*/
...
const char *ckgen_array[] = {"hd_faxi_ck",
				 "hf_fddrphycfg_ck",
				 "hf_fmm_ck",
				 "f_fpwm_ck",
				 "hf_fvdec_ck",
...
const char *ckgen_abist_array[] = {"AD_CSI0_DELAY_TSTCLK",
...
static int abist_meter_read(struct seq_file *m, void *v)
{
	...
	for (i = 1; i <= 57; i++) {
		/*skip unknown case */
		if (i == 18 || i == 19 || i == 21 || i == 37 || i == 47 ||
			i == 48)
			continue;
		if (i == 39)
			seq_printf(m, "[%d] %s: %d\n", i,
				   ckgen_abist_array[i - 1], abist_meter_32k());
		else
			seq_printf(m, "[%d] %s: %d\n", i,
				   ckgen_abist_array[i - 1], abist_meter(i));
	}
	return 0;
}
...
static unsigned int mt_get_emi_freq(void)
{
	...
	clk_writel(CLK_DBG_CFG,
		   (clk_dbg_cfg & 0xFFFFFFFC) | (50 << 16)); /*AD_MPLL_208M_CK*/

	clk_misc_cfg_0 = clk_readl(CLK_MISC_CFG_0);
	clk_writel(CLK_MISC_CFG_0,
		   (clk_misc_cfg_0 & 0x00FFFFFF)); /* select divider*/
...
static const struct file_operations cpu_fops = {
	.owner = THIS_MODULE, .open = proc_cpu_open, .read = seq_read,
};
...
static const struct file_operations mm_fops = {
	.owner = THIS_MODULE, .open = proc_mm_open, .read = seq_read,
};
...
static const struct file_operations venc_fops = {
	.owner = THIS_MODULE, .open = proc_venc_open, .read = seq_read,
};
...
static const struct file_operations emi_fops = {
	.owner = THIS_MODULE, .open = proc_emi_open, .read = seq_read,
};
...
static const struct file_operations bus_fops = {
	.owner = THIS_MODULE, .open = proc_bus_open, .read = seq_read,
};
...
void mt_clkmgr_debug_init(void)
{
	...
	entry = proc_create("armlpll_fsel", 0664,
				clkmgr_dir, &armlpll_fsel_proc_fops);
	entry = proc_create("armllpll_fsel", 0664,
				clkmgr_dir, &armllpll_fsel_proc_fops);
	entry = proc_create("armccipll_fsel", 0664,
				clkmgr_dir, &armccipll_fsel_proc_fops);
	entry = proc_create("mmpll_fsel", 0664,
				clkmgr_dir, &mmpll_fsel_proc_fops);
	entry = proc_create("vencpll_fsel", 0664,
				clkmgr_dir, &vencpll_fsel_proc_fops);
	entry = proc_create("pll_mtcmos_clk", 0664,
				clkmgr_dir, &clk_fsel_proc_fops);
...
static int __init mt_clkmgr_meter_init(void)
{
	...
	if (!pm_init_dir) {
		...
		entry = proc_create("cpu_speed_dump", 0444, pm_init_dir,
					&cpu_fops);
		entry = proc_create("mm_speed_dump", 0464,
					pm_init_dir, &mm_fops);
		entry = proc_create("venc_speed_dump", 0464,
					pm_init_dir, &venc_fops);
		entry = proc_create("emi_speed_dump", 0464,
					pm_init_dir, &emi_fops);
		entry = proc_create("abist_meter_test", 0644,
					pm_init_dir, &abist_meter_fops);
		entry = proc_create("ckgen_meter_test", 0644,
					pm_init_dir, &ckgen_meter_fops);
	}
