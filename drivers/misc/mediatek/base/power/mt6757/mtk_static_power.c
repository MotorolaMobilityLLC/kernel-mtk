...
int interpolate_2d(sptbl_t *tab, int v1, int v2, int t1, int t2, int voltage,
		   int degree)
{
	int c1, c2, p1, p2, p;
...
void interpolate_table(sptbl_t *spt, int c1, int c2, int c3, sptbl_t *tab1,
		       sptbl_t *tab2)
{
	int v, t;
...
int mt_spower_make_table(sptbl_t *spt, spower_raw_t *spower_raw, int wat,
...
void mt_spower_ut(void)
{
	...
	for (i = 0; i < MT_SPOWER_MAX; i++) {
		...
		case MT_SPOWER_MODEM_SRAM:
			SPOWER_INFO(
			    "[SPOWER] - This is MT_SPOWER_MODEM_SRAM\n");
			break;
		case MT_SPOWER_MD:
		...
		case MT_SPOWER_VCORE_SRAM:
			SPOWER_INFO(
			    "[SPOWER] - This is MT_SPOWER_VCORE_SRAM\n");
			break;
		default:
			break;
		}
...
int mt_spower_init(void)
{
	...
	SPOWER_INFO("[SPOWER] -LeakPW:c7l/c7ll/gpu/mod/mod_m/md/vcor/vcor_m = "
		    "%d/%d/%d/%d/%d/%d/%d/%d\n",
		    cpul, cpull, gpu, modem, modem_sram, md, vcore, vcore_sram);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_CPUL\n");
	mt_spower_make_table(&sptab[MT_SPOWER_CPUL], &cpul_spower_raw, cpul,
			     V_OF_FUSE, T_OF_FUSE);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_CPULL\n");
	mt_spower_make_table(&sptab[MT_SPOWER_CPULL], &cpull_spower_raw, cpull,
			     V_OF_FUSE, T_OF_FUSE);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_GPU\n");
	mt_spower_make_table(&sptab[MT_SPOWER_GPU], &gpu_spower_raw, gpu,
			     V_OF_FUSE, T_OF_FUSE);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_MODEM\n");
	mt_spower_make_table(&sptab[MT_SPOWER_MODEM], &modem_spower_raw, modem,
			     V_OF_FUSE, T_OF_FUSE);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_MODEM_SRAM\n");
	mt_spower_make_table(&sptab[MT_SPOWER_MODEM_SRAM],
			     &modem_sram_spower_raw, modem_sram, V_OF_FUSE,
			     T_OF_FUSE);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_MD\n");
	mt_spower_make_table(&sptab[MT_SPOWER_MD], &md_spower_raw, md,
			     V_OF_FUSE, T_OF_FUSE);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_VCORE\n");
	mt_spower_make_table(&sptab[MT_SPOWER_VCORE], &vcore_spower_raw, vcore,
			     V_OF_FUSE, T_OF_FUSE);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_VCORE_SRAM\n");
	mt_spower_make_table(&sptab[MT_SPOWER_VCORE_SRAM],
			     &vcore_sram_spower_raw, vcore_sram, V_OF_FUSE,
			     T_OF_FUSE);
