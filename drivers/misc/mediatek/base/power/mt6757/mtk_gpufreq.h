...
/* And this will cause power budge issue, so we add an extra para to support
 * skipping PBM operation
 */
...
/*Special API for MT6757p found an issue that when GPU power off, driver will
 * kick PBM to lower resource required
 */
...
extern void
mt_gpufreq_power_limit_notify_registerCB(gpufreq_power_limit_notify pCB);
