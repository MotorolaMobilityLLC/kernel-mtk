...
static const char *power_mode_str[NUM_PPB_POWER_MODE] = {
	"Default(Normal) mode", "Low Power mode", "Just Make mode",
...
enum cpu_level { CPU_LEVEL_0, CPU_LEVEL_1, CPU_LEVEL_2, NUM_CPU_LEVEL };
...
tag_pr_notice("@%s: missing clock-frequency property, use default CPU level\n",
		 __func__);
	return CPU_LEVEL_0;
	}
...
#define FP(khz, pos, clk)						       \
	{								   \
	.target_f = khz, .vco_dds = khz * pos * clk, .pos_div = pos,   \
	.clk_div = clk,						       \
	}
...
#define OP(khz, volt)							       \
	{								   \
	.cpufreq_khz = khz, .cpufreq_volt = volt,		       \
...
struct mt_cpu_dvfs {
	...
	*freq_tbl_for_cpufreq; /* freq table for cpufreq */
...
struct mt_cpu_dvfs_ops {
	...
	unsigned int (*get_cur_phy_freq)(
	struct mt_cpu_dvfs *p); /* return (physical) freq (KHz) */
	void (*set_cur_freq)(struct mt_cpu_dvfs *p, unsigned int cur_khz,
		 unsigned int target_khz); /* set freq	*/

	/* for vproc change (extBuck) */
	unsigned int (*get_cur_volt)(
	struct mt_cpu_dvfs *p); /* return volt (mV * 100) */
	int (*set_cur_volt)(struct mt_cpu_dvfs *p,
		unsigned int volt); /* set volt (mv * 100) */

	/* for vsram change (LDO) */
	unsigned int (*get_cur_vsram)(
	struct mt_cpu_dvfs *p); /* return volt (mV * 100) */
	int (*set_cur_vsram)(struct mt_cpu_dvfs *p,
		 unsigned int volt); /* set volt (mv * 100) */
...
static int set_cur_volt_extbuck(struct mt_cpu_dvfs *p,
		unsigned int volt); /* volt: mv * 100 */
static unsigned int get_cur_volt_sram(struct mt_cpu_dvfs *p);
static int set_cur_volt_sram(struct mt_cpu_dvfs *p,
		 unsigned int volt); /* volt: mv * 100 */
...
#define for_each_cpu_dvfs_only(i, p)					       \
	for (i = 0, p = cpu_dvfs;				   \
	 (i < NR_MT_CPU_DVFS) && (i != MT_CPU_DVFS_CCI);		   \
	 i++, p = &cpu_dvfs[i])
...
#else
#define cpu_dvfs_is_extbuck_valid()					       \
...
#define OPP_TBL(cluster, seg, lv, vol)					       \
	...
	OP(CPU_DVFS_FREQ0_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT0_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ1_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT1_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ2_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT2_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ3_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT3_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ4_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT4_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ5_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT5_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ6_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT6_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ7_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT7_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ8_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT8_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ9_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT9_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ10_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT10_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ11_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT11_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ12_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT12_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ13_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT13_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ14_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT14_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ15_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT15_VPROC##vol##_##seg),				   \
	}

#define OPP_TBL2(cluster, seg, lv, vol)					       \
	static struct mt_cpu_freq_info opp_tbl_##cluster##_e##lv##_1[] = {   \
	OP(CPU_DVFS_FREQ0_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT1_VPROC##vol##_##seg /* NOTE */),		   \
	OP(CPU_DVFS_FREQ1_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT1_VPROC##vol##_##seg),				   \
	OP(CPU_DVFS_FREQ2_##cluster##_##seg,				   \
	   CPU_DVFS_VOLT2_VPROC##vol##_##seg),				   \
...
static struct mt_cpu_freq_method opp_tbl_method_LL_e0[] = {
	...
	FP(CPU_DVFS_FREQ0_LL_FY, 1, 1),  FP(CPU_DVFS_FREQ1_LL_FY, 1, 1),
	FP(CPU_DVFS_FREQ2_LL_FY, 1, 1),  FP(CPU_DVFS_FREQ3_LL_FY, 2, 1),
	FP(CPU_DVFS_FREQ4_LL_FY, 2, 1),  FP(CPU_DVFS_FREQ5_LL_FY, 2, 1),
	FP(CPU_DVFS_FREQ6_LL_FY, 2, 1),  FP(CPU_DVFS_FREQ7_LL_FY, 2, 1),
	FP(CPU_DVFS_FREQ8_LL_FY, 2, 1),  FP(CPU_DVFS_FREQ9_LL_FY, 2, 1),
	FP(CPU_DVFS_FREQ10_LL_FY, 2, 1), FP(CPU_DVFS_FREQ11_LL_FY, 2, 2),
	FP(CPU_DVFS_FREQ12_LL_FY, 2, 2), FP(CPU_DVFS_FREQ13_LL_FY, 2, 2),
	FP(CPU_DVFS_FREQ14_LL_FY, 2, 2), FP(CPU_DVFS_FREQ15_LL_FY, 2, 4),
};

static struct mt_cpu_freq_method opp_tbl_method_L_e0[] = {
	/* Target Frequency,	POS,	CLK */
	FP(CPU_DVFS_FREQ0_L_FY, 1, 1),  FP(CPU_DVFS_FREQ1_L_FY, 1, 1),
	FP(CPU_DVFS_FREQ2_L_FY, 1, 1),  FP(CPU_DVFS_FREQ3_L_FY, 1, 1),
	FP(CPU_DVFS_FREQ4_L_FY, 1, 1),  FP(CPU_DVFS_FREQ5_L_FY, 1, 1),
	FP(CPU_DVFS_FREQ6_L_FY, 1, 1),  FP(CPU_DVFS_FREQ7_L_FY, 1, 1),
	FP(CPU_DVFS_FREQ8_L_FY, 1, 1),  FP(CPU_DVFS_FREQ9_L_FY, 2, 1),
	FP(CPU_DVFS_FREQ10_L_FY, 2, 1), FP(CPU_DVFS_FREQ11_L_FY, 2, 1),
	FP(CPU_DVFS_FREQ12_L_FY, 2, 1), FP(CPU_DVFS_FREQ13_L_FY, 2, 1),
	FP(CPU_DVFS_FREQ14_L_FY, 2, 2), FP(CPU_DVFS_FREQ15_L_FY, 2, 2),
};

static struct mt_cpu_freq_method opp_tbl_method_CCI_e0[] = {
	/* Target Frequency,	POS,	CLK */
	FP(CPU_DVFS_FREQ0_CCI_FY, 2, 1),  FP(CPU_DVFS_FREQ1_CCI_FY, 2, 1),
	FP(CPU_DVFS_FREQ2_CCI_FY, 2, 1),  FP(CPU_DVFS_FREQ3_CCI_FY, 2, 1),
	FP(CPU_DVFS_FREQ4_CCI_FY, 2, 1),  FP(CPU_DVFS_FREQ5_CCI_FY, 2, 1),
	FP(CPU_DVFS_FREQ6_CCI_FY, 2, 2),  FP(CPU_DVFS_FREQ7_CCI_FY, 2, 2),
	FP(CPU_DVFS_FREQ8_CCI_FY, 2, 2),  FP(CPU_DVFS_FREQ9_CCI_FY, 2, 2),
	FP(CPU_DVFS_FREQ10_CCI_FY, 2, 2), FP(CPU_DVFS_FREQ11_CCI_FY, 2, 2),
	FP(CPU_DVFS_FREQ12_CCI_FY, 2, 4), FP(CPU_DVFS_FREQ13_CCI_FY, 2, 4),
	FP(CPU_DVFS_FREQ14_CCI_FY, 2, 4), FP(CPU_DVFS_FREQ15_CCI_FY, 2, 4),
};

static struct mt_cpu_freq_method opp_tbl_method_LL_e1[] = {
	/* Target Frequency,	POS,	CLK */
	FP(CPU_DVFS_FREQ0_LL_FY2, 1, 1),  FP(CPU_DVFS_FREQ1_LL_FY2, 1, 1),
	FP(CPU_DVFS_FREQ2_LL_FY2, 1, 1),  FP(CPU_DVFS_FREQ3_LL_FY2, 1, 1),
	FP(CPU_DVFS_FREQ4_LL_FY2, 2, 1),  FP(CPU_DVFS_FREQ5_LL_FY2, 2, 1),
	FP(CPU_DVFS_FREQ6_LL_FY2, 2, 1),  FP(CPU_DVFS_FREQ7_LL_FY2, 2, 1),
	FP(CPU_DVFS_FREQ8_LL_FY2, 2, 1),  FP(CPU_DVFS_FREQ9_LL_FY2, 2, 1),
	FP(CPU_DVFS_FREQ10_LL_FY2, 2, 1), FP(CPU_DVFS_FREQ11_LL_FY2, 2, 2),
	FP(CPU_DVFS_FREQ12_LL_FY2, 2, 2), FP(CPU_DVFS_FREQ13_LL_FY2, 2, 2),
	FP(CPU_DVFS_FREQ14_LL_FY2, 2, 2), FP(CPU_DVFS_FREQ15_LL_FY2, 2, 4),
};

static struct mt_cpu_freq_method opp_tbl_method_L_e1[] = {
	/* Target Frequency,	POS,	CLK */
	FP(CPU_DVFS_FREQ0_L_FY2, 1, 1),  FP(CPU_DVFS_FREQ1_L_FY2, 1, 1),
	FP(CPU_DVFS_FREQ2_L_FY2, 1, 1),  FP(CPU_DVFS_FREQ3_L_FY2, 1, 1),
	FP(CPU_DVFS_FREQ4_L_FY2, 1, 1),  FP(CPU_DVFS_FREQ5_L_FY2, 1, 1),
	FP(CPU_DVFS_FREQ6_L_FY2, 1, 1),  FP(CPU_DVFS_FREQ7_L_FY2, 1, 1),
	FP(CPU_DVFS_FREQ8_L_FY2, 1, 1),  FP(CPU_DVFS_FREQ9_L_FY2, 2, 1),
	FP(CPU_DVFS_FREQ10_L_FY2, 2, 1), FP(CPU_DVFS_FREQ11_L_FY2, 2, 1),
	FP(CPU_DVFS_FREQ12_L_FY2, 2, 1), FP(CPU_DVFS_FREQ13_L_FY2, 2, 1),
	FP(CPU_DVFS_FREQ14_L_FY2, 2, 2), FP(CPU_DVFS_FREQ15_L_FY2, 2, 2),
};

static struct mt_cpu_freq_method opp_tbl_method_CCI_e1[] = {
	/* Target Frequency,	POS,	CLK */
	FP(CPU_DVFS_FREQ0_CCI_FY2, 2, 1),  FP(CPU_DVFS_FREQ1_CCI_FY2, 2, 1),
	FP(CPU_DVFS_FREQ2_CCI_FY2, 2, 1),  FP(CPU_DVFS_FREQ3_CCI_FY2, 2, 1),
	FP(CPU_DVFS_FREQ4_CCI_FY2, 2, 1),  FP(CPU_DVFS_FREQ5_CCI_FY2, 2, 1),
	FP(CPU_DVFS_FREQ6_CCI_FY2, 2, 1),  FP(CPU_DVFS_FREQ7_CCI_FY2, 2, 2),
	FP(CPU_DVFS_FREQ8_CCI_FY2, 2, 2),  FP(CPU_DVFS_FREQ9_CCI_FY2, 2, 2),
	FP(CPU_DVFS_FREQ10_CCI_FY2, 2, 2), FP(CPU_DVFS_FREQ11_CCI_FY2, 2, 2),
	FP(CPU_DVFS_FREQ12_CCI_FY2, 2, 4), FP(CPU_DVFS_FREQ13_CCI_FY2, 2, 4),
	FP(CPU_DVFS_FREQ14_CCI_FY2, 2, 4), FP(CPU_DVFS_FREQ15_CCI_FY2, 2, 4),
};

static struct mt_cpu_freq_method opp_tbl_method_LL_e2[] = {
	/* Target Frequency,	POS,	CLK */
	FP(CPU_DVFS_FREQ0_LL_FY3, 1, 1),  FP(CPU_DVFS_FREQ1_LL_FY3, 1, 1),
	FP(CPU_DVFS_FREQ2_LL_FY3, 1, 1),  FP(CPU_DVFS_FREQ3_LL_FY3, 1, 1),
	FP(CPU_DVFS_FREQ4_LL_FY3, 1, 1),  FP(CPU_DVFS_FREQ5_LL_FY3, 2, 1),
	FP(CPU_DVFS_FREQ6_LL_FY3, 2, 1),  FP(CPU_DVFS_FREQ7_LL_FY3, 2, 1),
	FP(CPU_DVFS_FREQ8_LL_FY3, 2, 1),  FP(CPU_DVFS_FREQ9_LL_FY3, 2, 1),
	FP(CPU_DVFS_FREQ10_LL_FY3, 2, 1), FP(CPU_DVFS_FREQ11_LL_FY3, 2, 2),
	FP(CPU_DVFS_FREQ12_LL_FY3, 2, 2), FP(CPU_DVFS_FREQ13_LL_FY3, 2, 2),
	FP(CPU_DVFS_FREQ14_LL_FY3, 2, 2), FP(CPU_DVFS_FREQ15_LL_FY3, 2, 4),
};

static struct mt_cpu_freq_method opp_tbl_method_L_e2[] = {
	/* Target Frequency,	POS,	CLK */
	FP(CPU_DVFS_FREQ0_L_FY3, 1, 1),  FP(CPU_DVFS_FREQ1_L_FY3, 1, 1),
	FP(CPU_DVFS_FREQ2_L_FY3, 1, 1),  FP(CPU_DVFS_FREQ3_L_FY3, 1, 1),
	FP(CPU_DVFS_FREQ4_L_FY3, 1, 1),  FP(CPU_DVFS_FREQ5_L_FY3, 1, 1),
	FP(CPU_DVFS_FREQ6_L_FY3, 1, 1),  FP(CPU_DVFS_FREQ7_L_FY3, 1, 1),
	FP(CPU_DVFS_FREQ8_L_FY3, 1, 1),  FP(CPU_DVFS_FREQ9_L_FY3, 2, 1),
	FP(CPU_DVFS_FREQ10_L_FY3, 2, 1), FP(CPU_DVFS_FREQ11_L_FY3, 2, 1),
	FP(CPU_DVFS_FREQ12_L_FY3, 2, 1), FP(CPU_DVFS_FREQ13_L_FY3, 2, 1),
	FP(CPU_DVFS_FREQ14_L_FY3, 2, 2), FP(CPU_DVFS_FREQ15_L_FY3, 2, 2),
};

static struct mt_cpu_freq_method opp_tbl_method_CCI_e2[] = {
	/* Target Frequency,	POS,	CLK */
	FP(CPU_DVFS_FREQ0_CCI_FY3, 2, 1),  FP(CPU_DVFS_FREQ1_CCI_FY3, 2, 1),
	FP(CPU_DVFS_FREQ2_CCI_FY3, 2, 1),  FP(CPU_DVFS_FREQ3_CCI_FY3, 2, 1),
	FP(CPU_DVFS_FREQ4_CCI_FY3, 2, 1),  FP(CPU_DVFS_FREQ5_CCI_FY3, 2, 1),
	FP(CPU_DVFS_FREQ6_CCI_FY3, 2, 1),  FP(CPU_DVFS_FREQ7_CCI_FY3, 2, 2),
	FP(CPU_DVFS_FREQ8_CCI_FY3, 2, 2),  FP(CPU_DVFS_FREQ9_CCI_FY3, 2, 2),
	FP(CPU_DVFS_FREQ10_CCI_FY3, 2, 2), FP(CPU_DVFS_FREQ11_CCI_FY3, 2, 2),
	FP(CPU_DVFS_FREQ12_CCI_FY3, 2, 4), FP(CPU_DVFS_FREQ13_CCI_FY3, 2, 4),
	FP(CPU_DVFS_FREQ14_CCI_FY3, 2, 4), FP(CPU_DVFS_FREQ15_CCI_FY3, 2, 4),
};
...
static struct opp_tbl_info opp_tbls_v12d[NR_MT_CPU_DVFS][NUM_CPU_LEVEL] = {
	...
	[CPU_LEVEL_0] = {opp_tbl_LL_e0_0, ARRAY_SIZE(opp_tbl_LL_e0_0),
		 DVFS_TABLE_TYPE_FY},
	[CPU_LEVEL_1] = {opp_tbl_LL_e1_1, ARRAY_SIZE(opp_tbl_LL_e1_1),
		 DVFS_TABLE_TYPE_FY},
...
static struct opp_tbl_m_info opp_tbls_m_v12d[NR_MT_CPU_DVFS][NUM_CPU_LEVEL] = {
	...
	[CPU_LEVEL_0] = {opp_tbl_method_LL_e0,
		 ARRAY_SIZE(opp_tbl_method_LL_e0)},
	[CPU_LEVEL_1] = {opp_tbl_method_LL_e1,
		 ARRAY_SIZE(opp_tbl_method_LL_e1)},
	[CPU_LEVEL_2] = {opp_tbl_method_LL_e2,
		 ARRAY_SIZE(opp_tbl_method_LL_e2)},
	},
	/* L */
	{
	[CPU_LEVEL_0] = {opp_tbl_method_L_e0,
		 ARRAY_SIZE(opp_tbl_method_L_e0)},
	[CPU_LEVEL_1] = {opp_tbl_method_L_e1,
		 ARRAY_SIZE(opp_tbl_method_L_e1)},
	[CPU_LEVEL_2] = {opp_tbl_method_L_e2,
		 ARRAY_SIZE(opp_tbl_method_L_e2)},
	},
	/* CCI */
	{
	[CPU_LEVEL_0] = {opp_tbl_method_CCI_e0,
		 ARRAY_SIZE(opp_tbl_method_CCI_e0)},
	[CPU_LEVEL_1] = {opp_tbl_method_CCI_e1,
		 ARRAY_SIZE(opp_tbl_method_CCI_e1)},
	[CPU_LEVEL_2] = {opp_tbl_method_CCI_e2,
		 ARRAY_SIZE(opp_tbl_method_CCI_e2)},
	},
};
...
static int _set_cur_volt_locked(struct mt_cpu_dvfs *p,
		unsigned int volt) /* volt: mv * 100 */
{
	int ret = -1;
...
unsigned int mt_cpufreq_get_leakage_mw(enum mt_cpu_dvfs_id id)
{
	...
	for_each_cpu_dvfs_only(i, p) {
		if (cpu_dvfs_is(p, MT_CPU_DVFS_LL) &&
			p->armpll_is_available) {
			temp = tscpu_get_temp_by_bank(THERMAL_BANK0) /
				1000;
			mw += mt_spower_get_leakage(
			MT_SPOWER_CPULL,
			cpu_dvfs_get_cur_volt(p) / 100, temp);
		} else if (cpu_dvfs_is(p, MT_CPU_DVFS_L) &&
			p->armpll_is_available) {
	...
	if (cpu_dvfs_is(p, MT_CPU_DVFS_LL)) {
		...
		mw = mt_spower_get_leakage(
		MT_SPOWER_CPULL, cpu_dvfs_get_cur_volt(p) / 100,
		temp);
	} else if (cpu_dvfs_is(p, MT_CPU_DVFS_L)) {
		temp = tscpu_get_temp_by_bank(THERMAL_BANK1) / 1000;
		mw = mt_spower_get_leakage(
		MT_SPOWER_CPUL, cpu_dvfs_get_cur_volt(p) / 100,
		temp);
	}
	}
...
static void _kick_PBM_by_cpu(struct mt_cpu_dvfs *p)
{
	...
	cpufreq_ver("%d: core = %d, idx = %d, volt = %d\n", i,
		ppm_data[i].core_num, ppm_data[i].freq_idx,
		ppm_data[i].volt);
	}
...
static enum top_ckmuxsel _get_cpu_clock_switch(struct mt_cpu_dvfs *p)
{
	...
	mask = (cpu_dvfs_is(p, MT_CPU_DVFS_LL)
	    ? _BITMASK_(9 : 8)
	...
	val = (cpu_dvfs_is(p, MT_CPU_DVFS_LL)
	   ? val >> 8
...
static void adjust_armpll_dds(struct mt_cpu_dvfs *p, unsigned int vco,
...
static void adjust_posdiv(struct mt_cpu_dvfs *p, unsigned int pos_div)
{
	...
	sel = (pos_div == 1 ? 0 : pos_div == 2 ? 1 : pos_div == 4 ? 2 : 0);
...
static void adjust_clkdiv(struct mt_cpu_dvfs *p, unsigned int clk_div)
{
	...
	sel = (clk_div == 1 ? 8 : clk_div == 2 ? 10 : clk_div == 4 ? 11 : 8);
	     ...
	     unsigned int target_khz)
{
	...
	cur_posdiv = (sel == 0 ? 1 : sel == 1 ? 2 : sel == 2 ? 4 : 1);
	sel = _GET_BITS_VAL_(4 : 0, cpufreq_read(p->ckdiv_addr));
	cur_clkdiv = (sel == 8 ? 1 : sel == 10 ? 2 : sel == 11 ? 4 : 1);
	cpufreq_ver("[CUR_OPP_IDX][NAME][IDX][FREQ] => %s:%d:%d\n",
	    cpu_dvfs_get_name(p), p->idx_opp_tbl,
	...
	cpufreq_ver("[TARGET_OPP_IDX][NAME][IDX][FREQ] => %s:%d:%d\n",
	    cpu_dvfs_get_name(p), idx,
	    cpu_dvfs_get_freq_by_idx(p, idx));
	...
	tag_pr_debug("%s: %s: cur_khz = %u(%d), target_khz = %u(%d), clkdiv = %u->%u\n",
			__func__, cpu_dvfs_get_name(p), cur_khz,
			p->idx_opp_tbl, target_khz, idx, cur_clkdiv,
	...
	dds = _GET_BITS_VAL_(
	    21 : 0,
	    _cpu_dds_calc(opp_tbl_m[TARGET_OPP_IDX].slot->vco_dds));
	mt_dfs_armpll(p->hopping_id, dds);
	} else {
	aee_record_cpu_dvfs_step(8);

	adjust_armpll_dds(p, opp_tbl_m[TARGET_OPP_IDX].slot->vco_dds,
...
#ifdef CONFIG_HYBRID_CPU_DVFS
static void set_cur_freq_hybrid(struct mt_cpu_dvfs *p, unsigned int cur_khz,
		unsigned int target_khz)
{
	int r, index;
	...
	if (r) {
	pr_debug(
	    "cluster%u dvfs failed (%d): opp = %d, freq = %u (%u)\n",
...
{
#ifdef CONFIG_MTK_PMIC_CHIP_MT6355
	pmic_set_register_value(PMIC_RG_LDO_VSRAM_PROC_VOSEL,
...
static unsigned int get_cur_volt_extbuck(struct mt_cpu_dvfs *p)
{
	...
	do {
		...
		ret_val =
		pmic_get_register_value(PMIC_RG_BUCK_VPROC11_VOSEL);
		if (ret_val > 0x7f) {
			ret_volt = 0;
#else
		       ...
		       unsigned int new_vsram)
{
	...
	if (new_vsram >= old_vsram)
		delay =
			MAX(PMIC_VOLT_UP_SETTLE_TIME(old_vsram, new_vsram),
...
static void dump_all_opp_table(void)
{
	...
	tag_pr_notice("[%s/%u] available = %d, oppidx = %d (%u, %u)\n",
		  ...
		  p->idx_opp_tbl, cpu_dvfs_get_cur_freq(p),
		     ...
		     unsigned int old_vproc)
{
	if (unlikely(cur_vsram < cur_vproc ||
		...
		pr_debug("@%d, volt = %u, cur_vsram = %u (%u), cur_vproc = %u (%u)\n",
			line, volt, cur_vsram, old_vsram, cur_vproc,
			old_vproc);
		WARN_ON(1);
	}
}
...
{ /* volt: vproc (mv*100) */
	...
	next_vsram =
		MIN((MAX_DIFF_VSRAM_VPROC - 2500) + cur_vproc,
		target_vsram);
	...
	if (cur_vsram > max_vsram_volt) {
		...
		target_vsram =
		    max_vsram_volt; /* to end the loop */
	}
	...
	delta[SET_VSRAM] =
		ktime_sub(ktime_get(), now[SET_VSRAM]);
	if (ktime_to_us(delta[SET_VSRAM]) >
		ktime_to_us(max[SET_VSRAM]))
...
#ifdef CONFIG_MTK_PMIC_CHIP_MT6355
	pmic_set_register_value(PMIC_RG_BUCK_VPROC11_VOSEL,
			VOLT_TO_EXTBUCK_VAL(cur_vproc));
#else
	mt6311_set_vdvfs11_vosel_on(
		VOLT_TO_EXTBUCK_VAL(cur_vproc));
#endif
	delta[SET_VPROC] =
		ktime_sub(ktime_get(), now[SET_VPROC]);
	if (ktime_to_us(delta[SET_VPROC]) >
		ktime_to_us(max[SET_VPROC]))
		max[SET_VPROC] = delta[SET_VPROC];

	delay_us = _calc_pmic_settle_time(old_vproc, old_vsram,
			      cur_vproc, cur_vsram);
	udelay(delay_us);

cpufreq_ver(
"@%s():UP, old_vsram=%u, cur_vsram=%u, old_vproc=%u, cur_vproc=%u, delay=%u\n",
		    __func__, old_vsram, cur_vsram, old_vproc,
		    cur_vproc, delay_us);
	...
	do {
		...
		next_vproc = MAX(
			next_vsram - (MAX_DIFF_VSRAM_VPROC - 2500), volt);
...
cpufreq_ver(
"@%s(): DOWN old_vsram=%u cur_vsram=%u old_vproc=%u cur_vproc=%u delay=%u\n",
		    __func__, old_vsram, cur_vsram, old_vproc,
		    cur_vproc, delay_us);
	...
	cpufreq_ver(
	"DVFS: End @%s(): %s, cur_vsram = %u (%u), cur_vproc = %u (%u)\n",
...
static int _cpufreq_set_locked_cci(unsigned int cur_cci_khz,
		   unsigned int target_cci_khz,
		   unsigned int target_cci_volt)
{
	...
	cpufreq_ver("@%s(), %s: cur_cci_volt = %d, target_cci_volt = %d\n",
	    __func__, cpu_dvfs_get_name(p_cci), cur_cci_volt,
	    target_cci_volt);
	...
	cpufreq_ver("@%s(): Vproc = %dmv, Vsram = %dmv, freq = %dKHz\n",
	    __func__, (p_cci->ops->get_cur_volt(p_cci)) / 100,
	    (p_cci->ops->get_cur_vsram(p_cci) / 100),
	    p_cci->ops->get_cur_phy_freq(p_cci));
		   ...
		   unsigned int target_volt_vproc1, int log)
{
	...
	cpufreq_ver("DVFS:  cpufreq set locked: target_vproc1 = %d\n",
	    target_volt_vproc1);
...
"@%s(), %s:(%d,%d): freq=%d, volt=%d, on=%d, cur=%d, cci(%d,%d)\n",
		     ...
		     target_khz, target_volt, num_online_cpus(),
		     cur_khz, cur_cci_khz, target_cci_khz);
	else
cpufreq_ver("@%s(), %s:(%d,%d): freq=%d, volt=%d, on=%d, cur=%d, cci(%d,%d)\n",
		    __func__, cpu_dvfs_get_name(p),
		    p->idx_opp_ppm_base, p->idx_opp_ppm_limit,
		    target_khz, target_volt, num_online_cpus(),
		    cur_khz, cur_cci_khz, target_cci_khz);
	...
	__func__, (p->ops->get_cur_volt(p)) / 100,
	(p->ops->get_cur_vsram(p) / 100), p->name,
	...
	unsigned int freq = p->ops->get_cur_phy_freq(p);
		...
		if (volt < target_volt || freq != target_khz) {
			pr_debug("volt = %u, target_volt = %u, freq = %u, target_khz = %u\n",
...
static void _mt_cpufreq_set(struct cpufreq_policy *policy,
		enum mt_cpu_dvfs_id id, int new_opp_idx, int lock)
{
	struct mt_cpu_dvfs *p = id_to_cpu_dvfs(id);
	...
	/* Get cci opp idx */
	new_cci_opp_idx =
	_calc_new_cci_opp_idx(p, new_opp_idx, &target_volt_vpro1);
	...
	ret =
	_cpufreq_set_locked(p, cur_freq, target_freq, policy, cur_cci_freq,
		target_cci_freq, target_volt_vpro1, log);
...
static int _cpufreq_dfs_locked(struct cpufreq_policy *policy,
		   struct mt_cpu_dvfs *p, unsigned int freq_idx,
...
cpufreq_ver(
	"@%s() : %d , cpu = %d , action = %lu , oppidx = %d , num_online_cpus = %d \n",
	...
	if (dev) {
	if (online_cpus == SINGLE_CORE_BOUNDARY_CPU_NUM &&
		cpu_dvfs_is(p, MT_CPU_DVFS_LL)) {
		switch (action) {
		case CPU_UP_PREPARE:
	...
	case CPU_ONLINE:
		...
		cpufreq_ver("CB - adjust the freq to V : %d  due to L/LL on\n",
			...
			freq_idx =
				MAX(freq_idx,
				_calc_new_opp_idx(p, freq_idx));
			_cpufreq_dfs_locked(p->mt_policy, p,
					freq_idx, action);
		}
#if defined(CONFIG_HYBRID_CPU_DVFS) && defined(CPUHVFS_HW_GOVERNOR)
UNLOCK_OL:
	...
	case CPU_DOWN_PREPARE:
		...
		if (cpus == 1) {
			...
			_cpufreq_dfs_locked(NULL, p, p->nr_opp_tbl - 1,
				action);
#ifdef CONFIG_HYBRID_CPU_DVFS
		if (!enable_cpuhvfs) {
#endif
			new_cci_opp_idx = _calc_new_cci_opp_idx(
			p, p->idx_opp_tbl,
			&target_volt_vpro1);
			/* set cci freq/volt */
			_cpufreq_set_locked_cci(
			cpu_dvfs_get_cur_freq(p_cci),
			cpu_dvfs_get_freq_by_idx(
			p_cci, new_cci_opp_idx),
			target_volt_vpro1);
			p_cci->idx_opp_tbl = new_cci_opp_idx;
			aee_record_freq_idx(p_cci,
				p_cci->idx_opp_tbl);
#ifdef CONFIG_HYBRID_CPU_DVFS
		}
#endif
...
UNLOCK_DF:
	...
	__LINE__, num_online_cpus_delta);
		      ...
		      unsigned int *target_cci_volt)
{
	...
	if (pp->armpll_is_available ||
		...
		target_cci_freq =
		MAX(target_cci_freq,
		cpu_dvfs_get_freq_by_idx(pp, freq_idx[i]));
		/* V */
	...
	*target_cci_volt =
		_search_available_volt(p_cci, target_cci_freq);

	/* Determine dominating voltage */
	*target_cci_volt = MAX(*target_cci_volt,
		MAX(volt[MT_CPU_DVFS_LL], volt[MT_CPU_DVFS_L]));
#else
...
static void ppm_limit_callback(struct ppm_client_req req)
{
	...
	if (ppm->cpu_limit[i].has_advise_freq) {
		...
		p->idx_opp_ppm_base =
			ppm->cpu_limit[i]
			.min_cpufreq_idx; /* ppm update base */
		p->idx_opp_ppm_limit =
			ppm->cpu_limit[i]
			.max_cpufreq_idx; /* ppm update limit */
	if ((p->idx_opp_tbl >=
		ppm->cpu_limit[i].max_cpufreq_idx) &&
		(p->idx_opp_tbl <=
		 ppm->cpu_limit[i].min_cpufreq_idx)) {
			cpufreq_ver("idx = %d , idx_opp_ppm_base = %d , idx_opp_ppm_limit = %d \n",
			p->idx_opp_tbl, p->idx_opp_ppm_base,
			p->idx_opp_ppm_limit);
		ignore_ppm[i] = 1;
		}
	}
#if defined(CONFIG_HYBRID_CPU_DVFS) && defined(CPUHVFS_HW_GOVERNOR)
	if (enable_cpuhvfs && enable_hw_gov) {
		cpuhvfs_set_opp_limit(
		cpu_dvfs_to_cluster(p),
	...
	for_each_cpu_dvfs_only(i, p) {
		if (p->armpll_is_available && p->mt_policy->governor) {
			...
			p->mt_policy->max = cpu_dvfs_get_freq_by_idx(
				p, p->idx_opp_ppm_limit);

		if (p->idx_opp_ppm_base == -1)
			p->mt_policy->min = cpu_dvfs_get_min_freq(p);
		else
			p->mt_policy->min = cpu_dvfs_get_freq_by_idx(
				p, p->idx_opp_ppm_base);
 ...
 * cpufreq driver
 */
int mt_cpufreq_get_ppb_state(void) { return dvfs_power_mode; }
...
static int _mt_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	cpufreq_ver("DVFS: _mt_ cpufreq_ init: %s(cpu_id = %d)\n",
	...
	policy->cpuinfo.max_freq =
	    cpu_dvfs_get_max_freq(id_to_cpu_dvfs(id));
	policy->cpuinfo.min_freq =
	    cpu_dvfs_get_min_freq(id_to_cpu_dvfs(id));

	policy->cur = mt_cpufreq_get_cur_phy_freq(
	    id); /* use cur phy freq is better */
...
static int _mt_cpufreq_exit(struct cpufreq_policy *policy) { return 0; }
...
static void __set_cpuhvfs_init_sta(struct init_sta *sta)
{
	...
	volt = get_cur_volt_extbuck(p);
	WARN_ON(volt < EXTBUCK_VAL_TO_VOLT(0));
...
static void notify_cpuhvfs_change_cb(struct dvfs_log *log_box, int num_log)
{
	...
	if (num_log == 1) {
		...
		for (j = num_log - 1; j >= 1; j--) {
			tf_sum +=
			(log_box[j].time - log_box[j - 1].time) *
			cpu_dvfs_get_freq_mhz(
			p, log_box[j - 1].opp[i]);
	}
		...
		if (j < 0) {
			tag_pr_notice("tf_sum = %u, t_diff = %u, avg_f "
			  "= %u, max_f = %u\n",
			  tf_sum, t_diff, avg_f,
			  cpu_dvfs_get_freq_mhz(p, 0));
		j = 0;
	    }
	}
	...
	tag_pr_debug("new_freq = (%u %u) <- (%u %u) @ %08x\n", new_f[0],
		 new_f[1], old_f[0], old_f[1], log_box[0].time);
 ...
 * Platform driver
 */
static int _mt_cpufreq_pm_callback(struct notifier_block *nb,
...
static int _mt_cpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	if (!p->freq_tbl_for_cpufreq) {
		table =
		kzalloc((opp_tbl_info->size + 1) * sizeof(*table),
			GFP_KERNEL);
	...
	if (j != MT_CPU_DVFS_CCI)
		mt_ppm_set_dvfs_table(
		p->cpu_id, p->freq_tbl_for_cpufreq,
		opp_tbl_info->size, opp_tbl_info->type);
	}
...
static int cpufreq_power_mode_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%s\n", mode < NUM_PPB_POWER_MODE ? power_mode_str[mode]
			    : "Unknown");
...
static ssize_t ppb_hispeed_freq_proc_write(struct file *file,
			const char __user *buffer,
			size_t count, loff_t *pos)
{
	char *tok;
...
static ssize_t enable_cpuhvfs_proc_write(struct file *file,
...
static ssize_t enable_hw_gov_proc_write(struct file *file,
			const char __user *ubuf, size_t count,
			loff_t *ppos)
{
	int r;
	...
	case 1: /* force on */
		cpufreq_lock();
		hwgov_en_map =
		(hwgov_en_map & ~HWGOV_DIS_FORCE) | HWGOV_EN_FORCE;
		__switch_hwgov_on_off(1);
	cpufreq_unlock();
	break;
	...
	case 0: /* force off */
		cpufreq_lock();
		hwgov_en_map =
			(hwgov_en_map & ~HWGOV_EN_FORCE) | HWGOV_DIS_FORCE;
		__switch_hwgov_on_off(0);
		cpufreq_unlock();
	break;
	}
 ...
 */
		...
		cpuhvfs_set_target_opp(cpu_dvfs_to_cluster(p), oppidx,
			   NULL);
#endif
	} else {
		p->dvfs_disable_by_procfs = false;
...
static int _create_procfs(void)
{
	...
	PROC_ENTRY(cpufreq_oppidx), PROC_ENTRY(cpufreq_freq),
	PROC_ENTRY(cpufreq_volt), PROC_ENTRY(ppb_hispeed_freq),
	};
	...
	for (i = 0; i < ARRAY_SIZE(cpu_entries); i++) {
		...
		tag_pr_notice(
		    ...
		    __func__, p->name, cpu_entries[i].name);
	}
	}
