...
#define DEF_CA7L_LEAKAGE		(150 * V_OF_FUSE/1000)
#define DEF_CA7LL_LEAKAGE		(100 * V_OF_FUSE/1000)
...
static sptbl_t sptab[MT_SPOWER_MAX]; /* CA7L, CA7LL, GPU, MODEM, MODEM_SRAM, MD, VCORE, VCORE_SRAM  */
...
void mt_spower_ut(void)
{
	...
	for (i = 0; i < MT_SPOWER_MAX; i++) {
		...
		switch (i) {
		case  MT_SPOWER_CA7L:
			SPOWER_INFO("[SPOWER] - This is MT_SPOWER_CA7L\n");
			break;
		case  MT_SPOWER_CA7LL:
			SPOWER_INFO("[SPOWER] - This is MT_SPOWER_CA7LL\n");
			break;
		case  MT_SPOWER_GPU:
		...
		switch (i) {
		case  MT_SPOWER_CA7L:
			SPOWER_INFO("[SPOWER] -  CA7L efuse:%d\n", mt_spower_get_efuse_lkg(MT_SPOWER_CA7L));
			SPOWER_INFO("[SPOWER] -  MT_SPOWER_CA7L Done\n");
			break;
		case  MT_SPOWER_CA7LL:
			SPOWER_INFO("[SPOWER] -  CA7LL efuse:%d\n", mt_spower_get_efuse_lkg(MT_SPOWER_CA7LL));
			SPOWER_INFO("[SPOWER] -  MT_SPOWER_CA7LL Done\n");
			break;
		case  MT_SPOWER_GPU:
...
int mt_spower_init(void)
{
	...
	int ca7l, ca7ll, gpu, modem, modem_sram, md, vcore, vcore_sram;

	if (1 == mtSpowerInited)
		return 0;

	/* avoid side effect from multiple invocation */
	if (tab_validate(&sptab[MT_SPOWER_CA7L]))
		return 0;
	...
	ca7l, ca7ll, gpu, modem, modem_sram, md, vcore, vcore_sram);

	if (ca7l != 0) {
		ca7l = (int)devinfo_table[ca7l];
		ca7l = (int)(ca7l*V_OF_FUSE/1000);
	} else {
		ca7l = DEF_CA7L_LEAKAGE;
	}

	if (ca7ll != 0) {
		ca7ll = (int)devinfo_table[ca7ll];
		ca7ll = (int)(ca7ll*V_OF_FUSE/1000);
	} else {
		ca7ll = DEF_CA7LL_LEAKAGE;
	}
	...
	ca7l, ca7ll, gpu, modem, modem_sram, md, vcore, vcore_sram);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_CA7L\n");
	mt_spower_make_table(&sptab[MT_SPOWER_CA7L], &ca7l_spower_raw, ca7l, V_OF_FUSE, T_OF_FUSE);
	SPOWER_INFO("[SPOWER] - MT_SPOWER_CA7LL\n");
	mt_spower_make_table(&sptab[MT_SPOWER_CA7LL], &ca7ll_spower_raw, ca7ll, V_OF_FUSE, T_OF_FUSE);
...
int mt_spower_get_efuse_lkg(int dev)
{
	...
	case  MT_SPOWER_CA7L:
		...
		efuse_lkg_mw = (efuse_lkg == 0) ? DEF_CA7L_LEAKAGE : (int)(devinfo_table[efuse_lkg]*V_OF_FUSE/1000);
		break;
	case  MT_SPOWER_CA7LL:
		devinfo = (int)get_devinfo_with_index(DEVINFO_IDX4); /* P_OD4 */
		efuse_lkg = (devinfo >> 8) & 0xff;
		efuse_lkg_mw = (efuse_lkg == 0) ? DEF_CA7LL_LEAKAGE : (int)(devinfo_table[efuse_lkg]*V_OF_FUSE/1000);
		break;
	case  MT_SPOWER_GPU:
	...
	switch (dev) {
	case  MT_SPOWER_CA7L:
		efuse_lkg_mw = DEF_CA7L_LEAKAGE;
		break;
	case  MT_SPOWER_CA7LL:
		efuse_lkg_mw = DEF_CA7LL_LEAKAGE;
		break;
	case  MT_SPOWER_GPU:
