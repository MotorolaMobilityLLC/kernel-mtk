...
#define picachu_err(fmt, args...) pr_debug(TAG "[ERROR]" fmt, ##args)
#define picachu_warn(fmt, args...) pr_debug(TAG "[WARNING]" fmt, ##args)
#define picachu_info(fmt, args...) pr_debug(TAG "" fmt, ##args)
...
#define picachu_cont(fmt, args...) pr_cont(fmt, ##args)
...
static int create_procfs(void)
{
	...
	struct pentry entries[] = {
	    PROC_ENTRY(picachu_enable), PROC_ENTRY(picachu_offset),
	    PROC_ENTRY(picachu_dump), PROC_ENTRY(picachu_clear_emmc),
	};
...
static int __init picachu_init(void)
{
	...
	picachu_data =
	    (struct picachu_info *)ioremap_nocache(PICACHU_BASE, PICACHU_SIZE);
