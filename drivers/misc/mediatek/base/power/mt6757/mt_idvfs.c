...
#include "mt_freqhopping_drv.h"  /* mt6757_0x1001AXXX_lock */
...
int BigiDVFSEnable_hp(void) /* for cpu hot plug call */
{
	...
	mt6757_0x1001AXXX_reg_set(0x274, 0x1f, 0x8);
	udelay(2);
...
int BigIDVFSFreq(unsigned int Freqpct_x100)
{
	...
	if ((Freqpct_x100 < 2000) && (idvfs_init_opt.channel[IDVFS_CHANNEL_SWP].percentage > 2000)) {
		...
		if (Freqpct_x100 < 1300)
			mt6757_0x1001AXXX_reg_set(0x274, 0x1f, 0x12);
		else
			mt6757_0x1001AXXX_reg_set(0x274, 0x1f, 0x1a);
		udelay(2);
	}
