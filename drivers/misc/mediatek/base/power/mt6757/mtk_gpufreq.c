 ...
 * 0: olympus, 1: kibo, 3:kibo+, 7: kibo+dual
 **************************************************************************************/
...
#if 1	/* todo: need to confirm ready */
#ifdef CONFIG_THERMAL
...
unsigned int mt_gpufreq_target(unsigned int idx, bool enable_kick_pbm)
{
	 ...
	 *******************************/
	mt_gpufreq_set(g_cur_gpu_freq, target_freq, g_cur_gpu_volt, target_volt, enable_kick_pbm);
...
static int mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(mt_gpufreq_pmic->reg_vgpu)) {
		dev_err(&pdev->dev, "mt_gpufreq_pdrv_probe, cannot get vgpu\n");
	...
	if (IS_ERR(mt_gpufreq_pmic->reg_vsram)) {
		dev_err(&pdev->dev, "mt_gpufreq_pdrv_probe, cannot get vsram_vgpu\n");
...
static int __init mt_gpufreq_init(void)
{
	...
	pr_err("@%s: ED2016: GPU DVFS bring up bypass\n", __func__);
	return 0;
#endif
	gpufreq_info("@%s\n", __func__);
