...
int srclken_fs_init(void)
{
	...
	if (r)
		pr_err("FAILED TO CREATE /sys/power/srclken (%d)\n", r);
...
int srclken_dts_map(void)
{
	...
	if (node) {
		...
		if (!srclken_base) {
			pr_err("%s() can't find iomem for srclken\n",
				__func__);
			return -1;
		}
	} else {
		pr_err("%s can't find compatible node for srclken\n",
			__func__);
		return -1;
	}
	...
	if (node) {
		...
		if (!pwrap_base) {
			pr_err("%s() can't find iomem for pwrap\n",
				__func__);
			return -1;
		}
	} else {
	...
	if (node) {
		...
		if (!scp_base) {
			pr_err("%s() can't find iomem for scp\n",
				__func__);
			return -1;
		}
	} else {
		pr_err("%s can't find compatible node for scp\n",
			__func__);
		return -1;
	}
#if RC_GPIO_DBG_ENABLE
	...
	if (node) {
		...
		if (!gpio_base) {
			pr_err("%s() can't find iomem for gpio\n",
				__func__);
			return -1;
		}
	} else {
		pr_err("%s can't find compatible node for gpio\n",
			__func__);
		return -1;
	}
#endif
