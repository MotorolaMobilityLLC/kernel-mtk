...
/* spec is 220, but more time means more safety */
#define VPROC_ENABLE_TIME_US 240
/* spec is 110, but more time means more safety */
#define VSRAM_GPU_ENABLE_TIME_US 120
...
static struct mt_gpufreq_table_info mt_gpufreq_opp_tbl_e1_1[] = {
	GPUOP(GPU_DVFS_FREQ0_6763T, GPU_DVFS_VOLT0_6763T,
		GPU_DVFS_VSRAM0_6763T, 0),
	GPUOP(GPU_DVFS_FREQ1_6763T, GPU_DVFS_VOLT1_6763T,
		GPU_DVFS_VSRAM1_6763T, 1),
#ifdef SET_EXT_FREQ_LEVEL
	GPUOP(GPU_DVFS_FREQ2_6763T, GPU_DVFS_VOLT1_6763T,
		GPU_DVFS_VSRAM1_6763T, 2),
#endif
};

static struct mt_gpufreq_table_info mt_gpufreq_opp_tbl_e1_2[] = {
	GPUOP(GPU_DVFS_FREQ0_6763, GPU_DVFS_VOLT0_6763,
		GPU_DVFS_VSRAM0_6763, 0),
	GPUOP(GPU_DVFS_FREQ1_6763, GPU_DVFS_VOLT1_6763,
		GPU_DVFS_VSRAM1_6763, 1),
#ifdef SET_EXT_FREQ_LEVEL
	GPUOP(GPU_DVFS_FREQ2_6763, GPU_DVFS_VOLT1_6763,
		GPU_DVFS_VSRAM1_6763, 2),
#endif
};
...
/* In default settiing, freq_table[0] is max frequency,
 * freq_table[num-1] is min frequency
 */
...
static void mt_gpufreq_volt_switch_vcore(unsigned int volt_old,
				unsigned int volt_new);
...
static int _mt_gpufreq_set_lpm_status(unsigned int unit,
				unsigned int enable_lpm);
...
static unsigned int mt_gpufreq_pbm_limited_index;
#ifdef MTK_SSPM
 ...
 * value e.g: VCO range is 1.5GHz - 3.8GHz
 ...
 *****************************************************************************/
static enum post_div_order_enum _get_post_div_order(unsigned int freq,
				unsigned int efuse)
{
...
int mt_gpufreq_ap2sspm(unsigned int eCMD, unsigned int arg0, unsigned int arg1)
{
	...
	/*
	 * sspm_ipi_send_sync( $IPI_ID, $IPI_OPT, $sending_message,
	 * $IPI_ID_size, $pointer_to_save_md32return);
	 * $IPI_ID_size == 0 -> use registed default size according to $IPI_ID
	 */
#if 0
	apDebug = sspm_ipi_send_sync(IPI_ID_GPU_DVFS, IPI_OPT_POLLING, &gdvfs_d,
				0, &md32Ret, 1);
	/* apDebug = sspm_ipi_send_sync(IPI_ID_GPU_DVFS, IPI_OPT_WAIT, &gdvfs_d,
	 *			0, &md32Ret, 1);
	 */
	...
	if (apDebug < 0 || md32Ret < 0) {
		dump_stack();
		gpufreq_err(
			"%s: Error when calling cmd =(%d) arg0 = (%d) arg1 = (%d)\n",
			__func__, eCMD, arg0, arg1);
...
#endif
	gpufreq_dbg("[MTK_GPUFREQ] Into %s", __func__);
...
unsigned int mt_gpufreq_voltage_enable_set(unsigned int enable)
{
	 ...
	 */
	ret = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_STATUS_OP, SET_VOLT_SWITCH,
		enable);
	...
	gpufreq_dbg(
		"[MTK_GPUFREQ][+]@%s: enable = %d, mt_gpufreq_volt_enable_state = %d\n",
	...
	if (ext_buck_exist) {
		...
		if (enable == BUCK_ON) {
			...
			do {
				...
				if (regulator_is_enabled(
					mt_gpufreq_pmic->reg_vproc) == 0 &&
					regulator_is_enabled(
					mt_gpufreq_pmic->reg_vsram) == 0) {
					enable = BUCK_OFF;
				}
		...
		g_latest_freq_at_lowest =
			(g_cur_gpu_freq == GPUFREQ_LAST_FREQ_LEVEL_6763T) ||
		...
		if (enable == BUCK_ON) {
			if (!g_latest_freq_at_lowest) {
				if (ddr_type == TYPE_LPDDR3)
					ret = vcorefs_request_dvfs_opp(KIR_GPU,
						OPP_1);
				else
					ret = vcorefs_request_dvfs_opp(KIR_GPU,
						OPP_0);
				...
				unsigned int enable_lpm)
{
	...
	case VPROC:
		if (enable_lpm) /* Set to LPM */
			ret = pmic_config_interface(PMIC_RG_BUCK_VPROC_LP_ADDR,
			1, PMIC_RG_BUCK_VPROC_LP_MASK,
			PMIC_RG_BUCK_VPROC_LP_SHIFT);
		else /* Back to normal */
			ret = pmic_config_interface(PMIC_RG_BUCK_VPROC_LP_ADDR,
			0, PMIC_RG_BUCK_VPROC_LP_MASK,
			PMIC_RG_BUCK_VPROC_LP_SHIFT);
		if (ret != 0)
			gpufreq_err("@%s: Set VPROC LPM status %d failed! (%d)",
				__func__, enable_lpm, ret);
		break;
	case VSRAM:
		if (enable_lpm) /* Set to LPM */
			ret = pmic_config_interface(
				PMIC_RG_LDO_VSRAM_GPU_LP_ADDR, 1,
				PMIC_RG_LDO_VSRAM_GPU_LP_MASK,
				PMIC_RG_LDO_VSRAM_GPU_LP_SHIFT);
		else /* Back to normal */
			ret = pmic_config_interface(
				PMIC_RG_LDO_VSRAM_GPU_LP_ADDR, 0,
				PMIC_RG_LDO_VSRAM_GPU_LP_MASK,
				PMIC_RG_LDO_VSRAM_GPU_LP_SHIFT);
		if (ret != 0)
			gpufreq_err("@%s: Set VSRAM LPM status %d failed! (%d)",
				__func__, enable_lpm, ret);
		break;
	default:
...
unsigned int mt_gpufreq_voltage_lpm_set(unsigned int enable_lpm)
{
	...
	gpufreq_dbg(
		"[MTK_GPUFREQ][+]@%s: enable_lpm = %d, mt_gpufreq_volt_enable_state = %d\n",
...
void mt_gpufreq_enable_by_ptpod(void)
{
	...
	gpufreq_dbg("[MTK_GPUFREQ] Power off by %s\n", __func__);
	mt_gpufreq_voltage_enable_set(0);
#endif

	mt_gpufreq_ptpod_disable = false;
	gpufreq_info("mt_gpufreq enabled by %s\n", __func__);
}
...
void mt_gpufreq_disable_by_ptpod(void)
{
	...
	gpufreq_info("mt_gpufreq disabled by %s\n", __func__);
...
#ifdef VOLT_SET_READY
	gpufreq_dbg("[MTK_GPUFREQ] Power on by %s\n", __func__);
...
void mt_gpufreq_restore_default_volt(void)
{
	...
	if (ext_buck_exist) /* Use VPROC for power Control */
		...
		mt_gpufreq_volt_switch_vcore(g_cur_gpu_volt,
			mt_gpufreqs[g_cur_gpu_OPPidx].gpufreq_volt);
	...
	for (i = 0; i < mt_gpufreqs_num; i++) {
		mt_gpufreqs[i].gpufreq_volt =
			mt_gpufreqs_power[i].gpufreq_volt =
				mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, i,
					QUERY_VOLT);
...
static void mt_setup_gpufreqs_power_table(int num)
{
	...
	mt_gpufreqs_power = kzalloc((num) * sizeof(
	...
	for (i = 0; i < num ; i++) {
		...
		mt_gpufreqs_power[i].gpufreq_power = mt_gpufreq_ap2sspm(
			IPI_GPU_DVFS_OPPIDX_INFO, i, QUERY_POWER);
	}
...
static unsigned int mt_gpufreq_dds_calc(unsigned int freq_khz,
	enum post_div_order_enum post_div_order)
{
	unsigned int dds = 0;
	...
	if ((freq_khz >= GPU_DVFS_FREQ15) && (freq_khz <= GPU_DVFS_FREQ0)) {
		dds = (((freq_khz / TO_MHz_HEAD * (1 << post_div_order)) <<
...
static void mt_gpufreq_clock_switch(unsigned int freq_new)
{
	...
	if (mt_gpufreq_dvfs_table_cid == 0)
		mt_gpufreq_clock_switch_transient(freq_new,
		_get_post_div_order(freq_new, mt_gpufreq_dvfs_table_cid));
	else
...
static unsigned int _calculate_vproc_sfchg_rate(bool isRising)
{
	...
	gpufreq_dbg(
		"@%s: isRising(%d), sfchg_rate_vproc = %d, sfchg_rate_reg = %x\n",
...
static unsigned int _calculate_vsram_sfchg_rate(bool isRising)
{
	...
	gpufreq_dbg(
		"@%s: isRising(%d), sfchg_rate_vsram = %d, sfchg_rate_reg = %x\n",
				...
				unsigned int volt_new)
{
	...
	if (volt_new > volt_old) {
		...
		regulator_set_voltage(mt_gpufreq_pmic->reg_vsram,
				target_vsram*10,
				(PMIC_MAX_VSRAM*10) + 125);
		max_diff = target_vsram - target_old_vsram;
		delay_unit_us = (max_diff / DELAY_FACTOR) + 1;
		gpufreq_dbg(
			"1. udelay us(%d): delay_unit_us(%d) x sfchg_rate_vsram(%d)\n",
			delay_unit_us * sfchg_rate_vsram,
			delay_unit_us,
			sfchg_rate_vsram);
		...
		regulator_set_voltage(mt_gpufreq_pmic->reg_vproc,
				volt_new*10,
				(PMIC_MAX_VPROC*10) + 125);
		max_diff = volt_new - volt_old;
		delay_unit_us = (max_diff / DELAY_FACTOR) + 1;
		gpufreq_dbg(
			"2. udelay us(%d): delay_unit_us(%d) x sfchg_rate_vproc(%d)\n",
			delay_unit_us * sfchg_rate_vproc,
			delay_unit_us,
			sfchg_rate_vproc);
		...
		delay_unit_us = (max_diff / DELAY_FACTOR) + 1;
		gpufreq_dbg(
			"3. udelay us(%d): delay_unit_us(%d) x sfchg_rate_vproc(%d)\n",
			delay_unit_us * sfchg_rate_vproc,
			delay_unit_us,
			sfchg_rate_vproc);
		udelay(delay_unit_us * sfchg_rate_vproc);
		...
		delay_unit_us = (max_diff / DELAY_FACTOR) + 1;
		gpufreq_dbg(
			"4. udelay us(%d): delay_unit_us(%d) x sfchg_rate_vsram(%d)\n",
			delay_unit_us * sfchg_rate_vsram,
			delay_unit_us,
			sfchg_rate_vsram);
		udelay(delay_unit_us * sfchg_rate_vsram);
...
static unsigned int _mt_gpufreq_get_cur_freq(void)
{
...
#ifdef MTK_SSPM
	freq_khz = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_STATUS_OP, QUERY_CUR_FREQ,
			0);
#else
	unsigned long dds;
	...
	freq_khz = (((dds * TO_MHz_TAIL + ROUNDING_VALUE)*GPUPLL_FIN)
			>> DDS_SHIFT) / (1 << post_div_order) *  TO_MHz_HEAD;
...
#ifdef MTK_SSPM
	gpu_volt = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_STATUS_OP, QUERY_CUR_VOLT,
			0);
#else
	if (ext_buck_exist) {
		/* WARRNING: regulator_get_voltage prints uV */
		gpu_volt = regulator_get_voltage(mt_gpufreq_pmic->reg_vproc)
				/ 10;
		gpufreq_dbg(
			"gpu_dvfs_get_cur_volt:[PMIC] volt = %d\n", gpu_volt);
	} else {
		gpu_volt = regulator_get_voltage(mt_gpufreq_pmic->reg_vcore)
				/ 10;
		gpufreq_dbg(
			"gpu_dvfs_get_cur_volt:[PMIC] volt = %d\n", gpu_volt);
	}
	...
	if (enable) {
		for (i = 0; i < mt_gpufreqs_num; i++) {
			if (mt_gpufreqs_power[i].gpufreq_khz == cur_freq) {
				/* record idx since current voltage may not
				 * in DVFS table
				 */
				tmp_idx = i;
		...
		if (!found) {
			...
			if (tmp_idx != -1 && tmp_idx < mt_gpufreqs_num) {
				/* use freq to found corresponding power
				 * budget
				 */
				power = mt_gpufreqs_power[tmp_idx]
						.gpufreq_power;
				kicker_pbm_by_gpu(true, power, cur_volt / 100);
				gpufreq_dbg(
					"@%s: request GPU power = %d, cur_volt = %d, cur_freq = %d\n",
...
unsigned int mt_gpufreq_target(unsigned int idx)
{
	...
	if (mt_gpufreq_pause == false)
		return mt_gpufreq_ap2sspm(IPI_GPU_DVFS_TARGET_FREQ_IDX, idx,
					SUB_CMD_MOD_FREQ);
	...
	if (mt_gpufreq_keep_opp_frequency_state == true) {
		target_freq = mt_gpufreqs[mt_gpufreq_keep_opp_index]
				.gpufreq_khz;
		target_volt = mt_gpufreqs[mt_gpufreq_keep_opp_index]
				.gpufreq_volt;
		target_idx = mt_gpufreqs[mt_gpufreq_keep_opp_index]
				.gpufreq_idx;
	...
	if (mt_gpufreq_opp_max_frequency_state == true) {
		if (target_freq > mt_gpufreq_opp_max_frequency) {
			target_freq = mt_gpufreqs[mt_gpufreq_opp_max_index]
					.gpufreq_khz;
			target_volt = mt_gpufreqs[mt_gpufreq_opp_max_index]
					.gpufreq_volt;
			target_idx = mt_gpufreqs[mt_gpufreq_opp_max_index]
					.gpufreq_idx;
	...
	if (mt_gpufreq_pbm_limited_index != 0) {
		if (target_freq > mt_gpufreqs[mt_gpufreq_pbm_limited_index]
								.gpufreq_khz) {
			/*********************************************
	...
	if (g_limited_max_id != 0) {
		if (target_freq > mt_gpufreqs[g_limited_max_id].gpufreq_khz) {
			 ...
			 * target_freq > limited_freq, need to adjust
			 ********************************************/
			target_freq = mt_gpufreqs[g_limited_max_id]
					.gpufreq_khz;
			target_volt = mt_gpufreqs[g_limited_max_id]
					.gpufreq_volt;
			target_idx = mt_gpufreqs[g_limited_max_id]
					.gpufreq_idx;
	...
	if (mt_gpufreq_ptpod_disable == true) {
#if 1
		target_freq = mt_gpufreqs[mt_gpufreq_ptpod_disable_idx]
				.gpufreq_khz;
		target_volt = GPU_DVFS_PTPOD_DISABLE_VOLT;
		target_idx = mt_gpufreqs[mt_gpufreq_ptpod_disable_idx]
				.gpufreq_idx;
	 ...
	 *******************************/
	mt_gpufreq_set(g_cur_gpu_freq, target_freq, g_cur_gpu_volt,
		target_volt);
...
/* < 485 MHz */
#define MT_GPUFREQ_OC_LIMIT_FREQ_1		GPU_DVFS_FREQ14

#ifndef SET_EXT_FREQ_LEVEL
/* 530 MHz */
#define MT_GPUFREQ_OC_LIMIT_FREQ_1_6763T	GPU_DVFS_FREQ1_6763T
/* 530 MHz */
#define MT_GPUFREQ_OC_LIMIT_FREQ_1_6763		GPU_DVFS_FREQ1_6763
#else
/* 390 MHz */
#define MT_GPUFREQ_OC_LIMIT_FREQ_1_6763T	GPU_DVFS_FREQ2_6763T
/* 390 MHz */
#define MT_GPUFREQ_OC_LIMIT_FREQ_1_6763		GPU_DVFS_FREQ2_6763
#endif
...
#ifndef SET_EXT_FREQ_LEVEL
/* 530 MHz */
#define MT_GPUFREQ_LOW_BATT_VOLT_LIMIT_FREQ_2_6763T	GPU_DVFS_FREQ1_6763T
/* 530 MHz */
#define MT_GPUFREQ_LOW_BATT_VOLT_LIMIT_FREQ_2_6763	GPU_DVFS_FREQ1_6763
#else
/* 390 MHz */
#define MT_GPUFREQ_LOW_BATT_VOLT_LIMIT_FREQ_2_6763T	GPU_DVFS_FREQ2_6763T
/* 390 MHz */
#define MT_GPUFREQ_LOW_BATT_VOLT_LIMIT_FREQ_2_6763	GPU_DVFS_FREQ2_6763
#endif
...
static unsigned int mt_gpufreq_prev_thermal_limited_freq;
/* limit frequency index array */
static unsigned int mt_gpufreq_power_limited_index_array[NR_IDX_POWER_LIMITED]
						= { 0 };
...
unsigned int mt_gpufreq_get_thermal_limit_freq(void)
{
	...
	gpufreq_khz = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO,
					g_limited_max_id, QUERY_FREQ);
...
#ifdef MTK_SSPM
	g_cur_gpu_OPPidx
		= g_cur_gpu_idx = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_STATUS_OP,
							QUERY_CUR_OPPIDX, 0);
#endif
 ...
 *		__func__, gdvfs_rev_buf[0], gdvfs_rev_buf[1], gdvfs_rev_buf[2]);
 */
...
static void mt_gpufreq_sspm_recv_daemon_init(void)
{
	...
	g_kt = kthread_create(k_gpu_dvfs_host_recv_thread, &gdvfs_act,
				"gpu_dvfs_host_recv");
...
{
#ifdef MTK_SSPM
	mt_gpufreq_ap2sspm(IPI_GPU_DVFS_STATUS_OP, SET_GPU_LOADING,
		gpu_loading);
#endif
}
...
static int mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	if ((get_devinfo_with_index(30) & 0xf0) == 0x10) {
		...
		device_id = C_MT6763;
	} else if ((get_devinfo_with_index(30) & 0xf0) == 0x0
			|| (get_devinfo_with_index(30) & 0xf0) == 0x20) {
	...
	if (ext_buck_exist) {
		mt_gpufreq_pmic = kzalloc(sizeof(struct mt_gpufreq_pmic_t),
					GFP_KERNEL);
	...
	gpufreq_dbg("[MTK_GPUFREQ] OPP Table Set Up Done (%d)",
	...
	if (ext_buck_exist) {
		/* rising rate */
		g_vproc_sfchg_rrate = _calculate_vproc_sfchg_rate(true);
		/* falling rate */
		g_vproc_sfchg_frate = _calculate_vproc_sfchg_rate(false);
		/* rising rate */
		g_vsram_sfchg_rrate = _calculate_vsram_sfchg_rate(true);
		/* falling rate */
		g_vsram_sfchg_frate = _calculate_vsram_sfchg_rate(false);
...
#ifdef MT_GPUFREQ_INPUT_BOOST
	mt_gpufreq_up_task =
		kthread_create(mt_gpufreq_input_boost_task, NULL,
...
static int mt_gpufreq_limited_oc_ignore_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "oc_limited_id = %d, g_limited_oc_ignore_state = %d\n",
		mt_gpufreq_ap2sspm(IPI_GPU_DVFS_STATUS_OP, QUERY_LMITED_FACTOR,
		IDX_OC_LIMITED), g_limited_oc_ignore_state);
#else
						...
						loff_t *data)
{
	...
	mt_gpufreq_power_limited_ignore[IDX_OC_LIMITED] =
		g_limited_oc_ignore_state;
	return count;
}
#endif
						...
						size_t count, loff_t *data)
{
	...
	mt_gpufreq_power_limited_ignore[IDX_LOW_BATT_VOLUME_LIMITED] =
		g_limited_low_batt_volume_ignore_state;
	return count;
}
#endif
						...
						size_t count, loff_t *data)
{
	...
	mt_gpufreq_power_limited_ignore[IDX_LOW_BATT_VOLT_LIMITED]
			= g_limited_low_batt_volt_ignore_state;
	return count;
}
#endif
...
#ifdef MTK_SSPM
	seq_printf(m,
			"thermal_limited_id = %d, g_limited_thermal_ignore_state = %d\n",
			mt_gpufreq_ap2sspm(
				IPI_GPU_DVFS_STATUS_OP,
				QUERY_LMITED_FACTOR,
				IDX_THERMAL_LIMITED),
...
#else
	seq_printf(m,
			"g_limited_max_id = %d, g_limited_thermal_ignore_state = %d\n",
						...
						size_t count, loff_t *data)
{
	...
	mt_gpufreq_power_limited_ignore[IDX_THERMAL_LIMITED]
		= g_limited_thermal_ignore_state;
	return count;
}
...
{
	seq_printf(m,
		"mt_gpufreq_pbm_limited_index = %d, g_limited_pbm_ignore_state = %d\n",
...
#ifdef MTK_SSPM
		g_limited_max_id = mt_gpufreq_ap2sspm(
					IPI_GPU_DVFS_STATUS_OP,
					SET_THROTTLE_POWER, power);
#else
			...
			const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (kstrtoint(desc, 0, &enabled) == 0) {
		if (enabled == 1) {
			len = mt_gpufreq_ap2sspm(
					IPI_GPU_DVFS_SET_FREQ,
					GPU_DVFS_MAX_FREQ,
					SUB_CMD_REL_FREQ);
		} else if (enabled == 0) {
			mt_gpufreq_ap2sspm(
				IPI_GPU_DVFS_STATUS_OP, SET_VOLT_SWITCH, 1);
			/*
			 *	release locked-freq before lock down
			 */
			mt_gpufreq_ap2sspm(
				IPI_GPU_DVFS_SET_FREQ, GPU_DVFS_MAX_FREQ,
				SUB_CMD_REL_FREQ);
			len = mt_gpufreq_ap2sspm(
				IPI_GPU_DVFS_SET_FREQ, GPU_DVFS_MAX_FREQ,
				SUB_CMD_FIX_FREQ);
...
static int mt_gpufreq_opp_dump_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < mt_gpufreqs_num; i++) {
		...
		gpufreq_khz = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, i,
							QUERY_FREQ);
		gpufreq_volt = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, i,
							QUERY_VOLT);
...
static int mt_gpufreq_power_dump_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < mt_gpufreqs_num; i++) {
		...
		gpufreq_volt = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, i,
							QUERY_VOLT);
		gpufreq_power = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, i,
							QUERY_POWER);
					...
					size_t count, loff_t *data)
{
	...
	if (kstrtoint(desc, 0, &fixed_freq) == 0) {
		if (fixed_freq == 0) {
			...
			mt_gpufreq_ap2sspm(IPI_GPU_DVFS_SET_FREQ,
				GPU_DVFS_MAX_FREQ,
				SUB_CMD_REL_FREQ);
				mt_gpufreq_pause = 0;
		} else {
			 ...
			 *	release locked-freq before lock down
			 */
			mt_gpufreq_ap2sspm(IPI_GPU_DVFS_SET_FREQ, fixed_freq,
				SUB_CMD_REL_FREQ);
			if (mt_gpufreq_ap2sspm(IPI_GPU_DVFS_SET_FREQ,
					fixed_freq,
					SUB_CMD_FIX_FREQ) == 0) {
				mt_gpufreq_pause = 1;
				mt_gpufreq_keep_opp_index =
					mt_gpufreq_get_idx_by_target_freq(
						fixed_freq);
			}
		}
	} else
...
static int mt_gpufreq_var_dump_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "g_cur_gpu_freq = %d, g_cur_gpu_volt = %d\n",
		mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, g_cur_gpu_idx,
			QUERY_FREQ),
		mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, g_cur_gpu_idx,
			QUERY_VOLT));
	...
	g_limited_max_id = 0;
	for (i = 0; i < NR_IDX_POWER_LIMITED; i++) {
		limited_id = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_STATUS_OP,
						QUERY_LMITED_FACTOR, i);
		...
		seq_printf(m, "mt_gpufreq_power_limited_index_array[%d] = %d %s\n",
				i,
				mt_gpufreq_power_limited_index_array[i],
				mt_gpufreq_power_limited_ignore[i] ?
					"(ignore)" : "");
	}
...
static void _mt_gpufreq_fixed_freq(int fixed_freq)
{
	...
	if ((fixed_freq >= GPUFREQ_LAST_FREQ_LEVEL)
		...
		gpufreq_dbg(
			"@ %s, mt_gpufreq_clock_switch1 fix frq = %d, fix volt = %d, volt = %d\n",
...
static int __init mt_gpufreq_init(void)
{
	...
	mt_gpufreqs_num = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_INIT,
						mt_gpufreq_dvfs_table_type, 0);
	if (mt_gpufreqs_num <= 0) {
	...
	for (i = 0; i < mt_gpufreqs_num; i++)	{
		freqs[i].gpufreq_khz = mt_gpufreq_ap2sspm(
						IPI_GPU_DVFS_OPPIDX_INFO, i,
						QUERY_FREQ);
		freqs[i].gpufreq_volt = mt_gpufreq_ap2sspm(
						IPI_GPU_DVFS_OPPIDX_INFO, i,
						QUERY_VOLT);
