...
static short get_delta_pwr_shared(unsigned int core_c1, unsigned int core_c2, unsigned int opp, int prefer)
{
	...
	if (opp == COBRA_OPP_NUM - 1) {
		if (prefer == CLUSTER_ID_SHARED_C1) {
			prev_pwr = (core_c1) ?
				((core_c1 > 1) ? (cobra_tbl->basic_pwr_tbl[idx_c1+core_c1-2][cur_opp].power_idx +
				((core_c2) ? cobra_tbl->basic_pwr_tbl[idx_c2+core_c2-1][cur_opp].power_idx : 0))
				: ((core_c2) ? cobra_tbl->basic_pwr_tbl[idx_c2+core_c2-1][cur_opp].power_idx : 0))
				: ((core_c2 > 1) ? cobra_tbl->basic_pwr_tbl[idx_c2+core_c2-2][cur_opp].power_idx : 0);
		} else {
			prev_pwr = (core_c2) ?
...
static short get_delta_perf_shared(unsigned int core_c1, unsigned int core_c2, unsigned int opp, int prefer)
{
#if PPM_COBRA_RUNTIME_CALC_DELTA
	unsigned int idx_c1, idx_c2;
	unsigned int cur_opp, prev_opp;
	...
	if (opp == COBRA_OPP_NUM - 1) {
		if (prefer == CLUSTER_ID_SHARED_C1) {
			prev_perf = (core_c1) ?
				((core_c1 > 1) ? (cobra_tbl->basic_pwr_tbl[idx_c1+core_c1-2][cur_opp].perf_idx +
				((core_c2) ? cobra_tbl->basic_pwr_tbl[idx_c2+core_c2-1][cur_opp].perf_idx : 0))
				: ((core_c2) ? cobra_tbl->basic_pwr_tbl[idx_c2+core_c2-1][cur_opp].perf_idx : 0))
				: ((core_c2 > 1) ? cobra_tbl->basic_pwr_tbl[idx_c2+core_c2-2][cur_opp].perf_idx : 0);
		} else {
			prev_perf = (core_c2) ?
...
static short get_delta_eff_shared(unsigned int core_c1, unsigned int core_c2, unsigned int opp, int prefer)
{
#if PPM_COBRA_RUNTIME_CALC_DELTA
	...
	delta_pwr = get_delta_pwr_shared(core_c1, core_c2, opp, prefer);
	delta_perf = get_delta_perf_shared(core_c1, core_c2, opp, prefer);
...
void ppm_cobra_update_limit(void *user_req)
{
	...
	ppm_dbg(COBRA_ALGO, "[PREV]Core_Limit=%d%d%d, policy_limit=%d%d%d\n",
...
#if PPM_COBRA_USE_CORE_LIMIT
	ppm_dbg(COBRA_ALGO, "[COBRA] max throttle core num = %d,%d,%d\n",
		max_throttle_core_num[CLUSTER_ID_SHARED_C1],
		max_throttle_core_num[CLUSTER_ID_SHARED_C2],
		max_throttle_core_num[CLUSTER_ID_SINGLE]);
#endif

	/* Which shared-cluster is active */
	share_cl_sel = (ACT_CORE(SHARED_C2) > 0) ? CLUSTER_ID_SHARED_C2
	...
	if (delta_power >= 0) {
		while (1) {
			...
			/* TODO: check this! */
			if (opp[share_cl_sel] == 0 && ACT_CORE(SINGLE) == 0) {
			...
			/* Shared-cluster */
			if (share_cl_sel != CLUSTER_ID_SINGLE && opp[share_cl_sel] > 0
				...
				target_delta_pwr = get_delta_pwr_shared(ACT_CORE(SHARED_C1), ACT_CORE(SHARED_C2),
					opp[share_cl_sel]-1, share_cl_sel);
...
#if PPM_COBRA_USE_CORE_LIMIT
			/* no enough budget */
			if (opp[share_cl_sel] != 0)
				goto end;

			for_each_ppm_clusters(i) {
				int max_core = get_cluster_max_cpu_core(i);
				...
				while (core_limit_tmp[i] < max_core) {
					target_delta_pwr = (i == CLUSTER_ID_SINGLE)
						...
						? get_delta_pwr_shared(CORE_LIMIT(SHARED_C1)+1,
							ACT_CORE(SHARED_C2), COBRA_OPP_NUM-1, share_cl_sel)
						: get_delta_pwr_shared(ACT_CORE(SHARED_C1),
							CORE_LIMIT(SHARED_C2)+1, COBRA_OPP_NUM-1, share_cl_sel));
...
prepare_next_round:
		...
		while (delta_power < 0) {
			...
			ppm_dbg(COBRA_ALGO, "exclusive_core_flag = %d,%d,%d(select=%d)\n",
				ex_core_flag[CLUSTER_ID_SHARED_C1], ex_core_flag[CLUSTER_ID_SHARED_C2],
				ex_core_flag[CLUSTER_ID_SINGLE], share_cl_sel);

			/* Single-cluster */
			if (ACT_CORE(SINGLE) > 0 && opp[CLUSTER_ID_SINGLE] < PPM_COBRA_MAX_FREQ_IDX
				&& (!ex_core_flag[CLUSTER_ID_SINGLE])) {
			...
			/* Shared-cluster */
			if (share_cl_sel != CLUSTER_ID_SINGLE && opp[share_cl_sel] < PPM_COBRA_MAX_FREQ_IDX
				&& !ex_core_flag[share_cl_sel]) {
				target_delta_eff = get_delta_eff_shared(ACT_CORE(SHARED_C1), ACT_CORE(SHARED_C2),
					opp[share_cl_sel], share_cl_sel);
				if (MinEff > target_delta_eff) {
					MinEff = target_delta_eff;
					ChoosenCl = CLUSTER_ID_SHARED_C1;
					ChoosenPwr = get_delta_pwr_shared(ACT_CORE(SHARED_C1), ACT_CORE(SHARED_C2),
						opp[share_cl_sel], share_cl_sel);
				}
			}
			...
			/* if choose shared-cluster, change opp of active cluster only */
			if (ChoosenCl == CLUSTER_ID_SHARED_C1) {
				if (ACT_CORE(SHARED_C2) > 0 && opp[CLUSTER_ID_SHARED_C2] < PPM_COBRA_MAX_FREQ_IDX)
					opp[CLUSTER_ID_SHARED_C2] += 1;
				if (ACT_CORE(SHARED_C1) > 0 && opp[CLUSTER_ID_SHARED_C1] < PPM_COBRA_MAX_FREQ_IDX)
					opp[CLUSTER_ID_SHARED_C1] += 1;
			} else
				opp[ChoosenCl] += 1;
			...
			} else if (opp[share_cl_sel] == PPM_COBRA_MAX_FREQ_IDX) {
			...
			for (i = NR_PPM_CLUSTERS-1; i >= 0; i--) {
				if (i == CLUSTER_ID_SINGLE
					...
					if (!ignore_ex_core && max_throttle_core_num[CLUSTER_ID_SINGLE] <= 0) {
						ex_core_flag[i] = 1;
						opp[CLUSTER_ID_SINGLE]--;
						goto check_exclusive_core_flag;
					}
					if (ACT_CORE(SINGLE) > 0) {
						req->limit[CLUSTER_ID_SINGLE].max_cpu_core = --ACT_CORE(SINGLE);
						max_throttle_core_num[CLUSTER_ID_SINGLE]--;
						opp[CLUSTER_ID_SINGLE] = PPM_COBRA_MAX_FREQ_IDX - 1;
						ex_core_flag[i] = (!ignore_ex_core
							&& max_throttle_core_num[CLUSTER_ID_SINGLE] <= 0)
							? 1 : ex_core_flag[i];
					}
					break;
				} else if (i == share_cl_sel && opp[share_cl_sel] == PPM_COBRA_MAX_FREQ_IDX) {
					if (!ignore_ex_core && max_throttle_core_num[i] <= 0) {
						ex_core_flag[i] = 1;
						opp[CLUSTER_ID_SHARED_C2] = (ACT_CORE(SHARED_C2) > 0)
							? opp[CLUSTER_ID_SHARED_C2] - 1 : opp[CLUSTER_ID_SHARED_C2];
						opp[CLUSTER_ID_SHARED_C1] = (ACT_CORE(SHARED_C1) > 0)
							? opp[CLUSTER_ID_SHARED_C1] - 1 : opp[CLUSTER_ID_SHARED_C1];
						/* check C1 next round if C2 has no more core to throttle */
						share_cl_sel = (i == CLUSTER_ID_SHARED_C2)
							? CLUSTER_ID_SHARED_C1 : share_cl_sel;
						goto check_exclusive_core_flag;
					}
					if ((ignore_ex_core || !ex_core_flag[CLUSTER_ID_SHARED_C2])
						&& (ACT_CORE(SHARED_C2) > 1
						|| (ACT_CORE(SHARED_C1) > 0 && ACT_CORE(SHARED_C2) > 0))) {
						req->limit[CLUSTER_ID_SHARED_C2].max_cpu_core = --ACT_CORE(SHARED_C2);
						max_throttle_core_num[CLUSTER_ID_SHARED_C2]--;
						ex_core_flag[CLUSTER_ID_SHARED_C2] = (!ignore_ex_core
							&& max_throttle_core_num[CLUSTER_ID_SHARED_C2] <= 0)
							? 1 : ex_core_flag[CLUSTER_ID_SHARED_C2];
						share_cl_sel = (!ignore_ex_core
							&& max_throttle_core_num[CLUSTER_ID_SHARED_C2] <= 0)
							? CLUSTER_ID_SHARED_C1 : share_cl_sel;
					} else if ((ignore_ex_core || !ex_core_flag[CLUSTER_ID_SHARED_C1])
						&& ACT_CORE(SHARED_C1) > 1) {
						req->limit[CLUSTER_ID_SHARED_C1].max_cpu_core = --ACT_CORE(SHARED_C1);
						max_throttle_core_num[CLUSTER_ID_SHARED_C1]--;
						ex_core_flag[CLUSTER_ID_SHARED_C1] = (!ignore_ex_core
							&& max_throttle_core_num[CLUSTER_ID_SHARED_C1] <= 0)
							? 1 : ex_core_flag[CLUSTER_ID_SHARED_C1];
					}
					...
					if (ACT_CORE(SHARED_C2) > 0)
						...
						share_cl_sel = CLUSTER_ID_SHARED_C1;
					if (ACT_CORE(SHARED_C1) > 0)
...
check_exclusive_core_flag:
			...
			if (ex_core_flag[CLUSTER_ID_SINGLE]
				&& ex_core_flag[CLUSTER_ID_SHARED_C2]
				&& ex_core_flag[CLUSTER_ID_SHARED_C1]) {
				ignore_ex_core = 1;
				ex_core_flag[CLUSTER_ID_SINGLE] = 0;
				ex_core_flag[CLUSTER_ID_SHARED_C2] = 0;
				ex_core_flag[CLUSTER_ID_SHARED_C1] = 0;
				if (ACT_CORE(SHARED_C2) > 0)
					share_cl_sel = CLUSTER_ID_SHARED_C2; /* re-scan from C2 */
			}
	...
	for_each_ppm_clusters(i) {
		if (i == CLUSTER_ID_SINGLE) {
			...
			req->limit[i].max_cpufreq_idx = opp[share_cl_sel];
	}
			...
			for (j = 0; j <= get_cluster_max_cpu_core(CLUSTER_ID_SHARED_C1); j++) {
				for (k = 0; k < COBRA_OPP_NUM; k++) {
					seq_printf(m, "[%d][%d][%d] = (%d, %d, %d)\n", i, j, k,
						get_delta_pwr_shared(j, i, k, CLUSTER_ID_SHARED_C2),
						get_delta_perf_shared(j, i, k, CLUSTER_ID_SHARED_C2),
						get_delta_eff_shared(j, i, k, CLUSTER_ID_SHARED_C2));
				}
			}
		}
	}
