...
prepare_next_round:
		...
		while (delta_power < 0) {
			int ChoosenCl = -1;
			int MinEff = 10000;
		/* should be bigger than max value of efficiency_* array */
			int ChoosenPwr = 0;
		...
		for (i = 0; i < TOTAL_CORE_NUM; i++) {
			for (j = 0; j < DVFS_OPP_NUM; j++) {
				...
				dyn_c = upower_get_power(
					i/4 + NR_PPM_CLUSTERS, j, UPOWER_DYN);
				lkg_c = upower_get_power(
					i/4 + NR_PPM_CLUSTERS, j, UPOWER_LKG);
				cap = upower_get_power(
					i/4, j, UPOWER_CPU_STATES);
				...
				ppm_ver("[%d][%d] = (%d, %d)\n", i, j,
					cobra_tbl->
						basic_pwr_tbl[i][j].power_idx,
					cobra_tbl->
						basic_pwr_tbl[i][j].perf_idx);
			}
		}
	}
#else
		...
		for (j = 0;
			...
			for_each_ppm_clusters(k) {
				...
				power += (k == PPM_CLUSTER_B)
					? cobra_tbl->
						basic_pwr_tbl[idx][j].power_idx
					: cobra_tbl->
						basic_pwr_tbl[idx][i].power_idx;
			}
