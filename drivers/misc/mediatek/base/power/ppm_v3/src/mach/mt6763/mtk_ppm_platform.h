...
#define get_cluster_lcmoff_min_freq(id)		\
		LCMOFF_MIN_FREQ	/* the same for each cluster */
#define get_cluster_ptpod_fix_freq_idx(id)	\
		PTPOD_FREQ_IDX	/* the same for each cluster */
...
struct ppm_cobra_data {
	...
	struct ppm_cobra_delta_data
		delta_tbl_LxLL[CORE_NUM_L+1][CORE_NUM_LL+1][COBRA_OPP_NUM];
#endif
};
