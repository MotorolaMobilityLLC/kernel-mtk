...
#endif
#if defined(CONFIG_MICROTRUST_TEE_SUPPORT) && defined(CONFIG_ARCH_MT6580)
...
{
#if defined(CONFIG_ARCH_MT6580) && (defined(CONFIG_TRUSTONIC_TEE_SUPPORT) || defined(CONFIG_TRUSTY) || defined(CONFIG_MICROTRUST_TEE_SUPPORT))
	int cpuid, clusterid;
...
static int mt_cpu_dormant_abort(unsigned long index)
{
	...
	if (cpuid == 0)
		mt_secure_call(MC_FC_SLEEP_CANCELLED, 0, 0, 0);
#elif defined(CONFIG_MICROTRUST_TEE_SUPPORT) && defined(CONFIG_ARCH_MT6580)
	if (cpuid == 0)
		teei_secure_call(TEEI_FC_CPU_DORMANT_CANCEL, 0, 0, 0);
...
#elif defined(CONFIG_MICROTRUST_TEE_SUPPORT) && defined(CONFIG_ARCH_MT6580)
	teei_secure_call(TEEI_FC_CPU_DORMANT, virt_to_phys(cpu_resume), cpuid, 0);
#elif defined(CONFIG_TRUSTY) && defined(CONFIG_ARCH_MT6580)
...
#elif defined(CONFIG_MICROTRUST_TEE_SUPPORT) && defined(CONFIG_ARCH_MT6580)
		teei_secure_call(TEEI_FC_CPU_ON, virt_to_phys(cpu_wake_up_errata_802022), 1, 1);
		if (num_possible_cpus() == 4) {
			teei_secure_call(TEEI_FC_CPU_ON, virt_to_phys(cpu_wake_up_errata_802022), 2, 1);
			teei_secure_call(TEEI_FC_CPU_ON, virt_to_phys(cpu_wake_up_errata_802022), 3, 1);
		}
#elif defined(CONFIG_TRUSTY) && defined(CONFIG_ARCH_MT6580)
