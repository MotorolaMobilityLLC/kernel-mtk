...
static void mt_gpufreq_mtcmos_control(enum mt_power_state power)
{
	...
	if (power == POWER_ON) {
		if (clk_prepare_enable(g_clk->mtcmos_mfg_async))
			gpufreq_pr_info("@%s: failed when enable mtcmos_mfg_async\n",
					__func__);

		if (clk_prepare_enable(g_clk->mtcmos_mfg))
			gpufreq_pr_info("@%s: failed when enable mtcmos_mfg\n",
					__func__);

		if (shader_present & MFG2_SHADER_STACK0)
			if (clk_prepare_enable(g_clk->mtcmos_mfg_core0))
				gpufreq_pr_info("@%s: failed when enable mtcmos_mfg_core0\n",
						__func__);

		if (shader_present & MFG3_SHADER_STACK2)
			if (clk_prepare_enable(g_clk->mtcmos_mfg_core1))
				gpufreq_pr_info("@%s: failed when enable mtcmos_mfg_core1\n",
						__func__);
	} else {
		if (shader_present & MFG3_SHADER_STACK2)
...
void mt_gpufreq_software_trigger_dfd(void)
{
	...
	val = readl(g_toprgu + 0x048) | (0x95 << 24) | 0x20000;
	writel(val, g_toprgu + 0x048);

	val = readl(g_toprgu + 0x044) | (0x95 << 24) | 0x11;
	writel(val, g_toprgu + 0x044);

	writel(0x0F111100, g_mfg_base + 0xA00);
...
static void __mt_gpufreq_toprgu_pwr_on(bool enable)
{
#if MT_GPUFREQ_DFD_ENABLE
	...
	while (retry) {
		//set wdt_mfg_pwr_on=0x1 0x10007068
		if (enable)
			val = readl(g_toprgu + 0x068) | (0x77 << 24) | 0x1;
		else
			val = (0x77 << 24) & 0x0;

		writel(val, g_toprgu + 0x068);
		udelay(80);
		rgu_pwr = readl(g_toprgu + 0x068);
...
static void __mt_gpufreq_config_dfd(bool enable)
{
	...
	if (enable) {
		...
		writel(0x000106d0, g_mfg_base + 0xA04);
		writel(0x0001829f, g_mfg_base + 0xA08);
		writel(0x0010001f, g_mfg_base + 0xA0C);
		...
		mtk_wdt_dfd_timeout(0x3E8); // 500 ms
			...
			enum mt_mtcmos_state mtcmos, enum mt_buck_state buck)
{
	...
	if (__mt_gpufreq_is_dfd_triggered()) {
		...
		unsigned int rgu_pwr = readl(g_toprgu + 0x060);
...
static int __mt_gpufreq_init_clk(struct platform_device *pdev)
{
	...
	// 0x1000C000
	g_apmixed_base =
			__mt_gpufreq_of_ioremap("mediatek,apmixed", 0);
	if (!g_apmixed_base) {
...
#endif
	//writel(0x77 << 24, g_toprgpu + 0x060);
	__mt_gpufreq_toprgu_pwr_on(false);
...
#endif
	__mt_gpufreq_toprgu_pwr_on(true);
}
