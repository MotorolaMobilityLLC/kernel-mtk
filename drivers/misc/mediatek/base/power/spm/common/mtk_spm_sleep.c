...
static unsigned int spm_output_wake_reason(struct wake_status *wakesta)
{
...
#endif
	spm_crit2("dormant = %d, s_ddr = %d, s_vcore = %d, ",
		  spm_dormant_sta, sleep_ddr_status, sleep_vcore_status);
	spm_crit2("ddr = %d, vcore = %d, sleep_count = %d\n",
		  ddr_status, vcore_status, spm_sleep_count);

	if (spm_ap_mdsrc_req_cnt != 0) {
		spm_crit2("warning: spm_ap_mdsrc_req_cnt = %d, ",
			spm_ap_mdsrc_req_cnt);
		spm_crit2("r7[ap_mdsrc_req] = 0x%x\n",
	...
	if (wakesta->r12 & WAKE_SRC_R12_MD2AP_PEER_EVENT_B)
		exec_ccci_kern_func_by_md_id(0, ID_GET_MD_WAKEUP_SRC, NULL,
			0, 0);
	if (wakesta->r12 & WAKE_SRC_R12_CCIF0_EVENT_B)
		exec_ccci_kern_func_by_md_id(0, ID_GET_MD_WAKEUP_SRC, NULL,
			0, 0);
	if (wakesta->r12 & WAKE_SRC_R12_CCIF1_EVENT_B)
		exec_ccci_kern_func_by_md_id(2, ID_GET_MD_WAKEUP_SRC, NULL,
			0, 0);
#endif
#endif
	return wr;
...
static void check_ap_mdsrc_ack(void)
{
	...
	while ((spm_read(AP_MDSRC_REQ) & AP_MDSMSRC_ACK_LSB) == 0) {
		if (i++ < 10) {
			...
			spm_crit2("WARNING: MD SLEEP = %d\n", md_sleep);
			spm_crit2("%s CAN NOT polling AP_MD1SRC_ACK\n",
				__func__);
			break;
		}
	}
