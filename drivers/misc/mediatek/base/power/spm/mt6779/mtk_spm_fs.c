...
static ssize_t show_pwr_ctrl
(int id, const struct pwr_ctrl *pwrctrl, char *buf)
{
	char *p = buf;
	...
	p += sprintf(p,
		"reg_spm_xowcn_req = 0x%zx\n",
		SMC_CALL(GET_PWR_CTRL_ARGS,
		id, PW_REG_SPM_XOWCN_REQ, 0));
	p += sprintf(p,
...
(int id, struct pwr_ctrl *pwrctrl, const char *buf, size_t count)
{
	...
	pwr_ctrl_str[PW_REG_SPM_XOWCN_REQ])) {
		pwrctrl->reg_spm_xowcn_req = val;
		SMC_CALL(PWR_CTRL_ARGS,
		id, PW_REG_SPM_XOWCN_REQ, val);
	} else if (!strcmp(cmd,
