...
#define sodi_notice(fmt, args...)  pr_notice(SODI3_TAG fmt, ##args)
#define sodi_info(fmt, args...)    pr_info(SODI3_TAG fmt, ##args)
#define sodi3_err(fmt, args...)    pr_info(SODI3_TAG fmt, ##args)
#define sodi3_warn(fmt, args...)   pr_info(SODI3_TAG fmt, ##args)
#define sodi3_notice(fmt, args...) pr_notice(SODI3_TAG fmt, ##args)
#define sodi3_info(fmt, args...)   pr_info(SODI3_TAG fmt, ##args)
...
#define so_warn(fg, fmt, args...)    \
		((fg&SODI_FLAG_3P0) ?\
		pr_info(SODI3_TAG fmt, ##args):pr_info(SODI_TAG fmt, ##args))
#define so_notice(fg, fmt, args...)  \
		((fg&SODI_FLAG_3P0) ?\
		pr_notice(SODI3_TAG fmt, ##args) :\
		pr_notice(SODI_TAG fmt, ##args))
#define so_info(fg, fmt, args...)    \
		((fg&SODI_FLAG_3P0) ?\
		pr_info(SODI3_TAG fmt, ##args):pr_info(SODI_TAG fmt, ##args))
...
enum wk_reason
spm_sodi_output_log(struct wake_status *wakesta,
	struct pcm_desc *pcmdesc, u32 sodi_flags, u32 operation_cond);
