...
s32 cmdq_sec_init_context_impl(struct cmdqSecContextStruct *handle)
{
	...
	do {
		...
		/* Currently, a process context limits to open mobicore
		 * device once,
		...
		if (mcRet == MC_DRV_ERR_INVALID_OPERATION) {
			CMDQ_MSG(
				"[SEC]_MOBICORE_OPEN: already opened, continue to execution\n");
			status = 0;
	...
	if (retry_cnt >= max_retry) {
		/* print error message */
		CMDQ_ERR(
		"[SEC]_MOBICORE_OPEN fail: status[%d], mcRet[0x%x], retry[%d]\n",
		status, mcRet, retry_cnt);
	} else {
		CMDQ_MSG(
			"[SEC]_MOBICORE_OPEN: status[%d], mcRet[0x%x], retry[%d]\n",
			status, mcRet, retry_cnt);
	}
...
s32 cmdq_sec_deinit_context_impl(struct cmdqSecContextStruct *handle)
{
#if defined(CMDQ_GP_SUPPORT)
	...
	if (handle->openMobicoreByOther == 1) {
		...
		CMDQ_MSG(
			"[SEC]_MOBICORE_CLOSE: status[%d], ret[%0x], openMobicoreByOther[%d]\n",
...
s32 cmdq_sec_allocate_wsm_impl(struct cmdqSecContextStruct *handle)
{
	s32 status = 0;
	...
	do {
		...
		status = TEEC_AllocateSharedMemory(&(handle->gp_context),
			&(handle->shared_mem));
		if (status != TEEC_SUCCESS) {
			...
			CMDQ_MSG(
				"[SEC]TEEC_WSM_ALLOC: status[0x%x], pWsm: 0x%p\n",
		...
		/* because world shared mem(WSM) will ba managed by mobicore
		 * device, not linux kernel instead of vmalloc/kmalloc,
		 * call mc_malloc_wasm to alloc WSM to prvent error such as
		 * "can not resolve tci physicall address" etc
		 */
		mcRet = mc_malloc_wsm(MC_DEVICE_ID_DEFAULT, 0,
			sizeof(struct iwcCmdqMessage_t),
			(u8 **)&(handle->iwcMessage), 0);
		if (mcRet != MC_DRV_OK) {
...
s32 cmdq_sec_free_wsm_impl(struct cmdqSecContextStruct *handle)
{
#if defined(CMDQ_GP_SUPPORT)
...
#else
	s32 status = 0;
	enum mc_result mcRet = mc_free_wsm(MC_DEVICE_ID_DEFAULT,
		handle->iwcMessage);
...
s32 cmdq_sec_open_session_impl(struct cmdqSecContextStruct *handle)
{
	s32 status = 0;
	...
	do {
#if defined(CMDQ_GP_SUPPORT)
		status = TEEC_OpenSession(&handle->gp_context,
			&handle->sessionHandle, &handle->uuid,
		...
		do {
			mcRet = mc_open_session(&handle->sessionHandle,
				&handle->uuid, handle->iwcMessage,
				sizeof(struct iwcCmdqMessage_t));
			if (mcRet != MC_DRV_OK) {
				CMDQ_ERR(
					"[SEC]_SESSION_OPEN: err[0x%x], retry[%d]\n",
					mcRet, retry_cnt);
				retry_cnt++;
				msleep_interruptible(2000);
				status = -1;
		...
		if (retry_cnt >= max_retry) {
			/* print error message */
			CMDQ_ERR(
				"[SEC]_SESSION_OPEN fail: status[%d], mcRet[0x%x], retry[%d]\n",
				status, mcRet, retry_cnt);
		} else {
			CMDQ_MSG(
				"[SEC]_SESSION_OPEN: status[%d], mcRet[0x%x], retry[%d]\n",
				status, mcRet, retry_cnt);
		}
#endif
	} while (0);
...
s32 cmdq_sec_close_session_impl(struct cmdqSecContextStruct *handle)
{
#if defined(CMDQ_GP_SUPPORT)
...
s32 cmdq_sec_init_session_unlocked(struct cmdqSecContextStruct *handle)
{
	s32 openRet = 0;
	s32 status = 0;
...
s32 cmdq_sec_execute_session_unlocked(
	struct cmdqSecContextStruct *handle, s32 timeout_ms)
{
#if defined(CMDQ_GP_SUPPORT)
	s32 status = 0;
	...
	status = TEEC_InvokeCommand(&(handle->sessionHandle), cmd, &operation,
		NULL);
	if (status != TEEC_SUCCESS) {
	...
	s32 status = 0;
	const s32 secureWoldTimeout_ms = timeout_ms > 0 ?
	...
	do {
		...
		/* wait respond */
		mcRet = mc_wait_notification(&(handle->sessionHandle),
...
s32 cmdq_sec_fill_iwc_command_basic_unlocked(
	s32 iwcCommand, void *_pTask, s32 thread, void *_pIwc)
{
	...
	s32 iwcCommand, void *_pTask, s32 thread, void *_pIwc)
{
	...
	const u32 reservedCommandSize = 4 * CMDQ_INST_SIZE;
	...
	if (thread != CMDQ_INVALID_THREAD) {
		...
		pIwc->command.commandSize = (u32)(current_va -
			(u8 *)pIwc->command.pVABase);
		...
		CMDQ_MSG(
			...
			pTask, thread, pTask->commandSize, pTask->engineFlag,
			sizeof(pIwc->command));
...
s32 cmdq_sec_fill_iwc_resource_msg_unlocked(
	s32 iwcCommand, void *_pTask, s32 thread, void *_pIwc)
{
	struct iwcCmdqMessage_t *pIwc;
...
s32 cmdq_sec_fill_iwc_cancel_msg_unlocked(
	s32 iwcCommand, void *_pTask, s32 thread, void *_pIwc)
{
	const struct TaskStruct *pTask = (struct TaskStruct *) _pTask;
	struct iwcCmdqMessage_t *pIwc;
...
s32 cmdq_sec_setup_context_session(
...
s32 cmdq_sec_teardown_context_session(struct cmdqSecContextStruct *handle)
{
	s32 status = 0;
...
void cmdq_sec_handle_attach_status(struct TaskStruct *pTask, u32 iwcCommand,
	const struct iwcCmdqMessage_t *pIwc, s32 sec_status_code,
...
s32 cmdq_sec_handle_session_reply_unlocked(
	const struct iwcCmdqMessage_t *pIwc, const s32 iwcCommand,
	struct TaskStruct *pTask, void *data)
{
	...
	if (iwcCommand == CMD_CMDQ_TL_CANCEL_TASK) {
		...
		/* note we etnry SWd to config GCE, and wait execution
		 * result in NWd update taskState only if config failed
		 */
		if (pTask && iwcRsp < 0)
...
CmdqSecFillIwcCB cmdq_sec_get_iwc_msg_fill_cb_by_iwc_command(u32 iwcCommand)
{
...
s32 cmdq_sec_send_context_session_message(struct cmdqSecContextStruct *handle,
	u32 iwcCommand, struct TaskStruct *pTask, s32 thread, void *data)
{
	s32 status;
	...
	do {
		...
		status = cmdq_sec_handle_session_reply_unlocked(
			handle->iwcMessage, iwcCommand, pTask, data);
	} while (0);

	return status;
}

void cmdq_sec_track_task_record(const u32 iwcCommand,
...
s32 cmdq_sec_submit_to_secure_world_async_unlocked(u32 iwcCommand,
	struct TaskStruct *pTask, s32 thread, void *data, bool throwAEE)
{
	const s32 tgid = current->tgid;
	const s32 pid = current->pid;
...
#endif
		/* Note we entry secure for config only and wait result
		 * in normal world.
	...
	struct cmdqSecCancelTaskResultStruct *pResult)
{
	...
	status = cmdq_sec_submit_to_secure_world_async_unlocked(
		CMD_CMDQ_TL_CANCEL_TASK, pTask, thread, (void *)pResult, true);
	return status;
#else
	CMDQ_ERR("secure path not support\n");
	return -EFAULT;
