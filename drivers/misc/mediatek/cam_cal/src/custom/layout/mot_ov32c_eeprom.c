...
unsigned int ov32c_mot_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData);

#define OV32C_MOT_EEPROM_ADDR 0x00
#define OV32C_MOT_EEPROM_DATA_SIZE 0x14E8
#define OV32C_MOT_SERIAL_NUMBER_ADDR 0x15
#define OV32C_MOT_MNF_ADDR 0x00
#define OV32C_MOT_MNF_DATA_SIZE 37
#define OV32C_MOT_MNF_CHECKSUM_ADDR 0x25
#define OV32C_MOT_AWB_ADDR 0x41
#define OV32C_MOT_AWB_DATA_SIZE 43
...
struct STRUCT_CAM_CAL_CONFIG_STRUCT mot_ov32c_eeprom = {
	.name = "mot_ov32c_eeprom",
	.sensor_type = FRONT_CAMERA,
	.check_layout_function = mot_layout_no_ck,
	.read_function = Common_read_region,
	.mot_do_factory_verify_function = ov32c_mot_do_factory_verify,
	...
	.enable_preload = 1,
	.preload_size = OV32C_MOT_EEPROM_DATA_SIZE,
};
...
unsigned int ov32c_mot_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData)
{
	...
	//mnf check
	checkSum = (pCamCalData->DumpAllEepromData[OV32C_MOT_MNF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[OV32C_MOT_MNF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData, OV32C_MOT_MNF_DATA_SIZE, checkSum)) {
	...
	memcpy(pCamCalData->serial_number, &pCamCalData->DumpAllEepromData[OV32C_MOT_SERIAL_NUMBER_ADDR],
		pCamCalData->serial_number_bit);
	//awb check
	pCamCalData->CalibrationStatus.awb_status =
		mot_check_awb_data(pCamCalData->DumpAllEepromData+OV32C_MOT_AWB_ADDR, OV32C_MOT_AWB_DATA_SIZE + 2);
	...
	//lsc check
	checkSum = (pCamCalData->DumpAllEepromData[OV32C_MOT_LSC_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[OV32C_MOT_LSC_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+OV32C_MOT_LSC_ADDR, OV32C_MOT_LSC_DATA_SIZE, checkSum)) {
