...
unsigned int mot_s5kgd2_do_2a_gain(struct EEPROM_DRV_FD_DATA *pdata,
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000003, 0x32443832, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000027, 0x00000045, mot_s5kgd2_do_2a_gain},
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	int af_addr = S5KGD2_MOT_AF_ADDR;
	int af_size = S5KGD2_MOT_AF_DATA_SIZE;
	int awb_addr = S5KGD2_MOT_AWB_ADDR;
	int awb_size = S5KGD2_MOT_AWB_DATA_SIZE;
	...
	if( pCamCalData->sensorID == MOT_AION_S5KGD2_SENSOR_ID ) {
		//af posture calibration data
		unsigned char AF_POSTURE[21];
		unsigned int af_posture_data_offset = 0x18f6;
