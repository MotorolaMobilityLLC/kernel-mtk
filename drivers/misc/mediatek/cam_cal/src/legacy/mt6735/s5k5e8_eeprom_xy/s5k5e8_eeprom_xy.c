...
static int s5k5e8_eeprom_read = 0;
//int read_s5k5e8_eeprom_size(u8 slv_id, u16 offset, u8* data,int size);
//kal_uint16 s5k5e8_read_cmos_sensor_eeprom(kal_uint32 addr);
typedef struct {
#if 0
...
OTP_DATA s5k5e8_eeprom_data_xy = {{0}};
//u8 af_inf_data=0;

extern kal_uint16 R_gain_xy,B_gain_xy; 
extern kal_uint8  vendor_id_xy;
#define CAL_VERSION_MAGIC ""
int read_s5k5e8_eeprom_mtk_fmt_xy(void)
{
#if 1
	//int i = 0;
	int offset = 0;
	...
	if(1 == s5k5e8_eeprom_read ) {
		CAM_CALDB("OTP readed ! skip\n");
		return 1;
	}
	spin_lock(&g_CAM_CALLock);
	s5k5e8_eeprom_read = 1;
	spin_unlock(&g_CAM_CALLock);
	offset = 0;
	s5k5e8_eeprom_data_xy.Data[2] = vendor_id_xy;
	s5k5e8_eeprom_data_xy.Data[3] = R_gain_xy & 0xFF; 
	s5k5e8_eeprom_data_xy.Data[4] = R_gain_xy >> 8;
	s5k5e8_eeprom_data_xy.Data[5] = B_gain_xy & 0xFF;
	s5k5e8_eeprom_data_xy.Data[6] = B_gain_xy >> 8;
	s5k5e8_eeprom_data_xy.Data[7] = 0x91;
	s5k5e8_eeprom_data_xy.Data[8] = 0x02;
	s5k5e8_eeprom_data_xy.Data[9] = 0x77;
	s5k5e8_eeprom_data_xy.Data[10] = 0x02;
	CAM_CALINF("wangkangmin s5k5e8 vendor_id =%x\n",vendor_id_xy);
	CAM_CALINF("wangkangmin s5k5e8 R_gain_h =%x,R_gain_l=%x\n",s5k5e8_eeprom_data_xy.Data[4],s5k5e8_eeprom_data_xy.Data[3]);
	CAM_CALINF("wangkangmin s5k5e8 B_gain_h =%x,B_gain_l=%x\n",s5k5e8_eeprom_data_xy.Data[6],s5k5e8_eeprom_data_xy.Data[5]);
	CAM_CALINF("wangkangmin s5k5e8 R_gain =%x,B_gain=%x\n",R_gain_xy,B_gain_xy);	
#endif
	return 0;
}
#if 0
...
{
  CAM_CALINF("wangkangmin s5k5e8 vendor id =%x \n",s5k5e8_eeprom_data_xy.Data[3956]);
  return s5k5e8_eeprom_data_xy.Data[3956];
}
#endif
#ifdef CONFIG_COMPAT
...
static long s5k5e8eeprom_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	CAM_CALDB("[CAMERA SENSOR] S5K5E8_EEPROM_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
{
    memcpy((void *)data,(void *)&s5k5e8_eeprom_data_xy.Data[offset],size);
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, S5K5E8_EEPROM_DEVICE_ID_XY,ptempbuf->u4Length);
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = s5k5e8eeprom_Ioctl_Compat,
#endif
...
static int __init CAM_CAL_init(void)
{
  ...
  //  i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        CAM_CALERR("failed to register s5k5e8_eeprom driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        CAM_CALERR("failed to register s5k5e8_eeprom driver, 2nd time\n");
        return -ENODEV;
    }
