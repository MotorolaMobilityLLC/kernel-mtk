...
enum { ERR_SIM_HOT_PLUG_NULL_POINTER = -13,
...
static int is_bulitin_eint_hw_deb(u32 eint_num)
{
	...
	if (eint_num >= EINT_BI_HW_DB_START &&
		eint_num < EINT_BI_HW_DB_START + EINT_BI_HW_DB_CNT)
		return 1;
...
int hwgpio_to_vgpio(int gpio)
{
	...
	if (gpio_base == 0) {
		pr_debug("[EIC] unexpected gpio base %d\n", gpio_base);
		WARN_ON(1);
	}
...
int mt_eint_clr_deint(u32 eint_num)
{
	...
	if (eint_num >= EINT_MAX_CHANNEL) {
		pr_debug("%s: eint_num(%u) is not in (0, %d)\n", __func__,
	...
	if (deint_mapped == MAX_DEINT_CNT) {
		pr_debug("%s: no deint(%d) used now\n", __func__, eint_num);
		return -1;
	}
...
int mt_eint_set_deint(u32 eint_num, u32 irq_num)
{
	...
	if (irq_num < deint_possible_irq[0]) {
		pr_debug("%s: irq_num(%u) out of range\n", __func__, irq_num);
		return -1;
	}
	...
	if (mt_eint_get_deint_sec_en(deint_mapped) == 1) {
		pr_debug("%s: eint_num(%u) can't use secure deint(%u)\n",
	...
	if (deint_descriptors[deint_mapped].used == 0) {
		...
		pr_debug("%s: deint(%u) already in use\n", __func__, irq_num);
		return -1;
	}
	...
	if (i == MAX_DEINT_CNT) {
		pr_debug("%s: no matched possible deint irq for %u\n", __func__,
				irq_num);
		dump_stack();
...
void mt_eint_emu_set(unsigned int eint_num)
{
	...
	pr_debug("[EINT] emul set addr:%x = %x, bit=%x\n", base, value, bit);
...
void mt_eint_emu_clr(unsigned int eint_num)
{
	...
	pr_debug("[EINT] emul clr addr:%x = %x, bit=%x\n", base, value, bit);
...
void mt_eint_unmask(unsigned int eint_num)
{
	...
	pr_debug("[EINT] unmask addr:%lx = %x\n", base, bit);
}
...
int mt_eint_get_polarity(unsigned int eint_num)
{
	...
	pr_debug("[EINT] %s :%lx, bit:%x, val:%x\n", __func__, base, bit, val);
	if (val & bit)
		pol = MT_EINT_POL_POS;
	else
		pol = MT_EINT_POL_NEG;
...
unsigned int mt_eint_set_sens(unsigned int eint_num, unsigned int sens)
{
	...
	if (sens == MT_EDGE_SENSITIVE) {
		...
		pr_debug("%s invalid sensitivity value\n", __func__);
		return 0;
	}
	mt_reg_sync_writel(bit, base);
	pr_debug("[EINT] %s :%lx, bit: %x\n", __func__, base, bit);
...
void mt_eint_set_hw_debounce(unsigned int gpio_pin, unsigned int us)
{
	...
	pr_debug("%s, gpio=%d, eint=%d, %dus\n", __func__, gpio_pin, eint_num,
		 us);

	if (eint_num >= EINT_MAX_CHANNEL) {
		pr_debug("%s: eint_num %d invalid\n", __func__, eint_num);
		return;
	}

	if (is_bulitin_eint_hw_deb(eint_num))
		offset = ((eint_num - EINT_BI_HW_DB_START) / 4) * 4 +
...
static void mt_eint_timer_event_handler(unsigned long eint_num)
{
	...
	pr_debug("EINT Module_IRQ - EINT_STA = 0x%x, in %s\n",
		status, __func__);
	if (status)
...
static void mt_eint_set_timer_event(unsigned int eint_num)
{
	...
	pr_debug(
		"EINT Module - expires:%lu, jiffies:%lu, deb_in_jiffies:%lu, ",
...
static void insert_trigger_entity(unsigned int eint, unsigned char arch,
				  unsigned long long ts_trigger)
{
	...
	eint_tri_his.entities[cur_wp].irq_status =
		mt_eint_read_raw_status(eint);
...
void dump_eint_trigger_history(void)
{
	...
	if (rb_status & BUFFER_OVERLAPED) {
		pr_debug("[EINT DEBUG] Buffer full\n");
		for (i = cur_wp; i != cur_wp - 1;
			 i = (i + 1) % EINT_ENTITY_NUM) {
			pr_debug(
				"[EINT DEBUG] cpu:%d,num:%d,ts:%lld,eint:%u,irq:%u,pol:0x%x,sen:0x%x,status:0x%x\n",
				eint_tri_his.entities[i].cpu, i,
		...
		pr_debug("[EINT DEBUG] Buffer not full\n");
...
static ssize_t eint_dump_history_store(struct device_driver *driver,
					const char *buf, size_t count)
{
	...
	if (kstrtoul(p, 10, &num) != 0) {
		pr_debug("[EIC] can not kstrtoul for %s\n", p);
		return -1;
	}
	eint_history_dump_enable = num;
	return count;
}
DRIVER_ATTR(eint_history, 0644, eint_dump_history_show,
		eint_dump_history_store);
#endif
...
static ssize_t per_eint_dump_show(struct device_driver *driver, char *buf)
{
	...
	ret += snprintf(
		...
		cur_debug_eint, mt_eint_get_mask(cur_debug_eint),
		mt_eint_get_polarity(cur_debug_eint),
		mt_eint_get_debounce_cnt(cur_debug_eint),
		mt_eint_get_sens(cur_debug_eint),
		mt_eint_get_sens(cur_debug_eint) == MT_EDGE_SENSITIVE ? "edge"
								  : "level");

	if (desc) {
		ret += snprintf(
			buf + ret, PAGE_SIZE, "irq: %ld, wakeup src=%s\n", irq,
...
static ssize_t per_deint_dump_show(struct device_driver *driver, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE,
		...
		cur_debug_deint, deint_sec, deint_sel, deint_en);
	return ret;
}
...
static irqreturn_t mt_eint_demux(struct irq_desc *desc)
{
	...
	if (!desc) {
		pr_debug("mtk-eic:%s: input irq_desc is NULL\n", __func__);
		return IRQ_NONE;
	}
	...
	if (!chip || !eint_chip) {
		pr_debug("mtk-eic:%s: fail to get irq_chip or eint_chip\n",
			__func__);
		return IRQ_NONE;
	}
	...
	/*
	 * NoteXXX: Need to get the wake up for 0.5 seconds when an EINT intr
	 * tirggers.
	 *		  This is used to prevent system from suspend such that
	 *other
	 * drivers
	 *		  or applications can have enough time to obtain their
	 *own
	 * wake lock.
	 *		  (This information is gotten from the power management
	 * owner.)
	 */

	tasklet_schedule(&eint_tasklet);
	pr_debug("EINT Module - %s ISR Start\n", __func__);
	...
	for (reg_base = 0; reg_base < EINT_MAX_CHANNEL; reg_base += 32) {
		...
		for (offset = 0; offset < 32; offset++) {
			...
			pr_debug("Got EINT %d: go with new mt_eint\n", index);
			if ((EINT_FUNC.is_deb_en[index] == 1) &&
				(index >= MAX_HW_DEBOUNCE_CNT)) {
				/* if its debounce is enable and it is a sw
				 * debounce
				 */
				mt_eint_mask(index);
				pr_debug("got sw index %d\n", index);
				mt_eint_set_timer_event(index);
			} else {
				pr_debug("got hw index %d\n", index);
				t1 = sched_clock();
	...
	pr_debug("EINT Module - %s ISR END\n", __func__);
	chained_irq_exit(chip, desc);
	return IRQ_HANDLED;
}

#if (EINT_DEBUG == 1)
static int mt_eint_max_channel(void) { return EINT_MAX_CHANNEL; }
#endif
...
static void mt_eint_setdomain0(void)
{
	...
	for (i = 0; i < ap_cnt; i++) {
		...
		pr_debug("[EINT] domain addr:%lx = %x\n", base,
				readl(IOMEM(base)));
	}
}
...
int get_type(char *name)
{
	...
	int type1 = 0x0;
	int type2 = 0x0;
#if defined(CONFIG_MTK_SIM1_SOCKET_TYPE) || \
	defined(CONFIG_MTK_SIM2_SOCKET_TYPE)
	char *p;
	ssize_t ret;
#endif
	...
	if (ret != 0) {
		/* kstrtoul error */
		pr_debug("[EINT] %s: cannot get value of type1\n", __func__);
		type1 = 0x0;
	}
#endif
	...
	if (ret != 0) {
		/* kstrtoul error */
		pr_debug("[EINT] %s: cannot get value of type2\n", __func__);
		type2 = 0x0;
	}
#endif
	if (!strncmp(name, "MD1_SIM1_HOT_PLUG_EINT",
			 strlen("MD1_SIM1_HOT_PLUG_EINT")))
		return type1;
	else if (!strncmp(name, "MD1_SIM1_HOT_PLUG_EINT",
			  strlen("MD1_SIM1_HOT_PLUG_EINT")))
		return type1;
	else if (!strncmp(name, "MD2_SIM2_HOT_PLUG_EINT",
...
static void pin_init(void)
{
	...
	for (i = 0; i < mapping_table_entry; i++) {
		...
		if (!pin_insert(&pins[i])) {
			pr_debug(
				"duplicate record? i = %d, gpio = %d, eint = %d\n",
...
static int gpio_to_eint(unsigned int gpio)
{
	...
	/*
	 * if not builtin eint, just find the mapping from normal mapping
	 * table,
...
unsigned int mt_gpio_to_irq(unsigned int gpio)
{
	...
	if (eint < 0) {
		pr_debug("[EIC] no mapped eint for gpio %u\n", gpio);
		WARN_ON(1);
		return -1;
	}

	/*
	 * since we linearly map eint irq from the first empty irq after the
	 * gic,
	 * we just add eint number to the first virq used by eint to get a new
	 * one.
	 */
...
static void mt_eint_soft_clr(unsigned int eint_num)
{
	...
	if (eint_num < EINT_MAX_CHANNEL) {
		...
		pr_debug("Err in %s [EINT] num:%d is larger than MAX_CHANNEL\n",
				__func__, eint_num);
		return;
	}
	writel(bit, IOMEM(base));
	pr_debug("[EINT] soft clr addr:%x = %x\n", base, bit);
}
...
struct platform_driver eint_driver = {
	.driver = {
		.name = "eint", .bus = &platform_bus_type, .owner = THIS_MODULE,
	}
};
...
static int __init mt_eint_init(void)
{
	...
	if (of_property_read_u32(node, "mediatek,max_hw_deb_cnt",
				 &MAX_HW_DEBOUNCE_CNT)) {
		pr_debug("[EIC] no mediatek,max_hw_deb_cnt specified\n");
		MAX_HW_DEBOUNCE_CNT = 16;
		pr_debug("[EIC] setup default value\n");
	}

	pr_debug("[EIC] max debunce num = %d\n", MAX_HW_DEBOUNCE_CNT);

	EINT_FUNC.eint_auto_umask =
		kmalloc(sizeof(unsigned int) * EINT_MAX_CHANNEL, GFP_KERNEL);
	EINT_FUNC.is_deb_en =
		kmalloc(sizeof(unsigned int) * EINT_MAX_CHANNEL, GFP_KERNEL);
	EINT_FUNC.deb_time =
		kmalloc(sizeof(unsigned int) * EINT_MAX_CHANNEL, GFP_KERNEL);
	EINT_FUNC.eint_sw_deb_timer =
		kmalloc(sizeof(struct timer_list) * EINT_MAX_CHANNEL,
			GFP_KERNEL);
	EINT_FUNC.count =
		kmalloc(sizeof(unsigned int) * EINT_MAX_CHANNEL, GFP_KERNEL);
	EINT_FUNC.gpio =
	...
	if (mapping_table_entry > 0) {
		...
		pins = kmalloc(
			sizeof(struct pin_node) * (mapping_table_entry + 1),
			GFP_KERNEL);
		for (i = 0; i < mapping_table_entry; i++) {
	...
	if (of_property_read_u32(node, "mediatek,max_deint_cnt",
				 &MAX_DEINT_CNT)) {
		pr_debug("[EIC] no max_deint_cnt specified\n");
	} else {
		deint_possible_irq =
			kzalloc(sizeof(u32) * MAX_DEINT_CNT, GFP_KERNEL);

		deint_descriptors = kzalloc(
			sizeof(struct deint_des) * MAX_DEINT_CNT, GFP_KERNEL);
		...
		if (of_property_read_u32_array(
			...
			pr_debug("[EINT] deint function would fail...\n");
	}

	/* deint hw deboucen*/
	if (of_property_read_u32_array(node, "mediatek,builtin_eint_hw_deb",
					&builtin_eint_dw_deb_array[0],
					ARRAY_SIZE(builtin_eint_dw_deb_array)))
		pr_debug("[EIC] no built in eint hw deb property\n");
	else {
		...
		pr_debug(
			"built in eint hw debounce: start=%d, count=%d,offset=0x%x\n",
	...
	if (of_property_read_u32(node, "mediatek,builtin_entry",
				 &builtin_entry)) {
		pr_debug("[EIC] no builtin_entry property\n");
	} else {
		builtin_mapping = kcalloc(
			builtin_entry, sizeof(struct builtin_eint), GFP_KERNEL);
		...
		pr_debug("[EIC] builtin_mapping: spec=%d, len=%d\n",
			be32_to_cpup(spec), len);
		...
		for (i = 0; i < builtin_entry; ++i) {
			...
			pr_debug(
				"[EIC] gpio, func_mode, builtin = %u, %u,%u\n",
	...
	if (of_property_read_u32(node, "mediatek,debtime_setting_entry",
		...
		pr_debug("[EIC] debtime_setting refer to default.\n");
		eint_debtime_setting.setting =
			kcalloc(eint_debtime_setting.deb_entry,
		...
		eint_debtime_setting.setting =
			kcalloc(eint_debtime_setting.deb_entry,
				sizeof(struct debcnt_setting), GFP_KERNEL);
		spec = of_get_property(node, "mediatek,debtime_setting_array",
						&len);
		if (spec == NULL)
			return -EINVAL;
		len /= sizeof(*spec);
		pr_debug("[EIC] debtime_setting: entry=%d, spec=%d, len=%d\n",
			eint_debtime_setting.deb_entry, be32_to_cpup(spec),
			len);

		for (i = 0; i < eint_debtime_setting.deb_entry; i++) {
			eint_debtime_setting.setting[i].setting =
				be32_to_cpup(spec + (i * 2));
			eint_debtime_setting.setting[i].deb_time =
				be32_to_cpup(spec + (i * 2) + 1);
	...
	if (!EINT_IRQ_BASE) {
		pr_debug("get_supported_irq_num returns %d\n", EINT_IRQ_BASE);
		return -1;
	}
	...
	if (irq_base != EINT_IRQ_BASE) {
		pr_debug("EINT alloc desc error %d\n", irq_base);
		return -1;
	}
	...
	domain = irq_domain_add_legacy(node, EINT_MAX_CHANNEL, EINT_IRQ_BASE,
					0, &mt_eint_domain_simple_ops, NULL);
	if (!domain)
		pr_debug("EINT domain add error\n");
	...
	if (ret)
		pr_debug("Fail to register eint_driver");

	ret = driver_create_file(&eint_driver.driver,
				 &driver_attr_per_eint_dump);

	if (ret) {
		pr_debug("Fail to create eint_driver sysfs files");
		return -1;
	}
...
static unsigned int mt_eint_get_debounce_cnt(unsigned int eint_num)
{
	...
	if (eint_num >= EINT_MAX_CHANNEL)
		...
		if (dben == 0) {
			pr_debug("[EIC] debounce disable, return directly");
			return 0;
		}

		dbnc = ((dbnc >> EINT_DBNC_SET_DBNC_BITS) >> shift_bit) &
				EINT_DBNC;
...
void mt_eint_dump_status(unsigned int eint)
{
	...
	pr_notice("[EINT] eint:%d,mask:%x,pol:%x,deb:%d us,sens:%x(%s)\n",
		  eint, mt_eint_get_mask(eint), mt_eint_get_polarity(eint),
