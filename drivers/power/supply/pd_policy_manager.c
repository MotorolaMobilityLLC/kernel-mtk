...
//static struct wake_lock sc_wake_lock;
...
static void usbpd_pm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
  int qc_protocol;
   ...
   qc_protocol = wt6670f_get_protocol();

   if(qc_protocol < 0) {
       pr_err("get qc protocol fail\n");
       return;
   }

   if(qc_protocol >= 0x08) {
       if(qc_protocol == 0x08)//18W
       {
           pdpm->apdo_max_volt = 12000;
           pdpm->apdo_max_curr = 2000;
       }
       else //27W | 45W
       {
           pdpm->apdo_max_volt = 12000;
       ...
       pdpm->pps_supported = true;
	   pr_err("qc3+ get,pps_supported : %d",pdpm->pps_supported);
       return;
   }
...
static int usbpd_set_qc_voltage(u16 voltage)
{
    return wt6670f_set_voltage(voltage);
}
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	if (effective_fcc_val >= 0) {
		...
		curr_fcc_limit = pm_config.bat_curr_lp_lmt;
	...
	if (pdpm->request_voltage - pdpm->cp.vbus_volt > 200){
		Charger_Detect_Init();
		pr_info("algo need inits\n");
		}
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_FC2_ENTRY_1:
		...
		ret = usbpd_set_qc_voltage(pdpm->request_voltage);
	...
	case PD_PM_STATE_FC2_EXIT:
    	...
    	ret = usbpd_set_qc_voltage(5000);
		pd_set_bq_charge_done(pdpm, true);

		usbpd_check_usb_psy(pdpm);
		if(ch1_dev != NULL)
			charger_dev_set_input_current(ch1_dev,pdpm->bat_curr_max * 1000);
...
static void usbpd_pd_contact(struct usbpd_pm *pdpm, bool connected)
{
   ...
   if(pdpm->pps_supported)
   		//wake_lock(&sc_wake_lock);
		    schedule_delayed_work(&pdpm->pm_work, 0);

		pr_err("%s pd is connected.\n", __func__);
	} else {
		////wake_unlock(&sc_wake_lock);
		usbpd_pm_disconnect(pdpm);
		pr_err("%s pd is not connected.\n", __func__);
	}
}
...
static void usb_psy_change_work(struct work_struct *work)
{
 ...
 ret = power_supply_get_property(pdpm->cp_psy,
				POWER_SUPPLY_PROP_TI_VBUS_PRESENT, &val);
	if (ret) {
		pr_err("Failed to read typec power online\n");
		goto out;
	}

   if(val.intval && !pdpm->pd_active) {
	usbpd_pd_contact(pdpm, true);
	   pr_err("this is commom pps TA.\n");
   }
   else if(!val.intval && pdpm->pd_active) {
     usbpd_pd_contact(pdpm, false);
		 pr_err("this is commom pd TA.\n");
   }
			...
			unsigned long event, void *data)
{
	...
	if(pdpm->usb_psy) {
		ret = power_supply_get_property(pdpm->usb_psy,POWER_SUPPLY_PROP_ONLINE,&pval);
		if(ret >= 0){
			pr_err("usb online :%d",pval.intval);
		}
	}
...
static int pd_policy_parse_dt(struct usbpd_pm *pdpm)
{
	...
	rc = of_property_read_u32(node,
			"moto,pd-bat-volt-max", &pdpm->bat_volt_max);
	if (rc < 0) {
		pr_err("pd-bat-volt-max property missing, use default val\n");
		pdpm->bat_volt_max = pm_config.bat_volt_lp_lmt;
	}
	else
		pm_config.bat_volt_lp_lmt = pdpm->bat_volt_max;
	pr_info("pm_config.bat_volt_lp_lmt:%d,bat max : %d\n", pm_config.bat_volt_lp_lmt,pdpm->bat_volt_max);

	rc = of_property_read_u32(node,
			"moto,pd-bat-curr-max", &pdpm->bat_curr_max);
	if (rc < 0) {
		pr_err("pd-bat-curr-max property missing, use default val\n");
		pdpm->bat_curr_max = pm_config.bat_curr_lp_lmt;
		}
	else
		pm_config.bat_curr_lp_lmt = pdpm->bat_curr_max;
	...
	rc = of_property_read_u32(node,
			"moto,pd-bus-volt-max", &pdpm->bus_volt_max);
	if (rc < 0) {
		pr_err("pd-bus-volt-max property missing, use default val\n");
		pdpm->bus_volt_max = pm_config.bus_volt_lp_lmt;
		}
	else
	...
	rc = of_property_read_u32(node,
			"moto,pd-bus-curr-max", &pdpm->bus_curr_max);
	if (rc < 0) {
		pr_err("pd-bus-curr-max property missing, use default val\n");
		pdpm->bus_curr_max = pm_config.bus_curr_lp_lmt;
		}
	else
	...
	rc = of_property_read_u32(node,
			"moto,pd-non-ffc-bat-volt-max", &pdpm->non_ffc_bat_volt_max);
...
static int usbpd_pm_probe(struct platform_device *pdev)
{
	...
	pr_info("%s enter\n", __func__);
	//wake_lock_init(&sc_wake_lock,WAKE_LOCK_SUSPEND,"sc_wake_lock");
