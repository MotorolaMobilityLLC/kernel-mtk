...
static void sgm4154x_chg_type_hvdcp_work(struct work_struct *work)
{
    ...
    if (sgm->chg_consumer != NULL) {
        charger_manager_set_qc_input_current_limit(sgm->chg_consumer, 0, 500000);
        charger_manager_set_qc_charging_current_limit(sgm->chg_consumer, 0, 500000);
    ...
    if (sgm->chg_consumer != NULL) {
        charger_manager_set_qc_input_current_limit(sgm->chg_consumer, 0, -1);
        charger_manager_set_qc_charging_current_limit(sgm->chg_consumer, 0, -1);
    }
...
static void charger_detect_work_func(struct work_struct *work)
{
    ...
    /* set curr default after BC1.2*/
    if ((sgm->chg_consumer != NULL) && (sgm->state.chrg_type != SGM4154x_NON_STANDARD)) {
        charger_manager_set_qc_input_current_limit(sgm->chg_consumer, 0, -1);
        charger_manager_set_qc_charging_current_limit(sgm->chg_consumer, 0, -1);
    }
...
static irqreturn_t sgm4154x_irq_handler_thread(int irq, void *private)
{
    ...
    pr_err("%s entry\n",__func__);
    schedule_delayed_work(&sgm->charge_detect_delayed_work, 1);
    return IRQ_HANDLED;
}
...
static int sgm4154x_do_event(struct charger_device *chg_dev, u32 event, u32 args)
{
    ...
    switch (event) {
        case EVENT_EOC:
            pr_info("%s: into EVENT_EOC event = %d\n", __func__, event);
            charger_dev_notify(chg_dev, CHARGER_DEV_NOTIFY_EOC);
            break;
        case EVENT_RECHARGE:
            pr_info("%s: into EVENT_RECHARGE event = %d\n", __func__, event);
        ...
        const struct i2c_device_id *id)
{
    ...
    pr_err("sgm4154x parse dts done\n");
    ...
    pr_err("%d:sgm4154x wakelock register done\n", __LINE__);
    ...
    pr_err("%s:dev register done, chg ops is %p, &tchg_adc is %d\n", __func__, &sgm4154x_chg_ops, sgm->chg_dev->ops->get_tchg_adc);
    /* otg regulator */
    ...
    pr_err("%s:after hw_init, chg ops is %p, &tchg_adc is %d\n", __func__, &sgm4154x_chg_ops, sgm->chg_dev->ops->get_tchg_adc);
    //OTG setting
    //sgm4154x_set_otg_voltage(s_chg_dev_otg, 5000000); //5V
    //sgm4154x_set_otg_current(s_chg_dev_otg, 1200000); //1.2A
    sgm->chg_consumer = charger_manager_get_by_name(sgm->dev, "primary_chg");
    if(!sgm->chg_consumer) {
