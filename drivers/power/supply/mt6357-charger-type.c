...
static inline void dump_charger_name(int type)
{
	switch (type) {
...
static int get_charger_type(struct mtk_charger_type *info)
{
	...
	if (hw_bc11_DCD(info)) {
        ...
        pr_info("usb type: unknown\n");
	} else {
		if (hw_bc11_stepA2(info)) {
			if (hw_bc11_stepB2(info)) {
				type = POWER_SUPPLY_USB_TYPE_DCP;
                pr_info("usb type: dcp\n");
			} else {
				type = POWER_SUPPLY_USB_TYPE_CDP;
                pr_info("usb type: cdp\n");
			}
		} else {
			type = POWER_SUPPLY_USB_TYPE_SDP;
            pr_info("usb type: sdp\n");
		}
	}
...
static inline irqreturn_t chrdet_int_handler(int irq, void *data)
{
    struct mtk_charger_type *info = (struct mtk_charger_type *)data;
			...
			const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
        ...
        pr_info("<%s> chrdet: %d\n", __func__, chrdet);
        if (val->intval) {
            pr_info("<%s> set online\n", __func__);
            info->type = get_charger_type(info);  
        } else {
            pr_info("<%s> set offline\n", __func__);
            hw_bc11_done(info);
            info->type = POWER_SUPPLY_USB_TYPE_UNKNOWN;
        }
		break;
...
{
	if (strstr(boot_command_line, "primary_chg=sgm41542")) {
		pr_info("primary charge is sgm41542, mt6357 charge driver skip\n");
		return 0;
	}
