...
#if 0
	wt->i2c_scl_pin = of_get_named_gpio(wt->dev->of_node, "wt6670f-i2c-scl-pin", 0);
	if (wt->i2c_scl_pin < 0)
		pr_info("[%s] of get wt6670 gpio failed, i2c_clk_pin:%d\n",
			__func__, wt->i2c_scl_pin);

	if (gpio_request(wt->i2c_scl_pin, "wt6670_i2c_clk_pin"))
		pr_info("[%s] gpio_request reset failed", __func__);

	if (gpio_direction_output(wt->i2c_scl_pin, 0))
		pr_info("[%s] gpio_direction_output failed", __func__);

	wt->i2c_sda_pin = of_get_named_gpio(wt->dev->of_node, "wt6670f-i2c-sda-pin", 0);
	if (wt->i2c_sda_pin < 0)
		pr_info("[%s] of get wt6670 gpio failed, i2c_sda_pin:%d\n",
			__func__, wt->i2c_sda_pin);

	if (gpio_request(wt->i2c_sda_pin, "wt6670_i2c_sda_pin"))
		pr_info("[%s] gpio_request reset failed", __func__);

	if (gpio_direction_output(wt->i2c_sda_pin, 0))
		pr_info("[%s] gpio_direction_output failed", __func__);
#else
...
static int __init wt6670f_init(void)
{
	...
	#if 0
	np = of_find_node_by_name(NULL, "wt6670f_qc3p");//usb_type_c
	if (np != NULL)
		pr_info("wt6670f_qc3p node found...\n");
	else
		pr_info("wt6670f_qc3p node not found...\n");
	#else
