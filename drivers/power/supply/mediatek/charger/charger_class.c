...
int charger_dev_get_charger_type_hvdcp(struct charger_device *chg_dev, bool *state)
{

   if (chg_dev != NULL && chg_dev->ops != NULL &&
        chg_dev->ops->get_charger_type_hvdcp)
        return chg_dev->ops->get_charger_type_hvdcp(chg_dev, state);

    return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_charger_type_hvdcp);
		...
		int *tchg_max)
{
	...
	if ((chg_dev != NULL) && (chg_dev->ops != NULL) &&
	    (chg_dev->ops->get_tchg_adc != NULL)) { 
        pr_err("get temperture ops not NULL, ops is %p, tchg_adc is %d\n", chg_dev->ops, chg_dev->ops->get_tchg_adc);
		return chg_dev->ops->get_tchg_adc(chg_dev, tchg_min,
						  tchg_max);
    }

    pr_err("get temperture ops is NULL\n");
	return -ENOTSUPP;
}
