...
int check_cable_in;
int g_quick_charging_flag = 0;
static bool g_probe_down_flag;
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		...
		break;
	case POWER_SUPPLY_PROP_QUICK_CHARGING:
		val->intval = g_quick_charging_flag;
		break;
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		...
		if (mtk_chg->chg_type != CHARGER_UNKNOWN)
			...
			pr_info("%s disable_power_path true nothing\n", __func__);
			/*charger_manager_force_disable_power_path(
...
int set_probe_down_flag(bool flag)
{
	g_probe_down_flag = flag;

	return 1;
}
EXPORT_SYMBOL(set_probe_down_flag);

bool get_probe_down_flag()
{
    return g_probe_down_flag;
}
EXPORT_SYMBOL(get_probe_down_flag);
...
static int chgdet_task_threadfn(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (cti->chg_consumer && (g_probe_down_flag == true)) {
			charger_manager_enable_chg_type_det(cti->chg_consumer,
							attach);
            g_probe_down_flag = false;
            pr_err("g_probe_down_flag = %d\n", g_probe_down_flag);
        }
#else
