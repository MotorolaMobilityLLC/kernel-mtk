...
#define MAX_CHARGING_CURRENT         3600000
#define CHARGER_LIMIT_T0_TO_T1       1000000
#define CHARGER_LIMIT_T1_TO_T2       1000000
#define CHARGER_LIMIT_T1_TO_T2_ABV   2500000


extern int charger_get_qc_type(void);
static int g_constant_voltage = 0;
...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (info->tcmd_chg_limited) {
		pdata->input_current_limit =
			info->chg1_data.tcmd_input_current_limit;
		pdata->charging_current_limit =
			info->chg1_data.tcmd_charging_current_limit;
		goto done;
	}
	...
	if (is_typec_adapter(info)) {
		...
		pdata->charging_current_limit =
				info->data.ac_charger_current;
		ret = charger_dev_get_charger_type_hvdcp(info->chg1_dev, &state);
		if (ret < 0) {
			pr_err("%s get 18W type error",__func__);
		}
		if (state == true) {
			pdata->charging_current_limit = MAX_CHARGING_CURRENT;
		} else {
			pr_info("%s is not 18W charging",__func__);
		}
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			if (info->sw_jeita.sm == TEMP_T0_TO_T1) {
				if(pdata->input_current_limit > CHARGER_LIMIT_T0_TO_T1)
					pdata->input_current_limit = CHARGER_LIMIT_T0_TO_T1;
			} else if (info->sw_jeita.sm == TEMP_T1_TO_T2) {
				if(info->battery_temp >= 10) { //default limit
					if(pdata->input_current_limit > CHARGER_LIMIT_T1_TO_T2_ABV)
						pdata->input_current_limit = CHARGER_LIMIT_T1_TO_T2_ABV;
				} else {
					if(pdata->input_current_limit > CHARGER_LIMIT_T1_TO_T2)
						pdata->input_current_limit = CHARGER_LIMIT_T1_TO_T2;
				}
			}
		}
	}
	...
	if (pdata->thermal_input_current_limit != -1 || pdata->qc_input_current_limit != -1) {
		if (pdata->thermal_input_current_limit != -1 && pdata->qc_input_current_limit == -1) {
			min_thermal_limit = pdata->thermal_input_current_limit;
		} else if (pdata->thermal_input_current_limit == -1 && pdata->qc_input_current_limit != -1) {
			min_thermal_limit = pdata->qc_input_current_limit;
		} else {
			if (pdata->qc_input_current_limit <	pdata->thermal_input_current_limit) {
				min_thermal_limit = pdata->qc_input_current_limit;
			} else {
				min_thermal_limit = pdata->thermal_input_current_limit;
			}
		}
		if (min_thermal_limit < pdata->input_current_limit) {
			pdata->input_current_limit = min_thermal_limit;
		}
	}
...
int charger_set_thread_cv(u32 uV)
{
	g_constant_voltage = uV;

	return 1;
}
EXPORT_SYMBOL(charger_set_thread_cv);

static void  charger_set_cv_by_ffc(struct charger_manager *info, u32 quick_cv)
{
    struct sw_jeita_data *sw_jeita;
    sw_jeita = &info->sw_jeita;

    if (sw_jeita->sm == TEMP_T2_TO_T3) {
        sw_jeita->cv = quick_cv;
    }
    chr_err("%s: sw_jeita->cv:%d\n", __func__, sw_jeita->cv);
}

static void swchg_set_normal_charge_cv(struct charger_manager *info)
{
    int bat_current = 0;
    int charger_qc_type;

    charger_qc_type = charger_get_qc_type();
    bat_current = battery_get_bat_current();

    chr_err("%s: charger qc type :%d\n", __func__, charger_qc_type);
    if (charger_qc_type == 0) {
        if (bat_current < 8000) { // Switch battery cv when the current is less than 10000
            chr_err("%s: into ibatt_curr < 8000\n", __func__);
            charger_set_cv_by_ffc(info, 4450000);
        } else if (bat_current < 26000) {             //Switch battery cv when the current is less than 26000
            chr_err("%s: into ibatt_curr < 26000\n", __func__);
            if ((bat_current >= 8000) && (bat_current < 8160)) {
                chr_err("%s: into (ibatt_curr >= 8000) && (ibatt_curr < 8160)\n", __func__);
                charger_set_cv_by_ffc(info, 4450000);
            } else {
                charger_set_cv_by_ffc(info, 4464000);
            }
        } else if (bat_current > 26160) {             //Switch battery cv when the current is greater than 2616ma
            chr_err("%s: into ibatt_curr > 26160\n", __func__);
            charger_set_cv_by_ffc(info, 4480000);
        } else if ((bat_current >= 26000) && (bat_current <= 26160)) {             //Switch battery cv when the current is greater than 25000
            chr_err("%s: into (ibatt_curr >= 26000) && (ibatt_curr < 26160)\n", __func__);
            charger_set_cv_by_ffc(info ,4464000);
        }
    } else {
        charger_set_cv_by_ffc(info, g_constant_voltage);
    }
}
...
static void swchg_select_cv(struct charger_manager *info)
{
	...
	chr_info("%s: into swchg_select_cv\n", __func__);
	...
	/*if (constant_voltage == 4450000) {  //Default battery cv in charger thread
		constant_voltage = 4464000;     ////Default battery cv in charger thread
	}*/
	if (g_constant_voltage != 0) {
		constant_voltage = g_constant_voltage;
	}
	mtk_get_dynamic_cv(info, &constant_voltage);
	chr_info("%s: set  constant_voltage is %d\n", __func__, constant_voltage);
...
static void swchg_turn_on_charging(struct charger_manager *info)
{
	...
	charger_dev_set_input_current(info->chg1_dev,
		...
		swchg_set_normal_charge_cv(info);
