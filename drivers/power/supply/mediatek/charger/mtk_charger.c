...
bool g_capacity_level_limited_flag = false;
bool g_battery_protect_enable_flag = false;
static struct charger_manager *pinfo;
static struct list_head consumer_head = LIST_HEAD_INIT(consumer_head);
static DEFINE_MUTEX(consumer_mutex);
int g_charger_status = 0;
static int g_charger_qc_type = 0;
...
int charger_manager_set_qc_input_current_limit(struct charger_consumer *consumer,
	...
	if (info != NULL) {
		...
		if (info->data.parallel_vbus) {
			...
			if (idx == MAIN_CHARGER)
				...
				return -ENOTSUPP;
			pdata->qc_input_current_limit = input_current;
		}
...
int charger_manager_set_qc_charging_current_limit(
	struct charger_consumer *consumer, int idx, int charging_current)
{
	...
	if (info != NULL) {
		...
		pdata->qc_charging_current_limit = charging_current;
		chr_err("%s: dev:%s idx:%d en:%d\n", __func__,
...
static int qc_chgstat_notify(struct charger_manager *info)
{
	int ret = 0;
	char *env[2] = { "QUICK_CHARGER=YES", NULL };
...
void hvdcp3_chgstat_notify(void)
{
	int ret = 0;
	g_quick_charging_flag = 1;

	if (pinfo == NULL) {
		chr_err("mt charger pinfo is not rdy ,skip hvdcp3\n");
		return;
	}

	if (g_battery_protect_enable_flag) {
		chr_err("overcharge protection is on, skip hvdcp toast\n");
		return;
	}
	ret = qc_chgstat_notify(pinfo);
	pr_info("%s uevented\n", __func__);
	if (ret)
		chr_err("%s: kobject_uevent_fail, ret=%d", __func__, ret);
	return ;
...
void hvdcp_chgstat_notify(void)
{
	int ret = 0;
	g_quick_charging_flag = 1;

	if (pinfo == NULL) {
		chr_err("mt charger pinfo is not rdy ,skip hvdcp\n");
		return;
	}
	if (g_battery_protect_enable_flag) {
		chr_err("overcharge protection is on, skip hvdcp toast\n");
...
void mtk_charger_int_handler(void)
{
	...
	} else if (g_battery_protect_enable_flag) {
	        charger_manager_notifier(pinfo, CHARGER_NOTIFY_STOP_CHARGING);
		chr_err("overcharge protection is on ,skip\n");
	} else {
	...
	if (info->enable_dynamic_mivr) {
		if (!mtk_pe40_get_is_connect(info) &&
			...
			!mtk_pdc_check_charger(info)) {
					...
					charger_dev_set_mivr(info->chg1_dev,
						4700000); // soc more than 80, set mivr 4700mv
				}
#ifndef CONFIG_CHARGER_DEVICE_QC
...
static void charger_check_status(struct charger_manager *info)
{
	...
	/* demo version capacity over 70% not charging */
	if (info->capacity_level_limited && (battery_get_uisoc() >= 70)) {
		charging = false;
		goto stop_charging;
	}
	/* demo version capacity lower than 65% chargind & disable limit flag */
	if ((!info->capacity_level_limited) || (battery_get_uisoc() <= 65)) {
		charging = true;
	}
...
stop_charging:
	...
	chr_err("tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d sc:%d %d %d cl:%d\n",
		temperature, info->enable_sw_jeita, info->sw_jeita.sm,
		info->sw_jeita.cv, info->sw_jeita.charging, thermal->sm,
		charging, info->cmd_discharging, info->safety_timeout,
		info->vbusov_stat, info->sc.disable_charger,
		info->can_charging, charging, info->capacity_level_limited);
...
int charger_set_qc_type(int type)
{
	g_charger_qc_type = type;

	return 1;
}
EXPORT_SYMBOL(charger_set_qc_type);

int charger_get_qc_type(void)
{
    return g_charger_qc_type;
}
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		if (info->init_done && get_probe_down_flag()) {
			charger_manager_enable_chg_type_det(info->chg1_consumer, true);
			set_probe_down_flag(false);
		}
				...
				struct device *dev)
{
	...
	chr_err("temp_t0_thres_plus_x_degree:%d\n", val);
...
static ssize_t show_Charge_Status(struct device *dev,
		                struct device_attribute *attr, char *buf)
{
	int chargeType = g_charger_status;

	pr_err("%s,%d show Charger Status %d \n", __func__, __LINE__, chargeType);
	return sprintf(buf, "%u\n", chargeType);
}

static DEVICE_ATTR(Charge_Status, 0444, show_Charge_Status, NULL);
...
static ssize_t show_tcmd_input_current(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	struct charger_manager *pinfo = dev->driver_data;

	pr_debug("[Battery] %s : %d\n",
		__func__, pinfo->chg1_data.input_current_limit);
	return sprintf(buf, "%u\n",
		pinfo->chg1_data.input_current_limit);
}

static ssize_t store_tcmd_input_current(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
	...
	if (buf != NULL && size != 0) {
		...
		pinfo->tcmd_chg_limited = 1;
		pinfo->chg1_data.tcmd_input_current_limit = reg;

		pr_err("[Battery] %s: %d\n",
			__func__, pinfo->chg1_data.tcmd_input_current_limit);
	}
	return size;
}
static DEVICE_ATTR(tcmd_input_current, 0664, show_tcmd_input_current,
        store_tcmd_input_current);

static ssize_t show_tcmd_chg1_current(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	struct charger_manager *pinfo = dev->driver_data;

	pr_debug("[Battery] %s : %d\n",
		__func__, pinfo->chg1_data.charging_current_limit);
	return sprintf(buf, "%u\n",
		pinfo->chg1_data.charging_current_limit);
}

static ssize_t store_tcmd_chg1_current(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
	...
	if (buf != NULL && size != 0) {
		...
		ret = kstrtouint(buf, 10, &reg);
		pinfo->tcmd_chg_limited = 1;
		pinfo->chg1_data.tcmd_charging_current_limit = reg;
		pr_err("[Battery] %s: %d\n", __func__,
			pinfo->chg1_data.tcmd_charging_current_limit);
	}
	return size;
}
static DEVICE_ATTR(tcmd_chg1_current, 0664, show_tcmd_chg1_current, store_tcmd_chg1_current);

static ssize_t show_tcmd_capacity_level_limited(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	struct charger_manager *pinfo = dev->driver_data;

	pr_debug("[Battery] %s : %d\n",
		__func__, pinfo->capacity_level_limited);
	return sprintf(buf, "%u\n",
		pinfo->capacity_level_limited);
}

static ssize_t store_tcmd_capacity_level_limited(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
	...
	if (buf != NULL && size != 0) {
		pr_debug("[Battery] buf is %s and size is %zu\n", buf, size);
		ret = kstrtouint(buf, 10, &reg);
		pinfo->capacity_level_limited = reg;
		g_capacity_level_limited_flag = pinfo->capacity_level_limited;
		pr_err("[Battery] %s: %d\n", __func__,
			pinfo->capacity_level_limited);
	}
	return size;
}
static DEVICE_ATTR(tcmd_capacity_level_limited, 0664, show_tcmd_capacity_level_limited, store_tcmd_capacity_level_limited);
...
static int mtk_chg_en_battery_protect_show(struct seq_file *m, void *data)
{
        struct charger_manager *pinfo = m->private;
...
static ssize_t mtk_chg_en_battery_protect_write(struct file *file,
                const char *buffer, size_t count, loff_t *data)
{
        int len = 0, ret = 0;
        char desc[32] = {0};
        ...
        if (ret == 0) {
           if (enable == 1) {
                    g_battery_protect_enable_flag = true;
                    info->cmd_discharging = true;
                    charger_dev_enable(info->chg1_dev, false);
                    charger_manager_notifier(info,
                                        CHARGER_NOTIFY_STOP_CHARGING);
                    ...
                    g_battery_protect_enable_flag = false;
                    info->cmd_discharging = false;
                    charger_dev_enable(info->chg1_dev, true);
                    charger_manager_notifier(info,
                                        CHARGER_NOTIFY_START_CHARGING);
            }
            chr_info("%s: enable battery protect = %d\n", __func__, enable);
        } else {
            chr_err("bad argument, echo [enable] > en_battery_protect\n");
        }
...
PROC_FOPS_RW(en_battery_protect);
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_Charge_Status);
	if (ret)
		goto _out;
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_tcmd_chg1_current);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_tcmd_input_current);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_tcmd_capacity_level_limited);
	if (ret)
		goto _out;
	...
	if (!battery_dir) {
        ...
        proc_create_data("en_battery_protect", 0640, battery_dir,
                        &mtk_chg_en_battery_protect_fops, info);
...
void sc_select_charging_current(struct charger_manager *info, struct charger_data *pdata)
{
	...
	chr_err("sck: en:%d pid:%d %d %d %d %d %d qc:%d input:%d thermal.dis:%d\n",
			info->sc.enable,
			info->sc.g_scd_pid,
			info->sc.pre_ibat,
			info->sc.sc_ibat,
			...
			info->sc.solution,
			pdata->qc_charging_current_limit,
			pdata->qc_input_current_limit,
	...
	if (pdata->thermal_charging_current_limit != -1 || pdata->qc_charging_current_limit != -1) {
		if (pdata->thermal_charging_current_limit != -1 && pdata->qc_charging_current_limit == -1) {
			min_thermal_limit = pdata->thermal_charging_current_limit;
		} else if (pdata->thermal_charging_current_limit == -1 && pdata->qc_charging_current_limit != -1) {
			min_thermal_limit = pdata->qc_charging_current_limit;
		} else {
			if (pdata->qc_charging_current_limit <	pdata->thermal_charging_current_limit) {
				min_thermal_limit = pdata->qc_charging_current_limit;
			} else {
				min_thermal_limit = pdata->thermal_charging_current_limit;
			}
		}
		if (min_thermal_limit < pdata->charging_current_limit) {
			pdata->charging_current_limit = min_thermal_limit;
		}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	info->enable_dynamic_cv = true;
	info->capacity_level_limited = false;
	...
	info->chg1_data.qc_input_current_limit = -1;
	info->chg1_data.qc_charging_current_limit = -1;
	info->chg2_data.thermal_charging_current_limit = -1;
	info->chg2_data.thermal_input_current_limit = -1;
	info->chg2_data.qc_input_current_limit = -1;
	info->chg2_data.qc_charging_current_limit = -1;
	info->dvchg1_data.thermal_input_current_limit = -1;
	info->dvchg1_data.qc_input_current_limit = -1;
	info->dvchg1_data.qc_charging_current_limit = -1;
	info->dvchg2_data.thermal_input_current_limit = -1;
	info->dvchg2_data.qc_charging_current_limit = -1;
	info->dvchg2_data.qc_input_current_limit = -1;
...
#ifndef MTK_CHARGER_DIS_POWER_PATH
