...
static int hl7005_dump_register(struct charger_device *chg_dev)
{
	...
	for (i = 0; i < HL7005_REG_NUM; i++) {
		...
		pr_err("[0x%x]=0x%x ", i, hl7005_reg[i]);
	}
	pr_err("\n");
...
int hl7005_hw_init(struct charger_device *chg_dev)
{
	unsigned int status = 0;
	hl7005_reg_config_interface(0x06, 0x7c);	/* ISAFE = 1450mA, VSAFE = 4.44V */
	hl7005_reg_config_interface(0x06, 0x7c);	/* ISAFE = 1450mA, VSAFE = 4.44V */
	...
	if (!hw_init_flag)
	{
		hl7005_reg_config_interface(0x04, 0x73);	/* 146mA */
		hl7005_reg_config_interface(0x01, 0x78);	/* 146mA */
		hw_init_flag = true;
	}
...
{
	hl7005_reg_config_interface(0x06, 0x7c);	/* ISAFE = 2450mA, VSAFE = 4.44V */
	hl7005_reg_config_interface(0x06, 0x7c);	/* ISAFE = 2450mA, VSAFE = 4.44V */
