...
void fg_custom_init_from_header(void)
{
	...
	if (gauge_dev != NULL) {
		...
		fg_cust_data.pl_charger_status =
			gauge_dev->fg_hw_info.pl_charger_status;
	}
	...
	fg_cust_data.q_max_sys_voltage =
		UNIT_TRANS_10 * g_Q_MAX_SYS_VOLTAGE[g_fg_battery_id];

	fg_cust_data.pseudo1_t0 = UNIT_TRANS_100 *
		g_FG_PSEUDO1_T0[g_fg_battery_id];
	fg_cust_data.pseudo1_t1 = UNIT_TRANS_100 *
		g_FG_PSEUDO1_T1[g_fg_battery_id];
	fg_cust_data.pseudo1_t2 = UNIT_TRANS_100 *
		g_FG_PSEUDO1_T2[g_fg_battery_id];
	fg_cust_data.pseudo1_t3 = UNIT_TRANS_100 *
		g_FG_PSEUDO1_T3[g_fg_battery_id];
	fg_cust_data.pseudo1_t4 = UNIT_TRANS_100 *
		g_FG_PSEUDO1_T4[g_fg_battery_id];
	fg_cust_data.pseudo100_t0 = UNIT_TRANS_100 *
		g_FG_PSEUDO100_T0[g_fg_battery_id];
	fg_cust_data.pseudo100_t1 = UNIT_TRANS_100 *
		g_FG_PSEUDO100_T1[g_fg_battery_id];
	fg_cust_data.pseudo100_t2 = UNIT_TRANS_100 *
		g_FG_PSEUDO100_T2[g_fg_battery_id];
	fg_cust_data.pseudo100_t3 = UNIT_TRANS_100 *
		g_FG_PSEUDO100_T3[g_fg_battery_id];
	fg_cust_data.pseudo100_t4 = UNIT_TRANS_100 *
		g_FG_PSEUDO100_T4[g_fg_battery_id];
	...
	fg_cust_data.pseudo1_iq_offset = UNIT_TRANS_100 *
		g_FG_PSEUDO1_OFFSET[g_fg_battery_id];
	...
	/* shutdown_hl_zcv */
	fg_cust_data.shutdown_hl_zcv_t0 =
		g_SHUTDOWN_HL_ZCV_T0[g_fg_battery_id];
	fg_cust_data.shutdown_hl_zcv_t1 =
		g_SHUTDOWN_HL_ZCV_T1[g_fg_battery_id];
	fg_cust_data.shutdown_hl_zcv_t2 =
		g_SHUTDOWN_HL_ZCV_T2[g_fg_battery_id];
	fg_cust_data.shutdown_hl_zcv_t3 =
		g_SHUTDOWN_HL_ZCV_T3[g_fg_battery_id];
	fg_cust_data.shutdown_hl_zcv_t4 =
		g_SHUTDOWN_HL_ZCV_T4[g_fg_battery_id];
...
int force_get_tbat(bool update)
{
	...
	if (bat_temperature_val <=
		BATTERY_TMP_TO_DISABLE_GM30 && gDisableGM30 == false) {
		bm_err(
...
int get_customized_d0_c_soc(int origin_d0_c_soc)
{
	...
	bm_err("[get_customized_d0_c_soc] EC_en %d EC_value %d original %d val %d\n",
		Bat_EC_ctrl.debug_d0_c_en, Bat_EC_ctrl.debug_d0_c_value,
		origin_d0_c_soc, val);
...
int get_customized_d0_v_soc(int origin_d0_v_soc)
{
	...
	bm_err("[get_customized_d0_c_soc] EC_en %d EC_value %d original %d val %d\n",
		Bat_EC_ctrl.debug_d0_v_en, Bat_EC_ctrl.debug_d0_v_value,
		origin_d0_v_soc, val);
...
int get_customized_uisoc(int origin_uisoc)
{
	...
	bm_err("[get_customized_d0_c_soc] EC_en %d EC_value %d original %d val %d\n",
		Bat_EC_ctrl.debug_uisoc_en, Bat_EC_ctrl.debug_uisoc_value,
		origin_uisoc, val);
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_IS_BAT_PLUGOUT:
		{
			...
			gauge_dev_get_boot_battery_plug_out_status(
				gauge_dev, &is_bat_plugout, &bat_plugout_time);
	...
	case FG_DAEMON_CMD_SET_INIT_FLAG:
		{
			memcpy(&init_flag,
				&msg->fgd_data[0], sizeof(init_flag));
	...
	case FG_DAEMON_CMD_SET_DAEMON_PID:
		{
			memcpy(&g_fgd_pid,
				&msg->fgd_data[0], sizeof(g_fgd_pid));
			bm_debug(
				"[fr] FG_DAEMON_CMD_SET_DAEMON_PID = %d\n",
				g_fgd_pid);
		}
		break;
	...
	case FG_DAEMON_CMD_CHECK_FG_DAEMON_VERSION:
		{
			memcpy(&g_fgd_version,
				&msg->fgd_data[0], sizeof(g_fgd_version));
			bm_debug(
				"[fr] FG_DAEMON_CMD_CHECK_FG_DAEMON_VERSION = %d\n",
				g_fgd_version);
	...
	case FG_DAEMON_CMD_GET_CUSTOM_SETTING:
		{
			...
			bm_debug("[fr] data len:%d %d custom data length = %d\n",
	...
	case FG_DAEMON_CMD_SET_FG_BAT_INT1_GAP:
	{
		...
		memcpy(&fg_bat_int1_gap,
			&msg->fgd_data[0], sizeof(fg_bat_int1_gap));
	...
	{
		memcpy(&fg_bat_int2_ht,
			&msg->fgd_data[0], sizeof(fg_bat_int2_ht));
	...
	{
		memcpy(&fg_bat_int2_lt,
			&msg->fgd_data[0], sizeof(fg_bat_int2_lt));
	...
	{
		memcpy(&fg_bat_int2_ht_en,
			&msg->fgd_data[0], sizeof(fg_bat_int2_ht_en));
	...
	{
		memcpy(&fg_bat_int2_lt_en,
			&msg->fgd_data[0], sizeof(fg_bat_int2_lt_en));
	...
	case FG_DAEMON_CMD_SET_FG_BAT_TMP_C_GAP:
	{
		...
		memcpy(&fg_bat_tmp_c_int_gap,
			&msg->fgd_data[0], sizeof(fg_bat_tmp_c_int_gap));
	...
	case FG_DAEMON_CMD_SET_FG_BAT_TMP_GAP:
	{
		...
		memcpy(
			&fg_bat_tmp_int_gap, &msg->fgd_data[0],
			sizeof(fg_bat_tmp_int_gap));
	...
	{
		memcpy(&bat_cycle_thr,
			&msg->fgd_data[0], sizeof(bat_cycle_thr));
	...
	case FG_DAEMON_CMD_GET_HW_INFO:
	{
		...
		intr_no = gauge_dev_get_hw_status(gauge_dev,
			&FG_status, intr_no);
		bm_trace(
	...
	case FG_DAEMON_CMD_SET_KERNEL_SOC:
	{
		...
		bm_debug(
			"[fr] FG_DAEMON_CMD_SET_KERNEL_SOC = %d %d\n",
	...
	case FG_DAEMON_CMD_SET_KERNEL_UISOC:
	{
		...
		bm_debug(
			"[fr] FG_DAEMON_CMD_SET_KERNEL_UISOC = %d %d GM3:%d\n",
	...
	case FG_DAEMON_CMD_SET_CAR_TUNE_VALUE:
	{
		...
		bm_notice("[fr] cali_car_tune = %d, default = %d, Use [cali_car_tune]\n",
			cali_car_tune, fg_cust_data.car_tune_value);
		fg_cust_data.car_tune_value = cali_car_tune;
#else
		bm_notice("[fr] cali_car_tune = %d, default = %d, Use [default]\n",
			cali_car_tune, fg_cust_data.car_tune_value);
#endif
	}
	break;
...
void fg_bat_temp_int_init(void)
{
	...
	gauge_dev_enable_battery_tmp_lt_interrupt(
		gauge_dev, true, fg_bat_new_lt);
	gauge_dev_enable_battery_tmp_ht_interrupt(
		gauge_dev, true, fg_bat_new_ht);
#endif
}
...
void fg_drv_update_hw_status(void)
{
	...
	gauge_dev_get_boot_battery_plug_out_status(
		gauge_dev, &plugout_status, &bat_plugout_time);
	...
	bm_err("car[%d,%ld,%ld,%ld,%ld] c:%d %d vbat:%d vbus:%d soc:%d %d gm3:%d %d %d %d\n",
		fg_coulomb, coulomb_plus.end,
		coulomb_minus.end, soc_plus.end,
		soc_minus.end,
		fg_current_state, fg_current, bat_vol, chr_vol,
		battery_get_soc(), battery_get_uisoc(),
		gDisableGM30, fg_cust_data.disable_nafg,
		ntc_disable_nafg, cmd_disable_nafg);
	...
	case 706:
		{
			...
			bm_err(
				"exe_BAT_EC cmd %d, param %d, poweron_system_iboot\n",
				cmd, param);
		}
		break;
	case 707:
					 ...
					 const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		bm_err(
			"[FG_daemon_log_level] gFG_daemon_log_level=%d\n",
			gFG_daemon_log_level);
	}
	return size;
}
...
signed int battery_meter_meta_tool_cali_car_tune(int meta_current)
{
	...
	ret = gauge_dev_enable_car_tune_value_calibration(
		gauge_dev, meta_current, &cali_car_tune);
...
long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int *user_data_addr;
	int *naram_data_addr;
	int i = 0;
	...
	case Get_META_BAT_SOC:
		user_data_addr = (int *)arg;
		ret = copy_from_user(adc_in_data, user_data_addr, 8);
		BMT_status.UI_SOC2 = battery_get_uisoc();
...
int adc_cali_open(struct inode *inode, struct file *file)
{
	return 0;
}

int adc_cali_release(struct inode *inode, struct file *file)
{
	return 0;
}
...
/*****************************/
	...
	if (shutdown_time == NULL) {
		...
		bm_err(" shutdown_time = %s len %d shutdown_time_tmp %s pl_shutdown_time[%d]\n",
			...
			shutdown_time_tmp, pl_shutdown_time);
	}
