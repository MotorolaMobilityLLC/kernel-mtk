...
#define BAT_SN_LEN 12
#define BAT_SN_SIZE 10

static char *bat_id_ser[] = {
	"SB18D45495",
	"SB18D34202",
	"SB18D60229",
	"SB18D60228",
...
static int get_battery_serialnumber(void)
{
	char batword[] = "androidboot.battid=";
	char *ptr = NULL, *ptr_e = NULL;
	char battsn_buf[BAT_SN_LEN] = {0};
	int size = 0, i = 0, ret = 0;

	ptr = strstr(saved_command_line, batword);
	if (ptr != 0) {
		ptr_e = strstr(ptr, " ");
		if (ptr_e == 0)
			goto end;

		size = ptr_e - (ptr + strlen(batword));
		if (size <= 0)
			goto end;
		if(size > BAT_SN_SIZE)
			size = BAT_SN_SIZE;
		strncpy(battsn_buf, ptr + strlen(batword), size);
		battsn_buf[size] = '\0';
	}

	ret = sizeof(bat_id_ser)/sizeof(bat_id_ser[0]);
	pr_err("bat_id_ser size  = %d\n", ret);
	for (i = 0; i < ret; i++) {
		if(!strncmp(battsn_buf, bat_id_ser[i], 10)) {
			break;
		}
	}
	if (i == 4) {
		i = 0;//compatible with SB18D45495
	}
	if (i == 5 || i == 6 || i == 7) {
		i = 1;//compatible with ATL NH50
	}
	if (i == 8) {
		i = 3;//compatible with ATL PH50 SB18D60228
	}
	if (i >= ret) {
		pr_err("not find bat_id, use Default Serial Number 0\n");
		i = 0;
	}
...
void fgauge_get_profile_id(void)
{
	if (get_ec()->debug_bat_id_en == 1)
		...
		gm.battery_id = get_battery_serialnumber();//BATTERY_PROFILE_ID;
