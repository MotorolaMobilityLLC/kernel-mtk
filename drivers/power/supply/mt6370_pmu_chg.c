...
#if IS_ENABLED(CONFIG_MT6370_APPLE_SAMSUNG_TA_SUPPORT)
	ret = mt6370_detect_apple_samsung_ta(chg_data);
	if (ret < 0)
		dev_dbg(chg_data->dev, "%s: detect a/ss ta fail(%d)\n",
			__func__, ret);
#endif
	...
	int *tchg_max)
{
	...
	while (adc_temp >= 120 && retry_cnt > 0) {
		dev_dbg(chg_data->dev, "%s: [WARNING] t = %d\n",
			__func__, adc_temp);
	...
	struct mt6370_pmu_charger_data *chg_data)
{
	...
	if (of_property_read_u32(np, "lbp_hys_sel", &chg_desc->lbp_hys_sel) < 0)
		dev_dbg(chg_data->dev, "%s: no lbp_hys_sel\n", __func__);

	if (of_property_read_u32(np, "lbp_dt", &chg_desc->lbp_dt) < 0)
		dev_dbg(chg_data->dev, "%s: no lbp_dt\n", __func__);
...
static int mt6370_chg_init_setting(struct mt6370_pmu_charger_data *chg_data)
{
	...
	if (ret < 0)
		dev_dbg(chg_data->dev, "%s: set otg lbp fail\n", __func__);

	ret = mt6370_disable_vlgc(chg_data, chg_desc->disable_vlgc);
	if (ret < 0)
		dev_dbg(chg_data->dev, "%s: set vlgc fail\n", __func__);
...
static int mt6370_pmu_charger_probe(struct platform_device *pdev)
{
	...
	if (!chg_data->regmap) {
		dev_dbg(chg_data->dev, "Fail to get parent regmap\n");
		ret = -ENODEV;
