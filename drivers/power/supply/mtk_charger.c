...
//pony.ma, DATE20210406, stop charging when reach set soc on demomode
#ifdef CONFIG_TINNO_DEMOMODECHG_CONTROL
int demomode_enable = 0;
int demomode_max_soc = 100;
int demomode_min_soc = 0;
static int mtk_chg_demomode_en_show(struct seq_file *m, void *data)
{
	seq_printf(m, "demomode_en = %d\n", demomode_enable);
	return 0;
}

static int mtk_chg_demomode_en_open(struct inode *node, struct file *file)
{
	return single_open(file, mtk_chg_demomode_en_show, PDE_DATA(node));
}

static ssize_t mtk_chg_demomode_en_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int len = 0, enable= 0;
	char desc[32] = {0};
	chr_err("mtk_chg_demomode_en_write\n");
	...
	if (sscanf(desc, "%d", &enable) == 1) {
		demomode_enable = enable;
		chr_err("demomode_enable_write enable=%d,cmd_discharging=%d\n", demomode_enable);
		return count;
	}

	chr_err("bad argument, echo [enable] > demomode_en\n");
	return count;
}

static const struct file_operations mtk_chg_demomode_en_fops = {
	.owner = THIS_MODULE,
	.open = mtk_chg_demomode_en_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write = mtk_chg_demomode_en_write,
};

static int mtk_chg_demomode_max_soc_show(struct seq_file *m, void *data)
{
	seq_printf(m, "demomode_max_soc = %d\n", demomode_max_soc);
	return 0;
}

static int mtk_chg_demomode_max_soc_open(struct inode *node, struct file *file)
{
	return single_open(file, mtk_chg_demomode_max_soc_show, PDE_DATA(node));
}

static ssize_t mtk_chg_demomode_max_soc_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int len = 0, max_soc= 0;
	char desc[32] = {0};
	chr_err("mtk_chg_demomode_max_soc_write\n");
    ...
    {
		demomode_max_soc = max_soc;
        chr_err("demomode_max_soc = %d\n", demomode_max_soc);
		return count;
    }

	chr_err("bad argument, echo [max_soc] > demomode_max_soc\n");
	return count;
}

static const struct file_operations mtk_chg_demomode_max_soc_fops = {
	.owner = THIS_MODULE,
	.open = mtk_chg_demomode_max_soc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write = mtk_chg_demomode_max_soc_write,
};

static int mtk_chg_demomode_min_soc_show(struct seq_file *m, void *data)
{
	seq_printf(m, "demomode_min_soc = %d\n", demomode_min_soc);
	return 0;
}

static int mtk_chg_demomode_min_soc_open(struct inode *node, struct file *file)
{
	return single_open(file, mtk_chg_demomode_min_soc_show, PDE_DATA(node));
}

static ssize_t mtk_chg_demomode_min_soc_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int len = 0, min_soc= 0;
	char desc[32] = {0};
	chr_err("mtk_chg_demomode_min_soc_write\n");
    ...
    {
		demomode_min_soc = min_soc;
        chr_err("demomode_min_soc = %d\n", demomode_min_soc);
		return count;
    }

	chr_err("bad argument, echo [min_soc] > demomode_min_soc\n");
	return count;
}

static const struct file_operations mtk_chg_demomode_min_soc_fops = {
	.owner = THIS_MODULE,
	.open = mtk_chg_demomode_min_soc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write = mtk_chg_demomode_min_soc_write,
};
#endif
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	//pony.ma, DATE20210406, stop charging when reach set soc on demomode
	#ifdef CONFIG_TINNO_DEMOMODECHG_CONTROL
	entry = proc_create_data("demomode_en", 0644, battery_dir,
			&mtk_chg_demomode_en_fops, info);
	if (!entry) {
		ret = -ENODEV;
		goto fail_procfs;
	}
	entry = proc_create_data("demomode_max_soc", 0644, battery_dir,
			&mtk_chg_demomode_max_soc_fops, info);
	if (!entry) {
		ret = -ENODEV;
		goto fail_procfs;
	}
	entry = proc_create_data("demomode_min_soc", 0644, battery_dir,
			&mtk_chg_demomode_min_soc_fops, info);
	if (!entry) {
		ret = -ENODEV;
		goto fail_procfs;
	}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (strstr(boot_command_line, "primary_chg=eta6963")) {
		...
		pr_info("primary charge is eta6963, just create files\n");
		return 0;
	}
	...
	//	"charger");
	info->chg_psy= power_supply_get_by_name("mtk_charger_type");
