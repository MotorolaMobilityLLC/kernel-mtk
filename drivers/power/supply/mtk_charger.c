...
void mmi_charge_rate_check(struct mtk_charger *info)
{
	...
	rp_level = adapter_dev_get_property(info->pd_adapter, TYPEC_RP_LEVEL);
        ...
        if ((info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30) && ((icl <= 500)
...
bool mmi_get_fg_force_notify_full(void)
{
	...
	pr_info("[%s]force_notify_gauge_full is %d\n", __func__, gauge->gm->fg_cust_data.force_notify_gauge_full);

	return gauge->gm->fg_cust_data.force_notify_gauge_full;
}
...
static int parse_mmi_dt(struct mtk_charger *info, struct device *dev)
{
	...
	if (of_find_property(node, "mmi,mmi-normal-zones", &byte_len)) {
		...
		for (i = 0; i < info->mmi.num_normal_zones; i++) {
			pr_info("normal:Zone %d,Temp %d,Volt %d,Ich %d", i,
