...
#if defined(CONFIG_MOTO_SWQC_SUPPORT)
		if (qc_chg_type == USB_TYPE_QC3P_45 || qc_chg_type == USB_TYPE_QC3P_27) {
...
#elif defined(CONFIG_MTK_PE_AND_FFC_DETACH)
					!((info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) || chg_alg_pe_get_connect(pe_cable)))) {
#else
					!(info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO))) {
#endif
/*
...
#ifdef CONFIG_MTK_PE_AND_FFC_DETACH
				if ((current_charge_state == STEP_MAX || current_charge_state == STEP_NORM) && !(chg_alg_pe_get_connect(pe_cable))){
#else
