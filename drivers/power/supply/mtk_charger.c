...
static int mtk_chg_en_battery_protect_open(struct inode *node, struct file *file)
{
        return single_open(file, mtk_chg_en_battery_protect_show, PDE_DATA(node));
}
...
static const struct file_operations mtk_chg_en_battery_protect_fops = {
        .owner = THIS_MODULE,
        .open = mtk_chg_en_battery_protect_open,
        .read = seq_read,
        .llseek = seq_lseek,
        .release = single_release,
        .write = mtk_chg_en_battety_protect_write,
};
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	if (!entry) {
        ...
        entry = proc_create_data("en_battery_protect", 0644, battery_dir,
