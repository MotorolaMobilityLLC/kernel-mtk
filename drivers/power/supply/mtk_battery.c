...
{
	const char *battery_info = NULL;
	struct device_node *battery_id = of_find_node_by_path("/firmware/android");

	if (battery_id)
		of_property_read_string(battery_id, "battery_info", &battery_info);
	if (battery_info && (!strncmp(battery_info, "P104-CDC001-000", 15) || !strncmp(battery_info, "SB18D34205", 10)))
		gm->battery_id = 1;
	else if (battery_info && !strncmp(battery_info, "SB18D40666", 10))
		gm->battery_id = 2;
	else
		gm->battery_id = 0;
...
int BattThermistorConverTemp(struct mtk_battery *gm, int Res)
{
	...
	if (Res >= ptable[0].TemperatureR) {
		TBatt_Value = -400;
	} else if (Res <= ptable[110].TemperatureR) {
		TBatt_Value = 700;
	} else {
...
int force_get_tbat_internal(struct mtk_battery *gm, bool update)
{
	...
	if (update == true || pre_bat_temperature_val == -1) {
		...
		bm_notice("[%s] BATTERY_SHUTDOWN_TEMPERATURE = %d\n",
			__func__,BATTERY_SHUTDOWN_TEMPERATURE);
...
void fg_custom_init_from_header(struct mtk_battery *gm)
{
	...
	for (i = 0; i < MAX_TABLE; i++) {
		...
		bm_err("g_FG_PSEUDO100: %d, g_Q_MAX: %d, g_Q_MAX_H_CURRENT: %d\n",
			g_FG_PSEUDO100[i][gm->battery_id],
			g_Q_MAX[i][gm->battery_id],
			g_Q_MAX_H_CURRENT[i][gm->battery_id]);
...
int battery_psy_init(struct platform_device *pdev)
{
	...
	//						 "charger");
	gm->bs_data.chg_psy = power_supply_get_by_name("mtk_charger_type");
