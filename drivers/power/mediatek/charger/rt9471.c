...
static u8 rt9471_irq_maskall[RT9471_IRQIDX_MAX] = {
	0xFF, 0xFE, 0xF3, 0xE7,
};

static u32 rt9471_wdt[] = {
	0, 40, 80, 160,
};
...
{
	struct rt_regmap_properties *prop;
...
static inline u32 rt9471_closest_value(u32 min, u32 max, u32 step, u8 reg_val)
{
	u32 ret_val;
...
static int rt9471_sw_workaround(struct rt9471_chip *chip)
{
	...
	if (ret < 0) {
		dev_notice(chip->dev, "%s enter hidden mode fail(%d)\n",
				      __func__, ret);
		return ret;
	}
...
static int rt9471_init_setting(struct rt9471_chip *chip)
{
	...
	ret = rt9471_i2c_block_write(chip, RT9471_REG_MASK0,
		ARRAY_SIZE(rt9471_irq_maskall), rt9471_irq_maskall);
	if (ret < 0) {
...
static int __rt9471_dump_registers(struct rt9471_chip *chip)
{
	...
	u8 stats[RT9471_STATIDX_MAX] = {0}, regval;
...
static int rt9471_is_charging_done(struct charger_device *chg_dev, bool *done)
{
	...
	ret = __rt9471_get_ic_stat(chip, &stat);
	if (ret < 0)
		return ret;
	*done = (stat == RT9471_ICSTAT_CHGDONE);
	return ret;
}
			...
			const struct i2c_device_id *id)
{
	...
	chip->chg_dev = charger_device_register(
			chip->desc->chg_name, chip->dev, chip, &rt9471_chg_ops,
			&chip->chg_props);
