...
#define PCA_MTK_CHG_VERSION	"1.0.0_MTK"
...
static inline int to_chgclass_adc(enum prop_chgalgo_adc_channel chan)
{
	...
	case PCA_ADCCHAN_BIFVBAT:
	case PCA_ADCCHAN_BIFTBAT:
	default:
		break;
	}
	return -ENOTSUPP;
...
static bool pca_mtk_chg_is_bif_exist(struct prop_chgalgo_device *pca)
{
	return false;
}
...
static struct prop_chgalgo_chg_ops pca_chg_ops = {
	...
	.is_bif_exist = pca_mtk_chg_is_bif_exist,
...
static int pca_mtk_chg_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MTK_CHGTYP_MAX; i++) {
		...
		if (!info->chgdev[i])
			dev_info(info->dev, "%s get %s fail\n", __func__,
				 mtk_chgtyp_name[i]);
	}
	...
	if (!info->chg_consumer) {
		dev_info(info->dev, "%s get charger consumer fail\n", __func__);
		return -ENODEV;
	}

	ret = pca_mtk_chg_register(info);
	if (ret < 0) {
		dev_info(info->dev, "%s register pca fail(%d)\n", __func__,
			 ret);
		return ret;
	}
