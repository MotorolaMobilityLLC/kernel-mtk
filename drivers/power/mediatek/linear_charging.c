...
/*baidabin modified for low temp charger current*/
			...
			if((BMT_status.temperature >= batt_cust_data.min_charge_temperature)&&(BMT_status.temperature <= 15)) {
				...
				if(g_bcct_flag !=1) //baidabin modified for thermal
				g_temp_CC_value = batt_cust_data.ac_charger_current;
			}
#else
			g_temp_CC_value = batt_cust_data.ac_charger_current;
...
/*baidabin modified for high temp charger volt*/
			...
			{
				if((BMT_status.temperature >= 45)&&(BMT_status.temperature <= batt_cust_data.max_charge_temperature))
				{
					cv_voltage = BATTERY_CV_VOLT_HIGH_TEMP;
				}
				else			
				{
					cv_voltage = BATTERY_VOLT_04_350000_V;	
...
/*baidabin modified for high temp charger volt*/
#ifdef WT_BAT_CHARGE_CV_TEMP_LIMIT
	if(((BMT_status.temperature >= 45)&&(BMT_status.bat_vol > BATTERY_TOPOFF_VOLT_HIGH_TEMP))
		|| ((BMT_status.temperature < 45)&&(BMT_status.bat_vol > v_cc2topoff_threshold)))
	{
...
/*baidabin modified for high temp charger volt*/
#ifdef WT_BAT_CHARGE_CV_TEMP_LIMIT
	if(((BMT_status.temperature >= 45)&&(BMT_status.bat_vol < BATTERY_RECHARGE_VOLT_HIGH_TEMP))||
		   ((BMT_status.temperature < 45)&&(BMT_status.bat_vol < batt_cust_data.recharging_voltage)))
#else	
