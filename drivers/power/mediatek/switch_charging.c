...
unsigned int get_constant_voltage(void)
{
	unsigned int cv;
...
{
	if (pe20_check_chr_type == KAL_TRUE &&
		...
		battery_log(BAT_LOG_CRTI, "[PE+20]Starting PE+20 Adaptor detection %d %d %d %d\n"
			, pe20_check_chr_type == KAL_TRUE, (BMT_status.charger_type == STANDARD_CHARGER)
			, batt_cust_data.ta_start_battery_soc <= BMT_status.SOC
			, batt_cust_data.ta_stop_battery_soc > BMT_status.SOC);
		...
		if (pe20_cable_out_occur == KAL_TRUE)
			pe20_check_chr_type = KAL_TRUE;
		else
...
static void battery_pump_express_charger_check(void)
{
	if (ta_check_chr_type == KAL_TRUE &&
		...
		/* need to re-check if the charger plug out during ta detector */
		if (ta_cable_out_occur == KAL_TRUE)
			ta_check_chr_type = KAL_TRUE;
		else
...
static void battery_pump_express_algorithm_start(void)
{
	...
	if (is_ta_connect == KAL_TRUE) {
		...
		} else if (batt_cust_data.ta_stop_battery_soc < BMT_status.SOC) {
