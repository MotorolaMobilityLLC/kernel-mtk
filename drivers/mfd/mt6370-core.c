...
static int mt6370_pmu_probe(struct i2c_client *client)
{
	...
	if ((mpd->chip_rev != MT6370_VENDOR_ID) && (mpd->chip_rev != MT6371_VENDOR_ID) &&
		...
		dev_dbg(&client->dev, "Device not supported\n");
		return -ENODEV;
	}
	...
	if (ret) {
		dev_dbg(&client->dev, "Failed to add Regmap IRQ Chip\n");
		return ret;
	}
