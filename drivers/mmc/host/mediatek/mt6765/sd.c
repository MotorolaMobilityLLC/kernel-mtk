...
#ifdef CONFIG_MTK_HW_FDE
#define msdc_dma_on()           { msdc_fde(mmc, mrq); \
...
void msdc_dump_register_core(struct msdc_host *host, struct seq_file *m)
{
	...
	MSDC_RST_REG_PRINT_BUF(msg_size, PRINTF_REGISTER_BUFFER_SIZE, buffer,
		buffer_cur_ptr);
...
void msdc_detect_bad_sd(struct msdc_host *host, u32 condition)
{
	...
	&& time_after(time_current, (bad_sd_timer[condition]
	+ bad_sd_forget[condition] * HZ)))
	...
	if (++(bad_sd_detecter[condition]) >= bad_sd_tolerance[condition]) {
		...
		pr_notice(
			"msdc1: detected bad sd, just power off for bad sd\n");
	}
}
...
void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	pr_notice("msdc%d -> !!! Set<%dKHz> Source<%dKHz> -> sclk<%dKHz> ",
		host->id, hz/1000, hclk/1000, sclk/1000);
	pr_notice("timing<%d> mode<%d> div<%d> hs400_div_dis<%d>",
		(int)timing, mode, div, hs400_div_dis);
	...
	if (host->power_mode == MMC_POWER_OFF && mode != MMC_POWER_OFF) {
		...
		if (is_card_sdio(host) || (host->hw->flags & MSDC_SDIO_IRQ)) {
			...
			if (host->hw->host_function == MSDC_SD) {
				 ...
				 * sdcard_reset_tuning()
				 * or else it will be set as block_bad_card
				 * when power cycle
				 */
	...
	struct mmc_command *cmd)
{
	if (g_flush_error_happened &&
		...
		if (g_flush_error_count >= MSDC_MAX_FLUSH_COUNT) {
			...
			pr_notice(
			"msdc%d:flush cache error count = %d, Disable cache\n",
		...
		if (host->autocmd & MSDC_AUTOCMD12) {
			...
			if (sbc) {
				...
				if (reg_blk_num != (sbc->arg & 0xFFFF))
					pr_notice(
"msdc%d: acmd23 arg(0x%x) fail to match block num(0x%x), SDC_BLK_NUM(0x%x)\n",
						host->id, sbc->arg,
	...
	case SD_IO_RW_EXTENDED:
		...
		if (cmd->data->flags & MMC_DATA_READ) {
			if ((cmd->data->blocks * host->blksz) > 256)
				MSDC_SET_FIELD(EMMC50_CFG0,
				MSDC_EMMC50_CFG_ENDBIT_CNT, (host->blksz + 17));
			else
				MSDC_SET_FIELD(EMMC50_CFG0,
					MSDC_EMMC50_CFG_ENDBIT_CNT, 273);
		} else {
			MSDC_SET_FIELD(EMMC50_CFG0, MSDC_EMMC50_CFG_ENDBIT_CNT,
				73);
		}
		break;
	case SD_IO_RW_DIRECT:
	...
	unsigned long       timeout)
{
	...
	while (1) {
		...
		if (time_after(jiffies, tmo)) {
			pr_notice(
"[%s]: msdc%d CMD<%d> polling_for_completion timeout ARG<0x%.8x>",
...
"[%s]: msdc%d XXX CMD<%d> resp<0x%.8x>, write protection violation\n",
				...
				if ((*rsp & R1_OUT_OF_RANGE)
				 && (host->hw->host_function != MSDC_SDIO)) {
					pr_notice(
"[%s]: msdc%d XXX CMD<%d> arg<0x%.8x> resp<0x%.8x>, out of range\n",
				__func__, host->id, cmd->opcode, cmd->arg,
						*rsp);
				}
			}
		...
		if ((cmd->opcode != 19) && (cmd->opcode != 21)) {
			pr_notice(
"[%s]: msdc%d CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>",
				__func__, host->id, cmd->opcode, cmd->arg);
			if (host->hw->host_function == MSDC_SDIO)
				msdc_dump_info(host->id);
		}
		if (((mmc_resp_type(cmd) == MMC_RSP_R1B) || (cmd->opcode == 13))
			&& (host->hw->host_function != MSDC_SDIO)) {
			pr_notice(
"[%s]: msdc%d CMD<%d> ARG<0x%.8X> is R1B, CRC not reset hw\n",
				__func__, host->id, cmd->opcode, cmd->arg);
		} else {
			msdc_reset_hw(host->id);
		}
...
"[%s]: msdc%d CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x>",
		...
		if (cmd->opcode == 13) {
			...
			pr_notice(
"%s: %d: CMD%d cmd13_timeout_cont = %d\n", __func__, __LINE__,
		...
		if (((mmc_resp_type(cmd) == MMC_RSP_R1B) || (cmd->opcode == 13))
			&& (host->hw->host_function != MSDC_SDIO)) {
			pr_notice(
"[%s]: msdc%d XXX CMD<%d> ARG<0x%.8X> is R1B, TMO not reset hw\n",
				__func__, host->id, cmd->opcode, cmd->arg);
		} else {
			msdc_reset_hw(host->id);
		}
...
"[%s]: msdc%d CMD<%d> polling_for_completion timeout ARG<0x%.8x>",
	...
	if (intsts & cmdsts) {
		if (intsts & MSDC_INT_CMDRDY) {
			...
			cmd->error = (unsigned int)-EILSEQ;
			pr_notice(
"[%s]: msdc%d XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>",
				__func__, host->id, cmd->opcode, cmd->arg);
			/* msdc_dump_info(host->id); */
		} else if (intsts & MSDC_INT_CMDTMO) {
			cmd->error = (unsigned int)-ETIMEDOUT;
			pr_notice(
"[%s]: msdc%d XXX CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x>",
				__func__, host->id, cmd->opcode, cmd->arg);
			mmc_cmd_dump(host->mmc);
			msdc_dump_info(host->id);
		}
...
"[msdc%d] MSDC_INT_DATCRCERR (0x%x), Left DAT: %d bytes\n",
		...
		ERR_MSG(
"[msdc%d] MSDC_INT_DATTMO (0x%x), Left DAT: %d bytes\n",
			host->id, ints, left);
		msdc_dump_info(host->id);
		data->error = (unsigned int)-ETIMEDOUT;
	}
...
static void msdc_fde_switch_config(struct msdc_host *host, u32 block_address,
	u32 dir)
{
	void __iomem *base = host->base;
...
int msdc_do_request_prepare(struct msdc_host *host, struct mmc_request *mrq)
{
	...
	if (msdc_txfifocnt() || msdc_rxfifocnt()) {
		pr_notice(
"[SD%d] register abnormal, please check! fifo = 0x%x, content = 0x%x\n",
...
int msdc_error_tuning(struct mmc_host *mmc,  struct mmc_request *mrq)
{
	...
	pr_notice("msdc%d saved device status: %x",
		host->id, host->device_status);
	/* clear device status */
