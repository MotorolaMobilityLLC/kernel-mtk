...
int autok_res_check(u8 *res_h, u8 *res_l)
{
	...
	for (i = 0; i < TUNING_PARAM_COUNT; i++) {
		if ((i == CMD_RD_D_DLY1) || (i == DAT_RD_D_DLY1)) {
			if ((res_h[i] > res_l[i]) && (res_h[i] - res_l[i]
			> SDIO_AUTOK_DIFF_MARGIN))
				ret = -1;
			if ((res_l[i] > res_h[i]) && (res_l[i] - res_h[i]
			> SDIO_AUTOK_DIFF_MARGIN))
				ret = -1;
...
void sdio_execute_dvfs_autok_mode(struct msdc_host *host, bool ddr208)
{
	...
	if (autok_res_check(host->autok_res[AUTOK_VCORE_LEVEL3],
		...
		/* Enable HW DVFS, but setting used now is at register
		 * offset <=0x104.
		 * Setting at register offset >=0x300 will effect after
		 * SPM handshakes with MSDC.
		 */
...
void sdio_plus_set_device_rx(struct msdc_host *host)
{
	...
	msdc_retry(!(MSDC_READ32(MSDC_CFG) & MSDC_CFG_CKSTB), retry, cnt,
		host->id);
...
int sdio_plus_set_device_ddr208(struct msdc_host *host)
{
	...
	if (err) {
		autok_execute_tuning(host, autok_res104);
		err = msdc_io_rw_direct_host(mmc, 0, 0, SDIO_CCCR_MTK_DDR208,
			0, &data);
	}
...
void msdc_dump_autok(struct msdc_host *host)
{
	...
	for (i = AUTOK_VCORE_LEVEL1; i >= AUTOK_VCORE_LEVEL0; i--) {
		...
		/* cmd response use ds pin, but window is different with data
		 * pin, because cmd response is SDR.
		 */
		start = DS_CMD_SCAN_0;
		for (j = 0; j < 64; j++) {
			bit_pos = j % 8;
