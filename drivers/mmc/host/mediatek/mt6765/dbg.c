...
"%d [%5llu.%06llu]%2d %2d 0x%08x tag=%d type=%s %s %s\n",
		...
		((dbg_run_host_log_dat[i].arg >> 24) & 0x1) ? "fprg" : NULL
			...
			&& !dbg_run_host_log_dat[i].type) {
			tag = (dbg_run_host_log_dat[i].arg >> 16) & 0xf;
			pr_notice("%d [%5llu.%06llu]%2d %2d 0x%08x tag=%d\n",
				i, dbg_run_host_log_dat[i].time_sec,
				dbg_run_host_log_dat[i].time_usec,
				dbg_run_host_log_dat[i].type,
				dbg_run_host_log_dat[i].cmd,
			...
			pr_notice("%d [%5llu.%06llu]%2d %2d 0x%08x\n",
				i, dbg_run_host_log_dat[i].time_sec,
				dbg_run_host_log_dat[i].time_usec,
				dbg_run_host_log_dat[i].type,
				dbg_run_host_log_dat[i].cmd,
...
void emmc_cq_state(void)
{
	...
	for (i = 0; i < 31; i++)
		MET_PRINTK("%d,", dbg_cq_state[i]);
	MET_PRINTK("%d", dbg_cq_state[31]);
}
	...
	uint type, uint compare)
{
...
#ifdef CONFIG_MTK_EMMC_CQ_SUPPORT
	is_cmdq_en = false;
	if (host_num == 0 && host_ctl->mmc->card
	&& host_ctl->mmc->card->ext_csd.cmdq_mode_en) {
	...
	for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN; forIndex++) {
		if (rPtr[forIndex] != wData[forIndex % wData_len]) {
			pr_notice(
			"index[%d]\tW_buffer[0x%x]\tR_buffer[0x%x]\tfailed\n",
...
void msdc_dump_gpd_bd(int id)
{
	...
	if (gpd == NULL) {
...
#endif
		pr_notice("hwo:0x%x, bdp:0x%x, rsv0:0x%x, chksum:0x%x,",
			gpd->hwo, gpd->bdp, gpd->rsv0, gpd->chksum);
		pr_notice("intr:0x%x,rsv1:0x%x,nexth4:0x%x,ptrh4:0x%x\n",
		...
		pr_notice("arg:0x%x,blknum:0x%x,cmd:0x%x\n",
			gpd->arg, gpd->blknum, gpd->cmd);
	}
	...
	if (bd == NULL) {
		...
		for (i = 0; i < host->dma.sglen; i++) {
			pr_notice("%4d BD eol:0x%x, rsv0:0x%x, chksum:0x%x, ",
				i, bd->eol, bd->rsv0, bd->chksum);
			pr_notice("rsv1:0x%x,blkpad:0x%x,dwpad:0x%x,rsv2:0x%x\n",
				bd->rsv1, bd->blkpad, bd->dwpad, bd->rsv2);
			pr_notice("nexth4:0x%x, ptrh4:0x%x, ",
				(unsigned int)bd->nexth4, (unsigned int)bd->ptrh4);
			pr_notice("next:0x%x, ptr:0x%x, buflen:0x%x, rsv3:0x%x\n",
		...
		struct msdc_host *host, int enable)
{
	...
	if (c_ctrl == enable)
		...
		if (err)
			seq_printf(m,
			"msdc%d: Cache is supported, but %s failed\n",
				host->id, enable ? "enable" : "disable");
		else
	...
	struct mmc_command *sbc, u32 *intsts)
{
	...
	if ((g_err_tune_dbg_cmd != MMC_SWITCH)
	 || ((g_err_tune_dbg_cmd == MMC_SWITCH) &&
		...
		pr_notice("[%s]: got the error cmd:%d, arg=%d,",
			__func__, g_err_tune_dbg_cmd, g_err_tune_dbg_arg);
		pr_notice(" dbg error=%d, cmd->error=%d, count=%d\n",
	...
	u32 *intsts)
{
	...
	if ((g_err_tune_dbg_cmd == MMC_STOP_TRANSMISSION) &&
		...
		pr_notice("[%s]: got the error cmd:%d, dbg error 0x%x,",
			__func__, g_err_tune_dbg_cmd, g_err_tune_dbg_error);
		pr_notice(" stop->error=%d, host->error=%d, count=%d\n",
...
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	...
	if (cmd == -1) {
		seq_puts(m, "Please run: echo cmd [p1] [p2] [p3] [p4]");
		seq_puts(m, " [p5] [p6] [p7] [p8]> /proc/msdc_debug\n");
	...
	if (cmd == SD_TOOL_ZONE) {
		...
		if (p1 == 1) {
			...
			if ((unsigned char)p3 > 7 || (unsigned char)p4 > 7 ||
			    ...
			    (unsigned char)p7 > 7) {
				seq_puts(m,
		"[SD_Debug]Some drving value was invalid (valid:0~7)\n");
				goto out;
			}
		...
		seq_printf(m, "cmd_drv=%d, dat_drv=%d, rst_drv=%d, ds_drv=%d\n",
		...
		if ((unsigned char)p2 > 1 || (unsigned char)p3 > 1
		 ...
		 || (unsigned char)p6 > 1) {
			seq_puts(m,
			"[SD_Debug]Some sr value was invalid(correct:0(disable),1(enable))\n");
		} else {
			msdc_set_sr(host, p2, p3, p4, p5, p6);
			seq_printf(m,
			"[SD_Debug]msdc%d, clk_sr=%d, cmd_sr=%d, dat_sr=%d, rst=%d, ds=%d\n",
				id, p2, p3, p4, p5, p6);
		}
		...
		if (p2 == 0) {
			...
			seq_puts(m, "mtk_sd.h if you want dump dma address\n");
#endif
		} else if (p2 == 1) {
		...
		host = mtk_msdc_host[id];
		pr_notice(
		"[****AutoK test****]msdc host_id<%d> vcore<%d> mode<%d>\n",
			id, vcore, mode);
