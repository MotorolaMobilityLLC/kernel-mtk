...
u64 msdc_get_user_capacity(struct msdc_host *host)
{
	...
	if (mmc_card_mmc(card)) {
		if (card->csd.read_blkbits) {
			legacy_capacity = (2 << (card->csd.read_blkbits - 1)) *
					  card->csd.capacity;
		} else {
			legacy_capacity = card->csd.capacity;
			ERR_MSG("XXX read_blkbits = 0 XXX");
		}
		capacity = (u64)(card->ext_csd.sectors) * 512 > legacy_capacity
			       ? (u64)(card->ext_csd.sectors) * 512
			       : legacy_capacity;
...
int msdc_can_apply_cache(unsigned long long start_addr, unsigned int size)
{
	if (!g_cache_part_start && !g_cache_part_end && !g_usrdata_part_start &&
	    !g_usrdata_part_end)
		return 0;
...
void msdc_get_cache_region(struct work_struct *work)
{
	...
	if (likely(lp_hd_struct)) {
		...
		pr_debug("There is no cache info\n");
	}
	...
	if (likely(lp_hd_struct)) {
		...
		g_usrdata_part_end =
		    g_usrdata_part_start + lp_hd_struct->nr_sects;
		put_part(lp_hd_struct);
	} else {
	...
	pr_debug("cache(0x%llX~0x%llX, usrdata(0x%llX~0x%llX)\n",
		 g_cache_part_start, g_cache_part_end, g_usrdata_part_start,
		 g_usrdata_part_end);
}
...
void msdc_get_otp_region(struct work_struct *work)
{
	...
	if (likely(lp_hd_struct)) {
		...
		pr_debug("There is no otp partition\n");
	}
}
		       ...
		       unsigned int size)
{
	if ((otp_part_start == 0) || (otp_part_end == 0))
		return 0;
	if ((opcode == MMC_SET_WRITE_PROT) || (opcode == MMC_CLR_WRITE_PROT) ||
	    (opcode == MMC_SEND_WRITE_PROT) || (opcode == 31)) {
		if ((start_addr > otp_part_end) ||
		    (start_addr < otp_part_start) ||
		    ((start_addr + size) > otp_part_end)) {
			pr_debug("OTP operation invalid address %llx\n",
				 start_addr);
			return 1;
		}
	}
...
struct gendisk *mmc_get_disk(struct mmc_card *card)
{
	...
	WARN_ON(!card);
	if (!card)
		pr_debug("[%s:%d] card is NULL", __func__, __LINE__);
	md = dev_get_drvdata(&card->dev);
	WARN_ON(!md || !md->disk);
	if (!md || !md->disk)
		pr_debug("[%s:%d] md or disk is NULL", __func__, __LINE__);
...
{
	proc_emmc = proc_create("emmc", 0x0, NULL, &proc_emmc_fops);
}
#endif
#endif
