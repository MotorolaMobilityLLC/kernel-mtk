...
#define AUTOK_LATCH_CK_SDIO_TUNE_TIMES                                         \
	(20) /* 4.5IP 1KB fifo CMD19 need send 20 times  */
#define AUTOK_LATCH_CK_SD_TUNE_TIMES                                           \
	(20) /* 4.5IP 1KB fifo CMD19 need send 20 times  */
...
#define autok_msdc_reset()                                                     \
	do {                                                                   \
		...
		mb();                                                          \
		autok_msdc_retry(MSDC_READ32(MSDC_CFG) & MSDC_CFG_RST, retry,  \
				 cnt);                                         \
	} while (0)
...
const struct AUTOK_PARAM_INFO autok_param_info[] = {
	...
	{{0, 7},
	 "INT_DAT_LATCH_CK"}, /* Latch CK Delay for data read when clock stop */
...
static int autok_f0_rw_direct(struct msdc_host *host, unsigned int u4Addr,
			      char *out, char in, bool write)
{
	...
	cmd.arg |= (write && out) ? 0x08000000
				  : 0x00000000; /* read after write flage */
	cmd.arg |= u4Addr << 9;
	cmd.arg |= in;
...
static int autok_f0_rw_extended(struct msdc_host *host, unsigned int u4Addr,
				void *pBuf, unsigned int byte_cnt, bool write,
				bool opmode, bool bkmode)
{
	struct mmc_request mrq = {NULL};
	struct mmc_command cmd = {0};
	...
	if (pBuf == NULL)
		pr_debug("[%s] [ERR]host = %p, memory malloc failed!!\n",
			 __func__, host);
...
static int autok_cmd53_tune_cccr_write(struct msdc_host *host)
{
	...
	do {
		...
		if (ret != 0) {
			AUTOK_RAWPRINT(
			    "[ERROR]CMD53(%d) Write CCCR0 Error: 0x%x\n", i,
			    ret);
...
static int autok_cmd53_tune_cccr_read(struct msdc_host *host)
{
	...
	do {
		...
		ret = autok_f0_rw_extended(host,
			SDIO_CCCR_CCCR, rDat, 32, 0, 0, 0);
		if (ret != 0) {
...
static int autok_simple_score(char *res_str, unsigned int result)
{
	...
	if (result == 0) {
		strcpy(
		    res_str,
		    "OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO"); /* maybe result is 0 */
		return 32;
	}
...
static int autok_simple_score64(char *res_str64, u64 result64)
{
	...
	if (result64 == 0xFFFFFFFFFFFFFFFF) {
		strcpy(res_str64, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
		return 0;
	}
			...
			case PASS_POSITION:
				...
				if (bit == 63) {
					...
					scan_res
					    ->fail_bd_info[fail_bd_info_cnt++]
					    .Bound_Start = bit;
				scan_res->pass_bd_info[pass_bd_info_cnt - 1]
				    .Bound_End = bit - 1;
				scan_res->fail_bd_cnt++;
				break;
			case FAIL_POSITION:
			...
			case FAIL_POSITION:
				...
				if (bit == 63) {
					...
					scan_res
					    ->pass_bd_info[pass_bd_info_cnt++]
					    .Bound_Start = bit;
				scan_res->fail_bd_info[fail_bd_info_cnt - 1]
				    .Bound_End = bit - 1;
				scan_res->pass_bd_cnt++;
				break;
			default:
		...
		if ((i >= 1) && ((scan_res->fail_bd_info[i].Bound_Start -
			...
			for (j = i; j < (scan_res->fail_bd_cnt - 1); j++) {
				scan_res->fail_bd_info[j].Bound_Start =
				    scan_res->fail_bd_info[j + 1].Bound_Start;
				scan_res->fail_bd_info[j].Bound_End =
				    scan_res->fail_bd_info[j + 1].Bound_End;
			}
			scan_res->fail_bd_info[scan_res->fail_bd_cnt - 1]
			    .Bound_Start = 0;
			scan_res->fail_bd_info[scan_res->fail_bd_cnt - 1]
			    .Bound_End = 0;
			scan_res->fail_bd_cnt--;
...
static int autok_pad_dly_sel(struct AUTOK_REF_INFO *pInfo)
{
	...
	struct AUTOK_SCAN_RES *pBdInfo_F =
	    NULL;			   /* scan result @ falling edge */
	struct BOUND_INFO *pBdPrev = NULL; /* Save the first boundary info for
					    *  calc optimised dly count
					    */
	struct BOUND_INFO
	    *pBdNext =
		NULL;
	...
	for (j = 0; j < 2; j++) {
		...
		if (((pBdInfo_Temp[0]->bd_cnt == 1) &&
			...
			/* can not know 1T size,need check max pass bound,select
			 * mid
			 */
			switch (pBdInfo_Temp[1]->bd_cnt) {
			case 4:
				pass_bd_size[0] =
				    pBdInfo_Temp[1]->bd_info[0].Bound_Start - 0;
				pass_bd_size[1] =
				    pBdInfo_Temp[1]->bd_info[1].Bound_Start -
				    pBdInfo_Temp[1]->bd_info[0].Bound_End;
				pass_bd_size[2] =
				    pBdInfo_Temp[1]->bd_info[2].Bound_Start -
				    pBdInfo_Temp[1]->bd_info[1].Bound_End;
				pass_bd_size[3] =
				    pBdInfo_Temp[1]->bd_info[3].Bound_Start -
				    pBdInfo_Temp[1]->bd_info[2].Bound_End;
				pass_bd_size[4] =
				    63 - pBdInfo_Temp[1]->bd_info[3].Bound_End;
				...
				if (max_pass_loca == 0)
					pInfo->opt_dly_cnt = pBdInfo_Temp[1]
								 ->bd_info[0]
								 .Bound_Start /
							     2;
				else if (max_pass_loca == 4)
					pInfo->opt_dly_cnt = (63 +
							      pBdInfo_Temp[1]
								  ->bd_info[3]
								  .Bound_End) /
							     2;
					...
					pInfo->opt_dly_cnt =
					    (pBdInfo_Temp[1]
						 ->bd_info[max_pass_loca]
						 .Bound_Start +
					     pBdInfo_Temp[1]
						 ->bd_info[max_pass_loca - 1]
						 .Bound_End) /
					    2;
				}
			break;
			case 3:
				...
				pass_bd_size[2] =
				    ...
				    pBdInfo_Temp[1]->bd_info[1].Bound_End;
				pass_bd_size[3] =
				    63 - pBdInfo_Temp[1]->bd_info[2].Bound_End;
				max_size = pass_bd_size[0];
				max_pass_loca = 0;
				for (i = 0; i < 4; i++) {
					if (pass_bd_size[i] >= max_size) {
			...
			case 2:
				...
				pass_bd_size[1] =
				    ...
				    pBdInfo_Temp[1]->bd_info[0].Bound_End;
				pass_bd_size[2] =
				    63 - pBdInfo_Temp[1]->bd_info[1].Bound_End;
				max_size = pass_bd_size[0];
				max_pass_loca = 0;
				for (i = 0; i < 3; i++) {
					if (pass_bd_size[i] >= max_size) {
			...
			case 1:
				pass_bd_size[0] =
				    pBdInfo_Temp[1]->bd_info[0].Bound_Start - 0;
				pass_bd_size[1] =
				    63 - pBdInfo_Temp[1]->bd_info[0].Bound_End;
				max_size = pass_bd_size[0];
				max_pass_loca = 0;
				for (i = 0; i < 2; i++) {
					if (pass_bd_size[i] >= max_size) {
	...
	case 2: /* mode_1 : 2 full boudary */
		...
		if (pBdPrev && pBdNext) {
			...
			uBD_width =
			    (pBdPrev->Bound_width + pBdNext->Bound_width) / 2;
	...
	case 1: /* rising edge find one full boundary */
		if (Bound_Cnt_R > 1) {
			...
			pBdPrev = &(
			    pBdInfo_R
				->bd_info[0]); /* this boundary is full bound */
			...
			if (pBdNext->Bound_Start == 0) {
				cycle_cnt =
				    (pBdPrev->Bound_End - pBdNext->Bound_End) *
				    2;
				...
				if (uBD_mid_next > uBD_mid_prev)
					cycle_cnt =
					    (uBD_mid_next - uBD_mid_prev) * 2;
				else
					cycle_cnt =
					    (uBD_mid_prev - uBD_mid_next) * 2;
			}
			...
			if (uBD_mid_prev >= cycle_cnt / 2) { /* case 1 */
				...
				uMgLost_R = 0;
			} else if (32 - uBD_mid_prev <=
				   AUTOK_MARGIN_THOLD) { /* case 4 */
				uDlySel_R = 0;
				uMgLost_R = cycle_cnt / 2 - uBD_mid_prev;
			} else { /* case 5 */
				uDlySel_R = 63;
			...
			/* mode_4: falling edge no boundary found & rising edge
			 * only one full boundary exist */
			pBdPrev = &(
			    pBdInfo_R
				->bd_info[0]); /* this boundary is full bound */
			uBD_mid_prev =
			...
			uDlySel_R = 0xFF;
			uMgLost_R =
			    0xFF; /* Margin enough donot care margin lost */
			uDlySel_F = uBD_mid_prev;
			uMgLost_F =
			    0xFF; /* Margin enough donot care margin lost */
	...
	case 0: /* rising edge cannot find full boudary */
		if (Bound_Cnt_R == 2) {
			...
			if (pBdNext->is_fullbound) {
				/* mode_5: rising_edge 2 boundary (not full
				 * bound), falling edge 1 full boundary
				 */
				uBD_width = pBdNext->Bound_width;
			...
			if (Bound_Cnt_F > 1) {
				/* when rising_edge have only one boundary (not
				 * full bound),
				 * falling edge should not more than 1Bound
				 * existthis is a corner case, rising boundary
				 * may scan miss.
				 * xooooooooooooooooo
				 * oooxooooooooxooooo
				 */
				pInfo->cycle_cnt =
				    (pBdInfo_F->bd_info[1].Bound_End +
				     pBdInfo_F->bd_info[1].Bound_Start) /
					2 -
				    (pBdInfo_F->bd_info[0].Bound_End +
				     pBdInfo_F->bd_info[0].Bound_Start) /
					2;
				pInfo->opt_edge_sel = 1;
				pInfo->opt_dly_cnt =
				    ((pBdInfo_F->bd_info[1].Bound_End +
				...
				if (pBdPrev->Bound_Start == 0) {
					...
					if (pBdPrev->Bound_End - uBD_width / 2 +
					    ...
					    63) {
						uDlySel_R = 63;
						uMgLost_R = pBdPrev->Bound_End -
							    uBD_width / 2 +
							    cycle_cnt / 2 - 63;
					} else {
						uDlySel_R = pBdPrev->Bound_End -
							    uBD_width / 2 +
							    cycle_cnt / 2;
						uMgLost_R = 0;
					}
			   ...
			   0) { /* Rising Edge No Boundary found */
			if (Bound_Cnt_F > 1) {
				...
				if (pBdPrev->is_fullbound) {
					...
					uDlySel_R = (pBdPrev->Bound_Start == 0)
							? 0
							: 63;
					uMgLost_R = 0xFF;
					uDlySel_F = 0xFF;
...
static int autok_pad_dly_sel_single_edge(struct AUTOK_SCAN_RES *pInfo,
					 ...
					 unsigned int *pDlySel)
{
	...
	if (Bound_Cnt > 1) {
		...
		if (!(pBdPrev->is_fullbound)) {
			...
			uDlySel =
			    (pBdPrev->Bound_End + pBdNext->Bound_Start) / 2;
		...
		if (pBdPrev->is_fullbound) {
			...
			if (uBD_mid_prev >= cycle_cnt_ref / 2) {
				...
				/* Left Margin not enough must need to select
				 * the right side */
				uDlySel = uBD_mid_prev + cycle_cnt_ref / 2;
				uMgLost = 0;
			} else {
	...
	*pDlySel = uDlySel;
	if (uDlySel > 31) {
		AUTOK_RAWPRINT("[AUTOK]Warning Dly Sel %d > 31 easily effected by Voltage Swing\r\n",
			       uDlySel);
	}
			    ...
			    unsigned int *pDlySel)
{
	...
	uDlySel = (pInfo->pass_bd_info[max_pass_win_position].Bound_Start +
		   pInfo->pass_bd_info[max_pass_win_position].Bound_End) /
		  2;
	*pDlySel = uDlySel;
				   ...
				   enum AUTOK_PARAM param, u32 *value, int rw)
{
	...
	case CMD_FIFO_EDGE:
		if ((rw == AUTOK_WRITE) && (*value > 1)) {
			pr_debug("[%s] Input value(%d) for CMD_FIFO_EDGE is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_CFG0;
	...
	case EMMC50_WDATA_MUX_EN:
		if ((rw == AUTOK_WRITE) && (*value > 1)) {
			pr_debug("[%s] Input value(%d) for EMMC50_WDATA_MUX_EN is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_CFG0;
	...
	case EMMC50_CMD_MUX_EN:
		if ((rw == AUTOK_WRITE) && (*value > 1)) {
			pr_debug("[%s] Input value(%d) for EMMC50_CMD_MUX_EN is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_CFG0;
	...
	case EMMC50_WDATA_EDGE:
		if ((rw == AUTOK_WRITE) && (*value > 1)) {
			pr_debug("[%s] Input value(%d) for EMMC50_WDATA_EDGE is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_CFG0;
	...
	case EMMC50_DS_Z_DLY1:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DS_Z_DLY1 is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
	...
	case EMMC50_DS_Z_DLY1_SEL:
		if ((rw == AUTOK_WRITE) && (*value > 1)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DS_Z_DLY1_SEL is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_PAD_DS_TUNE;
	...
	case EMMC50_DS_Z_DLY2:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DS_Z_DLY2 is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_PAD_DS_TUNE;
	...
	case EMMC50_CMD_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_CMD_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
	...
	case EMMC50_DAT0_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DAT0_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
	...
	case EMMC50_DAT1_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DAT1_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_PAD_DAT01_TUNE;
	...
	case EMMC50_DAT2_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DAT2_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
	...
	case EMMC50_DAT3_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DAT3_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_PAD_DAT23_TUNE;
	...
	case EMMC50_DAT4_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DAT4_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
	...
	case EMMC50_DAT5_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DAT5_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_PAD_DAT45_TUNE;
	...
	case EMMC50_DAT6_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DAT6_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
	...
	case EMMC50_DAT7_TX_DLY:
		if ((rw == AUTOK_WRITE) && (*value > 31)) {
			pr_debug("[%s] Input value(%d) for EMMC50_DAT7_TX_DLY is out of range, it should be [0~1]\n",
				 __func__, *value);
			return -1;
		}
		reg = (u32 *)EMMC50_PAD_DAT67_TUNE;
...
static int autok_result_dump(struct msdc_host *host, u8 *autok_tune_res)
{
	AUTOK_RAWPRINT(
	    ...
	    autok_tune_res[0], autok_tune_res[1], autok_tune_res[5],
	    autok_tune_res[7]);
	AUTOK_RAWPRINT(
				     ...
				     unsigned int *value, unsigned int pad_sel)
{
	...
	switch (pad_sel) {
	case CMD_PAD_RDLY:
		msdc_autok_adjust_param(host, CMD_RD_D_DLY1, &uCfgL,
					AUTOK_WRITE);
		msdc_autok_adjust_param(host, CMD_RD_D_DLY2, &uCfgH,
					AUTOK_WRITE);

		msdc_autok_adjust_param(host, CMD_RD_D_DLY1_SEL, &uCfgLSel,
					AUTOK_WRITE);
		msdc_autok_adjust_param(host, CMD_RD_D_DLY2_SEL, &uCfgHSel,
					AUTOK_WRITE);
	break;
	case DAT_PAD_RDLY:
		msdc_autok_adjust_param(host, DAT_RD_D_DLY1, &uCfgL,
					AUTOK_WRITE);
		msdc_autok_adjust_param(host, DAT_RD_D_DLY2, &uCfgH,
					AUTOK_WRITE);

		msdc_autok_adjust_param(host, DAT_RD_D_DLY1_SEL, &uCfgLSel,
					AUTOK_WRITE);
		msdc_autok_adjust_param(host, DAT_RD_D_DLY2_SEL, &uCfgHSel,
					AUTOK_WRITE);
	break;
	case DS_PAD_RDLY:
		msdc_autok_adjust_param(host, EMMC50_DS_Z_DLY1, &uCfgL,
					AUTOK_WRITE);
		msdc_autok_adjust_param(host, EMMC50_DS_Z_DLY2, &uCfgH,
					AUTOK_WRITE);

		msdc_autok_adjust_param(host, EMMC50_DS_Z_DLY1_SEL, &uCfgLSel,
					AUTOK_WRITE);
		msdc_autok_adjust_param(host, EMMC50_DS_Z_DLY2_SEL, &uCfgHSel,
					AUTOK_WRITE);
	break;
	}
}
				...
				u8 *autok_tune_res)
{
	...
	case DS_PAD_RDLY:
		...
		autok_param_update(EMMC50_DS_Z_DLY1_SEL, uCfgLSel,
				   autok_tune_res);
		autok_param_update(EMMC50_DS_Z_DLY2_SEL, uCfgHSel,
				   autok_tune_res);
	break;
	}
}
...
int autok_init_sdr104(struct msdc_host *host)
{
	...
	if (host->sclk <= 100000000) {
		...
		MSDC_SET_FIELD(MSDC_PATCH_BIT2, MSDC_PB2_CRCSTSENSEL,
			       platform_para_rx.latch_en_crc_sdr104);
		/* LATCH_TA_EN Config for CMD Path SDR104 mode */
		MSDC_SET_FIELD(MSDC_PATCH_BIT2, MSDC_PB2_RESPSTENSEL,
			       platform_para_rx.latch_en_cmd_sdr104);
	}
	/* enable dvfs feature */
	...
	do {
		...
		for (j = 0; j < 64; j++) {
			...
			for (k = 0; k < AUTOK_CMD_TIMES / 2; k++) {
				ret =
				    autok_send_tune_cmd(host, opcode, TUNE_CMD);
		...
		if (uCmdEdge)
			msdc_autok_window_apply(CMD_FALL, RawData64,
						p_autok_tune_res);
		else
			msdc_autok_window_apply(CMD_RISE, RawData64,
						p_autok_tune_res);
...
/* Step2 : Tuning DS Clk Path-ZCLK only tune DLY1 */
	...
	for (j = 0; j < 32; j++) {
		...
		for (k = 0; k < AUTOK_CMD_TIMES / 4; k++) {
			...
			if ((ret & (E_RESULT_CMD_TMO | E_RESULT_RSP_CRC)) !=
			    0) {
				AUTOK_RAWPRINT("[AUTOK]Error Autok CMD Failed while tune DATA PAD Delay\r\n");
				return -1;
			} else if ((ret &
				    (E_RESULT_DAT_CRC | E_RESULT_DAT_TMO)) != 0)
				break;
			else if ((ret & E_RESULT_FATAL_ERR) != 0)
				return -1;
		}
	...
	for (j = 0; j < 32; j++) {
		...
		for (k = 0; k < AUTOK_CMD_TIMES / 4; k++) {
			...
			if ((ret & (E_RESULT_CMD_TMO | E_RESULT_RSP_CRC)) !=
			    0) {
				AUTOK_RAWPRINT("[AUTOK]Error Autok CMD Failed while tune DS Delay\r\n");
				return -1;
	...
	for (j = 0; j < 32; j++) {
		...
		for (k = 0; k < AUTOK_CMD_TIMES / 4; k++) {
			...
			if ((ret & (E_RESULT_CMD_TMO | E_RESULT_RSP_CRC)) !=
			    0) {
				AUTOK_RAWPRINT("[AUTOK]Error Autok CMD Failed while tune DS(ZDLY) Delay\r\n");
				return -1;
			} else if ((ret & (E_RESULT_DAT_CRC |
					   E_RESULT_DAT_TMO)) != 0) {
				RawData |= (u64)(1 << j);
	...
	for (i = 0; i < BD_MAX_CNT; i++) {
		AUTOK_DBGPRINT(AUTOK_DBG_RES, "[AUTOK]BoundInf[%d]: S:%d \t E:%d \t W:%d \t FullBound:%d\r\n",
			       i, pBdInfo->bd_info[i].Bound_Start,
			       pBdInfo->bd_info[i].Bound_End,
			       pBdInfo->bd_info[i].Bound_width,
			       pBdInfo->bd_info[i].is_fullbound);
	}

	autok_ds_dly_sel(pInfo, &uDatDly);
	autok_param_update(EMMC50_DS_ZDLY_DLY, uDatDly, p_autok_tune_res);
	AUTOK_DBGPRINT(AUTOK_DBG_RES, "[AUTOK]================Analysis Result[HS400]================\r\n");
...
int execute_cmd_online_tuning(struct msdc_host *host, u8 *res)
{
	...
	if (autok_pad_dly_sel(&uCmdDatInfo) == 0) {
		...
		msdc_autok_adjust_paddly(host, &uCmdDatInfo.opt_dly_cnt,
					 CMD_PAD_RDLY);
				  ...
				  unsigned int latch_ck_initail_value)
{
	...
	for (j = latch_ck_initail_value; j < 8;
		...
		for (k = 0; k < tune_time; k++) {
			...
			if ((ret & (E_RESULT_CMD_TMO | E_RESULT_RSP_CRC)) !=
			    0) {
				AUTOK_RAWPRINT("[AUTOK]Error Autok CMD Failed "
					       "while tune LATCH CK\r\n");
				break;
			} else if ((ret & (E_RESULT_DAT_CRC |
					   E_RESULT_DAT_TMO)) != 0) {
				AUTOK_RAWPRINT("[AUTOK]Error Autok  tune "
					       "LATCH_CK error %d\r\n",
					       j);
				break;
			}
...
int execute_online_tuning_hs200(struct msdc_host *host, u8 *res)
{
	...
	do {
		...
		msdc_autok_adjust_param(host, RD_FIFO_EDGE, &uDatEdge,
					AUTOK_WRITE);
		RawData64 = 0LL;
		for (j = 0; j < 64; j++) {
			msdc_autok_adjust_paddly(host, &j, DAT_PAD_RDLY);
		...
		if (uDatEdge)
			msdc_autok_window_apply(DAT_FALL, RawData64,
						p_autok_tune_res);
		else
			msdc_autok_window_apply(DAT_RISE, RawData64,
						p_autok_tune_res);
		if (autok_check_scan_res64(RawData64, pBdInfo,
					   AUTOK_TUNING_INACCURACY) != 0) {
			host->autok_error = -1;
	...
	if (platform_para_func.new_path_hs200 == 0) {
		opcode = MMC_SEND_TUNING_BLOCK_HS200;
		p_autok_tune_res[INT_DAT_LATCH_CK] =
		    autok_execute_tuning_latch_ck(
			host, opcode, p_autok_tune_res[INT_DAT_LATCH_CK]);
	}
...
int execute_online_tuning_stress(struct msdc_host *host)
{
	...
	AUTOK_DBGPRINT(AUTOK_DBG_RES,
		       "[AUTOK]CMD_EDGE \t CMD_RD_R_DLY1~2 \r\n");
	for (i = 0; i < 32; i++) {
		...
		do {
			...
			for (j = 0; j < 64; j++) {
				msdc_autok_adjust_paddly(host, &j,
							 CMD_PAD_RDLY);
				for (k = 0; k < AUTOK_CMD_TIMES; k++) {
					ret = autok_send_tune_cmd(
					    host, MMC_SEND_TUNING_BLOCK,
					    TUNE_CMD);
					if ((ret & (E_RESULT_CMD_TMO |
						    E_RESULT_RSP_CRC)) != 0) {
						RawData64 |= (u64)(1LL << j);
			...
			score =
			    autok_simple_score64(tune_result_str64, RawData64);
			AUTOK_DBGPRINT(AUTOK_DBG_RES,
				       "[AUTOK][%d] %d \t %d \t %s\r\n", i,
				       uCmdEdge, score, tune_result_str64);
			/*get Boundary info */
			...
			AUTOK_DBGPRINT(
			    AUTOK_DBG_RES, "[AUTOK]Edge:%d \t BoundaryCnt:%d \t FullBoundaryCnt:%d \t\r\n",
			    uCmdEdge, pBdInfo->bd_cnt, pBdInfo->fbd_cnt);
			AUTOK_DBGPRINT(AUTOK_DBG_RES, "[AUTOK]BoundInf[0]: S:%d \t E:%d \t W:%d \t FullBound:%d\r\n",
				       pBdInfo->bd_info[0].Bound_Start,
				       pBdInfo->bd_info[0].Bound_End,
				       pBdInfo->bd_info[0].Bound_width,
				       pBdInfo->bd_info[0].is_fullbound);
			AUTOK_DBGPRINT(AUTOK_DBG_RES, "[AUTOK]BoundInf[1]: S:%d \t E:%d \t W:%d \t FullBound:%d\r\n",
				       pBdInfo->bd_info[1].Bound_Start,
				       pBdInfo->bd_info[1].Bound_End,
				       pBdInfo->bd_info[1].Bound_width,
				       pBdInfo->bd_info[1].is_fullbound);
			AUTOK_DBGPRINT(AUTOK_DBG_RES, "[AUTOK]BoundInf[2]: S:%d \t E:%d \t W:%d \t FullBound:%d\r\n",
		...
		AUTOK_DBGPRINT(AUTOK_DBG_RES, "[AUTOK]================Analysis Result[SD(IO)]===============\r\n");
		AUTOK_DBGPRINT(
		    AUTOK_DBG_RES,
		    "[AUTOK]Sample Edge Sel: %d, Delay Cnt Sel:%d\r\n",
...
int execute_offline_tuning_hs400(struct msdc_host *host)
{
	...
	do {
		...
		for (i = 0; i < 8; i++) {
			...
			AUTOK_DBGPRINT(
			    AUTOK_DBG_RES, "[AUTOK]%d \t %d\t %d \t %s\r\n",
			    uCmdEdge, i, g_ta_score[i], tune_result_str32);
		}

		if (autok_ta_sel(g_ta_raw, &res_cmd_ta) == 0) {
			/* autok_param_update(CMD_RSP_TA_CNTR, res_cmd_ta,
			 * p_autok_tune_res);
			 */
		...
		AUTOK_RAWPRINT("[AUTOK]Internal Boundary Occours,need switch cmd edge for rescan!\r\n");
		uCmdEdge ^=
		    1; /* TA select Fail need switch cmd edge for rescan */
	} while (uCmdEdge);
...
int execute_offline_tuning(struct msdc_host *host)
{
	...
	************************************************************/
	...
	do {
		...
		for (i = 0; i < 32; i++) {
			...
			AUTOK_DBGPRINT(
			    AUTOK_DBG_RES,
			    "[AUTOK][%d] \t %d \t %d \t %d \t %s\r\n", uCmdEdge,
			    uDatEdge, i, score, tune_result_str64);
		}
	...
	do {
		msdc_autok_adjust_param(host, WD_FIFO_EDGE, &uWCrcEdge,
					AUTOK_WRITE);
		for (i = 0; i < 32; i++) {
			RawData64 = 0LL;
			msdc_autok_adjust_param(host, CKGEN_MSDC_DLY_SEL, &i,
			...
			AUTOK_DBGPRINT(
			    AUTOK_DBG_RES,
			    "[AUTOK][%d] \t %d \t %d \t %d \t %s\r\n", uCmdEdge,
			    uWCrcEdge, i, score, tune_result_str64);
		}
	...
	do {
		...
		for (i = 0; i < 8; i++) {
			g_ta_raw[i] = 0;
			msdc_autok_adjust_param(host, WRDAT_CRCS_TA_CNTR, &i,
						AUTOK_WRITE);
			for (j = 0; j < 32; j++) {
				msdc_autok_adjust_paddly(host, &j,
							 DAT_PAD_RDLY);
...
void autok_msdc_tx_setting(struct msdc_host *host, struct mmc_ios *ios)
{
	...
	if (host->id == 0) {
		if (ios->timing == MMC_TIMING_MMC_HS400) {
			MSDC_SET_FIELD(EMMC50_CFG0, MSDC_EMMC50_CFG_TXSKEW_SEL,
			...
			if (ios->timing == MMC_TIMING_MMC_DDR52) {
				MSDC_SET_FIELD(MSDC_IOCON, MSDC_IOCON_DDR50CKD,
		...
		case DATA_ERROR:
			...
			if (ios->timing == MMC_TIMING_MMC_DDR52) {
				...
				MSDC_GET_FIELD(MSDC_PATCH_BIT0,
					       ...
					       cur_read_fifo_edge);
				AUTOK_RAWPRINT(
			"[AUTOK][read err]PB0[3]_VALID orig_fifo_edge = %d",
					       orig_read_fifo_edge);
				AUTOK_RAWPRINT(
				    "cur_edge = %d cur_fifo_edge = %d\r\n",
				    cur_read_edge, cur_read_fifo_edge);
						...
						unsigned int cycle_value)
{
	...
	/* store tx setting */
	MSDC_GET_FIELD(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT0_TXDLY,
		       dat_tx[0]);
	MSDC_GET_FIELD(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT1_TXDLY,
		       dat_tx[1]);
	MSDC_GET_FIELD(EMMC50_PAD_DAT23_TUNE, MSDC_EMMC50_PAD_DAT2_TXDLY,
		       dat_tx[2]);
	MSDC_GET_FIELD(EMMC50_PAD_DAT23_TUNE, MSDC_EMMC50_PAD_DAT3_TXDLY,
		       dat_tx[3]);
	MSDC_GET_FIELD(EMMC50_PAD_DAT45_TUNE, MSDC_EMMC50_PAD_DAT4_TXDLY,
		       dat_tx[4]);
	MSDC_GET_FIELD(EMMC50_PAD_DAT45_TUNE, MSDC_EMMC50_PAD_DAT5_TXDLY,
		       dat_tx[5]);
	MSDC_GET_FIELD(EMMC50_PAD_DAT67_TUNE, MSDC_EMMC50_PAD_DAT6_TXDLY,
		       dat_tx[6]);
	MSDC_GET_FIELD(EMMC50_PAD_DAT67_TUNE, MSDC_EMMC50_PAD_DAT7_TXDLY,
		       dat_tx[7]);
	...
	for (tune_tx_value = 0; tune_tx_value < 32; tune_tx_value++) {
		...
		MSDC_SET_FIELD(EMMC50_PAD_DAT01_TUNE,
			       MSDC_EMMC50_PAD_DAT0_TXDLY, tune_tx_value);
		MSDC_SET_FIELD(EMMC50_PAD_DAT01_TUNE,
			       MSDC_EMMC50_PAD_DAT1_TXDLY, tune_tx_value);
		MSDC_SET_FIELD(EMMC50_PAD_DAT23_TUNE,
			       MSDC_EMMC50_PAD_DAT2_TXDLY, tune_tx_value);
		MSDC_SET_FIELD(EMMC50_PAD_DAT23_TUNE,
			       MSDC_EMMC50_PAD_DAT3_TXDLY, tune_tx_value);
		MSDC_SET_FIELD(EMMC50_PAD_DAT45_TUNE,
			       MSDC_EMMC50_PAD_DAT4_TXDLY, tune_tx_value);
		MSDC_SET_FIELD(EMMC50_PAD_DAT45_TUNE,
			       MSDC_EMMC50_PAD_DAT5_TXDLY, tune_tx_value);
		MSDC_SET_FIELD(EMMC50_PAD_DAT67_TUNE,
			       MSDC_EMMC50_PAD_DAT6_TXDLY, tune_tx_value);
		MSDC_SET_FIELD(EMMC50_PAD_DAT67_TUNE,
			       MSDC_EMMC50_PAD_DAT7_TXDLY, tune_tx_value);
		...
		for (tune_cnt = 0; tune_cnt < TUNE_DAT_TX_CNT; tune_cnt++) {
			if (opcode == MMC_WRITE_BLOCK) {
				...
				while (((response >> 9) & 0xF) != 4) {
					ret = autok_send_tune_cmd(
					    host, MMC_SEND_STATUS, TUNE_CMD);
					if ((ret & (E_RESULT_RSP_CRC |
						    E_RESULT_CMD_TMO |
			...
			if ((ret & (E_RESULT_RSP_CRC | E_RESULT_CMD_TMO |
				    E_RESULT_FATAL_ERR)) != 0) {
	...
	if (dat_tx_sel.tx_err != 0) {
		...
		AUTOK_RAWPRINT(
		    "[AUTOK][tune dat TX]=========err may occur========\r\n");
		MSDC_SET_FIELD(EMMC50_PAD_DAT01_TUNE,
			       MSDC_EMMC50_PAD_DAT0_TXDLY, dat_tx[0]);
		MSDC_SET_FIELD(EMMC50_PAD_DAT01_TUNE,
			       MSDC_EMMC50_PAD_DAT1_TXDLY, dat_tx[1]);
		MSDC_SET_FIELD(EMMC50_PAD_DAT23_TUNE,
			       MSDC_EMMC50_PAD_DAT2_TXDLY, dat_tx[2]);
		MSDC_SET_FIELD(EMMC50_PAD_DAT23_TUNE,
			       MSDC_EMMC50_PAD_DAT3_TXDLY, dat_tx[3]);
		MSDC_SET_FIELD(EMMC50_PAD_DAT45_TUNE,
			       MSDC_EMMC50_PAD_DAT4_TXDLY, dat_tx[4]);
		MSDC_SET_FIELD(EMMC50_PAD_DAT45_TUNE,
			       MSDC_EMMC50_PAD_DAT5_TXDLY, dat_tx[5]);
		MSDC_SET_FIELD(EMMC50_PAD_DAT67_TUNE,
			       MSDC_EMMC50_PAD_DAT6_TXDLY, dat_tx[6]);
		MSDC_SET_FIELD(EMMC50_PAD_DAT67_TUNE,
			       MSDC_EMMC50_PAD_DAT7_TXDLY, dat_tx[7]);
	}
end:
	return dat_tx_sel;
}
					     ...
					     unsigned int value)
{
	...
	for (tune_tx_value = 0; tune_tx_value < 32; tune_tx_value++) {
		...
		tune_pass_cnt[tune_tx_value] = 0;
		MSDC_SET_FIELD(EMMC50_PAD_CMD_TUNE,
			       MSDC_EMMC50_PAD_CMD_TUNE_TXDLY, tune_tx_value);
	...
	if (cmd_tx_sel.tx_err != 0) {
		MSDC_SET_FIELD(EMMC50_PAD_CMD_TUNE,
			       MSDC_EMMC50_PAD_CMD_TUNE_TXDLY, cmd_tx);
		AUTOK_RAWPRINT(
		    "[AUTOK][tune cmd TX]=========err may occur========\r\n");
		goto out;
	}
...
int autok_execute_tuning(struct msdc_host *host, u8 *res)
{
	...
	if (execute_offline_tuning(host) != 0)
		AUTOK_RAWPRINT(
		    "[AUTOK] ========Error: Autok OFFLINE Failed========\r\n");
#endif
	ret = execute_online_tuning(host, res);
	if (ret != 0) {
		AUTOK_RAWPRINT(
		    "[AUTOK] ========Error: Autok Failed========\r\n");
		AUTOK_RAWPRINT(
		    "[AUTOK] ========restore pre autok parameters========\r\n");
...
int hs400_execute_tuning(struct msdc_host *host, u8 *res)
{
	...
	if (ret != 0) {
		AUTOK_RAWPRINT(
		    "[AUTOK] ========Error: Autok HS400 Failed========\r\n");
		AUTOK_RAWPRINT(
		    "[AUTOK] ========restore pre autok parameters========\r\n");
		/* restore pre autok parameter */
		for (i = 0; i < TUNING_PARAM_COUNT; i++) {
...
int hs400_execute_tuning_cmd(struct msdc_host *host, u8 *res)
{
	...
	tmo = AUTOK_CMD_TIMEOUT << 2;
	wait_cond_tmo(!(MSDC_READ32(SDC_STS) & SDC_STS_SDCBUSY), tmo);
	if (tmo == 0) {
		AUTOK_RAWPRINT("[AUTOK]MSDC busy tmo ...\n");
		/* msdc reset */
		autok_msdc_reset();
	}
	do_gettimeofday(&tm_s);
...
int hs200_execute_tuning(struct msdc_host *host, u8 *res)
{
	...
	if (ret != 0) {
		AUTOK_RAWPRINT(
		    "[AUTOK] ========Error: Autok HS200 Failed========\r\n");
		AUTOK_RAWPRINT(
		    "[AUTOK] ========restore pre autok parameters========\r\n");
		/* restore pre autok parameter */
		for (i = 0; i < TUNING_PARAM_COUNT; i++) {
...
int hs200_execute_tuning_cmd(struct msdc_host *host, u8 *res)
{
	...
	tm_val = (tm_e.tv_sec - tm_s.tv_sec) * 1000 +
		 (tm_e.tv_usec - tm_s.tv_usec) / 1000;
	AUTOK_RAWPRINT(
	    "[AUTOK][HS200 only for cmd]=========Time Cost:%d ms========\r\n",
	    tm_val);
