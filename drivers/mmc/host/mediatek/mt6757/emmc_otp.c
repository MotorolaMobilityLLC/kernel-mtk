...
unsigned int emmc_get_wp_size(void)
{
	...
	if (sg_wp_size == 0) {
		...
		pr_debug("otp: write_prot_grpsz %d\n", write_prot_grpsz);
		...
		if (l_ext_csd[EXT_CSD_ERASE_GROUP_DEF] & 0x1) {
			...
			pr_debug("otp: hc unit sg_wp_size %d\n", sg_wp_size);
		} else {
			...
			sg_wp_size = (512 * host_ctl->mmc->card->erase_size) *
				     (write_prot_grpsz + 1);
			pr_debug("otp: non-hc unit sg_wp_size %d\n",
				 sg_wp_size);
		}
		kfree(l_ext_csd);
	}
...
unsigned int emmc_otp_read(unsigned int blk_offset, void *BufferPtr)
{
	...
	if (!host_ctl || !host_ctl->mmc) {
		pr_debug("host_ctl or mmc is NULL\n");
		return -1;
	}
...
static int mt_otp_access(unsigned int access_type, unsigned int offset,
			 void *buff_ptr, unsigned int length,
			 unsigned int *status)
{
	unsigned int ret = 0;
	...
	pr_debug("[%s]: %s (0x%x) length:(%d bytes) !\n", __func__,
		 access_type ? "WRITE" : "READ", offset, length);
	...
	do {
		...
		if (access_type == FS_EMMC_OTP_READ) {
			...
			*status = Status;
			...
			if (Status != OTP_SUCCESS) {
				pr_debug("[%s]: read before write, Read status "
					 "(%d) blkno (0x%x)\n",
			...
			*status = Status;
			...
			pr_debug(
			    "[%s]: Error, neither read nor write operations!\n",
			    __func__);
			break;
		}
...
static long mt_otp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (g_bInitDone == false) {
		pr_debug("ERROR: EMMC Flash Not initialized !!\n");
		ret = -EFAULT;
		goto exit;
	}
#endif
	...
	case EMMC_OTP_READ:
		pr_debug(
		    "OTP IOCTL: EMMC_OTP_READ Offset(0x%x), Length(0x%x)\n",
		...
		if (copy_to_user(otpctl.BufferPtr, pbuf,
			...
			pr_debug(
			    "EMMC_OTP IOCTL: Copy to user buffer Error!\n");
			goto error;
		}
		break;
	case EMMC_OTP_WRITE:
		pr_debug(
		    "OTP IOCTL: EMMC_OTP_WRITE Offset(0x%x), Length(0x%x)\n",
		    otpctl.Offset, otpctl.Length);
		if (copy_from_user(pbuf, otpctl.BufferPtr,
				   (sizeof(char) * otpctl.Length))) {
			pr_debug(
			    "EMMC_OTP IOCTL: Copy from user buffer Error!\n");
			goto error;
		}
				...
				unsigned long arg)
{
	...
	case COMPAT_EMMC_OTP_GET_LENGTH:
		ret = mt_otp_ioctl(file, EMMC_OTP_GET_LENGTH,
				   (unsigned long)arg64);
		/*pr_debug("OTP IOCTL: COMPAT_EMMC_OTP_GET_LENGTH\n");*/
		if (ret)
			pr_debug("EMMC_OTP_GET_LENGTH unlocked_ioctl failed.");

		break;
	case COMPAT_EMMC_OTP_READ:
		/*pr_debug("OTP IOCTL: COMPAT_EMMC_OTP_READ\n");*/
		ret = mt_otp_ioctl(file, EMMC_OTP_READ, (unsigned long)arg64);

		if (ret)
			pr_debug("EMMC_OTP_READ unlocked_ioctl failed.");

		break;
	case COMPAT_EMMC_OTP_WRITE:
		/*pr_debug("OTP IOCTL: COMPAT_EMMC_OTP_WRITE\n");*/
		ret = mt_otp_ioctl(file, EMMC_OTP_WRITE, (unsigned long)arg64);

		if (ret)
			pr_debug("EMMC_OTP_WRITE unlocked_ioctl failed.");
...
static struct miscdevice emmc_otp_dev = {
    .minor = MISC_DYNAMIC_MINOR, .name = "otp", .fops = &emmc_otp_fops,
};
...
static int emmc_otp_remove(struct platform_device *pdev) { return 0; }
...
static int __init emmc_otp_init(void)
{
	...
	if (entry == NULL) {
		pr_debug("emmc OTP: unable to create /proc entry\n");
		return -ENOMEM;
	}
	...
	if (unlikely(err)) {
		pr_debug("OTP: failed to register EMMC OTP device!\n");
		return err;
	}
