...
static int dma_force[HOST_MAX_NUM] = /* used for sd ioctrol */
	{FORCE_NOTHING, FORCE_NOTHING, FORCE_NOTHING};
...
static int simple_sd_open(struct inode *inode, struct file *file) { return 0; }
...
static int simple_sd_ioctl_set_bootpart(struct msdc_ioctl *msdc_ctl)
{
	...
	if ((bootpart != EMMC_BOOT1_EN) && (bootpart != EMMC_BOOT2_EN) &&
...
static int simple_sd_ioctl_set_driving(struct msdc_ioctl *msdc_ctl)
{
	...
	if (ret < 0) {
		pr_debug("host%d enable clock fail\n", msdc_ctl->host_num);
		return -EINVAL;
	}
		  ...
		  enum GET_STORAGE_INFO info_type, struct storage_info *info)
{
	...
	case SD_CARD:
		...
		break;
	default:
		pr_debug("No supported storage type!");
		return 0;
	}
	host = msdc_get_host(host_function, boot, 0);
	WARN_ON(!host || !host->mmc);
	if (!host || !host->mmc) {
		pr_debug("host or mmc is NULL\n");
		return -EINVAL;
	}
	switch (info_type) {
	...
	case CARD_INFO:
		if (host->mmc->card)
			...
			pr_debug("CARD was not ready<get card>!");
			return 0;
		}
		break;
	case DISK_INFO:
		if (host->mmc->card)
			...
			pr_debug("CARD was not ready<get disk>!");
			return 0;
		}
		break;
...
/* FIX ME: check if this case can be removed */
		...
		break;
	default:
		pr_debug("Please check INFO_TYPE");
		return 0;
	}
	return 1;
}
...
static int simple_mmc_erase_func(unsigned int start, unsigned int size)
{
	...
	if (mmc_can_discard(mmc->card)) {
		...
		pr_debug(
		    "[%s]: emmc card can't support trim / discard / erase\n",
		    __func__);
		goto end;
	}

	pr_debug("[%s]: start=0x%x, size=%d, arg=0x%x, can_trim=(0x%x), "
		 "EXT_CSD_SEC_GB_CL_EN=0x%lx\n",
		 __func__, start, size, arg,
		 mmc->card->ext_csd.sec_feature_support, EXT_CSD_SEC_GB_CL_EN);
...
static int simple_mmc_erase_partition(unsigned char *name)
{
	...
	if (name && (strncmp(name, "usrdata", 7) == 0 ||
		...
		if (l_ret == 0) {
			...
			pr_debug("erase %s start sector: 0x%x size: 0x%x\n",
				 mbr_part.part_name, mbr_part.start_sector,
				 mbr_part.nr_sects);
			    ...
			    unsigned long arg)
{
	...
	if ((struct msdc_ioctl *)arg == NULL) {
		...
		default:
			pr_debug(
			    "mt_sd_ioctl:this opcode value is illegal!!\n");
			return -EINVAL;
		}
		return ret;
	}
	...
	case MSDC_ERASE_SELECTED_AREA:
		msdc_ctl.result =
		    simple_sd_ioctl_erase_selected_area(&msdc_ctl);
		break;
...
static int
compat_get_simple_ion_allocation(struct compat_simple_sd_ioctl __user *arg32,
...
static int
compat_put_simple_ion_allocation(struct compat_simple_sd_ioctl __user *arg32,
				 struct msdc_ioctl __user *arg64)
{
	compat_int_t i;
	compat_uint_t u;
...
static int simple_sd_remove(struct platform_device *pdev) { return 0; }
...
static struct platform_driver simple_sd_driver = {
	...
	.driver = {
	    .name = DRV_NAME_MISC, .owner = THIS_MODULE,
	},
};
