...
static unsigned int print_k_cpu_test = UINT_MAX;
...
void mmc_cmd_dump(struct mmc_host *mmc)
{
	...
	for (i = dbg_host_cnt; i < dbg_max_cnt; i++) {
		if (dbg_run_host_log_dat[i].cmd == 44
			...
			pr_debug("%d [%5llu.%06llu]%2d %2d 0x%08x tag=%d type=%s %s %s\n",
			...
			&& !dbg_run_host_log_dat[i].type) {
			tag = (dbg_run_host_log_dat[i].arg >> 16) & 0xf;
			pr_debug("%d [%5llu.%06llu]%2d %2d 0x%08x tag=%d\n",
				i, dbg_run_host_log_dat[i].time_sec,
				dbg_run_host_log_dat[i].time_usec,
				dbg_run_host_log_dat[i].type,
				dbg_run_host_log_dat[i].cmd,
			...
			pr_debug("%d [%5llu.%06llu]%2d %2d 0x%08x\n",
				i, dbg_run_host_log_dat[i].time_sec,
				dbg_run_host_log_dat[i].time_usec,
				dbg_run_host_log_dat[i].type,
				dbg_run_host_log_dat[i].cmd,
...
{
	MET_PRINTK("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%"
...
void emmc_cq_state_log(struct mmc_host *mmc, unsigned int idx, unsigned int val)
{
	dbg_cq_state[idx] = val;
	emmc_cq_state();
}

void emmc_cq_state_pr(struct mmc_host *mmc, unsigned int idx, unsigned int val)
{
	dbg_cq_state[idx] = val;
}
#else
...
void msdc_cmdq_status_print(struct msdc_host *host, struct seq_file *m)
{
	...
	seq_printf(m, "cmdq mode    : %s\n",
		   mmc_card_cmdq(mmc->card) ? "enable" : "disable");
...
void msdc_cmdq_func(struct msdc_host *host, const int num, struct seq_file *m)
{
	...
	case 4:
		...
		break;
	case 5:
		MSDC_GET_FIELD(EMMC50_PAD_DS_TUNE, MSDC_EMMC50_PAD_DS_TUNE_DLY2,
			       a);
		MSDC_SET_FIELD(EMMC50_PAD_DS_TUNE, MSDC_EMMC50_PAD_DS_TUNE_DLY2,
			       a + 1);
		MSDC_GET_FIELD(EMMC50_PAD_DS_TUNE, MSDC_EMMC50_PAD_DS_TUNE_DLY2,
			       b);
...
void msdc_dump_register_debug(struct seq_file *m, u32 id, void __iomem *base)
{
	...
	for (i = 0; msdc_offsets[i] != 0xFFFF; i++) {
		...
		if ((id != 0) && (msdc_offsets[i] >= OFFSET_EMMC50_PAD_DS_TUNE))
			break;
...
static void msdc_select_card_type(struct mmc_host *host)
{
	...
	if (caps & MMC_CAP_1_2V_DDR && card_type & EXT_CSD_CARD_TYPE_DDR_1_2V) {
		hs_max_dtr = MMC_HIGH_DDR_MAX_DTR;
...
void msdc_set_host_mode_speed(struct mmc_host *mmc, int spd_mode, int cmdq)
{
	...
	case MMC_TIMING_UHS_DDR50:
		mmc->caps |=
		    MMC_CAP_UHS_DDR50 | MMC_CAP_UHS_SDR25 | MMC_CAP_UHS_SDR12;
		break;
	...
	case MMC_TIMING_MMC_HS400:
		...
		break;
	default:
		pr_debug("[SD_Debug]invalid speed mode:%d\n", spd_mode);
		break;
	}
...
/* invoke msdc_get_host_mode_speed for printing purpose */
	...
	if (mmc_hw_reset(mmc))
		pr_debug("[SD_Debug] Reinit card failed, Can not switch speed "
			 "mode\n");
...
static void msdc_set_field(struct seq_file *m, void __iomem *address,
			   unsigned int start_bit, unsigned int len,
			   unsigned int value)
{
	unsigned long field;
...
static u8 wData_emmc[16] = {0x67, 0x45, 0x23, 0x01, 0xef, 0xcd, 0xab, 0x89,
			    0xce, 0x8a, 0x46, 0x02, 0xde, 0x9b, 0x57, 0x13};
...
static u8 wData_sd[200] = {
	...
	0x7f, 0x7f, 0x40, 0x40, 0x04, 0xfb, 0x04, 0x04, 0x04, 0xfb, 0xfb, 0xfb,
	0x04, 0xfb, 0xfb, 0xfb, 0x02, 0x02, 0x02, 0xfd, 0x02, 0x02, 0x02, 0xfd,
	0xfd, 0xfd, 0x02, 0xfd, 0xfd, 0xfd, 0x01, 0x01, 0x01, 0xfe, 0x01, 0x01,
	0x01, 0xfe, 0xfe, 0xfe, 0x01, 0xfe, 0xfe, 0xfe, 0x80, 0x80, 0x80, 0x7f,
	0x80, 0x80, 0x80, 0x7f, 0x7f, 0x7f, 0x80, 0x7f, 0x7f, 0x7f, 0x40, 0x40,
	0x40, 0x40, 0x80, 0x7f, 0x7f, 0x7f, 0x40, 0x40, 0x20, 0xdf, 0x20, 0x20,
	0x20, 0xdf, 0xdf, 0xdf, 0x10, 0x10, 0x10, 0xef, 0xef, 0x10, 0xef, 0xef,
};
				 ...
				 uint type, uint compare)
{
...
#ifdef CONFIG_MTK_EMMC_CQ_SUPPORT
	is_cmdq_en = false;
	if (host_num == 0 && mmc_card_cmdq(host_ctl->mmc->card)) {
	...
	for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN; forIndex++) {
		if (rPtr[forIndex] != wData[forIndex % wData_len]) {
			pr_debug("index[%d]\tW_buffer[0x%x]\tR_buffer[0x%x]"
				 "\tfailed\n",
...
int multi_rw_compare(struct seq_file *m, int host_num, uint address, int count,
		     uint type, int multi_thread)
{
	int i = 0, j = 0;
	int error = 0;
	...
	for (i = 0; i < count; i++) {
#if 0
		pr_debug("== cpu[%d] pid[%d]: start %d time compare ==\n",
		...
		if (error) {
			if (!multi_thread)
				seq_printf(
				    m, "[%s]: failed to write data, error=%d\n",
				    __func__, error);
			else
				pr_debug(
		...
		for (j = 0; j < 1; j++) {
			...
			if (error) {
				if (!multi_thread)
					seq_printf(m, "[%s]: failed to read "
						      "data, error=%d\n",
						   __func__, error);
				else
					pr_debug("[%s]: failed to read data, "
						 "error=%d\n",
						 __func__, error);
				break;
			}
		...
		if (!multi_thread)
			seq_printf(
			    m, "== cpu[%d] pid[%d]: %s %d time compare ==\n",
	...
	if (i == count) {
		if (!multi_thread)
			seq_printf(
			    m,
			    "pid[%d]: success to compare data for %d times\n",
			    current->pid, count);
		else
			pr_debug(
			    "pid[%d]: success to compare data for %d times\n",
...
static uint smp_address_on_sd[MAX_THREAD_NUM_FOR_SMP] = {
	0x2000,   0x80000,  0x100000, 0x180000, 0x200000,		/* 1GB */
	0x202000, 0x280000, 0x300000, 0x380000, 0x400000,		/* 2GB */
	0x402000, 0x480000, 0x500000, 0x580000, 0x600000, 0x602000, /* 3GB */
	0x660000, /* real total size of 4GB sd card is < 4GB */
	0x680000, 0x6a0000, 0x6b0000,
};
...
static uint smp_address_on_mmc[MAX_THREAD_NUM_FOR_SMP] = {
	0x402000, 0x410000, 0x520000, 0x530000, 0x640000, 0x452000,
	0x460000, 0x470000, 0x480000, 0x490000, 0x4a2000, 0x4b0000,
	0x5c0000, 0x5d0000, 0x6e0000, 0x602000, 0x660000, /* real total size of 4GB
							 sd card is < 4GB */
	0x680000, 0x6a0000, 0x6b0000,
};
 ...
 */
static int smp_test_on_hosts(struct seq_file *m, int thread_num, int host_id,
			     int count, int multi_address)
{
	...
	seq_printf(
	    ...
	    __func__);
	seq_printf(
	    m, " each host run %d thread, each thread run %d RW comparison\n",
	    thread_num, count);
	...
	for (i = id_start; i < id_end; i++) {
		...
		if (type == MMC_TYPE_MMC) {
			...
			seq_puts(m, "   please manually run wifi application "
				    "instead of write/read SDIO card\n");
			continue;
		} else {
...
static int msdc_help_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "\n   LOG control:        echo %x [host_id] [debug_zone] "
		      "> msdc_debug\n",
		   SD_TOOL_ZONE);
	seq_printf(m, "          [debug_zone] DMA:0x%x, CMD:0x%x, RSP:0x%x, "
		      "INT:0x%x, CFG:0x%x, FUC:0x%x,\n",
		   DBG_EVT_DMA, DBG_EVT_CMD, DBG_EVT_RSP, DBG_EVT_INT,
		   DBG_EVT_CFG, DBG_EVT_FUC);
	seq_printf(m, "                        OPS:0x%x, FIO:0x%x, WRN:0x%x, "
		      "PWR:0x%x, CLK:0x%x, RW:0x%x, NRW:0x%x, CHE:0x%x\n",
		   DBG_EVT_OPS, DBG_EVT_FIO, DBG_EVT_WRN, DBG_EVT_PWR,
		   DBG_EVT_CLK, DBG_EVT_RW, DBG_EVT_NRW, DBG_EVT_CHE);
	seq_puts(m, "\n   DMA mode:\n");
	seq_printf(m, "   *set DMA mode:   echo %x 0 [host_id] [dma_mode] "
		      "[dma_size] > msdc_debug\n",
		   SD_TOOL_DMA_SIZE);
	seq_printf(m, "   *get DMA mode:   echo %x 1 [host_id] > msdc_debug\n",
		   SD_TOOL_DMA_SIZE);
	seq_puts(m, "        [dma_mode]   0:PIO, 1:DMA, 2:SIZE_DEP\n");
	seq_puts(m, "        [dma_size]   valid for SIZE_DEP mode, the min "
		    "size can trigger the DMA mode\n");
	seq_printf(m,
		   "\n   SDIO profile:  echo %x [enable] [time] > msdc_debug\n",
		   SD_TOOL_SDIO_PROFILE);
	seq_puts(m, "\n   CLOCK control:\n");
	seq_printf(m, "   *set clk src:       echo %x 0 [host_id] [clk_src] > "
		      "msdc_debug\n",
		   SD_TOOL_CLK_SRC_SELECT);
	seq_printf(m,
		   "   *get clk src:       echo %x 1 [host_id] > msdc_debug\n",
		   SD_TOOL_CLK_SRC_SELECT);
	seq_puts(m, "      [clk_src]       msdc0: 0:26M, 1:800M, 2:400M, "
		    "3:200M, 4:182M, 5:136M, 6:156M, 7:48M, 8:91M\n");
	seq_puts(m, "	  [clk_src]  msdc1/2/3: 0:26M, 1:208M, 2:200M, 3:182M, "
		    "4:182M, 5:136M, 6:156M, 7:48M, 8:91M\n");
	seq_puts(m, "\n   REGISTER control:\n");
	seq_printf(m, "        write register:    echo %x 0 [host_id] "
		      "[register_offset] [value] > msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_printf(m, "        read register:     echo %x 1 [host_id] "
		      "[register_offset] > msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_printf(m, "        write mask:        echo %x 2 [host_id] "
		      "[register_offset] [start_bit] [len] [value] > "
		      "msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_printf(m, "        read mask:         echo %x 3 [host_id] "
		      "[register_offset] [start_bit] [len] > msdc_debug\n",
		   SD_TOOL_REG_ACCESS);
	seq_printf(
	    m, "        dump all:          echo %x 4 [host_id] > msdc_debug\n",
	    SD_TOOL_REG_ACCESS);
	seq_puts(m, "\n   DRVING control:\n");
	seq_printf(m, "        set driving:       echo %x [host_id] [clk_drv] "
		      "[cmd_drv] [dat_drv] [rst_drv] [ds_drv] [voltage] > "
		      "msdc_debug\n",
		   SD_TOOL_SET_DRIVING);
	seq_puts(m, "            [voltage]           1: 18v, 0: 33v\n");
	seq_printf(
	    m, "        get driving:       echo %x 0 [host_id]  > msdc_debug\n",
	    SD_TOOL_SET_DRIVING);
	seq_printf(m, "\n   RW_COMPARE test:       echo %x [host_id] "
		      "[compare_count] > msdc_debug\n",
		   RW_BIT_BY_BIT_COMPARE);
	seq_puts(m, "          [compare_count]    how many time you want to "
		    "\"write=>read=>compare\"\n");
	seq_printf(m, "\n   SMP_ON_ONE_HOST test:  echo %x [host_id] "
		      "[thread_num] [compare_count] [multi_address] > "
		      "msdc_debug\n",
		   SMP_TEST_ON_ONE_HOST);
	seq_puts(m, "          [thread_num]       how many R/W comparision "
		    "thread you want to run at host_id\n");
	seq_puts(m, "          [compare_count]    how many time you want to "
		    "\"write=>read=>compare\" in each thread\n");
	seq_puts(m, "          [multi_address]    whether read/write different "
		    "address in each thread, 0:No, 1:Yes\n");
	seq_printf(m, "\n   SMP_ON_ALL_HOST test:  echo %x [thread_num] "
		      "[compare_count] [multi_address] > msdc_debug\n",
		   SMP_TEST_ON_ALL_HOST);
	seq_puts(m, "          [thread_num]       how many R/W comparision "
		    "thread you want to run at each host\n");
	seq_puts(m, "          [compare_count]    how many time you want to "
		    "\"write=>read=>compare\" in each thread\n");
	seq_puts(m, "          [multi_address]    whether read/write different "
		    "address in each thread, 0:No, 1:Yes\n");
	seq_puts(m, "\n   SPEED_MODE control:\n");
	seq_printf(m, "          set speed mode:    echo %x 1 [host_id] "
		      "[speed_mode] [cmdq]> msdc_debug\n",
		   SD_TOOL_MSDC_HOST_MODE);
	seq_printf(m, "          get speed mode:    echo %x 0 [host_id]\n",
		   SD_TOOL_MSDC_HOST_MODE);
	seq_puts(
	    m, "            [speed_mode]       0: MMC_TIMING_LEGACY	1: "
	       "MMC_TIMING_MMC_HS	2: MMC_TIMING_SD_HS	 3: "
	       "MMC_TIMING_UHS_SDR12\n"
	       "                               4: MMC_TIMING_UHS_SDR25	5: "
	       "MMC_TIMING_UHS_SDR50	6: MMC_TIMING_UHS_SDR104 7: "
	       "MMC_TIMING_UHS_DDR50\n"
	       "                               8: MMC_TIMING_MMC_DDR52	9: "
	       "MMC_TIMING_MMC_HS200	A: MMC_TIMING_MMC_HS400\n"
	...
	seq_printf(
	    ...
	    SD_TOOL_DMA_STATUS);
	seq_puts(m, "          [ops]              0:get latest dma address, "
		    "1:start violation test\n");
	seq_printf(m, "\n   SET Slew Rate:         echo %x [host_id] [clk] "
		      "[cmd] [dat] [rst] [ds]> msdc_debug\n",
		   SD_TOOL_ENABLE_SLEW_RATE);
	seq_puts(m, "\n   TD/RD SEL:\n");
	seq_printf(m, "          set rdsel:             echo %x [host_id] 0 "
		      "[value] > msdc_debug\n",
		   SD_TOOL_SET_RDTDSEL);
	seq_printf(m, "          set tdsel:             echo %x [host_id] 1 "
		      "[value] > msdc_debug\n",
		   SD_TOOL_SET_RDTDSEL);
	seq_printf(m, "          get tdsel/rdsel:       echo %x [host_id] 2 > "
		      "msdc_debug\n",
		   SD_TOOL_SET_RDTDSEL);
	seq_puts(m, "            [value]              rdsel: 0x0<<4 ~ 0x3f<<4, "
		    "   tdsel: 0x0~0xf\n");
	seq_printf(m, "\n   EMMC/SD RW test:       echo %x [host_id] [mode] > "
		      "msdc_debug\n",
		   MSDC_READ_WRITE);
	seq_puts(
	    m, "          [mode]               mode 0:stop, 1:read, 2:write\n");
	seq_printf(m, "\n   Error tune debug:       echo %x [host_id] [cmd_id] "
		      "[arg] [error_type] [count] > msdc_debug\n",
		   MMC_ERROR_TUNE);
	seq_puts(
	    m,
	    "            [cmd_id]           0: CMD0, 1: CMD1, 2: CMD2......\n");
	seq_puts(m, "            [arg]              for CMD6, arg means "
		    "ext_csd index......\n");
	seq_puts(m, "            [error]            0: disable error tune "
		    "debug, 1: cmd timeout, 2: cmd crc, 4: dat timeout, 8: dat "
		    "crc, 16: acmd timeout, 32: acmd crc\n");
	seq_puts(m, "            [count]            error count\n");
	seq_printf(m, "\n   eMMC Cache Control: echo %x [host_id] [action_id] "
		      "> /proc/msdc_debug\n",
		   MMC_EDC_EMMC_CACHE);
	seq_puts(m, "            [action_id]        0:Disable cache 1:Enable "
		    "cache 2:check cache status\n");
	seq_printf(m, "\n   eMMC Dump GPD/BD:      echo %x [host_id] > "
		      "/proc/msdc_debug\n",
		   MMC_DUMP_GPD);
	seq_puts(m, "            [type]             0:tune cmd  1:tune read  "
		    "2:tune write  3:tune HS400\n");
	seq_puts(m, "            [start_voltage]    ?mV\n");
	seq_puts(m, "            [end_voltage]      ?mV, we try ETT from "
		    "higher voltage to lower voltage\n");
	seq_printf(m, "\n   CRC Stress Test:       echo %x [action_id]> "
		      "/proc/msdc_debug\n",
		   MMC_CRC_STRESS);
	seq_puts(m, "            [action_id]        0:disable 1:enable\n");
	seq_printf(m, "\n   SDIO AutoK Result :    echo %x "
		      "[host_id][vcore][rw]> /proc/msdc_debug\n",
		   SDIO_AUTOK_RESULT);
...
void msdc_dump_gpd_bd(int id)
{
...
#endif
		pr_notice("hwo:0x%x, bdp:0x%x, rsv0:0x%x, "
			  "chksum:0x%x,intr:0x%x,rsv1:0x%x,nexth4:0x%x,ptrh4:"
			  "0x%x\n",
			  gpd->hwo, gpd->bdp, gpd->rsv0, gpd->chksum, gpd->intr,
			  gpd->rsv1, (unsigned int)gpd->nexth4,
			  (unsigned int)gpd->ptrh4);
		pr_notice("next:0x%x, ptr:0x%x, buflen:0x%x, extlen:0x%x, "
			  "arg:0x%x,blknum:0x%x,cmd:0x%x\n",
	...
	if (bd == NULL) {
		...
		for (i = 0; i < host->dma.sglen; i++) {
			pr_notice("the %d BD\n", i);
			pr_notice("        eol:0x%x, rsv0:0x%x, chksum:0x%x, "
				  "rsv1:0x%x,blkpad:0x%x,dwpad:0x%x,rsv2:0x%"
				  "x\n",
				  bd->eol, bd->rsv0, bd->chksum, bd->rsv1,
				  bd->blkpad, bd->dwpad, bd->rsv2);
			pr_notice("        nexth4:0x%x, ptrh4:0x%x, next:0x%x, "
				  "ptr:0x%x, buflen:0x%x, rsv3:0x%x\n",
				  (unsigned int)bd->nexth4,
				  (unsigned int)bd->ptrh4,
				  (unsigned int)bd->next, (unsigned int)bd->ptr,
				  bd->buflen, bd->rsv3);
					...
					struct msdc_host *host)
{
	...
	seq_printf(m,
		   "msdc%d: Current eMMC Cache status: %s, Cache size:%dKB\n",
		   host->id,
...
/* FIX ME: Move to user space */
static void msdc_enable_emmc_cache(struct seq_file *m, struct msdc_host *host,
				   int enable)
{
	int err;
	u8 c_ctrl;
	...
	if (c_ctrl == enable)
		...
		if (err)
			seq_printf(
			    m, "msdc%d: Cache is supported, but %s failed\n",
			    host->id, enable ? "enable" : "disable");
		else
			    ...
			    struct mmc_command *sbc, u32 *intsts)
{
	...
	if (g_err_tune_dbg_cmd == opcode) {
		if ((opcode != MMC_SWITCH) ||
			...
			pr_debug("[%s]: got the error cmd:%d, arg=%d, dbg "
				 "error=%d, cmd->error=%d, count=%d\n",
	...
	if ((g_err_tune_dbg_cmd == MMC_SET_BLOCK_COUNT) && sbc &&
		...
		if (g_err_tune_dbg_error & MTK_MSDC_ERROR_ACMD_TMO)
			...
			*intsts = MSDC_INT_ACMDCRCERR;
			...
			return;
		pr_debug("[%s]: got the error cmd:%d, dbg error=%d, "
			 "sbc->error=%d, count=%d\n",
			    ...
			    u32 *intsts)
{
	...
	if (g_err_tune_dbg_cmd == (MSDC_READ32(SDC_CMD) & 0x3f)) {
		...
		pr_debug("[%s]: got the error cmd:%d, dbg error 0x%x, "
			 "data->error=%d, count=%d\n",
			 __func__, g_err_tune_dbg_cmd, g_err_tune_dbg_error,
			 host->data->error, g_err_tune_dbg_count);
	}
	if ((g_err_tune_dbg_cmd == MMC_STOP_TRANSMISSION) && stop &&
		...
		pr_debug("[%s]: got the error cmd:%d, dbg error 0x%x, "
			 "stop->error=%d, host->error=%d, count=%d\n",
			 __func__, g_err_tune_dbg_cmd, g_err_tune_dbg_error,
			    ...
			    u32 *intsts)
{
	...
	if ((g_err_tune_dbg_cmd != MMC_SWITCH) ||
		...
		pr_debug("[%s]: got the error cmd:%d, arg=%d, dbg error=%d, "
			 "cmd->error=%d, count=%d\n",
			 __func__, g_err_tune_dbg_cmd, g_err_tune_dbg_arg,
			 g_err_tune_dbg_error, cmd->error,
			 g_err_tune_dbg_count);
	}
}
...
static void msdc_dump_autok_setting(struct msdc_host *host, struct seq_file *m)
{
	...
	seq_printf(
	    m, "[AUTOK]VER : 0x%02x%02x%02x%02x\r\n",
	    host->autok_res[0][AUTOK_VER3], host->autok_res[0][AUTOK_VER2],
	    host->autok_res[0][AUTOK_VER1], host->autok_res[0][AUTOK_VER0]);
	...
	for (i = AUTOK_VCORE_HIGH; i >= AUTOK_VCORE_LOW; i--) {
		...
		seq_printf(m, "[AUTOK]DAT [RDAT_EDGE:%d RD_FIFO_EDGE:%d "
			      "WD_FIFO_EDGE:%d]\r\n",
...
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	...
	sscanf_num = sscanf(cmd_buf, "%x %x %x %x %x %x %x %x %x", &cmd, &p1,
			    &p2, &p3, &p4, &p5, &p6, &p7, &p8);

	if (cmd == -1) {
		seq_puts(m, "Please run: echo cmd [p1] [p2] [p3] [p4] [p5] "
			    "[p6] [p7] [p8]> /proc/msdc_debug\n");
	...
	if (cmd == SD_TOOL_ZONE) {
		...
		if (host) {
			seq_printf(m, "[SD_Debug] msdc host<%d> caps<0x%.8x>\n",
				   id, host->mmc->caps);
			seq_printf(m,
				   "[SD_Debug] msdc host<%d> caps2<0x%.8x>\n",
				   id, host->mmc->caps2);
		}
		...
		if (p1 == 0) {
			if (p3 >= 0 && p3 < CLK_SRC_MAX_NUM) {
				...
				seq_printf(m, "[SD_Debug] msdc%d's clk source "
					      "changed to %d\n",
					   id, msdc_clock_src[id]);
				seq_puts(m, "[SD_Debug] to enable the above "
					    "settings, suspend and resume the "
					    "phone again\n");
			} else {
				seq_printf(m, "[SD_Debug] invalid clock src "
					      "id:%d, check /proc/msdc_help\n",
					   p3);
			}
		} else if (p1 == 1) {
		...
		if ((offset == 0x18 || offset == 0x1C) && p1 != 4) {
			seq_puts(m, "[SD_Debug] Err: Accessing TXDATA and "
				    "RXDATA is forbidden\n");
			goto out;
		}
		...
		if (ret < 0)
			pr_debug("msdc%d enable clock fail\n", host->id);
		...
		if (p1 == 1) {
			...
			if ((unsigned char)p3 > 7 || (unsigned char)p4 > 7 ||
			    ...
			    (unsigned char)p7 > 7) {
				seq_puts(m, "[SD_Debug]Some drving value was "
					    "invalid(invalid:0~7)\n");
				goto out;
			}
		...
		seq_printf(m, "[SD_Debug] %s %s driving: clk_drv=%d, "
			      "cmd_drv=%d, dat_drv=%d, rst_drv=%d, ds_drv=%d\n",
		...
		if ((unsigned char)p2 > 1 || (unsigned char)p3 > 1 ||
		    ...
		    (unsigned char)p6 > 1) {
			seq_puts(m, "[SD_Debug]Some sr value was "
				    "invalid(correct:0(disable),1(enable))\n");
		} else {
			msdc_set_sr(host, p2, p3, p4, p5, p6);
			seq_printf(m, "[SD_Debug]msdc%d, clk_sr=%d, cmd_sr=%d, "
				      "dat_sr=%d, rst=%d, ds=%d\n",
				   id, p2, p3, p4, p5, p6);
		}
		...
		host = mtk_msdc_host[id];
		if ((p2 < 0) || (p2 > 2)) {
			seq_puts(m, "[SD_Debug]invalid option ( set rd:0, set "
				    "td:1, get td/rd: 2)\n");
		} else {
			...
			if (p2 == 0) {
				...
				seq_printf(
				    m,
				    "[SD_Debug]msdc%d, rd : 0x%x, td : 0x%x\n",
				    id, p3, p4);
			}
		}
		...
		if (mode == 1 && read_write_state == 2) {
			seq_puts(m, "[SD_Debug]: cannot read in write state, "
				    "please stop first\n");
			goto out;
		}
		if (mode == 2 && read_write_state == 1) {
			seq_puts(m, "[SD_Debug]: cannot write in read state, "
				    "please stop first\n");
			goto out;
		}
		...
		if (p2 == 0) {
			static char const *const str[] = {
			    "No data transaction or the device is not present "
			    "until now\n",
...
#else
			seq_puts(m, "please enable MSDC_DMA_ADDR_DEBUG at "
				    "mtk_sd.h if you want dump dma address\n");
#endif
		} else if (p2 == 1) {
		...
		seq_printf(m, "g_mtk_mmc_perf_dbg = 0x%x, g_mtk_mmc_dbg_range "
			      "= 0x%x, start = 0x%x, end = 0x%x\n",
		...
		for (i = 1; i <= g_dbg_req_count; i++) {
			seq_printf(
			    m, "analysis: %s 0x%x %d block, PGh %d\n",
...
/*autok_offline_tuning_TX(host);*/
		...
		if (mode == 1) {
			...
			seq_printf(m, "set dma retry %s\n\n",
				   host->dma_retry_force_dis == 2 ? "enable"
								  : "disable");
...
int msdc_debug_proc_init(void)
{
	...
	if (voltage_flag)
		...
		pr_debug("[%s]: failed to create /proc/msdc_voltage_flag\n",
			 __func__);
#endif
