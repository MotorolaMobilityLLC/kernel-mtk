...
static lct_emmc_info lct_emmc_info_str[] = {
	...
	{"KMRX60014M_B614",		"150100525836344D42", 4096},
};
...
//	memset(dev_list[i],0,sizeof(struct devinfo_struct));
    ...
    //lct_pr_debug("emcp list info.[%x][%x][%x][%x][%x][%x][%x][%x]\n",emi_settings[i].ID[0],emi_settings[i].ID[1],emi_settings[i].ID[2],emi_settings[i].ID[3],emi_settings[i].ID[4],emi_settings[i].ID[5],emi_settings[i].ID[6],emi_settings[i].ID[7]);
		...
		#endif
		dev_list[i]->device_version = "_NULL";
...
//					)
 		...
 		//void devinfo_declare(char* type,char* module,char* vendor,char* ic,char* version,char* info,int used)
			DEVINFO_CHECK_DECLARE(dev_list[i]->device_type,dev_list[i]->device_module,dev_list[i]->device_vendor,dev_list[i]->device_ic,dev_list[i]->device_version,dev_list[i]->device_info,DEVINFO_USED );	//used device regist
			...
			#endif
	//sprintf(info,"ram:%dMB",ram_size);
		DEVINFO_CHECK_DECLARE(dev_list[i]->device_type,dev_list[i]->device_module,dev_list[i]->device_vendor,dev_list[i]->device_ic,dev_list[i]->device_version,info1,DEVINFO_UNUSED );	//unused device regist
		}
		}
