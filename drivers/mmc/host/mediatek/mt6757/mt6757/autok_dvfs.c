...
int sdio_autok_res_exist(struct msdc_host *host)
{
	...
	if (filp == NULL) {
		pr_debug("autok result open fail\n");
		return 0;
	}
...
int sdio_autok_res_apply(struct msdc_host *host, int vcore)
{
	...
	if (size == TUNING_PARAM_COUNT) {
		...
		for (i = 1; i < TUNING_PARAM_COUNT; i++)
			pr_debug("autok result exist!, result[%d] = %d\n", i,
				 res[i]);
		ret = 0;
	}
...
int sdio_autok_res_save(struct msdc_host *host, int vcore, u8 *res)
{
	...
	if (vcore <= AUTOK_VCORE_LOW) {
		memcpy((void *)host->autok_res[AUTOK_VCORE_LOW],
		       (const void *)res, TUNING_PARAM_COUNT);
		filp = msdc_file_open(SDIO_AUTOK_LOW_RES_PATH,
				      O_CREAT | O_WRONLY, 0644);
	} else {
		memcpy((void *)host->autok_res[AUTOK_VCORE_HIGH],
		       (const void *)res, TUNING_PARAM_COUNT);
		filp = msdc_file_open(SDIO_AUTOK_HIGH_RES_PATH,
				      O_CREAT | O_WRONLY, 0644);
	}
...
int wait_sdio_autok_ready(void *data) { return 0; }
...
void sdio_autok_wait_dvfs_ready(void)
{
	...
	while (dvfs == 0) {
		pr_debug("DVFS not ready\n");
		msleep(100);
		dvfs = is_vcorefs_can_work();
	}

	if (dvfs == -1)
		pr_debug("DVFS feature not enable\n");
...
int sd_execute_dvfs_autok(struct msdc_host *host, u32 opcode, u8 *res)
{
	...
	if (host->mmc->ios.timing == MMC_TIMING_UHS_SDR104 ||
		...
		if (host->is_autok_done == 0) {
			pr_debug("[AUTOK]SDcard autok\n");
			ret = autok_execute_tuning(
			    host, host->autok_res[AUTOK_VCORE_HIGH]);
...
void sdio_execute_dvfs_autok(struct msdc_host *host)
{
	...
	if (host->is_autok_done) {
		...
		if (vcorefs_get_hw_opp() == OPPI_PERF)
			...
			autok_tuning_parameter_init(
			    host, host->autok_res[AUTOK_VCORE_LOW]);
	...
	pr_debug("[AUTOK]SDIO SDR104 Tune\n");
	...
	if (vcorefs_request_dvfs_opp(KIR_AUTOK_SDIO, OPPI_PERF) != 0)
		pr_debug("vcorefs_request_dvfs_opp@OPPI_PERF fail!\n");
	if (sdio_res_exist)
	...
	if (vcorefs_request_dvfs_opp(KIR_AUTOK_SDIO, OPPI_LOW_PWR) != 0)
		pr_debug("vcorefs_request_dvfs_opp@OPPI_LOW_PWR fail!\n");
	if (sdio_res_exist)
	...
	if (vcorefs_request_dvfs_opp(KIR_AUTOK_SDIO, OPPI_UNREQ) != 0)
		pr_debug("vcorefs_request_dvfs_opp@OPPI_UNREQ fail!\n");
...
int emmc_autok(void)
{
	...
	if (mmc == NULL) {
		pr_debug("eMMC device not ready\n");
		return -1;
	}
...
int sdio_autok(void)
{
	...
	if (!wait_for_completion_timeout(&host->autok_done, 10 * HZ)) {
		pr_debug("SDIO wait device autok ready timeout");
		return -1;
	}

	pr_debug("sdio autok done!");
...
void msdc_dump_autok(struct msdc_host *host)
{
	...
	for (i = AUTOK_VCORE_HIGH; i >= AUTOK_VCORE_LOW; i--) {
		...
		pr_info("[AUTOK]DAT [RDAT_EDGE:%d RD_FIFO_EDGE:%d "
			"WD_FIFO_EDGE:%d]\r\n",
