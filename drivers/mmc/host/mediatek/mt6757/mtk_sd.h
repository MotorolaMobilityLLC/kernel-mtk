...
enum { OPER_TYPE_READ, OPER_TYPE_WRITE, OPER_TYPE_NUM };
...
#define sdc_send_cmdq_cmd(opcode, arg)                                         \
	do {                                                                   \
		MSDC_SET_FIELD(EMMC51_CFG0,                                    \
			       MSDC_EMMC51_CFG_CMDQEN | MSDC_EMMC51_CFG_NUM |  \
				   MSDC_EMMC51_CFG_RSPTYPE |                   \
				   MSDC_EMMC51_CFG_DTYPE,                      \
			       (0x81) | (opcode << 1));                        \
...
#define msdc_reset(id)                                                         \
	do {                                                                   \
		...
		msdc_retry(MSDC_READ32(MSDC_CFG) & MSDC_CFG_RST, retry, cnt,   \
			   id);                                                \
	} while (0)
...
int msdc_clk_stable(struct msdc_host *host, u32 mode, u32 div, u32 hs400_src);
...
#define check_mmc_cmd001213(opcode)                                            \
	((opcode == MMC_GO_IDLE_STATE) || (opcode == MMC_STOP_TRANSMISSION) || \
...
#define check_mmc_cmd2425(opcode)                                              \
	((opcode == MMC_WRITE_MULTIPLE_BLOCK) || (opcode == MMC_WRITE_BLOCK))
...
#define check_mmc_cmd46(opcode) (opcode == MMC_READ_REQUESTED_QUEUE)
#define check_mmc_cmd47(opcode) (opcode == MMC_WRITE_REQUESTED_QUEUE)
#define check_mmc_cmd4647(opcode)                                              \
	((opcode == MMC_READ_REQUESTED_QUEUE) ||                               \
	 (opcode == MMC_WRITE_REQUESTED_QUEUE))
#define check_mmc_cmd48(opcode) (opcode == MMC_CMDQ_TASK_MGMT)
#define check_mmc_cmd44(x) ((x) && ((x)->opcode == MMC_SET_QUEUE_CONTEXT))
#define check_mmc_cmd13_sqs(x)                                                 \
	(((x)->opcode == MMC_SEND_STATUS) && ((x)->arg & (1 << 15)))
#define check_mmc_cmd_r1b(opcode)                                              \
	((opcode == MMC_SWITCH) || (opcode == MMC_CMDQ_TASK_MGMT))
