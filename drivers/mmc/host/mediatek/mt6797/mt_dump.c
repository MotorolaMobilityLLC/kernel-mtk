...
#ifndef CONFIG_OF
static void __iomem *u_msdc_base[HOST_MAX_NUM] = {
	MSDC0_BASE, MSDC1_BASE, MSDC2_BASE, MSDC3_BASE
};
#endif
...
static unsigned int simp_mmc_enable_clk(struct simp_mmc_host *host)
{
	...
	if (NULL == apmixed_reg_base || NULL == topckgen_reg_base
		|| NULL == pericfg_reg_base) {
		pr_err("apmixed_reg_base=%p, topckgen_reg_base=%p,pericfg_reg_base=%p\n",
...
static unsigned int simple_mmc_attach_mmc(struct simp_mmc_host *host)
{
	...
	if (!host2 || !host2->mmc || !host2->mmc->card) {
		...
		MTK_DUMP_PR_DBG("[%s]: cache is not enabled, no need to disable it\n",
			__func__);
	}
#endif
	 ...
	 */
	if (ocr & 0x7F) {
		MTK_DUMP_PR_DBG("msdc0: card claims to support voltages below the defined range");
		MTK_DUMP_PR_DBG(" These will be ignored.\n");
		ocr &= ~0x7F;
	}
...
static int simp_mmc_init(int card_type, bool boot)
{
	...
	pr_notice("msdc @ 0x%p, id:%d\n", cur_msdc_host->base, id);
#else
	cur_msdc_host->base = u_msdc_base[id];
#endif
	cur_msdc_host->clksrc = MSDC_CLKSRC;
	cur_msdc_host->clk = clks[MSDC_CLKSRC];
