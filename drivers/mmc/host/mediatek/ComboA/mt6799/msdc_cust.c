...
static void msdc_dump_clock_sts_core(struct msdc_host *host, struct seq_file *m)
{
	...
	if (topckgen_base && pericfg_base) {
		buf_ptr += sprintf(buf_ptr,
			"MSDC0 CLK_MUX@0x%p= 0x%x, CG[0x%p] = (%d, %d), CG[0x%p] = (%d, %d)\n",
			topckgen_base + 0x140, /* mux at bits16~18 */
			(MSDC_READ32(topckgen_base + 0x140) >> 16) & 7,
			topckgen_base + 0x140, /* cg at bit 15, 23 */
			(MSDC_READ32(topckgen_base + 0x140) >> 15) & 1,
			(MSDC_READ32(topckgen_base + 0x140) >> 23) & 1,
			pericfg_base + 0x290, /* cg at bit 0, 24~26 */
			(MSDC_READ32(pericfg_base + 0x290) >> 0) & 1,
			(MSDC_READ32(pericfg_base + 0x290) >> 24) & 7);
		buf_ptr += sprintf(buf_ptr,
			"MSDC1 CLK_MUX@0x%p= 0x%x, CG[0x%p] = %d, CG[0x%p] = (%d, %d)\n",
			topckgen_base + 0x140, /* mux at bits24~26 */
			(MSDC_READ32(topckgen_base + 0x140) >> 24) & 7,
			topckgen_base + 0x140, /* cg at bit31 */
			(MSDC_READ32(topckgen_base + 0x140) >> 31) & 1,
			pericfg_base + 0x290, /*cg at bit 1, 27 */
			(MSDC_READ32(pericfg_base + 0x290) >> 1) & 1,
			(MSDC_READ32(pericfg_base + 0x290) >> 27) & 1);
		buf_ptr += sprintf(buf_ptr,
			"MSDC3 CLK_MUX@0x%p= 0x%x, CLK_CG[0x%p] = (%d, %d), CLK_CG[0x%p] = (%d, %d)\n",
			topckgen_base + 0x150, /* mux at bits8~10 */
			(MSDC_READ32(topckgen_base + 0x150) >> 8) & 7,
			topckgen_base + 0x150, /* cg at bit 15, 23 */
			(MSDC_READ32(topckgen_base + 0x150) >> 15) & 1,
			(MSDC_READ32(topckgen_base + 0x150) >> 23) & 1,
			pericfg_base + 0x290,  /* cg at bit 3, 29 */
			(MSDC_READ32(pericfg_base + 0x290) >> 3) & 1,
			(MSDC_READ32(pericfg_base + 0x290) >> 29) & 1);
		*buf_ptr = '\0';
		if (!m)
			pr_info("%s", buffer);
		else
		...
		if (CHIP_IS_VER1()) {
			while (sdc_is_busy()) {
				if ((host->use_hw_dvfs == 1) && (MSDC_READ32(MSDC_CFG) >> 28 == 0x5)) {
					pr_notice("%s: sdc_busy when clock enable!\n", __func__);
...
void msdc_dump_padctl_by_id(u32 id)
{
	...
	if (id == 0) {
		pr_info("MSDC0 MODE10[0x%p] =0x%8x\tshould: 0x1???????\n",
			MSDC0_GPIO_MODE10, MSDC_READ32(MSDC0_GPIO_MODE10));
		pr_info("MSDC0 MODE11[0x%p] =0x%8x\tshould: 0x11111111\n",
			MSDC0_GPIO_MODE11, MSDC_READ32(MSDC0_GPIO_MODE11));
		pr_info("MSDC0 MODE12[0x%p] =0x%8x\tshould: 0x?????111\n",
