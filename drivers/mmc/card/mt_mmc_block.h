...
void mt_biolog_mmcqd_req_start(void);
...
struct mt_bio_context_task {
	int task_id;
	u32 arg;
	uint64_t request_start_t;     /* DMA start time */
	uint64_t request_duration_t;  /* DMA duration time */
	uint64_t wait_start_t;   /* Wait ready start time */
};
