...
void wdt_pmic_full_reset(struct ufs_hba *hba)
{
	...
	/*
	 *	Cmd issue to PMIC on MT6763 will take around 20us ~ 30us, in
	 *order
	 * to speed up VEMC disable time,
	 * we disable VEMC first coz PMIC cold reset may take longer to
	 *disable
	 * VEMC in it's reset flow.
	 * Can not use regulator_disable() here because it can not use in
 ...
 * Release all resources: DRAM/26M clk/Main PLL and dsiable 26M ref clk if in
 * H8.
 *
...
int ufs_mtk_pltfrm_deepidle_check_h8(void)
{
	 ...
	 * Plus deepidle/SODI can not enter in ufs suspend/resume callback by
	 * idle_lock_by_ufs()
	 * Therefore, it's guranteed that UFS is in H8 now and 26MHz ref clk is
	 * disabled by suspend callback
	 * deepidle/SODI do not need to disable 26MHz ref clk here.
	 * Not use hba->uic_link_state to judge it's after ufshcd_suspend() is
	 * because
	...
	ret = ufs_mtk_generic_read_dme(UIC_CMD_DME_GET, VENDOR_POWERSTATE, 0,
				       &tmp, 100);

	if (ret) {
		/* ret == -1 means there is outstanding req/task/uic/pm
		 * ongoing,
		 * not an error
		 */
		if (ret != -1)
	...
	if (tmp == VENDOR_POWERSTATE_HIBERNATE) {
		/* delay 100us before DeepIdle/SODI disable XO_UFS for Toshiba
		 * device
		 */
...
/* SSPM project will disable MPHY 26MHz ref clock in SSPM deepidle/SODI IPI
 * handler
 */
...
/* SSPM project will enable MPHY 26MHz ref clock in SSPM deepidle/SODI IPI
 * handler
 */
	 ...
	 */
	/* a. runtime or system pm b. ufshcd_shutdown */
	/* And deepidle/SODI can not enter in ufs suspend/resume callback by
	 * idle_lock_by_ufs()
	 */
	/* Therefore, it's guranteed that UFS is in H8 now and 26MHz ref clk is
	 * disabled by suspend callback
...
int ufs_mtk_pltfrm_init(void) { return 0; }
...
int ufs_mtk_pltfrm_parse_dt(struct ufs_hba *hba)
{
	...
	if (node_gpio) {
		...
		if (IS_ERR(*(void **)&ufs_mtk_mmio_base_gpio)) {
			...
			dev_err(hba->dev,
				"error: ufs_mtk_mmio_base_gpio init fail\n");
	...
	if (node_pericfg) {
		...
		if (IS_ERR(*(void **)&ufs_mtk_mmio_base_pericfg)) {
			...
			dev_err(hba->dev,
				"error: mmio_base_pericfg init fail\n");
	...
	if (node_ufs_mphy) {
		...
		if (IS_ERR(*(void **)&ufs_mtk_mmio_base_ufs_mphy)) {
			...
			dev_err(
			    hba->dev,
			    "error: ufs_mtk_mmio_base_ufs_mphy init fail\n");
	...
	ufs_mtk_pins_va09_on =
	    pinctrl_lookup_state(ufs_mtk_pinctrl, "state_va09_on");
	...
	ufs_mtk_pins_va09_off =
	    pinctrl_lookup_state(ufs_mtk_pinctrl, "state_va09_off");
...
int ufs_mtk_pltfrm_res_req(struct ufs_hba *hba, u32 option)
{
	if (option == UFS_MTK_RESREQ_DMA_OP) {
		...
		ufshcd_vops_deepidle_resource_req(
		    hba, SPM_RESOURCE_MAINPLL | SPM_RESOURCE_DRAM |
			     SPM_RESOURCE_CK_26M);

	} else if (option == UFS_MTK_RESREQ_MPHY_NON_H8) {

		/* request resource for mphy not in H8, e.g., main PLL, 26 mhz
		 * clock
		 */

		ufshcd_vops_deepidle_resource_req(
		    hba, SPM_RESOURCE_MAINPLL | SPM_RESOURCE_CK_26M);
	}
