...
#if defined(CONFIG_MTK_PMIC_CHIP_MT6355) /*--for (MT6757)--*/
...
void long_press_reboot_function_init_pmic(const char *mode)
{
	...
	pmic_set_register_value(KPD_PMIC_RG_PWRKEY_RST_TD,
...
void mt_eint_register(void)
{
	...
	if (!node)
		...
		ret = of_property_read_u32_array(node, "debounce", ints,
						 ARRAY_SIZE(ints));
		if (ret)
		...
		ret =
		    request_irq(mrdump_ext_rst_irq, mrdump_rst_eint_handler,
