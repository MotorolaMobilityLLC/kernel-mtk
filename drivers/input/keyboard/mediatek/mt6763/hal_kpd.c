...
#endif
static u16 kpd_keymap_state[KPD_NUM_MEMS] = {0xffff, 0xffff, 0xffff, 0xffff,
					     0x00ff};
...
void sb_kpd_enable(void)
{
	kpd_print("%s empty function for HAL!\n", __func__);
}
...
void kpd_slide_qwerty_init(void)
{
	...
	if (!power_op)
		kpd_print(KPD_SAY
			  "Qwerty slide pin interface power on fail\n");
	else
	...
	if (!power_op)
		kpd_print(KPD_SAY
			  "Qwerty slide pin interface power off fail\n");
	else
...
static void kpd_factory_mode_handler(void)
{
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			input_report_key(kpd_input_dev, linux_keycode,
					 pressed);
...
/* FM@suspend */
bool __attribute__((weak)) ConditionEnterSuspend(void) { return true; }
...
void kpd_init_keymap_state(u16 keymap_state[])
{
	...
	kpd_info("init_keymap_state done: %x %x %x %x %x!\n", keymap_state[0],
		 keymap_state[1], keymap_state[2], keymap_state[3],
		 keymap_state[4]);
}
...
void kpd_pmic_pwrkey_hal(unsigned long pressed)
{
	...
	if (!kpd_sb_enable) {
		input_report_key(kpd_input_dev, kpd_dts_data.kpd_sw_pwrkey,
				 pressed);
		input_sync(kpd_input_dev);
		if (kpd_show_hw_keycode) {
