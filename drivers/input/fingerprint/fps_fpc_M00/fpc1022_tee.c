...
struct spi_device *spi_fingerprint = NULL;
...
static void fpc1022_get_irqNum(struct fpc1022_data *fpc1022)
{
	...
	if (node) {
		...
		fpc1022->irq_gpio = of_get_named_gpio(node, "fpc,gpio-irq", 0);
		printk("%s , fpc1022->irq_num = %d, fpc1022->irq_gpio = %d\n",
		       __func__, fpc1022->irq_num, fpc1022->irq_gpio);
		if (!fpc1022->irq_num)
			printk("irq_of_parse_and_map fail!!\n");

        fpc1022->power_gpio = of_get_named_gpio(node, "fpc,gpio-power", 0);
        printk("%s , fpc1022->power_gpio = %d",
               __func__, fpc1022->power_gpio);
        ret = devm_gpio_request(fpc1022->dev, fpc1022->power_gpio, "fpc,gpio-power");
        if (ret) {
            printk("request power gpio failed, ret= %d",ret);
        }
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(!strncmp(buf, "enable", strlen("enable")) && fpc1022->compatible_enabled != 1){

		
		   fpc1022->pinctrl = devm_pinctrl_get(&pdev->dev);
			if (IS_ERR(fpc1022->pinctrl)) {
				dev_err(dev, "Cannot find pinctrl!");
				ret = PTR_ERR(fpc1022->pinctrl);
			        devm_kfree(&pdev->dev, fpc1022);
				goto exit;
			}
			
			fpc1022->st_irq = pinctrl_lookup_state(fpc1022->pinctrl, "fpsensor_spi_default");
			if (IS_ERR(fpc1022->st_irq)) {
				ret = PTR_ERR(fpc1022->st_irq);
				dev_err(&pdev->dev, "pinctrl err, irq\n");
				//goto err_lookup_state;
			}
		
			fpc1022->st_rst_h =
				pinctrl_lookup_state(fpc1022->pinctrl, "fpsensor_rst_high");
			if (IS_ERR(fpc1022->st_rst_h)) {
				ret = PTR_ERR(fpc1022->st_rst_h);
				dev_err(&pdev->dev, "pinctrl err, rst_high\n");
				devm_kfree(&pdev->dev, fpc1022);
				goto exit;
			}
		
			fpc1022->st_rst_l = pinctrl_lookup_state(fpc1022->pinctrl, "fpsensor_rst_low");
			if (IS_ERR(fpc1022->st_rst_l)) {
				ret = PTR_ERR(fpc1022->st_rst_l);
				dev_err(&pdev->dev, "pinctrl err, rst_low\n");
				devm_kfree(&pdev->dev, fpc1022);
				goto exit;
			}
            ...
            gpio_direction_output(fpc1022->power_gpio, 1);
            printk("the value after set power_gpio is %d\n", gpio_get_value(fpc1022->power_gpio));

		    fpc1022->compatible_enabled = 1;

		    hw_reset(fpc1022);
		
	        dev_info(dev, "%s: ok\n", __func__);
	}else if(!strncmp(buf, "disable", strlen("disable")) && fpc1022->compatible_enabled != 0){
		if (gpio_is_valid(fpc1022->irq_gpio))
		{
			devm_gpio_free(dev, fpc1022->irq_gpio);
			dev_info(dev, "remove irq_gpio success\n");
		}
		devm_free_irq(dev, gpio_to_irq(fpc1022->irq_gpio), fpc1022);
		fpc1022->compatible_enabled = 0;
	}
	return count;
exit:
	return -EINVAL;
...
static int fpc1022_platform_probe(struct platform_device *pldev)
{
    ...
    printk(KERN_INFO "%s no-1\n", __func__);

	if (!np) {
		dev_err(dev, "no of node found\n");
        printk(KERN_INFO "%s no-2\n", __func__);
		ret = -EINVAL;
		return ret;
	}
	...
	dev_info(dev, "%s: ok\n", __func__);
    printk(KERN_INFO "%s no-14\n", __func__);

	return ret;
#endif
    printk(KERN_INFO "%s no-15\n", __func__);
...
static int check_hwid(struct spi_device *spi)
{
	...
	do {
		...
		printk(KERN_INFO "%s, fpc chip version is 0x%x, 0x%x\n",
		...
		if ((FPC_CHIP_MASK_SENSOR_TYPE & hardware_id) ==
		    FPC_CHIP) {
...
static int __init fpc1022_init(void)
{
	...
	if (0 != platform_driver_register(&fpc1022_driver)) {
        ...
        /*if (!fpc1022) {
            printk("no-16 the value of fpc1022 is %d\n", !fpc1022);
            return -EINVAL;
        }*/
