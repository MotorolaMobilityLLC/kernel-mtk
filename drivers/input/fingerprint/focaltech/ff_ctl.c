...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FF_IOC_SET_IC_INFORMATION: {
		ic_information_t ic_info;
        if (copy_from_user(&ic_info, (ic_information_t *)arg, sizeof(ic_information_t))) {
            FF_LOGE("copy_from_user(..) failed.");
            err = (-EFAULT);
            break;
        }
		FF_LOGD("ic_info chipid= %s ca_version = %s\n", 
						ic_info.ic_name, ic_info.ca_version);
		FF_LOGD("ic_info ta_version = %s\n", ic_info.ta_version);
