...
const char *ff_err_strerror(int err)
{
	...
	switch (err) {
		...
		default: sprintf(errstr, "%d", err); break;
	}
...
static int ff_ctl_init_driver(void)
{
	...
	do {
		...
		if (err) {
			FF_LOGE("error enable_irq_wake(%d) = %d.\n", g_context->irq_num, err);
		}
		printk("%s +%d, enable_irq_wake(%d) = %d.\n", __func__, __LINE__, g_context->irq_num, err);
