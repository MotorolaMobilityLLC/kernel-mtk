...
extern int set_clks(bool enable);
...
struct of_device_id egistec_match_table_platform[] = {
	{
...
// struct of_device_id egistec_match_table_spi[] = {
//     {
//         .compatible = "mediatek,egis",
//     },
//     {},
// };
...
static struct platform_driver egisfp_driver = {
	.driver = {
		.name = EGIS_DEV_NAME,
		.owner = THIS_MODULE,
		.of_match_table = egistec_match_table_platform,
	},
	.probe = egisfp_probe,
...
// 		.owner = THIS_MODULE,
// 		.of_match_table = egistec_match_table_spi,
// 	},
...
long egisfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GET_SCREEN_ONOFF:
		...
		break;
	case GET_HWINFO:
		DEBUG_PRINT(" %s : GET_HWINFO,add hwinfo for egis \n", __func__);
		...
		if (!egis_dev->platforminit_done)
		{
			...
			if (egis_dev->ctrl_power)
			{
				if (egis_dev->pwr_by_gpio)
				{
					...
					status = gpio_request(egis_dev->vcc_33v_Pin, "egistec,gpio_vcc_en");
					if (status < 0)
					{
						ERROR_PRINT(" %s : gpio_requset egistec,gpio_vcc_en pin failed \n", __func__);
			...
			/* Initial Reset Pin */
			status = gpio_request(egis_dev->rstPin, "egistec,gpio_reset");
			if (status < 0)
			{
				ERROR_PRINT(" %s : gpio_requset egistec,gpio_reset pin failed \n", __func__);
				goto egisfp_reset_request_fail;
			}

			status = gpio_direction_output(egis_dev->rstPin, 0);

			status = gpio_request(egis_dev->irqPin, "egistec,gpio_irq");
			if (status < 0)
			{
				ERROR_PRINT(" %s : gpio_requset egistec,gpio_irq pin failed \n", __func__);
			...
			if (egis_dev->dd)
			{
                ...
                pinctrl_node = of_find_compatible_node(NULL, NULL, "mediatek,fingerprint-pinctrl");
                if (pinctrl_node){
                    pdev = of_find_device_by_node(pinctrl_node);
                }else{
                    ERROR_PRINT(" %s : cannot find mediatek,fingerprint-pinctrl node \n", __func__);
                }
				egis_dev->pinctrl = devm_pinctrl_get(&pdev->dev);
				...
				egis_dev->reset_high = pinctrl_lookup_state(egis_dev->pinctrl, "fpsensor_rst_high");
				if (IS_ERR(egis_dev->reset_high))
				{
					status = PTR_ERR(egis_dev->reset_high);
			 		ERROR_PRINT(" %s : can't find fingerprint pinctrl fpsensor_rst_high \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->reset_low = pinctrl_lookup_state(egis_dev->pinctrl, "fpsensor_rst_low");
				if (IS_ERR(egis_dev->reset_low))
				{
					status = PTR_ERR(egis_dev->reset_low);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl fpsensor_rst_low \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				if (pinctrl_select_state(egis_dev->pinctrl, egis_dev->reset_low))
					goto egisfp_pinctrl_fail;
				egis_dev->irq_active = pinctrl_lookup_state(egis_dev->pinctrl, "fpsensor_irq_active");
				if (IS_ERR(egis_dev->irq_active))
				{
					status = PTR_ERR(egis_dev->irq_active);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl fpsensor_irq_active \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				...
				{
					egis_dev->vcc_high = pinctrl_lookup_state(egis_dev->pinctrl, "fpsensor_vcc_high");
					if (IS_ERR(egis_dev->vcc_high))
					{
						status = PTR_ERR(egis_dev->vcc_high);
						ERROR_PRINT(" %s : can't find fingerprint pinctrl fpsensor_vcc_high \n", __func__);
						goto egisfp_pinctrl_fail;
					}
					egis_dev->vcc_low = pinctrl_lookup_state(egis_dev->pinctrl, "fpsensor_vcc_low");
					if (IS_ERR(egis_dev->vcc_low))
					{
						status = PTR_ERR(egis_dev->vcc_low);
						ERROR_PRINT(" %s : can't find fingerprint pinctrl fpsensor_vcc_low \n", __func__);
						goto egisfp_pinctrl_fail;
					}
			...
			/*if (egis_dev->dd)
			{
				...
				egis_dev->spi_active = pinctrl_lookup_state(egis_dev->pinctrl, "fpsensor_spi_active");
				if (IS_ERR(egis_dev->spi_active))
				{
					status = PTR_ERR(egis_dev->spi_active);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl fpsensor_spi_active \n", __func__);
					goto egisfp_pinctrl_fail;
				}
				egis_dev->spi_default = pinctrl_lookup_state(egis_dev->pinctrl, "fpsensor_spi_default");
				if (IS_ERR(egis_dev->spi_default))
				{
					status = PTR_ERR(egis_dev->spi_default);
			 		ERROR_PRINT(" %s : can't find fingerprint pinctrl fpsensor_spi_default \n", __func__);
					goto egisfp_pinctrl_fail;
				}
...
int egisfp_parse_dt(struct egisfp_dev_t *egis_dev)
{
	...
	struct device_node *node = of_find_compatible_node(NULL, NULL, "mediatek,egis-eint");
