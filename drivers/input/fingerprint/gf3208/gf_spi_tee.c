...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	gf_dev->ldo_1v8_off = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_ldo1v8_off");
    if (IS_ERR(gf_dev->ldo_1v8_off)) {
        ret = PTR_ERR(gf_dev->ldo_1v8_off);
        pr_info("%s can't find fingerprint pinctrl ldo_1v8_off\n", __func__);
        return ret;
    }
    gf_dev->ldo_1v8_on = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_ldo1v8_on");
    if (IS_ERR(gf_dev->ldo_1v8_on)) {
        ret = PTR_ERR( gf_dev->ldo_1v8_on);
        pr_info("%s can't find fingerprint pinctrl ldo_1v8_on\n", __func__);
        return ret;
    }
    gf_dev->ldo_3v3_off = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_ldo3v3_off");
    if (IS_ERR(gf_dev->ldo_3v3_off)) {
        ret = PTR_ERR(gf_dev->ldo_3v3_off);
        pr_info("%s can't find fingerprint pinctrl ldo_3v3_off\n", __func__);
        return ret;
    }
    gf_dev->ldo_3v3_on = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_ldo3v3_on");
    if (IS_ERR(gf_dev->ldo_3v3_on)) {
        ret = PTR_ERR(gf_dev->ldo_3v3_on);
...
static void gf_hw_power_enable(struct gf_device *gf_dev ,u8 onoff)
{
   ...
   // pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->ldo_1v8_off);
    //pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->ldo_3v3_off);
    		enable = 1;
	}
}
