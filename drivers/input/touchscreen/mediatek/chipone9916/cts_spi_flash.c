...
int cts_prepare_flash_operation(struct cts_device *cts_dev)
{
	...
	if (ret == 0 && diva == 0x0C)
		...
		do {
			...
			ret =
			    cts_hw_reg_readb_relaxed(cts_dev, 0x3401B, &state);
			 ...
			 u32 flash_addr, void *dst, size_t size, int retry)
{
	...
	if (flash == NULL ||
	    sfctrl == NULL ||
	    sfctrl->ops == NULL || sfctrl->ops->read == NULL) {
				 ...
				 int retry)
{
	...
	if (flash == NULL ||
	    sfctrl == NULL ||
	    sfctrl->ops == NULL || sfctrl->ops->read_to_sram == NULL) {
	...
	cts_info("Read to sram actually from 0x%06x size %zu", flash_addr,
		 size);
	...
	do {
		retries++;
		ret =
		    sfctrl->ops->read_to_sram(cts_dev, flash_addr, sram_addr,
					      size);
		if (ret) {
			cts_err
					   ...
					   size_t size, u32 crc, int retry)
{
	...
	do {
		...
		ret =
		    sfctrl->ops->calc_sram_crc(cts_dev, sram_addr, size,
					       &crc_sram);
		if (ret) {
			cts_err
			    ("Get crc for read from 0x%06x to sram 0x%06x size %zu "
			     "failed %d retries %d", flash_addr, sram_addr,
			     size, ret, retries);
			continue;
		}
		...
		cts_err
		    ("Check crc for read from 0x%06x to sram 0x%06x size %zu "
		     "mismatch %x != %x, retries %d", flash_addr, sram_addr,
		     size, crc, crc_sram, retries);
		ret = -EFAULT;
	} while (retries < retry);
		      ...
		      u32 flash_addr, const void *src, size_t size)
{
	...
	if (flash == NULL ||
	    sfctrl == NULL ||
	    sfctrl->ops == NULL || sfctrl->ops->program == NULL) {
	...
	while (size) {
		...
		if (ret) {
			cts_err("Program to 0x%06x size %zu failed %d",
				flash_addr, l, ret);
			return ret;
		}
				...
				u32 flash_addr, u32 sram_addr, size_t size)
{
	...
	if (flash == NULL ||
	    sfctrl == NULL ||
	    sfctrl->ops == NULL || sfctrl->ops->program_from_sram == NULL) {
	...
	while (size) {
		...
		ret =
		    sfctrl->ops->program_from_sram(cts_dev, flash_addr,
						   sram_addr, l);
		if (ret) {
			cts_err
			    ("Program to 0x%06x from sram 0x%06x size %zu failed %d",
