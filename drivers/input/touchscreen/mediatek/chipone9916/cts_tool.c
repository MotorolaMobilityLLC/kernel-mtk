...
#define RAWDATA_BUFFER_SIZE(cts_dev) \
	...
	case CTS_TOOL_CMD_TCS_READ_HW_CMD:
		cts_dev->ops->read_hw_reg(cts_dev,
					  get_unaligned_le32(cmd->addr),
					  cmd->data, cmd->data_len);
		break;
	case CTS_TOOL_CMD_TCS_READ_DDI_CMD:
		cts_dev->ops->read_ddi_reg(cts_dev,
					   get_unaligned_le32(cmd->addr),
					   cmd->data, cmd->data_len);
		break;
	case CTS_TOOL_CMD_TCS_READ_FW_CMD:
		cts_dev->ops->read_reg(cts_dev, get_unaligned_le16(cmd->tcs),
				       cmd->data, cmd->data_len);
		break;
	...
	case CTS_TOOL_CMD_GET_BASE_DATA:
		...
		if (cmd->cmd == CTS_TOOL_CMD_GET_RAW_DATA) {
			...
			ret =
			    cts_dev->ops->top_get_rawdata(cts_dev,
							  (u8 *) cmd->data,
							  RAWDATA_BUFFER_SIZE
							  (cts_dev));
			...
			ret =
			    cts_dev->ops->top_get_manual_diff(cts_dev,
							      (u8 *) cmd->data,
							      RAWDATA_BUFFER_SIZE
							      (cts_dev));
			...
			ret =
			    cts_dev->ops->top_get_basedata(cts_dev,
							   (u8 *) cmd->data,
							   RAWDATA_BUFFER_SIZE
							   (cts_dev));
		}
	...
	case CTS_TOOL_CMD_I2C_DIRECT_READ:
		{
			...
			while (left_size) {
				...
				if (ret) {
					cts_err
					    ("Direct read i2c_addr 0x%02x addr 0x%06x "
			      ...
			      loff_t *ppos)
{
	...
	cts_info
	    ("write: flag:%d, addr:0x%06x, tcmd:0x%04x data:0x%02x, datalen:%d",
	     cmd->cmd, get_unaligned_le32(cmd->addr),
	...
	if (cmd->cmd & BIT(0)) {
		if (cmd->data_len) {
			ret =
			    copy_from_user(cmd->data,
					   buffer + CTS_TOOL_CMD_HEADER_LENGTH,
					   cmd->data_len);
			if (ret) {
				cts_err
	...
	case CTS_TOOL_CMD_TCS_WRITE_HW_CMD:
		cts_dev->ops->write_hw_reg(cts_dev,
					   get_unaligned_le32(cmd->addr),
					   cmd->data, cmd->data_len);
		break;
	case CTS_TOOL_CMD_TCS_WRITE_DDI_CMD:
		cts_dev->ops->write_ddi_reg(cts_dev,
					    get_unaligned_le32(cmd->addr),
					    cmd->data, cmd->data_len);
		break;
	case CTS_TOOL_CMD_TCS_WRITE_FW_CMD:
		cts_dev->ops->write_reg(cts_dev, get_unaligned_le16(cmd->tcs),
					cmd->data, cmd->data_len);
		break;
			   ...
			   unsigned long arg)
{
	...
	case CTS_TOOL_IOCTL_GET_ROW_COL:{
		u16 rows_cols = (cts_dev->fwdata.rows << 8)
		       | cts_dev->fwdata.cols;
