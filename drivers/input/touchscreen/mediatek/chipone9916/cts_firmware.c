...
const struct cts_firmware *cts_request_driver_builtin_firmware_by_name(const
								       char
								       *name)
{
...
const struct cts_firmware *cts_request_driver_builtin_firmware_by_index(u32
									index)
{
	const struct cts_firmware *firmware;
...
static const struct cts_firmware *cts_request_newer_driver_builtin_firmware(u32
									    hwid,
									    u16
									    fwid,
									    u16
			       ...
			       u8 *buf, bool calc_crc, u32 crc)
{
		...
		if (ret) {
			cts_err
			    ("Calc data in flash from 0x%06x size %zu crc failed %d, "
			     "try to validate by direct readback", flash_addr,
			     size, ret);
	...
	if (ret) {
		cts_err("Read flash from 0x%06x size %zu failed %d", flash_addr,
			size, ret);
		goto err_free_buf;
	}
	...
	for (i = 0; i < size; i++) {
		if (buf[i] != data[i]) {
			...
			if (ret < 100) {
				cts_err("  0x%06x: %02x %02x", i, buf[i],
					data[i]);
			} else if (ret == 100) {
				cts_err("  ...");
			}
			...
			const struct cts_firmware *firmware, bool to_flash)
{
	...
	if (!to_flash || !cts_dev->rtdata.has_flash) {
		...
		if (ret)
			cts_err("Write firmware section to sram failed %d",
				ret);
	...
	if (ret == 0 && firmware_info.ddiparam_sect_size != 0) {
		...
		do {
			...
			if (ret) {
				cts_err
				    ("Erase DDIParam section failed %d, retries %d",
				     ret, retries);
				continue;
			}
			...
			if (ret) {
				cts_err
				    ("Program DDIParam & crc section failed %d retries %d",
				     ret, retries);
			}
		} while (ret && retries < 3);
	}
