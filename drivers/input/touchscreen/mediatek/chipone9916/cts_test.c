...
static int is_mnt_enabled(struct cts_device *cts_dev, u8 *enabled)
{
	int ret;

	ret = cts_dev->ops->is_mnt_enabled(cts_dev, enabled);
	if (ret) {
		cts_err("Get firmware monitor mode failed %d", ret);
		return ret;
	}
	cts_info("Get firmware monitor mode: %d", *enabled);

	return 0;
}

static int set_mnt_enabled(struct cts_device *cts_dev, u8 enabled)
{
	int retries = 5;
	u8 new_enabled;
	int ret;

	cts_info("Set firmware monitor mode to %d", enabled);
	ret = cts_dev->ops->set_mnt_enable(cts_dev, enabled ? 0x01 : 0x00);
	if (ret) {
		cts_err("Set firmware monitor mode failed %d", ret);
		return ret;
	}

	while (retries--) {
		ret = is_mnt_enabled(cts_dev, &new_enabled);
		if (ret) {
			cts_err("Get firmware monitor mode failed %d", ret);
			return ret;
		}
		cts_info("Get firmware monitor mode %d", new_enabled);
	...
	return (new_enabled == enabled) ? 0 : -1;
}
...
static int prepare_test(struct cts_device *cts_dev)
{
	...
	ret = is_mnt_enabled(cts_dev, &old_mnt_enabled);
	if (ret) {
		cts_err("Get mnt enabled failed: %d", ret);
		return ret;
	}
	if (old_mnt_enabled) {
		cts_info("Set monitor mode to 0");
		ret = set_mnt_enabled(cts_dev, 0x00);
		if (ret) {
			cts_err("Disable firmware monitor mode failed %d", ret);
			return ret;
		}
	} else {
		cts_info("Monitor mode unsupported");
	}
...
int cts_test_rawdata(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	if (old_mnt_enabled) {
		cts_info("Set monitor mode to 1");
		ret = set_mnt_enabled(cts_dev, 0x01);
		if (ret) {
			cts_err("Restore firmware monitor mode failed %d", ret);
		}
	} else {
		cts_info("Monitor mode, do nothing");
	}
