...
static int cts_tcs_spi_read_pack(u8 *tx, TcsCmdValue_t *tcv, u16 rdatalen)
{
	...
	txhdr->cmd =
	    (tcv->baseFlag << 15) |
	    (tcv->isRead << 14) | (tcv->classID << 8) | (tcv->cmdID << 0);
				  ...
				  u8 *wdata, u16 wdatalen)
{
	...
	txhdr->cmd =
	    (tcv->baseFlag << 15) |
	    (tcv->isWrite << 13) | (tcv->classID << 8) | (tcv->cmdID << 0);
	...
	if (wdatalen > 0) {
		...
		*(tx + sizeof(tcs_tx_head) + wdatalen + 1) =
		    ((crc16 >> 8) & 0xFF);
		     ...
		     enum TcsCmdIndex cmdIdx, u8 *rdata, size_t rdatalen)
{
	...
	txlen =
	    cts_tcs_spi_read_pack(cts_dev->tx, TcsCmdValue + cmdIdx, rdatalen);
			  ...
			  enum TcsCmdIndex cmdIdx, u8 *rdata, size_t rdatalen)
{
     ...
     */
	crc16_recv =
	    (cts_dev->rx[rdatalen - 1] << 8) | cts_dev->rx[rdatalen - 2];
		      ...
		      enum TcsCmdIndex cmdIdx, u8 *wdata, size_t wdatalen)
{
	...
	txlen =
	    cts_tcs_spi_write_pack(cts_dev->tx, TcsCmdValue + cmdIdx, wdata,
				   wdatalen);
	/* dump_spi(">> ", cts_dev->tx, txlen); */
	cts_tcs_spi_xtrans(cts_dev, cts_dev->tx, txlen, cts_dev->rx,
			   sizeof(tcs_rx_tail));
			      ...
			      u16 *rawdata_target)
{
	...
	ret = cts_tcs_spi_read(cts_dev, TP_STD_CMD_CNEG_OPTIONS_RW, buf, sizeof(buf));
	if (!ret) {
...
int cts_tcs_enable_get_rawdata(const struct cts_device *cts_dev)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev,
			      TP_STD_CMD_SYS_STS_DAT_TRANS_IN_NORMAL_RW, buf,
			      sizeof(buf));
	if (ret != 0)
		return ret;
			...
			u8 *regbuf, size_t size)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev,
			      TP_STD_CMD_TP_DATA_OFFSET_AND_TYPE_CFG_RW, buf,
			      sizeof(buf));
	if (ret != 0)
		return ret;
			...
			u8 *regbuf, size_t size)
{
	...
	ret =
	    cts_tcs_spi_read(cts_dev, TP_STD_CMD_SYS_STS_READ_RO, regbuf, size);
	if (ret != 0)
		return ret;
			 ...
			 u8 *regbuf, size_t size)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev,
			      TP_STD_CMD_TP_DATA_OFFSET_AND_TYPE_CFG_RW, regbuf,
			      sizeof(buf));
	if (ret != 0)
		return ret;
...
#define INT_DATA_TYPE_U16_SIZ		\
	...
	if (data_types != INT_DATA_TYPE_NONE) {
		...
		if ((data_types & INT_DATA_TYPE_RAWDATA)) {
			data_size +=
			    (INT_DATA_TYPE_LEN_SIZ + INT_DATA_TYPE_U16_SIZ);
		}
		...
		if ((data_types & INT_DATA_TYPE_CNEGDATA)) {
			data_size +=
			    (INT_DATA_TYPE_LEN_SIZ + INT_DATA_TYPE_U8_SIZ);
		}
	}
...
static int cts_tcs_polling_data(const struct cts_device *cts_dev, u8 *buf,
				size_t size)
{
	int ret = -1;
	int retries = 100;
...
static int cts_tcs_polling_test_data(const struct cts_device *cts_dev, u8 *buf,
				     size_t size)
{
...
int cts_tcs_get_workmode(const struct cts_device *cts_dev, u8 *workmode)
{
	...
	ret =
	    cts_tcs_spi_read(cts_dev, TP_STD_CMD_SYS_STS_CURRENT_WORKMODE_RO,
			     buf, sizeof(buf));
	if (!ret) {
		*workmode = buf[0];
		return 0;
...
int cts_tcs_set_workmode(const struct cts_device *cts_dev, u8 workmode)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_SYS_STS_WORK_MODE_RW,
...
int cts_tcs_set_openshort_mode(const struct cts_device *cts_dev, u8 mode)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_OPENSHORT_MODE_SEL_RW, &mode,
			      sizeof(mode));
	if (!ret)
		return 0;
...
int cts_tcs_set_tx_vol(const struct cts_device *cts_dev, u8 txvol)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_SYS_STS_VSTIM_LVL_RW, &txvol,
			      sizeof(txvol));
	if (!ret)
		return 0;
				   ...
				   u8 *enabled)
{
	...
	ret =
	    cts_tcs_spi_read(cts_dev, TP_STD_CMD_SYS_STS_DAT_TRANS_IN_NORMAL_RW,
			     buf, sizeof(buf));
	if (!ret) {
		*enabled = buf[0];
		return 0;
...
int cts_tcs_set_short_test_type(const struct cts_device *cts_dev, u8 short_type)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_OPENSHORT_SHORT_SEL_RW,
...
int cts_tcs_is_openshort_enabled(const struct cts_device *cts_dev, u8 *enabled)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_OPENSHORT_EN_RW, buf,
			      sizeof(buf));
	if (!ret) {
		*enabled = buf[0];
		return 0;
...
int cts_tcs_set_openshort_enable(const struct cts_device *cts_dev, u8 enable)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_OPENSHORT_EN_RW, &enable,
			      sizeof(enable));
	if (!ret)
		return 0;
...
int cts_tcs_set_esd_enable(const struct cts_device *cts_dev, u8 enable)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_DDI_ESD_EN_RW, &enable,
			      sizeof(enable));
	if (!ret)
		return 0;
...
int cts_tcs_is_cneg_enabled(const struct cts_device *cts_dev, u8 *enabled)
{
	...
	ret =
	    cts_tcs_spi_read(cts_dev, TP_STD_CMD_CNEG_EN_RW, enabled,
			     sizeof(*enabled));
	if (!ret)
		return 0;
...
int cts_tcs_is_mnt_enabled(const struct cts_device *cts_dev, u8 *enabled)
{
	...
	ret =
	    cts_tcs_spi_read(cts_dev, TP_STD_CMD_MNT_EN_RW, enabled,
			     sizeof(*enabled));
	if (!ret)
		return 0;
...
int cts_tcs_set_cneg_enable(const struct cts_device *cts_dev, u8 enable)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_CNEG_EN_RW, &enable,
			      sizeof(enable));
	if (!ret)
		return 0;
...
int cts_tcs_set_mnt_enable(const struct cts_device *cts_dev, u8 enable)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_MNT_EN_RW, &enable,
			      sizeof(enable));
	if (!ret)
		return 0;
...
int cts_tcs_is_display_on(const struct cts_device *cts_dev, u8 *display_on)
{
	...
	ret =
	    cts_tcs_spi_read(cts_dev, TP_STD_CMD_OPENSHORT_SHORT_DISP_ON_EN_RW,
			     buf, sizeof(buf));
	if (!ret) {
		*display_on = buf[0];
		return 0;
...
int cts_tcs_set_pwr_mode(const struct cts_device *cts_dev, u8 pwr_mode)
{
	...
	buf[0] = pwr_mode;
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_SYS_STS_PWR_STATE_RW, buf, sizeof(buf));
	return ret;
}
...
int cts_tcs_set_display_on(const struct cts_device *cts_dev, u8 display_on)
{
	...
	ret =
	    cts_tcs_spi_write(cts_dev, TP_STD_CMD_OPENSHORT_SHORT_DISP_ON_EN_RW,
...
int cts_tcs_get_charger_plug(const struct cts_device *cts_dev, u8 *isset)
{
	...
	ret = cts_tcs_spi_read(cts_dev, TP_STD_CMD_SYS_STS_CHARGER_PLUGIN_RW, buf, sizeof(buf));
	if (!ret) {
		*isset = buf[0];
		return 0;
	}
...
int cts_tcs_get_earjack_plug(const struct cts_device *cts_dev, u8 *isset)
{
	...
	ret = cts_tcs_spi_read(cts_dev, TP_STD_CMD_SYS_STS_EP_PLUGIN_RW, buf, sizeof(buf));
	if (!ret) {
		*isset = buf[0];
		return 0;
	}
...
int cts_tcs_set_panel_direction(const struct cts_device *cts_dev, u8 direction)
{
	...
	ret = cts_tcs_spi_write(cts_dev, TP_STD_CMD_SYS_STS_PANEL_DIRECTION_RW, buf, sizeof(buf));
	if (!ret) {
		return 0;
	}
...
int cts_tcs_get_panel_direction(const struct cts_device *cts_dev, u8 *direction)
{
	...
	ret = cts_tcs_spi_read(cts_dev, TP_STD_CMD_SYS_STS_PANEL_DIRECTION_RW, buf, sizeof(buf));
	if (!ret) {
		*direction = buf[0];
		return 0;
	}
...
int cts_tcs_set_game_mode(const struct cts_device *cts_dev, u8 enable)
{
	...
	ret = cts_tcs_spi_write(cts_dev, TP_STD_CMD_SYS_STS_GAME_MODE_RW, buf, sizeof(buf));
	if (!ret) {
		return 0;
	}
...
int cts_tcs_get_game_mode(const struct cts_device *cts_dev, u8 *enabled)
{
	...
	ret = cts_tcs_spi_read(cts_dev, TP_STD_CMD_SYS_STS_GAME_MODE_RW, buf, sizeof(buf));
	if (!ret) {
		*enabled = buf[0];
		return 0;
	}
				  ...
				  int delay)
{
	...
	xfer[0].tx_buf = tx, xfer[0].rx_buf = rx, xfer[0].len = 13,
    /**
     * xfer[0].tx_dma = 0,
     * xfer[0].rx_dma = 0,
     */
	...
	xfer[1].tx_buf = tx1, xfer[1].rx_buf = rx, xfer[1].len = 5,
    /**
     * xfer[0].tx_dma = 0,
     * xfer[0].rx_dma = 0,
     */
	...
	xfer[0].delay_usecs = 0,
	xfer[0].speed_hz = cts_data->pdata->spi_speed * 1000u,
	xfer[0].tx_buf = tx, xfer[0].rx_buf = rx, xfer[0].len = 7,
    /**
     * xfer[0].tx_dma = 0,
     * xfer[0].rx_dma = 0,
     */
	...
	xfer[1].delay_usecs = 0,
	xfer[1].speed_hz = cts_data->pdata->spi_speed * 1000u,
	xfer[1].tx_buf = tx1, xfer[1].rx_buf = rx, xfer[1].len = len + 5,
    /**
     * xfer[0].tx_dma = 0,
     * xfer[0].rx_dma = 0,
     */
...
int cts_tcs_init_int_data(struct cts_device *cts_dev)
{
	if (!cts_dev->rtdata.int_data) {
		cts_dev->rtdata.int_data =
		    kmalloc(INT_DATA_MAX_SIZ, GFP_KERNEL);
		if (!cts_dev->rtdata.int_data) {
			     ...
			     bool *has_int_data)
{
	...
	ret =
	    cts_tcs_spi_read(cts_dev,
			     TP_STD_CMD_SYS_STS_DATA_CAPTURE_SUPPORT_RO, buf,
			     sizeof(buf));
	if (!ret) {
		*has_int_data = !!buf[0];
		return 0;
			       ...
			       u16 *int_data_types)
{
	...
	ret =
	    cts_tcs_spi_read(cts_dev,
			     TP_STD_CMD_SYS_STS_DATA_CAPTURE_FUNC_MAP_RW, buf,
			     sizeof(buf));
	if (!ret) {
