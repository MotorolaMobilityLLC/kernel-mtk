...
#define OEM_RAWDATA_TEST_DATA_FILEPATH      OEM_TEST_DATA_DIR"/FWMutualTest"
#define OEM_NOISE_TEST_DATA_FILEPATH        OEM_TEST_DATA_DIR"/NoiseTest"
#define OEM_OPEN_TEST_DATA_FILEPATH         OEM_TEST_DATA_DIR"/OpenTest"
#define OEM_SHORT_TEST_DATA_FILEPATH        OEM_TEST_DATA_DIR"/ShortTest"
#define OEM_COMP_CAP_TEST_DATA_FILEPATH     OEM_TEST_DATA_DIR"/FWCCTest"
	...
	int flags, const u16 *data, int frames, int rows, int cols,  int hw_cols)
{
    ...
    cts_info("Dump tsdata to csv file: '%s' flags: 0x%x data: %p frames: %d "
		"row: %d col: %d hw_cols: %d",
	...
	int flags, const u8 *cap, int rows, int cols, int hw_cols)
{
    ...
    cts_info("Dump compensate cap to csv file: '%s' flags: 0x%x "
		"row: %d col: %d hw_cols: %d",
        filepath, flags, rows, cols, hw_cols);
...
char result_patch[MP_TEST_FILE_COUNT] = {0};
static void selftest_result_file_path(int test_result, struct rtc_time rtc_now_time, char *file_path)
{
    if (!file_path) {
        cts_err("file_patch is NULL");
        return;
    }

    if (test_result) {
        snprintf(result_patch, MP_TEST_FILE_COUNT, "%s_%02d%02d%02d-%02d%02d%02d_fail.csv", file_path,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
    } else {
        snprintf(result_patch, MP_TEST_FILE_COUNT, "%s_%02d%02d%02d-%02d%02d%02d_pass.csv", file_path,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
    }
}
...
static int save_selftest_data_to_file(struct cts_oem_data *oem_data)
{
    ...
    if (oem_data->test_rawdata) {
        selftest_result_file_path(oem_data->rawdata_test_result,
            rtc_now_time, OEM_RAWDATA_TEST_DATA_FILEPATH);
        ret = dump_tsdata_to_csv_file(result_patch,
    ...
    if (oem_data->test_noise) {
        selftest_result_file_path(oem_data->noise_test_result,
            rtc_now_time, OEM_NOISE_TEST_DATA_FILEPATH);
        ret = dump_tsdata_to_csv_file(result_patch,
    ...
    if (oem_data->test_open) {
        selftest_result_file_path(oem_data->open_test_result,
            rtc_now_time, OEM_OPEN_TEST_DATA_FILEPATH);
        ret = dump_tsdata_to_csv_file(result_patch,
    ...
    if (oem_data->test_short) {
        selftest_result_file_path(oem_data->short_test_result,
            rtc_now_time, OEM_SHORT_TEST_DATA_FILEPATH);
        ret = dump_tsdata_to_csv_file(result_patch,
    ...
    if (oem_data->test_comp_cap) {
        selftest_result_file_path(oem_data->comp_cap_test_result,
            rtc_now_time, OEM_COMP_CAP_TEST_DATA_FILEPATH);
        ret = dump_comp_cap_to_csv_file(result_patch,
...
static int selftest_seq_show(struct seq_file *m, void *v)
{
    ...
    int i, rows, cols, hw_rows, hw_cols, result;
        ...
        if (oem_data->rawdata_test_result == 0) {
            ...
            for(i = 0; i < oem_data->rawdata_test_frames; i++) {
                dump_tsdata_to_seq_file(m,
                    oem_data->rawdata_test_data + i * hw_rows * hw_cols, rows, cols, hw_cols);
                seq_putc(m, '\n');
            }
        } else {
            seq_printf(m, "FW Rawdata Test ERROR(%d)!\n\n", oem_data->rawdata_test_result);
        }
        result |= oem_data->rawdata_test_result;
    }
    if (oem_data->test_noise) {
        if (oem_data->noise_test_result == 0) {
            ...
            for(i = 0; i < oem_data->noise_test_frames; i++) {
                dump_tsdata_to_seq_file(m,
                    oem_data->noise_test_data + i * hw_rows * hw_cols , rows, cols, hw_cols);
                seq_putc(m, '\n');
            }
        } else {
            seq_printf(m, "Noise Test ERROR(%d)!\n\n", oem_data->noise_test_result);
        }
        result |= oem_data->noise_test_result;
    }
    if (oem_data->test_open) {
        if (oem_data->open_test_result == 0) {
            ...
            dump_tsdata_to_seq_file(m,
                oem_data->open_test_data, rows, cols, hw_cols);
        } else {
            seq_printf(m, " Open Test ERROR(%d)!\n\n", oem_data->open_test_result);
        }
        result |= oem_data->open_test_result;
    }
    if (oem_data->test_short) {
        if (oem_data->short_test_result == 0) {
            ...
            for (i = 0; i < 7; i++) {
                dump_tsdata_to_seq_file(m,
                    oem_data->short_test_data + i * hw_rows * hw_cols, rows, cols, hw_cols);
                seq_putc(m, '\n');
            }
        } else {
            seq_printf(m, "Short Test ERROR(%d)!\n\n", oem_data->short_test_result);
        }
        result |= oem_data->short_test_result;
    }
    if (oem_data->test_comp_cap) {
        if (oem_data->comp_cap_test_result == 0) {
            ...
            seq_printf(m, "Compensate-Cap Test ERROR(%d)!\n\n", oem_data->comp_cap_test_result);
        }
        result |= oem_data->comp_cap_test_result;
    }
    seq_printf(m, "MP Test %s!\n\n", (result == 0 ? "PASS" : "FAIL"));
