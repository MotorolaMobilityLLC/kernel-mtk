...
int cts_spi_send_recv(struct cts_platform_data *pdata, size_t len , u8 *tx_buffer, u8 *rx_buffer)
{
    ...
    ret = spi_sync(pdata->spi_client, &msg);
    if (ret) {
        cts_err("spi_sync failed %d(%s)", ret, cts_strerror(ret));
    }
#ifdef CFG_CTS_MANUAL_CS
...
static void cts_plat_handle_irq(struct cts_platform_data *pdata)
{
    ...
    if (ret) {
        cts_err("Device handle IRQ failed %d(%s)",
    ...
    struct device *device, enum cts_bus_type bus_type)
{
...
#ifdef CONFIG_CTS_I2C_HOST
    if (bus_type == CTS_I2C_BUS) {
        pdata->i2c_client = to_i2c_client(device);
    }
#endif /* CONFIG_CTS_I2C_HOST */

#ifdef CONFIG_CTS_SPI_HOST
    if (bus_type == CTS_SPI_BUS) {
        pdata->spi_client = to_spi_device(device);
    }
#endif /* CONFIG_CTS_SPI_HOST */
    ...
    if (IS_ERR(pdata->irq_thread)) {
        ...
        cts_err("Run thread for IRQ failed %d(%s)",
            ret, cts_strerror(ret));
...
#ifdef CONFIG_CTS_SPI_HOST
    if (bus_type == CTS_SPI_BUS) {
        ret = cts_plat_init_dts(pdata, device);
...
int cts_plat_request_irq(struct cts_platform_data *pdata)
{
    ...
    if (ret) {
        cts_err("Request IRQ failed %d(%s)", ret, cts_strerror(ret));
        return ret;
    }
