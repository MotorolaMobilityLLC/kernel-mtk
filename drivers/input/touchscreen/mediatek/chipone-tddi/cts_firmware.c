...
u32 tddicrc32(const u8 *data, size_t len)
{
        ...
        const struct cts_firmware *firmware)
{
    ...
    crc = tddicrc32(FIRMWARE_SECTION(firmware), FIRMWARE_SECTION_SIZE(firmware));
    ...
    if (DDIPARAM_SECTION_ENABLE(firmware) == CTS_SECTION_ENABLE_FLAG) {
        crc = tddicrc32(DDIPARAM_SECTION(firmware), DDIPARAM_SECTION_SIZE(firmware));
        ...
        struct cts_firmware_sect_info *info)
{
    ...
    info->firmware_sect_crc = tddicrc32(firmware->data, firmware->size);
