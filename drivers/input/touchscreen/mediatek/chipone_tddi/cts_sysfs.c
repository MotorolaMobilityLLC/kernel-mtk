        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret > 0) {
        return scnprintf(buf, PAGE_SIZE,
            "Open test has %d nodes FAIL, min: %u\n",
            ret, min);
    } else if (ret < 0) {
        return scnprintf(buf, PAGE_SIZE,
            "Open test FAIL %d(%s)\n", ret, cts_strerror(ret));
    } else {
        return scnprintf(buf, PAGE_SIZE, "Open test PASS\n");
    }
}
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret > 0) {
        return scnprintf(buf, PAGE_SIZE,
            "Short test has %d nodes FAIL, min: %u\n",
            ret, min);
    } else if (ret < 0) {
        return scnprintf(buf, PAGE_SIZE,
            "Short test FAIL %d(%s)\n", ret, cts_strerror(ret));
    } else {
        return scnprintf(buf, PAGE_SIZE, "Short test PASS\n");
    }
}
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret > 0) {
        return scnprintf(buf, PAGE_SIZE,
            "Compensate cap test has %d nodes FAIL, threshold[%u, %u]\n",
            ret, min, max);
    } else if (ret < 0) {
        return scnprintf(buf, PAGE_SIZE,
            "Compensate cap test FAIL %d(%s)\n", ret, cts_strerror(ret));
    } else {
        return scnprintf(buf, PAGE_SIZE, "Compensate cap test PASS\n");
    }
}
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret > 0) {
        ...
        return scnprintf(buf, PAGE_SIZE,
            "Rawdata test FAIL %d(%s)\n", ret, cts_strerror(ret));
    } else {
        return scnprintf(buf, PAGE_SIZE, "Rawdata test PASS\n");
    }
}
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret > 0) {
        return scnprintf(buf, PAGE_SIZE,
            "Noise test has %d nodes FAIL, max: %u\n",
            ret, max);
    } else if (ret < 0) {
        return scnprintf(buf, PAGE_SIZE,
            "Noise test FAIL %d(%s)\n", ret, cts_strerror(ret));
    } else {
        return scnprintf(buf, PAGE_SIZE, "Noise test PASS\n");
    }
}
