...
show_test_result:
    if (ret > 0) {
        cts_info("Short test has %d nodes FAIL", ret);
    } else if (ret < 0) {
        cts_info("Short test FAIL %d(%s)", ret, cts_strerror(ret));
    } else {
...
show_test_result:
    if (ret > 0) {
        cts_info("Open test has %d nodes FAIL", ret);
    } else if (ret < 0) {
        cts_info("Open test FAIL %d(%s)", ret, cts_strerror(ret));
    } else {
...
show_test_result:
    if (ret) {
        cts_info("Reset-Pin test FAIL %d(%s)", ret, cts_strerror(ret));
    } else {
...
show_test_result:
    if (ret) {
        cts_info("Int-Pin test FAIL %d(%s)", ret, cts_strerror(ret));
    } else {
...
show_test_result:
    if (ret > 0) {
        cts_info("Compensate-Cap test has %d nodes FAIL", ret);
    } else if (ret < 0) {
        cts_info("Compensate-Cap test FAIL %d(%s)",
...
show_test_result:
    if (ret > 0) {
        cts_info("Rawdata test has %d nodes FAIL", ret);
    } else if (ret < 0) {
        cts_info("Rawdata test FAIL %d(%s)", ret, cts_strerror(ret));
    } else {
        cts_info("Rawdata test PASS");
    }
        ...
        struct cts_test_param *param)
{
    ...
    if (cts_dev == NULL || param == NULL ||
        param->priv_param_size != sizeof(*priv_param) ||
        param->priv_param == NULL) {
        cts_err("Noise test with invalid param: priv param");
        return -EINVAL;
    }
...
show_test_result:
    if (ret > 0) {
        cts_info("Noise test has %d nodes FAIL", ret);
    } else if (ret < 0) {
        cts_info("Noise test FAIL %d(%s)", ret, cts_strerror(ret));
    } else {
