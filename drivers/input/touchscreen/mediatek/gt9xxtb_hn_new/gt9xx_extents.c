...
static ssize_t gtp_gesture_data_read(struct file *file, char __user *page, size_t size,
				     loff_t *ppos)
{
	s32 ret = -1;
...
static ssize_t gtp_gesture_data_write(struct file *filp, const char __user *buff, size_t len,
				      loff_t *off)
{
	s32 ret = 0;
...
s32 gesture_event_handler(struct input_dev *dev)
{
	...
	if (DOZE_ENABLED == gesture_data.doze_status) {
		...
		GTP_DEBUG("0x%x = 0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE, doze_buf[0],
			  doze_buf[1], doze_buf[2], doze_buf[3]);
		...
		if (ret == 0 && doze_buf[0] != 0) {
			...
			if (len > 0) {
				ret =
				    ges_i2c_read_bytes(GTP_REG_WAKEUP_GESTURE_DETAIL,
			...
			if (extra_len > 0) {
				ret =
				    ges_i2c_read_bytes(GTP_REG_WAKEUP_GESTURE + 4,
						       &gesture_data.data[4 + len * 4], extra_len);
				if (ret < 0) {
...
static s32 io_iic_read(u8 *data, void __user *arg)
{
	...
	if (!err) {
		err =
		    copy_to_user(&((u8 __user *) arg)[CMD_HEAD_LENGTH], &data[CMD_HEAD_LENGTH],
				 data_length);
		if (err) {
			GTP_ERROR("ERROR when copy to user.[addr: %04x], [read length:%d]", addr,
				  data_length);
			return err;
		}
...
(_IOC_DIR(cmd) & _IOC_READ) ? "read" : (_IOC_DIR(cmd) & _IOC_WRITE) ? "write" : "-");*/
	...
	case GESTURE_ENABLE_PARTLY:
		...
		gesture_data.enabled = 1;
		GTP_DEBUG("ENABLE_GESTURE_PARTLY, gesture = 0x%02X, gesture_data.enabled = %d",
			  value, gesture_data.enabled);
		break;

	case GESTURE_DISABLE_PARTLY:
		CLEARBIT(gestures_flag, (u8) value);
		GTP_DEBUG("DISABLE_GESTURE_PARTLY, gesture = 0x%02X, gesture_data.enabled = %d",
			  value, gesture_data.enabled);
		break;
