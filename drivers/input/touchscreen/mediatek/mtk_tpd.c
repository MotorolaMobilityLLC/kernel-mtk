...
struct pinctrl_state *eint_as_int, *eint_output0, *eint_output1, *rst_output0, *rst_output1,*eint_suspend;
...
void tpd_gpio_as_int(int pin)
{
	...
	if (pin == 2)
		pinctrl_select_state(pinctrl1, eint_suspend);
...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	eint_suspend = pinctrl_lookup_state(pinctrl1, "state_eint_suspend"); //zhangwenkang add
	if (IS_ERR(eint_suspend)) {
		ret = PTR_ERR(eint_suspend);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_rst_output1!\n");
		return ret;
	}
	TPD_DEBUG("[tpd%d] mt_tpd_pinctrl----------\n", pdev->id);
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	if (input_register_device(tpd->dev))
		...
		tpd_register_flag = 1;
	/*
	if (NULL != g_tpd_drv->tpd_device_name)
	{
		#include <linux/hardware_info.h>
		hardwareinfo_set_prop(HARDWARE_TP, g_tpd_drv->tpd_device_name);
	}
	*/
