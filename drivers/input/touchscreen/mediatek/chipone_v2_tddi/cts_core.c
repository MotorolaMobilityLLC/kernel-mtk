...
//        cts_dev->rtdata.slave_addr, cts_dev->rtdata.addr_width * 2, addr, len);
	...
	while (len) {
		...
		ret =
		    cts_plat_spi_read(cts_dev->pdata,
				      cts_dev->rtdata.slave_addr, addr_buf,
				      cts_dev->rtdata.addr_width, dst, xfer_len,
				      retry, delay);
		if (ret) {
...
//        cts_dev->rtdata.slave_addr, cts_dev->rtdata.addr_width * 2, addr, len);
	...
	while (len) {
		...
		ret =
		    cts_plat_spi_read_delay_idle(cts_dev->pdata,
				    ...
				    size_t len, u32 crc, int retry)
{
	...
	retries = 0;
	do {
		ret = cts_dev_writel(cts_dev, 0x015ff0, 0xCC33555A, 3, 1);
		if (ret != 0) {
			cts_err("SRAM writesb failed %d", ret);
			continue;
		}

		ret = cts_dev_writel(cts_dev, 0x08fffc, crc, 3, 1);
		if (ret != 0) {
			cts_err("SRAM writesb failed %d", ret);
			continue;
		}

		ret = cts_dev_writel(cts_dev, 0x08fff8, len, 3, 1);
		if (ret != 0) {
			cts_err("SRAM writesb failed %d", ret);
			continue;
		}
...
int cts_get_lib_version(const struct cts_device *cts_dev, u16 *lib_version)
{
	...
	ret =
	    cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_FW_LIB_MAIN_VERSION,
			     &main_version);
	if (ret) {
	...
	ret =
	    cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_FW_LIB_SUB_VERSION,
			     &sub_version);
	if (ret) {
				 ...
				 u8 *boot_mode)
{
	...
	ret = cts_hw_reg_readb_retry(cts_dev,
				     CTS_DEV_HW_REG_CURRENT_MODE, boot_mode, 5,
				     10);
	if (ret) {
...
int cts_irq_handler(struct cts_device *cts_dev)
{
	...
	if (unlikely(cts_dev->rtdata.suspended)) {
...
#ifdef CFG_CTS_FW_LOG_REDIRECT
		ret =
		    cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_TOUCH_INFO,
				      touch_info, 1);
		if (ret) {
		...
		ret =
		    cts_plat_process_vkey(cts_dev->pdata,
...
#endif
	{
		cts_err("BOOT_MODE readback %u != I2C/SPI PROMGRAM mode",
			boot_mode);
		return -EFAULT;
	}
...
*/
	...
	for (retries = 5; retries >= 0; retries--) {
		if (first_boot == 1 || auto_boot == 0) {
			...
			if (ret) {
				cts_err
				    ("Set BOOT_MODE to SRAM failed %d, try to reset device",
				     ret);
			}
			mdelay(30);
		}
...
int cts_get_hwid(struct cts_device *cts_dev, u32 *hwid)
{
	...
	ret =
	    cts_hw_reg_readl_retry(cts_dev, CTS_DEV_HW_REG_HARDWARE_ID, hwid, 5,
				   0);
	if (ret) {
		goto err_out;
	}
			 ...
			 void *gesture_info, bool trace_point)
{
	...
	ret = cts_fw_reg_readsb(cts_dev,
				CTS_DEVICE_FW_REG_GESTURE_INFO, gesture_info,
				2);
	if (ret) {
