...
const static struct cts_sfctrl icnl9911_sfctrl = {
    ...
    .xchg_sram_size = 8 * 1024u, /* For non firmware programming */
...
//extern int sprd_touch_get_usb_status();

int cts_irq_handler(struct cts_device *cts_dev)
{
    int ret;
//	bool cts_usb_status = 0;
	...
	//add usb status
//	cts_info("sprd usb status:%d(1--attached;0--detached)",sprd_touch_get_usb_status());
//	cts_usb_status = sprd_touch_get_usb_status();
//    cts_info("Set dev charger %s", cts_usb_status ? "ATTACHED" : "DETATCHED");
/*    ret = cts_send_command(cts_dev,
        cts_usb_status ? CTS_CMD_CHARGER_ATTACHED : CTS_CMD_CHARGER_DETACHED);
    if (ret) {
        if (ret) {
            cts_err("Send CMD_CHARGER_%s failed %d",
                cts_usb_status ? "ATTACHED" : "DETACHED", ret);
        }
    }
*/
...
static void cts_esd_protection_work(struct work_struct *work)
{
    ...
    if (cts_data->esd_check_fail_cnt >= CFG_CTS_ESD_FAILED_CONFIRM_CNT) {
        ...
        firmware = cts_request_firmware(&cts_data->cts_dev, cts_data->cts_dev.hwdata->hwid,
