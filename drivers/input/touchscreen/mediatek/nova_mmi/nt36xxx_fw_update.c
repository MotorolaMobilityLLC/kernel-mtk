...
static void update_firmware_release(void)
{
	if ((request_and_download_normal_complete == true) &&
		...
		NVT_LOG("ignore release backup firmware\n");
		return;
	}
...
static int32_t update_firmware_request(char *filename)
{
	...
	while (1) {
		...
		if ((request_and_download_normal_complete == false) &&
			...
			if (ret) {
				NVT_LOG("request normal firmware failed\n");
				goto request_fail;
			}
		}
...
static int32_t nvt_write_sram(const u8 *fwdata,
		uint32_t SRAM_addr, size_t size, uint32_t BIN_addr)
{
	int32_t ret = 0;
	uint32_t i = 0;
	int32_t len = 0;
...
static struct device_attribute touchscreen_attributes[] = {
	__ATTR_RO(path),
	__ATTR_RO(vendor),
	__ATTR(buildid, S_IRUGO, nvt_build_id_show, NULL),
	__ATTR(poweron, 0444, nvt_poweron_show, NULL),
	__ATTR(productinfo, 0444, nvt_productinfo_show, NULL),
	__ATTR(flashprog, 0444, nvt_flashprog_show, NULL),
	__ATTR(forcereflash, 0220, NULL, nvt_force_reflash_store),
	__ATTR(doreflash, 0220, NULL, nvt_do_reflash_store),
	__ATTR_NULL
};
...
int32_t nvt_fw_class_init(bool create)
{
	...
	if (create) {
		...
		for (i = 0; attrs[i].attr.name != NULL; ++i) {
			error = device_create_file(&ts->client->dev, &attrs[i]);
			if (error)
				goto err_device_spi;
		}
...
err_device_spi:
	for (--i; i >= 0; --i)
		device_remove_file(&ts->client->dev, &attrs[i]);
	NVT_ERR("error creating spi node\n");
	return -ENODEV;
}
