 ...
 *
 * $Revision: 39514 $
 * $Date: 2019-01-03 11:20:21 +0800 (週四, 03 一月 2019) $
 *
...
DECLARE_COMPLETION(load_fw_completion);
		...
		for (list = 0; list < (sizeof(trim_id_table) / sizeof(struct nvt_ts_trim_id_table)); list++) {
			...
			if (found_nvt_chip) {
				...
				if (list == 2) {
					...
					snprintf(ts->product_id, sizeof(ts->product_id), "NT36xxx");
				}
#endif
				ret = 0;
				goto out;
...
static int32_t nvt_ts_suspend(struct device *dev)
{
	...
	init_completion(&load_fw_completion);
...
void nvt_nvt_update_firmware_work(struct completion *load_fw_completion) {
	...
	ts->loading_fw = 0;
#endif
	NVT_LOG("nvt_nvt_update_firmware_work end\n");
}
