...
s32 gup_enter_update_mode(struct i2c_client *client)
{
	...
	tpd_gpio_output(GTP_RST_PORT, 0);/* DELETE->gpio_direction_output(tpd_rst_gpio_number, 0); */
	msleep(20);
	...
	tpd_gpio_output(GTP_RST_PORT, 0);/* DELETE->gpio_direction_output(tpd_rst_gpio_number, 1); */
...
{
	s32 ret = 0;
	/* GTP_GPIO_AS_INT(tpd_int_gpio_number); */
...
static u8 gup_download_fw_ss51(struct i2c_client *client)
{
	...
	ret = i2c_write_bytes(client, 0xC000,
		&fw_ss51[FW_DOWNLOAD_LENGTH], FW_DOWNLOAD_LENGTH);	/* write the second bank */
