...
#define TEST_CODE_VERSION			"tp_test_v1.0.4"
/* test switch */
...
#define GTX8_RESULT_SAVE_PATH			"/etc/firmware/"
...
#define FORMAT_PATH(path,mdir,name,suffix) do{\
    ...
    sprintf((char*)path,"%s%s_%04d%02d%02d%02d%02d%02d%s",mdir,name,(tm.tm_year+1900),(tm.tm_mon + 1),tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec,suffix);\
}while(0)
...
static int ts_test_reset(struct gtx8_ts_test *ts_test)
{
	struct goodix_ts_core *cd = ts_test->ts;

	return cd->ts_dev->hw_ops->reset(cd->ts_dev);
}

static int ts_test_read_trans(struct gtx8_ts_test *ts_test,
	u32 addr, u8 *data, u32 len)
{
	struct goodix_ts_core *cd = ts_test->ts;

	return cd->ts_dev->hw_ops->read_trans(cd->ts_dev, addr, data, len);
}

static int ts_test_write(struct gtx8_ts_test *ts_test,
	u32 addr, u8 *data, u32 len)
{
	struct goodix_ts_core *cd = ts_test->ts;

	return cd->ts_dev->hw_ops->write(cd->ts_dev, addr, data, len);
}

static int ts_test_write_trans(struct gtx8_ts_test *ts_test,
	u32 addr, u8 *data, u32 len)
{
	struct goodix_ts_core *cd = ts_test->ts;

	return cd->ts_dev->hw_ops->write_trans(cd->ts_dev, addr, data, len);
}

static int ts_test_send_config(struct gtx8_ts_test *ts_test,
		struct goodix_ts_config *config)
{
	struct goodix_ts_core *cd = ts_test->ts;

	return cd->ts_dev->hw_ops->send_config(cd->ts_dev, config);
}
...
static int disable_hopping(struct gtx8_ts_test *ts_test, struct goodix_ts_config *test_config)
{
	...
	core_data = (struct goodix_ts_core*)ts_test->ts;
	if (core_data->ts_dev->ic_type == IC_TYPE_YELLOWSTONE)
		ret = goodix_get_cfg_value(core_data, test_config->data, &value, 1, 11, 2); /* T11 2 0 0*/
	else
		ret = goodix_get_cfg_value(core_data, test_config->data, &value, 1, 10, 2); /* T10 2 0 0*/
	if (ret < 0 ){
...
static int gtx8_tptest_prepare(struct gtx8_ts_test *ts_test)
{
	...
	if (core_data->ts_dev->ic_type != IC_TYPE_YELLOWSTONE) {
		...
		ts_test->test_params.sen_num = core_data->sen_num;
		ts_test->test_params.drv_num = core_data->drv_num;
	}
	/* parse test limits from csv */
	ret = gtx8_init_testlimits(ts_test);
	if (ret) {
		ts_err("Failed to init testlimits from csv:%d", ret);
		return ret;
	}

	if (ts_test->test_config.initialized) {
		ret = ts_test_send_config(ts_test, &ts_test->test_config);
		if (ret) {
			ts_err("Failed to send noise test config config:%d", ret);
			return ret;
		}
		ts_info("send test config success");
	}
	...
	cmds[0] = 0x01;
	cmds[1] = 0x00;
	cmds[2] = 0 - cmds[0] - cmds[1];
	ret = ts_test_write(ts_test,
			core_data->ts_dev->reg.command, cmds, sizeof(cmds));
	if (ret < 0) {
...
static int gtx8_tptest_finish(struct gtx8_ts_test *ts_test)
{
	...
	ret = ts_test_reset(ts_test);
	if (ret)
		ts_err("chip reset failed");

	/* send normal config */
	ret = ts_test_send_config(ts_test, &core_data->ts_dev->normal_cfg);
	if (ret)
...
static int gtx8_cache_rawdata(struct gtx8_ts_test *ts_test)
{
	...
	if (rawdata_size > MAX_DRV_NUM * MAX_SEN_NUM || rawdata_size <= 0) {
		ts_err("Invalid rawdata size(%u)", rawdata_size);
		return ret;
	}
	...
	irq_flag = 0;
	ts_test_write_trans(ts_test, core_data->ts_dev->reg.coor, &irq_flag, 1);
	while (retry--) {
		usleep_range(5000, 5100);
		ts_test_read_trans(ts_test, core_data->ts_dev->reg.coor, &irq_flag, 1);
		if (irq_flag & 0x80)
			break;
	}
	if (retry < 0) {
	...
	ret = ts_test_read_trans(ts_test, rawdata_addr,
			(u8 *)&ts_test->rawdata.data[0], rawdata_size * sizeof(u16));
	if (ret < 0) {
		ts_err("Failed to read rawdata:%d", ret);
		return ret;
	}
	for (i = 0; i < rawdata_size; i++)
		ts_test->rawdata.data[i] = be16_to_cpu(ts_test->rawdata.data[i]);
	goodix_rotate_abcd2cbad(ts_test->test_params.drv_num,
			ts_test->test_params.sen_num, ts_test->rawdata.data);
...
static int gtx8_cache_self_rawdata(struct gtx8_ts_test *ts_test)
{
	...
	if (self_rawdata_size > MAX_DRV_NUM + MAX_SEN_NUM || self_rawdata_size <= 0) {
		ts_err("Invalid selfrawdata size(%u)", self_rawdata_size);
		return ret;
	}

	self_rawdata_addr = ts_test->test_params.self_rawdata_addr;
	ts_info("Selfraw address=0x%x", self_rawdata_addr);
	...
	if (retry < 0) {
		ts_err("self rawdata is not ready irq_flag:0x%02x, exit", irq_flag);
		return -EAGAIN;
	}
	ret = ts_test_read_trans(ts_test, self_rawdata_addr,
			(u8 *)&ts_test->self_rawdata.data[0], self_rawdata_size * sizeof(u16));
	if (ret < 0) {
		ts_err("Failed to read self_rawdata:%d", ret);
		return ret;
	}
...
static int gtx8_noisetest_prepare(struct gtx8_ts_test *ts_test)
{
	...
	if (noise_data_size <= 0 || noise_data_size > MAX_DRV_NUM * MAX_SEN_NUM) {
		ts_err("Bad noise_data_size[%d]", noise_data_size);
	...
	if (self_noise_data_size <= 0 || self_noise_data_size > MAX_DRV_NUM + MAX_SEN_NUM) {
		ts_err("Bad self_noise_data_size[%d]", self_noise_data_size);
...
static int gtx8_cache_noisedata(struct gtx8_ts_test *ts_test)
{
	...
	if (retry < 0) {
		ts_err("noisedata is not ready irq_flag:0x%02x, exit", irq_flag);
		return -EAGAIN;
	}

#ifdef NOISE_DATA_TEST
	/* read noise data */
	ret = ts_test_read_trans(ts_test, noisedata_addr,
			(u8 *)&ts_test->noisedata.data[0], noise_data_size * sizeof(u16));
	if (ret < 0) {
		ts_err("Failed read noise data");
		ts_test->noisedata.size = 0;
		ts_test->test_result[GTP_NOISE_TEST] = SYS_SOFTWARE_REASON;
	}
	for (i = 0; i < noise_data_size; i++)
		ts_test->noisedata.data[i] = be16_to_cpu(ts_test->noisedata.data[i]);
	goodix_rotate_abcd2cbad(ts_test->test_params.drv_num,
			ts_test->test_params.sen_num, ts_test->noisedata.data);
#endif
#ifdef SELFNOISE_DATA_TEST
	/* read self noise data */
	ret = ts_test_read_trans(ts_test, self_noisedata_addr,
			(u8 *)&ts_test->self_noisedata.data[0], self_noise_data_size * sizeof(u16));
	if (ret < 0) {
		ts_err("Failed read self noise data");
		ts_test->self_noisedata.size = 0;
		ts_test->test_result[GTP_SELFNOISE_TEST] = SYS_SOFTWARE_REASON;
	}
	for (i = 0; i < self_noise_data_size; i++)
		ts_test->self_noisedata.data[i] = be16_to_cpu(ts_test->self_noisedata.data[i]);
#endif
	return ret;
}
...
static void gtx8_analyse_self_noisedata(struct gtx8_ts_test *ts_test)
{
	...
	for (i = 0; i < ts_test->self_noisedata.size; i++) {
		...
		if (ts_test->self_noisedata.data[i] > ts_test->test_params.self_noise_threshold) {
			self_find_bad_node++;
			ts_err("self noise check failed: self_noise[%d]:%u, > %u",i,
...
static void gtx8_test_noisedata(struct gtx8_ts_test *ts_test)
{
	...
	if (ret) {
		ts_err("Noisetest prepare failed");
		goto soft_err_out;
	}
...
static int gtx8_accord_test_spec(struct gtx8_ts_test *ts_test, u8 *p_beyond_cnt)
{
	...
	if (cols <= 0) {
		ts_err("%s: parmas invalid", __func__);
		return RESULT_ERR;
	}
	...
	u8 *p_beyond_min_cnt, u8 *p_beyond_accord_cnt, u8 frame_cnt)
{
	...
	for (i=0;i<rawdata->size;i++){
		...
		if (p_beyond_max_cnt[i]*10/frame_cnt >= high_ratio){
			ts_err("max_cnt:%d, rawdata[%d][%d]:%u > max_limit:%u, NG",p_beyond_max_cnt[i],
		...
		if (p_beyond_min_cnt[i]*10/frame_cnt >= high_ratio){
			ts_err("min_cnt:%d,rawdata[%d][%d]:%u < min_limit:%u, NG",p_beyond_min_cnt[i],
				(u32)div_s64(i, test_params->drv_num), i % test_params->drv_num,
...
static void gtx8_rawdata_test_spec(struct gtx8_ts_test *ts_test)
{
	...
	if (!p_beyond_max_limit_cnt){
		ts_err("%s: Failed to alloc mem", __func__);
		goto RAW_TEST_ERR;
	}
...
int gtx8_get_rawdata(void *tsdev,struct ts_rawdata_info *info)
{
	...
	if (ret) {
		ts_err("Failed parse test peremeters, exit test");
		...
		struct gtx8_ts_test *ts_test)
{
	...
	for (i = 1; i < MAX_TEST_ITEMS; i++) {
		...
		ts_info("test_result_info [%d]%d", i, ts_test->test_result[i]);
	}
	...
	ts_info("ts_test->test_info:%s", ts_test->test_info);
...
static int gtx8_short_test_prepare(struct gtx8_ts_test *ts_test)
{
	...
	while (--retry) {
		...
		for (i = 0; i < 20; i++) {
			...
			ret = ts_test_read_trans(ts_test,
	...
	if (core_data->ts_dev->ic_type == IC_TYPE_NORMANDY) {
		...
		ret = ts_test_write_trans(ts_test, WATCH_DOG_TIMER_REG, data, 1);
		if (ret < 0) {
	...
	ret = ts_test_write_trans(ts_test, TXRX_THRESHOLD_REG,
	...
	ret = ts_test_write_trans(ts_test, GNDVDD_THRESHOLD_REG,
			data, 2);  /* SHORT THRESHOLD_REG 0X8808 */
	if (ret < 0) {
	...
	ret = ts_test_write_trans(ts_test, ADC_DUMP_NUM_REG, data, 2);
	if (ret < 0) {
	...
	data[0] = 0x01;
	ret = ts_test_write_trans(ts_test, SHORT_STATUS_REG,
...
static int gtx8_shortcircut_analysis(struct gtx8_ts_test *ts_test)
{
	...
	ret = ts_test_read_trans(ts_test, TEST_RESTLT_REG,
	...
	if (short_flag & 0x08) {
		...
		ret = ts_test_read_trans(ts_test, DIFF_CODE_REG,
	...
	/* read self-capdata+ */
	size = (MAX_DRV_NUM + MAX_SEN_NUM) * 2;
	ret = ts_test_read_trans(ts_test, DRV_SELF_CODE_REG,
		...
		for (j = short_die_num + 1; j < MAX_DRV_NUM; j++) {
			...
			if (adc_signal > test_params->short_threshold) {
				...
				if (self_capdata[short_die_num] == 0xffff ||
					...
					ts_info("invalid self_capdata:0x%x", self_capdata[short_die_num]);
					continue;
				    }
...
static void gtx8_shortcircut_test(struct gtx8_ts_test *ts_test)
{
	...
	for (i = 0; i < 110; i++) {
		msleep(50);
		ts_info("waitting for short test end...:retry=%d", i);
		ret = ts_test_read_trans(ts_test,
				SHORT_TESTEND_REG, data, 1);   /* SHORT_TESTEND_REG   0x8400 */
		if (ret)
			ts_err("Failed get short test result: retry%d", i);
	...
	if (i < 110) {
		...
		if (ret) {
			...
			ts_info("Short test success");
		}
	} else {
		ts_err("Wait short test finish timeout:reg_val=0x%x", data[0]);
		ts_test->test_result[GTP_SHORT_TEST] = SYS_SOFTWARE_REASON;
	}
}
