...
#if 0
static int ili_spi_parse_dt(struct device *dev, struct ilitek_ts_data *data)
{
    int ret = 0;
    ...
    if (IS_ERR(data->pinctrl)) {
        ...
        pr_err("Cannot find touch pc!\n");
        goto err_out;
    }

    data->spi_active = pinctrl_lookup_state(data->pinctrl, "ctp_pins_spi_mode");
    if (IS_ERR(data->spi_active)) {
        ret = PTR_ERR(data->spi_active);
        pr_err("Cannot find touch ctp_pins_spi_mode1111111111!");
        goto err_out;
    }
...
int ili_core_spi_setup(int num)
{
	...
	if (spi_setup(ilits->spi) < 0) {
       ...
       // ili_spi_parse_dt(&ilits->spi->dev, ilits);
...
{
	struct touch_bus_info *info;
	ILI_ERR("woailuo1ili_interface_dev_init\n");
	...
	ILI_ERR("2ili_interface_dev_init\n");
	hwif->info = info;
