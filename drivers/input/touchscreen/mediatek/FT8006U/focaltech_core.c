...
//                    printk(KERN_ERR "[ontim]reverse flag is %d,line %d\n",ontim_reverse_virtualkey,__LINE__);
    for (i = 0; i < key_num; i++) {
...
//printk(KERN_ERR "[ontim]version is %s ",version);
    do {
...
#ifdef CONFIG_ONTIM_CTP_PROC_INTERFACE
    ret = fts_create_proc();
    if (ret) {
        FTS_ERROR("create proc fs node fail");
    }
#endif
