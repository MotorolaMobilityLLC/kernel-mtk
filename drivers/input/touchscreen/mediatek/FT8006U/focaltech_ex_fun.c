...
#define GESTURE_LF_ARR                          0x51
#define GESTURE_RT_ARR                          0x52
#define GESTURE_ANTI_O                          0x57
#define GESTURE_DOUB_UP                          0x27
#define GESTURE_DOUB_DN                          0x28
...
//add by ontim_yzw for proc interface 0227/2018 begin
#ifdef CONFIG_ONTIM_CTP_PROC_INTERFACE
static struct proc_dir_entry *fts_dir_ontim,*fts_file_ontim;
static u8 last_senstive_ontim = '1';
/************************************************************************
*   Name: 
*  Brief:
* Input: 
...
void refresh_senstive_ontim(void)
{
    ...
    fts_esdcheck_switch(DISABLE);
#endif
  if(g_fts_mode_flag.fts_glove_mode_flag==true)
   {
           for (i = 0; i < 5; i++) {
               fts_i2c_write_reg(client, FTS_REG_CHARGER_MODE_EN, DISABLE);
               msleep(1);
               fts_i2c_read_reg(client, FTS_REG_CHARGER_MODE_EN, &state);
               if (state == DISABLE)
                 break;
           }
          ret = fts_enter_glove_mode(client, true);
          if (ret >= 0)
		FTS_INFO("wuyx--------glove  set   success!!!------------");
    }
   else
    {
          FTS_INFO("wuyx--------  set   senstive  success!!!---last_senstive_ontim=%c---------",last_senstive_ontim);
           for (i = 0; i < 5; i++) {
              fts_i2c_write_reg(client, FTS_REG_CHARGER_MODE_EN, ENABLE);
              msleep(1);
              fts_i2c_read_reg(client, FTS_REG_CHARGER_MODE_EN, &state);
              if (state == ENABLE)
                break;
           }
       switch(last_senstive_ontim){
       case '0':
           fts_i2c_write_reg(client, 0x80, 0x0b);
           break;
       case '1':
           fts_i2c_write_reg(client, 0x80, 0x09);
           break;
       case '2':
           fts_i2c_write_reg(client, 0x80, 0x07);
           break;
       default:
           break;
       }
       fts_i2c_read_reg(client, 0x80, &state);
       FTS_INFO("[ontim]last_senstive_ontim=%d,now state=%d",last_senstive_ontim,state);
}
#if FTS_ESDCHECK_EN
   fts_esdcheck_switch(ENABLE);
#endif
...
* Return: data len
***********************************************************************/
static ssize_t fts_senstive_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    struct i2c_client *client = ts_data->client;
    ...
    if (copy_from_user(&buf, buff, 1)) {
        FTS_ERROR("[ontim]: copy from user error!!");
        return -EFAULT;
    }
    if ((buf[0] <= '2')&&(buf[0]>= '0')){
       if(g_fts_mode_flag.fts_glove_mode_flag==true)
       {
    	last_senstive_ontim = buf[0];
        }
       else
       {
           mutex_lock(&input_dev->mutex);
        ...
        default:
            ...
            fts_i2c_read_reg(client, 0x80, &state);
        FTS_INFO("[ontim]last_senstive_ontim=%d,buf %d,state=%d",last_senstive_ontim,buf[0]-0x30,state);
#if FTS_ESDCHECK_EN
        fts_esdcheck_switch(ENABLE);
#endif
          fts_irq_enable();
...
* Return: 
***********************************************************************/
static ssize_t fts_senstive_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    int cnt;
    u8 buf[PROC_READ_BUF_SIZE] = { 0 };
    if(*ppos) 
    ...
    if (copy_to_user(buff, buf, cnt)) {
        FTS_ERROR("[ontim]: copy to user error!!");
        return -EFAULT;
    }
    *ppos += cnt;
    return cnt;
...
static const struct file_operations fts_senstive_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = fts_senstive_ontim_read,
    .write  = fts_senstive_ontim_write,
};
/************************************************************************
*   Name: 
*  Brief:
...
* Return: 
***********************************************************************/
static ssize_t fts_gesture_id_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    int cnt = 0;
    u8 buf[PROC_READ_BUF_SIZE] = { 0 };
...
static const struct file_operations fts_gesture_id_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = fts_gesture_id_ontim_read,
};
/************************************************************************
*   Name: 
*  Brief:
...
* Return: 
***********************************************************************/
static ssize_t fts_gesture_coordinate_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    int cnt = 0;
    struct input_dev *input_dev = fts_data->input_dev;
    u8 upload_id = 0;
    ...
    switch(fts_gesture_data.header[0]){
    ...
    case GESTURE_O:
        upload_id = 6;break;    
    case GESTURE_DOUB_UP:
        upload_direct = 1;    
    case GESTURE_DOUB_DN:
        upload_id = 7;break;    
    case GESTURE_M:
        upload_id = 12;break;    
    case GESTURE_W:
...
//        upload_id = ;break;  
    ...
    cnt = sprintf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",upload_id, 
            fts_gesture_data.coordinate_x[0], fts_gesture_data.coordinate_y[0], 
            fts_gesture_data.coordinate_x[1], fts_gesture_data.coordinate_y[1], 
            fts_gesture_data.coordinate_x[2], fts_gesture_data.coordinate_y[2], 
            fts_gesture_data.coordinate_x[3], fts_gesture_data.coordinate_y[3], 
            fts_gesture_data.coordinate_x[4], fts_gesture_data.coordinate_y[4], 
            fts_gesture_data.coordinate_x[5], fts_gesture_data.coordinate_y[5],
            upload_direct);
...
static const struct file_operations fts_gesture_coordinate_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = fts_gesture_coordinate_ontim_read,
};
...
* Return: data len
***********************************************************************/
static ssize_t fts_update_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    FTS_INFO("upgrade with bin file through ontim proc node");
...
* Return: 
***********************************************************************/
//static ssize_t fts_update_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
//{
//    return -EPERM;
//}
//
static const struct file_operations fts_update_ontim_fops = {
    .owner  = THIS_MODULE,
//    .read   = fts_update_ontim_read,
    .write  = fts_update_ontim_write,
};
/************************************************************************
*   Name:
*  Brief:
...
static int last_psensor_cmd = -1;
static ssize_t fts_unti_mistouch_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    struct input_dev *input_dev = fts_data->input_dev;
    u8 buf[4] = {0};
    ...
    mutex_lock(&input_dev->mutex);
    if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_INFO("[ontim]enable irq");
        fts_release_all_finger();
        fts_irq_enable();
        last_psensor_cmd = 0;
    } else if (FTS_SYSFS_ECHO_ON(buf)) {
        FTS_INFO("[ontim]disable irq");
        fts_irq_disable();
        fts_release_all_finger();
        last_psensor_cmd = 1;
    }
...
* Return:
***********************************************************************/
static ssize_t fts_unti_mistouch_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
...
static const struct file_operations fts_unti_mistouch_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = fts_unti_mistouch_ontim_read,
    .write  = fts_unti_mistouch_ontim_write,
};
///************************************************************************
//*   Name:
//*  Brief:
...
//* Return:
//***********************************************************************/
//static ssize_t fts_reverse_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
//{
//    u8 buf[4] = {0};
//
//    if (copy_from_user(&buf, buff, 1)) {
...
//    }
//    if (FTS_SYSFS_ECHO_ON(buf)) {
//        FTS_INFO("[ontim]enable reverse virtualkeys");
//        ontim_reverse_virtualkey = 1;
//    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
//        FTS_INFO("[ontim]disable reverse virtualkeys");
//        ontim_reverse_virtualkey = 0;
//    }
//    *ppos += count;
//    return count;
//}
...
//* Return:
//***********************************************************************/
//static ssize_t fts_reverse_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
//{
////    if ((count == 0) || (count > PROC_READ_BUF_SIZE)) {
////    if ( (count > PROC_READ_BUF_SIZE)) {
////        FTS_ERROR("ontim proc read count(%d) fail", (int)count);
////        return -EINVAL;
////    }
//    if (*ppos) return 0;
//
//    if (copy_to_user(buff,(char *)&ontim_reverse_virtualkey,sizeof(ontim_reverse_virtualkey))) {
//        FTS_ERROR("[ontim]: copy to user error!!");
//        return -EFAULT;
//    }
////        FTS_INFO("[ontim]reverse key size of is %d,char is 0x%x",sizeof(ontim_reverse_virtualkey),((char *)&ontim_reverse_virtualkey)[0]);
//    *ppos += sizeof(ontim_reverse_virtualkey);
//    return sizeof(ontim_reverse_virtualkey);
//}
//
//static const struct file_operations fts_reverse_ontim_fops = {
//    .owner  = THIS_MODULE,
//    .read   = fts_reverse_ontim_read,
//    .write  = fts_reverse_ontim_write,
//};
//
...
* Return: data len
***********************************************************************/
static ssize_t fts_glove_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    int ret;
    struct fts_ts_data *ts_data = fts_data;
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!g_fts_mode_flag.fts_glove_mode_flag) {
            FTS_INFO("[ontim]enter glove mode");
	  for (i = 0; i < 5; i++) {
            fts_i2c_write_reg(client, FTS_REG_CHARGER_MODE_EN, DISABLE);
            msleep(1);
            fts_i2c_read_reg(client, FTS_REG_CHARGER_MODE_EN, &state);
        ...
        if (g_fts_mode_flag.fts_glove_mode_flag) {
            FTS_INFO("[ontim]exit glove mode");
    ...
    FTS_INFO("[ontim]glove mode status:  %d", g_fts_mode_flag.fts_glove_mode_flag);
    *ppos += count;
    return count;
}
...
* Return: read char number
***********************************************************************/
static ssize_t fts_glove_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    int cnt;
    u8 val;
    u8 buf[PROC_READ_BUF_SIZE] = { 0 };
    ...
    FTS_INFO("[ontim]glove read:::%s, val %d", buf,val);
    if (copy_to_user(buff, buf, cnt)) {
        FTS_ERROR("[ontim]: copy to user error!!");
        return -EFAULT;
    }
...
static const struct file_operations fts_glove_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = fts_glove_ontim_read,
    .write  = fts_glove_ontim_write,
};
...
* Return: data len
***********************************************************************/
static ssize_t fts_gestrue_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
//    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev = fts_data->input_dev;
//    struct i2c_client *client = ts_data->client;
...
//
//        if (i >= 5) {
//            FTS_ERROR("[ontim]Enter into gesture(suspend) failed!\n");
//            fts_gesture_data.mode = DISABLE;
//        }else{
//            FTS_INFO("[ontim]enable gesture");
...
//
//        if (i >= 5) {
//            FTS_ERROR("[ontim]Clear gesture(resume) failed!\n");
//            fts_gesture_data.mode = ENABLE;
//        }else{
//            FTS_INFO("[ontim]disable gesture");
//            fts_gesture_data.mode = DISABLE;
//        }
            fts_gesture_data.mode = DISABLE;
    }
...
* Return: read char number
***********************************************************************/
static ssize_t fts_gestrue_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    int cnt;
    u8 val;
    u8 buf[PROC_READ_BUF_SIZE] = { 0 };
...
//    }
    ...
    mutex_unlock(&input_dev->mutex);
    FTS_INFO("[ontim]gesture read:::%s, val %d", buf,val);
...
static const struct file_operations fts_gestrue_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = fts_gestrue_ontim_read,
    .write  = fts_gestrue_ontim_write,
};
#endif
/************************************************************************
* Name: fts_create_proc
* Brief: creat proc interface
* Input:
* Output:
* Return:return 0 if success
***********************************************************************/
...
{
    fts_dir_ontim = proc_mkdir("touchpanel", NULL);
    if (NULL == fts_dir_ontim) {
        FTS_ERROR("create proc entry fail");
        return -ENOMEM;
    }

    fts_file_ontim = proc_create("firmware_update_enable", 0664, fts_dir_ontim, &fts_update_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc update entry fail");
        return -ENOMEM;
    } 

    fts_file_ontim = proc_create("psensor_incall_disable_tp", 0664, fts_dir_ontim, &fts_unti_mistouch_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc unti-mistouch entry fail");
        return -ENOMEM;
    } 

//    fts_file_ontim = proc_create("button_reverse_enable", 0777, fts_dir_ontim, &fts_reverse_ontim_fops);
//    if (NULL == fts_file_ontim) {
//        FTS_ERROR("create proc reverse entry fail");
//        return -ENOMEM;
//    } 
    fts_file_ontim = proc_create("tp_thresh_value", 0664, fts_dir_ontim, &fts_senstive_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc dist thresh  entry fail");
        return -ENOMEM;
    } 
    
    refresh_senstive_ontim();

    fts_file_ontim = proc_create("support_gesture_id", 0664, fts_dir_ontim, &fts_gesture_id_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc dist thresh  entry fail");
        return -ENOMEM;
    } 

    fts_file_ontim = proc_create("gesture_coordinate", 0664, fts_dir_ontim, &fts_gesture_coordinate_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc dist thresh  entry fail");
        return -ENOMEM;
    } 

    fts_file_ontim = proc_create("glove_mode_enable", 0664, fts_dir_ontim, &fts_glove_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc glove entry fail");
        return -ENOMEM;
    } 
#if FTS_GESTURE_EN
    fts_file_ontim = proc_create("double_tap_enable", 0664, fts_dir_ontim, &fts_gestrue_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc black gesture entry fail");
        return -ENOMEM;
    } 
#endif
...
/************************************************************************
* Name: fts_remove_proc
* Brief: remove proc interface
* Input:
* Output:
* Return:
***********************************************************************/
...
{
    remove_proc_entry("firmware_update_enable",fts_dir_ontim);    
    remove_proc_entry("psensor_incall_disable_tp",fts_dir_ontim);    
//    remove_proc_entry("button_reverse_enable",fts_dir_ontim);    
    remove_proc_entry("tp_thresh_value",fts_dir_ontim);    
    remove_proc_entry("support_gesture_id",fts_dir_ontim);    
    remove_proc_entry("gesture_coordinate",fts_dir_ontim);    
    remove_proc_entry("glove_mode_enable",fts_dir_ontim);    
#if FTS_GESTURE_EN
    remove_proc_entry("double_tap_enable",fts_dir_ontim);    
#endif
