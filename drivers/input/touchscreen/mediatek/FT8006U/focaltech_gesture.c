...
*****************************************************************************/
#ifdef TPLINK_PRJ
#define KEY_GESTURE_U                          KEY_F4
#define KEY_GESTURE_UP                         KEY_F4
#define KEY_GESTURE_DOWN                       KEY_F4
#define KEY_GESTURE_LEFT                       KEY_F4
#define KEY_GESTURE_RIGHT                      KEY_F4
#define KEY_GESTURE_O                          KEY_F4
#define KEY_GESTURE_E                          KEY_F4
#define KEY_GESTURE_M                          KEY_F4
#define KEY_GESTURE_L                          KEY_F4
#define KEY_GESTURE_W                          KEY_F4
#define KEY_GESTURE_S                          KEY_F4
#define KEY_GESTURE_V                          KEY_F4
#define KEY_GESTURE_C                          KEY_F4
#define KEY_GESTURE_Z                          KEY_F4
#define KEY_GESTURE_LF_ARR                     KEY_F4
#define KEY_GESTURE_RT_ARR                     KEY_F4
#define KEY_GESTURE_ANTI_O                     KEY_F4
#define KEY_GESTURE_DOUB_UP                    KEY_F4
#define KEY_GESTURE_DOUB_DN                    KEY_F4
#define KEY_GESTURE_DC                         KEY_F4
#else
...
* Return:
***********************************************************************/
static ssize_t gesture_on_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    int count;
    u8 val;
    struct input_dev *input_dev = fts_data->input_dev;
...
* Return:
***********************************************************************/
static ssize_t gesture_on_store(struct kobject *kobj,struct kobj_attribute *attr, const char *buf, size_t count)
{
    struct input_dev *input_dev = fts_data->input_dev;
    mutex_lock(&input_dev->mutex);
    if (FTS_SYSFS_ECHO_ON(buf)) {
...
static struct kobj_attribute gesture_on_attr = {
	.attr = {
		.name = "gesture_on",
		.mode = 0666,
	},
	.show = &gesture_on_show,
    .store = &gesture_on_store,
};
/*
 *   read example: cat fts_gesture_buf        ---read gesture buf
 */
static struct attribute *gesture_mode_attrs[] = {
    &gesture_on_attr.attr,
    NULL,
};

static struct attribute_group gesture_group = {
    .attrs = gesture_mode_attrs,
};
/*****************************************************************************
*   Name: fts_create_gesture_sysfs
*  Brief:
...
static int fts_create_gesture_switchnode(void)
{
	int ret;

	virtual_dir = virtual_device_parent(NULL);
	if (!virtual_dir) {
		FTS_ERROR("Get virtual dir failed\n");
		return -ENOMEM;
	}
	touchscreen_dir = kobject_create_and_add("touch", virtual_dir);
	if (!touchscreen_dir) {
		FTS_ERROR("Create touchscreen dir failed\n");
		return -ENOMEM;
	}
	touchscreen_dev_dir = kobject_create_and_add("tp_dev", touchscreen_dir);
	if (!touchscreen_dev_dir) {
		FTS_ERROR("Create touchscreen_dev dir failed\n");
		return -ENOMEM;
	}
	ret = sysfs_create_group(touchscreen_dev_dir, &gesture_group);
	if (ret)
		FTS_ERROR("create gesture_attr_group error\n");
...
//        break;
    case GESTURE_V:
        ...
        break;
    case GESTURE_LF_ARR:
        gesture = KEY_GESTURE_LF_ARR;
        break;
    case GESTURE_RT_ARR:
        gesture = KEY_GESTURE_RT_ARR;
        break;
    case GESTURE_ANTI_O:
        gesture = KEY_GESTURE_ANTI_O;
        break;
    case GESTURE_DOUB_UP:
        gesture = KEY_GESTURE_DOUB_UP;
        break;
    case GESTURE_DOUB_DN:
        gesture = KEY_GESTURE_DOUB_DN;
        break;
//    case GESTURE_Z:
