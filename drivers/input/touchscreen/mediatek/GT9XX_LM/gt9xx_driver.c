...
static void gtp_charger_switch(int on);
#endif
...
s32 gtp_init_panel(struct i2c_client *client)
{
...
#ifndef CONFIG_MID_ITEMS_SUPPORT
	...
	if (item_string(ts_model, "ts.model", 0) == 0)
		for (i = 0; i < ARRAY_SIZE(project_cfg_data); i++)
			if (strcmp(ts_model, project_cfg_data[i].name) == 0)
				ts_selection = i;

	for (i = 0; i < 6; i++) {
		send_cfg_buf[i] = project_cfg_data[ts_selection].cfg->info[i];
		cfg_info_len[i] = project_cfg_data[ts_selection].cfg->lens[i];
		if (cfg_info_len[i] < 2)
			cfg_info_len[i] = 0;
	}
#endif
...
#else
			{
				...
				if (flash_cfg_version < 90
					...
					config[GTP_ADDR_LENGTH] =
					    drv_cfg_version;
					check_sum = 0;
					for (i = GTP_ADDR_LENGTH; i < cfg_len;
					     i++)
						check_sum += config[i];
...
#ifdef CONFIG_GTP_PEN_HAVE_BUTTON
...
static s8 gtp_enter_sleep(struct i2c_client *client)
{
	...
	if (gtp_chip_type == CHIP_TYPE_GT9F) {
		...
		if (ret <= 0)
			GTP_ERROR
			    ("[gtp_enter_sleep]Read ref status reg error.");
		...
		if (i2c_status_buf[2] & 0x80) {
			...
			if (ret == FAIL)
				GTP_ERROR
				    ("[gtp_enter_sleep]Store bak ref failed.");
		}
	}
#endif
...
#ifdef CONFIG_MID_ITEMS_SUPPORT
	item_send_cfg = (item_integer("touch.gt9xx.send_cfg", 0) == 0) ? 0 : 1;
#endif
	return 0;
}
