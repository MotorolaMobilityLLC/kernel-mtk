...
static void fts_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	...
	switch(gesture_id)
	{
		...
		case GESTURE_LEFT:
		case GESTURE_RIGHT:
			fts_gesture_letter = 0x20;
			break;
		case GESTURE_DOUBLECLICK:
			fts_gesture_letter = 0x24;
			break;
		case GESTURE_O:
			fts_gesture_letter = 0x33;
			break;
		case GESTURE_V:
			fts_gesture_letter = 0x34;
			break;
		default:
			fts_gesture_letter = 0 ;
			break;
...
int fts_read_Gestruedata(void)
{
	...
	gestrue_id = fetch_object_sample(buf, pointnum);  //change by lixh10 
	fts_check_gesture(fts_input_dev,gestrue_id);
