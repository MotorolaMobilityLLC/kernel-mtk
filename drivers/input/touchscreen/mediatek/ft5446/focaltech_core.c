...
MODULE_DEVICE_TABLE(i2c, fts_tpd_id);
		...
		struct device_attribute *attr, char *buf)
{
    ...
    unsigned char fw_vesion_addr=FTS_REG_FW_VER;
    unsigned char vendor_id_addr=FTS_REG_VENDOR_ID;
	char uc_reg_value[10]={0,};
	...
	retval = fts_i2c_write(fts_i2c_client, &uc_reg_addr, 1);
	 if(retval<0)
	 	goto I2C_FAIL; 
	retval = fts_i2c_read(fts_i2c_client, &uc_reg_addr, 0, &uc_reg_value[0], 1);
    retval += fts_i2c_read(fts_i2c_client, &fw_vesion_addr, 0, &uc_reg_value[3], 1);
    retval += fts_i2c_read(fts_i2c_client, &vendor_id_addr, 0, &uc_reg_value[5], 1);
	 if(retval<0)
	 	goto I2C_FAIL; 
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo,struct touch_info *ptest)
{
	int i = 0;
	char data[128] = {0};
       char high_byte,low_byte,reg;
	//u8 report_rate =0;
	p_point_num = point_num;
	if (tpd_halt)
	{
		printk( "tpd_touchinfo return ..\n");
		return false;
	}
...
static int fts_read_Touchdata(struct ts_event *data)//(struct ts_event *pinfo)
{
	...
	if (tpd_halt)
	{
		printk( "fts while suspend cancel read touchdata, return ..\n");
		return ret;
	}
...
#else
    mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, IRQF_TRIGGER_FALLING, tpd_eint_interrupt_handler, 1);
    printk("[wj]CUST_EINT_TOUCH_PANEL_NUM = %d.\n", CUST_EINT_TOUCH_PANEL_NUM);
#endif
    	#ifdef VELOCITY_CUSTOM_fts
...
***********************************************************************/
 static int tpd_local_init(void)
 {
    ...
    printk("[wj]open vldo2V8.\n");
	tpd->reg = regulator_get(tpd->tpd_dev, "vtouch-cust");
	ret = regulator_set_voltage(tpd->reg, 2800000, 2800000);	/*set 2.8v*/
	if (ret) {
		printk("rmi4 regulator_set_voltage(%d) failed!\n", ret);
		return -1;
	}
...
***********************************************************************/
 static int __init tpd_driver_init(void) {
        ...
        i2c_register_board_info(0, &fts_i2c_tpd, 1);//change by lixh10
