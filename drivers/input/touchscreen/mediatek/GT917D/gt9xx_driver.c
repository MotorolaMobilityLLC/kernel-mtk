...
int delay_waiter_flag = 0;
static struct delayed_work waiter_work;
#ifdef CONFIG_GTP_CHARGER_DETECT
...
extern wait_queue_head_t delay_waiter;
static void waiter_func(struct work_struct *work)
{
	delay_waiter_flag = 1;
	wake_up_interruptible(&delay_waiter);
}
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* Add by zhangle zwx450315 20181106 for i2c communication */
	if(client->addr != 0x5d){
		GTP_INFO("Change i2c address 0x%x to 0x5d.",client->addr);        
		client->addr = 0x5d;
	}
	...
	tpd_load_status = 1;
	INIT_DELAYED_WORK(&waiter_work, waiter_func);
	queue_delayed_work(gtp_workqueue, &waiter_work, 30 * HZ);
