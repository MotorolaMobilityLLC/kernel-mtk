			...
			if (global_state.other.active) {
				...
				for (j = 0; j < LINE_SIZE+1; j++) {
					if (!(j >= LINE_SIZE || py[j + 1] == NULL ||
						multi_y[j].range == 0 || point_now[i].other.y < multi_y[j].range))
						continue;
	...
	for (i = 0; i < POINT_MAX; i++) {
		if (pp[1][i].all != 0) {
			if (global_state.other.interpolation != 0 &&
				...
				PointPredictOne(i);
			} else if (pp[2][i].all != 0 && (avg[0] != avg[1] || avg[1] != avg[2])
				&& avg[0] != 0 && avg[1] != 0) {
				PointPredictSpeed(i);
...
static unsigned int PointDistance(union gsl_POINT_TYPE *p1, union gsl_POINT_TYPE *p2)
{
	int a, b, ret;
	...
	if (global_state.other.only || global_state.other.active) {
		...
		for (j = 0; j < point_num && j < POINT_MAX; j++) {
			if (DistanceMin(&distance) == 0)
				break;
			if (distance.min >= (id_static_coe + id_speed[distance.i] * id_speed_coe)
...
static void PointDelay(void)
{
	...
	for (i = 0; i < POINT_MAX; i++) {
		...
		if (id_flag.other.first_avg && pp[0][i].all == 0 && pp[1][i].all != 0
			&& point_delay[i].other.able == 0 && point_delay[i].other.init != 0) {
			if (PointDelayAvg(i)) {
				point_delay[i].other.able = 1;
		...
		if (report_delete == 0 && point_delay[i].other.report) {
			if (PointDistance(&pp[point_delay[i].other.report][i], &pp[point_delay[i].other.report-1][i])
				< 3*3) {
...
static void PointMenu(void)
{
	...
	if (edge_start == 0)
		return;
	if (pp[0][0].all == 0 || pp[1][0].all == 0 || (pp[2][0].all != 0
		&& global_state.other.menu == 0) || pp[3][0].all != 0) {
...
static void PointRound(void)
{
	...
	for (i = 0; i < 4; i++) {
		if (stretch->up[i].range)
			...
			sen_num_nokey * drv_num_nokey * 64 / screen_x_max;
		...
		if (stretch->left[i].range)
			...
			sen_num_nokey * drv_num_nokey * 64 / screen_y_max;
		if (stretch->right[i].range)
			stretch->right[i].range = stretch->right[i].range *
			sen_num_nokey * drv_num_nokey * 64 / screen_y_max;
	}
	...
	for (i = 0; i < 4; i++) {
		...
		|| id_flag.other.screen_real) {
			for (i = 0; i < 4; i++) {
				...
				if (stretch->down[i].range)
					stretch->down[i].range = (screen_x_max - stretch->down[i].range)
					* drv_num_nokey * 64 / screen_x_max;
				if (stretch->left[i].range)
					stretch->left[i].range = stretch->left[i].range * sen_num_nokey
					* 64 / screen_y_max;
				if (stretch->right[i].range)
					stretch->right[i].range = (screen_y_max - stretch->right[i].range)
...
for (i = 0; i < POINT_MAX; i++) {
	...
	if (i == 0) {
		...
		if (point_edge.rate != 0 && point_edge.dis != 0) {
			...
			(ps[0][i].other.x - point_edge.coor.other.x)*(0x1000 - point_edge.rate) / 0x1000;
			ps[0][i].other.y = point_edge.coor.other.y +
			(ps[0][i].other.y - point_edge.coor.other.y)*(0x1000 - point_edge.rate) / 0x1000;
		}
	}
	if (ps[1][i].all == 0) {
		continue;
...
if (len == 1) {
	...
	for (i = 0; i < POINT_MAX; i++) {
		...
		if (dn == 0 || dr == 0) {
			...
			ps[0][i].other.x = (int)ps[1][i].other.x + ((int)pp[0][i].other.x
			- (int)ps[1][i].other.x) * dr / dn;
			ps[0][i].other.y = (int)ps[1][i].other.y + ((int)pp[0][i].other.y
	...
	for (i = 0; i < POINT_MAX; i++) {
		...
		if ((point_shake & (0x1<<i)) == 0)
			continue;
		if (dc[len] == 3)/* dc == 2 */ {
			dn = pp[0][i].other.x > ps[1][i].other.x ? pp[0][i].other.x - ps[1][i].other.x :
			...
			if (dn < ds[0]) {
				for (j = 0; j <= len; j++) {
					if (j == len || dn == 0) {
						...
						(((int)pp[0][i].other.x - (int)ps[1][i].other.x) * dr+64) / 128;
						break;
					}
				}
			}
			dn = pp[0][i].other.y > ps[1][i].other.y ? pp[0][i].other.y - ps[1][i].other.y :
			...
			if (dn < ds[0]) {
				for (j = 0; j <= len; j++) {
					if (j == len || dn == 0) {
						...
						(((int)pp[0][i].other.y - (int)ps[1][i].other.y) * dr+64) / 128;
						break;
					}
				}
			}
			...
			if (dn < save_dn[i]) {
				...
				save_dn[i] = dn;
				ps[0][i].other.x = (int)ps[1][i].other.x +
				(((int)pp[0][i].other.x - (int)ps[1][i].other.x) * dr) / 128;
				ps[0][i].other.y = (int)ps[1][i].other.y +
				(((int)pp[0][i].other.y - (int)ps[1][i].other.y) * dr) / 128;
				continue;
			}
			for (j = 0; j <= len; j++) {
				if (j == len || dn == 0) {
...
static void ResetMask(void)
{
	...
	if (reset_mask_max == 0)
		...
		if (PointDistance((union gsl_POINT_TYPE *)(&reset_mask_max), pp[0])
...
static int DiagonalDistance(union gsl_POINT_TYPE *p, int type)
{
	...
	if (type == 0)
		...
		(int)drv_num_nokey*(int)(p->other.y) - (int)sen_num_nokey*(int)drv_num_nokey * 64) / 4;
	return square * square / divisor;
}

static void DiagonalCompress(union gsl_POINT_TYPE *p, int type, int dis, int dis_max)
{
	int x, y;
	int tx, ty;
	int cp_ceof;
