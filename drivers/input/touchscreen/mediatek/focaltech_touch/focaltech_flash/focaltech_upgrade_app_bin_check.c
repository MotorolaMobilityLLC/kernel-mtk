...
bool fts_check_app_bin(u8 *pbt_buf, u32 dw_lenth)
{
	...
	if (!task_check_mem_ecc
	    (pbt_buf, APP_ADDR_PART1_START, APP_ADDR_PART1_LEN,
	     ADDR_PART1_ECC)) {
...
bool fts_check_app_bin(u8 *pbt_buf, u32 dw_lenth)
{
	...
	FTS_DEBUG("[UPGRADE][ECC] : len = 0x%x, dw_lenth = 0x%x!!", len,
		  dw_lenth);
