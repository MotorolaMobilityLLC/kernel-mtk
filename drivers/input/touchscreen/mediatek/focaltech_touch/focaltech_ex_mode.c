...
int fts_enter_glove_mode(struct i2c_client *client, int mode)
{
	...
	if (ret < 0) {
		FTS_DEBUG
		    ("[fts][Touch] fts_enter_glove_mode write value fail\n");
	}
...
int fts_enter_cover_mode(struct i2c_client *client, int mode)
{
	...
	if (ret < 0) {
		FTS_DEBUG
		    ("[fts][Touch] fts_enter_cover_mode write value fail\n");
	}
...
int fts_enter_charger_mode(struct i2c_client *client, int mode)
{
	...
	if (ret < 0) {
		FTS_DEBUG
		    ("[fts][Touch] fts_enter_charger_mode write value fail\n");
	}
...
int fts_ex_mode_init(struct i2c_client *client)
{
	...
	if (0 != err) {
		...
		FTS_DEBUG
		    ("[focal] %s(void) - fts_touch_glove_init(void) succeeded.\n",
		     __func__);
	}
