...
static void esd_process(u8 *writebuf, int buflen, bool flag)
{
	if (flag) {
		...
		if ((writebuf[1] == 0x07) && (buflen == 0x02)) {
			FTS_DEBUG
			    ("[ESD]: Upgrade finish-trigger reset(07)(%x %x)!!",
			       ...
			       size_t count, loff_t *ppos)
{
	...
	case PROC_WRITE_DATA:
		...
		if (writelen > 0) {
			ret =
			    fts_i2c_write(fts_i2c_client, writebuf + 1,
					  writelen);
			if (ret < 0) {
				FTS_ERROR("[APK]: write iic error!!");
			}
				 ...
				 size_t count)
{
	...
	if (2 == num_read_chars) {
		...
		g_rwreg_result.result =
		    fts_i2c_read_reg(client, regaddr, &regvalue);
		if (g_rwreg_result.result < 0) {
		...
		g_rwreg_result.value = regvalue;
		g_rwreg_result.result =
		    fts_i2c_write_reg(client, regaddr, regvalue);
		if (g_rwreg_result.result < 0) {
				  ...
				  const char *buf, size_t count)
{
	...
	if (i_ret == 0) {
		...
		FTS_DEBUG("[EX]: upgrade to new version 0x%x!!",
			  uc_host_fm_ver);
	} else {
