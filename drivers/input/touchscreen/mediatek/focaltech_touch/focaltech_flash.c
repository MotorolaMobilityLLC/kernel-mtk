					...
					char *pProjectCode)
{
	...
	for (j = 0; j < 33; j++) {
		...
		fts_i2c_read(client, auc_i2c_write_buf, 0x20, pProjectCode + j,
			     1);
		if (*(pProjectCode + j) == '\0')
			break;
	}
...
void fts_ctpm_get_upgrade_array(void)
{
	...
	for (i = 0; i < 5; i++) {
		ret =
		    fts_i2c_read_reg(fts_i2c_client, FTS_REG_CHIP_ID, &chip_id);
		if (ret < 0) {
...
int fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	if ((chip_types.chip_idh == 0x11) || (chip_types.chip_idh == 0x12)
	    || (chip_types.chip_idh == 0x13) || (chip_types.chip_idh == 0x14)) {	/* 5x36,5x36i */
		for (i = 0; i < 100; i++) {
...
bool fts_ctpm_check_need_upgrade(struct i2c_client *client)
{
	...
	if (fw_status == FTS_RUN_IN_APP) {	/* call fts_flash_get_upgrade_info in probe function firstly. */
		...
		FTS_DEBUG
		    ("[UPGRADE]: uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x!!",
	...
	if (bUpgradeFlag) {
		...
		do {
			...
			if (i_ret == 0) {	/* upgrade success */
				fts_i2c_read_reg(client, FTS_REG_FW_VER,
						 &uc_tp_fm_ver);
				FTS_DEBUG
				    ("[UPGRADE]: upgrade to new version 0x%x",
				     uc_tp_fm_ver);
			} else {	/* upgrade fail */

				/* if upgrade fail, reset to run ROM. if app in flash is ok. TP will work success */
				FTS_INFO
				    ("[UPGRADE]: upgrade fail, reset now!!");
