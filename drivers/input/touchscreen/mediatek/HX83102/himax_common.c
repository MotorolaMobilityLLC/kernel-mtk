...
{
#include "ZN-M3903_CID1A03_D03_C06_07122018.i"
};
#endif
...
//    set_bit(KEY_CUST_14, ts->input_dev->keybit);
    ...
    input_mt_init_slots(ts->input_dev, ts->nFinger_support,2);
#else
...
#ifdef HX_GESTURE_TRACK
    memset(gn_gesture_coor,0,sizeof(gn_gesture_coor));
    switch(gesture_flag){
    case 0x24:
        gn_gesture_coor[0] = 0x01;
        break;
    case 0x27:
        gn_gesture_coor[13] = 1;
    case 0x28:
        gn_gesture_coor[0] = 0x07;
        for(i=0;i<4;i++){
            gn_gesture_coor[i*2+1] = buf[i*2+8]*(ic_data->HX_X_RES)/255;
            gn_gesture_coor[i*2+2] = buf[i*2+9]*(ic_data->HX_Y_RES)/255;
        }
        break;
    case 0x30:
    case 0x57:
        gn_gesture_coor[0] = 0x06;
        for(i=0;i<6;i++){
            gn_gesture_coor[i*2+1] = buf[i*2+8]*(ic_data->HX_X_RES)/255;
            gn_gesture_coor[i*2+2] = buf[i*2+9]*(ic_data->HX_Y_RES)/255;
        }
        gn_gesture_coor[13] = buf[20];
        break;
    case 0x31:
        gn_gesture_coor[0] = 0x0d;
        for(i=0;i<5;i++){
            gn_gesture_coor[i*2+1] = buf[i*2+8]*(ic_data->HX_X_RES)/255;
            gn_gesture_coor[i*2+2] = buf[i*2+9]*(ic_data->HX_Y_RES)/255;
        }
