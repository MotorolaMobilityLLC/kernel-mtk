...
#ifdef HX_THRESHOLD_ADJUST_ENABLE
static int himax_sensitivity_show(struct seq_file *m, void *v)
{
    struct himax_ts_data *ts = private_ts;
    size_t ret = 0;
...
static ssize_t himax_sensitivity_write(struct file *file, const char *buff,
                                size_t len, loff_t *pos)
{
    struct himax_ts_data *ts = private_ts;
    char buf[80] = {0};
    ...
    if (buf[0] == '0') {
        ...
        return -EINVAL;
    E("%s: sensitivity = %d.\n", __func__, ts->sensitivity);
    himax_set_sensitivity_enable(ts->client, ts->sensitivity);
    return len;
}
static int himax_sensitivity_open(struct inode *inode, struct file *file) {
    I("%s: enter, %d \n", __func__, __LINE__);
    return single_open(file, himax_sensitivity_show, NULL);
}

static struct file_operations himax_proc_sensitivity_ops =
{
    .owner = THIS_MODULE,
    .read = seq_read,
    .write = himax_sensitivity_write,
    .open = himax_sensitivity_open,
};
#endif
...
* Return: data len
***********************************************************************/
static ssize_t himax_update_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    if ((count <= 1) || (count >= FILE_NAME_LENGTH - 32)) {
        E("fw bin name's length(%d) fail", (int)count);
        return -EINVAL;
    }
    memset(fwname, 0, sizeof(fwname));
    if (copy_from_user(&fwname, buff, count)) {
        E("[ontim]: copy from user error!!");
        return -EFAULT;
    }
    fwname[count - 1] = '\0';
   ...
   // parse the file name
   I("%s: upgrade from file(%s) start!\n", __func__, fwname);
   result = request_firmware(&fw, fwname, private_ts->dev);
   if (result < 0) {
   	I("fail to request_firmware fwpath: %s (ret:%d)\n", fwname, result);
   	return result;
   }
...
* Return: 
***********************************************************************/
static ssize_t himax_update_ontim_read(struct file *filp, char __user *buff, size_t len, loff_t *ppos)
{
    size_t ret = 0;
    char *temp_buf;
...
static const struct file_operations himax_update_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = himax_update_ontim_read,
    .write  = himax_update_ontim_write,
};
...
* Return: 
***********************************************************************/
static ssize_t himax_gesture_id_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    int cnt = 0;
    u8 buf[56] = { 0 };
    ...
    cnt = sprintf(buf, "0x1,0x2,0x4,0x5,0x6,0x7,0xc,0xd\n");
  
    if (copy_to_user(buff, buf, cnt)) {
        E("[ontim]: copy to user error!!");
        return -EFAULT;
    }
    *ppos += cnt;
    return cnt;
...
static const struct file_operations himax_gesture_id_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = himax_gesture_id_ontim_read,
};

static ssize_t himax_psensor_incall_read(struct file *file, char *buf,
                                 size_t len, loff_t *pos)
{
    struct himax_ts_data *ts = private_ts;
    size_t ret = 0;
   ...
   if(!HX_PROC_SEND_FLAG)
    {
		temp_buf = kzalloc(len,GFP_KERNEL);
        tmp = 1 - ts->irq_enabled;
		ret += snprintf(temp_buf + ret, len - ret, "%d ", tmp);
        ret += snprintf(temp_buf + ret, len - ret, "\n");
        if(copy_to_user(buf, temp_buf, len))
            I("%s,here:%d\n",__func__,__LINE__);
        kfree(temp_buf);
...
static ssize_t himax_psensor_incall_write(struct file *file, const char *buff,
                                  size_t len, loff_t *pos)
{
    struct himax_ts_data *ts = private_ts;
    char buf_tmp[12]= {0};
...
static struct file_operations himax_proc_psensor_incall_ops =
{
    .owner = THIS_MODULE,
    .read = himax_psensor_incall_read,
    .write = himax_psensor_incall_write,
};
/************************************************************************
*   Name: 
*  Brief:
...
* Return: 
***********************************************************************/
static ssize_t himax_gesture_coordinate_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    u8 buf[128] = { 0 };
    int cnt = 0,i=0;
    ...
    for(i=0;i<14;i++)
        cnt += sprintf(buf+cnt, "%d,",gn_gesture_coor[i]); 
    cnt += sprintf(buf+cnt,"\n");

    if (copy_to_user(buff,buf,cnt)) {
        E("%s:[ontim]: copy to user error!!",__func__);
        return -EFAULT;
    }
    *ppos += cnt;
    return cnt;
...
static const struct file_operations himax_gesture_coordinate_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = himax_gesture_coordinate_ontim_read,
};
...
* Return: data len
***********************************************************************/
struct himax_ontim_flag{
    unsigned int glove_sta:1;
    unsigned int sens_leval:2;
    unsigned int gesture_en:1;
    unsigned int resolve:4;
};

static struct himax_ontim_flag himax_state_info_ontim = {0};
void himax_refresh_senstive_ontim(void)
{
    struct himax_ts_data *ts = private_ts;
  if(1 == himax_state_info_ontim.glove_sta)
   {
       ts->HSEN_enable = 1; 
       himax_set_HSEN_enable(ts->client, ts->HSEN_enable, ts->suspended);
   }
    ...
    {
       switch(himax_state_info_ontim.sens_leval){
       case 0:
            himax_set_HSEN_enable(ts->client, 0, ts->suspended);
           break;
       case 1:
            himax_set_HSEN_enable(ts->client, 0, ts->suspended);
           break;
       case 2:
            himax_set_HSEN_enable(ts->client, 1, ts->suspended);
           break;
       default:
           break;
       }
...
* Return: data len
***********************************************************************/
static ssize_t himax_glove_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    struct himax_ts_data *ts = private_ts;
    u8 buf[4] = {0};

    if (copy_from_user(&buf, buff, 1)) {
        E("[ontim]: copy from user error!!");
        return -EFAULT;
    }
   ...
   if ('1' == buf[0]) {
        ts->HSEN_enable = 1;
//        himax_register_write(private_ts->client,addr,1,data,false);
        himax_state_info_ontim.glove_sta = 1;
    } else if ('0' == buf[0]) {
        ts->HSEN_enable = 0;
        himax_state_info_ontim.glove_sta = 0;
//	    himax_refresh_senstive_ontim();
    }
    himax_set_HSEN_enable(ts->client, ts->HSEN_enable, ts->suspended);

    I("%s: glove_enable = %d.\n", __func__, ts->HSEN_enable);
    I("[ontim]glove mode status:  %d", himax_state_info_ontim.glove_sta);
    *ppos += count;
    return count;
}
...
***********************************************************************/
static ssize_t himax_glove_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    int cnt = 0;
    u8 buf[8] = {0};
    struct himax_ts_data *ts = private_ts;
    ...
    I("%s: glove_enable = %d.\n", __func__, ts->HSEN_enable);
    I("[ontim]glove mode status:  %d", himax_state_info_ontim.glove_sta);
    cnt = sprintf(buf, "%d\n",himax_state_info_ontim.glove_sta);
...
static const struct file_operations himax_glove_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = himax_glove_ontim_read,
    .write  = himax_glove_ontim_write,
};
...
* Return: data len
***********************************************************************/
static ssize_t himax_senstive_ontim_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    u8 buf[4] = {0};
    struct himax_ts_data *ts = private_ts;
    ...
    if ((buf[0] <= '2')&&(buf[0]>= '0')){
       if(himax_state_info_ontim.glove_sta == 1)
       {
            himax_state_info_ontim.sens_leval = 2;
       }
       else
       {
        switch(buf[0]){
        case '0':
            himax_set_HSEN_enable(ts->client, 0, ts->suspended);
            himax_state_info_ontim.sens_leval = 0;
            break;
        case '1':
            himax_set_HSEN_enable(ts->client, 0, ts->suspended);
            himax_state_info_ontim.sens_leval = 1;
            break;
        case '2':
            himax_set_HSEN_enable(ts->client, 1, ts->suspended);
            himax_state_info_ontim.sens_leval = 2;
        ...
        I("[ontim]last_senstive_ontim=%d,buf %d\n",himax_state_info_ontim.sens_leval,buf[0]-0x30);
	}
    }
    *ppos += count; 
    return count;
...
* Return: 
***********************************************************************/
static ssize_t himax_senstive_ontim_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    int cnt;
    u8 buf[8] = { 0 };
    if(*ppos) 
    ...
    cnt = sprintf(buf, "%d\n",himax_state_info_ontim.sens_leval );
...
static const struct file_operations himax_senstive_ontim_fops = {
    .owner  = THIS_MODULE,
    .read   = himax_senstive_ontim_read,
    .write  = himax_senstive_ontim_write,
};
...
int himax_touch_proc_init(void)
{
...
#ifdef HX_THRESHOLD_ADJUST_ENABLE
    himax_proc_sensitivity_file = proc_create(HIMAX_PROC_SENSITIVITY_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
                                       himax_touch_proc_dir, &himax_proc_sensitivity_ops);
    if(himax_proc_sensitivity_file == NULL)
    {
        E(" %s: proc SENSITIVITY file create failed!\n", __func__);
        goto fail_19;
    }
#endif
    ...
    himax_proc_fwup_ontim_file = proc_create("firmware_update_enable", 0664, himax_touch_proc_dir, &himax_update_ontim_fops);
    if (NULL == himax_proc_fwup_ontim_file) {
        E(" %s: proc firmware_update_ ontim file create failed!\n", __func__);
        goto fail_20;
    } 

#ifdef HX_SMART_WAKEUP
    himax_proc_gest_en_ontim_file = proc_create("double_tap_enable", (S_IWUSR|S_IRUGO|S_IWUGO),
                                       himax_touch_proc_dir, &himax_proc_SMWP_ops);
    if(himax_proc_gest_en_ontim_file == NULL)
    {
        E(" %s: proc SMWP en file create failed!\n", __func__);
        goto fail_21;
    }
#endif

    himax_proc_psensor_distouch_ontim_file = proc_create("psensor_incall_disable_tp", (S_IWUSR|S_IRUGO),
                                         himax_touch_proc_dir, &himax_proc_psensor_incall_ops);
    if(himax_proc_psensor_distouch_ontim_file == NULL)
    {
        E(" %s: proc psensor dis touch file create failed!\n", __func__);
        goto fail_22;
    }

#ifdef HX_SMART_WAKEUP
    himax_proc_gest_id_ontim_file = proc_create("support_gesture_id", 0444, himax_touch_proc_dir, &himax_gesture_id_ontim_fops);
    if (NULL == himax_proc_gest_id_ontim_file) {
        E(" %s: proc support_gesture_id  file create failed!\n", __func__);
        goto fail_23;
    } 

    himax_proc_gest_coor_ontim_file = proc_create("gesture_coordinate", 0444, himax_touch_proc_dir, &himax_gesture_coordinate_ontim_fops);
    if (NULL == himax_proc_gest_coor_ontim_file) {
        E(" %s: proc gesture_coordinate file create failed!\n", __func__);
        goto fail_24;
    } 

#endif
  #if 0
    himax_proc_glove_en_ontim_file = proc_create("glove_mode_enable", 0664, himax_touch_proc_dir, &himax_glove_ontim_fops);
    if (NULL == himax_proc_glove_en_ontim_file) {
        E(" %s: proc glove_mode_enable file create failed!\n", __func__);
        goto fail_25;
    } 

    himax_proc_senstive_ontim_file = proc_create("tp_thresh_value", 0664, himax_touch_proc_dir, &himax_senstive_ontim_fops);
    if (NULL == himax_proc_senstive_ontim_file) {
        E(" %s: proc tp_thresh_value file create failed!\n", __func__);
        goto fail_26;
    } 
#endif 
    return 0 ;

//    remove_proc_entry("tp_thresh_value", himax_touch_proc_dir);
//fail_26:
 //remove_proc_entry("glove_mode_enable", himax_touch_proc_dir);  
//fail_25:
 
#ifdef HX_SMART_WAKEUP
    remove_proc_entry("gesture_coordinate", himax_touch_proc_dir);
fail_24:
    remove_proc_entry("support_gesture_id", himax_touch_proc_dir);
fail_23:
#endif
    remove_proc_entry("psensor_incall_disable_tp", himax_touch_proc_dir);
fail_22:
#ifdef HX_SMART_WAKEUP
    remove_proc_entry("double_tap_enable", himax_touch_proc_dir);
fail_21:
#endif
    remove_proc_entry("firmware_update_enable", himax_touch_proc_dir);
fail_20:
...
#ifdef HX_THRESHOLD_ADJUST_ENABLE
    remove_proc_entry( HIMAX_PROC_SENSITIVITY_FILE, himax_touch_proc_dir );
fail_19:
#endif
