...
void himax_regulator_switch(int enable)
{
    static unsigned int himax_power_flag = 0;
    
    if (enable){
        if(0 == himax_power_flag){
            enable = regulator_enable(tpd->reg);
            if (enable != 0){
                TPD_DMESG("Failed to regulator_enable touch: %d\n", enable);
            }
            else{
                himax_power_flag=1;
//+add by fxz
...
//-add by fxz
        ...
        if (0 != himax_power_flag){
            enable = regulator_disable(tpd->reg);
            if (enable != 0){
                TPD_DMESG("Failed to regulator_disable touch: %d\n",enable);
            }
            else {
                himax_power_flag=0;
//+add by fxz
