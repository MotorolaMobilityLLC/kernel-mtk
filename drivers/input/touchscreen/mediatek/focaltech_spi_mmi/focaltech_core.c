...
int fts_wait_tp_to_valid(void)
{
    ...
    do {
        ...
        if ((ret < 0) || (idh != chip_idh)|| (idl != chip_idl)) {
            FTS_DEBUG("TP Not Ready,ReadData:0x%x%x", idh, idl);
        } else if ((idh == chip_idh) && (idl == chip_idl)) {
            FTS_INFO("TP Ready,Device ID:0x%x%x", idh, idl);
            return 0;
        }
        cnt++;
...
static void fts_charger_notify_work(struct work_struct *work)
{
	...
	if (USB_DETECT_IN == fts_usb_detect_flag) {
		...
		fts_ex_mode_switch(MODE_CHARGER,USB_DETECT_IN);
		...
		fts_ex_mode_switch(MODE_CHARGER,USB_DETECT_OUT);
		mutex_unlock(&input_dev->mutex);
	}
}
		...
		unsigned long val, void *v)
{
	...
	if (!strcmp(psy->desc->name, "charger")) {
		if (psy && charger_detection && val == POWER_SUPPLY_PROP_STATUS) {
			...
			if (ret < 0) {
				FTS_ERROR("Couldn't get POWER_SUPPLY_PROP_ONLINE rc=%d", ret);
				return ret;
			} else {
				...
				if(fts_usb_detect_flag != charger_detection->usb_connected) {
					...
					if (!ts_data->suspended)
						 queue_work(charger_detection->fts_charger_notify_wq,
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	if (ts_data->charger_detection_enable){
		ts_data->charger_detection = kzalloc(sizeof(struct usb_charger_detection), GFP_KERNEL);
		if (ts_data->charger_detection == NULL) {
			FTS_ERROR("failed to allocated memory for usb_charger_detection\n");
			goto err_charger_detection_alloc_failed;
		}
	}

	if (ts_data->charger_detection) {
		struct power_supply *psy = NULL;
		union power_supply_propval prop = {0};

		FTS_INFO("charger_detection on");
		ts_data->charger_detection->usb_connected = 0;
		ts_data->charger_detection->fts_charger_notify_wq = create_singlethread_workqueue("fts_charger_wq");
		if (!ts_data->charger_detection->fts_charger_notify_wq){
			FTS_ERROR("allocate nvt_charger_notify_wq failed");
			goto err_charger_notify_wq_failed;
		}
		INIT_WORK(&ts_data->charger_detection->charger_notify_work, fts_charger_notify_work);

		ts_data->charger_detection->charger_notif.notifier_call = charger_notifier_callback;
		ret = power_supply_reg_notifier(&ts_data->charger_detection->charger_notif);
		if (ret) {
			FTS_ERROR("Unable to register charger_notifier:%d", ret);
		...
		if (psy) {
			...
			if (ret < 0) {
				...
				fts_usb_detect_flag = prop.intval;
				if(fts_usb_detect_flag != ts_data->charger_detection->usb_connected) {
					 if (USB_DETECT_IN == fts_usb_detect_flag) {
						  ts_data->charger_detection->usb_connected = USB_DETECT_IN;
					 }else{
						  ts_data->charger_detection->usb_connected = USB_DETECT_OUT;
					 }
					 fts_ex_mode_switch(MODE_CHARGER,ts_data->charger_detection->usb_connected);
				}
			}
		}
	}
...
err_register_charger_notify_failed:
    if ( ts_data->charger_detection) {
        if ( ts_data->charger_detection->charger_notif.notifier_call)
            power_supply_unreg_notifier(& ts_data->charger_detection->charger_notif);
        destroy_workqueue( ts_data->charger_detection->fts_charger_notify_wq);
         ts_data->charger_detection->fts_charger_notify_wq = NULL;
        kfree( ts_data->charger_detection);
    }
...
void fts_lcm_update_firmware_work(void)
{
	if (!fts_data)
		return;
...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    ts_data->dev = &spi->dev;
    ts_data->log_level = 1;
    strlcpy(ts_data->fts_firmware_name, FTS_FW_NAME_PREX_WITH_REQUEST, sizeof(FTS_FW_NAME_PREX_WITH_REQUEST));
    spi_set_drvdata(spi, ts_data);

    if (of_property_read_bool(spi->dev.of_node, "focaltech,usb_charger")) {
        FTS_INFO("novatek,usb_charger set");
        ts_data->charger_detection_enable = 1;
	}
    else
        ts_data->charger_detection_enable = 0;
...
void fts_tp_enter_sleep_mode(void)
{
	int ret = 0;
	if (!fts_data)
		return;
...
EXPORT_SYMBOL(fts_tp_enter_sleep_mode);
