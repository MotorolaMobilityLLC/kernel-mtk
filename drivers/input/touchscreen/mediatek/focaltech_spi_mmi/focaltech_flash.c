...
static int fts_get_fw_file_for_test(struct fts_upgrade *upg)
{
    int ret = 0;
    const struct firmware *fw = NULL;
    u8 *tmpbuf = NULL;
    char firmware_name[128] = "focaltech-holi-FT8006S-09-0001-lima.bin";

    if (upg->fwt && upg->fwt_size) {
            FTS_INFO("fwt has fw, no need get firmware again");
            return 0;
    }

    ret = request_firmware(&fw, firmware_name, upg->ts_data->dev);
    if (0 == ret) {
            FTS_INFO("firmware(%s) request successfully", firmware_name);
            ...
            if (NULL == tmpbuf) {
                ...
                upg->fwt = tmpbuf;
                upg->fwt_size = fw->size;
            }
    } else {
        FTS_INFO("firmware(%s) request fail,ret=%d", firmware_name, ret);
    }
...
int fts_enter_test_environment(bool test_state)
{
    ...
    ret = fts_get_fw_file_for_test(upg);
    if (ret < 0) {
            FTS_ERROR("get test fw file fail");
            return ret;
    }

    if (!upg->fwt || (upg->fwt_size <= FTS_MAX_LEN_APP)) {
        FTS_INFO("not multi-app");
        return 0;
    }

    if (test_state) {
        ret = fts_fw_download(upg->fwt + FTS_MAX_LEN_APP, upg->fwt_size, true);
    } else {
        ret = fts_fw_download(upg->fwt, upg->fwt_size, true);
    }
    if (ret < 0) {
...
int fts_fw_resume(void)
{
    ...
    /* 1. request firmware */
    ret = request_firmware(&fw, fts_data->fts_firmware_name, upg->ts_data->dev);
    if (ret != 0) {
        FTS_ERROR("%s:firmware(%s) request fail,ret=%d\n",
                  __func__, fts_data->fts_firmware_name, ret);
        FTS_INFO("download fw from bootimage");
        ret = fts_fw_download(upg->fw, upg->fw_length, false);
    } else {
        FTS_INFO("firmware(%s) request successfully", fts_data->fts_firmware_name);
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    ...
    if (0 == ret) {
        ...
        FTS_INFO("firmware(%s) request fail,ret=%d", fts_data->fts_firmware_name, ret);
    }
