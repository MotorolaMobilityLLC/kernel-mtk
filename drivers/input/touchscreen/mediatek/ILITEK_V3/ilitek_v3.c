...
static int ilitek_charger_notifier_callback(struct notifier_block *nb, unsigned long val, void *v)
{
	...
	if (!psy) {
		ILI_ERR("Couldn't get sgm4154x-charger psy\n");
		return -EINVAL;
	}
...
int ili_fw_upgrade_handler(void *data)
{
	...
	ilitek_tddi_flash_protect(DISABLE, OFF); //flash un-protect for i2c project
	...
	ilitek_tddi_flash_protect(ENABLE, OFF); //flash protect for i2c project
...
static int ilitek_get_tp_module(void)
{
	...
	printk("ilitek_get_tp_module %d\n", ili_tp_module);
	return ili_tp_module;
}
