...
int ili_interface_dev_init(struct ilitek_hwif_info *hwif)
{
	...
	info->hwif = hwif;
	if(strstr(saved_command_line, "ili9883a_hdp_dsi_vdo_txd")){
		ili_tp_module = 0;
		ILI_INFO("Panle is ili_txd_interface_dev_init\n");
	} else if (strstr(saved_command_line, "ili7807s_hdp_dsi_vdo_tm")){
		ili_tp_module = MODEL_TM;
		ILI_INFO("Panle is ili_tm_interface_dev_init, ili_tp_module = %d\n", ili_tp_module);
	} else {
		ILI_ERR("Panel is not ili_interface_dev_init\n");
		return -EINVAL;
	}
