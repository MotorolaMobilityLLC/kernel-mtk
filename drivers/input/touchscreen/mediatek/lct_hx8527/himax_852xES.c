...
static u8 g_proximity_en = 0;
	...
	static unsigned char i_CTPM_FW[]=
	{
		...
		//#include "LQ_L3500_HLT_C03_2017-5-08.i"
		#include "LQ_L3500_HLT_C04_2017-5-22.i"
	};
#endif
#ifdef MTK
#ifdef MTK_I2C_DMA
...
int himax_tpd_rst_gpio_number = 10;
...
/** add by lct wangjiaxing 20170301 start**/
...
/**add by lct wangjiaxing 20170301 end**/
	...
	size_t len, loff_t *pos)
{
	...
	if(!HX_PROC_SEND_FLAG)
	{
		...
		{
			ret = snprintf(buf, PAGE_SIZE, "chip_monitor :%d\n", HX_CHIP_MONITOR_EN);
		}
#endif
		HX_PROC_SEND_FLAG=1;
	}
...
*/
	uint8_t key_index = 0;
	printk("[Simon]Enter himax_vk_parser\n");
	vk = kzalloc(key_num * (sizeof *vk), GFP_KERNEL);
	printk("[Simon]kzalloc \n");
	for(key_index = 0;key_index < key_num ;key_index++)
	{
		/* index ex:back,home.. */
		vk[key_index].index = key_index+1;
		/* key size */
		vk[key_index].x_range_min = key_range[key_index][0], vk[key_index].x_range_max = key_range[key_index][1];
		vk[key_index].y_range_min = key_range[key_index][2], vk[key_index].y_range_max = key_range[key_index][3];
		//I(" [Simon]vk[%d] idx:%d x_min:%d, y_max:%d", key_index,pdata->virtual_key[key_index].index,pdata->virtual_key[key_index].x_range_min, pdata->virtual_key[key_index].y_range_max);
	}
	
	pdata->virtual_key = vk;
	printk("[Simon] Exit  himax_vk_parser\n");
...
static int himax_parse_dt(struct himax_ts_data *ts,struct himax_i2c_platform_data *pdata)
{
	...
	himax_vk_parser(pdata,3);  //now 3 keys
...
#ifdef MTK_I2C_DMA// Allocate the MTK's DMA memory
...
#ifndef MTK_INT_NOT_WORK_WORKAROUND
	himax_int_enable(ts->client->irq,1,true);
#else
...
void himax852xes_suspend(struct early_suspend *h)
{
	int ret;
	uint8_t buf[2] = {0};
#ifdef HX_CHIP_STATUS_MONITOR
...
#endif
static void himax852xes_resume(struct early_suspend *h)
{
#ifdef HX_SMART_WAKEUP
	int ret;
	uint8_t buf[2] = {0};
