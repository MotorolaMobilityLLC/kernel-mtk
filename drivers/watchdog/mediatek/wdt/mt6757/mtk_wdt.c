 ...
 */
void mtk_wdt_mode_config(bool dual_mode_en, bool irq, bool ext_en, bool ext_pol,
			 bool wdt_en)
{
#ifndef CONFIG_KICK_SPM_WDT
	unsigned int tmp;
...
#else
	/* pr_debug(" mtk_wdt_mode_config  mode
...
void wdt_arch_reset(char mode)
{
	...
	/* clear autorestart bit: autoretart: 1, bypass power key, 0: not bypass
	 * power key
	 */
	...
	if (mode & WD_SW_RESET_BYPASS_PWR_KEY) {
		/* Bypass power key reboot, We using auto_restart bit as by pass
		 * power key flag
		 */
		wdt_mode_val =
	...
	/*
	 * disable ddr reserve mode if we are doing normal reboot to avoid
	 * unexpected dram issue.
	 * exception types:
	 *   0: normal
	 *   1: HWT
	 *   2: KE
	...
	if (mark_bit == MTK_WDT_REQ_MODE_SPM_SCPSYS) {
		...
		if (en == WD_REQ_EN) {
			if (ext_debugkey_io != -1) {
				ext_debugkey_io_eint = ext_debugkey_io;
				/* mt_gpio_to_eint(ext_debugkey_io); */
				pr_debug("RGU ext_debugkey_io_eint is %d\n",
					 ext_debugkey_io_eint);
				ext_req_con =
				    (ext_debugkey_io_eint << 4) | 0x01;
...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	pr_debug("%s: ext_debugkey_io=%d\n", __func__, ext_debugkey_io);
...
#else
	ret =
	    request_fiq(AP_RGU_WDT_IRQ_ID, wdt_fiq, IRQF_TRIGGER_FALLING, NULL);
#endif /* CONFIG_KICK_SPM_WDT */
#endif
	...
	if (ret != 0) {
		...
		ret = 0;
		pr_debug("%s : ok to fail IRQ in non-secure\n", __func__);
	}
 ...
 * And the default value is 0x3 which means reset from a power off and power on
 * reset
 */
...
void mtk_wd_resume(void)
{
	...
	/*aee_sram_pr_debug("[WDT] resume(%d)\n", g_wdt_enable);*/
...
#ifndef CONFIG_OF
struct platform_device mtk_device_wdt = {.name = "mtk-wdt", .id = 0, .dev = {} };
#endif
...
static void spm_wdt_init(void)
{
	...
	/* pr_debug("WDT REQ_MODE=0x%x,  WDT REQ_EN=0x%x\n",*/
	/* __raw_readl(MTK_WDT_REQ_MODE), __raw_readl(MTK_WDT_REQ_IRQ_EN)); */

	tmp = __raw_readl(MTK_WDT_MODE);
	tmp |= MTK_WDT_MODE_KEY;
	/* disable wdt */
	tmp &= (~(MTK_WDT_MODE_IRQ | MTK_WDT_MODE_ENABLE |
		  MTK_WDT_MODE_DUAL_MODE));
...
static int __init mtk_wdt_init(void)
{
	...
	if (ret) {
		pr_debug("****[mtk_wdt_driver] Unable to device register(%d)\n",
			 ret);
		return ret;
	}
#endif
	...
	if (ret) {
		pr_debug(
		    "****[mtk_wdt_driver] Unable to register driver (%d)\n",
		    ret);
		return ret;
	}
	pr_debug("%s ok\n", __func__);
...
static int __init mtk_wdt_get_base_addr(void)
{
	...
	np_rgu =
	    of_find_compatible_node(NULL, NULL, rgu_of_match[0].compatible);
