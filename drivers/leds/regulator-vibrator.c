...
#define DEFAULT_DLS_LIMIT 70

struct reg_vibr_config {
	unsigned int min_limit;
	unsigned int max_limit;
	unsigned int dls_limit;
	unsigned int min_volt;
	unsigned int max_volt;
	unsigned int lv_volt;
	unsigned int sv_volt;
	int	vib_cam_front_gpio;
	int	vib_cam_rear_gpio;
		...
		struct reg_vibr_config *vibr_conf)
{
	...
	ret = of_property_read_u32(dev->of_node, "dls-limit",
		&(vibr_conf->dls_limit));
	if (ret)
		vibr_conf->dls_limit = DEFAULT_DLS_LIMIT;
	...
	ret = of_property_read_u32(dev->of_node, "lv-volt",
		&vibr_conf->lv_volt);
	if (ret) {
		pr_notice("Error load dts: get lv-volt failed!\n");
		ret = -EINVAL;
		return ret;
	}

	ret = of_property_read_u32(dev->of_node, "sv-volt",
		&vibr_conf->sv_volt);
	if (ret) {
		pr_notice("Error load dts: get sv-volt failed!\n");
		ret = -EINVAL;
		return ret;
	}
	...
	vibr_conf->vib_cam_front_gpio = of_get_named_gpio(dev->of_node, "vib-cam-front-gpio", 0);
	vibr_conf->vib_cam_rear_gpio = of_get_named_gpio(dev->of_node,  "vib-cam-rear-gpio", 0);
	pr_info("%s: vibr_conf->vib-cam-front-gpio=%d.\n", __func__, vibr_conf->vib_cam_front_gpio);
	pr_info("%s: vibr_conf->vib-cam-rear-gpio=%d.\n", __func__, vibr_conf->vib_cam_rear_gpio);
	
	pr_info("vibr_conf = %u, %u, %u, %u-%u, %u, %u\n",
		vibr_conf->min_limit, vibr_conf->max_limit, vibr_conf->dls_limit,
		vibr_conf->min_volt, vibr_conf->max_volt,
		vibr_conf->lv_volt, vibr_conf->sv_volt);
...
static void vibr_enable(struct reg_vibr *vibr)
{
	...
	if ((gpio_is_valid(vibr->vibr_conf.vib_cam_front_gpio) &&
			gpio_get_value(vibr->vibr_conf.vib_cam_front_gpio)) ||
			(gpio_is_valid(vibr->vibr_conf.vib_cam_rear_gpio) &&
			gpio_get_value(vibr->vibr_conf.vib_cam_rear_gpio))){
			pr_info("%s, (&vibr->state_cam... vibr->reg_status=%d )\n", __func__, vibr->reg_status);
			vibr->state_cam = 1;
			return;
	}
	
	if (!vibr->reg_status) {
		if(vibr->vibr_dur > vibr->vibr_conf.dls_limit) {// long vibrate
			ret = regulator_set_voltage(vibr->vibr_conf.reg,
				vibr->vibr_conf.lv_volt, vibr->vibr_conf.lv_volt);
			if (ret < 0)
				pr_notice("set lv voltage fail, ret = %d\n", ret);
		} else {
			ret = regulator_set_voltage(vibr->vibr_conf.reg,
				vibr->vibr_conf.sv_volt, vibr->vibr_conf.sv_volt);
			if (ret < 0)
				pr_notice("set sv voltage fail, ret = %d\n", ret);
		}
...
static void vibr_disable(struct reg_vibr *vibr)
{
	...
	if (((gpio_is_valid(vibr->vibr_conf.vib_cam_front_gpio) &&
			gpio_get_value(vibr->vibr_conf.vib_cam_front_gpio)) ||
			(gpio_is_valid(vibr->vibr_conf.vib_cam_rear_gpio) &&
			gpio_get_value(vibr->vibr_conf.vib_cam_rear_gpio)))  && vibr->state_cam){
			pr_info("%s, (&vibr->state_cam...)\n", __func__);
			vibr->state_cam = 0;
			return;
	}
