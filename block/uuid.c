...
static struct page *read_bdev_page(struct block_device *dev, int page_num)
{
	...
	if (!page) {
		pr_err("Failed to allocate a page for reading data " "in UUID checks.");
		return NULL;
	}
	...
	if (bio_add_page(bio, page, PAGE_SIZE, 0) < PAGE_SIZE) {
		pr_debug("ERROR: adding page to bio at %d\n", page_num);
		bio_put(bio);
		__free_page(page);
		pr_debug("read_bdev_page freed page %p (in error " "path).\n", page);
		return NULL;
	}
...
no_uuid:
		PRINT_HEX_DUMP(KERN_EMERG, "fs_info_from_block_dev returning uuid ", DUMP_PREFIX_NONE, 16, 1,
			fs_info->uuid, 16, 0);
