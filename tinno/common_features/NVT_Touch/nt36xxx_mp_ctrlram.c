...
{
	return scnprintf(buf, PAGE_SIZE, "noavtek");
}
...
//		snprintf(mpcriteria, PAGE_SIZE, "novatek-mp-criteria-%04X", ts->nvt_pid);
	...
	ret = (TestResult_FW_Rawdata | TestResult_Noise | TestResult_Short | TestResult_Open);
...
int nvt_fw_class_init(void *_data,bool create)
{
	...
	NVT_LOG("%s touchscreen class files", create ? "Add" : "Remove");

	if (create) {
		error = alloc_chrdev_region(&devno, 0, 1, NVT_SPI_NAME);
			if (error) {
			NVT_LOG
		...
		NVT_LOG("Create class 'touchscreen'");
		touchscreen_class = class_create(THIS_MODULE, "touchscreen");
		if (IS_ERR(touchscreen_class)) {
			NVT_ERR("Create class 'touchscreen' failed %ld",
		...
		ts_class_dev = device_create(touchscreen_class, NULL,
					     devno,
					     data, "%s", NVT_SPI_NAME);
			NVT_LOG("Create device '" NVT_SPI_NAME "'");

		if (IS_ERR(ts_class_dev)) {
			NVT_ERR("Create device '" NVT_SPI_NAME
		...
		NVT_LOG("Create attr files");
		for (i = 0; attrs[i].attr.name != NULL; ++i) {
			NVT_LOG("  Create attr file '%s'", attrs[i].attr.name);
			error = device_create_file(ts_class_dev, &attrs[i]);
			if (error) {
				NVT_ERR("Create attr file '%s' failed %d",
		...
		if (error) {
			...
			NVT_LOG("Create /sys/class/touchscreen/ Succeeded");
		}
		} else {
		...
		for (i = 0; attrs[i].attr.name != NULL; ++i) {
			NVT_LOG("Remove device file '%s'", attrs[i].attr.name);
...
device_destroy:
	...
	ts_class_dev = NULL;
	class_unregister(touchscreen_class);
	NVT_ERR("Creating touchscreen class failed %d", error);
