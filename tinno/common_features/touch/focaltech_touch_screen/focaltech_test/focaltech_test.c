...
* Return: EPERM
***********************************************************************/
extern struct selftest_result fts_selftest_result[5];
static ssize_t fts_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (global_vendor_id == 0x82) //HLT
	ini_name = "TP_selftest_config_HLT.ini";
    else if (global_vendor_id == 0x87) //LC
	ini_name = "TP_selftest_config_LC.ini";
    else {
	FTS_TEST_DBG("invalid INI file.\n");
	return count;
    }

    FTS_TEST_DBG("INI file:%s.", ini_name);
	...
	if (0)
	for (i=0; i<5; i++) {
    		count += snprintf(buf + count, PAGE_SIZE, "%s%s\n", fts_selftest_result[i].item, 
			fts_selftest_result[i].supported?(fts_selftest_result[i].result? "Pass": "Failed"): "Not Supported");
	}

	for (i=0; i<4; i++) {
	//skip panel differ test
		test_result = test_result && fts_selftest_result[i].result;
	}

	if (test_result)
    		count += snprintf(buf + count, PAGE_SIZE, "RETURN=PASS");
	else
    		count += snprintf(buf + count, PAGE_SIZE, "RETURN=ERROR");
