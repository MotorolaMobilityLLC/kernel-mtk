...
#include <dev_info.h>
static char tpd_venor_name[50];
#define ILITEK_CTP_VENDOR_GPO 0x1
#endif
...
#ifdef CONFIG_TINNO_PRODUCT_INFO
static char *ilitek_get_vendor_name(u8 vendor_id)
{
    switch(vendor_id)
    {
        case 0:
        case ILITEK_CTP_VENDOR_GPO:
            return "TXD";
        default:
            return "UNKNOWN";
    }
...
char *ilitek_tpd_get_fw_vendor_name(void)
{
    sprintf(tpd_venor_name, "%s", ilitek_get_vendor_name(1));
    return tpd_venor_name;
}

int ilitek_get_tp_fw_info(char *buf, void *arg0)
{
    return sprintf(buf,
        "%s-%s-%s-V%02x",
        ilitek_tpd_get_fw_vendor_name(),
...
static int ilitek_plat_probe(void)
{
	...
	if (ilitek_tddi_init() < 0) {
		...
		ilitek_tddi_dev_remove(DISABLE);
		return -ENODEV;
	}
		...
		{
			int ilitek_get_tp_fw_info(char *buf, void *arg0);
			FULL_PRODUCT_DEVICE_CB(ID_TP, ilitek_get_tp_fw_info, NULL);
		}
#endif
