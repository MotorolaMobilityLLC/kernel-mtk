...
#define RDSR_XCHG_SRAM_ADDR \
    ...
    if((ret = wait_sfctrl_xfer_comp(cts_dev)) != 0) {
        cts_err("Wait sfctrl ready failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if (ret) {
        cts_err("Read exchange sram failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
...
#define FLASH_SR_WIP        BIT(0)      /*!< Write in progress */
    ...
    do {
        ...
        if (ret) {
            cts_err("Read flash status register failed %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
        ...
        u8 cmd, void *data, u32 flash_addr, u32 sram_addr, size_t size, u32 flags)
{
    ...
    if (flags & SFCTRLv2_OPFLAG_SRAM_DATA_XCHG) {
        ...
        if ((!(flags & SFCTRLv2_OPFLAG_READ)) && data) {
            ...
            if (ret) {
                cts_err("Write data to exchange sram "
                    "0x%06x size %zu failed %d(%s)",
                    sram_addr, size, ret, cts_strerror(ret));
                return ret;
            }
        }
    }
    ...
    if (flags & SFCTRLv2_OPFLAG_WAIT_WIP_CLR &&
        ...
        cts_err("Wait WIP clear failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
    ...
    if ((flags & SFCTRLv2_OPFLAG_READ) && data) {
        ...
        if (ret) {
            cts_err("Read data from exchange sram "
                    "0x%06x size %zu failed %d(%s)",
                sram_addr, size, ret, cts_strerror(ret));
            return ret;
        }
