    ...
    struct device_node *np)
{
    ...
    if (ret) {
        cts_warn("Parse power supply prop failed %d(%s)",
            ret, cts_strerror(ret));
        ...
        if (psp < 0) {
            cts_warn("Power supply prop: '%s' NOT supported", psp_str);
            cd_data->psp = CFG_CTS_DEF_CHGR_DET_PSY_PROP;
        } else {
            cd_data->psp = psp;
        }
    ...
    if (ret) {
        cts_warn("Parse power supply prop polling interval failed %d(%s)",
            ret, cts_strerror(ret));
    }
...
static int start_charger_detect(struct cts_charger_detect_data *cd_data)
{
    ...
    if(cd_data->type == CTS_CHGR_DET_TYPE_PSY_NOTIFY) {
        ...
        if (ret) {
            cts_err("Register power supply notifier failed: %d(%s)",
                ret, cts_strerror(ret));
            return ret;
        }
...
static void set_dev_charger_state_work(struct work_struct *work)
{
    ...
    if (ret) {
        cts_err("Set dev charger state to %s failed %d(%s)",
    ...
    unsigned long action, void *data)
{
    ...
    if (action != PSY_EVENT_PROP_CHANGED) {
        cts_dbg("Power supply notify event %ul not care", action);
        return NOTIFY_DONE;
    }
...
static int disable_charger_detect(struct cts_charger_detect_data *cd_data)
{
    ...
    if (ret) {
        cts_err("Disable detect failed %d(%s)",
            ret, cts_strerror(ret));
    }
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret) {
        cts_err("%s charger detect failed %d(%s)",
    ...
    enum cts_charger_detect_type type)
{
    ...
    if (cd_data->type == type) {
        cts_warn("Switch detect type same with current");
        return 0;
    }
    ...
    if (running) {
        ...
        if (ret) {
            cts_err("Start detect failed %d(%s)",
                ret, cts_strerror(ret));
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret) {
        cts_err("Switch charger detect type failed %d(%s)",
            ret, cts_strerror(ret));
        return ret;
    }
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (cd_data->psy_name == NULL ||
        strcasecmp(cd_data->psy_name, argv[0]) != 0) {
...
int cts_init_charger_detect(struct chipone_ts_data *cts_data)
{
    ...
    if (ret) {
        cts_err("Init detect failed %d(%s)", ret, cts_strerror(ret));
        goto free_psy_name;
    }
    ...
    if (ret) {
        cts_warn("Create sysfs attr group '%s' failed %d(%s)",
            CHARGER_DET_SYSFS_GROUP_NAME, ret, cts_strerror(ret));
    } else {
