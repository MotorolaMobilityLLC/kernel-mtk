...
#define COMP_CAP_TEST_DATA_FILENAME     "comp-cap-test-data.txt"
	...
	cts_info("result:%d,%d,%d,%d\n",rawdata_test_result,
	noise_test_result,open_test_result,short_test_result);
...
#define COMP_CAP_TEST_DATA_FILENAME     "comp-cap-test-data.txt"
    ...
    if((rawdata_test_result ^ noise_test_result
	^ open_test_result ^ short_test_result) ==0){
	count = 1;
    } else {
	count = 0;
    }
