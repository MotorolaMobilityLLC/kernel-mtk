...
extern int cts_read_flash_to_sram_check_crc_retry(const struct cts_device *cts_dev,
        u32 flash_addr, u32 sram_addr, size_t size, u32 crc, int retry);
static inline int cts_read_flash_to_sram_check_crc(const struct cts_device *cts_dev,
            u32 flash_addr, u32 sram_addr, size_t size, u32 crc)
{
    return cts_read_flash_to_sram_check_crc_retry(cts_dev,
            flash_addr, sram_addr, size, crc, CTS_FLASH_READ_DEFAULT_RETRY);
}

extern int cts_erase_flash(const struct cts_device *cts_dev, u32 addr, size_t size);
