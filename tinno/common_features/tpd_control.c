...
static struct delayed_work tpd_init_work;        //yajun temp for tp use
			...
			unsigned long event, void *data)
{
	...
	//add by lizhongyu begin
	//if (event != FB_EVENT_BLANK)
	//	return 0;
	// add by lizhongyu end
...
static int __init tpd_device_init(void)
{
	...
	INIT_DELAYED_WORK(&tpd_init_work, tpd_init_work_callback);    //yajun temp for tp use

	//res = queue_work(tpd_init_workqueue, &tpd_init_work);

	res =queue_delayed_work(tpd_init_workqueue, &tpd_init_work,              //yajun temp for tp use
