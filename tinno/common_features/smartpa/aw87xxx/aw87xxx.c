...
int  isphonecall = 0 ;  //  1 :iscallphone 0: is not phonecall
EXPORT_SYMBOL(isphonecall);//  export    zane

/*****************************************************************
* aw87xxx marco
******************************************************************/
#define AW87XXX_I2C_NAME	"aw87xxx_pa"
#define AW87XXX_DRIVER_VERSION	"v2.1.0"
...
int aw87xxx_set_profile(int dev_index, char *profile)
{
	...
	AW_LOGE("zane not found struct aw87xxx, dev_index = [%d]", dev_index);
	return -EINVAL;
}
 ...
 *
 ****************************************************************************/
 static ssize_t aw87xxx_attr_get_isphonecall(struct device *dev,    
			struct device_attribute *attr, char *buf)
{                                                             // zane for get isphonecall statu
         isphonecall=1;                                              
         printk("  %s %d \n",__FUNCTION__,__LINE__);
		 return 0;
}

static ssize_t aw87xxx_attr_set_isphonecall(struct device *dev,                 
			struct device_attribute *attr, const char *buf,
			size_t len)
{                                                             // zane  for set isphonecall statu
          long val;
          printk("%s********isphonecall=%d**** \n",__FUNCTION__,isphonecall);
...
static DEVICE_ATTR(isphonecall, 0664,         
		aw87xxx_attr_get_isphonecall, aw87xxx_attr_set_isphonecall); // zane  
...
static struct attribute *aw87xxx_attributes[] = {
	...
	&dev_attr_isphonecall.attr,  
	NULL
};             // zane   added  isphonecall
				...
				struct device_node *dev_node)
{
	...
	AW_DEV_LOGE(aw87xxx->dev, " aw87xxx_dtsi_parse ");
	if (ret < 0) {
