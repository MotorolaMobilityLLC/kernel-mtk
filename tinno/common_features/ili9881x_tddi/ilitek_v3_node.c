...
int ili_fw_class_init(void *_data, bool create)
{
	struct ilitek_ts_data *data = _data;
	...
	ILI_INFO("%s touchscreen class files", create ? "Add" : "Remove");

	if (create) {
		if(NULL != data->hwif->name) {
			error = alloc_chrdev_region(&devno, 0, 1, data->hwif->name);
		} else {
			error = alloc_chrdev_region(&devno, 0, 1, ILITEK_CHIP_NAME);
		}
		if (error) {
			ILI_INFO
		...
		ILI_INFO("Create class 'touchscreen'");
		touchscreen_class = class_create(THIS_MODULE, "touchscreen");
		if (IS_ERR(touchscreen_class)) {
			ILI_ERR("Create class 'touchscreen' failed %ld",
		...
		if(NULL != data->hwif->name) {
			ts_class_dev = device_create(touchscreen_class, NULL,
					     devno,
					     data, "%s", data->hwif->name);
			ILI_INFO("Create device for IC: %s", data->hwif->name);
		} else {
			ts_class_dev = device_create(touchscreen_class, NULL,
					     devno,
					     data, "%s", ILITEK_CHIP_NAME);
			ILI_INFO("Create device '" ILITEK_CHIP_NAME "'");
		}
		if (IS_ERR(ts_class_dev)) {
			ILI_ERR("Create device '" ILITEK_CHIP_NAME
		...
		ILI_INFO("Create attr files");
		for (i = 0; attrs[i].attr.name != NULL; ++i) {
			ILI_INFO("  Create attr file '%s'", attrs[i].attr.name);
			error = device_create_file(ts_class_dev, &attrs[i]);
			if (error) {
				ILI_ERR("Create attr file '%s' failed %d",
		...
		if (error) {
			...
			ILI_INFO("Create /sys/class/touchscreen/ Succeeded");
		}
	} else {
		...
		for (i = 0; attrs[i].attr.name != NULL; ++i) {
			ILI_INFO("Remove device file '%s'", attrs[i].attr.name);
...
device_destroy:
	...
	ts_class_dev = NULL;
	class_unregister(touchscreen_class);
	ILI_ERR("Creating touchscreen class failed %d", error);
