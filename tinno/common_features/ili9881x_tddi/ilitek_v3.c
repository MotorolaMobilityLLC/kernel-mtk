...
extern int ili_fw_class_init(void *_data, bool create);
...
#ifdef CONFIG_TINNO_PRODUCT_INFO
int ili_get_tp_fw_info(char *buf, void *arg0) 
{
	u8 cmd[2] = {0};
	u8 buf1[10] = {0};

	if (ilits->info_from_hex) {
		buf1[1] = ilits->fw_info[48];
		buf1[2] = ilits->fw_info[49];
		buf1[3] = ilits->fw_info[50];
		buf1[4] = ilits->fw_info[51];
	}
	...
	if (ilits->wrapper(&cmd[1], sizeof(u8), buf1, ilits->protocol->fw_ver_len, ON, OFF)) {
	...
	ILI_INFO("Firmware version in ilitek_main.c = %d.%d.%d.%d\n", buf1[1], buf1[2], buf1[3], buf1[4]);
	ilits->chip->fw_ver = buf1[1] << 24 | buf1[2] << 16 | buf1[3] << 8 | buf1[4];
...
int ili_tddi_init(void)
{
	...
	if(ili_fw_class_init(ilits,true) < 0)
		ILI_ERR("Create touchscreen class failed\n");
...
{
	int ili_get_tp_fw_info(char *buf, void *arg0);
	FULL_PRODUCT_DEVICE_CB(ID_TP, ili_get_tp_fw_info, NULL);
}
#endif
