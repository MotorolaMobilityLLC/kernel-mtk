...
static struct regulator *ldoreg;//lijian add
////////////////////////////////////////////////////////////////////////////////
...
//For ftm test
#if 0
static char chip_name[64] = {'\0', };
static char hal_version[64] = {'\0', };
static unsigned short chip_id = 0x0000;
static int finger_state = -1;
#else
//static char chip_name[64] = {'\0', };
static char hal_version[64] = {'\0', };
static unsigned short chip_id = 0x9362;
static int finger_state = 0;
#endif

static int get_chip_id(char *buf, void *args)
{
...
{
     FF_LOGE("'%s' enter.finger_state=%d", __func__,finger_state);
    return sprintf(buf, "%d", finger_state);
}
...
#if 0//init by driver,not ca,add lijian.
    if (g_config) {
...
#else
	input_set_capability(g_context->input, EV_KEY, 212); //lijian,20190524:init KEY_CAMERA = 212
#endif
...
static int ff_ctl_free_driver(void)
{
    ...
    if(0) {//add by lijian for disable other fp ic irq wake fun.
	FF_LOGE("focalfp---disable_irq_wake(%d).", g_context->irq_num);
...
static int ff_ctl_init_driver(void)
{
    ...
    if (err) {
		FF_LOGE("ff_spi_init(err),start ff_ctl_free_driver..... ");
        ff_ctl_free_driver();
        return err;
    }
    FF_LOGE("start ff_ctl_register_input....");
	...
	// for ftm test
	//REGISTER_FP_DEV_INFO(NULL, NULL, NULL.miscdev.this_device, get_finger_state, NULL);
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	// For ftm test
	#if 0
	case FF_IOC_WRITE_CHIP_NAME: {
        if (copy_from_user(chip_name, (char *)arg, 64)) {
            FF_LOGE("copy_from_user(..) failed.");
            err = (-EFAULT);
            break;
        }
		FF_LOGI("%s: chip_name is %s.", __func__, chip_name);
		//REGISTER_FP_DEV_INFO(chip_name, NULL, NULL, NULL, NULL);
        break;
    }
	case FF_IOC_WRITE_CHIP_ID: {
        if (copy_from_user(&chip_id, (unsigned short *)arg, sizeof(unsigned short))) {
            FF_LOGE("copy_from_user(..) failed.");
            err = (-EFAULT);
            break;
        }
		FF_LOGI("%s: chip_id is %d.", __func__, chip_id);
		REGISTER_FP_DEV_INFO(NULL, NULL, NULL, NULL, get_chip_id);
        break;
    }
	#endif
	case FF_IOC_WRITE_HAL_VERSION: {
        if (copy_from_user(hal_version, (char *)arg, 64)) {
            FF_LOGE("copy_from_user(..) failed.");
            err = (-EFAULT);
            break;
        }
		FF_LOGI("%s: hal_version is %s.", __func__, hal_version);
		REGISTER_FP_DEV_INFO(NULL, hal_version, NULL, NULL, NULL);
        break;
    }
	case FF_IOC_WRITE_FINGER_STATE: {
        if (copy_from_user(&finger_state, (int *)arg, sizeof(int))) {
            FF_LOGE("copy_from_user(..)finger_state failed.");
            err = (-EFAULT);
            break;
        }
		FF_LOGI("%s: finger_state is %d.", __func__, finger_state);
        break;
    }
...
static int ff_power_init(void)
{
	...
	ldoreg = regulator_get(ff_ctl_context.miscdev.this_device, "vio28");
	...
	//if (regulator_count_voltages(reg) > 0) {
		ret = regulator_set_voltage(ldoreg, 2800000,2800000);
		if (ret) {
	...
	ret = regulator_enable(ldoreg);	/*enable regulator*/
	if (ret)
	...
	#if 1
reg_vdd_put:
	regulator_put(ldoreg);
	
	return ret;
	#endif
}
#endif
int ff_power_enable(bool on)
{
	int ret = 0;
	if(!ldoreg){
		FF_LOGE("error!!!!!!!!!regulator not exit return !!!!");
		return -1;
	}
	if(on){
		ret = regulator_enable(ldoreg);
		FF_LOGI("fp regulator enable=%d",ret);
	}else{
		ret = regulator_disable(ldoreg);
		FF_LOGI("fp regulator disable=%d",ret);
	}
	return ret;
}
...
static int is_use_focalfp(void)
{
    char * tinno_fp_info;
    ...
    {
        printk(KERN_INFO "ssss---cannot get focal--fpinfo \n");
        return 0;
    }
    printk(KERN_INFO "ssss---ft9362_is_open :fp_info = %s \n",tinno_fp_info);

    if (NULL != strstr(tinno_fp_info, "focal_fp"))
	return 1;
    else
	return 0;
}
#if (0==FF_DRV_READID_EN)
...
static int __init ff_ctl_driver_init(void)
{
    int err = 0;
    FF_LOGV("'%s' enter.", __func__);
    if(0 == is_use_focalfp()){//lijian add for check fp info
   	FF_LOGE("ssss---not use focal fp\n");
   	return (-ENODEV);
   }
	...
	err = ff_power_init();
	 FF_LOGI("FocalTech fingerprint poweron=%d",err);
...
#if 1
    /* Initialize the chip. */
    //if (ff_ctl_init_driver()){//&& ff_chip_init() != 0) {
    #if FF_DRV_READID_EN
		if (!ff_ctl_init_driver()&& ff_chip_init() == 0x9362){
		REGISTER_FP_DEV_INFO("focaltech_fp", NULL, ff_ctl_context.miscdev.this_device, get_finger_state, get_chip_id);
    	}
	#else
    	if (!ff_ctl_init_driver()){
	...
	////init by driver,not ca,add lijian.
		if (g_context->b_driver_inited) {
	        err = ff_ctl_free_driver();
	        g_context->b_driver_inited = false;
		}
