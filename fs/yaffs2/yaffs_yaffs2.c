...
int yaffs_block_ok_for_gc(struct yaffs_dev *dev, struct yaffs_block_info *bi)
{
	 ...
	 * discarded pages.
	 */
	ret = (bi->seq_number <= dev->oldest_dirty_seq);
	return ret;
}
...
static int yaffs2_rd_checkpt_tnodes(struct yaffs_obj *obj)
{
	...
	struct yaffs_file_var *file_struct_ptr = &obj->variant.file_variant;
...
int yaffs2_scan_backwards(struct yaffs_dev *dev)
{
	...
	block_index = kcalloc(n_blocks, sizeof(struct yaffs_block_index),
			GFP_NOFS);
	...
	pr_err("yaffsdebug scandumpblockseq devn:%s scann:%d s:%d e:%d\n",
			dev->param.name, n_to_scan, dev->internal_start_block, dev->internal_end_block);
	if (!strcmp(dev->param.name, "userdata")) {/*only dump userdata partition seq num.*/
		...
		pr_err("yaffsdebug scandumpblocknum:\n");
	...
	for (block_iter = end_iter; !alloc_failed && block_iter >= start_iter;
		...
		for (c = dev->param.chunks_per_block - 1;
			...
			if (!tags.chunk_used) {
				...
				if (found_chunks) {
					...
					if (dev->seq_number == bi->seq_number) {
						 ...
						 */
						yaffs_trace(YAFFS_TRACE_SCAN,
							"Partially written block %d detected", blk);
				}
			...
			if (check_on && (check_block > 0)) {
				...
				if (retv < 0) {
					dev->n_free_chunks++;
					pr_err("yaffsdebug re-check page fail!retv:%d i:%d eccr:%d cb:%d sn:%d\n",
							retv, i, tags.ecc_result, check_block, bi->seq_number);

					check_on = 0;
					check_block--;
#ifdef YAFFS_MVG_TEST_DEBUG_LOG
					pr_err("dump data cant fixed ecc!chunk:%d addr:0x%x\n",
							chunk, chunk * dev->data_bytes_per_chunk);
					mtk_dump_byte(chunk_data, dev->data_bytes_per_chunk, 0);
					pr_err("dump tag\n");
					mtk_dump_byte(&tags, sizeof(struct yaffs_ext_tags), 0);

					pr_err("dump tmpdata cant fixed ecc!chunk:%d addr:0x%x\n",
							chunk, chunk * dev->data_bytes_per_chunk);
			...
			if (tags.chunk_id > 0) {
				...
				unsigned int endpos;
				u32 chunk_base = (tags.chunk_id - 1) * dev->data_bytes_per_chunk;
				...
				if (in &&
					 ...
					 * seen an object header yet. Stop this practice once we find an object
					 * header.
					 */
			...
			if (!in->valid &&
				...
				/* Note re hardlinks.
				 * Since we might scan a hardlink before its equivalent object is
				 * scanned we put them all in a list.
				...
				case YAFFS_OBJECT_TYPE_FILE:
					...
					if (in->variant.file_variant.scanned_size < file_size) {
				...
				case YAFFS_OBJECT_TYPE_HARDLINK:
					if (!is_unlinked) {
						in->variant.hardlink_variant.equiv_id = equiv_id;
						in->hard_links.next = (struct list_head *)hard_list;
						hard_list = in;
					}
					break;
				...
				case YAFFS_OBJECT_TYPE_SYMLINK:
					if (!oh)
						break;
					in->variant.symlink_variant.alias = yaffs_clone_str(oh->alias);
