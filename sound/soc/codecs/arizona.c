...
static void arizona_disable_fll(struct arizona_fll *fll)
{
	...
	arizona_wait_for_fll(fll, false);
        ...
        /*patch by karl for reset FLL related registers to default state that keep FLL write without cache protect*/
  	regmap_write(arizona->regmap, fll->base + 2,0x0008);
	regmap_write(arizona->regmap, fll->base + 3,0x0018);	
	regmap_write(arizona->regmap, fll->base + 4,0x007D);	
	regmap_write(arizona->regmap, fll->base + 5,0x0006);	
	regmap_write(arizona->regmap, fll->base + 6,0x0000);	
	regmap_write(arizona->regmap, fll->base + 7,0x0281);	
	regmap_write(arizona->regmap, fll->base + 8,0x0000);	
	regmap_write(arizona->regmap, fll->base + 9,0x0000);	
	
	arizona_fll_dbg(fll, "Disabling FLL related other registers\n");	
	if (change)
