...
void switch_type(int type)
{
	switch(type) {
		case FS15XX :
			pa_type = 0;
			pr_err("%s,pa_type is FS15xx",__func__);
			break;
		case AW87XXX :
			pa_type = 1;
			pr_err("%s,pa_type is aw87xxx",__func__);
			break;
		default :
			pa_type = -1;
			pr_err("%s,can not get pa_type",__func__);
			break;
	}
	return;
}
...
{
	if (input) {
		input_report_key(input,handset_monitor_key,1);
		input_sync(input);
		msleep(10);
		input_report_key(input,handset_monitor_key,0);
		input_sync(input);
	} else {
...
int handset_monitor_init(void)
{
	...
	input_set_capability(input,EV_KEY,HANDSET_MONITOR_ON_KEY);
	input_set_capability(input,EV_KEY,HANDSET_MONITOR_OFF_KEY);
	input->name="HandsetStatus";

	ret = input_register_device(input);
	if(ret)
	{
		pr_err("%s: input_register_device fail \n",__func__);
