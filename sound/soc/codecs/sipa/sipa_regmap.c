...
static const struct reg_map_info reg_map_info_table[] = {
	...
	[CHIP_TYPE_SIA9177] = {
		...
		.reg_val_width = 16,
		.chip_id_addr = 0x06,
		.chip_id_ranges = {{0x5880, 0x5880},{0x5890, 0x5892},{0x58A0, 0x58A1}},
		.chip_id_range_num = 3
	},
	[CHIP_TYPE_SIA8150] = {
	...
	[CHIP_TYPE_SIA5118] = {
		...
		.reg_val_width = 16,
		.chip_id_addr = 0x00,
		.chip_id_ranges = {{0x3B00, 0x3B01}},
		.chip_id_range_num = 1
	},
	[CHIP_TYPE_SIA9197X] = {
		.chip_type = CHIP_TYPE_SIA9197X,
		.reg_addr_width = 8,
		.reg_val_width = 16,
		.chip_id_addr = 0x00,
		.chip_id_ranges = {{0x6280, 0x62FF}, {0x6300, 0x637F}, {0x5e80, 0x5eFF}},
		.chip_id_range_num = 3
	},
	[CHIP_TYPE_SIA8132] = {
		.chip_type = CHIP_TYPE_SIA8132,
		.reg_addr_width = 8,
		.reg_val_width = 8,
		.chip_id_addr = 0x00,
		.chip_id_ranges = {{0x32, 0x32}},
		.chip_id_range_num = 1
	},
	[CHIP_TYPE_SIA5118S] = {
		.chip_type = CHIP_TYPE_SIA5118S,
		.reg_addr_width = 8,
		.reg_val_width = 16,
		.chip_id_addr = 0x00,
		.chip_id_ranges = {{0x3B80, 0x3B82}},
		.chip_id_range_num = 1
	},
	[CHIP_TYPE_SIA9306] = {
		.chip_type = CHIP_TYPE_SIA9306,
		.reg_addr_width = 8,
		.reg_val_width = 16,
		.chip_id_addr = 0x00,
		.chip_id_ranges = {{0x5300, 0x537F}},
		.chip_id_range_num = 1
	},
	[CHIP_TYPE_SIA8168] = {
	...
	[CHIP_TYPE_SIA9287] = {
		.chip_type = CHIP_TYPE_SIA9287,
		.reg_addr_width = 8,
		.reg_val_width = 16,
		.chip_id_addr = 0x06,
		.chip_id_ranges = {{0x8F80, 0x8FFF}},
		.chip_id_range_num = 1
	},
	[CHIP_TYPE_SIA9165] = {
		.chip_type = CHIP_TYPE_SIA9165,
		.reg_addr_width = 8,
		.reg_val_width = 16,
		.chip_id_addr = 0x00,
		.chip_id_ranges = {{0x9880, 0x98FF}},
		.chip_id_range_num = 1
	},
};
...
check_chip_id_retry:
	if (0 != sipa_read_reg(regmap, reg_map_info_table[chip_type].chip_id_addr, &val)) {
		pr_err("[  err][%s] %s: failed to read CHIP_ID \r\n", LOG_FLAG, __func__);
		return -1;
	}

	pr_info("[ info][%s] %s: chip_id_addr(0x%04x), val(0x%04x)\r\n",
	...
	pr_err("[  err][%s] %s: chip_id out of range\r\n",
		LOG_FLAG, __func__);

	if (need_retry && retries) {
			retries--;
			usleep_range(I2C_RETRY_DELAY*1000, I2C_RETRY_DELAY*1000);
			goto check_chip_id_retry;
		}
	...
	SIPA_REG_PROC *reg, unsigned int val_width)
{
	...
	if (NULL == si_pa || NULL == reg) {
		pr_err("[  err][%s] %s: si_pa or reg is null \r\n", LOG_FLAG, __func__);
		return -EINVAL;
	}
	...
	case SIPA_REG_READ:
		...
		// if (reg->addr <= 0x14)
			pr_info("[ info][%s] %s: addr:0x%x, val:0x%04x\n", LOG_FLAG, __func__,
				reg->addr, val);
		break;
	case SIPA_REG_WRITE:
	...
	sipa_dev_t *si_pa)
{
	...
	if (crc_cal != crc) {
		pr_err("[  err][%s] %s: ch%d check trim crc failed! crc_cal = 0x%x, crc = 0x%x\r\n",
