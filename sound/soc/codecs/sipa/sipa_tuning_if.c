...
{
    //if the queue is full, can't insert
    if (obj->head == 0 && obj->tail + 1 == MAX_SIZE){
        pr_err("[ err] %s: case 1 queue is full \n", __func__);
        return -1;
    }

    else if((obj->tail + 1) % MAX_SIZE == obj->head){
        pr_err("[ err] %s: case 2 queue is full \n", __func__);
        return -1;
    }

    //insert data
    if (copy_from_user(obj->q[obj->tail].data, arg, len)){
        pr_err("[ err] %s: copy_from_user failed \n", __func__);
        return -EFAULT;
    }
    obj->q[obj->tail].len  = len;
    obj->q[obj->tail].flag = true;
    obj->tail++;
    //Loop subscript to determine .if the boundary is touched, it is corrected manually
    if (obj->tail == obj->k) {
        obj->tail = 0;
    }
    return 0;
...
{
    mutex_lock(&obj->lock);
    //if the queue is empty , it cannot be deleted.
    if (obj->head == obj->tail) {
        mutex_unlock(&obj->lock);
        pr_err("[ err] %s: queue is empty \n", __func__);
        return -1;
    }

    if (copy_to_user(arg, obj->q[obj->head].data, obj->q[obj->head].len)) {
        mutex_unlock(&obj->lock);
        pr_err("[ err] %s: copy_to_user failed \n", __func__);
        return -EFAULT;
    }
