...
static int max98926_spk_zcd_get(struct snd_kcontrol *kcontrol,
...
error_get_vtg_i2c:
    if (regulator_count_voltages(max98926_vcc_i2c) > 0)
        regulator_set_voltage(max98926_vcc_i2c, 0,
                              VCC_I2C_MAX_UV);
error_reg_en_vcc_i2c:
    if (pullup) { reg_set_optimum_mode_check(max98926_vcc_i2c, 0); }
error_reg_opt_i2c:
...
// temp modify stream name for porting
...
static int max98926_probe(struct snd_soc_codec *codec)
{
    ...
    */
    ...
    //snd_soc_write(codec, max98926_CONFIGURATION, 0xF0);
    /* Disable ALC muting */
...
static int __init max98926_init(void)
{
    ...
    pr_err("platform_device_alloc \n");
    Max98926_dev = platform_device_alloc("max98926L", -1);
    if (!Max98926_dev)
    {
        return -ENOMEM;
    }
    pr_err("platform_device_add \n");
