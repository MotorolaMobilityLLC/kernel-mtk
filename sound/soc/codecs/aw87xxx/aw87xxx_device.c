...
int aw87xxx_dev_esd_reg_status_check(struct aw_device *aw_dev)
{
	...
	if (esd_desc->sysctl_reg_addr != AW_REG_NONE) {
		ret = aw87xxx_dev_i2c_read_byte(aw_dev, esd_desc->sysctl_reg_addr,
				&reg_val);
		if (ret < 0) {
			AW_DEV_LOGE(aw_dev->dev, "read reg 0x%02x failed",
				esd_desc->sysctl_reg_addr);
			return ret;
		}

		AW_DEV_LOGD(aw_dev->dev, "0x%02x:default val=0x%02x real val=0x%02x",
			esd_desc->sysctl_reg_addr,
			esd_desc->sysctl_reg_val, reg_val);

		reg_val &= esd_desc->sysctl_reg_mask;
		if (reg_val != esd_desc->sysctl_reg_val) {
			AW_DEV_LOGE(aw_dev->dev, "sysctl reg status check failed, reg_val=0x%02x", reg_val);
			return -EINVAL;
		}
	}
...
static void aw_dev_pid_9b_init(struct aw_device *aw_dev)
{
	...
	aw_dev->esd_desc.sysctl_reg_addr = AW_REG_NONE;
}
...
static void aw_dev_chipid_76_init(struct aw_device *aw_dev)
{
	...
	aw_dev->esd_desc.sysctl_reg_addr = AW87XXX_PID_76_SYSCTRL_REG;
	aw_dev->esd_desc.sysctl_reg_mask = AW87XXX_PID_76_SYSCTRL_MASK;
	aw_dev->esd_desc.sysctl_reg_val = AW87XXX_PID_76_SYSCTRL_CHECK;
}
