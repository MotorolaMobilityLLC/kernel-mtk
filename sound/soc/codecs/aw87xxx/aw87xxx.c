...
#define AW87XXX_DRIVER_VERSION	"v2.6.2"
...
void aw87xxx_get_rcv_enable(int dev_index)
{
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx->dev_index == dev_index) {
			AW_DEV_LOGI(aw87xxx->dev, "current profile is [%s]",
					aw87xxx->current_profile);
			if (!strncmp(aw87xxx->current_profile, "Receiver", AW_PROFILE_STR_MAX)) {
				handset_monitor_select(input,HANDSET_MONITOR_ON_KEY);
				handset_mode_flag = 1;
			}
			if (strncmp(aw87xxx->current_profile, "Receiver", AW_PROFILE_STR_MAX) && (handset_mode_flag == 1)) {
