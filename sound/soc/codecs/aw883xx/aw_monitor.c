...
#define AW881XX_MONITOR_NAME "aw883xx_monitor.bin"
						...
						uint32_t vmax)
{
	...
	aw_dev_info(aw_dev->dev, "set vmax = 0x%x", vmax);
}
...
static int aw_monitor_real_time_update_monitor(struct aw_device *aw_dev)
{
	...
	ret = request_firmware(&cont, AW881XX_MONITOR_NAME, aw_dev->dev);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "failed to read %s", AW881XX_MONITOR_NAME);
