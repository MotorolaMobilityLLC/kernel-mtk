...
#define AW883XX_DRIVER_VERSION "v0.3.0"
...
int aw883xx_reg_reads(struct aw883xx *aw883xx,
			uint8_t reg_addr, uint8_t *data_buf,
			uint16_t data_len)
{
	int ret = -1;
	...
	ret = aw883xx_i2c_reads(aw883xx, reg_addr, data_buf, data_len);
	if (ret < 0)
		aw_dev_err(aw883xx->dev, "aw883xx_i2c_reads fail, ret=%d", ret);
...
static int aw883xx_request_firmware_file(struct aw883xx *aw883xx)
{
	...
	if (ret < 0) {
		aw_dev_err(aw883xx->dev, "failed to read %s",
			AW883XX_ACF_FILE);
...
static int aw883xx_awrw_write(struct aw883xx *aw883xx, const char *buf, size_t count)
{
	...
	/*aw_dev_dbg(aw883xx->dev, "reg_addr: 0x%02x", data_buf[0]);*/

	for (i = 0; i < data_len; i++) {
		sscanf(buf + AWRW_HDR_LEN + 1 + i * 5, "0x%02x", &temp_data);
		data_buf[i] = temp_data;
		aw_dev_dbg(aw883xx->dev, "[%d] : 0x%02x", i, data_buf[i]);
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < data_len; i++) {
		len += snprintf(buf + len, PAGE_SIZE - len,
			"0x%02x,", reg_data[i]);
		aw_dev_dbg(aw883xx->dev, "0x%02x", reg_data[i]);
	}
	ret = len;
