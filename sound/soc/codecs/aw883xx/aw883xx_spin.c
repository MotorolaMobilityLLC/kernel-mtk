...
/* aw883xx_spin.c   aw883xx codec driver
 *
...
static int aw883xx_hold_reg_spin_st(struct aw_spin_desc *spin_desc)
{
...
static int aw883xx_hold_dsp_spin_st(struct aw_spin_desc *spin_desc)
{
	struct aw_device *aw_dev = container_of(spin_desc,
						struct aw_device, spin_desc);
	int ret = -1;
...
int aw883xx_spin_hold_chan_params(struct aw_spin_desc *spin_desc, uint8_t type)
{
	...
	switch (type) {
	case AW_REG_SPIN_ST:
		return aw883xx_hold_reg_spin_st(spin_desc);
	case AW_DSP_SPIN_ST:
		return aw883xx_hold_dsp_spin_st(spin_desc);
	default:
...
static int aw_set_reg_spin_angle(struct aw883xx *aw883xx, uint32_t spin_angle)
{
	...
	list_for_each(pos, dev_list) {
		local_pa = container_of(pos, struct aw_device, list_node);
		ret = aw883xx_device_params(local_pa, AW_DEV_CHANNEL_MODE_PARAMS,
				(void *)&spin_angle, sizeof(spin_angle), AW_SET_DEV_PARAMS);
		if (ret < 0) {
			aw_dev_err(local_pa->dev, "set channal mode failed");
			return ret;
		}
	}
			...
			struct snd_ctl_elem_info *uinfo)
{
	...
	if (ret < 0)
		aw_pr_err("copy enumerated name failed");
