...
void aw_cali_get_cali_re(struct aw_cali_desc *cali_desc)
{
	int ret;
	...
	if (ret < 0) {
		...
		aw_dev_err(aw_dev->dev, "get re failed, use default value[0]");
	}
}
...
static int aw_cali_svc_get_devs_r0(struct aw_device *aw_dev, int32_t *r0_buf, int num)
{
	...
	list_for_each (pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel < num) {
			ret = aw_cali_svc_get_dev_re(local_dev, &r0_buf[local_dev->channel]);
			if (ret) {
...
static int aw_cali_svc_get_smooth_cali_re(struct aw_device *aw_dev)
{
	...
	if (dsp_re < aw_dev->re_range.re_min || dsp_re > aw_dev->re_range.re_max) {
		aw_dev_err(aw_dev->dev,
				"out range re value: %d, set default re: 0",
				dsp_re);
		dsp_re = 0;
	}
	...
	aw_cali_write_re_to_nvram(dsp_re, aw_dev->channel);
...
static int aw_cali_svc_devs_cali_re(struct aw_device *aw_dev, unsigned int flag)
{
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		ret = aw_cali_svc_cali_mode_enable(local_dev, CALI_TYPE_RE,
					flag, true);
		if (ret < 0)
			goto error;
	}
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		aw_cali_svc_cali_mode_enable(local_dev, CALI_TYPE_RE,
					flag, false);
		aw_dev_clear_int_status(local_dev);
	}
...
static int aw_cali_svc_devs_cali_f0_q(struct aw_device *aw_dev, unsigned int flag)
{
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		ret = aw_cali_svc_cali_mode_enable(local_dev, CALI_TYPE_F0, flag, true);
		if (ret < 0)
			goto error;
	}
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		aw_cali_svc_cali_mode_enable(local_dev, CALI_TYPE_F0, flag, false);
		aw_dev_clear_int_status(local_dev);
	}
...
static void aw_cali_class_attr_init(struct aw_device *aw_dev)
{
	...
	if (ret)
		aw_dev_info(aw_dev->dev, "creat class_attr_f0_calib fail");


	ret = class_create_file(&aw_cali_class, &class_attr_f0_q_calib);
	if (ret)
		aw_dev_info(aw_dev->dev, "creat class_attr_f0_q_calib fail");

	ret = class_create_file(&aw_cali_class, &class_att_re_range);
	if (ret)
		aw_dev_info(aw_dev->dev, "creat class_att_re_range fail");
}
			...
			unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case AW_IOCTL_GET_REAL_R0: {
			ret = aw_cali_svc_get_dev_re(aw_dev, data_32_ptr);
		} break;
...
static ssize_t aw_cali_misc_read(struct file *filp, char __user *buf, size_t size, loff_t *pos)
{
	...
	case CALI_STR_SHOW_RE: {
		...
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
				len += snprintf(local_buf+len, PAGE_SIZE-len, "dev[%d]:%u mOhms ", i, temp_data[i]);
