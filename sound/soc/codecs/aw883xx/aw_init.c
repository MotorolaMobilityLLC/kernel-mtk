...
static int aw_pid_2049_get_hw_mon_st(struct aw_device *aw_dev, bool *is_enable)
{
	int ret = 0;
	uint32_t dsp_data = 0;
	struct aw883xx *aw883xx = (struct aw883xx *)aw_dev->private_data;
	...
	ret = aw883xx_dsp_read(aw883xx,
		AW_PID_2049_DSP_REG_CFG_MBMEC_GLBCFG, &dsp_data, AW_DSP_16_DATA);
	if (ret < 0) {
		aw_dev_err(aw883xx->dev, "read hardware monitor status failed");
		return ret;
	}

	dsp_data &= (~AW_PID_2049_DSP_MONITOR_MASK);
	if (dsp_data)
		*is_enable = true;
	else
		*is_enable = false;
...
static int aw883xx_dev_init(struct aw883xx *aw883xx)
{
	...
	aw_pa->re_range.re_min = AW_RE_MIN;
	aw_pa->re_range.re_max = AW_RE_MAX;

	/*aw_pa->index = g_aw883xx_dev_cnt;*/
...
{
	if (aw883xx->chipid == AW883XX_PID_2049) {
