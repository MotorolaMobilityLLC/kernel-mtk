...
#endif
#ifdef CONFIG_SND_SOC_AW87XXX // zane added 20211010
extern int aw87xxx_set_profile(int dev_index, char *profile);
static char *aw_profile[] = {"Music", "Off","Voice"};  // zane  music
enum aw87xxx_dev_index {
 AW_DEV_0 = 0,
};
#endif
extern int isphonecall;  // zane added for  load phonecall PA  parameters 
...
static void Speaker_Amp_Change(bool enable)
{
	if (enable) {
		...
		if(isphonecall==1){
		    printk("***Speaker_Amp_Change()***set_profile 2*isphonecall=%d=",isphonecall);
	        aw87xxx_set_profile(AW_DEV_0, aw_profile[2]);  // zane added 20211214  for  phonecall
		
		}
		else{
			printk("***Speaker_Amp_Change()***set_profile 0*isphonecall=%d=",isphonecall);
	        aw87xxx_set_profile(AW_DEV_0, aw_profile[0]);  // zane added 20211214  for  phonecall
...
static void Headset_Speaker_Amp_Change(bool enable)
{
	if (enable) {
		...
		/* disable Pull-down HPL/R to AVSS28_AUD */
			hp_pull_down(false);
		#ifdef CONFIG_SND_SOC_AW87XXX   //  zane added 20220222
			if(isphonecall==1){
				printk("***Headset_Speaker_Amp_Change()***set_profile 2*isphonecall=%d=",isphonecall);
				aw87xxx_set_profile(AW_DEV_0, aw_profile[2]);  // zane added 20211214  for	phonecall
			
			}
			else{
				printk("***Headset_Speaker_Amp_Change()***set_profile 0*isphonecall=%d=",isphonecall);
				aw87xxx_set_profile(AW_DEV_0, aw_profile[0]);  // zane added 20211214  for	phonecall
		...
		if (GetDLStatus() == false) {
			...
			/* Disable NCP */
			Ana_Set_Reg(AUDNCP_CLKDIV_CON3, 0x1, 0x1);
			#ifdef CONFIG_SND_SOC_AW87XXX    // zane added 2022022
               aw87xxx_set_profile(AW_DEV_0, aw_profile[1]);
            #endif
			TurnOffDacPower();
		}
