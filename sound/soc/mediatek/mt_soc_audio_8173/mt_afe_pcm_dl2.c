 ...
 */
static int mt_pcm_dl2_close(struct snd_pcm_substream *substream);
static int mt_pcm_dl2_prestart(struct snd_pcm_substream *substream);
static int mt_pcm_dl2_post_stop(struct snd_pcm_substream *substream);
...
static const struct snd_pcm_hardware mt_pcm_dl2_hardware = {
	...
	.period_bytes_max = (DL2_MAX_BUFFER_SIZE / SOC_NORMAL_USE_PERIODS_MIN),
...
static int mt_pcm_dl2_open(struct snd_pcm_substream *substream)
{
	...
	snd_soc_set_runtime_hwparams(substream, &mt_pcm_dl2_hardware);

	ret = snd_pcm_hw_constraint_list(runtime, 0, SNDRV_PCM_HW_PARAM_RATE,
					 &mt_pcm_dl2_constraints_rates);
	if (unlikely(ret < 0))
...
static int mt_pcm_dl2_hw_params(struct snd_pcm_substream *substream,
				 struct snd_pcm_hw_params *hw_params)
{
	...
	if (ret >= 0)
		mt_afe_init_dma_buffer(MT_AFE_MEM_CTX_DL2, runtime);
	else
...
static int mt_pcm_dl2_hw_free(struct snd_pcm_substream *substream)
{
	pr_debug("%s\n", __func__);
	return snd_pcm_lib_free_pages(substream);
}

static int mt_pcm_dl2_prepare(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt_pcm_dl2_priv *priv = snd_soc_platform_get_drvdata(rtd->platform);
	...
	/* HW sequence: */
	/* mt_pcm_dl2_prestart->codec->mt_pcm_dl2_start */
	if (likely(!priv->prepared)) {
		mt_afe_add_ctx_substream(MT_AFE_MEM_CTX_DL2, substream);
...
static int mt_pcm_dl2_start(struct snd_pcm_substream *substream)
{
...
static int mt_pcm_dl2_stop(struct snd_pcm_substream *substream)
{
	pr_debug("%s\n", __func__);
...
static int mt_pcm_dl2_trigger(struct snd_pcm_substream *substream, int cmd)
{
	pr_debug("%s cmd=%d\n", __func__, cmd);
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		return mt_pcm_dl2_start(substream);
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		return mt_pcm_dl2_stop(substream);
	}
	return -EINVAL;
}

static snd_pcm_uframes_t mt_pcm_dl2_pointer(struct snd_pcm_substream *substream)
{
	return mt_afe_update_hw_ptr(MT_AFE_MEM_CTX_DL2);
}

static struct snd_pcm_ops mt_pcm_dl2_ops = {
	.open = mt_pcm_dl2_open,
	.close = mt_pcm_dl2_close,
	.ioctl = snd_pcm_lib_ioctl,
	.hw_params = mt_pcm_dl2_hw_params,
	.hw_free = mt_pcm_dl2_hw_free,
	.prepare = mt_pcm_dl2_prepare,
	.trigger = mt_pcm_dl2_trigger,
	.pointer = mt_pcm_dl2_pointer,
};

static const char *const mt_pcm_dl2_playback_mux_function[] = {
...
static int dl2_playback_mux_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	struct mt_pcm_dl2_priv *priv = snd_soc_component_get_drvdata(component);

	ucontrol->value.integer.value[0] = priv->playback_mux;
	return 0;
}

static int dl2_playback_mux_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_dl2_priv *priv = snd_soc_component_get_drvdata(component);
...
static const char *const mt_pcm_dl2_i2s0_clock_function[] = { "Normal", "Low Jitter" };

static int dl2_i2s0_clock_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_dl2_priv *priv = snd_soc_component_get_drvdata(component);
...
static int dl2_i2s0_clock_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_dl2_priv *priv = snd_soc_component_get_drvdata(component);
...
static int dl2_i2s1_clock_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_dl2_priv *priv = snd_soc_component_get_drvdata(component);
...
static int dl2_i2s1_clock_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_dl2_priv *priv = snd_soc_component_get_drvdata(component);
...
static const struct soc_enum mt_pcm_dl2_control_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt_pcm_dl2_playback_mux_function),
			mt_pcm_dl2_playback_mux_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt_pcm_dl2_i2s0_clock_function),
			mt_pcm_dl2_i2s0_clock_function),
};

static const struct snd_kcontrol_new mt_pcm_dl2_controls[] = {
	SOC_ENUM_EXT("DL2_Playback_Mux", mt_pcm_dl2_control_enum[0],
		dl2_playback_mux_get, dl2_playback_mux_set),
	SOC_ENUM_EXT("DL2_I2S0_Clock", mt_pcm_dl2_control_enum[1],
		dl2_i2s0_clock_get, dl2_i2s0_clock_set),
	SOC_ENUM_EXT("DL2_I2S1_Clock", mt_pcm_dl2_control_enum[1],
		dl2_i2s1_clock_get, dl2_i2s1_clock_set),
};

static int mt_pcm_dl2_probe(struct snd_soc_platform *platform)
{
	snd_soc_add_platform_controls(platform, mt_pcm_dl2_controls,
				ARRAY_SIZE(mt_pcm_dl2_controls));
	return 0;
}

static struct snd_soc_platform_driver mt_pcm_dl2_platform = {
	.ops = &mt_pcm_dl2_ops,
	.probe = mt_pcm_dl2_probe,
};

static int mt_pcm_dl2_dev_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct mt_pcm_dl2_priv *priv;
	int rc;
	...
	if (dev->of_node) {
		dev_set_name(dev, "%s", MT_SOC_DL2_PCM);
		pr_debug("%s set dev name %s\n", __func__, dev_name(dev));
	}

	priv = devm_kzalloc(dev, sizeof(struct mt_pcm_dl2_priv), GFP_KERNEL);
	if (unlikely(!priv)) {
	...
	return snd_soc_register_platform(dev, &mt_pcm_dl2_platform);
}

static int mt_pcm_dl2_dev_remove(struct platform_device *pdev)
{
	pr_debug("%s\n", __func__);
...
static const struct of_device_id mt_pcm_dl2_dt_match[] = {
	{.compatible = "mediatek," MT_SOC_DL2_PCM,},
	{}
};

MODULE_DEVICE_TABLE(of, mt_pcm_dl2_dt_match);

static struct platform_driver mt_pcm_dl2_driver = {
	.driver = {
		   .name = MT_SOC_DL2_PCM,
		   .owner = THIS_MODULE,
		   .of_match_table = mt_pcm_dl2_dt_match,
		   },
	.probe = mt_pcm_dl2_dev_probe,
	.remove = mt_pcm_dl2_dev_remove,
};

module_platform_driver(mt_pcm_dl2_driver);

MODULE_DESCRIPTION("AFE PCM DL2 platform driver");
MODULE_LICENSE("GPL");
