...
#include <sound/pcm.h>
#include <sound/soc.h>
#if !defined(CONFIG_MTK_AUDIO_LEGACY)
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	...
	pr_debug("Ext_Speaker_Amp_Change pin_extspkamp:0x%x, pin_mode_extspkamp:%d\n", pin_extspkamp, pin_mode_extspkamp);
#endif
	if (enable) {
		...
		mt_set_gpio_mode(pin_extspkamp, GPIO_MODE_00);	/* GPIO61, mode 0 */
		...
		if (pin_extspkamp_2 != NULL_PIN_DEFINITION) {
			mt_set_gpio_mode(pin_extspkamp_2, GPIO_MODE_00);	/* GPIO61, mode 0 */
		...
		pr_debug("Ext_Speaker_Amp_Change start AW8737_MODE2+\n");
		AW8737_MODE3;
		pr_debug("Ext_Speaker_Amp_Change start AW8737_MODE2-\n");
#else
...
#define GAP (2)			/* unit: us */
/*1.2w*/
static void AW8737_MODE1(void)
{
	mt_set_gpio_out(GPIO_EXT_SPKAMP_EN_PIN, GPIO_OUT_ONE);
}
/*1.0w*/
static void AW8737_MODE2(void)
{
	do {
		mt_set_gpio_out(GPIO_EXT_SPKAMP_EN_PIN, GPIO_OUT_ONE);
		udelay(GAP);
