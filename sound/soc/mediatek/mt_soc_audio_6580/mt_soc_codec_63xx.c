...
/* AW8737 PA output power mode control */
#define AW8737_MODE_CTRL
	...
	if (enable) {
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			if (mIsExtSPKUse) {
				...
				//+Bug 259263 zhangjun.wt modified turn on HPH_L and HPH_R 2017/4/21
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	...
	if (enable) {
		...
		mt_set_gpio_mode(pin_extspkamp, GPIO_MODE_00);	/* GPIO9, mode 0 */
		...
		pr_debug("Ext_Speaker_Amp_Change AW8737_MODE3 start\n");
		AW8737_MODE3;
		pr_debug("Ext_Speaker_Amp_Change AW8737_MODE3 end\n");
#else
...
#define GAP (2)			/* unit: us */
/*1.2w*/
static void AW8155_MODE1(void)
{
	mt_set_gpio_out(GPIO_EXT_SPKAMP_EN_PIN, GPIO_OUT_ONE);
}
/*1.0w*/
static void AW8155_MODE2(void)
{
	do {
		mt_set_gpio_out(GPIO_EXT_SPKAMP_EN_PIN, GPIO_OUT_ONE);
		udelay(GAP);
