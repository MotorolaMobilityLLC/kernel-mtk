...
static const uint16_t kSideToneCoefficientTable48k[] = {
	0x0401, 0xFFB0, 0xFF5A, 0xFECE, 0xFE10, 0xFD28, 0xFC21, 0xFB08,
	0xF9EF, 0xF8E8, 0xF80A, 0xF76C, 0xF724, 0xF746, 0xF7E6, 0xF90F,
	0xFACC, 0xFD1E, 0xFFFF, 0x0364, 0x0737, 0x0B62, 0x0FC1, 0x1431,
...
static const unsigned int afe_buffer_regs
	[Soc_Aud_AFE_IO_Block_NUM_OF_IO_BLOCK][aud_buffer_ctrl_num] = {
...
static const int MEM_BLOCK_ENABLE_REG_NUM =
	ARRAY_SIZE(mMemAudioBlockEnableReg);
...
void Enable4pin_I2S0_I2S3(unsigned int SampleRate, unsigned int wLenBit)
{
	...
	/*Afe_Set_Reg(AUDIO_TOP_CON1, 0x2,  0x2);*/ /* I2S_SOFT_Reset  4 wire
	 *i2s mode
	 */
...
bool EnableSideToneFilter(bool stf_on)
{
	...
	pr_debug("+%s(), stf_on = %d, rate %u, kSTFCoef[0]=0x%x\n", __func__,
		 stf_on, eSamplingRate, kSideToneCoefficientTable[0]);
	AudDrv_Clk_On();
...
bool set_chip_adc_in(unsigned int rate)
{
	...
	/* enable aud_pad_top fifo, need set after GPIO enable, pmic miso clk
	 * on
	 */
...
bool SetChannels(unsigned int Memory_Interface, unsigned int channel)
{
	...
	/* pr_debug("%s Memory_Interface = %d channels = %d\n",
...
ssize_t AudDrv_Reg_Dump(char *buffer, int size)
{
	...
	n += scnprintf(buffer + n, size - n, "AFE_ADDA_SRC_DEBUG = 0x%x\n",
		       Afe_Get_Reg(AFE_ADDA_SRC_DEBUG));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_SRC_DEBUG_MON0 = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_SRC_DEBUG_MON0));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_SRC_DEBUG_MON1 = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_SRC_DEBUG_MON1));
	...
	n += scnprintf(buffer + n, size - n, "AFE_IRQ2_MCU_CNT_MON = 0x%x\n",
		       Afe_Get_Reg(AFE_IRQ2_MCU_CNT_MON));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_IRQ1_MCU_EN_CNT_MON = 0x%x\n",
		      Afe_Get_Reg(AFE_IRQ1_MCU_EN_CNT_MON));
	...
	n += scnprintf(buffer + n, size - n,
		       ...
		       Afe_Get_Reg(AFE_ADDA_DL_SDM_FIFO_MON));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_DL_SRC_LCH_MON = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_DL_SRC_LCH_MON));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_DL_SRC_RCH_MON = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_DL_SRC_RCH_MON));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_DL_SDM_OUT_MON = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_DL_SDM_OUT_MON));
	...
	n += scnprintf(buffer + n, size - n, "AFE_ADDA_MTKAIF_CFG0 = 0x%x\n",
		       Afe_Get_Reg(AFE_ADDA_MTKAIF_CFG0));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_MTKAIF_TX_CFG1 = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_MTKAIF_TX_CFG1));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_MTKAIF_RX_CFG0 = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_MTKAIF_RX_CFG0));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_MTKAIF_RX_CFG1 = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_MTKAIF_RX_CFG1));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_MTKAIF_RX_CFG2 = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_MTKAIF_RX_CFG2));
...
void RunIRQHandler(enum Soc_Aud_IRQ_MCU_MODE irqIndex)
{
	if (Aud_IRQ_Handler_Funcs[irqIndex] != NULL)
		...
		pr_debug("%s(), Aud_IRQ%d_Handler is Null", __func__,
			 irqIndex);
}
...
bool IsNeedToSetHighAddr(bool usingdram, dma_addr_t addr) { return false; }
...
{
	Afe_Set_Reg(AFE_ADDA_DL_SDM_DCCOMP_CON, (enable ? 1 : 0) << 8,
		    0x1 << 8);
	return 0;
}
