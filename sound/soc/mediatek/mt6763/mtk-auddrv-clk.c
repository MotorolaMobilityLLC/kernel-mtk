...
static const unsigned int mI2SAPLLDivSelect[Soc_Aud_I2S_CLKDIV_NUMBER] = {
	...
	AUDIO_APLL12_DIV4, AUDIO_APLL12_DIVB};
...
static struct audio_clock_attr aud_clks[CLOCK_NUM] = {
	 ...
	 [CLOCK_DAC] = {"aud_dac_clk", false, false,
		    NULL}, /* AudDrv_Clk_On only */
	 [CLOCK_DAC_PREDIS] = {"aud_dac_predis_clk", false, false,
			   NULL}, /* AudDrv_Clk_On only */
	 [CLOCK_ADC] = {"aud_adc_clk", false, false,
		    NULL}, /* AudDrv_ADC_Clk_On only */
	 ...
	 [CLOCK_MUX_AUDIOINTBUS] = {"top_mux_audio_int", false, false,
				NULL}, /* AudDrv_AUDINTBUS_Sel */
	 [CLOCK_TOP_SYSPLL1_D4] = {"top_sys_pll1_d4", false, false,
			       NULL}, /* AudDrv_AUDINTBUS_Sel */
...
static int audio_idle_notify_call(struct notifier_block *nfb, unsigned long id,
				  void *arg)
{
	switch (id) {
...
static int apll1_mux_setting(bool enable)
{
	...
	if (enable) {
		...
		if (ret) {
			pr_debug("%s clk_set_parent %s-%s fail %d\n", __func__,
			       aud_clks[CLOCK_TOP_MUX_AUD_1].name,
...
static int apll2_mux_setting(bool enable)
{
	...
	if (enable) {
		...
		if (ret) {
			pr_debug("%s clk_set_parent %s-%s fail %d\n", __func__,
			       aud_clks[CLOCK_TOP_MUX_AUD_2].name,
		...
		if (ret) {
			pr_debug("%s clk_set_parent %s-%s fail %d\n", __func__,
			       aud_clks[CLOCK_TOP_MUX_AUD_ENG2].name,
...
void AudDrv_Clk_On(void)
{
	...
	if (Aud_AFE_Clk_cntr == 1) {
		...
		/* TODO: apmixed apll rate is set in preloader, consider set it
		 * using CCF
		 */
	}
EXIT:
...
{
	pr_debug(
	    "%s(), APLL1Counter = %d, APLL2Counter = %d, SampleRate = %d\n",
