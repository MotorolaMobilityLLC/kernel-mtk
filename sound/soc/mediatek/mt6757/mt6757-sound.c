...
static const uint16_t kSideToneCoefficientTable16k[] = {
	0x049C, 0x09E8, 0x09E0, 0x089C, 0xFF54, 0xF488, 0xEAFC,
	0xEBAC, 0xFA40, 0x17AC, 0x3D1C, 0x6028, 0x7538};

static const uint16_t kSideToneCoefficientTable32k[] = {
	0xFE52, 0x0042, 0x00C5, 0x0194, 0x029A, 0x03B7, 0x04BF, 0x057D,
	0x05BE, 0x0555, 0x0426, 0x0230, 0xFF92, 0xFC89, 0xF973, 0xF6C6,
	0xF500, 0xF49D, 0xF603, 0xF970, 0xFEF3, 0x065F, 0x0F4F, 0x1928,
	0x2329, 0x2C80, 0x345E, 0x3A0D, 0x3D08};

/* reg, bit position, bit mask */
static const unsigned int
	mMemIfSampleRate[Soc_Aud_Digital_Block_MEM_I2S + 1][3] = {
...
/* reg, bit position, bit mask */
static const unsigned int
	mMemIfMonoChSelect[Soc_Aud_Digital_Block_MEM_I2S + 1][3] = {
...
/* reg, bit position, bit mask */
static const unsigned int
	mMemDuplicateWrite[Soc_Aud_Digital_Block_MEM_I2S + 1][3] = {
	    [Soc_Aud_Digital_Block_MEM_DL1] = {AFE_REG_UNDEFINED, 0, 0x0},
	    [Soc_Aud_Digital_Block_MEM_DL2] = {AFE_REG_UNDEFINED, 0, 0x0},
...
static const unsigned int
	afe_buffer_regs[Soc_Aud_AFE_IO_Block_NUM_OF_IO_BLOCK][aud_buffer_ctrl_num] = {
		[Soc_Aud_AFE_IO_Block_MEM_DL1] = {AFE_DL1_BASE, AFE_DL1_END,
						  AFE_DL1_CUR}, /* DL1 */
		[Soc_Aud_AFE_IO_Block_MEM_DL2] = {AFE_DL2_BASE, AFE_DL2_END,
						  AFE_DL2_CUR}, /* DL2 */
};
...
void Enable4pin_I2S0_I2S3(unsigned int SampleRate, unsigned int wLenBit)
{
	...
	/*Afe_Set_Reg(AUDIO_TOP_CON1, 0,  0x2);*/ /* Clear I2S_SOFT_Reset  4
						     wire i2s mode*/
...
bool SetChipModemPcmEnable(int modem_index, bool modem_pcm_on)
{
	...
	if (modem_index ==
		...
		if (modem_pcm_on ==
			...
			if (mPcm1AsyncFifo == 0) {
				/* Afe_Set_Reg(AFE_ASRC_CON6, 0x005f188f,
				 * MASK_ALL);   // MT6735 marked */
				Afe_Set_Reg(AFE_ASRC_CON0, 0x86083031,
					    MASK_ALL);
				/* Afe_Set_Reg(AFE_ASRC4_CON6, 0x005f188f,
				 * MASK_ALL);   // MT6735 marked */
				Afe_Set_Reg(AFE_ASRC4_CON0, 0x06003031,
					    MASK_ALL);
			}
			...
			dNeedDisableASM =
			    (Afe_Get_Reg(AFE_ASRC_CON0) & 0x1) ? 1 : 0;
			Afe_Set_Reg(AFE_ASRC_CON0, 0,
				    (1 << 4 | 1 << 5 | dNeedDisableASM));
...
bool set_chip_afe_enable(bool enable)
{
	if (enable) {
		...
		while ((Afe_Get_Reg(AFE_DAC_CON0) & 0x8000000) &&
		       ++retry < 100000)
			udelay(10);
...
bool set_chip_ul2_src_enable(bool enable) { return true; }
...
bool SetI2SASRCConfig(bool bIsUseASRC, unsigned int dToSampleRate)
{
	...
	if (true == bIsUseASRC) {
		...
		SetSampleRate(Soc_Aud_Digital_Block_MEM_I2S,
			      dToSampleRate); /* To target sample rate */
		...
		Afe_Set_Reg(AFE_ASRC_CON20, 0x00036000,
			    AFE_MASK_ALL); /* Calibration setting */
...
bool EnableSideToneFilter(bool stf_on)
{
	...
	unsigned int eSamplingRate2 =
	    (Afe_Get_Reg(AFE_ADDA_UL_SRC_CON0) >> 17) & 0x3;

	pr_debug("+%s(), eSamplingRate = %d, eSamplingRate2=%d\n", __func__,
		 eSamplingRate, eSamplingRate2);
		...
		for (coef_addr = 0; coef_addr < kSideToneHalfTapNum;
			...
			for (try_cnt = 0; try_cnt < 10;
				...
				if (new_write_ready ==
					...
					if (try_cnt == 9) {
						AUDIO_AEE(
		"Enable Side Tone Filter new_write_ready == old_write_ready");
						AudDrv_Clk_Off();
						return false;
					}
				} else {
...
bool SetDLSrc2(unsigned int SampleRate)
{
	...
	if (AfeAddaDLSrc2Con0 == 0 || AfeAddaDLSrc2Con0 == 3) {
		...
		AfeAddaDLSrc2Con0 = (AfeAddaDLSrc2Con0 << 28) | (0x03 << 24) |
				    (0x03 << 11) | (0x01 << 5);
	} else {
		AfeAddaDLSrc2Con0 =
		    (AfeAddaDLSrc2Con0 << 28) | (0x03 << 24) | (0x03 << 11);
	}

	/* SA suggest apply -0.3db to audio/speech path */
	AfeAddaDLSrc2Con0 =
	    AfeAddaDLSrc2Con0 |
	    (0x01 << 1); /* 2013.02.22 for voice mode degrade 0.3 db */
...
bool set_chip_adc_in(unsigned int rate)
{
	...
	Afe_Set_Reg(AFE_ADDA_UL_SRC_CON0,
		    (dVoiceModeSelect << 19) | (dVoiceModeSelect << 17),
		    0x001E0000);
	Afe_Set_Reg(AFE_ADDA_NEWIF_CFG0, 0x03F87201,
		    0xFFFFFFFF); /* up8x txif sat on */
...
bool set_chip_adc2_in(unsigned int rate) { return true; }

bool setChipDmicPath(bool _enable, unsigned int sample_rate) { return true; }
...
bool SetSampleRate(unsigned int Aud_block, unsigned int SampleRate)
{
	...
	default:
		pr_debug(
		    "audio_error: %s(): given Aud_block is not valid!!!!\n",
		    __func__);
		return false;
	}
	return true;
...
bool SetChannels(unsigned int Memory_Interface, unsigned int channel)
{
	...
	case Soc_Aud_Digital_Block_MEM_MOD_DAI:
		...
		break;
	default:
		pr_debug("[%s] SetChannels  Memory_Interface = %d, "
			 "channel = %d, bMono = %d\n", __func__,
...
bool SetMemIfFormatReg(unsigned int InterfaceType, unsigned int eFetchFormat)
{
	switch (InterfaceType) {
	case Soc_Aud_Digital_Block_MEM_DL1: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 16,
			    0x00030000);
		break;
	}
	case Soc_Aud_Digital_Block_MEM_DL1_DATA2: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 12,
			    0x00003000);
		break;
	}
	case Soc_Aud_Digital_Block_MEM_DL2: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 18,
			    0x000c0000);
		break;
	}
	case Soc_Aud_Digital_Block_MEM_I2S: {
		/* Afe_Set_Reg(AFE_DAC_CON1,
		 * mAudioMEMIF[InterfaceType].mSampleRate << 8 , 0x00000f00); */
		pr_debug("Unsupport MEM_I2S");
		break;
	}
	case Soc_Aud_Digital_Block_MEM_AWB: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 20,
			    0x00300000);
		break;
	}
	case Soc_Aud_Digital_Block_MEM_VUL: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 22,
			    0x00C00000);
		break;
	}
	case Soc_Aud_Digital_Block_MEM_VUL_DATA2: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 14,
			    0x0000C000);
		break;
	}
	case Soc_Aud_Digital_Block_MEM_VUL2: {
		Afe_Set_Reg(AFE_MEMIF_MSB, eFetchFormat << 26, 0x0C000000);
		break;
	}
	case Soc_Aud_Digital_Block_MEM_DAI: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 24,
			    0x03000000);
		break;
	}

	case Soc_Aud_Digital_Block_MEM_MOD_DAI: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 26,
			    0x0C000000);
		break;
	}

	case Soc_Aud_Digital_Block_MEM_HDMI: {
		Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, eFetchFormat << 28,
			    0x30000000);
		break;
	}
	default:
...
ssize_t AudDrv_Reg_Dump(char *buffer, int size)
{
	...
	n += scnprintf(buffer + n, size - n, "AFE_ADDA_UL_DL_CON0    = 0x%x\n",
		       Afe_Get_Reg(AFE_ADDA_UL_DL_CON0));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_ADDA_SRC_DEBUG	   = 0x%x\n",
		      Afe_Get_Reg(AFE_ADDA_SRC_DEBUG));
	...
	n += scnprintf(buffer + n, size - n, "AFE_ADDA_NEWIF_CFG1    = 0x%x\n",
		       Afe_Get_Reg(AFE_ADDA_NEWIF_CFG1));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_SIDETONE_DEBUG	   = 0x%x\n",
		      Afe_Get_Reg(AFE_SIDETONE_DEBUG));
	...
	n += scnprintf(buffer + n, size - n, "AFE_SIDETONE_CON0	   = 0x%x\n",
		       Afe_Get_Reg(AFE_SIDETONE_CON0));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_SIDETONE_COEFF	   = 0x%x\n",
		      Afe_Get_Reg(AFE_SIDETONE_COEFF));
	...
	n += scnprintf(buffer + n, size - n, "AFE_IRQ_MCU_CON	   = 0x%x\n",
		       Afe_Get_Reg(AFE_IRQ_MCU_CON));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_IRQ_MCU_STATUS	   = 0x%x\n",
		      Afe_Get_Reg(AFE_IRQ_MCU_STATUS));
	...
	n += scnprintf(buffer + n, size - n,
		       ...
		       Afe_Get_Reg(AFE_CONN9));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN10		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN10));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN11		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN11));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN12		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN12));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN13		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN13));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN14		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN14));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN15		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN15));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN16		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN16));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN17		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN17));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN18		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN18));
	n += scnprintf(buffer + n, size - n,
		       "AFE_CONN19			   = 0x%x\n",
		       Afe_Get_Reg(AFE_CONN19));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN20		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN20));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN21		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN21));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN22		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN22));
	n += scnprintf(buffer + n, size - n,
		       "AFE_CONN23			   = 0x%x\n",
		       Afe_Get_Reg(AFE_CONN23));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN24		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN24));
	n += scnprintf(buffer + n, size - n,
		       "AFE_CONN_RS			   = 0x%x\n",
		       Afe_Get_Reg(AFE_CONN_RS));
	...
	n += scnprintf(buffer + n, size - n,
		       ...
		       Afe_Get_Reg(AFE_CONN25));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN26		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN26));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN27		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN27));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN28		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN28));
	n += scnprintf(buffer + n, size - n,
		       "AFE_CONN29			   = 0x%x\n",
		       Afe_Get_Reg(AFE_CONN29));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN30		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN30));
	n += scnprintf(buffer + n, size - n,
		       "AFE_CONN31			   = 0x%x\n",
		       Afe_Get_Reg(AFE_CONN31));
	n +=
	    scnprintf(buffer + n, size - n, "AFE_CONN32		   = 0x%x\n",
		      Afe_Get_Reg(AFE_CONN32));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CON0		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CON0));
	...
	n += scnprintf(buffer + n, size - n,
		       ...
		       Afe_Get_Reg(AFE_ASRC4_CON9));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC4_CON10		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC4_CON10));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC4_CON11		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC4_CON11));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC4_CON12		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC4_CON12));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC4_CON13		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC4_CON13));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC4_CON14		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC4_CON14));
	n += scnprintf(buffer + n, size - n,
		       "AFE_CONN_RS1		   = 0x%x\n",
		       Afe_Get_Reg(AFE_CONN_RS1));
	...
	n += scnprintf(buffer + n, size - n,
		       ...
		       Afe_Get_Reg(AFE_CONNSYS_I2S_CON));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON0		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON0));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON1		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON1));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON2		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON2));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON3		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON3));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON4		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON4));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON5		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON5));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON6		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON6));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON7		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON7));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON8		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON8));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON9		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON9));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON10		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON10));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON11		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON11));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON13		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON13));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON14		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON14));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON15		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON15));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON16		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON16));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON17		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON17));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON18		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON18));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON19		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON19));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON20		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON20));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON21		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON21));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON23		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON23));
	n += scnprintf(buffer + n, size - n,
		       "AFE_ASRC_CONNSYS_CON24		   = 0x%x\n",
		       Afe_Get_Reg(AFE_ASRC_CONNSYS_CON24));
...
int SetFmI2sInEnable(bool enable) { return setConnsysI2SInEnable(enable); }
...
int SetFmI2sAsrcEnable(bool enable) { return setConnsysI2SEnable(enable); }
...
bool SetAncRecordReg(unsigned int value, unsigned int mask) { return false; }
				    ...
				    enum aud_buffer_ctrl_info buffer_ctrl)
{
	if (!afe_buffer_regs[memif_type][buffer_ctrl])
		pr_debug(
		    "%s, invalid afe_buffer_regs, memif: %d, buffer_ctrl: %d",
...
int set_sram_mode(enum audio_sram_mode sram_mode) { return 0; }
...
static bool platform_set_dpd_module(bool enable)
{
	...
	pr_debug(
	    "%s, a2_lch, a3_lch = 0x%x, 0x%x; a2_rch, a3_rch = 0x%x, 0x%x\n",
	    __func__, dpd_param.a2_lch, dpd_param.a3_lch, dpd_param.a2_rch,
	    dpd_param.a3_rch);
...
static bool platform_handle_suspend(bool suspend)
{
	...
	pr_debug("%s(), segment = %d, ddr_type = %d", __func__, segment,
		 ddr_type);
