...
static const unsigned int mI2SAPLLDivSelect[Soc_Aud_I2S_CLKDIV_NUMBER] = {
	AUDIO_APLL1_DIV0,  AUDIO_APLL2_DIV0,  AUDIO_APLL12_DIV1,
	AUDIO_APLL12_DIV2,
	AUDIO_APLL12_DIV3, AUDIO_APLL12_DIV4, AUDIO_APLL12_DIVB};
...
{
	/*volatile unsigned int *AFE_Register = (volatile unsigned int
	 * *)Get_Afe_Powertop_Pointer();*/
...
void AudDrv_AUDINTBUS_Sel(int parentidx)
{
	...
	/* pr_debug("+AudDrv AUDINTBUS Sel, parentidx = %d\n",
	 * parentidx);
	 */
	if (parentidx == 1) {
		if (aud_clks[CLOCK_MUX_AUDIOINTBUS].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s [CCF]Aud enable_clock enable_clock CLOCK_MUX_AUDIOINTBUS fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s [CCF]clk_prepare error Aud enable_clock CLOCK_MUX_AUDIOINTBUS fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
...
void AudDrv_AUD_Sel(int parentidx) { /* Keep for extension */ }
...
void AudDrv_Clk_On(void)
{
	...
	pr_debug("+AudDrv Clk On, Aud_AFE_Clk_cntr:%d\n", Aud_AFE_Clk_cntr);
	spin_lock_irqsave(&auddrv_Clk_lock, flags);
	Aud_AFE_Clk_cntr++;
	if (Aud_AFE_Clk_cntr == 1) {
		 ...
		 *		if
		 *(aud_clks[CLOCK_INFRA_SYS_AUDIO_26M].clk_prepare) {
		 */
		/*
		 * Clock enable order:
		 * 26M -> Infra -> Int bus -> AFE -> DAC -> DAC pre-distortion ->
		 * MTCMOS
		 */
		if (aud_clks[CLOCK_PERI_AUDIO26M].clk_prepare) {
			...
			/* MT6755 CLOCK_PERI_AUDIO26M
			 * ret =
			 * clk_enable(aud_clks[CLOCK_INFRA_SYS_AUDIO_26M].clock);
			 */
			...
			pr_debug("%s [CCF]clk_prepare error Aud enable_clock CLOCK_INFRA_SYS_AUDIO_26M fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_INFRA_SYS_AUDIO].clk_prepare) {
			...
			pr_debug("%s [CCF]clk_prepare error Aud enable_clock MT_CG_INFRA_AUDIO fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_MUX_AUDIOINTBUS].clk_prepare) {
			ret = clk_enable(aud_clks[CLOCK_MUX_AUDIOINTBUS].clock);
			if (ret) {
				pr_debug("%s [CCF]Aud enable_clock CLOCK_MUX_AUDIOINTBUS fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			pr_debug("AudDrv_Clk On: enable CLOCK_MUX_AUDIOINTBUS\n");
		} else {
			pr_debug("%s [CCF]clk_status error Aud enable_clock CLOCK_MUX_AUDIOINTBUS fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_AFE].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s [CCF]Aud enable_clock %s fail\n",
					 __func__, aud_clks[CLOCK_AFE].name);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s [CCF]clk_prepare error Aud enable_clock MT_CG_AUDIO_AFE fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_DAC].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s [CCF]Aud enable clock MT_CG_AUDIO_DAC fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s [CCF]clk_status error Aud enable_clock MT_CG_AUDIO_DAC fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_DAC_PREDIS].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s [CCF]Aud enable clock MT_CG_AUDIO_DAC_PREDIS fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s [CCF]clk status error Aud enable_clock MT_CG_AUDIO_DAC_PREDIS fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_SCP_SYS_AUD].clk_status) {
			...
			if (ret) {
				pr_debug(
				    "%s [CCF]Aud clk_prepare_enable %s fail\n",
				    __func__, aud_clks[CLOCK_SCP_SYS_AUD].name);
				AUDIO_AEE("");
		...
		if (aud_clks[CLOCK_APMIXED_APLL1_CK].clk_prepare) {
			 ...
			 */
			ret =
			    clk_set_rate(aud_clks[CLOCK_APMIXED_APLL1_CK].clock,
					 180633600 / 2);
			if (ret) {
				pr_debug(
				    "%s clk_set_rate %s-180633600/2 fail %d\n",
				    __func__,
				    aud_clks[CLOCK_APMIXED_APLL1_CK].name, ret);
				AUDIO_AEE("");
				goto EXIT_SKIP_UNLOCK;
			}

			ret = clk_set_rate(
			    aud_clks[CLOCK_APMIXED_APLL1_CK].clock, 180633600);
			if (ret) {
				pr_debug(
				    "%s clk_set_rate %s-180633600 fail %d\n",
				    __func__,
				    aud_clks[CLOCK_APMIXED_APLL1_CK].name, ret);
				AUDIO_AEE("");
				goto EXIT_SKIP_UNLOCK;
		...
		if (aud_clks[CLOCK_APMIXED_APLL2_CK].clk_prepare) {
			 ...
			 */
			ret =
			    clk_set_rate(aud_clks[CLOCK_APMIXED_APLL2_CK].clock,
					 196608000 / 2);
			if (ret) {
				pr_debug(
				    "%s clk_set_rate %s-196607998/2 fail %d\n",
				    __func__,
				    aud_clks[CLOCK_APMIXED_APLL2_CK].name, ret);
				AUDIO_AEE("");
				goto EXIT_SKIP_UNLOCK;
			}

			ret = clk_set_rate(
			    aud_clks[CLOCK_APMIXED_APLL2_CK].clock, 196608000);
			if (ret) {
				pr_debug(
				    "%s clk_set_rate %s-196607998 fail %d\n",
				    __func__,
				    aud_clks[CLOCK_APMIXED_APLL2_CK].name, ret);
				AUDIO_AEE("");
				goto EXIT_SKIP_UNLOCK;
		...
		if (ret) {
			pr_debug("%s clk_set_parent %s-%s fail %d\n", __func__,
				 aud_clks[CLOCK_TOP_AUD_MUX1].name,
		...
		if (ret) {
			pr_debug("%s clk_set_parent %s-%s fail %d\n", __func__,
				 aud_clks[CLOCK_TOP_AUD_MUX2].name,
...
void AudDrv_Clk_Off(void)
{
	...
	pr_debug("+ AudDrv Clk_Off, Aud_AFE_Clk_cntr:%d\n", Aud_AFE_Clk_cntr);
	spin_lock_irqsave(&auddrv_Clk_lock, flags);
...
/* Disable AFE clock */
		...
		/*
		 * Clock disable order:
		 * MTCMOS -> DAC pre-distortion -> DAC ->AFE -> Int bus -> Infra
		 * -> 26M
		 */
...
void AudDrv_ANA_Clk_Off(void)
{
	...
	Aud_ANA_Clk_cntr--;
	if (Aud_ANA_Clk_cntr == 0) {
		pr_debug("+%s disable_clock Ana clk(%x)\n", __func__,
			 Aud_ANA_Clk_cntr);
/* Disable ADC clock */
#ifdef PM_MANAGER_API
...
/* Afe_Set_Reg(AUDIO_TOP_CON0, 0 << 24 , 1 << 24); */
		...
		if (aud_clks[CLOCK_ADC].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s [CCF]Aud enable_clock enable_clock ADC fail",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s [CCF]clk_prepare error Aud enable_clock ADC fail",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
...
{
	/* pr_debug("+AudDrv_ADC Clk_Off, Aud_ADC_Clk_cntr:%d\n",
	 * Aud_ADC_Clk_cntr);
	 */
	mutex_lock(&auddrv_pmic_mutex);
	Aud_ADC_Clk_cntr--;
	if (Aud_ADC_Clk_cntr == 0) {
		pr_debug("+AudDrv ADC_Clk_On disable_clock ADC clk(%x)\n",
			 Aud_ADC_Clk_cntr);
#ifdef PM_MANAGER_API
		if (aud_clks[CLOCK_ADC].clk_prepare)
	...
	if (Aud_ADC_Clk_cntr < 0) {
		pr_debug(" AudDrv ADC_Clk_Off, Aud_ADC_Clk_cntr<0 (%d)\n",
			 Aud_ADC_Clk_cntr);
		Aud_ADC_Clk_cntr = 0;
	}
	mutex_unlock(&auddrv_pmic_mutex);
...
void AudDrv_ADC_Hires_Clk_On(void) { /* No Hires Clk in mt6757 */ }

void AudDrv_ADC_Hires_Clk_Off(void) { /* No Hires Clk in mt6757 */ }
...
void AudDrv_ADC2_Hires_Clk_On(void) { /* No Hires Clk in mt6757 */ }

void AudDrv_ADC2_Hires_Clk_Off(void) { /* No Hires Clk in mt6757 */ }
...
void AudDrv_APLL22M_Clk_On(void)
{
	...
	if (Aud_APLL22M_Clk_cntr == 0) {
#ifdef PM_MANAGER_API
		/* pdn_aud_1 => power down hf_faud_1_ck, hf_faud_1_ck is mux of
		 * 26M and APLL1_CK
		 */
		/* pdn_aud_2 => power down hf_faud_2_ck, hf_faud_2_ck is mux of
		 * 26M and APLL2_CK (D1 is WHPLL)
		 */

		if (aud_clks[CLOCK_TOP_AD_APLL1_CK].clk_prepare) {
			ret = clk_enable(aud_clks[CLOCK_TOP_AD_APLL1_CK].clock);
			if (ret) {
				pr_debug("%s, [CCF] clk_enable CLOCK_TOP_AD_APLL1_CK fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s, [CCF] clk_prepare CLOCK_TOP_AD_APLL1_CK fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_TOP_AUD_MUX1].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s, [CCF] clk_enable CLOCK_TOP_AUD_MUX1 fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			pr_debug(
			    "%s, [CCF] clk_prepare CLOCK_TOP_AUD_MUX1 fail\n",
			    __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_APLL22M].clk_prepare) {
			...
			if (ret) {
				pr_debug(
				    "%s, [CCF] clk_enable CLOCK_APLL22M fail\n",
				    __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s, [CCF] clk_prepare CLOCK_APLL22M fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
...
void AudDrv_APLL22M_Clk_Off(void)
{
	...
	if (Aud_APLL22M_Clk_cntr == 0) {
		...
		if (aud_clks[CLOCK_TOP_AD_APLL1_CK].clk_prepare) {
			...
			pr_debug(
			    "%s, [CCF] clk_disable CLOCK_TOP_AD_APLL1_CK\n",
			    __func__);
		} else {
			pr_debug("%s, [CCF] clk_prepare CLOCK_TOP_AD_APLL1_CK fail\n",
				 __func__);
...
void AudDrv_APLL24M_Clk_On(void)
{
	...
	if (Aud_APLL24M_Clk_cntr == 0) {
		...
		if (aud_clks[CLOCK_TOP_AD_APLL2_CK].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s, [CCF] clk_enable CLOCK_TOP_AD_APLL2_CK fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s, [CCF] clk_prepare CLOCK_TOP_AD_APLL2_CK fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
		...
		if (aud_clks[CLOCK_TOP_AUD_MUX2].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s, [CCF] clk_enable CLOCK_TOP_AUD_MUX2 fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		...
		if (aud_clks[CLOCK_APLL24M].clk_prepare) {
			...
			if (ret) {
				pr_debug(
				    "%s, [CCF] clk_enable CLOCK_APLL24M fail\n",
				    __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s, [CCF] clk_prepare CLOCK_APLL24M fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
...
void AudDrv_APLL24M_Clk_Off(void)
{
	...
	if (Aud_APLL24M_Clk_cntr == 0) {
		...
		if (aud_clks[CLOCK_TOP_AD_APLL2_CK].clk_prepare) {
			...
			pr_debug(
			    "%s, [CCF] clk_disable CLOCK_TOP_AD_APLL2_CK\n",
			    __func__);
		} else {
			pr_debug("%s, [CCF] clk_prepare CLOCK_TOP_AD_APLL2_CK "
				 "fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
...
void AudDrv_I2S_Clk_Off(void)
{
	...
	if (Aud_I2S_Clk_cntr == 0) {
		...
		pr_debug("!! AudDrv I2S_Clk_Off, Aud_I2S_Clk_cntr<0 (%d)\n",
			 Aud_I2S_Clk_cntr);
		AUDIO_ASSERT(true);
		Aud_I2S_Clk_cntr = 0;
	}
 ...
 ****************************************************************************
 */
void aud_top_con_pdn_tdm_ck(bool _pdn) { /* mt6757 no TDM */ }

void AudDrv_TDM_Clk_On(void) { /* mt6757 no TDM */ }
EXPORT_SYMBOL(AudDrv_TDM_Clk_On);

void AudDrv_TDM_Clk_Off(void) { /* mt6757 no TDM */ }
...
{
	/* pr_debug("+AudDrv_Core_Clk_On, Aud_Core_Clk_cntr:%d\n",
	 * Aud_Core_Clk_cntr);
	 */
	unsigned long flags = 0;
	int ret = 0;
	...
	if (Aud_Core_Clk_cntr == 0) {
		...
		if (aud_clks[CLOCK_AFE].clk_prepare) {
			ret = clk_enable(aud_clks[CLOCK_AFE].clock);
			if (ret) {
				pr_debug("%s [CCF]Aud enable_clock enable_clock aud_afe_clk fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s [CCF]clk_prepare error Aud enable_clock aud_afe_clk fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
...
void AudDrv_APLL1Tuner_Clk_On(void)
{
	...
	if (Aud_APLL1_Tuner_cntr == 0) {
		/* pr_debug("+AudDrv APLLTuner_Clk_On,
		 *Aud_APLL1_Tuner_cntr:%d\n",
		...
		if (aud_clks[CLOCK_APLL1_TUNER].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s [CCF]Aud enable_clock enable_clock aud_apll1_tuner_clk fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s [CCF]clk_prepare error Aud enable_clock aud_apll1_tuner_clk fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
...
/*Afe_Set_Reg(AFE_APLL1_TUNER_CFG, 0x00000033, 0x1 << 19);*/
	...
	/* handle for clock error */
		...
		pr_debug("!! AudDrv APLLTuner_Clk_Off, Aud_APLL1_Tuner_cntr<0 (%d)\n",
			 Aud_APLL1_Tuner_cntr);
...
void AudDrv_APLL2Tuner_Clk_On(void)
{
	...
	if (Aud_APLL2_Tuner_cntr == 0) {
		/* pr_debug("+Aud_APLL2_Tuner_cntr, Aud_APLL2_Tuner_cntr:%d\n",
		...
		if (aud_clks[CLOCK_APLL2_TUNER].clk_prepare) {
			...
			if (ret) {
				pr_debug("%s [CCF]Aud enable_clock enable_clock aud_apll2_tuner_clk fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		} else {
			pr_debug("%s [CCF]clk_prepare error Aud enable_clock aud_apll2_tuner_clk fail\n",
				 __func__);
			AUDIO_AEE("");
			goto EXIT;
		}
...
void AudDrv_HDMI_Clk_On(void)
{
	...
	/* pr_debug("+AudDrv HDMI_Clk_On, Aud_I2S_Clk_cntr:%d\n",
	 * Aud_HDMI_Clk_cntr);
	 */
	...
	if (Aud_Core_Clk_cntr > 0) {
		...
		if (Aud_APLL22M_Clk_cntr > 0) {
			...
			if (aud_clks[CLOCK_TOP_AUD_MUX1].clk_prepare) {
				...
				pr_debug("%s [CCF]clk_prepare error clk_disable CLOCK_TOP_AUD_MUX1 fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			if (aud_clks[CLOCK_TOP_AD_APLL1_CK].clk_prepare) {
				...
				pr_debug("%s [CCF]clk_prepare error CLOCK_TOP_AD_APLL1_CK fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		...
		if (Aud_APLL24M_Clk_cntr > 0) {
			...
			if (aud_clks[CLOCK_TOP_AUD_MUX2].clk_prepare) {
				...
				pr_debug("%s [CCF]clk_prepare error clk_disable CLOCK_TOP_AUD_MUX2 fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			if (aud_clks[CLOCK_TOP_AD_APLL2_CK].clk_prepare) {
				...
				pr_debug("%s [CCF]clk_prepare error CLOCK_TOP_AD_APLL2_CK fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
	...
	if (Aud_Core_Clk_cntr > 0) {
		...
		if (Aud_I2S_Clk_cntr > 0) {
			if (aud_clks[CLOCK_I2S].clk_prepare) {
				...
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock enable_clock aud_i2s_clk fail\n",
						 __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error Aud enable_clock aud_i2s_clk fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		...
		if (Aud_ADC_Clk_cntr > 0) {
			if (aud_clks[CLOCK_ADC].clk_prepare) {
				ret = clk_enable(aud_clks[CLOCK_ADC].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock aud_adc_clk fail\n",
						 __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error aud_adc_clk fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		...
		if (Aud_APLL22M_Clk_cntr > 0) {
			...
			if (aud_clks[CLOCK_TOP_AD_APLL1_CK].clk_prepare) {
				ret = clk_enable(
				    aud_clks[CLOCK_TOP_AD_APLL1_CK].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock CLOCK_TOP_AD_APLL1_CK fail\n",
						 __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error Aud CLOCK_TOP_AD_APLL1_CK fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			if (aud_clks[CLOCK_TOP_AUD_MUX1].clk_prepare) {
				ret = clk_enable(
				    aud_clks[CLOCK_TOP_AUD_MUX1].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock enable_clock CLOCK_TOP_AUD_MUX1 fail\n",
						 __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error Aud enable_clock CLOCK_TOP_AUD_MUX1 fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			if (aud_clks[CLOCK_APMIXED_APLL1_CK].clk_prepare) {
				...
				ret = clk_set_rate(
				    aud_clks[CLOCK_APMIXED_APLL1_CK].clock,
				    180633600);
				if (ret) {
					pr_debug(
					    "%s clk_set_rate %s-180633600 fail %d\n",
					    __func__,
					    aud_clks[CLOCK_APMIXED_APLL1_CK]
						.name,
					    ret);
					AUDIO_AEE("");
					goto EXIT;
			...
			if (aud_clks[CLOCK_APLL22M].clk_prepare) {
				ret = clk_enable(aud_clks[CLOCK_APLL22M].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock enable_clock aud_apll22m_clk fail\n",
						 __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error Aud enable_clock aud_apll22m_clk fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			if (aud_clks[CLOCK_APLL1_TUNER].clk_prepare) {
				ret = clk_enable(
				    aud_clks[CLOCK_APLL1_TUNER].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock enable_clock aud_apll1_tuner_clk fail\n",
						 __func__);
					AUDIO_AEE("");
		...
		if (Aud_APLL24M_Clk_cntr > 0) {
			...
			if (aud_clks[CLOCK_TOP_AD_APLL2_CK].clk_prepare) {
				ret = clk_enable(
				    aud_clks[CLOCK_TOP_AD_APLL2_CK].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock CLOCK_TOP_AD_APLL2_CK fail\n",
						 __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error Aud CLOCK_TOP_AD_APLL2_CK fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			if (aud_clks[CLOCK_TOP_AUD_MUX2].clk_prepare) {
				ret = clk_enable(
				    aud_clks[CLOCK_TOP_AUD_MUX2].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock enable_clock CLOCK_TOP_AUD_MUX2 fail\n",
						 __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error Aud enable_clock CLOCK_TOP_AUD_MUX2 fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			if (aud_clks[CLOCK_APMIXED_APLL2_CK].clk_prepare) {
				...
				ret = clk_set_rate(
				    aud_clks[CLOCK_APMIXED_APLL2_CK].clock,
				    196607998);
				if (ret) {
					pr_debug("%s clk_set_rate %s-196607998 fail %d\n",
					    __func__,
					    aud_clks[CLOCK_APMIXED_APLL2_CK]
						.name,
					    ret);
					AUDIO_AEE("");
					goto EXIT;
			...
			if (aud_clks[CLOCK_APLL24M].clk_prepare) {
				ret = clk_enable(aud_clks[CLOCK_APLL24M].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock enable_clock aud_apll24m_clk fail\n",
						 __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error Aud enable_clock aud_apll24m_clk fail\n",
					 __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
			...
			if (aud_clks[CLOCK_APLL2_TUNER].clk_prepare) {
				ret = clk_enable(
				    aud_clks[CLOCK_APLL2_TUNER].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock enable_clock aud_apll2_tuner_clk fail\n",
						 __func__);
					AUDIO_AEE("");
		...
		if (Aud_ADC_Clk_cntr > 0) {
			if (aud_clks[CLOCK_ADC].clk_prepare) {
				ret = clk_enable(aud_clks[CLOCK_ADC].clock);
				if (ret) {
					pr_debug("%s [CCF]Aud enable_clock enable_clock ADC fail", __func__);
					AUDIO_AEE("");
					goto EXIT;
				}
			} else {
				pr_debug("%s [CCF]clk_prepare error Aud enable_clock ADC fail", __func__);
				AUDIO_AEE("");
				goto EXIT;
			}
		}
...
void AudDrv_ANC_Clk_On(void) { /* MT6755 ANC? */ }

void AudDrv_ANC_Clk_Off(void) { /* MT6755 ANC? */ }
...
void EnableALLbySampleRate(unsigned int SampleRate)
{
	...
	default:
		pr_debug(
		    "[AudioWarn] GetApllbySampleRate(%d) = %d not recognized\n",
...
void EnableApll1(bool bEnable)
{
	...
	if (bEnable) {
		if (Aud_APLL_DIV_APLL1_cntr == 0) {
			...
			/* TODO: KC: this should be unnecessary //
			 * Afe_Set_Reg(CLK_AUDDIV_0, 1 << 1, 1 << 1);
			 */

			/* set appl1_ck_div0 = 7,  f_faud_engen1_ck = clock
			 * source / 8
			 */
			...
			/* TODO: KC: this should be unnecessary //
			 * Afe_Set_Reg(CLK_AUDDIV_0, 0x0, 0x2);
			 */
		}
...
void EnableApll2(bool bEnable)
{
	...
	if (bEnable) {
		if (Aud_APLL_DIV_APLL2_cntr == 0) {
			...
			/* set appl2_ck_div0 = 7,  f_faud_engen2_ck = clock
			 * source / 8
			 */
			/* 196.608 / 8 = 24.576MHz */
		...
		unsigned int Channels, unsigned int Wlength)
{
	...
	pr_debug(
	    "%s MckDiv = %dv SampleRate = %d  Channels = %d Wlength = %d\n",
