...
{
	hdmi_stream_format->mI2Snum = Soc_Aud_I2S4;
	hdmi_stream_format->mI2S_MCKDIV = Soc_Aud_I2S4_MCKDIV;
	hdmi_stream_format->mI2S_BCKDIV = Soc_Aud_I2S4_BCKDIV;
}
...
void mtk_Hdmi_Configuration_Set(void *hdmi_stream_format, struct snd_pcm_substream *substream, int displaytype)
{
	...
	pr_warn("[%s] format=%d, displaytype=%d\n", __func__, runtime->format, displaytype);

	hdmi_stream_format->mHDMI_DisplayType = displaytype;
	hdmi_stream_format->mHDMI_Samplerate = runtime->rate;
	hdmi_stream_format->mHDMI_Channels = runtime->channels;
	...
	if (runtime->format == SNDRV_PCM_FORMAT_S32_LE
		...
		hdmi_stream_format->mMemIfFetchFormatPerSample = AFE_WLEN_32_BIT_ALIGN_8BIT_0_24BIT_DATA;
		hdmi_stream_format->mHDMI_Data_Lens = Soc_Aud_I2S_WLEN_WLEN_32BITS;
		hdmi_stream_format->mTDM_Data_Lens = Soc_Aud_I2S_WLEN_WLEN_32BITS;
		hdmi_stream_format->mClock_Data_Lens = Soc_Aud_I2S_WLEN_WLEN_32BITS;
		hdmi_stream_format->mTDM_LRCK = ((Soc_Aud_I2S_WLEN_WLEN_32BITS + 1) * 16) - 1;
	} else {
		hdmi_stream_format->mMemIfFetchFormatPerSample = AFE_WLEN_16_BIT;
		hdmi_stream_format->mHDMI_Data_Lens = Soc_Aud_I2S_WLEN_WLEN_16BITS;
		hdmi_stream_format->mTDM_Data_Lens = Soc_Aud_I2S_WLEN_WLEN_16BITS;
		hdmi_stream_format->mClock_Data_Lens = Soc_Aud_I2S_WLEN_WLEN_16BITS;
		hdmi_stream_format->mTDM_LRCK = ((Soc_Aud_I2S_WLEN_WLEN_16BITS + 1) * 16) - 1;
	}



	if (displaytype == HDMI_DISPLAY_SILMPORT) {	/* ANX7805 only I2s  */
		hdmi_stream_format->mHDMI_Channels = _ANX7805_SLIMPORT_CHANNEL;
		hdmi_stream_format->mClock_Data_Lens = Soc_Aud_I2S_WLEN_WLEN_32BITS;
		/* overwrite 16 bits settings for ANX7805 */
		hdmi_stream_format->mTDM_LRCK = ((Soc_Aud_I2S_WLEN_WLEN_32BITS + 1) * 16) - 1;
		/* overwrite 16 bits settings for ANX7805 */
		hdmi_stream_format->msDATA_Channels = _ANX7805_SLIMPORT_CHANNEL;
		hdmi_stream_format->mSdata0 = true;
		hdmi_stream_format->mSdata1 = true;
		hdmi_stream_format->mSdata2 = true;
		hdmi_stream_format->mSdata3 = true;
	} else {
		hdmi_stream_format->msDATA_Channels = runtime->channels;
		hdmi_stream_format->mSdata0 = true;
		hdmi_stream_format->mSdata1 = false;
		hdmi_stream_format->mSdata2 = false;
		hdmi_stream_format->mSdata3 = false;
	}
}
...
void mtk_Hdmi_Set_Interconnection(void *hdmi_stream_format)
{
	...
	if (hdmi_stream_format->mHDMI_DisplayType == HDMI_DISPLAY_SILMPORT) {
