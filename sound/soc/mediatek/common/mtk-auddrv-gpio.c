...
static DEFINE_MUTEX(gpio_aud_dat_mosi_mutex);
...
static int set_aud_dat_mosi_l(bool _enable)
{
	if (_enable)
...
static int set_aud_dat_mosi(bool _enable)
{
	...
	mutex_lock(&gpio_aud_dat_mosi_mutex);
	ret = set_aud_dat_mosi_l(_enable);
	mutex_unlock(&gpio_aud_dat_mosi_mutex);
	return ret;
}
