...
#include "../../codecs/fs15xx/fsm_public.h"
#endif
				       ...
				       int event)
{
	...
	dev_info(card->dev, "%s(), Auido pa_type = %d(-1 = NONE; 0 = FS; 1 = AW)\n", __func__, pa_type);
...
static ssize_t audio_show_pa_type(struct device* cd,struct device_attribute *attr, char* buf)
{
	ssize_t ret = 0;

	if (pa_type == 0) {
		sprintf(buf, "pa_type:FS15XX\n");
	} else if (pa_type == 1) {
		sprintf(buf, "pa_type:AW87XXX\n");
	} else {
		sprintf(buf, "pa_type:NONE\n");
	}
...
static DEVICE_ATTR(pa_type, S_IRUGO, audio_show_pa_type, NULL);

static int audio_create_pa_type_sysfs(struct platform_device *client)
{
	struct device *dev = &(client->dev);
	int err = 0;

	err = device_create_file(dev, &dev_attr_pa_type);
	if (err) {
		device_remove_file(dev, &dev_attr_pa_type);
		return err;
	}
...
static int mt6768_mt6358_dev_probe(struct platform_device *pdev)
{
	...
	ret = audio_create_pa_type_sysfs(pdev);
	if (ret) {
		dev_err(&pdev->dev, "%s create_sysfs_pa_type fail %d\n",
				__func__, ret);
	}
