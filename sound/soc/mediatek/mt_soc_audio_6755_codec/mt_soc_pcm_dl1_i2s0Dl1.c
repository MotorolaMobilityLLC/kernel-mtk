...
static int Audio_I2S0dl1_hdoutput_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
   ...
   /* lenovo-sw zhangrc2 resolve sometimes silence when use mic recording begin */
 //   unsigned long flags;
   /* lenovo-sw zhangrc2 resolve sometimes silence when use mic recording end */
    printk("%s()\n", __func__);
   /* lenovo-sw zhangrc2 resolve sometimes silence when use mic recording begin */
//   spin_lock_irqsave(&auddrv_I2S0dl1_hdoutput_lock, flags);
   /* lenovo-sw zhangrc2 resolve sometimes silence when use mic recording end */
    if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(I2S0dl1_HD_output))
    {
        printk("return -EINVAL\n");
    /* lenovo-sw zhangrc2 resolve sometimes silence when use mic recording begin */
	//    spin_unlock_irqrestore(&auddrv_I2S0dl1_hdoutput_lock,flags);
    /* lenovo-sw zhangrc2 resolve sometimes silence when use mic recording end */
        return -EINVAL;
    }
...
static int mtk_pcm_I2S0dl1_prepare(struct snd_pcm_substream *substream)
{
	...
	if (mPrepareDone == false) {
		...
		if (mI2S0dl1_hdoutput_control == true) {
			...
			/* here to open APLL */
         /* lenovo-sw zhangrc2 resolve sometimes silence when use mic recording begin */
		//	EnableALLbySampleRate(runtime->rate);       //zhouwl 
         /* lenovo-sw zhangrc2 resolve sometimes silence when use mic recording end */
