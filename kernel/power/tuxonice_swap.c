...
static void enable_swapfile(void)
{
	...
	if (swapfilename[0]) {
		...
		if (activateswapresult && activateswapresult != -EBUSY)
			pr_err("TuxOnIce: /sys/power/tuxonice/swap/swapfile (%s) could not be turned on (error %d)",
				      ...
				      unsigned long start, unsigned long end)
{
	if (test_action_state(TOI_TEST_BIO))
		toi_message(TOI_IO, TOI_VERBOSE, 0, "Adding extent %lu-%lu to chain %p.", start << chain->bmap_shift,
...
static int get_main_pool_phys_params(struct toi_bdev_info *chain)
{
	...
	toi_message(TOI_IO, TOI_VERBOSE, 0, "get main pool phys params for chain %d.", chain->allocator_index);
	...
	toi_extent_for_each(&chain->allocations, extentpointer, address) {
		...
		if (add_blocks_to_extent_chain(chain, extent_min, extent_max)) {
			pr_err("Out of memory while making block " "chains.\n");
			return -ENOMEM;
		}
...
static int toi_swap_allocate_storage(struct toi_bdev_info *chain, unsigned long request)
{
	...
	toi_message(TOI_IO, TOI_VERBOSE, 0, "  Swap allocate storage: Asked to allocate %lu pages from device %d.",
		request, chain->allocator_index);
	...
	while (gotten < request) {
		...
		if (start.val) {
			...
			if (toi_add_to_extent_chain(&chain->allocations, start.val, end.val)) {
				pr_warn("Failed to allocate extent for %lu-%lu.\n", start.val, end.val);
...
static int toi_swap_register_storage(void)
{
	...
	for (i = 0; i < MAX_SWAPFILES; i++) {
		...
		if (!devinfo) {
			pr_warn("Failed to allocate devinfo struct for swap " "device %d.\n", i);
			return -ENOMEM;
		}
		...
		if (!fs_info)
			pr_warn("fs_info from block dev returned %d.\n", result);
		...
		toi_message(TOI_IO, TOI_VERBOSE, 0, "Registering swap storage: Device %d (%lx), prio %d.", i,
	...
	for (i = 0; i < MAX_SWAPFILES; i++) {
		...
		if (S_ISBLK(si->swap_file->f_mapping->host->i_mode)) {
			...
			if (!zone) {
				...
				len += sprintf(output + len,
					       "For swapfile `%s`, use resume=swap:/dev/%s:0x%x.\n",
					       path_page,
