...
#if 1
#define PRINTK(a, b...) pr_debug(a, ##b)
#else
		    ...
		    struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (h->frag_off & htons(IP_OFFSET | IP_MF)) {
		if (net_ratelimit())
			pr_err("TuxOnIce: Ignoring fragmented " "cluster message.\n");
		goto drop;
	}
	...
	for (index = 0; index < num_local_nodes; index++) {
		...
		if (result == -1) {
			pr_warn("Failed to add new cluster member "
			       NIPQUAD_FMT ".\n", NIPQUAD(addr));
			goto drop_unlock;
		}
		...
		case MSG_IMAGE:
			...
			if (new_message != old_message)
				pr_err("Setting whether I can resume to %d.\n", new_message);
			break;
		case MSG_IO:
		...
		default:
			if (net_ratelimit())
				pr_err("Unrecognised TuxOnIce cluster message %d from " NIPQUAD_FMT ".",
				       b->message, NIPQUAD(addr));
		};

		if (old_message != new_message) {
			node_array[index].current_message = new_message;
			pr_warn(">>> Sending new message for node " "%d.\n", index);
			toi_send_if(new_message, index);
		} else if (!ack) {
			pr_warn(">>> Resending message for node %d.\n", index);
			toi_send_if(new_message, index);
		}
drop_unlock:
...
static int peers_not_in_message(int index, int message, int precise)
{
	...
	list_for_each_entry(this, &node_array[index].member_list, list) {
		...
		PRINTK("Peer %d.%d.%d.%d sending %s. Seeking %s.",
...
static void cluster_startup(void)
{
	...
	for (i = 0; i < num_local_nodes; i++) {
		...
		if (IS_ERR(p)) {
			pr_err("Failed to start ktoiclusterd.\n");
			return;
		}
	...
	pr_warn("Continuing. I %shave an image. Peers with image: %d.",
	...
	if (others_have_image) {
		...
		pr_warn("Waiting for other nodes to resume.\n");
		start = jiffies;
		wait_event(node_array[0].member_events, time_to_continue(0, start, MSG_RUNNING));
		if (peers_not_in_message(0, MSG_RUNNING, 0))
			pr_warn("Timed out while waiting for other " "nodes to resume.\n");
	}
...
static int toi_cluster_open_iface(void)
{
	...
	if (!net_dev) {
		pr_err(MYNAME ": Device %s not found.\n", toi_cluster_iface);
		return -ENODEV;
	}
...
#ifdef MODULE
static int __init toi_cluster_init(void)
#else
static int toi_cluster_init(void)
#endif
{
...
#ifdef MODULE
static void __exit toi_cluster_exit(void)
#else
static void toi_cluster_exit(void)
#endif
{
	int i;
