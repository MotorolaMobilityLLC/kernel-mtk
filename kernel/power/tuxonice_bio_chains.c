...
void dump_block_chains(void)
{
	...
	while (cur_chain) {
		...
		pr_debug("Chain %d (prio %d):", i, cur_chain->prio);

		while (this) {
			pr_warn(" [%lu-%lu]%s", this->start,
			       this->end, this->next ? "," : "");
			this = this->next;
		}
	...
	pr_debug("Saved states:\n");
	for (i = 0; i < 4; i++) {
		pr_debug("Slot %d: Chain %d.\n", i, toi_writer_posn.saved_chain_number[i]);

		cur_chain = prio_chain_head;
		j = 0;
		while (cur_chain) {
			pr_debug(" Chain %d: Extent %d. Offset %lu.\n",
...
void toi_extent_state_save(int slot)
{
	...
	while (cur_chain) {
		i++;
		toi_message(TOI_BIO, TOI_VERBOSE, 0, "Saving chain %d (%p) state, slot %d.", i, cur_chain, slot);
		...
		if (toi_writer_posn.current_chain == cur_chain) {
			...
			toi_message(TOI_BIO, TOI_VERBOSE, 0, "This is the chain we were on => chain_num is %d.", i);
		}

		if (!cur_chain->blocks.current_extent) {
			chain_state->extent_num = 0;
			toi_message(TOI_BIO, TOI_VERBOSE, 0, "No current extent for chain => extent_num %d is 0.", i);
			cur_chain = cur_chain->next;
			continue;
		}
...
void toi_extent_state_restore(int slot)
{
	...
	while (cur_chain) {
		...
		i++;
		toi_message(TOI_BIO, TOI_VERBOSE, 0, "Restoring chain %d (%p) state, slot %d.", i, cur_chain, slot);
...
int toi_bio_allocate_storage(unsigned long request)
{
	...
	if (!chain) {
		...
		toi_message(TOI_BIO, TOI_VERBOSE, 0, "toi_bio_allocate_storage: Registering storage.");
		if (result)
			return 0;
		chain = prio_chain_head;
		if (!chain) {
			pr_warn("TuxOnIce: No storage was registered.\n");
			return 0;
		}
	}

	toi_message(TOI_BIO, TOI_VERBOSE, 0, "toi_bio_allocate_storage: Request is %lu pages.", request);
	extra_pages = DIV_ROUND_UP(request * (sizeof(unsigned long)
					      + sizeof(int)), PAGE_SIZE);
	needed = request + extra_pages + header_pages_reserved;
	toi_message(TOI_BIO, TOI_VERBOSE, 0, "Adding %lu extra pages and %lu for header => %lu.",
	...
	while (to_get && chain) {
		...
		toi_message(TOI_BIO, TOI_VERBOSE, 0, " Loop finished. Got a total of %lu pages from %d allocators.",
