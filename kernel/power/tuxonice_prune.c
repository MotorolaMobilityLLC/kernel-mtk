...
static int toi_prune_crypto_prepare(void)
{
	...
	if (!*toi_prune_hash_algo_name) {
		pr_warn("TuxOnIce: Pruning enabled but no " "hash algorithm set.\n");
		return 1;
	}
	...
	for_each_online_cpu(cpu) {
		...
		if (IS_ERR(this->desc.tfm)) {
			pr_warn("TuxOnIce: Failed to allocate the %s prune hash algorithm.\n",
		...
		if (ret < 0) {
			pr_warn("TuxOnIce: Failed to initialise the %s prune hash algorithm.\n",
				toi_prune_hash_algo_name);
			kfree(this->digest);
			this->digest = NULL;
			crypto_free_shash(this->desc.tfm);
...
static int toi_prune_rw_init(int rw, int stream_number)
{
	if (toi_prune_crypto_prepare()) {
		pr_err("Failed to initialise prune " "algorithm.\n");
		if (rw == READ) {
			pr_warn("Unable to read the image.\n");
			return -ENODEV;
		}
		pr_warn("Continuing without " "pruning the image.\n");
				...
				void *buffer_page, unsigned int buf_size)
{
	...
	if (ctx->desc.tfm) {
		...
		if (ret) {
			pr_warn("TuxOnIce: Failed to calculate digest (%d).\n", ret);
		} else {
...
static int toi_prune_print_debug_stats(char *buffer, int size)
{
	...
	if (toi_pruned_pages)
		len += scnprintf(buffer + len, size - len, "  Pruned %lu pages).\n", toi_pruned_pages);
	return len;
}
