...
int toi_bio_mark_have_image(void)
{
...
#if 1 /* FIXME: check this uuid is mismatch, such that it will failed on toi_bio_read_header_init() !!! */
	...
	if (!result) {
		...
		toi_message(TOI_IO, TOI_VERBOSE, 0, "Could not get uuid for dev_t %s.",
			    format_dev_t(buf, get_header_dev_t()));
...
int toi_bio_image_exists(int quiet)
{
	...
	if (!resume_dev_t) {
		if (!quiet)
			pr_warn("Not even trying to read header because resume_dev_t is not set.");
		return -1;
	}
	...
	if (result == -1)
		msg = "TuxOnIce: Unable to find a signature. Could you have moved a swap file?\n";
	else if (!result)
