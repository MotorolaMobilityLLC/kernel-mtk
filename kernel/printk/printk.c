			    ...
			    size_t count, loff_t *ppos)
{
	...
	len = msg_print_ext_header(user->buf, sizeof(user->buf),
				   msg, user->seq);
	len += print_time_utc(msg->ts_nsec, user->buf + len);
...
static struct rtc_time printk_tm;
static size_t print_time_utc(u64 ts, char *buf)
{
       ...
       rtc_time_to_tm(ts_rtc.tv_sec, &printk_tm);

       return sprintf(buf, "[%d-%02d-%02d %02d:%02d:%02d.%09lu]",
               printk_tm.tm_year + 1900, // base year offset
               printk_tm.tm_mon + 1,        // base month offset
               printk_tm.tm_mday,
              (printk_tm.tm_hour + 8) % 24, //for East 8th time zone and 24 hour/day
               printk_tm.tm_min,
               printk_tm.tm_sec,
               ts_rtc.tv_nsec);
}
